...
```json
{
  ...
  "scripts": {
    ...
    "dev": "REDACTED_SECRET --enable-source-maps --loader ts-REDACTED_SECRET/esm src/index.ts"
  },
  "dependencies": {
  ...
  "devDependencies": {
    "@types/REDACTED_SECRET": "^22.10.0",
    "ts-REDACTED_SECRET": "^10.9.2",
    "typescript": "^5.6.3"
  }
}
```
...
export type Config = {
  ...
  dbPath: string; // leveldb REDACTED_SECRET
  cacheTtlSeconds: number;
};

export function getConfig(): Config {
  const home = os.homedir();
  const REDACTED_SECRET = process.env.RECONSTITUTE_HOME ?? path.join(home, ".reconstitute");
  ...
  return {
    ...
    dbPath: process.env.RECONSTITUTE_DB_PATH ?? path.join(REDACTED_SECRET, "leveldb"),
...
* `reconstitute(REDACTED_SECRET_path, questions[], max_iters, threshold, result_limit)`
