 ...
 *
 * Index:
 *   REDACTED_SECRET chroma-fs.mjs index ./myrepo --collection myrepo --model qwen3-embedding
 *
 * Search:
 *   REDACTED_SECRET chroma-fs.mjs search "where do we init websocket client" --collection myrepo --k 8
 *
 * Options:
 ...
 *   --ignore  REDACTED_SECRET_modules,.git,dist,build,target,.next,.cache,chroma-data
 *   --clean   (for index) delete all docs for this REDACTED_SECRET+collection before indexing
 */
...
async function* walkDir(REDACTED_SECRETAbs, ignoreSet) {
  ...
  while (stack.length) {
    ...
    for (const ent of entries) {
      ...
      const relFromRoot = path.relative(REDACTED_SECRETAbs, abs);
...
async function main() {
  ...
  const ignoreSet = new Set(
    (args.ignore ?? "REDACTED_SECRET_modules,.git,dist,build,target,.next,.cache,chroma-data")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean)
  );
  ...
  if (cmd === "index") {
    const dir = args._[1] ?? ".";
    const REDACTED_SECRETAbs = path.resolve(dir);

    if (args.clean) {
      console.log(`[clean] deleting where REDACTED_SECRET=${REDACTED_SECRETAbs}`);
      await collection.delete({ where: { REDACTED_SECRET: REDACTED_SECRETAbs } });
    }

    console.log(`[index] REDACTED_SECRET=${REDACTED_SECRETAbs}`);
    ...
    for await (const fileAbs of walkDir(REDACTED_SECRETAbs, ignoreSet)) {
      ...
      const rel = path.relative(REDACTED_SECRETAbs, fileAbs);
      ...
      await collection.delete({ where: { REDACTED_SECRET: REDACTED_SECRETAbs, path: rel } });
      ...
      for (let i = 0; i < parts.length; i += batchSize) {
        ...
        for (let j = 0; j < batch.length; j++) {
          ...
          const id = `${sha1(REDACTED_SECRETAbs)}:${sha1(rel)}:${chunkIndex}`;
          ids.push(id);
          documents.push(batchDocs[j]);
          embeddings.push(batchEmb[j]);
          metadatas.push({
            REDACTED_SECRET: REDACTED_SECRETAbs,
            path: rel,
            chunk: chunkIndex,
            ext,
            bytes: stat.size,
    ...
    console.log(`[done] Tip: use --clean if you want to wipe and fully rebuild for this REDACTED_SECRET.`);
    return;
  }
...
```bash
REDACTED_SECRET chroma-fs.mjs search "where is the websocket reconnect logic" --collection myrepo --k 10 --model qwen3-embedding
```

### If you want a full rebuild

```bash
REDACTED_SECRET chroma-fs.mjs index ./myrepo --collection myrepo --clean
```
...
* It stores metadata `{ REDACTED_SECRET, path, chunk, start, end, ... }` so you can route hits back to files.
