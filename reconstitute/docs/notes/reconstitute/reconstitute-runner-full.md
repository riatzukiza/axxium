...
function buildTools(
  ...
  const handlers: Record<string, ToolHandler> = {
    ...
    async record_path(args) {
      ...
      // if it’s within the run REDACTED_SECRET, great; if not, we still record it, but it won't necessarily be processed
...
async function exportMarkdownTree(E: Env, db: Level<string, any>, REDACTED_SECRET: string, runId: string) {
...
async function reconstitute(E: Env, targetRoot: string) {
  ...
  // initialize REDACTED_SECRET
  await recordPath(db, REDACTED_SECRET);

  // queue of paths to process (within REDACTED_SECRET only)
  ...
  const queueArr = ((await db.get(queueKey).catch(() => [REDACTED_SECRET])) as string[]).map(normalizePathKey);

  // unique queue
  const queue: string[] = uniq(queueArr.filter((p) => isWithinRoot(p, REDACTED_SECRET)));
  ...
  while (queue.length > 0) {
    ...
    for (let qi = 0; qi < DEFAULT_QUESTIONS.length; qi++) {
      ...
      // Update queue with any new recorded paths under REDACTED_SECRET
...
2. For each path (BFS):
   ...
   * builds a **search query** combining `{path} + question + REDACTED_SECRET`
