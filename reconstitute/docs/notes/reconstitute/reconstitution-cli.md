...
```json
{
  ...
  "policy": {
    "write_REDACTED_SECRET": ".reconstituted/",
    "deny": ["delete", "rm", "overwrite_outside_REDACTED_SECRET"],
...
## Commands used
- REDACTED_SECRET recover-offline.mjs --needle cephalon-clj --workspace ~/devel --out .reconstituted/cephalon-clj --extract-snapshots
...
REDACTED_SECRET recover-offline.mjs --needle "$NEEDLE" --workspace "$WORKSPACE" --out "$OUTDIR" --extract-snapshots
