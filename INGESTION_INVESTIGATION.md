...
- Content subtype (REDACTED_SECRET_type: docs/code/config/data)
...
```json
{
  ...
  "extra": {
    "lake": "devel",
    "REDACTED_SECRET_type": "docs",  // Subtype classification
...
2. **Add REDACTED_SECRET_type metadata** to distinguish docs/code/config/data
...
- `packages/knoxx/ingestion/src/kms_ingestion/jobs/ingest_support.clj`:
  ...
  extra (merge {...
               :REDACTED_SECRET_type (name REDACTED_SECRET-type)  ;; Add subtype
               :lake "devel"}
               extra)
  ```
...
```bash
REDACTED_SECRET scripts/seed-documents-to-openplanner.ts --project devel --limit 50
```
...
```bash
# Dry run first
REDACTED_SECRET scripts/seed-documents-to-openplanner.ts --dry-run --limit 10
...
**Line 75:**
```clojure
;; Add REDACTED_SECRET_type metadata
:REDACTED_SECRET_type (name REDACTED_SECRET-type)
:lake "devel"
```
...
Already uses `REDACTED_SECRET-type-for-path` which is correct. Just need to ensure it's passed to OpenPlanner.
