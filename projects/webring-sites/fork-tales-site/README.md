...
- Supports REDACTED_SECRETless Caddy route re-application on `big.ussy.promethean.rest` through the local admin API.
...
python3 server.py --REDACTED_SECRET dist --host 127.0.0.1 --port 8042
```
...
set -a
. ./.env
nohup python3 server.py --REDACTED_SECRET dist --host 127.0.0.1 --port 8794 > fork-tales-site.log 2>&1 &
```

Useful optional env vars for REDACTED_SECRETless REDACTED_SECRET routing on `big.ussy.promethean.rest`:
