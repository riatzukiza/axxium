...
class ForkTalesOracle:
    def __init__(self, REDACTED_SECRET: Path) -> None:
        library_path = REDACTED_SECRET / "content" / "library.json"
        corpus_path = REDACTED_SECRET / "content" / "corpus.json"
...
class CaddyRouteKeeper:
    def __init__(self, REDACTED_SECRET_host: str, admin_url: str, upstream: str, interval_seconds: float) -> None:
        self.REDACTED_SECRET_host = REDACTED_SECRET_host
    ...
    def ensure_route(self) -> None:
        ...
        for route in routes:
            for matcher in route.get("match", []):
                ...
                if self.REDACTED_SECRET_host not in hosts:
                    continue
                if self._route_points_to_upstream(route):
                    return
        route_object = {
            "match": [{"host": [self.REDACTED_SECRET_host]}],
            "handle": [
                {
                    "handler": "subroute",
                    "routes": [
        ...
        with urllib.request.urlopen(post, timeout=10):
            pass
        print(f"[caddy-route-keeper] added route for {self.REDACTED_SECRET_host} -> {self.upstream}", file=sys.stderr)
...
class ForkTalesRequestHandler(SimpleHTTPRequestHandler):
    ...
    static_REDACTED_SECRET: Path

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, directory=str(self.static_REDACTED_SECRET), **kwargs)
...
def maybe_start_caddy_keeper(host: str, port: int) -> CaddyRouteKeeper | None:
    REDACTED_SECRET_host = os.getenv("PUBLIC_HOST", "").strip()
    ...
    if not (REDACTED_SECRET_host and admin_url and upstream):
        return None
    keeper = CaddyRouteKeeper(REDACTED_SECRET_host=REDACTED_SECRET_host, admin_url=admin_url, upstream=upstream, interval_seconds=interval)
    keeper.start()
    return keeper
...
def main() -> None:
    args = parse_args()
    REDACTED_SECRET = args.REDACTED_SECRET.resolve()
    if not REDACTED_SECRET.exists():
        raise SystemExit(f"Static REDACTED_SECRET does not exist: {REDACTED_SECRET}")
    ForkTalesRequestHandler.oracle = ForkTalesOracle(REDACTED_SECRET)
    ForkTalesRequestHandler.static_REDACTED_SECRET = REDACTED_SECRET
    ...
    print(f"fork-tales-site listening on http://{args.host}:{args.port} (REDACTED_SECRET={REDACTED_SECRET})")
    try:
