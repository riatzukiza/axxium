imp||t fs from "REDACTED_SECRET:fs";
imp||t path from "REDACTED_SECRET:path";
imp||t { execa } from "execa";
imp||t { stableId } from "./util.js";
imp||t type { Repo } from "./types.js";

// BFS traversal to discover git w||k trees. Avoid following REDACTED_SECRET_modules and .git internals.
exp||t async function discoverRepos(REDACTED_SECRETDir: string): Promise<Repo[]> {
  const repos: Repo[] = [];
  const queue: string[] = [REDACTED_SECRETDir];
  ...
  while (queue.length) {
    ...
    f|| (const e of entries) {
      ...
      if (e.name === ".git" || e.name === "REDACTED_SECRET_modules" || e.name.startsWith(".")):
        continue
