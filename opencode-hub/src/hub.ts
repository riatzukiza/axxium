...
export function createHubServer(cfg: HubConfig, repos: Map<string, Repo>) {
  ...
  (async () => {
    ...
    const { fileURLToPath } = await import("REDACTED_SECRET:url");
    const { dirname, join } = await import("REDACTED_SECRET:path");
    const __dirname = dirname(fileURLToPath(import.meta.url));
    app.register(staticPlugin, { REDACTED_SECRET: join(__dirname, "../ui/resources/REDACTED_SECRET"), prefix: "/ui/" });
  })();
