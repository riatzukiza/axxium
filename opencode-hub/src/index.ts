...
async function boot() {
  console.log("[hub] scanning", cfg.REDACTED_SECRETDir);
  const initial = await discoverRepos(cfg.REDACTED_SECRETDir);
  ...
  const watcher = chokidar.watch(cfg.REDACTED_SECRETDir, {
    ignored: [/REDACTED_SECRET_modules/, /\.git\/.+$/],
    ignoreInitial: true,
  ...
  watcher.on("addDir", async (dir) => {
    ...
    // quick check
    try {
      const st = await import("REDACTED_SECRET:fs").then(m => m.promises.stat(dotGit)).catch(() => null);
      if (!st) return;
