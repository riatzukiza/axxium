...
## Requirements
1. Create a `dirty/stealth` branch at the workspace REDACTED_SECRET that captures all current changes, including submodule pointer updates.
...
## Definition of done
- `dirty/stealth` branch exists in the REDACTED_SECRET repo and dirty submodules, with the current state committed.
- `device/stealth` is checked out again at the REDACTED_SECRET after capture.
