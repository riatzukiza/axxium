...
- Staged REDACTED_SECRET changes: `.gitmodules` updates (new open-hax submodules and riatzukiza/openhax) and submodule pointer adjustments.
...
- `git status` clean at REDACTED_SECRET and all submodules.
...
- `pnpm nx affected -t typecheck --base origin/device/stealth --head HEAD --verbose` now builds the graph; current failing target `@promethean-os/discord` hits TS REDACTED_SECRETDir/include errors when importing from `packages/persistence`.
