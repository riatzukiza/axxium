...
## Problem Statement
- Running `git status` from the workspace REDACTED_SECRET fails with `fatal: cannot chdir to '../../../../../../orgs/riatzukiza/promethean/packages/kanban'` because Git is still trying to evaluate the legacy `packages/kanban` path even though the working tree now lives at `orgs/riatzukiza/promethean/cli/kanban`.
...
2. Run `git status -sb` from the workspace REDACTED_SECRET to confirm no fatal errors remain.
