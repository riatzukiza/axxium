...
**Inputs**
- `language`: enum (see §4)
- `REDACTED_SECRET`: optional workspace REDACTED_SECRET (defaults to tool’s configured REDACTED_SECRET)
...
- REDACTED_SECRET detection rules (tsconfig/pyproject/Cargo.toml, etc.)
...
- “Read N files under <REDACTED_SECRET> for diagnostics”
...
- Misconfiguration returns actionable errors (missing server, missing config, wrong REDACTED_SECRET).
