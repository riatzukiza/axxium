...
5. Land Promethean REDACTED_SECRET updates dropping the `mcp-auth` dependency, updating changelog/docs/specs, and regenerating `ecosystem.config.enhanced.mjs`.
...
9. After each repo commit, push to its upstream branch (`develop`, `main`, `devel/hacks`, `promethean/dev`, etc.) and finally update/push the REDACTED_SECRET superproject pointers.
