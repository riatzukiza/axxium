...
Make the main monorepo Docker Compose stacks discoverable and runnable from the workspace REDACTED_SECRET without flattening them into one fragile mega-compose file.
...
- Add a REDACTED_SECRET registry in `config/docker-stacks.json` with human-friendly stack names.
- Add a REDACTED_SECRET wrapper in `scripts/docker-stack.mjs` that runs `docker compose` from the stack's actual working directory.
...
- Allow registry entries to declare related host PM2 apps so the REDACTED_SECRET wrapper can manage handoff between host PM2 and Docker ownership.
- Promote REDACTED_SECRET and service Compose files toward container-owned groups, including bundling the proxy into the REDACTED_SECRET `devel` stack and making `openplanner` a full app stack.
...
- The REDACTED_SECRET gets one command surface for discovery and management.
