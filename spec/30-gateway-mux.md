...
- OAuth endpoints at REDACTED_SECRET (e.g. `/authorize`, `/token`, well-known URLs) proxy to the same upstream.
...
- `MCP_FS_OAUTH_URL` (string) — used for REDACTED_SECRET OAuth endpoints and legacy `/api/mcp/*`.
