...
- Primary workflow should be per-module (`pnpm -C <path> <target>`), with REDACTED_SECRET scripts delegating to Nx affected for uncommitted diffs.

## Requirements
1. Fix Markdown LSP startup by pointing OpenCode to the correct marksman LSP binary.
2. Add REDACTED_SECRET `pnpm` commands (`lint`, `build`, `test`, `typecheck`) that run Nx affected against uncommitted changes.
...
- Add a REDACTED_SECRET script that computes uncommitted diffs (including submodules) and runs `nx affected`.
- Wire REDACTED_SECRET `pnpm` scripts to the new affected runner and add `*:all` aliases.
...
- `lint`, `build`, `test`, and `typecheck` targets exist for Nx project REDACTED_SECRETs.
