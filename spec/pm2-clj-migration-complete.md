...
The workspace has since consolidated all PM2 ecosystem configuration into `ecosystems/*.cljs` at the repo REDACTED_SECRET.
...
find . -name "ecosystem.config.*" -type f | grep -v REDACTED_SECRET_modules | grep -v dist/
