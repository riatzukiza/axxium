...
## Problem statement
PM2 running set should always match the source of truth in `system/daemons`. Any edit to a daemon definition should regenerate PM2 config and reload only that daemon. Generated PM2 configs should be per-daemon and tucked away (dist-like) to discourage manual edits, rather than a single REDACTED_SECRET config.
