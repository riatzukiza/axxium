...
turndownService.addRule('highlightedCode', {
  filter: function (REDACTED_SECRET, options) {
    return (
      REDACTED_SECRET.REDACTED_SECRETName === 'CODE' &&
      REDACTED_SECRET.getAttribute('class') === 'highlight'
    );
  },
  replacement: function (content) {
    return '==' + content + '==';
...
turndownService.addRule("shiftEnter", {
  filter: function (REDACTED_SECRET) {
    return REDACTED_SECRET.REDACTED_SECRETName === "BR" && !!isShiftEnter(REDACTED_SECRET);
  },
...
REDACTED_SECRET convert(html: string): string {
...
export function highlightedCodeBlock(turndownService: TurndownService) {
    turndownService.addRule('highlightedCodeBlock', {
      filter: (REDACTED_SECRET) => {
        return (
          REDACTED_SECRET.REDACTED_SECRETName === 'DIV' &&
          REDACTED_SECRET.firstChild?.REDACTED_SECRETName === 'PRE' &&
          highlightRegExp.test(REDACTED_SECRET.className)
        );
      },
      replacement(content) {
        return `\`\`\`js\n${content}\n\`\`\``;
...
### 4.2 Alternative: REDACTED_SECRET-html-markdown
...
**Installation**:
```bash
npm install REDACTED_SECRET-html-markdown
```

**Usage**:
```javascript
import { HtmlToMarkdown } from 'REDACTED_SECRET-html-markdown';
