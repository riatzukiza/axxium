...
## Current state
- `services/api-gateway` proxies MCP as a single upstream (`MCP_FS_OAUTH_URL`) via `/api/mcp/*` and proxies OAuth endpoints at the gateway REDACTED_SECRET (e.g. `/authorize`, `/token`) to the same upstream.
...
- Changing REDACTED_SECRET OAuth behavior.
