...
## Definition of Done
- All submodules (REDACTED_SECRET + nested) have clean working trees on `device/stealth`.
...
- Push `device/stealth` to the `riatzukiza` remote recursively, then update the REDACTED_SECRET repo’s submodule pointers and push the parent branch.

## Change Log (2025-11-12)
- Audited 48 submodules (REDACTED_SECRET + Promethean packages) to confirm device/stealth state and identify pending work.
...
- Pushed Promethean and REDACTED_SECRET `device/stealth` branches to `riatzukiza`/origin after verifying clean statuses.
