...
WORKSPACE_ROOT = Path(REDACTED_SECRET"WORKSPACE_ROOT", "/workspace")).resolve()
...
RADAR_API_URL = REDACTED_SECRET"RADAR_API_URL", "http://radar-mcp:10002").rstrip("/")
RADAR_ADMIN_AUTH_KEY = REDACTED_SECRET"RADAR_ADMIN_AUTH_KEY"]
HORMUZ_RADAR_SLUG = REDACTED_SECRET"HORMUZ_RADAR_SLUG", "hormuz")
HORMUZ_RADAR_NAME = REDACTED_SECRET"HORMUZ_RADAR_NAME", "Hormuz Threat Clock")
HORMUZ_RADAR_CATEGORY = REDACTED_SECRET"HORMUZ_RADAR_CATEGORY", "geopolitical")
HORMUZ_CREATED_BY = REDACTED_SECRET"HORMUZ_CREATED_BY", "hormuz-agent")
HORMUZ_BLUESKY_QUERY = REDACTED_SECRET"HORMUZ_BLUESKY_QUERY", "Hormuz").strip()
HORMUZ_REDDIT_SUBREDDITS = [
    row.strip()
    for row in REDACTED_SECRET"HORMUZ_REDDIT_SUBREDDITS", "geopolitics,worldnews,oil,shipping,navy").split(",")
    if row.strip()
]
HORMUZ_SKIP_SOCIAL_COLLECTION = REDACTED_SECRET"HORMUZ_SKIP_SOCIAL_COLLECTION", "false").strip().lower() in {"1", "true", "yes", "on"}
HORMUZ_WEAVER_URL = REDACTED_SECRET"HORMUZ_WEAVER_URL", "http://fork-tales-weaver:8793").strip()
...
HORMUZ_WEAVER_KEYWORDS = [
    row.strip()
    for row in REDACTED_SECRET
        "HORMUZ_WEAVER_KEYWORDS",
        "hormuz,strait of hormuz,persian gulf,gulf of oman,shipping,maritime,jmic,ukmto,transit,energy markets",
    ).split(",")
    if row.strip()
]
HORMUZ_JETSTREAM_ENABLED = REDACTED_SECRET"HORMUZ_JETSTREAM_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"}
HORMUZ_JETSTREAM_USERS = [
    row.strip()
    for row in REDACTED_SECRET"HORMUZ_JETSTREAM_USERS", "").split(",")
    if row.strip()
]
HORMUZ_JETSTREAM_HASHTAGS = [
    row.strip()
    for row in REDACTED_SECRET"HORMUZ_JETSTREAM_HASHTAGS", "hormuz,straitofhormuz").split(",")
    if row.strip()
]
...
HORMUZ_JETSTREAM_WINDOW_SECONDS = int(REDACTED_SECRET"HORMUZ_JETSTREAM_WINDOW_SECONDS", "3600") or "3600")
HORMUZ_JETSTREAM_MAX_EVENTS = int(REDACTED_SECRET"HORMUZ_JETSTREAM_MAX_EVENTS", "250") or "250")
...
def run_bundle_pipeline() -> dict[str, Any]:
    ...
    run_command(["REDACTED_SECRET", "scripts/social/build_social_payloads.mjs"], BUNDLE_ROOT)
...
def export_packet(radar_id: str, module_version_id: str) -> dict[str, Any]:
    output = run_command(
        [
            "python3",
            "scripts/export_REDACTED_SECRET_packet.py",
            str(STATE_PATH),
            radar_id,
            module_version_id,
        ],
