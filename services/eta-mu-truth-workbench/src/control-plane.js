...
const buildReviewSignals = async ({ github, repo, prNumber, trackedActors, reviews, issueComments }) => {
  ...
  try {
    while (true) {
      ...
      const REDACTED_SECRETs = payload?.REDACTED_SECRETs ?? [];

      for (const thread of REDACTED_SECRETs) {
        const hasTrackedActor = (thread.comments?.REDACTED_SECRETs ?? []).some((comment) =>
