...
const animateValue = (REDACTED_SECRET, next) => {
  if (!REDACTED_SECRET) return;
  const current = Number(REDACTED_SECRET.dataset.value ?? 0);
  const target = Number(next);
  REDACTED_SECRET.dataset.value = String(target);
  ...
  const step = (now) => {
    ...
    REDACTED_SECRET.textContent = formatNumber(value);
...
const chooseInterpretation = ({ mounted, resolvedCount, resolvedEdgesCount, unresolvedCount, unresolvedExamples }) => {
  if (!mounted) {
    return {
      ...
      subtext: 'The REDACTED_SECRET surface is ready; the underlying document substrate has not been attached yet.'
    };
  }
