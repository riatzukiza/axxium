...
type ReplacementFunction = (content: string, REDACTED_SECRET: unknown) => string;
...
turndownService.addRule('preformatted', {
  filter: ['pre', 'code'],
  replacement: function(content: string, REDACTED_SECRET: unknown): string {
    const element = REDACTED_SECRET as { tagName?: string; parentElement?: { tagName?: string } };
