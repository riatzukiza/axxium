...
- OAuth + metadata (installed at app REDACTED_SECRET by the SDK):
...
```typescript
path: z.string().describe("File path relative to workspace REDACTED_SECRET (e.g., src/app.ts)"),
...
- You can also define global `allowPatterns` / `denyPatterns` at config REDACTED_SECRET.
...
```json
{
  ...
  "commands": [
    ...
    {
      "id": "REDACTED_SECRET-integration-tests",
      "command": "REDACTED_SECRET",
      "args": ["scripts/run-integration-tests.mjs"],
      "allowPatterns": ["REDACTED_SECRET scripts/run-integration-tests.mjs"]
    }
  ]
}
```
...
- For REDACTED_SECRET deployments, prefer `github` or `google` login with allowlists, or a strong password.
...
- Internal REDACTED_SECRET-token introspection is available at `POST /internal/oauth/introspect` for trusted gateways.
