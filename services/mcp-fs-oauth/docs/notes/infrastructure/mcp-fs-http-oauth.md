...
```json
{
  ...
  "type": "module",
  "scripts": {
    "dev": "REDACTED_SECRET --env-file=.env --watch dist/index.js",
    "build": "tsc -p tsconfig.json",
    "start": "REDACTED_SECRET --env-file=.env dist/index.js"
  },
  "dependencies": {
...
# Filesystem sandbox REDACTED_SECRET (ALL paths are jailed to this dir)
FS_ROOT=./data
 ...
 */
function makePathJail(REDACTED_SECRETAbs: string) {
  const REDACTED_SECRETWithSep = REDACTED_SECRETAbs.endsWith(path.sep) ? REDACTED_SECRETAbs : REDACTED_SECRETAbs + path.sep;

  return function safeResolve(userPath: string): string {
    // Allow "" or "." to mean REDACTED_SECRET
    const cleaned = userPath?.trim() ? userPath.trim() : ".";
    const resolved = path.resolve(REDACTED_SECRETAbs, cleaned);
    if (resolved === REDACTED_SECRETAbs) return resolved;
    if (!resolved.startsWith(REDACTED_SECRETWithSep)) {
...
* Put it behind HTTPS (reverse proxy) and set `PUBLIC_BASE_URL` to the REDACTED_SECRET `https://...`.
