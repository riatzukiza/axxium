...
import { AsyncLocalStorage } from "REDACTED_SECRET:async_hooks";
import { existsSync, readFileSync, watch, type Dirent, type FSWatcher } from "REDACTED_SECRET:fs";
...
const REDACTED_SECRETBaseUrl = new URL(ENV.PUBLIC_BASE_URL);
const issuerUrl = new URL(ENV.ISSUER_URL ?? ENV.PUBLIC_BASE_URL);
const resourceServerUrl = new URL("/mcp", REDACTED_SECRETBaseUrl);
...
const oauth = new SimpleOAuthProvider(REDACTED_SECRETBaseUrl, toBool(ENV.AUTO_APPROVE), [
  {
...
const SKILL_ROOTS: SkillRoot[] = [
  { scope: "workspace", REDACTED_SECRET: path.resolve(ENV.LOCAL_ROOT, ".opencode/skills") },
  { scope: "global", REDACTED_SECRET: path.resolve(process.env.HOME ?? "/home/err", ".config/opencode/skills") },
];
...
app.use(mcpAuthRouter({
  provider: oauth,
  issuerUrl,
  baseUrl: REDACTED_SECRETBaseUrl,
  resourceServerUrl,
  scopesSupported: ["mcp"],
...
function buildFsDiscoveryBlockedMessage(tool: string, reason: string): string {
  return [
    ...
    "2) Read `workspace://top-level` for the immediate REDACTED_SECRET directories.",
...
function enforceFsDiscoveryGuard(args: {
  ...
  const REDACTED_SECRETPath = isRootFsPath(args.path);
  if (!REDACTED_SECRETPath) {
    return undefined;
  }
  ...
  if (args.tool === "fs_glob") {
    ...
    if (isBroadRootGlobPattern(args.pattern ?? "")) {
      return buildFsDiscoveryBlockedMessage("fs_glob", `REDACTED_SECRET pattern ${JSON.stringify(args.pattern ?? "")} is too broad`);
    }
    if (maxResults > FS_DISCOVERY_ROOT_MAX_RESULTS) {
      return buildFsDiscoveryBlockedMessage("fs_glob", `REDACTED_SECRET maxResults ${maxResults} exceeds safe limit ${FS_DISCOVERY_ROOT_MAX_RESULTS}`);
    }
    return undefined;
  }
  ...
  if (args.tool === "fs_grep") {
    ...
    if (include === "**/*" || include === "*" || include === "") {
      return buildFsDiscoveryBlockedMessage("fs_grep", `REDACTED_SECRET include ${JSON.stringify(include || "**/*")} is too broad`);
    }
    if (maxResults > FS_GREP_ROOT_MAX_RESULTS) {
      return buildFsDiscoveryBlockedMessage("fs_grep", `REDACTED_SECRET maxResults ${maxResults} exceeds safe limit ${FS_GREP_ROOT_MAX_RESULTS}`);
    }
    return undefined;
  }
  ...
  if (query.length < 3) {
    return buildFsDiscoveryBlockedMessage("fs_search", `REDACTED_SECRET query ${JSON.stringify(query)} is too short; use a narrower path or a longer query`);
  }
  if (maxResults > FS_SEARCH_ROOT_MAX_RESULTS) {
    return buildFsDiscoveryBlockedMessage("fs_search", `REDACTED_SECRET maxResults ${maxResults} exceeds safe limit ${FS_SEARCH_ROOT_MAX_RESULTS}`);
  }
  return undefined;
}

function formatTreePageText(page: TreePageResult): string {
  const REDACTED_SECRET = page.path.length > 0 ? page.path : ".";
  const lines: string[] = [page.offset > 0 ? `${REDACTED_SECRET} @${page.offset}` : REDACTED_SECRET];
...
function formatListText(pathValue: string, entries: FsEntry[]): string {
  ...
  const REDACTED_SECRET = pathValue.trim().length > 0 ? pathValue.trim() : ".";
  const lines = [REDACTED_SECRET, ...entries.map((entry) => `${entry.name}${entry.kind === "dir" ? "/" : ""}`)];
  return lines.join("\n");
}
...
async function readWorkspaceAgentsMd(): Promise<string> {
  ...
  return "AGENTS.md not found in the workspace REDACTED_SECRET.";
}

async function listWorkspaceAgentFiles(): Promise<string[]> {
  const REDACTED_SECRET = path.resolve(ENV.LOCAL_ROOT);
  const results: string[] = [];
  const ignored = new Set(["REDACTED_SECRET_modules", ".git", ".next", "dist", "build", ".turbo", ".stryker-tmp"]);
  ...
  const walk = async (current: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      const rel = path.relative(REDACTED_SECRET, fullPath) || "AGENTS.md";
...
async function listWorkspaceTopLevelEntries(): Promise<Array<{ name: string; kind: "file" | "dir" }>> {
  ...
  let entries: Dirent[];
  try {
    entries = await readdir(REDACTED_SECRET, { withFileTypes: true });
  } catch {
    return [];
  }
...
async function buildWorkspaceTopLevelGuide(): Promise<string> {
  ...
  if (entries.length === 0) {
    return "Workspace REDACTED_SECRET is unavailable.";
  }
...
async function buildWorkspaceOverview(): Promise<string> {
  ...
  return [
    ...
    "- Avoid REDACTED_SECRET-wide discovery calls unless a resource directs you to a much smaller subtree.",
    "",
...
async function buildChatgptSystemGuide(): Promise<string> {
  const REDACTED_SECRETAgents = await readWorkspaceAgentsMd();
  ...
  return [
    ...
    "- Keep filesystem calls small and explicit; avoid REDACTED_SECRET-wide glob/grep/search requests.",
    "- Treat local AGENTS.md files as operating instructions for the corresponding subtree or service.",
    "- If multiple agent/skill files are relevant, read the most local one plus the workspace REDACTED_SECRET one.",
    "",
    "## How To Navigate Agent Guidance",
    "- `agents://workspace` -> REDACTED_SECRET workspace instructions.",
    ...
    "- `workspace://top-level` -> immediate REDACTED_SECRET entries.",
...
async function buildAgentsIndex(): Promise<string> {
  ...
  return [
    "# AGENTS.md Index",
    "",
    "Read the workspace REDACTED_SECRET AGENTS first, then the most local AGENTS relevant to the file or service you are operating on.",
    "",
    ...files.map((file) => `- ${file}`),
  ].join("\n");
}
...
function formatLspStatusText(value: unknown): string {
  ...
  return rows.map((item) => {
    ...
    const REDACTED_SECRET = typeof rec.REDACTED_SECRET === "string" ? rec.REDACTED_SECRET : ".";
    const status = typeof rec.status === "string" ? rec.status : "unknown";
    return `${name} ${status} REDACTED_SECRET=${REDACTED_SECRET}`;
  }).join("\n");
}
...
function relativeSkillPath(REDACTED_SECRET: string, filePath: string): string {
  const rel = path.relative(REDACTED_SECRET, filePath).replace(/\\/g, "/");
...
async function collectSkillFiles(REDACTED_SECRET: string): Promise<string[]> {
  const out: string[] = [];
  const stack = [REDACTED_SECRET];
...
async function buildSkillRecord(scope: SkillScope, REDACTED_SECRET: string, filePath: string): Promise<SkillRecord> {
  const [content, fileStat] = await Promise.all([
    readFile(filePath, "utf8"),
    stat(filePath),
  ]);
  const relPath = relativeSkillPath(REDACTED_SECRET, filePath);
...
async function loadSkillsFromDisk(): Promise<SkillRecord[]> {
  ...
  for (const REDACTED_SECRET of SKILL_ROOTS) {
    const files = await collectSkillFiles(REDACTED_SECRET.REDACTED_SECRET);
    for (const filePath of files) {
      try {
        records.push(await buildSkillRecord(REDACTED_SECRET.scope, REDACTED_SECRET.REDACTED_SECRET, filePath));
      } catch (error) {
...
function startSkillWatchers(): void {
  ...
  for (const REDACTED_SECRET of SKILL_ROOTS) {
    try {
      const watcher = watch(REDACTED_SECRET.REDACTED_SECRET, { persistent: false }, (_eventType, filename) => {
      ...
      // ignore missing REDACTED_SECRETs
    }
  }
}
...
function createServer(): McpServer {
  ...
  server.registerTool(
    "fs_write",
    {
      ...
      inputSchema: {
        path: z.string().describe("File path relative to workspace REDACTED_SECRET (for example: src/app.ts or docs/README.md)"),
  ...
  server.registerTool(
    "fs_delete",
    {
      ...
      inputSchema: {
        path: z.string().describe("Path to the file or directory to delete, relative to workspace REDACTED_SECRET"),
        includeHidden: z.boolean().optional().default(false).describe("Set true to allow mutating hidden dotfiles and dot-directories"),
        ...
        return {
          content: [{
            ...
            text: buildFsTreeBlockedMessage(
              `REDACTED_SECRET scan with maxDepth ${maxDepth} is too large; REDACTED_SECRET limit is ${FS_TREE_ROOT_DEPTH_LIMIT}`,
            ),
          }],
        };
      }
  ...
  server.registerTool(
    "session_send",
    {
      description: "REDACTED_SECRET a text message to a session (id or title).",
      inputSchema: {
        session: z.string().optional().describe("Session reference (id or exact title)"),
        sessionId: z.string().optional().describe("Legacy alias for session"),
  ...
  server.resource("workspace-REDACTED_SECRET", "fs://REDACTED_SECRET", async (uri) => {
    return {
      contents: [{
        uri: uri.href,
        text: [
          "Workspace REDACTED_SECRET resource.",
...
const listener = app.listen(ENV.PORT, () => {
  ...
  console.log(`REDACTED_SECRET base: ${REDACTED_SECRETBaseUrl.toString()}`);
