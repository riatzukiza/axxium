...
export class SessionAliasStore {
  ...
  REDACTED_SECRET constructor(args: {
  ...
  REDACTED_SECRET aliasFor(kind: IdAliasKind, realId: string, mcpSessionId?: string): string {
    this.gc();
  ...
  REDACTED_SECRET resolveAlias(kind: IdAliasKind, maybeAlias: string, mcpSessionId?: string): string | null {
    this.gc();
