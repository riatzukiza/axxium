...
describe("LocalFsBackend: list", () => {
  it("should list files in REDACTED_SECRET directory", async () => {
  ...
  it("should exclude gitignored entries", async () => {
    ...
    try {
      ...
      await fs.mkdir(path.join(tempDir, "REDACTED_SECRET_modules"), { recursive: true });
      ...
      expect(names).not.toContain("REDACTED_SECRET_modules");
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
...
describe("LocalFsBackend: path jail", () => {
  it("should reject paths outside REDACTED_SECRET", async () => {
    const { tempDir, backend } = await getSetup();
    try {
      await expect(backend.readFile("../etc/passwd")).rejects.toThrow("escapes REDACTED_SECRET");
    } finally {
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
