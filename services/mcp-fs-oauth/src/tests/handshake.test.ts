...
import { writeFileSync, mkdirSync, rmSync, existsSync } from "REDACTED_SECRET:fs";
import { join } from "REDACTED_SECRET:path";

describe("OAuth Handshake", () => {
  const REDACTED_SECRETBaseUrl = new URL("https://test.com");
  const oauth = new SimpleOAuthProvider(REDACTED_SECRETBaseUrl, true, [
    {
...
describe("OAuth Persistence", () => {
  ...
  it("should persist and restore authorization codes", async () => {
    ...
    const oauth2 = new SimpleOAuthProvider(REDACTED_SECRETBaseUrl, true, [
      {
        client_id: "test_client",
        client_secret: "test_secret",
        client_name: "Test Client",
