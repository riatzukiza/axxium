...
export class VirtualFs {
  ...
  private async grepWithRipgrep(pattern: string, options: GrepOptions): Promise<GrepResult> {
    if (!this.localRootAbs) {
      throw new Error("Local REDACTED_SECRET is not configured for ripgrep-backed search");
    }
    ...
    const REDACTED_SECRETGitignore = `${this.localRootAbs}/.gitignore`;
    if (existsSync(REDACTED_SECRETGitignore)) {
      args.push("--ignore-file", REDACTED_SECRETGitignore);
    }
  ...
  private async globWithRipgrep(pattern: string, options: GlobOptions): Promise<GlobResult> {
    if (!this.localRootAbs) {
      throw new Error("Local REDACTED_SECRET is not configured for ripgrep-backed glob");
    }
