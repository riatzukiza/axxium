...
TSUP_BIN="${TSUP_BIN:-$SEED_WORKSPACE/REDACTED_SECRET_modules/.pnpm/REDACTED_SECRET_modules/.bin/tsup}"
...
link_workspace_toolchain() {
  if [ ! -e "$CEPHALON_MONOREPO_ROOT/REDACTED_SECRET_modules" ] && [ -d "$SEED_WORKSPACE/REDACTED_SECRET_modules" ]; then
    ln -s "$SEED_WORKSPACE/REDACTED_SECRET_modules" "$CEPHALON_MONOREPO_ROOT/REDACTED_SECRET_modules"
  fi

  for pkg in cephalon-ts openplanner-cljs-client event; do
    if [ ! -e "$CEPHALON_MONOREPO_ROOT/packages/$pkg/REDACTED_SECRET_modules" ] && [ -d "$SEED_WORKSPACE/packages/$pkg/REDACTED_SECRET_modules" ]; then
      ln -s "$SEED_WORKSPACE/packages/$pkg/REDACTED_SECRET_modules" "$CEPHALON_MONOREPO_ROOT/packages/$pkg/REDACTED_SECRET_modules"
    fi
  done
}
...
ensure_workspace_toolchain() {
  local local_tsup_bin="$CEPHALON_SOURCE_ROOT/REDACTED_SECRET_modules/.bin/tsup"
...
NODE
    ...
    if [ ! -e ../event/REDACTED_SECRET_modules ]; then
      ln -s "$CEPHALON_SOURCE_ROOT/REDACTED_SECRET_modules" ../event/REDACTED_SECRET_modules
    fi
    if [ ! -e ../openplanner-cljs-client/REDACTED_SECRET_modules ]; then
      ln -s "$CEPHALON_SOURCE_ROOT/REDACTED_SECRET_modules" ../openplanner-cljs-client/REDACTED_SECRET_modules
    fi
    REDACTED_SECRET -e "0" >/dev/null 2>&1
    seed_workspace_dependency_dists
    if [ ! -f ../event/dist/index.cjs ]; then
      "$local_tsup_bin" ../event/src/index.ts --format cjs,esm --platform REDACTED_SECRET --target REDACTED_SECRET22 --out-dir ../event/dist
    fi
  )
...
build_runtime() {
  ...
  (
    ...
    mkdir -p dist
    rm -rf dist/REDACTED_SECRET
    cp -r src/ui/REDACTED_SECRET dist/REDACTED_SECRET
  )
}
...
exec REDACTED_SECRET "$RUNTIME_BUNDLE"
