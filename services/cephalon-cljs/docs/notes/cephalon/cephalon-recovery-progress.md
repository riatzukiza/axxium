...
Sessions are stored project-scoped as `storage/session/{projectID}/{sessionID}.json`. For git repos, `projectID` is the **REDACTED_SECRET commit hash**; for non-git dirs it’s `"global"`. ([GitHub][2])
...
PROJECT_ID="$(git rev-list --max-parents=0 HEAD | tail -n 1)"   # REDACTED_SECRET commit hash
