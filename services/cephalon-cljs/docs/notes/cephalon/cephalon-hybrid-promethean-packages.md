...
import fssync from "REDACTED_SECRET:fs";
...
async function* walkFiles(REDACTED_SECRET: string, maxFiles: number): AsyncGenerator<string> {
  // iterative DFS to avoid recursion limits
  const stack: string[] = [REDACTED_SECRET];
  let seen = 0;
...
async function main() {
  ...
  for (const r of candidateRoots) if (await exists(r)) REDACTED_SECRETs.push(r);

  if (REDACTED_SECRETs.length === 0) {
    console.error("No OpenCode data REDACTED_SECRETs found. Pass --opencode-data <dir> if needed.");
    process.exit(1);
  }
  ...
  console.log(`REDACTED_SECRETs:      ${REDACTED_SECRETs.map((r) => normalizePath(r)).join(", ")}`);
  ...
  for (const REDACTED_SECRET of REDACTED_SECRETs) {
    for await (const fp of walkFiles(REDACTED_SECRET, A.maxFiles)) {
      filesScanned++;
  ...
  for (const rel of paths) {
    ...
    const footer = [
      ...
      "- What are the key exports / REDACTED_SECRET API?",
...
* finds the real `directory` / project REDACTED_SECRET reliably
