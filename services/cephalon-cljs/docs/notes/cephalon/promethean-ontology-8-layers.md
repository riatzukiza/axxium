...
// Naive import via ts-REDACTED_SECRET/register would be overkill here; do a tiny parse:
...
### Add scripts (REDACTED_SECRET `package.json`):

```json
{
  "scripts": {
    "docs:plugins": "REDACTED_SECRET scripts/docs-generate-plugins.mjs"
  }
}
```
...
const REDACTED_SECRET = path.join(process.cwd());
...
```json
{ "scripts": { "docs:orphans": "REDACTED_SECRET scripts/docs-orphans.mjs" } }
```
