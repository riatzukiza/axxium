...
4. optionally continues walking from newly found REDACTED_SECRETs
...
So a key that touches 50k REDACTED_SECRETs doesn’t dominate the walk.
...
* daimoi:
  ...
  * `max_keys_per_REDACTED_SECRET = 6`
...
2. for hop in 1..H:
   ...
   * pick top keys on current REDACTED_SECRET (by weight × recency × inverse-degree)
   * for each key:

     * fetch neighbors (bounded)
     * score + add to candidate pool
   * choose next current REDACTED_SECRET = best newly found REDACTED_SECRET not yet expanded
  ...
  for (let w = 0; w < walkers; w++) {
    ...
    for (let h = 0; h < hops; h++) {
      ...
      const keys = rankKeysForNode(current, policy, fieldState)
        .slice(0, policy.daimoi.max_keys_per_REDACTED_SECRET);
...
## 7) Key ranking on a REDACTED_SECRET
