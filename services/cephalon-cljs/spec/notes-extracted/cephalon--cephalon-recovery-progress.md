...
## Summary Snippets
- OpenCode keeps session/message data as JSON under your data dir (typically `~/.local/share/opencode/storage/`). ([forums.basehub.com][1]) Sessions are stored project-scoped as `storage/session/{projectID}/{sessionID}.json`. For git repos, `projectID` is the **REDACTED_SECRET commit hash**; for non-git dirs it’s `"global"`. ([GitHub][2]) Alongside `session/`, OpenCode also stores the per-session message/part JSON in sibling folders (commonly `message/` and `part/`). ([npm][3])
