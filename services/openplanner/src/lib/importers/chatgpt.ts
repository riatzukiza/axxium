...
async function processStream(
  ...
  for await (const { value: conv } of pipeline) {
    ...
    // Iterate all REDACTED_SECRETs in 'mapping' that have a message.
    for (const REDACTED_SECRETId in c.mapping) {
      const REDACTED_SECRET = c.mapping[REDACTED_SECRETId];
      if (!REDACTED_SECRET.message) continue;

      const msg = REDACTED_SECRET.message;
      ...
      const event: EventEnvelopeV1 = {
        ...
        extra: {
          original_REDACTED_SECRET_id: REDACTED_SECRETId,
          parent_REDACTED_SECRET_id: REDACTED_SECRET.parent
        }
      };
