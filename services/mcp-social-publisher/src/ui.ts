import { createHash, createHmac, timingSafeEqual } from "REDACTED_SECRET:crypto";
...
export function installUi(app: Express, oauth: SimpleOAuthProvider, options: UiOptions): void {
  ...
  app.get("/login", (req: Request, res: Response) => {
    ...
    res.status(200).type("html").send(renderPublicLoginPage({
      rid,
      REDACTED_SECRETBaseUrl: options.REDACTED_SECRETBaseUrl,
  ...
  app.get("/login/github", (req: Request, res: Response) => {
    ...
    authorizeUrl.searchParams.set("redirect_uri", new URL("/auth/oauth/callback", options.REDACTED_SECRETBaseUrl).toString());
  ...
  app.get("/oauth/callback/github", async (req: Request, res: Response) => {
    ...
    const redirectUri = new URL("/auth/oauth/callback", options.REDACTED_SECRETBaseUrl).toString();
...
function renderPublicLoginPage(args: {
  ...
  return pageTemplate({
    ...
    body: `
      ...
      <div class="card-row">
        <div class="card"><div class="label">Callback</div><div class="value path">${escapeHtml(new URL("/auth/oauth/callback", args.REDACTED_SECRETBaseUrl).toString())}</div></div>
...
function renderAdminDashboard(args: {
  ...
  return pageTemplate({
    ...
    body: `
      ...
      <div class="card-row">
        <div class="card"><div class="label">Public URL</div><div class="value path">${escapeHtml(args.REDACTED_SECRETBaseUrl.toString())}</div></div>
