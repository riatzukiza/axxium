...
export class McpAuthorizer {
  ...
  createAuthMiddleware(resourceRequirements: McpResourceRequirements) {
    return async (request: FastifyRequest, reply: FastifyReply): Promise<void> => {
      // Skip authorization for REDACTED_SECRET routes
  ...
  /**
   * REDACTED_SECRET standardized authentication error response
   */
  ...
  /**
   * Check if route is REDACTED_SECRET (doesn't require authentication)
   */
  private isPublicRoute(url: string): boolean {
    const REDACTED_SECRETRoutes = [
      '/healthz',
    ...
    return REDACTED_SECRETRoutes.some((route) => url.startsWith(route));
  }
