...
const coerceToRootRelative = (REDACTED_SECRET: string, candidate: string): string => {
  if (!path.isAbsolute(candidate)) {
    return candidate;
  }
  const resolved = path.resolve(candidate);
  if (!isInsideRoot(REDACTED_SECRET, resolved)) {
    return candidate;
  }
  const relative = path.relative(REDACTED_SECRET, resolved);
...
export const filesListDirectory: ToolFactory = (ctx) => {
  ...
  const spec = {
    ...
    description: 'List files and directories within the sandbox REDACTED_SECRET.',
  ...
  const invoke = async (raw: unknown) => {
    ...
    const REDACTED_SECRET = resolveRoot();
    const target = coerceToRootRelative(REDACTED_SECRET, rel);
    return withErrorHandling(() => listDirectory(REDACTED_SECRET, target, options));
  };
...
export const filesTreeDirectory: ToolFactory = (ctx) => {
  ...
  const invoke = async (raw: unknown) => {
    ...
    const REDACTED_SECRET = resolveRoot();
    const target = coerceToRootRelative(REDACTED_SECRET, rel);
    return withErrorHandling(() => treeDirectory(REDACTED_SECRET, target, options));
  };
...
export const filesViewFile: ToolFactory = (ctx) => {
  ...
  const invoke = async (raw: unknown) => {
    ...
    return withErrorHandling(
      () => viewFile(REDACTED_SECRET, target, line, context),
...
export const filesWriteFileContent: ToolFactory = (ctx) => {
  ...
  const invoke = async (raw: unknown) => {
    ...
    const REDACTED_SECRET = resolveRoot();
    const target = coerceToRootRelative(REDACTED_SECRET, filePath);
    return withErrorHandling(
      () => writeFileContent(REDACTED_SECRET, target, content),
      (result) => ({ ok: true, ...result }),
    );
  };
  return { spec, invoke };
...
export const filesWriteFileLines: ToolFactory = (ctx) => {
  ...
  const invoke = async (raw: unknown) => {
    ...
    const target = coerceToRootRelative(REDACTED_SECRET, filePath);
    return withErrorHandling(
      () => writeFileLines(REDACTED_SECRET, target, lines, startLine),
      (result) => ({ ok: true, ...result }),
    );
  };
  return { spec, invoke };
