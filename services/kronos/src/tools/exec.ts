...
const resolveWorkingDirectory = (
  ...
  options: Readonly<{ defaultCwd?: string; REDACTED_SECRETFromEnv?: string }>,
): string => {
  if (command.cwd) {
    return command.cwd;
  }
  ...
  if (options.REDACTED_SECRETFromEnv) {
    return options.REDACTED_SECRETFromEnv;
  }
  return process.cwd();
};
...
const runApprovedCommand = async (
  ...
  const cwd = resolveWorkingDirectory(command, {
    ...
    REDACTED_SECRETFromEnv: options.REDACTED_SECRETFromEnv,
  });
...
export const execRunTool: ToolFactory = (ctx) => {
  ...
  const invoke = (raw: unknown) => {
    ...
    return runApprovedCommand(command, parsed.args, {
      ...
      REDACTED_SECRETFromEnv: ctx.env.MCP_ROOT_PATH,
      timeoutMs,
      forceKillDelayMs,
    });
  };
