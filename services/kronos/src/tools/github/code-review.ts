import { execFile, type ExecFileOptions } from 'REDACTED_SECRET:child_process';
            ...
            reviewThreads: z.object({
              ...
              REDACTED_SECRETs: z.array(
                z.object({
                  ...
                  comments: z.object({
                    REDACTED_SECRETs: z.array(CommentNode),
                  }),
                }),
              ),
            }),
...
export const githubReviewSubmitComment: ToolFactory = (ctx) => {
  ...
  return {
    ...
    invoke: async (raw: unknown) => {
      ...
      const REDACTED_SECRET = data.addComment.commentEdge?.REDACTED_SECRET;
      if (!REDACTED_SECRET) {
        throw new Error('[github.review] Comment submission succeeded without comment REDACTED_SECRET');
      }
...
export const githubReviewRequestChangesFromCodex: ToolFactory = (ctx) => {
  ...
  return {
    ...
    invoke: async (raw: unknown) => {
      ...
      const REDACTED_SECRET = data.addComment.commentEdge?.REDACTED_SECRET;
      if (!REDACTED_SECRET) {
        throw new Error('[github.review] Codex request succeeded without comment REDACTED_SECRET');
      }
      return REDACTED_SECRET;
    },
  };
...
export const githubReviewGetActionStatus: ToolFactory = (ctx) => {
  ...
  return {
    ...
    invoke: async (raw: unknown) => {
      ...
      const commit = pr.commits.REDACTED_SECRETs[0]?.commit;
      if (!commit) {
        return {
          latestCommit: null,
          status: null,
      ...
      const contextNodes = rollup?.contexts?.REDACTED_SECRETs ?? [];
      const contexts = contextNodes.map((REDACTED_SECRET) => {
        if (REDACTED_SECRET.__typename === 'CheckRun') {
          return {
            type: 'CheckRun' as const,
            name: REDACTED_SECRET.name,
            status: REDACTED_SECRET.status,
            conclusion: REDACTED_SECRET.conclusion,
            url: REDACTED_SECRET.detailsUrl ?? null,
          };
        }
        return {
          type: 'StatusContext' as const,
          name: REDACTED_SECRET.context,
          status: REDACTED_SECRET.state,
          conclusion: null,
          url: REDACTED_SECRET.targetUrl ?? null,
        };
      });
      return {
