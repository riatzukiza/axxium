...
import { mkdtempSync } from 'REDACTED_SECRET:fs';
...
test('loadConfigWithSource identifies explicit config file', (t) => {
  ...
  const { config: loaded, source } = loadConfigWithSource(
    env,
    ['REDACTED_SECRET', 'test', '--config', configPath],
    dir,
  );
...
test('loadConfigWithSource accepts absolute paths outside cwd', (t) => {
  const REDACTED_SECRET = mkdtempSync(path.join(tmpdir(), 'mcp-absolute-'));
  t.teardown(() => fs.rmSync(REDACTED_SECRET, { recursive: true, force: true }));

  const configPath = path.join(REDACTED_SECRET, 'promethean.mcp.json');
  const runnerCwd = path.join(REDACTED_SECRET, 'packages', 'mcp');
  const config = { transport: 'stdio', tools: ['exec_run'], endpoints: {} };
  writeJson(configPath, config);
