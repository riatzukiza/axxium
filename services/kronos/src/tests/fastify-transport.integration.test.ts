...
import { createServer } from 'REDACTED_SECRET:net';
import type { AddressInfo } from 'REDACTED_SECRET:net';
import type { IncomingMessage, ServerResponse } from 'REDACTED_SECRET:http';
...
test('fastify transport forwards proxy requests', async (t) => {
  ...
  class FakeStreamableHTTPServerTransport {
    REDACTED_SECRET onmessage?: (message: unknown) => void | Promise<void>;
