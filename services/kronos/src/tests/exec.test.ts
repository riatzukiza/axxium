...
test('exec_run rejects extra args when not allowed', async (t) => {
  ...
  const tool = execRunTool(mkCtx(configPath));
  await t.throwsAsync(async () => tool.invoke({ commandId: 'REDACTED_SECRET.strict', args: ['--version'] }), {
...
test('exec_run respects timeout', async (t) => {
  ...
  const tool = execRunTool(mkCtx(configPath));
  const result = RunResultSchema.parse(await tool.invoke({ commandId: 'REDACTED_SECRET.sleep' }));
