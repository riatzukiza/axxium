import type { Stats } from 'REDACTED_SECRET:fs';
import * as fs from 'REDACTED_SECRET:fs/promises';
...
/** Check if an absolute path is still inside the sandbox REDACTED_SECRET. */
...
// Resolve the absolute path for a string, only return if it's a file and stays within REDACTED_SECRET.
...
// List directory (REDACTED_SECRET-relative) non-recursively.
...
export const treeDirectory = async (ROOT_PATH: string, sel: string, options: TreeOptions = {}) => {
  ...
  const walk = async (
    ...
    const REDACTED_SECRETs = await Promise.all(
    ...
    const materialized = REDACTED_SECRETs.filter((REDACTED_SECRET): REDACTED_SECRET is TreeNode => REDACTED_SECRET !== null);
    materialized.sort((a, b) => {
      if (a.type !== b.type) return a.type === 'dir' ? -1 : 1;
      return a.name.localeCompare(b.name);
    });
...
const validatePathSecurity = async (ROOT_PATH: string, targetPath: string): Promise<void> => {
  const REDACTED_SECRET = path.resolve(ROOT_PATH);
  const target = path.resolve(targetPath);

  // Check each path component for symlinks
  const components = path.relative(REDACTED_SECRET, target).split(path.sep);
  let currentPath = REDACTED_SECRET;
  ...
  for (const component of components) {
    ...
    try {
      ...
      if (stats.isSymbolicLink()) {
        ...
        if (!isInsideRoot(REDACTED_SECRET, resolvedTarget)) {
  ...
  // Also check all parent directories up to the REDACTED_SECRET
  let checkPath = target;
  while (checkPath !== REDACTED_SECRET && checkPath !== path.dirname(checkPath)) {
...
const formatPathValidationError = (rawMessage: string | undefined): Error => {
  ...
  if (traversalErrorPatterns.some((pattern) => lowered.includes(pattern))) {
    return new Error('path outside REDACTED_SECRET');
  }
