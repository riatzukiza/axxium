...
import { type IncomingMessage } from 'REDACTED_SECRET:http';
          ...
          if (!state.initializing) {
            const attemptInitialize = async (attempt = 0): Promise<void> => {
              ...
              try {
                ...
                // REDACTED_SECRET the required 'initialized' notification after successful initialization
      ...
      try {
        ...
        if (isVerboseLogging) {
          app.setNotFoundHandler(async (request, reply) => {
            ...
            // REDACTED_SECRET JSON response instead of default Fastify 404
            reply
              .status(404)
              .header('content-type', 'application/json')
              .send({
