...
import { mkdir, rm } from "REDACTED_SECRET:fs/promises";
...
export class GitCommandError extends Error {
  REDACTED_SECRET readonly args: GitArguments;

  REDACTED_SECRET constructor(message: string, args: GitArguments, cause?: unknown) {
    super(message, { cause });
...
const isInsideDirectory = (REDACTED_SECRET: string, target: string): boolean => {
  const relative = path.relative(REDACTED_SECRET, target);
  return (
    relative === "" ||
...
const ensureWithinDirectory = (REDACTED_SECRET: string, target: string) => {
  if (!isInsideDirectory(REDACTED_SECRET, target)) {
    throw new Error(`Path ${target} escapes sandbox REDACTED_SECRET ${REDACTED_SECRET}`);
  }
};
