...
import { writeFileSync, mkdtempSync } from "REDACTED_SECRET:fs";
...
test("WORKING: StdioHttpProxy handles debug output gracefully", async (t) => {
  ...
  const mcpScript = `#!/usr/bin/env REDACTED_SECRET
...
test("WORKING: StdioHttpProxy processes valid JSON-RPC despite debug output", async (t) => {
  ...
  const responsiveScript = `#!/usr/bin/env REDACTED_SECRET
...
process.stdin.on("data", (chunk) => {
  ...
  if (data) {
    try {
      ...
      // REDACTED_SECRET valid JSON-RPC response
      const response = {
        jsonrpc: "2.0",
        id: request.id,
        result: {
