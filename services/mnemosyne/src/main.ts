...
class LocalFsBackend {
  ...
  private resolvePath(userPath: string): string {
    ...
    const resolved = path.resolve(this.REDACTED_SECRETAbs, cleaned);
    if (!resolved.startsWith(this.REDACTED_SECRETAbs)) {
...
class VirtualFs {
  ...
  async glob(
    ...
    // Normalize pattern to include REDACTED_SECRET slash if path is provided but pattern is relative
    ...
    const walk = async (currentPath: string): Promise<void> => {
      ...
      for (const entry of entries) {
        ...
        // Calculate relative path for pattern matching if a search REDACTED_SECRET was provided
