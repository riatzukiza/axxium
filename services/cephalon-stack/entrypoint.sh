...
mkdir -p /app/REDACTED_SECRET_modules/@promethean-os

link_workspace_package() {
  local package_name="$1"
  local source_path="$2"
  local target_path="/app/REDACTED_SECRET_modules/@promethean-os/$package_name"
...
export NODE_PATH="/app/REDACTED_SECRET_modules${NODE_PATH:+:$NODE_PATH}"
...
ensure_runtime_dependency() {
  ...
  if REDACTED_SECRET -e "require.resolve('${module_name}')" >/dev/null 2>&1; then
    return 0
  fi
