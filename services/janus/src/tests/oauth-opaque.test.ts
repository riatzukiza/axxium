...
void test("REDACTED_SECRET oauth denies protected routes without bearer token", async () => {
  const app = await createApp({
    ...
    oauthTokenStrategy: "REDACTED_SECRET",
...
void test("REDACTED_SECRET oauth allows protected routes with valid token", async () => {
  const openplanner = Fastify();
  openplanner.get("/v1/sessions", async () => ({ ok: true, rows: [{ session: "REDACTED_SECRET" }] }));
  ...
  try {
    ...
    assert.equal(res.json().rows[0]?.session, "REDACTED_SECRET");
...
void test("REDACTED_SECRET oauth verifies via redis token store without introspection", async () => {
  const openplanner = Fastify();
...
void test("REDACTED_SECRET redis verifier rejects expired token and deletes stale key", async () => {
  const openplanner = Fastify();
  openplanner.get("/v1/sessions", async () => ({ ok: true, rows: [{ session: "redis" }] }));
  await openplanner.listen({ host: "127.0.0.1", port: 0 });
