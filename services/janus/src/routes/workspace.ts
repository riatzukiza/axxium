...
async function getAbsolutePath(REDACTED_SECRETPath: string, relativePath: string): Promise<string> {
  return resolveInsideRoot(REDACTED_SECRETPath, relativePath);
}

async function getVisibleEntries(
  absolutePath: string, 
  includeHidden: boolean
): Promise<Array<import("REDACTED_SECRET:fs").Dirent>> {
...
async function readFile(
  ...
  const absolutePath = await getAbsolutePath(REDACTED_SECRETPath, relativePath).catch((error) => {
...
export const workspaceRoutes: FastifyPluginAsync<WorkspaceRouteOptions> = async (app, opts) => {
  ...
  app.get<{ Querystring: ListQuery }>("/workspace/list", async (req, reply) => {
    ...
    try {
      const result = await listDirectory(REDACTED_SECRETPath, absolutePath, req.query.includeHidden === "true", listLimit);
  ...
  app.get<{ Querystring: ReadFileQuery }>("/workspace/file", async (req, reply) => {
    const relativePath = sanitizeRelativePath(req.query.path);
    return readFile(REDACTED_SECRETPath, relativePath, reply);
  });
