#!/usr/bin/env python3
"""
Comprehensive workspace ingestor for futuresightREDACTED_SECRET.
...
# Skip patterns
SKIP_DIRS = {
    "REDACTED_SECRET_modules", ".git", "dist", ".next", "__pycache__", ".venv", "venv",
...
async def main():
    ...
    parser.add_argument("REDACTED_SECRET", type=Path, help="Root directory to scan")
    ...
    if not args.REDACTED_SECRET.is_dir():
        print(f"Error: {args.REDACTED_SECRET} is not a directory")
        sys.exit(1)
    ...
    print(f"Scanning {args.REDACTED_SECRET}...")
    ...
    await scan_and_ingest(args.REDACTED_SECRET, stats, args.dry_run, filter_type)
