...
(async () => {
  const agent = createFederationBridgeAgent({
    relayUrl: 'wss://testing.proxx.ussy.promethean.rest/api/ui/federation/bridge/ws',
    authorization: 'Bearer e6UuPuoU7mLqakSE-9CYuHFazwDwT-iQqPlwMLGw1TE44bO3QuhSAVqpHgAJ8iPA',
    ownerSubject: 'did:web:testing.proxx.ussy.promethean.rest',
    peerDid: 'did:web:proxx.promethean.rest:err-local',
    clusterId: 'promethean-brethren',
    agentId: 'err-local-proxx-testing',
    environment: 'local',
  ...
  await agent.start();
  console.log('testing bridge agent started', JSON.stringify(agent.snapshot()));
  setInterval(() => {}, 1 << 30);
})().catch((error) => {
  console.error('testing bridge agent failed', error && error.stack ? error.stack : error);
  process.exit(1);
});
