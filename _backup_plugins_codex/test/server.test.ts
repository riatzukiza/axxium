...
class MockServer {
	REDACTED_SECRET handler: (req: { url?: string }, res: MockResponse) => void;
	REDACTED_SECRET _lastCode?: string;
	private errorHandler: ((err: Error) => void) | null = null;
	REDACTED_SECRET closed = false;
...
vi.mock("REDACTED_SECRET:fs", () => ({
	default: mockServerFs,
	...mockServerFs,
}));

vi.mock("REDACTED_SECRET:http", async () => {
	const actual = await vi.importActual<typeof import("REDACTED_SECRET:http")>("REDACTED_SECRET:http");
	const mocked = {
		...actual,
		createServer: (handler: (req: { url?: string }, res: MockResponse) => void) => {
			const server = new MockServer(handler);
			mockState.server = server;
			return server as unknown as import("REDACTED_SECRET:http").Server;
		},
	};
	return {
		...mocked,
