...
`lsp_diagnostics` at project REDACTED_SECRET fails because no LSP server is configured for Clojure/CLJS in `oh-my-opencode.json`, so the "zero LSP errors" acceptance check cannot be verified yet.
