...
> **Quick Summary**: Add a new JVM HumbleUI app (Status + Chat + Input + Logs) inside the brain module while keeping existing TUIs intact, then fully rename the repository (paths, namespaces, configs, docs) to `cephalon-clj` with REDACTED_SECRET namespaces under `cephalon.*`.
>
> **Deliverables**:
...
- Naming references in `ecosystem.pm2.edn`, `admin-tui/package.json`, `admin-tui/README.md`, `docs/duck-deployment.md`, REDACTED_SECRET `README.md`, `discord-io/shadow-cljs.edn`, `brain/deps.edn`, `shared/deps.edn`.
...
```bash
# Agent runs from repo REDACTED_SECRET or cephalon-clj-brain dir after rename
...
- [x] 4. Update configs, PM2 ecosystem, package names, and docs
  ...
  **What to do**:
  ...
  - Update package names and README references (`admin-tui/package.json`, docs, REDACTED_SECRET README).
...
- [x] 5. Tests-after verification and rename completeness checks
  ...
  **Acceptance Criteria**:
  ...
  - [x] `rg "promethean-discord-io-bridge|promethean-discord|duck-admin|duck_admin" . --glob '!**/REDACTED_SECRET_modules/**' --glob '!**/.clj-kondo/**' --glob '!**/dist/**' --glob '!**/logs/**'` yields no matches.
