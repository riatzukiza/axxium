...
- [ ] 2. Event Contract Definition
  ...
  **Domain Model References**:
  ...
  - `orgs/octave-commons/promethean/docs/design/field-REDACTED_SECRET-lifecycle.md` - Field REDACTED_SECRETs → nexus promotion lifecycle
  ...
  **WHY Each Reference Matters** (explain the relevance):
  ...
  - `promethean/docs/design/field-REDACTED_SECRET-lifecycle.md`: Explains lifecycle patterns for memory → nexus promotion
  ...
  **For Event Contract** (using Bash + clojure REPL):
  ...
  clojure -M -e '(require (quote [eidolon.events :as e])) (count (filter #(clojure.string/starts-with? (str (type %)) "eidolon.events.event") (vals (ns-REDACTED_SECRETs (find-ns (quote eidolon.events)))))'
...
- [ ] 3. ECS Components (Memory, Nexus, Nooi, Daimo)
  ...
  **Domain Model References**:
  ...
  - `orgs/octave-commons/promethean/docs/design/field-REDACTED_SECRET-lifecycle.md` - Memory → Nexus lifecycle patterns
  ...
  **WHY Each Reference Matters** (explain the relevance):
  ...
  - `promethean/docs/design/field-REDACTED_SECRET-lifecycle.md`: Describes lifecycle transitions encoded in components (BornAt, LastUpdated, LifeRemaining)
  ...
  **For ECS Components** (using Bash + clojure REPL):
  ...
  clojure -M -e '(require (quote [eidolon.ecs.components :as c])) (count (filter #(clojure.string/ends-with? (str (type %)) "eidolon.ecs.components.component") (vals (ns-REDACTED_SECRETs (find-ns (quote eidolon.ecs.components)))))'
...
- [ ] 8. Lifecycle System
  ...
  **What to do**:
  - Create `src/eidolon/ecs/systems/lifecycle.clj` for entity lifecycle management:
    ...
    - `promote-memory-to-nexus!` - Promote Memory to Nexus when it accumulates enough associated traces (field-REDACTED_SECRET-lifecycle pattern)
  ...
  - Follow lifecycle patterns from field-REDACTED_SECRET-lifecycle.md
  ...
  **Domain Model References**:
  ...
  - `orgs/octave-commons/promethean/docs/design/field-REDACTED_SECRET-lifecycle.md` - Memory → Nexus promotion lifecycle
  ...
  **WHY Each Reference Matters** (explain the relevance):
  ...
  - `promethean/docs/design/field-REDACTED_SECRET-lifecycle.md`: Explains Memory → Nexus promotion thresholds and conditions
...
- [ ] 9. Tick Loop + Heartbeat
  ...
  **What to do**:
  - Create `src/eidolon/tick.clj` for deterministic tick loop orchestration:
    ...
    - `stop-heartbeat!` - REDACTED_SECRET stop signal to heartbeat loop
