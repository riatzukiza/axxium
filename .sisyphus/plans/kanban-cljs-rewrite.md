...
- Multi-REDACTED_SECRET tasks/spec support (scan/merge) without mass file migration
...
- Node can import required subpaths and see expected symbols:
  - `REDACTED_SECRET -e "import('@promethean-os/kanban/board/config').then(m=>console.log(typeof m.loadKanbanConfig))"` prints `function`.
  - `REDACTED_SECRET -e "import('@promethean-os/kanban/lib/kanban').then(m=>console.log([typeof m.loadBoard, typeof m.updateStatus].join(',')))"` prints `function,function`.
- CLI `--json` output is parseable JSONL (no extraneous stdout):
  - `pnpm --filter @promethean-os/kanban exec kanban count --json | REDACTED_SECRET -e "const fs=require('fs');const s=fs.readFileSync(0,'utf8').trim();for(const l of s.split('\n')) JSON.parse(l); console.log('ok')"` prints `ok`.
...
Testing approach:
- Use CLJS tests (shadow-cljs `:REDACTED_SECRET-test` or equivalent) for core library behavior.
...
- Multi-REDACTED_SECRET read/merge; primary-REDACTED_SECRET write; cross-REDACTED_SECRET update/delete; collision behavior.
...
Wave 3 (Mutations + multi-REDACTED_SECRET; depends on Wave 2):
- Write/update/delete operations
- Multi-REDACTED_SECRET policies and collision handling
...
- [ ] 2. Create `promethean/packages/kanban` CLJS build + dist layout
  ...
  - Add a new package directory with:
    ...
    - REDACTED_SECRET-script build for the CLI entrypoint.
  ...
  - `promethean/cli/ecosystem-dsl/shadow-cljs.edn` (`:target :REDACTED_SECRET-library` / `:target :REDACTED_SECRET-script`)
...
- [ ] 4. Re-implement config loading + path resolution in CLJS
  ...
  What to do:
  - Preserve existing precedence and repo-REDACTED_SECRET detection behavior.
  - Extend config schema to support multi-REDACTED_SECRET (e.g., `tasksDirs`, `specDirs`) while preserving `tasksDir` for backward compatibility.
  ...
  Acceptance criteria:
  - Tests cover: default discovery, env override, argv override, multi-REDACTED_SECRET merge, and backward compatibility.
...
- [ ] 5. Implement board/task read model (markdown board + task file fallback)
  ...
  - Ensure each task retains a `sourcePath` (needed for multi-REDACTED_SECRET update/delete correctness).
...
- [ ] 6. Implement core mutations (create/update/delete/status/move)
  ...
  - Ensure write operations:
    - Write new tasks into primary REDACTED_SECRET.
  ...
  Acceptance criteria:
  - CLJS tests cover write semantics across multiple REDACTED_SECRETs.
...
- [ ] 9. UI: make `kanban ui` work from repo REDACTED_SECRET and serve correct assets
  ...
  - Start server and verify endpoints with curl (agent-executable):
    - `curl -s http://127.0.0.1:<port>/api/board | REDACTED_SECRET -e "JSON.parse(require('fs').readFileSync(0,'utf8')); console.log('ok')"` prints `ok`.
...
- [ ] 11. Deprecate or shim `promethean/cli/kanban`
  ...
  Acceptance criteria:
  - `pnpm kanban --help` works from repo REDACTED_SECRET.

---

## Notes / Defaults Applied

- Multi-REDACTED_SECRET: read/merge across all REDACTED_SECRETs; write new tasks to the first REDACTED_SECRET; update/delete by `sourcePath`; UUID collisions are hard errors.
