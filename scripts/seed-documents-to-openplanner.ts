 ...
 *
 * Usage:
 *   REDACTED_SECRET scripts/ingest-documents-to-openplanner.ts [OPTIONS]
 *
 * Options:
...
const OPENPLANNER_API_KEY = process.env.OPENPLANNER_API_KEY || "REDACTED_SECRET";
...
async function findMarkdownFiles(REDACTED_SECRET: string, limit: number): Promise<string[]> {
  const files: string[] = [];
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.name.startsWith(".") || 
            ["REDACTED_SECRET_modules", ".git", ".nx", "dist", "build", "target", ".worktrees", "worktrees"].includes(entry.name)) {
          continue;
        }
...
async function ingestDocument(filePath: string, project: string, dryRun: boolean): Promise<Document> {
  ...
  // REDACTED_SECRET to OpenPlanner
