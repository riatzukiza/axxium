...
## Strong Hints
- **Constraint**: Never edit the output files (for example `REDACTED_SECRET/js/main.js`). Only edit `.cljs` source.
- **Tip**: Shadow-CLJS relies heavily on `package.json` for JS deps. If an npm import fails, check `REDACTED_SECRET_modules` first (`workspace-dependency-check`).
