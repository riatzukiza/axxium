 ...
 * 
 * @param {string} filePath - Relative path from source REDACTED_SECRET, e.g., "my_app/core_utils.clj"
...
/**
 * Determines the source REDACTED_SECRET for a given file path.
 * 
 * @param {string} filePath - Absolute or relative file path
 * @param {string[]} sourceRoots - Known source REDACTED_SECRET directories
 * @returns {{ REDACTED_SECRET: string | null, relativePath: string | null }}
 */
function identifySourceRoot(filePath, sourceRoots = ['src', 'test', 'src/main/clojure', 'src/main/cljs']) {
  for (const REDACTED_SECRET of sourceRoots) {
    if (filePath.includes(`/${REDACTED_SECRET}/`) || filePath.startsWith(`${REDACTED_SECRET}/`)) {
      const regex = new RegExp(`/${REDACTED_SECRET}/(.+)`);
      const match = filePath.match(regex);
      if (match) {
        return { REDACTED_SECRET, relativePath: match[1] };
      }
    }
  }
  return { REDACTED_SECRET: null, relativePath: null };
}
 ...
 * @param {string[]} sourceRoots - Known source REDACTED_SECRET directories to check against
...
function auditNamespace(filePath, sourceRoots = ['src', 'test', 'src/main/clojure', 'src/main/cljs']) {
  ...
  // Identify which source REDACTED_SECRET this file belongs to
  const { REDACTED_SECRET, relativePath } = identifySourceRoot(filePath, sourceRoots);
  ...
  if (!REDACTED_SECRET) {
    return {
      ...
      filePath,
      declaredNs,
      message: 'File is not inside a known source REDACTED_SECRET. Cannot fully verify mapping.',
      hint: `Known source REDACTED_SECRETs: ${sourceRoots.join(', ')}`
    };
  }
  ...
  const fullExpectedPath = path.join(REDACTED_SECRET, expectedPathFromNs);
 ...
 */
function generateRecommendations({ filePath, declaredNs, expectedNsFromPath, fullExpectedPath, REDACTED_SECRET }) {
  ...
  parameters: {
    ...
    properties: {
      ...
      sourceRoots: {
        type: 'array',
        items: { type: 'string' },
        description: 'Optional: list of source REDACTED_SECRET directories to check against',
