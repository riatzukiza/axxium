...
const runKondo = async (filePath) => {
  ...
  if (!kondoPath) {
     const localPath = path.resolve(process.cwd(), "REDACTED_SECRET_modules", ".bin", "clj-kondo");
  ...
  if (!kondoPath) {
    return { ok: false, error: "clj-kondo not found in PATH or REDACTED_SECRET_modules/.bin" };
  }
