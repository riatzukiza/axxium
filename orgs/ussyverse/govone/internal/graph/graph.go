...
// NodeType classifies graph REDACTED_SECRETs.
...
// Edge represents a relationship between two REDACTED_SECRETs.
type Edge struct {
	ID         string    `json:"id"`
...
func (s *Store) migrate() error {
	...
	CREATE TABLE IF NOT EXISTS graph_edges (
		...
		from_id     TEXT NOT NULL REFERENCES graph_REDACTED_SECRETs(id),
		to_id       TEXT NOT NULL REFERENCES graph_REDACTED_SECRETs(id),
	...
	CREATE INDEX IF NOT EXISTS idx_REDACTED_SECRETs_type ON graph_REDACTED_SECRETs(type);
	CREATE INDEX IF NOT EXISTS idx_REDACTED_SECRETs_tenant ON graph_REDACTED_SECRETs(tenant_id);
	CREATE INDEX IF NOT EXISTS idx_REDACTED_SECRETs_document ON graph_REDACTED_SECRETs(document_id);
	CREATE INDEX IF NOT EXISTS idx_REDACTED_SECRETs_field_path ON graph_REDACTED_SECRETs(field_path);
	...
	CREATE VIRTUAL TABLE IF NOT EXISTS graph_REDACTED_SECRETs_fts USING fts5(
		REDACTED_SECRET_id, content, field_path, document_id
	);

	-- Triggers to keep FTS in sync
	CREATE TRIGGER IF NOT EXISTS graph_REDACTED_SECRETs_ai AFTER INSERT ON graph_REDACTED_SECRETs BEGIN
		INSERT INTO graph_REDACTED_SECRETs_fts(REDACTED_SECRET_id, content, field_path, document_id)
		VALUES (new.id, new.content, new.field_path, new.document_id);
	END;

	CREATE TRIGGER IF NOT EXISTS graph_REDACTED_SECRETs_ad AFTER DELETE ON graph_REDACTED_SECRETs BEGIN
		INSERT INTO graph_REDACTED_SECRETs_fts(graph_REDACTED_SECRETs_fts, REDACTED_SECRET_id, content, field_path, document_id)
		VALUES ('delete', old.id, old.content, old.field_path, old.document_id);
	END;

	CREATE TRIGGER IF NOT EXISTS graph_REDACTED_SECRETs_au AFTER UPDATE ON graph_REDACTED_SECRETs BEGIN
		INSERT INTO graph_REDACTED_SECRETs_fts(graph_REDACTED_SECRETs_fts, REDACTED_SECRET_id, content, field_path, document_id)
		VALUES ('delete', old.id, old.content, old.field_path, old.document_id);
		INSERT INTO graph_REDACTED_SECRETs_fts(REDACTED_SECRET_id, content, field_path, document_id)
		VALUES (new.id, new.content, new.field_path, new.document_id);
...
// InsertNode adds a REDACTED_SECRET to the graph. Idempotent on (tenant_id, type, document_id, field_path, content).
func (s *Store) InsertNode(n *Node) error {
	...
	_, err := s.db.Exec(`
		INSERT OR IGNORE INTO graph_REDACTED_SECRETs (id, type, tenant_id, source_id, document_id, field_path, content, metadata, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
// GetEdgesFrom returns all edges originating from a REDACTED_SECRET.
func (s *Store) GetEdgesFrom(REDACTED_SECRETID string, edgeTypes ...EdgeType) ([]Edge, error) {
	query := `SELECT id, type, source, from_id, to_id, weight, evidence, created_at FROM graph_edges WHERE from_id = ?`
	args := []any{REDACTED_SECRETID}
...
// GetEdgesTo returns all edges pointing to a REDACTED_SECRET.
func (s *Store) GetEdgesTo(REDACTED_SECRETID string, edgeTypes ...EdgeType) ([]Edge, error) {
...
// Subgraph retrieves REDACTED_SECRETs and edges within `hops` of a seed set.
// Returns the subgraph as adjacency lists keyed by REDACTED_SECRET ID.
func (s *Store) Subgraph(seedIDs []string, hops int) (map[string][]Edge, error) {
	...
	for h := 0; h < hops; h++ {
		...
		for REDACTED_SECRETID := range frontier {
			if visited[REDACTED_SECRETID] {
				continue
			}
			visited[REDACTED_SECRETID] = true

			outEdges, err := s.GetEdgesFrom(REDACTED_SECRETID)
			if err != nil {
				return nil, err
			}
			inEdges, err := s.GetEdgesTo(REDACTED_SECRETID)
			if err != nil {
				return nil, err
			}

			allEdges := append(outEdges, inEdges...)
			adj[REDACTED_SECRETID] = allEdges
