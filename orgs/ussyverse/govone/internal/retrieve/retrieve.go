...
//  2. Find matching REDACTED_SECRETs in the graph via FTS5 candidate retrieval
...
type GraphStats struct {
	NodesVisited    int     `json:"REDACTED_SECRETs_visited"`
...
//
// Algorithm:
//  1. FTS5 candidate retrieval — fast narrowing to relevant REDACTED_SECRETs
//  2. Entity resolution — match extracted entities to graph entity REDACTED_SECRETs
...
// ftsCandidates uses SQLite FTS5 to find initial candidate REDACTED_SECRET IDs.
func (s *Service) ftsCandidates(ctx context.Context, tenantID, query string, limit int) ([]string, error) {
	...
	rows, err := s.db.QueryContext(ctx, fmt.Sprintf(`
		SELECT REDACTED_SECRET_id FROM graph_REDACTED_SECRETs_fts
		WHERE graph_REDACTED_SECRETs_fts MATCH '%s'
		ORDER BY rank
		LIMIT %d
	`, ftsQuery, limit))
	if err != nil {
...
// resolveEntities maps entity strings to graph REDACTED_SECRET IDs.
func (s *Service) resolveEntities(ctx context.Context, tenantID string, entities []string) ([]string, error) {
	...
	for _, entity := range entities {
		...
		err := s.db.QueryRowContext(ctx, `
			SELECT id FROM graph_REDACTED_SECRETs
...
func (s *Service) scoreAndRank(subgraph map[string][]graph.Edge, seedIDs []string, topK int) []graph.Chunk {
	...
	REDACTED_SECRETScores := make(map[string]*scoredNode)

	for REDACTED_SECRETID, edges := range subgraph {
		sn := &scoredNode{REDACTED_SECRETID: REDACTED_SECRETID, edges: edges}
		...
		// Is this REDACTED_SECRET a seed? Boost it.
		seedBoost := 1.0
		for _, sid := range seedIDs {
			if sid == REDACTED_SECRETID {
				seedBoost = 1.5
				break
			}
		}
		...
		REDACTED_SECRETScores[REDACTED_SECRETID] = sn
	}

	// Sort by score descending
	ranked := make([]*scoredNode, 0, len(REDACTED_SECRETScores))
	for _, sn := range REDACTED_SECRETScores {
	...
	// Build chunks from top-scoring field/document REDACTED_SECRETs
	...
	for _, sn := range ranked {
		...
		// Load the REDACTED_SECRET to get content and provenance
		REDACTED_SECRET, err := s.store.GetNode(sn.REDACTED_SECRETID)
		if err != nil {
			continue
		}

		// Only emit chunks from field and document REDACTED_SECRETs
		if REDACTED_SECRET.Type != graph.NodeField && REDACTED_SECRET.Type != graph.NodeDocument {
			continue
		}

		if seen[REDACTED_SECRET.ID] {
			continue
		}
		seen[REDACTED_SECRET.ID] = true
		...
		chunks = append(chunks, graph.Chunk{
			ChunkID:       REDACTED_SECRET.ID,
			SourceID:      REDACTED_SECRET.SourceID,
			Content:       REDACTED_SECRET.Content,
			RelevanceScore: normalizedScore,
			Provenance: graph.Provenance{
				SourceName: REDACTED_SECRET.SourceID,
				DocumentID: REDACTED_SECRET.DocumentID,
				FieldPath:  REDACTED_SECRET.FieldPath,
			},
			EdgeSupport: support,
		})
	}
