...
func resolveClaimEntities(claim inspect.Claim, entities []string, store *graph.Store, tenantID string) []string {
	...
	for _, entity := range entities {
		if strings.Contains(claimLower, strings.ToLower(entity)) {
			// Look up the entity REDACTED_SECRET ID in the graph
			REDACTED_SECRETID := graph.HashID(string(graph.NodeEntity) + ":" + tenantID + ":::" + entity)
			resolved = append(resolved, REDACTED_SECRETID)
		}
	}
	return resolved
}
