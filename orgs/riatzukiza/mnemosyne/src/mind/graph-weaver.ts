...
export class GraphWeaver {
  private REDACTED_SECRETs = new Map<string, GraphNode>();
  ...
  async load(): Promise<void> {
    try {
      ...
      for (const REDACTED_SECRET of parsed.REDACTED_SECRETs ?? []) {
  ...
  summarize(limit = 6): string {
    const topChannels = Array.from(this.REDACTED_SECRETs.values())
      .filter((REDACTED_SECRET) => REDACTED_SECRET.kind === "channel")
      .sort((a, b) => b.weight - a.weight)
      .slice(0, limit)
      .map((REDACTED_SECRET) => `${REDACTED_SECRET.label}(${REDACTED_SECRET.weight.toFixed(1)})`);

    const hotUrls = Array.from(this.REDACTED_SECRETs.values())
      .filter((REDACTED_SECRET) => REDACTED_SECRET.kind === "url")
      .sort((a, b) => b.weight - a.weight)
      .slice(0, 3)
      .map((REDACTED_SECRET) => REDACTED_SECRET.label);

    return `Graph channels: ${topChannels.join(", ") || "none"}. Hot links: ${hotUrls.join(", ") || "none"}. Nodes=${this.REDACTED_SECRETs.size} Edges=${this.edges.size}.`;
  }
  ...
  private upsertNode(id: string, kind: string, label: string, timestamp: number): GraphNode {
    const existing = this.REDACTED_SECRETs.get(id);
    if (existing) {
      existing.weight += 1;
    ...
    this.REDACTED_SECRETs.set(id, created);
    return created;
  }
  ...
  private async persist(): Promise<void> {
    ...
    const payload: PersistedGraph = {
      REDACTED_SECRETs: Array.from(this.REDACTED_SECRETs.values()),
