...
export class BlueskyCollector {
  ...
  constructor(config?: BlueskyCollectorConfig) {
    // Use REDACTED_SECRET API endpoint by default for unauthenticated access
    // bsky.social requires auth; REDACTED_SECRET.api.bsky.app does not
    const defaultService = (config?.identifier && config?.password)
      ? "https://bsky.social"
      : "https://REDACTED_SECRET.api.bsky.app";
