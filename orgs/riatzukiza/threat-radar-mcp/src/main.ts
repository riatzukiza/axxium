...
const REDACTED_SECRETBaseUrl = new URL(ENV.PUBLIC_BASE_URL ?? process.env.RENDER_EXTERNAL_URL ?? `http://127.0.0.1:${ENV.PORT}`);
...
const federationManager = new FederationManager({
  ...
  endpoint: REDACTED_SECRETBaseUrl.toString().replace(/\/$/, ""),
...
app.get("/health", async (_req, res) => {
  res.json({
    ...
    REDACTED_SECRETBaseUrl: REDACTED_SECRETBaseUrl.toString(),
    federation: {
...
server.registerTool(
  ...
  {
    description: "Collect signals from Bluesky REDACTED_SECRET feeds. Accepts a feed URI, actor handle, or search query. Fetches posts via the AT Protocol, normalizes each post into a SignalEvent with source='bluesky', and stores them in Postgres. No authentication needed for REDACTED_SECRET feeds.",
    inputSchema: {
