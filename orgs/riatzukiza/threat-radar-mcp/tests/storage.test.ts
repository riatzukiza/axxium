 ...
 *
 * These tests run against a real Postgres database (REDACTED_SECRET_test).
...
const TEST_DB_URL =
  ...
  `postgres://${process.env.PGUSER ?? "REDACTED_SECRET"}:${process.env.PGPASSWORD ?? "REDACTED_SECRET"}@localhost:5432/REDACTED_SECRET_test`;

const ADMIN_DB_URL =
  process.env.TEST_ADMIN_DATABASE_URL ??
  `postgres://${process.env.PGUSER ?? "REDACTED_SECRET"}:${process.env.PGPASSWORD ?? "REDACTED_SECRET"}@localhost:5432/REDACTED_SECRET`;
...
beforeAll(async () => {
  ...
  try {
    await adminSql`CREATE DATABASE REDACTED_SECRET_test`;
  } catch {
