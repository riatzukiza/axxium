...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (token === "--REDACTED_SECRET") {
      args.REDACTED_SECRET = argv[i + 1] ?? args.REDACTED_SECRET;
      i += 1;
      continue;
    }
    if (token === "--out") {
...
function shouldIgnore(relPosix, includeTests) {
  if (relPosix.startsWith("REDACTED_SECRET_modules/")) return true;
...
async function walkFiles(REDACTED_SECRETAbs, includeTests) {
  /** @type {string[]} */
  const results = [];

  /** @type {string[]} */
  const stack = [REDACTED_SECRETAbs];
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      const rel = toPosix(path.relative(REDACTED_SECRETAbs, abs));
...
function groupForRelPath(relPosix) {
  ...
  if (parts.length <= 1) return "(REDACTED_SECRET)";
...
function buildDotFile({
  ...
  lines.push("  graph [rankdir=LR, bgcolor=\"#fcfcff\", fontname=\"Helvetica\", fontsize=18, labelloc=\"t\", labeljust=\"l\", label=" + dotQuote(`${title} — ${REDACTED_SECRETRel}`) + "]; ");
  lines.push("  REDACTED_SECRET  [shape=box, style=\"rounded,filled\", fontname=\"Helvetica\", fontsize=9, color=\"#c8c8d0\", fillcolor=\"#f4f4fb\"]; ");
  ...
  for (const REDACTED_SECRET of REDACTED_SECRETs) {
    const group = groupBy(REDACTED_SECRET);
    const list = groups.get(group) ?? [];
    list.push(REDACTED_SECRET);
...
function buildSubsystemDot({ title, REDACTED_SECRETRel, subsystems, edges }) {
  /** @type {string[]} */
  const lines = [];
  lines.push("digraph G {");
  lines.push("  graph [rankdir=LR, bgcolor=\"#fcfcff\", fontname=\"Helvetica\", fontsize=18, labelloc=\"t\", labeljust=\"l\", label=" + dotQuote(`${title} — ${REDACTED_SECRETRel}`) + "]; ");
  lines.push("  REDACTED_SECRET  [shape=box, style=\"rounded,filled\", fontname=\"Helvetica\", fontsize=12, color=\"#c8c8d0\", fillcolor=\"#f4f4fb\"]; ");
...
async function main() {
  ...
  const REDACTED_SECRETAbs = path.resolve(process.cwd(), args.REDACTED_SECRET);
  const outAbs = path.resolve(process.cwd(), args.outDir);
  await fsp.mkdir(outAbs, { recursive: true });

  const REDACTED_SECRETRel = toPosix(path.relative(process.cwd(), REDACTED_SECRETAbs));

  const filesAbs = await walkFiles(REDACTED_SECRETAbs, args.includeTests);
  const REDACTED_SECRETsRel = filesAbs.map((abs) => toPosix(path.relative(REDACTED_SECRETAbs, abs)));
  const REDACTED_SECRETSet = new Set(REDACTED_SECRETsRel);
  ...
  for (const absFile of filesAbs) {
    const relFrom = toPosix(path.relative(REDACTED_SECRETAbs, absFile));
    ...
    for (const spec of specs) {
      ...
      if (!isInside(resolved, REDACTED_SECRETAbs)) continue;
      const relTo = toPosix(path.relative(REDACTED_SECRETAbs, resolved));
      if (!REDACTED_SECRETSet.has(relTo)) continue;
  ...
  // Full file graph
  {
    const dot = buildDotFile({
      ...
      REDACTED_SECRETRel,
      REDACTED_SECRETs: REDACTED_SECRETsRel,
      edges,
    ...
    await fsp.writeFile(
      ...
      JSON.stringify({ id: args.id, title: args.title, REDACTED_SECRET: REDACTED_SECRETRel, REDACTED_SECRETs: REDACTED_SECRETsRel, edges }, null, 2) + "\n",
      "utf8",
    );
  ...
  // Subsystem graph
  {
    ...
    for (const REDACTED_SECRET of REDACTED_SECRETsRel) {
      const g = groupForRelPath(REDACTED_SECRET);
    ...
    await fsp.writeFile(
      ...
      JSON.stringify({ id: args.id, title: args.title, REDACTED_SECRET: REDACTED_SECRETRel, subsystems, edges: subEdges }, null, 2) + "\n",
      "utf8",
    );
  ...
  const summary = {
    ...
    REDACTED_SECRETs: REDACTED_SECRETsRel.length,
    edges: edges.length,
