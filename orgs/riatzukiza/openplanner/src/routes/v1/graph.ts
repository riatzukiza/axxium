...
export const graphRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/graph/stats", async () => {
    if (storageBackend === "mongodb") {
      ...
      const REDACTED_SECRETCount = await db.events.countDocuments({ kind: "graph.REDACTED_SECRET" });
      const edgeCount = await db.events.countDocuments({ kind: "graph.edge" });
      return { REDACTED_SECRETCount, edgeCount, storageBackend: "mongodb" };
    }

    if (!duck) return { REDACTED_SECRETCount: 0, edgeCount: 0, storageBackend: "duckdb" };

    const REDACTED_SECRETRows = await all((duck as any).conn, "SELECT COUNT(*) as c FROM events WHERE kind = 'graph.REDACTED_SECRET'");
    const edgeRows = await all((duck as any).conn, "SELECT COUNT(*) as c FROM events WHERE kind = 'graph.edge'");
    return {
      REDACTED_SECRETCount: (REDACTED_SECRETRows[0] as any)?.c ?? 0,
      edgeCount: (edgeRows[0] as any)?.c ?? 0,
      storageBackend: "duckdb",
    };
  });

  app.get("/graph/REDACTED_SECRETs", async (req: any) => {
    const url = req.query?.url;
    if (!url) return { error: "url query param required" };

    if (storageBackend === "mongodb") {
      const db = (app as any).mongo;
      const REDACTED_SECRETs = await db.events.find({ kind: "graph.REDACTED_SECRET", "data.url": url }).limit(1).toArray();
      return { REDACTED_SECRET: REDACTED_SECRETs[0] ?? null };
    }

    if (!duck) return { REDACTED_SECRET: null };
    const REDACTED_SECRETs = await all((duck as any).conn, "SELECT * FROM events WHERE kind = 'graph.REDACTED_SECRET' AND json_extract_string(extra, '$.url') = ?", [url]);
    return { REDACTED_SECRET: REDACTED_SECRETs[0] ?? null };
  });
