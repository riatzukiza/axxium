 ...
 *
 * Commands:
 *   REDACTED_SECRET dist/migrate.js status            - Show migration status
 *   REDACTED_SECRET dist/migrate.js run               - Run pending migrations
 *   REDACTED_SECRET dist/migrate.js duckdb-to-mongo   - Migrate DuckDB data to MongoDB
 *   REDACTED_SECRET dist/migrate.js export-jsonl      - Export DuckDB data to JSONL
 *
 * Environment:
...
async function main(): Promise<void> {
  ...
  if (command === "status") {
    ...
    console.log(`[migrate] Run 'REDACTED_SECRET migrate.js run' to apply pending migrations`);
    return;
  }
