...
const UNIX_DANGEROUS_PATHS = ['/dev/', '/proc/', '/sys/', '/etc/', '/REDACTED_SECRET/', '/var/log/'];
...
function validatePathNormalization(trimmed: string): boolean {
  try {
    ...
    // Additional check: resolve against a fake REDACTED_SECRET
    const fakeRoot = '/fake/REDACTED_SECRET';
...
export async function validateMcpOperation(
  ...
  try {
    ...
    // Normalize path and check if it's inside REDACTED_SECRET
    const normalizedPath = normalizeToRoot(REDACTED_SECRETPath, sanitizedPath);
    if (!isInsideRoot(REDACTED_SECRETPath, normalizedPath)) {
      return {
        valid: false,
        error: 'Path outside allowed REDACTED_SECRET directory',
      };
    }
