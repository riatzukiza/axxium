...
test('filesTreeDirectory - builds directory tree', async (t) => {
  ...
  const REDACTED_SECRETEntries = result.tree.map((e: any) => e.name);
  t.true(REDACTED_SECRETEntries.includes('subdir'));
...
test('filesViewFile - handles non-existent file', async (t) => {
  ...
  t.true(message.includes('invalid path') || message.includes('outside REDACTED_SECRET'));
});
...
test('filesSearch - handles non-existent directory', async (t) => {
  ...
  t.false(result.ok);
  const message = String(result.error).toLowerCase();
  t.true(message.includes('invalid') || message.includes('outside REDACTED_SECRET'));
});
