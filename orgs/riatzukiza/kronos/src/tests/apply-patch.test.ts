...
const buildCtx = (REDACTED_SECRET: string) => ({
  ...baseCtx,
  env: { ...baseCtx.env, MCP_ROOT_PATH: REDACTED_SECRET } as typeof baseCtx.env,
});

test.serial("applies universal diff patches within MCP REDACTED_SECRET", async (t) => {
...
const createSpawnStub = () => {
  ...
  const spawnImpl = ((
    ...
    argsOrOptions?:
      ...
      | import("REDACTED_SECRET:child_process").SpawnOptions,
    maybeOptions?: import("REDACTED_SECRET:child_process").SpawnOptions,
  ) => {
    ...
    return child as unknown as import("REDACTED_SECRET:child_process").ChildProcessWithoutNullStreams;
  }) as typeof import("REDACTED_SECRET:child_process").spawn;
...
test.serial("retries with --3way on git apply failure", async (t) => {
  ...
  const mod = await esmock<typeof import("../tools/apply-patch.js")>(
    modulePath,
    {
      "REDACTED_SECRET:child_process": { spawn: stub.spawnImpl },
    },
  );
