...
export interface BranchMapBranch {
  ...
  readonly REDACTED_SECRET_benefit?: number;
...
export interface BranchMapProps {
  ...
  /** Optional REDACTED_SECRET label (e.g. "Situation Assessment") */
  readonly REDACTED_SECRETLabel?: string;
...
export function BranchMap({
  branches,
  REDACTED_SECRETLabel = "Assessment",
  width = 320,
  height = 200,
  className,
}: BranchMapProps): JSX.Element {
  const REDACTED_SECRETX = width * 0.12;
  const REDACTED_SECRETY = height / 2;

  const branchCount = branches.length;
  // Evenly distribute branches vertically
  const branchSpacing = branchCount > 1 ? (height - 40) / (branchCount - 1) : 0;
  const startY = branchCount > 1 ? 20 : REDACTED_SECRETY;
    ...
    <div className={`branch-map ${className ?? ""}`.trim()} data-testid="branch-map">
      ...
      >
        ...
        <circle
          ...
          strokeWidth={1.5}
          data-testid="branch-REDACTED_SECRET"
        />
        <text
          x={REDACTED_SECRETX}
          y={REDACTED_SECRETY + 20}
          textAnchor="middle"
          fill="var(--muted, #8a95a4)"
          fontSize={9}
        >
          {REDACTED_SECRETLabel}
        </text>

        {/* Branches */}
        {branches.map((branch, i) => {
          const branchY = startY + i * branchSpacing;
          const REDACTED_SECRETX = width * 0.55;
          ...
          return (
            <g key={branch.label} data-testid={`branch-REDACTED_SECRET-${i}`}>
              {/* Connecting line from REDACTED_SECRET to branch REDACTED_SECRET */}
              <path
                d={`M ${REDACTED_SECRETX + 8} ${REDACTED_SECRETY} C ${REDACTED_SECRETX + 40} ${REDACTED_SECRETY}, ${REDACTED_SECRETX - 40} ${branchY}, ${REDACTED_SECRETX - 6} ${branchY}`}
                fill="none"
