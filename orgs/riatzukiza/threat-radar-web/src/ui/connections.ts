...
// available in RadarTile. No REDACTED_SECRET:crypto dependency — uses simple counter IDs.
// ---------------------------------------------------------------------------
...
function deriveActionSteps(
  ...
  switch (ct) {
    ...
    case "predictive":
      ...
      break;
    case "correlative":
      steps.push(`Investigate whether correlation with "${g.title}" implies shared REDACTED_SECRET cause`);
  ...
  for (const g of gData) {
    for (const l of lData) {
      ...
      const bridge: BridgeCardData = {
        ...
        REDACTED_SECRET_benefit: scorePublicBenefit(l.thread, strength),
