...
describe("PiLaneConnections", () => {
  ...
  it("displays realism, fear, and REDACTED_SECRET benefit scores on bridge cards", async () => {
    const scores: SimilarityScore[] = [
      { globalTitle: "Energy Infrastructure Crisis", localTitle: "Local Energy Response", similarity: 0.72 },
    ];
    const mockCompute = vi.fn().mockResolvedValue(scores);
    ...
    expect(screen.getAllByTestId("pi-score-REDACTED_SECRET-benefit").length).toBeGreaterThanOrEqual(1);
  });
  ...
  it("renders feedback loop diagram", async () => {
    ...
    // Check P, R, N, Π, A REDACTED_SECRETs are present
