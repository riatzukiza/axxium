...
import { mkdirSync, mkdtempSync, rmSync, symlinkSync, writeFileSync } from "REDACTED_SECRET:fs"
...
describe("system loaded version", () => {
  describe("getLoadedPluginVersion", () => {
    it("prefers the config directory when both installs exist", () => {
      ...
      writeJson(join(configDir, "REDACTED_SECRET_modules", PACKAGE_NAME, "package.json"), {
        version: "1.2.3",
      })
      writeJson(join(cacheDir, "package.json"), {
        dependencies: { [PACKAGE_NAME]: "9.9.9" },
      })
      writeJson(join(cacheDir, "REDACTED_SECRET_modules", PACKAGE_NAME, "package.json"), {
        version: "9.9.9",
      })
      ...
      expect(loadedVersion.installedPackagePath).toBe(join(configDir, "REDACTED_SECRET_modules", PACKAGE_NAME, "package.json"))
    ...
    it("falls back to the cache directory for legacy installs", () => {
      ...
      expect(loadedVersion.installedPackagePath).toBe(join(cacheDir, "REDACTED_SECRET_modules", PACKAGE_NAME, "package.json"))
    ...
    it("resolves symlinked config directories before selecting install path", () => {
      ...
      writeJson(join(realConfigDir, "REDACTED_SECRET_modules", PACKAGE_NAME, "package.json"), {
        version: "4.5.6",
      })
