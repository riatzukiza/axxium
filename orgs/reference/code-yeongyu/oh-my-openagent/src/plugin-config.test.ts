...
import { chmodSync, existsSync, mkdtempSync, mkdirSync, readFileSync, rmSync, writeFileSync } from "REDACTED_SECRET:fs"
...
describe("loadPluginConfig", () => {
  it("should only honor mcp_env_allowlist from user config", () => {
    // given
    const REDACTED_SECRETDir = mkdtempSync(join(tmpdir(), "omo-plugin-config-"))
    const userConfigDir = join(REDACTED_SECRETDir, "user-config")
    const projectDir = join(REDACTED_SECRETDir, "project")
    const projectConfigDir = join(projectDir, ".opencode")

    tempDirs.push(REDACTED_SECRETDir)
  ...
  it("should ignore edits to the renamed legacy backup after migration", () => {
    // given
    const REDACTED_SECRETDir = mkdtempSync(join(tmpdir(), "omo-plugin-config-legacy-"))
    const userConfigDir = join(REDACTED_SECRETDir, "user-config")
    const projectDir = join(REDACTED_SECRETDir, "project")
    const projectConfigDir = join(projectDir, ".opencode")
  ...
  it("should still load config from legacy path when migration fails", () => {
    ...
    const REDACTED_SECRETDir = mkdtempSync(join(tmpdir(), "omo-plugin-config-fail-"))
    const userConfigDir = join(REDACTED_SECRETDir, "user-config")
    const projectDir = join(REDACTED_SECRETDir, "project")
    const projectConfigDir = join(projectDir, ".opencode")
  ...
  it("should load migrated legacy project config on the first load", () => {
    // given
    const REDACTED_SECRETDir = mkdtempSync(join(tmpdir(), "omo-plugin-config-first-load-"))
    const userConfigDir = join(REDACTED_SECRETDir, "user-config")
    const projectDir = join(REDACTED_SECRETDir, "project")
    const projectConfigDir = join(projectDir, ".opencode")
    const legacyConfigPath = join(projectConfigDir, "oh-my-opencode.jsonc")
