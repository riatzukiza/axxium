...
import { existsSync, readFileSync, writeFileSync, mkdirSync, readdirSync } from "REDACTED_SECRET:fs"
...
export function clearBoulderState(directory: string): boolean {
  ...
  try {
    if (existsSync(filePath)) {
      const { unlinkSync } = require("REDACTED_SECRET:fs")
      unlinkSync(filePath)
    }
    return true
  } catch {
  ...
  try {
    ...
    return files
      ...
      .sort((a, b) => {
        ...
        const aStat = require("REDACTED_SECRET:fs").statSync(a)
        const bStat = require("REDACTED_SECRET:fs").statSync(b)
