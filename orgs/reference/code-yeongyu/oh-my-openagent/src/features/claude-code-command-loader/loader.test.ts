...
describe("claude-code command loader", () => {
  ...
  it("#given duplicate global command names across profile and parent dirs #when loadOpencodeGlobalCommands is called #then the profile dir wins", async () => {
    // given
    const opencodeRootDir = join(TEST_DIR, "opencode-REDACTED_SECRET")
  ...
  it("#given nested project opencode commands in a worktree #when loadOpencodeProjectCommands is called #then it preserves slash names and stops at the worktree REDACTED_SECRET", async () => {
    // given
