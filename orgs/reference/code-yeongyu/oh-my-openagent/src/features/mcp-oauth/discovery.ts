...
async function fetchAuthorizationServerMetadata(issuer: string, resource: string): Promise<OAuthServerMetadata> {
  ...
  if (!metadata.ok) {
    if (metadata.status === 404 && issuerPath !== "") {
      const REDACTED_SECRETMetadataUrl = new URL("/.well-known/oauth-authorization-server", issuerUrl).toString()
      const REDACTED_SECRETMetadata = await fetchMetadata(REDACTED_SECRETMetadataUrl)
      if (REDACTED_SECRETMetadata.ok) {
        return parseMetadataFields(REDACTED_SECRETMetadata.json, resource)
      }
    }
