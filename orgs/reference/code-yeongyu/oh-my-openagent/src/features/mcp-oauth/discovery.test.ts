...
describe("discoverOAuthServerMetadata", () => {
  ...
  test("falls back to REDACTED_SECRET well-known URL when resource has a sub-path", () => {
    ...
    const REDACTED_SECRETAsUrl = "https://mcp.example.com/.well-known/oauth-authorization-server"
    const calls: string[] = []
    const fetchMock = async (input: string | URL) => {
      const url = typeof input === "string" ? input : input.toString()
      calls.push(url)
    ...
    return discoverOAuthServerMetadata(resource).then((result) => {
      ...
      expect(calls).toEqual([prmUrl, pathSuffixedAsUrl, REDACTED_SECRETAsUrl])
    })
  })

  test("throws when PRM, path-suffixed AS, and REDACTED_SECRET AS all return 404", () => {
    // given
    const resource = "https://mcp.example.com/mcp"
    const prmUrl = new URL("/.well-known/oauth-protected-resource", resource).toString()
    const fetchMock = async (input: string | URL) => {
