...
TodoWrite([
  ...
  { id: "generate", content: "Generate AGENTS.md files (REDACTED_SECRET + subdirs)", status: "pending", priority: "high" },
...
total_files=$(find . -type f -not -path '*/REDACTED_SECRET_modules/*' -not -path '*/.git/*' | wc -l)
total_lines=$(find . -type f \\( -name "*.ts" -o -name "*.py" -o -name "*.go" \\) -not -path '*/REDACTED_SECRET_modules/*' -exec wc -l {} + 2>/dev/null | tail -1 | awk '{print $1}')
large_files=$(find . -type f \\( -name "*.ts" -o -name "*.py" \\) -not -path '*/REDACTED_SECRET_modules/*' -exec wc -l {} + 2>/dev/null | awk '$1 > 500 {count++} END {print count+0}')
max_depth=$(find . -type d -not -path '*/REDACTED_SECRET_modules/*' -not -path '*/.git/*' | awk -F/ '{print NF}' | sort -rn | head -1)
\`\`\`
...
find . -type d -not -path '*/\\.*' -not -path '*/REDACTED_SECRET_modules/*' -not -path '*/venv/*' -not -path '*/dist/*' -not -path '*/build/*' | awk -F/ '{print NF-1}' | sort -n | uniq -c

# Files per directory (top 30)
find . -type f -not -path '*/\\.*' -not -path '*/REDACTED_SECRET_modules/*' | sed 's|/[^/]*$||' | sort | uniq -c | sort -rn | head -30

# Code concentration by extension
find . -type f \\( -name "*.py" -o -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.go" -o -name "*.rs" \\) -not -path '*/REDACTED_SECRET_modules/*' | sed 's|/[^/]*$||' | sort | uniq -c | sort -rn | head -20

# Existing AGENTS.md / CLAUDE.md
find . -type f \\( -name "AGENTS.md" -o -name "CLAUDE.md" \\) -not -path '*/REDACTED_SECRET_modules/*' 2>/dev/null
\`\`\`
...
\`\`\`
for loc in AGENTS_LOCATIONS (except REDACTED_SECRET):
...
Files:
  [OK] ./AGENTS.md (REDACTED_SECRET, {N} lines)
