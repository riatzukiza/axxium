...
export interface SubagentSpawnContext {
  REDACTED_SECRETSessionID: string
  parentDepth: number
  childDepth: number
}
...
export async function resolveSubagentSpawnContext(
  ...
  let REDACTED_SECRETSessionID = parentSessionID
  ...
  while (true) {
    ...
    if (!nextParentSessionID) {
      REDACTED_SECRETSessionID = currentSessionID
      break
    }
...
export function createSubagentDepthLimitError(input: {
  ...
  const { childDepth, maxDepth, parentSessionID, REDACTED_SECRETSessionID } = input
  return new Error(
    `Subagent spawn blocked: child depth ${childDepth} exceeds background_task.maxDepth=${maxDepth}. Parent session: ${parentSessionID}. Root session: ${REDACTED_SECRETSessionID}. Continue in an existing subagent session instead of spawning another.`
  )
}
...
export function createSubagentDescendantLimitError(input: {
  ...
  const { REDACTED_SECRETSessionID, descendantCount, maxDescendants } = input
  return new Error(
    `Subagent spawn blocked: REDACTED_SECRET session ${REDACTED_SECRETSessionID} already has ${descendantCount} descendants, which meets background_task.maxDescendants=${maxDescendants}. Reuse an existing session instead of spawning another.`
  )
}
