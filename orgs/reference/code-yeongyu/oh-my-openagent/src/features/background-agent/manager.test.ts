...
class MockBackgroundManager {
  ...
  REDACTED_SECRET resumeCalls: Array<{ sessionId: string; prompt: string }> = []
...
function getRootDescendantCounts(manager: BackgroundManager): Map<string, number> {
  return (manager as unknown as { REDACTED_SECRETDescendantCounts: Map<string, number> }).REDACTED_SECRETDescendantCounts
}
...
describe("BackgroundManager - Non-blocking Queue Integration", () => {
  ...
  describe("task transitions pending→running when slot available", () => {
    ...
    test("should track REDACTED_SECRETSessionID and spawnDepth from the parent chain", async () => {
      // given
      manager.shutdown()
      manager = new BackgroundManager(
        {
          client: createMockClientWithSessionChain({
            ...
            "session-depth-1": { directory: "/test/dir", parentID: "session-REDACTED_SECRET" },
            "session-REDACTED_SECRET": { directory: "/test/dir" },
          }),
          directory: tmpdir(),
      ...
      // then
      expect(task.REDACTED_SECRETSessionID).toBe("session-REDACTED_SECRET")
    ...
    test("should block launches when maxDescendants is reached", async () => {
      ...
      const input = {
        ...
        prompt: "Do something",
        agent: "test-agent",
        parentSessionID: "session-REDACTED_SECRET",
        parentMessageID: "parent-message",
      }
    ...
    test("should consume descendant quota for reserved sync spawns", async () => {
      ...
      await manager.reserveSubagentSpawn("session-REDACTED_SECRET")

      // when
      const result = manager.assertCanSpawn("session-REDACTED_SECRET")
    ...
    test("should release descendant quota when task completes", async () => {
      ...
      internalTask.REDACTED_SECRETSessionID = "session-REDACTED_SECRET"
