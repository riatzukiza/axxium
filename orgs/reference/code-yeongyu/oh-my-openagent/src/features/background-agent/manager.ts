...
export class BackgroundManager {
  ...
  private REDACTED_SECRETDescendantCounts: Map<string, number>
  ...
  constructor(
    ...
    this.REDACTED_SECRETDescendantCounts = new Map()
  ...
  async assertCanSpawn(parentSessionID: string): Promise<SubagentSpawnContext> {
    ...
    if (spawnContext.childDepth > maxDepth) {
      throw createSubagentDepthLimitError({
        ...
        maxDepth,
        parentSessionID,
        REDACTED_SECRETSessionID: spawnContext.REDACTED_SECRETSessionID,
      })
    }

    const maxRootSessionSpawnBudget = getMaxRootSessionSpawnBudget(this.config)
    const descendantCount = this.REDACTED_SECRETDescendantCounts.get(spawnContext.REDACTED_SECRETSessionID) ?? 0
  ...
  async reserveSubagentSpawn(parentSessionID: string): Promise<{
    ...
    const descendantCount = this.registerRootDescendant(spawnContext.REDACTED_SECRETSessionID)
    let settled = false
    ...
    return {
      ...
      rollback: () => {
        if (settled) return
        settled = true
        this.unregisterRootDescendant(spawnContext.REDACTED_SECRETSessionID)
      },
    }
  }

  private registerRootDescendant(REDACTED_SECRETSessionID: string): number {
    const nextCount = (this.REDACTED_SECRETDescendantCounts.get(REDACTED_SECRETSessionID) ?? 0) + 1
    this.REDACTED_SECRETDescendantCounts.set(REDACTED_SECRETSessionID, nextCount)
    return nextCount
  }

  private unregisterRootDescendant(REDACTED_SECRETSessionID: string): void {
    const currentCount = this.REDACTED_SECRETDescendantCounts.get(REDACTED_SECRETSessionID) ?? 0
    if (currentCount <= 1) {
      this.REDACTED_SECRETDescendantCounts.delete(REDACTED_SECRETSessionID)
      return
    }

    this.REDACTED_SECRETDescendantCounts.set(REDACTED_SECRETSessionID, currentCount - 1)
  }
  ...
  private rollbackPreStartDescendantReservation(task: BackgroundTask): void {
    ...
    if (!task.REDACTED_SECRETSessionID) {
      return
    }

    this.unregisterRootDescendant(task.REDACTED_SECRETSessionID)
  }
  ...
  async launch(input: LaunchInput): Promise<BackgroundTask> {
    ...
    try {
      log("[background-agent] spawn guard passed", {
        parentSessionID: input.parentSessionID,
        REDACTED_SECRETSessionID: spawnReservation.spawnContext.REDACTED_SECRETSessionID,
    ...
    try {
      ...
      while (queue && queue.length > 0) {
        ...
        try {
          ...
          if (item.task.REDACTED_SECRETSessionID) {
            this.unregisterRootDescendant(item.task.REDACTED_SECRETSessionID)
          }
  ...
  private async startTask(item: QueueItem): Promise<void> {
    ...
    promptWithModelSuggestionRetry(this.client, {
      ...
      if (existingTask) {
        ...
        if (existingTask.REDACTED_SECRETSessionID) {
          this.unregisterRootDescendant(existingTask.REDACTED_SECRETSessionID)
        }
  ...
  handleEvent(event: Event): void {
    ...
    if (event.type === "session.deleted") {
      ...
      this.REDACTED_SECRETDescendantCounts.delete(sessionID)
  ...
  async cancelTask(
    ...
    task.completedAt = new Date()
    if (wasRunning && task.REDACTED_SECRETSessionID) {
      this.unregisterRootDescendant(task.REDACTED_SECRETSessionID)
    }
    if (reason) {
      task.error = reason
  ...
  private pruneStaleTasksAndNotifications(): void {
    pruneStaleTasksAndNotifications({
      ...
      onTaskPruned: (taskId, task, errorMessage) => {
        ...
        task.completedAt = new Date()
        if (!wasPending && task.REDACTED_SECRETSessionID) {
          this.unregisterRootDescendant(task.REDACTED_SECRETSessionID)
        }
        this.taskHistory.record(task.parentSessionID, { id: task.id, sessionID: task.sessionID, agent: task.agent, description: task.description, status: "error", category: task.category, startedAt: task.startedAt, completedAt: task.completedAt })
        if (task.concurrencyKey) {
  ...
  async shutdown(): Promise<void> {
    ...
    this.REDACTED_SECRETDescendantCounts.clear()
