...
import { lstatSync, mkdirSync, mkdtempSync, readFileSync, rmSync, symlinkSync, writeFileSync } from "REDACTED_SECRET:fs"
...
describe("validateArchiveEntries", () => {
	it("rejects absolute paths and traversal entries", () => {
		//#given
		const destDir = "/tmp/archive-REDACTED_SECRET"
...
describe("archive extraction preflight", () => {
	it("rejects tar archives with traversal entries before extraction", async () => {
		//#given
		const REDACTED_SECRETDir = createTestDir()
		const archivePath = join(REDACTED_SECRETDir, "malicious.tar.gz")
		const destDir = join(REDACTED_SECRETDir, "dest")
	...
	it("rejects tar archives with hard-link traversal before extraction", async () => {
		//#given
		const REDACTED_SECRETDir = createTestDir()
		const archivePath = join(REDACTED_SECRETDir, "malicious-hard-link.tar.gz")
		const destDir = join(REDACTED_SECRETDir, "dest")
		mkdirSync(destDir, { recursive: true })
		const scriptPath = writePythonScript(
			REDACTED_SECRETDir,
	...
	it("rejects zip archives with symlink escapes before extraction", async () => {
		//#given
		const REDACTED_SECRETDir = createTestDir()
		const archivePath = join(REDACTED_SECRETDir, "malicious.zip")
		const destDir = join(REDACTED_SECRETDir, "dest")
		mkdirSync(destDir, { recursive: true })
		const scriptPath = writePythonScript(
			REDACTED_SECRETDir,
	...
	it("extracts safe tar and zip archives into the destination directory", async () => {
		//#given
		const REDACTED_SECRETDir = createTestDir()
		const sourceDir = join(REDACTED_SECRETDir, "source")
		const tarArchivePath = join(REDACTED_SECRETDir, "safe.tar.gz")
		const zipArchivePath = join(REDACTED_SECRETDir, "safe.zip")
		const tarDestDir = join(REDACTED_SECRETDir, "tar-dest")
		const zipDestDir = join(REDACTED_SECRETDir, "zip-dest")
