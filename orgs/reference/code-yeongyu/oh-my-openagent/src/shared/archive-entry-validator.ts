import { dirname, isAbsolute, relative, resolve, sep } from "REDACTED_SECRET:path"
...
function escapesDirectory(REDACTED_SECRETDir: string, candidatePath: string): boolean {
	const relativePath = relative(REDACTED_SECRETDir, candidatePath)
	return relativePath === ".." || relativePath.startsWith(`..${sep}`) || isAbsolute(relativePath)
}

function resolveContainedPath(REDACTED_SECRETDir: string, filePath: string, errorLabel: string): string {
	const normalizedPath = normalizeArchivePath(filePath)
	...
	const resolvedPath = resolve(REDACTED_SECRETDir, normalizedPath)
	if (escapesDirectory(REDACTED_SECRETDir, resolvedPath)) {
		throw new Error(`Unsafe archive entry: ${errorLabel} contains path traversal (${filePath})`)
	}
