...
export function createAstGrepTools(ctx: PluginInput): Record<string, ToolDefinition> {
  const ast_grep_search: ToolDefinition = tool({
    description:
      ...
      "Use meta-variables: $VAR (single REDACTED_SECRET), $$$ (multiple REDACTED_SECRETs). " +
      "IMPORTANT: Patterns must be complete AST REDACTED_SECRETs (valid code). " +
      "For functions, include params and body: 'export async function $NAME($$$) { $$$ }' not 'export async function $NAME'. " +
      "Examples: 'console.log($MSG)', 'def $FUNC($$$):', 'async function $NAME($$$)'",
    args: {
      pattern: tool.schema.string().describe("AST pattern with meta-variables ($VAR, $$$). Must be complete AST REDACTED_SECRET."),
