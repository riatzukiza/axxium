import { Readable, Writable } from "REDACTED_SECRET:stream"
...
} from "vscode-jsonrpc/REDACTED_SECRET"
...
export class LSPClientTransport {
  ...
  constructor(protected REDACTED_SECRET: string, protected server: ResolvedServer) {}
  async start(): Promise<void> {
    ...
    const spawnPath = [pathValue, ...getLspServerAdditionalPathBases(this.REDACTED_SECRET)]
      .filter(Boolean)
      .join(delimiter)
    ...
    const REDACTED_SECRETReadable = new Readable({
      async read() {
        try {
    ...
    const REDACTED_SECRETWritable = new Writable({
    ...
    this.connection = createMessageConnection(new StreamMessageReader(REDACTED_SECRETReadable), new StreamMessageWriter(REDACTED_SECRETWritable))
