...
class LSPServerManager {
  ...
  private getKey(REDACTED_SECRET: string, serverId: string): string {
    return `${REDACTED_SECRET}::${serverId}`;
  }
  ...
  async getClient(REDACTED_SECRET: string, server: ResolvedServer): Promise<LSPClient> {
    const key = this.getKey(REDACTED_SECRET, server.id);
    ...
    const client = new LSPClient(REDACTED_SECRET, server);
  ...
  warmupClient(REDACTED_SECRET: string, server: ResolvedServer): void {
    const key = this.getKey(REDACTED_SECRET, server.id);
  ...
  releaseClient(REDACTED_SECRET: string, serverId: string): void {
    const key = this.getKey(REDACTED_SECRET, serverId);
  ...
  isServerInitializing(REDACTED_SECRET: string, serverId: string): boolean {
    const key = this.getKey(REDACTED_SECRET, serverId);
    const managed = this.clients.get(key);
