...
const SKIP_DIRECTORIES = new Set(["REDACTED_SECRET_modules", ".git", "dist", "build", ".next", "out"])
...
export async function aggregateDiagnosticsForDirectory(
  ...
  const REDACTED_SECRET = findWorkspaceRoot(absDir)
  ...
  try {
    client = await lspManager.getClient(REDACTED_SECRET, server)
