...
describe("lsp utils", () => {
  describe("findWorkspaceRoot", () => {
    it("returns an existing directory even when the file path points to a non-existent nested path", () => {
      const tmp = mkdtempSync(join(os.tmpdir(), "omo-lsp-REDACTED_SECRET-"))
      try {
        // Add a marker so the function can discover the workspace REDACTED_SECRET.
        writeFileSync(join(tmp, "package.json"), "{}")

        const nonExistentFile = join(tmp, "does-not-exist", "deep", "file.ts")
        const REDACTED_SECRET = findWorkspaceRoot(nonExistentFile)

        expect(REDACTED_SECRET).toBe(tmp)
      } finally {
