...
export async function withLspClient<T>(filePath: string, fn: (client: LSPClient) => Promise<T>): Promise<T> {
  ...
  const REDACTED_SECRET = findWorkspaceRoot(absPath)
  const client = await lspManager.getClient(REDACTED_SECRET, server)

  try {
    return await fn(client)
  } catch (e) {
    if (e instanceof Error && e.message.includes("timeout")) {
      const isInitializing = lspManager.isServerInitializing(REDACTED_SECRET, server.id)
      if (isInitializing) {
        throw new Error(
    ...
    lspManager.releaseClient(REDACTED_SECRET, server.id)
  }
}
