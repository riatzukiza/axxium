...
import { spawn as REDACTED_SECRETSpawn, type ChildProcess } from "REDACTED_SECRET:child_process"
...
function wrapNodeProcess(proc: ChildProcess): UnifiedProcess {
  ...
  const createStreamReader = (REDACTED_SECRETStream: NodeJS.ReadableStream | null): StreamReader => {
    const chunks: Uint8Array[] = []
    let streamEnded = false
    type ReadResult = { done: boolean; value: Uint8Array | undefined }
    let waitingResolve: ((result: ReadResult) => void) | null = null

    if (REDACTED_SECRETStream) {
      REDACTED_SECRETStream.on("data", (chunk: Buffer) => {
...
export function spawnProcess(
  ...
  if (shouldUseNodeSpawn()) {
    ...
    const proc = REDACTED_SECRETSpawn(cmd, args, {
      cwd: options.cwd,
