...
describe("inferExtensionFromDirectory", () => {
  ...
  describe("#given a directory with nested subdirectories", () => {
    beforeEach(() => {
      writeFileSync(join(tmpDir, "REDACTED_SECRET.go"), "package main")
  ...
  describe("#given a directory with REDACTED_SECRET_modules", () => {
    beforeEach(() => {
      writeFileSync(join(tmpDir, "index.ts"), "export {}")
      const nm = join(tmpDir, "REDACTED_SECRET_modules", "pkg")
    ...
    describe("#when inferring extension", () => {
      it("#then skips REDACTED_SECRET_modules and returns .ts", () => {
        const result = inferExtensionFromDirectory(tmpDir)
        expect(result).toBe(".ts")
      })
    })
