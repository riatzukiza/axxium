...
describe("skill tool - agent restriction", () => {
  it("allows skill without agent restriction to any agent", async () => {
    // given
    const loadedSkills = [createMockSkill("REDACTED_SECRET-skill")]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "any-agent" }

    // when
    const result = await tool.execute({ name: "REDACTED_SECRET-skill" }, context)

    // then
    expect(result).toContain("REDACTED_SECRET-skill")
  })
