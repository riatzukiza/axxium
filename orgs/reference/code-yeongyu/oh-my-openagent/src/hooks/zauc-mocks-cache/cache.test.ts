...
import { existsSync, mkdirSync, readFileSync, rmSync, writeFileSync } from "REDACTED_SECRET:fs"
...
async function importFreshCacheModule(): Promise<typeof import("../auto-update-checker/cache")> {
  mock.module("../auto-update-checker/constants", () => ({
    ...
    INSTALLED_PACKAGE_JSON: join(TEST_OPENCODE_CACHE_DIR, "REDACTED_SECRET_modules", "oh-my-opencode", "package.json"),
...
function resetTestCache(): void {
  ...
  mkdirSync(join(TEST_OPENCODE_CACHE_DIR, "REDACTED_SECRET_modules", "oh-my-opencode"), { recursive: true })
  writeFileSync(
  ...
  writeFileSync(
    join(TEST_OPENCODE_CACHE_DIR, "REDACTED_SECRET_modules", "oh-my-opencode", "package.json"),
    '{"name":"oh-my-opencode"}'
  )
}
...
describe("invalidatePackage", () => {
  ...
  it("invalidates the installed package from the OpenCode cache directory", async () => {
    ...
    expect(existsSync(join(TEST_OPENCODE_CACHE_DIR, "REDACTED_SECRET_modules", "oh-my-opencode"))).toBe(false)
