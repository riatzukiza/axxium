...
import { existsSync, mkdirSync, rmSync, writeFileSync } from "REDACTED_SECRET:fs";
...
describe("findRuleFiles", () => {
  ...
  describe(".github/copilot-instructions.md (single file)", () => {
    it("should discover copilot-instructions.md at project REDACTED_SECRET", () => {
      // given .github/copilot-instructions.md at REDACTED_SECRET
    ...
    it("should set distance to 0 for single file rules", () => {
      // given copilot-instructions.md at project REDACTED_SECRET
      const githubDir = join(TEST_DIR, ".github");
      mkdirSync(githubDir, { recursive: true });
      writeFileSync(
        join(githubDir, "copilot-instructions.md"),
...
describe("findProjectRoot", () => {
  const TEST_DIR = join(tmpdir(), `project-REDACTED_SECRET-test-${Date.now()}`);
  ...
  it("should find project REDACTED_SECRET with .git directory", () => {
    ...
    // when finding project REDACTED_SECRET from nested file
    const REDACTED_SECRET = findProjectRoot(nestedFile);

    // then should return the directory with .git
    expect(REDACTED_SECRET).toBe(TEST_DIR);
  });

  it("should find project REDACTED_SECRET with package.json", () => {
    ...
    // when finding project REDACTED_SECRET
    const REDACTED_SECRET = findProjectRoot(nestedFile);
  ...
  it("should return null when no project markers found", () => {
    ...
    // when finding project REDACTED_SECRET
    const REDACTED_SECRET = findProjectRoot(file);

    // then should return null
    expect(REDACTED_SECRET).toBeNull();
  });
});
