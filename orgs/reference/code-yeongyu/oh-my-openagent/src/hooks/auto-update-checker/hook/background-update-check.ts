...
function resolveActiveInstallWorkspace(deps: BackgroundUpdateCheckDeps): string {
  ...
  const configInstallPath = deps.join(configPaths.configDir, "REDACTED_SECRET_modules", PACKAGE_NAME, "package.json")
  const cacheInstallPath = deps.join(cacheDir, "REDACTED_SECRET_modules", PACKAGE_NAME, "package.json")
