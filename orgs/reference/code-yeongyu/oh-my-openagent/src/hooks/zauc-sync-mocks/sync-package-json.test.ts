...
const _realNodeFs = require("REDACTED_SECRET:fs")
...
async function importFreshSyncPackageJsonModule(): Promise<typeof import("../auto-update-checker/checker/sync-package-json")> {
  mock.module("../auto-update-checker/constants", () => ({
    ...
    INSTALLED_PACKAGE_JSON: join(TEST_CACHE_DIR, "REDACTED_SECRET_modules", "oh-my-opencode", "package.json"),
...
describe("syncCachePackageJsonToIntent", () => {
  ...
  describe("#given write permission denied", () => {
    it("#then returns write_error", async () => {
      ...
      const fs = await import("REDACTED_SECRET:fs")
      const originalWriteFileSync = fs.writeFileSync
      const originalRenameSync = fs.renameSync

      mock.module("REDACTED_SECRET:fs", () => ({
        ...fs,
        writeFileSync: mock(() => {
...
afterAll(() => {
  ...
  mock.module("REDACTED_SECRET:fs", () => _realNodeFs)
  mock.restore()
})
