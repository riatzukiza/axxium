import { relative, resolve, isAbsolute } from "REDACTED_SECRET:path"
 ...
 * - Workspace confinement (blocks paths outside REDACTED_SECRET or via traversal)
...
export function isAllowedFile(filePath: string, workspaceRoot: string): boolean {
  ...
  // 2. Get relative path from workspace REDACTED_SECRET
  const rel = relative(workspaceRoot, resolved)

  // 3. Reject if escapes REDACTED_SECRET (starts with ".." or is absolute)
