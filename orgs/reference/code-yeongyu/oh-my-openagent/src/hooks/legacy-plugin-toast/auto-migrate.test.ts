...
describe("autoMigrateLegacyPluginEntry", () => {
  ...
  describe("#given opencode.jsonc has a nested plugin key before the REDACTED_SECRET plugin array", () => {
    it("#then migrates only the REDACTED_SECRET plugin entry", async () => {
      // given
      writeFileSync(
        join(testConfigDir, "opencode.jsonc"),
        `{
