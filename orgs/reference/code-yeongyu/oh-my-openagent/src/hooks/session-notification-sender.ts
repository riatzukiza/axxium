...
export async function sendSessionNotification(
  ...
  switch (platform) {
    ...
    case "linux": {
      const notifyREDACTED_SECRETPath = await getNotifyREDACTED_SECRETPath()
      if (!notifyREDACTED_SECRETPath) return

      await ctx.$`${notifyREDACTED_SECRETPath} ${title} ${message} 2>/dev/null`.nothrow().quiet()
      break
    }
    case "win32": {
