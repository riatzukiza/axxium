...
describe("processFilePathForReadmeInjection", () => {
  ...
  it("includes REDACTED_SECRET-level README.md (unlike agents-injector)", async () => {
    // given
    writeFileSync(join(testRoot, "README.md"), "# Root README\nREDACTED_SECRET context")
    ...
    await processFilePathForReadmeInjection({
      ...
      sessionID: "session-REDACTED_SECRET",
      output,
    })
    ...
    expect(output.output).toContain("REDACTED_SECRET context")
  })
