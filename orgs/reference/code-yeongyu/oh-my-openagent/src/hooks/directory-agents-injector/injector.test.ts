...
import { mkdirSync, rmSync, writeFileSync } from "REDACTED_SECRET:fs"
...
describe("processFilePathForAgentsInjection", () => {
  ...
  const REDACTED_SECRETAgentsContent = "# ROOT AGENTS\nREDACTED_SECRET-level directives"
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(testRoot, "AGENTS.md"), REDACTED_SECRETAgentsContent)
    ...
    writeFileSync(join(testRoot, "file.ts"), "export const REDACTED_SECRETFile = true\n")
  })
  ...
  it("skips REDACTED_SECRET-level AGENTS.md", async () => {
    // given
    ...
    await processFilePathForAgentsInjection({
      ...
      sessionID: "session-REDACTED_SECRET-skip",
      output,
    })

    // then
    expect(output.output).not.toContain(REDACTED_SECRETAgentsContent)
