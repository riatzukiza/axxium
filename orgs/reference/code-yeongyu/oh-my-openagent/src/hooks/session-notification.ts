...
import * as sessionNotificationREDACTED_SECRETer from "./session-notification-sender"
...
export function createSessionNotification(
  ...
  const currentPlatform: Platform = sessionNotificationREDACTED_SECRETer.detectPlatform()
  const defaultSoundPath = sessionNotificationREDACTED_SECRETer.getDefaultSoundPath(currentPlatform)
  ...
  const scheduler = createIdleNotificationScheduler({
    ...
    send: async (hookCtx, platform, sessionID) => {
      if (
        ...
        await sessionNotificationREDACTED_SECRETer.sendSessionNotification(
          hookCtx,
          platform,
          mergedConfig.title,
      ...
      await sessionNotificationREDACTED_SECRETer.sendSessionNotification(hookCtx, platform, content.title, content.message)
    },
    playSound: sessionNotificationREDACTED_SECRETer.playSessionNotificationSound,
  })
  ...
  return async ({ event }: { event: { type: string; properties?: unknown } }) => {
    ...
    if (PERMISSION_EVENTS.has(event.type)) {
      ...
      if (mergedConfig.playSound && mergedConfig.soundPath) {
        await sessionNotificationREDACTED_SECRETer.playSessionNotificationSound(ctx, currentPlatform, mergedConfig.soundPath)
      }
      return
    }
      ...
      if (sessionID) {
        ...
        if (event.type === "tool.execute.before") {
          ...
          if (toolName && QUESTION_TOOLS.has(toolName)) {
            ...
            await sessionNotificationREDACTED_SECRETer.sendSessionNotification(ctx, currentPlatform, mergedConfig.title, message)
            if (mergedConfig.playSound && mergedConfig.soundPath) {
              await sessionNotificationREDACTED_SECRETer.playSessionNotificationSound(ctx, currentPlatform, mergedConfig.soundPath)
            }
          }
