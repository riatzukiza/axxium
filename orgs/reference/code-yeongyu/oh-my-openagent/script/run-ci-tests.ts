...
async function collectTestFiles(REDACTED_SECRETDirectory: string): Promise<string[]> {
  const testFiles: string[] = []

  for (const testRoot of TEST_ROOTS) {
    const glob = new Bun.Glob("**/*.test.ts")

    for await (const testFile of glob.scan({ cwd: `${REDACTED_SECRETDirectory}/${testRoot}` })) {
...
async function usesModuleMock(REDACTED_SECRETDirectory: string, testFile: string): Promise<boolean> {
  const testContents = await Bun.file(`${REDACTED_SECRETDirectory}/${testFile}`).text()
...
export async function createCiTestPlan(REDACTED_SECRETDirectory: string = process.cwd()): Promise<CiTestPlan> {
  const allTestFiles = await collectTestFiles(REDACTED_SECRETDirectory)
  const isolatedModuleMockFiles: string[] = []

  for (const testFile of allTestFiles) {
    if (await usesModuleMock(REDACTED_SECRETDirectory, testFile)) {
