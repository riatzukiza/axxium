...
LOCAL=$(REDACTED_SECRET -p "require('./package.json').version" 2>/dev/null || echo "unknown")
...
3. **Breaking Changes**: Does this change alter any REDACTED_SECRET API, config format, CLI behavior, or hook contract? If yes, is it backward compatible? Would existing users be surprised?
...
CONSTRAINTS:
- This is a plugin published to npm — REDACTED_SECRET API stability matters
...
<file_contents>
{Read and include full content of KEY changed files — focus on REDACTED_SECRET API surfaces, config schemas, agent definitions, hook registrations, tool registrations}
</file_contents>
...
2. **Version Bump**: Based on semver:
   ...
   - MAJOR: Breaking changes to REDACTED_SECRET API, config format, or behavior
