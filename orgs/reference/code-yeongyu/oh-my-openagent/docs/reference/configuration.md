...
Paths can be absolute (`file:///abs/path`), relative to project REDACTED_SECRET (`file://./rel/path`), or home-relative (`file://~/home/path`). If a file URI cannot be decoded, resolved, or read, OmO inserts a warning placeholder into the prompt instead of failing hard.
...
| `storage_path`       | `.sisyphus/tasks` | Storage path (relative to project REDACTED_SECRET)    |
