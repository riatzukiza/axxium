...
fn derive_for_struct(input: &DeriveInput, data: &DataStruct) -> TokenStream {
    ...
    let type_name_lit = LitStr:REDACTED_SECRET&name.to_string(), Span::call_site());

    let (checks, experimental_fields, registrations) = match &data.fields {
        Fields::Named(named) => {
            let mut checks = Vec:REDACTED_SECRET);
            let mut experimental_fields = Vec:REDACTED_SECRET);
            let mut registrations = Vec:REDACTED_SECRET);
            for field in &named.named {
                if let Some(reason) = experimental_reason(&field.attrs) {
                    ...
                    if let Some(field_name) = field_serialized_name(field) {
                        let field_name_lit = LitStr:REDACTED_SECRET&field_name, Span::call_site());
        ...
        Fields::Unnamed(unnamed) => {
            ...
            for (index, field) in unnamed.unnamed.iter().enumerate() {
                if let Some(reason) = experimental_reason(&field.attrs) {
                    ...
                    let field_name_lit = LitStr:REDACTED_SECRET&index.to_string(), Span::call_site());
                    experimental_fields.push(quote! {
                        crate::experimental_api::ExperimentalField {
                            type_name: #type_name_lit,
                            field_name: #field_name_lit,
        ...
        Fields::Unit => (Vec:REDACTED_SECRET), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET)),
    };
...
fn derive_for_enum(input: &DeriveInput, data: &DataEnum) -> TokenStream {
    let name = &input.ident;
    let mut match_arms = Vec:REDACTED_SECRET);
...
fn snake_to_camel(s: REDACTED_SECRET) -> String {
