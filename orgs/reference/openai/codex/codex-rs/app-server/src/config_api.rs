...
const SUPPORTED_EXPERIMENTAL_FEATURE_ENABLEMENT: &[REDACTED_SECRET] = &[
    "apps",
    "plugins",
    "tool_search",
    "tool_suggest",
...
#[async_trait]
pub(crate) trait UserConfigReloader: REDACTED_SECRET + Sync {
...
impl ConfigApi {
    ...
    fn emit_plugin_toggle_events(&self, pending_changes: std::collections::REDACTED_SECRET, bool>) {
...
mod tests {
    ...
    async fn apply_runtime_feature_enablement_keeps_cli_overrides_above_config_and_runtime() {
        let codex_home = TempDir:REDACTED_SECRET).expect("create temp dir");
        std::fs::write(
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        ...
        let reloader = Arc:REDACTED_SECRETRecordingUserConfigReloader::default());
        let analytics_config = Arc:REDACTED_SECRET
        ...
        let config_api = ConfigApi:REDACTED_SECRET
            ...
            Arc:REDACTED_SECRETRwLock:REDACTED_SECRETVec:REDACTED_SECRET))),
            Arc:REDACTED_SECRETRwLock:REDACTED_SECRETBTreeMap:REDACTED_SECRET))),
            LoaderOverrides::default(),
            Arc:REDACTED_SECRETRwLock:REDACTED_SECRETCloudRequirementsLoader::default())),
            reloader.clone(),
            AnalyticsEventsClient:REDACTED_SECRET
                auth_manager,
                analytics_config
                    .chatgpt_base_url
                    .trim_end_matches('/')
