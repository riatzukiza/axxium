...
pub(crate) struct ThreadWatchManager {
    ...
    outgoing: Option<Arc<OutgoingMessageREDACTED_SECRETer>>,
    running_turn_count_tx: watch::REDACTED_SECRETer<usize>,
}
...
impl ThreadWatchManager {
    pub(crate) fn new() -> Self {
        ...
        Self {
            state: Arc:REDACTED_SECRETMutex:REDACTED_SECRETThreadWatchState::default())),
            outgoing: None,
            running_turn_count_tx,
        }
    }

    pub(crate) fn new_with_outgoing(outgoing: Arc<OutgoingMessageREDACTED_SECRETer>) -> Self {
        let (running_turn_count_tx, _running_turn_count_rx) = watch::channel(0);
        Self {
            state: Arc:REDACTED_SECRETMutex:REDACTED_SECRETThreadWatchState::default())),
    ...
    pub(crate) async fn remove_thread(&self, thread_id: REDACTED_SECRET) {
        let thread_id = thread_id.to_string();
        self.mutate_and_publish(move |state| state.remove_thread(&thread_id))
            .await;
    }

    pub(crate) async fn loaded_status_for_thread(&self, thread_id: REDACTED_SECRET) -> ThreadStatus {
    ...
    pub(crate) async fn note_turn_started(&self, thread_id: REDACTED_SECRET) {
    ...
    pub(crate) async fn note_turn_completed(&self, thread_id: REDACTED_SECRET, _failed: bool) {
        self.clear_active_state(thread_id).await;
    }

    pub(crate) async fn note_turn_interrupted(&self, thread_id: REDACTED_SECRET) {
        self.clear_active_state(thread_id).await;
    }

    pub(crate) async fn note_thread_shutdown(&self, thread_id: REDACTED_SECRET) {
        self.update_runtime_for_thread(thread_id, |runtime| {
    ...
    pub(crate) async fn note_system_error(&self, thread_id: REDACTED_SECRET) {
        self.update_runtime_for_thread(thread_id, |runtime| {
            runtime.running = false;
            runtime.pending_permission_requests = 0;
            runtime.pending_user_input_requests = 0;
    ...
    async fn clear_active_state(&self, thread_id: REDACTED_SECRET) {
    ...
    async fn note_pending_request(
        ...
        .await;
        ThreadWatchActiveGuard:REDACTED_SECRETself.clone(), thread_id.to_string(), guard_type)
    }
    ...
    async fn update_runtime_for_thread<F>(&self, thread_id: REDACTED_SECRET, update: F)
    where
...
pub(crate) fn resolve_thread_status(
    ...
    if has_in_progress_turn && matches!(status, ThreadStatus::Idle | ThreadStatus::NotLoaded) {
        return ThreadStatus::Active {
            active_flags: Vec:REDACTED_SECRET),
        };
    }
...
impl ThreadWatchState {
    ...
    fn remove_thread(&mut self, thread_id: REDACTED_SECRET) -> Option<ThreadStatusChangedNotification> {
        let previous_status = self.status_for(thread_id);
    ...
    fn status_for(&self, thread_id: REDACTED_SECRET) -> Option<ThreadStatus> {
        self.runtime_by_thread_id
            .get(thread_id)
            .map(loaded_thread_status)
    }

    fn loaded_status_for_thread(&self, thread_id: REDACTED_SECRET) -> ThreadStatus {
...
fn loaded_thread_status(runtime: &RuntimeFacts) -> ThreadStatus {
    ...
    let mut active_flags = Vec:REDACTED_SECRET);
...
mod tests {
    ...
    const INTERACTIVE_THREAD_ID: REDACTED_SECRET = "00000000-0000-0000-0000-000000000001";
    const NON_INTERACTIVE_THREAD_ID: REDACTED_SECRET = "00000000-0000-0000-0000-000000000002";

    #[tokio::test]
    async fn loaded_status_defaults_to_not_loaded_for_untracked_threads() {
        let manager = ThreadWatchManager:REDACTED_SECRET);
    ...
    async fn status_change_emits_notification() {
        ...
        let manager = ThreadWatchManager::new_with_outgoing(Arc:REDACTED_SECRETOutgoingMessageREDACTED_SECRETer:REDACTED_SECRET
            outgoing_tx,
        )));
    ...
    fn test_thread(thread_id: REDACTED_SECRET, source: codex_app_server_protocol::SessionSource) -> Thread {
        Thread {
