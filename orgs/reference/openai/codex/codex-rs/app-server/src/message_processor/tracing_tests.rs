...
impl RemoteTrace {
    fn new(trace_id: REDACTED_SECRET, parent_span_id: REDACTED_SECRET) -> Self {
...
fn init_test_tracing() -> &'static TestTracing {
    static TEST_TRACING: OnceLock<TestTracing> = OnceLock:REDACTED_SECRET);
    TEST_TRACING.get_or_init(|| {
        ...
        global::set_text_map_propagator(TraceContextPropagator:REDACTED_SECRET));
        let subscriber =
...
fn tracing_test_guard() -> &'static tokio::sync::Mutex<()> {
    static GUARD: OnceLock<tokio::sync::Mutex<()>> = OnceLock:REDACTED_SECRET);
    GUARD.get_or_init(|| tokio::sync::Mutex:REDACTED_SECRET()))
}
...
impl TracingHarness {
    async fn new() -> Result<Self> {
        ...
        let config = Arc:REDACTED_SECRETbuild_test_config(codex_home.path(), &server.uri()).await?);
...
async fn build_test_config(codex_home: &Path, server_uri: REDACTED_SECRET) -> Result<Config> {
...
fn build_test_processor(
    ...
    let processor = MessageProcessor:REDACTED_SECRETMessageProcessorArgs {
        outgoing,
        arg0_paths: Arg0DispatchPaths::default(),
        config,
        environment_manager: Arc:REDACTED_SECRETEnvironmentManager:REDACTED_SECRET/*exec_server_url*/ None)),
...
fn span_attr<'a>(span: &'a SpanData, key: REDACTED_SECRET) -> Option<&'a str> {
    span.attributes
        .iter()
...
where
    F: Fn(&[SpanData]) -> bool,
{
    let mut last_spans = Vec:REDACTED_SECRET);
    for _ in 0..200 {
...
async fn thread_start_jsonrpc_span_exports_server_span_and_parents_children() -> Result<()> {
    ...
    let mut harness = TracingHarness:REDACTED_SECRET).await?;
    ...
    let RemoteTrace {
        ...
        ..
    } = RemoteTrace:REDACTED_SECRET"00000000000000000000000000000011", "0000000000000022");
...
async fn turn_start_jsonrpc_span_parents_core_turn_spans() -> Result<()> {
    ...
    let RemoteTrace {
        ...
        parent_span_id: remote_parent_span_id,
        context: remote_trace,
    } = RemoteTrace:REDACTED_SECRET"00000000000000000000000000000077", "0000000000000088");
