...
impl MessageProcessor {
    ...
    /// `REDACTED_SECRETer` so handlers can enqueue messages to be written to stdout.
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let auth_manager = AuthManager::shared_with_external_auth(
            ...
            Arc:REDACTED_SECRETExternalAuthRefreshBridge {
                outgoing: outgoing.clone(),
            }),
        );
        let thread_manager = Arc:REDACTED_SECRETThreadManager:REDACTED_SECRET
            config.as_ref(),
            auth_manager.clone(),
            session_source,
        ...
        let analytics_events_client = AnalyticsEventsClient:REDACTED_SECRET
        ...
        let cli_overrides = Arc:REDACTED_SECRETRwLock:REDACTED_SECRETcli_overrides));
        let runtime_feature_enablement = Arc:REDACTED_SECRETRwLock:REDACTED_SECRETBTreeMap:REDACTED_SECRET)));
        let cloud_requirements = Arc:REDACTED_SECRETRwLock:REDACTED_SECRETcloud_requirements));
        let codex_message_processor = CodexMessageProcessor:REDACTED_SECRETCodexMessageProcessorArgs {
        ...
        let config_api = ConfigApi:REDACTED_SECRET
            config.codex_home.clone(),
            cli_overrides,
        ...
        let external_agent_config_api = ExternalAgentConfigApi:REDACTED_SECRETconfig.codex_home.clone());
        let fs_api = FsApi::default();
        let fs_watch_manager = FsWatchManager:REDACTED_SECREToutgoing.clone());
        ...
        Self {
            ...
            fs_watch_manager,
            config,
            config_warnings: Arc:REDACTED_SECRETconfig_warnings),
            rpc_transport,
        }
    }
    ...
    pub(crate) async fn process_request(
        ...
        let request_context = RequestContext:REDACTED_SECRETrequest_id.clone(), request_span, request_trace);
    ...
    pub(crate) async fn process_client_request(
        ...
        let request_context =
            RequestContext:REDACTED_SECRETrequest_id.clone(), request_span, /*parent_trace*/ None);
        tracing::trace!(
            ?connection_id,
            ...
            ClientRequest::Initialize { request_id, params } => {
                ...
                let (experimental_api_enabled, opt_out_notification_methods) =
                    match params.capabilities {
                        ...
                        None => (false, Vec:REDACTED_SECRET)),
                    };
