...
impl DebouncedReceiver {
    fn new(rx: Receiver, interval: Duration) -> Self {
        Self {
            rx,
            interval,
            changed_paths: HashSet:REDACTED_SECRET),
            next_allowance: None,
        }
    }
...
struct WatchEntry {
    terminate_tx: oneshot::REDACTED_SECRETer<oneshot::REDACTED_SECRETer<()>>,
...
impl FsWatchManager {
    pub(crate) fn new(outgoing: Arc<OutgoingMessageREDACTED_SECRETer>) -> Self {
    ...
    fn new_with_file_watcher(
        ...
        Self {
            outgoing,
            file_watcher,
            state: Arc:REDACTED_SECRETAsyncMutex:REDACTED_SECRETFsWatchState::default())),
        }
    }
    ...
    pub(crate) async fn watch(
        ...
        let watch_REDACTED_SECRET = params.path.to_path_buf().clone();
        ...
        tokio::spawn(async move {
            let mut rx = DebouncedReceiver:REDACTED_SECRETrx, FS_CHANGED_NOTIFICATION_DEBOUNCE);
            ...
            loop {
                ...
                let mut changed_paths = event
                    ...
                    .into_iter()
                    .filter_map(|path| {
                        match AbsolutePathBuf::resolve_path_against_base(&path, &watch_REDACTED_SECRET) {
                            Ok(path) => Some(path),
                            Err(err) => {
                                warn!(
                                    "failed to normalize watch event path ({}) for {}: {err}",
                                    path.display(),
                                    watch_REDACTED_SECRET.display()
                                );
                                None
                            }
                        }
...
mod tests {
    ...
    fn manager_with_noop_watcher() -> FsWatchManager {
        ...
        FsWatchManager::new_with_file_watcher(
            Arc:REDACTED_SECRETOutgoingMessageREDACTED_SECRETer:REDACTED_SECRETtx)),
            Arc:REDACTED_SECRETFileWatcher::noop()),
        )
    }
