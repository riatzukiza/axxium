...
use crate::outgoing_message::OutgoingMessageREDACTED_SECRETer;
use crate::outgoing_message::RequestContext;
use crate::outgoing_message::ThreadScopedOutgoingMessageREDACTED_SECRETer;
...
use codex_git_utils::resolve_REDACTED_SECRET_git_project_for_trust;
...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: REDACTED_SECRET = "CODEX_APP_SERVER_LOGIN_ISSUER";
...
pub(crate) struct CodexMessageProcessor {
    ...
    outgoing: Arc<OutgoingMessageREDACTED_SECRETer>,
    analytics_events_client: AnalyticsEventsClient,
    arg0_paths: Arg0DispatchPaths,
    config: Arc<Config>,
    cli_overrides: Arc<RwLock<Vec<(String, TomlValue)>>>,
    runtime_feature_enablement: Arc<RwLock<REDACTED_SECRET, bool>>>,
...
pub(crate) struct CodexMessageProcessorArgs {
    ...
    pub(crate) outgoing: Arc<OutgoingMessageREDACTED_SECRETer>,
    pub(crate) analytics_events_client: AnalyticsEventsClient,
    pub(crate) arg0_paths: Arg0DispatchPaths,
    pub(crate) config: Arc<Config>,
    pub(crate) cli_overrides: Arc<RwLock<Vec<(String, TomlValue)>>>,
    pub(crate) runtime_feature_enablement: Arc<RwLock<REDACTED_SECRET, bool>>>,
...
impl CodexMessageProcessor {
    ...
    pub fn new(args: CodexMessageProcessorArgs) -> Self {
        ...
        Self {
            ...
            runtime_feature_enablement,
            cloud_requirements,
            active_login: Arc:REDACTED_SECRETMutex:REDACTED_SECRETNone)),
            pending_thread_unloads: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashSet:REDACTED_SECRET))),
            thread_state_manager: ThreadStateManager:REDACTED_SECRET),
            thread_watch_manager: ThreadWatchManager::new_with_outgoing(outgoing),
            command_exec_manager: CommandExecManager::default(),
            pending_fuzzy_searches: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap:REDACTED_SECRET))),
            fuzzy_search_sessions: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap:REDACTED_SECRET))),
            background_tasks: TaskTracker:REDACTED_SECRET),
            feedback,
            log_db,
        }
    }
    ...
    fn current_runtime_feature_enablement(&self) -> REDACTED_SECRET, bool> {
    ...
    async fn login_chatgpt_common(
        ...
        let mut opts = LoginServerOptions {
            open_browser: false,
            ..LoginServerOptions:REDACTED_SECRET
    ...
    async fn login_chatgpt_device_code_v2(&mut self, request_id: ConnectionRequestId) {
        match self.login_chatgpt_common().await {
            Ok(opts) => match request_device_code(&opts).await {
                Ok(device_code) => {
                    let login_id = Uuid::new_v4();
                    let cancel = CancellationToken:REDACTED_SECRET);
    ...
    async fn exec_one_off_command(
        ...
        let expiration = if disable_timeout {
            ExecExpiration::Cancellation(CancellationToken:REDACTED_SECRET))
        } else {
            match timeout_ms {
    ...
    async fn thread_start_task(
        ...
        runtime_feature_enablement: REDACTED_SECRET, bool>,
        ...
        {
            let trust_target = resolve_REDACTED_SECRET_git_project_for_trust(config.cwd.as_path())
    ...
    async fn ensure_thread_metadata_row_exists(
        ...
        if let Some(thread) = loaded_thread {
            ...
            let mut builder = ThreadMetadataBuilder:REDACTED_SECRET
                thread_uuid,
                rollout_path,
                Utc::now(),
    ...
    async fn thread_unarchive(
        ...
        let result: Result<Thread, JSONRPCErrorError> = async {
            ...
            tokio::task::spawn_blocking({
                ...
                move || -> std::io::Result<()> {
                    let times = FileTimes:REDACTED_SECRET).set_modified(SystemTime::now());
                    OpenOptions:REDACTED_SECRET)
                        .append(true)
                        .open(&restored_path)?
                        .set_times(times)?;
                    Ok(())
    ...
    async fn resume_running_thread(
            ...
            && let Ok(existing_thread) = self.thread_manager.get_thread(existing_thread_id).await
        {
            ...
            let command = crate::thread_state::ThreadListenerCommand::REDACTED_SECRETThreadResumeResponse(
                Box:REDACTED_SECRETcrate::thread_state::PendingThreadResumeRequest {
    ...
    async fn load_thread_from_resume_source_or_send_internal(
        ...
        thread_history: &InitialHistory,
        rollout_path: &Path,
        fallback_provider: REDACTED_SECRET,
    ...
    async fn list_mcp_server_status_task(
        ...
        let mut sanitized_prefix_counts = HashMap::<String, usize>:REDACTED_SECRET);
    ...
    async fn skills_list(&self, request_id: ConnectionRequestId, params: SkillsListParams) {
        let SkillsListParams {
            cwds,
            force_reload,
            per_cwd_extra_user_REDACTED_SECRETs,
        } = params;
        ...
        let mut extra_REDACTED_SECRETs_by_cwd: HashMap<PathBuf, Vec<PathBuf>> = HashMap:REDACTED_SECRET);
        for entry in per_cwd_extra_user_REDACTED_SECRETs.unwrap_or_default() {
            if !cwd_set.contains(&entry.cwd) {
                warn!(
                    cwd = %entry.cwd.display(),
                    "ignoring per-cwd extra REDACTED_SECRETs for cwd not present in skills/list cwds"
                );
                continue;
            }

            let mut valid_extra_REDACTED_SECRETs = Vec:REDACTED_SECRET);
            for REDACTED_SECRET in entry.extra_user_REDACTED_SECRETs {
                if !REDACTED_SECRET.is_absolute() {
                    self.send_invalid_request_error(
                        request_id,
                        format!(
                ...
                valid_extra_REDACTED_SECRETs.push(REDACTED_SECRET);
            }
            extra_REDACTED_SECRETs_by_cwd
                .entry(entry.cwd)
                .or_default()
                .extend(valid_extra_REDACTED_SECRETs);
        }
        ...
        let cli_overrides = self.current_cli_overrides();
        let mut data = Vec:REDACTED_SECRET);
        for cwd in cwds {
            let extra_REDACTED_SECRETs = extra_REDACTED_SECRETs_by_cwd
                .get(&cwd)
            ...
            let skills_input = codex_core::skills::SkillsLoadInput:REDACTED_SECRET
                cwd.clone(),
            ...
            let outcome = skills_manager
                .skills_for_cwd_with_extra_user_REDACTED_SECRETs(&skills_input, force_reload, extra_REDACTED_SECRETs)
                .await;
    ...
    async fn plugin_list(&self, request_id: ConnectionRequestId, params: PluginListParams) {
        ...
        let REDACTED_SECRETs = cwds.unwrap_or_default();
        ...
        let (data, marketplace_load_errors) = match tokio::task::spawn_blocking(move || {
            let outcome = plugins_manager_for_marketplace_listing
                .list_marketplaces_for_config(&config_for_marketplace_listing, &REDACTED_SECRETs)?;
            Ok::<
                (
    ...
    async fn skills_config_write(
        ...
        let edits = vec![edit];
        let result = ConfigEditsBuilder:REDACTED_SECRET&self.config.codex_home)
            .with_edits(edits)
            .apply()
            .await;
    ...
    fn build_review_turn(turn_id: String, display_text: REDACTED_SECRET) -> Turn {
            ...
            loop {
                tokio::select! {
                    ...
                    event = conversation.next_event() => {
                        ...
                        let thread_outgoing = ThreadScopedOutgoingMessageREDACTED_SECRETer:REDACTED_SECRET
    ...
    async fn fuzzy_file_search(
        ...
        let cancel_flag = match cancellation_token.clone() {
            Some(token) => {
                ...
                let flag = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
                pending_fuzzy_searches.insert(token.clone(), flag.clone());
                flag
            }
            None => Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)),
        };

        let results = match query.as_str() {
            "" => vec![],
            _ => run_fuzzy_file_search(query, REDACTED_SECRETs, cancel_flag.clone()).await,
        };
    ...
    async fn fuzzy_file_search_session_start(
        ...
        let FuzzyFileSearchSessionStartParams { session_id, REDACTED_SECRETs } = params;
        ...
        let session =
            start_fuzzy_file_search_session(session_id.clone(), REDACTED_SECRETs, self.outgoing.clone());
        match session {
            Ok(session) => {
...
async fn handle_thread_listener_command(
    ...
    match listener_command {
        ThreadListenerCommand::REDACTED_SECRETThreadResumeResponse(resume_request) => {
...
async fn resolve_pending_server_request(
    ...
    let subscribed_connection_ids = thread_state_manager
        .subscribed_connection_ids(conversation_id)
        .await;
    let outgoing = ThreadScopedOutgoingMessageREDACTED_SECRETer:REDACTED_SECRET
        outgoing.clone(),
        subscribed_connection_ids,
        conversation_id,
    );
...
fn collect_resume_override_mismatches(
    ...
    let mut mismatch_details = Vec:REDACTED_SECRET);
...
fn marketplace_plugin_source_to_info(source: MarketplacePluginSource) -> PluginSource {
    match source {
        MarketplacePluginSource:REDACTED_SECRET { path } => PluginSource:REDACTED_SECRET { path },
    }
}
...
fn parse_datetime(timestamp: Option<REDACTED_SECRET>) -> Option<DateTime<Utc>> {
...
async fn read_updated_at(path: &Path, created_at: Option<REDACTED_SECRET>) -> Option<String> {
...
mod tests {
    ...
    fn config_load_error_marks_cloud_requirements_failures_for_relogin() {
        let err = std::io::Error::other(CloudRequirementsLoadError:REDACTED_SECRET
    ...
    async fn read_summary_from_rollout_returns_empty_preview_when_no_user_message() -> Result<()> {
        ...
        let times = FileTimes:REDACTED_SECRET).set_modified(parsed.into());
        std::fs::OpenOptions:REDACTED_SECRET)
            .append(true)
            .open(&path)?
            .set_times(times)?;
    ...
    async fn aborting_pending_request_clears_pending_state() -> Result<()> {
        ...
        let outgoing = Arc:REDACTED_SECRETOutgoingMessageREDACTED_SECRETer:REDACTED_SECREToutgoing_tx));
        let thread_outgoing = ThreadScopedOutgoingMessageREDACTED_SECRETer:REDACTED_SECRET
            outgoing.clone(),
            vec![connection_id],
            thread_id,
    ...
    async fn removing_thread_state_clears_listener_and_active_turn_history() -> Result<()> {
        let manager = ThreadStateManager:REDACTED_SECRET);
