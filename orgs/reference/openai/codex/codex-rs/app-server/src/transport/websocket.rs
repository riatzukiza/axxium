...
fn colorize(text: REDACTED_SECRET, style: Style) -> String {
...
fn print_websocket_startup_banner(addr: SocketAddr) {
    let title = colorize("codex app-server (WebSockets)", Style:REDACTED_SECRET).bold().cyan());
    let listening_label = colorize("listening on:", Style:REDACTED_SECRET).dimmed());
    let listen_url = colorize(&format!("ws://{addr}"), Style:REDACTED_SECRET).green());
    let ready_label = colorize("readyz:", Style:REDACTED_SECRET).dimmed());
    let ready_url = colorize(&format!("http://{addr}/readyz"), Style:REDACTED_SECRET).green());
    let health_label = colorize("healthz:", Style:REDACTED_SECRET).dimmed());
    let health_url = colorize(&format!("http://{addr}/healthz"), Style:REDACTED_SECRET).green());
    let note_label = colorize("note:", Style:REDACTED_SECRET).dimmed());
    eprintln!("{title}");
...
pub(crate) async fn start_websocket_acceptor(
    ...
    let router = Router:REDACTED_SECRET)
        ...
        .with_state(WebSocketListenerState {
            transport_event_tx,
            connection_counter: Arc:REDACTED_SECRETAtomicU64:REDACTED_SECRET1)),
            auth_policy: Arc:REDACTED_SECRETauth_policy),
        });
...
async fn run_websocket_connection(
    ...
    let disconnect_token = CancellationToken:REDACTED_SECRET);
...
async fn run_websocket_inbound_loop(
    ...
    transport_event_tx: mpsc::REDACTED_SECRETer<TransportEvent>,
    writer_tx_for_reader: mpsc::REDACTED_SECRETer<QueuedOutgoingMessage>,
    writer_control_tx: mpsc::REDACTED_SECRETer<WebSocketMessage>,
    connection_id: ConnectionId,
    disconnect_token: CancellationToken,
) {
    loop {
                ...
                match incoming_message {
                    ...
                    Some(Ok(WebSocketMessage::Ping(payload))) => {
                        match writer_control_tx.try_send(WebSocketMessage::Pong(payload)) {
                            Ok(()) => {}
                            Err(tokio::sync::mpsc::error::TryREDACTED_SECRETError::Closed(_)) => break,
                            Err(tokio::sync::mpsc::error::TryREDACTED_SECRETError::Full(_)) => {
