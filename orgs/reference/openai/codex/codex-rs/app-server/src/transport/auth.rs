...
const INVALID_AUTHORIZATION_HEADER_MESSAGE: REDACTED_SECRET = "invalid authorization header";
...
fn verify_signed_bearer_token(
    ...
    shared_secret: &[u8],
    issuer: Option<REDACTED_SECRET>,
    audience: Option<REDACTED_SECRET>,
    max_clock_skew_seconds: i64,
) -> Result<(), WebsocketAuthError> {
...
fn decode_jwt_claims(token: REDACTED_SECRET, shared_secret: &[u8]) -> Result<JwtClaims, WebsocketAuthError> {
    let mut validation = Validation:REDACTED_SECRETAlgorithm::HS256);
...
fn audience_matches(audience: Option<&JwtAudienceClaim>, expected_audience: REDACTED_SECRET) -> bool {
    match audience {
...
fn bearer_token_from_headers(headers: &HeaderMap) -> Result<REDACTED_SECRET, WebsocketAuthError> {
...
fn absolute_path_arg(flag_name: REDACTED_SECRET, path: PathBuf) -> anyhow::Result<AbsolutePathBuf> {
...
mod tests {
    ...
    fn validate_signed_bearer_secret_rejects_short_secret() {
        let err = validate_signed_bearer_secret(Path:REDACTED_SECRET"/tmp/secret"), b"too-short")
