...
impl AppServerTransport {
    ...
    pub fn from_listen_url(listen_url: REDACTED_SECRET) -> Result<Self, AppServerTransportParseError> {
...
pub(crate) enum TransportEvent {
    ConnectionOpened {
        ...
        writer: mpsc::REDACTED_SECRETer<QueuedOutgoingMessage>,
...
pub(crate) struct OutboundConnectionState {
    ...
    pub(crate) writer: mpsc::REDACTED_SECRETer<QueuedOutgoingMessage>,
    disconnect_sender: Option<CancellationToken>,
}
...
async fn forward_incoming_message(
    transport_event_tx: &mpsc::REDACTED_SECRETer<TransportEvent>,
    writer: &mpsc::REDACTED_SECRETer<QueuedOutgoingMessage>,
    connection_id: ConnectionId,
...
async fn enqueue_incoming_message(
    ...
    match transport_event_tx.try_send(event) {
        Ok(()) => true,
        Err(mpsc::error::TryREDACTED_SECRETError::Closed(_)) => false,
        Err(mpsc::error::TryREDACTED_SECRETError::Full(TransportEvent::IncomingMessage {
            connection_id,
            ...
            match writer.try_send(QueuedOutgoingMessage:REDACTED_SECREToverload_error)) {
                Ok(()) => true,
                Err(mpsc::error::TryREDACTED_SECRETError::Closed(_)) => false,
                Err(mpsc::error::TryREDACTED_SECRETError::Full(_overload_error)) => {
                    warn!(
        ...
        Err(mpsc::error::TryREDACTED_SECRETError::Full(event)) => transport_event_tx.send(event).await.is_ok(),
    }
}
...
async fn send_message_to_connection(
    ...
    connection_id: ConnectionId,
    message: OutgoingMessage,
    write_complete_tx: Option<tokio::sync::oneshot::REDACTED_SECRETer<()>>,
) -> bool {
...
mod tests {
    ...
    async fn enqueue_incoming_request_does_not_block_when_writer_queue_is_full() {
        ...
        writer_tx
            .send(QueuedOutgoingMessage:REDACTED_SECRET
    ...
    async fn to_connection_notification_respects_opt_out_filters() {
        ...
        let (writer_tx, mut writer_rx) = mpsc::channel(1);
        let initialized = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue));
        let opted_out_notification_methods =
            Arc:REDACTED_SECRETRwLock:REDACTED_SECRETHashSet::from(["configWarning".to_string()])));

        let mut connections = HashMap:REDACTED_SECRET);
        connections.insert(
            connection_id,
    ...
    async fn to_connection_notifications_are_preserved_for_non_opted_out_clients() {
        ...
        connections.insert(
            ...
            OutboundConnectionState:REDACTED_SECRET
                ...
                Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue)),
                Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue)),
                Arc:REDACTED_SECRETRwLock:REDACTED_SECRETHashSet:REDACTED_SECRET))),
                /*disconnect_sender*/ None,
            ),
        );
    ...
    async fn command_execution_request_approval_strips_additional_permissions_without_capability() {
        ...
        connections.insert(
            ...
            OutboundConnectionState:REDACTED_SECRET
                writer_tx,
                Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue)),
                Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)),
                Arc:REDACTED_SECRETRwLock:REDACTED_SECRETHashSet:REDACTED_SECRET))),
                /*disconnect_sender*/ None,
            ),
        );
    ...
    async fn broadcast_does_not_block_on_slow_connection() {
        ...
        let fast_disconnect_token = CancellationToken:REDACTED_SECRET);
        let slow_disconnect_token = CancellationToken:REDACTED_SECRET);
        ...
        slow_writer_tx
            .try_send(QueuedOutgoingMessage:REDACTED_SECRETqueued_message))
