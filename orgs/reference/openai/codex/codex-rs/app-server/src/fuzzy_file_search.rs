...
pub(crate) async fn run_fuzzy_file_search(
    ...
    let limit = NonZero:REDACTED_SECRETMATCH_LIMIT).expect("MATCH_LIMIT should be a valid non-zero usize");
    ...
    #[expect(clippy::expect_used)]
    let threads = NonZero:REDACTED_SECRETthreads.max(1)).expect("threads should be non-zero");
    let search_dirs: Vec<PathBuf> = REDACTED_SECRETs.iter().map(PathBuf::from).collect();
        ...
        Ok(Ok(res)) => res
            ...
            .map(|m| {
                ...
                FuzzyFileSearchResult {
                    REDACTED_SECRET: m.REDACTED_SECRET.to_string_lossy().to_string(),
...
pub(crate) fn start_fuzzy_file_search_session(
    ...
    let threads = NonZero:REDACTED_SECRETthreads.max(1)).expect("threads should be non-zero");
    let search_dirs: Vec<PathBuf> = REDACTED_SECRETs.iter().map(PathBuf::from).collect();
    let canceled = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));

    let shared = Arc:REDACTED_SECRETSessionShared {
        session_id,
        latest_query: Mutex:REDACTED_SECRETString:REDACTED_SECRET)),
        outgoing,
        runtime: tokio::runtime::Handle::current(),
        canceled: canceled.clone(),
    });

    let reporter = Arc:REDACTED_SECRETSessionReporterImpl {
        shared: shared.clone(),
    });
