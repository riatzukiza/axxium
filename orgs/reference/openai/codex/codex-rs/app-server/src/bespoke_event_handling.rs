...
pub(crate) async fn apply_bespoke_event_handling(
    ...
    outgoing: ThreadScopedOutgoingMessageREDACTED_SECRETer,
    ...
    match msg {
        ...
        EventMsg::RequestUserInput(request) => {
            if matches!(api_version, ApiVersion::V2) {
                ...
                let empty = CoreRequestUserInputResponse {
                    answers: HashMap:REDACTED_SECRET),
                };
        ...
        EventMsg::CollabAgentSpawnEnd(end_event) => {
            ...
            let (receiver_thread_ids, agents_states) = match end_event.new_thread_id {
                ...
                None => (Vec:REDACTED_SECRET), HashMap:REDACTED_SECRET)),
            };
            let item = ThreadItem::CollabAgentToolCall {
        ...
        EventMsg::CollabAgentInteractionBegin(begin_event) => {
            ...
            let item = ThreadItem::CollabAgentToolCall {
                id: begin_event.call_id,
                tool: CollabAgentTool::REDACTED_SECRETInput,
                status: V2CollabToolCallStatus::InProgress,
                sender_thread_id: begin_event.sender_thread_id.to_string(),
                receiver_thread_ids,
                prompt: Some(begin_event.prompt),
...
const REVIEW_FALLBACK_MESSAGE: REDACTED_SECRET = "Reviewer failed to output a response.";
...
mod tests {
    ...
    fn new_thread_state() -> Arc<Mutex<ThreadState>> {
        Arc:REDACTED_SECRETMutex:REDACTED_SECRETThreadState::default()))
    }
    ...
    fn guardian_assessment_started_uses_event_turn_id_fallback() {
        ...
        let notification = guardian_auto_approval_review_notification(
            ...
            &GuardianAssessmentEvent {
                id: "item-1".to_string(),
                turn_id: String:REDACTED_SECRET),
    ...
    fn request_permissions_response_accepts_partial_network_and_file_system_grants() {
        ...
        let absolute_path = |path: REDACTED_SECRET| {
    ...
    fn collab_resume_begin_maps_to_item_started_resume_agent() {
        let event = CollabResumeBeginEvent {
            ...
            sender_thread_id: ThreadId:REDACTED_SECRET),
            receiver_thread_id: ThreadId:REDACTED_SECRET),
    ...
    async fn test_construct_mcp_tool_call_begin_notification_with_args() {
        ...
        let thread_id = ThreadId:REDACTED_SECRET).to_string();
    ...
    async fn test_handle_turn_complete_emits_error_multiple_turns() -> Result<()> {
        ...
        let conversation_a = ThreadId:REDACTED_SECRET);
        let conversation_b = ThreadId:REDACTED_SECRET);
        let thread_state = new_thread_state();
