...
//! 3. REDACTED_SECRET requests via [`InProcessClientHandle::request`], notifications via
...
enum InProcessClientMessage {
    Request {
        ...
        response_tx: oneshot::REDACTED_SECRETer<PendingClientRequestResponse>,
    },
    Notification {
    ...
    Shutdown {
        done_tx: oneshot::REDACTED_SECRETer<()>,
    },
}
...
#[derive(Clone)]
pub struct InProcessClientREDACTED_SECRETer {
    client_tx: mpsc::REDACTED_SECRETer<InProcessClientMessage>,
}

impl InProcessClientREDACTED_SECRETer {
    pub async fn request(&self, request: ClientRequest) -> IoResult<PendingClientRequestResponse> {
        ...
        self.try_send_client_message(InProcessClientMessage::Request {
            request: Box:REDACTED_SECRETrequest),
            response_tx,
        })?;
        response_rx.await.map_err(|err| {
            IoError:REDACTED_SECRET
    ...
    fn try_send_client_message(&self, message: InProcessClientMessage) -> IoResult<()> {
        match self.client_tx.try_send(message) {
            Ok(()) => Ok(()),
            Err(mpsc::error::TryREDACTED_SECRETError::Full(_)) => Err(IoError:REDACTED_SECRET
                ErrorKind::WouldBlock,
                "in-process app-server client queue is full",
            )),
            Err(mpsc::error::TryREDACTED_SECRETError::Closed(_)) => Err(IoError:REDACTED_SECRET
                ErrorKind::BrokenPipe,
...
pub struct InProcessClientHandle {
    client: InProcessClientREDACTED_SECRETer,
    event_rx: mpsc::Receiver<InProcessServerEvent>,
    runtime_handle: tokio::task::JoinHandle<()>,
}

impl InProcessClientHandle {
    /// REDACTED_SECRETs a typed client request into the in-process runtime.
    ///
    ...
    /// REDACTED_SECRETs a typed client notification into the in-process runtime.
    ///
    ...
    pub fn sender(&self) -> InProcessClientREDACTED_SECRETer {
        self.client.clone()
    }
}
...
pub async fn start(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    if let Err(error) = initialize_response {
        ...
        return Err(IoError:REDACTED_SECRET
...
fn start_uninitialized(args: InProcessStartArgs) -> InProcessClientHandle {
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let outgoing_message_sender = Arc:REDACTED_SECRETOutgoingMessageREDACTED_SECRETer:REDACTED_SECREToutgoing_tx));

        let (writer_tx, mut writer_rx) = mpsc::channel::<QueuedOutgoingMessage>(channel_capacity);
        let outbound_initialized = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        let outbound_experimental_api_enabled = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        let outbound_opted_out_notification_methods = Arc:REDACTED_SECRETRwLock:REDACTED_SECRETHashSet:REDACTED_SECRET)));

        let mut outbound_connections = HashMap::<ConnectionId, OutboundConnectionState>:REDACTED_SECRET);
        outbound_connections.insert(
            IN_PROCESS_CONNECTION_ID,
            OutboundConnectionState:REDACTED_SECRET
                writer_tx,
        ...
        let mut processor_handle = tokio::spawn(async move {
            let mut processor = MessageProcessor:REDACTED_SECRETMessageProcessorArgs {
                outgoing: Arc::clone(&processor_outgoing),
                arg0_paths: args.arg0_paths,
                config: args.config,
                environment_manager: Arc:REDACTED_SECRETEnvironmentManager::from_env()),
                        ...
                        match created {
                            Ok(thread_id) => {
                                let connection_ids = if session.initialized {
                                    ...
                                    Vec::<ConnectionId>:REDACTED_SECRET)
                                };
                                processor
        ...
        let mut pending_request_responses =
            HashMap::<RequestId, oneshot::REDACTED_SECRETer<PendingClientRequestResponse>>:REDACTED_SECRET);
                ...
                message = client_rx.recv() => {
                    match message {
                        Some(InProcessClientMessage::Request { request, response_tx }) => {
                            ...
                            match processor_tx.try_send(ProcessorCommand::Request(Box:REDACTED_SECRETrequest))) {
                                Ok(()) => {}
                                Err(mpsc::error::TryREDACTED_SECRETError::Full(_)) => {
                                ...
                                Err(mpsc::error::TryREDACTED_SECRETError::Closed(_)) => {
                                    if let Some(response_tx) =
                                        pending_request_responses.remove(&request_id)
                                    {
                                        let _ = response_tx.send(Err(JSONRPCErrorError {
                ...
                queued_message = writer_rx.recv() => {
                    ...
                    match outgoing_message {
                        ...
                        OutgoingMessage::Request(request) => {
                            // REDACTED_SECRET directly to avoid cloning; on failure the
                                ...
                                .try_send(InProcessServerEvent::ServerRequest(request))
                            {
                                let (code, message, inner) = match send_error {
                                    mpsc::error::TryREDACTED_SECRETError::Full(inner) => (
                                        OVERLOADED_ERROR_CODE,
                                        "in-process server request queue is full",
                                        inner,
                                    ),
                                    mpsc::error::TryREDACTED_SECRETError::Closed(inner) => (
                                        INTERNAL_ERROR_CODE,
                        ...
                        OutgoingMessage::AppServerNotification(notification) => {
                            ...
                            {
                                match send_error {
                                    mpsc::error::TryREDACTED_SECRETError::Full(_) => {
                                        warn!("dropping in-process server notification (queue full)");
                                    }
                                    mpsc::error::TryREDACTED_SECRETError::Closed(_) => {
                                        break;
                                    }
                                }
                            }
    ...
    InProcessClientHandle {
        client: InProcessClientREDACTED_SECRETer { client_tx },
        event_rx,
        runtime_handle,
    }
}
...
mod tests {
    ...
    async fn build_test_config() -> Config {
        match ConfigBuilder::default().build().await {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides(Vec:REDACTED_SECRET))
    ...
    async fn start_test_client_with_capacity(
        ...
        let args = InProcessStartArgs {
            ...
            config: Arc:REDACTED_SECRETbuild_test_config().await),
            cli_overrides: Vec:REDACTED_SECRET),
            loader_overrides: LoaderOverrides::default(),
            cloud_requirements: CloudRequirementsLoader::default(),
            feedback: CodexFeedback:REDACTED_SECRET),
            config_warnings: Vec:REDACTED_SECRET),
            session_source,
