...
pub(crate) enum ThreadListenerCommand {
    // REDACTED_SECRETThreadResumeResponse is used to resume an already running thread by sending the thread's history to the client and atomically subscribing for new updates.
    REDACTED_SECRETThreadResumeResponse(Box<PendingThreadResumeRequest>),
    ...
    ResolveServerRequest {
        ...
        completion_tx: oneshot::REDACTED_SECRETer<()>,
    },
}
...
pub(crate) struct ThreadState {
    ...
    pub(crate) cancel_tx: Option<oneshot::REDACTED_SECRETer<()>>,
    pub(crate) experimental_raw_events: bool,
    pub(crate) listener_generation: u64,
    listener_command_tx: Option<mpsc::UnboundedREDACTED_SECRETer<ThreadListenerCommand>>,
...
impl ThreadState {
    ...
    pub(crate) fn set_listener(
        &mut self,
        cancel_tx: oneshot::REDACTED_SECRETer<()>,
    ...
    pub(crate) fn listener_command_tx(
        &self,
    ) -> Option<mpsc::UnboundedREDACTED_SECRETer<ThreadListenerCommand>> {
...
impl Default for ThreadEntry {
    fn default() -> Self {
        Self {
            state: Arc:REDACTED_SECRETMutex:REDACTED_SECRETThreadState::default())),
            connection_ids: HashSet:REDACTED_SECRET),
        }
    }
}
