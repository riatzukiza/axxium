...
impl RequestContext {
    ...
    fn record_turn_id(&self, turn_id: REDACTED_SECRET) {
...
pub(crate) enum OutgoingEnvelope {
    ToConnection {
        ...
        write_complete_tx: Option<oneshot::REDACTED_SECRETer<()>>,
    },
    Broadcast {
        message: OutgoingMessage,
    },
...
pub(crate) struct QueuedOutgoingMessage {
    ...
    pub(crate) write_complete_tx: Option<oneshot::REDACTED_SECRETer<()>>,
}
...
/// REDACTED_SECRETs messages to the client and manages request callbacks.
pub(crate) struct OutgoingMessageREDACTED_SECRETer {
    next_server_request_id: AtomicI64,
    sender: mpsc::REDACTED_SECRETer<OutgoingEnvelope>,
...
#[derive(Clone)]
pub(crate) struct ThreadScopedOutgoingMessageREDACTED_SECRETer {
...
struct PendingCallbackEntry {
    callback: oneshot::REDACTED_SECRETer<ClientRequestResult>,
    thread_id: Option<ThreadId>,
    request: ServerRequest,
}

impl ThreadScopedOutgoingMessageREDACTED_SECRETer {
    pub(crate) fn new(
        outgoing: Arc<OutgoingMessageREDACTED_SECRETer>,
        ...
        Self {
            outgoing,
            connection_ids: Arc:REDACTED_SECRETconnection_ids),
            thread_id,
        }
    }
...
impl OutgoingMessageREDACTED_SECRETer {
    pub(crate) fn new(sender: mpsc::REDACTED_SECRETer<OutgoingEnvelope>) -> Self {
        Self {
            next_server_request_id: AtomicI64:REDACTED_SECRET0),
            sender,
            request_id_to_callback: Mutex:REDACTED_SECRETHashMap:REDACTED_SECRET)),
            request_contexts: Mutex:REDACTED_SECRETHashMap:REDACTED_SECRET)),
        }
    }
...
mod tests {
    ...
    async fn send_response_routes_to_target_connection() {
        ...
        let outgoing = OutgoingMessageREDACTED_SECRETer:REDACTED_SECRETtx);
    ...
    async fn send_response_clears_registered_request_context() {
        ...
        outgoing
            .register_request_context(RequestContext:REDACTED_SECRET
                request_id.clone(),
        ...
        let (request_id, wait_for_result) = outgoing
            .send_request(ServerRequestPayload::ApplyPatchApproval(
                ApplyPatchApprovalParams {
                    conversation_id: ThreadId:REDACTED_SECRET),
                    call_id: "call-id".to_string(),
                    file_changes: HashMap:REDACTED_SECRET),
                    reason: None,
    ...
    async fn pending_requests_for_thread_returns_thread_requests_in_request_id_order() {
        ...
        let outgoing = Arc:REDACTED_SECRETOutgoingMessageREDACTED_SECRETer:REDACTED_SECRETtx));
