...
impl Default for CommandExecManager {
    fn default() -> Self {
        Self {
            sessions: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap:REDACTED_SECRET))),
            next_generated_process_id: Arc:REDACTED_SECRETAtomicI64:REDACTED_SECRET1)),
        }
    }
}
...
enum CommandExecSession {
    Active {
        control_tx: mpsc::REDACTED_SECRETer<CommandControlRequest>,
    },
...
struct CommandControlRequest {
    ...
    response_tx: Option<oneshot::REDACTED_SECRETer<Result<(), JSONRPCErrorError>>>,
}
...
fn spawn_process_output(params: SpawnProcessOutputParams) -> tokio::task::JoinHandle<String> {
    ...
    tokio::spawn(async move {
        let mut buffer: Vec<u8> = Vec:REDACTED_SECRET);
    ...
    async fn windows_sandbox_streaming_exec_is_rejected() {
        ...
        let err = manager
            .start(StartCommandExecParams {
                outgoing: Arc:REDACTED_SECRETOutgoingMessageREDACTED_SECRETer:REDACTED_SECRETtx)),
                request_id: ConnectionRequestId {
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        manager
            .start(StartCommandExecParams {
                ...
                exec_request: ExecRequest:REDACTED_SECRET
