...
fn write_skill(REDACTED_SECRET: &TempDir, name: REDACTED_SECRET) -> Result<()> {
    let skill_dir = REDACTED_SECRET.path().join("skills").join(name);
...
#[tokio::test]
async fn skills_list_includes_skills_from_per_cwd_extra_user_REDACTED_SECRETs() -> Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let cwd = TempDir:REDACTED_SECRET)?;
    let extra_REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    write_skill(&extra_REDACTED_SECRET, "extra-skill")?;
    ...
    let request_id = mcp
        .send_skills_list_request(SkillsListParams {
            ...
            per_cwd_extra_user_REDACTED_SECRETs: Some(vec![SkillsListExtraRootsForCwd {
                ...
                extra_user_REDACTED_SECRETs: vec![extra_REDACTED_SECRET.path().to_path_buf()],
            }]),
        })
        .await?;
...
#[tokio::test]
async fn skills_list_rejects_relative_extra_user_REDACTED_SECRETs() -> Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let cwd = TempDir:REDACTED_SECRET)?;
    ...
    let request_id = mcp
        .send_skills_list_request(SkillsListParams {
            ...
            per_cwd_extra_user_REDACTED_SECRETs: Some(vec![SkillsListExtraRootsForCwd {
                cwd: cwd.path().to_path_buf(),
                extra_user_REDACTED_SECRETs: vec![std::path::PathBuf::from("relative/skills")],
            }]),
        })
        .await?;
...
#[tokio::test]
async fn skills_list_ignores_per_cwd_extra_REDACTED_SECRETs_for_unknown_cwd() -> Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let requested_cwd = TempDir:REDACTED_SECRET)?;
    let unknown_cwd = TempDir:REDACTED_SECRET)?;
    let extra_REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    write_skill(&extra_REDACTED_SECRET, "ignored-extra-skill")?;
...
async fn skills_list_uses_cached_result_until_force_reload() -> Result<()> {
    ...
    let cwd = TempDir:REDACTED_SECRET)?;
    let extra_REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    write_skill(&extra_REDACTED_SECRET, "late-extra-skill")?;

    let mut mcp = McpProcess:REDACTED_SECRETcodex_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;

    // Seed the cwd cache first without extra REDACTED_SECRETs.
