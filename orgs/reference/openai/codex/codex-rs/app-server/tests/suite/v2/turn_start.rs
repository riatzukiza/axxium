...
const TEST_ORIGINATOR: REDACTED_SECRET = "codex_vscode";
const LOCAL_PRAGMATIC_TEMPLATE: REDACTED_SECRET = "You are a deeply pragmatic, effective software engineer.";

fn body_contains(req: &wiremock::Request, text: REDACTED_SECRET) -> bool {
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // REDACTED_SECRET a second turn that exercises the overrides path: change the model.
    let turn_req2 = mcp
        .send_turn_start_request(TurnStartParams {
            thread_id: thread.id.clone(),
            input: vec![V2UserInput::Text {
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            thread_id: thread.id.clone(),
            input: vec![V2UserInput:REDACTED_SECRETImage { path: image_path }],
            ..Default::default()
        })
        .await?;
    let turn_resp: JSONRPCResponse = timeout(
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir:REDACTED_SECRET)?;
    ...
    let mut mcp = McpProcess:REDACTED_SECRETcodex_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let workspace_REDACTED_SECRET = tmp.path().join("workspace");
    std::fs::create_dir(&workspace_REDACTED_SECRET)?;
    let first_cwd = workspace_REDACTED_SECRET.join("turn1");
    let second_cwd = workspace_REDACTED_SECRET.join("turn2");
    ...
    let mut mcp = McpProcess:REDACTED_SECRET&codex_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(codex_app_server_protocol::SandboxPolicy::WorkspaceWrite {
                writable_REDACTED_SECRETs: vec![first_cwd.try_into()?],
...
async fn turn_start_emits_spawn_agent_item_with_model_metadata_v2() -> Result<()> {
    ...
    const CHILD_PROMPT: REDACTED_SECRET = "child: do work";
    const PARENT_PROMPT: REDACTED_SECRET = "spawn a child and continue";
    const SPAWN_CALL_ID: REDACTED_SECRET = "spawn-call-1";
    const REQUESTED_MODEL: REDACTED_SECRET = "gpt-5.1";
    ...
    assert_eq!(
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            receiver_thread_ids: Vec:REDACTED_SECRET),
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    const REQUESTED_MODEL: REDACTED_SECRET = "gpt-5.1";
    const REQUESTED_REASONING_EFFORT: ReasoningEffort = ReasoningEffort::Low;
    const ROLE_MODEL: REDACTED_SECRET = "gpt-5.1-codex-max";
...
fn create_config_toml(
    codex_home: &Path,
    server_uri: REDACTED_SECRET,
    approval_policy: REDACTED_SECRET,
...
fn create_config_toml_with_sandbox(
    ...
    approval_policy: REDACTED_SECRET,
    feature_flags: &BTreeMap<Feature, bool>,
    sandbox_mode: REDACTED_SECRET,
) -> std::io::Result<()> {
    let mut features = BTreeMap:REDACTED_SECRET);
