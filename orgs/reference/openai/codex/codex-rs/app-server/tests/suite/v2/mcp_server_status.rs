...
async fn mcp_server_status_list_returns_tools_for_hyphenated_server_names() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec:REDACTED_SECRET)).await;
...
async fn start_mcp_server(tool_name: REDACTED_SECRET) -> Result<(String, JoinHandle<()>)> {
    let listener = TcpListener::bind("127.0.0.1:0").await?;
    let addr = listener.local_addr()?;
    let tool_name = Arc:REDACTED_SECRETtool_name.to_string());
    ...
    let router = Router:REDACTED_SECRET).nest_service("/mcp", mcp_service);
