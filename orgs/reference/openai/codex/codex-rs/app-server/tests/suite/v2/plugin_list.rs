...
const TEST_CURATED_PLUGIN_SHA: REDACTED_SECRET = "0123456789abcdef0123456789abcdef01234567";
const STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE: REDACTED_SECRET = ".tmp/app-server-remote-plugin-sync-v1";
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_REDACTED_SECRET.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(codex_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_REDACTED_SECRET.path().join(".agents/plugins/marketplace.json"))?;
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            cwds: Some(vec![AbsolutePathBuf::try_from(repo_REDACTED_SECRET.path())?]),
...
async fn plugin_list_rejects_relative_cwds() -> Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let mut mcp = McpProcess:REDACTED_SECRETcodex_home.path()).await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
{
    let codex_home = TempDir:REDACTED_SECRET)?;
    let valid_repo_REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    let invalid_repo_REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    std::fs::create_dir_all(valid_repo_REDACTED_SECRET.path().join(".git"))?;
    std::fs::create_dir_all(valid_repo_REDACTED_SECRET.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(
        valid_repo_REDACTED_SECRET
            .path()
            .join("plugins/valid-plugin/.codex-plugin"),
    )?;
    std::fs::create_dir_all(invalid_repo_REDACTED_SECRET.path().join(".git"))?;
    std::fs::create_dir_all(invalid_repo_REDACTED_SECRET.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(codex_home.path())?;
    ...
    let invalid_marketplace_path = AbsolutePathBuf::try_from(
        invalid_repo_REDACTED_SECRET
            .path()
            .join(".agents/plugins/marketplace.json"),
    )?;
    let valid_plugin_path =
        AbsolutePathBuf::try_from(valid_repo_REDACTED_SECRET.path().join("plugins/valid-plugin"))?;
  ...
  "plugins": [
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            cwds: Some(vec![
                AbsolutePathBuf::try_from(valid_repo_REDACTED_SECRET.path())?,
                AbsolutePathBuf::try_from(invalid_repo_REDACTED_SECRET.path())?,
            ]),
            force_remote_sync: false,
        })
        .await?;
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::write(
        repo_REDACTED_SECRET.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "codex-curated",
  "interface": {
...
"#,
    ...
    let workspace_enabled = TempDir:REDACTED_SECRET)?;
...
"#,
    ...
    let workspace_default = TempDir:REDACTED_SECRET)?;
    let home = codex_home.path().to_string_lossy().into_owned();
    let mut mcp = McpProcess::new_with_env(
        codex_home.path(),
        &[
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let repo_REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    let plugin_REDACTED_SECRET = repo_REDACTED_SECRET.path().join("plugins/demo-plugin");
    std::fs::create_dir_all(repo_REDACTED_SECRET.path().join(".git"))?;
    std::fs::create_dir_all(repo_REDACTED_SECRET.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(plugin_REDACTED_SECRET.join(".codex-plugin"))?;
    write_plugins_enabled_config(codex_home.path())?;
    std::fs::write(
        repo_REDACTED_SECRET.path().join(".agents/plugins/marketplace.json"),
        r#"{
  ...
  "interface": {
    ...
    assert_eq!(
        ...
        Some(AbsolutePathBuf::try_from(
            plugin_REDACTED_SECRET.join("assets/icon.png")
        )?)
    );
    assert_eq!(
        interface.logo,
        Some(AbsolutePathBuf::try_from(
            plugin_REDACTED_SECRET.join("assets/logo.png")
        )?)
    );
    assert_eq!(
        interface.screenshots,
        vec![
            AbsolutePathBuf::try_from(plugin_REDACTED_SECRET.join("assets/screenshot1.png"))?,
            AbsolutePathBuf::try_from(plugin_REDACTED_SECRET.join("assets/screenshot2.png"))?,
        ]
    );
    Ok(())
}
...
async fn plugin_list_force_remote_sync_reconciles_curated_plugin_state() -> Result<()> {
    ...
    write_chatgpt_auth(
        codex_home.path(),
        ChatGptAuthFixture:REDACTED_SECRET"chatgpt-token")
...
async fn app_server_startup_remote_plugin_sync_runs_once() -> Result<()> {
  ...
  {"id":"1","name":"linear","marketplace_name":"openai-curated","version":"1.0.0","enabled":true}
    ...
    Mock::given(method("GET"))
        ...
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(ResponseTemplate:REDACTED_SECRET200).set_body_string(r#"["linear@openai-curated"]"#))
        .mount(&server)
        .await;
...
fn write_installed_plugin(
    codex_home: &TempDir,
    marketplace_name: REDACTED_SECRET,
    plugin_name: REDACTED_SECRET,
) -> Result<()> {
    ...
    std::fs::create_dir_all(&plugin_REDACTED_SECRET)?;
    std::fs::write(
        plugin_REDACTED_SECRET.join("plugin.json"),
        format!(r#"{{"name":"{plugin_name}"}}"#),
    )?;
    Ok(())
}

fn write_plugin_sync_config(codex_home: &std::path::Path, base_url: REDACTED_SECRET) -> std::io::Result<()> {
    std::fs::write(
        codex_home.join("config.toml"),
        format!(
            r#"
...
fn write_openai_curated_marketplace(
    ...
    plugin_names: &[REDACTED_SECRET],
) -> std::io::Result<()> {
    let curated_REDACTED_SECRET = codex_home.join(".tmp/plugins");
    std::fs::create_dir_all(curated_REDACTED_SECRET.join(".git"))?;
    std::fs::create_dir_all(curated_REDACTED_SECRET.join(".agents/plugins"))?;
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_REDACTED_SECRET = curated_REDACTED_SECRET.join(format!("plugins/{plugin_name}/.codex-plugin"));
        std::fs::create_dir_all(&plugin_REDACTED_SECRET)?;
        std::fs::write(
            plugin_REDACTED_SECRET.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
