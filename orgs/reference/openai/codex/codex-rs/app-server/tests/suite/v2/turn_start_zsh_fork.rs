...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(codex_app_server_protocol::SandboxPolicy::WorkspaceWrite {
                writable_REDACTED_SECRETs: vec![workspace.clone().try_into()?],
    ...
    let mut approved_subcommand_strings = Vec:REDACTED_SECRET);
    let mut approved_subcommand_ids = Vec:REDACTED_SECRET);
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_REDACTED_SECRET = codex_utils_cargo_bin::repo_REDACTED_SECRET()?;
    let dotslash_zsh = repo_REDACTED_SECRET.join("codex-rs/app-server/tests/suite/zsh");
...
fn supports_exec_wrapper_intercept(zsh_path: &Path) -> bool {
    let status = std::process::Command:REDACTED_SECRETzsh_path)
        .arg("-fc")
        .arg("/usr/bin/true")
