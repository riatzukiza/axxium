...
async fn plugin_install_returns_invalid_request_for_not_available_plugin() -> Result<()> {
    ...
    write_plugin_source(repo_REDACTED_SECRET.path(), "sample-plugin", &[])?;
...
async fn plugin_install_force_remote_sync_enables_remote_plugin_before_local_install() -> Result<()>
{
    ...
    assert_eq!(response.apps_needing_auth, Vec::<AppSummary>:REDACTED_SECRET));
...
async fn plugin_install_filters_disallowed_apps_needing_auth() -> Result<()> {
    ...
    let (server_url, server_handle) = start_apps_server(connectors, Vec:REDACTED_SECRET)).await?;
...
async fn plugin_install_makes_bundled_mcp_servers_available_to_followup_requests() -> Result<()> {
    ...
    write_plugin_source(repo_REDACTED_SECRET.path(), "sample-plugin", &[])?;
    std::fs::write(
        repo_REDACTED_SECRET.path().join("sample-plugin/.mcp.json"),
        r#"{
  "mcpServers": {
    "sample-mcp": {
      "command": "echo"
...
fn write_analytics_config(codex_home: &std::path::Path, base_url: REDACTED_SECRET) -> std::io::Result<()> {
    std::fs::write(
        codex_home.join("config.toml"),
...
fn write_plugin_marketplace(
    ...
    install_policy: Option<REDACTED_SECRET>,
    auth_policy: Option<REDACTED_SECRET>,
) -> std::io::Result<()> {
