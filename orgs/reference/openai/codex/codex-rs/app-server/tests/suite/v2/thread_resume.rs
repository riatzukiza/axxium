...
const CODEX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: REDACTED_SECRET = "You are Codex, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
"#,
    ...
    let head_branch = Command:REDACTED_SECRET"git")
        .current_dir(&repo_path)
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = McpProcess:REDACTED_SECRETcodex_home.path()).await?;
    ...
    let mut secondary = McpProcess:REDACTED_SECRETcodex_home.path()).await?;
...
"#;
    ...
    let mut primary = McpProcess:REDACTED_SECRET&codex_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
...
async fn thread_resume_surfaces_cloud_requirements_load_errors() -> Result<()> {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate:REDACTED_SECRET401)
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate:REDACTED_SECRET401).set_body_json(json!({
...
async fn start_materialized_thread_and_restart(
    codex_home: &Path,
    seed_text: REDACTED_SECRET,
) -> Result<RestartedThreadFixture> {
    let mut first_mcp = McpProcess:REDACTED_SECRETcodex_home).await?;
    ...
    let mut second_mcp = McpProcess:REDACTED_SECRETcodex_home).await?;
...
fn create_config_toml(codex_home: &std::path::Path, server_uri: REDACTED_SECRET) -> std::io::Result<()> {
...
fn create_config_toml_with_chatgpt_base_url(
    ...
    chatgpt_base_url: REDACTED_SECRET,
...
#[allow(dead_code)]
fn set_rollout_mtime(path: &Path, updated_at_rfc3339: REDACTED_SECRET) -> Result<()> {
...
fn setup_rollout_fixture(codex_home: &Path, server_uri: REDACTED_SECRET) -> Result<RolloutFixture> {
