...
async fn list_apps_force_refetch_preserves_previous_cache_on_failure() -> Result<()> {
    ...
    write_chatgpt_auth(
        codex_home.path(),
        ChatGptAuthFixture:REDACTED_SECRET"chatgpt-token-invalid")
            .account_id("account-123")
            .chatgpt_user_id("user-123")
            .chatgpt_account_id("account-123"),
        AuthCredentialsStoreMode::File,
...
async fn list_apps_force_refetch_patches_updates_from_cached_snapshots() -> Result<()> {
    ...
    server_control.set_tools(Vec:REDACTED_SECRET));
...
async fn start_apps_server_with_delays_and_control(
    ...
    let response = Arc:REDACTED_SECRETStdMutex:REDACTED_SECRET
    ...
    let state = Arc:REDACTED_SECRETstate);
    ...
    let mcp_service = StreamableHttpService:REDACTED_SECRET
        {
            ...
            move || Ok(AppListMcpServer:REDACTED_SECRETtools.clone(), tools_delay))
        },
