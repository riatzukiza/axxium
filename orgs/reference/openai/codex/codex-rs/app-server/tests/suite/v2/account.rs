...
const LOGIN_ISSUER_ENV_VAR: REDACTED_SECRET = "CODEX_APP_SERVER_LOGIN_ISSUER";
...
fn create_config_toml(codex_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
    ...
    let requires_line = match params.requires_openai_auth {
        ...
        Some(false) => String:REDACTED_SECRET),
...
async fn mock_device_code_usercode_failure(server: &MockServer, status: u16) {
    Mock::given(method("POST"))
        .and(path("/api/accounts/deviceauth/usercode"))
        .respond_with(ResponseTemplate:REDACTED_SECRETstatus))
        .mount(server)
        .await;
}
...
async fn mock_device_code_oauth_token(server: &MockServer, id_token: REDACTED_SECRET) {
    Mock::given(method("POST"))
...
async fn set_auth_token_updates_account_and_notifies() -> Result<()> {
    ...
    let access_token = encode_id_token(
        &ChatGptIdTokenClaims:REDACTED_SECRET)
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let unauthorized = ResponseTemplate:REDACTED_SECRET401).set_body_json(json!({
...
async fn get_account_with_chatgpt() -> Result<()> {
    ...
    write_chatgpt_auth(
        codex_home.path(),
        ChatGptAuthFixture:REDACTED_SECRET"access-chatgpt")
...
async fn get_account_with_chatgpt_missing_plan_claim_returns_unknown() -> Result<()> {
    ...
    write_chatgpt_auth(
        codex_home.path(),
        ChatGptAuthFixture:REDACTED_SECRET"access-chatgpt").email("user@example.com"),
        AuthCredentialsStoreMode::File,
    )?;
