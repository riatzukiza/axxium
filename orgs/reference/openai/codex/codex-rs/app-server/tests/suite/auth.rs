...
fn create_config_toml_forced_login(codex_home: &Path, forced_method: REDACTED_SECRET) -> std::io::Result<()> {
    let config_toml = codex_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn login_with_api_key_via_request(mcp: &mut McpProcess, api_key: REDACTED_SECRET) -> Result<()> {
...
async fn get_auth_status_omits_token_after_permanent_refresh_failure() -> Result<()> {
    ...
    write_chatgpt_auth(
        codex_home.path(),
        ChatGptAuthFixture:REDACTED_SECRET"stale-access-token")
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate:REDACTED_SECRET401).set_body_json(serde_json::json!({
            "error": {
...
async fn get_auth_status_returns_token_after_proactive_refresh_recovery() -> Result<()> {
    ...
    write_chatgpt_auth(
        codex_home.path(),
        ChatGptAuthFixture:REDACTED_SECRET"recovered-access-token")
