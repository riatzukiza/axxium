...
const COMPACT_PROMPT: REDACTED_SECRET = "Summarize the conversation.";
...
async fn auto_compaction_remote_emits_started_and_completed_items() -> Result<()> {
    ...
    write_chatgpt_auth(
        codex_home.path(),
        ChatGptAuthFixture:REDACTED_SECRET"access-chatgpt").plan_type("pro"),
...
async fn send_turn_and_wait(mcp: &mut McpProcess, thread_id: REDACTED_SECRET, text: REDACTED_SECRET) -> Result<String> {
    let turn_id = mcp
...
async fn wait_for_turn_completed(mcp: &mut McpProcess, turn_id: REDACTED_SECRET) -> Result<()> {
    loop {
