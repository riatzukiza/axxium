...
const SESSION_UPDATED_METHOD: REDACTED_SECRET = "fuzzyFileSearch/sessionUpdated";
const SESSION_COMPLETED_METHOD: REDACTED_SECRET = "fuzzyFileSearch/sessionCompleted";
...
async fn test_fuzzy_file_search_sorts_and_includes_indices() -> Result<()> {
    // Prepare a temporary Codex home and a separate REDACTED_SECRET with test files.
    let codex_home = TempDir:REDACTED_SECRET)?;
    create_config_toml(codex_home.path())?;
    let REDACTED_SECRET = TempDir:REDACTED_SECRET)?;

    // Create files designed to have deterministic ordering for query "abe".
    std::fs::write(REDACTED_SECRET.path().join("abc"), "x")?;
    std::fs::write(REDACTED_SECRET.path().join("abcde"), "x")?;
    std::fs::write(REDACTED_SECRET.path().join("abexy"), "x")?;
    std::fs::write(REDACTED_SECRET.path().join("zzz.txt"), "x")?;
    let sub_dir = REDACTED_SECRET.path().join("sub");
    std::fs::create_dir_all(&sub_dir)?;
    let sub_abce_path = sub_dir.join("abce");
    std::fs::write(&sub_abce_path, "x")?;
    let sub_abce_rel = sub_abce_path
        .strip_prefix(REDACTED_SECRET.path())?
        .to_string_lossy()
        .to_string();
    ...
    let REDACTED_SECRET_path = REDACTED_SECRET.path().to_string_lossy().to_string();
    // REDACTED_SECRET fuzzyFileSearch request.
    let request_id = mcp
        .send_fuzzy_file_search_request(
            "abe",
            vec![REDACTED_SECRET_path.clone()],
    ...
    assert_eq!(
        ...
        json!({
            "files": [
                {
                    "REDACTED_SECRET": REDACTED_SECRET_path.clone(),
                    "path": "abexy",
                    "match_type": "file",
                    "file_name": "abexy",
                    "score": 84,
...
async fn test_fuzzy_file_search_accepts_cancellation_token() -> Result<()> {
    ...
    std::fs::write(REDACTED_SECRET.path().join("alpha.txt"), "contents")?;
    ...
    assert_eq!(files[0]["REDACTED_SECRET"], REDACTED_SECRET_path);
...
async fn test_fuzzy_file_search_session_streams_updates() -> Result<()> {
    ...
    mcp.start_fuzzy_file_search_session(session_id, vec![REDACTED_SECRET_path.clone()])
        .await?;
    ...
    assert_eq!(payload.files[0].REDACTED_SECRET, REDACTED_SECRET_path);
...
async fn test_fuzzy_file_search_session_no_updates_after_complete_until_query_edited() -> Result<()>
{
    ...
    mcp.start_fuzzy_file_search_session(session_id, vec![REDACTED_SECRET_path])
        .await?;
...
-> Result<()> {
    ...
    let start_request_id = mcp
        .send_fuzzy_file_search_session_start_request(session_id, vec![REDACTED_SECRET_path.clone()])
        .await?;
...
async fn test_fuzzy_file_search_session_multiple_query_updates_work() -> Result<()> {
    ...
    let REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    std::fs::write(REDACTED_SECRET.path().join("alpha.txt"), "contents")?;
    std::fs::write(REDACTED_SECRET.path().join("alphabet.txt"), "contents")?;
    let mut mcp = initialized_mcp(&codex_home).await?;
    ...
    let alp_payload =
        wait_for_session_updated(&mut mcp, session_id, "alp", FileExpectation::NonEmpty).await?;
    assert_eq!(
        alp_payload.files.iter().all(|file| file.REDACTED_SECRET == REDACTED_SECRET_path),
        true
    );
    wait_for_session_completed(&mut mcp, session_id).await?;
...
async fn test_fuzzy_file_search_session_stops_sending_updates_after_stop() -> Result<()> {
    ...
    let REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    for i in 0..512 {
        let file_path = REDACTED_SECRET.path().join(format!("file-{i:04}.txt"));
...
async fn test_fuzzy_file_search_two_sessions_are_independent() -> Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let REDACTED_SECRET_a = TempDir:REDACTED_SECRET)?;
    let REDACTED_SECRET_b = TempDir:REDACTED_SECRET)?;
    std::fs::write(REDACTED_SECRET_a.path().join("alpha.txt"), "contents")?;
    std::fs::write(REDACTED_SECRET_b.path().join("beta.txt"), "contents")?;
    let mut mcp = initialized_mcp(&codex_home).await?;

    let REDACTED_SECRET_a_path = REDACTED_SECRET_a.path().to_string_lossy().to_string();
    let REDACTED_SECRET_b_path = REDACTED_SECRET_b.path().to_string_lossy().to_string();
    let session_a = "session-a";
    let session_b = "session-b";

    mcp.start_fuzzy_file_search_session(session_a, vec![REDACTED_SECRET_a_path.clone()])
        .await?;
    mcp.start_fuzzy_file_search_session(session_b, vec![REDACTED_SECRET_b_path.clone()])
        .await?;
    ...
    assert_eq!(session_a_update.files[0].REDACTED_SECRET, REDACTED_SECRET_a_path);
    ...
    assert_eq!(session_b_update.files[0].REDACTED_SECRET, REDACTED_SECRET_b_path);
