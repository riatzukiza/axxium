...
fn find_tool<'a>(body: &'a Value, name: REDACTED_SECRET) -> Option<&'a Value> {
    body.get("tools")
...
fn function_call_output_payload(body: &Value, call_id: REDACTED_SECRET) -> Option<FunctionCallOutputPayload> {
    function_call_output_raw_output(body, call_id)
        .and_then(|output| serde_json::from_value(output).ok())
}

fn function_call_output_raw_output(body: &Value, call_id: REDACTED_SECRET) -> Option<Value> {
    body.get("input")
        .and_then(Value::as_array)
        .and_then(|items| {
