...
use rmcp::transport::streamable_http_server::session::local:REDACTED_SECRETSessionManager;
use serde_json::json;
...
async fn plugin_read_returns_plugin_details_with_bundle_contents() -> Result<()> {
    ...
    std::fs::create_dir_all(plugin_REDACTED_SECRET.join("skills/thread-summarizer"))?;
    std::fs::create_dir_all(plugin_REDACTED_SECRET.join("skills/chatgpt-only"))?;
    std::fs::write(
  ...
  "interface": {
    ...
    std::fs::write(
        plugin_REDACTED_SECRET.join("skills/thread-summarizer/SKILL.md"),
        r#"---
...
"#,
    ...
    std::fs::write(
        plugin_REDACTED_SECRET.join("skills/chatgpt-only/SKILL.md"),
        r#"---
name: chatgpt-only
...
"#,
    ...
    std::fs::create_dir_all(plugin_REDACTED_SECRET.join("skills/thread-summarizer/agents"))?;
    std::fs::write(
        plugin_REDACTED_SECRET.join("skills/thread-summarizer/agents/openai.yaml"),
        r#"policy:
  products:
    - CODEX
"#,
    )?;
    std::fs::create_dir_all(plugin_REDACTED_SECRET.join("skills/chatgpt-only/agents"))?;
    std::fs::write(
        plugin_REDACTED_SECRET.join("skills/chatgpt-only/agents/openai.yaml"),
        r#"policy:
  products:
    - CHATGPT
"#,
...
async fn plugin_read_returns_app_needs_auth() -> Result<()> {
    ...
    write_plugin_source(repo_REDACTED_SECRET.path(), "sample-plugin", &["alpha", "beta"])?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_REDACTED_SECRET.path().join(".agents/plugins/marketplace.json"))?;
...
impl ServerHandler for PluginReadMcpServer {
    ...
    ) -> impl std::future::Future<Output = Result<ListToolsResult, rmcp::ErrorData>> + REDACTED_SECRET + '_
    {
...
async fn start_apps_server(
    ...
    let state = Arc:REDACTED_SECRETAppsServerState {
        response: Arc:REDACTED_SECRETStdMutex:REDACTED_SECRET
            json!({ "apps": connectors, "next_token": null }),
        )),
    });
    let tools = Arc:REDACTED_SECRETStdMutex:REDACTED_SECRETtools));

    let listener = TcpListener::bind("127.0.0.1:0").await?;
    let addr = listener.local_addr()?;
    let mcp_service = StreamableHttpService:REDACTED_SECRET
        {
        ...
        Arc:REDACTED_SECRETLocalSessionManager::default()),
        StreamableHttpServerConfig::default(),
    );
    let router = Router:REDACTED_SECRET)
...
fn connector_tool(connector_id: REDACTED_SECRET, connector_name: REDACTED_SECRET) -> Result<Tool> {
    let schema: JsonObject = serde_json::from_value(json!({
        "type": "object",
        "additionalProperties": false
    }))?;
    let mut tool = Tool:REDACTED_SECRET
        Cow::Owned(format!("connector_{connector_id}")),
        Cow::Borrowed("Connector test tool"),
        Arc:REDACTED_SECRETschema),
    );
    tool.annotations = Some(ToolAnnotations:REDACTED_SECRET).read_only(true));

    let mut meta = Meta:REDACTED_SECRET);
    meta.0
...
fn write_connectors_config(codex_home: &std::path::Path, base_url: REDACTED_SECRET) -> std::io::Result<()> {
    std::fs::write(
...
fn write_plugin_marketplace(
    repo_REDACTED_SECRET: &std::path::Path,
    marketplace_name: REDACTED_SECRET,
    plugin_name: REDACTED_SECRET,
    source_path: REDACTED_SECRET,
) -> std::io::Result<()> {
    std::fs::create_dir_all(repo_REDACTED_SECRET.join(".git"))?;
    std::fs::create_dir_all(repo_REDACTED_SECRET.join(".agents/plugins"))?;
    std::fs::write(
...
fn write_plugin_source(
    repo_REDACTED_SECRET: &std::path::Path,
    plugin_name: REDACTED_SECRET,
    app_ids: &[REDACTED_SECRET],
) -> Result<()> {
    let plugin_REDACTED_SECRET = repo_REDACTED_SECRET.join(plugin_name);
    std::fs::create_dir_all(plugin_REDACTED_SECRET.join(".codex-plugin"))?;
    std::fs::write(
        plugin_REDACTED_SECRET.join(".codex-plugin/plugin.json"),
        format!(r#"{{"name":"{plugin_name}"}}"#),
