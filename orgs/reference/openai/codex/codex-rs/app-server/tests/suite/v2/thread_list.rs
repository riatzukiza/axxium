...
async fn init_mcp(codex_home: &Path) -> Result<McpProcess> {
    let mut mcp = McpProcess:REDACTED_SECRETcodex_home).await?;
...
fn create_runtime_config(codex_home: &std::path::Path, server_uri: REDACTED_SECRET) -> std::io::Result<()> {
    let config_toml = codex_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
async fn thread_list_includes_git_info() -> Result<()> {
    ...
    let git_info = CoreGitInfo {
        commit_hash: Some(GitSha:REDACTED_SECRET"abc123")),
