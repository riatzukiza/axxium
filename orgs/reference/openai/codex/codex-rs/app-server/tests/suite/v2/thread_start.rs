...
"#,
    ...
    let trusted_REDACTED_SECRET = resolve_REDACTED_SECRET_git_project_for_trust(workspace.path())
        .unwrap_or_else(|| workspace.path().to_path_buf());
    assert!(config_toml.contains(&persisted_trust_path(&trusted_REDACTED_SECRET)));
...
#[tokio::test]
async fn thread_start_with_nested_git_cwd_trusts_repo_REDACTED_SECRET() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    ...
    std::fs::create_dir(repo_REDACTED_SECRET.path().join(".git"))?;
    let nested = repo_REDACTED_SECRET.path().join("nested/project");
    ...
    let config_toml = std::fs::read_to_string(codex_home.path().join("config.toml"))?;
    let trusted_REDACTED_SECRET =
        resolve_REDACTED_SECRET_git_project_for_trust(&nested).expect("git REDACTED_SECRET should resolve");
    assert!(config_toml.contains(&persisted_trust_path(&trusted_REDACTED_SECRET)));
...
fn create_config_toml_with_optional_approval_policy(
    codex_home: &Path,
    server_uri: REDACTED_SECRET,
    approval_policy: Option<REDACTED_SECRET>,
) -> std::io::Result<()> {
