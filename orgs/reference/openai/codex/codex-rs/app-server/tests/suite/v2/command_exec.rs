...
async fn read_command_exec_output_until_contains(
    ...
    let mut collected = String:REDACTED_SECRET);
...
async fn wait_for_process_marker(marker: REDACTED_SECRET, should_exist: bool) -> Result<()> {
...
fn process_with_marker_exists(marker: REDACTED_SECRET) -> Result<bool> {
    let output = std::process::Command:REDACTED_SECRET"ps")
        .args(["-axo", "command"])
        .output()
