...
async fn expect_error_message(
    mcp: &mut McpProcess,
    request_id: i64,
    expected_message: REDACTED_SECRET,
) -> Result<()> {
    let error = timeout(
...
async fn fs_methods_cover_current_fs_utils_surface() -> Result<()> {
    ...
    let source_file = source_dir.join("REDACTED_SECRET.txt");
    ...
    let REDACTED_SECRET_write_request_id = mcp
        .send_fs_write_file_request(FsWriteFileParams {
            path: absolute_path(source_file.clone()),
            data_base64: STANDARD.encode("hello from source REDACTED_SECRET"),
        })
        .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(REDACTED_SECRET_write_request_id)),
    )
    .await??;
    ...
    assert_eq!(
        ...
        vec![
            ...
            FsReadDirectoryEntry {
                file_name: "REDACTED_SECRET.txt".to_string(),
                is_directory: false,
                is_file: true,
            },
        ]
...
async fn fs_copy_ignores_unknown_special_files_in_recursive_copy() -> Result<()> {
    ...
    let output = Command:REDACTED_SECRET"mkfifo").arg(&fifo_path).output()?;
...
fn replace_file_atomically(path: &PathBuf, contents: REDACTED_SECRET) -> Result<()> {
