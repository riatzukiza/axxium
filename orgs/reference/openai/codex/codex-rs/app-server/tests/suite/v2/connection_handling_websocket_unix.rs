...
async fn send_turn_start_request(stream: &mut WsClient, id: i64, thread_id: REDACTED_SECRET) -> Result<()> {
    send_request(
        stream,
        "turn/start",
        id,
...
fn send_signal(process: &Child, signal: REDACTED_SECRET) -> Result<()> {
    let pid = process
        .id()
        .context("websocket app-server process has no pid")?;
    let status = StdCommand:REDACTED_SECRET"kill")
        .arg(signal)
        .arg(pid.to_string())
        .status()
