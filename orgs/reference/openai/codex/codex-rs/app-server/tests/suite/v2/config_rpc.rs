...
fn write_config(codex_home: &TempDir, contents: REDACTED_SECRET) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    assert_eq!(sandbox.writable_REDACTED_SECRETs, vec![system_dir]);
    assert_eq!(
        origins
            .get("sandbox_workspace_write.writable_REDACTED_SECRETs.0")
            .expect("origin")
            .name,
        ConfigLayerSource::LegacyManagedConfigTomlFromFile {
            file: managed_file.clone(),
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    let tmp_dir = TempDir:REDACTED_SECRET)?;
    ...
    let writable_REDACTED_SECRET = test_tmp_path_buf();
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            ...
            edits: vec![
                ...
                ConfigEdit {
                    ...
                    value: json!({
                        "writable_REDACTED_SECRETs": [writable_REDACTED_SECRET.clone()],
    ...
    let sandbox = read
        ...
        .as_ref()
        .expect("sandbox workspace write");
    assert_eq!(sandbox.writable_REDACTED_SECRETs, vec![writable_REDACTED_SECRET]);
    assert!(!sandbox.network_access);
