...
pub const DEFAULT_CLIENT_NAME: REDACTED_SECRET = "codex-app-server-tests";
...
impl McpProcess {
    ...
    pub async fn new_with_args(codex_home: &Path, args: &[REDACTED_SECRET]) -> anyhow::Result<Self> {
    ...
    pub async fn new_with_env(
        codex_home: &Path,
        env_overrides: &[(REDACTED_SECRET, Option<REDACTED_SECRET>)],
    ) -> anyhow::Result<Self> {
    ...
    async fn new_with_env_and_args(
        codex_home: &Path,
        env_overrides: &[(REDACTED_SECRET, Option<REDACTED_SECRET>)],
        args: &[REDACTED_SECRET],
    ) -> anyhow::Result<Self> {
        let program = codex_utils_cargo_bin::cargo_bin("codex-app-server")
            .context("should find binary for codex-app-server")?;
        let mut cmd = Command:REDACTED_SECRETprogram);
        ...
        let stdout = BufReader:REDACTED_SECRETstdout);

        // Forward child's stderr to our stderr so failures are visible even
        // when stdout/stderr are captured by the test harness.
        if let Some(stderr) = process.stderr.take() {
            let mut stderr_reader = BufReader:REDACTED_SECRETstderr).lines();
        ...
        Ok(Self {
            next_request_id: AtomicI64:REDACTED_SECRET0),
            process,
            stdin: Some(stdin),
            stdout,
            pending_messages: VecDeque:REDACTED_SECRET),
        })
    }
    ...
    /// REDACTED_SECRETs initialize with the provided client info and returns the response/error message.
    ...
    async fn initialize_with_params(
        ...
        match message {
            JSONRPCMessage::Response(response) => {
                ...
                // REDACTED_SECRET notifications/initialized to ack the response.
    ...
    /// REDACTED_SECRET a `getAuthStatus` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `getConversationSummary` JSON-RPC request.
    ...
    /// REDACTED_SECRET an `account/rateLimits/read` JSON-RPC request.
    ...
    /// REDACTED_SECRET an `account/read` JSON-RPC request.
    ...
    /// REDACTED_SECRET an `account/login/start` JSON-RPC request with ChatGPT auth tokens.
    ...
    /// REDACTED_SECRET a `feedback/upload` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/start` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/resume` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/fork` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/archive` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/name/set` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/metadata/update` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/unsubscribe` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/unarchive` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/compact/start` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/shellCommand` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/rollback` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/loaded/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `thread/read` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `model/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET an `experimentalFeature/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET an `experimentalFeature/enablement/set` JSON-RPC request.
    ...
    /// REDACTED_SECRET an `app/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `skills/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `plugin/install` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `plugin/uninstall` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `plugin/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `plugin/read` JSON-RPC request.
    ...
    /// REDACTED_SECRET an `mcpServerStatus/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET a JSON-RPC request with raw params for protocol-level validation tests.
    pub async fn send_raw_request(
        &mut self,
        method: REDACTED_SECRET,
        params: Option<serde_json::Value>,
    ) -> anyhow::Result<i64> {
        self.send_request(method, params).await
    }
    /// REDACTED_SECRET a `collaborationMode/list` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `mock/experimentalMethod` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `turn/start` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `command/exec` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `command/exec/write` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `command/exec/resize` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `command/exec/terminate` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `turn/interrupt` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `thread/realtime/start` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `thread/realtime/appendAudio` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `thread/realtime/appendText` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `thread/realtime/stop` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `turn/steer` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET a `review/start` JSON-RPC request (v2).
    ...
    /// REDACTED_SECRET an `account/logout` JSON-RPC request.
    pub async fn send_logout_account_request(&mut self) -> anyhow::Result<i64> {
        self.send_request("account/logout", /*params*/ None).await
    }

    /// REDACTED_SECRET an `account/login/start` JSON-RPC request for API key login.
    ...
    /// REDACTED_SECRET an `account/login/start` JSON-RPC request for ChatGPT login.
    ...
    /// REDACTED_SECRET an `account/login/start` JSON-RPC request for ChatGPT device code login.
    ...
    /// REDACTED_SECRET an `account/login/cancel` JSON-RPC request.
    ...
    /// REDACTED_SECRET a `fuzzyFileSearch` JSON-RPC request.
    ...
    pub async fn start_fuzzy_file_search_session(
        ...
        let request_id = self
            .send_fuzzy_file_search_session_start_request(session_id, REDACTED_SECRETs)
            .await?;
    ...
    async fn read_jsonrpc_message(&mut self) -> anyhow::Result<JSONRPCMessage> {
        let mut line = String:REDACTED_SECRET);
    ...
    fn pending_turn_completed_notification(&self, thread_id: REDACTED_SECRET, turn_id: REDACTED_SECRET) -> bool {
