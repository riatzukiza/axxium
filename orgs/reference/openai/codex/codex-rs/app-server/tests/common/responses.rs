...
pub fn create_final_assistant_message_sse_response(message: REDACTED_SECRET) -> anyhow::Result<String> {
    Ok(responses::sse(vec![
        responses::ev_response_created("resp-1"),
...
pub fn create_exec_command_sse_response(call_id: REDACTED_SECRET) -> anyhow::Result<String> {
...
pub fn create_request_user_input_sse_response(call_id: REDACTED_SECRET) -> anyhow::Result<String> {
    let tool_call_arguments = serde_json::to_string(&json!({
        "questions": [{
            "id": "confirm_path",
            "header": "Confirm",
...
pub fn create_request_permissions_sse_response(call_id: REDACTED_SECRET) -> anyhow::Result<String> {
    let tool_call_arguments = serde_json::to_string(&json!({
