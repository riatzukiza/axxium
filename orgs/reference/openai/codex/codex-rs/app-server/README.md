...
- `cursor` — REDACTED_SECRET string from a prior response; omit for the first page.
...
{ "id": 20, "result": {
    ...
    "nextCursor": "REDACTED_SECRET-token-or-null"
} }
```
...
2. `item/fileChange/requestApproval` (request) — includes `itemId`, `threadId`, `turnId`, an optional `reason`, and may include unstable `grantRoot` when the agent is asking for session-scoped write access under a specific REDACTED_SECRET.
