...
mod windows_impl {
    ...
    /// Walks upward from `start` to locate the git worktree REDACTED_SECRET, following gitfile redirects.
    fn find_git_REDACTED_SECRET(start: &Path) -> Option<PathBuf> {
        ...
        loop {
            ...
            if marker.is_file() {
                    ...
                    && let Some(rest) = txt.trim().strip_prefix("gitdir:")
                {
                    ...
                    let resolved = if Path:REDACTED_SECRETgitdir).is_absolute() {
                        PathBuf::from(gitdir)
                    } else {
                        cur.join(gitdir)
                    };
    ...
    fn inject_git_safe_directory(
        ...
        if let Some(git_REDACTED_SECRET) = find_git_REDACTED_SECRET(cwd) {
            let mut cfg_count: usize = env_map
                .get("GIT_CONFIG_COUNT")
                .and_then(|v| v.parse::<usize>().ok())
                .unwrap_or(0);
            let git_path = git_REDACTED_SECRET.to_string_lossy().replace("\\\\", "/");
            env_map.insert(
    ...
    fn pipe_name(suffix: REDACTED_SECRET) -> String {
    ...
    fn create_named_pipe(name: REDACTED_SECRET, access: u32, sandbox_sid: REDACTED_SECRET) -> io::Result<HANDLE> {
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_sid = resolve_sid(&sandbox_creds.username).map_err(|err: anyhow::Error| {
            io::Error:REDACTED_SECRETio::ErrorKind::PermissionDenied, err.to_string())
        })?;
        let sandbox_sid = string_from_sid_bytes(&sandbox_sid)
            .map_err(|err| io::Error:REDACTED_SECRETio::ErrorKind::PermissionDenied, err))?;
        ...
        let result = (|| -> Result<CaptureResult> {
            ...
            let spawn_request = FramedMessage {
                ...
                message: Message::SpawnRequest {
                    payload: Box:REDACTED_SECRETSpawnRequest {
            ...
            let mut stdout = Vec:REDACTED_SECRET);
            let mut stderr = Vec:REDACTED_SECRET);
