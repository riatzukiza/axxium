    ...
    if path.exists() {
        ...
        if t.starts_with('{') && t.ends_with('}') {
            ...
            let caps = CapSids {
                ...
                workspace_by_cwd: HashMap:REDACTED_SECRET),
            };
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        std::fs::create_dir_all(&workspace).expect("create workspace REDACTED_SECRET");

        let canonical = dunce::canonicalize(&workspace).expect("canonical workspace REDACTED_SECRET");
