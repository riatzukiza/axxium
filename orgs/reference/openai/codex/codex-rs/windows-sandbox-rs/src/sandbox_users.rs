...
pub const SANDBOX_USERS_GROUP: REDACTED_SECRET = "CodexSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: REDACTED_SECRET = "Codex sandbox internal group (managed)";
const SID_ADMINISTRATORS: REDACTED_SECRET = "S-1-5-32-544";
const SID_USERS: REDACTED_SECRET = "S-1-5-32-545";
const SID_AUTHENTICATED_USERS: REDACTED_SECRET = "S-1-5-11";
const SID_EVERYONE: REDACTED_SECRET = "S-1-1-0";
const SID_SYSTEM: REDACTED_SECRET = "S-1-5-18";
...
pub fn provision_sandbox_users(
    codex_home: &Path,
    offline_username: REDACTED_SECRET,
    online_username: REDACTED_SECRET,
    proxy_ports: &[u16],
...
pub fn ensure_sandbox_user(username: REDACTED_SECRET, password: REDACTED_SECRET, log: &mut File) -> Result<()> {
    ensure_local_user(username, password, log)?;
    ensure_local_group_member(SANDBOX_USERS_GROUP, username)?;
    Ok(())
}

pub fn ensure_local_user(name: REDACTED_SECRET, password: REDACTED_SECRET, log: &mut File) -> Result<()> {
    let name_w = to_wide(OsStr:REDACTED_SECRETname));
    let pwd_w = to_wide(OsStr:REDACTED_SECRETpassword));
    unsafe {
        ...
        if let Ok(group_name) = lookup_account_name_for_sid(SID_USERS) {
            let group = to_wide(OsStr:REDACTED_SECRET&group_name));
...
pub fn ensure_local_group(name: REDACTED_SECRET, comment: REDACTED_SECRET, log: &mut File) -> Result<()> {
    const ERROR_ALIAS_EXISTS: u32 = 1379;
    const NERR_GROUP_EXISTS: u32 = 2223;

    let name_w = to_wide(OsStr:REDACTED_SECRETname));
    let comment_w = to_wide(OsStr:REDACTED_SECRETcomment));
    unsafe {
...
pub fn ensure_local_group_member(group_name: REDACTED_SECRET, member_name: REDACTED_SECRET) -> Result<()> {
    // If the member is already in the group, NetLocalGroupAddMembers may
    // return an error code. We don't care.
    let group_w = to_wide(OsStr:REDACTED_SECRETgroup_name));
    let member_w = to_wide(OsStr:REDACTED_SECRETmember_name));
    unsafe {
        let member = LOCALGROUP_MEMBERS_INFO_3 {
...
pub fn resolve_sid(name: REDACTED_SECRET) -> Result<Vec<u8>> {
    ...
    let mut domain: Vec<u16> = Vec:REDACTED_SECRET);
...
fn well_known_sid_str(name: REDACTED_SECRET) -> Option<&'static str> {
    match name {
...
fn sid_bytes_from_string(sid_str: REDACTED_SECRET) -> Result<Vec<u8>> {
    let sid_w = to_wide(OsStr:REDACTED_SECRETsid_str));
...
fn lookup_account_name_for_sid(sid_str: REDACTED_SECRET) -> Result<String> {
    let sid_w = to_wide(OsStr:REDACTED_SECRETsid_str));
    let mut psid: *mut c_void = std::ptr::null_mut();
    if unsafe { ConvertStringSidToSidW(sid_w.as_ptr(), &mut psid) } == 0 {
        return Err(anyhow::anyhow!(
...
fn write_secrets(
    codex_home: &Path,
    offline_user: REDACTED_SECRET,
    offline_pwd: REDACTED_SECRET,
    online_user: REDACTED_SECRET,
    online_pwd: REDACTED_SECRET,
    proxy_ports: &[u16],
    allow_local_binding: bool,
) -> Result<()> {
    ...
    let marker = SetupMarker {
        ...
        allow_local_binding,
        read_REDACTED_SECRETs: Vec:REDACTED_SECRET),
        write_REDACTED_SECRETs: Vec:REDACTED_SECRET),
    };
