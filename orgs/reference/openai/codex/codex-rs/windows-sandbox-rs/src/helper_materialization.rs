...
static HELPER_PATH_CACHE: OnceLock<Mutex<HashMap<String, PathBuf>>> = OnceLock:REDACTED_SECRET);
...
pub fn resolve_current_exe_for_launch(
    codex_home: &Path,
    fallback_executable: REDACTED_SECRET,
) -> PathBuf {
...
fn cached_helper_path(cache_key: REDACTED_SECRET) -> Option<PathBuf> {
    let cache = HELPER_PATH_CACHE.get_or_init(|| Mutex:REDACTED_SECRETHashMap:REDACTED_SECRET)));
...
fn copy_from_source_if_needed(source: &Path, destination: &Path) -> Result<CopyOutcome> {
    ...
    let mut temp_file = fs::OpenOptions:REDACTED_SECRET)
        .write(true)
        .truncate(true)
        .open(&temp_path_buf)
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let codex_home = Path:REDACTED_SECRETr"C:\Users\example\.codex");
