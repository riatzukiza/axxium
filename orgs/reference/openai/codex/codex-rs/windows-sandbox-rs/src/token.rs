...
pub unsafe fn convert_string_sid_to_sid(s: REDACTED_SECRET) -> Option<*mut c_void> {
    #[link(name = "advapi32")]
...
unsafe fn enable_single_privilege(h_token: HANDLE, name: REDACTED_SECRET) -> Result<()> {
    let mut luid = LUID {
        LowPart: 0,
        HighPart: 0,
    };
