...
impl ConptyInstance {
    ...
    pub fn into_raw(self) -> (HANDLE, HANDLE, HANDLE) {
        let me = std::mem::ManuallyDrop:REDACTED_SECRETself);
...
///
/// This is REDACTED_SECRET so callers that need lower-level PTY setup can build on the same
...
pub fn create_conpty(cols: i16, rows: i16) -> Result<ConptyInstance> {
    let raw = RawConPty:REDACTED_SECRETcols, rows)?;
...
pub fn spawn_conpty_process_as_user(
    ...
    let mut attrs = ProcThreadAttributeList:REDACTED_SECRET/*attr_count*/ 1)?;
