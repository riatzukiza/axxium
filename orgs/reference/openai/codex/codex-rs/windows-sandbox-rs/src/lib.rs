...
#[cfg(target_os = "windows")]
pub use setup::run_setup_refresh_with_extra_read_REDACTED_SECRETs;
#[cfg(target_os = "windows")]
...
#[cfg(target_os = "windows")]
pub use workspace_acl::is_command_cwd_REDACTED_SECRET;
#[cfg(target_os = "windows")]
...
mod windows_impl {
    ...
    use super::workspace_acl::is_command_cwd_REDACTED_SECRET;
    ...
    pub fn run_windows_sandbox_capture(
        policy_json_or_preset: REDACTED_SECRET,
    ...
    pub fn run_windows_sandbox_capture_with_extra_deny_write_paths(
        ...
        let mut guards: Vec<(PathBuf, *mut c_void)> = Vec:REDACTED_SECRET);
        unsafe {
            for p in &allow {
                let psid = if is_workspace_write && is_command_cwd_REDACTED_SECRET(p, &canonical_cwd) {
    ...
    pub fn run_windows_sandbox_legacy_preflight(
        ...
        unsafe {
            for p in &allow {
                let psid = if is_command_cwd_REDACTED_SECRET(p, &canonical_cwd) {
                    psid_workspace
                } else {
                    psid_generic
                };
...
mod stub {
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn run_windows_sandbox_capture(
        _policy_json_or_preset: REDACTED_SECRET,
