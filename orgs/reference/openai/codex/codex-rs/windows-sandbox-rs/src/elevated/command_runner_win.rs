...
fn open_pipe(name: REDACTED_SECRET, access: u32) -> Result<HANDLE> {
...
/// REDACTED_SECRET an error frame back to the parent process.
fn send_error(writer: &Arc<StdMutex<File>>, code: REDACTED_SECRET, message: String) -> Result<()> {
...
fn spawn_ipc_process(
    ...
    let mut cap_psids: Vec<*mut c_void> = Vec:REDACTED_SECRET);
...
pub fn main() -> Result<()> {
    ...
    let pipe_write = Arc:REDACTED_SECRETStdMutex:REDACTED_SECRETunsafe {
    ...
    let process_handle = Arc:REDACTED_SECRETStdMutex:REDACTED_SECRETSome(pi.hProcess)));
