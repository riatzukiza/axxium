...
fn junction_REDACTED_SECRET_for_userprofile(userprofile: REDACTED_SECRET) -> PathBuf {
...
pub fn create_cwd_junction(requested_cwd: &Path, log_dir: Option<&Path>) -> Option<PathBuf> {
    ...
    let junction_REDACTED_SECRET = junction_REDACTED_SECRET_for_userprofile(&userprofile);
    if let Err(err) = std::fs::create_dir_all(&junction_REDACTED_SECRET) {
        log_note(
            &format!(
                "junction: failed to create {}: {err}",
                junction_REDACTED_SECRET.display()
            ),
            log_dir,
        );
        return None;
    ...
    let junction_path = junction_REDACTED_SECRET.join(junction_name_for_path(requested_cwd));
    ...
    let output = match std::process::Command:REDACTED_SECRET"cmd")
        .raw_arg("/c")
        .raw_arg("mklink")
        .raw_arg("/J")
        .raw_arg(&link_quoted)
