...
pub fn create_named_pipe(name: REDACTED_SECRET, access: u32, sandbox_username: REDACTED_SECRET) -> io::Result<HANDLE> {
    let sandbox_sid = resolve_sid(sandbox_username)
        .map_err(|err| io::Error:REDACTED_SECRETio::ErrorKind::PermissionDenied, err.to_string()))?;
