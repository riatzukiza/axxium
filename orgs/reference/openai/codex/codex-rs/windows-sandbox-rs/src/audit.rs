...
const SKIP_DIR_SUFFIXES: &[REDACTED_SECRET] = &[
    "/windows/installer",
...
fn gather_candidates(cwd: &Path, env: &std::collections::HashMap<String, String>) -> Vec<PathBuf> {
    let mut set: HashSet<PathBuf> = HashSet:REDACTED_SECRET);
    ...
    {
        for part in std::env::split_paths(OsStr:REDACTED_SECRET&path)) {
            if !part.as_os_str().is_empty() {
                unique_push(&mut set, &mut out, part);
            }
        }
    }
    // 5) Core system REDACTED_SECRETs last
...
pub fn audit_everyone_writable(
    ...
    let mut flagged: Vec<PathBuf> = Vec:REDACTED_SECRET);
    ...
    for REDACTED_SECRET in candidates {
        if start.elapsed() > Duration::from_secs(AUDIT_TIME_LIMIT_SECS as u64)
            || checked > MAX_CHECKED_LIMIT as usize
        {
            break;
        ...
        checked += 1;
        let has_REDACTED_SECRET = check_world_writable(&REDACTED_SECRET);
        if has_REDACTED_SECRET {
            let key = canonical_path_key(&REDACTED_SECRET);
            if seen.insert(key) {
                flagged.push(REDACTED_SECRET.clone());
            }
        }
        // one level down best-effort
        if let Ok(read) = std::fs::read_dir(&REDACTED_SECRET) {
            for ent in read.flatten().take(MAX_ITEMS_PER_DIR as usize) {
                let p = ent.path();
                if start.elapsed() > Duration::from_secs(AUDIT_TIME_LIMIT_SECS as u64)
                    || checked > MAX_CHECKED_LIMIT as usize
    ...
    if !flagged.is_empty() {
        let mut list = String:REDACTED_SECRET);
        for p in &flagged {
...
pub fn apply_capability_denies_for_world_writable(
    ...
    let (active_sid, workspace_REDACTED_SECRETs): (*mut c_void, Vec<PathBuf>) = match sandbox_policy {
        SandboxPolicy::WorkspaceWrite { writable_REDACTED_SECRETs, .. } => {
            ...
            let mut REDACTED_SECRETs: Vec<PathBuf> =
                vec![dunce::canonicalize(cwd).unwrap_or_else(|_| cwd.to_path_buf())];
            for REDACTED_SECRET in writable_REDACTED_SECRETs {
                let candidate = REDACTED_SECRET.as_path();
                REDACTED_SECRETs.push(dunce::canonicalize(candidate).unwrap_or_else(|_| REDACTED_SECRET.to_path_buf()));
            }
            (sid, REDACTED_SECRETs)
        }
    ...
    for path in flagged {
        if workspace_REDACTED_SECRETs.iter().any(|REDACTED_SECRET| path.starts_with(REDACTED_SECRET)) {
            continue;
        }
