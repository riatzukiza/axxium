...
use crate::setup::gather_read_REDACTED_SECRETs;
use crate::setup::gather_write_REDACTED_SECRETs;
...
pub fn require_logon_sandbox_creds(
    ...
    let needed_read = gather_read_REDACTED_SECRETs(command_cwd, policy, codex_home);
    let needed_write = gather_write_REDACTED_SECRETs(policy, policy_cwd, command_cwd, env_map);
    ...
    if identity.is_none() {
        ...
        run_elevated_setup(
            ...
            crate::setup::SetupRootOverrides {
                read_REDACTED_SECRETs: Some(needed_read),
                write_REDACTED_SECRETs: Some(needed_write),
            },
        )?;
        identity = select_identity(network_identity, codex_home)?;
    }
    // Always refresh ACLs (non-elevated) for current REDACTED_SECRETs via the setup binary.
