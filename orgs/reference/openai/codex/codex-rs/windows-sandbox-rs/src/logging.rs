...
pub const LOG_FILE_NAME: REDACTED_SECRET = "sandbox.log";

fn exe_label() -> &'static str {
    static LABEL: OnceLock<String> = OnceLock:REDACTED_SECRET);
    LABEL.get_or_init(|| {
        std::env::current_exe()
            .ok()
...
fn append_line(line: REDACTED_SECRET, base_dir: Option<&Path>) {
    if let Some(dir) = base_dir
        && let Some(path) = log_file_path(dir)
        && let Ok(mut f) = OpenOptions:REDACTED_SECRET).create(true).append(true).open(path)
    {
...
pub fn log_failure(command: &[String], detail: REDACTED_SECRET, base_dir: Option<&Path>) {
    let p = preview(command);
    log_note(&format!("FAILURE: {p} ({detail})"), base_dir);
}

// Debug logging helper. Emits only when SBX_DEBUG=1 to avoid noisy logs.
pub fn debug_log(msg: REDACTED_SECRET, base_dir: Option<&Path>) {
...
pub fn log_note(msg: REDACTED_SECRET, base_dir: Option<&Path>) {
    let ts = chrono:REDACTED_SECRET::now().format("%Y-%m-%d %H:%M:%S%.3f");
