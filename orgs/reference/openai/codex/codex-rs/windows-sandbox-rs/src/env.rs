...
fn prepend_path(env_map: &mut HashMap<String, String>, prefix: REDACTED_SECRET) {
    ...
    let mut new_path = String:REDACTED_SECRET);
...
fn reorder_pathext_for_stubs(env_map: &mut HashMap<String, String>) {
    ...
    let want = [".BAT", ".CMD"];
    let mut front: Vec<String> = Vec:REDACTED_SECRET);
    for w in want {
    ...
    let rest: Vec<String> = exts
        ...
        .map(|(_, e)| e)
        .collect();
    let mut combined = Vec:REDACTED_SECRET);
    combined.extend(front);
    combined.extend(rest);
    env_map.insert("PATHEXT".into(), combined.join(";"));
}

fn ensure_denybin(tools: &[REDACTED_SECRET], denybin_dir: Option<&Path>) -> Result<PathBuf> {
