...
def _resolve_codex_cmd() -> List[str]:
    ...
    """
    REDACTED_SECRET = Path(__file__).parent
    ws_REDACTED_SECRET = REDACTED_SECRET.parent
    cargo_target = REDACTED_SECRET"CARGO_TARGET_DIR")

    candidates = [
        ws_REDACTED_SECRET / "target" / "debug" / "codex.exe",
        ws_REDACTED_SECRET / "target" / "release" / "codex.exe",
    ]
    if cargo_target:
...
WS_ROOT = Path(REDACTED_SECRET"USERPROFILE"]) / "sbx_ws_tests"
OUTSIDE = Path(REDACTED_SECRET"USERPROFILE"]) / "sbx_ws_outside"  # outside CWD for deny checks
EXTRA_ROOT = Path(REDACTED_SECRET"USERPROFILE"]) / "WorkspaceRoot"  # additional writable REDACTED_SECRET
...
def run_sbx(
    ...
    additional_REDACTED_SECRET: Optional[Path] = None,
) -> Tuple[int, str, str]:
    env = os.environ.copy()
    env.update(ENV_BASE)
    if env_extra:
    ...
    overrides: List[str] = []
    if policy == "workspace-write" and additional_REDACTED_SECRET is not None:
        # Use config override to inject an additional writable REDACTED_SECRET.
        overrides = [
            "-c",
            f'sandbox_workspace_write.writable_REDACTED_SECRETs=["{additional_REDACTED_SECRET.as_posix()}"]',
        ]
...
def main() -> int:
    ...
    # 3b. WS: allow write in additional workspace REDACTED_SECRET
    ...
    rc, out, err = run_sbx(
        ...
        WS_ROOT,
        additional_REDACTED_SECRET=EXTRA_ROOT,
    )
    add("WS: write in additional REDACTED_SECRET allowed", rc == 0 and assert_exists(extra_target), f"rc={rc}, err={err}")

    # 3c. RO: deny write in additional workspace REDACTED_SECRET
    ...
    add(
        "RO: write in additional REDACTED_SECRET denied",
    ...
    codex_home = Path(REDACTED_SECRET"USERPROFILE"]) / ".codex"
    ...
    race_REDACTED_SECRET = WS_ROOT / "race"
    inside = race_REDACTED_SECRET / "inside"
    outside = race_REDACTED_SECRET / "outside"
    make_dir_clean(race_REDACTED_SECRET)
    inside.mkdir(parents=True, exist_ok=True)
    outside.mkdir(parents=True, exist_ok=True)
    link = race_REDACTED_SECRET / "flip"
    ...
    subprocess.Popen(toggle, cwd=str(race_REDACTED_SECRET), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo race > flip\\race.txt"], race_REDACTED_SECRET)
    ...
    # 38. WS: policy poisoning via workspace symlink REDACTED_SECRET denied
    # Simulate workspace replaced by symlink to C:\; expect writes to be denied.
    fake_REDACTED_SECRET = WS_ROOT / "fake_REDACTED_SECRET"
    if make_symlink(fake_REDACTED_SECRET, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > codex_escape.txt"], fake_REDACTED_SECRET)
        add("WS: workspace-REDACTED_SECRET symlink poisoning denied", rc != 0, f"rc={rc}")
    else:
        add("WS: workspace-REDACTED_SECRET symlink poisoning denied (setup skipped)", True, "symlink creation failed")
