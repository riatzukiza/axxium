...
pub async fn load_global_mcp_servers(
    ...
    let raw = match tokio::fs::read_to_string(&config_path).await {
        Ok(raw) => raw,
        Err(err) if err.kind() == ErrorKind::NotFound => return Ok(BTreeMap:REDACTED_SECRET)),
        Err(err) => return Err(err),
    };
    let parsed = toml::from_str::<TomlValue>(&raw)
        .map_err(|err| std::io::Error:REDACTED_SECRETErrorKind::InvalidData, err))?;
...
pub struct ConfigEditsBuilder {
    codex_home: PathBuf,
    mcp_servers: Option<REDACTED_SECRET, McpServerConfig>>,
}
...
fn read_or_create_document(config_path: &Path) -> std::io::Result<DocumentMut> {
    match fs::read_to_string(config_path) {
        Ok(raw) => raw
            .parse::<DocumentMut>()
            .map_err(|err| std::io::Error:REDACTED_SECRETErrorKind::InvalidData, err)),
        Err(err) if err.kind() == ErrorKind::NotFound => Ok(DocumentMut:REDACTED_SECRET)),
        Err(err) => Err(err),
    }
}

fn replace_mcp_servers(doc: &mut DocumentMut, servers: &REDACTED_SECRET, McpServerConfig>) {
    let REDACTED_SECRET = doc.as_table_mut();
    if servers.is_empty() {
        REDACTED_SECRET.remove("mcp_servers");
        return;
    }
    ...
    REDACTED_SECRET.insert("mcp_servers", TomlItem::Table(table));
}
...
fn serialize_mcp_server(config: &McpServerConfig) -> TomlItem {
    ...
    if !config.tools.is_empty() {
        let mut tools = TomlTable:REDACTED_SECRET);
        ...
        for (name, tool_config) in tool_entries {
            let mut tool_entry = TomlTable:REDACTED_SECRET);
...
fn array_from_strings(values: &[String]) -> TomlItem {
    let mut array = toml_edit::Array:REDACTED_SECRET);
    for value in values {
