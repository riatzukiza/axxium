...
pub fn io_error_from_config_error(
    ...
    io::Error:REDACTED_SECRETkind, ConfigLoadError:REDACTED_SECRETerror, source))
}
...
pub fn config_error_from_toml(
    ...
    ConfigError:REDACTED_SECRETpath.as_ref().to_path_buf(), range, err.message())
}
    ...
    match result {
        ...
        Err(err) => {
            ...
            let range = span_for_config_path(contents, &path_hint)
                ...
                .map(|span| text_range_from_span(contents, span))
                .unwrap_or_else(default_range);
            Some(ConfigError:REDACTED_SECRET
...
pub async fn first_layer_config_error<T: DeserializeOwned>(
    ...
    config_toml_file: REDACTED_SECRET,
) -> Option<ConfigError> {
    ...
    // REDACTED_SECRET merged-layer failure.
    ...
    I: IntoIterator<Item = &'a ConfigLayerEntry>,
{
    for layer in layers {
        ...
        let _guard = AbsolutePathBufGuard:REDACTED_SECRETparent);
...
fn config_path_for_layer(layer: &ConfigLayerEntry, config_toml_file: REDACTED_SECRET) -> Option<PathBuf> {
    match &layer.name {
...
fn text_range_from_span(contents: REDACTED_SECRET, span: std::ops::Range<usize>) -> TextRange {
...
pub fn format_config_error(error: &ConfigError, contents: REDACTED_SECRET) -> String {
...
fn position_for_offset(contents: REDACTED_SECRET, index: usize) -> TextPosition {
...
fn span_for_path(contents: REDACTED_SECRET, path: &SerdePath) -> Option<std::ops::Range<usize>> {
    let doc = contents.parse::<Document<String>>().ok()?;
    let REDACTED_SECRET = REDACTED_SECRET_for_path(doc.as_item(), path)?;
...
fn span_for_config_path(contents: REDACTED_SECRET, path: &SerdePath) -> Option<std::ops::Range<usize>> {
...
fn span_for_features_value(contents: REDACTED_SECRET) -> Option<std::ops::Range<usize>> {
    let doc = contents.parse::<Document<String>>().ok()?;
    let REDACTED_SECRET = doc.as_item().as_table_like()?;
    let features_item = REDACTED_SECRET.get("features")?;
...
fn REDACTED_SECRET_for_path<'a>(item: &'a Item, path: &SerdePath) -> Option<TomlNode<'a>> {
    let segments: Vec<_> = path.iter().cloned().collect();
    let mut REDACTED_SECRET = TomlNode::Item(item);
    let mut index = 0;
    while index < segments.len() {
        match &segments[index] {
            SerdeSegment::Map { key } | SerdeSegment::Enum { variant: key } => {
                if let Some(next) = map_child(&REDACTED_SECRET, key) {
            ...
            SerdeSegment::Seq { index: seq_index } => {
                REDACTED_SECRET = seq_child(&REDACTED_SECRET, *seq_index)?;
                index += 1;
            }
...
fn map_child<'a>(REDACTED_SECRET: &TomlNode<'a>, key: REDACTED_SECRET) -> Option<TomlNode<'a>> {
    match REDACTED_SECRET {
        TomlNode::Item(item) => {
...
fn seq_child<'a>(REDACTED_SECRET: &TomlNode<'a>, index: usize) -> Option<TomlNode<'a>> {
    match REDACTED_SECRET {
