...
impl Default for CloudRequirementsLoader {
    fn default() -> Self {
        Self:REDACTED_SECRETasync { Ok(None) })
    }
}
...
mod tests {
    ...
    async fn shared_future_runs_once() {
        let counter = Arc:REDACTED_SECRETAtomicUsize:REDACTED_SECRET0));
        let counter_clone = Arc::clone(&counter);
        let loader = CloudRequirementsLoader:REDACTED_SECRETasync move {
