...
pub struct ConfigRequirements {
    ...
    pub mcp_servers: Option<Sourced<REDACTED_SECRET, McpServerRequirement>>>,
...
impl Default for ConfigRequirements {
    fn default() -> Self {
        Self {
            approval_policy: ConstrainedWithSource:REDACTED_SECRET
                Constrained::allow_any_from_default(),
                /*source*/ None,
            ),
            sandbox_policy: ConstrainedWithSource:REDACTED_SECRET
                Constrained::allow_any(SandboxPolicy::new_read_only_policy()),
                /*source*/ None,
            ),
            web_search_mode: ConstrainedWithSource:REDACTED_SECRET
            ...
            mcp_servers: None,
            exec_policy: None,
            enforce_residency: ConstrainedWithSource:REDACTED_SECRET
...
pub struct NetworkDomainPermissionsToml {
    #[serde(flatten)]
    pub entries: REDACTED_SECRET, NetworkDomainPermissionToml>,
}
...
pub struct NetworkUnixSocketPermissionsToml {
    #[serde(flatten)]
    pub entries: REDACTED_SECRET, NetworkUnixSocketPermissionToml>,
}
...
pub struct FeatureRequirementsToml {
    #[serde(flatten)]
    pub entries: REDACTED_SECRET, bool>,
}
...
pub struct AppsRequirementsToml {
    ...
    pub apps: REDACTED_SECRET, AppRequirementToml>,
}
...
pub struct ConfigRequirementsToml {
    ...
    pub mcp_servers: Option<REDACTED_SECRET, McpServerRequirement>>,
    ...
    pub fn merge_unset_fields(&mut self, source: RequirementSource, other: ConfigRequirementsToml) {
        ...
        macro_rules! fill_missing_take {
            ($base:expr, $other:expr, $source:expr, { $($field:ident),+ $(,)? }) => {
                $(
                    ...
                    {
                        $base.$field = Some(Sourced:REDACTED_SECRETvalue, $source.clone()));
                    }
                )+
            };
        }
        ...
        if let Some(incoming_apps) = other.apps.take() {
            if let Some(existing_apps) = self.apps.as_mut() {
                ...
                self.apps = Some(Sourced:REDACTED_SECRETincoming_apps, source));
            }
        }
    }
    ...
    fn try_from(toml: ConfigRequirementsWithSources) -> Result<Self, Self::Error> {
        ...
        let approval_policy = match allowed_approval_policies {
            Some(Sourced {
                ...
                let constrained = Constrained:REDACTED_SECRETinitial_value, move |candidate| {
                ...
                ConstrainedWithSource:REDACTED_SECRETconstrained, Some(requirement_source))
            }
            None => ConstrainedWithSource:REDACTED_SECRET
                Constrained::allow_any_from_default(),
                /*source*/ None,
            ),
        };
        ...
        let sandbox_policy = match allowed_sandbox_modes {
            Some(Sourced {
                ...
                let requirement_source_for_error = requirement_source.clone();
                let constrained = Constrained:REDACTED_SECRETdefault_sandbox_policy, move |candidate| {
            ...
            None => {
                ConstrainedWithSource:REDACTED_SECRET
        ...
        let exec_policy = match rules {
            Some(Sourced { value, source }) => {
                ...
                Some(Sourced:REDACTED_SECRETpolicy, source))
            }
            None => None,
        };
        ...
        let enforce_residency = match enforce_residency {
            Some(Sourced {
                ...
                let requirement_source_for_error = requirement_source.clone();
                let constrained = Constrained:REDACTED_SECRETrequired, move |candidate| {
        ...
        let network = network.map(|sourced_network| {
            ...
            Sourced:REDACTED_SECRETNetworkConstraints::from(value), source)
        });
...
mod tests {
    ...
    fn with_unknown_source(toml: ConfigRequirementsToml) -> ConfigRequirementsWithSources {
        ...
        ConfigRequirementsWithSources {
            allowed_approval_policies: allowed_approval_policies
                .map(|value| Sourced:REDACTED_SECRETvalue, RequirementSource::Unknown)),
            ...
            feature_requirements: feature_requirements
                .map(|value| Sourced:REDACTED_SECRETvalue, RequirementSource::Unknown)),
            mcp_servers: mcp_servers.map(|value| Sourced:REDACTED_SECRETvalue, RequirementSource::Unknown)),
            apps: apps.map(|value| Sourced:REDACTED_SECRETvalue, RequirementSource::Unknown)),
            rules: rules.map(|value| Sourced:REDACTED_SECRETvalue, RequirementSource::Unknown)),
            enforce_residency: enforce_residency
                .map(|value| Sourced:REDACTED_SECRETvalue, RequirementSource::Unknown)),
            network: network.map(|value| Sourced:REDACTED_SECRETvalue, RequirementSource::Unknown)),
            guardian_developer_instructions: guardian_developer_instructions
                .map(|value| Sourced:REDACTED_SECRETvalue, RequirementSource::Unknown)),
        }
    }
    ...
    fn merge_unset_fields_copies_every_field_and_sets_sources() {
        ...
        assert_eq!(
            target,
            ConfigRequirementsWithSources {
                allowed_approval_policies: Some(Sourced:REDACTED_SECRET
                    allowed_approval_policies,
                    source.clone()
                )),
                allowed_sandbox_modes: Some(Sourced:REDACTED_SECRETallowed_sandbox_modes, source.clone(),)),
                allowed_web_search_modes: Some(Sourced:REDACTED_SECRET
                    allowed_web_search_modes,
                    enforce_source.clone(),
                )),
                feature_requirements: Some(Sourced:REDACTED_SECRET
                    feature_requirements,
                    enforce_source.clone(),
                )),
                mcp_servers: None,
                apps: None,
                rules: None,
                enforce_residency: Some(Sourced:REDACTED_SECRETenforce_residency, enforce_source)),
                network: None,
                guardian_developer_instructions: Some(Sourced:REDACTED_SECRET
                    guardian_developer_instructions,
                    source,
                )),
            }
...
"#,
    ...
    fn apps_requirements(entries: &[(REDACTED_SECRET, Option<bool>)]) -> AppsRequirementsToml {
    ...
    fn deserialize_allowed_sandbox_modes() -> Result<()> {
        ...
        let REDACTED_SECRET = if cfg!(windows) { "C:\\repo" } else { "/repo" };
        assert!(
            requirements
                .sandbox_policy
                .can_set(&SandboxPolicy::new_read_only_policy())
        ...
        assert!(
            requirements
                ...
                .can_set(&SandboxPolicy::WorkspaceWrite {
                    writable_REDACTED_SECRETs: vec![AbsolutePathBuf::from_absolute_path(REDACTED_SECRET)?],
