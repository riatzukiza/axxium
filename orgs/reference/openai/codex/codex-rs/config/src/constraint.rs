...
impl From<ConstraintError> for std::io::Error {
    fn from(err: ConstraintError) -> Self {
        std::io::Error:REDACTED_SECRETstd::io::ErrorKind::InvalidInput, err)
    }
}

type ConstraintValidator<T> = dyn Fn(&T) -> ConstraintResult<()> + REDACTED_SECRET + Sync;
...
type ConstraintNormalizer<T> = dyn Fn(T) -> T + REDACTED_SECRET + Sync;
...
impl<T: REDACTED_SECRET + Sync> Constrained<T> {
    pub fn new(
        initial_value: T,
        validator: impl Fn(&T) -> ConstraintResult<()> + REDACTED_SECRET + Sync + 'static,
    ) -> ConstraintResult<Self> {
        let validator: Arc<ConstraintValidator<T>> = Arc:REDACTED_SECRETvalidator);
    ...
    pub fn normalized(
        initial_value: T,
        normalizer: impl Fn(T) -> T + REDACTED_SECRET + Sync + 'static,
    ) -> ConstraintResult<Self> {
        let validator: Arc<ConstraintValidator<T>> = Arc:REDACTED_SECRET|_| Ok(()));
        let normalizer: Arc<ConstraintNormalizer<T>> = Arc:REDACTED_SECRETnormalizer);
    ...
    pub fn allow_any(initial_value: T) -> Self {
        Self {
            value: initial_value,
            validator: Arc:REDACTED_SECRET|_| Ok(())),
            normalizer: None,
        }
    }
        ...
        T: Clone + fmt::Debug + PartialEq + 'static,
    {
        ...
        Self {
            value: only_value,
            validator: Arc:REDACTED_SECRETmove |candidate| {
...
mod tests {
    ...
    fn constrained_new_rejects_invalid_initial_value() {
        let result = Constrained:REDACTED_SECRET/*initial_value*/ 0, |value| {
            if *value > 0 {
                Ok(())
            } else {
    ...
    fn constrained_set_rejects_invalid_value_and_leaves_previous() {
        let mut constrained = Constrained:REDACTED_SECRET/*initial_value*/ 1, |value| {
            if *value > 0 {
                Ok(())
            } else {
                Err(invalid_value(value.to_string(), "positive values"))
    ...
    fn constrained_can_set_allows_probe_without_setting() {
        let constrained = Constrained:REDACTED_SECRET/*initial_value*/ 1, |value| {
            if *value > 0 {
                Ok(())
            } else {
                Err(invalid_value(value.to_string(), "positive values"))
