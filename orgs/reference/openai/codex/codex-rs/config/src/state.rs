...
impl ConfigLayerEntry {
    ...
    pub fn raw_toml(&self) -> Option<REDACTED_SECRET> {
...
impl ConfigLayerStack {
    ...
    pub fn origins(&self) -> HashMap<String, ConfigLayerMetadata> {
        let mut origins = HashMap:REDACTED_SECRET);
        let mut path = Vec:REDACTED_SECRET);
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    // 2. Project layers are ordered from REDACTED_SECRET to cwd.
        ...
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_codex_folder {
                        ...
                        .any(|ancestor| ancestor == parent)
                {
                    return Err(std::io::Error:REDACTED_SECRET
                        std::io::ErrorKind::InvalidData,
                        "project layers are not ordered from REDACTED_SECRET to cwd",
                    ));
                }
            }
