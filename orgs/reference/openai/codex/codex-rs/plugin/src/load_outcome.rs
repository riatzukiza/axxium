...
pub struct LoadedPlugin<M> {
    ...
    pub enabled: bool,
    pub skill_REDACTED_SECRETs: Vec<PathBuf>,
...
pub fn prompt_safe_plugin_description(description: Option<REDACTED_SECRET>) -> Option<String> {
...
/// Outcome of loading configured plugins (skills REDACTED_SECRETs, MCP, apps, errors).
#[derive(Debug, Clone, PartialEq)]
...
impl<M: Clone> Default for PluginLoadOutcome<M> {
    fn default() -> Self {
        Self::from_plugins(Vec:REDACTED_SECRET))
    }
}
...
impl<M: Clone> PluginLoadOutcome<M> {
    ...
    pub fn effective_skill_REDACTED_SECRETs(&self) -> Vec<PathBuf> {
        let mut skill_REDACTED_SECRETs: Vec<PathBuf> = self
            .plugins
            .iter()
            .filter(|plugin| plugin.is_active())
            .flat_map(|plugin| plugin.skill_REDACTED_SECRETs.iter().cloned())
            .collect();
        skill_REDACTED_SECRETs.sort_unstable();
        skill_REDACTED_SECRETs.dedup();
        skill_REDACTED_SECRETs
    }
    ...
    pub fn effective_apps(&self) -> Vec<AppConnectorId> {
        let mut apps = Vec:REDACTED_SECRET);
        let mut seen_connector_ids = HashSet:REDACTED_SECRET);
...
pub trait EffectiveSkillRoots {
    fn effective_skill_REDACTED_SECRETs(&self) -> Vec<PathBuf>;
}

impl<M: Clone> EffectiveSkillRoots for PluginLoadOutcome<M> {
    fn effective_skill_REDACTED_SECRETs(&self) -> Vec<PathBuf> {
        PluginLoadOutcome::effective_skill_REDACTED_SECRETs(self)
    }
}
