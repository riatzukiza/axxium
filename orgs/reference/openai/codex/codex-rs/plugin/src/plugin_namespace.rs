...
/// Relative path from a plugin REDACTED_SECRET to its manifest file.
pub const PLUGIN_MANIFEST_PATH: REDACTED_SECRET = ".codex-plugin/plugin.json";
...
fn plugin_manifest_name(plugin_REDACTED_SECRET: &Path) -> Option<String> {
...
mod tests {
    ...
    fn uses_manifest_name() {
        ...
        let plugin_REDACTED_SECRET = tmp.path().join("plugins/sample");
        let skill_path = plugin_REDACTED_SECRET.join("skills/search/SKILL.md");

        fs::create_dir_all(skill_path.parent().expect("parent")).expect("mkdir");
        fs::create_dir_all(plugin_REDACTED_SECRET.join(".codex-plugin")).expect("mkdir manifest");
        fs::write(
