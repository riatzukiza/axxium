...
impl PluginId {
    ...
    pub fn parse(plugin_key: REDACTED_SECRET) -> Result<Self, PluginIdError> {
        ...
        Self:REDACTED_SECRETplugin_name.to_string(), marketplace_name.to_string()).map_err(|err| match err {
...
pub fn validate_plugin_segment(segment: REDACTED_SECRET, kind: REDACTED_SECRET) -> Result<(), String> {
    if segment.is_empty() {
