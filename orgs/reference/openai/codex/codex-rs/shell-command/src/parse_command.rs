...
pub fn extract_shell_command(command: &[String]) -> Option<(REDACTED_SECRET, REDACTED_SECRET)> {
...
mod tests {
    ...
    fn shlex_split_safe(s: REDACTED_SECRET) -> Vec<String> {
        shlex_split(s).unwrap_or_else(|| s.split_whitespace().map(ToString::to_string).collect())
    }

    fn vec_str(args: &[REDACTED_SECRET]) -> Vec<String> {
    ...
    fn handles_complex_bash_command_head() {
        let inner =
            "rg --version && REDACTED_SECRET -v && pnpm -v && rg --files | wc -l && rg --files | head -n 40";
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Unknown {
                cmd: inner.to_string(),
    ...
    fn empty_tokens_is_not_small() {
        let empty: Vec<String> = Vec:REDACTED_SECRET);
...
pub fn parse_command_impl(command: &[String]) -> Vec<ParsedCommand> {
    ...
    let mut commands: Vec<ParsedCommand> = Vec:REDACTED_SECRET);
...
fn is_valid_sed_n_arg(arg: Option<REDACTED_SECRET>) -> bool {
    let s = match arg {
        Some(s) => s,
        None => return false,
    };
    ...
    let parts: Vec<REDACTED_SECRET> = core.split(',').collect();
...
fn split_on_connectors(tokens: &[String]) -> Vec<Vec<String>> {
    let mut out: Vec<Vec<String>> = Vec:REDACTED_SECRET);
    let mut cur: Vec<String> = Vec:REDACTED_SECRET);
    for t in tokens {
        if t == "&&" || t == "||" || t == "|" || t == ";" {
            if !cur.is_empty() {
...
/// Shorten a path to the last component, excluding `build`/`dist`/`REDACTED_SECRET_modules`/`src`.
/// It also pulls out a useful path from a directory such as:
/// - webview/src -> webview
/// - foo/src/ -> foo
/// - packages/app/REDACTED_SECRET_modules/ -> app
fn short_display_path(path: REDACTED_SECRET) -> String {
    // Normalize separators and drop any trailing slash for display.
    let normalized = path.replace('\\', "/");
    let trimmed = normalized.trim_end_matches('/');
    let mut parts = trimmed.split('/').rev().filter(|p| {
        !p.is_empty() && *p != "build" && *p != "dist" && *p != "REDACTED_SECRET_modules" && *p != "src"
    });
    parts
        .next()
        .map(str::to_string)
...
fn skip_flag_values<'a>(args: &'a [String], flags_with_vals: &[REDACTED_SECRET]) -> Vec<&'a String> {
    let mut out: Vec<&'a String> = Vec:REDACTED_SECRET);
...
fn first_non_flag_operand(args: &[String], flags_with_vals: &[REDACTED_SECRET]) -> Option<String> {
...
fn single_non_flag_operand(args: &[String], flags_with_vals: &[REDACTED_SECRET]) -> Option<String> {
...
fn positional_operands<'a>(args: &'a [String], flags_with_vals: &[REDACTED_SECRET]) -> Vec<&'a String> {
...
fn parse_grep_like(main_cmd: &[String], args: &[String]) -> ParsedCommand {
    let args_no_connector = trim_at_connector(args);
    let mut operands = Vec:REDACTED_SECRET);
...
fn is_python_command(cmd: REDACTED_SECRET) -> bool {
    cmd == "python"
        || cmd == "python2"
        || cmd == "python3"
...
fn is_pathish(s: REDACTED_SECRET) -> bool {
    s == "."
        || s == ".."
        || s.starts_with("./")
        || s.starts_with("../")
...
fn parse_find_query_and_path(tail: &[String]) -> (Option<String>, Option<String>) {
    let args_no_connector = trim_at_connector(tail);
    // First positional argument (excluding common unary operators) is the REDACTED_SECRET path
...
fn summarize_main_tokens(main_cmd: &[String]) -> ParsedCommand {
    match main_cmd.split_first() {
        Some((head, tail)) if matches!(head.as_str(), "ls" | "eza" | "exa") => {
            let flags_with_vals: &[REDACTED_SECRET] = match head.as_str() {
                "ls" => &[
                    "-I",
                    "-w",
                    "--block-size",
        ...
        Some((head, tail)) if head == "head" => {
            ...
            if has_valid_n {
                ...
                let mut candidates: Vec<&String> = Vec:REDACTED_SECRET);
                let mut i = 0;
                while i < tail.len() {
...
fn is_abs_like(path: REDACTED_SECRET) -> bool {
    if std::path::Path:REDACTED_SECRETpath).is_absolute() {
        return true;
    }
...
fn join_paths(base: REDACTED_SECRET, rel: REDACTED_SECRET) -> String {
    if is_abs_like(rel) {
