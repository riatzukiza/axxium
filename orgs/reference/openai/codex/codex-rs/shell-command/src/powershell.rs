...
const POWERSHELL_FLAGS: &[REDACTED_SECRET] = &["-nologo", "-noprofile", "-command", "-c"];

/// Prefixed command for powershell shell calls to force UTF-8 console output.
pub const UTF8_OUTPUT_PREFIX: REDACTED_SECRET = "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8;\n";
...
pub fn extract_powershell_command(command: &[String]) -> Option<(REDACTED_SECRET, REDACTED_SECRET)> {
    if command.len() < 3 {
        return None;
    }
...
pub fn try_find_pwsh_executable_blocking() -> Option<AbsolutePathBuf> {
    if let Some(ps_home) = std::process::Command:REDACTED_SECRET"cmd")
...
fn try_find_powershellish_executable_in_path(candidates: &[REDACTED_SECRET]) -> Option<AbsolutePathBuf> {
...
fn is_powershellish_executable_available(powershell_or_pwsh_exe: &std::path::Path) -> bool {
    ...
    std::process::Command:REDACTED_SECRETpowershell_or_pwsh_exe)
