...
fn is_dangerous_cmd(command: &[String]) -> bool {
    ...
    const CMD_SEPARATORS: &[REDACTED_SECRET] = &["&", "&&", "|", "||"];
    tokens
...
fn split_embedded_cmd_operators(token: REDACTED_SECRET) -> Vec<String> {
...
fn has_force_delete_cmdlet(tokens: &[String]) -> bool {
    const DELETE_CMDLETS: &[REDACTED_SECRET] = &["remove-item", "ri", "rm", "del", "erase", "rd", "rmdir"];
    ...
    let mut segments: Vec<Vec<String>> = vec![Vec:REDACTED_SECRET)];
    for tok in tokens {
        // If token itself contains segment separators, split it (best-effort)
        let mut cur = String:REDACTED_SECRET);
        for ch in tok.chars() {
            if SEG_SEPS.contains(&ch) {
                ...
                if let Some(last) = segments.last()
                    && !last.is_empty()
                {
                    segments.push(Vec:REDACTED_SECRET));
                }
            } else {
                cur.push(ch);
            }
...
fn looks_like_url(token: REDACTED_SECRET) -> bool {
    ...
    static RE: Lazy<Option<Regex>> =
        Lazy:REDACTED_SECRET|| Regex:REDACTED_SECRETr#"^[ "'\(\s]*([^\s"'\);]+)[\s;\)]*$"#).ok());
...
fn executable_basename(exe: REDACTED_SECRET) -> Option<String> {
    Path:REDACTED_SECRETexe)
        .file_name()
...
fn is_browser_executable(name: REDACTED_SECRET) -> bool {
    matches!(
        name,
        "chrome"
            | "chrome.exe"
