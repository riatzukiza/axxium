...
fn is_git_global_option_with_value(arg: REDACTED_SECRET) -> bool {
    matches!(
        arg,
        "-C" | "-c"
            | "--config-env"
...
fn is_git_global_option_with_inline_value(arg: REDACTED_SECRET) -> bool {
    matches!(
        arg,
...
pub(crate) fn git_global_option_requires_prompt(arg: REDACTED_SECRET) -> bool {
    matches!(
        arg,
        "-c" | "--config-env"
            | "--exec-path"
...
pub(crate) fn executable_name_lookup_key(raw: REDACTED_SECRET) -> Option<String> {
    #[cfg(windows)]
    {
        Path:REDACTED_SECRETraw)
            .file_name()
