...
const POWERSHELL_PARSER_SCRIPT: REDACTED_SECRET = include_str!("powershell_parser.ps1");
...
pub(super) fn parse_with_powershell_ast(executable: REDACTED_SECRET, script: REDACTED_SECRET) -> PowershellParseOutcome {
    static PARSER_PROCESSES: LazyLock<Mutex<HashMap<String, PowershellParserProcess>>> =
        LazyLock:REDACTED_SECRET|| Mutex:REDACTED_SECRETHashMap:REDACTED_SECRET)));
...
fn encode_powershell_base64(script: REDACTED_SECRET) -> String {
...
fn encoded_parser_script() -> &'static str {
    static ENCODED: LazyLock<String> =
        LazyLock:REDACTED_SECRET|| encode_powershell_base64(POWERSHELL_PARSER_SCRIPT));
    &ENCODED
}
...
impl PowershellParserProcess {
    fn spawn(executable: REDACTED_SECRET) -> std::io::Result<Self> {
        let mut child = Command:REDACTED_SECRETexecutable)
            .args([
                "-NoLogo",
                "-NoProfile",
                "-NonInteractive",
    ...
    fn parse(&mut self, script: REDACTED_SECRET) -> std::io::Result<PowershellParseOutcome> {
...
fn deserialize_response(response_line: REDACTED_SECRET) -> std::io::Result<PowershellParserResponse> {
