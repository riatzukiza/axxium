...
fn is_safe_to_call_with_exec(command: &[String]) -> bool {
    ...
    match executable_name_lookup_key(cmd0).as_deref() {
        ...
        Some("base64") => {
            const UNSAFE_BASE64_OPTIONS: &[REDACTED_SECRET] = &["-o", "--output"];
        ...
        Some("find") => {
            ...
            #[rustfmt::skip]
            const UNSAFE_FIND_OPTIONS: &[REDACTED_SECRET] = &[
...
fn git_subcommand_args_are_read_only(args: &[String]) -> bool {
    ...
    const UNSAFE_GIT_FLAGS: &[REDACTED_SECRET] = &[
        "--output",
        "--ext-diff",
        "--textconv",
        "--exec",
...
mod tests {
    ...
    fn unknown_or_partial() {
        ...
        for args in [
            ...
            vec_str(&["find", ".", "-fprintf", "/REDACTED_SECRET/suid.txt", "%#m %u %p\n"]),
        ] {
            assert!(
