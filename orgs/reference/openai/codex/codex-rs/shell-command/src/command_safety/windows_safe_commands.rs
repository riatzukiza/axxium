...
fn parse_powershell_invocation(executable: REDACTED_SECRET, args: &[String]) -> Option<Vec<Vec<String>>> {
    if args.is_empty() {
    ...
    while idx < args.len() {
        ...
        match lower.as_str() {
            ...
            // Explicitly forbidden/REDACTED_SECRET or unnecessary for read-only operations.
...
fn parse_powershell_script(executable: REDACTED_SECRET, script: REDACTED_SECRET) -> Option<Vec<Vec<String>>> {
...
fn is_powershell_executable(exe: REDACTED_SECRET) -> bool {
    let executable_name = Path:REDACTED_SECRETexe)
        .file_name()
...
fn quote_argument(arg: REDACTED_SECRET) -> String {
    if arg.is_empty() {
        return "''".to_string();
    }
...
fn is_safe_ripgrep(words: &[String]) -> bool {
    const UNSAFE_RIPGREP_OPTIONS_WITH_ARGS: &[REDACTED_SECRET] = &["--pre", "--hostname-bin"];
    const UNSAFE_RIPGREP_OPTIONS_WITHOUT_ARGS: &[REDACTED_SECRET] = &["--search-zip", "-z"];
...
fn is_safe_git_command(words: &[String]) -> bool {
    const SAFE_SUBCOMMANDS: &[REDACTED_SECRET] = &["status", "log", "show", "diff", "cat-file"];
