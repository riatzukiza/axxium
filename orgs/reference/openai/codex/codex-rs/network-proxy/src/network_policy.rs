...
const AUDIT_TARGET: REDACTED_SECRET = "codex_otel.network_proxy";
const POLICY_DECISION_EVENT_NAME: REDACTED_SECRET = "codex.network_proxy.policy_decision";
const POLICY_SCOPE_DOMAIN: REDACTED_SECRET = "domain";
const POLICY_SCOPE_NON_DOMAIN: REDACTED_SECRET = "non_domain";
const POLICY_DECISION_ALLOW: REDACTED_SECRET = "allow";
const POLICY_DECISION_DENY: REDACTED_SECRET = "deny";
const POLICY_REASON_ALLOW: REDACTED_SECRET = "allow";
const DEFAULT_METHOD: REDACTED_SECRET = "none";
const DEFAULT_CLIENT_ADDRESS: REDACTED_SECRET = "unknown";
...
#[async_trait]
pub trait NetworkPolicyDecider: REDACTED_SECRET + Sync + 'static {
...
where
    F: Fn(NetworkPolicyRequest) -> Fut + REDACTED_SECRET + Sync + 'static,
    Fut: Future<Output = NetworkDecision> + REDACTED_SECRET,
{
    async fn decide(&self, req: NetworkPolicyRequest) -> NetworkDecision {
        (self)(req).await
    }
...
pub(crate) mod test_support {
    pub(crate) const POLICY_DECISION_EVENT_NAME: REDACTED_SECRET = super::POLICY_DECISION_EVENT_NAME;
    ...
    pub(crate) struct CapturedEvent {
        pub target: String,
        pub fields: REDACTED_SECRET, String>,
    }

    impl CapturedEvent {
        pub fn field(&self, name: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
    ...
    impl Visit for FieldVisitor {
        fn record_str(&mut self, field: &Field, value: REDACTED_SECRET) {
...
mod tests {
    ...
    const LEGACY_DOMAIN_POLICY_DECISION_EVENT_NAME: REDACTED_SECRET =
        "codex.network_proxy.domain_policy_decision";
    const LEGACY_BLOCK_DECISION_EVENT_NAME: REDACTED_SECRET = "codex.network_proxy.block_decision";
    ...
    fn state_with_metadata(metadata: NetworkProxyAuditMetadata) -> NetworkProxyState {
        ...
        let reloader = Arc:REDACTED_SECRETStaticReloader {
            state: state.clone(),
        });
        NetworkProxyState::with_reloader_and_audit_metadata(state, reloader, metadata)
    }

    fn is_rfc3339_utc_millis(timestamp: REDACTED_SECRET) -> bool {
    ...
    async fn evaluate_host_policy_emits_domain_event_for_decider_allow_override() {
        ...
        let calls = Arc:REDACTED_SECRETAtomicUsize:REDACTED_SECRET0));
        let decider: Arc<dyn NetworkPolicyDecider> = Arc:REDACTED_SECRET{
        ...
        let request = NetworkPolicyRequest:REDACTED_SECRETNetworkPolicyRequestArgs {
    ...
    async fn evaluate_host_policy_emits_domain_event_for_decider_ask() {
        ...
        let decider: Arc<dyn NetworkPolicyDecider> =
            Arc:REDACTED_SECRET|_req| async { NetworkDecision::ask(REASON_NOT_ALLOWED) });
        let request = NetworkPolicyRequest:REDACTED_SECRETNetworkPolicyRequestArgs {
            protocol: NetworkProtocol::Http,
            host: "example.com".to_string(),
            port: 80,
