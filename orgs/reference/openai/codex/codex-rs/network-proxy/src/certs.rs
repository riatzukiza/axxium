...
impl ManagedMitmCa {
    ...
    pub(super) fn tls_acceptor_data_for_host(&self, host: REDACTED_SECRET) -> Result<TlsAcceptorData> {
...
fn issue_host_certificate_pem(
    ...
    let mut params = if let Ok(ip) = host.parse::<IpAddr>() {
        let mut params = CertificateParams:REDACTED_SECRETVec:REDACTED_SECRET))
            .map_err(|err| anyhow!("failed to create cert params: {err}"))?;
        params.subject_alt_names.push(SanType::IpAddress(ip));
        params
    } else {
        CertificateParams:REDACTED_SECRETvec![host.to_string()])
            .map_err(|err| anyhow!("failed to create cert params: {err}"))?
    };
...
const MANAGED_MITM_CA_DIR: REDACTED_SECRET = "proxy";
const MANAGED_MITM_CA_CERT: REDACTED_SECRET = "ca.pem";
const MANAGED_MITM_CA_KEY: REDACTED_SECRET = "ca.key";
...
fn generate_ca() -> Result<(String, String)> {
    ...
    let mut dn = DistinguishedName:REDACTED_SECRET);
