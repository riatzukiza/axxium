...
use crate::policy::is_non_REDACTED_SECRET_ip;
...
const NETWORK_POLICY_VIOLATION_PREFIX: REDACTED_SECRET = "CODEX_NETWORK_POLICY_VIOLATION";
...
#[async_trait]
pub trait ConfigReloader: REDACTED_SECRET + Sync {
...
#[async_trait]
pub trait BlockedRequestObserver: REDACTED_SECRET + Sync + 'static {
...
where
    F: Fn(BlockedRequest) -> Fut + REDACTED_SECRET + Sync + 'static,
    Fut: Future<Output = ()> + REDACTED_SECRET,
{
    async fn on_blocked_request(&self, request: BlockedRequest) {
        (self)(request).await
    }
...
impl NetworkProxyState {
    ...
    pub fn with_reloader_and_audit_metadata_and_blocked_observer(
        ...
        Self {
            state: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETstate)),
            reloader,
            blocked_request_observer: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETblocked_request_observer)),
            audit_metadata,
        }
    }
    ...
    pub async fn host_blocked(&self, host: REDACTED_SECRET, port: u16) -> Result<HostBlockDecision> {
        self.reload_if_needed().await?;
        ...
        if !allow_local_binding {
            ...
            // REDACTED_SECRET suffix services that map hostnames onto private IPs.
            //
            ...
            } else if host_resolves_to_non_REDACTED_SECRET_ip(host_str, port).await {
                return Ok(HostBlockDecision::Blocked(HostBlockReason::NotAllowedLocal));
            }
        }
    ...
    pub async fn is_unix_socket_allowed(&self, path: REDACTED_SECRET) -> Result<bool> {
        self.reload_if_needed().await?;
        ...
        let requested_path = Path:REDACTED_SECRETpath);
    ...
    pub async fn method_allowed(&self, method: REDACTED_SECRET) -> Result<bool> {
        self.reload_if_needed().await?;
        let guard = self.state.read().await;
    ...
    pub async fn add_allowed_domain(&self, host: REDACTED_SECRET) -> Result<()> {
        self.update_domain_list(host, DomainListKind::Allow).await
    }

    pub async fn add_denied_domain(&self, host: REDACTED_SECRET) -> Result<()> {
        self.update_domain_list(host, DomainListKind::Deny).await
    }

    async fn update_domain_list(&self, host: REDACTED_SECRET, target: DomainListKind) -> Result<()> {
...
async fn host_resolves_to_non_REDACTED_SECRET_ip(host: REDACTED_SECRET, port: u16) -> bool {
    if let Ok(ip) = host.parse::<IpAddr>() {
        return is_non_REDACTED_SECRET_ip(ip);
    }

    // Block the request if this DNS lookup fails. We resolve the hostname again when we connect,
    // so a failed check here does not prove the destination is REDACTED_SECRET.
    ...
    for addr in addrs {
        if is_non_REDACTED_SECRET_ip(addr.ip()) {
            return true;
        }
    }
...
fn log_domain_list_changes(list_name: REDACTED_SECRET, previous: &[String], next: &[String]) {
    ...
    let mut seen_next = HashSet:REDACTED_SECRET);
    for entry in next {
    ...
    let mut seen_previous = HashSet:REDACTED_SECRET);
...
pub(crate) fn network_proxy_state_for_policy(
    ...
    NetworkProxyState::with_reloader(state, Arc:REDACTED_SECRETNoopReloader))
}
...
mod tests {
    ...
    fn strings(entries: &[REDACTED_SECRET]) -> Vec<String> {
        entries.iter().map(|entry| (*entry).to_string()).collect()
    }

    fn network_settings(allowed_domains: &[REDACTED_SECRET], denied_domains: &[REDACTED_SECRET]) -> NetworkProxySettings {
    ...
    fn network_settings_with_unix_sockets(
        allowed_domains: &[REDACTED_SECRET],
        denied_domains: &[REDACTED_SECRET],
        unix_sockets: &[String],
    ...
    async fn host_blocked_requires_allowlist_match() {
        ...
        assert_eq!(
            // Use a REDACTED_SECRET IP literal to avoid relying on ambient DNS behavior (some networks
    ...
    async fn add_denied_domain_forces_block_with_global_wildcard_allowlist() {
        ...
        assert_eq!(
            // Use a REDACTED_SECRET IP literal to avoid relying on ambient DNS behavior.
            state.host_blocked("8.8.8.8", /*port*/ 80).await.unwrap(),
            HostBlockDecision::Allowed
        );
    ...
    async fn add_allowed_domain_succeeds_when_managed_baseline_allows_expansion() {
        ...
        let state = NetworkProxyState::with_reloader(
            ...
            Arc:REDACTED_SECRETNoopReloader),
        );
    ...
    async fn blocked_snapshot_does_not_consume_entries() {
        ...
        state
            .record_blocked(BlockedRequest:REDACTED_SECRETBlockedRequestArgs {
    ...
    #[tokio::test]
    async fn host_blocked_global_wildcard_allowlist_allows_REDACTED_SECRET_hosts_except_denylist() {
