...
impl ReservedListeners {
    fn new(http: StdTcpListener, socks: Option<StdTcpListener>) -> Self {
        Self {
            http: Mutex:REDACTED_SECRETSome(http)),
            socks: Mutex:REDACTED_SECRETsocks),
        }
    }
...
impl ReservedListenerSet {
    ...
    fn into_reserved_listeners(self) -> Arc<ReservedListeners> {
        Arc:REDACTED_SECRETReservedListeners:REDACTED_SECRET
            self.http_listener,
            self.socks_listener,
        ))
    }
...
impl NetworkProxyBuilder {
    ...
    {
        self.policy_decider = Some(Arc:REDACTED_SECRETdecider));
        self
    }
    ...
    {
        self.blocked_request_observer = Some(Arc:REDACTED_SECRETobserver));
        self
    }
...
fn reserve_loopback_ephemeral_listeners(
    ...
    Ok(ReservedListenerSet:REDACTED_SECREThttp_listener, socks_listener))
}
...
pub const PROXY_URL_ENV_KEYS: &[REDACTED_SECRET] = &[
    "HTTP_PROXY",
    "HTTPS_PROXY",
    "WS_PROXY",
    "WSS_PROXY",
...
pub const ALL_PROXY_ENV_KEYS: &[REDACTED_SECRET] = &["ALL_PROXY", "all_proxy"];
pub const ALLOW_LOCAL_BINDING_ENV_KEY: REDACTED_SECRET = "CODEX_NETWORK_ALLOW_LOCAL_BINDING";

const FTP_PROXY_ENV_KEYS: &[REDACTED_SECRET] = &["FTP_PROXY", "ftp_proxy"];
const WEBSOCKET_PROXY_ENV_KEYS: &[REDACTED_SECRET] = &["WS_PROXY", "WSS_PROXY", "ws_proxy", "wss_proxy"];

pub const NO_PROXY_ENV_KEYS: &[REDACTED_SECRET] = &[
    "NO_PROXY",
    "no_proxy",
    "npm_config_noproxy",
    "NPM_CONFIG_NOPROXY",
...
pub const DEFAULT_NO_PROXY_VALUE: REDACTED_SECRET = concat!(
...
fn set_env_keys(env: &mut HashMap<String, String>, keys: &[REDACTED_SECRET], value: REDACTED_SECRET) {
    for key in keys {
...
mod tests {
    ...
    async fn managed_proxy_builder_uses_loopback_ports() {
        let state = Arc:REDACTED_SECRETnetwork_proxy_state_for_policy(NetworkProxySettings {
    ...
    async fn non_codex_managed_proxy_builder_uses_configured_ports() {
        ...
        let state = Arc:REDACTED_SECRETnetwork_proxy_state_for_policy(settings));
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        apply_proxy_env_overrides(
            &mut env,
            SocketAddr:REDACTED_SECRETIpAddr::V4(Ipv4Addr::LOCALHOST), 3128),
            SocketAddr:REDACTED_SECRETIpAddr::V4(Ipv4Addr::LOCALHOST), 8081),
            /*socks_enabled*/ true,
