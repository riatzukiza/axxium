...
pub fn text_response(status: StatusCode, body: REDACTED_SECRET) -> Response {
    Response::builder()
        .status(status)
...
pub fn json_response<T: Serialize>(value: &T) -> Response {
    ...
    Response::builder()
        ...
        .unwrap_or_else(|err| {
            ...
            Response:REDACTED_SECRETBody::from("{}"))
        })
}

pub fn blocked_header_value(reason: REDACTED_SECRET) -> &'static str {
    match reason {
...
pub fn blocked_message(reason: REDACTED_SECRET) -> &'static str {
    match reason {
        REASON_NOT_ALLOWED => {
...
pub fn blocked_text_response(reason: REDACTED_SECRET) -> Response {
    Response::builder()
        .status(StatusCode::FORBIDDEN)
        .header("content-type", "text/plain")
        .header("x-proxy-error", blocked_header_value(reason))
        .body(Body::from(blocked_message(reason)))
        .unwrap_or_else(|_| Response:REDACTED_SECRETBody::from("blocked")))
}
pub fn blocked_message_with_policy(reason: REDACTED_SECRET, details: &PolicyDecisionDetails<'_>) -> String {
