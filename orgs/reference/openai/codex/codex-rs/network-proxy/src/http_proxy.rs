...
pub async fn run_http_proxy(
    ...
    let listener = TcpListener::build()
        .bind(addr)
        .await
        // Rama's `BoxError` is a `Box<dyn Error + REDACTED_SECRET + Sync>` without an explicit `'static`
...
async fn run_http_proxy_with_listener(
    ...
    listener
        .serve(AddInputExtensionLayer:REDACTED_SECRETstate).into_layer(http_service))
        .await;
    Ok(())
}
...
async fn http_connect_accept(
    ...
    Ok((
        Response::builder()
            ...
            .body(Body::empty())
            .unwrap_or_else(|_| Response:REDACTED_SECRETBody::empty())),
        req,
    ))
}
...
async fn forward_connect_tunnel(
    ...
    let proxy_connector = HttpProxyConnector::optional(TcpConnector:REDACTED_SECRET));
    let tls_config = TlsConnectorDataBuilder:REDACTED_SECRET)
...
async fn proxy_via_unix_socket(req: Request, socket_path: REDACTED_SECRET) -> Result<Response> {
    #[cfg(target_os = "macos")]
    {
...
fn json_blocked(host: REDACTED_SECRET, reason: REDACTED_SECRET, details: Option<&PolicyDecisionDetails<'_>>) -> Response {
...
fn blocked_text_with_details(reason: REDACTED_SECRET, details: &PolicyDecisionDetails<'_>) -> Response {
...
async fn proxy_disabled_response(
    ...
    audit_endpoint_override: Option<(REDACTED_SECRET, u16)>,
) -> Response {
...
fn internal_error(context: REDACTED_SECRET, err: impl std::fmt::Display) -> Response {
    error!("{context}: {err}");
    text_response(StatusCode::INTERNAL_SERVER_ERROR, "error")
}

fn text_response(status: StatusCode, body: REDACTED_SECRET) -> Response {
    Response::builder()
        .status(status)
        .header("content-type", "text/plain")
        .body(Body::from(body.to_string()))
        .unwrap_or_else(|_| Response:REDACTED_SECRETBody::from(body.to_string())))
}
...
mod tests {
    ...
    async fn http_connect_accept_blocks_in_limited_mode() {
        ...
        let state = Arc:REDACTED_SECRETnetwork_proxy_state_for_policy(policy));
    ...
    async fn http_proxy_listener_accepts_plain_http1_connect_requests() {
        ...
        let state = Arc:REDACTED_SECRETnetwork_proxy_state_for_policy({
    ...
    fn remove_hop_by_hop_request_headers_keeps_forwarding_headers() {
        let mut headers = HeaderMap:REDACTED_SECRET);
        headers.insert(
            header::CONNECTION,
