...
async fn run_socks5_with_listener(
    ...
    let base = Socks5Acceptor:REDACTED_SECRET).with_connector(socks_connector);
    ...
    if enable_socks5_udp {
        ...
        listener
            .serve(AddInputExtensionLayer:REDACTED_SECRETstate).into_layer(socks_acceptor))
            .await;
    } else {
        listener
            .serve(AddInputExtensionLayer:REDACTED_SECRETstate).into_layer(base))
            .await;
    }
    Ok(())
}
...
async fn handle_socks5_tcp(
    ...
    if host.is_empty() {
        return Err(io::Error:REDACTED_SECRETio::ErrorKind::InvalidInput, "invalid host").into());
    }
...
async fn inspect_socks5_udp(
    ...
    if host.is_empty() {
        return Err(io::Error:REDACTED_SECRETio::ErrorKind::InvalidInput, "invalid host"));
    }
...
fn emit_socks_block_decision_audit_event(
    ...
    host: REDACTED_SECRET,
    port: u16,
    client_addr: Option<REDACTED_SECRET>,
) {
...
fn policy_denied_error(reason: REDACTED_SECRET, details: &PolicyDecisionDetails<'_>) -> io::Error {
...
mod tests {
    ...
    fn state_for_settings(network: NetworkProxySettings) -> Arc<NetworkProxyState> {
        ...
        Arc:REDACTED_SECRETNetworkProxyState::with_reloader(state, reloader))
    }
    ...
    async fn handle_socks5_tcp_emits_block_decision_for_proxy_disabled() {
        ...
        let mut request =
            TcpRequest:REDACTED_SECRETHostWithPort::try_from("example.com:443").expect("valid authority"));
    ...
    async fn inspect_socks5_udp_emits_block_decision_for_mode_guard_deny() {
        ...
        let request = RelayRequest {
            ...
            server_address: SocketAddress:REDACTED_SECRETIpAddr::V4(Ipv4Addr:REDACTED_SECRET93, 184, 216, 34)), 53),
