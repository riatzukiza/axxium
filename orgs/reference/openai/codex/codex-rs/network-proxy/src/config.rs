...
impl NetworkDomainPermissions {
    fn effective_entries(&self) -> Vec<NetworkDomainPermissionEntry> {
        let mut order = Vec:REDACTED_SECRET);
        let mut effective_permissions = BTreeMap:REDACTED_SECRET);
...
pub struct NetworkUnixSocketPermissions {
    #[serde(flatten)]
    pub entries: REDACTED_SECRET, NetworkUnixSocketPermission>,
}
...
impl NetworkProxySettings {
    ...
    pub fn upsert_domain_permission(
        ...
        normalize: impl Fn(REDACTED_SECRET) -> String,
    ) {
...
impl NetworkMode {
    pub fn allows_method(self, method: REDACTED_SECRET) -> bool {
        match self {
            Self::Full => true,
...
fn clamp_non_loopback(
    ...
    override_setting_name: REDACTED_SECRET,
) -> SocketAddr {
...
impl UnixStyleAbsolutePath {
    fn parse(value: REDACTED_SECRET) -> Option<Self> {
...
impl ValidatedUnixSocketPath {
    pub(crate) fn parse(socket_path: REDACTED_SECRET) -> Result<Self> {
        let path = Path:REDACTED_SECRETsocket_path);
        if path.is_absolute() {
...
fn resolve_addr(url: REDACTED_SECRET, default_port: u16) -> Result<SocketAddr> {
    ...
    match host.parse::<IpAddr>() {
        Ok(ip) => Ok(SocketAddr:REDACTED_SECRETip, addr_parts.port)),
        Err(_) => Ok(SocketAddr::from(([127, 0, 0, 1], addr_parts.port))),
    }
}

pub fn host_and_port_from_network_addr(value: REDACTED_SECRET, default_port: u16) -> String {
...
fn format_host_and_port(host: REDACTED_SECRET, port: u16) -> String {
    if host.contains(':') {
        format!("[{host}]:{port}")
    } else {
        format!("{host}:{port}")
...
fn parse_host_port(url: REDACTED_SECRET, default_port: u16) -> Result<SocketAddressParts> {
...
fn parse_host_port_fallback(input: REDACTED_SECRET, default_port: u16) -> Result<SocketAddressParts> {
...
mod tests {
    ...
    fn settings_with_unix_sockets(unix_sockets: &[REDACTED_SECRET]) -> NetworkProxySettings {
