...
impl MitmState {
    ...
    fn tls_acceptor_data_for_host(&self, host: REDACTED_SECRET) -> Result<TlsAcceptorData> {
...
pub(crate) async fn mitm_tunnel(upgraded: Upgraded) -> Result<()> {
    ...
    let request_ctx = Arc:REDACTED_SECRETMitmRequestContext {
        policy: MitmPolicyContext {
            target_host,
            target_port,
            mode,
    ...
    let https_service = TlsAcceptorLayer:REDACTED_SECRETacceptor_data)
...
fn respond_with_inspection(
    ...
    log_path: REDACTED_SECRET,
    authority: REDACTED_SECRET,
) -> Result<Response> {
    if !inspect {
        return Ok(resp);
    }
...
fn inspect_body<T: BodyLoggable + REDACTED_SECRET + 'static>(
    body: Body,
    max_body_bytes: usize,
    ctx: T,
) -> Body {
    Body::from_stream(InspectStream {
        ...
        ctx: Some(Box:REDACTED_SECRETctx)),
        len: 0,
        max_body_bytes,
    })
}
...
fn authority_header_value(host: REDACTED_SECRET, port: u16) -> String {
...
fn build_https_uri(authority: REDACTED_SECRET, path: REDACTED_SECRET) -> Result<Uri> {
