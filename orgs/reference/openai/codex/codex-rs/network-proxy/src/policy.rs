...
impl Host {
    pub fn parse(input: REDACTED_SECRET) -> Result<Self> {
...
pub fn is_non_REDACTED_SECRET_ip(ip: IpAddr) -> bool {
    match ip {
        IpAddr::V4(ip) => is_non_REDACTED_SECRET_ipv4(ip),
        IpAddr::V6(ip) => is_non_REDACTED_SECRET_ipv6(ip),
    }
}

fn is_non_REDACTED_SECRET_ipv4(ip: Ipv4Addr) -> bool {
    // Use the standard library classification helpers where possible; they encode the intent more
    // clearly than hand-rolled range checks. Some non-REDACTED_SECRET ranges (e.g., CGNAT and TEST-NET
...
fn is_non_REDACTED_SECRET_ipv6(ip: Ipv6Addr) -> bool {
    if let Some(v4) = ip.to_ipv4() {
        return is_non_REDACTED_SECRET_ipv4(v4) || ip.is_loopback();
    }
...
pub fn normalize_host(host: REDACTED_SECRET) -> String {
    let host = host.trim();
    if host.starts_with('[')
...
fn normalize_dns_host(host: REDACTED_SECRET) -> String {
    let host = host.to_ascii_lowercase();
    host.trim_end_matches('.').to_string()
}

fn normalize_pattern(pattern: REDACTED_SECRET) -> String {
...
pub(crate) fn is_global_wildcard_domain_pattern(pattern: REDACTED_SECRET) -> bool {
...
fn compile_globset_with_policy(
    ...
    let mut builder = GlobSetBuilder:REDACTED_SECRET);
    ...
    for pattern in patterns {
        ...
        for candidate in expand_domain_pattern(&pattern) {
            ...
            let glob = GlobBuilder:REDACTED_SECRET&candidate)
...
impl DomainPattern {
    ...
    pub(crate) fn parse(input: REDACTED_SECRET) -> Self {
        let input = input.trim();
        if input.is_empty() {
            return Self::Exact(String:REDACTED_SECRET));
        }
    ...
    pub(crate) fn parse_for_constraints(input: REDACTED_SECRET) -> Self {
        let input = input.trim();
        if input.is_empty() {
            return Self::Exact(String:REDACTED_SECRET));
        }
    ...
    fn parse_domain(domain: REDACTED_SECRET, build: impl FnOnce(String) -> Self) -> Self {
...
fn parse_domain_for_constraints(domain: REDACTED_SECRET) -> String {
    ...
    match UrlHost::parse(host) {
        ...
        Err(_) => String:REDACTED_SECRET),
    }
}

fn expand_domain_pattern(pattern: REDACTED_SECRET) -> Vec<String> {
...
fn normalize_domain(domain: REDACTED_SECRET) -> String {
    domain.trim_end_matches('.').to_ascii_lowercase()
}

fn domain_eq(left: REDACTED_SECRET, right: REDACTED_SECRET) -> bool {
    normalize_domain(left) == normalize_domain(right)
}

fn is_subdomain_or_equal(child: REDACTED_SECRET, parent: REDACTED_SECRET) -> bool {
...
fn is_strict_subdomain(child: REDACTED_SECRET, parent: REDACTED_SECRET) -> bool {
    let child = normalize_domain(child);
    let parent = normalize_domain(parent);
...
mod tests {
    ...
    #[test]
    fn is_non_REDACTED_SECRET_ip_rejects_private_and_loopback_ranges() {
        assert!(is_non_REDACTED_SECRET_ip("127.0.0.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("10.0.0.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("192.168.0.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("100.64.0.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("192.0.0.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("192.0.2.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("198.18.0.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("198.51.100.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("203.0.113.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("240.0.0.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("0.1.2.3".parse().unwrap()));
        assert!(!is_non_REDACTED_SECRET_ip("8.8.8.8".parse().unwrap()));

        assert!(is_non_REDACTED_SECRET_ip("::ffff:127.0.0.1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("::ffff:10.0.0.1".parse().unwrap()));
        assert!(!is_non_REDACTED_SECRET_ip("::ffff:8.8.8.8".parse().unwrap()));

        assert!(is_non_REDACTED_SECRET_ip("::1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("fe80::1".parse().unwrap()));
        assert!(is_non_REDACTED_SECRET_ip("fc00::1".parse().unwrap()));
    }
