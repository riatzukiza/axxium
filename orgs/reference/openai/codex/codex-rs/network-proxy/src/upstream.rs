...
fn read_proxy_env(keys: &[REDACTED_SECRET]) -> Option<ProxyAddress> {
    for key in keys {
...
impl UpstreamClient {
    pub(crate) fn direct() -> Self {
        Self:REDACTED_SECRETProxyConfig::default())
    }

    pub(crate) fn from_env_proxy() -> Self {
        Self:REDACTED_SECRETProxyConfig::from_env())
    }

    #[cfg(target_os = "macos")]
    pub(crate) fn unix_socket(path: REDACTED_SECRET) -> Self {
...
> {
    ...
    let tls = TlsConnectorLayer::auto()
        ...
        .into_layer(proxy);
    let tls = RequestVersionAdapter:REDACTED_SECRETtls);
    let connector = HttpConnector:REDACTED_SECRETtls);
    connector.boxed()
}
...
> {
    ...
    let connector = HttpConnector:REDACTED_SECRETtransport);
    connector.boxed()
}
