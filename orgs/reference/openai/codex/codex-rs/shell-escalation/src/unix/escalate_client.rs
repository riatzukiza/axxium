...
fn duplicate_fd_for_transfer(fd: impl AsFd, name: REDACTED_SECRET) -> anyhow::Result<OwnedFd> {
    fd.as_fd()
        .try_clone_to_owned()
...
pub async fn run_shell_escalation_execve_wrapper(
    ...
    match message.action {
        ...
        EscalateAction::Run => {
            ...
            use std::ffi::CString;
            let file = CString:REDACTED_SECRETfile).context("NUL in file")?;

            let argv_cstrs: Vec<CString> = argv
                .iter()
                .map(|s| CString:REDACTED_SECRETs.as_str()).context("NUL in argv"))
