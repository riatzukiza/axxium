...
pub trait ShellCommandExecutor: REDACTED_SECRET + Sync {
...
impl EscalateServer {
    ...
    where
        Policy: EscalationPolicy + REDACTED_SECRET + Sync + 'static,
    {
        Self {
            shell_path,
            execve_wrapper,
            policy: Arc:REDACTED_SECRETpolicy),
        }
    }
    ...
    pub fn start_session(
        ...
        let cancellation_token = CancellationToken:REDACTED_SECRET);
        ...
        let client_socket = Arc:REDACTED_SECRETMutex:REDACTED_SECRETSome(client_socket)));
...
async fn handle_escalate_session_with_policy(
    ...
    match decision {
        ...
        EscalationDecision::Escalate(execution) => {
            ...
            let mut command = Command:REDACTED_SECRETprogram);
            command
                .args(args)
...
mod tests {
    ...
    static ESCALATE_SERVER_TEST_LOCK: LazyLock<tokio::sync::Mutex<()>> =
        LazyLock:REDACTED_SECRET|| tokio::sync::Mutex:REDACTED_SECRET()));
    ...
    impl ShellCommandExecutor for ForwardingShellCommandExecutor {
        async fn run(
            ...
            _after_spawn: Option<Box<dyn FnOnce() + REDACTED_SECRET>>,
        ) -> anyhow::Result<ExecResult> {
    ...
    async fn start_session_exposes_wrapper_env_overlay() -> anyhow::Result<()> {
        ...
        let server = EscalateServer:REDACTED_SECRET
        ...
        let session = server.start_session(
            CancellationToken:REDACTED_SECRET),
            Arc:REDACTED_SECRETForwardingShellCommandExecutor),
        )?;
        let env = session.env();
    ...
    async fn exec_closes_parent_socket_after_shell_spawn() -> anyhow::Result<()> {
        let _guard = ESCALATE_SERVER_TEST_LOCK.lock().await;
        let after_spawn_invoked = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        let server = EscalateServer:REDACTED_SECRET
        ...
        let result = server
            .exec(
                ...
                CancellationToken:REDACTED_SECRET),
                Arc:REDACTED_SECRETAfterSpawnAssertingShellCommandExecutor {
    ...
    async fn handle_escalate_session_respects_run_in_sandbox_decision() -> anyhow::Result<()> {
        ...
        let server_task = tokio::spawn(handle_escalate_session_with_policy(
            server,
            Arc:REDACTED_SECRETDeterministicEscalationPolicy {
                decision: EscalationDecision::run(),
            }),
            Arc:REDACTED_SECRETForwardingShellCommandExecutor),
            CancellationToken:REDACTED_SECRET),
    ...
    -> anyhow::Result<()> {
        let _guard = ESCALATE_SERVER_TEST_LOCK.lock().await;
        let (server, client) = AsyncSocket::pair()?;
        let tmp = tempfile::TempDir:REDACTED_SECRET)?;
        ...
        let server_task = tokio::spawn(handle_escalate_session_with_policy(
            server,
            Arc:REDACTED_SECRETAssertingEscalationPolicy {
                expected_file,
    ...
    async fn handle_escalate_session_executes_escalated_command() -> anyhow::Result<()> {
        ...
        client
            .send_with_fds(SuperExecMessage { fds: Vec:REDACTED_SECRET) }, &[])
            .await?;
    ...
    async fn handle_escalate_session_passes_permissions_to_executor() -> anyhow::Result<()> {
        ...
        let server_task = tokio::spawn(handle_escalate_session_with_policy(
            ...
            Arc:REDACTED_SECRETPermissionAssertingShellCommandExecutor {
