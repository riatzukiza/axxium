...
fn extract_fds(control: &[u8]) -> Vec<OwnedFd> {
    let mut fds = Vec:REDACTED_SECRET);
    ...
    while filled < LENGTH_PREFIX_SIZE {
        ...
        let read = if !captured_control {
            match guard.try_io(|inner| {
                let mut bufs = [MaybeUninitSlice:REDACTED_SECRET&mut header[filled..])];
                let (read, control_len) = {
                    let mut msg = MsgHdrMut:REDACTED_SECRET)
...
fn send_datagram_bytes(socket: &Socket, data: &[u8], fds: &[OwnedFd]) -> std::io::Result<()> {
    ...
    let payload = [IoSlice:REDACTED_SECRETdata)];
    let msg = if control.is_empty() {
        MsgHdr:REDACTED_SECRET).with_buffers(&payload)
    } else {
        MsgHdr:REDACTED_SECRET).with_buffers(&payload).with_control(&control)
    };
...
fn receive_datagram_bytes(socket: &Socket) -> std::io::Result<(Vec<u8>, Vec<OwnedFd>)> {
    ...
    let mut control = vec![MaybeUninit::<u8>::uninit(); control_space_for_fds(MAX_FDS_PER_MESSAGE)];
    let (read, control_len) = {
        let mut bufs = [MaybeUninitSlice:REDACTED_SECRET&mut buffer)];
        let mut msg = MsgHdrMut:REDACTED_SECRET)
            .with_buffers(&mut bufs)
            .with_control(&mut control);
...
impl AsyncSocket {
    fn new(socket: Socket) -> std::io::Result<AsyncSocket> {
        ...
        let async_socket = AsyncFd:REDACTED_SECRETsocket)?;
        Ok(AsyncSocket {
            inner: async_socket,
        })
    }
    ...
    pub fn from_fd(fd: OwnedFd) -> std::io::Result<AsyncSocket> {
        AsyncSocket:REDACTED_SECRETSocket::from(fd))
    }
    ...
    pub fn pair() -> std::io::Result<(AsyncSocket, AsyncSocket)> {
        ...
        Ok((AsyncSocket:REDACTED_SECRETserver)?, AsyncSocket:REDACTED_SECRETclient)?))
    }
...
fn send_stream_chunk(
    ...
    let payload = [IoSlice:REDACTED_SECRETframe)];
    let msg = if control.is_empty() {
        MsgHdr:REDACTED_SECRET).with_buffers(&payload)
    } else {
        MsgHdr:REDACTED_SECRET).with_buffers(&payload).with_control(&control)
...
impl AsyncDatagramSocket {
    fn new(socket: Socket) -> std::io::Result<Self> {
        socket.set_nonblocking(true)?;
        Ok(Self {
            inner: AsyncFd:REDACTED_SECRETsocket)?,
        })
    }

    pub unsafe fn from_raw_fd(fd: RawFd) -> std::io::Result<Self> {
        Self:REDACTED_SECRETunsafe { Socket::from_raw_fd(fd) })
    }
    ...
    pub fn pair() -> std::io::Result<(Self, Self)> {
        ...
        server.set_cloexec(true)?;
        client.set_cloexec(true)?;
        Ok((Self:REDACTED_SECRETserver)?, Self:REDACTED_SECRETclient)?))
    }
...
mod tests {
    ...
    fn fd_list(count: usize) -> std::io::Result<Vec<OwnedFd>> {
        let file = NamedTempFile:REDACTED_SECRET)?;
        let mut fds = Vec:REDACTED_SECRET);
        for _ in 0..count {
