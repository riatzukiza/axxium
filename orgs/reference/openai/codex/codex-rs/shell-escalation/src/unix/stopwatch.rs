...
impl Stopwatch {
    pub fn new(limit: Duration) -> Self {
        Self {
            inner: Arc:REDACTED_SECRETMutex:REDACTED_SECRETStopwatchState {
                elapsed: Duration::ZERO,
                running_since: Some(Instant::now()),
                active_pauses: 0,
            })),
            notify: Arc:REDACTED_SECRETNotify:REDACTED_SECRET)),
            limit: Some(limit),
        }
    }
    ...
    pub fn cancellation_token(&self) -> CancellationToken {
        let token = CancellationToken:REDACTED_SECRET);
...
mod tests {
    ...
    async fn cancellation_receiver_fires_after_limit() {
        let stopwatch = Stopwatch:REDACTED_SECRETDuration::from_millis(50));
