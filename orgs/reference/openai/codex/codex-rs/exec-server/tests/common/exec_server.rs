...
pub(crate) async fn exec_server() -> anyhow::Result<ExecServerHarness> {
    ...
    let mut child = Command:REDACTED_SECRETbinary);
...
impl ExecServerHarness {
    pub(crate) fn websocket_url(&self) -> REDACTED_SECRET {
        &self.websocket_url
    }
    ...
    pub(crate) async fn send_raw_text(&mut self, text: REDACTED_SECRET) -> anyhow::Result<()> {
        self.websocket
...
async fn read_listen_url_from_stdout(child: &mut Child) -> anyhow::Result<String> {
    ...
    let mut lines = BufReader:REDACTED_SECRETstdout).lines();
    let deadline = Instant::now() + CONNECT_TIMEOUT;
