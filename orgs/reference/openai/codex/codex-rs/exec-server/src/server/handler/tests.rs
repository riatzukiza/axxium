...
fn exec_params(process_id: REDACTED_SECRET) -> ExecParams {
...
async fn initialized_handler() -> Arc<ExecServerHandler> {
    ...
    let handler = Arc:REDACTED_SECRETExecServerHandler:REDACTED_SECRETRpcNotificationREDACTED_SECRETer:REDACTED_SECRET
        outgoing_tx,
    )));
    assert_eq!(
