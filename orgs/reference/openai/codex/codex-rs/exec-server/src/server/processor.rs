...
pub(crate) async fn run_connection(connection: JsonRpcConnection) {
    let router = Arc:REDACTED_SECRETbuild_router());
    let (json_outgoing_tx, mut incoming_rx, connection_tasks) = connection.into_parts();
    let (outgoing_tx, mut outgoing_rx) =
        mpsc::channel::<RpcServerOutboundMessage>(CHANNEL_CAPACITY);
    let notifications = RpcNotificationREDACTED_SECRETer:REDACTED_SECREToutgoing_tx.clone());
    let handler = Arc:REDACTED_SECRETExecServerHandler:REDACTED_SECRETnotifications));
