...
impl ExecServerClient {
    ...
    pub(crate) async fn register_session(
        ...
        let state = Arc:REDACTED_SECRETSessionState:REDACTED_SECRET));
        self.inner
    ...
    async fn connect(
        ...
        let (rpc_client, mut events_rx) = RpcClient:REDACTED_SECRETconnection);
        let inner = Arc::new_cyclic(|weak| {
            ...
            Inner {
                client: rpc_client,
                sessions: ArcSwap::from_pointee(HashMap:REDACTED_SECRET)),
                sessions_write_lock: Mutex:REDACTED_SECRET()),
                reader_task,
            }
        });
...
impl SessionState {
    fn new() -> Self {
        ...
        Self {
            wake_tx,
            failure: Mutex:REDACTED_SECRETNone),
        }
    }
...
impl Inner {
    ...
    async fn insert_session(
        ...
        self.sessions.store(Arc:REDACTED_SECRETnext_sessions));
        Ok(())
    }
    ...
    async fn take_all_sessions(&self) -> HashMap<ProcessId, Arc<SessionState>> {
        ...
        self.sessions.store(Arc:REDACTED_SECRETHashMap:REDACTED_SECRET)));
        drained_sessions
    }
}

fn disconnected_message(reason: Option<REDACTED_SECRET>) -> String {
    match reason {
