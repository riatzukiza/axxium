...
use crate::local_file_system:REDACTED_SECRETFileSystem;
use crate::local_process:REDACTED_SECRETProcess;
use crate::process::ExecBackend;
use crate::remote_file_system::RemoteFileSystem;
use crate::remote_process::RemoteProcess;

pub const CODEX_EXEC_SERVER_URL_ENV_VAR: REDACTED_SECRET = "CODEX_EXEC_SERVER_URL";

pub trait ExecutorEnvironment: REDACTED_SECRET + Sync {
...
impl EnvironmentManager {
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self {
            ...
            current_environment: OnceCell:REDACTED_SECRET),
        }
    }

    pub fn from_env() -> Self {
        Self:REDACTED_SECRETstd::env::var(CODEX_EXEC_SERVER_URL_ENV_VAR).ok())
    }
...
impl Default for Environment {
    fn default() -> Self {
        ...
        Self {
            ...
            exec_backend: Arc:REDACTED_SECRETlocal_process),
        }
    }
}
...
impl Environment {
    pub async fn create(exec_server_url: Option<String>) -> Result<Self, ExecServerError> {
        ...
        let exec_backend: Arc<dyn ExecBackend> =
            if let Some(client) = remote_exec_server_client.clone() {
                Arc:REDACTED_SECRETRemoteProcess:REDACTED_SECRETclient))
            } else {
                let local_process = LocalProcess::default();
                local_process
                    .initialize()
                ...
                Arc:REDACTED_SECRETlocal_process)
            };
    ...
    pub fn get_filesystem(&self) -> Arc<dyn ExecutorFileSystem> {
        if let Some(client) = self.remote_exec_server_client.clone() {
            Arc:REDACTED_SECRETRemoteFileSystem:REDACTED_SECRETclient))
        } else {
            Arc:REDACTED_SECRETLocalFileSystem)
        }
    }
}
...
mod tests {
    ...
    fn environment_manager_normalizes_empty_url() {
        let manager = EnvironmentManager:REDACTED_SECRETSome(String:REDACTED_SECRET)));
    ...
    async fn environment_manager_current_caches_environment() {
        let manager = EnvironmentManager:REDACTED_SECRET/*exec_server_url*/ None);
