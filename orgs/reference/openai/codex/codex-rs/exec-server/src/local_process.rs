...
use crate::rpc::RpcNotificationREDACTED_SECRETer;
...
struct RunningProcess {
    ...
    wake_tx: watch::REDACTED_SECRETer<u64>,
...
struct Inner {
    notifications: RpcNotificationREDACTED_SECRETer,
...
impl Default for LocalProcess {
    fn default() -> Self {
        ...
        Self:REDACTED_SECRETRpcNotificationREDACTED_SECRETer:REDACTED_SECREToutgoing_tx))
    }
}

impl LocalProcess {
    pub(crate) fn new(notifications: RpcNotificationREDACTED_SECRETer) -> Self {
        Self {
            inner: Arc:REDACTED_SECRETInner {
                notifications,
                processes: Mutex:REDACTED_SECRETHashMap:REDACTED_SECRET)),
                initialize_requested: AtomicBool:REDACTED_SECRETfalse),
                initialized: AtomicBool:REDACTED_SECRETfalse),
            }),
        }
    }
    ...
    async fn start_process(
        &self,
        params: ExecParams,
    ) -> Result<(ExecResponse, watch::REDACTED_SECRETer<u64>), JSONRPCErrorError> {
        ...
        let (wake_tx, _wake_rx) = watch::channel(0);
        {
            ...
            process_map.insert(
                process_id.clone(),
                ProcessEntry::Running(Box:REDACTED_SECRETRunningProcess {
                    session: spawned.session,
                    tty: params.tty,
                    output: VecDeque:REDACTED_SECRET),
                    retained_bytes: 0,
                    next_seq: 1,
                    exit_code: None,
...
impl ExecBackend for LocalProcess {
    async fn start(&self, params: ExecParams) -> Result<StartedExecProcess, ExecServerError> {
        ...
        Ok(StartedExecProcess {
            process: Arc:REDACTED_SECRETLocalExecProcess {
