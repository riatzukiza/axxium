...
pub(crate) struct JsonRpcConnection {
    outgoing_tx: mpsc::REDACTED_SECRETer<JSONRPCMessage>,
...
impl JsonRpcConnection {
    ...
    where
        R: AsyncRead + Unpin + REDACTED_SECRET + 'static,
        W: AsyncWrite + Unpin + REDACTED_SECRET + 'static,
    {
        ...
        let reader_task = tokio::spawn(async move {
            let mut lines = BufReader:REDACTED_SECRETreader).lines();
            loop {
        ...
        let writer_task = tokio::spawn(async move {
            let mut writer = BufWriter:REDACTED_SECRETwriter);
    ...
    where
        S: AsyncRead + AsyncWrite + Unpin + REDACTED_SECRET + 'static,
    {
        let (outgoing_tx, mut outgoing_rx) = mpsc::channel(CHANNEL_CAPACITY);
        let (incoming_tx, incoming_rx) = mpsc::channel(CHANNEL_CAPACITY);
    ...
    pub(crate) fn into_parts(
        ...
        mpsc::REDACTED_SECRETer<JSONRPCMessage>,
...
async fn send_disconnected(
    incoming_tx: &mpsc::REDACTED_SECRETer<JsonRpcConnectionEvent>,
