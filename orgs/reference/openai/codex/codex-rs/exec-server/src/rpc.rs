...
type PendingRequest = oneshot::REDACTED_SECRETer<Result<Value, JSONRPCErrorError>>;
type BoxFuture<T> = Pin<Box<dyn Future<Output = T> + REDACTED_SECRET + 'static>>;
type RequestRoute<S> =
    Box<dyn Fn(Arc<S>, JSONRPCRequest) -> BoxFuture<RpcServerOutboundMessage> + REDACTED_SECRET + Sync>;
type NotificationRoute<S> =
    Box<dyn Fn(Arc<S>, JSONRPCNotification) -> BoxFuture<Result<(), String>> + REDACTED_SECRET + Sync>;
...
#[allow(dead_code)]
#[derive(Clone)]
pub(crate) struct RpcNotificationREDACTED_SECRETer {
    outgoing_tx: mpsc::REDACTED_SECRETer<RpcServerOutboundMessage>,
}

impl RpcNotificationREDACTED_SECRETer {
    pub(crate) fn new(outgoing_tx: mpsc::REDACTED_SECRETer<RpcServerOutboundMessage>) -> Self {
        Self { outgoing_tx }
    }
...
impl<S> Default for RpcRouter<S> {
    fn default() -> Self {
        Self {
            request_routes: HashMap:REDACTED_SECRET),
            notification_routes: HashMap:REDACTED_SECRET),
        }
    }
}
    ...
    S: REDACTED_SECRET + Sync + 'static,
{
    ...
    where
        P: DeserializeOwned + REDACTED_SECRET + 'static,
        R: Serialize + REDACTED_SECRET + 'static,
        F: Fn(Arc<S>, P) -> Fut + REDACTED_SECRET + Sync + 'static,
        Fut: Future<Output = Result<R, JSONRPCErrorError>> + REDACTED_SECRET + 'static,
    {
        self.request_routes.insert(
            method,
            Box:REDACTED_SECRETmove |state, request| {
    ...
    where
        P: DeserializeOwned + REDACTED_SECRET + 'static,
        F: Fn(Arc<S>, P) -> Fut + REDACTED_SECRET + Sync + 'static,
        Fut: Future<Output = Result<(), String>> + REDACTED_SECRET + 'static,
    {
        self.notification_routes.insert(
            method,
            Box:REDACTED_SECRETmove |state, notification| {
    ...
    pub(crate) fn request_route(&self, method: REDACTED_SECRET) -> Option<&RequestRoute<S>> {
        self.request_routes.get(method)
    }

    pub(crate) fn notification_route(&self, method: REDACTED_SECRET) -> Option<&NotificationRoute<S>> {
        self.notification_routes.get(method)
    }
}

pub(crate) struct RpcClient {
    write_tx: mpsc::REDACTED_SECRETer<JSONRPCMessage>,
...
impl RpcClient {
    pub(crate) fn new(connection: JsonRpcConnection) -> (Self, mpsc::Receiver<RpcClientEvent>) {
        ...
        let pending = Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap::<RequestId, PendingRequest>:REDACTED_SECRET)));
        ...
        (
            Self {
                write_tx,
                pending,
                next_request_id: AtomicI64:REDACTED_SECRET1),
                transport_tasks,
                reader_task,
            },
            event_rx,
    ...
    pub(crate) async fn notify<P: Serialize>(
        ...
        self.write_tx
            ...
            .await
            .map_err(|_| {
                serde_json::Error::io(std::io::Error:REDACTED_SECRET
    ...
    pub(crate) async fn call<P, T>(&self, method: REDACTED_SECRET, params: &P) -> Result<T, RpcCallError>
    where
        P: Serialize,
        T: DeserializeOwned,
    {
...
async fn handle_server_message(
    ...
    event_tx: &mpsc::REDACTED_SECRETer<RpcClientEvent>,
...
mod tests {
    ...
    async fn rpc_client_matches_out_of_order_responses_by_request_id() {
        ...
        let (client, _events_rx) = RpcClient:REDACTED_SECRETJsonRpcConnection::from_stdio(
            client_stdout,
            client_stdin,
            "test-rpc".to_string(),
        ));
        ...
        let server = tokio::spawn(async move {
            let mut lines = BufReader:REDACTED_SECRETserver_reader).lines();
