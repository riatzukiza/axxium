...
fn absolute_path(path: REDACTED_SECRET) -> AbsolutePathBuf {
    AbsolutePathBuf::from_absolute_path(Path:REDACTED_SECRETpath)).expect("absolute path")
}

fn seatbelt_policy_arg(args: &[String]) -> REDACTED_SECRET {
...
#[test]
fn base_policy_allows_REDACTED_SECRET_cpu_sysctls() {
    assert!(
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    ...
    let unreadable_REDACTED_SECRETs = file_system_policy.get_unreadable_REDACTED_SECRETs_with_cwd(Path:REDACTED_SECRET"/"));
    let unreadable_REDACTED_SECRET = unreadable_REDACTED_SECRETs.first().expect("expected unreadable REDACTED_SECRET");
    assert!(
    ...
    assert!(
        args.iter().any(
            |arg| arg == &format!("-DREADABLE_ROOT_0_EXCLUDED_0={}", unreadable_REDACTED_SECRET.display())
        ),
    ...
    assert_eq!(
        ...
        vec![
            ...
            format!("-DWRITABLE_ROOT_0_EXCLUDED_1={}", unreadable_REDACTED_SECRET.display()),
        ],
...
#[test]
fn explicit_unreadable_paths_are_excluded_from_readable_REDACTED_SECRETs() {
    let REDACTED_SECRET = absolute_path("/tmp/codex-readable");
    ...
    let policy = seatbelt_policy_arg(&args);
    let readable_REDACTED_SECRETs = file_system_policy.get_readable_REDACTED_SECRETs_with_cwd(Path:REDACTED_SECRET"/"));
    let readable_REDACTED_SECRET = readable_REDACTED_SECRETs.first().expect("expected readable REDACTED_SECRET");
    let unreadable_REDACTED_SECRETs = file_system_policy.get_unreadable_REDACTED_SECRETs_with_cwd(Path:REDACTED_SECRET"/"));
    let unreadable_REDACTED_SECRET = unreadable_REDACTED_SECRETs.first().expect("expected unreadable REDACTED_SECRET");
    assert!(
        policy.contains("(require-not (literal (param \"READABLE_ROOT_0_EXCLUDED_0\")))"),
    ...
    assert!(
        args.iter()
            .any(|arg| arg == &format!("-DREADABLE_ROOT_0={}", readable_REDACTED_SECRET.display())),
        "expected readable REDACTED_SECRET parameter in args: {args:#?}"
    );
    assert!(
        args.iter().any(
            |arg| arg == &format!("-DREADABLE_ROOT_0_EXCLUDED_0={}", unreadable_REDACTED_SECRET.display())
...
#[test]
fn seatbelt_legacy_workspace_write_nested_readable_REDACTED_SECRET_stays_writable() {
    ...
    let args = create_seatbelt_command_args(
        ...
        &SandboxPolicy::WorkspaceWrite {
            ...
            read_only_access: ReadOnlyAccess::Restricted {
                ...
                readable_REDACTED_SECRETs: vec![docs.clone()],
            },
    ...
    assert!(
        ...
        "legacy workspace-write readable REDACTED_SECRETs under cwd should not become seatbelt carveouts:\n{args:#?}",
    );
    assert!(
        args.iter()
            .any(|arg| arg.starts_with("-DWRITABLE_ROOT_0_EXCLUDED_")
                && arg.ends_with("/workspace/.codex")),
        "expected proactive .codex carveout for cwd REDACTED_SECRET: {args:#?}",
    );
}
...
fn normalize_path_for_sandbox_rejects_relative_paths() {
    assert_eq!(normalize_path_for_sandbox(Path:REDACTED_SECRET"relative.sock")), None);
}
...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    // Create a temporary workspace with two writable REDACTED_SECRETs: one containing
    // top-level .git and .codex directories and one without them.
    let tmp = TempDir:REDACTED_SECRET).expect("tempdir");
    let PopulatedTmp {
        vulnerable_REDACTED_SECRET,
        vulnerable_REDACTED_SECRET_canonical,
        dot_git_canonical,
        dot_codex_canonical,
        empty_REDACTED_SECRET,
        empty_REDACTED_SECRET_canonical,
    } = populate_tmpdir(tmp.path());
    let cwd = tmp.path().join("cwd");
    fs::create_dir_all(&cwd).expect("create cwd");

    // Build a policy that only includes the two test REDACTED_SECRETs as writable and
    // does not automatically include defaults TMPDIR or /tmp.
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![vulnerable_REDACTED_SECRET, empty_REDACTED_SECRET]
            .into_iter()
    ...
    // write to .codex/config.toml in the vulnerable REDACTED_SECRET.
    ...
    assert!(
        ...
        "expected cwd writable REDACTED_SECRET to carry protected carveouts:\n{policy_text}",
    );
    assert!(
    ...
    assert!(
        ...
        "expected explicit writable REDACTED_SECRET .git/.codex carveouts in policy:\n{policy_text}",
    );
    assert!(
        policy_text.contains("(subpath (param \"WRITABLE_ROOT_2\"))"),
        "expected second explicit writable REDACTED_SECRET grant in policy:\n{policy_text}",
    );
    ...
    let expected_definitions = [
        ...
        format!(
            "-DWRITABLE_ROOT_1={}",
            vulnerable_REDACTED_SECRET_canonical.to_string_lossy()
        ),
        format!(
        ...
        format!(
            "-DWRITABLE_ROOT_2={}",
            empty_REDACTED_SECRET_canonical.to_string_lossy()
        ),
    ];
    let writable_definitions: Vec<String> = args
        .iter()
    ...
    assert_eq!(
        ...
        "unexpected writable-REDACTED_SECRET parameter definitions in {args:#?}"
    );
    ...
    let output = Command:REDACTED_SECRETMACOS_PATH_TO_SEATBELT_EXECUTABLE)
        .args(&args)
        .current_dir(&cwd)
        .output()
    ...
    let allowed_file = vulnerable_REDACTED_SECRET_canonical.join("allowed.txt");
...
fn create_seatbelt_args_block_first_time_dot_codex_creation_with_exact_and_descendant_carveouts() {
    ...
    fs::create_dir_all(&repo_REDACTED_SECRET).expect("create repo REDACTED_SECRET");

    Command:REDACTED_SECRET"git")
        .arg("init")
        .arg(".")
        .current_dir(&repo_REDACTED_SECRET)
        .output()
        .expect("git init .");

    let dot_codex = repo_REDACTED_SECRET.join(".codex");
    let config_toml = dot_codex.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![repo_REDACTED_SECRET.as_path().try_into().expect("absolute repo REDACTED_SECRET")],
        read_only_access: Default::default(),
        network_access: false,
        exclude_tmpdir_env_var: true,
        exclude_slash_tmp: true,
...
fn create_seatbelt_args_with_read_only_git_pointer_file() {
    let tmp = TempDir:REDACTED_SECRET).expect("tempdir");
    let worktree_REDACTED_SECRET = tmp.path().join("worktree_REDACTED_SECRET");
    fs::create_dir_all(&worktree_REDACTED_SECRET).expect("create worktree_REDACTED_SECRET");
    let gitdir = worktree_REDACTED_SECRET.join("actual-gitdir");
    ...
    let dot_git = worktree_REDACTED_SECRET.join(".git");
    ...
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![worktree_REDACTED_SECRET.try_into().expect("worktree_REDACTED_SECRET is absolute")],
        read_only_access: Default::default(),
        network_access: false,
        exclude_tmpdir_env_var: true,
        exclude_slash_tmp: true,
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    // Build a policy that does not specify any writable_REDACTED_SECRETs, but does
    ...
    let args = create_seatbelt_command_args(
        shell_command.clone(),
        &policy,
        vulnerable_REDACTED_SECRET.as_path(),
        /*enforce_managed_network*/ false,
        /*network*/ None,
    );
...
struct PopulatedTmp {
    ...
    /// For the purposes of this test, we consider this a "vulnerable" REDACTED_SECRET
    ...
    vulnerable_REDACTED_SECRET: PathBuf,
    vulnerable_REDACTED_SECRET_canonical: PathBuf,
    dot_git_canonical: PathBuf,
    dot_codex_canonical: PathBuf,

    /// Path without .git or .codex subfolders.
    empty_REDACTED_SECRET: PathBuf,
    /// Canonicalized version of `empty_REDACTED_SECRET`.
    empty_REDACTED_SECRET_canonical: PathBuf,
}

fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    let vulnerable_REDACTED_SECRET = tmp.join("vulnerable_REDACTED_SECRET");
    fs::create_dir_all(&vulnerable_REDACTED_SECRET).expect("create vulnerable_REDACTED_SECRET");
    ...
    Command:REDACTED_SECRET"git")
        .arg("init")
        .arg(".")
        .current_dir(&vulnerable_REDACTED_SECRET)
        .output()
        .expect("git init .");

    fs::create_dir_all(vulnerable_REDACTED_SECRET.join(".codex")).expect("create .codex");
    fs::write(
        vulnerable_REDACTED_SECRET.join(".codex").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .codex/config.toml");

    let empty_REDACTED_SECRET = tmp.join("empty_REDACTED_SECRET");
    fs::create_dir_all(&empty_REDACTED_SECRET).expect("create empty_REDACTED_SECRET");

    // Ensure we have canonical paths for -D parameter matching.
    let vulnerable_REDACTED_SECRET_canonical = vulnerable_REDACTED_SECRET
        .canonicalize()
        .expect("canonicalize vulnerable_REDACTED_SECRET");
    let dot_git_canonical = vulnerable_REDACTED_SECRET_canonical.join(".git");
    let dot_codex_canonical = vulnerable_REDACTED_SECRET_canonical.join(".codex");
    let empty_REDACTED_SECRET_canonical = empty_REDACTED_SECRET.canonicalize().expect("canonicalize empty_REDACTED_SECRET");
    PopulatedTmp {
        vulnerable_REDACTED_SECRET,
        vulnerable_REDACTED_SECRET_canonical,
        dot_git_canonical,
