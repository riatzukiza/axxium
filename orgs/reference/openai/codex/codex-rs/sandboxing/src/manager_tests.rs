...
fn transform_additional_permissions_preserves_denied_entries() {
    ...
    let cwd = std::env::current_dir().expect("current dir");
    let temp_dir = TempDir:REDACTED_SECRET).expect("create temp dir");
    let workspace_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(
        canonicalize(temp_dir.path()).expect("canonicalize temp dir"),
    )
    .expect("absolute temp dir");
    let allowed_path = workspace_REDACTED_SECRET.join("allowed").expect("allowed path");
    let denied_path = workspace_REDACTED_SECRET.join("denied").expect("denied path");
    let exec_request = manager
        .transform(SandboxTransformRequest {
            command: SandboxCommand {
                program: "true".into(),
