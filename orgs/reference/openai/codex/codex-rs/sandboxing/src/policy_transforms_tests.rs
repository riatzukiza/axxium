...
#[test]
fn REDACTED_SECRET_write_policy_with_carveouts_still_uses_platform_sandbox() {
...
fn normalize_additional_permissions_canonicalizes_symlinked_write_paths() {
    ...
    let write_dir = real_REDACTED_SECRET.join("write");
    ...
    let link_write_dir =
        AbsolutePathBuf::from_absolute_path(link_REDACTED_SECRET.join("write")).expect("link write dir");
...
fn read_only_additional_permissions_can_enable_network_without_writes() {
    ...
    let policy = sandbox_policy_with_additional_permissions(
        &SandboxPolicy::ReadOnly {
            access: ReadOnlyAccess::Restricted {
                ...
                readable_REDACTED_SECRETs: vec![path.clone()],
            },
        ...
        &PermissionProfile {
            ...
            file_system: Some(FileSystemPermissions {
                read: Some(vec![path.clone()]),
                write: Some(Vec:REDACTED_SECRET)),
            }),
        },
    );
    ...
    assert_eq!(
        ...
        SandboxPolicy::ReadOnly {
            access: ReadOnlyAccess::Restricted {
                include_platform_defaults: true,
                readable_REDACTED_SECRETs: vec![path],
            },
            network_access: true,
        }
    );
...
#[test]
fn merge_file_system_policy_with_additional_permissions_preserves_unreadable_REDACTED_SECRETs() {
    let temp_dir = TempDir:REDACTED_SECRET).expect("create temp dir");
...
#[test]
fn effective_file_system_sandbox_policy_merges_additional_write_REDACTED_SECRETs() {
    let temp_dir = TempDir:REDACTED_SECRET).expect("create temp dir");
    let cwd = AbsolutePathBuf::from_absolute_path(
        canonicalize(temp_dir.path()).expect("canonicalize temp dir"),
    )
