...
fn intersect_permission_paths(
    ...
    requested.and_then(|requested_paths| {
        if requested_paths.is_empty() {
            return granted.map(|_| Vec:REDACTED_SECRET));
        }
...
fn normalize_permission_paths(
    ...
    _permission_kind: REDACTED_SECRET,
) -> Vec<AbsolutePathBuf> {
...
fn additional_permission_REDACTED_SECRETs(
    additional_permissions: &PermissionProfile,
...
pub fn effective_file_system_sandbox_policy(
    ...
    let (extra_reads, extra_writes) = additional_permission_REDACTED_SECRETs(additional_permissions);
...
fn merge_read_only_access_with_additional_reads(
    ...
    match read_only_access {
        ...
        ReadOnlyAccess::Restricted {
            ...
            let mut merged = readable_REDACTED_SECRETs.clone();
            merged.extend(extra_reads);
            ReadOnlyAccess::Restricted {
                include_platform_defaults: *include_platform_defaults,
                readable_REDACTED_SECRETs: dedup_absolute_paths(merged),
            }
        }
    }
}
...
fn sandbox_policy_with_additional_permissions(
    ...
    match sandbox_policy {
        ...
        SandboxPolicy::WorkspaceWrite {
            ...
            let mut merged_writes = writable_REDACTED_SECRETs.clone();
            merged_writes.extend(extra_writes);
            SandboxPolicy::WorkspaceWrite {
                writable_REDACTED_SECRETs: dedup_absolute_paths(merged_writes),
        ...
        SandboxPolicy::ReadOnly {
            ...
            if extra_writes.is_empty() {
                ...
                SandboxPolicy::WorkspaceWrite {
                    writable_REDACTED_SECRETs: dedup_absolute_paths(extra_writes),
                    read_only_access: merge_read_only_access_with_additional_reads(
                        access,
                        extra_reads,
                    ),
