...
fn write_fake_bwrap(contents: REDACTED_SECRET) -> tempfile::TempPath {
    write_fake_bwrap_in(
...
fn write_fake_bwrap_in(dir: &Path, contents: REDACTED_SECRET) -> tempfile::TempPath {
    use std::fs;
    ...
    let temp_file = NamedTempFile::new_in(dir)
        .ok()
        .unwrap_or_else(|| NamedTempFile:REDACTED_SECRET).expect("temp file"));
