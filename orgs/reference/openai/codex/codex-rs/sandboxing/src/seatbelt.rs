...
const MACOS_SEATBELT_BASE_POLICY: REDACTED_SECRET = include_str!("seatbelt_base_policy.sbpl");
const MACOS_SEATBELT_NETWORK_POLICY: REDACTED_SECRET = include_str!("seatbelt_network_policy.sbpl");
const MACOS_RESTRICTED_READ_ONLY_PLATFORM_DEFAULTS: REDACTED_SECRET =
    include_str!("restricted_read_only_platform_defaults.sbpl");

/// When working with `sandbox-exec`, only consider `sandbox-exec` in `/usr/bin`
/// to defend against an attacker trying to inject a malicious version on the
/// PATH. If /usr/bin/sandbox-exec has been tampered with, then the attacker
/// already has REDACTED_SECRET access.
pub const MACOS_PATH_TO_SEATBELT_EXECUTABLE: REDACTED_SECRET = "/usr/bin/sandbox-exec";

fn is_loopback_host(host: REDACTED_SECRET) -> bool {
    host.eq_ignore_ascii_case("localhost") || host == "127.0.0.1" || host == "::1"
}

fn proxy_scheme_default_port(scheme: REDACTED_SECRET) -> u16 {
    match scheme {
        "https" => 443,
    ...
    if let Some(network) = network {
        ...
        let unix_domain_socket_policy = if network.dangerously_allow_all_unix_sockets() {
            ...
            let allowed = network
                ...
                .iter()
                .filter_map(
                    |socket_path| match normalize_path_for_sandbox(Path:REDACTED_SECRETsocket_path)) {
...
fn unix_socket_path_params(proxy: &ProxyPolicyInputs) -> Vec<UnixSocketPathParam> {
    let mut deduped_paths: REDACTED_SECRET, AbsolutePathBuf> = BTreeMap:REDACTED_SECRET);
...
fn unix_socket_policy(proxy: &ProxyPolicyInputs) -> String {
    ...
    let mut policy = String:REDACTED_SECRET);
...
fn REDACTED_SECRET_absolute_path() -> AbsolutePathBuf {
    match AbsolutePathBuf::from_absolute_path(Path:REDACTED_SECRET"/")) {
        Ok(path) => path,
        Err(err) => panic!("REDACTED_SECRET path must be absolute: {err}"),
    }
}

#[derive(Debug, Clone)]
struct SeatbeltAccessRoot {
    REDACTED_SECRET: AbsolutePathBuf,
    excluded_subpaths: Vec<AbsolutePathBuf>,
}

fn build_seatbelt_access_policy(
    action: REDACTED_SECRET,
    param_prefix: REDACTED_SECRET,
    REDACTED_SECRETs: Vec<SeatbeltAccessRoot>,
) -> (String, Vec<(String, PathBuf)>) {
    let mut policy_components = Vec:REDACTED_SECRET);
    let mut params = Vec:REDACTED_SECRET);

    for (index, access_REDACTED_SECRET) in REDACTED_SECRETs.into_iter().enumerate() {
        let REDACTED_SECRET =
            normalize_path_for_sandbox(access_REDACTED_SECRET.REDACTED_SECRET.as_path()).unwrap_or(access_REDACTED_SECRET.REDACTED_SECRET);
        let REDACTED_SECRET_param = format!("{param_prefix}_{index}");
        params.push((REDACTED_SECRET_param.clone(), REDACTED_SECRET.into_path_buf()));

        if access_REDACTED_SECRET.excluded_subpaths.is_empty() {
            policy_components.push(format!("(subpath (param \"{REDACTED_SECRET_param}\"))"));
            continue;
        }

        let mut require_parts = vec![format!("(subpath (param \"{REDACTED_SECRET_param}\"))")];
        for (excluded_index, excluded_subpath) in
            access_REDACTED_SECRET.excluded_subpaths.into_iter().enumerate()
        {
    ...
    if policy_components.is_empty() {
        (String:REDACTED_SECRET), Vec:REDACTED_SECRET))
    } else {
        (
...
pub fn create_seatbelt_command_args_for_policies(
    ...
    let unreadable_REDACTED_SECRETs =
        file_system_sandbox_policy.get_unreadable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd);
    let (file_write_policy, file_write_dir_params) =
        if file_system_sandbox_policy.has_full_disk_write_access() {
            if unreadable_REDACTED_SECRETs.is_empty() {
                ...
                build_seatbelt_access_policy(
                    ...
                    vec![SeatbeltAccessRoot {
                        REDACTED_SECRET: REDACTED_SECRET_absolute_path(),
                        excluded_subpaths: unreadable_REDACTED_SECRETs.clone(),
                    }],
                )
            }
        } else {
            build_seatbelt_access_policy(
                ...
                file_system_sandbox_policy
                    .get_writable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
                    .into_iter()
                    .map(|REDACTED_SECRET| SeatbeltAccessRoot {
                        REDACTED_SECRET: REDACTED_SECRET.REDACTED_SECRET,
                        excluded_subpaths: REDACTED_SECRET.read_only_subpaths,
                    })
                    .collect(),
            )
        };
        ...
        if file_system_sandbox_policy.has_full_disk_read_access() {
            if unreadable_REDACTED_SECRETs.is_empty() {
                ...
                let (policy, params) = build_seatbelt_access_policy(
                    ...
                    vec![SeatbeltAccessRoot {
                        REDACTED_SECRET: REDACTED_SECRET_absolute_path(),
                        excluded_subpaths: unreadable_REDACTED_SECRETs,
                    }],
                );
                (
            ...
            let (policy, params) = build_seatbelt_access_policy(
                ...
                "READABLE_ROOT",
                file_system_sandbox_policy
                    .get_readable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
                    .into_iter()
                    .map(|REDACTED_SECRET| SeatbeltAccessRoot {
                        excluded_subpaths: unreadable_REDACTED_SECRETs
                            .iter()
                            .filter(|path| path.as_path().starts_with(REDACTED_SECRET.as_path()))
                            .cloned()
                            .collect(),
                        REDACTED_SECRET,
                    })
