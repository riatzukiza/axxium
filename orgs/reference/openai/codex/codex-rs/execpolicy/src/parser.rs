...
use starlark::environment:REDACTED_SECRETsBuilder;
...
impl PolicyParser {
    pub fn new() -> Self {
        Self {
            builder: RefCell:REDACTED_SECRETPolicyBuilder:REDACTED_SECRET)),
        }
    }

    /// Parses a policy, tagging parser errors with `policy_identifier` so failures include the
    /// identifier alongside line numbers.
    pub fn parse(&mut self, policy_identifier: REDACTED_SECRET, policy_file_contents: REDACTED_SECRET) -> Result<()> {
        ...
        let module = Module:REDACTED_SECRET);
        {
            let mut eval = Evaluator:REDACTED_SECRET&module);
...
impl PolicyBuilder {
    fn new() -> Self {
        Self {
            rules_by_program: MultiMap:REDACTED_SECRET),
            network_rules: Vec:REDACTED_SECRET),
            host_executables_by_name: HashMap:REDACTED_SECRET),
            pending_example_validations: Vec:REDACTED_SECRET),
        }
    }
    ...
    fn validate_pending_examples_from(&self, start: usize) -> Result<()> {
        for validation in &self.pending_example_validations[start..] {
            let mut rules_by_program = MultiMap:REDACTED_SECRET);
...
fn parse_literal_absolute_path(raw: REDACTED_SECRET) -> Result<AbsolutePathBuf> {
    if !Path:REDACTED_SECRETraw).is_absolute() {
...
fn validate_host_executable_name(name: REDACTED_SECRET) -> Result<()> {
    if name.is_empty() {
    ...
    let path = Path:REDACTED_SECRETname);
...
fn parse_network_rule_decision(raw: REDACTED_SECRET) -> Result<Decision> {
    match raw {
...
fn parse_string_example(raw: REDACTED_SECRET) -> Result<Vec<String>> {
...
fn policy_builtins(builder: &mut GlobalsBuilder) {
    ...
    fn host_executable<'v>(
        ...
        let mut parsed_paths = Vec:REDACTED_SECRET);
