...
impl Policy {
    pub fn new(rules_by_program: MultiMap<String, RuleRef>) -> Self {
        Self::from_parts(rules_by_program, Vec:REDACTED_SECRET), HashMap:REDACTED_SECRET))
    }
    ...
    pub fn empty() -> Self {
        Self:REDACTED_SECRETMultiMap:REDACTED_SECRET))
    }
    ...
    pub fn get_allowed_prefixes(&self) -> Vec<Vec<String>> {
        let mut prefixes = Vec:REDACTED_SECRET);
    ...
    pub fn add_prefix_rule(&mut self, prefix: &[String], decision: Decision) -> Result<()> {
        ...
        let rule: RuleRef = Arc:REDACTED_SECRETPrefixRule {
    ...
    pub fn compiled_network_domains(&self) -> (Vec<String>, Vec<String>) {
        let mut allowed = Vec:REDACTED_SECRET);
        let mut denied = Vec:REDACTED_SECRET);
...
fn upsert_domain(entries: &mut Vec<String>, host: REDACTED_SECRET) {
