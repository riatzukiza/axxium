...
impl PatternToken {
    fn matches(&self, token: REDACTED_SECRET) -> bool {
        match self {
...
impl NetworkRuleProtocol {
    pub fn parse(raw: REDACTED_SECRET) -> Result<Self> {
        match raw {
            "http" => Ok(Self::Http),
...
pub(crate) fn normalize_network_rule_host(raw: REDACTED_SECRET) -> Result<String> {
...
pub trait Rule: Any + Debug + REDACTED_SECRET + Sync {
    fn program(&self) -> REDACTED_SECRET;
...
pub(crate) fn validate_match_examples(
    ...
    let mut unmatched_examples = Vec:REDACTED_SECRET);
