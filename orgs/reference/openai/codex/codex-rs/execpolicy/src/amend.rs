...
fn append_rule_line(policy_path: &Path, rule: REDACTED_SECRET) -> Result<(), AmendError> {
    let dir = policy_path
        .parent()
...
fn append_locked_line(policy_path: &Path, line: REDACTED_SECRET) -> Result<(), AmendError> {
    let mut file = OpenOptions:REDACTED_SECRET)
        .create(true)
        .read(true)
        .append(true)
        .open(policy_path)
