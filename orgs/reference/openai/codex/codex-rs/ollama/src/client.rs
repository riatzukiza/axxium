...
use crate::url::base_url_to_host_REDACTED_SECRET;
...
const OLLAMA_CONNECTION_ERROR: REDACTED_SECRET = "No running Ollama server detected. Start it with: `ollama serve` (after installing). Install instructions: https://github.com/ollama/ollama?tab=readme-ov-file#ollama";

/// Client for interacting with a local Ollama instance.
pub struct OllamaClient {
    client: reqwest::Client,
    host_REDACTED_SECRET: String,
...
impl OllamaClient {
    ...
    #[cfg(test)]
    async fn try_from_provider_with_base_url(base_url: REDACTED_SECRET) -> io::Result<Self> {
    ...
    pub(crate) async fn try_from_provider(provider: &ModelProviderInfo) -> io::Result<Self> {
        ...
        let host_REDACTED_SECRET = base_url_to_host_REDACTED_SECRET(base_url);
        let client = reqwest::Client::builder()
            .connect_timeout(std::time::Duration::from_secs(5))
            .build()
            .unwrap_or_else(|_| reqwest::Client:REDACTED_SECRET));
        let client = Self {
            client,
            host_REDACTED_SECRET,
            uses_openai_compat,
        };
    ...
    async fn probe_server(&self) -> io::Result<()> {
        let url = if self.uses_openai_compat {
            format!("{}/v1/models", self.host_REDACTED_SECRET.trim_end_matches('/'))
        } else {
            format!("{}/api/tags", self.host_REDACTED_SECRET.trim_end_matches('/'))
        };
    ...
    pub async fn fetch_models(&self) -> io::Result<Vec<String>> {
        let tags_url = format!("{}/api/tags", self.host_REDACTED_SECRET.trim_end_matches('/'));
        let resp = self
            .client
            .get(tags_url)
            .send()
    ...
    pub async fn fetch_version(&self) -> io::Result<Option<Version>> {
        let version_url = format!("{}/api/version", self.host_REDACTED_SECRET.trim_end_matches('/'));
        let resp = self
            .client
            .get(version_url)
            .send()
    ...
    pub async fn pull_model_stream(
        ...
        let url = format!("{}/api/pull", self.host_REDACTED_SECRET.trim_end_matches('/'));
        let resp = self
            .client
            .post(url)
        ...
        let mut buf = BytesMut:REDACTED_SECRET);
        let _pending: VecDeque<PullEvent> = VecDeque:REDACTED_SECRET);
    ...
    /// Low-level constructor given a raw host REDACTED_SECRET, e.g. "http://localhost:11434".
    #[cfg(test)]
    fn from_host_REDACTED_SECRET(host_REDACTED_SECRET: impl Into<String>) -> Self {
        let client = reqwest::Client::builder()
            .connect_timeout(std::time::Duration::from_secs(5))
            .build()
            .unwrap_or_else(|_| reqwest::Client:REDACTED_SECRET));
        Self {
            client,
            host_REDACTED_SECRET: host_REDACTED_SECRET.into(),
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            ...
            .respond_with(
                wiremock::ResponseTemplate:REDACTED_SECRET200).set_body_raw(
                    serde_json::json!({
        ...
        let client = OllamaClient::from_host_REDACTED_SECRET(server.uri());
    ...
    async fn test_fetch_version() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/api/tags"))
            .respond_with(wiremock::ResponseTemplate:REDACTED_SECRET200).set_body_raw(
        ...
        assert_eq!(version, Some(Version:REDACTED_SECRET0, 14, 1)));
    }
    ...
    async fn test_probe_server_happy_path_openai_compat_and_native() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/api/tags"))
            .respond_with(wiremock::ResponseTemplate:REDACTED_SECRET200))
            .mount(&server)
            .await;
        let native = OllamaClient::from_host_REDACTED_SECRET(server.uri());
