...
pub const DEFAULT_OSS_MODEL: REDACTED_SECRET = "gpt-oss:20b";
    ...
    match ollama_client.fetch_models().await {
        Ok(models) => {
            if !models.iter().any(|m| m == model) {
                let mut reporter = crate::CliProgressReporter:REDACTED_SECRET);
                ollama_client
...
fn min_responses_version() -> Version {
    Version:REDACTED_SECRET0, 13, 4)
}

fn supports_responses(version: &Version) -> bool {
    *version == Version:REDACTED_SECRET0, 0, 0) || *version >= min_responses_version()
}
...
mod tests {
    ...
    fn supports_responses_for_dev_zero() {
        assert!(supports_responses(&Version:REDACTED_SECRET0, 0, 0)));
    }

    #[test]
    fn does_not_support_responses_before_cutoff() {
        assert!(!supports_responses(&Version:REDACTED_SECRET0, 13, 3)));
    }

    #[test]
    fn supports_responses_at_or_after_cutoff() {
        assert!(supports_responses(&Version:REDACTED_SECRET0, 13, 4)));
        assert!(supports_responses(&Version:REDACTED_SECRET0, 14, 0)));
    }
}
