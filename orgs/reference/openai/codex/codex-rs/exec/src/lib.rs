...
fn exec_REDACTED_SECRET_span() -> tracing::Span {
    info_span!(
        "codex.exec",
        otel.kind = "internal",
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let overrides = ConfigOverrides {
        ...
        js_repl_REDACTED_SECRET_module_dirs: None,
        zsh_path: None,
        ...
        additional_writable_REDACTED_SECRETs: add_dir,
    };
    ...
    let exec_span = exec_REDACTED_SECRET_span();
    ...
    let in_process_start_args = InProcessClientStartArgs {
        arg0_paths,
        config: std::sync::Arc:REDACTED_SECRETconfig.clone()),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let mut event_processor: Box<dyn EventProcessor> = match json_mode {
        true => Box:REDACTED_SECRETEventProcessorWithJsonOutput:REDACTED_SECRETlast_message_file.clone())),
        _ => Box:REDACTED_SECRETEventProcessorWithHumanOutput::create_with_ansi(
            stderr_with_ansi,
            &config,
    ...
    if !skip_git_repo_check
        && !dangerously_bypass_approvals_and_sandbox
        && get_git_repo_REDACTED_SECRET(&default_cwd).is_none()
    {
        eprintln!("Not inside a trusted directory and --skip-git-repo-check was not specified.");
        std::process::exit(1);
    }

    let mut request_ids = RequestIdSequencer:REDACTED_SECRET);
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        ...
        (Some(ExecCommand::Resume(args)), REDACTED_SECRET_prompt, imgs) => {
            let prompt_arg = args
                .prompt
                .clone()
                .or_else(|| {
        ...
        (None, REDACTED_SECRET_prompt, imgs) => {
            let prompt_text = resolve_REDACTED_SECRET_prompt(REDACTED_SECRET_prompt);
            let mut items: Vec<UserInput> = imgs
                .into_iter()
                .map(|path| UserInput:REDACTED_SECRETImage { path })
                .collect();
...
fn read_prompt_from_stdin(behavior: StdinPromptBehavior) -> Option<String> {
    ...
    let mut bytes = Vec:REDACTED_SECRET);
...
fn prompt_with_stdin_context(prompt: REDACTED_SECRET, stdin_text: REDACTED_SECRET) -> String {
...
fn resolve_REDACTED_SECRET_prompt(prompt_arg: Option<String>) -> String {
    match prompt_arg {
