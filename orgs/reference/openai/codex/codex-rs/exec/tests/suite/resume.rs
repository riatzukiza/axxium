...
fn find_session_file_containing_marker(
    ...
    for entry in WalkDir:REDACTED_SECRETsessions_dir) {
...
fn exec_repo_REDACTED_SECRET() -> anyhow::Result<std::path::PathBuf> {
    Ok(codex_utils_cargo_bin::repo_REDACTED_SECRET()?)
}
...
fn exec_resume_last_appends_to_existing_file() -> anyhow::Result<()> {
    ...
    let repo_REDACTED_SECRET = exec_repo_REDACTED_SECRET()?;
...
fn exec_resume_last_respects_cwd_filter_and_all_flag() -> anyhow::Result<()> {
    ...
    let dir_a = TempDir:REDACTED_SECRET)?;
    let dir_b = TempDir:REDACTED_SECRET)?;
