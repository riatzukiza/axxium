...
async fn spawn_command_under_sandbox(
    ...
    let codex_linux_sandbox_exe = core_test_support::find_codex_linux_sandbox_exe()
        .map_err(|err| io::Error:REDACTED_SECRETio::ErrorKind::NotFound, err))?;
...
async fn linux_sandbox_test_env() -> Option<HashMap<String, String>> {
    ...
    if can_apply_linux_sandbox_policy(&policy, &command_cwd, sandbox_cwd.as_path(), HashMap:REDACTED_SECRET))
        .await
    {
        return Some(HashMap:REDACTED_SECRET));
    }
...
async fn python_multiprocessing_lock_works_under_sandbox() {
    ...
    let sandbox_env = HashMap:REDACTED_SECRET);
    #[cfg(target_os = "macos")]
    let writable_REDACTED_SECRETs = Vec::<AbsolutePathBuf>:REDACTED_SECRET);
    ...
    #[cfg(target_os = "linux")]
    let writable_REDACTED_SECRETs: Vec<AbsolutePathBuf> = vec!["/dev/shm".try_into().unwrap()];
...
async fn python_getpwuid_works_under_sandbox() {
    ...
    if std::process::Command:REDACTED_SECRET"python3")
        .arg("--version")
        .status()
        .is_err()
    {
...
async fn sandbox_distinguishes_command_and_policy_cwds() {
    ...
    let sandbox_REDACTED_SECRET = temp.path().join("sandbox");
    let command_REDACTED_SECRET = temp.path().join("command");
    create_dir_all(&sandbox_REDACTED_SECRET).await.expect("mkdir");
    create_dir_all(&command_REDACTED_SECRET).await.expect("mkdir");
    let canonical_sandbox_REDACTED_SECRET = tokio::fs::canonicalize(&sandbox_REDACTED_SECRET)
        .await
        .expect("canonicalize sandbox REDACTED_SECRET");
    let canonical_allowed_path = canonical_sandbox_REDACTED_SECRET.join("allowed.txt");

    let disallowed_path = command_REDACTED_SECRET.join("forbidden.txt");

    // Note writable_REDACTED_SECRETs is empty: verify that `canonical_allowed_path` is
    // writable only because it is under the sandbox policy cwd, not because it
    // is under a writable REDACTED_SECRET.
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let mut child = spawn_command_under_sandbox(
        ...
        command_REDACTED_SECRET.clone(),
        &policy,
        canonical_sandbox_REDACTED_SECRET.as_path(),
        StdioPolicy::Inherit,
        sandbox_env.clone(),
    )
    .await
    ...
    .await
    .expect("should spawn command writing to sandbox REDACTED_SECRET");
...
async fn sandbox_blocks_first_time_dot_codex_creation() {
    ...
    let temp = tempfile::tempdir().expect("should be able to create temp dir");
    let repo_REDACTED_SECRET = temp.path().join("repo");
    create_dir_all(&repo_REDACTED_SECRET).await.expect("mkdir repo");
...
const IN_SANDBOX_ENV_VAR: REDACTED_SECRET = "IN_SANDBOX";

#[expect(clippy::expect_used)]
pub async fn run_code_under_sandbox<F, Fut>(
    test_selector: REDACTED_SECRET,
    policy: &SandboxPolicy,
    child_body: F,
) -> io::Result<Option<ExitStatus>>
where
    F: FnOnce() -> Fut + REDACTED_SECRET + 'static,
    Fut: Future<Output = ()> + REDACTED_SECRET + 'static,
{
