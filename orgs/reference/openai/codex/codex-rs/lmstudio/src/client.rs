...
const LMSTUDIO_CONNECTION_ERROR: REDACTED_SECRET = "LM Studio is not responding. Install from https://lmstudio.ai/download and run 'lms server start'.";
...
impl LMStudioClient {
    ...
    pub async fn load_model(&self, model: REDACTED_SECRET) -> io::Result<()> {
    ...
    pub async fn fetch_models(&self) -> io::Result<Vec<String>> {
        ...
        if response.status().is_success() {
            let json: serde_json::Value = response.json().await.map_err(|e| {
                io::Error:REDACTED_SECRETio::ErrorKind::InvalidData, format!("JSON parse error: {e}"))
            })?;
            let models = json["data"]
                .as_array()
                .ok_or_else(|| {
                    io::Error:REDACTED_SECRETio::ErrorKind::InvalidData, "No 'data' array in response")
                })?
                .iter()
    ...
    fn find_lms_with_home_dir(home_dir: Option<REDACTED_SECRET>) -> std::io::Result<String> {
        ...
        if Path:REDACTED_SECRET&fallback_path).exists() {
            Ok(fallback_path)
        } else {
    ...
    pub async fn download_model(&self, model: REDACTED_SECRET) -> std::io::Result<()> {
        let lms = Self::find_lms()?;
        eprintln!("Downloading model: {model}");

        let status = std::process::Command:REDACTED_SECRET&lms)
            .args(["get", "--yes", model])
    ...
    /// Low-level constructor given a raw host REDACTED_SECRET, e.g. "http://localhost:1234".
    #[cfg(test)]
    fn from_host_REDACTED_SECRET(host_REDACTED_SECRET: impl Into<String>) -> Self {
        ...
        Self {
            client,
            base_url: host_REDACTED_SECRET.into(),
        }
    }
}
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        ...
        let client = LMStudioClient::from_host_REDACTED_SECRET(server.uri());
    ...
    async fn test_fetch_models_no_data_array() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/models"))
            .respond_with(
                wiremock::ResponseTemplate:REDACTED_SECRET200)
    ...
    async fn test_fetch_models_server_error() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/models"))
            .respond_with(wiremock::ResponseTemplate:REDACTED_SECRET500))
            .mount(&server)
            .await;
    ...
    async fn test_check_server_error() {
        ...
        wiremock::Mock::given(wiremock::matchers::method("GET"))
            .and(wiremock::matchers::path("/models"))
            .respond_with(wiremock::ResponseTemplate:REDACTED_SECRET404))
            .mount(&server)
            .await;
    ...
    #[test]
    fn test_from_host_REDACTED_SECRET() {
        let client = LMStudioClient::from_host_REDACTED_SECRET("http://localhost:1234");
        assert_eq!(client.base_url, "http://localhost:1234");

        let client = LMStudioClient::from_host_REDACTED_SECRET("https://example.com:8080/api");
