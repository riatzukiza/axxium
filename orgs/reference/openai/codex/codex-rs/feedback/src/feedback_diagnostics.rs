...
pub const FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME: REDACTED_SECRET = "codex-connectivity-diagnostics.txt";
const PROXY_ENV_VARS: &[REDACTED_SECRET] = &[
    "HTTP_PROXY",
    "http_proxy",
    "HTTPS_PROXY",
    "https_proxy",
...
impl FeedbackDiagnostics {
        ...
        V: Into<String>,
    {
        let env = pairs
            ...
            .collect::<HashMap<_, _>>();
        let mut diagnostics = Vec:REDACTED_SECRET);
    ...
    pub fn attachment_text(&self) -> Option<String> {
        ...
        let mut lines = vec!["Connectivity diagnostics".to_string(), String:REDACTED_SECRET)];
...
- Proxy environment variables are set and may affect connectivity.
  ...
  - all_proxy = socks5h://all-proxy.example.com:1080"#
    ...
    fn collect_from_pairs_ignores_absent_values() {
        let diagnostics = FeedbackDiagnostics::collect_from_pairs(Vec::<(String, String)>:REDACTED_SECRET));
