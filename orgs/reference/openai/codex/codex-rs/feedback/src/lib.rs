...
const SENTRY_DSN: REDACTED_SECRET =
    "https://ae32ed50620d7a7792c1ce5df38b3e3e@o33249.ingest.us.sentry.io/4510195390611458";
const UPLOAD_TIMEOUT_SECS: u64 = 10;
const FEEDBACK_TAGS_TARGET: REDACTED_SECRET = "feedback_tags";
...
impl CodexFeedback {
    ...
    pub(crate) fn with_capacity(max_bytes: usize) -> Self {
        Self {
            inner: Arc:REDACTED_SECRETFeedbackInner:REDACTED_SECRETmax_bytes)),
        }
    }
    ...
    pub fn logger_layer<S>(&self) -> impl Layer<S> + REDACTED_SECRET + Sync + 'static
    where
        S: tracing::Subscriber + for<'a> LookupSpan<'a>,
    {
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(Targets:REDACTED_SECRET).with_default(Level::TRACE))
    }
    ...
    /// uploads later.
    pub fn metadata_layer<S>(&self) -> impl Layer<S> + REDACTED_SECRET + Sync + 'static
    where
        S: tracing::Subscriber + for<'a> LookupSpan<'a>,
    {
        ...
        .with_filter(Targets:REDACTED_SECRET).with_target(FEEDBACK_TAGS_TARGET, Level::TRACE))
    }
    ...
    pub fn snapshot(&self, session_id: Option<ThreadId>) -> FeedbackSnapshot {
        ...
        FeedbackSnapshot {
            ...
            thread_id: session_id
                .map(|id| id.to_string())
                .unwrap_or("no-active-thread-".to_string() + &ThreadId:REDACTED_SECRET).to_string()),
        }
    }
}
...
struct FeedbackInner {
    ...
    tags: Mutex<REDACTED_SECRET, String>>,
}

impl FeedbackInner {
    fn new(max_bytes: usize) -> Self {
        Self {
            ring: Mutex:REDACTED_SECRETRingBuffer:REDACTED_SECRETmax_bytes)),
            tags: Mutex:REDACTED_SECRETBTreeMap:REDACTED_SECRET)),
        }
    }
}
...
impl FeedbackSnapshot {
    ...
    pub fn upload_feedback(
        &self,
        classification: REDACTED_SECRET,
        reason: Option<REDACTED_SECRET>,
        include_logs: bool,
        ...
        let client = Client::from_config(ClientOptions {
            ...
            transport: Some(Arc:REDACTED_SECRETDefaultTransportFactory {})),
            ..Default::default()
        });
        ...
        let mut envelope = Envelope:REDACTED_SECRET);
        let title = format!(
            "[{}]: Codex session {}",
    ...
    fn feedback_attachments(
        ...
        let mut attachments = Vec:REDACTED_SECRET);
...
fn display_classification(classification: REDACTED_SECRET) -> String {
...
impl Visit for FeedbackTagsVisitor {
    ...
    fn record_str(&mut self, field: &tracing::field::Field, value: REDACTED_SECRET) {
        self.tags
            .insert(field.name().to_string(), value.to_string());
    }
...
mod tests {
    ...
    fn metadata_layer_records_tags_from_feedback_target() {
        let fb = CodexFeedback:REDACTED_SECRET);
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("codex-feedback-extra-{}.jsonl", ThreadId:REDACTED_SECRET));
        let extra_path = std::env::temp_dir().join(&extra_filename);
        fs::write(&extra_path, "rollout").expect("extra attachment should be written");

        let snapshot_with_diagnostics = CodexFeedback:REDACTED_SECRET)
            .snapshot(/*session_id*/ None)
            .with_feedback_diagnostics(FeedbackDiagnostics:REDACTED_SECRETvec![FeedbackDiagnostic {
        ...
        assert_eq!(
            OsStr:REDACTED_SECRETattachments_with_diagnostics[2].filename.as_str()),
            OsStr:REDACTED_SECRETextra_filename.as_str())
        );
        let attachments_without_diagnostics = CodexFeedback:REDACTED_SECRET)
            .snapshot(/*session_id*/ None)
