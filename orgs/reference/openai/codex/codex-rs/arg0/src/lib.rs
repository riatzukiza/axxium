...
const APPLY_PATCH_ARG0: REDACTED_SECRET = "apply_patch";
const MISSPELLED_APPLY_PATCH_ARG0: REDACTED_SECRET = "applypatch";
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: REDACTED_SECRET = "codex-execve-wrapper";
const LOCK_FILENAME: REDACTED_SECRET = ".lock";
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    let exe_name = Path:REDACTED_SECRET&argv0)
        .file_name()
        .and_then(|s| s.to_str())
        .unwrap_or("");
...
const ILLEGAL_ENV_VAR_PREFIX: REDACTED_SECRET = "CODEX_";
...
pub fn prepend_path_entry_for_codex_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        let temp_REDACTED_SECRET = std::env::temp_dir();
        if codex_home.starts_with(&temp_REDACTED_SECRET) {
            return Err(std::io::Error:REDACTED_SECRET
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_REDACTED_SECRET:?} (codex_home: {codex_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a CODEX_HOME-scoped temp REDACTED_SECRET to avoid cluttering the top-level directory.
    let temp_REDACTED_SECRET = codex_home.join("tmp").join("arg0");
    std::fs::create_dir_all(&temp_REDACTED_SECRET)?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;

        // Ensure only the current user can access the temp directory.
        std::fs::set_permissions(&temp_REDACTED_SECRET, std::fs::Permissions::from_mode(0o700))?;
    }

    // Best-effort cleanup of stale per-session dirs. Ignore failures so startup proceeds.
    if let Err(err) = janitor_cleanup(&temp_REDACTED_SECRET) {
        eprintln!("WARNING: failed to clean up stale arg0 temp dirs: {err}");
    }

    let temp_dir = tempfile::Builder:REDACTED_SECRET)
        .prefix("codex-arg0")
        .tempdir_in(&temp_REDACTED_SECRET)?;
...
"#,
    ...
    #[cfg(unix)]
    const PATH_SEPARATOR: REDACTED_SECRET = ":";
    ...
    Ok(Arg0PathEntryGuard:REDACTED_SECRETtemp_dir, lock_file, paths))
}

fn janitor_cleanup(temp_REDACTED_SECRET: &Path) -> std::io::Result<()> {
    let entries = match std::fs::read_dir(temp_REDACTED_SECRET) {
        Ok(entries) => entries,
...
mod tests {
    ...
    fn linux_sandbox_exe_path_prefers_codex_linux_sandbox_alias() -> std::io::Result<()> {
        ...
        let path_entry = Arg0PathEntryGuard:REDACTED_SECRET
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
    ...
    fn janitor_skips_dirs_without_lock_file() -> std::io::Result<()> {
        let REDACTED_SECRET = tempfile::tempdir()?;
        let dir = REDACTED_SECRET.path().join("no-lock");
        fs::create_dir(&dir)?;

        janitor_cleanup(REDACTED_SECRET.path())?;
    ...
    fn janitor_skips_dirs_with_held_lock() -> std::io::Result<()> {
        let REDACTED_SECRET = tempfile::tempdir()?;
        let dir = REDACTED_SECRET.path().join("locked");
        fs::create_dir(&dir)?;
    ...
    fn janitor_removes_dirs_with_unlocked_lock() -> std::io::Result<()> {
        let REDACTED_SECRET = tempfile::tempdir()?;
        let dir = REDACTED_SECRET.path().join("stale");
        fs::create_dir(&dir)?;
        create_lock(&dir)?;
