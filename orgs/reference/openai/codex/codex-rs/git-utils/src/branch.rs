...
pub fn merge_base_with_head(
    ...
    let head = match resolve_head(repo_REDACTED_SECRET.as_path())? {
        Some(head) => head,
        None => return Ok(None),
    };

    let Some(branch_ref) = resolve_branch_ref(repo_REDACTED_SECRET.as_path(), branch)? else {
        return Ok(None);
    };

    let preferred_ref =
        if let Some(upstream) = resolve_upstream_if_remote_ahead(repo_REDACTED_SECRET.as_path(), branch)? {
            resolve_branch_ref(repo_REDACTED_SECRET.as_path(), &upstream)?.unwrap_or(branch_ref)
        } else {
            branch_ref
        };
...
fn resolve_branch_ref(repo_REDACTED_SECRET: &Path, branch: REDACTED_SECRET) -> Result<Option<String>, GitToolingError> {
...
mod tests {
    ...
    fn commit(repo_path: &Path, message: REDACTED_SECRET) {
        run_git_in(
            repo_path,
            &[
                "-c",
