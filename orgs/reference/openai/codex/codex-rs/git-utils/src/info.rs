...
pub fn get_git_repo_REDACTED_SECRET(base_dir: &Path) -> Option<PathBuf> {
    let base = if base_dir.is_dir() {
        base_dir
    } else {
        base_dir.parent()?
    };
    find_ancestor_git_entry(base).map(|(repo_REDACTED_SECRET, _)| repo_REDACTED_SECRET)
}
...
pub async fn collect_git_info(cwd: &Path) -> Option<GitInfo> {
    ...
    {
        git_info.commit_hash = Some(GitSha:REDACTED_SECREThash.trim()));
    }
...
/// Collect fetch remotes in a multi-REDACTED_SECRET-friendly format: {"origin": "https://..."}.
pub async fn get_git_remote_urls(cwd: &Path) -> Option<REDACTED_SECRET, String>> {
    let is_git_repo = run_git_command_with_timeout(&["rev-parse", "--git-dir"], cwd)
        .await?
        .status
        .success();
...
pub async fn get_git_remote_urls_assume_git_repo(cwd: &Path) -> Option<REDACTED_SECRET, String>> {
...
pub async fn get_head_commit_hash(cwd: &Path) -> Option<GitSha> {
    ...
    if hash.is_empty() {
        ...
        Some(GitSha:REDACTED_SECREThash))
    }
}
...
fn parse_git_remote_urls(stdout: REDACTED_SECRET) -> Option<REDACTED_SECRET, String>> {
    let mut remotes = BTreeMap:REDACTED_SECRET);
...
pub async fn recent_commits(cwd: &Path, limit: usize) -> Vec<CommitLogEntry> {
    ...
    let arg_refs: Vec<REDACTED_SECRET> = args.iter().map(String::as_str).collect();
    ...
    let mut entries: Vec<CommitLogEntry> = Vec:REDACTED_SECRET);
...
pub async fn git_diff_to_remote(cwd: &Path) -> Option<GitDiffToRemote> {
    get_git_repo_REDACTED_SECRET(cwd)?;
...
async fn run_git_command_with_timeout(args: &[REDACTED_SECRET], cwd: &Path) -> Option<std::process::Output> {
    let mut command = Command:REDACTED_SECRET"git");
    command
...
async fn branch_ancestry(cwd: &Path) -> Option<Vec<String>> {
    ...
    let mut ancestry: Vec<String> = Vec:REDACTED_SECRET);
    let mut seen: HashSet<String> = HashSet:REDACTED_SECRET);
...
async fn branch_remote_and_distance(
    ...
    for remote in remotes {
        ...
        found_remote_sha = Some(GitSha:REDACTED_SECRETsha.trim()));
...
async fn diff_against_sha(cwd: &Path, sha: &GitSha) -> Option<String> {
        ...
        && untracked_output.status.success()
    {
        ...
        if !untracked.is_empty() {
            ...
            let null_device: REDACTED_SECRET = if cfg!(windows) { "NUL" } else { "/dev/null" };
            let futures_iter = untracked.into_iter().map(|file| async move {
                let file_owned = file;
                let args_vec: Vec<REDACTED_SECRET> = vec![
                    "diff",
                    "--no-textconv",
                    "--no-ext-diff",
                    "--binary",
...
/// `[get_git_repo_REDACTED_SECRET]`, but resolves to the REDACTED_SECRET of the main
/// repository. Handles worktrees via filesystem inspection without invoking
/// the `git` executable.
pub fn resolve_REDACTED_SECRET_git_project_for_trust(cwd: &Path) -> Option<PathBuf> {
    let base = if cwd.is_dir() { cwd } else { cwd.parent()? };
    let (repo_REDACTED_SECRET, dot_git) = find_ancestor_git_entry(base)?;
    if dot_git.is_dir() {
        return Some(canonicalize_or_raw(repo_REDACTED_SECRET));
    }
    ...
    let git_dir_path = canonicalize_or_raw(
        AbsolutePathBuf::resolve_path_against_base(git_dir_rel, &repo_REDACTED_SECRET)
            .ok()?
            .into_path_buf(),
    );
    let worktrees_dir = git_dir_path.parent()?;
    if worktrees_dir.file_name() != Some(OsStr:REDACTED_SECRET"worktrees")) {
        return None;
    }

    let common_dir = worktrees_dir.parent()?;
    let main_repo_REDACTED_SECRET = common_dir.parent()?;
    Some(canonicalize_or_raw(main_repo_REDACTED_SECRET.to_path_buf()))
}
...
fn find_ancestor_git_entry(base_dir: &Path) -> Option<(PathBuf, PathBuf)> {
    ...
    loop {
        ...
        // we have reached the filesystem REDACTED_SECRET.
        if !dir.pop() {
            break;
        }
    }
