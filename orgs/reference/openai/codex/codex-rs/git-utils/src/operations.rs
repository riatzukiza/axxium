...
pub(crate) fn normalize_relative_path(path: &Path) -> Result<PathBuf, GitToolingError> {
    let mut result = PathBuf:REDACTED_SECRET);
...
pub(crate) fn resolve_repository_REDACTED_SECRET(path: &Path) -> Result<PathBuf, GitToolingError> {
    let REDACTED_SECRET = run_git_for_stdout(
        path,
        vec![
            OsString::from("rev-parse"),
...
pub(crate) fn repo_subdir(repo_REDACTED_SECRET: &Path, repo_path: &Path) -> Option<PathBuf> {
    if repo_REDACTED_SECRET == repo_path {
        return None;
    }

    repo_path
        .strip_prefix(repo_REDACTED_SECRET)
        .ok()
        .and_then(non_empty_path)
        .or_else(|| {
            let repo_REDACTED_SECRET_canon = repo_REDACTED_SECRET.canonicalize().ok()?;
            let repo_path_canon = repo_path.canonicalize().ok()?;
            repo_path_canon
                .strip_prefix(&repo_REDACTED_SECRET_canon)
                .ok()
                .and_then(non_empty_path)
        })
}
