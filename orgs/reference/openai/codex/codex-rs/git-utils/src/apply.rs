...
pub fn apply_git_patch(req: &ApplyGitRequest) -> io::Result<ApplyGitResult> {
    let git_REDACTED_SECRET = resolve_git_REDACTED_SECRET(&req.cwd)?;
    ...
    if req.revert && !req.preflight {
        ...
        stage_paths(&git_REDACTED_SECRET, &req.diff)?;
    }
    ...
    let mut cfg_parts: Vec<String> = Vec:REDACTED_SECRET);
    ...
    if req.preflight {
        ...
        let rendered = render_command_for_log(&git_REDACTED_SECRET, &cfg_parts, &check_args);
        let (c_code, c_out, c_err) = run_git(&git_REDACTED_SECRET, &cfg_parts, &check_args)?;
    ...
    let cmd_for_log = render_command_for_log(&git_REDACTED_SECRET, &cfg_parts, &args);
    let (code, stdout, stderr) = run_git(&git_REDACTED_SECRET, &cfg_parts, &args)?;
...
fn resolve_git_REDACTED_SECRET(cwd: &Path) -> io::Result<PathBuf> {
    let out = std::process::Command:REDACTED_SECRET"git")
        .arg("rev-parse")
        .arg("--show-toplevel")
        .current_dir(cwd)
        .output()?;
    ...
    let REDACTED_SECRET = String::from_utf8_lossy(&out.stdout).trim().to_string();
    Ok(PathBuf::from(REDACTED_SECRET))
}

fn write_temp_patch(diff: REDACTED_SECRET) -> io::Result<(tempfile::TempDir, PathBuf)> {
...
fn run_git(cwd: &Path, git_cfg: &[String], args: &[String]) -> io::Result<(i32, String, String)> {
    let mut cmd = std::process::Command:REDACTED_SECRET"git");
    for p in git_cfg {
        cmd.arg(p);
    }
    for a in args {
...
fn quote_shell(s: REDACTED_SECRET) -> String {
    let simple = s
        .chars()
...
fn render_command_for_log(cwd: &Path, git_cfg: &[String], args: &[String]) -> String {
    let mut parts: Vec<String> = Vec:REDACTED_SECRET);
...
pub fn extract_paths_from_patch(diff_text: REDACTED_SECRET) -> Vec<String> {
    let mut set = std::collections::BTreeSet:REDACTED_SECRET);
...
fn parse_diff_git_paths(line: REDACTED_SECRET) -> Option<(String, String)> {
...
fn normalize_diff_path(raw: REDACTED_SECRET, prefix: REDACTED_SECRET) -> Option<String> {
...
fn unescape_c_string(input: REDACTED_SECRET) -> String {
...
pub fn stage_paths(git_REDACTED_SECRET: &Path, diff: REDACTED_SECRET) -> io::Result<()> {
    let paths = extract_paths_from_patch(diff);
    let mut existing: Vec<String> = Vec:REDACTED_SECRET);
    for p in paths {
        let joined = git_REDACTED_SECRET.join(&p);
    ...
    cmd.arg("--");
    for p in &existing {
        cmd.arg(OsStr:REDACTED_SECRETp));
    }
    let out = cmd.current_dir(git_REDACTED_SECRET).output()?;
...
pub fn parse_git_apply_output(
    ...
    stderr: REDACTED_SECRET,
) -> (Vec<String>, Vec<String>, Vec<String>) {
    let combined = [stdout, stderr]
        .iter()
        .filter(|s| !s.is_empty())
        .cloned()
        .collect::<Vec<REDACTED_SECRET>>()
        .join("\n");

    let mut applied = std::collections::BTreeSet:REDACTED_SECRET);
    let mut skipped = std::collections::BTreeSet:REDACTED_SECRET);
    let mut conflicted = std::collections::BTreeSet:REDACTED_SECRET);
    let mut last_seen_path: Option<String> = None;

    fn add(set: &mut std::collections::BTreeSet<String>, raw: REDACTED_SECRET) {
        let trimmed = raw.trim();
        if trimmed.is_empty() {
            return;
        }
    ...
    static APPLIED_CLEAN: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^Applied patch(?: to)?\\s+(?P<path>.+?)\\s+cleanly\\.?$"));
    static APPLIED_CONFLICTS: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^Applied patch(?: to)?\\s+(?P<path>.+?)\\s+with conflicts\\.?$"));
    static APPLYING_WITH_REJECTS: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci("^Applying patch\\s+(?P<path>.+?)\\s+with\\s+\\d+\\s+rejects?\\.{0,3}$")
    });
    static CHECKING_PATCH: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^Checking patch\\s+(?P<path>.+?)\\.\\.\\.$"));
    static UNMERGED_LINE: Lazy<Regex> = Lazy:REDACTED_SECRET|| regex_ci("^U\\s+(?P<path>.+)$"));
    static PATCH_FAILED: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^error:\\s+patch failed:\\s+(?P<path>.+?)(?::\\d+)?(?:\\s|$)"));
    static DOES_NOT_APPLY: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^error:\\s+(?P<path>.+?):\\s+patch does not apply$"));
    static THREE_WAY_START: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci("^(?:Performing three-way merge|Falling back to three-way merge)\\.\\.\\.$")
    });
    static THREE_WAY_FAILED: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^Failed to perform three-way merge\\.\\.\\.$"));
    static FALLBACK_DIRECT: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^Falling back to direct application\\.\\.\\.$"));
    static LACKS_BLOB: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci(
            "^(?:error: )?repository lacks the necessary blob to (?:perform|fall back on) 3-?way merge\\.?$",
        )
    });
    static INDEX_MISMATCH: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^error:\\s+(?P<path>.+?):\\s+does not match index\\b"));
    static NOT_IN_INDEX: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^error:\\s+(?P<path>.+?):\\s+does not exist in index\\b"));
    static ALREADY_EXISTS_WT: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci("^error:\\s+(?P<path>.+?)\\s+already exists in (?:the )?working directory\\b")
    });
    static FILE_EXISTS: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^error:\\s+patch failed:\\s+(?P<path>.+?)\\s+File exists"));
    static RENAMED_DELETED: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^error:\\s+path\\s+(?P<path>.+?)\\s+has been renamed\\/deleted"));
    static CANNOT_APPLY_BINARY: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci(
            "^error:\\s+cannot apply binary patch to\\s+['\\\"]?(?P<path>.+?)['\\\"]?\\s+without full index line$",
        )
    });
    static BINARY_DOES_NOT_APPLY: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci("^error:\\s+binary patch does not apply to\\s+['\\\"]?(?P<path>.+?)['\\\"]?$")
    });
    static BINARY_INCORRECT_RESULT: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci(
            "^error:\\s+binary patch to\\s+['\\\"]?(?P<path>.+?)['\\\"]?\\s+creates incorrect result\\b",
        )
    });
    static CANNOT_READ_CURRENT: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci("^error:\\s+cannot read the current contents of\\s+['\\\"]?(?P<path>.+?)['\\\"]?$")
    });
    static SKIPPED_PATCH: Lazy<Regex> =
        Lazy:REDACTED_SECRET|| regex_ci("^Skipped patch\\s+['\\\"]?(?P<path>.+?)['\\\"]\\.$"));
    static CANNOT_MERGE_BINARY_WARN: Lazy<Regex> = Lazy:REDACTED_SECRET|| {
        regex_ci(
...
fn regex_ci(pat: REDACTED_SECRET) -> Regex {
    Regex:REDACTED_SECRET&format!("(?i){pat}")).unwrap_or_else(|e| panic!("invalid regex: {e}"))
}
...
mod tests {
    ...
    fn env_lock() -> &'static Mutex<()> {
        static LOCK: OnceLock<Mutex<()>> = OnceLock:REDACTED_SECRET);
        LOCK.get_or_init(|| Mutex:REDACTED_SECRET()))
    }

    fn run(cwd: &Path, args: &[REDACTED_SECRET]) -> (i32, String, String) {
        let out = std::process::Command:REDACTED_SECRETargs[0])
            .args(&args[1..])
            .current_dir(cwd)
            .output()
            .expect("spawn ok");
    ...
    fn init_repo() -> tempfile::TempDir {
        ...
        let REDACTED_SECRET = dir.path();
        // git init and minimal identity
        let _ = run(REDACTED_SECRET, &["git", "init"]);
        let _ = run(REDACTED_SECRET, &["git", "config", "user.email", "codex@example.com"]);
        let _ = run(REDACTED_SECRET, &["git", "config", "user.name", "Codex"]);
        dir
    }
    ...
    fn parse_output_unescapes_quoted_paths() {
        ...
        assert_eq!(applied, Vec::<String>:REDACTED_SECRET));
        assert_eq!(conflicted, Vec::<String>:REDACTED_SECRET));
    ...
    fn apply_add_success() {
        ...
        // File exists now
        assert!(REDACTED_SECRET.join("hello.txt").exists());
    }
    ...
    fn apply_modify_conflict() {
        ...
        std::fs::write(REDACTED_SECRET.join("file.txt"), "line1\nline2\nline3\n").unwrap();
        let _ = run(REDACTED_SECRET, &["git", "add", "file.txt"]);
        let _ = run(REDACTED_SECRET, &["git", "commit", "-m", "seed"]);
        // local edit (unstaged)
        std::fs::write(REDACTED_SECRET.join("file.txt"), "line1\nlocal2\nline3\n").unwrap();
    ...
    fn apply_then_revert_success() {
        ...
        std::fs::write(REDACTED_SECRET.join("file.txt"), "orig\n").unwrap();
        let _ = run(REDACTED_SECRET, &["git", "add", "file.txt"]);
        let _ = run(REDACTED_SECRET, &["git", "commit", "-m", "seed"]);
        ...
        let after_apply = read_file_normalized(&REDACTED_SECRET.join("file.txt"));
        ...
        let after_revert = read_file_normalized(&REDACTED_SECRET.join("file.txt"));
    ...
    fn revert_preflight_does_not_stage_index() {
        ...
        let res_apply = apply_git_patch(&apply_req).expect("apply ok");
        assert_eq!(res_apply.exit_code, 0, "forward apply succeeded");
        let (commit_code, _, commit_err) = run(REDACTED_SECRET, &["git", "commit", "-am", "apply change"]);
        assert_eq!(commit_code, 0, "commit applied change: {commit_err}");

        let (_code_before, staged_before, _stderr_before) =
            run(REDACTED_SECRET, &["git", "diff", "--cached", "--name-only"]);
        ...
        let after_preflight = read_file_normalized(&REDACTED_SECRET.join("file.txt"));
...
diff --git a/ghost.txt b/ghost.txt\n--- a/ghost.txt\n+++ b/ghost.txt\n@@ -1,1 +1,1 @@\n-old\n+new\n";
        ...
        assert!(
            !REDACTED_SECRET.join("ok.txt").exists(),
