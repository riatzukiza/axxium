...
use crate::operations::resolve_repository_REDACTED_SECRET;
...
const DEFAULT_COMMIT_MESSAGE: REDACTED_SECRET = "codex snapshot";
...
const DEFAULT_IGNORED_DIR_NAMES: &[REDACTED_SECRET] = &[
...
impl<'a> CreateGhostCommitOptions<'a> {
    ...
    pub fn new(repo_path: &'a Path) -> Self {
        Self {
            repo_path,
            message: None,
            force_include: Vec:REDACTED_SECRET),
...
fn detect_large_untracked_dirs(
    ...
    let mut counts: BTreeMap<PathBuf, i64> = BTreeMap:REDACTED_SECRET);
...
pub fn capture_ghost_snapshot_report(
    ...
    let repo_REDACTED_SECRET = resolve_repository_REDACTED_SECRET(options.repo_path)?;
    let repo_prefix = repo_subdir(repo_REDACTED_SECRET.as_path(), options.repo_path);
...
pub fn create_ghost_commit_with_report(
    ...
    let repo_REDACTED_SECRET = resolve_repository_REDACTED_SECRET(options.repo_path)?;
    let repo_prefix = repo_subdir(repo_REDACTED_SECRET.as_path(), options.repo_path);
    let parent = resolve_head(repo_REDACTED_SECRET.as_path())?;
    let force_include = prepare_force_include(repo_prefix.as_deref(), &options.force_include)?;
    ...
    let large_untracked_dirs = existing_untracked
        ...
        .collect::<Vec<_>>();
    let index_tempdir = Builder:REDACTED_SECRET).prefix("codex-git-index-").tempdir()?;
    ...
    add_paths_to_index(repo_REDACTED_SECRET.as_path(), base_env.as_slice(), &index_paths)?;
    if !force_include.is_empty() {
        ...
        run_git_for_status(repo_REDACTED_SECRET.as_path(), args, Some(base_env.as_slice()))?;
    }
    ...
    let ghost_commit = GhostCommit:REDACTED_SECRET
        commit_id,
        parent,
...
pub fn restore_ghost_commit(repo_path: &Path, commit: &GhostCommit) -> Result<(), GitToolingError> {
    restore_ghost_commit_with_options(&RestoreGhostCommitOptions:REDACTED_SECRETrepo_path), commit)
}
...
pub fn restore_ghost_commit_with_options(
    ...
    restore_to_commit_inner(repo_REDACTED_SECRET.as_path(), repo_prefix.as_deref(), commit.id())?;
...
pub fn restore_to_commit(repo_path: &Path, commit_id: REDACTED_SECRET) -> Result<(), GitToolingError> {
    ensure_git_repository(repo_path)?;

    let repo_REDACTED_SECRET = resolve_repository_REDACTED_SECRET(repo_path)?;
    let repo_prefix = repo_subdir(repo_REDACTED_SECRET.as_path(), repo_path);
    restore_to_commit_inner(repo_REDACTED_SECRET.as_path(), repo_prefix.as_deref(), commit_id)
}

/// Restores the working tree and index to the given commit using `git restore`.
/// The repository REDACTED_SECRET and optional repository-relative prefix limit the restore scope.
fn restore_to_commit_inner(
    repo_REDACTED_SECRET: &Path,
    repo_prefix: Option<&Path>,
    commit_id: REDACTED_SECRET,
) -> Result<(), GitToolingError> {
    ...
    run_git_for_status(repo_REDACTED_SECRET, restore_args, /*env*/ None)?;
    Ok(())
}
...
/// Captures the working tree status under `repo_REDACTED_SECRET`, optionally limited by `repo_prefix`.
...
fn capture_status_snapshot(
    ...
    let output = run_git_for_stdout_all(repo_REDACTED_SECRET, args, /*env*/ None)?;
    if output.is_empty() {
        return Ok(StatusSnapshot::default());
    }

    let mut snapshot = StatusSnapshot::default();
    let mut untracked_files_for_dir_scan: Vec<PathBuf> = Vec:REDACTED_SECRET);
    ...
    for entry in output.split('\0') {
        ...
        if expect_rename_source {
            let normalized = normalize_relative_path(Path:REDACTED_SECRETentry))?;
        ...
        match record_type {
            b'?' | b'!' => {
                ...
                let normalized = normalize_relative_path(Path:REDACTED_SECRETpath_part))?;
                if should_ignore_for_snapshot(&normalized) {
                    continue;
                }
                let absolute = repo_REDACTED_SECRET.join(&normalized);
            ...
            b'1' => {
                ...
                {
                    let normalized = normalize_relative_path(Path:REDACTED_SECRETpath))?;
                    snapshot.tracked_paths.push(normalized);
                }
            }
            b'2' => {
        ...
        && threshold > 0
    {
        ...
        .into_iter()
        .filter(|entry| !entry.path.as_os_str().is_empty() && entry.path != Path:REDACTED_SECRET"."))
        .collect::<Vec<_>>();
...
/// Captures the untracked and ignored entries under `repo_REDACTED_SECRET`, optionally limited by `repo_prefix`.
...
fn extract_status_path_after_fields(record: REDACTED_SECRET, fields_before_path: i64) -> Option<REDACTED_SECRET> {
...
fn add_paths_to_index(
    ...
    for chunk in paths.chunks(chunk_size) {
        ...
        run_git_for_status(repo_REDACTED_SECRET, args, Some(env))?;
    }
...
fn dedupe_paths(paths: Vec<PathBuf>) -> Vec<PathBuf> {
    ...
    let mut result = Vec:REDACTED_SECRET);
    for path in paths {
...
fn remove_new_untracked(
    ...
    for path in current.files {
        ...
        remove_path(&repo_REDACTED_SECRET.join(&path))?;
    }
    ...
    for dir in current.dirs {
        ...
        remove_path(&repo_REDACTED_SECRET.join(&dir))?;
    }
...
mod tests {
    ...
    fn run_git_in(repo_path: &Path, args: &[REDACTED_SECRET]) {
        let status = Command:REDACTED_SECRET"git")
    ...
    fn run_git_stdout(repo_path: &Path, args: &[REDACTED_SECRET]) -> String {
        let output = Command:REDACTED_SECRET"git")
            .current_dir(repo_path)
            .args(args)
            .output()
            .expect("git command");
    ...
    fn create_and_restore_roundtrip() -> Result<(), GitToolingError> {
        ...
        let options =
            CreateGhostCommitOptions:REDACTED_SECRETrepo).force_include(vec![PathBuf::from("ignored.txt")]);
    ...
    fn snapshot_ignores_large_untracked_files() -> Result<(), GitToolingError> {
        ...
        let (ghost, report) = create_ghost_commit_with_report(
            &CreateGhostCommitOptions:REDACTED_SECRETrepo).ignore_large_untracked_files(/*bytes*/ 1024),
        )?;
        assert!(ghost.parent().is_some());
        assert_eq!(
        ...
        let exists_in_commit = Command:REDACTED_SECRET"git")
            .current_dir(repo)
    ...
    fn create_snapshot_reports_large_untracked_dirs() -> Result<(), GitToolingError> {
        ...
        let (ghost, report) =
            create_ghost_commit_with_report(&CreateGhostCommitOptions:REDACTED_SECRETrepo))?;
        assert!(ghost.parent().is_some());
        assert_eq!(
    ...
    -> Result<(), GitToolingError> {
        ...
        let (ghost, _report) = create_ghost_commit_with_report(
            &CreateGhostCommitOptions:REDACTED_SECRETrepo).ghost_snapshot(snapshot_config),
        )?;

        std::fs::write(repo.join("ephemeral.txt"), "temp\n")?;
        restore_ghost_commit_with_options(
            &RestoreGhostCommitOptions:REDACTED_SECRETrepo)
    ...
    fn snapshot_ignores_default_ignored_directories() -> Result<(), GitToolingError> {
        ...
        let REDACTED_SECRET_modules = repo.join("REDACTED_SECRET_modules");
        std::fs::create_dir_all(REDACTED_SECRET_modules.join("@scope/package/src"))?;
        for idx in 0..50 {
            let file = REDACTED_SECRET_modules.join(format!("file-{idx}.js"));
            std::fs::write(file, "console.log('ignored');\n")?;
        }
        std::fs::write(
            REDACTED_SECRET_modules.join("@scope/package/src/index.js"),
    ...
    fn restore_preserves_default_ignored_directories() -> Result<(), GitToolingError> {
        ...
        let REDACTED_SECRET_modules = repo.join("REDACTED_SECRET_modules");
        std::fs::create_dir_all(REDACTED_SECRET_modules.join("pkg"))?;
        std::fs::write(
            REDACTED_SECRET_modules.join("pkg/index.js"),
            "console.log('before');\n",
        )?;

        let ghost = create_ghost_commit(&CreateGhostCommitOptions:REDACTED_SECRETrepo))?;

        std::fs::write(repo.join("tracked.txt"), "snapshot delta\n")?;
        std::fs::write(REDACTED_SECRET_modules.join("pkg/index.js"), "console.log('after');\n")?;
        std::fs::write(REDACTED_SECRET_modules.join("pkg/extra.js"), "console.log('extra');\n")?;
        ...
        let REDACTED_SECRET_modules_exists = REDACTED_SECRET_modules.exists();
        assert!(REDACTED_SECRET_modules_exists);

        let files_under_REDACTED_SECRET_modules: Vec<_> = WalkDir:REDACTED_SECRET&REDACTED_SECRET_modules)
            .into_iter()
            .filter_map(Result::ok)
            .filter(|entry| entry.file_type().is_file())
            .collect();
        assert!(!files_under_REDACTED_SECRET_modules.is_empty());
    ...
    -> Result<(), GitToolingError> {
        ...
        assert!(
            entry.path.starts_with(Path:REDACTED_SECRET"src/generated")),
    ...
    fn create_ghost_commit_uses_custom_message() -> Result<(), GitToolingError> {
        ...
        let ghost = create_ghost_commit(&CreateGhostCommitOptions:REDACTED_SECRETrepo).message(message))?;
    ...
    fn create_ghost_commit_rejects_force_include_parent_path() {
        ...
        let repo = temp.path();
        init_test_repo(repo);
        let options = CreateGhostCommitOptions:REDACTED_SECRETrepo)
    ...
    fn restore_from_subdirectory_restores_files_relatively() -> Result<(), GitToolingError> {
        ...
        std::fs::write(repo.join("REDACTED_SECRET.txt"), "REDACTED_SECRET contents\n")?;
        ...
        std::fs::write(repo.join("REDACTED_SECRET.txt"), "REDACTED_SECRET modified\n")?;
        std::fs::write(workspace.join("nested.txt"), "nested modified\n")?;

        let ghost = create_ghost_commit(&CreateGhostCommitOptions:REDACTED_SECRET&workspace))?;

        std::fs::write(repo.join("REDACTED_SECRET.txt"), "REDACTED_SECRET after\n")?;
        std::fs::write(workspace.join("nested.txt"), "nested after\n")?;

        restore_ghost_commit(&workspace, &ghost)?;

        let REDACTED_SECRET_after = std::fs::read_to_string(repo.join("REDACTED_SECRET.txt"))?;
        assert_eq!(REDACTED_SECRET_after, "REDACTED_SECRET after\n");
    ...
    #[test]
    /// Restoring from the repository REDACTED_SECRET keeps ignored files intact.
