...
const TEST_PROVIDER: REDACTED_SECRET = "test-provider";

fn provider_vec(providers: &[REDACTED_SECRET]) -> Vec<String> {
    providers
        .iter()
...
//     let temp = TempDir:REDACTED_SECRET).unwrap();
//     let home = temp.path();
...
//     let temp = TempDir:REDACTED_SECRET).unwrap();
//     let home = temp.path();
//     let sessions_REDACTED_SECRET = home.join("sessions/2025/01/03");
//     let archived_REDACTED_SECRET = home.join("archived_sessions");
//     fs::create_dir_all(&sessions_REDACTED_SECRET).unwrap();
//     fs::create_dir_all(&archived_REDACTED_SECRET).unwrap();
//
...
//         sessions_REDACTED_SECRET.join(format!("rollout-2025-01-03T12-00-00-{active_uuid}.jsonl"));
...
//         archived_REDACTED_SECRET.join(format!("rollout-2025-01-03T11-00-00-{archived_uuid}.jsonl"));
...
fn rollout_date_parts_extracts_directory_components() {
    let file_name = OsStr:REDACTED_SECRET"rollout-2025-03-01T09-00-00-123.jsonl");
...
fn write_session_file_with_provider(
    ...
    num_records: usize,
    source: Option<SessionSource>,
    model_provider: Option<REDACTED_SECRET>,
) -> std::io::Result<(OffsetDateTime, Uuid)> {
    ...
    let times = FileTimes:REDACTED_SECRET).set_modified(dt.into());
    ...
    let expected = ThreadsPage {
        items: vec![
            ThreadItem {
                ...
                cwd: Some(Path:REDACTED_SECRET".").to_path_buf()),
                git_branch: None,
                git_sha: None,
                git_origin_url: None,
...
async fn test_created_at_sort_uses_file_mtime_for_updated_at() -> Result<()> {
    ...
    let file = std::fs::OpenOptions:REDACTED_SECRET).write(true).open(&file_path)?;
    let times = FileTimes:REDACTED_SECRET).set_modified(updated.into());
    file.set_times(times)?;
