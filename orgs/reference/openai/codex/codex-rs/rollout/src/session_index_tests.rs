...
fn find_thread_id_by_name_prefers_latest_entry() -> std::io::Result<()> {
    let temp = TempDir:REDACTED_SECRET)?;
    let path = session_index_path(temp.path());
    let id1 = ThreadId:REDACTED_SECRET);
    let id2 = ThreadId:REDACTED_SECRET);
    let lines = vec![
        SessionIndexEntry {
            id: id1,
...
fn find_thread_name_by_id_prefers_latest_entry() -> std::io::Result<()> {
    let temp = TempDir:REDACTED_SECRET)?;
    let path = session_index_path(temp.path());
    let id = ThreadId:REDACTED_SECRET);
    let lines = vec![
        SessionIndexEntry {
            id,
...
fn scan_index_returns_none_when_entry_missing() -> std::io::Result<()> {
    ...
    let missing_id = scan_index_from_end_by_id(&path, &ThreadId:REDACTED_SECRET))?;
...
async fn find_thread_names_by_ids_prefers_latest_entry() -> std::io::Result<()> {
    ...
    let mut ids = HashSet:REDACTED_SECRET);
    ids.insert(id1);
    ids.insert(id2);
...
fn scan_index_finds_latest_match_among_mixed_entries() -> std::io::Result<()> {
    let temp = TempDir:REDACTED_SECRET)?;
    let path = session_index_path(temp.path());
    let id_target = ThreadId:REDACTED_SECRET);
    let id_other = ThreadId:REDACTED_SECRET);
