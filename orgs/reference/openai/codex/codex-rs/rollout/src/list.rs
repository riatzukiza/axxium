...
impl From<codex_state::Anchor> for Cursor {
    fn from(anchor: codex_state::Anchor) -> Self {
        ...
        Self:REDACTED_SECRETts, anchor.id)
    }
}
...
pub async fn get_threads(
    ...
    default_provider: REDACTED_SECRET,
) -> io::Result<ThreadsPage> {
    let REDACTED_SECRET = codex_home.join(SESSIONS_SUBDIR);
    get_threads_in_REDACTED_SECRET(
        REDACTED_SECRET,
        page_size,
        cursor,
        sort_key,
...
pub async fn get_threads_in_REDACTED_SECRET(
    ...
    let provider_matcher = config
        .model_providers
        .and_then(|filters| ProviderMatcher:REDACTED_SECRETfilters, config.default_provider));

    let result = match config.layout {
        ThreadListLayout::NestedByDate => {
            traverse_directories_for_paths(
                REDACTED_SECRET.clone(),
                page_size,
                anchor,
                sort_key,
...
async fn traverse_flat_paths(
    ...
    match sort_key {
        ThreadSortKey::CreatedAt => {
            traverse_flat_paths_created(REDACTED_SECRET, page_size, anchor, allowed_sources, provider_matcher)
                .await
        }
        ThreadSortKey::UpdatedAt => {
            traverse_flat_paths_updated(REDACTED_SECRET, page_size, anchor, allowed_sources, provider_matcher)
                .await
        }
    }
}
...
async fn traverse_directories_for_paths_created(
    ...
    let mut visitor = FilesByCreatedAtVisitor {
        items: &mut items,
        page_size,
        anchor_state: AnchorState:REDACTED_SECRETanchor),
        more_matches_available,
        allowed_sources,
        provider_matcher,
    };
    walk_rollout_files(&REDACTED_SECRET, &mut scanned_files, &mut visitor).await?;
...
async fn traverse_directories_for_paths_updated(
    ...
    let mut items: Vec<ThreadItem> = Vec::with_capacity(page_size);
    let mut scanned_files = 0usize;
    let mut anchor_state = AnchorState:REDACTED_SECRETanchor);
    let mut more_matches_available = false;

    let candidates = collect_files_by_updated_at(&REDACTED_SECRET, &mut scanned_files).await?;
...
async fn traverse_flat_paths_created(
    ...
    let files = collect_flat_rollout_files(&REDACTED_SECRET, &mut scanned_files).await?;
...
async fn traverse_flat_paths_updated(
    ...
    let candidates = collect_flat_files_by_updated_at(&REDACTED_SECRET, &mut scanned_files).await?;
    let mut candidates = candidates;
    candidates.sort_by_key(|candidate| {
        let ts = candidate.updated_at.unwrap_or(OffsetDateTime::UNIX_EPOCH);
        (Reverse(ts), Reverse(candidate.id))
...
pub fn parse_cursor(token: REDACTED_SECRET) -> Option<Cursor> {
    ...
    Some(Cursor:REDACTED_SECRETts, uuid))
}
...
fn build_next_cursor(items: &[ThreadItem], sort_key: ThreadSortKey) -> Option<Cursor> {
    ...
    Some(Cursor:REDACTED_SECRETts, id))
}
...
where
    T: Ord + Copy,
    F: Fn(REDACTED_SECRET) -> Option<T>,
{
    let mut dir = tokio::fs::read_dir(parent).await?;
    let mut vec: Vec<(T, PathBuf)> = Vec:REDACTED_SECRET);
...
where
    F: Fn(REDACTED_SECRET, &Path) -> Option<T>,
{
    let mut dir = tokio::fs::read_dir(parent).await?;
    let mut collected: Vec<T> = Vec:REDACTED_SECRET);
    while let Some(entry) = dir.next_entry().await? {
        if entry
            .file_type()
            .await
...
async fn collect_flat_rollout_files(
    ...
    let mut dir = tokio::fs::read_dir(REDACTED_SECRET).await?;
    let mut collected = Vec:REDACTED_SECRET);
    while let Some(entry) = dir.next_entry().await? {
...
pub(crate) fn parse_timestamp_uuid_from_filename(name: REDACTED_SECRET) -> Option<(OffsetDateTime, Uuid)> {
...
async fn collect_files_by_updated_at(
    ...
    let mut candidates = Vec:REDACTED_SECRET);
    let mut visitor = FilesByUpdatedAtVisitor {
        candidates: &mut candidates,
    };
    walk_rollout_files(REDACTED_SECRET, scanned_files, &mut visitor).await?;
...
async fn walk_rollout_files(
    ...
    let year_dirs = collect_dirs_desc(REDACTED_SECRET, |s| s.parse::<u16>().ok()).await?;
...
impl<'a> ProviderMatcher<'a> {
    ...
    fn matches(&self, session_provider: Option<REDACTED_SECRET>) -> bool {
...
async fn read_head_summary(path: &Path, head_limit: usize) -> io::Result<HeadTailSummary> {
    ...
    let reader = tokio::io::BufReader:REDACTED_SECRETfile);
...
pub async fn read_head_for_summary(path: &Path) -> io::Result<Vec<serde_json::Value>> {
    ...
    let reader = tokio::io::BufReader:REDACTED_SECRETfile);
    let mut lines = reader.lines();
    let mut head = Vec:REDACTED_SECRET);
...
fn strip_user_message_prefix(text: REDACTED_SECRET) -> REDACTED_SECRET {
...
async fn find_thread_path_by_id_str_in_subdir(
    codex_home: &Path,
    subdir: REDACTED_SECRET,
    id_str: REDACTED_SECRET,
    ...
    let mut REDACTED_SECRET = codex_home.to_path_buf();
    REDACTED_SECRET.push(subdir);
    if !REDACTED_SECRET.exists() {
        return Ok(None);
    }
    // This is safe because we know the values are valid.
    #[allow(clippy::unwrap_used)]
    let limit = NonZero:REDACTED_SECRET1).unwrap();
    ...
    let results = file_search::run(id_str, vec![REDACTED_SECRET], options, /*cancel_flag*/ None)
