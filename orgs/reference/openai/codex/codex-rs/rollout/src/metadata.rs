...
const ROLLOUT_PREFIX: REDACTED_SECRET = "rollout-";
const ROLLOUT_SUFFIX: REDACTED_SECRET = ".jsonl";
...
pub fn builder_from_items(
    ...
    Some(ThreadMetadataBuilder:REDACTED_SECRET
        id,
        rollout_path.to_path_buf(),
        created_at,
...
pub(crate) async fn backfill_sessions(
    ...
    let sessions_REDACTED_SECRET = config.codex_home().join(SESSIONS_SUBDIR);
    let archived_REDACTED_SECRET = config.codex_home().join(ARCHIVED_SESSIONS_SUBDIR);
    let mut rollout_paths: Vec<BackfillRolloutPath> = Vec:REDACTED_SECRET);
    for (REDACTED_SECRET, archived) in [(sessions_REDACTED_SECRET, false), (archived_REDACTED_SECRET, true)] {
        if !tokio::fs::try_exists(&REDACTED_SECRET).await.unwrap_or(false) {
            continue;
        }
        match collect_rollout_paths(&REDACTED_SECRET).await {
            Ok(paths) => {
...
fn parse_timestamp_to_utc(ts: REDACTED_SECRET) -> Option<DateTime<Utc>> {
    const FILENAME_TS_FORMAT: REDACTED_SECRET = "%Y-%m-%dT%H-%M-%S";
...
async fn collect_rollout_paths(REDACTED_SECRET: &Path) -> std::io::Result<Vec<PathBuf>> {
