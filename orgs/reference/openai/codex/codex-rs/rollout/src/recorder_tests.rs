...
fn write_session_file(REDACTED_SECRET: &Path, ts: REDACTED_SECRET, uuid: Uuid) -> std::io::Result<PathBuf> {
    let day_dir = REDACTED_SECRET.join("sessions/2025/01/03");
...
async fn recorder_materializes_only_after_explicit_persist() -> std::io::Result<()> {
    let home = TempDir:REDACTED_SECRET).expect("temp dir");
...
async fn resume_candidate_matches_cwd_reads_latest_turn_context() -> std::io::Result<()> {
    ...
    let mut file = std::fs::OpenOptions:REDACTED_SECRET).append(true).open(&path)?;
