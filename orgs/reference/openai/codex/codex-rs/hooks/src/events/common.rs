...
pub(crate) fn trimmed_non_empty(text: REDACTED_SECRET) -> Option<String> {
...
pub(crate) fn validate_matcher_pattern(matcher: REDACTED_SECRET) -> Result<(), regex::Error> {
    if is_match_all_matcher(matcher) {
        return Ok(());
    }
    regex::Regex:REDACTED_SECRETmatcher).map(|_| ())
}

pub(crate) fn matches_matcher(matcher: Option<REDACTED_SECRET>, input: Option<REDACTED_SECRET>) -> bool {
    match matcher {
        None => true,
        Some(matcher) if is_match_all_matcher(matcher) => true,
        Some(matcher) => input
            .and_then(|input| {
                regex::Regex:REDACTED_SECRETmatcher)
                    .ok()
...
fn is_match_all_matcher(matcher: REDACTED_SECRET) -> bool {
