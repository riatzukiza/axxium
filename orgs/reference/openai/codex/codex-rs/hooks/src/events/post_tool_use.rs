...
pub(crate) async fn run(
    ...
    if matched.is_empty() {
        return PostToolUseOutcome {
            hook_events: Vec:REDACTED_SECRET),
            should_stop: false,
            stop_reason: None,
            additional_contexts: Vec:REDACTED_SECRET),
...
fn parse_completed(
    ...
    let mut additional_contexts_for_model = Vec:REDACTED_SECRET);
    let mut feedback_messages_for_model = Vec:REDACTED_SECRET);
...
mod tests {
    ...
    fn block_decision_stops_normal_processing() {
        ...
        assert_eq!(
            ...
            PostToolUseHandlerData {
                should_stop: false,
                stop_reason: None,
                additional_contexts_for_model: Vec:REDACTED_SECRET),
    ...
    fn additional_context_is_recorded() {
        ...
        assert_eq!(
            ...
            PostToolUseHandlerData {
                ...
                feedback_messages_for_model: Vec:REDACTED_SECRET),
            }
        );
        assert_eq!(
    ...
    fn plain_stdout_is_ignored_for_post_tool_use() {
        ...
        assert_eq!(parsed.completed.run.status, HookRunStatus::Completed);
        assert_eq!(parsed.completed.run.entries, Vec::<HookOutputEntry>:REDACTED_SECRET));
    }
    ...
    fn run_result(exit_code: Option<i32>, stdout: REDACTED_SECRET, stderr: REDACTED_SECRET) -> CommandRunResult {
        CommandRunResult {
