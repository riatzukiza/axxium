...
pub type HookFn = Arc<dyn for<'a> Fn(&'a HookPayload) -> BoxFuture<'a, HookResult> + REDACTED_SECRET + Sync>;
...
pub enum HookResult {
    ...
    FailedContinue(Box<dyn std::error::Error + REDACTED_SECRET + Sync + 'static>),
    /// FailedAbort: hook failed, other subsequent hooks should not execute, and the operation
    /// should be aborted.
    FailedAbort(Box<dyn std::error::Error + REDACTED_SECRET + Sync + 'static>),
}
...
impl Default for Hook {
    fn default() -> Self {
        Self {
            ...
            func: Arc:REDACTED_SECRET|_| Box::pin(async { HookResult::Success })),
        }
    }
}
        ...
        let payload = HookPayload {
            ...
            hook_event: HookEvent::AfterToolUse {
                event: HookEventAfterToolUse {
                    ...
                    tool_kind: HookToolKind:REDACTED_SECRETShell,
                    tool_input: HookToolInput:REDACTED_SECRETShell {
                        params: HookToolInputLocalShell {
