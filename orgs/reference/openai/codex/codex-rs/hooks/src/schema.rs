...
const GENERATED_DIR: REDACTED_SECRET = "generated";
const POST_TOOL_USE_INPUT_FIXTURE: REDACTED_SECRET = "post-tool-use.command.input.schema.json";
const POST_TOOL_USE_OUTPUT_FIXTURE: REDACTED_SECRET = "post-tool-use.command.output.schema.json";
const PRE_TOOL_USE_INPUT_FIXTURE: REDACTED_SECRET = "pre-tool-use.command.input.schema.json";
const PRE_TOOL_USE_OUTPUT_FIXTURE: REDACTED_SECRET = "pre-tool-use.command.output.schema.json";
const SESSION_START_INPUT_FIXTURE: REDACTED_SECRET = "session-start.command.input.schema.json";
const SESSION_START_OUTPUT_FIXTURE: REDACTED_SECRET = "session-start.command.output.schema.json";
const USER_PROMPT_SUBMIT_INPUT_FIXTURE: REDACTED_SECRET = "user-prompt-submit.command.input.schema.json";
const USER_PROMPT_SUBMIT_OUTPUT_FIXTURE: REDACTED_SECRET = "user-prompt-submit.command.output.schema.json";
const STOP_INPUT_FIXTURE: REDACTED_SECRET = "stop.command.input.schema.json";
const STOP_OUTPUT_FIXTURE: REDACTED_SECRET = "stop.command.output.schema.json";
...
pub fn write_schema_fixtures(schema_REDACTED_SECRET: &Path) -> anyhow::Result<()> {
    let generated_dir = schema_REDACTED_SECRET.join(GENERATED_DIR);
    ...
    T: JsonSchema,
{
    SchemaSettings::draft07()
        ...
        .into_generator()
        .into_REDACTED_SECRET_schema_for::<T>()
}
...
fn string_const_schema(value: REDACTED_SECRET) -> Schema {
...
fn string_enum_schema(values: &[REDACTED_SECRET]) -> Schema {
    let mut schema = SchemaObject {
        instance_type: Some(InstanceType::String.into()),
        ..Default::default()
    };
...
mod tests {
    ...
    fn expected_fixture(name: REDACTED_SECRET) -> &'static str {
        match name {
    ...
    fn normalize_newlines(value: REDACTED_SECRET) -> String {
        value.replace("\r\n", "\n")
    }

    #[test]
    fn generated_hook_schemas_match_fixtures() {
        let temp_dir = TempDir:REDACTED_SECRET).expect("create temp dir");
        let schema_REDACTED_SECRET = temp_dir.path().join("schema");
        write_schema_fixtures(&schema_REDACTED_SECRET).expect("write generated hook schemas");
        ...
        for fixture in [
            ...
            let actual = std::fs::read_to_string(schema_REDACTED_SECRET.join("generated").join(fixture))
    ...
    fn turn_scoped_hook_inputs_include_codex_turn_id_extension() {
        // Codex intentionally diverges from Claude's REDACTED_SECRET hook docs here so
