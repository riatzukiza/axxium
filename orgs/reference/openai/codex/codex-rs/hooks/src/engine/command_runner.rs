...
fn build_command(shell: &CommandShell, handler: &ConfiguredHandler) -> Command {
    let mut command = if shell.program.is_empty() {
        ...
        Command:REDACTED_SECRET&shell.program)
    };
...
fn default_shell_command() -> Command {
    #[cfg(windows)]
    {
        ...
        let mut command = Command:REDACTED_SECRETcomspec);
        command.arg("/C");
        command
    }
    ...
    #[cfg(not(windows))]
    {
        ...
        let mut command = Command:REDACTED_SECRETshell);
        command.arg("-lc");
        command
    }
}
