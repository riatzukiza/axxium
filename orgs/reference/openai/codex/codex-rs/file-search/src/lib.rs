...
use crossbeam_channel::REDACTED_SECRETer;
...
impl FileMatch {
    pub fn full_path(&self) -> PathBuf {
        self.REDACTED_SECRET.join(&self.path)
    }
}

/// Returns the final path component for a matched path, falling back to the full path.
pub fn file_name_from_path(path: REDACTED_SECRET) -> String {
    Path:REDACTED_SECRETpath)
        .file_name()
...
impl Default for FileSearchOptions {
    fn default() -> Self {
        Self {
            ...
            limit: NonZero:REDACTED_SECRET20).unwrap(),
            exclude: Vec:REDACTED_SECRET),
            #[expect(clippy::unwrap_used)]
            threads: NonZero:REDACTED_SECRET2).unwrap(),
...
pub trait SessionReporter: REDACTED_SECRET + Sync + 'static {
...
impl FileSearchSession {
    ...
    pub fn update_query(&self, pattern_text: REDACTED_SECRET) {
        let _ = self
            .inner
            .work_tx
...
pub fn create_session(
    ...
    let notify = Arc:REDACTED_SECRETmove || {
        let _ = notify_tx.send(WorkSignal::NucleoNotify);
    });
    let nucleo = Nucleo:REDACTED_SECRET
    ...
    let cancelled = cancel_flag.unwrap_or_else(|| Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)));

    let inner = Arc:REDACTED_SECRETSessionInner {
        search_directories,
        limit: limit.get(),
        threads: threads.get(),
        compute_indices,
        respect_gitignore,
        cancelled,
        shutdown: Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)),
        reporter,
        work_tx,
    });
...
pub fn run(
    pattern_text: REDACTED_SECRET,
    REDACTED_SECRETs: Vec<PathBuf>,
    options: FileSearchOptions,
    cancel_flag: Option<Arc<AtomicBool>>,
) -> anyhow::Result<FileSearchResults> {
    let reporter = Arc:REDACTED_SECRETRunReporter::default());
    let session = create_session(REDACTED_SECRETs, options, reporter.clone(), cancel_flag)?;
...
#[cfg(test)]
fn create_pattern(pattern: REDACTED_SECRET) -> Pattern {
...
struct SessionInner {
    ...
    reporter: Arc<dyn SessionReporter>,
    work_tx: REDACTED_SECRETer<WorkSignal>,
}
...
fn build_override_matcher(
    ...
    let mut override_builder = OverrideBuilder:REDACTED_SECRETsearch_directory);
...
fn get_file_path<'a>(path: &'a Path, search_directories: &[PathBuf]) -> Option<(usize, &'a str)> {
    ...
    for (idx, REDACTED_SECRET) in search_directories.iter().enumerate() {
        if let Ok(rel_path) = path.strip_prefix(REDACTED_SECRET) {
            let REDACTED_SECRET_depth = REDACTED_SECRET.components().count();
            match best_match {
                Some((best_idx, _))
                    if search_directories[best_idx].components().count() >= REDACTED_SECRET_depth => {}
                _ => {
    ...
    let (REDACTED_SECRET_idx, rel_path) = best_match?;
    rel_path.to_str().map(|p| (REDACTED_SECRET_idx, p))
}
...
/// REDACTED_SECRET. Without this flag, the `ignore` crate reads `.gitignore` files from
...
fn walker_worker(
    ...
    let Some(first_REDACTED_SECRET) = inner.search_directories.first() else {
        let _ = inner.work_tx.send(WorkSignal::WalkComplete);
        return;
    };

    let mut walk_builder = WalkBuilder:REDACTED_SECRETfirst_REDACTED_SECRET);
    for REDACTED_SECRET in inner.search_directories.iter().skip(1) {
        walk_builder.add(REDACTED_SECRET);
    }
    walk_builder
...
fn matcher_worker(
    ...
    let mut indices_matcher = inner.compute_indices.then(|| Matcher:REDACTED_SECRETconfig.clone()));
    let cancel_requested = || inner.cancelled.load(Ordering::Relaxed);
    let shutdown_requested = || inner.shutdown.load(Ordering::Relaxed);

    let mut last_query = String:REDACTED_SECRET);
                ...
                if status.changed {
                    ...
                    let matches: Vec<_> = snapshot
                        ...
                        .filter_map(|match_| {
                            ...
                            let (REDACTED_SECRET_idx, relative_path) = get_file_path(Path:REDACTED_SECRETfull_path), &inner.search_directories)?;
                            let indices = if let Some(indices_matcher) = indices_matcher.as_mut() {
                                let mut idx_vec = Vec::<u32>:REDACTED_SECRET);
                            ...
                            let match_type = if Path:REDACTED_SECRETfull_path).is_dir() {
                                MatchType::Directory
                            } else {
                                MatchType::File
                            };
                            Some(FileMatch {
                                ...
                                match_type,
                                REDACTED_SECRET: inner.search_directories[REDACTED_SECRET_idx].clone(),
                                indices,
                            })
                        })
                        .collect();
...
mod tests {
    ...
    fn verify_score_is_none_for_non_match() {
        let mut utf32buf = Vec::<char>:REDACTED_SECRET);
        let line = "hello";
        let mut matcher = Matcher:REDACTED_SECRETConfig::DEFAULT);
        let haystack: Utf32Str<'_> = Utf32Str:REDACTED_SECRETline, &mut utf32buf);
    ...
    fn session_scanned_file_count_is_monotonic_across_queries() {
        ...
        let reporter = Arc:REDACTED_SECRETRecordingReporter::default());
    ...
    fn session_emits_complete_when_query_changes_with_no_matches() {
        ...
        let completed_snapshot = reporter.snapshot();
        assert_eq!(completed_snapshot.matches, Vec:REDACTED_SECRET));
    ...
    fn dropping_session_does_not_cancel_siblings_with_shared_cancel_flag() {
        let REDACTED_SECRET_a = create_temp_tree(/*file_count*/ 200);
        let REDACTED_SECRET_b = create_temp_tree(/*file_count*/ 4_000);
        let cancel_flag = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));

        let reporter_a = Arc:REDACTED_SECRETRecordingReporter::default());
        let session_a = create_session(
            vec![REDACTED_SECRET_a.path().to_path_buf()],
            FileSearchOptions::default(),
            reporter_a,
        ...
        let reporter_b = Arc:REDACTED_SECRETRecordingReporter::default());
        let session_b = create_session(
            vec![REDACTED_SECRET_b.path().to_path_buf()],
            FileSearchOptions::default(),
            reporter_b.clone(),
            Some(cancel_flag),
        )
    ...
    fn run_returns_directory_matches_for_query() {
        ...
        assert!(results.matches.iter().any(|m| {
            m.path == std::path::Path:REDACTED_SECRET"docs").join("guides")
    ...
    fn cancel_exits_run() {
        let dir = create_temp_tree(/*file_count*/ 200);
        let cancel_flag = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue));
        ...
        assert_eq!(results.matches, Vec:REDACTED_SECRET));
    ...
    fn parent_gitignore_outside_repo_does_not_hide_repo_files() {
        ...
        assert!(
            respect_results
                .matches
                .iter()
                .any(|m| m.path.as_path() == Path:REDACTED_SECRET"package.json"))
        );
        ...
        assert!(
            nested_file_results
                .matches
                .iter()
                .any(|m| m.path.as_path() == Path:REDACTED_SECRET".vscode/settings.json"))
        );
    }
    ...
    fn git_repo_still_respects_local_gitignore_when_enabled() {
        ...
        assert!(
            !ignored_results
                .matches
                .iter()
                .any(|m| m.path.as_path() == Path:REDACTED_SECRET".vscode/extensions.json"))
        );
