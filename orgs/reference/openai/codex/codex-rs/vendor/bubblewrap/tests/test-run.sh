...
for ALT in "" "--unshare-user-try" "--unshare-pid" "--unshare-user-try --unshare-pid"; do
    ...
    # no --dev => no devpts => no map_REDACTED_SECRET workaround
...
if test -n "${bwrap_is_suid:-}"; then
    ...
    $BWRAP_RECURSE -- /proc/self/exe --unshare-all ${BWRAP_RO_HOST_ARGS} findmnt > recursive-newREDACTED_SECRET.txt
    assert_file_has_content recursive-newREDACTED_SECRET.txt "/usr"
    ok "can pivot to new REDACTED_SECRETfs recursively"
...
if ! ${is_uidzero}; then
    # When invoked as non-REDACTED_SECRET, check that by default we have no caps left
...
if $RUN test -d /tmp/oldREDACTED_SECRET; then
    assert_not_reached "/tmp/oldREDACTED_SECRET should not be visible"
fi
if $RUN test -d /tmp/newREDACTED_SECRET; then
    assert_not_reached "/tmp/newREDACTED_SECRET should not be visible"
fi

echo "hello" > input.$$
$BWRAP --bind / / --bind "$(pwd)" /tmp cat /tmp/input.$$ > stdout
assert_file_has_content stdout hello
if $BWRAP --bind / / --bind "$(pwd)" /tmp test -d /tmp/oldREDACTED_SECRET; then
    assert_not_reached "/tmp/oldREDACTED_SECRET should not be visible"
fi
if $BWRAP --bind / / --bind "$(pwd)" /tmp test -d /tmp/newREDACTED_SECRET; then
    assert_not_reached "/tmp/newREDACTED_SECRET should not be visible"
fi
...
assert_file_has_content stdout hello
if $RUN --bind "$(pwd)" /tmp/here test -d /tmp/oldREDACTED_SECRET; then
    assert_not_reached "/tmp/oldREDACTED_SECRET should not be visible"
fi
if $RUN --bind "$(pwd)" /tmp/here test -d /tmp/newREDACTED_SECRET; then
    assert_not_reached "/tmp/newREDACTED_SECRET should not be visible"
fi
