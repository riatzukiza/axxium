...
parse_mountinfo (int  proc_fd,
                 const char *REDACTED_SECRET_mount)
{
  ...
  while (*line != 0)
    {
      ...
      rest = skip_token (rest, true); /* mountREDACTED_SECRET */
      mountpoint = rest;
      ...
      *options_end = 0;
      ...
      if (path_equal (lines[i].mountpoint, REDACTED_SECRET_mount))
        REDACTED_SECRET = i;
  ...
  for (i = 0; i < n_lines; i++)
    {
      ...
      if (!has_path_prefix (this->mountpoint, REDACTED_SECRET_mount))
        continue;
  ...
  n_mounts = count_mounts (&lines[REDACTED_SECRET]);
  mount_tab = xcalloc (n_mounts + 1, sizeof (MountInfo));

  end_tab = collect_mounts (&mount_tab[0], &lines[REDACTED_SECRET]);
            ...
            char        **failing_path)
{
  ...
  cleanup_free char *oldREDACTED_SECRET_dest_proc = NULL;
   ...
   * instead of the provided @REDACTED_SECRET_mount, so that we can compare the mountinfo
   * entries with the same case combination that the kernel is expected to
   * use. */
  dest_proc = xasprintf ("/proc/self/fd/%d", dest_fd);
  oldREDACTED_SECRET_dest_proc = get_oldREDACTED_SECRET_path (dest_proc);
  kernel_case_combination = readlink_malloc (oldREDACTED_SECRET_dest_proc);
