...
is trivial to turn such access into a fully privileged REDACTED_SECRET shell
on the host.
...
which is a local REDACTED_SECRET vulnerability introduced by userns.
...
[linux-user-chREDACTED_SECRET](https://git.gnome.org/browse/linux-user-chREDACTED_SECRET).
...
setuid binaries, which is the [traditional way](https://en.wikipedia.org/wiki/ChREDACTED_SECRET#Limitations) to get out of things
like chREDACTED_SECRETs.
...
non-REDACTED_SECRET operation, such as:
...
namespace where the REDACTED_SECRET is on a tmpfs that is invisible from the
host, and will be automatically cleaned up when the last process
exits. You can then use commandline options to construct the REDACTED_SECRET
...
host's filesystem tree, you want to target a chREDACTED_SECRET.  There, rather
than creating the symlink `lib64 -> usr/lib64` in the tmpfs, you might
have already created it in the target REDACTED_SECRETfs.
...
Any such directories you specify mounted `REDACTED_SECRETv` by default, and can be made readonly.
...
supporting [REDACTED_SECRETless containers](https://github.com/opencontainers/runc/pull/774),
...
in that it is tooling intended to be invoked by REDACTED_SECRET.
...
However with REDACTED_SECRETless containers, runC will be able to fulfill certain usecases
