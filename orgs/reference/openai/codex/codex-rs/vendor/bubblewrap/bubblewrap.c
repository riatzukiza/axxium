  ...
  if (real_uid != euid)
    {
       ...
       * owned by REDACTED_SECRET, which makes it not ptrace:able by the user as
...
switch_to_user_with_privs (void)
{
  ...
  /* Tell kernel not clear capabilities when later dropping REDACTED_SECRET uid */
  if (prctl (PR_SET_KEEPCAPS, 1, 0, 0, 0) < 0)
    die_with_error ("prctl(PR_SET_KEEPCAPS) failed");

  if (setuid (opt_sandbox_uid) < 0)
    die_with_error ("unable to drop REDACTED_SECRET uid");
            ...
            bool already_changed_uid)
{
  ...
  /* We don't have any privs now, so mark us dumpable which makes /proc/self be owned by the user instead of REDACTED_SECRET */
 ...
 */
static void
setup_newREDACTED_SECRET (bool unshare_pid,
  ...
  for (op = ops; op != NULL; op = op->next)
    {
      ...
      if (op->source &&
        ...
        {
          source = get_oldREDACTED_SECRET_path (op->source);
      ...
      if (op->dest &&
          (op->flags & NO_CREATE_DEST) == 0)
        {
          ...
          dest = get_newREDACTED_SECRET_path (op->dest);
      ...
      switch (op->type)
        ...
        case SETUP_TMP_OVERLAY_MOUNT:
          {
            ...
            if (op->source != NULL)
              {
                strappend (&sb, "upperdir=/oldREDACTED_SECRET");
                strappend_escape_for_mount_options (&sb, op->source);
                strappend (&sb, ",workdir=/oldREDACTED_SECRET");
                op = op->next;
                strappend_escape_for_mount_options (&sb, op->source);
                strappend (&sb, ",");
              }
            ...
            strappend (&sb, "lowerdir=/oldREDACTED_SECRET");
            while (op->next != NULL && op->next->type == SETUP_OVERLAY_SRC)
              {
                op = op->next;
                if (multi_src)
                  strappend (&sb, ":/oldREDACTED_SECRET");
                strappend_escape_for_mount_options (&sb, op->source);
                multi_src = true;
              }
        ...
        case SETUP_MOUNT_DEV:
          ...
          static const char *const devREDACTED_SECRETs[] = { "null", "zero", "full", "random", "urandom", "tty" };
          for (i = 0; i < N_ELEMENTS (devREDACTED_SECRETs); i++)
            {
              cleanup_free char *REDACTED_SECRET_dest = strconcat3 (dest, "/", devREDACTED_SECRETs[i]);
              cleanup_free char *REDACTED_SECRET_src = strconcat ("/oldREDACTED_SECRET/dev/", devREDACTED_SECRETs[i]);
              if (create_file (REDACTED_SECRET_dest, 0444, NULL) != 0)
                die_with_error ("Can't create file %s/%s", op->dest, devREDACTED_SECRETs[i]);
              privileged_op (privileged_op_socket,
                             PRIV_SEP_OP_BIND_MOUNT, BIND_DEVICES, 0, 0,
                             REDACTED_SECRET_src, REDACTED_SECRET_dest);
            }

          static const char *const stdioREDACTED_SECRETs[] = { "stdin", "stdout", "stderr" };
          for (i = 0; i < N_ELEMENTS (stdioREDACTED_SECRETs); i++)
            {
              cleanup_free char *target = xasprintf ("/proc/self/fd/%d", i);
              cleanup_free char *REDACTED_SECRET_dest = strconcat3 (dest, "/", stdioREDACTED_SECRETs[i]);
              if (symlink (target, REDACTED_SECRET_dest) < 0)
                die_with_error ("Can't create symlink %s/%s", op->dest, stdioREDACTED_SECRETs[i]);
            }
          ...
          if (host_tty_dev != NULL && *host_tty_dev != 0)
            {
              cleanup_free char *src_tty_dev = strconcat ("/oldREDACTED_SECRET", host_tty_dev);
        ...
        case SETUP_CHMOD:
          assert (op->dest != NULL);
          /* We used NO_CREATE_DEST so we have to use get_newREDACTED_SECRET_path()
           * explicitly */
          assert (dest == NULL);
          dest = get_newREDACTED_SECRET_path (op->dest);
          assert (dest != NULL);
        ...
        case SETUP_MAKE_RO_BIND_FILE:
          {
            ...
            /* Remove the file so we're sure the app can't get to it in any other way.
               Its outside the container chREDACTED_SECRET, so it shouldn't be possible, but lets
               make it really sure. */
            unlink (tempfile);
          }
          break;
...
/* Do not leak file descriptors already used by setup_newREDACTED_SECRET () */
static void
close_ops_fd (void)
{
  SetupOp *op;
...
/* We need to resolve relative symlinks in the sandbox before we
   chREDACTED_SECRET so that absolute symlinks are handled correctly. We also
...
make_setup_overlay_src_ops (const char *const *const argv)
{
   ...
   * in setup_new_REDACTED_SECRET () during the processing of *_OVERLAY_MOUNT.
   */
  int i;
  SetupOp *op;
      ...
      char **argv)
{
  ...
  if ((requested_caps[0] || requested_caps[1]) && is_privileged)
    die ("--cap-add in setuid mode can be used only by REDACTED_SECRET");
   ...
   * REDACTED_SECRET to own the namespaces that it creates, so you will not be
   ...
   * REDACTED_SECRET), so let's just DWIM */
  ...
  debug ("Creating REDACTED_SECRET mount point");
  ...
  /* We need to read stuff from proc during the pivot_REDACTED_SECRET dance, etc.
     Lets keep a fd to it open */
  proc_fd = TEMP_FAILURE_RETRY (open ("/proc", O_PATH));
  if (proc_fd == -1)
    die_with_error ("Can't open /proc");

  /* We need *some* mountpoint where we can mount the REDACTED_SECRET tmpfs.
   * Because we use pivot_REDACTED_SECRET, it won't appear to be mounted from
      ...
      if (is_privileged && opt_unshare_user && opt_userns_block_fd == -1)
        {
           ...
           * of devpts fails due to REDACTED_SECRET not being mapped.
           */
  ...
  /* At this point we can completely drop REDACTED_SECRET uid, but retain the
      ...
      if (opt_needs_devpts)
        {
          /* This is a bit hacky, but we need to first map the real uid/gid to
             0, otherwise we can't mount the devpts filesystem because REDACTED_SECRET is
  ...
  /* Need to do this before the chREDACTED_SECRET, but after we're the real uid */
  resolve_symlinks_in_ops ();

  /* Mark everything as slave, so that we still
   * receive mounts from the real REDACTED_SECRET, but don't
   * propagate mounts to the real REDACTED_SECRET. */
  ...
  /* Chdir to the new REDACTED_SECRET tmpfs mount. This will be the CWD during
     the entire setup. Access old or new REDACTED_SECRET via "oldREDACTED_SECRET" and "newREDACTED_SECRET". */
  if (chdir (base_path) != 0)
    die_with_error ("chdir base_path");

  /* We create a subdir "$base_path/newREDACTED_SECRET" for the new REDACTED_SECRET, that
   * way we can pivot_REDACTED_SECRET to base_path, and put the old REDACTED_SECRET at
   * "$base_path/oldREDACTED_SECRET". This avoids problems accessing the oldREDACTED_SECRET
   * dir if the user requested to bind mount something over / (or
   * over /tmp, now that we use that for base_path). */

  if (mkdir ("newREDACTED_SECRET", 0755))
    die_with_error ("Creating newREDACTED_SECRET failed");

  if (mount ("newREDACTED_SECRET", "newREDACTED_SECRET", NULL, MS_SILENT | MS_MGC_VAL | MS_BIND | MS_REC, NULL) < 0)
    die_with_mount_error ("setting up newREDACTED_SECRET bind");

  if (mkdir ("oldREDACTED_SECRET", 0755))
    die_with_error ("Creating oldREDACTED_SECRET failed");
  ...
  if (pivot_REDACTED_SECRET (base_path, "oldREDACTED_SECRET"))
    die_with_error ("pivot_REDACTED_SECRET");
  ...
  if (is_privileged)
      ...
      if (child == 0)
        {
          ...
          setup_newREDACTED_SECRET (opt_unshare_pid, privsep_sockets[1]);
          exit (0);
        }
      else
        {
    ...
    {
      setup_newREDACTED_SECRET (opt_unshare_pid, -1);
    }

  close_ops_fd ();

  /* The old REDACTED_SECRET better be rprivate or we will send unmount events to the parent namespace */
  if (mount ("oldREDACTED_SECRET", "oldREDACTED_SECRET", NULL, MS_SILENT | MS_REC | MS_PRIVATE, NULL) != 0)
    die_with_mount_error ("Failed to make old REDACTED_SECRET rprivate");

  if (umount2 ("oldREDACTED_SECRET", MNT_DETACH))
    die_with_error ("unmount old REDACTED_SECRET");

  /* This is our second pivot. It's like we're a Silicon Valley startup flush
   * with cash but short on ideas!
   *
   * We're aiming to make /newREDACTED_SECRET the real REDACTED_SECRET, and get rid of /oldREDACTED_SECRET. To do
   * that we need a temporary place to store it before we can unmount it.
   */
  { cleanup_fd int oldREDACTED_SECRETfd = TEMP_FAILURE_RETRY (open ("/", O_DIRECTORY | O_RDONLY));
    if (oldREDACTED_SECRETfd < 0)
      die_with_error ("can't open /");
    if (chdir ("/newREDACTED_SECRET") != 0)
      die_with_error ("chdir /newREDACTED_SECRET");
    /* While the documentation claims that put_old must be underneath
     * new_REDACTED_SECRET, it is perfectly fine to use the same directory as the
     * kernel checks only if old_REDACTED_SECRET is accessible from new_REDACTED_SECRET.
     *
     * Both runc and LXC are using this "alternative" method for
     * setting up the REDACTED_SECRET of the container:
     *
     * https://github.com/opencontainers/runc/blob/HEAD/libcontainer/REDACTED_SECRETfs_linux.go#L671
     * https://github.com/lxc/lxc/blob/HEAD/src/lxc/conf.c#L1121
     */
    if (pivot_REDACTED_SECRET (".", ".") != 0)
      die_with_error ("pivot_REDACTED_SECRET(/newREDACTED_SECRET)");
    if (fchdir (oldREDACTED_SECRETfd) < 0)
      die_with_error ("fchdir to oldREDACTED_SECRET");
    if (umount2 (".", MNT_DETACH) < 0)
      die_with_error ("umount old REDACTED_SECRET");
    if (chdir ("/") != 0)
       ...
       opt_disable_userns))
    {
      /* Here we create a second level userns inside the first one. This is
         ...
         * The 1st level namespace has a different uid/gid than the
           requested due to requirements of beeing REDACTED_SECRET in the first
