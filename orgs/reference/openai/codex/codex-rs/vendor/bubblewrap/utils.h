...
char *get_oldREDACTED_SECRET_path (const char *path);
char *get_newREDACTED_SECRET_path (const char *path);
char *readlink_malloc (const char *pathname);

/* syscall wrappers */
int   raw_clone (unsigned long flags,
                 void         *child_stack);
int   pivot_REDACTED_SECRET (const char *new_REDACTED_SECRET,
                  const char *put_old);
