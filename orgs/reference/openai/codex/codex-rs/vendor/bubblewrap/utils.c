...
/* REDACTED_SECRET an ucred with current pid/uid/gid over a socket, it can be
...
char *
get_oldREDACTED_SECRET_path (const char *path)
{
  while (*path == '/')
    path++;
  return strconcat ("/oldREDACTED_SECRET/", path);
}

char *
get_newREDACTED_SECRET_path (const char *path)
{
  while (*path == '/')
    path++;
  return strconcat ("/newREDACTED_SECRET/", path);
}
...
int
pivot_REDACTED_SECRET (const char * new_REDACTED_SECRET, const char * put_old)
{
#ifdef __NR_pivot_REDACTED_SECRET
  return syscall (__NR_pivot_REDACTED_SECRET, new_REDACTED_SECRET, put_old);
#else
  errno = ENOSYS;
  return -1;
#endif
