...
const REPO_ROOT_CONFIG_DIR_NAME: REDACTED_SECRET = ".codex";
...
fn project_layers_for_cwd(cwd: &Path) -> Vec<ConfigLayerEntry> {
    ...
    let project_REDACTED_SECRET = cwd_dir
        .ancestors()
    ...
    let mut layers = cwd_dir
        ...
        .scan(false, |done, dir| {
            if *done {
                ...
                if dir == project_REDACTED_SECRET {
                    *done = true;
                }
...
async fn make_config_for_cwd(codex_home: &TempDir, cwd: PathBuf) -> TestConfig {
    ...
    TestConfig {
        cwd,
        config_layer_stack: ConfigLayerStack:REDACTED_SECRET
            layers,
...
fn load_skills_for_test(config: &TestConfig) -> SkillLoadOutcome {
    ...
    super::load_skills_from_REDACTED_SECRETs(super::skill_REDACTED_SECRETs_with_home_dir(
...
fn mark_as_git_repo(dir: &Path) {
    // Config/project-REDACTED_SECRET discovery only checks for the presence of `.git` (file or dir),
...
#[test]
fn skill_REDACTED_SECRETs_from_layer_stack_maps_user_to_user_and_system_cache_and_system_to_admin()
-> anyhow::Result<()> {
    ...
    let got = skill_REDACTED_SECRETs_from_layer_stack(&stack, Some(&home_folder))
        .into_iter()
        .map(|REDACTED_SECRET| (REDACTED_SECRET.scope, REDACTED_SECRET.path))
        .collect::<Vec<_>>();
...
#[test]
fn skill_REDACTED_SECRETs_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
    let tmp = tempfile::tempdir()?;
    ...
    let project_REDACTED_SECRET = tmp.path().join("repo");
    let dot_codex = project_REDACTED_SECRET.join(".codex");
...
fn loads_skills_from_home_agents_dir_for_user_scope() -> anyhow::Result<()> {
    ...
    let user_file = AbsolutePathBuf::from_absolute_path(user_folder.join("config.toml"))?;
    let layers = vec![ConfigLayerEntry:REDACTED_SECRET
        ConfigLayerSource::User { file: user_file },
        TomlValue::Table(toml::map::Map:REDACTED_SECRET)),
    )];
    let stack = ConfigLayerStack:REDACTED_SECRET
    ...
    let outcome = load_skills_from_REDACTED_SECRETs(skill_REDACTED_SECRETs_from_layer_stack(&stack, Some(&home_folder)));
    assert!(
...
fn write_skill(codex_home: &TempDir, dir: REDACTED_SECRET, name: REDACTED_SECRET, description: REDACTED_SECRET) -> PathBuf {
    write_skill_at(&codex_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(codex_home: &TempDir, dir: REDACTED_SECRET, name: REDACTED_SECRET, description: REDACTED_SECRET) -> PathBuf {
    write_skill_at(
...
fn write_skill_at(REDACTED_SECRET: &Path, dir: REDACTED_SECRET, name: REDACTED_SECRET, description: REDACTED_SECRET) -> PathBuf {
    let skill_dir = REDACTED_SECRET.join(dir);
...
fn write_raw_skill_at(REDACTED_SECRET: &Path, dir: REDACTED_SECRET, frontmatter: REDACTED_SECRET) -> PathBuf {
    let skill_dir = REDACTED_SECRET.join(dir);
    fs::create_dir_all(&skill_dir).unwrap();
    let path = skill_dir.join(SKILLS_FILENAME);
    let content = format!("---\n{frontmatter}\n---\n\n# Body\n");
...
fn write_skill_metadata_at(skill_dir: &Path, contents: REDACTED_SECRET) -> PathBuf {
    let path = skill_dir
...
fn write_skill_interface_at(skill_dir: &Path, contents: REDACTED_SECRET) -> PathBuf {
...
async fn ignores_symlinked_skill_file_for_user_scope() {
    ...
    assert_eq!(outcome.skills, Vec:REDACTED_SECRET));
}
...
fn loads_skills_via_symlinked_subdir_for_admin_scope() {
    let admin_REDACTED_SECRET = tempfile::tempdir().expect("tempdir");
    let shared = tempfile::tempdir().expect("tempdir");

    let shared_skill_path =
        write_skill_at(shared.path(), "demo", "admin-linked-skill", "from link");
    fs::create_dir_all(admin_REDACTED_SECRET.path()).unwrap();
    symlink_dir(shared.path(), &admin_REDACTED_SECRET.path().join("shared"));

    let outcome = load_skills_from_REDACTED_SECRETs([SkillRoot {
        path: admin_REDACTED_SECRET.path().to_path_buf(),
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let repo_skills_REDACTED_SECRET = repo_dir
        .path()
        .join(REPO_ROOT_CONFIG_DIR_NAME)
        .join(SKILLS_DIR_NAME);
    fs::create_dir_all(&repo_skills_REDACTED_SECRET).unwrap();
    symlink_dir(shared.path(), &repo_skills_REDACTED_SECRET.join("shared"));
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_REDACTED_SECRET = codex_home.path().join("skills/.system");
    fs::create_dir_all(&system_REDACTED_SECRET).unwrap();
    symlink_dir(shared.path(), &system_REDACTED_SECRET.join("shared"));

    let outcome = load_skills_from_REDACTED_SECRETs([SkillRoot {
        path: system_REDACTED_SECRET,
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_REDACTED_SECRET = codex_home.path().join("skills");
    let outcome = load_skills_from_REDACTED_SECRETs([SkillRoot {
        path: skills_REDACTED_SECRET,
        scope: SkillScope::User,
    }]);
...
async fn namespaces_plugin_skills_using_plugin_name() {
    let REDACTED_SECRET = tempfile::tempdir().expect("tempdir");
    let plugin_REDACTED_SECRET = REDACTED_SECRET.path().join("plugins/sample");
    let skill_path = write_raw_skill_at(
    ...
    let outcome = load_skills_from_REDACTED_SECRETs([SkillRoot {
        path: plugin_REDACTED_SECRET.join("skills"),
        scope: SkillScope::User,
    }]);
...
#[tokio::test]
async fn loads_skills_from_repo_REDACTED_SECRET() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());

    let skills_REDACTED_SECRET = repo_dir
        .path()
        .join(REPO_ROOT_CONFIG_DIR_NAME)
        .join(SKILLS_DIR_NAME);
    let skill_path = write_skill_at(&skills_REDACTED_SECRET, "repo", "repo-skill", "from repo");
    let cfg = make_config_for_cwd(&codex_home, repo_dir.path().to_path_buf()).await;
...
#[tokio::test]
async fn loads_skills_from_all_codex_dirs_under_project_REDACTED_SECRET() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
    ...
    let REDACTED_SECRET_skill_path = write_skill_at(
        &repo_dir
            .path()
            .join(REPO_ROOT_CONFIG_DIR_NAME)
            .join(SKILLS_DIR_NAME),
        "REDACTED_SECRET",
        "REDACTED_SECRET-skill",
    ...
    assert_eq!(
        ...
        vec![
            ...
            SkillMetadata {
                name: "REDACTED_SECRET-skill".to_string(),
                description: "from REDACTED_SECRET".to_string(),
                short_description: None,
                interface: None,
                dependencies: None,
                policy: None,
                path_to_skills_md: normalized(&REDACTED_SECRET_skill_path),
                scope: SkillScope::Repo,
            },
        ]
    );
...
#[tokio::test]
async fn deduplicates_by_path_preferring_first_REDACTED_SECRET() {
    let REDACTED_SECRET = tempfile::tempdir().expect("tempdir");

    let skill_path = write_skill_at(REDACTED_SECRET.path(), "dupe", "dupe-skill", "from repo");

    let outcome = load_skills_from_REDACTED_SECRETs([
        SkillRoot {
            path: REDACTED_SECRET.path().to_path_buf(),
            scope: SkillScope::Repo,
        },
        SkillRoot {
            path: REDACTED_SECRET.path().to_path_buf(),
...
async fn keeps_duplicate_names_from_nested_codex_dirs() {
    ...
    let REDACTED_SECRET_path = canonicalize_path(&REDACTED_SECRET_skill_path).unwrap_or_else(|_| REDACTED_SECRET_skill_path.clone());
    let nested_path =
    ...
    let (first_path, second_path, first_description, second_description) =
        if REDACTED_SECRET_path <= nested_path {
            (REDACTED_SECRET_path, nested_path, "from REDACTED_SECRET", "from nested")
        } else {
            (nested_path, REDACTED_SECRET_path, "from nested", "from REDACTED_SECRET")
        };
    assert_eq!(
        outcome.skills,
        vec![
...
#[tokio::test]
async fn repo_skills_search_does_not_escape_repo_REDACTED_SECRET() {
    let codex_home = tempfile::tempdir().expect("tempdir");
...
#[tokio::test]
async fn skill_REDACTED_SECRETs_include_admin_with_lowest_priority() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    let cfg = make_config(&codex_home).await;

    let scopes: Vec<SkillScope> = super::skill_REDACTED_SECRETs(&cfg.config_layer_stack, &cfg.cwd, Vec:REDACTED_SECRET))
        .into_iter()
        .map(|REDACTED_SECRET| REDACTED_SECRET.scope)
        .collect();
