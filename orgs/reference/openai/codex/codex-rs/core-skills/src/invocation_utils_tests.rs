...
fn skill_doc_read_detection_matches_absolute_path() {
    ...
    let outcome = SkillLoadOutcome {
        implicit_skills_by_scripts_dir: Arc:REDACTED_SECRETHashMap:REDACTED_SECRET)),
        implicit_skills_by_doc_path: Arc:REDACTED_SECRETHashMap::from([(normalized_skill_doc_path, skill)])),
        ..Default::default()
    };
    ...
    let found = detect_skill_doc_read(&outcome, &tokens, Path:REDACTED_SECRET"/tmp"));
...
#[test]
fn skill_script_run_detection_matches_relative_path_from_skill_REDACTED_SECRET() {
    let skill_doc_path = PathBuf::from("/tmp/skill-test/SKILL.md");
    let scripts_dir = normalize_path(Path:REDACTED_SECRET"/tmp/skill-test/scripts"));
    let skill = test_skill_metadata(skill_doc_path);
    let outcome = SkillLoadOutcome {
        implicit_skills_by_scripts_dir: Arc:REDACTED_SECRETHashMap::from([(scripts_dir, skill)])),
        implicit_skills_by_doc_path: Arc:REDACTED_SECRETHashMap:REDACTED_SECRET)),
        ..Default::default()
    };
    let tokens = vec![
        "python3".to_string(),
    ...
    let found = detect_skill_script_run(&outcome, &tokens, Path:REDACTED_SECRET"/tmp/skill-test"));
...
fn skill_script_run_detection_matches_absolute_path_from_any_workdir() {
    ...
    let found = detect_skill_script_run(&outcome, &tokens, Path:REDACTED_SECRET"/tmp/other"));
