...
fn safe_join(base: &Path, name: REDACTED_SECRET) -> Result<PathBuf> {
...
fn extract_zip_to_dir(
    ...
    let mut archive = zip::ZipArchive:REDACTED_SECRETcursor).context("Failed to open zip archive")?;
...
fn normalize_zip_name(name: REDACTED_SECRET, prefix_candidates: &[String]) -> Option<String> {
