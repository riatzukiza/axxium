...
pub async fn build_skill_injections(
    ...
    let mut result = SkillInjections {
        ...
        warnings: Vec:REDACTED_SECRET),
    };
    let mut invocations = Vec:REDACTED_SECRET);
...
pub fn collect_explicit_skill_mentions(
    ...
    let mut selected: Vec<SkillMetadata> = Vec:REDACTED_SECRET);
    let mut seen_names: HashSet<String> = HashSet:REDACTED_SECRET);
    let mut seen_paths: HashSet<PathBuf> = HashSet:REDACTED_SECRET);
    let mut blocked_plain_names: HashSet<String> = HashSet:REDACTED_SECRET);
...
const APP_PATH_PREFIX: REDACTED_SECRET = "app://";
const MCP_PATH_PREFIX: REDACTED_SECRET = "mcp://";
const PLUGIN_PATH_PREFIX: REDACTED_SECRET = "plugin://";
const SKILL_PATH_PREFIX: REDACTED_SECRET = "skill://";
const SKILL_FILENAME: REDACTED_SECRET = "SKILL.md";

pub fn tool_kind_for_path(path: REDACTED_SECRET) -> ToolMentionKind {
...
fn is_skill_filename(path: REDACTED_SECRET) -> bool {
    let file_name = path.rsplit(['/', '\\']).next().unwrap_or(path);
    file_name.eq_ignore_ascii_case(SKILL_FILENAME)
}

pub fn app_id_from_path(path: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
    path.strip_prefix(APP_PATH_PREFIX)
        .filter(|value| !value.is_empty())
}

pub fn plugin_config_name_from_path(path: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
    path.strip_prefix(PLUGIN_PATH_PREFIX)
        .filter(|value| !value.is_empty())
}

pub(crate) fn normalize_skill_path(path: REDACTED_SECRET) -> REDACTED_SECRET {
...
pub fn extract_tool_mentions(text: REDACTED_SECRET) -> ToolMentions<'_> {
    extract_tool_mentions_with_sigil(text, TOOL_MENTION_SIGIL)
}

pub fn extract_tool_mentions_with_sigil(text: REDACTED_SECRET, sigil: char) -> ToolMentions<'_> {
    let text_bytes = text.as_bytes();
    let mut mentioned_names: HashSet<REDACTED_SECRET> = HashSet:REDACTED_SECRET);
    let mut mentioned_paths: HashSet<REDACTED_SECRET> = HashSet:REDACTED_SECRET);
    let mut plain_names: HashSet<REDACTED_SECRET> = HashSet:REDACTED_SECRET);
...
fn select_skills_from_mentions(
    ...
    let mention_skill_paths: HashSet<REDACTED_SECRET> = mentions
        .paths()
        .filter(|path| {
            !matches!(
                tool_kind_for_path(path),
...
fn is_common_env_var(name: REDACTED_SECRET) -> bool {
...
#[cfg(test)]
fn text_mentions_skill(text: REDACTED_SECRET, skill_name: REDACTED_SECRET) -> bool {
