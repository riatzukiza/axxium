...
fn make_skill(name: REDACTED_SECRET, path: REDACTED_SECRET) -> SkillMetadata {
    SkillMetadata {
        name: name.to_string(),
...
fn assert_mentions(text: REDACTED_SECRET, expected_names: &[REDACTED_SECRET], expected_paths: &[REDACTED_SECRET]) {
...
fn collect_explicit_skill_mentions_text_respects_skill_order() {
    ...
    let connector_counts = HashMap:REDACTED_SECRET);

    let selected = collect_mentions(&inputs, &skills, &HashSet:REDACTED_SECRET), &connector_counts);
