...
fn write_user_skill(codex_home: &TempDir, dir: REDACTED_SECRET, name: REDACTED_SECRET, description: REDACTED_SECRET) {
...
fn write_plugin_skill(
    ...
    let plugin_REDACTED_SECRET = codex_home
        ...
        .join(plugin_name)
        .join("local");
    let skill_dir = plugin_REDACTED_SECRET.join("skills").join(dir);
...
fn test_skill(name: REDACTED_SECRET, path: PathBuf) -> SkillMetadata {
    SkillMetadata {
        name: name.to_string(),
...
fn user_config_layer(codex_home: &TempDir, config_toml: REDACTED_SECRET) -> ConfigLayerEntry {
...
fn config_stack(codex_home: &TempDir, user_config_toml: REDACTED_SECRET) -> ConfigLayerStack {
...
fn config_stack_with_session_flags(
    codex_home: &TempDir,
    user_config_toml: REDACTED_SECRET,
    session_flags_toml: REDACTED_SECRET,
) -> ConfigLayerStack {
    ConfigLayerStack:REDACTED_SECRET
        vec![
...
fn name_toggle_config(name: REDACTED_SECRET, enabled: bool) -> String {
    format!(
        r#"[[skills.config]]
name = "{name}"
enabled = {enabled}
...
fn skills_for_config_with_stack(
    ...
    effective_skill_REDACTED_SECRETs: &[PathBuf],
) -> SkillLoadOutcome {
    let skills_input = SkillsLoadInput:REDACTED_SECRET
        cwd.path().to_path_buf(),
        effective_skill_REDACTED_SECRETs.to_vec(),
...
fn new_with_disabled_bundled_skills_removes_stale_cached_system_skills() {
    ...
    let _skills_manager = SkillsManager:REDACTED_SECRET
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let skills_manager = SkillsManager:REDACTED_SECRET
        codex_home.path().to_path_buf(),
...
async fn skills_for_config_disables_plugin_skills_by_name() {
    ...
    let plugin_skill_REDACTED_SECRET = skill_path
        .parent()
        .and_then(std::path::Path::parent)
        .expect("plugin skill should live under a skills REDACTED_SECRET")
        .to_path_buf();
    let skills_manager = SkillsManager:REDACTED_SECRET
        codex_home.path().to_path_buf(),
        /*bundled_skills_enabled*/ true,
...
#[tokio::test]
async fn skills_for_cwd_reuses_cached_entry_even_when_entry_has_extra_REDACTED_SECRETs() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let extra_REDACTED_SECRET = tempfile::tempdir().expect("tempdir");
    let config_layer_stack = config_stack(&codex_home, "");
    let skills_manager = SkillsManager:REDACTED_SECRET
        codex_home.path().to_path_buf(),
        /*bundled_skills_enabled*/ true,
    ...
    write_user_skill(&extra_REDACTED_SECRET, "x", "extra-skill", "from extra REDACTED_SECRET");
    let extra_REDACTED_SECRET_path = extra_REDACTED_SECRET.path().to_path_buf();
    let base_input = SkillsLoadInput:REDACTED_SECRET
        cwd.path().to_path_buf(),
        Vec:REDACTED_SECRET),
        config_layer_stack.clone(),
        bundled_skills_enabled_from_stack(&config_layer_stack),
    ...
    let outcome_with_extra = skills_manager
        .skills_for_cwd_with_extra_user_REDACTED_SECRETs(
            &base_input,
            /*force_reload*/ true,
            std::slice::from_ref(&extra_REDACTED_SECRET_path),
        )
        .await;
    assert!(
        outcome_with_extra
    ...
    // was produced with extra REDACTED_SECRETs.
    let base_input = SkillsLoadInput:REDACTED_SECRET
        cwd.path().to_path_buf(),
        Vec:REDACTED_SECRET),
        config_layer_stack.clone(),
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    ...
    fs::write(
        ...
        "---\nname: bundled-skill\ndescription: from bundled REDACTED_SECRET\n---\n\n# Body\n",
    )
    ...
    // REDACTED_SECRET selection rather than relying on directory removal succeeding.
...
#[tokio::test]
async fn skills_for_cwd_with_extra_REDACTED_SECRETs_only_refreshes_on_force_reload() {
    let codex_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let extra_REDACTED_SECRET_a = tempfile::tempdir().expect("tempdir");
    let extra_REDACTED_SECRET_b = tempfile::tempdir().expect("tempdir");
    let config_layer_stack = config_stack(&codex_home, "");
    let skills_manager = SkillsManager:REDACTED_SECRET
        codex_home.path().to_path_buf(),
        /*bundled_skills_enabled*/ true,
    ...
    write_user_skill(&extra_REDACTED_SECRET_a, "x", "extra-skill-a", "from extra REDACTED_SECRET a");
    write_user_skill(&extra_REDACTED_SECRET_b, "x", "extra-skill-b", "from extra REDACTED_SECRET b");

    let extra_REDACTED_SECRET_a_path = extra_REDACTED_SECRET_a.path().to_path_buf();
    let base_input = SkillsLoadInput:REDACTED_SECRET
        cwd.path().to_path_buf(),
        Vec:REDACTED_SECRET),
        config_layer_stack.clone(),
    ...
    let outcome_a = skills_manager
        .skills_for_cwd_with_extra_user_REDACTED_SECRETs(
            &base_input,
            /*force_reload*/ true,
            std::slice::from_ref(&extra_REDACTED_SECRET_a_path),
        )
        .await;
    assert!(
        outcome_a
    ...
    let extra_REDACTED_SECRET_b_path = extra_REDACTED_SECRET_b.path().to_path_buf();
    let outcome_b = skills_manager
        .skills_for_cwd_with_extra_user_REDACTED_SECRETs(
            &base_input,
            /*force_reload*/ false,
            std::slice::from_ref(&extra_REDACTED_SECRET_b_path),
        )
        .await;
    assert!(
        outcome_b
...
#[test]
fn normalize_extra_user_REDACTED_SECRETs_is_stable_for_equivalent_inputs() {
    let a = PathBuf::from("/tmp/a");
    let b = PathBuf::from("/tmp/b");

    let first = normalize_extra_user_REDACTED_SECRETs(&[a.clone(), b.clone(), a.clone()]);
    let second = normalize_extra_user_REDACTED_SECRETs(&[b, a]);
...
fn disabled_paths_for_skills_allows_session_flags_to_override_user_layer() {
    ...
    let user_file = AbsolutePathBuf::try_from(tempdir.path().join("config.toml"))
        .expect("user config path should be absolute");
    let user_layer = ConfigLayerEntry:REDACTED_SECRET
        ConfigLayerSource::User { file: user_file },
        toml::from_str(&path_toggle_config(&skill_path, /*enabled*/ false))
            .expect("user layer toml"),
    );
    let session_layer = ConfigLayerEntry:REDACTED_SECRET
        ConfigLayerSource::SessionFlags,
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    ...
    let parent_input = SkillsLoadInput:REDACTED_SECRET
        cwd.path().to_path_buf(),
        Vec:REDACTED_SECRET),
        parent_stack.clone(),
