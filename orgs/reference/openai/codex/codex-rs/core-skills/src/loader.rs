...
use crate::system::system_cache_REDACTED_SECRET_dir;
use codex_app_server_protocol::ConfigLayerSource;
use codex_config::ConfigLayerStack;
use codex_config::ConfigLayerStackOrdering;
use codex_config::default_project_REDACTED_SECRET_markers;
use codex_config::merge_toml_values;
use codex_config::project_REDACTED_SECRET_markers_from_config;
...
const SKILLS_FILENAME: REDACTED_SECRET = "SKILL.md";
const AGENTS_DIR_NAME: REDACTED_SECRET = ".agents";
const SKILLS_METADATA_DIR: REDACTED_SECRET = "agents";
const SKILLS_METADATA_FILENAME: REDACTED_SECRET = "openai.yaml";
const SKILLS_DIR_NAME: REDACTED_SECRET = "skills";
...
// Traversal depth from the skills REDACTED_SECRET.
...
pub fn load_skills_from_REDACTED_SECRETs<I>(REDACTED_SECRETs: I) -> SkillLoadOutcome
where
    I: IntoIterator<Item = SkillRoot>,
{
    let mut outcome = SkillLoadOutcome::default();
    for REDACTED_SECRET in REDACTED_SECRETs {
        discover_skills_under_REDACTED_SECRET(&REDACTED_SECRET.path, REDACTED_SECRET.scope, &mut outcome);
    }

    let mut seen: HashSet<PathBuf> = HashSet:REDACTED_SECRET);
    outcome
        .skills
        .retain(|skill| seen.insert(skill.path_to_skills_md.clone()));

    fn scope_rank(scope: SkillScope) -> u8 {
        // Higher-priority scopes first (matches REDACTED_SECRET scan order for dedupe).
        match scope {
            SkillScope::Repo => 0,
            SkillScope::User => 1,
            SkillScope::System => 2,
...
pub(crate) fn skill_REDACTED_SECRETs(
    config_layer_stack: &ConfigLayerStack,
    cwd: &Path,
    plugin_skill_REDACTED_SECRETs: Vec<PathBuf>,
) -> Vec<SkillRoot> {
    skill_REDACTED_SECRETs_with_home_dir(
        config_layer_stack,
        cwd,
        home_dir().as_deref(),
        plugin_skill_REDACTED_SECRETs,
    )
}

fn skill_REDACTED_SECRETs_with_home_dir(
    config_layer_stack: &ConfigLayerStack,
    cwd: &Path,
    home_dir: Option<&Path>,
    plugin_skill_REDACTED_SECRETs: Vec<PathBuf>,
) -> Vec<SkillRoot> {
    let mut REDACTED_SECRETs = skill_REDACTED_SECRETs_from_layer_stack_inner(config_layer_stack, home_dir);
    REDACTED_SECRETs.extend(plugin_skill_REDACTED_SECRETs.into_iter().map(|path| SkillRoot {
        path,
        scope: SkillScope::User,
    }));
    REDACTED_SECRETs.extend(repo_agents_skill_REDACTED_SECRETs(config_layer_stack, cwd));
    dedupe_skill_REDACTED_SECRETs_by_path(&mut REDACTED_SECRETs);
    REDACTED_SECRETs
}

fn skill_REDACTED_SECRETs_from_layer_stack_inner(
    config_layer_stack: &ConfigLayerStack,
    home_dir: Option<&Path>,
) -> Vec<SkillRoot> {
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ConfigLayerSource::Project { .. } => {
                REDACTED_SECRETs.push(SkillRoot {
            ...
            ConfigLayerSource::User { .. } => {
                ...
                REDACTED_SECRETs.push(SkillRoot {
                    path: system_cache_REDACTED_SECRET_dir(config_folder.as_path()),
...
fn repo_agents_skill_REDACTED_SECRETs(config_layer_stack: &ConfigLayerStack, cwd: &Path) -> Vec<SkillRoot> {
    let project_REDACTED_SECRET_markers = project_REDACTED_SECRET_markers_from_stack(config_layer_stack);
    let project_REDACTED_SECRET = find_project_REDACTED_SECRET(cwd, &project_REDACTED_SECRET_markers);
    let dirs = dirs_between_project_REDACTED_SECRET_and_cwd(cwd, &project_REDACTED_SECRET);
    let mut REDACTED_SECRETs = Vec:REDACTED_SECRET);
    for dir in dirs {
...
fn project_REDACTED_SECRET_markers_from_stack(config_layer_stack: &ConfigLayerStack) -> Vec<String> {
    let mut merged = TomlValue::Table(toml::map::Map:REDACTED_SECRET));
    for layer in config_layer_stack.get_layers(
    ...
    match project_REDACTED_SECRET_markers_from_config(&merged) {
        Ok(Some(markers)) => markers,
        Ok(None) => default_project_REDACTED_SECRET_markers(),
        Err(err) => {
            tracing::warn!("invalid project_REDACTED_SECRET_markers: {err}");
            default_project_REDACTED_SECRET_markers()
        }
    }
}

fn find_project_REDACTED_SECRET(cwd: &Path, project_REDACTED_SECRET_markers: &[String]) -> PathBuf {
    if project_REDACTED_SECRET_markers.is_empty() {
        return cwd.to_path_buf();
    }

    for ancestor in cwd.ancestors() {
        for marker in project_REDACTED_SECRET_markers {
...
fn dirs_between_project_REDACTED_SECRET_and_cwd(cwd: &Path, project_REDACTED_SECRET: &Path) -> Vec<PathBuf> {
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
...
fn dedupe_skill_REDACTED_SECRETs_by_path(REDACTED_SECRETs: &mut Vec<SkillRoot>) {
    let mut seen: HashSet<PathBuf> = HashSet:REDACTED_SECRET);
    REDACTED_SECRETs.retain(|REDACTED_SECRET| seen.insert(REDACTED_SECRET.path.clone()));
}

fn discover_skills_under_REDACTED_SECRET(REDACTED_SECRET: &Path, scope: SkillScope, outcome: &mut SkillLoadOutcome) {
    let Ok(REDACTED_SECRET) = canonicalize_path(REDACTED_SECRET) else {
        return;
    };

    if !REDACTED_SECRET.is_dir() {
        return;
    }
    ...
    let mut visited_dirs: HashSet<PathBuf> = HashSet:REDACTED_SECRET);
    visited_dirs.insert(REDACTED_SECRET.clone());

    let mut queue: VecDeque<(PathBuf, usize)> = VecDeque::from([(REDACTED_SECRET.clone(), 0)]);
    ...
    if truncated_by_dir_limit {
        tracing::warn!(
            "skills scan truncated after {} directories (REDACTED_SECRET: {})",
...
fn namespaced_skill_name(path: &Path, base_name: REDACTED_SECRET) -> String {
...
fn load_skill_metadata(skill_path: &Path) -> LoadedSkillMetadata {
    ...
    let parsed: SkillMetadataFile = {
        let _guard = AbsolutePathBufGuard:REDACTED_SECRETskill_dir);
...
fn resolve_asset_path(
    ...
    let mut normalized = PathBuf:REDACTED_SECRET);
...
fn sanitize_single_line(raw: REDACTED_SECRET) -> String {
...
fn extract_frontmatter(contents: REDACTED_SECRET) -> Option<String> {
    let mut lines = contents.lines();
    if !matches!(lines.next(), Some(line) if line.trim() == "---") {
        return None;
    }

    let mut frontmatter_lines: Vec<REDACTED_SECRET> = Vec:REDACTED_SECRET);
...
#[cfg(test)]
pub(crate) fn skill_REDACTED_SECRETs_from_layer_stack(
    config_layer_stack: &ConfigLayerStack,
    home_dir: Option<&Path>,
) -> Vec<SkillRoot> {
    skill_REDACTED_SECRETs_with_home_dir(config_layer_stack, Path:REDACTED_SECRET"."), home_dir, Vec:REDACTED_SECRET))
}
