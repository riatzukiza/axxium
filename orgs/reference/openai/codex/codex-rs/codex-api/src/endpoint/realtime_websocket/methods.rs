...
impl RealtimeWebsocketConnection {
    ...
    fn new(
        ...
        let stream = Arc:REDACTED_SECRETstream);
        let is_closed = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        Self {
            writer: RealtimeWebsocketWriter {
                stream: Arc::clone(REDACTED_SECRETeam),
                is_closed: Arc::clone(&is_closed),
                event_parser,
            },
            events: RealtimeWebsocketEvents {
                rx_message: Arc:REDACTED_SECRETMutex:REDACTED_SECRETrx_message)),
                active_transcript: Arc:REDACTED_SECRETMutex:REDACTED_SECRETActiveTranscriptState::default())),
                event_parser,
                is_closed,
            },
        }
...
fn append_transcript_delta(entries: &mut Vec<RealtimeTranscriptEntry>, role: REDACTED_SECRET, delta: REDACTED_SECRET) {
    if delta.is_empty() {
        return;
    }
...
impl RealtimeWebsocketClient {
    ...
    pub async fn connect(
        ...
        let (stream, rx_message) = WsStream:REDACTED_SECRETstream);
        let connection = RealtimeWebsocketConnection:REDACTED_SECRETstream, rx_message, config.event_parser);
        debug!(
...
fn with_session_id_header(
    ...
    session_id: Option<REDACTED_SECRET>,
...
mod tests {
    ...
    fn parse_handoff_requested_event() {
        ...
        assert_eq!(
            ...
            Some(RealtimeEvent::HandoffRequested(RealtimeHandoffRequested {
                ...
                active_transcript: Vec:REDACTED_SECRET),
            }))
        );
    }
    ...
    async fn e2e_connect_and_exchange_events_against_mock_ws_server() {
        ...
        let provider = Provider {
            ...
            query_params: Some(HashMap:REDACTED_SECRET)),
        ...
        let client = RealtimeWebsocketClient:REDACTED_SECRETprovider);
