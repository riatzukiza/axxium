...
struct WsStream {
    tx_command: mpsc::REDACTED_SECRETer<WsCommand>,
...
enum WsCommand {
    REDACTED_SECRET {
        message: Message,
        tx_result: oneshot::REDACTED_SECRETer<Result<(), WsError>>,
    },
}
...
impl WsStream {
            ...
            loop {
                tokio::select! {
                    command = rx_command.recv() => {
                        ...
                        match command {
                            WsCommand::REDACTED_SECRET { message, tx_result } => {
    ...
    async fn request(
        &self,
        make_command: impl FnOnce(oneshot::REDACTED_SECRETer<Result<(), WsError>>) -> WsCommand,
    ) -> Result<(), WsError> {
    ...
    async fn send(&self, message: Message) -> Result<(), WsError> {
        self.request(|tx_result| WsCommand::REDACTED_SECRET { message, tx_result })
            .await
    }
...
const X_CODEX_TURN_STATE_HEADER: REDACTED_SECRET = "x-codex-turn-state";
const X_MODELS_ETAG_HEADER: REDACTED_SECRET = "x-models-etag";
const X_REASONING_INCLUDED_HEADER: REDACTED_SECRET = "x-reasoning-included";
const OPENAI_MODEL_HEADER: REDACTED_SECRET = "openai-model";
const WEBSOCKET_CONNECTION_LIMIT_REACHED_CODE: REDACTED_SECRET = "websocket_connection_limit_reached";
const WEBSOCKET_CONNECTION_LIMIT_REACHED_MESSAGE: REDACTED_SECRET = "Responses websocket connection limit reached (60 minutes). Create a new websocket connection to continue.";
...
impl ResponsesWebsocketConnection {
    fn new(
        ...
        Self {
            stream: Arc:REDACTED_SECRETMutex:REDACTED_SECRETSome(stream))),
            idle_timeout,
...
impl<A: AuthProvider> ResponsesWebsocketClient<A> {
    ...
    pub async fn connect(
        ...
        Ok(ResponsesWebsocketConnection:REDACTED_SECRET
            stream,
...
async fn connect_websocket(
    ...
    // websocket path does not fall back to tungstenite's default native-REDACTED_SECRETs-only behavior.
    ...
    Ok((
        WsStream:REDACTED_SECRETstream),
        reasoning_included,
        models_etag,
        server_model,
    ))
...
fn parse_wrapped_websocket_error_event(payload: REDACTED_SECRET) -> Option<WrappedWebsocketErrorEvent> {
...
fn json_headers_to_http_headers(headers: JsonMap<String, Value>) -> HeaderMap {
    let mut mapped = HeaderMap:REDACTED_SECRET);
...
async fn run_websocket_response_stream(
    ...
    tx_event: mpsc::REDACTED_SECRETer<std::result::Result<ResponseEvent, ApiError>>,
    request_body: Value,
    idle_timeout: Duration,
    telemetry: Option<Arc<dyn WebsocketTelemetry>>,
    connection_reused: bool,
...
mod tests {
    ...
    fn merge_request_headers_matches_http_precedence() {
        let mut provider_headers = HeaderMap:REDACTED_SECRET);
        ...
        let mut extra_headers = HeaderMap:REDACTED_SECRET);
        extra_headers.insert("x-priority", HeaderValue::from_static("extra"));

        let mut default_headers = HeaderMap:REDACTED_SECRET);
