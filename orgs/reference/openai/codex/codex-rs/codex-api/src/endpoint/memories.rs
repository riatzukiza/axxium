...
mod tests {
    ...
    impl CapturingTransport {
        fn new(response_body: Vec<u8>) -> Self {
            Self {
                ...
                response_body: Arc:REDACTED_SECRETresponse_body),
            }
        }
    }
    ...
    async fn summarize_input_posts_expected_payload_and_parses_output() {
        let transport = CapturingTransport:REDACTED_SECRET
        ...
        let client = MemoriesClient:REDACTED_SECRET
            transport.clone(),
        ...
        let output = client
            .summarize_input(&input, HeaderMap:REDACTED_SECRET))
            .await
