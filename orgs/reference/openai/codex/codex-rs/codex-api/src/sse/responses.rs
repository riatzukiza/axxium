...
pub fn stream_from_fixture(
    ...
    for line in std::io::BufReader:REDACTED_SECRETfile).lines() {
        let line = line.map_err(|err| ApiError::Stream(err.to_string()))?;
        content.push_str(&line);
        content.push_str("\n\n");
    }

    let reader = std::io::Cursor:REDACTED_SECRETcontent);
    let stream = ReaderStream:REDACTED_SECRETreader).map_err(|err| TransportError::Network(err.to_string()));
...
pub fn spawn_response_stream(
    ...
    let rate_limit_snapshots = parse_all_rate_limits(REDACTED_SECRETeam_response.headers);
...
impl ResponsesStreamEvent {
    pub fn kind(&self) -> REDACTED_SECRET {
        &self.kind
    }
...
pub async fn process_sse(
    stream: ByteStream,
    tx_event: mpsc::REDACTED_SECRETer<Result<ResponseEvent, ApiError>>,
    idle_timeout: Duration,
    telemetry: Option<Arc<dyn SseTelemetry>>,
) {
...
fn rate_limit_regex() -> &'static regex_lite::Regex {
    ...
    RE.get_or_init(|| {
        regex_lite::Regex:REDACTED_SECRETr"(?i)try again in\s*(\d+(?:\.\d+)?)\s*(s|ms|seconds?)").unwrap()
    })
}
...
mod tests {
    ...
    async fn collect_events(chunks: &[&[u8]]) -> Vec<Result<ResponseEvent, ApiError>> {
        let mut builder = IoBuilder:REDACTED_SECRET);
        for chunk in chunks {
            builder.read(chunk);
        }
        ...
        let stream =
            ReaderStream:REDACTED_SECRETreader).map_err(|err| TransportError::Network(err.to_string()));
    ...
    async fn run_sse(events: Vec<serde_json::Value>) -> Vec<ResponseEvent> {
        let mut body = String:REDACTED_SECRET);
        for e in events {
            let kind = e
                .get("type")
                .and_then(|v| v.as_str())
        ...
        let stream = ReaderStream:REDACTED_SECRETstd::io::Cursor:REDACTED_SECRETbody))
    ...
    async fn spawn_response_stream_emits_server_model_header() {
        ...
        let bytes = stream::iter(Vec::<Result<Bytes, TransportError>>:REDACTED_SECRET));
    ...
    const CYBER_RESTRICTED_MODEL_FOR_TESTS: REDACTED_SECRET = "gpt-5.3-codex";
}
