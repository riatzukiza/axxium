...
fn assert_path_ends_with(requests: &[Request], suffix: REDACTED_SECRET) {
...
impl HttpTransport for RecordingTransport {
    ...
    async fn stream(&self, req: Request) -> Result<StreamResponse, TransportError> {
        ...
        let stream = futures::stream::iter(Vec::<Result<Bytes, TransportError>>:REDACTED_SECRET));
        Ok(StreamResponse {
...
impl StaticAuth {
    fn new(token: REDACTED_SECRET, account_id: REDACTED_SECRET) -> Self {
        Self {
...
fn provider(name: REDACTED_SECRET) -> Provider {
    Provider {
        name: name.to_string(),
...
impl FlakyTransport {
    fn new() -> Self {
        Self {
            state: Arc:REDACTED_SECRETMutex:REDACTED_SECRET0)),
        }
    }
...
async fn responses_client_uses_responses_path() -> Result<()> {
    ...
    let transport = RecordingTransport:REDACTED_SECRETstate.clone());
    let client = ResponsesClient:REDACTED_SECRETtransport, provider("openai"), NoAuth);
...
async fn streaming_client_adds_auth_headers() -> Result<()> {
    let state = RecordingState::default();
    let transport = RecordingTransport:REDACTED_SECRETstate.clone());
    let auth = StaticAuth:REDACTED_SECRET"secret-token", "acct-1");
    let client = ResponsesClient:REDACTED_SECRETtransport, provider("openai"), auth);
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    let transport = FlakyTransport:REDACTED_SECRET);
    ...
    let request = ResponsesApiRequest {
        ...
        store: false,
        stream: true,
        include: Vec:REDACTED_SECRET),
        service_tier: None,
        prompt_cache_key: None,
        text: None,
    };
    let client = ResponsesClient:REDACTED_SECRETtransport.clone(), provider, NoAuth);
...
async fn azure_default_store_attaches_ids_and_headers() -> Result<()> {
    let state = RecordingState::default();
    let transport = RecordingTransport:REDACTED_SECRETstate.clone());
    let client = ResponsesClient:REDACTED_SECRETtransport, provider("azure"), NoAuth);
