...
async fn models_client_hits_models_endpoint() {
    ...
    let transport = ReqwestTransport:REDACTED_SECRETreqwest::Client:REDACTED_SECRET));
    let client = ModelsClient:REDACTED_SECRETtransport, provider(&base_url), DummyAuth);
