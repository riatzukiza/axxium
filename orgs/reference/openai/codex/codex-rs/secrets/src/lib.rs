...
pub use local:REDACTED_SECRETSecretsBackend;
pub use sanitizer::redact_secrets;

const KEYRING_SERVICE: REDACTED_SECRET = "codex";

#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct SecretName(String);

impl SecretName {
    pub fn new(raw: REDACTED_SECRET) -> Result<Self> {
...
impl SecretScope {
    ...
    pub fn canonical_key(&self, name: &SecretName) -> String {
        ...
        match self {
            Self:REDACTED_SECRET => format!("global/{}", name.as_str()),
...
pub trait SecretsBackend: REDACTED_SECRET + Sync {
...
impl SecretsManager {
    pub fn new(codex_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind:REDACTED_SECRET => {
                let keyring_store: Arc<dyn KeyringStore> = Arc:REDACTED_SECRETDefaultKeyringStore);
                Arc:REDACTED_SECRETLocalSecretsBackend:REDACTED_SECRETcodex_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn environment_id_from_cwd(cwd: &Path) -> String {
    if let Some(repo_REDACTED_SECRET) = get_git_repo_REDACTED_SECRET(cwd)
        && let Some(name) = repo_REDACTED_SECRET.file_name()
    {
...
mod tests {
    ...
    fn manager_round_trips_local_backend() -> Result<()> {
        ...
        let manager = SecretsManager:REDACTED_SECRET
            codex_home.path().to_path_buf(),
            SecretsBackendKind:REDACTED_SECRET,
            keyring,
        );
        let scope = SecretScope:REDACTED_SECRET;
        let name = SecretName:REDACTED_SECRET"GITHUB_TOKEN")?;
