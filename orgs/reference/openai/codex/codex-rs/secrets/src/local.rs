...
const LOCAL_SECRETS_FILENAME: REDACTED_SECRET = "local.age";

#[derive(Debug, Clone, Default, Serialize, Deserialize, PartialEq, Eq)]
struct SecretsFile {
    version: u8,
    secrets: REDACTED_SECRET, String>,
}
...
impl SecretsFile {
    fn new_empty() -> Self {
        Self {
            ...
            secrets: BTreeMap:REDACTED_SECRET),
        }
    }
}
...
impl LocalSecretsBackend {
    ...
    pub fn set(&self, scope: &SecretScope, name: &SecretName, value: REDACTED_SECRET) -> Result<()> {
    ...
    fn load_file(&self) -> Result<SecretsFile> {
        ...
        if !path.exists() {
            return Ok(SecretsFile:REDACTED_SECRET));
        }
        ...
        let mut parsed: SecretsFile = REDACTED_SECRET).with_context(|| {
            format!(
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        ...
        match loaded {
            Some(existing) => Ok(SecretString:REDACTED_SECRET)),
            None => {
...
impl SecretsBackend for LocalSecretsBackend {
    fn set(&self, scope: &SecretScope, name: &SecretName, value: REDACTED_SECRET) -> Result<()> {
        LocalSecretsBackend:REDACTED_SECRET, scope, name, value)
    }

    fn get(&self, scope: &SecretScope, name: &SecretName) -> Result<Option<String>> {
        LocalSecretsBackend:REDACTED_SECRET, scope, name)
    }
    ...
    fn list(&self, scope_filter: Option<&SecretScope>) -> Result<Vec<SecretListEntry>> {
        LocalSecretsBackend:REDACTED_SECRET, scope_filter)
    }
}
...
fn write_file_atomically(path: &Path, contents: &[u8]) -> Result<()> {
    ...
    {
        let mut tmp_file = fs::OpenOptions:REDACTED_SECRET)
            .create_new(true)
            .write(true)
            .open(&tmp_path)
            .with_context(|| {
...
fn generate_passphrase() -> Result<SecretString> {
    ...
    Ok(SecretString:REDACTED_SECRET))
}
...
fn encrypt_with_passphrase(plaintext: &[u8], passphrase: &SecretString) -> Result<Vec<u8>> {
    let recipient = ScryptRecipient:REDACTED_SECRETpassphrase.clone());
...
fn decrypt_with_passphrase(ciphertext: &[u8], passphrase: &SecretString) -> Result<Vec<u8>> {
    let identity = ScryptIdentity:REDACTED_SECRETpassphrase.clone());
    decrypt(&identity, ciphertext).context("failed to decrypt secrets file")
}

fn parse_canonical_key(canonical_key: REDACTED_SECRET) -> Option<SecretListEntry> {
    ...
    match scope_kind {
        "global" => {
            ...
            let name = SecretName:REDACTED_SECRETname).ok()?;
            Some(SecretListEntry {
                scope: SecretScope:REDACTED_SECRET,
                name,
            })
        }
        "env" => {
            ...
            let name = SecretName:REDACTED_SECRETname).ok()?;
            let scope = SecretScope:REDACTED_SECRET)).ok()?;
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let keyring = Arc:REDACTED_SECRETMockKeyringStore::default());
        let backend = LocalSecretsBackend:REDACTED_SECRETcodex_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        ...
        let backend = LocalSecretsBackend:REDACTED_SECRETcodex_home.path().to_path_buf(), keyring);
        let scope = SecretScope:REDACTED_SECRET;
        let name = SecretName:REDACTED_SECRET"TEST_SECRET")?;
        let error = backend
    ...
    fn save_file_does_not_leave_temp_files() -> Result<()> {
        ...
        let secrets_dir = REDACTED_SECRET);
