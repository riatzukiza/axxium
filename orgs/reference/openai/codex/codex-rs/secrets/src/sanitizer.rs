...
static OPENAI_KEY_REGEX: REDACTED_SECRET = LazyLock:REDACTED_SECRET|| compile_regex(r"sk-[A-Za-z0-9]{20,}"));
static AWS_ACCESS_KEY_ID_REGEX: REDACTED_SECRET =
    LazyLock:REDACTED_SECRET|| compile_regex(r"\bAKIA[0-9A-Z]{16}\b"));
static BEARER_TOKEN_REGEX: REDACTED_SECRET =
    LazyLock:REDACTED_SECRET|| compile_regex(r"(?i)\bBearer\s+[A-Za-z0-9._\-]{16,}\b"));
static SECRET_ASSIGNMENT_REGEX: REDACTED_SECRET = LazyLock:REDACTED_SECRET|| {
...
fn compile_regex(pattern: REDACTED_SECRET) -> Regex {
    match Regex:REDACTED_SECRETpattern) {
        Ok(regex) => regex,
