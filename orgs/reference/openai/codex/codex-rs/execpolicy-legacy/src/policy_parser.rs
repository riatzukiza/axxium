...
impl PolicyParser {
    pub fn new(policy_source: REDACTED_SECRET, unparsed_policy: REDACTED_SECRET) -> Self {
        Self {
    ...
    pub fn parse(&self) -> starlark::Result<Policy> {
        ...
        let policy_builder = PolicyBuilder:REDACTED_SECRET);
        {
...
impl PolicyBuilder {
    fn new() -> Self {
        Self {
            programs: RefCell:REDACTED_SECRETMultiMap:REDACTED_SECRET)),
            forbidden_program_regexes: RefCell:REDACTED_SECRETVec:REDACTED_SECRET)),
            forbidden_substrings: RefCell:REDACTED_SECRETVec:REDACTED_SECRET)),
        }
    }
    ...
    fn build(self) -> Result<Policy, regex_lite::Error> {
        ...
        Policy:REDACTED_SECRETprograms, forbidden_program_regexes, forbidden_substrings)
    }
...
fn policy_builtins(builder: &mut GlobalsBuilder) {
    fn define_program<'v>(
        ...
        let mut allowed_options = HashMap::<String, Opt>:REDACTED_SECRET);
        for opt in options {
        ...
        let program_spec = ProgramSpec:REDACTED_SECRET
            program,
            system_path,
            option_bundling,
            combined_format,
    ...
    fn forbid_substrings(
        ...
        let policy_builder = eval
            ...
            .downcast_ref::<PolicyBuilder>()
            .unwrap();
        policy_builder.add_forbidden_substrings(REDACTED_SECRETings.items.to_vec());
        Ok(NoneType)
    }
    ...
    fn forbid_program_regex(
        ...
        let policy_builder = eval
            ...
            .downcast_ref::<PolicyBuilder>()
            .unwrap();
        let compiled_regex = regex_lite::Regex:REDACTED_SECRET&regex)?;
    ...
    fn flag(name: String) -> anyhow::Result<Opt> {
        Ok(Opt:REDACTED_SECRETname, OptMeta::Flag, /*required*/ false))
    }
}
