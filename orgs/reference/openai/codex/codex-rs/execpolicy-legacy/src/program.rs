...
impl ProgramSpec {
    ...
    pub fn check(&self, exec_call: &ExecCall) -> Result<MatchedExec> {
        ...
        let mut args = Vec::<PositionalArg>:REDACTED_SECRET);
        let mut matched_flags = Vec::<MatchedFlag>:REDACTED_SECRET);
        let mut matched_opts = Vec::<MatchedOpt>:REDACTED_SECRET);
        ...
        for (index, arg) in exec_call.args.iter().enumerate() {
            if let Some(expected) = expecting_option_value {
                ...
                matched_opts.push(MatchedOpt:REDACTED_SECRET&name, arg, arg_type)?);
    ...
    pub fn verify_should_match_list(&self) -> Vec<PositiveExampleFailedCheck> {
        let mut violations = Vec:REDACTED_SECRET);
