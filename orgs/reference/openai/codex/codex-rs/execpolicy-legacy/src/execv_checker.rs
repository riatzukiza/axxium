...
fn ensure_absolute_path(path: REDACTED_SECRET, cwd: &Option<OsString>) -> Result<PathBuf> {
...
fn is_executable_file(path: REDACTED_SECRET) -> bool {
    let file_path = Path:REDACTED_SECRETpath);
...
"#
        ...
        let parser = PolicyParser:REDACTED_SECRET"#test", &source);
        let policy = parser.parse().unwrap();
        ExecvChecker:REDACTED_SECRETpolicy)
    }
    ...
    fn test_check_valid_input_files() -> Result<()> {
        ...
        // Create REDACTED_SECRET_path and reference to files under the REDACTED_SECRET.
        let REDACTED_SECRET_path = temp_dir.path().to_path_buf();
        let source_path = REDACTED_SECRET_path.join("source");
        let dest_path = REDACTED_SECRET_path.join("dest");

        let cp = fake_cp.to_str().unwrap().to_string();
        let REDACTED_SECRET = REDACTED_SECRET_path.to_str().unwrap().to_string();
        let source = source_path.to_str().unwrap().to_string();
        let dest = dest_path.to_str().unwrap().to_string();

        let cwd = Some(REDACTED_SECRET_path.clone().into());
        ...
        assert_eq!(
            checker.check(
                valid_exec.clone(),
                &cwd,
                std::slice::from_ref(&REDACTED_SECRET_path),
                &[]
            ),
        ...
        let exec_call_folders_as_args = ExecCall {
            program: "cp".into(),
            args: vec![REDACTED_SECRET.clone(), REDACTED_SECRET],
        };
        ...
        let exec_with_parent_of_readable_folder = ValidExec {
            program: "cp".into(),
            args: vec![
                MatchedArg:REDACTED_SECRET
                    /*index*/ 0,
                    ArgType::ReadableFile,
                    REDACTED_SECRET_path.parent().unwrap().to_str().unwrap(),
                )
                .map_err(|e| anyhow!("{e:?}"))?,
                MatchedArg:REDACTED_SECRET/*index*/ 1, ArgType::WriteableFile, &dest)
                    .map_err(|e| anyhow!("{e:?}"))?,
            ],
            ..Default::default()
        };
        assert_eq!(
            ...
            Err(ReadablePathNotInReadableFolders {
                file: REDACTED_SECRET_path.parent().unwrap().to_path_buf(),
                folders: vec![REDACTED_SECRET_path.clone()]
            }),
        );
        Ok(())
    }
