...
impl ValidExec {
    pub fn new(program: REDACTED_SECRET, args: Vec<MatchedArg>, system_path: &[REDACTED_SECRET]) -> Self {
        Self {
...
impl MatchedArg {
    pub fn new(index: usize, r#type: ArgType, value: REDACTED_SECRET) -> Result<Self> {
...
impl MatchedOpt {
    pub fn new(name: REDACTED_SECRET, value: REDACTED_SECRET, r#type: ArgType) -> Result<Self> {
        r#type.validate(value)?;
        Ok(Self {
            name: name.to_string(),
            value: value.to_string(),
...
impl MatchedFlag {
    pub fn new(name: REDACTED_SECRET) -> Self {
        Self {
            name: name.to_string(),
        }
    }
