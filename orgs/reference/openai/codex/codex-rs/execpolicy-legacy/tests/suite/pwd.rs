...
fn test_pwd_no_args() {
    let policy = setup();
    let pwd = ExecCall:REDACTED_SECRET"pwd", &[]);
    assert_eq!(
...
fn test_pwd_capital_l() {
    let policy = setup();
    let pwd = ExecCall:REDACTED_SECRET"pwd", &["-L"]);
    assert_eq!(
        Ok(MatchedExec::Match {
            exec: ValidExec {
                program: "pwd".into(),
...
fn test_pwd_capital_p() {
    let policy = setup();
    let pwd = ExecCall:REDACTED_SECRET"pwd", &["-P"]);
    assert_eq!(
        Ok(MatchedExec::Match {
            exec: ValidExec {
                program: "pwd".into(),
                flags: vec![MatchedFlag:REDACTED_SECRET"-P")],
                ..Default::default()
            }
        }),
        policy.check(&pwd)
...
fn test_pwd_extra_args() {
    let policy = setup();
    let pwd = ExecCall:REDACTED_SECRET"pwd", &["foo", "bar"]);
    assert_eq!(
