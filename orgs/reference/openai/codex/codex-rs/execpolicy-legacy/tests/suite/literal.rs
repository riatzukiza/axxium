...
"#;
    let parser = PolicyParser:REDACTED_SECRET"test_invalid_subcommand", unparsed_policy);
    let policy = parser.parse().expect("failed to parse policy");
    let valid_call = ExecCall:REDACTED_SECRET"fake_executable", &["subcommand", "sub-subcommand"]);
    assert_eq!(
    ...
    let invalid_call = ExecCall:REDACTED_SECRET"fake_executable", &["subcommand", "not-a-real-subcommand"]);
    assert_eq!(
