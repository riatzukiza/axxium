...
fn test_head_no_args() {
    let policy = setup();
    let head = ExecCall:REDACTED_SECRET"head", &[]);
...
fn test_head_one_file_no_flags() -> Result<()> {
    let policy = setup();
    let head = ExecCall:REDACTED_SECRET"head", &["src/extension.ts"]);
    assert_eq!(
...
fn test_head_one_flag_one_file() -> Result<()> {
    let policy = setup();
    let head = ExecCall:REDACTED_SECRET"head", &["-n", "100", "src/extension.ts"]);
    assert_eq!(
        Ok(MatchedExec::Match {
            exec: ValidExec {
                program: "head".to_string(),
                flags: vec![],
                opts: vec![
                    MatchedOpt:REDACTED_SECRET"-n", "100", ArgType::PositiveInteger)
...
fn test_head_invalid_n_as_0() {
    let policy = setup();
    let head = ExecCall:REDACTED_SECRET"head", &["-n", "0", "src/extension.ts"]);
    assert_eq!(
...
fn test_head_invalid_n_as_nonint_float() {
    let policy = setup();
    let head = ExecCall:REDACTED_SECRET"head", &["-n", "1.5", "src/extension.ts"]);
    assert_eq!(
        Err(Error::InvalidPositiveInteger {
            value: "1.5".to_string(),
        }),
...
fn test_head_invalid_n_as_float() {
    let policy = setup();
    let head = ExecCall:REDACTED_SECRET"head", &["-n", "1.0", "src/extension.ts"]);
    assert_eq!(
        Err(Error::InvalidPositiveInteger {
            value: "1.0".to_string(),
        }),
...
fn test_head_invalid_n_as_negative_int() {
    let policy = setup();
    let head = ExecCall:REDACTED_SECRET"head", &["-n", "-1", "src/extension.ts"]);
    assert_eq!(
