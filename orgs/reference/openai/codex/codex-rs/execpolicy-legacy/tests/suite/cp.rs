...
fn test_cp_no_args() {
    let policy = setup();
    let cp = ExecCall:REDACTED_SECRET"cp", &[]);
    assert_eq!(
...
fn test_cp_one_arg() {
    let policy = setup();
    let cp = ExecCall:REDACTED_SECRET"cp", &["foo/bar"]);
...
fn test_cp_one_file() -> Result<()> {
    let policy = setup();
    let cp = ExecCall:REDACTED_SECRET"cp", &["foo/bar", "../baz"]);
    assert_eq!(
        Ok(MatchedExec::Match {
            exec: ValidExec:REDACTED_SECRET
                "cp",
                vec![
                    MatchedArg:REDACTED_SECRET/*index*/ 0, ArgType::ReadableFile, "foo/bar")?,
                    MatchedArg:REDACTED_SECRET/*index*/ 1, ArgType::WriteableFile, "../baz")?,
                ],
                &["/bin/cp", "/usr/bin/cp"]
            )
        }),
        ...
        Ok(MatchedExec::Match {
            exec: ValidExec:REDACTED_SECRET
                ...
                vec![
                    ...
                    MatchedArg:REDACTED_SECRET/*index*/ 2, ArgType::WriteableFile, "baz")?,
                ],
                &["/bin/cp", "/usr/bin/cp"]
            )
        }),
