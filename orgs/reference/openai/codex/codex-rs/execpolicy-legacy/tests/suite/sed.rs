...
fn test_sed_print_specific_lines() -> Result<()> {
    let policy = setup();
    let sed = ExecCall:REDACTED_SECRET"sed", &["-n", "122,202p", "hello.txt"]);
    assert_eq!(
        Ok(MatchedExec::Match {
            exec: ValidExec {
                program: "sed".to_string(),
                flags: vec![MatchedFlag:REDACTED_SECRET"-n")],
                args: vec![
                    MatchedArg:REDACTED_SECRET/*index*/ 1, ArgType::SedCommand, "122,202p")?,
                    MatchedArg:REDACTED_SECRET/*index*/ 2, ArgType::ReadableFile, "hello.txt")?,
                ],
...
fn test_sed_print_specific_lines_with_e_flag() -> Result<()> {
    let policy = setup();
    let sed = ExecCall:REDACTED_SECRET"sed", &["-n", "-e", "122,202p", "hello.txt"]);
    assert_eq!(
        Ok(MatchedExec::Match {
            exec: ValidExec {
                program: "sed".to_string(),
                flags: vec![MatchedFlag:REDACTED_SECRET"-n")],
                opts: vec![
                    MatchedOpt:REDACTED_SECRET"-e", "122,202p", ArgType::SedCommand)
...
fn test_sed_reject_dangerous_command() {
    let policy = setup();
    let sed = ExecCall:REDACTED_SECRET"sed", &["-e", "s/y/echo hi/e", "hello.txt"]);
    assert_eq!(
...
fn test_sed_verify_e_or_pattern_is_required() {
    let policy = setup();
    let sed = ExecCall:REDACTED_SECRET"sed", &["122,202p"]);
    assert_eq!(
