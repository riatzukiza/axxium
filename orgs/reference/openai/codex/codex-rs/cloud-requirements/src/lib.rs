...
const CLOUD_REQUIREMENTS_CACHE_FILENAME: REDACTED_SECRET = "cloud-requirements-cache.json";
...
const CLOUD_REQUIREMENTS_FETCH_ATTEMPT_METRIC: REDACTED_SECRET = "codex.cloud_requirements.fetch_attempt";
const CLOUD_REQUIREMENTS_FETCH_FINAL_METRIC: REDACTED_SECRET = "codex.cloud_requirements.fetch_final";
const CLOUD_REQUIREMENTS_LOAD_METRIC: REDACTED_SECRET = "codex.cloud_requirements.load";
const CLOUD_REQUIREMENTS_LOAD_FAILED_MESSAGE: REDACTED_SECRET = "failed to load your workspace-managed config";
const CLOUD_REQUIREMENTS_AUTH_RECOVERY_FAILED_MESSAGE: REDACTED_SECRET = "Your authentication session could not be refreshed automatically. Please log out and sign in again.";
...
fn refresher_task_slot() -> &'static Mutex<Option<JoinHandle<()>>> {
    static REFRESHER_TASK: OnceLock<Mutex<Option<JoinHandle<()>>>> = OnceLock:REDACTED_SECRET);
    REFRESHER_TASK.get_or_init(|| Mutex:REDACTED_SECRETNone))
}
...
fn verify_cache_signature(payload_bytes: &[u8], signature: REDACTED_SECRET) -> bool {
...
#[async_trait]
trait RequirementsFetcher: REDACTED_SECRET + Sync {
...
impl CloudRequirementsService {
    ...
    async fn fetch_with_timeout(
        ...
        let fetch_result = timeout(self.timeout, self.fetch())
            ...
            .map_err(|_| {
                CloudRequirementsLoadError:REDACTED_SECRET
    ...
    async fn fetch_with_retries(
                        ...
                        match auth_recovery.next().await {
                            Ok(_) => {
                                let Some(refreshed_auth) = self.auth_manager.auth().await else {
                                    ...
                                    return Err(CloudRequirementsLoadError:REDACTED_SECRET
        ...
        Err(CloudRequirementsLoadError:REDACTED_SECRET
    ...
    async fn load_cache(
        &self,
        chatgpt_user_id: Option<REDACTED_SECRET>,
        account_id: Option<REDACTED_SECRET>,
...
pub fn cloud_requirements_loader(
    ...
    let service = CloudRequirementsService:REDACTED_SECRET
        auth_manager,
        Arc:REDACTED_SECRETBackendRequirementsFetcher:REDACTED_SECRETchatgpt_base_url)),
        codex_home,
    ...
    CloudRequirementsLoader:REDACTED_SECRETasync move {
...
fn emit_fetch_attempt_metric(
    trigger: REDACTED_SECRET,
    attempt: usize,
    outcome: REDACTED_SECRET,
    status_code: Option<u16>,
) {
...
fn emit_load_metric(trigger: REDACTED_SECRET, outcome: REDACTED_SECRET) {
    emit_metric(
...
fn emit_metric(metric_name: REDACTED_SECRET, tags: Vec<(REDACTED_SECRET, String)>) {
...
mod tests {
    ...
    fn auth_manager_with_api_key() -> Arc<AuthManager> {
        ...
        Arc:REDACTED_SECRETAuthManager:REDACTED_SECRET
    ...
    fn chatgpt_auth_json_with_mode(
        ...
        refresh_token: REDACTED_SECRET,
        last_refresh: REDACTED_SECRET,
        auth_mode: Option<REDACTED_SECRET>,
    ) -> serde_json::Value {
    ...
    fn managed_auth_context(
        ...
        .expect("write auth");
        ManagedAuthContext {
            manager: Arc:REDACTED_SECRETAuthManager:REDACTED_SECRET
    ...
    fn auth_manager_with_plan(plan_type: REDACTED_SECRET) -> Arc<AuthManager> {
        auth_manager_with_plan_and_identity(plan_type, Some("user-12345"), Some("account-12345"))
    }

    fn parse_for_fetch(contents: Option<REDACTED_SECRET>) -> Option<ConfigRequirementsToml> {
    ...
    impl SequenceFetcher {
        fn new(responses: Vec<Result<Option<String>, FetchAttemptError>>) -> Self {
            Self {
                responses: tokio::sync::Mutex:REDACTED_SECRETVecDeque::from(responses)),
                request_count: AtomicUsize:REDACTED_SECRET0),
            }
        }
    }
    ...
    async fn fetch_cloud_requirements_skips_non_chatgpt_auth() {
        ...
        let service = CloudRequirementsService:REDACTED_SECRET
            auth_manager,
            Arc:REDACTED_SECRETStaticFetcher { contents: None }),
    ...
    async fn fetch_cloud_requirements_skips_team_like_usage_based_plan() {
        ...
        let service = CloudRequirementsService:REDACTED_SECRET
            ...
            Arc:REDACTED_SECRETStaticFetcher {
...
"#,
    ...
    async fn fetch_cloud_requirements_times_out() {
        ...
        let service = CloudRequirementsService:REDACTED_SECRET
            auth_manager,
            Arc:REDACTED_SECRETPendingFetcher),
            codex_home.path().to_path_buf(),
            CLOUD_REQUIREMENTS_TIMEOUT,
        );
    ...
    async fn fetch_cloud_requirements_retries_until_success() {
        let fetcher = Arc:REDACTED_SECRETSequenceFetcher:REDACTED_SECRETvec![
            Err(request_error()),
    ...
    async fn fetch_cloud_requirements_recovers_after_unauthorized_reload() {
        ...
        let auth_manager = Arc:REDACTED_SECRETAuthManager:REDACTED_SECRET
        ...
        let fetcher = Arc:REDACTED_SECRETTokenFetcher {
    ...
    async fn fetch_cloud_requirements_surfaces_auth_recovery_message() {
        ...
        let fetcher = Arc:REDACTED_SECRETUnauthorizedFetcher {
            message: "GET /config/requirements failed: 401".to_string(),
            request_count: AtomicUsize:REDACTED_SECRET0),
        });
        let codex_home = tempdir().expect("tempdir");
    ...
    async fn fetch_cloud_requirements_uses_cache_when_valid() {
        let codex_home = tempdir().expect("tempdir");
        let prime_service = CloudRequirementsService:REDACTED_SECRET
            auth_manager_with_plan("business"),
            Arc:REDACTED_SECRETStaticFetcher {
                contents: Some("allowed_approval_policies = [\"never\"]".to_string()),
            }),
        ...
        let fetcher = Arc:REDACTED_SECRETSequenceFetcher:REDACTED_SECRETvec![Err(request_error())]));
        let service = CloudRequirementsService:REDACTED_SECRET
            auth_manager_with_plan("business"),
            fetcher.clone(),
            codex_home.path().to_path_buf(),
    ...
    async fn fetch_cloud_requirements_does_not_use_cache_when_auth_identity_is_incomplete() {
        ...
        let fetcher = Arc:REDACTED_SECRETSequenceFetcher:REDACTED_SECRETvec![Ok(Some(
    ...
    async fn fetch_cloud_requirements_ignores_expired_cache() {
        ...
        let cache_file = CloudRequirementsCacheFile {
            ...
            signature: String:REDACTED_SECRET),
        };
    ...
    async fn fetch_cloud_requirements_none_is_success_without_retry() {
        let fetcher = Arc:REDACTED_SECRETSequenceFetcher:REDACTED_SECRETvec![Ok(None), Err(request_error())]));
        let codex_home = tempdir().expect("tempdir");
        let service = CloudRequirementsService:REDACTED_SECRET
            auth_manager_with_plan("enterprise"),
            fetcher.clone(),
