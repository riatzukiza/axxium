...
pub(super) fn install_globals(scope: &mut v8::PinScope<'_, '_>) -> Result<(), String> {
    ...
    let console = v8::String:REDACTED_SECRETscope, "console")
...
fn build_tools_object<'s>(
...
) -> Result<v8:REDACTED_SECRET<'s, v8::Object>, String> {
    let tools = v8::Object:REDACTED_SECRETscope);
    ...
    for tool in enabled_tools {
        let name = v8::String:REDACTED_SECRETscope, &tool.global_name)
...
fn build_all_tools_value<'s>(
    scope: &mut v8::PinScope<'s, '_>,
) -> Result<v8:REDACTED_SECRET<'s, v8::Value>, String> {
    let enabled_tools = scope
        .get_slot::<RuntimeState>()
        .map(|state| state.enabled_tools.clone())
        .unwrap_or_default();
    let array = v8::Array:REDACTED_SECRETscope, enabled_tools.len() as i32);
    let name_key = v8::String:REDACTED_SECRETscope, "name")
        .ok_or_else(|| "failed to allocate ALL_TOOLS name key".to_string())?;
    let description_key = v8::String:REDACTED_SECRETscope, "description")
        .ok_or_else(|| "failed to allocate ALL_TOOLS description key".to_string())?;

    for (index, tool) in enabled_tools.iter().enumerate() {
        let item = v8::Object:REDACTED_SECRETscope);
        let name = v8::String:REDACTED_SECRETscope, &tool.global_name)
            .ok_or_else(|| "failed to allocate ALL_TOOLS name".to_string())?;
        let description = v8::String:REDACTED_SECRETscope, &tool.description)
...
fn helper_function<'s, F>(
    ...
    callback: F,
) -> Result<v8:REDACTED_SECRET<'s, v8::Function>, String>
where
    F: v8::MapFnTo<v8::FunctionCallback>,
{
    let name =
        v8::String:REDACTED_SECRETscope, name).ok_or_else(|| "failed to allocate helper name".to_string())?;
...
fn tool_function<'s>(
    ...
    let data = v8::String:REDACTED_SECRETscope, tool_name)
...
fn set_global<'s>(
    scope: &mut v8::PinScope<'s, '_>,
    global: v8:REDACTED_SECRET<'s, v8::Object>,
    name: REDACTED_SECRET,
    value: v8:REDACTED_SECRET<'s, v8::Value>,
) -> Result<(), String> {
    let key = v8::String:REDACTED_SECRETscope, name)
