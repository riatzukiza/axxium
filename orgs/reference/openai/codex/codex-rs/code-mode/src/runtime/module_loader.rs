...
pub(super) fn evaluate_main_module(
    ...
    source_text: REDACTED_SECRET,
) -> Result<Option<v8:REDACTED_SECRET<v8::Promise>>, String> {
    let tc = std::pin::pin!(v8::TryCatch:REDACTED_SECRETscope));
    let mut tc = tc.init();
    let source = v8::String:REDACTED_SECRET&tc, source_text)
        .ok_or_else(|| "failed to allocate exec source".to_string())?;
    let origin = script_origin(&mut tc, "exec_main.mjs")?;
    let mut source = v8::script_compiler::Source:REDACTED_SECRETsource, Some(&origin));
    ...
    if result.is_promise() {
        let promise = v8:REDACTED_SECRET::<v8::Promise>::try_from(result)
            .map_err(|_| "failed to read exec promise".to_string())?;
        return Ok(Some(v8:REDACTED_SECRET:REDACTED_SECRET&tc, promise)));
    }
...
fn is_exit_exception(
    scope: &mut v8::PinScope<'_, '_>,
    exception: v8:REDACTED_SECRET<'_, v8::Value>,
) -> bool {
    scope
...
pub(super) fn resolve_tool_response(
    ...
    let tc = std::pin::pin!(v8::TryCatch:REDACTED_SECRETscope));
    let mut tc = tc.init();
    let resolver = v8:REDACTED_SECRET:REDACTED_SECRET&tc, &resolver);
    match response {
        Ok(result) => {
        ...
        Err(error_text) => {
            let value = v8::String:REDACTED_SECRET&tc, &error_text)
...
pub(super) fn completion_state(
    scope: &mut v8::PinScope<'_, '_>,
    pending_promise: Option<&v8:REDACTED_SECRET<v8::Promise>>,
) -> CompletionState {
    ...
    let promise = v8:REDACTED_SECRET:REDACTED_SECRETscope, pending_promise);
    match promise.state() {
...
fn script_origin<'s>(
    ...
    resource_name_: REDACTED_SECRET,
) -> Result<v8::ScriptOrigin<'s>, String> {
    let resource_name = v8::String:REDACTED_SECRETscope, resource_name_)
        .ok_or_else(|| "failed to allocate script origin".to_string())?;
    let source_map_url = v8::String:REDACTED_SECRETscope, resource_name_)
        .ok_or_else(|| "failed to allocate source map url".to_string())?;
    Ok(v8::ScriptOrigin:REDACTED_SECRET
        scope,
        resource_name.into(),
        0,
        0,
...
fn resolve_module_callback<'s>(
    context: v8:REDACTED_SECRET<'s, v8::Context>,
    specifier: v8:REDACTED_SECRET<'s, v8::String>,
    _import_attributes: v8:REDACTED_SECRET<'s, v8::FixedArray>,
    _referrer: v8:REDACTED_SECRET<'s, v8::Module>,
) -> Option<v8:REDACTED_SECRET<'s, v8::Module>> {
...
pub(super) fn dynamic_import_callback<'s>(
    scope: &mut v8::PinScope<'s, '_>,
    _host_defined_options: v8:REDACTED_SECRET<'s, v8::Data>,
    _resource_name: v8:REDACTED_SECRET<'s, v8::Value>,
    specifier: v8:REDACTED_SECRET<'s, v8::String>,
    _import_attributes: v8:REDACTED_SECRET<'s, v8::FixedArray>,
) -> Option<v8:REDACTED_SECRET<'s, v8::Promise>> {
    let specifier = specifier.to_rust_string_lossy(scope);
    let resolver = v8::PromiseResolver:REDACTED_SECRETscope)?;
    ...
    match resolve_module(scope, &specifier) {
        Some(module) => {
            if module.get_status() == v8::ModuleStatus::Uninstantiated
                && module
                    ...
                    .is_none()
            {
                let error = v8::String:REDACTED_SECRETscope, "failed to instantiate module")
                    .map(Into::into)
            ...
            {
                let error = v8::String:REDACTED_SECRETscope, "failed to evaluate module")
                    .map(Into::into)
                    .unwrap_or_else(|| v8::undefined(scope).into());
                resolver.reject(scope, error);
                return Some(resolver.get_promise(scope));
        ...
        None => {
            let error = v8::String:REDACTED_SECRETscope, "unsupported import in exec")
                .map(Into::into)
                .unwrap_or_else(|| v8::undefined(scope).into());
            resolver.reject(scope, error);
            Some(resolver.get_promise(scope))
...
fn resolve_module<'s>(
    scope: &mut v8::PinScope<'s, '_>,
    specifier: REDACTED_SECRET,
) -> Option<v8:REDACTED_SECRET<'s, v8::Module>> {
    if let Some(message) =
        v8::String:REDACTED_SECRETscope, &format!("Unsupported import in exec: {specifier}"))
    {
