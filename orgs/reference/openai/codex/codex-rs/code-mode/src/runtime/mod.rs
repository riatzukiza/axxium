...
const EXIT_SENTINEL: REDACTED_SECRET = "__codex_code_mode_exit__";
...
pub(crate) fn spawn_runtime(
    ...
    event_tx: mpsc::UnboundedREDACTED_SECRETer<RuntimeEvent>,
) -> Result<(std_mpsc::REDACTED_SECRETer<RuntimeCommand>, v8::IsolateHandle), String> {
...
pub(super) struct RuntimeState {
    event_tx: mpsc::UnboundedREDACTED_SECRETer<RuntimeEvent>,
    pending_tool_calls: HashMap<String, v8:REDACTED_SECRET<v8::PromiseResolver>>,
    stored_values: HashMap<String, JsonValue>,
    enabled_tools: Vec<EnabledToolMetadata>,
...
fn initialize_v8() {
    static PLATFORM: OnceLock<v8::SharedRef<v8::Platform>> = OnceLock:REDACTED_SECRET);
...
fn run_runtime(
    ...
    isolate_handle_tx: std_mpsc::SyncREDACTED_SECRETer<v8::IsolateHandle>,
) {
    initialize_v8();

    let isolate = &mut v8::Isolate:REDACTED_SECRETv8::CreateParams::default());
    ...
    scope.set_slot(RuntimeState {
        ...
        pending_tool_calls: HashMap:REDACTED_SECRET),
    ...
    if let Err(error_text) = globals::install_globals(scope) {
        send_result(&event_tx, HashMap:REDACTED_SECRET), Some(error_text));
        return;
    }
    ...
    loop {
        ...
        if let Some(promise) = pending_promise.as_ref() {
            let promise = v8:REDACTED_SECRET:REDACTED_SECRETscope, promise);
...
mod tests {
    ...
    async fn terminate_execution_stops_cpu_bound_module() {
        ...
        assert_eq!(stored_values, HashMap:REDACTED_SECRET));
