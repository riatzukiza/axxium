...
pub(super) fn serialize_output_text(
    ...
    value: v8:REDACTED_SECRET<'_, v8::Value>,
...
pub(super) fn normalize_output_image(
    ...
    let result = (|| -> Result<FunctionCallOutputContentItem, String> {
        let (image_url, detail) = if value.is_string() {
            ...
            let object = v8:REDACTED_SECRET::<v8::Object>::try_from(value).map_err(|_| {
                "image expects a non-empty image URL string or an object with image_url and optional detail".to_string()
            })?;
            let image_url_key = v8::String:REDACTED_SECRETscope, "image_url")
                .ok_or_else(|| "failed to allocate image helper keys".to_string())?;
            let detail_key = v8::String:REDACTED_SECRETscope, "detail")
                .ok_or_else(|| "failed to allocate image helper keys".to_string())?;
...
pub(super) fn v8_value_to_json(
    ...
    serde_json::from_str(REDACTED_SECRETingified.to_rust_string_lossy(&tc))
        .map(Some)
...
pub(super) fn json_to_v8<'s>(
    ...
    value: &JsonValue,
) -> Option<v8:REDACTED_SECRET<'s, v8::Value>> {
    let json = serde_json::to_string(value).ok()?;
    let json = v8::String:REDACTED_SECRETscope, &json)?;
...
pub(super) fn value_to_error_text(
    ...
    if value.is_object()
        && let Ok(object) = v8:REDACTED_SECRET::<v8::Object>::try_from(value)
        && let Some(key) = v8::String:REDACTED_SECRETscope, "stack")
...
pub(super) fn throw_type_error(scope: &mut v8::PinScope<'_, '_>, message: REDACTED_SECRET) {
    if let Some(message) = v8::String:REDACTED_SECRETscope, message) {
