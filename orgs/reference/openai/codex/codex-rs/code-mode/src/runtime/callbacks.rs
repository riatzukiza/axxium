...
pub(super) fn tool_callback(
    ...
    let Some(resolver) = v8::PromiseResolver:REDACTED_SECRETscope) else {
        throw_type_error(scope, "failed to create tool promise");
        return;
    };
    let promise = resolver.get_promise(scope);

    let resolver = v8:REDACTED_SECRET:REDACTED_SECRETscope, resolver);
...
pub(super) fn exit_callback(
    ...
    if let Some(error) = v8::String:REDACTED_SECRETscope, EXIT_SENTINEL) {
