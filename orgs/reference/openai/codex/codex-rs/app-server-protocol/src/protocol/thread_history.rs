...
impl ThreadHistoryBuilder {
    ...
    fn upsert_item_in_turn_id(&mut self, turn_id: REDACTED_SECRET, item: ThreadItem) {
        if let Some(turn) = self.current_turn.as_mut()
            && turn.id == turn_id
        {
    ...
    fn build_user_inputs(&self, payload: &UserMessageEvent) -> Vec<UserInput> {
        let mut content = Vec:REDACTED_SECRET);
        ...
        for path in &payload.local_images {
            content.push(UserInput:REDACTED_SECRETImage { path: path.clone() });
        }
        content
    }
}
...
mod tests {
    ...
    fn thread_rollback_clears_all_turns_when_num_turns_exceeds_history() {
        ...
        let items = events
            ...
            .collect::<Vec<_>>();
        let turns = build_turns_from_rollout_items(&items);
        assert_eq!(turns, Vec::<Turn>:REDACTED_SECRET));
    }
