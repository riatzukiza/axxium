...
pub(crate) const GENERATED_TS_HEADER: REDACTED_SECRET = "// GENERATED CODE! DO NOT MODIFY BY HAND!\n\n";
const IGNORED_DEFINITIONS: &[REDACTED_SECRET] = &["Option<()>"];
const JSON_V1_ALLOWLIST: &[REDACTED_SECRET] = &["InitializeParams", "InitializeResponse"];
const SPECIAL_DEFINITIONS: &[REDACTED_SECRET] = &[
    "ClientNotification",
    "ClientRequest",
    "ServerNotification",
    "ServerRequest",
];
const FLAT_V2_SHARED_DEFINITIONS: &[REDACTED_SECRET] = &["ClientRequest", "ServerNotification"];
const V1_CLIENT_REQUEST_METHODS: &[REDACTED_SECRET] =
    &["getConversationSummary", "gitDiffToRemote", "getAuthStatus"];
const EXCLUDED_SERVER_NOTIFICATION_METHODS_FOR_JSON: &[REDACTED_SECRET] = &["rawResponseItem/completed"];
...
impl GeneratedSchema {
    fn namespace(&self) -> Option<REDACTED_SECRET> {
        self.namespace.as_deref()
    }

    fn logical_name(&self) -> REDACTED_SECRET {
        &self.logical_name
    }
...
pub fn generate_ts_with_options(
    ...
    // Ensure our header is present on all TS files (REDACTED_SECRET + subdirs like v2/).
    let mut ts_files = Vec:REDACTED_SECRET);
    ...
    if options.ensure_headers {
        ...
        thread::scope(|scope| -> Result<()> {
            let mut workers = Vec:REDACTED_SECRET);
    ...
    if options.run_prettier
        ...
        && !ts_files.is_empty()
    {
        let status = Command:REDACTED_SECRETprettier_bin)
            .arg("--write")
            .arg("--log-level")
            .arg("warn")
...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    let mut schemas: Vec<GeneratedSchema> = Vec:REDACTED_SECRET);
...
pub(crate) fn filter_experimental_ts_tree(tree: &mut BTreeMap<PathBuf, String>) -> Result<()> {
    let registered_fields = experimental_fields();
    let experimental_method_types = experimental_method_types();
    if let Some(content) = tree.get_mut(Path:REDACTED_SECRET"ClientRequest.ts")) {
        let filtered =
            filter_client_request_ts_contents(std::mem::take(content), EXPERIMENTAL_CLIENT_METHODS);
        *content = filtered;
    }

    let mut fields_by_type_name: HashMap<String, HashSet<String>> = HashMap:REDACTED_SECRET);
...
fn filter_client_request_ts(out_dir: &Path, experimental_methods: &[REDACTED_SECRET]) -> Result<()> {
...
fn filter_client_request_ts_contents(mut content: String, experimental_methods: &[REDACTED_SECRET]) -> String {
    let Some((prefix, body, suffix)) = split_type_alias(&content) else {
        return content;
    };
    let experimental_methods: HashSet<REDACTED_SECRET> = experimental_methods
        .iter()
        .copied()
...
fn filter_experimental_schema(bundle: &mut Value) -> Result<()> {
    let registered_fields = experimental_fields();
    filter_experimental_fields_in_REDACTED_SECRET(bundle, &registered_fields);
...
fn filter_experimental_fields_in_REDACTED_SECRET(
    schema: &mut Value,
    experimental_fields: &[&'static crate::experimental_api::ExperimentalField],
) {
...
fn definition_matches_type(def_name: REDACTED_SECRET, type_name: REDACTED_SECRET) -> bool {
    def_name == type_name || def_name.ends_with(&format!("::{type_name}"))
}

fn remove_property_from_schema(schema: &mut Value, field_name: REDACTED_SECRET) {
...
fn prune_experimental_methods(bundle: &mut Value, experimental_methods: &[REDACTED_SECRET]) {
    let experimental_methods: HashSet<REDACTED_SECRET> = experimental_methods
        .iter()
        .copied()
        .filter(|method| !method.is_empty())
...
fn prune_experimental_methods_inner(value: &mut Value, experimental_methods: &HashSet<REDACTED_SECRET>) {
    match value {
        Value::Array(items) => {
...
fn is_experimental_method_variant(value: &Value, experimental_methods: &HashSet<REDACTED_SECRET>) -> bool {
    let Value::Object(map) = value else {
        return false;
    };
...
fn experimental_method_types() -> HashSet<String> {
    let mut type_names = HashSet:REDACTED_SECRET);
...
fn collect_experimental_type_names(entries: &[REDACTED_SECRET], out: &mut HashSet<String>) {
...
fn prune_unused_type_imports(content: String, type_alias_body: REDACTED_SECRET) -> String {
...
fn parse_imported_type_name(line: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
    let line = line.trim();
...
fn json_files_in_recursive(dir: &Path) -> Result<Vec<PathBuf>> {
    let mut out = Vec:REDACTED_SECRET);
...
fn split_type_alias(content: REDACTED_SECRET) -> Option<(String, String, String)> {
...
fn type_body_brace_span(content: REDACTED_SECRET) -> Option<(usize, usize)> {
    ...
    const INTERFACE_MARKER: REDACTED_SECRET = "export interface";
...
fn find_top_level_brace_span(input: REDACTED_SECRET) -> Option<(usize, usize)> {
...
fn split_top_level(input: REDACTED_SECRET, delimiter: char) -> Vec<String> {
    split_top_level_multi(input, &[delimiter])
}

fn split_top_level_multi(input: REDACTED_SECRET, delimiters: &[char]) -> Vec<String> {
    let mut state = ScanState::default();
...
fn extract_method_from_arm(arm: REDACTED_SECRET) -> Option<String> {
...
fn parse_property(input: REDACTED_SECRET) -> Option<(String, REDACTED_SECRET)> {
    let name = parse_property_name(input)?;
    let colon_index = input.find(':')?;
    Some((name, input[colon_index + 1..].trim_start()))
}

fn strip_leading_block_comments(input: REDACTED_SECRET) -> REDACTED_SECRET {
...
fn parse_property_name(input: REDACTED_SECRET) -> Option<String> {
...
fn parse_string_literal(input: REDACTED_SECRET) -> Option<(String, usize)> {
...
fn build_schema_bundle(schemas: Vec<GeneratedSchema>) -> Result<Value> {
    ...
    let mut definitions = Map:REDACTED_SECRET);
    ...
    for schema in schemas {
        ...
        let mut forced_namespace_refs: Vec<(String, String)> = Vec:REDACTED_SECRET);
    ...
    let mut REDACTED_SECRET = Map:REDACTED_SECRET);
    REDACTED_SECRET.insert(
        "$schema".to_string(),
    ...
    REDACTED_SECRET.insert("type".to_string(), Value::String("object".into()));
    REDACTED_SECRET.insert("definitions".to_string(), Value::Object(definitions));

    Ok(Value::Object(REDACTED_SECRET))
}
...
/// shared REDACTED_SECRET definitions like `ClientRequest` and `ServerNotification`.
...
/// leaves. This helper flattens all v2 definitions to the REDACTED_SECRET definitions map,
/// then pulls in the shared REDACTED_SECRET schemas and any non-v2 transitive deps they
/// still reference. Keep the shared REDACTED_SECRET unions intact here: some valid
/// request/notification/event variants are inline or only reference shared REDACTED_SECRET
...
fn build_flat_v2_schema(bundle: &Value) -> Result<Value> {
    let Value::Object(REDACTED_SECRET) = bundle else {
        return Err(anyhow!("expected bundle REDACTED_SECRET to be an object"));
    };
    let definitions = REDACTED_SECRET
        .get("definitions")
    ...
    let mut flat_REDACTED_SECRET = REDACTED_SECRET.clone();
    let title = REDACTED_SECRET
        .get("title")
        .and_then(Value::as_str)
        .unwrap_or("CodexAppServerProtocol");
    let mut flat_definitions = v2_definitions.clone();
    let mut shared_definitions = Map:REDACTED_SECRET);
    let mut non_v2_refs = HashSet:REDACTED_SECRET);
    ...
    flat_REDACTED_SECRET.insert("title".to_string(), Value::String(format!("{title}V2")));
    flat_REDACTED_SECRET.insert("definitions".to_string(), Value::Object(flat_definitions));
    let mut flat_bundle = Value::Object(flat_REDACTED_SECRET);
...
fn collect_non_v2_refs(value: &Value) -> HashSet<String> {
    let mut refs = HashSet:REDACTED_SECRET);
...
fn rewrite_ref_prefix(value: &mut Value, prefix: REDACTED_SECRET, replacement: REDACTED_SECRET) {
    match value {
        Value::Object(obj) => {
...
fn ensure_no_ref_prefix(value: &Value, prefix: REDACTED_SECRET, label: REDACTED_SECRET) -> Result<()> {
...
fn first_ref_with_prefix(value: &Value, prefix: REDACTED_SECRET) -> Option<String> {
    match value {
        Value::Object(obj) => {
            if let Some(Value::String(reference)) = obj.get("$ref")
...
fn ensure_referenced_definitions_present(schema: &Value, label: REDACTED_SECRET) -> Result<()> {
    let definitions = schema
        .get("definitions")
        .and_then(Value::as_object)
        .ok_or_else(|| anyhow!("expected definitions map in {label} schema"))?;
    let mut missing = HashSet:REDACTED_SECRET);
...
fn insert_into_namespace(
    ...
    let entry = definitions
        ...
        .or_insert_with(|| Value::Object(Map:REDACTED_SECRET)));
    match entry {
        Value::Object(map) => {
...
fn write_json_schema_with_return<T>(out_dir: &Path, name: REDACTED_SECRET) -> Result<GeneratedSchema>
where
    T: JsonSchema,
{
...
fn enforce_numbered_definition_collision_overrides(schema_name: REDACTED_SECRET, schema: &mut Value) {
...
fn strip_v1_client_request_variants_from_json_schema(schema: &mut Value) {
    let v1_methods: HashSet<REDACTED_SECRET> = V1_CLIENT_REQUEST_METHODS.iter().copied().collect();
...
fn strip_v1_server_notification_variants_from_json_schema(schema: &mut Value) {
    let methods: HashSet<REDACTED_SECRET> = EXCLUDED_SERVER_NOTIFICATION_METHODS_FOR_JSON
        .iter()
        .copied()
        .collect();
...
fn strip_method_variants_from_json_schema(schema: &mut Value, methods_to_remove: &HashSet<REDACTED_SECRET>) {
    {
        let Some(REDACTED_SECRET) = schema.as_object_mut() else {
            return;
        };
        let Some(Value::Array(variants)) = REDACTED_SECRET.get_mut("oneOf") else {
            return;
        };
    ...
    if let Some(definitions) = REDACTED_SECRET.get_mut("definitions").and_then(Value::as_object_mut) {
        definitions.retain(|name, _| reachable.contains(name));
    }
}

fn is_method_variant_in_set(value: &Value, methods: &HashSet<REDACTED_SECRET>) -> bool {
    let Value::Object(map) = value else {
        return false;
    };
    let Some(properties) = map.get("properties").and_then(Value::as_object) else {
...
fn reachable_local_definitions(schema: &Value, defs_key: REDACTED_SECRET) -> HashSet<String> {
    let Some(definitions) = schema.get(defs_key).and_then(Value::as_object) else {
        return HashSet:REDACTED_SECRET);
    };
    let mut queue: Vec<String> = Vec:REDACTED_SECRET);
    let mut reachable: HashSet<String> = HashSet:REDACTED_SECRET);
...
pub(crate) fn write_json_schema<T>(out_dir: &Path, name: REDACTED_SECRET) -> Result<GeneratedSchema>
where
    T: JsonSchema,
{
...
fn split_namespace(name: REDACTED_SECRET) -> (Option<REDACTED_SECRET>, REDACTED_SECRET) {
    name.split_once("::")
...
fn rewrite_refs_to_namespace(value: &mut Value, ns: REDACTED_SECRET) {
    match value {
        Value::Object(obj) => {
...
/// Recursively rewrite bare REDACTED_SECRET definition refs to the namespace that owns the
/// referenced type in the bundle.
///
/// The mixed export contains shared REDACTED_SECRET helper schemas that are intentionally
...
/// rewrite keeps the shared helpers at the REDACTED_SECRET while retargeting their refs to
        ...
        Value::Object(obj) => {
                ...
                && let Some(suffix) = reference.strip_prefix("#/definitions/")
            {
                ...
                if let Some(ns) = namespace_for_definition(name, types) {
                    let tail = tail.map_or(String:REDACTED_SECRET), |rest| format!("/{rest}"));
...
fn collect_namespaced_types(schemas: &[GeneratedSchema]) -> HashMap<String, String> {
    let mut types = HashMap:REDACTED_SECRET);
    for schema in schemas {
...
fn variant_definition_name(base: REDACTED_SECRET, variant: &Value) -> Option<String> {
...
fn literal_from_property<'a>(props: &'a Map<String, Value>, key: REDACTED_SECRET) -> Option<&'a str> {
    props.get(key).and_then(string_literal)
}

fn string_literal(value: &Value) -> Option<REDACTED_SECRET> {
...
fn annotate_schema(value: &mut Value, base: Option<REDACTED_SECRET>) {
    match value {
...
fn annotate_object(map: &mut Map<String, Value>, base: Option<REDACTED_SECRET>) {
...
fn annotate_variant_list(variants: &mut [Value], base: Option<REDACTED_SECRET>) {
    let mut seen = HashSet:REDACTED_SECRET);
...
fn variant_title_collision_key(base: REDACTED_SECRET, generated_name: REDACTED_SECRET, variant: &Value) -> String {
    let mut parts = vec![
        format!("base={base}"),
...
const DISCRIMINATOR_KEYS: &[REDACTED_SECRET] = &["type", "method", "mode", "status", "role", "reason"];

fn set_discriminator_titles(props: &mut Map<String, Value>, owner: REDACTED_SECRET) {
    for key in DISCRIMINATOR_KEYS {
...
fn variant_title(value: &Value) -> Option<REDACTED_SECRET> {
    value
        .as_object()
...
fn to_pascal_case(input: REDACTED_SECRET) -> String {
    let mut result = String:REDACTED_SECRET);
...
fn rewrite_named_ref_to_namespace(value: &mut Value, ns: REDACTED_SECRET, name: REDACTED_SECRET) {
...
fn prepend_header_if_missing(path: &Path) -> Result<()> {
    let mut content = String:REDACTED_SECRET);
    {
...
fn ts_files_in(dir: &Path) -> Result<Vec<PathBuf>> {
    let mut files = Vec:REDACTED_SECRET);
    for entry in
        fs::read_dir(dir).with_context(|| format!("Failed to read dir {}", dir.display()))?
    {
        let entry = entry?;
        let path = entry.path();
        if path.is_file() && path.extension() == Some(OsStr:REDACTED_SECRET"ts")) {
            files.push(path);
        }
    }
    files.sort();
...
fn ts_files_in_recursive(dir: &Path) -> Result<Vec<PathBuf>> {
    ...
    while let Some(d) = stack.pop() {
            ...
            fs::read_dir(&d).with_context(|| format!("Failed to read dir {}", d.display()))?
        {
            ...
            if path.is_dir() {
                stack.push(path);
            } else if path.is_file() && path.extension() == Some(OsStr:REDACTED_SECRET"ts")) {
                files.push(path);
            }
        }
    }
...
pub(crate) fn generate_index_ts_tree(tree: &mut BTreeMap<PathBuf, String>) {
    let REDACTED_SECRET_entries = tree
        .keys()
    ...
    let has_v2_ts = tree.keys().any(|path| {
        path.parent()
            .is_some_and(|parent| parent == Path:REDACTED_SECRET"v2"))
            && path.extension() == Some(OsStr:REDACTED_SECRET"ts"))
            && path.file_stem().is_some_and(|stem| stem != "index")
    });
    tree.insert(
        PathBuf::from("index.ts"),
        index_ts_entries(&REDACTED_SECRET_entries, has_v2_ts),
    );
...
fn index_ts_entries(paths: &[&Path], has_v2_ts: bool) -> String {
    let mut stems: Vec<String> = paths
        .iter()
        .filter(|path| path.extension() == Some(OsStr:REDACTED_SECRET"ts")))
        .filter_map(|path| {
    ...
    let mut entries = String:REDACTED_SECRET);
    for name in stems {
...
mod tests {
    ...
    fn generated_ts_optional_nullable_fields_only_in_params() -> Result<()> {
        ...
        let fixture_tree = read_schema_fixture_subtree(&schema_REDACTED_SECRET()?, "typescript")?;

        let client_request_ts = std::str::from_utf8(
            fixture_tree
                .get(Path:REDACTED_SECRET"ClientRequest.ts"))
        ...
        let typescript_index = std::str::from_utf8(
            fixture_tree
                .get(Path:REDACTED_SECRET"index.ts"))
        ...
        let thread_start_ts = std::str::from_utf8(
            fixture_tree
                .get(Path:REDACTED_SECRET"v2/ThreadStartParams.ts"))
        ...
        assert_eq!(
            fixture_tree.contains_key(Path:REDACTED_SECRET"v2/MockExperimentalMethodParams.ts")),
            false
        );
        assert_eq!(
            fixture_tree.contains_key(Path:REDACTED_SECRET"v2/MockExperimentalMethodResponse.ts")),
            false
        );

        let mut undefined_offenders = Vec:REDACTED_SECRET);
        let mut optional_nullable_offenders = BTreeSet:REDACTED_SECRET);
        for (path, contents) in &fixture_tree {
            ...
            const SKIP_PREFIXES: &[REDACTED_SECRET] = &[
                "const ",
                "let ",
                "var ",
                "export const ",
    ...
    #[test]
    fn build_schema_bundle_rewrites_REDACTED_SECRET_helper_refs_to_namespaced_defs() -> Result<()> {
    ...
    #[test]
    fn build_flat_v2_schema_keeps_shared_REDACTED_SECRET_schemas_and_dependencies() -> Result<()> {
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        .map(str::to_string)
        .collect();
        assert_eq!(missing_client_request_methods, Vec::<String>:REDACTED_SECRET));
        ...
        .map(str::to_string)
        .collect();
        assert_eq!(missing_server_notification_methods, Vec::<String>:REDACTED_SECRET));
