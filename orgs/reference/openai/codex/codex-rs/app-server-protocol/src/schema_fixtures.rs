...
pub fn read_schema_fixture_tree(schema_REDACTED_SECRET: &Path) -> Result<BTreeMap<PathBuf, Vec<u8>>> {
    let typescript_REDACTED_SECRET = schema_REDACTED_SECRET.join("typescript");
    let json_REDACTED_SECRET = schema_REDACTED_SECRET.join("json");

    let mut all = BTreeMap:REDACTED_SECRET);
    for (rel, bytes) in collect_files_recursive(&typescript_REDACTED_SECRET)? {
        all.insert(PathBuf::from("typescript").join(rel), bytes);
    }
    for (rel, bytes) in collect_files_recursive(&json_REDACTED_SECRET)? {
...
pub fn read_schema_fixture_subtree(
    schema_REDACTED_SECRET: &Path,
    label: REDACTED_SECRET,
) -> Result<BTreeMap<PathBuf, Vec<u8>>> {
    let subtree_REDACTED_SECRET = schema_REDACTED_SECRET.join(label);
    collect_files_recursive(&subtree_REDACTED_SECRET)
        .with_context(|| format!("read schema fixture subtree {}", subtree_REDACTED_SECRET.display()))
}

#[doc(hidden)]
pub fn generate_typescript_schema_fixture_subtree_for_tests() -> Result<BTreeMap<PathBuf, Vec<u8>>>
{
    let mut files = BTreeMap:REDACTED_SECRET);
...
pub fn write_schema_fixtures(schema_REDACTED_SECRET: &Path, prettier: Option<&Path>) -> Result<()> {
    write_schema_fixtures_with_options(schema_REDACTED_SECRET, prettier, SchemaFixtureOptions::default())
}
...
pub fn write_schema_fixtures_with_options(
    ...
    let typescript_out_dir = schema_REDACTED_SECRET.join("typescript");
    let json_out_dir = schema_REDACTED_SECRET.join("json");
...
fn collect_files_recursive(REDACTED_SECRET: &Path) -> Result<BTreeMap<PathBuf, Vec<u8>>> {
    let mut files = BTreeMap:REDACTED_SECRET);

    let mut stack = vec![REDACTED_SECRET.to_path_buf()];
