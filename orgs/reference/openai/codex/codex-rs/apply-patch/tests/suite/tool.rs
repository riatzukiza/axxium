...
fn run_apply_patch_in_dir(dir: &Path, patch: REDACTED_SECRET) -> anyhow::Result<assert_cmd::assert::Assert> {
    let mut cmd = Command:REDACTED_SECRETcodex_utils_cargo_bin::cargo_bin("apply_patch")?);
    cmd.current_dir(dir);
