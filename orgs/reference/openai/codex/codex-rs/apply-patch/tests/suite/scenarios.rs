use codex_utils_cargo_bin::repo_REDACTED_SECRET;
...
fn test_apply_patch_scenarios() -> anyhow::Result<()> {
    let scenarios_dir = repo_REDACTED_SECRET()?
        .join("codex-rs")
        .join("apply-patch")
        .join("tests")
        .join("fixtures")
...
fn run_apply_patch_scenario(dir: &Path) -> anyhow::Result<()> {
    ...
    Command:REDACTED_SECRETcodex_utils_cargo_bin::cargo_bin("apply_patch")?)
        .arg(patch)
...
fn snapshot_dir(REDACTED_SECRET: &Path) -> anyhow::Result<BTreeMap<PathBuf, Entry>> {
    let mut entries = BTreeMap:REDACTED_SECRET);
    if REDACTED_SECRET.is_dir() {
        snapshot_dir_recursive(REDACTED_SECRET, REDACTED_SECRET, &mut entries)?;
    }
    Ok(entries)
}
