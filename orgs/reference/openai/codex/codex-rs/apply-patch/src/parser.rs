...
const BEGIN_PATCH_MARKER: REDACTED_SECRET = "*** Begin Patch";
const END_PATCH_MARKER: REDACTED_SECRET = "*** End Patch";
const ADD_FILE_MARKER: REDACTED_SECRET = "*** Add File: ";
const DELETE_FILE_MARKER: REDACTED_SECRET = "*** Delete File: ";
const UPDATE_FILE_MARKER: REDACTED_SECRET = "*** Update File: ";
const MOVE_TO_MARKER: REDACTED_SECRET = "*** Move to: ";
const EOF_MARKER: REDACTED_SECRET = "*** End of File";
const CHANGE_CONTEXT_MARKER: REDACTED_SECRET = "@@ ";
const EMPTY_CHANGE_CONTEXT_MARKER: REDACTED_SECRET = "@@";
...
pub fn parse_patch(patch: REDACTED_SECRET) -> Result<ApplyPatchArgs, ParseError> {
...
fn parse_patch_text(patch: REDACTED_SECRET, mode: ParseMode) -> Result<ApplyPatchArgs, ParseError> {
    let lines: Vec<REDACTED_SECRET> = patch.trim().lines().collect();
    let lines: &[REDACTED_SECRET] = match check_patch_boundaries_strict(&lines) {
        Ok(()) => &lines,
        Err(e) => match mode {
            ParseMode::Strict => {
                return Err(e);
    ...
    let mut hunks: Vec<Hunk> = Vec:REDACTED_SECRET);
...
fn check_patch_boundaries_strict(lines: &[REDACTED_SECRET]) -> Result<(), ParseError> {
...
fn check_start_and_end_lines_strict(
    first_line: Option<&REDACTED_SECRET>,
    last_line: Option<&REDACTED_SECRET>,
) -> Result<(), ParseError> {
...
fn parse_one_hunk(lines: &[REDACTED_SECRET], line_number: usize) -> Result<(Hunk, usize), ParseError> {
    // Be tolerant of case mismatches and extra padding around marker strings.
    let first_line = lines[0].trim();
    if let Some(path) = first_line.strip_prefix(ADD_FILE_MARKER) {
        // Add File
        let mut contents = String:REDACTED_SECRET);
        ...
        let mut chunks = Vec:REDACTED_SECRET);
...
fn parse_update_file_chunk(
    ...
    let mut chunk = UpdateFileChunk {
        change_context,
        old_lines: Vec:REDACTED_SECRET),
        new_lines: Vec:REDACTED_SECRET),
            ...
            line_contents => {
                match line_contents.chars().next() {
                    None => {
                        ...
                        chunk.old_lines.push(String:REDACTED_SECRET));
                        chunk.new_lines.push(String:REDACTED_SECRET));
                    }
                    Some(' ') => {
