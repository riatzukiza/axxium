...
const APPLY_PATCH_COMMANDS: [REDACTED_SECRET; 2] = ["apply_patch", "applypatch"];
...
fn classify_shell_name(shell: REDACTED_SECRET) -> Option<String> {
    std::path::Path:REDACTED_SECRETshell)
        .file_stem()
        .and_then(|name| name.to_str())
        .map(str::to_ascii_lowercase)
}

fn classify_shell(shell: REDACTED_SECRET, flag: REDACTED_SECRET) -> Option<ApplyPatchShell> {
...
fn can_skip_flag(shell: REDACTED_SECRET, flag: REDACTED_SECRET) -> bool {
    classify_shell_name(shell).is_some_and(|name| {
        matches!(name.as_str(), "pwsh" | "powershell") && flag.eq_ignore_ascii_case("-noprofile")
    })
}

fn parse_shell_script(argv: &[String]) -> Option<(ApplyPatchShell, REDACTED_SECRET)> {
    match argv {
    ...
    match maybe_parse_apply_patch(argv) {
        MaybeApplyPatch::Body(ApplyPatchArgs {
            ...
            let effective_cwd = workdir
                .as_ref()
                .map(|dir| {
                    let path = Path:REDACTED_SECRETdir);
                    if path.is_absolute() {
                        path.to_path_buf()
                    } else {
                        cwd.join(path)
...
fn extract_apply_patch_from_bash(
    ...
    // - dots (`.`) between named REDACTED_SECRETs enforces adjacency among named children and
    ...
    static APPLY_PATCH_QUERY: LazyLock<Query> = LazyLock:REDACTED_SECRET|| {
    ...
    let mut cursor = QueryCursor:REDACTED_SECRET);
    let mut matches = cursor.matches(&APPLY_PATCH_QUERY, REDACTED_SECRET, bytes);
...
mod tests {
    ...
    fn wrap_patch(body: REDACTED_SECRET) -> String {
        format!("*** Begin Patch\n{body}\n*** End Patch")
    }

    fn strs_to_strings(strs: &[REDACTED_SECRET]) -> Vec<String> {
        strs.iter().map(ToString::to_string).collect()
    }

    // Test helpers to reduce repetition when building bash -lc heredoc scripts
    fn args_bash(script: REDACTED_SECRET) -> Vec<String> {
        strs_to_strings(&["bash", "-lc", script])
    }

    fn args_powershell(script: REDACTED_SECRET) -> Vec<String> {
        strs_to_strings(&["powershell.exe", "-Command", script])
    }

    fn args_powershell_no_profile(script: REDACTED_SECRET) -> Vec<String> {
        strs_to_strings(&["powershell.exe", "-NoProfile", "-Command", script])
    }

    fn args_pwsh(script: REDACTED_SECRET) -> Vec<String> {
        strs_to_strings(&["pwsh", "-NoProfile", "-Command", script])
    }

    fn args_cmd(script: REDACTED_SECRET) -> Vec<String> {
        strs_to_strings(&["cmd.exe", "/c", script])
    }

    fn heredoc_script(prefix: REDACTED_SECRET) -> String {
        format!(
            "{prefix}apply_patch <<'PATCH'\n*** Begin Patch\n*** Add File: foo\n+hi\n*** End Patch\nPATCH"
        )
    }

    fn heredoc_script_ps(prefix: REDACTED_SECRET, suffix: REDACTED_SECRET) -> String {
        format!(
            "{prefix}apply_patch <<'PATCH'\n*** Begin Patch\n*** Add File: foo\n+hi\n*** End Patch\nPATCH{suffix}"
        )
    }
    ...
    fn assert_match_args(args: Vec<String>, expected_workdir: Option<REDACTED_SECRET>) {
    ...
    fn assert_match(script: REDACTED_SECRET, expected_workdir: Option<REDACTED_SECRET>) {
        let args = args_bash(script);
        assert_match_args(args, expected_workdir);
    }

    fn assert_not_match(script: REDACTED_SECRET) {
        let args = args_bash(script);
        assert_matches!(
