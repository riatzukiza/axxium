...
const NOTIFICATIONS_TO_OPT_OUT: &[REDACTED_SECRET] = &[
    // v2 item deltas.
...
const OTEL_SERVICE_NAME: REDACTED_SECRET = "codex-app-server-test-client";
const TRACE_DISABLED_MESSAGE: REDACTED_SECRET =
...
enum CliCommand {
    ...
    /// REDACTED_SECRET a user message through the Codex app-server.
    REDACTED_SECRETMessage {
        /// User message to send to Codex.
        user_message: String,
    },
    /// REDACTED_SECRET a user message through the app-server V2 thread/turn APIs.
    REDACTED_SECRETMessageV2 {
    ...
    /// REDACTED_SECRET two sequential V2 turns in the same thread to test follow-up behavior.
    REDACTED_SECRETFollowUpV2 {
...
pub async fn run() -> Result<()> {
    ...
    match command {
        ...
        CliCommand::REDACTED_SECRETFollowUpV2 {
            first_message,
            follow_up_message,
        } => {
            let endpoint = resolve_endpoint(codex_bin, url)?;
...
impl BackgroundAppServer {
    fn spawn(codex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command:REDACTED_SECRETcodex_bin);
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: REDACTED_SECRET, kill: bool) -> Result<()> {
    ...
    let log_file = OpenOptions:REDACTED_SECRET)
        .create(true)
        .append(true)
        .open(&log_path)
    ...
    let child = Command:REDACTED_SECRET"nohup")
        .arg("sh")
        .arg("-c")
        .arg(cmdline)
        .stdin(Stdio::null())
...
fn kill_listeners_on_same_port(listen: REDACTED_SECRET) -> Result<()> {
    ...
    let output = Command:REDACTED_SECRET"lsof")
        .arg("-nP")
    ...
    for pid in pids {
        ...
        let term_status = Command:REDACTED_SECRET"kill")
            .arg(&pid_str)
            .status()
    ...
    for pid in remaining {
        ...
        let _ = Command:REDACTED_SECRET"kill").arg("-9").arg(pid.to_string()).status();
    }

    Ok(())
}

fn shell_quote(input: REDACTED_SECRET) -> String {
    format!("'{}'", input.replace('\'', "'\\''"))
}

struct REDACTED_SECRETMessagePolicies<'a> {
...
async fn send_message(
    ...
    send_message_v2_with_policies(
        ...
        config_overrides,
        user_message,
        REDACTED_SECRETMessagePolicies {
...
async fn trigger_zsh_fork_multi_cmd_approval(
    ...
    with_client(
        ...
        |client| {
            ...
            let mut approvals_per_item = std::collections::BTreeMap:REDACTED_SECRET);
...
async fn send_message_v2_with_policies(
    ...
    config_overrides: &[String],
    user_message: String,
    policies: REDACTED_SECRETMessagePolicies<'_>,
) -> Result<()> {
    with_client(
...
fn thread_increment_elicitation(url: REDACTED_SECRET, thread_id: String) -> Result<()> {
...
fn thread_decrement_elicitation(url: REDACTED_SECRET, thread_id: String) -> Result<()> {
    let endpoint = Endpoint::ConnectWs(url.to_string());
    let mut client = CodexClient::connect(&endpoint, &[])?;
...
fn parse_dynamic_tools_arg(dynamic_tools: &Option<String>) -> Result<Option<Vec<DynamicToolSpec>>> {
    ...
    let raw_json = if let Some(path) = raw_arg.strip_prefix('@') {
        fs::read_to_string(Path:REDACTED_SECRETpath))
...
impl CodexClient {
    ...
    fn spawn_stdio(codex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        Ok(Self {
            transport: ClientTransport::Stdio {
                ...
                stdin: Some(stdin),
                stdout: BufReader:REDACTED_SECRETstdout),
            },
            pending_notifications: VecDeque:REDACTED_SECRET),
            command_approval_behavior: CommandApprovalBehavior::AlwaysAccept,
            command_approval_count: 0,
            command_approval_item_ids: Vec:REDACTED_SECRET),
            command_execution_statuses: Vec:REDACTED_SECRET),
            command_execution_outputs: Vec:REDACTED_SECRET),
            command_output_stream: String:REDACTED_SECRET),
            command_item_started: false,
            helper_done_seen: false,
            turn_completed_before_helper_done: false,
            unexpected_items_before_helper_done: Vec:REDACTED_SECRET),
            last_turn_status: None,
            last_turn_error_message: None,
        })
    }

    fn connect_websocket(url: REDACTED_SECRET) -> Result<Self> {
        ...
        Ok(Self {
            transport: ClientTransport::WebSocket {
                url: url.to_string(),
                socket: Box:REDACTED_SECRETsocket),
            },
            pending_notifications: VecDeque:REDACTED_SECRET),
            command_approval_behavior: CommandApprovalBehavior::AlwaysAccept,
            command_approval_count: 0,
    ...
    fn note_helper_output(&mut self, output: REDACTED_SECRET) {
    ...
    fn wait_for_account_login_completion(
        &mut self,
        expected_login_id: REDACTED_SECRET,
    ...
    fn stream_turn(&mut self, thread_id: REDACTED_SECRET, turn_id: REDACTED_SECRET) -> Result<()> {
        loop {
            let notification = self.next_notification()?;
    ...
    fn wait_for_response<T>(&mut self, request_id: RequestId, method: REDACTED_SECRET) -> Result<T>
    where
        T: DeserializeOwned,
    {
        loop {
    ...
    fn approve_file_change_request(
        ...
        if let Some(grant_REDACTED_SECRET) = grant_REDACTED_SECRET.as_deref() {
            println!("< grant REDACTED_SECRET: {}", grant_REDACTED_SECRET.display());
        }
    ...
    fn write_payload(&mut self, payload: REDACTED_SECRET) -> Result<()> {
    ...
    fn read_payload(&mut self) -> Result<String> {
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                let mut response_line = String:REDACTED_SECRET);
                let bytes = stdout
...
fn print_multiline_with_prefix(prefix: REDACTED_SECRET, payload: REDACTED_SECRET) {
