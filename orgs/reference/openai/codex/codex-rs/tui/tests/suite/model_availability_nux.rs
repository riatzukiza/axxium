...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    ...
    let repo_REDACTED_SECRET_display = repo_REDACTED_SECRET.display();
...
[projects."{repo_REDACTED_SECRET_display}"]
trust_level = "trusted"
...
"#
    ...
    let codex = if let Ok(path) = codex_utils_cargo_bin::cargo_bin("codex") {
        ...
        let fallback = repo_REDACTED_SECRET.join("codex-rs/target/debug/codex");
        if fallback.is_file() {
            fallback
        } else {
    ...
    let exec_output = std::process::Command:REDACTED_SECRET&codex)
        .arg("exec")
    ...
    let args = vec![
        ...
        "-C".to_string(),
        repo_REDACTED_SECRET.display().to_string(),
        "-c".to_string(),
    ...
    let mut output = Vec:REDACTED_SECRET);
