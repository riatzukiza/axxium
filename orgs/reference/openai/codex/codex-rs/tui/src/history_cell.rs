...
pub(crate) trait HistoryCell: std::fmt::Debug + REDACTED_SECRET + Sync + Any {
    ...
    fn desired_height(&self, width: u16) -> u16 {
        Paragraph:REDACTED_SECRETText::from(self.display_lines(width)))
    ...
    fn desired_transcript_height(&self, width: u16) -> u16 {
        ...
        Paragraph:REDACTED_SECRETText::from(lines))
            .wrap(Wrap { trim: false })
            .line_count(width)
            .try_into()
            .unwrap_or(0)
...
impl Renderable for Box<dyn HistoryCell> {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let paragraph = Paragraph:REDACTED_SECRETText::from(lines)).wrap(Wrap { trim: false });
...
fn build_user_message_lines_with_elements(
    ...
    let mut raw_lines: Vec<Line<'static>> = Vec:REDACTED_SECRET);
    for line_text in message.split('\n') {
        let line_start = offset;
        let line_end = line_start + line_text.len();
        let mut spans: Vec<Span<'static>> = Vec:REDACTED_SECRET);
    ...
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let wrapped_remote_images = if self.remote_image_urls.is_empty() {
            ...
            Some(adaptive_wrap_lines(
                ...
                RtOptions:REDACTED_SECRETusize::from(wrap_width))
...
impl ReasoningSummaryCell {
    ...
    fn lines(&self, width: u16) -> Vec<Line<'static>> {
        let mut lines: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        append_markdown(
            &self.content,
        ...
        adaptive_wrap_lines(
            &summary_lines,
            RtOptions:REDACTED_SECRETwidth as usize)
...
impl HistoryCell for PrefixedWrappedHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let opts = RtOptions:REDACTED_SECRETwidth.max(1) as usize)
...
impl HistoryCell for UnifiedExecInteractionCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let mut out: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        let header_wrapped = adaptive_wrap_line(&header, RtOptions:REDACTED_SECRETwrap_width));
        ...
        let input_wrapped = adaptive_wrap_lines(
            input_lines,
            RtOptions:REDACTED_SECRETwrap_width)
...
pub(crate) fn new_unified_exec_interaction(
    ...
    UnifiedExecInteractionCell:REDACTED_SECRETcommand_display, stdin)
}
...
pub(crate) fn new_unified_exec_processes_output(
    ...
    let command = PlainHistoryCell:REDACTED_SECRETvec!["/ps".magenta().into()]);
    let summary = UnifiedExecProcessesCell:REDACTED_SECRETprocesses);
    CompositeHistoryCell:REDACTED_SECRETvec![Box:REDACTED_SECRETcommand), Box:REDACTED_SECRETsummary)])
}

fn truncate_exec_snippet(full_cmd: REDACTED_SECRET) -> String {
...
pub fn new_approval_decision_cell(
    ...
    Box:REDACTED_SECRETPrefixedWrappedHistoryCell:REDACTED_SECRET
        Line::from(summary),
        symbol,
        "  ",
    ))
...
pub fn new_guardian_denied_action_request(summary: String) -> Box<dyn HistoryCell> {
    ...
    Box:REDACTED_SECRETPrefixedWrappedHistoryCell:REDACTED_SECRETline, "✗ ".red(), "  "))
}
...
pub fn new_guardian_approved_action_request(summary: String) -> Box<dyn HistoryCell> {
    ...
    Box:REDACTED_SECRETPrefixedWrappedHistoryCell:REDACTED_SECRETline, "✔ ".green(), "  "))
}
...
pub(crate) fn padded_emoji(emoji: REDACTED_SECRET) -> String {
    format!("{emoji}\u{200A}")
}
...
pub(crate) fn new_session_info(
    config: &Config,
    requested_model: REDACTED_SECRET,
    ...
    let header = SessionHeaderHistoryCell:REDACTED_SECRET
        model.clone(),
        reasoning_effort,
        show_fast_status,
    ...
    let mut parts: Vec<Box<dyn HistoryCell>> = vec![Box:REDACTED_SECRETheader)];
    ...
    if is_first_event {
        ...
        parts.push(Box:REDACTED_SECRETPlainHistoryCell { lines: help_lines }));
    } else {
        if config.show_tooltips
            && let Some(tooltips) = tooltip_override
                ...
                .map(|tip| TooltipHistoryCell:REDACTED_SECRETtip, &config.cwd))
        {
            parts.push(Box:REDACTED_SECRETtooltips));
        }
        if requested_model != model {
            ...
            parts.push(Box:REDACTED_SECRETPlainHistoryCell { lines }));
        }
    }
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        const CHANGE_MODEL_HINT_COMMAND: REDACTED_SECRET = "/model";
        const CHANGE_MODEL_HINT_EXPLANATION: REDACTED_SECRET = " to change";
        const DIR_LABEL: REDACTED_SECRET = "directory:";
...
impl McpToolCallCell {
    ...
    pub(crate) fn call_id(&self) -> REDACTED_SECRET {
        &self.call_id
    }
    ...
    pub(crate) fn complete(
        ...
        let image_cell = try_new_completed_mcp_tool_call_with_image_output(&result)
            .map(|cell| Box:REDACTED_SECRETcell) as Box<dyn HistoryCell>);
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if inline_invocation {
            ...
            let opts = RtOptions:REDACTED_SECRET(width as usize).saturating_sub(4))
                .initial_indent("".into())
                .subsequent_indent("    ".into());
        ...
        let mut detail_lines: Vec<Line<'static>> = Vec:REDACTED_SECRET);
                        ...
                        for block in content {
                            ...
                            for segment in text.split('\n') {
                                ...
                                let wrapped = adaptive_wrap_line(
                                    &line,
                                    RtOptions:REDACTED_SECRETdetail_wrap_width)
                                        .initial_indent("".into())
                                        .subsequent_indent("    ".into()),
                                );
...
pub(crate) fn new_active_mcp_tool_call(
    ...
    McpToolCallCell:REDACTED_SECRETcall_id, invocation, animations_enabled)
}
...
impl HistoryCell for WebSearchCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        PrefixedWrappedHistoryCell:REDACTED_SECRETtext, vec![bullet, " ".into()], "  ").display_lines(width)
    }
}
...
pub(crate) fn new_active_web_search_call(
    ...
    WebSearchCell:REDACTED_SECRETcall_id, query, /*action*/ None, animations_enabled)
}
...
pub(crate) fn new_web_search_call(
    ...
    let mut cell = WebSearchCell:REDACTED_SECRET
        call_id,
        query,
        Some(action),
...
fn decode_mcp_image(block: &serde_json::Value) -> Option<DynamicImage> {
    ...
    let raw_data = base64::engine::general_purpose::STANDARD
        ...
        .ok()?;
    let reader = ImageReader:REDACTED_SECRETCursor:REDACTED_SECRETraw_data))
        .with_guessed_format()
        .map_err(|e| {
...
pub(crate) fn new_warning_event(message: String) -> PrefixedWrappedHistoryCell {
    PrefixedWrappedHistoryCell:REDACTED_SECRETmessage.yellow(), "⚠ ".yellow(), "  ")
}
...
impl HistoryCell for DeprecationNoticeCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(details) = &self.details {
            ...
            let wrapped = adaptive_wrap_line(&detail_line, RtOptions:REDACTED_SECRETwrap_width));
...
pub(crate) fn new_mcp_tools_output(
    ...
    let mcp_manager = McpManager:REDACTED_SECRETArc:REDACTED_SECRETPluginsManager:REDACTED_SECRETconfig.codex_home.clone())));
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    let mut statuses_by_name = HashMap:REDACTED_SECRET);
...
pub(crate) fn new_mcp_inventory_loading(animations_enabled: bool) -> McpInventoryLoadingCell {
    McpInventoryLoadingCell:REDACTED_SECRETanimations_enabled)
}
...
fn wrap_with_prefix(
    ...
    let opts = RtOptions:REDACTED_SECRETwidth.max(1))
...
fn split_request_user_input_answer(
    ...
    let mut options = Vec:REDACTED_SECRET);
    let mut note = None;
...
impl HistoryCell for ProposedPlanCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let wrap_width = width.saturating_sub(4).max(1) as usize;
        let mut body: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        append_markdown(
            &self.plan_markdown,
            Some(wrap_width),
            Some(self.cwd.as_path()),
...
impl HistoryCell for PlanUpdateCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        let render_note = |text: REDACTED_SECRET| -> Vec<Line<'static>> {
            let wrap_width = width.saturating_sub(4).max(1) as usize;
            let note = Line::from(text.to_string().dim().italic());
            let wrapped = adaptive_wrap_line(&note, RtOptions:REDACTED_SECRETwrap_width));
            let mut out = Vec:REDACTED_SECRET);
            push_owned_lines(&wrapped, &mut out);
            out
        };

        let render_step = |status: &StepStatus, text: REDACTED_SECRET| -> Vec<Line<'static>> {
            ...
            let opts = RtOptions:REDACTED_SECRETwidth.saturating_sub(4).max(1) as usize)
...
pub(crate) fn new_reasoning_summary_block(
    ...
    if let Some(open) = full_reasoning_buffer.find("**") {
        ...
        if let Some(close) = after_open.find("**") {
            ...
            if after_close_idx < full_reasoning_buffer.len() {
                ...
                return Box:REDACTED_SECRETReasoningSummaryCell:REDACTED_SECRET
                    header_buffer,
                    summary_buffer,
                    &cwd,
    ...
    Box:REDACTED_SECRETReasoningSummaryCell:REDACTED_SECRET
        "".to_string(),
...
impl HistoryCell for FinalMessageSeparator {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        let mut label_parts = Vec:REDACTED_SECRET);
...
pub(crate) fn runtime_metrics_label(summary: RuntimeMetricsSummary) -> Option<String> {
    ...
    {
        let mut ttft_parts = Vec:REDACTED_SECRET);
        if summary.responses_api_engine_iapi_ttft_ms > 0 {
    ...
    {
        let mut tbt_parts = Vec:REDACTED_SECRET);
        if summary.responses_api_engine_iapi_tbt_ms > 0 {
...
mod tests {
    ...
    const SMALL_PNG_BASE64: REDACTED_SECRET = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==";
    ...
    fn test_cwd() -> PathBuf {
        ...
        // Windows-specific REDACTED_SECRET semantics into the fixtures.
        std::env::temp_dir()
    }

    fn stdio_server_config(
        command: REDACTED_SECRET,
        args: Vec<REDACTED_SECRET>,
        env: Option<HashMap<String, String>>,
        env_vars: Vec<REDACTED_SECRET>,
    ) -> McpServerConfig {
        let mut table = toml::Table:REDACTED_SECRET);
        table.insert(
            "command".to_string(),
    ...
    fn streamable_http_server_config(
        url: REDACTED_SECRET,
        bearer_token_env_var: Option<REDACTED_SECRET>,
    ...
    fn image_block(data: REDACTED_SECRET) -> serde_json::Value {
        serde_json::to_value(Content::image(data.to_string(), "image/png"))
            .expect("image content should serialize")
    }

    fn text_block(text: REDACTED_SECRET) -> serde_json::Value {
    ...
    fn resource_link_block(
        ...
        title: Option<REDACTED_SECRET>,
        description: Option<REDACTED_SECRET>,
    ) -> serde_json::Value {
    ...
    fn session_configured_event(model: REDACTED_SECRET) -> SessionConfiguredEvent {
    ...
    fn unified_exec_interaction_cell_renders_wait() {
        let cell = new_unified_exec_interaction(/*command_display*/ None, String:REDACTED_SECRET));
        let lines = render_transcript(&cell);
    ...
    fn final_message_separator_hides_short_worked_label_and_includes_runtime_metrics() {
        ...
        let cell = FinalMessageSeparator:REDACTED_SECRETSome(12), Some(summary));
    ...
    fn final_message_separator_includes_worked_label_after_one_minute() {
        let cell = FinalMessageSeparator:REDACTED_SECRETSome(61), /*runtime_metrics*/ None);
    ...
    fn ps_output_empty_snapshot() {
        let cell = new_unified_exec_processes_output(Vec:REDACTED_SECRET));
    ...
    async fn mcp_tools_output_masks_sensitive_values() {
        ...
        let mut headers = HashMap:REDACTED_SECRET);
        headers.insert("Authorization".to_string(), "Bearer secret".to_string());
        let mut env_headers = HashMap:REDACTED_SECRET);
        ...
        let mut tools: HashMap<String, Tool> = HashMap:REDACTED_SECRET);
        tools.insert(
        ...
        let auth_statuses: HashMap<String, McpAuthStatus> = HashMap:REDACTED_SECRET);
    ...
    fn empty_agent_message_cell_transcript() {
        let cell = AgentMessageCell:REDACTED_SECRETvec![Line::default()], /*is_first_line*/ false);
    ...
    fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
        ...
        let cell = PrefixedWrappedHistoryCell:REDACTED_SECRETsummary, "✔ ".green(), "  ");
    ...
    fn prefixed_wrapped_history_cell_does_not_split_url_like_token() {
        ...
        let cell = PrefixedWrappedHistoryCell:REDACTED_SECRETLine::from(url_like), "✔ ".green(), "  ");
        let rendered = render_lines(&cell.display_lines(/*width*/ 24));
    ...
    fn unified_exec_interaction_cell_does_not_split_url_like_stdin_token() {
        let url_like =
            "example.test/api/v1/projects/alpha-team/releases/2026-02-17/builds/1234567890";
        let cell = UnifiedExecInteractionCell:REDACTED_SECRETSome("true".to_string()), url_like.to_string());
        let rendered = render_lines(&cell.display_lines(/*width*/ 24));
    ...
    fn prefixed_wrapped_history_cell_height_matches_wrapped_rendering() {
        ...
        let cell: Box<dyn HistoryCell> = Box:REDACTED_SECRETPrefixedWrappedHistoryCell:REDACTED_SECRET
            Line::from(url_like),
            "✔ ".green(),
            "  ",
        ));
        ...
        let area = Rect:REDACTED_SECRET0, 0, width, wrapped_height);
    ...
    fn unified_exec_interaction_cell_height_matches_wrapped_rendering() {
        let url_like = "example.test/api/v1/projects/alpha-team/releases/2026-02-17/builds/1234567890/artifacts/reports/performance/summary/detail/with/a/very/long/path";
        let cell: Box<dyn HistoryCell> = Box:REDACTED_SECRETUnifiedExecInteractionCell:REDACTED_SECRET
            Some("true".to_string()),
            url_like.to_string(),
        ));
    ...
    fn session_header_includes_reasoning_level_when_present() {
        let cell = SessionHeaderHistoryCell:REDACTED_SECRET
            "gpt-4o".to_string(),
    ...
    fn coalesces_sequential_reads_within_one_call() {
        ...
        let mut cell = ExecCell:REDACTED_SECRET
            ExecCall {
    ...
    fn user_history_cell_trims_trailing_blank_message_lines_with_text_elements() {
        ...
        let cell = UserHistoryCell {
            message,
            text_elements: vec![TextElement:REDACTED_SECRET
                (0..8).into(),
    ...
    fn render_uses_wrapping_for_long_url_like_line() {
        ...
        let cell: Box<dyn HistoryCell> = Box:REDACTED_SECRETUserHistoryCell {
        ...
        let area = Rect:REDACTED_SECRET0, 0, width, height);
        let mut buf = ratatui::buffer::Buffer::empty(area);
        cell.render(area, &mut buf);
    ...
    fn reasoning_summary_height_matches_wrapped_rendering_for_url_like_content() {
        ...
        let cell: Box<dyn HistoryCell> = Box:REDACTED_SECRETReasoningSummaryCell:REDACTED_SECRET
        ...
        let logical_height = cell.display_lines(width).len() as u16;
        let wrapped_height = cell.desired_height(width);
        let expected_wrapped_height = Paragraph:REDACTED_SECRETText::from(cell.display_lines(width)))
            .wrap(Wrap { trim: false })
