...
pub(crate) fn new_debug_config_output(
    ...
    PlainHistoryCell:REDACTED_SECRETlines)
}

fn session_all_proxy_url(http_addr: REDACTED_SECRET, socks_addr: REDACTED_SECRET, socks_enabled: bool) -> String {
    if socks_enabled {
...
fn render_debug_config_lines(stack: &ConfigLayerStack) -> Vec<Line<'static>> {
    ...
    let mut requirement_lines = Vec:REDACTED_SECRET);
...
fn render_non_file_layer_details(layer: &ConfigLayerEntry) -> Vec<Line<'static>> {
    match &layer.name {
        ...
        | ConfigLayerSource::LegacyManagedConfigTomlFromFile { .. } => Vec:REDACTED_SECRET),
    }
}

fn render_session_flag_details(config: &TomlValue) -> Vec<Line<'static>> {
    let mut pairs = Vec:REDACTED_SECRET);
...
fn format_network_permission_entries<T: Copy>(
    entries: &std::collections::REDACTED_SECRET, T>,
...
mod tests {
    ...
    fn debug_config_output_lists_requirement_sources() {
        ...
        let requirements = ConfigRequirements {
            ...
            mcp_servers: Some(Sourced:REDACTED_SECRET
                BTreeMap::from([(
                    "docs".to_string(),
            ...
            network: Some(Sourced:REDACTED_SECRET
                NetworkConstraints {
                    enabled: Some(true),
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let stack =
            ConfigLayerStack:REDACTED_SECRETVec:REDACTED_SECRET), requirements, ConfigRequirementsToml::default())
                .expect("config layer stack");
...
"#,
        ...
        assert!(rendered.contains("sandbox_workspace_write.writable_REDACTED_SECRETs"));
...
"#;
    ...
    fn debug_config_output_normalizes_empty_web_search_mode_list() {
        ...
        let requirements_toml = ConfigRequirementsToml {
            ...
            allowed_web_search_modes: Some(Vec:REDACTED_SECRET)),
            guardian_developer_instructions: None,
            feature_requirements: None,
            mcp_servers: None,
            apps: None,
        ...
        let stack = ConfigLayerStack:REDACTED_SECRETVec:REDACTED_SECRET), requirements, requirements_toml)
            .expect("config layer stack");
