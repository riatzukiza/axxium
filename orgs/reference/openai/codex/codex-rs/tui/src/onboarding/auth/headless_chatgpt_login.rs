...
use crate::local_chatgpt_auth:REDACTED_SECRETChatgptAuth;
...
pub(super) fn start_headless_chatgpt_login(widget: &mut AuthModeWidget) {
    let mut opts = ServerOptions:REDACTED_SECRET
...
mod tests {
    ...
    fn device_code_sign_in_state(cancel: Arc<Notify>) -> Arc<RwLock<SignInState>> {
        Arc:REDACTED_SECRETRwLock:REDACTED_SECRETSignInState::ChatGptDeviceCode(
    ...
    fn device_code_attempt_matches_only_for_matching_cancel() {
        ...
        assert_eq!(
            device_code_attempt_matches(&state, &Arc:REDACTED_SECRETNotify:REDACTED_SECRET))),
            false
        );
        assert_eq!(
    ...
    fn begin_device_code_attempt_sets_state() {
        let sign_in_state = Arc:REDACTED_SECRETRwLock:REDACTED_SECRETSignInState::PickMode));
    ...
    fn set_device_code_state_for_active_attempt_updates_only_when_active() {
        ...
        let sign_in_state = device_code_sign_in_state(Arc:REDACTED_SECRETNotify:REDACTED_SECRET)));
        assert_eq!(
            set_device_code_state_for_active_attempt(
                &sign_in_state,
                &request_frame,
    ...
    fn chatgpt_auth_tokens_success_sets_success_message_without_login_id() {
        let sign_in_state = device_code_sign_in_state(Arc:REDACTED_SECRETNotify:REDACTED_SECRET)));
        let request_frame = FrameRequester::test_dummy();
        let error = Arc:REDACTED_SECRETRwLock:REDACTED_SECRETNone));
