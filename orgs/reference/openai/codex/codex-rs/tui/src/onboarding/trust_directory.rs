...
impl WidgetRef for &TrustDirectoryWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        let options: Vec<(REDACTED_SECRET, TrustDirectorySelection)> = vec![
        ...
        if let Some(error) = &self.error {
            column.push(
                Paragraph:REDACTED_SECRETerror.to_string())
                    .red()
                    .wrap(Wrap { trim: true })
                    .inset(Insets::tlbr(
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        let target =
            resolve_REDACTED_SECRET_git_project_for_trust(&self.cwd).unwrap_or_else(|| self.cwd.clone());
...
mod tests {
    ...
    fn release_event_does_not_change_selection() {
        let codex_home = TempDir:REDACTED_SECRET).expect("temp home");
        ...
        let release = KeyEvent {
            ...
            ..KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE)
        };
        widget.handle_key_event(release);
        assert_eq!(widget.selection, None);

        let press = KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE);
    ...
    fn renders_snapshot_for_git_repo() {
        ...
        let mut terminal =
            Terminal:REDACTED_SECRETVT100Backend:REDACTED_SECRET/*width*/ 70, /*height*/ 14)).expect("terminal");
        terminal
