...
pub(crate) fn mark_url_hyperlink(buf: &mut Buffer, area: Rect, url: REDACTED_SECRET) {
...
const API_KEY_DISABLED_MESSAGE: REDACTED_SECRET = "API key login is disabled.";
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph:REDACTED_SECRETlines)
    ...
    fn render_api_key_entry(&self, area: Rect, buf: &mut Buffer, state: &ApiKeyInputState) {
        ...
        Paragraph:REDACTED_SECRETintro_lines)
            .wrap(Wrap { trim: false })
        ...
        Paragraph:REDACTED_SECRETcontent_line)
            .wrap(Wrap { trim: false })
            .block(
                Block::default()
                    .title("API key")
        ...
        Paragraph:REDACTED_SECRETfooter_lines)
            .wrap(Wrap { trim: false })
...
pub(super) fn maybe_open_auth_url_in_browser(request_handle: &AppServerRequestHandle, url: REDACTED_SECRET) {
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let widget = AuthModeWidget {
            ...
            error: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETNone)),
            sign_in_state: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETSignInState::PickMode)),
            codex_home: codex_home_path.clone(),
    ...
    async fn cancel_active_attempt_notifies_device_code_login() {
        let (widget, _tmp) = widget_forced_chatgpt().await;
        let cancel = Arc:REDACTED_SECRETNotify:REDACTED_SECRET));
        *widget.error.write().unwrap() = Some("still logging in".to_string());
        *widget.sign_in_state.write().unwrap() =
    ...
    fn collect_osc8_chars(buf: &Buffer, area: Rect, url: REDACTED_SECRET) -> String {
        let open = format!("\x1B]8;;{url}\x07");
        let close = "\x1B]8;;\x07";
        let mut chars = String:REDACTED_SECRET);
        for y in area.top()..area.bottom() {
            for x in area.left()..area.right() {
    ...
    fn continue_in_browser_renders_osc8_hyperlink() {
        let runtime = tokio::runtime::Runtime:REDACTED_SECRET).unwrap();
        ...
        *widget.sign_in_state.write().unwrap() =
        ...
        let area = Rect:REDACTED_SECRET0, 0, 30, 20);
    ...
    fn mark_url_hyperlink_wraps_cyan_underlined_cells() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 20, 1);
        let mut buf = Buffer::empty(area);
    ...
    fn mark_url_hyperlink_sanitizes_control_chars() {
        let area = Rect:REDACTED_SECRET0, 0, 10, 1);
        let mut buf = Buffer::empty(area);
