...
impl WelcomeWidget {
    pub(crate) fn new(
        ...
        Self {
            is_logged_in,
            animation: AsciiAnimation:REDACTED_SECRETrequest_frame),
            animations_enabled,
            layout_area: Cell:REDACTED_SECRETNone),
        }
    }
...
mod tests {
    ...
    static VARIANT_A: [REDACTED_SECRET; 1] = ["frame-a"];
    static VARIANT_B: [REDACTED_SECRET; 1] = ["frame-b"];
    static VARIANTS: [&[REDACTED_SECRET]; 2] = [&VARIANT_A, &VARIANT_B];

    fn row_containing(buf: &Buffer, needle: REDACTED_SECRET) -> Option<u16> {
    ...
    fn welcome_renders_animation_on_first_draw() {
        let widget = WelcomeWidget:REDACTED_SECRET
            /*is_logged_in*/ false,
            FrameRequester::test_dummy(),
            /*animations_enabled*/ true,
        );
        let area = Rect:REDACTED_SECRET0, 0, MIN_ANIMATION_WIDTH, MIN_ANIMATION_HEIGHT);
    ...
    fn welcome_skips_animation_below_height_breakpoint() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, MIN_ANIMATION_WIDTH, MIN_ANIMATION_HEIGHT - 1);
        let mut buf = Buffer::empty(area);
        (&widget).render(area, &mut buf);
    ...
    fn ctrl_dot_changes_animation_variant() {
        ...
        widget.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('.'), KeyModifiers::CONTROL));
