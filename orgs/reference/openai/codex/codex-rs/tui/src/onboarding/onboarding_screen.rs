...
impl OnboardingScreen {
    pub(crate) fn new(tui: &mut Tui, args: OnboardingScreenArgs) -> Self {
        ...
        let mut steps: Vec<Step> = Vec:REDACTED_SECRET);
        steps.push(Step::Welcome(WelcomeWidget:REDACTED_SECRET
    ...
    fn current_steps_mut(&mut self) -> Vec<&mut Step> {
        let mut out: Vec<&mut Step> = Vec:REDACTED_SECRET);
    ...
    fn current_steps(&self) -> Vec<&Step> {
        let mut out: Vec<&Step> = Vec:REDACTED_SECRET);
...
impl WidgetRef for &OnboardingScreen {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        while i < current_steps.len() && y < bottom {
            ...
            let scratch_area = Rect:REDACTED_SECRET0, 0, width, max_h);
...
pub(crate) async fn run_onboarding_app(
    ...
    let mut onboarding_screen = OnboardingScreen:REDACTED_SECRETtui, args);
