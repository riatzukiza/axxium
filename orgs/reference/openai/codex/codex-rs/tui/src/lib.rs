...
mod oss_selection;
mod pager_overlay;
pub mod REDACTED_SECRET_widgets;
mod render;
mod resume_picker;
mod selection_list;
mod session_log;
...
mod voice {
    ...
    impl VoiceCapture {
        pub fn start_realtime(_config: &Config, _tx: AppEventREDACTED_SECRETer) -> Result<Self, String> {
        ...
        pub fn stopped_flag(&self) -> Arc<AtomicBool> {
            Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue))
        }

        pub fn last_peak_arc(&self) -> Arc<AtomicU16> {
            Arc:REDACTED_SECRETAtomicU16:REDACTED_SECRET0))
        }
    }
...
pub use REDACTED_SECRET_widgets::composer_input::ComposerAction;
pub use REDACTED_SECRET_widgets::composer_input::ComposerInput;
...
fn remote_addr_has_explicit_port(addr: REDACTED_SECRET, parsed: &Url) -> bool {
...
pub fn normalize_remote_addr(addr: REDACTED_SECRET) -> color_eyre::Result<String> {
...
fn validate_remote_auth_token_transport(websocket_url: REDACTED_SECRET) -> color_eyre::Result<()> {
...
async fn connect_remote_app_server(
    ...
    let app_server = RemoteAppServerClient::connect(RemoteAppServerConnectArgs {
        ...
        opt_out_notification_methods: Vec:REDACTED_SECRET),
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        codex_feedback::CodexFeedback:REDACTED_SECRET),
    )
    .await?;
    Ok(AppServerSession:REDACTED_SECRETapp_server))
}
...
pub async fn run_main(
    ...
    let overrides = ConfigOverrides {
        ...
        additional_writable_REDACTED_SECRETs: additional_dirs,
        ..Default::default()
    };
    ...
    let mut log_file_opts = OpenOptions:REDACTED_SECRET);
    ...
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter:REDACTED_SECRET"codex_core=info,codex_tui=info,codex_rmcp_client=info")
        })
    };
    ...
    let feedback = codex_feedback::CodexFeedback:REDACTED_SECRET);
...
async fn run_ratatui_app(
    ...
    std::panic::set_hook(Box:REDACTED_SECRETmove |info| {
    ...
    let mut tui = Tui:REDACTED_SECRETterminal);
    let mut terminal_restore_guard = TerminalRestoreGuard:REDACTED_SECRET);
    ...
    let mut onboarding_app_server = if needs_onboarding_app_server {
        Some(
            AppServerSession:REDACTED_SECRET
                start_app_server(
                    &app_server_target,
                    arg0_paths.clone(),
    ...
    let mut missing_session_exit = |id_str: REDACTED_SECRET, action: REDACTED_SECRET| {
    ...
    let app_result = App::run(
        &mut tui,
        AppServerSession:REDACTED_SECRETapp_server).with_remote_cwd_override(remote_cwd_override),
        config,
        cli_kv_overrides.clone(),
        overrides.clone(),
        active_profile,
...
pub(crate) async fn resolve_session_thread_id(
    path: &Path,
    id_str_if_uuid: Option<REDACTED_SECRET>,
) -> Option<ThreadId> {
...
mod tests {
    ...
    -> std::io::Result<()> {
        let temp_dir = TempDir:REDACTED_SECRET)?;
        let config = build_config(&temp_dir).await?;
        let cwd = Path:REDACTED_SECRET"repo/on/server");
    ...
    fn config_cwd_for_app_server_target_uses_current_dir_for_remote_sessions() -> std::io::Result<()>
    {
        let remote_only_cwd = if cfg!(windows) {
            Path:REDACTED_SECRETr"C:\definitely\not\local\to\this\test")
        } else {
            Path:REDACTED_SECRET"/definitely/not/local/to/this/test")
        };
    ...
    async fn read_session_cwd_returns_none_without_sqlite_or_rollout_path() -> std::io::Result<()> {
        ...
        let cwd = read_session_cwd(&config, ThreadId:REDACTED_SECRET), /*path*/ None).await;
    ...
    -> color_eyre::Result<()> {
        ...
        let mut builder = codex_state::ThreadMetadataBuilder:REDACTED_SECRET
            thread_id,
            rollout_path.clone(),
            created_at,
            SessionSource::Cli,
        ...
        let mut app_server =
            AppServerSession:REDACTED_SECRETcodex_app_server_client::AppServerClient::InProcess(
    ...
    async fn read_session_cwd_prefers_latest_turn_context() -> std::io::Result<()> {
        ...
        let cwd = read_session_cwd(&config, ThreadId:REDACTED_SECRET), Some(&rollout_path))
            .await
            .expect("expected cwd");
    ...
    async fn should_prompt_when_meta_matches_current_but_latest_turn_differs() -> std::io::Result<()>
    {
        ...
        let session_cwd = read_session_cwd(&config, ThreadId:REDACTED_SECRET), Some(&rollout_path))
            .await
            .expect("expected cwd");
