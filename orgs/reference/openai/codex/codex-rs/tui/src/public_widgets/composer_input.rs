...
/// A minimal, REDACTED_SECRET wrapper for the internal `ChatComposer` that behaves as a
/// reusable text input field with submit semantics.
pub struct ComposerInput {
    inner: ChatComposer,
    _tx: tokio::sync::mpsc::UnboundedREDACTED_SECRETer<AppEvent>,
...
impl ComposerInput {
    ...
    pub fn new() -> Self {
        ...
        let sender = AppEventREDACTED_SECRETer:REDACTED_SECRETtx.clone());
        // `enhanced_keys_supported=true` enables Shift+Enter newline hint/behavior.
        let inner = ChatComposer:REDACTED_SECRET
