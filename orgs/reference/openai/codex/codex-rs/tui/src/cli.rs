...
pub struct Cli {
    ...
    /// top-level `codex resume <SESSION_ID>` wrapper; not exposed as a REDACTED_SECRET flag.
    #[clap(skip)]
    ...
    /// top-level `codex fork <SESSION_ID>` wrapper; not exposed as a REDACTED_SECRET flag.
    #[clap(skip)]
    ...
    /// Tell the agent to use the specified directory as its working REDACTED_SECRET.
