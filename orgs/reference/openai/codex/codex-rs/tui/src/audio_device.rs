...
pub(crate) fn list_realtime_audio_device_names(
    ...
    let mut device_names = Vec:REDACTED_SECRET);
...
fn configured_name(kind: RealtimeAudioDeviceKind, config: &Config) -> Option<REDACTED_SECRET> {
    match kind {
...
fn missing_device_error(kind: RealtimeAudioDeviceKind, configured_name: Option<REDACTED_SECRET>) -> String {
