...
pub(crate) type PromptSubmitted = Box<dyn Fn(String) + REDACTED_SECRET + Sync>;
...
impl Renderable for CustomPromptView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph:REDACTED_SECRETLine::from(title_spans)).render(title_area, buf);
        ...
        if let Some(context_label) = &self.context_label {
            ...
            Paragraph:REDACTED_SECRETLine::from(spans)).render(context_area, buf);
        ...
        if input_area.width >= 2 {
            ...
            if text_area_height > 0 {
                ...
                if self.textarea.text().is_empty() {
                    Paragraph:REDACTED_SECRETLine::from(self.placeholder.clone().dim()))
