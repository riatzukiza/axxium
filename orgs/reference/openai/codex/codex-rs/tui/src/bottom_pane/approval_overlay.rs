...
impl ApprovalRequest {
    ...
    fn thread_label(&self) -> Option<REDACTED_SECRET> {
        match self {
...
impl ApprovalOverlay {
    pub fn new(request: ApprovalRequest, app_event_tx: AppEventREDACTED_SECRETer, features: Features) -> Self {
        let mut view = Self {
            current_request: None,
            queue: Vec:REDACTED_SECRET),
            app_event_tx: app_event_tx.clone(),
            list: ListSelectionView:REDACTED_SECRETDefault::default(), app_event_tx),
            options: Vec:REDACTED_SECRET),
    ...
    fn set_current(&mut self, request: ApprovalRequest) {
        ...
        self.list = ListSelectionView:REDACTED_SECRETparams, self.app_event_tx.clone());
    }
    ...
    fn build_options(
        ...
        let header = Box:REDACTED_SECRETColumnRenderable::with([
    ...
    fn handle_exec_decision(&self, id: REDACTED_SECRET, command: &[String], decision: ReviewDecision) {
    ...
    fn handle_permissions_decision(
        ...
        if request.thread_label().is_none() {
            ...
            self.app_event_tx.send(AppEvent::InsertHistoryCell(Box:REDACTED_SECRET
                crate::history_cell::PlainHistoryCell:REDACTED_SECRETvec![message.into()]),
            )));
        }
        let thread_id = request.thread_id();
    ...
    fn handle_patch_decision(&self, id: REDACTED_SECRET, decision: ReviewDecision) {
...
fn build_header(request: &ApprovalRequest) -> Box<dyn Renderable> {
    match request {
        ApprovalRequest::Exec {
            ...
            let mut header: Vec<Line<'static>> = Vec:REDACTED_SECRET);
            ...
            Box:REDACTED_SECRETParagraph:REDACTED_SECRETheader).wrap(Wrap { trim: false }))
        }
        ApprovalRequest::Permissions {
            thread_label,
            reason,
        ...
        ApprovalRequest::ApplyPatch {
            ...
            let mut header: Vec<Box<dyn Renderable>> = Vec:REDACTED_SECRET);
            if let Some(thread_label) = thread_label {
                header.push(Box:REDACTED_SECRETLine::from(vec![
                    "Thread: ".into(),
                    thread_label.clone().bold(),
                ])));
                header.push(Box:REDACTED_SECRETLine::from("")));
            }
            if let Some(reason) = reason
                && !reason.is_empty()
            {
                header.push(Box:REDACTED_SECRET
                    Paragraph:REDACTED_SECRETLine::from_iter([
                        "Reason: ".into(),
                        reason.clone().italic(),
                    ]))
            ...
            header.push(DiffSummary:REDACTED_SECRETchanges.clone(), cwd.clone()).into());
            Box:REDACTED_SECRETColumnRenderable::with(header))
        }
        ApprovalRequest::McpElicitation {
            thread_label,
            server_name,
            ...
            let header = Paragraph:REDACTED_SECRETlines).wrap(Wrap { trim: false });
...
mod tests {
    ...
    fn render_overlay_lines(view: &ApprovalOverlay, width: u16) -> String {
        ...
        let mut buf = Buffer::empty(Rect:REDACTED_SECRET0, 0, width, height));
        view.render(Rect:REDACTED_SECRET0, 0, width, height), &mut buf);
        (0..buf.area.height)
            .map(|row| {
                (0..buf.area.width)
    ...
    fn ctrl_c_aborts_and_clears_queue() {
        ...
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx);
        let mut view = ApprovalOverlay:REDACTED_SECRETmake_exec_request(), tx, Features::with_defaults());
    ...
    fn shortcut_triggers_selection() {
        ...
        let mut view = ApprovalOverlay:REDACTED_SECRETmake_exec_request(), tx, Features::with_defaults());
        assert!(!view.is_complete());
        view.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('y'), KeyModifiers::NONE));
    ...
    fn o_opens_source_thread_for_cross_thread_approval() {
        ...
        let mut view = ApprovalOverlay:REDACTED_SECRET
            ApprovalRequest::Exec {
                thread_id,
        ...
        view.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('o'), KeyModifiers::NONE));
    ...
    fn cross_thread_footer_hint_mentions_o_shortcut() {
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx);
        let view = ApprovalOverlay:REDACTED_SECRET
            ApprovalRequest::Exec {
                thread_id: ThreadId:REDACTED_SECRET),
                thread_label: Some("Robie [explorer]".to_string()),
                id: "test".to_string(),
    ...
    fn exec_prefix_option_emits_execpolicy_amendment() {
        ...
        let mut view = ApprovalOverlay:REDACTED_SECRET
            ApprovalRequest::Exec {
                ...
                available_decisions: vec![
                    ...
                    ReviewDecision::ApprovedExecpolicyAmendment {
                        proposed_execpolicy_amendment: ExecPolicyAmendment:REDACTED_SECRETvec![
                            "echo".to_string(),
                        ]),
                    },
        ...
        view.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('p'), KeyModifiers::NONE));
        let mut saw_op = false;
        while let Ok(ev) = rx.try_recv() {
    ...
    fn network_deny_forever_shortcut_is_not_bound() {
        ...
        view.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('d'), KeyModifiers::NONE));
    ...
    fn header_includes_command_snippet() {
        ...
        let view = ApprovalOverlay:REDACTED_SECRETexec_request, tx, Features::with_defaults());
        let mut buf = Buffer::empty(Rect:REDACTED_SECRET0, 0, 80, view.desired_height(/*width*/ 80)));
        view.render(
            Rect:REDACTED_SECRET0, 0, 80, view.desired_height(/*width*/ 80)),
            &mut buf,
        );
    ...
    fn permissions_session_shortcut_submits_session_scope() {
        ...
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx);
        let mut view =
            ApprovalOverlay:REDACTED_SECRETmake_permissions_request(), tx, Features::with_defaults());

        view.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('a'), KeyModifiers::NONE));
    ...
    fn additional_permissions_prompt_shows_permission_rule_line() {
        ...
        let view = ApprovalOverlay:REDACTED_SECRETexec_request, tx, Features::with_defaults());
        let mut buf = Buffer::empty(Rect:REDACTED_SECRET0, 0, 120, view.desired_height(/*width*/ 120)));
        view.render(
            Rect:REDACTED_SECRET0, 0, 120, view.desired_height(/*width*/ 120)),
            &mut buf,
        );
    ...
    fn permissions_prompt_snapshot() {
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx);
        let view = ApprovalOverlay:REDACTED_SECRETmake_permissions_request(), tx, Features::with_defaults());
        assert_snapshot!(
    ...
    fn network_exec_prompt_title_includes_host() {
        ...
        let view = ApprovalOverlay:REDACTED_SECRETexec_request, tx, Features::with_defaults());
        let mut buf = Buffer::empty(Rect:REDACTED_SECRET0, 0, 100, view.desired_height(/*width*/ 100)));
        view.render(
            Rect:REDACTED_SECRET0, 0, 100, view.desired_height(/*width*/ 100)),
            &mut buf,
        );
    ...
    fn enter_sets_last_selected_index_without_dismissing() {
        ...
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let mut view = ApprovalOverlay:REDACTED_SECRETmake_exec_request(), tx, Features::with_defaults());
        view.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
