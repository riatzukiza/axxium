...
impl BottomPane {
    pub fn new(params: BottomPaneParams) -> Self {
        ...
        Self {
            composer,
            view_stack: Vec:REDACTED_SECRET),
            app_event_tx,
            frame_requester,
            has_input_focus,
            enhanced_keys_supported,
            ...
            status: None,
            unified_exec_footer: UnifiedExecFooter:REDACTED_SECRET),
            pending_input_preview: PendingInputPreview:REDACTED_SECRET),
            pending_thread_approvals: PendingThreadApprovals:REDACTED_SECRET),
        ...
        if running {
            if !was_running {
                if self.status.is_none() {
                    self.status = Some(StatusIndicatorWidget:REDACTED_SECRET
    ...
    pub(crate) fn show_selection_view(&mut self, params: list_selection_view::SelectionViewParams) {
        let view = list_selection_view::ListSelectionView:REDACTED_SECRETparams, self.app_event_tx.clone());
        self.push_view(Box:REDACTED_SECRETview));
    }
    ...
    pub fn push_approval_request(&mut self, request: ApprovalRequest, features: &Features) {
        ...
        let modal = ApprovalOverlay:REDACTED_SECRETrequest, self.app_event_tx.clone(), features.clone());
        self.pause_status_timer_for_modal();
        self.push_view(Box:REDACTED_SECRETmodal));
    }
    ...
    pub fn push_user_input_request(&mut self, request: RequestUserInputEvent) {
        ...
        let modal = RequestUserInputOverlay:REDACTED_SECRET
            request,
            self.app_event_tx.clone(),
    ...
    pub(crate) fn push_mcp_server_elicitation_request(
            ...
            && let Some(install_url) = tool_suggestion.install_url.clone()
        {
            ...
            let view = AppLinkView:REDACTED_SECRET
                AppLinkViewParams {
        ...
        let modal = McpServerElicitationOverlay:REDACTED_SECRET
            request,
            self.app_event_tx.clone(),
            self.has_input_focus,
            self.enhanced_keys_supported,
    ...
    fn as_renderable(&'_ self) -> RenderableItem<'_> {
        if let Some(view) = self.active_view() {
            ...
            let mut flex2 = FlexRenderable:REDACTED_SECRET);
            flex2.push(/*flex*/ 1, RenderableItem::Owned(flex.into()));
            flex2.push(/*flex*/ 0, RenderableItem::Borrowed(&self.composer));
            RenderableItem::Owned(Box:REDACTED_SECRETflex2))
        }
    }
...
impl BottomPane {
    pub(crate) fn insert_recording_meter_placeholder(&mut self, text: REDACTED_SECRET) -> String {
...
mod tests {
    ...
    fn exec_request() -> ApprovalRequest {
        ApprovalRequest::Exec {
            thread_id: codex_protocol::ThreadId:REDACTED_SECRET),
            thread_label: None,
            id: "1".to_string(),
    ...
    fn ctrl_c_on_modal_consumes_without_showing_quit_hint() {
        ...
        let mut pane = BottomPane:REDACTED_SECRETBottomPaneParams {
            app_event_tx: tx,
            ...
            skills: Some(Vec:REDACTED_SECRET)),
        });
    ...
    fn overlay_not_shown_above_approval_modal() {
        ...
        let mut r0 = String:REDACTED_SECRET);
    ...
    fn composer_shown_after_denied_while_task_running() {
        ...
        use crossterm::event::KeyEvent;
        use crossterm::event::KeyModifiers;
        pane.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('n'), KeyModifiers::NONE));
        ...
        let mut buf = Buffer::empty(area);
        pane.render(area, &mut buf);
        let mut row0 = String:REDACTED_SECRET);
        for x in 0..area.width {
    ...
    fn status_and_composer_fill_height_without_bottom_padding() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 30, height);
        assert_snapshot!(
    ...
    fn unified_exec_summary_does_not_increase_height_when_status_visible() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, width, after);
    ...
    fn esc_with_skill_popup_does_not_interrupt_task() {
        ...
        pane.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Esc, KeyModifiers::NONE));
    ...
    fn esc_routes_to_handle_key_event_when_requested() {
        ...
        let on_ctrl_c_calls = Rc:REDACTED_SECRETCell:REDACTED_SECRET0));
        let handle_calls = Rc:REDACTED_SECRETCell:REDACTED_SECRET0));
        pane.push_view(Box:REDACTED_SECRETEscRoutingView {
    ...
    fn release_events_are_ignored_for_active_view() {
        ...
        let handle_calls = Rc:REDACTED_SECRETCell:REDACTED_SECRET0));
        pane.push_view(Box:REDACTED_SECRETCountingView {
            handle_calls: Rc::clone(&handle_calls),
        }));
    ...
    fn paste_completion_clears_stacked_views_and_restores_composer_input() {
        ...
        let lower_view_handle_calls = Rc:REDACTED_SECRETCell:REDACTED_SECRET0));
        pane.push_view(Box:REDACTED_SECRETBlockingView {
            handle_calls: Rc::clone(&lower_view_handle_calls),
        }));
        pane.push_view(Box:REDACTED_SECRETPasteCompletesView::default()));
        ...
        pane.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Down, KeyModifiers::NONE));

        let area = Rect:REDACTED_SECRET0, 0, 40, pane.desired_height(/*width*/ 40).max(2));
