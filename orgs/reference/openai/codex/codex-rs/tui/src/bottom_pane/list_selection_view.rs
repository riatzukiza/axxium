...
pub(crate) type SelectionAction = Box<dyn Fn(&AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync>;
...
pub(crate) type OnSelectionChangedCallback =
    Option<Box<dyn Fn(usize, &AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync>>;
...
pub(crate) type OnCancelCallback = Option<Box<dyn Fn(&AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync>>;
...
impl Default for SelectionViewParams {
    fn default() -> Self {
        Self {
            ...
            side_content: Box:REDACTED_SECRET()),
...
impl ListSelectionView {
    ...
    pub fn new(params: SelectionViewParams, app_event_tx: AppEventREDACTED_SECRETer) -> Self {
        ...
        if params.title.is_some() || params.subtitle.is_some() {
            ...
            header = Box:REDACTED_SECRETColumnRenderable::with([
                header,
                Box:REDACTED_SECRETtitle),
                Box:REDACTED_SECRETsubtitle),
            ]));
        }
        let mut s = Self {
            ...
            items: params.items,
            state: ScrollState:REDACTED_SECRET),
            complete: false,
            app_event_tx,
            is_searchable: params.is_searchable,
            search_query: String:REDACTED_SECRET),
            ...
            filtered_indices: Vec:REDACTED_SECRET),
        ...
        self.filtered_indices
            ...
            .filter_map(|(visible_idx, actual_idx)| {
                self.items.get(*actual_idx).map(|item| {
                    ...
                    let mut name_prefix_spans = Vec:REDACTED_SECRET);
...
impl Renderable for ListSelectionView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if let Some(sw) = side_w {
            ...
            let side_area = Rect:REDACTED_SECRETside_x, content_area.y, sw, content_area.height);
            ...
            let clear_area = Rect:REDACTED_SECRET
...
mod tests {
    ...
    fn make_selection_view(subtitle: Option<REDACTED_SECRET>) -> ListSelectionView {
        ...
        ListSelectionView:REDACTED_SECRET
            SelectionViewParams {
    ...
    fn description_col(rendered: REDACTED_SECRET, item_marker: REDACTED_SECRET, description: REDACTED_SECRET) -> usize {
        let line = rendered
            .lines()
    ...
    fn render_before_after_scroll_snapshot(col_width_mode: ColumnWidthMode, width: u16) -> String {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let mut view = ListSelectionView:REDACTED_SECRET
            SelectionViewParams {
    ...
    fn theme_picker_subtitle_uses_fallback_text_in_94x35_terminal() {
        ...
        let view = ListSelectionView:REDACTED_SECRETparams, tx);
    ...
    fn preserve_side_content_bg_keeps_rendered_background_colors() {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let view = ListSelectionView:REDACTED_SECRET
            SelectionViewParams {
                title: Some("Debug".to_string()),
                items: vec![SelectionItem {
                ...
                side_content: Box:REDACTED_SECRETStyledMarkerRenderable {
                    marker: "+",
        ...
        let area = Rect:REDACTED_SECRET0, 0, 120, 35);
        let mut buf = Buffer::empty(area);
    ...
    fn enter_with_no_matches_triggers_cancel_callback() {
        ...
        let mut view = ListSelectionView:REDACTED_SECRET
            SelectionViewParams {
                ...
                is_searchable: true,
                on_cancel: Some(Box:REDACTED_SECRET|tx: &_| {
    ...
    fn move_down_without_selection_change_does_not_fire_callback() {
        ...
        let mut view = ListSelectionView:REDACTED_SECRET
            SelectionViewParams {
                ...
                on_selection_changed: Some(Box:REDACTED_SECRET|_idx, tx: &_| {
                    tx.send(AppEvent::OpenApprovalsPopup);
                })),
                ..Default::default()
            },
    ...
    fn width_changes_do_not_hide_rows() {
        ...
        let mut missing: Vec<u16> = Vec:REDACTED_SECRET);
        for width in 60..=90 {
    ...
    fn side_layout_width_half_uses_exact_split() {
        ...
        let view = ListSelectionView:REDACTED_SECRET
            SelectionViewParams {
                ...
                side_content: Box:REDACTED_SECRETMarkerRenderable {
                    marker: "W",
                    height: 1,
                }),
                side_content_width: SideContentWidth::Half,
    ...
    fn stacked_side_content_is_used_when_side_by_side_does_not_fit() {
        ...
        let view = ListSelectionView:REDACTED_SECRET
            SelectionViewParams {
                ...
                stacked_side_content: Some(Box:REDACTED_SECRETMarkerRenderable {
                    marker: "N",
                    height: 1,
                })),
                side_content_width: SideContentWidth::Half,
    ...
    fn side_content_clearing_handles_non_zero_buffer_origin() {
        ...
        let width = 120;
        let height = view.desired_height(width);
        let area = Rect:REDACTED_SECRET0, 20, width, height);
        let mut buf = Buffer::empty(area);
