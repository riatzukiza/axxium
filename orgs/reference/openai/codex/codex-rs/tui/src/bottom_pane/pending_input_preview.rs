...
impl PendingInputPreview {
    pub(crate) fn new() -> Self {
        Self {
            pending_steers: Vec:REDACTED_SECRET),
            rejected_steers: Vec:REDACTED_SECRET),
            queued_messages: Vec:REDACTED_SECRET),
    ...
    fn push_section_header(lines: &mut Vec<Line<'static>>, width: u16, header: Line<'static>) {
        ...
        lines.extend(adaptive_wrap_lines(
            ...
            RtOptions:REDACTED_SECRETwidth as usize).subsequent_indent(Line::from("  ".dim())),
        ));
    }
    ...
    fn as_renderable(&self, width: u16) -> Box<dyn Renderable> {
        ...
        Paragraph:REDACTED_SECRETlines).into()
    }
}
...
mod tests {
    ...
    fn desired_height_empty() {
        let queue = PendingInputPreview:REDACTED_SECRET);
        assert_eq!(queue.desired_height(/*width*/ 40), 0);
    }

    #[test]
    fn desired_height_one_message() {
        let mut queue = PendingInputPreview:REDACTED_SECRET);
    ...
    fn render_one_message() {
        ...
        queue.render(Rect:REDACTED_SECRET0, 0, width, height), &mut buf);
