...
impl ChatComposer {
    ...
    pub(crate) fn new_with_config(
        ...
        let mut this = Self {
            textarea: TextArea:REDACTED_SECRET),
            textarea_state: RefCell:REDACTED_SECRETTextAreaState::default()),
            active_popup: ActivePopup::None,
            app_event_tx,
            history: ChatComposerHistory:REDACTED_SECRET),
            ...
            pending_pastes: Vec:REDACTED_SECRET),
            large_paste_counters: HashMap:REDACTED_SECRET),
            has_focus: has_input_focus,
            frame_requester: None,
            attached_images: Vec:REDACTED_SECRET),
            placeholder_text,
            ...
            mention_bindings: HashMap:REDACTED_SECRET),
            recent_submission_mention_bindings: Vec:REDACTED_SECRET),
    ...
    pub(crate) fn take_mention_bindings(&mut self) -> Vec<MentionBinding> {
        ...
        let mut ordered = Vec:REDACTED_SECRET);
    ...
    pub(crate) fn apply_external_edit(&mut self, text: String) {
        ...
        let mut placeholder_counts: HashMap<String, usize> = HashMap:REDACTED_SECRET);
        ...
        let mut kept_images = Vec:REDACTED_SECRET);
        ...
        let mut remaining: HashMap<REDACTED_SECRET, usize> = HashMap:REDACTED_SECRET);
        for img in &self.attached_images {
            *remaining.entry(img.placeholder.as_str()).or_insert(0) += 1;
        }

        let mut occurrences: Vec<(usize, REDACTED_SECRET)> = Vec:REDACTED_SECRET);
    ...
    pub(crate) fn clear_for_ctrl_c(&mut self) -> Option<String> {
        ...
        self.set_text_content(String:REDACTED_SECRET), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    ...
    fn prune_attached_images_for_submission(&mut self, text: REDACTED_SECRET, text_elements: &[TextElement]) {
        if self.attached_images.is_empty() {
            return;
        }
        let image_placeholders: HashSet<REDACTED_SECRET> = text_elements
            .iter()
    ...
    #[inline]
    fn clamp_to_char_boundary(text: REDACTED_SECRET, pos: usize) -> usize {
    ...
    fn is_image_path(path: REDACTED_SECRET) -> bool {
    ...
    fn trim_text_elements(
        ...
        elements
            ...
            .filter_map(|elem| {
                ...
                Some(TextElement:REDACTED_SECRET
                    ByteRange {
                        start: new_start,
                        end: new_end,
                    },
    ...
    pub(crate) fn expand_pending_pastes(
        ...
        let mut pending_by_placeholder: HashMap<REDACTED_SECRET, VecDeque<REDACTED_SECRET>> = HashMap:REDACTED_SECRET);
        ...
        for elem in elements {
            ...
            if let Some(actual) = replacement {
                ...
                rebuilt_elements.push(TextElement:REDACTED_SECRET
                    ByteRange {
                        start: new_start,
                        end: new_end,
                    },
    ...
    ///   lone prefix character yields `Some(String:REDACTED_SECRET))` to surface hints.
    ...
    fn insert_selected_path(&mut self, path: REDACTED_SECRET) {
        let cursor_offset = self.textarea.cursor();
        let text = self.textarea.text();
        // Clamp to a valid char boundary to avoid panics when slicing.
        let safe_cursor = Self::clamp_to_char_boundary(text, cursor_offset);
    ...
    fn insert_selected_mention(&mut self, insert_text: REDACTED_SECRET, path: Option<REDACTED_SECRET>) {
        let cursor_offset = self.textarea.cursor();
        let text = self.textarea.text();
        let safe_cursor = Self::clamp_to_char_boundary(text, cursor_offset);
    ...
    fn mention_name_from_insert_text(insert_text: REDACTED_SECRET) -> Option<String> {
    ...
    pub(crate) fn sync_popups(&mut self) {
        ...
        if browsing_history {
            if self.current_file_query.is_some() {
                self.app_event_tx
                    .send(AppEvent::StartFileSearch(String:REDACTED_SECRET)));
    ...
    fn sync_slash_command_elements(&mut self) {
        ...
        let mut stale_ranges = Vec:REDACTED_SECRET);
    ...
    fn slash_command_element_range(&self, first_line: REDACTED_SECRET) -> Option<Range<usize>> {
    ...
    fn is_known_slash_name(&self, name: REDACTED_SECRET) -> bool {
    ...
    fn slash_command_under_cursor(first_line: REDACTED_SECRET, cursor: usize) -> Option<(REDACTED_SECRET, REDACTED_SECRET)> {
    ...
    fn looks_like_slash_prefix(&self, name: REDACTED_SECRET, rest_after_name: REDACTED_SECRET) -> bool {
        if !self.slash_commands_enabled() {
            return false;
        }
        if name.is_empty() {
        ...
        match &mut self.active_popup {
            ...
            _ => {
                if is_editing_slash_command_name {
                    ...
                    let mut command_popup = CommandPopup:REDACTED_SECRETCommandPopupFlags {
    ...
    fn sync_file_search_popup(&mut self, query: String) {
        ...
        match &mut self.active_popup {
            ...
            _ => {
                let mut popup = FileSearchPopup:REDACTED_SECRET);
                if query.is_empty() {
                    popup.set_empty_prompt();
                } else {
                    popup.set_query(&query);
    ...
    fn sync_mention_popup(&mut self, query: String) {
        ...
        match &mut self.active_popup {
            ...
            _ => {
                let mut popup = SkillPopup:REDACTED_SECRETmentions);
                popup.set_query(&query);
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
        let mut mentions = Vec:REDACTED_SECRET);
        ...
        if let Some(plugins) = self.plugins.as_ref() {
            for plugin in plugins {
                ...
                let mut capability_labels = Vec:REDACTED_SECRET);
...
#[cfg(not(target_os = "linux"))]
impl ChatComposer {
    pub fn update_recording_meter_in_place(&mut self, id: REDACTED_SECRET, text: REDACTED_SECRET) -> bool {
        self.textarea.update_named_element_by_id(id, text)
    }

    pub fn insert_recording_meter_placeholder(&mut self, text: REDACTED_SECRET) -> String {
        let id = self.next_id();
        self.textarea.insert_named_element(text, id.clone());
        id
    }

    pub fn remove_recording_meter_placeholder(&mut self, id: REDACTED_SECRET) {
        let _ = self.textarea.replace_element_by_id(id, "");
    }
}

fn skill_display_name(skill: &SkillMetadata) -> REDACTED_SECRET {
    skill
        .interface
        .as_ref()
...
fn find_next_mention_token_range(text: REDACTED_SECRET, token: REDACTED_SECRET, from: usize) -> Option<Range<usize>> {
...
impl ChatComposer {
    ...
    fn render_textarea(
        ...
        if !remote_images_rect.is_empty() {
            Paragraph:REDACTED_SECRETself.remote_images_lines(remote_images_rect.width))
                .style(style)
        ...
        if textarea_is_empty {
            ...
            if !textarea_rect.is_empty() {
                if is_zellij {
                    ...
                    line.render_ref(textarea_rect.inner(Margin:REDACTED_SECRET0, 0)), buf);
                }
            }
        }
    }
...
mod tests {
    ...
    use crate::bottom_pane::AppEventREDACTED_SECRETer;
    ...
    fn footer_hint_row_is_separated_from_composer() {
        ...
        let sender = AppEventREDACTED_SECRETer:REDACTED_SECRETtx);
        let composer = ChatComposer:REDACTED_SECRET
        ...
        let area = Rect:REDACTED_SECRET0, 0, 40, 6);
        let mut buf = Buffer::empty(area);
        composer.render(area, &mut buf);

        let row_to_string = |y: u16| {
            let mut row = String:REDACTED_SECRET);
    ...
    fn footer_flash_overrides_footer_hint_override() {
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let sender = AppEventREDACTED_SECRETer:REDACTED_SECRETtx);
        let mut composer = ChatComposer:REDACTED_SECRET
            /*has_input_focus*/ true,
            sender,
            /*enhanced_keys_supported*/ false,
            "Ask Codex to do anything".to_string(),
        ...
        let area = Rect:REDACTED_SECRET0, 0, 60, 6);
        let mut buf = Buffer::empty(area);
        composer.render(area, &mut buf);

        let mut bottom_row = String:REDACTED_SECRET);
        for x in 0..area.width {
            bottom_row.push(
                buf[(x, area.height - 1)]
                    .symbol()
        ...
        F: FnOnce(&mut ChatComposer),
    {
        ...
        let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRETwidth, height)).unwrap();
        terminal
    ...
    fn snapshot_composer_state<F>(name: REDACTED_SECRET, enhanced_keys_supported: bool, setup: F)
    where
        F: FnOnce(&mut ChatComposer),
    {
    ...
    fn snapshot_zellij_composer_state<F>(name: REDACTED_SECRET, setup: F)
    where
        F: FnOnce(&mut ChatComposer),
    {
        use ratatui::Terminal;
        ...
        let footer_spacing = ChatComposer::footer_spacing(footer_lines);
        let height = footer_lines + footer_spacing + 8;
        let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET100, height)).unwrap();
        terminal
            .draw(|f| composer.render(f.area(), f.buffer_mut()))
            .unwrap();
        insta::assert_snapshot!(name, terminal.backend());
    ...
    fn footer_mode_snapshots() {
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                let _ = composer
                    .handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('?'), KeyModifiers::NONE));
            },
        );
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Esc, KeyModifiers::NONE));
            },
        );
    ...
    fn footer_collapse_snapshots() {
        ...
        snapshot_composer_state_with_width(
            ...
            |composer| {
                ...
                composer.set_task_running(/*running*/ true);
                composer.set_text_content("Test".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
            },
        );
        snapshot_composer_state_with_width(
    ...
    fn base_footer_mode_tracks_empty_state_after_quit_hint_expires() {
        ...
        composer.set_text_content(String:REDACTED_SECRET), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        assert_eq!(composer.footer_mode(), FooterMode::ComposerEmpty);
    }
    ...
    fn clear_for_ctrl_c_records_cleared_draft() {
        ...
        composer.set_text_content("draft text".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        assert_eq!(composer.clear_for_ctrl_c(), Some("draft text".to_string()));
        assert!(composer.is_empty());

        assert_eq!(
            composer.history.navigate_up(&composer.app_event_tx),
            Some(HistoryEntry:REDACTED_SECRET"draft text".to_string()))
        );
    }
    ...
    fn clear_for_ctrl_c_preserves_pending_paste_history_entry() {
        ...
        let text_elements = vec![TextElement:REDACTED_SECRET
        ...
        let (result, _needs_redraw) =
            composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
        match result {
            InputResult::Submitted {
                text,
                text_elements,
    ...
    fn apply_history_entry_preserves_local_placeholders_after_remote_prefix() {
        ...
        composer.apply_history_entry(HistoryEntry::with_pending_and_remote(
            ...
            vec![TextElement:REDACTED_SECRET
                (0.."[Image #2]".len()).into(),
                Some("[Image #2]".to_string()),
            )],
    ...
    fn question_mark_only_toggles_on_first_char() {
        ...
        let (result, needs_redraw) =
            composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('?'), KeyModifiers::NONE));
        ...
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('?'), KeyModifiers::NONE));
        assert_eq!(composer.footer_mode, FooterMode::ComposerEmpty);
    ...
    fn question_mark_does_not_toggle_during_paste_burst() {
        ...
        composer
            .paste_burst
            .begin_with_retro_grabbed(String:REDACTED_SECRET), Instant::now());

        for ch in ['h', 'i', '?', 't', 'h', 'e', 'r', 'e'] {
            let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char(ch), KeyModifiers::NONE));
        }
    ...
    fn set_connector_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_text_content("$".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        assert!(matches!(composer.active_popup, ActivePopup::None));
        ...
        let connectors = vec![AppInfo {
            ...
            is_accessible: true,
            is_enabled: true,
            plugin_display_names: Vec:REDACTED_SECRET),
        }];
    ...
    fn set_plugin_mentions_refreshes_open_mention_popup() {
        ...
        composer.set_plugin_mentions(Some(vec![PluginCapabilitySummary {
            ...
            app_connector_ids: Vec:REDACTED_SECRET),
        }]));
    ...
    fn mention_items_show_plugin_owned_skill_and_app_duplicates() {
        ...
        composer.set_connectors_enabled(/*enabled*/ true);
        composer.set_text_content("$goog".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    ...
    fn plugin_mention_popup_snapshot() {
        snapshot_composer_state(
            ...
            /*enhanced_keys_supported*/ false,
            |composer| {
                composer.set_text_content("$sa".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
                composer.set_plugin_mentions(Some(vec![PluginCapabilitySummary {
                    config_name: "sample@test".to_string(),
                    display_name: "Sample Plugin".to_string(),
                    description: Some(
    ...
    fn test_current_at_token_basic_cases() {
        ...
        for (input, cursor_pos, expected, description) in test_cases {
            let mut textarea = TextArea:REDACTED_SECRET);
    ...
    fn ascii_prefix_survives_non_ascii_followup() {
        ...
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('1'), KeyModifiers::NONE));
        assert!(composer.is_in_paste_burst());

        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('あ'), KeyModifiers::NONE));
    ...
    fn non_ascii_burst_buffers_enter_and_flushes_multiline() {
        ...
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('你'), KeyModifiers::NONE));
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('好'), KeyModifiers::NONE));
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('h'), KeyModifiers::NONE));
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('i'), KeyModifiers::NONE));
    ...
    fn non_ascii_burst_buffers_large_multiline_mixed_ascii_and_unicode() {
        ...
        const LARGE_MIXED_PAYLOAD: REDACTED_SECRET = "天地玄黄 宇宙洪荒\n\
日月盈昃 辰宿列张\n\
寒来暑往 秋收冬藏\n\
\n\
你好世界 编码测试\n\
...
程序员 与 Unicode 同行";
        ...
        for ch in LARGE_MIXED_PAYLOAD.chars() {
            ...
            let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETcode, KeyModifiers::NONE));
        }
    ...
    fn ascii_burst_treats_enter_as_newline() {
        ...
        let _ = composer.handle_input_basic_with_time(
            KeyEvent:REDACTED_SECRETKeyCode::Char('h'), KeyModifiers::NONE),
            now,
        );
        now += step;
        let _ = composer.handle_input_basic_with_time(
            KeyEvent:REDACTED_SECRETKeyCode::Char('i'), KeyModifiers::NONE),
            now,
        );
        now += step;
        ...
        for ch in ['t', 'h', 'e', 'r', 'e'] {
            now += step;
            let _ = composer.handle_input_basic_with_time(
                KeyEvent:REDACTED_SECRETKeyCode::Char(ch), KeyModifiers::NONE),
                now,
            );
        }
    ...
    fn non_char_key_flushes_active_burst_before_input() {
        ...
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Left, KeyModifiers::NONE));
        assert_eq!(composer.textarea.text(), "hi");
    ...
    fn disable_paste_burst_flushes_pending_first_char_and_inserts_immediately() {
        ...
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('a'), KeyModifiers::NONE));
        assert!(composer.is_in_paste_burst());
        assert!(composer.textarea.text().is_empty());
        ...
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('b'), KeyModifiers::NONE));
    ...
    fn edit_clears_pending_paste() {
        ...
        composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Backspace, KeyModifiers::NONE));
        assert!(composer.pending_pastes.is_empty());
    }
    ...
    fn ui_snapshots() {
        ...
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let sender = AppEventREDACTED_SECRETer:REDACTED_SECRETtx);
        let mut terminal = match Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET100, 10)) {
            Ok(t) => t,
    ...
    fn remote_image_rows_snapshots() {
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                composer.set_text_content("describe these".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
            },
        );
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::NONE));
            },
        );
        ...
        snapshot_composer_state(
            ...
            |composer| {
                ...
                let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::NONE));
                let _ =
                    composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Delete, KeyModifiers::NONE));
            },
        );
    }
    ...
    fn slash_popup_model_first_for_mo_ui() {
        ...
        let mut terminal = match Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET60, 5)) {
            Ok(t) => t,
            Err(e) => panic!("Failed to create terminal: {e}"),
        };
        terminal
    ...
    fn slash_popup_resume_for_res_ui() {
        ...
        let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET60, 6)).expect("terminal");
        terminal
            .draw(|f| composer.render(f.area(), f.buffer_mut()))
    ...
    fn kill_buffer_persists_after_submit() {
        ...
        let (_result, _needs_redraw) =
            composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('k'), KeyModifiers::CONTROL));
        ...
        let (_result, _needs_redraw) =
            composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('y'), KeyModifiers::CONTROL));
    ...
    fn slash_tab_completion_moves_cursor_to_end() {
        ...
        let (_result, _needs_redraw) =
            composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Tab, KeyModifiers::NONE));
    ...
    fn file_completion_preserves_large_paste_placeholder_elements() {
        ...
        composer.on_file_search_result(
            ...
            vec![FileMatch {
                ...
                REDACTED_SECRET: PathBuf::from("/tmp"),
                indices: None,
            }],
        );
    ...
    fn test_multiple_pastes_submission() {
        ...
        let mut expected_text = String:REDACTED_SECRET);
    ...
    fn history_navigation_leaves_cursor_at_end_of_line() {
        ...
        let (_result, _needs_redraw) =
            composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::NONE));
        assert_eq!(composer.textarea.text(), "second");
        assert_eq!(composer.textarea.cursor(), composer.textarea.text().len());
        ...
        let (_result, _needs_redraw) =
            composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Down, KeyModifiers::NONE));
        assert_eq!(composer.textarea.text(), "second");
        assert_eq!(composer.textarea.cursor(), composer.textarea.text().len());
    ...
    fn burst_paste_fast_small_buffers_and_flushes_on_stop() {
        ...
        for _ in 0..count {
            let _ = composer.handle_input_basic_with_time(
                KeyEvent:REDACTED_SECRETKeyCode::Char('a'), KeyModifiers::NONE),
                now,
            );
            assert!(
    ...
    fn burst_paste_fast_large_inserts_placeholder_on_flush() {
        ...
        for _ in 0..count {
            let _ = composer.handle_input_basic_with_time(
                KeyEvent:REDACTED_SECRETKeyCode::Char('x'), KeyModifiers::NONE),
                now,
            );
            now += step;
        }
    ...
    fn slash_popup_activated_for_bare_slash_and_valid_prefixes() {
        ...
        composer.set_text_content("/re".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        assert!(
            matches!(composer.active_popup, ActivePopup::Command(_)),
        ...
        composer.set_text_content("/ac".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        assert!(
            matches!(composer.active_popup, ActivePopup::Command(_)),
        ...
        composer.set_text_content("/zzz".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        assert!(
            matches!(composer.active_popup, ActivePopup::None),
    ...
    fn remote_images_do_not_modify_textarea_text_or_elements() {
        ...
        assert_eq!(composer.current_text(), "");
        assert_eq!(composer.text_elements(), Vec::<TextElement>:REDACTED_SECRET));
    }
    ...
    fn prepare_submission_keeps_remote_offset_local_placeholder_numbering() {
        ...
        let base_elements = vec![TextElement:REDACTED_SECRET
            (0.."[Image #2]".len()).into(),
            Some("[Image #2]".to_string()),
        )];
    ...
    fn delete_selected_remote_image_relabels_local_placeholders() {
        ...
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::NONE));
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Delete, KeyModifiers::NONE));
        assert_eq!(
        ...
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::NONE));
        let _ = composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Delete, KeyModifiers::NONE));
        assert_eq!(composer.remote_image_urls(), Vec::<String>:REDACTED_SECRET));
    ...
    fn input_disabled_ignores_keypresses_and_hides_cursor() {
        ...
        composer.set_text_content("hello".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        ...
        let (result, needs_redraw) =
            composer.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('x'), KeyModifiers::NONE));
