...
const BASE_CLI_BUG_ISSUE_URL: REDACTED_SECRET =
...
const CODEX_FEEDBACK_INTERNAL_URL: REDACTED_SECRET = "http://go/codex-feedback-internal";
...
impl Renderable for FeedbackNoteView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        for (offset, line) in intro_lines.iter().enumerate() {
            Paragraph:REDACTED_SECRETline.clone()).render(
                Rect {
                    x: area.x,
        ...
        if input_area.width >= 2 {
            for row in 0..input_area.height {
                Paragraph:REDACTED_SECRETLine::from(vec![gutter()])).render(
                    Rect {
                        x: input_area.x,
            ...
            if text_area_height > 0 {
                ...
                if self.textarea.text().is_empty() {
                    Paragraph:REDACTED_SECRETLine::from(placeholder.dim())).render(textarea_rect, buf);
                }
            }
        }
        ...
        if hint_y < area.y.saturating_add(area.height) {
            Paragraph:REDACTED_SECRETstandard_popup_hint_line()).render(
                Rect {
                    x: area.x,
                    y: hint_y,
                    width: area.width,
...
pub(crate) fn feedback_success_cell(
    ...
    history_cell::PlainHistoryCell:REDACTED_SECRETlines)
}
...
fn slack_feedback_url(_thread_id: REDACTED_SECRET) -> String {
...
pub(crate) fn feedback_disabled_params() -> super::SelectionViewParams {
    super::SelectionViewParams {
        title: Some("REDACTED_SECRETing feedback is disabled".to_string()),
...
fn make_feedback_item(
    ...
    let action: super::SelectionAction = Box:REDACTED_SECRETmove |_sender: &AppEventREDACTED_SECRETer| {
...
pub(crate) fn feedback_upload_consent_params(
    ...
    use super::popup_consts::standard_popup_hint_line;
    let yes_action: super::SelectionAction = Box:REDACTED_SECRET{
        let tx = app_event_tx.clone();
        move |sender: &AppEventREDACTED_SECRETer| {
            let _ = sender;
    ...
    let no_action: super::SelectionAction = Box:REDACTED_SECRET{
        let tx = app_event_tx;
        move |sender: &AppEventREDACTED_SECRETer| {
            let _ = sender;
            tx.send(AppEvent::OpenFeedbackNote {
    ...
    super::SelectionViewParams {
        ...
        header: Box:REDACTED_SECRETcrate::render::renderable::ColumnRenderable::with(
            header_lines,
        )),
        ..Default::default()
    }
...
mod tests {
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        FeedbackNoteView:REDACTED_SECRETcategory, tx, /*include_logs*/ true)
    }
    ...
    fn feedback_view_with_connectivity_diagnostics() {
        let (tx_raw, _rx) = tokio::sync::mpsc::unbounded_channel::<AppEvent>();
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let view = FeedbackNoteView:REDACTED_SECRETFeedbackCategory::Bug, tx, /*include_logs*/ false);
        let rendered = render(&view, /*width*/ 60);
    ...
    fn submit_feedback_emits_submit_event_with_trimmed_note() {
        ...
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let mut view = FeedbackNoteView:REDACTED_SECRETFeedbackCategory::Bug, tx, /*include_logs*/ true);
    ...
    fn submit_feedback_omits_empty_note() {
        let (tx_raw, mut rx) = tokio::sync::mpsc::unbounded_channel::<AppEvent>();
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let mut view = FeedbackNoteView:REDACTED_SECRET
            FeedbackCategory::GoodResult,
            tx,
            /*include_logs*/ false,
        );
    ...
    fn should_show_feedback_connectivity_details_only_for_non_good_result_with_diagnostics() {
        let diagnostics = FeedbackDiagnostics:REDACTED_SECRETvec![FeedbackDiagnostic {
