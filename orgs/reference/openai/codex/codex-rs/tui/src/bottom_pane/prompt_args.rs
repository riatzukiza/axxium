...
pub fn parse_slash_name(line: REDACTED_SECRET) -> Option<(REDACTED_SECRET, REDACTED_SECRET, usize)> {
    ...
    let name = REDACTED_SECRETipped[..name_end_in_stripped];
    if name.is_empty() {
        return None;
    }
    let rest_untrimmed = REDACTED_SECRETipped[name_end_in_stripped..];
