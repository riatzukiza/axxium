...
impl Renderable for UnifiedExecFooter {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph:REDACTED_SECRETself.render_lines(area.width)).render(area, buf);
    }
...
mod tests {
    ...
    fn desired_height_empty() {
        let footer = UnifiedExecFooter:REDACTED_SECRET);
        assert_eq!(footer.desired_height(/*width*/ 40), 0);
    }

    #[test]
    fn render_more_sessions() {
        let mut footer = UnifiedExecFooter:REDACTED_SECRET);
        ...
        footer.render(Rect:REDACTED_SECRET0, 0, width, height), &mut buf);
