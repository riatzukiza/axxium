...
const ANSWER_PLACEHOLDER: REDACTED_SECRET = "Type your answer";
const OPTIONAL_ANSWER_PLACEHOLDER: REDACTED_SECRET = "Type your answer (optional)";
const FOOTER_SEPARATOR: REDACTED_SECRET = " | ";
const MIN_COMPOSER_HEIGHT: u16 = 3;
const MIN_OVERLAY_HEIGHT: u16 = 8;
const APPROVAL_FIELD_ID: REDACTED_SECRET = "__approval";
const APPROVAL_ACCEPT_ONCE_VALUE: REDACTED_SECRET = "accept";
const APPROVAL_ACCEPT_SESSION_VALUE: REDACTED_SECRET = "accept_session";
const APPROVAL_ACCEPT_ALWAYS_VALUE: REDACTED_SECRET = "accept_always";
const APPROVAL_DECLINE_VALUE: REDACTED_SECRET = "decline";
const APPROVAL_CANCEL_VALUE: REDACTED_SECRET = "cancel";
const APPROVAL_META_KIND_KEY: REDACTED_SECRET = "codex_approval_kind";
const APPROVAL_META_KIND_MCP_TOOL_CALL: REDACTED_SECRET = "mcp_tool_call";
const APPROVAL_META_KIND_TOOL_SUGGESTION: REDACTED_SECRET = "tool_suggestion";
const APPROVAL_PERSIST_KEY: REDACTED_SECRET = "persist";
const APPROVAL_PERSIST_SESSION_VALUE: REDACTED_SECRET = "session";
const APPROVAL_PERSIST_ALWAYS_VALUE: REDACTED_SECRET = "always";
const APPROVAL_TOOL_PARAMS_KEY: REDACTED_SECRET = "tool_params";
const APPROVAL_TOOL_PARAMS_DISPLAY_KEY: REDACTED_SECRET = "tool_params_display";
const APPROVAL_TOOL_PARAM_DISPLAY_LIMIT: usize = 3;
const APPROVAL_TOOL_PARAM_VALUE_TRUNCATE_GRAPHEMES: usize = 60;
const TOOL_TYPE_KEY: REDACTED_SECRET = "tool_type";
const TOOL_ID_KEY: REDACTED_SECRET = "tool_id";
const TOOL_NAME_KEY: REDACTED_SECRET = "tool_name";
const TOOL_SUGGEST_SUGGEST_TYPE_KEY: REDACTED_SECRET = "suggest_type";
const TOOL_SUGGEST_REASON_KEY: REDACTED_SECRET = "suggest_reason";
const TOOL_SUGGEST_INSTALL_URL_KEY: REDACTED_SECRET = "install_url";
...
impl McpServerElicitationFormRequest {
    ...
    fn from_parts(
        ...
        {
            (McpServerElicitationResponseMode::FormContent, Vec:REDACTED_SECRET))
            ...
            (
                ...
                vec![McpServerElicitationField {
                    ...
                    label: String:REDACTED_SECRET),
                    prompt: String:REDACTED_SECRET),
                    required: true,
    ...
    pub(crate) fn server_name(&self) -> REDACTED_SECRET {
...
fn tool_approval_supports_persist_mode(meta: Option<&Value>, expected_mode: REDACTED_SECRET) -> bool {
...
fn format_tool_approval_display_message(
    ...
    let mut sections = Vec:REDACTED_SECRET);
    if !message.is_empty() {
...
fn parse_fields_from_schema(requested_schema: &Value) -> Option<Vec<McpServerElicitationField>> {
    ...
    let mut fields = Vec:REDACTED_SECRET);
...
impl McpServerElicitationOverlay {
    ...
    fn reset_for_request(&mut self) {
        self.answers = self
            ...
            .iter()
            .map(|field| {
                let mut selection = ScrollState:REDACTED_SECRET);
    ...
    fn footer_tips(&self) -> Vec<FooterTip> {
        ...
        if self.current_field_is_select() {
            if self.field_count() == 1 {
                ...
                tips.push(FooterTip:REDACTED_SECRET"enter to submit answer"));
            }
        ...
        if self.field_count() > 1 {
            if self.current_field_is_select() {
                tips.push(FooterTip:REDACTED_SECRET"←/→ to navigate fields"));
            } else {
                tips.push(FooterTip:REDACTED_SECRET"ctrl + p / ctrl + n change field"));
            }
        }
        tips.push(FooterTip:REDACTED_SECRET"esc to cancel"));
        tips
    }
    ...
    fn render_prompt(&self, area: Rect, buf: &mut Buffer) {
        ...
        for (offset, line) in self.wrapped_prompt_lines(area.width).iter().enumerate() {
            ...
            Paragraph:REDACTED_SECRETline).render(
                Rect {
                    x: area.x,
                    y,
                    width: area.width,
    ...
    fn render_footer(&self, area: Rect, input_area_height: u16, buf: &mut Buffer) {
        ...
        let option_tip = if options_hidden {
            ...
            Some(FooterTip:REDACTED_SECRETformat!("option {selected}/{total}")))
        } else {
            None
        };
        ...
        if let Some(prefix) = option_tip {
            ...
            if let Some(first_line) = tip_lines.first_mut() {
                let mut first = Vec:REDACTED_SECRET);
        ...
        for (row_idx, tips) in tip_lines.into_iter().take(area.height as usize).enumerate() {
            let mut spans = Vec:REDACTED_SECRET);
...
impl Renderable for McpServerElicitationOverlay {
    fn desired_height(&self, width: u16) -> u16 {
        let outer = Rect:REDACTED_SECRET0, 0, width, u16::MAX);
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph:REDACTED_SECRETprogress_line).render(progress_area, buf);
...
fn wrap_footer_tips(width: u16, tips: Vec<FooterTip>) -> Vec<Vec<FooterTip>> {
    ...
    let mut current = Vec:REDACTED_SECRET);
    let mut used = 0usize;
...
mod tests {
    ...
    fn test_sender() -> (AppEventREDACTED_SECRETer, UnboundedReceiver<AppEvent>) {
    ...
    fn tool_approval_meta(
        persist_modes: &[REDACTED_SECRET],
        tool_params: Option<Value>,
        tool_params_display: Option<Vec<(REDACTED_SECRET, Value, REDACTED_SECRET)>>,
    ) -> Option<Value> {
    ...
    fn parses_boolean_form_request() {
        ...
        assert_eq!(
            ...
            McpServerElicitationFormRequest {
                ...
                approval_display_params: Vec:REDACTED_SECRET),
    ...
    fn submit_sends_accept_with_typed_content() {
        ...
        .expect("expected supported form");
        let mut overlay = McpServerElicitationOverlay:REDACTED_SECRET
    ...
    fn boolean_form_snapshot() {
        ...
        .expect("expected supported form");
        let overlay = McpServerElicitationOverlay:REDACTED_SECRET
            request, tx, /*has_input_focus*/ true, /*enhanced_keys_supported*/ false,
            /*disable_paste_burst*/ false,
        );
        ...
        insta::assert_snapshot!(
            ...
            render_snapshot(&overlay, Rect:REDACTED_SECRET0, 0, 120, 16))
        );
    }
