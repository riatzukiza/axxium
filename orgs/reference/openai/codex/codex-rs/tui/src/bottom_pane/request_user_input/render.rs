...
impl RequestUserInputOverlay {
    ...
    pub(super) fn render_ui(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph:REDACTED_SECRETprogress_line).render(sections.progress_area, buf);
        ...
        for (offset, line) in sections.question_lines.iter().enumerate() {
            ...
            Paragraph:REDACTED_SECRETquestion_line).render(
                Rect {
        ...
        let option_tip = if options_hidden {
            ...
            Some(super::FooterTip:REDACTED_SECRETformat!("option {selected}/{total}")))
        } else {
            None
        };
            ...
            .enumerate()
        {
            ...
            Paragraph:REDACTED_SECRETline).render(row_area, buf);
        }
    }
...
fn render_rows_bottom_aligned(
    ...
    state: &ScrollState,
    max_results: usize,
    empty_message: REDACTED_SECRET,
) {
    if area.width == 0 || area.height == 0 {
        return;
    }

    let scratch_area = Rect:REDACTED_SECRET0, 0, area.width, area.height);
...
fn truncate_line_word_boundary_with_ellipsis(
    ...
    if max_width == 0 {
        return Line::from(Vec::<Span<'static>>:REDACTED_SECRET));
    }
    ...
    let mut spans_out: Vec<Span<'static>> = Vec:REDACTED_SECRET);
