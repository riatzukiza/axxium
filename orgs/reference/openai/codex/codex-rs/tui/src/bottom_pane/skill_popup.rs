...
impl SkillPopup {
    ...
    pub(crate) fn set_query(&mut self, query: REDACTED_SECRET) {
    ...
    fn rows_from_matches(
        ...
        matches
            ...
            .map(|(idx, indices, _score)| {
                ...
                GenericDisplayRow {
                    name,
                    name_prefix_spans: Vec:REDACTED_SECRET),
    ...
    fn filtered(&self) -> Vec<(usize, Option<Vec<usize>>, i32)> {
        ...
        let mut out: Vec<(usize, Option<Vec<usize>>, i32)> = Vec:REDACTED_SECRET);
...
mod tests {
    ...
    fn ranked_mention_item(
        ...
        search_terms: &[REDACTED_SECRET],
        category_tag: REDACTED_SECRET,
        sort_rank: u8,
    ) -> MentionItem {
        MentionItem {
    ...
    fn named_mention_item(display_name: REDACTED_SECRET, search_terms: &[REDACTED_SECRET]) -> MentionItem {
        ranked_mention_item(display_name, search_terms, "[Skill]", /*sort_rank*/ 1)
    }

    fn plugin_mention_item(display_name: REDACTED_SECRET, search_terms: &[REDACTED_SECRET]) -> MentionItem {
    ...
    fn filtered_mentions_preserve_results_beyond_popup_height() {
        let popup = SkillPopup:REDACTED_SECRET(0..(MAX_POPUP_ROWS + 2)).map(mention_item).collect());
    ...
    fn render_popup(popup: &SkillPopup, width: u16) -> String {
        let area = Rect:REDACTED_SECRET0, 0, width, popup.calculate_required_height(width));
    ...
    fn scrolling_mentions_shifts_rendered_window_snapshot() {
        let mut popup = SkillPopup:REDACTED_SECRET(0..(MAX_POPUP_ROWS + 2)).map(mention_item).collect());
    ...
    fn display_name_match_sorting_beats_worse_secondary_search_term_matches() {
        let mut popup = SkillPopup:REDACTED_SECRETvec![
