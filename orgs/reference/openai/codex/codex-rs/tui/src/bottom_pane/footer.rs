...
const MODE_CYCLE_HINT: REDACTED_SECRET = "shift+tab to cycle";
...
pub(crate) fn render_footer_line(area: Rect, buf: &mut Buffer, line: Line<'static>) {
    Paragraph:REDACTED_SECRETprefix_lines(
        vec![line],
...
mod tests {
    ...
    fn snapshot_footer(name: REDACTED_SECRET, props: FooterProps) {
    ...
    fn draw_footer_frame<B: Backend>(
        ...
        terminal
            .draw(|f| {
                let area = Rect:REDACTED_SECRET0, 0, f.area().width, height);
    ...
    fn render_footer_with_mode_indicator(
        ...
        let height = footer_height(props).max(1);
        let mut terminal = Terminal:REDACTED_SECRETVT100Backend:REDACTED_SECRETwidth, height)).expect("terminal");
        draw_footer_frame(&mut terminal, height, props, collaboration_mode_indicator);
