...
impl FileSearchPopup {
    pub(crate) fn new() -> Self {
        Self {
            display_query: String:REDACTED_SECRET),
            pending_query: String:REDACTED_SECRET),
            waiting: true,
            matches: Vec:REDACTED_SECRET),
    ...
    pub(crate) fn set_matches(&mut self, query: REDACTED_SECRET, matches: Vec<FileMatch>) {
        if query != self.pending_query {
            return; // stale
        }
...
mod tests {
    ...
    fn file_match(index: usize) -> FileMatch {
        FileMatch {
            ...
            REDACTED_SECRET: PathBuf::from("/tmp/repo"),
            indices: None,
        }
    }
