...
impl CommandPopup {
    pub(crate) fn new(flags: CommandPopupFlags) -> Self {
        ...
        let builtins: Vec<(&'static str, SlashCommand)> =
            slash_commands::builtins_for_input(flags.into())
                ...
                .collect();
        Self {
            command_filter: String:REDACTED_SECRET),
            builtins,
    ...
    fn filtered(&self) -> Vec<(CommandItem, Option<Vec<usize>>)> {
        ...
        let mut out: Vec<(CommandItem, Option<Vec<usize>>)> = Vec:REDACTED_SECRET);
        if filter.is_empty() {
        ...
        let mut exact: Vec<(CommandItem, Option<Vec<usize>>)> = Vec:REDACTED_SECRET);
        let mut prefix: Vec<(CommandItem, Option<Vec<usize>>)> = Vec:REDACTED_SECRET);
        let indices_for = |offset| Some((offset..offset + filter_chars).collect());

        let mut push_match =
            |item: CommandItem, display: REDACTED_SECRET, name: Option<REDACTED_SECRET>, name_offset: usize| {
...
mod tests {
    ...
    fn filter_includes_init_when_typing_prefix() {
        let mut popup = CommandPopup:REDACTED_SECRETCommandPopupFlags::default());
    ...
    fn filtered_commands_keep_presentation_order_for_prefix() {
        ...
        let cmds: Vec<REDACTED_SECRET> = popup
            .filtered_items()
            .into_iter()
            .map(|item| match item {
    ...
    fn collab_command_visible_when_collaboration_modes_enabled() {
        let mut popup = CommandPopup:REDACTED_SECRETCommandPopupFlags {
    ...
    fn debug_commands_are_hidden_from_popup() {
        let popup = CommandPopup:REDACTED_SECRETCommandPopupFlags::default());
        let cmds: Vec<REDACTED_SECRET> = popup
            .filtered_items()
            .into_iter()
            .map(|item| match item {
