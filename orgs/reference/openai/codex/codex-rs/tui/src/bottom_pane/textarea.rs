...
const WORD_SEPARATORS: REDACTED_SECRET = "`~!@#$%^&*()-=+[{]}\\|;:'\",.<>/?";
...
impl TextArea {
    pub fn new() -> Self {
        Self {
            ...
            cursor_pos: 0,
            wrap_cache: RefCell:REDACTED_SECRETNone),
            preferred_col: None,
            elements: Vec:REDACTED_SECRET),
            next_element_id: 1,
            kill_buffer: String:REDACTED_SECRET),
        }
    }
    ...
    pub fn set_text_clearing_elements(&mut self, text: REDACTED_SECRET) {
    ...
    pub fn set_text_with_elements(&mut self, text: REDACTED_SECRET, elements: &[UserTextElement]) {
        self.set_text_inner(text, Some(elements));
    }

    fn set_text_inner(&mut self, text: REDACTED_SECRET, elements: Option<&[UserTextElement]>) {
    ...
    pub fn text(&self) -> REDACTED_SECRET {
        &self.text
    }

    pub fn insert_str(&mut self, text: REDACTED_SECRET) {
        self.insert_str_at(self.cursor_pos, text);
    }

    pub fn insert_str_at(&mut self, pos: usize, text: REDACTED_SECRET) {
    ...
    pub fn replace_range(&mut self, range: std::ops::Range<usize>, text: REDACTED_SECRET) {
        let range = self.expand_range_to_element_boundaries(range);
        self.replace_range_raw(range, text);
    }

    fn replace_range_raw(&mut self, range: std::ops::Range<usize>, text: REDACTED_SECRET) {
    ...
    pub fn text_elements(&self) -> Vec<UserTextElement> {
        self.elements
            ...
            .map(|e| {
                ...
                UserTextElement:REDACTED_SECRET
                    ByteRange {
                        start: e.range.start,
                        end: e.range.end,
                    },
    ...
    pub fn replace_element_payload(&mut self, old: REDACTED_SECRET, new: REDACTED_SECRET) -> bool {
        let Some(idx) = self
            .elements
            .iter()
    ...
    pub fn insert_element(&mut self, text: REDACTED_SECRET) -> u64 {
    ...
    pub fn insert_named_element(&mut self, text: REDACTED_SECRET, id: String) {
        let start = self.clamp_pos_for_insertion(self.cursor_pos);
        self.insert_str_at(start, text);
        let end = start + text.len();
    ...
    #[cfg(not(target_os = "linux"))]
    pub fn replace_element_by_id(&mut self, id: REDACTED_SECRET, text: REDACTED_SECRET) -> bool {
    ...
    #[allow(dead_code)]
    pub fn update_named_element_by_id(&mut self, id: REDACTED_SECRET, text: REDACTED_SECRET) -> bool {
    ...
    #[allow(dead_code)]
    pub fn named_element_range(&self, id: REDACTED_SECRET) -> Option<std::ops::Range<usize>> {
        self.elements
            .iter()
    ...
    fn prev_atomic_boundary(&self, pos: usize) -> usize {
        ...
        let mut gc = unicode_segmentation::GraphemeCursor:REDACTED_SECRETpos, self.text.len(), false);
        ...
        // Ensure cache is ready (potentially mutably borrow, then drop)
        {
            ...
            if needs_recalc {
                let lines = crate::wrapping::wrap_ranges(
                    &self.text,
                    Options:REDACTED_SECRETwidth as usize).wrap_algorithm(textwrap::WrapAlgorithm::FirstFit),
                );
...
impl TextArea {
    ...
    fn render_lines(
        ...
        for (row, idx) in range.enumerate() {
            ...
            buf.set_style(Rect:REDACTED_SECRETarea.x, y, area.width, 1), base_style);
...
mod tests {
    ...
    fn ta_with(text: REDACTED_SECRET) -> TextArea {
        let mut t = TextArea:REDACTED_SECRET);
        t.insert_str(text);
        t
    }
    ...
    fn control_b_and_f_move_cursor() {
        ...
        t.input(KeyEvent:REDACTED_SECRETKeyCode::Char('f'), KeyModifiers::CONTROL));
        assert_eq!(t.cursor(), 2);

        t.input(KeyEvent:REDACTED_SECRETKeyCode::Char('b'), KeyModifiers::CONTROL));
        assert_eq!(t.cursor(), 1);
    }
    ...
    fn control_b_f_fallback_control_chars_move_cursor() {
        ...
        t.input(KeyEvent:REDACTED_SECRETKeyCode::Char('\u{0002}'), KeyModifiers::NONE));
        assert_eq!(t.cursor(), 1);

        // ^F (U+0006) should move right
        t.input(KeyEvent:REDACTED_SECRETKeyCode::Char('\u{0006}'), KeyModifiers::NONE));
        assert_eq!(t.cursor(), 2);
    }
    ...
    fn delete_backward_word_alt_keys() {
        ...
        t.input(KeyEvent:REDACTED_SECRET
            KeyCode::Char('h'),
        ...
        let mut t = ta_with("hello world");
        t.set_cursor(t.text().len()); // cursor at the end
        t.input(KeyEvent:REDACTED_SECRETKeyCode::Backspace, KeyModifiers::ALT));
        assert_eq!(t.text(), "hello ");
        assert_eq!(t.cursor(), 6);
    }
    ...
    fn delete_forward_word_with_without_alt_modifier() {
        let mut t = ta_with("hello world");
        t.set_cursor(/*pos*/ 0);
        t.input(KeyEvent:REDACTED_SECRETKeyCode::Delete, KeyModifiers::ALT));
        assert_eq!(t.text(), " world");
        assert_eq!(t.cursor(), 0);

        let mut t = ta_with("hello");
        t.set_cursor(/*pos*/ 0);
        t.input(KeyEvent:REDACTED_SECRETKeyCode::Delete, KeyModifiers::NONE));
    ...
    fn delete_forward_word_alt_d() {
        let mut t = ta_with("hello world");
        t.set_cursor(/*pos*/ 6);
        t.input(KeyEvent:REDACTED_SECRETKeyCode::Char('d'), KeyModifiers::ALT));
    ...
    fn control_h_backspace() {
        ...
        t.input(KeyEvent:REDACTED_SECRETKeyCode::Char('h'), KeyModifiers::CONTROL));
    ...
    fn wrapping_and_cursor_positions() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 6, 10); // width 6 -> wraps words
        ...
        let small_area = Rect:REDACTED_SECRET0, 0, 6, 1);
    ...
    fn cursor_pos_with_state_basic_and_scroll_behaviors() {
        ...
        let mut t = ta_with("hello world");
        t.set_cursor(/*pos*/ 3);
        let area = Rect:REDACTED_SECRET2, 5, 20, 3);
        ...
        let small_area = Rect:REDACTED_SECRET0, 0, wrap_width, 2);
        ...
        t.set_cursor(/*pos*/ 1);
        let area = Rect:REDACTED_SECRET0, 0, wrap_width, 3);
    ...
    fn cursor_pos_with_state_after_movements() {
        ...
        let _ = t.desired_height(/*width*/ 4);
        let area = Rect:REDACTED_SECRET0, 0, 4, 2);
        let mut state = TextAreaState::default();
    ...
    fn fuzz_textarea_randomized() {
        ...
        for _case in 0..500 {
            let mut ta = TextArea:REDACTED_SECRET);
            let mut state = TextAreaState::default();
            ...
            for _step in 0..60 {
                    ...
                    15 => {
                            ...
                            && let Some(start) = ta.text().find(&payload)
                        {
                            ...
                            if s < e {
                                ...
                                let mut srep = String:REDACTED_SECRET);
                ...
                let full_area = Rect:REDACTED_SECRET0, 0, width, total_lines.max(1));
