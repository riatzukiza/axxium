...
const NOTES_PLACEHOLDER: REDACTED_SECRET = "Add notes";
const ANSWER_PLACEHOLDER: REDACTED_SECRET = "Type your answer (optional)";
// Keep in sync with ChatComposer's minimum composer height.
const MIN_COMPOSER_HEIGHT: u16 = 3;
const SELECT_OPTION_PLACEHOLDER: REDACTED_SECRET = "Select an option to add notes";
pub(super) const TIP_SEPARATOR: REDACTED_SECRET = " | ";
pub(super) const DESIRED_SPACERS_BETWEEN_SECTIONS: u16 = 2;
const OTHER_OPTION_LABEL: REDACTED_SECRET = "None of the above";
const OTHER_OPTION_DESCRIPTION: REDACTED_SECRET = "Optionally, add details in notes (tab).";
const UNANSWERED_CONFIRM_TITLE: REDACTED_SECRET = "Submit with unanswered questions?";
const UNANSWERED_CONFIRM_GO_BACK: REDACTED_SECRET = "Go back";
const UNANSWERED_CONFIRM_GO_BACK_DESC: REDACTED_SECRET = "Return to the first unanswered question.";
const UNANSWERED_CONFIRM_SUBMIT: REDACTED_SECRET = "Proceed";
const UNANSWERED_CONFIRM_SUBMIT_DESC_SINGULAR: REDACTED_SECRET = "question";
const UNANSWERED_CONFIRM_SUBMIT_DESC_PLURAL: REDACTED_SECRET = "questions";
...
impl RequestUserInputOverlay {
    pub(crate) fn new(
        ...
        composer.set_footer_hint_override(Some(Vec:REDACTED_SECRET)));
        let mut overlay = Self {
            app_event_tx,
            request,
            queue: VecDeque:REDACTED_SECRET),
            composer,
            answers: Vec:REDACTED_SECRET),
            current_idx: 0,
            focus: Focus::Options,
            done: false,
    ...
    fn restore_current_draft(&mut self) {
        ...
        self.composer.set_footer_hint_override(Some(Vec:REDACTED_SECRET)));
        let Some(answer) = self.current_answer() else {
            self.composer
                .set_text_content(String:REDACTED_SECRET), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    ...
    fn footer_tips(&self) -> Vec<FooterTip> {
        let mut tips = Vec:REDACTED_SECRET);
        ...
        if self.has_options() {
            ...
            if self.selected_option_index().is_some() && notes_visible {
                tips.push(FooterTip:REDACTED_SECRET"tab or esc to clear notes"));
            }
        }
        ...
        let enter_tip = if question_count == 1 {
            ...
            FooterTip:REDACTED_SECRET"enter to submit answer")
        };
        tips.push(enter_tip);
        if question_count > 1 {
            if self.has_options() && !self.focus_is_notes() {
                tips.push(FooterTip:REDACTED_SECRET"←/→ to navigate questions"));
            } else if !self.has_options() {
                tips.push(FooterTip:REDACTED_SECRET"ctrl + p / ctrl + n change question"));
            }
        }
        if !(self.has_options() && notes_visible) {
            tips.push(FooterTip:REDACTED_SECRET"esc to interrupt"));
        }
        tips
    }
    ...
    fn wrap_footer_tips(&self, width: u16, tips: Vec<FooterTip>) -> Vec<Vec<FooterTip>> {
        ...
        if tips.is_empty() {
            return vec![Vec:REDACTED_SECRET)];
        }

        let mut lines: Vec<Vec<FooterTip>> = Vec:REDACTED_SECRET);
        let mut current: Vec<FooterTip> = Vec:REDACTED_SECRET);
        let mut used = 0usize;
        ...
        if current.is_empty() {
            lines.push(Vec:REDACTED_SECRET));
        } else {
            lines.push(current);
        }
        lines
        ...
        self.answers = self
            ...
            .map(|question| {
                let has_options = question
                    ...
                    .as_ref()
                    .is_some_and(|options| !options.is_empty());
                let mut options_state = ScrollState:REDACTED_SECRET);
                if has_options {
    ...
    fn submit_answers(&mut self) {
        self.confirm_unanswered = None;
        self.save_current_draft();
        let mut answers = HashMap:REDACTED_SECRET);
    ...
    fn open_unanswered_confirmation(&mut self) {
        let mut state = ScrollState:REDACTED_SECRET);
        state.selected_idx = Some(0);
    ...
    fn is_question_answered(&self, idx: usize, _current_text: REDACTED_SECRET) -> bool {
...
mod tests {
    ...
    fn test_sender() -> (
        AppEventREDACTED_SECRETer,
        tokio::sync::mpsc::UnboundedReceiver<AppEvent>,
    ) {
        let (tx_raw, rx) = unbounded_channel::<AppEvent>();
        (AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw), rx)
    }
    ...
    fn question_with_options(id: REDACTED_SECRET, header: REDACTED_SECRET) -> RequestUserInputQuestion {
    ...
    fn question_with_options_and_other(id: REDACTED_SECRET, header: REDACTED_SECRET) -> RequestUserInputQuestion {
        RequestUserInputQuestion {
            id: id.to_string(),
            header: header.to_string(),
            question: "Choose an option.".to_string(),
    ...
    fn question_with_wrapped_options(id: REDACTED_SECRET, header: REDACTED_SECRET) -> RequestUserInputQuestion {
        RequestUserInputQuestion {
            id: id.to_string(),
            header: header.to_string(),
    ...
    fn question_with_very_long_option_text(id: REDACTED_SECRET, header: REDACTED_SECRET) -> RequestUserInputQuestion {
        RequestUserInputQuestion {
            id: id.to_string(),
            header: header.to_string(),
    ...
    fn question_with_long_scroll_options(id: REDACTED_SECRET, header: REDACTED_SECRET) -> RequestUserInputQuestion {
        RequestUserInputQuestion {
            id: id.to_string(),
            header: header.to_string(),
            question:
    ...
    fn question_without_options(id: REDACTED_SECRET, header: REDACTED_SECRET) -> RequestUserInputQuestion {
        RequestUserInputQuestion {
            id: id.to_string(),
            header: header.to_string(),
    ...
    fn queued_requests_are_fifo() {
        ...
        let mut overlay = RequestUserInputOverlay:REDACTED_SECRET
    ...
    fn options_can_submit_empty_when_unanswered() {
        ...
        assert_eq!(answer.answers, Vec::<String>:REDACTED_SECRET));
    }
    ...
    fn enter_commits_default_selection_on_non_last_option_question() {
        ...
        let mut expected = HashMap:REDACTED_SECRET);
        expected.insert(
            "q1".to_string(),
            RequestUserInputAnswer {
    ...
    fn switching_from_freeform_with_text_resets_focus_and_keeps_last_option_empty() {
        ...
        overlay
            .composer
            .set_text_content("freeform notes".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    ...
    fn tab_in_notes_clears_notes_and_hides_ui() {
        ...
        overlay
            .composer
            .set_text_content("Some notes".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    ...
    fn skipped_option_questions_count_as_unanswered() {
        let (tx, _rx) = test_sender();
        let overlay = RequestUserInputOverlay:REDACTED_SECRET
            request_event("turn-1", vec![question_with_options("q1", "Pick one")]),
            tx,
            /*has_input_focus*/ true,
            /*enhanced_keys_supported*/ false,
    ...
    fn freeform_requires_enter_with_text_to_mark_answered() {
        ...
        overlay
            .composer
            .set_text_content("Draft".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        overlay.composer.move_cursor_to_end();
    ...
    fn freeform_draft_is_not_submitted_without_enter() {
        ...
        overlay
            .composer
            .set_text_content("Draft text".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        overlay.composer.move_cursor_to_end();
    ...
    fn freeform_commit_resets_when_draft_changes() {
        ...
        overlay
            .composer
            .set_text_content("Committed".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        overlay.composer.move_cursor_to_end();
        overlay.handle_key_event(KeyEvent::from(KeyCode::Enter));
        assert_eq!(overlay.answers[0].answer_committed, true);
        let _ = rx.try_recv();
        ...
        overlay
            .composer
            .set_text_content("Edited".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        overlay.composer.move_cursor_to_end();
        overlay.move_question(/*next*/ true);
        assert_eq!(overlay.answers[0].answer_committed, false);
    ...
    fn notes_are_captured_for_selected_option() {
        ...
        overlay
            .composer
            .set_text_content("Notes for option 2".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        overlay.composer.move_cursor_to_end();
    ...
    fn notes_submission_commits_selected_option() {
        ...
        overlay
            .composer
            .set_text_content("Notes".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        overlay.composer.move_cursor_to_end();
    ...
    fn is_other_adds_none_of_the_above_and_submits_it() {
        ...
        overlay
            .composer
            .set_text_content("Custom answer".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
        overlay.composer.move_cursor_to_end();
        let draft = overlay.capture_composer_draft();
        if let Some(answer) = overlay.current_answer_mut() {
            answer.draft = draft;
    ...
    fn pending_paste_placeholder_survives_submission_and_back_navigation() {
        ...
        overlay.handle_key_event(KeyEvent::from(KeyCode::Enter));
        overlay.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('p'), KeyModifiers::CONTROL));
    ...
    fn request_user_input_options_snapshot() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 120, 16);
    ...
    fn request_user_input_tight_height_snapshot() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 120, 10);
        insta::assert_snapshot!(
    ...
    fn layout_allocates_all_wrapped_options_when_space_allows() {
        ...
        let sections = overlay.layout_sections(Rect:REDACTED_SECRET0, 0, width, height));
    ...
    fn desired_height_keeps_spacers_and_preferred_options_visible() {
        ...
        let content_area = menu_surface_inset(Rect:REDACTED_SECRET0, 0, width, height));
    ...
    fn request_user_input_long_option_text_snapshot() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 120, 18);
        insta::assert_snapshot!(
    ...
    fn selected_long_wrapped_option_stays_visible() {
        ...
        let rendered = render_snapshot(&overlay, Rect:REDACTED_SECRET0, 0, 80, 20));
        assert!(
    ...
    fn request_user_input_scroll_options_snapshot() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 120, 12);
        insta::assert_snapshot!(
    ...
    fn request_user_input_hidden_options_footer_snapshot() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 80, 10);
        insta::assert_snapshot!(
    ...
    fn request_user_input_multi_question_first_snapshot() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 120, 15);
        insta::assert_snapshot!(
    ...
    fn request_user_input_unanswered_confirmation_snapshot() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 80, 12);
        insta::assert_snapshot!(
