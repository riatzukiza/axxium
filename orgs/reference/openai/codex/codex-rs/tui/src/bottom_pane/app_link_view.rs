...
impl AppLinkView {
    pub(crate) fn new(params: AppLinkViewParams, app_event_tx: AppEventREDACTED_SECRETer) -> Self {
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.extend(adaptive_wrap_lines(
            vec![url_line],
            RtOptions:REDACTED_SECRETusable_width),
        ));
...
impl crate::render::renderable::Renderable for AppLinkView {
    fn desired_height(&self, width: u16) -> u16 {
        ...
        let content_rows = Paragraph:REDACTED_SECRETcontent_lines)
...
mod tests {
    ...
    fn toggle_action_sends_set_app_enabled_and_updates_label() {
        ...
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let mut view = AppLinkView:REDACTED_SECRET
            AppLinkViewParams {
                app_id: "connector_1".to_string(),
                title: "Notion".to_string(),
                description: None,
        ...
        view.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('2'), KeyModifiers::NONE));
    ...
    fn install_suggestion_with_reason_snapshot() {
        ...
        assert_snapshot!(
            ...
            render_snapshot(
                &view,
                Rect:REDACTED_SECRET0, 0, 72, view.desired_height(/*width*/ 72))
            )
        );
    }
