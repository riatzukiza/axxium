...
impl ChatComposerHistory {
    pub fn new() -> Self {
        Self {
            ...
            local_history: Vec:REDACTED_SECRET),
            fetched_history: HashMap:REDACTED_SECRET),
            history_cursor: None,
    ...
    pub fn should_handle_navigation(&self, text: REDACTED_SECRET, cursor: usize) -> bool {
    ...
    pub fn navigate_up(&mut self, app_event_tx: &AppEventREDACTED_SECRETer) -> Option<HistoryEntry> {
    ...
    /// Handle <Down>.
    pub fn navigate_down(&mut self, app_event_tx: &AppEventREDACTED_SECRETer) -> Option<HistoryEntry> {
        let total_entries = self.history_entry_count + self.local_history.len();
        if total_entries == 0 {
            return None;
        }
        ...
        match next_idx_opt {
            ...
            None => {
                ...
                self.history_cursor = None;
                self.last_history_text = None;
                Some(HistoryEntry:REDACTED_SECRETString:REDACTED_SECRET)))
            }
        }
    }
    ...
    pub fn on_entry_response(
        ...
        let entry = HistoryEntry:REDACTED_SECRETentry?);
...
mod tests {
    ...
    fn duplicate_submissions_are_not_recorded() {
        let mut history = ChatComposerHistory:REDACTED_SECRET);

        // Empty submissions are ignored.
        history.record_local_submission(HistoryEntry:REDACTED_SECRETString:REDACTED_SECRET)));
        assert_eq!(history.local_history.len(), 0);

        // First entry is recorded.
        history.record_local_submission(HistoryEntry:REDACTED_SECRET"hello".to_string()));
        assert_eq!(history.local_history.len(), 1);
        assert_eq!(
            history.local_history.last().unwrap(),
            &HistoryEntry:REDACTED_SECRET"hello".to_string())
        );
        ...
        history.record_local_submission(HistoryEntry:REDACTED_SECRET"world".to_string()));
        assert_eq!(history.local_history.len(), 2);
        assert_eq!(
            history.local_history.last().unwrap(),
            &HistoryEntry:REDACTED_SECRET"world".to_string())
        );
    }
    ...
    fn navigation_with_async_fetch() {
        ...
        assert_eq!(
            Some(HistoryEntry:REDACTED_SECRET"latest".to_string())),
        ...
        assert_eq!(
            Some(HistoryEntry:REDACTED_SECRET"older".to_string())),
    ...
    fn reset_navigation_resets_cursor() {
        ...
        history
            .fetched_history
            .insert(1, HistoryEntry:REDACTED_SECRET"command2".to_string()));
        history
            .fetched_history
            .insert(2, HistoryEntry:REDACTED_SECRET"command3".to_string()));

        assert_eq!(
            Some(HistoryEntry:REDACTED_SECRET"command3".to_string())),
            history.navigate_up(&tx)
        );
        assert_eq!(
            Some(HistoryEntry:REDACTED_SECRET"command2".to_string())),
            history.navigate_up(&tx)
        );
