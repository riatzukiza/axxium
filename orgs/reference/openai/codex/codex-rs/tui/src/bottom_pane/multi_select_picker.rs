...
//!
//! ```ignore
//! let picker = MultiSelectPicker:REDACTED_SECRET
...
const SEARCH_PLACEHOLDER: REDACTED_SECRET = "Type to search";

/// Prefix displayed before the search query (mimics a command prompt).
const SEARCH_PROMPT_PREFIX: REDACTED_SECRET = "> ";
...
pub type ChangeCallBack = Box<dyn Fn(&[MultiSelectItem], &AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync>;

/// Callback invoked when the user confirms their selection (presses Enter).
/// Receives a list of IDs for all enabled items.
pub type ConfirmCallback = Box<dyn Fn(&[String], &AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync>;

/// Callback invoked when the user cancels the picker (presses Escape).
pub type CancelCallback = Box<dyn Fn(&AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync>;

/// Callback to generate an optional preview line based on current item states.
/// Returns `None` to hide the preview area.
pub type PreviewCallback = Box<dyn Fn(&[MultiSelectItem]) -> Option<Line<'static>> + REDACTED_SECRET + Sync>;
...
impl MultiSelectPicker {
    ...
    pub fn builder(
        ...
        MultiSelectPickerBuilder:REDACTED_SECRETtitle, subtitle, app_event_tx)
    }
    ...
    fn apply_filter(&mut self) {
        ...
        if filter.is_empty() {
            ...
            let mut matches: Vec<(usize, i32)> = Vec:REDACTED_SECRET);
...
///
/// ```ignore
/// let picker = MultiSelectPicker:REDACTED_SECRET"Title".into(), None, tx)
///     .items(items)
///     .enable_ordering()
...
impl MultiSelectPickerBuilder {
    ...
    pub fn new(title: String, subtitle: Option<String>, app_event_tx: AppEventREDACTED_SECRETer) -> Self {
        Self {
            title,
            subtitle,
            instructions: Vec:REDACTED_SECRET),
    ...
    where
        F: Fn(&[MultiSelectItem]) -> Option<Line<'static>> + REDACTED_SECRET + Sync + 'static,
    {
        self.preview_builder = Some(Box:REDACTED_SECRETcallback));
        self
    }
    ...
    where
        F: Fn(&[MultiSelectItem], &AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync + 'static,
    {
        self.on_change = Some(Box:REDACTED_SECRETcallback));
        self
    }
    ...
    where
        F: Fn(&[String], &AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync + 'static,
    {
        self.on_confirm = Some(Box:REDACTED_SECRETcallback));
        self
    }
    ...
    where
        F: Fn(&AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync + 'static,
    {
        self.on_cancel = Some(Box:REDACTED_SECRETcallback));
        self
    }
...
pub(crate) fn match_item(
    filter: REDACTED_SECRET,
    display_name: REDACTED_SECRET,
