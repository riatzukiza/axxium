...
/// Walks the spawn tree REDACTED_SECRETed at `primary_thread_id` and returns every descendant subagent.
///
...
pub(crate) fn find_loaded_subagent_threads_for_primary(
    ...
    let mut threads_by_id = HashMap:REDACTED_SECRET);
    ...
    let mut included = HashSet:REDACTED_SECRET);
