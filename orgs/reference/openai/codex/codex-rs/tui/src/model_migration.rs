...
pub(crate) async fn run_model_migration_prompt(
    ...
    let mut screen = ModelMigrationScreen:REDACTED_SECRETalt.tui.frame_requester(), copy);
...
impl WidgetRef for &ModelMigrationScreen {
    fn render_ref(&self, area: ratatui::layout::Rect, buf: &mut ratatui::buffer::Buffer) {
        ...
        let mut column = ColumnRenderable:REDACTED_SECRET);
        column.push("");
...
impl ModelMigrationScreen {
    ...
    fn render_lines(&self, lines: &[Line<'static>], column: &mut ColumnRenderable) {
        for line in lines {
            column.push(
                Paragraph:REDACTED_SECRETline.clone())
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph:REDACTED_SECRET"Choose how you'd like Codex to proceed.")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(
                    /*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0,
                )),
...
fn fill_migration_markdown(template: REDACTED_SECRET, current_model: REDACTED_SECRET, target_model: REDACTED_SECRET) -> String {
    template
...
mod tests {
    ...
    fn prompt_snapshot() {
        ...
        let screen = ModelMigrationScreen:REDACTED_SECRET
    ...
    fn prompt_snapshot_gpt5_family() {
        let backend = VT100Backend:REDACTED_SECRET/*width*/ 65, /*height*/ 22);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect:REDACTED_SECRET0, 0, 65, 22));
    ...
    fn prompt_snapshot_gpt5_codex() {
        let backend = VT100Backend:REDACTED_SECRET/*width*/ 60, /*height*/ 22);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect:REDACTED_SECRET0, 0, 60, 22));
    ...
    fn escape_key_accepts_prompt() {
        let mut screen = ModelMigrationScreen:REDACTED_SECRET
            FrameRequester::test_dummy(),
            migration_copy_for_models(
                "gpt-old",
                "gpt-new",
        ...
        screen.handle_key(KeyEvent:REDACTED_SECRET
            KeyCode::Esc,
    ...
    fn markdown_prompt_keeps_long_url_tail_visible_when_narrow() {
        ...
        let backend = VT100Backend:REDACTED_SECRET/*width*/ 40, /*height*/ 16);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect:REDACTED_SECRET0, 0, 40, 16));
