...
pub(crate) fn compose_model_display(
    ...
    entries: &[(REDACTED_SECRET, String)],
) -> (String, Vec<String>) {
    let mut details: Vec<String> = Vec:REDACTED_SECRET);
...
pub(crate) fn compose_agents_summary(config: &Config) -> String {
    match discover_project_doc_paths(config) {
        Ok(paths) => {
            let mut rels: Vec<String> = Vec:REDACTED_SECRET);
            for p in paths {
                let file_name = p
                    .file_name()
...
fn title_case(s: REDACTED_SECRET) -> String {
    if s.is_empty() {
        return String:REDACTED_SECRET);
    }
