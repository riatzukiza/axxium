...
pub(crate) fn new_status_output(
    ...
    now: DateTime<Local>,
    model_name: REDACTED_SECRET,
    collaboration_mode: Option<REDACTED_SECRET>,
...
pub(crate) fn new_status_output_with_rate_limits_handle(
    ...
    let command = PlainHistoryCell:REDACTED_SECRETvec!["/status".magenta().into()]);
    let (card, handle) = StatusHistoryCell:REDACTED_SECRET
        config,
        account_display,
        token_info,
        total_usage,
    ...
    (
        CompositeHistoryCell:REDACTED_SECRETvec![Box:REDACTED_SECRETcommand), Box:REDACTED_SECRETcard)]),
        handle,
    )
}
...
impl StatusHistoryCell {
    ...
    fn new(
        ...
        let rate_limit_state = Arc:REDACTED_SECRETRwLock:REDACTED_SECRETStatusRateLimitState {
            rate_limits,
            refreshing_rate_limits,
        }));
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(Vec::<Span<'static>>:REDACTED_SECRET)));
        ...
        let note_lines = adaptive_wrap_lines(
            ...
            RtOptions:REDACTED_SECRETavailable_inner_width),
        );
...
fn sanitize_base_url(raw: REDACTED_SECRET) -> Option<String> {
