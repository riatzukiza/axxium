...
fn token_info_for(model_slug: REDACTED_SECRET, config: &Config, usage: &TokenUsage) -> TokenUsageInfo {
    let context_window =
...
fn reset_at_from(captured_at: &chrono::DateTime<chrono:REDACTED_SECRET>, seconds: i64) -> i64 {
...
async fn status_snapshot_includes_reasoning_details() {
    let temp_home = TempDir:REDACTED_SECRET).expect("temp home");
    ...
    let captured_at = chrono:REDACTED_SECRET
...
async fn status_card_token_usage_excludes_cached_tokens() {
    ...
    let now = chrono:REDACTED_SECRET
