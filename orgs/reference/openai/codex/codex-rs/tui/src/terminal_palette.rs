...
static DEFAULT_PALETTE_VERSION: AtomicU64 = AtomicU64:REDACTED_SECRET0);
...
mod imp {
    ...
    fn default_colors_cache() -> &'static Mutex<Cache<DefaultColors>> {
        static CACHE: OnceLock<Mutex<Cache<DefaultColors>>> = OnceLock:REDACTED_SECRET);
        CACHE.get_or_init(|| Mutex:REDACTED_SECRETCache::default()))
    }
