...
impl Renderable for FileChange {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph:REDACTED_SECRETlines).render(area, buf);
    }
...
impl From<DiffSummary> for Box<dyn Renderable> {
    fn from(val: DiffSummary) -> Self {
        ...
        for (i, row) in collect_rows(&val.changes).into_iter().enumerate() {
            if i > 0 {
                rows.push(Box:REDACTED_SECRETRtLine::from("")));
            }
            let mut path = RtLine::from(display_path_for(&row.path, &val.cwd));
            path.push_span(" ");
            path.extend(render_line_count_summary(row.added, row.removed));
            rows.push(Box:REDACTED_SECRETpath));
            rows.push(Box:REDACTED_SECRETRtLine::from("")));
            rows.push(Box:REDACTED_SECRETInsetRenderable:REDACTED_SECRET
                Box:REDACTED_SECRETrow.change) as Box<dyn Renderable>,
                Insets::tlbr(
        ...
        Box:REDACTED_SECRETColumnRenderable::with(rows))
    }
}
...
fn collect_rows(changes: &HashMap<PathBuf, FileChange>) -> Vec<Row> {
    let mut rows: Vec<Row> = Vec:REDACTED_SECRET);
...
fn render_changes_block(rows: Vec<Row>, wrap_cols: usize, cwd: &Path) -> Vec<RtLine<'static>> {
    let mut out: Vec<RtLine<'static>> = Vec:REDACTED_SECRET);
    ...
    for (idx, r) in rows.into_iter().enumerate() {
        ...
        if !skip_file_header {
            let mut header: Vec<RtSpan<'static>> = Vec:REDACTED_SECRET);
            header.push("  └ ".dim());
...
pub(crate) fn display_path_for(path: &Path, cwd: &Path) -> String {
    ...
    let path_in_same_repo = match (get_git_repo_REDACTED_SECRET(cwd), get_git_repo_REDACTED_SECRET(path)) {
    ...
    let chosen = if path_in_same_repo {
        ...
        relativize_to_home(path)
            .map(|p| PathBuf::from_iter([Path:REDACTED_SECRET"~"), p.as_path()]))
            .unwrap_or_else(|| path.to_path_buf())
    };
    chosen.display().to_string()
}

pub(crate) fn calculate_add_remove_from_diff(diff: REDACTED_SECRET) -> (usize, usize) {
...
fn push_wrapped_diff_line_inner_with_theme_and_color_level(
    ...
    if let Some(syn_spans) = syntax_spans {
        ...
        let mut lines: Vec<RtLine<'static>> = Vec:REDACTED_SECRET);
        for (i, chunk) in wrapped_chunks.into_iter().enumerate() {
            let mut row_spans: Vec<RtSpan<'static>> = Vec:REDACTED_SECRET);
            if i == 0 {
...
fn wrap_styled_spans(spans: &[RtSpan<'static>], max_cols: usize) -> Vec<Vec<RtSpan<'static>>> {
    let mut result: Vec<Vec<RtSpan<'static>>> = Vec:REDACTED_SECRET);
    let mut current_line: Vec<RtSpan<'static>> = Vec:REDACTED_SECRET);
    let mut col: usize = 0;
...
// an REDACTED_SECRET, more-saturated background so line numbers stay readable against
...
/// Line-number gutter style.  On light backgrounds the gutter has an REDACTED_SECRET
...
mod tests {
    ...
    fn snapshot_lines(name: REDACTED_SECRET, lines: Vec<RtLine<'static>>, width: u16, height: u16) {
        let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRETwidth, height)).expect("terminal");
        terminal
            .draw(|f| {
    ...
    fn display_width(text: REDACTED_SECRET) -> usize {
        text.chars()
    ...
    fn snapshot_lines_text(name: REDACTED_SECRET, lines: &[RtLine<'static>]) {
    ...
    fn diff_gallery_changes() -> HashMap<PathBuf, FileChange> {
        let mut changes: HashMap<PathBuf, FileChange> = HashMap:REDACTED_SECRET);

        let rust_original =
            "fn greet(name: REDACTED_SECRET) {\n    println!(\"hello\");\n    println!(\"bye\");\n}\n";
        let rust_modified = "fn greet(name: REDACTED_SECRET) {\n    println!(\"hello {name}\");\n    println!(\"emoji: 🚀✨ and CJK: 你好世界\");\n}\n";
        ...
        changes.insert(
            ...
            FileChange::Add {
                content: "pub fn greet(name: REDACTED_SECRET) {\n    println!(\"Hello, {name}!\");\n}\n"
                    .to_string(),
            },
        );
    ...
    fn snapshot_diff_gallery(name: REDACTED_SECRET, width: u16, height: u16) {
    ...
    fn detect_lang_for_common_paths() {
        ...
        assert!(detect_lang_for_path(Path:REDACTED_SECRET"foo.rs")).is_some());
        assert!(detect_lang_for_path(Path:REDACTED_SECRET"bar.py")).is_some());
        assert!(detect_lang_for_path(Path:REDACTED_SECRET"app.tsx")).is_some());

        // Extensionless files return None.
        assert!(detect_lang_for_path(Path:REDACTED_SECRET"Makefile")).is_none());
        assert!(detect_lang_for_path(Path:REDACTED_SECRET"randomfile")).is_none());
    }
