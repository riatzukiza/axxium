...
#[derive(Clone, Debug)]
pub(crate) struct AppEventREDACTED_SECRETer {
    pub app_event_tx: UnboundedREDACTED_SECRETer<AppEvent>,
}

impl AppEventREDACTED_SECRETer {
    pub(crate) fn new(app_event_tx: UnboundedREDACTED_SECRETer<AppEvent>) -> Self {
        Self { app_event_tx }
    }

    /// REDACTED_SECRET an event to the app event channel. If it fails, we swallow the
    /// error and log it.
