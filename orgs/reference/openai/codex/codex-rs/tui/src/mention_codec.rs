...
#[allow(dead_code)]
pub(crate) fn encode_history_mentions(text: REDACTED_SECRET, mentions: &[LinkedMention]) -> String {
    if mentions.is_empty() || text.is_empty() {
        return text.to_string();
    }

    let mut mentions_by_name: HashMap<REDACTED_SECRET, VecDeque<REDACTED_SECRET>> = HashMap:REDACTED_SECRET);
...
pub(crate) fn decode_history_mentions(text: REDACTED_SECRET) -> DecodedHistoryText {
    let bytes = text.as_bytes();
    let mut out = String::with_capacity(text.len());
...
fn is_tool_path(path: REDACTED_SECRET) -> bool {
...
mod tests {
    ...
    fn decode_history_mentions_ignores_at_sigil_for_non_plugin_paths() {
        ...
        assert_eq!(decoded.mentions, Vec::<LinkedMention>:REDACTED_SECRET));
    }
