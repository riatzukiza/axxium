...
const DETAILS_PREFIX: REDACTED_SECRET = "  └ ";
...
impl StatusIndicatorWidget {
    ...
    #[cfg(test)]
    pub(crate) fn header(&self) -> REDACTED_SECRET {
        &self.header
    }

    #[cfg(test)]
    pub(crate) fn details(&self) -> Option<REDACTED_SECRET> {
    ...
    fn wrapped_details_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let opts = RtOptions:REDACTED_SECRETusize::from(width))
...
impl Renderable for StatusIndicatorWidget {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph:REDACTED_SECRETText::from(lines)).render_ref(area, buf);
    }
}
...
mod tests {
    ...
    fn renders_with_working_header() {
        ...
        let w = StatusIndicatorWidget:REDACTED_SECRET
            tx,
        ...
        let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET80, 2)).expect("terminal");
        terminal
    ...
    fn renders_truncated() {
        ...
        // Render into a fixed-size test terminal and snapshot the backend.
        let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET20, 2)).expect("terminal");
        terminal
            .draw(|f| w.render(f.area(), f.buffer_mut()))
            .expect("draw");
        insta::assert_snapshot!(terminal.backend());
    ...
    fn renders_wrapped_details_panama_two_lines() {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let mut w = StatusIndicatorWidget:REDACTED_SECRET
            tx,
            crate::tui::FrameRequester::test_dummy(),
        ...
        let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET30, 3)).expect("terminal");
        terminal
            .draw(|f| w.render(f.area(), f.buffer_mut()))
            .expect("draw");
        insta::assert_snapshot!(terminal.backend());
    ...
    fn timer_pauses_when_requested() {
        let (tx_raw, _rx) = unbounded_channel::<AppEvent>();
        let tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
        let mut widget = StatusIndicatorWidget:REDACTED_SECRET
            tx,
            crate::tui::FrameRequester::test_dummy(),
            /*animations_enabled*/ true,
        );
