...
#[tokio::test]
async fn preset_matching_accepts_workspace_write_with_extra_REDACTED_SECRETs() {
    let preset = builtin_approval_presets()
        .into_iter()
        .find(|p| p.id == "auto")
        .expect("auto preset exists");
    let current_sandbox = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![PathBuf::from("C:\\extra").abs()],
    ...
    assert!(
        ...
        "WorkspaceWrite with extra REDACTED_SECRETs should still match the Default preset"
    );
    assert!(
...
async fn approvals_popup_shows_disabled_presets() {
    ...
    chat.config.permissions.approval_policy =
        Constrained:REDACTED_SECRETAskForApproval::OnRequest, |candidate| match candidate {
...
async fn approvals_popup_navigation_skips_disabled() {
    ...
    let mut app_events = Vec:REDACTED_SECRET);
...
async fn permissions_selection_marks_guardian_approvals_current_with_custom_workspace_write_details()
 {
    ...
    let extra_REDACTED_SECRET = PathBuf::from("/tmp/guardian-approvals-extra").abs();
    ...
    chat.handle_codex_event(Event {
        ...
        msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
            ...
            sandbox_policy: SandboxPolicy::WorkspaceWrite {
                writable_REDACTED_SECRETs: vec![extra_REDACTED_SECRET],
...
async fn permissions_full_access_history_cell_emitted_only_after_confirmation() {
    ...
    let mut cells_before_confirmation = Vec:REDACTED_SECRET);
