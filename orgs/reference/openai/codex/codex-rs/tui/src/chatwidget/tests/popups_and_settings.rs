...
async fn apps_initial_load_applies_enabled_state_from_requirements_with_user_override() {
    ...
    chat.config.config_layer_stack =
        ConfigLayerStack:REDACTED_SECRETVec:REDACTED_SECRET), ConfigRequirements::default(), requirements)
...
async fn experimental_features_popup_snapshot() {
    ...
    let view = ExperimentalFeaturesView:REDACTED_SECRETfeatures, chat.app_event_tx.clone());
    chat.bottom_pane.show_view(Box:REDACTED_SECRETview));
...
async fn experimental_features_toggle_saves_on_exit() {
    ...
    let view = ExperimentalFeaturesView:REDACTED_SECRET
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char(' '), KeyModifiers::NONE));
...
#[tokio::test]
async fn experimental_popup_shows_js_repl_REDACTED_SECRET_requirement() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let REDACTED_SECRET_requirement = js_repl_description
        .split(". ")
    ...
    assert!(
        popup.contains(REDACTED_SECRET_requirement),
...
async fn model_picker_hides_show_in_picker_false_models_from_cache() {
    ...
    chat.thread_id = Some(ThreadId:REDACTED_SECRET));
    let preset = |slug: REDACTED_SECRET, show_in_picker: bool| ModelPreset {
        id: slug.to_string(),
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &codex_feedback::feedback_diagnostics::FeedbackDiagnostics:REDACTED_SECRETvec![
