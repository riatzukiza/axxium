...
async fn collab_spawn_end_shows_requested_model_and_effort() {
    ...
    let sender_thread_id = ThreadId:REDACTED_SECRET);
    let spawned_thread_id = ThreadId:REDACTED_SECRET);
...
async fn live_app_server_user_message_item_completed_does_not_duplicate_rendered_prompt() {
    ...
    chat.bottom_pane
        .set_composer_text("Hi, are you there?".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
