...
async fn submit_user_message_blocks_when_thread_model_is_unavailable() {
    ...
    chat.set_model("");
    chat.bottom_pane
        .set_composer_text("hello".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
...
"
    ...
    chat.bottom_pane
        .set_composer_text("Please continue.".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
...
"
    ...
    chat.bottom_pane
        .set_composer_text("Please revise.".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
...
async fn enter_submits_when_plan_stream_is_not_active() {
    ...
    chat.bottom_pane
        .set_composer_text("submitted immediately".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
...
async fn collab_slash_command_opens_picker_and_updates_mode() {
    ...
    chat.bottom_pane
        .set_composer_text("follow up".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
    match next_submit_op(&mut op_rx) {
        Op::UserTurn {
            collaboration_mode:
...
async fn plan_slash_command_with_args_submits_prompt_in_plan_mode() {
    ...
    chat.bottom_pane
        .set_composer_text("/plan build the plan".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
...
async fn collaboration_modes_defaults_to_code_on_startup() {
    ...
    let init = ChatWidgetInit {
        ...
        app_event_tx: AppEventREDACTED_SECRETer:REDACTED_SECRETunbounded_channel::<AppEvent>().0),
