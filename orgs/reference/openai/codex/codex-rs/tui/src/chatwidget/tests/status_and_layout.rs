...
async fn rate_limit_warnings_emit_thresholds() {
    ...
    let mut warnings: Vec<String> = Vec:REDACTED_SECRET);
...
async fn streaming_final_answer_keeps_task_running_state() {
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Tab, KeyModifiers::NONE));
...
async fn ui_snapshots_small_heights_idle() {
    ...
    for h in [1u16, 2, 3] {
        ...
        let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET40, h)).expect("create terminal");
        terminal
...
async fn status_line_fast_mode_footer_snapshot() {
    ...
    let width = 80;
    let height = chat.desired_height(width);
    let mut terminal = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRETwidth, height)).expect("create terminal");
    terminal
        .draw(|f| chat.render(f.area(), f.buffer_mut()))
...
async fn user_prompt_submit_app_server_hook_notifications_render_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::HookStarted(AppServerHookStartedNotification {
            thread_id: ThreadId:REDACTED_SECRET).to_string(),
            turn_id: Some("turn-1".to_string()),
...
async fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    let viewport = Rect:REDACTED_SECRET0, vt_height - ui_height - 1, width, ui_height);

    let backend = VT100Backend:REDACTED_SECRETwidth, vt_height);
...
async fn chatwidget_markdown_code_blocks_vt100_snapshot() {
    ...
    term.set_viewport_area(Rect:REDACTED_SECRET0, height - 1, width, 1));

    // Simulate streaming via AgentMessageDelta in 2-character chunks (no final AgentMessage).
    let source: REDACTED_SECRET = r#"
...
"#;
    ...
    loop {
        let mut delta = String:REDACTED_SECRET);
        match it.next() {
            Some(c) => delta.push(c),
            None => break,
        }
