...
impl ChatWidget {
    ...
    pub(crate) fn open_manage_skills_popup(&mut self) {
        ...
        let mut initial_state = HashMap:REDACTED_SECRET);
        ...
        let view = SkillsToggleView:REDACTED_SECRETitems, self.app_event_tx.clone());
    ...
    pub(crate) fn handle_manage_skills_closed(&mut self) {
        ...
        let mut current_state = HashMap:REDACTED_SECRET);
        for skill in &self.skills_all {
...
pub(crate) fn find_skill_mentions_with_tool_mentions(
    ...
    let mut seen_names = HashSet:REDACTED_SECRET);
    let mut seen_paths = HashSet:REDACTED_SECRET);
    let mut matches: Vec<SkillMetadata> = Vec:REDACTED_SECRET);
...
pub(crate) fn find_app_mentions(
    ...
    let mut explicit_names = HashSet:REDACTED_SECRET);
    let mut selected_ids = HashSet:REDACTED_SECRET);
    ...
    let mut slug_counts: HashMap<String, usize> = HashMap:REDACTED_SECRET);
...
fn extract_tool_mentions_from_text(text: REDACTED_SECRET) -> ToolMentions {
    extract_tool_mentions_from_text_with_sigil(text, TOOL_MENTION_SIGIL)
}

fn extract_tool_mentions_from_text_with_sigil(text: REDACTED_SECRET, sigil: char) -> ToolMentions {
    let text_bytes = text.as_bytes();
    let mut names: HashSet<String> = HashSet:REDACTED_SECRET);
    let mut linked_paths: HashMap<String, String> = HashMap:REDACTED_SECRET);
...
fn is_skill_path(path: REDACTED_SECRET) -> bool {
    !path.starts_with("app://") && !path.starts_with("mcp://") && !path.starts_with("plugin://")
}

fn normalize_skill_path(path: REDACTED_SECRET) -> REDACTED_SECRET {
    path.strip_prefix("skill://").unwrap_or(path)
}

fn app_id_from_path(path: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
