...
const PLUGINS_SELECTION_VIEW_ID: REDACTED_SECRET = "plugins-selection";
...
impl Renderable for DelayedLoadingHeader {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        Paragraph:REDACTED_SECRETlines).render_ref(area, buf);
    }
...
const APPS_HELP_ARTICLE_URL: REDACTED_SECRET = "https://help.openai.com/en/articles/11487775-apps-in-chatgpt";
...
impl Renderable for PluginDisclosureLine {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        Paragraph:REDACTED_SECRETself.line.clone())
...
impl ChatWidget {
    ...
    pub(crate) fn open_plugin_detail_loading_popup(&mut self, plugin_display_name: REDACTED_SECRET) {
    ...
    pub(crate) fn open_plugin_install_loading_popup(&mut self, plugin_display_name: REDACTED_SECRET) {
    ...
    pub(crate) fn open_plugin_uninstall_loading_popup(&mut self, plugin_display_name: REDACTED_SECRET) {
    ...
    fn plugin_install_auth_app_is_installed(&self, app_id: REDACTED_SECRET) -> bool {
    ...
    fn plugins_loading_popup_params(&self) -> SelectionViewParams {
        SelectionViewParams {
            view_id: Some(PLUGINS_SELECTION_VIEW_ID),
            header: Box:REDACTED_SECRETDelayedLoadingHeader:REDACTED_SECRET
    ...
    fn plugin_detail_loading_popup_params(&self, plugin_display_name: REDACTED_SECRET) -> SelectionViewParams {
        SelectionViewParams {
            view_id: Some(PLUGINS_SELECTION_VIEW_ID),
            header: Box:REDACTED_SECRETDelayedLoadingHeader:REDACTED_SECRET
                self.frame_requester.clone(),
    ...
    fn plugin_install_loading_popup_params(
        &self,
        plugin_display_name: REDACTED_SECRET,
    ) -> SelectionViewParams {
        let mut header = ColumnRenderable:REDACTED_SECRET);
        header.push(Line::from("Plugins".bold()));
        header.push(Line::from(
    ...
    fn plugins_error_popup_params(&self, err: REDACTED_SECRET) -> SelectionViewParams {
        let mut header = ColumnRenderable:REDACTED_SECRET);
        header.push(Line::from("Plugins".bold()));
...
fn plugin_brief_description(
    ...
    marketplace_label: REDACTED_SECRET,
