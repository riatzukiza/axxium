...
pub(super) async fn test_config() -> Config {
    ...
    let codex_home = tempfile::Builder:REDACTED_SECRET)
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_codex_home(codex_home.clone(), Vec:REDACTED_SECRET))
            .expect("config");
...
pub(super) fn truncated_path_variants(path: REDACTED_SECRET) -> Vec<String> {
...
pub(super) fn test_session_telemetry(config: &Config, model: REDACTED_SECRET) -> SessionTelemetry {
...
pub(super) fn test_model_catalog(config: &Config) -> Arc<ModelCatalog> {
    ...
    Arc:REDACTED_SECRETModelCatalog:REDACTED_SECRET
...
pub(super) async fn make_chatwidget_manual(
    ...
    let app_event_tx = AppEventREDACTED_SECRETer:REDACTED_SECRETtx_raw);
    ...
    let mut bottom = BottomPane:REDACTED_SECRETBottomPaneParams {
    ...
    let mut widget = ChatWidget {
        ...
        model_catalog,
        session_telemetry,
        session_header: SessionHeader:REDACTED_SECRETresolved_model.clone()),
...
pub(super) fn begin_unified_exec_startup(
    ...
    let event = ExecCommandBeginEvent {
        ...
        command,
        cwd,
        parsed_cmd: Vec:REDACTED_SECRET),
...
pub(super) fn pending_steer(text: REDACTED_SECRET) -> PendingSteer {
    PendingSteer {
...
pub(super) fn complete_user_message(chat: &mut ChatWidget, item_id: REDACTED_SECRET, text: REDACTED_SECRET) {
...
pub(super) fn get_available_model(chat: &ChatWidget, model: REDACTED_SECRET) -> ModelPreset {
    let models = chat
        .model_catalog
        .try_list_models()
...
pub(super) async fn assert_shift_left_edits_most_recent_queued_message_for_terminal(
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Left, KeyModifiers::SHIFT));
...
pub(super) fn strip_osc8_for_snapshot(text: REDACTED_SECRET) -> String {
...
pub(super) fn plugins_test_absolute_path(path: REDACTED_SECRET) -> AbsolutePathBuf {
    std::env::temp_dir()
...
pub(super) fn plugins_test_interface(
    display_name: Option<REDACTED_SECRET>,
    short_description: Option<REDACTED_SECRET>,
    long_description: Option<REDACTED_SECRET>,
) -> PluginInterface {
    PluginInterface {
        ...
        developer_name: None,
        category: None,
        capabilities: Vec:REDACTED_SECRET),
        website_url: None,
...
pub(super) fn plugins_test_summary(
    ...
    PluginSummary {
        id: id.to_string(),
        name: name.to_string(),
        source: PluginSource:REDACTED_SECRET {
...
pub(super) fn plugins_test_response(
    ...
    PluginListResponse {
        marketplaces,
        marketplace_load_errors: Vec:REDACTED_SECRET),
        remote_sync_error: None,
        featured_plugin_ids: Vec:REDACTED_SECRET),
    }
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<REDACTED_SECRET>,
    skills: &[REDACTED_SECRET],
    apps: &[(REDACTED_SECRET, bool)],
    mcp_servers: &[REDACTED_SECRET],
) -> PluginDetail {
    PluginDetail {
...
pub(super) fn plugins_test_popup_row_position(popup: REDACTED_SECRET, needle: REDACTED_SECRET) -> usize {
    popup
        .find(needle)
        .unwrap_or_else(|| panic!("expected popup to contain {needle}: {popup}"))
}

pub(super) fn type_plugins_search_query(chat: &mut ChatWidget, query: REDACTED_SECRET) {
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    run_id: REDACTED_SECRET,
    status_message: REDACTED_SECRET,
    snapshot_name: REDACTED_SECRET,
) {
