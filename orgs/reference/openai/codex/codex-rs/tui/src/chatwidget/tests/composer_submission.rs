...
async fn enter_with_only_remote_images_submits_user_turn() {
    ...
    let (stored_message, stored_remote_image_urls) =
        user_cell.expect("expected submitted user history cell");
    assert_eq!(stored_message, String:REDACTED_SECRET));
    assert_eq!(stored_remote_image_urls, vec![remote_url]);
}
...
async fn shift_enter_with_only_remote_images_does_not_submit_user_turn() {
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::SHIFT));
...
async fn blocked_image_restore_preserves_mention_bindings() {
    ...
    let expected_elements = vec![
        TextElement:REDACTED_SECRET(0..placeholder.len()).into(), Some(placeholder.to_string())),
...
async fn remap_placeholders_uses_byte_ranges_when_placeholder_missing() {
    ...
    let text = format!("{placeholder_two} before {placeholder_one}");
    let elements = vec![
        TextElement:REDACTED_SECRET(0..placeholder_two.len()).into(), /*placeholder*/ None),
        TextElement:REDACTED_SECRET
            ("[Image #2] before ".len().."[Image #2] before [Image #1]".len()).into(),
            /*placeholder*/ None,
        ),
...
async fn alt_up_edits_most_recent_queued_message() {
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::ALT));
...
async fn enqueueing_history_prompt_multiple_times_is_stable() {
    ...
    chat.bottom_pane
        .set_composer_text("repeat me".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
...
fn rendered_user_message_event_from_inputs_matches_flattened_user_message_shape() {
    ...
    let rendered = ChatWidget::rendered_user_message_event_from_inputs(&[
        UserInput::Text {
            ...
            text_elements: vec![TextElement:REDACTED_SECRET(0..5).into(), /*placeholder*/ None)],
        },
        UserInput::Image {
        ...
        UserInput::Text {
            ...
            text_elements: vec![TextElement:REDACTED_SECRET(0..5).into(), Some("planet".to_string()))],
        },
    ]);
    ...
    assert_eq!(
        ...
        ChatWidget::rendered_user_message_event_from_parts(
            ...
            vec![
                TextElement:REDACTED_SECRET(0..5).into(), Some("hello".to_string())),
                TextElement:REDACTED_SECRET(6..11).into(), Some("planet".to_string())),
            ],
            vec![local_image],
...
async fn interrupt_prepends_queued_messages_before_existing_composer_text() {
    ...
    chat.bottom_pane.set_task_running(/*running*/ true);
    chat.bottom_pane
        .set_composer_text("current draft".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
