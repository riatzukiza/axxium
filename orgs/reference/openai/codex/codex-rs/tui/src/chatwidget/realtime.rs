...
const REALTIME_CONVERSATION_PROMPT: REDACTED_SECRET = "You are in a realtime voice conversation in the Codex TUI. Respond conversationally and concisely.";
...
impl ChatWidget {
    ...
    pub(super) fn pending_steer_compare_key_from_items(
        ...
        for item in items {
            match item {
                ...
                UserInput::Image { .. } | UserInput:REDACTED_SECRETImage { .. } => image_count += 1,
    ...
    pub(super) fn rendered_user_message_event_from_inputs(
        ...
        let mut message = String:REDACTED_SECRET);
        let mut remote_image_urls = Vec:REDACTED_SECRET);
        let mut local_images = Vec:REDACTED_SECRET);
        let mut text_elements = Vec:REDACTED_SECRET);
        ...
        for item in items {
            match item {
                ...
                UserInput:REDACTED_SECRETImage { path } => local_images.push(path.clone()),
                UserInput::Skill { .. } | UserInput::Mention { .. } => {}
                _ => {}
            }
        }
    ...
    #[cfg(not(target_os = "linux"))]
    pub(crate) fn stop_realtime_conversation_for_deleted_meter(&mut self, id: REDACTED_SECRET) -> bool {
        if self.realtime_conversation.is_live()
    ...
    fn start_realtime_local_audio(&mut self) {
        ...
        std::thread::spawn(move || {
            let mut meter = crate::voice::RecordingMeterState:REDACTED_SECRET);
