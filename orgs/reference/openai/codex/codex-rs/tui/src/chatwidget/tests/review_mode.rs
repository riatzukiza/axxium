...
async fn interrupted_turn_restores_queued_messages_with_images_and_elements() {
    ...
    let first_elements = vec![TextElement:REDACTED_SECRET
    ...
    let second_elements = vec![TextElement:REDACTED_SECRET
    ...
    let existing_elements = vec![TextElement:REDACTED_SECRET
...
async fn steer_rejection_queues_review_follow_up_before_existing_queued_messages() {
    ...
    chat.thread_id = Some(ThreadId:REDACTED_SECRET));
    chat.handle_codex_event(Event {
        id: "turn-start".into(),
...
async fn restore_thread_input_state_restores_pending_steers_without_downgrading_them() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let mut pending_steers = VecDeque:REDACTED_SECRET);
    pending_steers.push_back(UserMessage::from("pending steer"));
    let mut rejected_steers_queue = VecDeque:REDACTED_SECRET);
    rejected_steers_queue.push_back(UserMessage::from("already rejected"));
    let mut queued_user_messages = VecDeque:REDACTED_SECRET);
...
async fn steer_enter_queues_while_plan_stream_is_active() {
    ...
    chat.bottom_pane
        .set_composer_text("queued submission".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
...
async fn steer_enter_uses_pending_steers_while_turn_is_running_without_streaming() {
    ...
    chat.bottom_pane
        .set_composer_text("queued while running".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
...
async fn item_completed_pops_pending_steer_with_local_image_and_text_elements() {
    ...
    let text_elements = vec![TextElement:REDACTED_SECRET(0..4).into(), Some("note".to_string()))];
    ...
    assert_eq!(
        stored_elements,
        vec![TextElement:REDACTED_SECRET(0..4).into(), Some("note".to_string()))]
    );
...
async fn steer_enter_during_final_stream_preserves_follow_up_prompts_in_order() {
    ...
    chat.bottom_pane
        .set_composer_text("first follow-up".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
    chat.bottom_pane
        .set_composer_text("second follow-up".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
...
async fn esc_interrupt_sends_all_pending_steers_immediately_and_keeps_existing_draft() {
    ...
    chat.bottom_pane
        .set_composer_text("first pending steer".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
    match next_submit_op(&mut op_rx) {
        Op::UserTurn { items, .. } => assert_eq!(
            items,
    ...
    chat.bottom_pane
        .set_composer_text("second pending steer".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
    ...
    chat.refresh_pending_input_preview();
    chat.bottom_pane
        .set_composer_text("still editing".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));

    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Esc, KeyModifiers::NONE));
...
async fn esc_with_pending_steers_overrides_agent_command_interrupt_behavior() {
    ...
    chat.bottom_pane
        .set_composer_text("pending steer".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
    match next_submit_op(&mut op_rx) {
        Op::UserTurn { .. } => {}
        other => panic!("expected Op::UserTurn, got {other:?}"),
    ...
    chat.bottom_pane
        .set_composer_text("/agent ".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Esc, KeyModifiers::NONE));
...
async fn ctrl_c_shutdown_works_with_caps_lock() {
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('C'), KeyModifiers::CONTROL));
...
async fn ctrl_c_closes_realtime_conversation_before_interrupt_or_quit() {
    ...
    chat.bottom_pane
        .set_composer_text("recording meter".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
...
async fn ctrl_c_cleared_prompt_is_recoverable_via_history() {
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::NONE));
...
async fn review_popup_custom_prompt_action_sends_event() {
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Down, KeyModifiers::NONE));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Down, KeyModifiers::NONE));
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Down, KeyModifiers::NONE));
    // Activate
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
...
async fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let area = ratatui::layout::Rect:REDACTED_SECRET0, 0, width, height);
    let mut buf = ratatui::buffer::Buffer::empty(area);
    chat.render(area, &mut buf);

    let mut blob = String:REDACTED_SECRET);
...
async fn review_queues_user_messages_snapshot() {
    ...
    term.set_viewport_area(Rect:REDACTED_SECRET0, height - desired_height, width, desired_height));
    term.draw(|f| {
