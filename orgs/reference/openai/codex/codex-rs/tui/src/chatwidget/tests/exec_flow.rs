...
async fn exec_approval_emits_proposed_command_and_decision_history() {
    ...
    let area = Rect:REDACTED_SECRET0, 0, 80, chat.desired_height(/*width*/ 80));
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('y'), KeyModifiers::NONE));
...
async fn exec_approval_decision_truncates_multiline_and_long_commands() {
    ...
    chat.handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('n'), KeyModifiers::NONE));
...
async fn preamble_keeps_working_status_snapshot() {
    ...
    let mut terminal = ratatui::Terminal:REDACTED_SECRETratatui::backend::TestBackend:REDACTED_SECRET80, height))
...
async fn unified_exec_begin_restores_working_status_snapshot() {
    ...
    let mut terminal = ratatui::Terminal:REDACTED_SECRETratatui::backend::TestBackend:REDACTED_SECRETwidth, height))
        .expect("create terminal");
...
async fn bang_shell_command_submits_run_user_shell_command_in_app_server_tui() {
    ...
    let conversation_id = ThreadId:REDACTED_SECRET);
    let rollout_file = NamedTempFile:REDACTED_SECRET).unwrap();
    ...
    chat.bottom_pane
        .set_composer_text("!echo hi".to_string(), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET));
...
async fn approval_modal_exec_snapshot() -> anyhow::Result<()> {
    ...
    let ev = ExecApprovalRequestEvent {
        ...
        network_approval_context: None,
        proposed_execpolicy_amendment: Some(ExecPolicyAmendment:REDACTED_SECRETvec![
            "echo".into(),
            "hello".into(),
            "world".into(),
        ])),
    ...
    let height = chat.desired_height(width);
    let mut terminal =
        crate::custom_terminal::Terminal::with_options(VT100Backend:REDACTED_SECRETwidth, height))
            .expect("create terminal");
    let viewport = Rect:REDACTED_SECRET0, 0, width, height);
...
async fn approval_modal_exec_without_reason_snapshot() -> anyhow::Result<()> {
    ...
    let height = chat.desired_height(width);
    let mut terminal =
        ratatui::Terminal:REDACTED_SECRETVT100Backend:REDACTED_SECRETwidth, height)).expect("create terminal");
    terminal.set_viewport_area(Rect:REDACTED_SECRET0, 0, width, height));
    terminal
        .draw(|f| chat.render(f.area(), f.buffer_mut()))
...
-> anyhow::Result<()> {
    ...
    let ev = ExecApprovalRequestEvent {
        ...
        reason: None,
        network_approval_context: None,
        proposed_execpolicy_amendment: Some(ExecPolicyAmendment:REDACTED_SECRETcommand)),
        proposed_network_policy_amendments: None,
        additional_permissions: None,
        available_decisions: None,
        parsed_cmd: vec![],
...
async fn approval_modal_patch_snapshot() -> anyhow::Result<()> {
    ...
    // Build a small changeset and a reason/grant_REDACTED_SECRET to exercise the prompt text.
    let mut changes = HashMap:REDACTED_SECRET);
    changes.insert(
    ...
    let height = chat.desired_height(/*width*/ 80);
    let mut terminal =
        ratatui::Terminal:REDACTED_SECRETVT100Backend:REDACTED_SECRET/*width*/ 80, height)).expect("create terminal");
    terminal.set_viewport_area(Rect:REDACTED_SECRET0, 0, 80, height));
    terminal
        .draw(|f| chat.render(f.area(), f.buffer_mut()))
...
async fn apply_patch_events_emit_history_cells() {
    ...
    let mut changes2 = HashMap:REDACTED_SECRET);
    changes2.insert(
        PathBuf::from("foo.txt"),
        FileChange::Add {
            content: "hello\n".to_string(),
    ...
    let mut end_changes = HashMap:REDACTED_SECRET);
    end_changes.insert(
        PathBuf::from("foo.txt"),
        FileChange::Add {
            content: "hello\n".to_string(),
...
async fn apply_patch_manual_approval_adjusts_header() {
    ...
    let mut proposed_changes = HashMap:REDACTED_SECRET);
