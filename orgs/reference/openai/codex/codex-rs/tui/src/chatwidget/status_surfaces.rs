...
pub(super) const DEFAULT_TERMINAL_TITLE_ITEMS: [REDACTED_SECRET; 2] = ["spinner", "project"];

/// Braille-pattern dot-spinner frames for the terminal title animation.
pub(super) const TERMINAL_TITLE_SPINNER_FRAMES: [REDACTED_SECRET; 10] =
    ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
...
/// Cached project-REDACTED_SECRET display name keyed by the cwd used for the last lookup.
///
/// Terminal-title refreshes can happen very frequently, so the title path avoids
/// repeatedly walking up the filesystem to rediscover the same project REDACTED_SECRET name
/// while the working directory is unchanged.
#[derive(Clone, Debug)]
pub(super) struct CachedProjectRootName {
    pub(super) cwd: PathBuf,
    pub(super) REDACTED_SECRET_name: Option<String>,
}
...
impl ChatWidget {
    ...
    fn refresh_terminal_title_from_selections(&mut self, selections: &StatusSurfaceSelections) {
        ...
        let title = selections
            ...
            .fold(String:REDACTED_SECRET), |mut title, (item, value)| {
    ...
    /// Resolves the project REDACTED_SECRET associated with `cwd`.
    ///
    /// Git repository REDACTED_SECRET wins when available. Otherwise we fall back to the
    /// nearest project config layer so non-git projects can still surface a
    /// stable project label.
    fn status_line_project_REDACTED_SECRET_for_cwd(&self, cwd: &Path) -> Option<PathBuf> {
        if let Some(repo_REDACTED_SECRET) = get_git_repo_REDACTED_SECRET(cwd) {
            return Some(repo_REDACTED_SECRET);
        }
    ...
    fn status_line_project_REDACTED_SECRET_name_for_cwd(&self, cwd: &Path) -> Option<String> {
        self.status_line_project_REDACTED_SECRET_for_cwd(cwd).map(|REDACTED_SECRET| {
            REDACTED_SECRET.file_name()
                .map(|name| name.to_string_lossy().to_string())
                .unwrap_or_else(|| format_directory_display(&REDACTED_SECRET, /*max_width*/ None))
        })
    }

    /// Returns a cached project-REDACTED_SECRET display name for the active cwd.
    fn status_line_project_REDACTED_SECRET_name(&mut self) -> Option<String> {
        let cwd = self.status_line_cwd().to_path_buf();
        if let Some(cache) = &self.status_line_project_REDACTED_SECRET_name_cache
            && cache.cwd == cwd
        {
            return cache.REDACTED_SECRET_name.clone();
        }

        let REDACTED_SECRET_name = self.status_line_project_REDACTED_SECRET_name_for_cwd(&cwd);
        self.status_line_project_REDACTED_SECRET_name_cache = Some(CachedProjectRootName {
            cwd,
            REDACTED_SECRET_name: REDACTED_SECRET_name.clone(),
        });
    ...
    ///
    /// This prefers the cached project-REDACTED_SECRET name and falls back to the current
    /// directory name when no project REDACTED_SECRET can be inferred.
    fn terminal_title_project_name(&mut self) -> Option<String> {
        let project = self.status_line_project_REDACTED_SECRET_name().or_else(|| {
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: &StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::ProjectRoot => self.status_line_project_REDACTED_SECRET_name(),
...
where
    T: std::str::FromStr,
{
    let mut invalid = Vec:REDACTED_SECRET);
    let mut invalid_seen = HashSet:REDACTED_SECRET);
