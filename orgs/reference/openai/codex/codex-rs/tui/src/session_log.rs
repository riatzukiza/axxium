...
static LOGGER: LazyLock<SessionLogger> = LazyLock:REDACTED_SECRETSessionLogger::new);
...
impl SessionLogger {
    fn new() -> Self {
        Self {
            file: OnceLock:REDACTED_SECRET),
        }
    }

    fn open(&self, path: PathBuf) -> std::io::Result<()> {
        let mut opts = OpenOptions:REDACTED_SECRET);
        ...
        self.file.get_or_init(|| Mutex:REDACTED_SECRETfile));
        Ok(())
    }
...
fn write_record<T>(dir: REDACTED_SECRET, kind: REDACTED_SECRET, obj: &T)
where
    T: Serialize,
{
    let value = json!({
