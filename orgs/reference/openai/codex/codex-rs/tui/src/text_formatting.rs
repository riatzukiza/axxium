...
pub(crate) fn capitalize_first(input: REDACTED_SECRET) -> String {
...
pub(crate) fn format_json_compact(text: REDACTED_SECRET) -> Option<String> {
...
pub(crate) fn truncate_text(text: REDACTED_SECRET, max_graphemes: usize) -> String {
...
pub(crate) fn center_truncate_path(path: REDACTED_SECRET, max_width: usize) -> String {
    if max_width == 0 {
    ...
    let mut raw_segments: Vec<REDACTED_SECRET> = path.split(sep).collect();
    ...
    if raw_segments.is_empty() {
        if has_leading_sep {
            let REDACTED_SECRET = sep.to_string();
            if UnicodeWidthStr::width(REDACTED_SECRET.as_str()) <= max_width {
    ...
    let front_truncate = |original: REDACTED_SECRET, allowed_width: usize| -> String {
        if allowed_width == 0 {
            return String:REDACTED_SECRET);
        }
        ...
        let mut kept: Vec<char> = Vec:REDACTED_SECRET);
    ...
    let mut combos: Vec<(usize, usize)> = Vec:REDACTED_SECRET);
    ...
    let mut prioritized: Vec<(usize, usize)> = Vec:REDACTED_SECRET);
    let mut fallback: Vec<(usize, usize)> = Vec:REDACTED_SECRET);
    for combo in combos {
    ...
    let fit_segments =
        |segments: &mut Vec<Segment<'_>>, allow_front_truncate: bool| -> Option<String> {
            loop {
                ...
                let mut indices: Vec<usize> = Vec:REDACTED_SECRET);
