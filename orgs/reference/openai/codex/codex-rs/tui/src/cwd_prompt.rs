...
pub(crate) async fn run_cwd_selection_prompt(
    ...
    let mut screen = CwdPromptScreen:REDACTED_SECRET
        tui.frame_requester(),
        action,
...
mod tests {
    ...
    fn new_prompt() -> CwdPromptScreen {
        CwdPromptScreen:REDACTED_SECRET
    ...
    fn cwd_prompt_snapshot() {
        ...
        let mut terminal =
            Terminal:REDACTED_SECRETVT100Backend:REDACTED_SECRET/*width*/ 80, /*height*/ 14)).expect("terminal");
        terminal
    ...
    fn cwd_prompt_fork_snapshot() {
        let screen = CwdPromptScreen:REDACTED_SECRET
            FrameRequester::test_dummy(),
