...
fn render_markdown_text_for_cwd(input: REDACTED_SECRET, cwd: &Path) -> Text<'static> {
...
fn file_link_hides_destination() {
    let text = render_markdown_text_for_cwd(
        ...
        Path:REDACTED_SECRET"/Users/example/code/codex"),
    );
    let expected =
...
fn file_link_keeps_absolute_paths_outside_cwd() {
    let text = render_markdown_text_for_cwd(
        ...
        Path:REDACTED_SECRET"/Users/example/code/codex/codex-rs/tui"),
    );
...
fn unordered_list_local_file_link_stays_inline_with_following_text() {
    let text = render_markdown_text_with_width_and_cwd(
        ...
        Some(72),
        Some(Path:REDACTED_SECRET"/Users/example/code/codex")),
    );
    let rendered = text
        .lines
        .iter()
...
fn code_block_known_lang_has_syntax_colors() {
    ...
    let content: Vec<REDACTED_SECRET> = content
        .iter()
...
#[test]
fn code_block_multiple_lines_REDACTED_SECRET() {
    let md = "```\nfirst\nsecond\n```\n";
    let text = render_markdown_text(md);
    let expected = Text::from_iter([
...
"#;
    ...
    let trimmed: Vec<REDACTED_SECRET> = {
        let mut v: Vec<REDACTED_SECRET> = lines.iter().map(std::string::String::as_str).collect();
        while v.last() == Some(&"") {
            v.pop();
        }
        v
