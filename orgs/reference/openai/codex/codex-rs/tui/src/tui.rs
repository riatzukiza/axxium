...
pub fn init() -> Result<Terminal> {
    ...
    let backend = CrosstermBackend:REDACTED_SECRETstdout());
...
fn set_panic_hook() {
    ...
    panic::set_hook(Box:REDACTED_SECRETmove |panic_info| {
...
pub struct Tui {
    ...
    draw_tx: broadcast::REDACTED_SECRETer<()>,
...
impl Tui {
    pub fn new(terminal: Terminal) -> Self {
        ...
        let frame_requester = FrameRequester:REDACTED_SECRETdraw_tx.clone());
        ...
        Self {
            frame_requester,
            draw_tx,
            event_broker: Arc:REDACTED_SECRETEventBroker:REDACTED_SECRET)),
            terminal,
            pending_history_lines: vec![],
            alt_saved_viewport: None,
            #[cfg(unix)]
            suspend_context: SuspendContext:REDACTED_SECRET),
            alt_screen_active: Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)),
            terminal_focused: Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue)),
    ...
    pub fn event_stream(&self) -> Pin<Box<dyn Stream<Item = TuiEvent> + REDACTED_SECRET + 'static>> {
        #[cfg(unix)]
        let stream = TuiEventStream:REDACTED_SECRET
    ...
    pub fn enter_alt_screen(&mut self) -> Result<()> {
        ...
        if let Ok(size) = self.terminal.size() {
            ...
            self.terminal.set_viewport_area(ratatui::layout::Rect:REDACTED_SECRET
                0,
                0,
                size.width,
                size.height,
    ...
    fn flush_pending_history_lines(
        ...
        crate::insert_history::insert_history_lines_with_mode(
            ...
            crate::insert_history::InsertHistoryMode:REDACTED_SECRETis_zellij),
        )?;
