...
pub fn paste_image_as_png() -> Result<(Vec<u8>, PastedImageInfo), PasteImageError> {
    ...
    let mut cb = arboard::Clipboard:REDACTED_SECRET)
    ...
    let mut png: Vec<u8> = Vec:REDACTED_SECRET);
    {
        let span =
            tracing::debug_span!("encode_image", byte_length = tracing::field::Empty).entered();
        let mut cursor = std::io::Cursor:REDACTED_SECRET&mut png);
        dyn_img
...
pub fn paste_image_to_temp_png() -> Result<(PathBuf, PastedImageInfo), PasteImageError> {
    ...
    match paste_image_as_png() {
        Ok((png, info)) => {
            ...
            let tmp = Builder:REDACTED_SECRET)
...
fn try_dump_windows_clipboard_image() -> Option<String> {
    ...
    for cmd in ["powershell.exe", "pwsh", "powershell"] {
        match std::process::Command:REDACTED_SECRETcmd)
...
pub fn normalize_pasted_path(pasted: REDACTED_SECRET) -> Option<PathBuf> {
    ...
    let parts: Vec<String> = shlex::Shlex:REDACTED_SECRETpasted).collect();
    if parts.len() == 1 {
...
#[cfg(target_os = "linux")]
fn convert_windows_path_to_wsl(input: REDACTED_SECRET) -> Option<PathBuf> {
...
fn normalize_windows_path(input: REDACTED_SECRET) -> Option<PathBuf> {
...
mod pasted_paths_tests {
    ...
    fn pasted_image_format_png_jpeg_unknown() {
        assert_eq!(
            pasted_image_format(Path:REDACTED_SECRET"/a/b/c.PNG")),
            EncodedImageFormat::Png
        );
        assert_eq!(
            pasted_image_format(Path:REDACTED_SECRET"/a/b/c.jpg")),
            EncodedImageFormat::Jpeg
        );
        assert_eq!(
            pasted_image_format(Path:REDACTED_SECRET"/a/b/c.JPEG")),
            EncodedImageFormat::Jpeg
        );
        assert_eq!(
            pasted_image_format(Path:REDACTED_SECRET"/a/b/c")),
            EncodedImageFormat::Other
        );
        assert_eq!(
            pasted_image_format(Path:REDACTED_SECRET"/a/b/c.webp")),
            EncodedImageFormat::Other
        );
    }
    ...
    fn pasted_image_format_with_windows_style_paths() {
        assert_eq!(
            pasted_image_format(Path:REDACTED_SECRETr"C:\\a\\b\\c.PNG")),
            EncodedImageFormat::Png
        );
        assert_eq!(
            pasted_image_format(Path:REDACTED_SECRETr"C:\\a\\b\\c.jpeg")),
            EncodedImageFormat::Jpeg
        );
        assert_eq!(
            pasted_image_format(Path:REDACTED_SECRETr"C:\\a\\b\\noext")),
            EncodedImageFormat::Other
        );
    }
