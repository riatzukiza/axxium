...
impl VoiceCapture {
    pub fn start_realtime(config: &Config, tx: AppEventREDACTED_SECRETer) -> Result<Self, String> {
        let (device, config) = select_realtime_input_device_and_config(config)?;

        let sample_rate = config.sample_rate().0;
        let channels = config.channels();
        let stopped = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        let last_peak = Arc:REDACTED_SECRETAtomicU16:REDACTED_SECRET0));
...
impl RealtimeAudioPlayer {
    pub(crate) fn start(config: &Config) -> Result<Self, String> {
        ...
        let queue = Arc:REDACTED_SECRETMutex:REDACTED_SECRETVecDeque:REDACTED_SECRET)));
