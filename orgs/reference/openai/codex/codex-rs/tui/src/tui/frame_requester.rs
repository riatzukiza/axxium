...
pub struct FrameRequester {
    frame_schedule_tx: mpsc::UnboundedREDACTED_SECRETer<Instant>,
}
...
impl FrameRequester {
    ...
    pub fn new(draw_tx: broadcast::REDACTED_SECRETer<()>) -> Self {
        let (tx, rx) = mpsc::unbounded_channel();
        let scheduler = FrameScheduler:REDACTED_SECRETrx, draw_tx);
...
impl FrameScheduler {
    ...
    fn new(receiver: mpsc::UnboundedReceiver<Instant>, draw_tx: broadcast::REDACTED_SECRETer<()>) -> Self {
        Self {
            receiver,
            draw_tx,
...
mod tests {
    ...
    async fn test_schedule_frame_immediate_triggers_once() {
        ...
        let requester = FrameRequester:REDACTED_SECRETdraw_tx);
