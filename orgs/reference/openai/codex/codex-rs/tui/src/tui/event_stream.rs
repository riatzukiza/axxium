...
pub trait EventSource: REDACTED_SECRET + 'static {
...
pub struct EventBroker<S: EventSource = CrosstermEventSource> {
    ...
    resume_events_tx: watch::REDACTED_SECRETer<()>,
}
...
impl<S: EventSource + Default> EventBroker<S> {
    pub fn new() -> Self {
        ...
        Self {
            state: Mutex:REDACTED_SECRETEventBrokerState::Start),
            resume_events_tx,
        }
    }
...
impl Default for CrosstermEventSource {
    fn default() -> Self {
        Self(crossterm::event::EventStream:REDACTED_SECRET))
    }
}

impl EventSource for CrosstermEventSource {
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<EventResult>> {
        Pin:REDACTED_SECRET&mut self.get_mut().0).poll_next(cx)
    }
}
...
impl<S: EventSource + Default + Unpin> TuiEventStream<S> {
    pub fn new(
        ...
        Self {
            broker,
            draw_stream: BroadcastStream:REDACTED_SECRETdraw_rx),
            resume_stream,
            terminal_focused,
            ...
            let poll_result = {
                ...
                let events = match state.active_event_source_mut() {
                    ...
                    None => {
                        ...
                        match Pin:REDACTED_SECRET&mut self.resume_stream).poll_next(cx) {
                ...
                match Pin:REDACTED_SECRETevents).poll_next(cx) {
    ...
    pub fn poll_draw_event(&mut self, cx: &mut Context<'_>) -> Poll<Option<TuiEvent>> {
        match Pin:REDACTED_SECRET&mut self.draw_stream).poll_next(cx) {
...
mod tests {
    ...
    struct FakeEventSource {
        ...
        tx: mpsc::UnboundedREDACTED_SECRETer<EventResult>,
    }
    ...
    impl EventSource for FakeEventSource {
        fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<EventResult>> {
            Pin:REDACTED_SECRET&mut self.get_mut().rx).poll_recv(cx)
        }
    }
    ...
    fn make_stream(
        ...
        TuiEventStream:REDACTED_SECRET
            broker,
            draw_rx,
            terminal_focused,
            #[cfg(unix)]
            crate::tui::job_control::SuspendContext:REDACTED_SECRET),
            #[cfg(unix)]
    ...
    type SetupState = (
        ...
        broadcast::REDACTED_SECRETer<()>,
        broadcast::Receiver<()>,
        Arc<AtomicBool>,
    );

    fn setup() -> SetupState {
        let source = FakeEventSource:REDACTED_SECRET);
        let broker = Arc:REDACTED_SECRETEventBroker:REDACTED_SECRET));
        *broker.state.lock().unwrap() = EventBrokerState::Running(source);
        let handle = FakeEventSourceHandle:REDACTED_SECRETbroker.clone());

        let (draw_tx, draw_rx) = broadcast::channel(1);
        let terminal_focused = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue));
    ...
    async fn key_event_skips_unmapped() {
        ...
        handle.send(Ok(Event::Key(KeyEvent:REDACTED_SECRET
            KeyCode::Char('a'),
            KeyModifiers::NONE,
        ))));
        ...
        match next {
            TuiEvent::Key(key) => {
                assert_eq!(key, KeyEvent:REDACTED_SECRETKeyCode::Char('a'), KeyModifiers::NONE));
            }
    ...
    async fn draw_and_key_events_yield_both() {
        ...
        let expected_key = KeyEvent:REDACTED_SECRETKeyCode::Char('a'), KeyModifiers::NONE);
        let _ = draw_tx.send(());
    ...
    async fn resume_wakes_paused_stream() {
        ...
        let expected_key = KeyEvent:REDACTED_SECRETKeyCode::Char('r'), KeyModifiers::NONE);
        handle.send(Ok(Event::Key(expected_key)));
    ...
    async fn resume_wakes_pending_stream() {
        ...
        broker.pause_events();
        broker.resume_events();
        let expected_key = KeyEvent:REDACTED_SECRETKeyCode::Char('p'), KeyModifiers::NONE);
        handle.send(Ok(Event::Key(expected_key)));
