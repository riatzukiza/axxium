...
impl SuspendContext {
    pub(crate) fn new() -> Self {
        Self {
            resume_pending: Arc:REDACTED_SECRETMutex:REDACTED_SECRETNone)),
            suspend_cursor_y: Arc:REDACTED_SECRETAtomicU16:REDACTED_SECRET0)),
        }
    }
    ...
    pub(crate) fn prepare_resume_action(
        ...
        match action {
            ResumeAction::RealignInline => {
                ...
                let viewport = Rect:REDACTED_SECRET0, cursor_pos.y, 0, 0);
        ...
        match self {
            ...
            PreparedResumeAction::RestoreAltScreen => {
                ...
                if let Ok(size) = terminal.size() {
                    terminal.set_viewport_area(Rect:REDACTED_SECRET0, 0, size.width, size.height));
                    terminal.clear()?;
                }
            }
        }
