...
#[cfg(windows)]
fn resolve_windows_program(program: REDACTED_SECRET) -> std::path::PathBuf {
    // On Windows, `Command:REDACTED_SECRET"code")` will not resolve `code.cmd` shims on PATH.
...
pub(crate) async fn run_editor(seed: REDACTED_SECRET, editor_cmd: &[String]) -> Result<String> {
    if editor_cmd.is_empty() {
        return Err(Report::msg("editor command is empty"));
    }

    // Convert to TempPath immediately so no file handle stays open on Windows.
    let temp_path = Builder:REDACTED_SECRET).suffix(".md").tempfile()?.into_temp_path();
    ...
    let mut cmd = {
        #[cfg(windows)]
        {
            ...
            Command:REDACTED_SECRETresolve_windows_program(&editor_cmd[0]))
        }
        #[cfg(not(windows))]
        {
            Command:REDACTED_SECRET&editor_cmd[0])
        }
    };
    if editor_cmd.len() > 1 {
...
mod tests {
    ...
    fn restore_env(key: REDACTED_SECRET, value: Option<String>) {
        match value {
    ...
    fn resolve_editor_prefers_visual() {
        let _guard = EnvGuard:REDACTED_SECRET);
        unsafe {
