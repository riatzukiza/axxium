...
where
    B: Backend + Write,
{
    let screen_size = terminal.backend().size().unwrap_or(Size:REDACTED_SECRET0, 0));
    ...
    let mut wrapped = Vec:REDACTED_SECRET);
    ...
    for line in &lines {
        let line_wrapped =
            if line_contains_url_like(line) && !line_has_mixed_url_and_non_url_tokens(line) {
                ...
                adaptive_wrap_line(line, RtOptions:REDACTED_SECRETwrap_width))
            };
...
fn write_history_line<W: Write>(writer: &mut W, line: &Line, wrap_width: usize) -> io::Result<()> {
    ...
    queue!(
        writer,
        SetColors(Colors:REDACTED_SECRET
            line.style
                .fg
    ...
    I: IntoIterator<Item = &'a Span<'a>>,
{
    ...
    for span in content {
        ...
        if next_fg != fg || next_bg != bg {
            queue!(
                writer,
                SetColors(Colors:REDACTED_SECRETnext_fg.into(), next_bg.into()))
            )?;
            fg = next_fg;
            bg = next_bg;
        }
...
mod tests {
    ...
    fn writes_bold_then_regular_spans() {
        ...
        let mut actual: Vec<u8> = Vec:REDACTED_SECRET);
        write_spans(&mut actual, spans.iter()).unwrap();

        let mut expected: Vec<u8> = Vec:REDACTED_SECRET);
        queue!(
            expected,
    ...
    fn vt100_blockquote_line_emits_green_fg() {
        ...
        let viewport = Rect:REDACTED_SECRET0, height - 1, width, 1);
    ...
    fn vt100_blockquote_wrap_preserves_color_on_all_wrapped_lines() {
        ...
        let mut non_empty_rows: Vec<u16> = Vec:REDACTED_SECRET);
        for row in 0..height {
            let mut any = false;
            for col in 0..width {
    ...
    fn vt100_deep_nested_mixed_list_third_level_marker_is_colored() {
        ...
        let backend = VT100Backend:REDACTED_SECRETwidth, height);
        let mut term = crate::custom_terminal::Terminal::with_options(backend).expect("terminal");
        let viewport = ratatui::layout::Rect:REDACTED_SECRET0, height - 1, width, 1);
        term.set_viewport_area(viewport);
    ...
    fn vt100_zellij_mode_inserts_history_and_updates_viewport() {
        ...
        let backend = VT100Backend:REDACTED_SECRETwidth, height);
        let mut term = crate::custom_terminal::Terminal::with_options(backend).expect("terminal");
        let viewport = Rect:REDACTED_SECRET0, 4, width, 2);
        term.set_viewport_area(viewport);
        ...
        assert_eq!(term.viewport_area, Rect:REDACTED_SECRET0, 5, width, 2));
