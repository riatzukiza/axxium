...
impl Overlay {
    pub(crate) fn new_transcript(cells: Vec<Arc<dyn HistoryCell>>) -> Self {
        Self::Transcript(TranscriptOverlay:REDACTED_SECRETcells))
    }
...
const PAGER_KEY_HINTS: &[(&[KeyBinding], REDACTED_SECRET)] = &[
...
fn render_key_hints(area: Rect, buf: &mut Buffer, pairs: &[(&[KeyBinding], REDACTED_SECRET)]) {
    ...
    Paragraph:REDACTED_SECRETvec![Line::from(spans).dim()]).render_ref(area, buf);
}
...
impl PagerView {
    ...
    fn render_content(&self, area: Rect, buf: &mut Buffer) {
        ...
        for renderable in &self.renderables {
            ...
            if top < 0 {
                ...
                let draw_area = Rect:REDACTED_SECRETarea.x, area.y + top as u16, area.width, draw_height);
    ...
    fn render_bottom_bar(
        ...
        let sep_rect = Rect:REDACTED_SECRETfull_area.x, sep_y, full_area.width, 1);
        ...
        Span::from(pct_text)
            .dim()
            .render_ref(Rect:REDACTED_SECRETpct_x, sep_rect.y, pct_w, 1), buf);
    }
...
impl CachedRenderable {
    fn new(renderable: impl Into<Box<dyn Renderable>>) -> Self {
        Self {
            ...
            height: std::cell::Cell:REDACTED_SECRETNone),
            last_width: std::cell::Cell:REDACTED_SECRETNone),
        }
    }
}
...
impl Renderable for CellRenderable {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        let p = Paragraph:REDACTED_SECRETText::from(self.cell.transcript_lines(area.width)))
            .style(self.style)
...
impl TranscriptOverlay {
    ...
    pub(crate) fn new(transcript_cells: Vec<Arc<dyn HistoryCell>>) -> Self {
        Self {
            view: PagerView:REDACTED_SECRET
    ...
    fn render_cells(
        ...
        cells
            ...
            .enumerate()
            .flat_map(|(i, c)| {
                let mut v: Vec<Box<dyn Renderable>> = Vec:REDACTED_SECRET);
                let mut cell_renderable = if c.as_any().is::<UserHistoryCell>() {
                    Box:REDACTED_SECRETCachedRenderable:REDACTED_SECRETCellRenderable {
                        cell: c.clone(),
                ...
                if !c.is_stream_continuation() && i > 0 {
                    cell_renderable = Box:REDACTED_SECRETInsetRenderable:REDACTED_SECRET
                        cell_renderable,
                        Insets::tlbr(
    ...
    pub(crate) fn insert_cell(&mut self, cell: Arc<dyn HistoryCell>) {
        ...
        if let Some(tail) = tail_renderable {
                    ...
                    .is_some_and(|key| !key.is_stream_continuation)
            {
                ...
                Box:REDACTED_SECRETInsetRenderable:REDACTED_SECRET
                    tail,
                    Insets::tlbr(
                        /*top*/ 1, /*left*/ 0, /*bottom*/ 0, /*right*/ 0,
                    ),
    ...
    fn live_tail_renderable(
        ...
        let mut renderable: Box<dyn Renderable> = Box:REDACTED_SECRETCachedRenderable:REDACTED_SECRETparagraph));
        if has_prior_cells && !is_stream_continuation {
            renderable = Box:REDACTED_SECRETInsetRenderable:REDACTED_SECRET
                renderable,
                Insets::tlbr(
                    /*top*/ 1, /*left*/ 0, /*bottom*/ 0, /*right*/ 0,
                ),
    ...
    fn render_hints(&self, area: Rect, buf: &mut Buffer) {
        let line1 = Rect:REDACTED_SECRETarea.x, area.y, area.width, 1);
        let line2 = Rect:REDACTED_SECRETarea.x, area.y.saturating_add(1), area.width, 1);
        render_key_hints(line1, buf, PAGER_KEY_HINTS);

        let mut pairs: Vec<(&[KeyBinding], REDACTED_SECRET)> = vec![(&[KEY_Q], "to quit")];
    ...
    pub(crate) fn render(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        let top = Rect:REDACTED_SECRETarea.x, area.y, area.width, top_h);
        let bottom = Rect:REDACTED_SECRETarea.x, area.y + top_h, area.width, 3);
...
impl StaticOverlay {
    pub(crate) fn with_title(lines: Vec<Line<'static>>, title: String) -> Self {
        let paragraph = Paragraph:REDACTED_SECRETText::from(lines)).wrap(Wrap { trim: false });
        Self::with_renderables(vec![Box:REDACTED_SECRETCachedRenderable:REDACTED_SECRETparagraph))], title)
    }

    pub(crate) fn with_renderables(renderables: Vec<Box<dyn Renderable>>, title: String) -> Self {
        Self {
            view: PagerView:REDACTED_SECRETrenderables, title, /*scroll_offset*/ 0),
            is_done: false,
        }
    }
    ...
    fn render_hints(&self, area: Rect, buf: &mut Buffer) {
        ...
        let line2 = Rect:REDACTED_SECRETarea.x, area.y.saturating_add(1), area.width, 1);
        render_key_hints(line1, buf, PAGER_KEY_HINTS);
        let pairs: Vec<(&[KeyBinding], REDACTED_SECRET)> = vec![(&[KEY_Q], "to quit")];
        render_key_hints(line2, buf, &pairs);
    }
...
fn render_offset_content(
    ...
    let mut tall_buf = Buffer::empty(Rect:REDACTED_SECRET
        0,
        0,
        area.width,
...
mod tests {
    ...
    fn paragraph_block(label: REDACTED_SECRET, lines: usize) -> Box<dyn Renderable> {
        let text = Text::from(
            (0..lines)
                .map(|i| Line::from(format!("{label}{i}")))
                .collect::<Vec<_>>(),
        );
        Box:REDACTED_SECRETParagraph:REDACTED_SECRETtext)) as Box<dyn Renderable>
    }

    #[test]
    fn edit_prev_hint_is_visible() {
        let mut overlay = TranscriptOverlay:REDACTED_SECRETvec![Arc:REDACTED_SECRETTestCell {
    ...
    fn transcript_overlay_snapshot_basic() {
        ...
        let mut overlay = TranscriptOverlay:REDACTED_SECRETvec![
            Arc:REDACTED_SECRETTestCell {
        ...
        let mut term = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET40, 10)).expect("term");
    ...
    fn transcript_overlay_sync_live_tail_is_noop_for_identical_key() {
        ...
        let calls = std::cell::Cell:REDACTED_SECRET0usize);
    ...
    fn transcript_overlay_apply_patch_scroll_vt100_clears_previous_page() {
        ...
        let mut cells: Vec<Arc<dyn HistoryCell>> = Vec:REDACTED_SECRET);

        let mut approval_changes = HashMap:REDACTED_SECRET);
        ...
        let approval_cell: Arc<dyn HistoryCell> = Arc:REDACTED_SECRETnew_patch_event(approval_changes, &cwd));
        cells.push(approval_cell);

        let mut apply_changes = HashMap:REDACTED_SECRET);
        ...
        let apply_begin_cell: Arc<dyn HistoryCell> = Arc:REDACTED_SECRETnew_patch_event(apply_changes, &cwd));
        ...
        let exec_cell: Arc<dyn HistoryCell> = Arc:REDACTED_SECRETexec_cell);
        cells.push(exec_cell);

        let mut overlay = TranscriptOverlay:REDACTED_SECRETcells);
    ...
    fn transcript_overlay_keeps_scroll_pinned_at_bottom() {
        let mut overlay = TranscriptOverlay:REDACTED_SECRET
            (0..20)
                .map(|i| {
                    Arc:REDACTED_SECRETTestCell {
        ...
        let mut term = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET40, 12)).expect("term");
        term.draw(|f| overlay.render(f.area(), f.buffer_mut()))
            .expect("draw");
        ...
        overlay.insert_cell(Arc:REDACTED_SECRETTestCell {
            lines: vec!["tail".into()],
        }));
    ...
    fn transcript_line_numbers(overlay: &mut TranscriptOverlay, area: Rect) -> Vec<usize> {
        ...
        let mut nums = Vec:REDACTED_SECRET);
    ...
    fn transcript_overlay_paging_is_continuous_and_round_trips() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 40, 15);
    ...
    fn static_overlay_wraps_long_lines() {
        ...
        let mut term = Terminal:REDACTED_SECRETTestBackend:REDACTED_SECRET24, 8)).expect("term");
        term.draw(|f| overlay.render(f.area(), f.buffer_mut()))
            .expect("draw");
        assert_snapshot!(term.backend());
    }
    ...
    fn pager_view_content_height_counts_renderables() {
        let pv = PagerView:REDACTED_SECRET
            vec![
    ...
    fn pager_view_ensure_chunk_visible_scrolls_down_when_needed() {
        let mut pv = PagerView:REDACTED_SECRET
            vec![
        ...
        let area = Rect:REDACTED_SECRET0, 0, 20, 8);
    ...
    fn pager_view_ensure_chunk_visible_scrolls_up_when_needed() {
        ...
        let area = Rect:REDACTED_SECRET0, 0, 20, 3);
