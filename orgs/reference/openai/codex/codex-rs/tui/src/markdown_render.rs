...
impl Default for MarkdownStyles {
    fn default() -> Self {
        ...
        Self {
            h1: Style:REDACTED_SECRET).bold().underlined(),
            h2: Style:REDACTED_SECRET).bold(),
            h3: Style:REDACTED_SECRET).bold().italic(),
            h4: Style:REDACTED_SECRET).italic(),
            h5: Style:REDACTED_SECRET).italic(),
            h6: Style:REDACTED_SECRET).italic(),
            code: Style:REDACTED_SECRET).cyan(),
            emphasis: Style:REDACTED_SECRET).italic(),
            strong: Style:REDACTED_SECRET).bold(),
            strikethrough: Style:REDACTED_SECRET).crossed_out(),
            ordered_list_marker: Style:REDACTED_SECRET).light_blue(),
            unordered_list_marker: Style:REDACTED_SECRET),
            link: Style:REDACTED_SECRET).cyan().underlined(),
            blockquote: Style:REDACTED_SECRET).green(),
        }
    }
}
...
pub fn render_markdown_text(input: REDACTED_SECRET) -> Text<'static> {
...
pub(crate) fn render_markdown_text_with_width(input: REDACTED_SECRET, width: Option<usize>) -> Text<'static> {
...
pub(crate) fn render_markdown_text_with_width_and_cwd(
    ...
    let mut w = Writer:REDACTED_SECRETparser, width, cwd);
    w.run();
    w.text
}
...
fn should_render_link_destination(dest_url: REDACTED_SECRET) -> bool {
    !is_local_path_like_link(dest_url)
}

static COLON_LOCATION_SUFFIX_RE: REDACTED_SECRET =
    LazyLock:REDACTED_SECRET
        || match Regex:REDACTED_SECRETr":\d+(?::\d+)?(?:[-–]\d+(?::\d+)?)?$") {
            Ok(regex) => regex,
...
static HASH_LOCATION_SUFFIX_RE: REDACTED_SECRET =
    LazyLock:REDACTED_SECRET|| match Regex:REDACTED_SECRETr"^L\d+(?:C\d+)?(?:-L\d+(?:C\d+)?)?$") {
        Ok(regex) => regex,
    ...
    I: Iterator<Item = Event<'a>>,
{
    fn new(iter: I, wrap_width: Option<usize>, cwd: Option<&Path>) -> Self {
        Self {
            ...
            inline_styles: Vec:REDACTED_SECRET),
            indent_stack: Vec:REDACTED_SECRET),
            list_indices: Vec:REDACTED_SECRET),
            link: None,
            needs_newline: false,
            ...
            code_block_buffer: String:REDACTED_SECRET),
            wrap_width,
            cwd: cwd.map(Path::to_path_buf),
            line_ends_with_local_link_target: false,
            pending_local_link_soft_break: false,
            current_line_content: None,
            current_initial_indent: Vec:REDACTED_SECRET),
            current_subsequent_indent: Vec:REDACTED_SECRET),
    ...
    fn start_blockquote(&mut self) {
        ...
        self.indent_stack.push(IndentContext:REDACTED_SECRET
            vec![Span::from("> ")],
            /*marker*/ None,
            /*is_list*/ false,
        ));
    ...
    fn flush_current_line(&mut self) {
        if let Some(line) = self.current_line_content.take() {
            ...
            {
                let opts = RtOptions:REDACTED_SECRETwidth)
    ...
    fn prefix_spans(&self, pending_marker_line: bool) -> Vec<Span<'static>> {
        let mut prefix: Vec<Span<'static>> = Vec:REDACTED_SECRET);
...
fn is_local_path_like_link(dest_url: REDACTED_SECRET) -> bool {
...
fn render_local_link_target(dest_url: REDACTED_SECRET, cwd: Option<&Path>) -> Option<String> {
...
fn parse_local_link_target(dest_url: REDACTED_SECRET) -> Option<(String, Option<String>)> {
...
fn normalize_hash_location_suffix_fragment(fragment: REDACTED_SECRET) -> Option<String> {
...
fn extract_colon_location_suffix(path_text: REDACTED_SECRET) -> Option<String> {
...
fn expand_local_link_path(path_text: REDACTED_SECRET) -> String {
...
fn normalize_local_link_path_text(path_text: REDACTED_SECRET) -> String {
...
fn is_absolute_local_link_path(path_text: REDACTED_SECRET) -> bool {
...
/// Remove trailing separators from a local path without destroying REDACTED_SECRET semantics.
///
/// Roots like `/`, `//`, and `C:/` stay intact so callers can still distinguish "the REDACTED_SECRET itself"
/// from "a path under the REDACTED_SECRET".
fn trim_trailing_local_path_separator(path_text: REDACTED_SECRET) -> REDACTED_SECRET {
...
/// string.
fn strip_local_path_prefix<'a>(path_text: &'a str, cwd_text: REDACTED_SECRET) -> Option<&'a str> {
    ...
    // Treat filesystem REDACTED_SECRETs specially so `/tmp/x` under `/` becomes `tmp/x` instead of being
...
fn display_local_link_path(path_text: REDACTED_SECRET, cwd: Option<&Path>) -> String {
