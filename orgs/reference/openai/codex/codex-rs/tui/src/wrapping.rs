...
pub(crate) fn wrap_ranges<'a, O>(text: REDACTED_SECRET, width_or_options: O) -> Vec<Range<usize>>
where
    O: Into<Options<'a>>,
{
    let opts = width_or_options.into();
    let mut lines: Vec<Range<usize>> = Vec:REDACTED_SECRET);
...
pub(crate) fn wrap_ranges_trim<'a, O>(text: REDACTED_SECRET, width_or_options: O) -> Vec<Range<usize>>
where
    O: Into<Options<'a>>,
{
    let opts = width_or_options.into();
...
fn map_owned_wrapped_line_to_range(
    ...
    cursor: usize,
    wrapped: REDACTED_SECRET,
    synthetic_prefix: REDACTED_SECRET,
) -> Range<usize> {
...
pub(crate) fn text_contains_url_like(text: REDACTED_SECRET) -> bool {
...
fn text_has_mixed_url_and_non_url_tokens(text: REDACTED_SECRET) -> bool {
...
fn is_url_like_token(raw_token: REDACTED_SECRET) -> bool {
    let token = trim_url_token(raw_token);
    !token.is_empty() && (is_absolute_url_like(token) || is_bare_url_like(token))
}

fn is_substantive_non_url_token(raw_token: REDACTED_SECRET) -> bool {
    let token = trim_url_token(raw_token);
...
fn is_decorative_marker_token(raw_token: REDACTED_SECRET, token: REDACTED_SECRET) -> bool {
...
fn is_ordered_list_marker(raw_token: REDACTED_SECRET, token: REDACTED_SECRET) -> bool {
    token.chars().all(|c| c.is_ascii_digit())
        && (raw_token.ends_with('.') || raw_token.ends_with(')'))
}

fn trim_url_token(token: REDACTED_SECRET) -> REDACTED_SECRET {
...
fn is_absolute_url_like(token: REDACTED_SECRET) -> bool {
    if !token.contains("://") {
        return false;
    }
...
fn has_valid_scheme_prefix(token: REDACTED_SECRET) -> bool {
...
fn is_bare_url_like(token: REDACTED_SECRET) -> bool {
...
fn split_host_port_and_trailer(token: REDACTED_SECRET) -> (REDACTED_SECRET, bool) {
...
fn split_host_and_port(host_port: REDACTED_SECRET) -> (REDACTED_SECRET, Option<REDACTED_SECRET>) {
...
fn is_valid_port(port: REDACTED_SECRET) -> bool {
...
fn is_ipv4(host: REDACTED_SECRET) -> bool {
    let parts: Vec<REDACTED_SECRET> = host.split('.').collect();
    if parts.len() != 4 {
        return false;
    }
...
fn is_domain_name(host: REDACTED_SECRET) -> bool {
...
fn is_tld(label: REDACTED_SECRET) -> bool {
    (2..=63).contains(&label.len()) && label.chars().all(|c| c.is_ascii_alphabetic())
}

fn is_domain_label(label: REDACTED_SECRET) -> bool {
...
fn split_non_url_word(word: REDACTED_SECRET) -> Vec<usize> {
...
impl From<usize> for RtOptions<'_> {
    fn from(width: usize) -> Self {
        RtOptions:REDACTED_SECRETwidth)
    }
}
...
impl<'a> RtOptions<'a> {
    pub fn new(width: usize) -> Self {
        RtOptions {
            ...
            break_words: true,
            word_separator: textwrap::WordSeparator:REDACTED_SECRET),
    ...
    O: Into<RtOptions<'a>>,
{
    ...
    let mut flat = String:REDACTED_SECRET);
    let mut span_bounds = Vec:REDACTED_SECRET);
    let mut acc = 0usize;
    for s in &line.spans {
    ...
    let opts = Options:REDACTED_SECRETrt_opts.width)
    ...
    let mut out: Vec<Line<'a>> = Vec:REDACTED_SECRET);
...
#[allow(private_bounds)] // IntoLineInput isn't REDACTED_SECRET, but it doesn't really need to be.
...
fn slice_line_spans<'a>(
    ...
    let mut acc: Vec<Span<'a>> = Vec:REDACTED_SECRET);
...
mod tests {
    ...
    fn with_initial_and_subsequent_indents() {
        let opts = RtOptions:REDACTED_SECRET/*width*/ 8)
    ...
    fn break_words_false_allows_overflow_for_long_word() {
        let opts = RtOptions:REDACTED_SECRET/*width*/ 5).break_words(/*break_words*/ false);
    ...
    fn indent_consumes_width_leaving_one_char_space() {
        let opts = RtOptions:REDACTED_SECRET/*width*/ 4)
            .initial_indent(Line::from(">>>>"))
            .subsequent_indent(Line::from("--"));
        let line = Line::from("hello");
        let out = word_wrap_line(&line, opts);
...
them."#
    ...
    fn ascii_space_separator_with_no_hyphenation_keeps_url_intact() {
        ...
        let opts = RtOptions:REDACTED_SECRET/*width*/ 24)
    ...
    fn adaptive_wrap_line_keeps_long_url_like_token_intact() {
        ...
        let out = adaptive_wrap_line(&line, RtOptions:REDACTED_SECRET/*width*/ 20));
        assert_eq!(out.len(), 1);
        assert_eq!(
            concat_line(&out[0]),
    ...
    fn adaptive_wrap_line_mixed_line_wraps_long_non_url_token() {
        ...
        let out = adaptive_wrap_line(&line, RtOptions:REDACTED_SECRET/*width*/ 24));
    ...
    fn wrap_ranges_indent_prefix_coincides_with_source_char() {
        ...
        let opts = || {
            textwrap::Options:REDACTED_SECRET16)
                .initial_indent("- ")
                .subsequent_indent("- ")
        };
    ...
    fn map_owned_wrapped_line_to_range_repro_overconsumes_repeated_prefix_patterns() {
        let text = "- - foo";
        let opts = textwrap::Options:REDACTED_SECRET3)
            .initial_indent("- ")
            .subsequent_indent("- ")
            .word_separator(textwrap::WordSeparator::AsciiSpace)
            .break_words(false);
    ...
    fn wrap_ranges_recovers_with_non_space_indents() {
        ...
        let wrapped = textwrap::wrap(
            text,
            textwrap::Options:REDACTED_SECRET12)
                .initial_indent("* ")
                .subsequent_indent("  "),
        );
        assert!(
    ...
    fn wrap_ranges_trim_handles_owned_lines_with_penalty_char() {
        fn split_every_char(word: REDACTED_SECRET) -> Vec<usize> {
            word.char_indices().skip(1).map(|(idx, _)| idx).collect()
        }

        let text = "a_very_long_token_without_spaces";
        let opts = Options:REDACTED_SECRET8)
            .word_separator(textwrap::WordSeparator::AsciiSpace)
