...
fn format_unified_exec_interaction(command: &[String], input: Option<REDACTED_SECRET>) -> String {
...
fn summarize_interaction_input(input: REDACTED_SECRET) -> String {
    ...
    let mut preview = String:REDACTED_SECRET);
...
pub(crate) fn output_lines(
    ...
    let lines: Vec<REDACTED_SECRET> = src.lines().collect();
    let total = lines.len();
        ...
        for (i, call) in self.iter_calls().enumerate() {
            ...
            if let Some(output) = call.output.as_ref() {
                if !call.is_unified_exec_interaction() {
                    ...
                    let wrap_opts = RtOptions:REDACTED_SECRETwrap_width);
...
impl ExecCell {
    fn exploring_display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let mut out_indented = Vec:REDACTED_SECRET);
        while !calls.is_empty() {
            ...
            let call_lines: Vec<(REDACTED_SECRET, Vec<Span<'static>>)> = if reads_only {
                let names = call
                    .parsed
                    .iter()
    ...
    fn command_display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let continuation_opts =
            RtOptions:REDACTED_SECRETcontinuation_wrap_width).word_splitter(WordSplitter::NoHyphenation);

        let mut continuation_lines: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        ...
        if let Some((first, rest)) = highlighted_lines.split_first() {
            ...
            let first_opts =
                RtOptions:REDACTED_SECRETavailable_first_width).word_splitter(WordSplitter::NoHyphenation);

            let mut first_wrapped: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        ...
        if let Some(output) = call.output.as_ref() {
            ...
            if raw_output.lines.is_empty() {
                ...
                let mut wrapped_output: Vec<Line<'static>> = Vec:REDACTED_SECRET);
                let output_wrap_width = layout.output_block.wrap_width(width);
                let output_opts =
                    RtOptions:REDACTED_SECREToutput_wrap_width).word_splitter(WordSplitter::NoHyphenation);
    ...
    fn truncate_lines_middle(
        ...
        let line_rows: Vec<usize> = lines
            ...
            .map(|line| {
                ...
                if is_whitespace_only {
                    ...
                    Paragraph:REDACTED_SECRETText::from(vec![line.clone()]))
        ...
        let mut head_lines: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        ...
        let mut tail_lines_reversed: Vec<Line<'static>> = Vec:REDACTED_SECRET);
...
const EXEC_DISPLAY_LAYOUT: ExecDisplayLayout = ExecDisplayLayout:REDACTED_SECRET
    PrefixedBlock:REDACTED_SECRET"  │ ", "  │ "),
...
mod tests {
    ...
    fn user_shell_output_is_limited_by_screen_lines() {
        ...
        let output_opts =
            RtOptions:REDACTED_SECREToutput_wrap_width).word_splitter(WordSplitter::NoHyphenation);
        let mut full_wrapped_output: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        for line in &raw_output.lines {
            push_owned_lines(
                &adaptive_wrap_line(line, output_opts.clone()),
        ...
        let full_screen_lines = Paragraph:REDACTED_SECRETText::from(full_prefixed_output))
            .wrap(Wrap { trim: false })
            .line_count(width);
        ...
        let cell = ExecCell:REDACTED_SECRETcall, /*animations_enabled*/ false);

        // Use a narrow width so each logical line wraps into many on-screen lines.
        let lines = cell.command_display_lines(width);
        let rendered_rows = Paragraph:REDACTED_SECRETText::from(lines.clone()))
            .wrap(Wrap { trim: false })
            .line_count(width);
        let header_rows = Paragraph:REDACTED_SECRETText::from(vec![lines[0].clone()]))
            .wrap(Wrap { trim: false })
            .line_count(width);
