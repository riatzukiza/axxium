...
const ANNOUNCEMENT_TIP_URL: REDACTED_SECRET =
    "https://raw.githubusercontent.com/openai/codex/main/announcement_tip.toml";

const IS_MACOS: bool = cfg!(target_os = "macos");
const IS_WINDOWS: bool = cfg!(target_os = "windows");

const PAID_TOOLTIP: REDACTED_SECRET = "*New* Try the **Codex App** with 2x rate limits until *April 2nd*. Run 'codex app' or visit https://chatgpt.com/codex?app-landing-page=true";
const PAID_TOOLTIP_WINDOWS: REDACTED_SECRET = "*New* Try the **Codex App**, now available on **Windows**, with 2x rate limits until *April 2nd*. Run 'codex app' or visit https://chatgpt.com/codex?app-landing-page=true";
const PAID_TOOLTIP_NON_MAC: REDACTED_SECRET = "*New* 2x rate limits until *April 2nd*.";
const FAST_TOOLTIP: REDACTED_SECRET = "*New* Use **/fast** to enable our fastest inference at 2X plan usage.";
const OTHER_TOOLTIP: REDACTED_SECRET = "*New* Build faster with the **Codex App**. Run 'codex app' or visit https://chatgpt.com/codex?app-landing-page=true";
const OTHER_TOOLTIP_NON_MAC: REDACTED_SECRET = "*New* Build faster with Codex.";
const FREE_GO_TOOLTIP: REDACTED_SECRET =
    "*New* For a limited time, Codex is included in your plan for free – let’s build together.";

const RAW_TOOLTIPS: REDACTED_SECRET = include_str!("../tooltips.txt");
...
pub(crate) mod announcement {
    ...
    static ANNOUNCEMENT_TIP: OnceLock<Option<String>> = OnceLock:REDACTED_SECRET);
    ...
    pub(crate) fn parse_announcement_tip_toml(text: REDACTED_SECRET) -> Option<String> {
    ...
    impl AnnouncementTip {
        fn from_raw(raw: AnnouncementTipRaw) -> Option<Self> {
            ...
            let version_regex = match raw.version_regex {
                Some(pattern) => Some(Regex:REDACTED_SECRET&pattern).ok()?),
                None => None,
            };
        ...
        fn version_matches(&self, version: REDACTED_SECRET) -> bool {
            self.version_regex
                .as_ref()
...
mod tests {
    ...
    fn paid_tooltip_pool_rotates_between_promos() {
        let mut seen = std::collections::BTreeSet:REDACTED_SECRET);
        for seed in 0..32 {
