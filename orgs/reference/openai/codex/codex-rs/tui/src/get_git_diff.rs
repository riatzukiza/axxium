...
//! repository, the function returns `Ok((false, String:REDACTED_SECRET)))`.
...
pub(crate) async fn get_git_diff() -> io::Result<(bool, String)> {
    ...
    if !inside_git_repo().await? {
        return Ok((false, String:REDACTED_SECRET)));
    }
    ...
    let mut untracked_diff = String:REDACTED_SECRET);
    let null_device: &Path = if cfg!(windows) {
        Path:REDACTED_SECRET"NUL")
    } else {
        Path:REDACTED_SECRET"/dev/null")
    };

    let null_path = null_device.to_str().unwrap_or("/dev/null").to_string();
    let mut join_set: tokio::task::JoinSet<io::Result<String>> = tokio::task::JoinSet:REDACTED_SECRET);
...
async fn run_git_capture_stdout(args: &[REDACTED_SECRET]) -> io::Result<String> {
...
async fn run_git_capture_diff(args: &[REDACTED_SECRET]) -> io::Result<String> {
    let output = Command:REDACTED_SECRET"git")
        .args(args)
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
