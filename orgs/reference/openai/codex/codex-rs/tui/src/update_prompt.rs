...
pub(crate) async fn run_update_prompt_if_needed(
    ...
    let mut screen =
        UpdatePromptScreen:REDACTED_SECRETtui.frame_requester(), latest_version.clone(), update_action);
...
impl UpdatePromptScreen {
    ...
    fn latest_version(&self) -> REDACTED_SECRET {
...
mod tests {
    ...
    fn new_prompt() -> UpdatePromptScreen {
        UpdatePromptScreen:REDACTED_SECRET
    ...
    fn update_prompt_snapshot() {
        ...
        let mut terminal = Terminal:REDACTED_SECRETVT100Backend:REDACTED_SECRET80, 12)).expect("terminal");
        terminal
    ...
    fn update_prompt_confirm_selects_update() {
        ...
        screen.handle_key(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
    ...
    fn update_prompt_dismiss_option_leaves_prompt_in_normal_state() {
        let mut screen = new_prompt();
        screen.handle_key(KeyEvent:REDACTED_SECRETKeyCode::Down, KeyModifiers::NONE));
        screen.handle_key(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
        assert!(screen.is_done());
    ...
    fn update_prompt_ctrl_c_skips_update() {
        let mut screen = new_prompt();
        screen.handle_key(KeyEvent:REDACTED_SECRETKeyCode::Char('c'), KeyModifiers::CONTROL));
        assert!(screen.is_done());
        assert_eq!(screen.selection(), Some(UpdateSelection::NotNow));
    }
    ...
    fn update_prompt_navigation_wraps_between_entries() {
        let mut screen = new_prompt();
        screen.handle_key(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::NONE));
