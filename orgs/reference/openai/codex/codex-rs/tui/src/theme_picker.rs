...
const NARROW_PREVIEW_ROWS: [PreviewRow; 4] = [
    PreviewRow {
        ...
        code: "fn greet(name: REDACTED_SECRET) -> String {",
    },
    PreviewRow {
        line_no: 13,
...
const WIDE_PREVIEW_ROWS: [PreviewRow; 8] = [
    ...
    PreviewRow {
        line_no: 33,
        kind: PreviewDiffKind::Context,
        code: "    let names: Vec<REDACTED_SECRET> = users.iter().map(User::name).take(3).collect();",
    },
    PreviewRow {
        line_no: 34,
        kind: PreviewDiffKind::Removed,
...
const PREVIEW_FALLBACK_SUBTITLE: REDACTED_SECRET = "Move up/down to live preview themes";
...
fn render_preview(
    ...
    for (idx, row) in preview_rows.iter().enumerate() {
        ...
        first_line.render(
            Rect:REDACTED_SECRETarea.x.saturating_add(left_pad), y, render_width, 1),
            buf,
        );
        y += 1;
    }
...
pub(crate) fn build_theme_picker_params(
    current_name: Option<REDACTED_SECRET>,
    ...
    let on_selection_changed = Some(Box:REDACTED_SECRETmove |idx: usize, _tx: &_| {
        ...
        as Box<dyn Fn(usize, &crate::app_event_sender::AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync>);

    // Restore original theme on cancel.
    let on_cancel = Some(Box:REDACTED_SECRETmove |_tx: &_| {
        highlight::set_syntax_theme(original_theme.clone());
    })
        as Box<dyn Fn(&crate::app_event_sender::AppEventREDACTED_SECRETer) + REDACTED_SECRET + Sync>);
    SelectionViewParams {
        ...
        side_content: Box:REDACTED_SECRETThemePreviewWideRenderable),
        side_content_width: SideContentWidth::Half,
        side_content_min_width: WIDE_PREVIEW_MIN_WIDTH,
        stacked_side_content: Some(Box:REDACTED_SECRETThemePreviewNarrowRenderable)),
...
mod tests {
    ...
    fn preview_line_number(line: REDACTED_SECRET) -> Option<usize> {
    ...
    fn preview_line_marker(line: REDACTED_SECRET) -> Option<char> {
        let trimmed = line.trim_start();
        let digits_len = trimmed.chars().take_while(char::is_ascii_digit).count();
        if digits_len == 0 {
            return None;
