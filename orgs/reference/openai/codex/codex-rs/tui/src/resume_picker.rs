...
type PageLoader = Arc<dyn Fn(PageLoadRequest) + REDACTED_SECRET + Sync>;
...
async fn run_session_picker_with_loader(
    ...
    let mut state = PickerState:REDACTED_SECRET
    ...
    let mut background_events = UnboundedReceiverStream:REDACTED_SECRETbg_rx).fuse();
...
fn spawn_rollout_page_loader(
    config: &Config,
    bg_tx: mpsc::UnboundedREDACTED_SECRETer<BackgroundEvent>,
) -> PageLoader {
    let config = config.clone();
    let loader_tx = bg_tx;
    Arc:REDACTED_SECRETmove |request: PageLoadRequest| {
...
impl Row {
    ...
    fn display_preview(&self) -> REDACTED_SECRET {
        self.thread_name.as_deref().unwrap_or(&self.preview)
    }

    fn matches_query(&self, query: REDACTED_SECRET) -> bool {
...
impl PickerState {
    fn new(
        ...
        Self {
            ...
            all_rows: Vec:REDACTED_SECRET),
            filtered_rows: Vec:REDACTED_SECRET),
            seen_rows: HashSet:REDACTED_SECRET),
            selected: 0,
            scroll_top: 0,
            query: String:REDACTED_SECRET),
            ...
            thread_name_cache: HashMap:REDACTED_SECRET),
            inline_error: None,
        }
    }
    ...
    async fn update_thread_names(&mut self) {
        let mut missing_ids = HashSet:REDACTED_SECRET);
...
#[cfg_attr(not(test), allow(dead_code))]
fn parse_timestamp_str(ts: REDACTED_SECRET) -> Option<DateTime<Utc>> {
...
fn render_list(
        ...
        .enumerate()
    {
        ...
        let rect = Rect:REDACTED_SECRETarea.x, y, area.width, 1);
...
fn calculate_column_metrics(rows: &[Row], include_cwd: bool) -> ColumnMetrics {
    fn right_elide(s: REDACTED_SECRET, max: usize) -> String {
    ...
    for row in rows {
        ...
        let cwd = if include_cwd {
            let cwd_raw = row
                .cwd
                .as_ref()
                .map(|p| display_path_for(p, std::path::Path:REDACTED_SECRET"/")))
                .unwrap_or_default();
...
mod tests {
    ...
    fn make_item(path: REDACTED_SECRET, ts: REDACTED_SECRET, preview: REDACTED_SECRET) -> ThreadItem {
        ThreadItem {
    ...
    fn cursor_from_str(repr: REDACTED_SECRET) -> Cursor {
    ...
    fn set_rollout_mtime(path: &Path, updated_at: DateTime<Utc>) {
        let times = FileTimes:REDACTED_SECRET).set_modified(updated_at.into());
    ...
    //     let sessions_REDACTED_SECRET = tempdir.path().join("sessions");
    //     std::fs::create_dir_all(&sessions_REDACTED_SECRET).expect("mkdir sessions REDACTED_SECRET");
    //
    //     let now = Utc::now();
    //
    //     let write_rollout = |ts: DateTime<Utc>, preview: REDACTED_SECRET| -> PathBuf {
    //         let dir = sessions_REDACTED_SECRET
    ...
    fn remote_thread_list_params_omit_provider_filter() {
        let params = thread_list_params(
            ...
            Some(Path:REDACTED_SECRET"repo/on/server")),
            ProviderFilter::Any,
            ThreadSortKey::UpdatedAt,
    ...
    fn remote_picker_does_not_filter_rows_by_local_cwd() {
        let loader: PageLoader = Arc:REDACTED_SECRET|_| {});
        let state = PickerState:REDACTED_SECRET
            PathBuf::from("/tmp"),
        ...
        let row = Row {
            ...
            thread_id: Some(ThreadId:REDACTED_SECRET)),
            thread_name: None,
            created_at: None,
            updated_at: None,
    ...
    fn resume_table_snapshot() {
        ...
        let width: u16 = 80;
        let height: u16 = 6;
        let backend = VT100Backend:REDACTED_SECRETwidth, height);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect:REDACTED_SECRET0, 0, width, height));
    ...
    //     let write_rollout = |ts: DateTime<Utc>, cwd: REDACTED_SECRET, branch: REDACTED_SECRET, preview: REDACTED_SECRET| {
    //         let dir = sessions_REDACTED_SECRET
    //             .join(ts.format("%Y").to_string())
    //             .join(ts.format("%m").to_string())
    //             .join(ts.format("%d").to_string());
    ...
    fn ensure_minimum_rows_prefetches_when_underfilled() {
        let recorded_requests: Arc<Mutex<Vec<PageLoadRequest>>> = Arc:REDACTED_SECRETMutex:REDACTED_SECRETVec:REDACTED_SECRET)));
        let request_sink = recorded_requests.clone();
        let loader: PageLoader = Arc:REDACTED_SECRETmove |req: PageLoadRequest| {
    ...
    async fn toggle_sort_key_reloads_with_new_sort() {
        ...
        state
            .handle_key(KeyEvent:REDACTED_SECRETKeyCode::Tab, KeyModifiers::NONE))
            .await
            .unwrap();
    ...
    async fn page_navigation_uses_view_rows() {
        ...
        state
            .handle_key(KeyEvent:REDACTED_SECRETKeyCode::PageDown, KeyModifiers::NONE))
            .await
            .unwrap();
        ...
        state
            .handle_key(KeyEvent:REDACTED_SECRETKeyCode::PageUp, KeyModifiers::NONE))
            .await
            .unwrap();
        assert_eq!(state.selected, 5);
    }
    ...
    async fn enter_on_row_without_resolvable_thread_id_shows_inline_error() {
        ...
        let selection = state
            .handle_key(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE))
            .await
    ...
    async fn up_at_bottom_does_not_scroll_when_visible() {
        ...
        state
            .handle_key(KeyEvent:REDACTED_SECRETKeyCode::Up, KeyModifiers::NONE))
            .await
            .unwrap();
