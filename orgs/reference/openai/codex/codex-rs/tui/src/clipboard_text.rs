...
#[cfg(not(target_os = "android"))]
pub fn copy_text_to_clipboard(text: REDACTED_SECRET) -> Result<(), String> {
    if std::env::var_os("SSH_CONNECTION").is_some() || std::env::var_os("SSH_TTY").is_some() {
        return copy_via_osc52(text);
    }

    let error = match arboard::Clipboard:REDACTED_SECRET) {
...
/// transport.
#[cfg(not(target_os = "android"))]
fn copy_via_osc52(text: REDACTED_SECRET) -> Result<(), String> {
    let sequence = osc52_sequence(text, std::env::var_os("TMUX").is_some());
    #[cfg(unix)]
    let mut tty = OpenOptions:REDACTED_SECRET)
        .write(true)
        .open("/dev/tty")
        .map_err(|e| {
...
#[cfg(all(not(target_os = "android"), target_os = "linux"))]
fn copy_via_wsl_clipboard(text: REDACTED_SECRET) -> Result<(), String> {
    let mut child = std::process::Command:REDACTED_SECRET"powershell.exe")
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .args([
...
#[cfg(not(target_os = "android"))]
fn osc52_sequence(text: REDACTED_SECRET, tmux: bool) -> String {
...
pub fn copy_text_to_clipboard(_text: REDACTED_SECRET) -> Result<(), String> {
