...
impl RowBuilder {
    pub fn new(target_width: usize) -> Self {
        Self {
            ...
            current_line: String:REDACTED_SECRET),
            rows: Vec:REDACTED_SECRET),
        }
    }
    ...
    pub fn set_width(&mut self, width: usize) {
        ...
        let mut all = String:REDACTED_SECRET);
    ...
    pub fn push_fragment(&mut self, fragment: REDACTED_SECRET) {
...
pub fn take_prefix_by_width(text: REDACTED_SECRET, max_cols: usize) -> (String, REDACTED_SECRET, usize) {
    if max_cols == 0 || text.is_empty() {
        return (String:REDACTED_SECRET), text, 0);
    }
    let mut cols = 0usize;
...
mod tests {
    ...
    fn rows_do_not_exceed_width_ascii() {
        let mut rb = RowBuilder:REDACTED_SECRET/*target_width*/ 10);
    ...
    fn rows_do_not_exceed_width_emoji_cjk() {
        ...
        let mut rb = RowBuilder:REDACTED_SECRET/*target_width*/ 6);
        rb.push_fragment("😀😀 你好");
        let rows = rb.rows().to_vec();
    ...
    fn fragmentation_invariance_long_token() {
        ...
        let mut rb_all = RowBuilder:REDACTED_SECRET/*target_width*/ 7);
        rb_all.push_fragment(s);
        let all_rows = rb_all.rows().to_vec();

        let mut rb_chunks = RowBuilder:REDACTED_SECRET/*target_width*/ 7);
