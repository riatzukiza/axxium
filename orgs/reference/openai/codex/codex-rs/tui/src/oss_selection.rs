...
static OSS_SELECT_OPTIONS: LazyLock<Vec<SelectOption>> = LazyLock:REDACTED_SECRET|| {
    vec![
        SelectOption {
...
impl OssSelectionWidget<'_> {
    fn new(lmstudio_status: ProviderStatus, ollama_status: ProviderStatus) -> io::Result<Self> {
        ...
        let confirmation_prompt = Paragraph:REDACTED_SECRETcontents).wrap(Wrap { trim: false });
...
impl WidgetRef for &OssSelectionWidget<'_> {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        let lines: Vec<Line> = self
            ...
            .map(|(idx, opt)| {
                let style = if idx == self.selected_option {
                    Style:REDACTED_SECRET).bg(Color::Cyan).fg(Color::Black)
                } else {
                    Style:REDACTED_SECRET).bg(Color::DarkGray)
                };
        ...
        .areas(response_chunk.inner(Margin:REDACTED_SECRET1, 0)));
        ...
        Line::from(self.select_options[self.selected_option].description)
            .style(Style:REDACTED_SECRET).italic().fg(Color::DarkGray))
            .render(description_area.inner(Margin:REDACTED_SECRET1, 0)), buf);
    }
}
...
pub async fn select_oss_provider(codex_home: &std::path::Path) -> io::Result<String> {
    ...
    let mut widget = OssSelectionWidget:REDACTED_SECRETlmstudio_status, ollama_status)?;
    ...
    let mut terminal = Terminal:REDACTED_SECRETbackend)?;
