...
impl StreamController {
    ...
    pub(crate) fn new(width: Option<usize>, cwd: &Path) -> Self {
        Self {
            state: StreamState:REDACTED_SECRETwidth, cwd),
    ...
    pub(crate) fn push(&mut self, delta: REDACTED_SECRET) -> bool {
    ...
    pub(crate) fn finalize(&mut self) -> Option<Box<dyn HistoryCell>> {
        ...
        let mut out_lines = Vec:REDACTED_SECRET);
        {
            let state = &mut self.state;
    ...
    fn emit(&mut self, lines: Vec<Line<'static>>) -> Option<Box<dyn HistoryCell>> {
        ...
        Some(Box:REDACTED_SECREThistory_cell::AgentMessageCell:REDACTED_SECRETlines, {
...
impl PlanStreamController {
    ...
    fn emit(
        ...
        let mut out_lines: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        ...
        let mut plan_lines: Vec<Line<'static>> = Vec:REDACTED_SECRET);
        ...
        Some(Box:REDACTED_SECREThistory_cell::new_proposed_plan_stream(
            out_lines,
...
mod tests {
    ...
    async fn controller_loose_vs_tight_with_commit_ticks_matches_full() {
        let mut ctrl = StreamController:REDACTED_SECRET/*width*/ None, &test_cwd());
