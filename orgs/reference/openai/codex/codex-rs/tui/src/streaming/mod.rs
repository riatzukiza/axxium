...
impl StreamState {
    ...
    pub(crate) fn new(width: Option<usize>, cwd: &Path) -> Self {
        Self {
            collector: MarkdownStreamCollector:REDACTED_SECRETwidth, cwd),
            queued_lines: VecDeque:REDACTED_SECRET),
...
mod tests {
    ...
    fn drain_n_clamps_to_available_lines() {
        let mut state = StreamState:REDACTED_SECRET/*width*/ None, &test_cwd());
