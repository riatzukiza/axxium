...
static SYNTAX_SET: OnceLock<SyntaxSet> = OnceLock:REDACTED_SECRET);
static THEME: OnceLock<RwLock<Theme>> = OnceLock:REDACTED_SECRET);
static THEME_OVERRIDE: OnceLock<Option<String>> = OnceLock:REDACTED_SECRET);
static CODEX_HOME: OnceLock<Option<PathBuf>> = OnceLock:REDACTED_SECRET);
...
pub(crate) fn validate_theme_name(name: Option<REDACTED_SECRET>, codex_home: Option<&Path>) -> Option<String> {
    let name = name?;
...
fn parse_theme_name(name: REDACTED_SECRET) -> Option<EmbeddedThemeName> {
    match name {
...
fn custom_theme_path(name: REDACTED_SECRET, codex_home: &Path) -> PathBuf {
    codex_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{codex_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: REDACTED_SECRET, codex_home: &Path) -> Option<Theme> {
...
fn resolve_theme_with_override(name: Option<REDACTED_SECRET>, codex_home: Option<&Path>) -> Theme {
...
fn theme_lock() -> &'static RwLock<Theme> {
    THEME.get_or_init(|| RwLock:REDACTED_SECRETbuild_default_theme()))
}
...
fn diff_scope_background_rgbs_for_theme(theme: &Theme) -> DiffScopeBackgroundRgbs {
    let highlighter = Highlighter:REDACTED_SECRETtheme);
...
fn scope_background_rgb(highlighter: &Highlighter<'_>, scope_name: REDACTED_SECRET) -> Option<(u8, u8, u8)> {
    let scope = Scope:REDACTED_SECRETscope_name).ok()?;
...
pub(crate) fn resolve_theme_by_name(name: REDACTED_SECRET, codex_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
const BUILTIN_THEME_NAMES: &[REDACTED_SECRET] = &[
    "1337",
    "ansi",
    "base16",
    "base16-256",
...
fn find_syntax(lang: REDACTED_SECRET) -> Option<&'static SyntaxReference> {
    let ss = syntax_set();
...
fn highlight_to_line_spans_with_theme(
    ...
    let mut h = HighlightLines:REDACTED_SECRETsyntax, theme);
    let mut lines: Vec<Vec<Span<'static>>> = Vec:REDACTED_SECRET);
    ...
    for line in LinesWithEndings::from(code) {
        ...
        if spans.is_empty() {
            spans.push(Span::raw(String:REDACTED_SECRET)));
        }
        lines.push(spans);
    }
...
fn highlight_to_line_spans(code: REDACTED_SECRET, lang: REDACTED_SECRET) -> Option<Vec<Vec<Span<'static>>>> {
...
pub(crate) fn highlight_code_to_lines(code: REDACTED_SECRET, lang: REDACTED_SECRET) -> Vec<Line<'static>> {
    if let Some(line_spans) = highlight_to_line_spans(code, lang) {
        ...
        if result.is_empty() {
            result.push(Line::from(String:REDACTED_SECRET)));
        }
        result
    }
}
...
pub(crate) fn highlight_bash_to_lines(script: REDACTED_SECRET) -> Vec<Line<'static>> {
...
<key>background</key><string>#000000</string>
    ...
    fn write_tmtheme_with_diff_backgrounds(
        path: &Path,
        inserted_scope: REDACTED_SECRET,
        inserted_background: REDACTED_SECRET,
        deleted_scope: REDACTED_SECRET,
        deleted_background: REDACTED_SECRET,
    ) {
        let contents = format!(
            r#"<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: REDACTED_SECRET) -> Vec<String> {
    ...
    fn theme_item(scope: REDACTED_SECRET, background: Option<(u8, u8, u8)>) -> ThemeItem {
        ThemeItem {
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("nord"), Some(Path:REDACTED_SECRET"/nonexistent"))).is_none());
    }
