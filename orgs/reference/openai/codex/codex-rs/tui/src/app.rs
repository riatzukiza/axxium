...
use tokio::sync::mpsc::error::TryREDACTED_SECRETError;
...
const EXTERNAL_EDITOR_HINT: REDACTED_SECRET = "Save and close external editor to continue.";
...
fn emit_skill_load_warnings(app_event_tx: &AppEventREDACTED_SECRETer, errors: &[SkillErrorInfo]) {
    if errors.is_empty() {
        return;
    }
...
fn emit_project_config_warnings(app_event_tx: &AppEventREDACTED_SECRETer, config: &Config) {
    let mut disabled_folders = Vec:REDACTED_SECRET);
...
fn emit_system_bwrap_warning(app_event_tx: &AppEventREDACTED_SECRETer, config: &Config) {
    let Some(message) =
...
impl ThreadEventStore {
    ...
    fn new(capacity: usize) -> Self {
        Self {
            session: None,
            turns: Vec:REDACTED_SECRET),
            buffer: VecDeque:REDACTED_SECRET),
    ...
    fn new_with_session(capacity: usize, session: ThreadSessionState, turns: Vec<Turn>) -> Self {
        let mut store = Self:REDACTED_SECRETcapacity);
    ...
    fn active_turn_id(&self) -> Option<REDACTED_SECRET> {
...
struct ThreadEventChannel {
    sender: mpsc::REDACTED_SECRETer<ThreadBufferedEvent>,
...
impl ThreadEventChannel {
    fn new(capacity: usize) -> Self {
        ...
        Self {
            ...
            store: Arc:REDACTED_SECRETMutex:REDACTED_SECRETThreadEventStore:REDACTED_SECRETcapacity))),
        }
    }
    ...
    fn new_with_session(capacity: usize, session: ThreadSessionState, turns: Vec<Turn>) -> Self {
        ...
        Self {
            sender,
            receiver: Some(receiver),
            store: Arc:REDACTED_SECRETMutex:REDACTED_SECRETThreadEventStore::new_with_session(
...
fn should_show_model_migration_prompt(
    current_model: REDACTED_SECRET,
    target_model: REDACTED_SECRET,
    seen_migrations: &REDACTED_SECRET, String>,
...
fn migration_prompt_hidden(config: &Config, migration_config_key: REDACTED_SECRET) -> bool {
...
async fn prepare_startup_tooltip_override(
    ...
    if let Err(err) = ConfigEditsBuilder:REDACTED_SECRET&config.codex_home)
...
fn normalize_harness_overrides_for_cwd(
    ...
    if overrides.additional_writable_REDACTED_SECRETs.is_empty() {
        return Ok(overrides);
    }

    let mut normalized = Vec::with_capacity(overrides.additional_writable_REDACTED_SECRETs.len());
    for REDACTED_SECRET in overrides.additional_writable_REDACTED_SECRETs.drain(..) {
        let absolute = AbsolutePathBuf::resolve_path_against_base(REDACTED_SECRET, base_cwd)?;
        normalized.push(absolute.into_path_buf());
    }
    overrides.additional_writable_REDACTED_SECRETs = normalized;
    Ok(overrides)
}
...
impl App {
    ...
    async fn refresh_in_memory_config_from_disk_best_effort(&mut self, action: REDACTED_SECRET) {
    ...
    fn try_set_approval_policy_on_config(
        ...
        user_message_prefix: REDACTED_SECRET,
        log_message: REDACTED_SECRET,
    ) -> bool {
    ...
    async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        let scoped_segments = |key: REDACTED_SECRET| {
        ...
        // the REDACTED_SECRET scope.
        let (REDACTED_SECRET_approvals_reviewer_blocks_profile_disable, profile_approvals_reviewer_configured) = {
            let effective_config = next_config.config_layer_stack.effective_config();
            let REDACTED_SECRET_blocks_disable = effective_config
                .as_table()
            ...
            (REDACTED_SECRET_blocks_disable, profile_configured)
        };
        let mut permissions_history_label: Option<&'static str> = None;
        let mut builder = ConfigEditsBuilder:REDACTED_SECRET&self.config.codex_home)
            .with_profile(self.active_profile.as_deref());

        for (feature, enabled) in updates {
            let feature_key = feature.key();
            let mut feature_edits = Vec:REDACTED_SECRET);
            if feature == Feature::GuardianApproval
                && !enabled
                && self.active_profile.is_some()
                && REDACTED_SECRET_approvals_reviewer_blocks_profile_disable
            {
    ...
    fn clear_ui_header_lines_with_version(
        ...
        history_cell::SessionHeaderHistoryCell:REDACTED_SECRET
    ...
    fn ensure_thread_channel(&mut self, thread_id: ThreadId) -> &mut ThreadEventChannel {
        self.thread_event_channels
            .entry(thread_id)
            .or_insert_with(|| ThreadEventChannel:REDACTED_SECRETTHREAD_EVENT_CHANNEL_CAPACITY))
    }
    ...
    async fn enqueue_thread_feedback_event(
        ...
        if should_send {
            match sender.try_send(ThreadBufferedEvent::FeedbackSubmission(event)) {
                Ok(()) => {}
                Err(TryREDACTED_SECRETError::Full(event)) => {
                    tokio::spawn(async move {
                ...
                Err(TryREDACTED_SECRETError::Closed(_)) => {
            ...
            AppCommandView::ListSkills { cwds, force_reload } => {
                let response = app_server
                    .skills_list(codex_app_server_protocol::SkillsListParams {
                        cwds: cwds.to_vec(),
                        force_reload,
                        per_cwd_extra_user_REDACTED_SECRETs: None,
                    })
                    .await?;
    ...
    async fn refresh_pending_thread_approvals(&mut self) {
        ...
        let mut pending_thread_ids = Vec:REDACTED_SECRET);
    ...
    fn reset_thread_event_state(&mut self) {
        ...
        self.chat_widget.set_pending_thread_approvals(Vec:REDACTED_SECRET));
        self.sync_active_agent_label();
    }
    ...
    async fn backfill_loaded_subagent_threads(
        ...
        let mut threads = Vec:REDACTED_SECRET);
    ...
    pub async fn run(
        ...
        let app_event_tx = AppEventREDACTED_SECRETer:REDACTED_SECRETapp_event_tx);
        ...
        let model_catalog = Arc:REDACTED_SECRETModelCatalog:REDACTED_SECRET
        ...
        let session_telemetry = SessionTelemetry:REDACTED_SECRET
            ThreadId:REDACTED_SECRET),
            model.as_str(),
            model.as_str(),
            /*account_id*/ None,
        ...
        let status_line_invalid_items_warned = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        let terminal_title_invalid_items_warned = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        ...
        let file_search = FileSearchManager:REDACTED_SECRETconfig.cwd.to_path_buf(), app_event_tx.clone());
        #[cfg(not(debug_assertions))]
        ...
        let mut app = Self {
            ...
            file_search,
            enhanced_keys_supported,
            transcript_cells: Vec:REDACTED_SECRET),
            overlay: None,
            deferred_history_lines: Vec:REDACTED_SECRET),
            has_emitted_history_lines: false,
            commit_anim_running: Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)),
            status_line_invalid_items_warned: status_line_invalid_items_warned.clone(),
            ...
            thread_event_channels: HashMap:REDACTED_SECRET),
            thread_event_listener_tasks: HashMap:REDACTED_SECRET),
            ...
            pending_primary_events: VecDeque:REDACTED_SECRET),
        ...
        let pre_loop_exit_reason = if let Some(latest_version) = upgrade_version {
            let control = app
                .handle_event(
                    tui,
                    &mut app_server,
                    AppEvent::InsertHistoryCell(Box:REDACTED_SECRETUpdateAvailableHistoryCell:REDACTED_SECRET
                        latest_version,
        ...
        match event {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let event = match result {
                            ...
                            Err(err) => {
                                ...
                                let mut tags: Vec<(REDACTED_SECRET, REDACTED_SECRET)> = Vec:REDACTED_SECRET);
            ...
            AppEvent::BeginWindowsSandboxGrantReadRoot { path } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let event = match codex_core::windows_sandbox_read_grants::grant_read_REDACTED_SECRET_non_elevated(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let builder = ConfigEditsBuilder:REDACTED_SECRET&self.config.codex_home)
            ...
            AppEvent::PersistModelSelection { model, effort } => {
                let profile = self.active_profile.as_deref();
                match ConfigEditsBuilder:REDACTED_SECRET&self.config.codex_home)
                    .with_profile(profile)
            ...
            AppEvent::PersistRealtimeAudioDeviceSelection { kind, name } => {
                let builder = match kind {
                    RealtimeAudioDeviceKind::Microphone => {
                        ConfigEditsBuilder:REDACTED_SECRET&self.config.codex_home)
            ...
            AppEvent::UpdateApprovalsReviewer(policy) => {
                ...
                if let Err(err) = ConfigEditsBuilder:REDACTED_SECRET&self.config.codex_home)
                    .with_profile(profile)
            ...
            AppEvent::FullScreenApprovalRequest(request) => match request {
                ApprovalRequest::ApplyPatch { cwd, changes, .. } => {
                    let _ = tui.enter_alt_screen();
                    let diff_summary = DiffSummary:REDACTED_SECRETchanges, cwd);
                ...
                ApprovalRequest::Permissions {
                    ...
                    self.overlay = Some(Overlay::new_static_with_renderables(
                        vec![Box:REDACTED_SECRETParagraph:REDACTED_SECRETlines).wrap(Wrap { trim: false }))],
                ...
                ApprovalRequest::McpElicitation {
                    ...
                    let _ = tui.enter_alt_screen();
                    let paragraph = Paragraph:REDACTED_SECRETvec![
                    ...
                    self.overlay = Some(Overlay::new_static_with_renderables(
                        vec![Box:REDACTED_SECRETparagraph)],
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let apply_result = ConfigEditsBuilder:REDACTED_SECRET&self.config.codex_home)
                    .with_edits([edit])
                    .apply()
                    .await;
                match apply_result {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if result.is_err() {
                ...
                tx.send(AppEvent::OpenWorldWritableWarningConfirmation {
                    preset: None,
                    sample_paths: Vec:REDACTED_SECRET),
                    extra_count: 0usize,
                    failed_scan: true,
                });
            }
...
async fn fetch_all_mcp_server_statuses(
    ...
    let mut cursor = None;
    let mut statuses = Vec:REDACTED_SECRET);
...
fn mcp_inventory_maps_from_statuses(statuses: Vec<McpServerStatus>) -> McpInventoryMaps {
    let mut tools = HashMap:REDACTED_SECRET);
    let mut resources = HashMap:REDACTED_SECRET);
    let mut resource_templates = HashMap:REDACTED_SECRET);
    let mut auth_statuses = HashMap:REDACTED_SECRET);
...
mod tests {
    ...
    fn test_absolute_path(path: REDACTED_SECRET) -> AbsolutePathBuf {
    ...
    fn normalize_harness_overrides_resolves_relative_add_dirs() -> Result<()> {
        ...
        let overrides = ConfigOverrides {
            additional_writable_REDACTED_SECRETs: vec![PathBuf::from("rel")],
            ..Default::default()
        };
        let normalized = normalize_harness_overrides_for_cwd(overrides, &base_cwd)?;

        assert_eq!(
            normalized.additional_writable_REDACTED_SECRETs,
    ...
    fn mcp_inventory_maps_prefix_tool_names_by_server() {
        let statuses = vec![
            McpServerStatus {
                ...
                resources: Vec:REDACTED_SECRET),
                resource_templates: Vec:REDACTED_SECRET),
                auth_status: codex_app_server_protocol::McpAuthStatus::Unsupported,
            },
            McpServerStatus {
                name: "disabled".to_string(),
                tools: HashMap:REDACTED_SECRET),
                resources: Vec:REDACTED_SECRET),
                resource_templates: Vec:REDACTED_SECRET),
                auth_status: codex_app_server_protocol::McpAuthStatus::Unsupported,
            },
    ...
    async fn handle_mcp_inventory_result_clears_committed_loading_cell() {
        ...
        app.transcript_cells
            .push(Arc:REDACTED_SECREThistory_cell::new_mcp_inventory_loading(
    ...
    async fn enqueue_primary_thread_session_replays_buffered_approval_after_attach() -> Result<()> {
        ...
        let thread_id = ThreadId:REDACTED_SECRET);
        ...
        app.chat_widget
            .handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Char('y'), KeyModifiers::NONE));
    ...
    -> Result<()> {
        ...
        app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
            ...
            feedback: codex_feedback::CodexFeedback:REDACTED_SECRET),
            is_first_run: false,
    ...
    async fn reset_thread_event_state_aborts_listener_tasks() {
        struct NotifyOnDrop(Option<tokio::sync::oneshot::REDACTED_SECRETer<()>>);
    ...
    async fn enqueue_thread_event_does_not_block_when_channel_full() -> Result<()> {
        ...
        app.thread_event_channels
            .insert(thread_id, ThreadEventChannel:REDACTED_SECRET/*capacity*/ 1));
    ...
    async fn active_turn_id_for_thread_uses_snapshot_turns() {
        ...
        app.thread_event_channels.insert(
            ...
            ThreadEventChannel::new_with_session(
                THREAD_EVENT_CHANNEL_CAPACITY,
                session,
                vec![test_turn("turn-1", TurnStatus::InProgress, Vec:REDACTED_SECRET))],
            ),
        );
    ...
    async fn replayed_turn_complete_submits_restored_queued_follow_up() {
        ...
        app.chat_widget
            .handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Tab, KeyModifiers::NONE));
        let input_state = app
            .chat_widget
            .capture_thread_input_state()
    ...
    async fn replay_thread_snapshot_in_progress_turn_restores_running_queue_state() {
        ...
        app.replay_thread_snapshot(
            ThreadEventSnapshot {
                session: None,
                turns: vec![test_turn("turn-1", TurnStatus::InProgress, Vec:REDACTED_SECRET))],
                events: Vec:REDACTED_SECRET),
                input_state: Some(input_state),
            },
            /*resume_restored_queue*/ true,
        );
    ...
    async fn replay_thread_snapshot_restores_pending_pastes_for_submit() {
        ...
        app.chat_widget
            .handle_key_event(KeyEvent:REDACTED_SECRETKeyCode::Enter, KeyModifiers::NONE));
...
"#;
    ...
    async fn refresh_pending_thread_approvals_only_lists_inactive_threads() {
        ...
        app.thread_event_channels
            .insert(main_thread_id, ThreadEventChannel:REDACTED_SECRET/*capacity*/ 1));

        let agent_channel = ThreadEventChannel:REDACTED_SECRET/*capacity*/ 1);
        {
    ...
    {
        let mut app = make_test_app().await;
        app.active_thread_id = Some(ThreadId:REDACTED_SECRET));
        app.primary_thread_id = Some(ThreadId:REDACTED_SECRET));
    ...
    {
        let mut app = make_test_app().await;
        let active_thread_id = ThreadId:REDACTED_SECRET);
        let primary_thread_id = ThreadId:REDACTED_SECRET);
    ...
    -> Result<()> {
        ...
        app.active_thread_id = Some(active_thread_id);
        app.primary_thread_id = Some(primary_thread_id);
        app.pending_shutdown_exit_thread_id = Some(ThreadId:REDACTED_SECRET));
    ...
    async fn render_clear_ui_header_after_long_transcript_for_snapshot() -> String {
        ...
        let user_cell = |text: REDACTED_SECRET| -> Arc<dyn HistoryCell> {
            Arc:REDACTED_SECRETUserHistoryCell {
                message: text.to_string(),
                text_elements: Vec:REDACTED_SECRET),
                local_image_paths: Vec:REDACTED_SECRET),
                remote_image_urls: Vec:REDACTED_SECRET),
            }) as Arc<dyn HistoryCell>
        };
        let agent_cell = |text: REDACTED_SECRET| -> Arc<dyn HistoryCell> {
            Arc:REDACTED_SECRETAgentMessageCell:REDACTED_SECRET
        ...
        let make_header = |is_first| -> Arc<dyn HistoryCell> {
            let event = SessionConfiguredEvent {
                session_id: ThreadId:REDACTED_SECRET),
                forked_from_id: None,
                thread_name: None,
                ...
                network_proxy: None,
                rollout_path: Some(PathBuf:REDACTED_SECRET)),
            };
            Arc:REDACTED_SECRETnew_session_info(
        ...
        app.transcript_cells = vec![
            make_header(true),
            Arc:REDACTED_SECRETcrate::history_cell::new_info_event(
    ...
    async fn make_test_app() -> App {
        ...
        App {
            ...
            config,
            active_profile: None,
            cli_kv_overrides: Vec:REDACTED_SECRET),
            ...
            enhanced_keys_supported: false,
            commit_anim_running: Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)),
            status_line_invalid_items_warned: Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)),
            terminal_title_invalid_items_warned: Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse)),
            backtrack: BacktrackState::default(),
            backtrack_render_pending: false,
            feedback: codex_feedback::CodexFeedback:REDACTED_SECRET),
    ...
    fn test_turn(turn_id: REDACTED_SECRET, status: TurnStatus, items: Vec<ThreadItem>) -> Turn {
        Turn {
    ...
    fn turn_started_notification(thread_id: ThreadId, turn_id: REDACTED_SECRET) -> ServerNotification {
        ServerNotification::TurnStarted(TurnStartedNotification {
            thread_id: thread_id.to_string(),
            turn: test_turn(turn_id, TurnStatus::InProgress, Vec:REDACTED_SECRET)),
        })
    }

    fn turn_completed_notification(
        thread_id: ThreadId,
        turn_id: REDACTED_SECRET,
        status: TurnStatus,
    ) -> ServerNotification {
        ServerNotification::TurnCompleted(TurnCompletedNotification {
            thread_id: thread_id.to_string(),
            turn: test_turn(turn_id, status, Vec:REDACTED_SECRET)),
        })
    }
    ...
    fn hook_started_notification(thread_id: ThreadId, turn_id: REDACTED_SECRET) -> ServerNotification {
    ...
    fn hook_completed_notification(thread_id: ThreadId, turn_id: REDACTED_SECRET) -> ServerNotification {
    ...
    fn agent_message_delta_notification(
        thread_id: ThreadId,
        turn_id: REDACTED_SECRET,
        item_id: REDACTED_SECRET,
        delta: REDACTED_SECRET,
    ) -> ServerNotification {
    ...
    fn exec_approval_request(
        ...
        turn_id: REDACTED_SECRET,
        item_id: REDACTED_SECRET,
        approval_id: Option<REDACTED_SECRET>,
    ) -> ServerRequest {
    ...
    fn thread_event_store_tracks_active_turn_lifecycle() {
        let mut store = ThreadEventStore:REDACTED_SECRET/*capacity*/ 8);
    ...
    fn thread_event_store_restores_active_turn_from_snapshot_turns() {
        ...
        let session = test_thread_session(thread_id, PathBuf::from("/tmp/project"));
        let turns = vec![
            test_turn("turn-1", TurnStatus::Completed, Vec:REDACTED_SECRET)),
            test_turn("turn-2", TurnStatus::InProgress, Vec:REDACTED_SECRET)),
        ];
        ...
        let mut refreshed_store = ThreadEventStore:REDACTED_SECRET/*capacity*/ 8);
    ...
    async fn feedback_submission_for_inactive_thread_replays_into_origin_thread() {
        let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
        let origin_thread_id = ThreadId:REDACTED_SECRET);
        let active_thread_id = ThreadId:REDACTED_SECRET);
        ...
        let mut rendered_cells = Vec:REDACTED_SECRET);
        while let Ok(event) = app_event_rx.try_recv() {
    ...
    fn next_user_turn_op(op_rx: &mut tokio::sync::mpsc::UnboundedReceiver<Op>) -> Op {
        let mut seen = Vec:REDACTED_SECRET);
    ...
    fn test_session_telemetry(config: &Config, model: REDACTED_SECRET) -> SessionTelemetry {
        let model_info = codex_core::test_support::construct_model_info_offline(model, config);
        SessionTelemetry:REDACTED_SECRET
            ThreadId:REDACTED_SECRET),
            model,
    ...
    fn app_enabled_in_effective_config(config: &Config, app_id: REDACTED_SECRET) -> Option<bool> {
        config
            .config_layer_stack
            .effective_config()
            .as_table()
    ...
    fn model_availability_nux_config(shown_count: &[(REDACTED_SECRET, u32)]) -> ModelAvailabilityNuxConfig {
    ...
    fn model_migration_copy_to_plain_text(
        ...
        let mut s = String:REDACTED_SECRET);
    ...
    async fn model_migration_prompt_only_shows_for_deprecated_models() {
        let seen = BTreeMap:REDACTED_SECRET);
    ...
    async fn model_migration_prompt_respects_hide_flag_and_self_target() {
        let mut seen = BTreeMap:REDACTED_SECRET);
    ...
    async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
        ...
        ConfigEditsBuilder:REDACTED_SECRET&app.config.codex_home)
            .with_edits([
                ConfigEdit::SetPath {
    ...
    async fn backtrack_selection_with_duplicate_history_targets_unique_turn() {
        ...
        let user_cell = |text: REDACTED_SECRET,
        ...
        let edited_text_elements = vec![TextElement:REDACTED_SECRET
            edited_range.into(),
            /*placeholder*/ None,
        )];
        ...
        app.transcript_cells = vec![
            make_header(true),
            user_cell("first question", Vec:REDACTED_SECRET), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET)),
            agent_cell("answer first"),
            user_cell("follow-up", Vec:REDACTED_SECRET), Vec:REDACTED_SECRET), Vec:REDACTED_SECRET)),
        ...
        let base_id = ThreadId:REDACTED_SECRET);
        app.chat_widget.handle_codex_event(Event {
            id: String:REDACTED_SECRET),
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                session_id: base_id,
    ...
    async fn backtrack_remote_image_only_selection_clears_existing_composer_draft() {
        ...
        app.transcript_cells = vec![Arc:REDACTED_SECRETUserHistoryCell {
                ...
                events: vec![ThreadBufferedEvent::Notification(
                    ServerNotification::ItemStarted(codex_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            sender_thread_id: ThreadId:REDACTED_SECRET).to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
                            reasoning_effort: None,
                            agents_states: HashMap:REDACTED_SECRET),
                        },
                    }),
                )],
                input_state: None,
        ...
        app.handle_thread_rollback_response(
            ...
            &ThreadRollbackResponse {
                thread: Thread {
                    ...
                    forked_from_id: None,
                    preview: String:REDACTED_SECRET),
                    ephemeral: false,
