...
impl MarkdownStreamCollector {
    ...
    pub fn push_delta(&mut self, delta: REDACTED_SECRET) {
    ...
    pub fn commit_complete_lines(&mut self) -> Vec<Line<'static>> {
        ...
        let mut rendered: Vec<Line<'static>> = Vec:REDACTED_SECRET);
...
pub(crate) fn simulate_stream_markdown_for_tests(
    deltas: &[REDACTED_SECRET],
    finalize: bool,
) -> Vec<Line<'static>> {
    let mut collector = MarkdownStreamCollector:REDACTED_SECRET/*width*/ None, &test_cwd());
...
mod tests {
    ...
    async fn no_commit_until_newline() {
        let mut c = super::MarkdownStreamCollector:REDACTED_SECRET/*width*/ None, &super::test_cwd());
    ...
    async fn e2e_stream_blockquote_wrap_preserves_green_style() {
        ...
        let wrapped = crate::wrapping::word_wrap_lines(
            out.iter(),
            crate::wrapping::RtOptions:REDACTED_SECRET/*width*/ 24),
        );
    ...
    async fn heading_not_inlined_when_split_across_chunks() {
        ...
        let mut rendered: Vec<ratatui::text::Line<'static>> = Vec:REDACTED_SECRET);
    ...
    async fn utf8_boundary_safety_and_wide_chars() {
        ...
        let streamed_str = lines_to_plain_strings(REDACTED_SECRETeamed);

        let mut rendered_all: Vec<ratatui::text::Line<'static>> = Vec:REDACTED_SECRET);
        let test_cwd = super::test_cwd();
        crate::markdown::append_markdown(
            input,
            /*width*/ None,
    ...
    async fn empty_fenced_block_is_dropped_and_separator_preserved_before_heading() {
        ...
        let streamed = simulate_stream_markdown_for_tests(&deltas, /*finalize*/ true);
        let texts = lines_to_plain_strings(REDACTED_SECRETeamed);
        assert!(
    ...
    async fn assert_streamed_equals_full(deltas: &[REDACTED_SECRET]) {
        let streamed = simulate_stream_markdown_for_tests(deltas, /*finalize*/ true);
        let streamed_strs = lines_to_plain_strings(REDACTED_SECRETeamed);
        let full: String = deltas.iter().copied().collect();
        let mut rendered: Vec<ratatui::text::Line<'static>> = Vec:REDACTED_SECRET);
