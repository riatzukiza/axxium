...
ON CONFLICT(child_thread_id) DO UPDATE SET
    ...
    /// List spawned descendants of `REDACTED_SECRET_thread_id` whose edges match `status`.
    ///
    ...
    pub async fn list_thread_spawn_descendants_with_status(
        ...
        self.list_thread_spawn_descendants_matching(REDACTED_SECRET_thread_id, Some(status))
            .await
    }
...
LIMIT 2
    ...
    /// Find a spawned descendant of `REDACTED_SECRET_thread_id` by canonical agent path.
...
LIMIT 2
        ...
        .bind(REDACTED_SECRET_thread_id.to_string())
        .bind(agent_path)
        .fetch_all(self.pool.as_ref())
        .await?;
        one_thread_id_from_rows(rows, agent_path)
...
ORDER BY depth ASC, child_thread_id ASC
        ...
        let mut sql = sqlx::query(query.as_str()).bind(REDACTED_SECRET_thread_id.to_string());
        if let Some(status) = status {
...
ON CONFLICT(child_thread_id) DO NOTHING
    ...
    pub async fn find_rollout_path_by_id(
        ...
        let mut builder =
            QueryBuilder::<Sqlite>:REDACTED_SECRET"SELECT rollout_path FROM threads WHERE id = ");
    ...
    pub async fn list_threads(
        ...
        archived_only: bool,
        search_term: Option<REDACTED_SECRET>,
    ) -> anyhow::Result<crate::ThreadsPage> {
        let limit = page_size.saturating_add(1);

        let mut builder = QueryBuilder::<Sqlite>:REDACTED_SECRET
            r#"
SELECT
    id,
    rollout_path,
...
FROM threads
    ...
    pub async fn list_thread_ids(
        ...
        let mut builder = QueryBuilder::<Sqlite>:REDACTED_SECRET"SELECT id FROM threads");
        push_thread_filters(
            &mut builder,
            archived_only,
            allowed_sources,
...
ON CONFLICT(id) DO NOTHING
    ...
    pub async fn update_thread_git_info(
        &self,
        thread_id: ThreadId,
        git_sha: Option<Option<REDACTED_SECRET>>,
        git_branch: Option<Option<REDACTED_SECRET>>,
        git_origin_url: Option<Option<REDACTED_SECRET>>,
    ) -> anyhow::Result<bool> {
        let result = sqlx::query(
            r#"
UPDATE threads
...
WHERE id = ?
    ...
    async fn upsert_thread_with_creation_memory_mode(
        &self,
        metadata: &crate::ThreadMetadata,
        creation_memory_mode: Option<REDACTED_SECRET>,
    ) -> anyhow::Result<()> {
        sqlx::query(
            r#"
INSERT INTO threads (
...
ON CONFLICT(thread_id, position) DO NOTHING
    ...
    pub async fn apply_rollout_items(
        ...
        items: &[RolloutItem],
        new_thread_memory_mode: Option<REDACTED_SECRET>,
...
fn thread_spawn_parent_thread_id_from_source_str(source: REDACTED_SECRET) -> Option<ThreadId> {
...
mod tests {
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let builder = ThreadMetadataBuilder:REDACTED_SECRET
            thread_id,
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(codex_git_utils::GitSha:REDACTED_SECRET"rollout-sha")),
    ...
    async fn thread_spawn_edges_track_directional_status() {
        ...
        let open_children = runtime
            ...
            .await
            .expect("open child list should load");
        assert_eq!(open_children, Vec::<ThreadId>:REDACTED_SECRET));
