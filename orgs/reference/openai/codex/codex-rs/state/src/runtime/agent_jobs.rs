...
INSERT INTO agent_job_items (
    ...
    pub async fn get_agent_job(&self, job_id: REDACTED_SECRET) -> anyhow::Result<Option<AgentJob>> {
...
WHERE job_id = ? AND item_id = ?
    ...
    pub async fn mark_agent_job_running(&self, job_id: REDACTED_SECRET) -> anyhow::Result<()> {
        let now = Utc::now().timestamp();
        sqlx::query(
            r#"
UPDATE agent_jobs
...
WHERE id = ?
    ...
    pub async fn mark_agent_job_completed(&self, job_id: REDACTED_SECRET) -> anyhow::Result<()> {
        let now = Utc::now().timestamp();
        sqlx::query(
            r#"
UPDATE agent_jobs
...
WHERE id = ? AND status IN (?, ?)
    ...
    pub async fn is_agent_job_cancelled(&self, job_id: REDACTED_SECRET) -> anyhow::Result<bool> {
        let row = sqlx::query(
            r#"
SELECT status
FROM agent_jobs
...
WHERE job_id = ? AND item_id = ? AND status = ?
    ...
    pub async fn mark_agent_job_item_pending(
        ...
        job_id: REDACTED_SECRET,
        item_id: REDACTED_SECRET,
        error_message: Option<REDACTED_SECRET>,
    ) -> anyhow::Result<bool> {
        let now = Utc::now().timestamp();
        let result = sqlx::query(
            r#"
...
WHERE job_id = ? AND item_id = ? AND status = ?
    ...
    pub async fn report_agent_job_item_result(
        ...
        job_id: REDACTED_SECRET,
        item_id: REDACTED_SECRET,
        reporting_thread_id: REDACTED_SECRET,
        result_json: &Value,
    ) -> anyhow::Result<bool> {
        let now = Utc::now().timestamp();
...
WHERE
    ...
    pub async fn get_agent_job_progress(&self, job_id: REDACTED_SECRET) -> anyhow::Result<AgentJobProgress> {
        let row = sqlx::query(
            r#"
SELECT
