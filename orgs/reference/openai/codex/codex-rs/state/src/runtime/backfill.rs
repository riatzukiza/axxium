...
WHERE id = 1
    ...
    pub async fn checkpoint_backfill(&self, watermark: REDACTED_SECRET) -> anyhow::Result<()> {
        self.ensure_backfill_state_row().await?;
        sqlx::query(
            r#"
UPDATE backfill_state
...
WHERE id = 1
    ...
    pub async fn mark_backfill_complete(&self, last_watermark: Option<REDACTED_SECRET>) -> anyhow::Result<()> {
        self.ensure_backfill_state_row().await?;
        let now = Utc::now().timestamp();
        sqlx::query(
            r#"
