...
const JOB_KIND_MEMORY_STAGE1: REDACTED_SECRET = "memory_stage1";
const JOB_KIND_MEMORY_CONSOLIDATE_GLOBAL: REDACTED_SECRET = "memory_consolidate_global";
const MEMORY_CONSOLIDATION_JOB_KEY: REDACTED_SECRET = "global";
...
LEFT JOIN jobs
   ...
   AND jobs.job_key = threads.id
        ...
        let mut claimed = Vec:REDACTED_SECRET);
...
LIMIT ?
        ...
        let mut retained_thread_ids = Vec:REDACTED_SECRET);
...
ORDER BY so.source_updated_at DESC, so.thread_id DESC
        ...
        let previous_selected = previous_rows
            ...
            .collect::<Result<Vec<_>, _>>()?;
        let mut removed = Vec:REDACTED_SECRET);
...
WHERE kind = ? AND job_key = ?
    ...
    pub async fn mark_stage1_job_succeeded(
        &self,
        thread_id: ThreadId,
        ownership_token: REDACTED_SECRET,
        source_updated_at: i64,
...
WHERE thread_id = ?
    ...
    pub async fn mark_stage1_job_failed(
        ...
        thread_id: ThreadId,
        ownership_token: REDACTED_SECRET,
        failure_reason: REDACTED_SECRET,
