...
impl StateRuntime {
    ...
    async fn prune_logs_after_insert(
        ...
        let thread_ids: BTreeSet<REDACTED_SECRET> = entries
            .iter()
        ...
        if !thread_ids.is_empty() {
            ...
            let mut over_limit_threads_query =
                QueryBuilder::<Sqlite>:REDACTED_SECRET"SELECT thread_id FROM logs WHERE thread_id IN (");
            {
            ...
            if !over_limit_thread_ids.is_empty() {
                ...
                let mut prune_threads = QueryBuilder::<Sqlite>:REDACTED_SECRET
                    r#"
DELETE FROM logs
WHERE id IN (
    SELECT id
...
"#,
        ...
        let threadless_process_uuids: BTreeSet<REDACTED_SECRET> = entries
            .iter()
        ...
        if !threadless_process_uuids.is_empty() {
            ...
            let mut over_limit_processes_query = QueryBuilder::<Sqlite>:REDACTED_SECRET
            ...
            if !over_limit_process_uuids.is_empty() {
                ...
                let mut prune_threadless_process_logs = QueryBuilder::<Sqlite>:REDACTED_SECRET
                    r#"
DELETE FROM logs
WHERE id IN (
    SELECT id
...
"#,
        ...
        if has_threadless_null_process_uuid {
            ...
            let mut null_process_usage_query = QueryBuilder::<Sqlite>:REDACTED_SECRET"SELECT SUM(");
            ...
            {
                let mut prune_threadless_null_process_logs = QueryBuilder::<Sqlite>:REDACTED_SECRET
                    r#"
DELETE FROM logs
WHERE id IN (
    SELECT id
...
"#
    ...
    pub async fn query_feedback_logs(&self, thread_id: REDACTED_SECRET) -> anyhow::Result<Vec<u8>> {
    ...
    pub async fn max_log_id(&self, query: &LogQuery) -> anyhow::Result<i64> {
        let mut builder =
            QueryBuilder::<Sqlite>:REDACTED_SECRET"SELECT MAX(id) AS max_id FROM logs WHERE 1 = 1");
        push_log_filters(&mut builder, query);
...
fn format_feedback_log_line(
    ts: i64,
    ts_nanos: i64,
    level: REDACTED_SECRET,
    feedback_log_body: REDACTED_SECRET,
) -> String {
...
mod tests {
    ...
    async fn open_db_pool(path: &Path) -> SqlitePool {
        SqlitePool::connect_with(
            SqliteConnectOptions:REDACTED_SECRET)
                .filename(path)
    ...
    async fn query_feedback_logs_excludes_oversized_newest_row() {
        ...
        assert_eq!(bytes, Vec::<u8>:REDACTED_SECRET));
