...
fn parse_timestamp(value: REDACTED_SECRET) -> anyhow::Result<i64> {
...
fn heuristic_formatting(message: REDACTED_SECRET) -> String {
...
mod matcher {
    pub(super) fn apply_patch(message: REDACTED_SECRET) -> bool {
...
mod formatter {
    ...
    pub(super) fn apply_patch(message: REDACTED_SECRET) -> String {
        message
            .lines()
            .map(|line| {
                if line.starts_with('+') {
    ...
    pub(super) fn level(level: REDACTED_SECRET) -> String {
