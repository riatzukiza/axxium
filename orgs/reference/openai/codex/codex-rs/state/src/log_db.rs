...
pub struct LogDbLayer {
    sender: mpsc::REDACTED_SECRETer<LogDbCommand>,
    process_uuid: String,
}
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        let _ = self.sender.try_send(LogDbCommand::Entry(Box:REDACTED_SECRETentry)));
    }
}

enum LogDbCommand {
    Entry(Box<LogEntry>),
    Flush(oneshot::REDACTED_SECRETer<()>),
}
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    if let Some(scope) = ctx.event_scope(event) {
        for span in scope.from_REDACTED_SECRET() {
...
where
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    let mut feedback_log_body = String:REDACTED_SECRET);
    if let Some(scope) = ctx.event_scope(event) {
        for span in scope.from_REDACTED_SECRET() {
            let extensions = span.extensions();
            if let Some(log_context) = extensions.get::<SpanLogContext>() {
    ...
    R: RecordFields,
{
    ...
    let mut formatted = FormattedFields::<DefaultFields>:REDACTED_SECRETString:REDACTED_SECRET));
...
fn append_fields(fields: &mut String, values: &Record<'_>) {
    let formatter = DefaultFields::default();
    let mut formatted = FormattedFields::<DefaultFields>:REDACTED_SECRETstd::mem::take(fields));
    let _ = formatter.add_fields(&mut formatted, values);
    *fields = formatted.fields;
}

fn current_process_log_uuid() -> &'static str {
    static PROCESS_LOG_UUID: OnceLock<String> = OnceLock:REDACTED_SECRET);
...
mod tests {
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        ...
        let subscriber = tracing_subscriber::registry()
            .with(
                tracing_subscriber::fmt::layer()
                    ...
                    .with_ansi(false)
                    .with_target(false)
                    .with_filter(Targets:REDACTED_SECRET).with_default(tracing::Level::TRACE)),
            )
            .with(
                start(runtime.clone())
                    .with_filter(Targets:REDACTED_SECRET).with_default(tracing::Level::TRACE)),
        ...
        let without_timestamps = |logs: REDACTED_SECRET| {
            logs.lines()
