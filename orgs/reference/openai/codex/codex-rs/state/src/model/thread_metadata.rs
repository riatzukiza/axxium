...
impl ThreadMetadataBuilder {
    ...
    pub fn build(&self, default_provider: REDACTED_SECRET) -> ThreadMetadata {
...
impl ThreadMetadata {
    ...
    pub fn diff_fields(&self, other: &Self) -> Vec<&'static str> {
        let mut diffs = Vec:REDACTED_SECRET);
        if self.id != other.id {
            diffs.push("id");
        }
...
mod tests {
    ...
    fn thread_row(reasoning_effort: Option<REDACTED_SECRET>) -> ThreadRow {
        ThreadRow {
            ...
            tokens_used: 1,
            first_user_message: String:REDACTED_SECRET),
            archived_at: None,
            git_sha: None,
            git_branch: None,
            git_origin_url: None,
