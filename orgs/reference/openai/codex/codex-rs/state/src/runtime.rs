...
impl StateRuntime {
    ...
    pub async fn init(codex_home: PathBuf, default_provider: String) -> anyhow::Result<Arc<Self>> {
        ...
        let runtime = Arc:REDACTED_SECRETSelf {
            pool,
            logs_pool,
            codex_home,
            default_provider,
...
async fn open_state_sqlite(path: &Path, migrator: &'static Migrator) -> anyhow::Result<SqlitePool> {
    ...
    let pool = SqlitePoolOptions:REDACTED_SECRET)
        .max_connections(5)
...
fn db_filename(base_name: REDACTED_SECRET, version: u32) -> String {
...
async fn remove_legacy_db_files(
    codex_home: &Path,
    current_name: REDACTED_SECRET,
    base_name: REDACTED_SECRET,
    db_label: REDACTED_SECRET,
) {
...
fn should_remove_db_file(file_name: REDACTED_SECRET, current_name: REDACTED_SECRET, base_name: REDACTED_SECRET) -> bool {
