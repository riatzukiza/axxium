...
fn expected_echo_result(message: REDACTED_SECRET) -> CallToolResult {
    CallToolResult {
...
async fn create_client(base_url: REDACTED_SECRET) -> anyhow::Result<RmcpClient> {
...
async fn call_echo_tool(client: &RmcpClient, message: REDACTED_SECRET) -> anyhow::Result<CallToolResult> {
    client
        .call_tool(
            "echo".to_string(),
...
async fn arm_session_post_failure(
    ...
    let response = reqwest::Client:REDACTED_SECRET)
...
async fn spawn_streamable_http_server() -> anyhow::Result<(Child, String)> {
    ...
    let mut child = Command:REDACTED_SECRETstreamable_http_server_bin()?)
        .kill_on_drop(true)
