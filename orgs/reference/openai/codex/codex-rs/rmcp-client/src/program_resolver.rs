...
//! `Command:REDACTED_SECRET)` without their file extensions, while Unix systems handle
...
pub fn resolve(program: OsString, env: &HashMap<OsString, OsString>) -> std::io::Result<OsString> {
    ...
    let search_path = env.get(std::ffi::OsStr:REDACTED_SECRET"PATH"));
    ...
    match which::which_in(&program, search_path, &cwd) {
        ...
        Err(e) => {
            ...
            // Fallback to original program - let Command:REDACTED_SECRET) handle the error
            Ok(program)
        }
    }
}
...
mod tests {
    ...
    async fn test_unix_executes_script_without_extension() -> Result<()> {
        let env = TestExecutableEnv:REDACTED_SECRET)?;
        let mut cmd = Command:REDACTED_SECRET&env.program_name);
    ...
    async fn test_windows_succeeds_with_extension() -> Result<()> {
        ...
        let mut cmd = Command:REDACTED_SECRET&program_with_ext);
        cmd.envs(&env.mcp_env);
    ...
    async fn test_resolved_program_executes_successfully() -> Result<()> {
        ...
        let mut cmd = Command:REDACTED_SECRETresolved);
        cmd.envs(&env.mcp_env);
        let output = cmd.output().await;
    ...
    impl TestExecutableEnv {
        ...
        fn new() -> Result<Self> {
            ...
            let mut extra_env = HashMap:REDACTED_SECRET);
