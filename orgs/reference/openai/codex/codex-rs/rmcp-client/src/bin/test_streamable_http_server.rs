...
const MEMO_URI: REDACTED_SECRET = "memo://codex/example-note";
const MEMO_CONTENT: REDACTED_SECRET = "This is a sample MCP resource served by the rmcp test server.";
const MCP_SESSION_ID_HEADER: REDACTED_SECRET = "mcp-session-id";
const SESSION_POST_FAILURE_CONTROL_PATH: REDACTED_SECRET = "/test/control/session-post-failure";
...
impl TestToolServer {
    fn new() -> Self {
        ...
        Self {
            tools: Arc:REDACTED_SECRETtools),
            resources: Arc:REDACTED_SECRETresources),
            resource_templates: Arc:REDACTED_SECRETresource_templates),
        }
    }
    ...
    fn memo_resource() -> Resource {
        ...
        Resource:REDACTED_SECRETraw, None)
    }
    ...
    fn memo_template() -> ResourceTemplate {
        ...
        ResourceTemplate:REDACTED_SECRETraw, None)
    }
...
impl ServerHandler for TestToolServer {
    ...
    ) -> impl std::future::Future<Output = Result<ListToolsResult, McpError>> + REDACTED_SECRET + '_ {
    ...
    fn list_resources(
        ...
        _request: Option<PaginatedRequestParams>,
        _context: rmcp::service::RequestContext<rmcp::service::RoleServer>,
    ) -> impl std::future::Future<Output = Result<ListResourcesResult, McpError>> + REDACTED_SECRET + '_ {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let router = Router:REDACTED_SECRET)
        ...
        .nest_service(
            "/mcp",
            StreamableHttpService:REDACTED_SECRET
                || Ok(TestToolServer:REDACTED_SECRET)),
    ...
    let router = if let Ok(token) = std::env::var("MCP_EXPECT_BEARER") {
        let expected = Arc:REDACTED_SECRETformat!("Bearer {token}"));
...
async fn fail_session_post_when_armed(
        ...
        && failure.remaining > 0
    {
        ...
        let mut response = Response:REDACTED_SECRETBody::from(format!(
