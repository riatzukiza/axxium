...
pub async fn perform_oauth_login(
    ...
    server_url: REDACTED_SECRET,
    store_mode: OAuthCredentialsStoreMode,
    http_headers: Option<HashMap<String, String>>,
    env_http_headers: Option<HashMap<String, String>>,
    scopes: &[String],
    oauth_resource: Option<REDACTED_SECRET>,
    callback_port: Option<u16>,
    callback_url: Option<REDACTED_SECRET>,
) -> Result<()> {
...
async fn perform_oauth_login_with_browser_output(
    ...
    OauthLoginFlow:REDACTED_SECRET
        server_name,
        server_url,
        store_mode,
        headers,
...
pub async fn perform_oauth_login_return_url(
    ...
    let flow = OauthLoginFlow:REDACTED_SECRET
        server_name,
        server_url,
        store_mode,
        headers,
    ...
    Ok(OauthLoginHandle:REDACTED_SECRETauthorization_url, completion))
}

fn spawn_callback_server(
    server: Arc<Server>,
    tx: oneshot::REDACTED_SECRETer<CallbackResult>,
...
fn parse_oauth_callback(path: REDACTED_SECRET, expected_callback_path: REDACTED_SECRET) -> CallbackOutcome {
    ...
    if error.is_some() || error_description.is_some() {
        return CallbackOutcome::Error(OAuthProviderError:REDACTED_SECRETerror, error_description));
    }
...
impl OauthLoginHandle {
    ...
    pub fn authorization_url(&self) -> REDACTED_SECRET {
...
fn resolve_redirect_uri(server: &Server, callback_url: Option<REDACTED_SECRET>) -> Result<String> {
...
fn callback_path_from_redirect_uri(redirect_uri: REDACTED_SECRET) -> Result<String> {
    let parsed = Url::parse(redirect_uri)
        .with_context(|| format!("invalid redirect URI `{redirect_uri}`"))?;
    Ok(parsed.path().to_string())
}

fn callback_bind_host(callback_url: Option<REDACTED_SECRET>) -> &'static str {
    let Some(callback_url) = callback_url else {
        return "127.0.0.1";
    };
...
impl OauthLoginFlow {
    ...
    async fn new(
        ...
        let server = Arc:REDACTED_SECRETServer::http(&bind_addr).map_err(|err| anyhow!(err))?);
        ...
        let http_client = apply_default_headers(ClientBuilder:REDACTED_SECRET), &default_headers).build()?;

        let mut oauth_state = OAuthState:REDACTED_SECRETserver_url, Some(http_client)).await?;
        let scope_refs: Vec<REDACTED_SECRET> = scopes.iter().map(String::as_str).collect();
        oauth_state
...
fn append_query_param(url: REDACTED_SECRET, key: REDACTED_SECRET, value: Option<REDACTED_SECRET>) -> String {
...
mod tests {
    ...
    fn parse_oauth_callback_returns_provider_error() {
        ...
        assert_eq!(
            parsed,
            CallbackOutcome::Error(OAuthProviderError:REDACTED_SECRET
