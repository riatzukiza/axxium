...
const EVENT_STREAM_MIME_TYPE: REDACTED_SECRET = "text/event-stream";
const JSON_MIME_TYPE: REDACTED_SECRET = "application/json";
const HEADER_LAST_EVENT_ID: REDACTED_SECRET = "Last-Event-Id";
const HEADER_SESSION_ID: REDACTED_SECRET = "Mcp-Session-Id";
...
pub type REDACTED_SECRETElicitation = Box<
    dyn Fn(RequestId, Elicitation) -> BoxFuture<'static, Result<ElicitationResponse>> + REDACTED_SECRET + Sync,
>;
...
impl RmcpClient {
    pub async fn new_stdio_client(
        ...
        Ok(Self {
            state: Mutex:REDACTED_SECRETClientState::Connecting {
                transport: Some(transport),
            }),
            transport_recipe,
            initialize_context: Mutex:REDACTED_SECRETNone),
            session_recovery_lock: Mutex:REDACTED_SECRET()),
        })
    }
    ...
    pub async fn initialize(
        ...
        timeout: Option<Duration>,
        send_elicitation: REDACTED_SECRETElicitation,
    ) -> Result<InitializeResult> {
        let client_handler = LoggingClientHandler:REDACTED_SECRETparams.clone(), send_elicitation);
    ...
    fn meta_string(meta: Option<&rmcp::model::Meta>, key: REDACTED_SECRET) -> Option<String> {
                    ...
                    service
                        .send_notification(ClientNotification::CustomNotification(
                            CustomNotification {
                                ...
                                params,
                                extensions: Extensions:REDACTED_SECRET),
                            },
                        ))
                        .await
                }
    ...
    pub async fn send_custom_request(
        ...
        let response = self
            .run_service_operation("requests/custom", /*timeout*/ None, move |service| {
                ...
                async move {
                    service
                        .send_request(ClientRequest::CustomRequest(CustomRequest:REDACTED_SECRET
                            method, params,
                        )))
                        .await
                }
    ...
    async fn create_pending_transport(
        ...
        match transport_recipe {
            TransportRecipe::Stdio {
                ...
                let mut command = Command:REDACTED_SECRETresolved_program);
                command
                    .kill_on_drop(true)
                    .stdin(Stdio::piped())
                    .stdout(Stdio::piped())
                    ...
                    .await
                    {
                            ...
                            }) =>
                        {
                            ...
                            let transport = StreamableHttpClientTransport::with_client(
                                StreamableHttpResponseClient:REDACTED_SECREThttp_client),
                                http_config,
                            );
    ...
    async fn connect_pending_transport(
        ...
        Ok((Arc:REDACTED_SECRETservice), oauth_persistor, process_group_guard))
    }
    ...
    fn is_session_expired_404(error: &ClientOperationError) -> bool {
        let ClientOperationError::Service(rmcp::service::ServiceError::TransportREDACTED_SECRET(error)) =
            error
        else {
            return false;
        };
...
async fn create_oauth_transport_and_runtime(
    ...
    let http_client = build_http_client(&default_headers)?;
    let mut oauth_state = OAuthState:REDACTED_SECRETurl.to_string(), Some(http_client.clone())).await?;
    ...
    let auth_client = AuthClient:REDACTED_SECRETStreamableHttpResponseClient:REDACTED_SECREThttp_client), manager);
    ...
    let runtime = OAuthPersistor:REDACTED_SECRET
