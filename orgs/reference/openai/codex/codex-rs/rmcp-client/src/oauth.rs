...
const KEYRING_SERVICE: REDACTED_SECRET = "Codex MCP Credentials";
...
fn load_oauth_tokens_from_keyring<K: KeyringStore>(
    ...
    match keyring_store.load(KEYRING_SERVICE, &key) {
        ...
        Ok(None) => Ok(None),
        Err(error) => Err(Error:REDACTED_SECRETerror.into_error())),
    }
}
...
fn save_oauth_tokens_with_keyring<K: KeyringStore>(
    ...
    match keyring_store.save(KEYRING_SERVICE, &key, &serialized) {
        ...
        Err(error) => {
            ...
            warn!("{message}");
            Err(Error:REDACTED_SECRETerror.into_error()).context(message))
        }
    }
}
...
impl OAuthPersistor {
    pub(crate) fn new(
        ...
        Self {
            inner: Arc:REDACTED_SECRETOAuthPersistorInner {
                server_name,
                url,
                authorization_manager,
                store_mode,
                last_credentials: Mutex:REDACTED_SECRETinitial_credentials),
            }),
        }
    }
...
const FALLBACK_FILENAME: REDACTED_SECRET = ".credentials.json";
const MCP_SERVER_TYPE: REDACTED_SECRET = "http";

type FallbackFile = REDACTED_SECRET, FallbackTokenEntry>;
...
fn load_oauth_tokens_from_file(server_name: REDACTED_SECRET, url: REDACTED_SECRET) -> Result<Option<StoredOAuthTokens>> {
    ...
    for entry in store.values() {
        ...
        let mut token_response = OAuthTokenResponse:REDACTED_SECRET
            AccessToken:REDACTED_SECRETentry.access_token.clone()),
            BasicTokenType::Bearer,
            EmptyExtraTokenFields {},
        );

        if let Some(refresh) = entry.refresh_token.clone() {
            token_response.set_refresh_token(Some(RefreshToken:REDACTED_SECRETrefresh)));
        }
...
fn delete_oauth_tokens_from_file(key: REDACTED_SECRET) -> Result<bool> {
...
fn compute_store_key(server_name: REDACTED_SECRET, server_url: REDACTED_SECRET) -> Result<String> {
    let mut payload = JsonMap:REDACTED_SECRET);
    payload.insert(
        "type".to_string(),
        Value::String(MCP_SERVER_TYPE.to_string()),
    );
    payload.insert("url".to_string(), Value::String(server_url.to_string()));
    payload.insert("headers".to_string(), Value::Object(JsonMap:REDACTED_SECRET)));
...
fn sha_256_prefix(value: &Value) -> Result<String> {
    ...
    let mut hasher = Sha256:REDACTED_SECRET);
...
mod tests {
    ...
    fn load_oauth_tokens_reads_from_keyring_when_available() -> Result<()> {
        let _env = TempCodexHome:REDACTED_SECRET);
    ...
    fn sample_tokens() -> StoredOAuthTokens {
        let mut response = OAuthTokenResponse:REDACTED_SECRET
            AccessToken:REDACTED_SECRET"access-token".to_string()),
            BasicTokenType::Bearer,
            EmptyExtraTokenFields {},
        );
        response.set_refresh_token(Some(RefreshToken:REDACTED_SECRET"refresh-token".to_string())));
        response.set_scopes(Some(vec![
            Scope:REDACTED_SECRET"scope-a".to_string()),
            Scope:REDACTED_SECRET"scope-b".to_string()),
        ]));
