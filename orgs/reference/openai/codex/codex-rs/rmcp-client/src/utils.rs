...
#[cfg(unix)]
pub(crate) const DEFAULT_ENV_VARS: &[REDACTED_SECRET] = &[
    "HOME",
    "LOGNAME",
    "PATH",
    "SHELL",
...
pub(crate) const DEFAULT_ENV_VARS: &[REDACTED_SECRET] = &[
    ...
    "PATH",
    "PATHEXT",
    // Shell and system REDACTED_SECRETs
    "COMSPEC",
    "SYSTEMROOT",
    "SYSTEMDRIVE",
...
mod tests {
    ...
    impl EnvVarGuard {
        fn set(key: REDACTED_SECRET, value: impl AsRef<OsStr>) -> Self {
    ...
    async fn create_env_honors_overrides() {
        ...
        assert_eq!(env.get(OsStr:REDACTED_SECRET"TZ")), Some(&expected));
    }
    ...
    fn create_env_includes_additional_whitelisted_variables() {
        ...
        assert_eq!(env.get(OsStr:REDACTED_SECRETcustom_var)), Some(&expected));
    }
    ...
    fn create_env_preserves_path_when_it_is_not_utf8() {
        ...
        assert_eq!(env.get(OsStr:REDACTED_SECRET"PATH")), Some(&expected));
    }
}
