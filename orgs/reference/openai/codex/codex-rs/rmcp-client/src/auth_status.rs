...
const OAUTH_DISCOVERY_HEADER: REDACTED_SECRET = "MCP-Protocol-Version";
const OAUTH_DISCOVERY_VERSION: REDACTED_SECRET = "2024-11-05";
...
pub async fn supports_oauth_login(url: REDACTED_SECRET) -> Result<bool> {
...
fn normalize_scopes(scopes_supported: Option<Vec<String>>) -> Option<Vec<String>> {
    ...
    let mut normalized = Vec:REDACTED_SECRET);
...
fn discovery_paths(base_path: REDACTED_SECRET) -> Vec<String> {
...
mod tests {
    ...
    async fn spawn_oauth_discovery_server(metadata: serde_json::Value) -> TestServer {
        ...
        let app = Router:REDACTED_SECRET).route(
    ...
    impl EnvVarGuard {
        fn set(key: REDACTED_SECRET, value: REDACTED_SECRET) -> Self {
