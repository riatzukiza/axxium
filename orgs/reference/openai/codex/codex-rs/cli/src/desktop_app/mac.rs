...
async fn open_codex_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    ...
    let status = Command:REDACTED_SECRET"open")
        .arg("-a")
        .arg(app_path)
        .arg(workspace)
        .status()
...
async fn download_and_install_codex_to_user_applications(dmg_url: REDACTED_SECRET) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder:REDACTED_SECRET)
        .prefix("codex-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_REDACTED_SECRET = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_REDACTED_SECRET.join("Codex.dmg");
...
async fn download_dmg(url: REDACTED_SECRET, dest: &Path) -> anyhow::Result<()> {
    eprintln!("Downloading installer...");
    let status = Command:REDACTED_SECRET"curl")
        .arg("-fL")
        .arg("--retry")
        .arg("3")
        .arg("--retry-delay")
...
async fn mount_dmg(dmg_path: &Path) -> anyhow::Result<PathBuf> {
    let output = Command:REDACTED_SECRET"hdiutil")
        .arg("attach")
        .arg("-nobrowse")
        .arg("-readonly")
        .arg(dmg_path)
...
async fn detach_dmg(mount_point: &Path) -> anyhow::Result<()> {
    let status = Command:REDACTED_SECRET"hdiutil")
        .arg("detach")
        .arg(mount_point)
        .status()
        .await
...
async fn copy_app_bundle(src_app: &Path, dest_app: &Path) -> anyhow::Result<()> {
    let status = Command:REDACTED_SECRET"ditto")
        .arg(src_app)
        .arg(dest_app)
        .status()
        .await
...
fn parse_hdiutil_attach_mount_point(output: REDACTED_SECRET) -> Option<String> {
