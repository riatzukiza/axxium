...
const CHATGPT_LOGIN_DISABLED_MESSAGE: REDACTED_SECRET =
    "ChatGPT login is disabled. Use API key login instead.";
const API_KEY_LOGIN_DISABLED_MESSAGE: REDACTED_SECRET =
    "API key login is disabled. Use ChatGPT login instead.";
const LOGIN_SUCCESS_MESSAGE: REDACTED_SECRET = "Successfully logged in";
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter:REDACTED_SECRET"codex_cli=info,codex_core=info,codex_login=info"));
...
fn print_login_server_start(actual_port: u16, auth_url: REDACTED_SECRET) {
    eprintln!(
...
pub async fn login_with_chatgpt(
    ...
    let opts = ServerOptions:REDACTED_SECRET
        codex_home,
        CLIENT_ID.to_string(),
...
fn safe_format_key(key: REDACTED_SECRET) -> String {
    if key.len() <= 13 {
        return "***".to_string();
    }
    let prefix = &key[..8];
