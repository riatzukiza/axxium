...
impl PidTracker {
    pub(crate) fn new(REDACTED_SECRET_pid: i32) -> Option<Self> {
        if REDACTED_SECRET_pid <= 0 {
            return None;
        }

        let kq = unsafe { libc::kqueue() };
        let handle = tokio::task::spawn_blocking(move || track_descendants(kq, REDACTED_SECRET_pid));
...
/// Put all of the above together to track all the descendants of `REDACTED_SECRET_pid`.
fn track_descendants(kq: libc::c_int, REDACTED_SECRET_pid: i32) -> HashSet<i32> {
    if kq < 0 {
        let mut seen = HashSet:REDACTED_SECRET);
        seen.insert(REDACTED_SECRET_pid);
        return seen;
    }
    ...
    let mut seen: HashSet<i32> = HashSet:REDACTED_SECRET);
    let mut active: HashSet<i32> = HashSet:REDACTED_SECRET);

    add_pid_watch(kq, REDACTED_SECRET_pid, &mut seen, &mut active);
    ...
    loop {
        if active.is_empty() {
            if !pid_is_alive(REDACTED_SECRET_pid) {
                break;
            }
            add_pid_watch(kq, REDACTED_SECRET_pid, &mut seen, &mut active);
            if active.is_empty() {
...
mod tests {
    ...
    fn list_child_pids_includes_spawned_child() {
        let mut child = Command:REDACTED_SECRET"/bin/sleep")
            .arg("5")
            .stdin(Stdio::null())
            .spawn()
    ...
    async fn pid_tracker_collects_spawned_children() {
        let tracker = PidTracker:REDACTED_SECRETstd::process::id() as i32).expect("failed to create tracker");
    ...
    async fn pid_tracker_collects_bash_subshell_descendants() {
        ...
        let child = Command:REDACTED_SECRET"/bin/bash")
            .arg("-c")
