...
#[command(
    group(
        ArgGroup:REDACTED_SECRET"transport")
            .args(["command", "url"])
            .required(true)
            .multiple(false)
    )
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    let mut stdio_rows: Vec<[String; 7]> = Vec:REDACTED_SECRET);
    let mut http_rows: Vec<[String; 5]> = Vec:REDACTED_SECRET);
...
fn parse_env_pair(raw: REDACTED_SECRET) -> Result<(String, String), String> {
...
fn validate_server_name(name: REDACTED_SECRET) -> Result<()> {
