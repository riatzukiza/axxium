...
enum DebugAppServerSubcommand {
    // REDACTED_SECRET message to app server V2.
    REDACTED_SECRETMessageV2(DebugAppServerREDACTED_SECRETMessageV2Command),
}

#[derive(Debug, Parser)]
struct DebugAppServerREDACTED_SECRETMessageV2Command {
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    let status = {
        #[cfg(windows)]
        {
            ...
            std::process::Command:REDACTED_SECRET"cmd")
                .args(["/C", &cmd_str])
                .status()?
        }
        #[cfg(not(windows))]
        {
            ...
            let normalized_args: Vec<String> = args
                ...
                .collect();
            std::process::Command:REDACTED_SECRET&command_path)
...
async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::REDACTED_SECRETMessageV2(cmd) => {
...
impl FeatureToggles {
    ...
    fn validate_feature(feature: REDACTED_SECRET) -> anyhow::Result<()> {
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    let MultitoolCli {
        config_overrides: mut REDACTED_SECRET_config_overrides,
        feature_toggles,
        remote,
        mut interactive,
        subcommand,
    ...
    REDACTED_SECRET_config_overrides.raw_overrides.extend(toggle_overrides);
    let REDACTED_SECRET_remote = remote.remote;
    let REDACTED_SECRET_remote_auth_token_env = remote.remote_auth_token_env;

    match subcommand {
        None => {
            prepend_config_flags(
                &mut interactive.config_overrides,
                REDACTED_SECRET_config_overrides.clone(),
            );
            let exit_info = run_interactive_tui(
                interactive,
                REDACTED_SECRET_remote.clone(),
                REDACTED_SECRET_remote_auth_token_env.clone(),
                arg0_paths.clone(),
            )
            .await?;
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            reject_remote_mode_for_subcommand(
                REDACTED_SECRET_remote.as_deref(),
                REDACTED_SECRET_remote_auth_token_env.as_deref(),
                "exec",
            )?;
            prepend_config_flags(
        ...
        Some(Subcommand::McpServer) => {
            ...
            codex_mcp_server::run_main(arg0_paths.clone(), REDACTED_SECRET_config_overrides).await?;
        }
        Some(Subcommand::Mcp(mut mcp_cli)) => {
            ...
            // Propagate any REDACTED_SECRET-level config overrides (e.g. `-c key=value`).
            prepend_config_flags(&mut mcp_cli.config_overrides, REDACTED_SECRET_config_overrides.clone());
            mcp_cli.run().await?;
        }
            ...
            match subcommand {
                None => {
                    ...
                    codex_app_server::run_main_with_transport(
                        arg0_paths.clone(),
                        REDACTED_SECRET_config_overrides,
        ...
        Some(Subcommand::Resume(ResumeCommand {
            ...
            let exit_info = run_interactive_tui(
                interactive,
                remote.remote.or(REDACTED_SECRET_remote.clone()),
                remote
                    .remote_auth_token_env
                    .or(REDACTED_SECRET_remote_auth_token_env.clone()),
                arg0_paths.clone(),
            )
            .await?;
            handle_app_exit(exit_info)?;
        ...
        Some(Subcommand::Debug(DebugCommand { subcommand })) => match subcommand {
            ...
            DebugSubcommand::ClearMemories => {
                ...
                run_debug_clear_memories_command(&REDACTED_SECRET_config_overrides, &interactive).await?;
            }
        },
        ...
        Some(Subcommand::Features(FeaturesCli { sub })) => match sub {
            FeaturesSubcommand::List => {
                ...
                // Respect REDACTED_SECRET-level `-c` overrides plus top-level flags like `--profile`.
                let mut cli_kv_overrides = REDACTED_SECRET_config_overrides
                    .parse_overrides()
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: REDACTED_SECRET) -> anyhow::Result<()> {
    FeatureToggles::validate_feature(feature)?;
    let codex_home = find_codex_home()?;
    ConfigEditsBuilder:REDACTED_SECRET&codex_home)
...
async fn disable_feature_in_config(interactive: &TuiCli, feature: REDACTED_SECRET) -> anyhow::Result<()> {
    FeatureToggles::validate_feature(feature)?;
    let codex_home = find_codex_home()?;
    ConfigEditsBuilder:REDACTED_SECRET&codex_home)
        .with_profile(interactive.config_profile.as_deref())
...
async fn run_debug_prompt_input_command(
    ...
    REDACTED_SECRET_config_overrides: CliConfigOverrides,
    interactive: TuiCli,
    ...
    let overrides = ConfigOverrides {
        ...
        ephemeral: Some(true),
        additional_writable_REDACTED_SECRETs: interactive.add_dir,
        ..Default::default()
    };
    let config =
    ...
    let mut input = interactive
        ...
        .into_iter()
        .chain(cmd.images)
        .map(|path| UserInput:REDACTED_SECRETImage { path })
        .collect::<Vec<_>>();
...
async fn run_debug_clear_memories_command(
    ...
    let cli_kv_overrides = REDACTED_SECRET_config_overrides
        .parse_overrides()
        .map_err(anyhow::Error::msg)?;
    let overrides = ConfigOverrides {
        config_profile: interactive.config_profile.clone(),
    ...
    let memory_REDACTED_SECRET = config.codex_home.join("memories");
    let removed_memory_REDACTED_SECRET = match tokio::fs::remove_dir_all(&memory_REDACTED_SECRET).await {
        Ok(()) => true,
    ...
    if removed_memory_REDACTED_SECRET {
        message.push_str(&format!(" Removed {}.", memory_REDACTED_SECRET.display()));
    } else {
...
/// Prepend REDACTED_SECRET-level overrides so they have lower precedence than
...
fn reject_remote_mode_for_subcommand(
    remote: Option<REDACTED_SECRET>,
    remote_auth_token_env: Option<REDACTED_SECRET>,
    subcommand: REDACTED_SECRET,
) -> anyhow::Result<()> {
...
fn read_remote_auth_token_from_env_var_with<F>(
    env_var_name: REDACTED_SECRET,
    get_var: F,
) -> anyhow::Result<String>
where
    F: FnOnce(REDACTED_SECRET) -> Result<String, std::env::VarError>,
{
...
fn read_remote_auth_token_from_env_var(env_var_name: REDACTED_SECRET) -> anyhow::Result<String> {
...
fn confirm(prompt: REDACTED_SECRET) -> std::io::Result<bool> {
    eprintln!("{prompt}");
...
fn finalize_resume_interactive(
    ...
    // Propagate any REDACTED_SECRET-level config overrides (e.g. `-c key=value`).
    prepend_config_flags(&mut interactive.config_overrides, REDACTED_SECRET_config_overrides);
...
/// REDACTED_SECRET-level flags. Only overrides fields explicitly set on the subcommand-scoped
...
mod tests {
    ...
    fn finalize_resume_from_args(args: &[REDACTED_SECRET]) -> TuiCli {
        let cli = MultitoolCli::try_parse_from(args).expect("parse");
        let MultitoolCli {
            interactive,
            config_overrides: REDACTED_SECRET_overrides,
            subcommand,
            feature_toggles: _,
            remote: _,
        } = cli;
    ...
    fn finalize_fork_from_args(args: &[REDACTED_SECRET]) -> TuiCli {
        let cli = MultitoolCli::try_parse_from(args).expect("parse");
        let MultitoolCli {
            interactive,
            config_overrides: REDACTED_SECRET_overrides,
        ...
        finalize_fork_interactive(interactive, REDACTED_SECRET_overrides, session_id, last, all, fork_cli)
    }
    ...
    fn app_server_from_args(args: &[REDACTED_SECRET]) -> AppServerCommand {
        let cli = MultitoolCli::try_parse_from(args).expect("parse");
    ...
    fn sample_exit_info(conversation_id: Option<REDACTED_SECRET>, thread_name: Option<REDACTED_SECRET>) -> AppExitInfo {
    ...
    #[test]
    fn resume_model_flag_applies_when_no_REDACTED_SECRET_flags() {
        let interactive =
    ...
    fn resume_merges_option_flags_and_full_auto() {
        ...
        assert_eq!(
            ...
            Some(std::path::Path:REDACTED_SECRET"/tmp"))
        );
        assert!(interactive.web_search);
        let has_a = interactive
            .images
            .iter()
            .any(|p| p == std::path::Path:REDACTED_SECRET"/tmp/a.png"));
        let has_b = interactive
            .images
            .iter()
            .any(|p| p == std::path::Path:REDACTED_SECRET"/tmp/b.png"));
        assert!(has_a && has_b);
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    #[test]
    fn remote_flag_parses_for_interactive_REDACTED_SECRET() {
    ...
    #[test]
    fn remote_auth_token_env_flag_parses_for_interactive_REDACTED_SECRET() {
        let cli = MultitoolCli::try_parse_from([
            "codex",
