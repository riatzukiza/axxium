...
impl TerminalInfo {
    ...
    fn from_term_program_and_term(
        ...
        Self:REDACTED_SECRETname, Some(term_program), version, term, multiplexer)
    }
...
static TERMINAL_INFO: OnceLock<TerminalInfo> = OnceLock:REDACTED_SECRET);
...
fn is_tmux_term_program(value: REDACTED_SECRET) -> bool {
...
fn split_term_program_and_version(value: REDACTED_SECRET) -> (String, Option<String>) {
...
fn tmux_display_message(format: REDACTED_SECRET) -> Option<String> {
    let output = std::process::Command:REDACTED_SECRET"tmux")
...
fn terminal_name_from_term_program(value: REDACTED_SECRET) -> Option<TerminalName> {
...
fn format_terminal_version(name: REDACTED_SECRET, version: &Option<String>) -> String {
