...
impl FakeEnvironment {
    fn new() -> Self {
        Self {
            vars: HashMap:REDACTED_SECRET),
            tmux_client_info: TmuxClientInfo::default(),
        }
    }

    fn with_var(mut self, key: REDACTED_SECRET, value: REDACTED_SECRET) -> Self {
        self.vars.insert(key.to_string(), value.to_string());
        self
    }

    fn with_tmux_client_info(mut self, termtype: Option<REDACTED_SECRET>, termname: Option<REDACTED_SECRET>) -> Self {
...
impl Environment for FakeEnvironment {
    fn var(&self, name: REDACTED_SECRET) -> Option<String> {
...
fn terminal_info(
    name: TerminalName,
    term_program: Option<REDACTED_SECRET>,
    version: Option<REDACTED_SECRET>,
    term: Option<REDACTED_SECRET>,
...
fn detects_term_program() {
    let env = FakeEnvironment:REDACTED_SECRET)
...
#[test]
fn detects_iterm2() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("ITERM_SESSION_ID", "w0t1p0");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
fn detects_apple_terminal() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("TERM_PROGRAM", "Apple_Terminal");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
    ...
    let env = FakeEnvironment:REDACTED_SECRET).with_var("TERM_SESSION_ID", "A1B2C3");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
#[test]
fn detects_ghostty() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("TERM_PROGRAM", "Ghostty");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
fn detects_zellij_multiplexer() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("ZELLIJ", "1");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        TerminalInfo {
...
#[test]
fn detects_wezterm() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("WEZTERM_VERSION", "2024.2");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
    ...
    let env = FakeEnvironment:REDACTED_SECRET).with_var("WEZTERM_VERSION", "");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
#[test]
fn detects_kitty() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("KITTY_WINDOW_ID", "1");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
fn detects_alacritty() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("ALACRITTY_SOCKET", "/tmp/alacritty");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
    ...
    let env = FakeEnvironment:REDACTED_SECRET).with_var("TERM", "alacritty");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
#[test]
fn detects_konsole() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("KONSOLE_VERSION", "230800");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
    ...
    let env = FakeEnvironment:REDACTED_SECRET).with_var("KONSOLE_VERSION", "");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
fn detects_gnome_terminal() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("GNOME_TERMINAL_SCREEN", "1");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
#[test]
fn detects_vte() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("VTE_VERSION", "7000");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
    ...
    let env = FakeEnvironment:REDACTED_SECRET).with_var("VTE_VERSION", "");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
fn detects_windows_terminal() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("WT_SESSION", "1");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
...
fn detects_term_fallbacks() {
    let env = FakeEnvironment:REDACTED_SECRET).with_var("TERM", "xterm-256color");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
    ...
    let env = FakeEnvironment:REDACTED_SECRET).with_var("TERM", "dumb");
    let terminal = detect_terminal_info_from_env(&env);
    assert_eq!(
        terminal,
        terminal_info(
