...
- When the default bubblewrap pipeline is active, writable REDACTED_SECRETs are layered with `--bind <REDACTED_SECRET> <REDACTED_SECRET>`.
- When the default bubblewrap pipeline is active, protected subpaths under writable REDACTED_SECRETs (for
  example `.git`,
...
- When the default bubblewrap pipeline is active, symlink-in-path and non-existent protected paths inside
  writable REDACTED_SECRETs are blocked by mounting `/dev/null` on the symlink or first
  missing component.
