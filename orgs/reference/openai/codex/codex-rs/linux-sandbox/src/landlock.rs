...
pub(crate) fn apply_sandbox_policy_to_current_thread(
    ...
    if apply_landlock_fs && !sandbox_policy.has_full_disk_write_access() {
        ...
        let writable_REDACTED_SECRETs = sandbox_policy
            .get_writable_REDACTED_SECRETs_with_cwd(cwd)
            .into_iter()
            .map(|writable_REDACTED_SECRET| writable_REDACTED_SECRET.REDACTED_SECRET)
            .collect();
        install_filesystem_landlock_rules_on_current_thread(writable_REDACTED_SECRETs)?;
    }
...
/// `/dev/null` and the provided list of `writable_REDACTED_SECRETs`.
///
/// # Errors
...
fn install_filesystem_landlock_rules_on_current_thread(
    ...
    if !writable_REDACTED_SECRETs.is_empty() {
        ruleset = ruleset.add_rules(landlock::path_beneath_rules(&writable_REDACTED_SECRETs, access_rw))?;
    }
...
fn install_network_seccomp_filter_on_current_thread(
    ...
    // Build rule map.
    let mut rules: BTreeMap<i64, Vec<SeccompRule>> = BTreeMap:REDACTED_SECRET);
    ...
    match mode {
        NetworkSeccompMode::Restricted => {
            ...
            // everything else.
            let unix_only_rule = SeccompRule:REDACTED_SECRETvec![SeccompCondition:REDACTED_SECRET
        ...
        NetworkSeccompMode::ProxyRouted => {
            ...
            let deny_non_ip_socket = SeccompRule:REDACTED_SECRETvec![
                SeccompCondition:REDACTED_SECRET
                    0,
                    SeccompCmpArgLen::Dword,
                    SeccompCmpOp::Ne,
                    libc::AF_INET as u64,
            ...
            let deny_unix_socketpair = SeccompRule:REDACTED_SECRETvec![SeccompCondition:REDACTED_SECRET
                0,
                SeccompCmpArgLen::Dword,
                SeccompCmpOp::Eq,
                libc::AF_UNIX as u64,
    ...
    let filter = SeccompFilter:REDACTED_SECRET
        rules,
