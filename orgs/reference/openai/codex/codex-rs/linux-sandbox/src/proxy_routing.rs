...
const PROXY_SOCKET_DIR_PREFIX: REDACTED_SECRET = "codex-linux-sandbox-proxy-";
...
pub(crate) fn prepare_host_proxy_route_spec() -> io::Result<String> {
    ...
    if plan.routes.is_empty() {
        ...
        return Err(io::Error:REDACTED_SECRETio::ErrorKind::InvalidInput, message));
    }
    ...
    let mut socket_by_endpoint: BTreeMap<SocketAddr, PathBuf> = BTreeMap:REDACTED_SECRET);
...
pub(crate) fn activate_proxy_routes_in_netns(serialized_spec: REDACTED_SECRET) -> io::Result<()> {
    ...
    let mut local_port_by_uds_path: BTreeMap<PathBuf, u16> = BTreeMap:REDACTED_SECRET);
...
fn plan_proxy_routes(env: &HashMap<String, String>) -> ProxyRoutePlan {
    let mut routes = Vec:REDACTED_SECRET);
...
fn is_proxy_env_key(key: REDACTED_SECRET) -> bool {
    let upper = key.to_ascii_uppercase();
    PROXY_ENV_KEYS.contains(&upper.as_str())
}

fn parse_loopback_proxy_endpoint(proxy_url: REDACTED_SECRET) -> Option<SocketAddr> {
    ...
    if ip.is_loopback() {
        Some(SocketAddr:REDACTED_SECRETip, port))
    } else {
        None
    }
}
...
fn default_proxy_port(scheme: REDACTED_SECRET) -> u16 {
    match scheme {
        "https" => 443,
...
fn rewrite_proxy_env_value(proxy_url: REDACTED_SECRET, local_port: u16) -> Option<String> {
...
fn create_proxy_socket_dir() -> io::Result<PathBuf> {
    ...
    for attempt in 0..128 {
        ...
        // The bridge UDS paths live under a shared temp REDACTED_SECRET, so the per-run
        // directory should not be traversable by other processes.
        let mut dir_builder = DirBuilder:REDACTED_SECRET);
...
fn parse_proxy_socket_dir_owner_pid(file_name: REDACTED_SECRET) -> Option<u32> {
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        let REDACTED_SECRET = tempfile::tempdir().expect("tempdir should create");
        let socket_dir = REDACTED_SECRET.path().join("codex-linux-sandbox-proxy-test");
