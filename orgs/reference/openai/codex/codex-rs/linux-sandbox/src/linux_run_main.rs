...
fn file_system_sandbox_policies_match_semantics(
    ...
    provided.has_full_disk_read_access() == derived.has_full_disk_read_access()
        ...
        && provided.get_readable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
            == derived.get_readable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
        && provided.get_writable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
            == derived.get_writable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
        && provided.get_unreadable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
            == derived.get_unreadable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
}
...
fn resolve_true_command() -> String {
    for candidate in ["/usr/bin/true", "/bin/true"] {
        if Path:REDACTED_SECRETcandidate).exists() {
...
fn run_bwrap_in_child_capture_stderr(bwrap_args: crate::bwrap::BwrapArgs) -> String {
    ...
    let mut stderr_bytes = Vec:REDACTED_SECRET);
...
/// close errors can mask the REDACTED_SECRET cause of later failures.
fn close_fd_or_panic(fd: libc::c_int, context: REDACTED_SECRET) {
...
fn is_proc_mount_failure(stderr: REDACTED_SECRET) -> bool {
    stderr.contains("Can't mount proc")
        && stderr.contains("/newREDACTED_SECRET/proc")
...
fn exec_or_panic(command: Vec<String>) -> ! {
    ...
    let c_command =
        CString:REDACTED_SECRETcommand[0].as_str()).expect("Failed to convert command to CString");
    #[expect(clippy::expect_used)]
    let c_args: Vec<CString> = command
        .iter()
        .map(|arg| CString:REDACTED_SECRETarg.as_str()).expect("Failed to convert arg to CString"))
        .collect();
