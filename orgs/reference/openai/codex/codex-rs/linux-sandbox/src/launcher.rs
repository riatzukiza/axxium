...
fn preferred_bwrap_launcher() -> BubblewrapLauncher {
    static LAUNCHER: OnceLock<BubblewrapLauncher> = OnceLock:REDACTED_SECRET);
    LAUNCHER
...
fn system_bwrap_supports_argv0(system_bwrap_path: &Path) -> bool {
    ...
    // in that case.
    let output = match Command:REDACTED_SECRETsystem_bwrap_path).arg("--help").output() {
        Ok(output) => output,
        Err(_) => return false,
    };
...
fn exec_system_bwrap(
    ...
    let program = CString:REDACTED_SECRETprogram.as_path().as_os_str().as_bytes())
...
fn argv_to_cstrings(argv: &[String]) -> Vec<CString> {
    ...
    for arg in argv {
        match CString:REDACTED_SECRETarg.as_str()) {
...
mod tests {
    ...
    fn prefers_system_bwrap_when_help_lists_argv0() {
        let fake_bwrap = NamedTempFile:REDACTED_SECRET).expect("temp file");
    ...
    fn falls_back_to_vendored_when_system_bwrap_is_missing() {
        assert_eq!(
            preferred_bwrap_launcher_for_path(Path:REDACTED_SECRET"/definitely/not/a/bwrap")),
    ...
    fn preserved_files_are_made_inheritable_for_system_exec() {
        let file = NamedTempFile:REDACTED_SECRET).expect("temp file");
