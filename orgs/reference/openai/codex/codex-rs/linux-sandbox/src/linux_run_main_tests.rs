...
fn detects_proc_mount_invalid_argument_failure() {
    let stderr = "bwrap: Can't mount proc on /newREDACTED_SECRET/proc: Invalid argument";
    assert!(is_proc_mount_failure(stderr));
}

#[test]
fn detects_proc_mount_operation_not_permitted_failure() {
    let stderr = "bwrap: Can't mount proc on /newREDACTED_SECRET/proc: Operation not permitted";
    assert!(is_proc_mount_failure(stderr));
}

#[test]
fn detects_proc_mount_permission_denied_failure() {
    let stderr = "bwrap: Can't mount proc on /newREDACTED_SECRET/proc: Permission denied";
    assert!(is_proc_mount_failure(stderr));
}
...
#[test]
fn REDACTED_SECRET_write_read_only_carveout_requires_direct_runtime_enforcement() {
    let temp_dir = tempfile::TempDir:REDACTED_SECRET).expect("tempdir");
    let docs = temp_dir.path().join("docs");
    std::fs::create_dir_all(&docs).expect("create docs");
    let docs = AbsolutePathBuf::from_absolute_path(&docs).expect("absolute docs");
...
fn managed_proxy_inner_command_includes_route_spec() {
    ...
    let args = build_inner_seccomp_command(InnerSeccompCommandArgs {
        sandbox_policy_cwd: Path:REDACTED_SECRET"/tmp"),
        command_cwd: Some(Path:REDACTED_SECRET"/tmp/link")),
...
fn resolve_sandbox_policies_accepts_semantically_equivalent_workspace_write_inputs() {
    ...
    let sandbox_policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![workspace],
