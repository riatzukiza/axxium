...
const BWRAP_UNAVAILABLE_ERR: REDACTED_SECRET = "build-time bubblewrap is not available in this build.";
...
async fn run_cmd(cmd: &[REDACTED_SECRET], writable_REDACTED_SECRETs: &[PathBuf], timeout_ms: u64) {
    let output = run_cmd_output(cmd, writable_REDACTED_SECRETs, timeout_ms).await;
...
async fn run_cmd_output(
    cmd: &[REDACTED_SECRET],
    writable_REDACTED_SECRETs: &[PathBuf],
    timeout_ms: u64,
) -> codex_protocol::exec_output::ExecToolCallOutput {
    run_cmd_result_with_writable_REDACTED_SECRETs(
        cmd,
...
async fn run_cmd_result_with_writable_REDACTED_SECRETs(
    cmd: &[REDACTED_SECRET],
    writable_REDACTED_SECRETs: &[PathBuf],
    timeout_ms: u64,
    ...
    let sandbox_policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: writable_REDACTED_SECRETs
            .iter()
            .map(|p| AbsolutePathBuf::try_from(p.as_path()).unwrap())
            .collect(),
        read_only_access: Default::default(),
        network_access,
        // Exclude tmp-related folders from writable REDACTED_SECRETs because we need a
...
fn is_bwrap_unavailable_output(output: &codex_protocol::exec_output::ExecToolCallOutput) -> bool {
    output.stderr.text.contains(BWRAP_UNAVAILABLE_ERR)
        || (output
            .stderr
            .text
            .contains("Can't mount proc on /newREDACTED_SECRET/proc")
...
async fn should_skip_bwrap_tests() -> bool {
    match run_cmd_result_with_writable_REDACTED_SECRETs(
        &["bash", "-lc", "true"],
        &[],
        NETWORK_TIMEOUT_MS,
        /*use_legacy_landlock*/ false,
...
#[tokio::test]
async fn test_REDACTED_SECRET_read() {
    run_cmd(&["ls", "-l", "/bin"], &[], SHORT_TIMEOUT_MS).await;
}

#[tokio::test]
#[should_panic]
async fn test_REDACTED_SECRET_write() {
    let tmpfile = NamedTempFile:REDACTED_SECRET).unwrap();
...
async fn test_dev_null_write() {
    ...
    let output = run_cmd_result_with_writable_REDACTED_SECRETs(
...
#[tokio::test]
async fn bwrap_populates_minimal_dev_REDACTED_SECRETs() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let output = run_cmd_result_with_writable_REDACTED_SECRETs(
        &[
            "bash",
            "-lc",
            "for REDACTED_SECRET in null zero full random urandom tty; do [ -c \"/dev/$REDACTED_SECRET\" ] || { echo \"missing /dev/$REDACTED_SECRET\" >&2; exit 1; }; done",
        ],
        &[],
        LONG_TIMEOUT_MS,
        /*use_legacy_landlock*/ false,
...
async fn bwrap_preserves_writable_dev_shm_bind_mount() {
    ...
    if !std::path::Path:REDACTED_SECRET"/dev/shm").exists() {
...
#[tokio::test]
async fn test_writable_REDACTED_SECRET() {
...
#[tokio::test]
async fn sandbox_ignores_missing_writable_REDACTED_SECRETs_under_bwrap() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let existing_REDACTED_SECRET = tempdir.path().join("existing");
    let missing_REDACTED_SECRET = tempdir.path().join("missing");
    std::fs::create_dir(&existing_REDACTED_SECRET).expect("create existing REDACTED_SECRET");

    let output = run_cmd_result_with_writable_REDACTED_SECRETs(
        &["bash", "-lc", "printf sandbox-ok"],
        &[existing_REDACTED_SECRET, missing_REDACTED_SECRET],
        LONG_TIMEOUT_MS,
        /*use_legacy_landlock*/ false,
        /*network_access*/ true,
    )
...
#[expect(clippy::expect_used)]
async fn assert_network_blocked(cmd: &[REDACTED_SECRET]) {
    let cwd = std::env::current_dir().expect("cwd should exist");
    let sandbox_cwd = cwd.clone();
    let params = ExecParams {
        command: cmd.iter().copied().map(str::to_owned).collect(),
...
#[tokio::test]
async fn sandbox_blocks_git_and_codex_writes_inside_writable_REDACTED_SECRET() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![AbsolutePathBuf::try_from(tmpdir.path()).expect("absolute tempdir")],
        read_only_access: Default::default(),
        network_access: true,
        exclude_tmpdir_env_var: true,
        exclude_slash_tmp: true,
...
#[tokio::test]
async fn sandbox_blocks_REDACTED_SECRET_read_carveouts_under_bwrap() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let output = expect_denied(
        ...
        .await,
        "REDACTED_SECRET-read carveout should be denied under bubblewrap",
    );
