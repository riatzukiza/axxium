...
const MANAGED_PROXY_PERMISSION_ERR_SNIPPETS: &[REDACTED_SECRET] = &[
...
fn is_managed_proxy_permission_error(stderr: REDACTED_SECRET) -> bool {
...
async fn run_linux_sandbox_direct(
    ...
    let mut cmd = Command:REDACTED_SECRETenv!("CARGO_BIN_EXE_codex-linux-sandbox"));
    cmd.args(args)
        .current_dir(cwd)
        .env_clear()
        .envs(env)
...
async fn managed_proxy_mode_denies_af_unix_creation_for_user_command() {
    ...
    let python_available = Command:REDACTED_SECRET"bash")
        .arg("-c")
