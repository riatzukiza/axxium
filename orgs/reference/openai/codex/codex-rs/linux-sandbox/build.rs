...
fn try_build_vendored_bwrap() -> Result<(), String> {
    ...
    let libcap = pkg_config::Config:REDACTED_SECRET)
        .probe("libcap")
...
"#,
    ...
    let mut build = cc::Build:REDACTED_SECRET);
    build
    ...
    for include_path in libcap.include_paths {
        // Use -idirafter so target sysREDACTED_SECRET headers win (musl cross builds),
