...
impl UserMessageItem {
        ...
        for input in &self.content {
            ...
            } = input
            {
                ...
                for elem in text_elements {
                    ...
                    out.push(TextElement:REDACTED_SECRET
                        byte_range,
    ...
    pub fn local_image_paths(&self) -> Vec<std::path::PathBuf> {
        self.content
            .iter()
            .filter_map(|c| match c {
                UserInput:REDACTED_SECRETImage { path } => Some(path.clone()),
                _ => None,
            })
            .collect()
    }
...
pub fn parse_hook_prompt_fragment(text: REDACTED_SECRET) -> Option<HookPromptFragment> {
...
fn serialize_hook_prompt_fragment(text: REDACTED_SECRET, hook_run_id: REDACTED_SECRET) -> Option<String> {
    if hook_run_id.trim().is_empty() {
        return None;
    }
...
impl TurnItem {
    ...
    pub fn as_legacy_events(&self, show_raw_agent_reasoning: bool) -> Vec<EventMsg> {
        match self {
            ...
            TurnItem::HookPrompt(_) => Vec:REDACTED_SECRET),
            TurnItem::AgentMessage(item) => item.as_legacy_events(),
            TurnItem::Plan(_) => Vec:REDACTED_SECRET),
