...
fn usage_limit_reached_error_formats_plus_plan() {
    let err = UsageLimitReachedError {
        ...
        resets_at: None,
        rate_limits: Some(Box:REDACTED_SECRETrate_limit_snapshot())),
        promo_message: None,
    };
    assert_eq!(
        err.to_string(),
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    let output = ExecToolCallOutput {
        ...
        aggregated_output: StreamOutput:REDACTED_SECRET"aggregate detail".to_string()),
...
fn sandbox_denied_reports_both_streams_when_available() {
    let output = ExecToolCallOutput {
        exit_code: 9,
        stdout: StreamOutput:REDACTED_SECRET"stdout detail".to_string()),
        stderr: StreamOutput:REDACTED_SECRET"stderr detail".to_string()),
        aggregated_output: StreamOutput:REDACTED_SECRETString:REDACTED_SECRET)),
        duration: Duration::from_millis(10),
        timed_out: false,
    };
    let err = CodexErr::Sandbox(SandboxErr::Denied {
...
fn sandbox_denied_reports_stdout_when_no_stderr() {
    let output = ExecToolCallOutput {
        exit_code: 11,
        stdout: StreamOutput:REDACTED_SECRET"stdout only".to_string()),
        stderr: StreamOutput:REDACTED_SECRETString:REDACTED_SECRET)),
        aggregated_output: StreamOutput:REDACTED_SECRETString:REDACTED_SECRET)),
...
fn usage_limit_reached_error_hides_upsell_for_non_codex_limit_name() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            plan_type: Some(PlanType::Known(KnownPlan::Plus)),
            resets_at: Some(resets_at),
            rate_limits: Some(Box:REDACTED_SECRETRateLimitSnapshot {
