...
impl FileSystemSpecialPath {
    pub fn project_REDACTED_SECRETs(subpath: Option<PathBuf>) -> Self {
...
struct FileSystemSemanticSignature {
    ...
    readable_REDACTED_SECRETs: Vec<AbsolutePathBuf>,
    writable_REDACTED_SECRETs: Vec<WritableRoot>,
    unreadable_REDACTED_SECRETs: Vec<AbsolutePathBuf>,
}
...
impl FileSystemSandboxPolicy {
    fn has_REDACTED_SECRET_access(&self, predicate: impl Fn(FileSystemAccessMode) -> bool) -> bool {
    ...
    /// reduces a broader `:REDACTED_SECRET = write` grant.
    ///
    ...
    ///
    /// Legacy `WorkspaceWrite` policies may list readable REDACTED_SECRETs that live
    /// under an already-writable REDACTED_SECRET. Those paths were redundant in the
    ...
    pub fn from_legacy_sandbox_policy(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_REDACTED_SECRETs, .. } = sandbox_policy {
            let legacy_writable_REDACTED_SECRETs = sandbox_policy.get_writable_REDACTED_SECRETs_with_cwd(cwd);
            file_system_policy.entries.retain(|entry| {
                ...
                match &entry.path {
                    FileSystemPath::Path { path } => !legacy_writable_REDACTED_SECRETs
                        .iter()
                        .any(|REDACTED_SECRET| REDACTED_SECRET.is_path_writable(path.as_path())),
                    FileSystemPath::Special { .. } => true,
                }
            });

            if let Ok(cwd_REDACTED_SECRET) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_REDACTED_SECRET(
                    &cwd_REDACTED_SECRET, /*protect_missing_dot_codex*/ true,
                ) {
            ...
            for writable_REDACTED_SECRET in writable_REDACTED_SECRETs {
                for protected_path in default_read_only_subpaths_for_writable_REDACTED_SECRET(
                    writable_REDACTED_SECRET,
    ...
    pub fn has_full_disk_read_access(&self) -> bool {
        match self.kind {
            ...
            FileSystemSandboxKind::Restricted => {
                self.has_REDACTED_SECRET_access(FileSystemAccessMode::can_read)
    ...
    pub fn has_full_disk_write_access(&self) -> bool {
        match self.kind {
            FileSystemSandboxKind::Unrestricted | FileSystemSandboxKind::ExternalSandbox => true,
            FileSystemSandboxKind::Restricted => {
                self.has_REDACTED_SECRET_access(FileSystemAccessMode::can_write)
                    && !self.has_write_narrowing_entries()
            }
        }
    }

    /// Returns true when platform-default readable REDACTED_SECRETs should be included.
    ...
    pub fn with_additional_readable_REDACTED_SECRETs(
        mut self,
        cwd: &Path,
        additional_readable_REDACTED_SECRETs: &[AbsolutePathBuf],
    ) -> Self {
        if self.has_full_disk_read_access() {
            return self;
        }

        for path in additional_readable_REDACTED_SECRETs {
    ...
    pub fn with_additional_writable_REDACTED_SECRETs(
        mut self,
        cwd: &Path,
        additional_writable_REDACTED_SECRETs: &[AbsolutePathBuf],
    ) -> Self {
        for path in additional_writable_REDACTED_SECRETs {
    ...
    /// Returns the explicit readable REDACTED_SECRETs resolved against the provided cwd.
    pub fn get_readable_REDACTED_SECRETs_with_cwd(&self, cwd: &Path) -> Vec<AbsolutePathBuf> {
        if self.has_full_disk_read_access() {
    ...
    /// Returns the writable REDACTED_SECRETs together with read-only carveouts resolved
    /// against the provided cwd.
    pub fn get_writable_REDACTED_SECRETs_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .into_iter()
        .map(|REDACTED_SECRET| {
            // Filesystem-REDACTED_SECRET policies stay in their effective canonical form
            // so REDACTED_SECRET-wide aliases do not create duplicate top-level masks.
            // Example: keep `/var/...` normalized under `/` instead of
            // materializing both `/var/...` and `/private/var/...`.
            let preserve_raw_carveout_paths = REDACTED_SECRET.as_path().parent().is_some();
            let raw_writable_REDACTED_SECRETs: Vec<&AbsolutePathBuf> = writable_entries
                .iter()
                .filter(|path| normalize_effective_absolute_path((*path).clone()) == REDACTED_SECRET)
                .collect();
            let protect_missing_dot_codex = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == REDACTED_SECRET);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_REDACTED_SECRET(&REDACTED_SECRET, protect_missing_dot_codex)
                    .into_iter()
                    .filter(|path| !has_explicit_resolved_path_entry(&resolved_entries, path))
                    .collect();
            // Narrower explicit non-write entries carve out broader writable REDACTED_SECRETs.
            ...
            // Preserve symlink path components that live under the writable REDACTED_SECRET
            // so downstream sandboxes can still mask the symlink iREDACTED_SECRET itself.
            // Example: if `<REDACTED_SECRET>/.codex -> <REDACTED_SECRET>/decoy`, bwrap must still see
            // `<REDACTED_SECRET>/.codex`, not only the resolved `<REDACTED_SECRET>/decoy`.
            read_only_subpaths.extend(
                resolved_entries
                    ...
                    .filter_map(|entry| {
                        ...
                        // Preserve the literal in-REDACTED_SECRET path whenever the
                        // carveout itself lives under this writable REDACTED_SECRET, even
                        // if following symlinks would resolve back to the REDACTED_SECRET
                        // or escape outside it. Downstream sandboxes need that
                        // raw path so they can mask the symlink iREDACTED_SECRET itself.
                        // Examples:
                        // - `<REDACTED_SECRET>/linked-private -> <REDACTED_SECRET>/decoy-private`
                        // - `<REDACTED_SECRET>/linked-private -> /tmp/outside-private`
                        // - `<REDACTED_SECRET>/alias-REDACTED_SECRET -> <REDACTED_SECRET>`
                        let raw_carveout_path = if preserve_raw_carveout_paths {
                            if entry.path == REDACTED_SECRET {
                                None
                            } else if entry.path.as_path().starts_with(REDACTED_SECRET.as_path()) {
                                Some(entry.path.clone())
                            } else {
                                raw_writable_REDACTED_SECRETs.iter().find_map(|raw_REDACTED_SECRET| {
                                    let suffix = entry
                                        .path
                                        .as_path()
                                        .strip_prefix(raw_REDACTED_SECRET.as_path())
                                        .ok()?;
                                    if suffix.as_os_str().is_empty() {
                                        return None;
                                    }
                                    REDACTED_SECRET.join(suffix).ok()
                                })
                            }
                        } else {
                            None
                        ...
                        if effective_path == REDACTED_SECRET
                            || !effective_path.as_path().starts_with(REDACTED_SECRET.as_path())
                        {
                            return None;
                        }
            ...
            WritableRoot {
                REDACTED_SECRET,
                // Preserve literal in-REDACTED_SECRET protected paths like `.git` and
    ...
    /// Returns explicit unreadable REDACTED_SECRETs resolved against the provided cwd.
    pub fn get_unreadable_REDACTED_SECRETs_with_cwd(&self, cwd: &Path) -> Vec<AbsolutePathBuf> {
        if !matches!(self.kind, FileSystemSandboxKind::Restricted) {
            return Vec:REDACTED_SECRET);
        }

        let REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(cwd)
            .ok()
            .map(|cwd| absolute_REDACTED_SECRET_path_for_cwd(&cwd));
        ...
        dedup_absolute_paths(
            self.resolved_entries_with_cwd(cwd)
                ...
                .filter(|entry| !self.can_read_path_with_cwd(entry.path.as_path(), cwd))
                // Restricted policies already deny reads outside explicit allow REDACTED_SECRETs,
                // so materializing the filesystem REDACTED_SECRET here would erase narrower
                // readable carveouts when downstream sandboxes apply deny masks last.
                .filter(|entry| REDACTED_SECRET.as_ref() != Some(&entry.path))
                .map(|entry| entry.path.clone())
                .collect(),
            /*normalize_effective_paths*/ true,
        )
    ...
    pub fn to_legacy_sandbox_policy(
        ...
        Ok(match self.kind {
            ...
            FileSystemSandboxKind::Restricted => {
                ...
                let mut workspace_REDACTED_SECRET_writable = false;
                let mut writable_REDACTED_SECRETs = Vec:REDACTED_SECRET);
                let mut readable_REDACTED_SECRETs = Vec:REDACTED_SECRET);
                        ...
                        FileSystemPath::Path { path } => {
                            if entry.access.can_write() {
                                if cwd_absolute.as_ref().is_some_and(|cwd| cwd == path) {
                                    workspace_REDACTED_SECRET_writable = true;
                                } else {
                                    writable_REDACTED_SECRETs.push(path.clone());
                                }
                            } else if entry.access.can_read() {
                                readable_REDACTED_SECRETs.push(path.clone());
                            }
                        }
                        FileSystemPath::Special { value } => match value {
                            ...
                            FileSystemSpecialPath::CurrentWorkingDirectory => {
                                ...
                                {
                                    readable_REDACTED_SECRETs.push(path);
                                }
                            }
                            FileSystemSpecialPath::ProjectRoots { subpath } => {
                                ...
                                {
                                    if entry.access.can_write() {
                                        writable_REDACTED_SECRETs.push(path);
                                    } else if entry.access.can_read() {
                                        readable_REDACTED_SECRETs.push(path);
                                    }
                                }
                ...
                let read_only_access = if has_full_disk_read_access {
                    ...
                    ReadOnlyAccess::Restricted {
                        ...
                        readable_REDACTED_SECRETs: dedup_absolute_paths(
                            readable_REDACTED_SECRETs,
                            /*normalize_effective_paths*/ false,
                        ),
                    }
                };

                if workspace_REDACTED_SECRET_writable {
                    SandboxPolicy::WorkspaceWrite {
                        writable_REDACTED_SECRETs: dedup_absolute_paths(
                            writable_REDACTED_SECRETs,
                            /*normalize_effective_paths*/ false,
                        ),
                        read_only_access,
                ...
                } else if !writable_REDACTED_SECRETs.is_empty() || tmpdir_writable || slash_tmp_writable {
                    return Err(io::Error:REDACTED_SECRET
                        io::ErrorKind::InvalidInput,
                        "permissions profile requests filesystem writes outside the workspace REDACTED_SECRET, which is not supported until the runtime enforces FileSystemSandboxPolicy directly",
                    ));
                } else {
    ...
    fn semantic_signature(&self, cwd: &Path) -> FileSystemSemanticSignature {
        FileSystemSemanticSignature {
            ...
            readable_REDACTED_SECRETs: self.get_readable_REDACTED_SECRETs_with_cwd(cwd),
            writable_REDACTED_SECRETs: self.get_writable_REDACTED_SECRETs_with_cwd(cwd),
            unreadable_REDACTED_SECRETs: self.get_unreadable_REDACTED_SECRETs_with_cwd(cwd),
        }
    }
}
    ...
    fn from(value: &SandboxPolicy) -> Self {
        match value {
            ...
            SandboxPolicy::ReadOnly { access, .. } => {
                let mut entries = Vec:REDACTED_SECRET);
                match access {
                    ...
                    ReadOnlyAccess::Restricted {
                        ...
                        entries.extend(readable_REDACTED_SECRETs.iter().cloned().map(|path| {
...
fn resolve_entry_path(
    ...
    match path {
        FileSystemPath::Special {
            value: FileSystemSpecialPath::Root,
        } => cwd.map(absolute_REDACTED_SECRET_path_for_cwd),
...
fn special_path_matches_absolute_path(
    ...
    match value {
        ...
        FileSystemSpecialPath::SlashTmp => path.as_path() == Path:REDACTED_SECRET"/tmp"),
        _ => false,
    }
}
...
fn absolute_REDACTED_SECRET_path_for_cwd(cwd: &AbsolutePathBuf) -> AbsolutePathBuf {
    let REDACTED_SECRET = cwd
        .as_path()
        .ancestors()
        .last()
        .unwrap_or_else(|| panic!("cwd must have a filesystem REDACTED_SECRET"));
    AbsolutePathBuf::from_absolute_path(REDACTED_SECRET)
        .unwrap_or_else(|err| panic!("cwd REDACTED_SECRET must be an absolute path: {err}"))
}
...
fn dedup_absolute_paths(
    ...
    let mut seen = HashSet:REDACTED_SECRET);
    for path in paths {
...
fn default_read_only_subpaths_for_writable_REDACTED_SECRET(
    writable_REDACTED_SECRET: &AbsolutePathBuf,
    protect_missing_dot_codex: bool,
) -> Vec<AbsolutePathBuf> {
    let mut subpaths: Vec<AbsolutePathBuf> = Vec:REDACTED_SECRET);
    #[allow(clippy::expect_used)]
    let top_level_git = writable_REDACTED_SECRET
        .join(".git")
        .expect(".git is a valid relative path");
    // This applies to typical repos (directory .git), worktrees/submodules
    // (file .git with gitdir pointer), and bare repos when the gitdir is the
    // writable REDACTED_SECRET itself.
    ...
    #[allow(clippy::expect_used)]
    let top_level_agents = writable_REDACTED_SECRET.join(".agents").expect("valid relative path");
    if top_level_agents.as_path().is_dir() {
        subpaths.push(top_level_agents);
    }

    // Keep top-level project metadata under .codex read-only to the agent by
    // default. For the workspace REDACTED_SECRET itself, protect it even before the
    // directory exists so first-time creation still goes through the
    // protected-path approval flow.
    #[allow(clippy::expect_used)]
    let top_level_codex = writable_REDACTED_SECRET.join(".codex").expect("valid relative path");
...
fn is_git_pointer_file(path: &AbsolutePathBuf) -> bool {
    path.as_path().is_file() && path.as_path().file_name() == Some(OsStr:REDACTED_SECRET".git"))
}
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: REDACTED_SECRET = "CODEX_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn unknown_special_paths_are_ignored_by_legacy_bridge() -> std::io::Result<()> {
        ...
        let sandbox_policy = policy.to_legacy_sandbox_policy(
            NetworkSandboxPolicy::Restricted,
            Path:REDACTED_SECRET"/tmp/workspace"),
        )?;
    ...
    #[cfg(unix)]
    #[test]
    fn writable_REDACTED_SECRETs_proactively_protect_missing_dot_codex() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let expected_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical REDACTED_SECRET");
        let expected_dot_codex = expected_REDACTED_SECRET.join(".codex").expect("expected .codex path");
        ...
        let writable_REDACTED_SECRETs = policy.get_writable_REDACTED_SECRETs_with_cwd(cwd.path());
        assert_eq!(writable_REDACTED_SECRETs.len(), 1);
        assert_eq!(writable_REDACTED_SECRETs[0].REDACTED_SECRET, expected_REDACTED_SECRET);
        assert!(
            writable_REDACTED_SECRETs[0]
                .read_only_subpaths
    ...
    #[cfg(unix)]
    #[test]
    fn writable_REDACTED_SECRETs_skip_default_dot_codex_when_explicit_user_rule_exists() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let expected_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical REDACTED_SECRET");
        let explicit_dot_codex = expected_REDACTED_SECRET.join(".codex").expect("expected .codex path");
        ...
        let writable_REDACTED_SECRETs = policy.get_writable_REDACTED_SECRETs_with_cwd(cwd.path());
        let workspace_REDACTED_SECRET = writable_REDACTED_SECRETs
            .iter()
            .find(|REDACTED_SECRET| REDACTED_SECRET.REDACTED_SECRET == expected_REDACTED_SECRET)
            .expect("workspace writable REDACTED_SECRET");
        assert!(
            !workspace_REDACTED_SECRET
                .read_only_subpaths
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        let relative_cwd = Path:REDACTED_SECRET"workspace");
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path:REDACTED_SECRET".codex/config.toml"), relative_cwd,)
        );
    }

    #[cfg(unix)]
    #[test]
    fn effective_runtime_REDACTED_SECRETs_canonicalize_symlinked_paths() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let real_REDACTED_SECRET = cwd.path().join("real");
        let link_REDACTED_SECRET = cwd.path().join("link");
        let blocked = real_REDACTED_SECRET.join("blocked");
        let codex_dir = real_REDACTED_SECRET.join(".codex");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&codex_dir).expect("create .codex");
        symlink_dir(&real_REDACTED_SECRET, &link_REDACTED_SECRET).expect("create symlinked REDACTED_SECRET");

        let link_REDACTED_SECRET =
            AbsolutePathBuf::from_absolute_path(&link_REDACTED_SECRET).expect("absolute symlinked REDACTED_SECRET");
        let link_blocked = link_REDACTED_SECRET.join("blocked").expect("symlinked blocked path");
        let expected_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(
            real_REDACTED_SECRET.canonicalize().expect("canonicalize real REDACTED_SECRET"),
        )
        .expect("absolute canonical REDACTED_SECRET");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Path { path: link_REDACTED_SECRET },
                access: FileSystemAccessMode::Write,
            },
            FileSystemSandboxEntry {
        ...
        assert_eq!(
            policy.get_unreadable_REDACTED_SECRETs_with_cwd(cwd.path()),
    ...
    fn current_working_directory_special_path_canonicalizes_symlinked_cwd() {
        ...
        let link_REDACTED_SECRET = cwd.path().join("link");
        let blocked = real_REDACTED_SECRET.join("blocked");
        let agents_dir = real_REDACTED_SECRET.join(".agents");
        let codex_dir = real_REDACTED_SECRET.join(".codex");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&agents_dir).expect("create .agents");
        fs::create_dir_all(&codex_dir).expect("create .codex");
        symlink_dir(&real_REDACTED_SECRET, &link_REDACTED_SECRET).expect("create symlinked cwd");

        let link_blocked =
            AbsolutePathBuf::from_absolute_path(link_REDACTED_SECRET.join("blocked")).expect("link blocked");
        let expected_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(
            real_REDACTED_SECRET.canonicalize().expect("canonicalize real REDACTED_SECRET"),
        )
        .expect("absolute canonical REDACTED_SECRET");
        ...
        assert_eq!(
            policy.get_readable_REDACTED_SECRETs_with_cwd(&link_REDACTED_SECRET),
            vec![expected_REDACTED_SECRET.clone()]
        );
        assert_eq!(
            policy.get_unreadable_REDACTED_SECRETs_with_cwd(&link_REDACTED_SECRET),
            vec![expected_blocked.clone()]
        );

        let writable_REDACTED_SECRETs = policy.get_writable_REDACTED_SECRETs_with_cwd(&link_REDACTED_SECRET);
        assert_eq!(writable_REDACTED_SECRETs.len(), 1);
        assert_eq!(writable_REDACTED_SECRETs[0].REDACTED_SECRET, expected_REDACTED_SECRET);
        assert!(
            writable_REDACTED_SECRETs[0]
    ...
    #[cfg(unix)]
    #[test]
    fn writable_REDACTED_SECRETs_preserve_symlinked_protected_subpaths() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let REDACTED_SECRET = cwd.path().join("REDACTED_SECRET");
        let decoy = REDACTED_SECRET.join("decoy-codex");
        let dot_codex = REDACTED_SECRET.join(".codex");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_codex).expect("create .codex symlink");

        let REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(&REDACTED_SECRET).expect("absolute REDACTED_SECRET");
        let expected_dot_codex = AbsolutePathBuf::from_absolute_path(
            REDACTED_SECRET.as_path()
                .canonicalize()
                .expect("canonicalize REDACTED_SECRET")
                .join(".codex"),
        )
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![FileSystemSandboxEntry {
            path: FileSystemPath::Path { path: REDACTED_SECRET },
            access: FileSystemAccessMode::Write,
        }]);
        ...
        assert_eq!(writable_REDACTED_SECRETs.len(), 1);
        assert_eq!(
            writable_REDACTED_SECRETs[0].read_only_subpaths,
    ...
    #[cfg(unix)]
    #[test]
    fn writable_REDACTED_SECRETs_preserve_explicit_symlinked_carveouts_under_symlinked_REDACTED_SECRETs() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let real_REDACTED_SECRET = cwd.path().join("real");
        let link_REDACTED_SECRET = cwd.path().join("link");
        let decoy = real_REDACTED_SECRET.join("decoy-private");
        let linked_private = real_REDACTED_SECRET.join("linked-private");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&real_REDACTED_SECRET, &link_REDACTED_SECRET).expect("create symlinked REDACTED_SECRET");
        ...
        let link_REDACTED_SECRET =
            AbsolutePathBuf::from_absolute_path(&link_REDACTED_SECRET).expect("absolute symlinked REDACTED_SECRET");
        let link_private = link_REDACTED_SECRET
            .join("linked-private")
        ...
        .expect("absolute canonical REDACTED_SECRET");
        let expected_linked_private = expected_REDACTED_SECRET
            .join("linked-private")
    ...
    #[cfg(unix)]
    #[test]
    fn writable_REDACTED_SECRETs_preserve_explicit_symlinked_carveouts_that_escape_REDACTED_SECRET() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let real_REDACTED_SECRET = cwd.path().join("real");
        let link_REDACTED_SECRET = cwd.path().join("link");
        ...
        let linked_private = real_REDACTED_SECRET.join("linked-private");
        fs::create_dir_all(&decoy).expect("create decoy");
        fs::create_dir_all(&real_REDACTED_SECRET).expect("create real REDACTED_SECRET");
        symlink_dir(&real_REDACTED_SECRET, &link_REDACTED_SECRET).expect("create symlinked REDACTED_SECRET");
        symlink_dir(&decoy, &linked_private).expect("create linked-private symlink");
    ...
    #[cfg(unix)]
    #[test]
    fn writable_REDACTED_SECRETs_preserve_explicit_symlinked_carveouts_that_alias_REDACTED_SECRET() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let REDACTED_SECRET = cwd.path().join("REDACTED_SECRET");
        let alias = REDACTED_SECRET.join("alias-REDACTED_SECRET");
        fs::create_dir_all(&REDACTED_SECRET).expect("create REDACTED_SECRET");
        symlink_dir(&REDACTED_SECRET, &alias).expect("create alias symlink");

        let REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(&REDACTED_SECRET).expect("absolute REDACTED_SECRET");
        let alias = REDACTED_SECRET.join("alias-REDACTED_SECRET").expect("alias REDACTED_SECRET path");
        let expected_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(
            REDACTED_SECRET.as_path().canonicalize().expect("canonicalize REDACTED_SECRET"),
        )
        .expect("absolute canonical REDACTED_SECRET");
        let expected_alias = expected_REDACTED_SECRET
            .join("alias-REDACTED_SECRET")
        ...
        assert_eq!(writable_REDACTED_SECRETs.len(), 1);
        assert_eq!(writable_REDACTED_SECRETs[0].REDACTED_SECRET, expected_REDACTED_SECRET);
        assert_eq!(writable_REDACTED_SECRETs[0].read_only_subpaths, vec![expected_alias]);
    }
    ...
    fn tmpdir_special_path_canonicalizes_symlinked_tmpdir() {
        if std::env::var_os(SYMLINKED_TMPDIR_TEST_ENV).is_none() {
            let output = std::process::Command:REDACTED_SECRETstd::env::current_exe().expect("test binary"))
    ...
    fn resolve_access_with_cwd_uses_most_specific_entry() {
        ...
        let docs_private_REDACTED_SECRET =
            AbsolutePathBuf::resolve_path_against_base("docs/private/REDACTED_SECRET", cwd.path())
                .expect("resolve docs/private/REDACTED_SECRET");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::CurrentWorkingDirectory,
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: docs_private_REDACTED_SECRET.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert_eq!(
            policy.resolve_access_with_cwd(docs_private_REDACTED_SECRET.as_path(), cwd.path()),
            FileSystemAccessMode::Write
        );
    }
    ...
    #[test]
    fn REDACTED_SECRET_write_with_read_only_child_is_not_full_disk_write() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let docs =
            AbsolutePathBuf::resolve_path_against_base("docs", cwd.path()).expect("resolve docs");
        let policy = FileSystemSandboxPolicy::restricted(vec![
    ...
    #[test]
    fn REDACTED_SECRET_deny_does_not_materialize_as_unreadable_REDACTED_SECRET() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let docs =
            AbsolutePathBuf::resolve_path_against_base("docs", cwd.path()).expect("resolve docs");
        ...
        assert_eq!(
            policy.get_readable_REDACTED_SECRETs_with_cwd(cwd.path()),
            vec![expected_docs]
        );
        assert!(policy.get_unreadable_REDACTED_SECRETs_with_cwd(cwd.path()).is_empty());
    }

    #[test]
    fn duplicate_REDACTED_SECRET_deny_prevents_full_disk_write_access() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(cwd.path())
            .map(|cwd| absolute_REDACTED_SECRET_path_for_cwd(&cwd))
            .expect("resolve filesystem REDACTED_SECRET");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::Root,
        ...
        assert!(!policy.has_full_disk_write_access());
        assert_eq!(
            policy.resolve_access_with_cwd(REDACTED_SECRET.as_path(), cwd.path()),
            FileSystemAccessMode::None
        );
    }
    ...
    #[test]
    fn with_additional_readable_REDACTED_SECRETs_skips_existing_effective_access() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let cwd_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(cwd.path()).expect("absolute cwd");
        let policy = FileSystemSandboxPolicy::restricted(vec![FileSystemSandboxEntry {
            path: FileSystemPath::Special {
                value: FileSystemSpecialPath::CurrentWorkingDirectory,
            },
        ...
        let actual = policy
            .clone()
            .with_additional_readable_REDACTED_SECRETs(cwd.path(), std::slice::from_ref(&cwd_REDACTED_SECRET));

        assert_eq!(actual, policy);
    }

    #[test]
    fn with_additional_writable_REDACTED_SECRETs_skips_existing_effective_access() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let cwd_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(cwd.path()).expect("absolute cwd");
        let policy = FileSystemSandboxPolicy::restricted(vec![FileSystemSandboxEntry {
            path: FileSystemPath::Special {
        ...
        let actual = policy
            .clone()
            .with_additional_writable_REDACTED_SECRETs(cwd.path(), std::slice::from_ref(&cwd_REDACTED_SECRET));

        assert_eq!(actual, policy);
    }

    #[test]
    fn with_additional_writable_REDACTED_SECRETs_adds_new_REDACTED_SECRET() {
        let temp_dir = TempDir:REDACTED_SECRET).expect("tempdir");
        let cwd = temp_dir.path().join("workspace");
        let extra = AbsolutePathBuf::from_absolute_path(temp_dir.path().join("extra"))
            .expect("resolve extra REDACTED_SECRET");
        let policy = FileSystemSandboxPolicy::restricted(vec![FileSystemSandboxEntry {
            path: FileSystemPath::Special {
                value: FileSystemSpecialPath::CurrentWorkingDirectory,
            },
        ...
        let actual = policy.with_additional_writable_REDACTED_SECRETs(&cwd, std::slice::from_ref(&extra));
