...
static SANDBOX_MODE_DANGER_FULL_ACCESS_TEMPLATE: LazyLock<Template> = LazyLock:REDACTED_SECRET|| {
...
static SANDBOX_MODE_WORKSPACE_WRITE_TEMPLATE: LazyLock<Template> = LazyLock:REDACTED_SECRET|| {
...
static SANDBOX_MODE_READ_ONLY_TEMPLATE: LazyLock<Template> = LazyLock:REDACTED_SECRET|| {
...
pub const BASE_INSTRUCTIONS_DEFAULT: REDACTED_SECRET = include_str!("prompts/base_instructions/default.md");
...
const APPROVAL_POLICY_NEVER: REDACTED_SECRET = include_str!("prompts/permissions/approval_policy/never.md");
const APPROVAL_POLICY_UNLESS_TRUSTED: REDACTED_SECRET =
    include_str!("prompts/permissions/approval_policy/unless_trusted.md");
const APPROVAL_POLICY_ON_FAILURE: REDACTED_SECRET =
    include_str!("prompts/permissions/approval_policy/on_failure.md");
const APPROVAL_POLICY_ON_REQUEST_RULE: REDACTED_SECRET =
    include_str!("prompts/permissions/approval_policy/on_request.md");
const APPROVAL_POLICY_ON_REQUEST_RULE_REQUEST_PERMISSION: REDACTED_SECRET =
    include_str!("prompts/permissions/approval_policy/on_request_rule_request_permission.md");
const GUARDIAN_SUBAGENT_APPROVAL_SUFFIX: REDACTED_SECRET = "`approvals_reviewer` is `guardian_subagent`: Sandbox escalations with require_escalated will be reviewed for compliance with the policy. If a rejection happens, you should proceed only with a materially safer alternative, or inform the user of the risk and send a final message to ask for approval.";

const SANDBOX_MODE_DANGER_FULL_ACCESS: REDACTED_SECRET =
    include_str!("prompts/permissions/sandbox_mode/danger_full_access.md");
const SANDBOX_MODE_WORKSPACE_WRITE: REDACTED_SECRET =
    include_str!("prompts/permissions/sandbox_mode/workspace_write.md");
const SANDBOX_MODE_READ_ONLY: REDACTED_SECRET = include_str!("prompts/permissions/sandbox_mode/read_only.md");

const REALTIME_START_INSTRUCTIONS: REDACTED_SECRET = include_str!("prompts/realtime/realtime_start.md");
const REALTIME_END_INSTRUCTIONS: REDACTED_SECRET = include_str!("prompts/realtime/realtime_end.md");
...
impl DeveloperInstructions {
    ...
    pub fn from(
        ...
        let with_request_permissions_tool = |text: REDACTED_SECRET| {
        ...
        DeveloperInstructions:REDACTED_SECRETtext)
    }
    ...
    pub fn model_switch_message(model_instructions: String) -> Self {
        DeveloperInstructions:REDACTED_SECRETformat!(
    ...
    pub fn realtime_start_message_with_instructions(instructions: REDACTED_SECRET) -> Self {
        DeveloperInstructions:REDACTED_SECRETformat!(
            "{REALTIME_CONVERSATION_OPEN_TAG}\n{instructions}\n{REALTIME_CONVERSATION_CLOSE_TAG}"
        ))
    }

    pub fn realtime_end_message(reason: REDACTED_SECRET) -> Self {
        DeveloperInstructions:REDACTED_SECRETformat!(
            "{REALTIME_CONVERSATION_OPEN_TAG}\n{}\n\nReason: {reason}\n{REALTIME_CONVERSATION_CLOSE_TAG}",
    ...
    pub fn personality_spec_message(spec: String) -> Self {
        ...
        DeveloperInstructions:REDACTED_SECRETmessage)
    }
    ...
    pub fn from_policy(
        ...
        let (sandbox_mode, writable_REDACTED_SECRETs) = match sandbox_policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => {
                let REDACTED_SECRETs = sandbox_policy.get_writable_REDACTED_SECRETs_with_cwd(cwd);
                (SandboxMode::WorkspaceWrite, Some(REDACTED_SECRETs))
            }
        };
    ...
    fn from_permissions_with_network(
        ...
        writable_REDACTED_SECRETs: Option<Vec<WritableRoot>>,
    ) -> Self {
        let start_tag = DeveloperInstructions:REDACTED_SECRET"<permissions instructions>");
        let end_tag = DeveloperInstructions:REDACTED_SECRET"</permissions instructions>");
        start_tag
            ...
            .concat(DeveloperInstructions::from_writable_REDACTED_SECRETs(writable_REDACTED_SECRETs))
            .concat(end_tag)
    }

    fn from_writable_REDACTED_SECRETs(writable_REDACTED_SECRETs: Option<Vec<WritableRoot>>) -> Self {
        let Some(REDACTED_SECRETs) = writable_REDACTED_SECRETs else {
            return DeveloperInstructions:REDACTED_SECRET"");
        };

        if REDACTED_SECRETs.is_empty() {
            return DeveloperInstructions:REDACTED_SECRET"");
        }

        let REDACTED_SECRETs_list: Vec<String> = REDACTED_SECRETs
            .iter()
            .map(|r| format!("`{}`", r.REDACTED_SECRET.to_string_lossy()))
            .collect();
        let text = if REDACTED_SECRETs_list.len() == 1 {
            format!(" The writable REDACTED_SECRET is {}.", REDACTED_SECRETs_list[0])
        } else {
            format!(" The writable REDACTED_SECRETs are {}.", REDACTED_SECRETs_list.join(", "))
        };
        DeveloperInstructions:REDACTED_SECRETtext)
    }
...
const TRUNCATED_MARKER: REDACTED_SECRET = "...\n[Some commands were truncated]";
...
pub const VIEW_IMAGE_TOOL_NAME: REDACTED_SECRET = "view_image";

const IMAGE_OPEN_TAG: REDACTED_SECRET = "<image>";
const IMAGE_CLOSE_TAG: REDACTED_SECRET = "</image>";
const LOCAL_IMAGE_OPEN_TAG_PREFIX: REDACTED_SECRET = "<image name=";
const LOCAL_IMAGE_OPEN_TAG_SUFFIX: REDACTED_SECRET = ">";
const LOCAL_IMAGE_CLOSE_TAG: REDACTED_SECRET = IMAGE_CLOSE_TAG;
...
pub fn is_local_image_open_tag_text(text: REDACTED_SECRET) -> bool {
    text.strip_prefix(LOCAL_IMAGE_OPEN_TAG_PREFIX)
        .is_some_and(|rest| rest.ends_with(LOCAL_IMAGE_OPEN_TAG_SUFFIX))
}

pub fn is_local_image_close_tag_text(text: REDACTED_SECRET) -> bool {
    is_image_close_tag_text(text)
}

pub fn is_image_open_tag_text(text: REDACTED_SECRET) -> bool {
    text == IMAGE_OPEN_TAG
}

pub fn is_image_close_tag_text(text: REDACTED_SECRET) -> bool {
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: REDACTED_SECRET) -> ContentItem {
    ContentItem::InputText {
        text: format!(
        ...
        Self::Message {
            ...
            content: items
                ...
                .flat_map(|c| match c {
                    ...
                    UserInput:REDACTED_SECRETImage { path } => {
                        image_index += 1;
                    ...
                    UserInput::Skill { .. } | UserInput::Mention { .. } => Vec:REDACTED_SECRET), // Tool bodies are injected later in core
                })
...
impl Default for FunctionCallOutputBody {
    fn default() -> Self {
        Self::Text(String:REDACTED_SECRET))
    }
}
...
impl FunctionCallOutputPayload {
    ...
    pub fn text_content(&self) -> Option<REDACTED_SECRET> {
        match &self.body {
...
mod tests {
    ...
    fn converts_sandbox_mode_into_developer_instructions() {
        ...
        assert_eq!(
            workspace_write,
            DeveloperInstructions:REDACTED_SECRET
                "Filesystem sandboxing defines which files can be read or written. `sandbox_mode` is `workspace-write`: The sandbox permits reading files, and editing files in `cwd` and `writable_REDACTED_SECRETs`. Editing files in other directories requires approval. Network access is restricted."
            )
        );
    ...
    fn builds_permissions_with_network_access_override() {
        let instructions = DeveloperInstructions::from_permissions_with_network(
            ...
            /*writable_REDACTED_SECRETs*/ None,
        );
    ...
    fn granular_categories_section(title: REDACTED_SECRET, categories: &[REDACTED_SECRET]) -> String {
        format!("{title}\n{}", categories.join("\n"))
    }

    fn granular_prompt_expected(
        prompted_categories: &[REDACTED_SECRET],
        rejected_categories: &[REDACTED_SECRET],
...
- ["a", "b", "c"]"#
    ...
    fn mixed_remote_and_local_images_share_label_sequence() -> Result<()> {
        ...
        let item = ResponseInputItem::from(vec![
            ...
            UserInput:REDACTED_SECRETImage { path: local_path },
        ]);
    ...
    fn local_image_read_error_adds_placeholder() -> Result<()> {
        ...
        let item = ResponseInputItem::from(vec![UserInput:REDACTED_SECRETImage {
