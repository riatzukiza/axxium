...
pub const USER_INSTRUCTIONS_OPEN_TAG: REDACTED_SECRET = "<user_instructions>";
pub const USER_INSTRUCTIONS_CLOSE_TAG: REDACTED_SECRET = "</user_instructions>";
pub const ENVIRONMENT_CONTEXT_OPEN_TAG: REDACTED_SECRET = "<environment_context>";
pub const ENVIRONMENT_CONTEXT_CLOSE_TAG: REDACTED_SECRET = "</environment_context>";
pub const APPS_INSTRUCTIONS_OPEN_TAG: REDACTED_SECRET = "<apps_instructions>";
pub const APPS_INSTRUCTIONS_CLOSE_TAG: REDACTED_SECRET = "</apps_instructions>";
pub const SKILLS_INSTRUCTIONS_OPEN_TAG: REDACTED_SECRET = "<skills_instructions>";
pub const SKILLS_INSTRUCTIONS_CLOSE_TAG: REDACTED_SECRET = "</skills_instructions>";
pub const PLUGINS_INSTRUCTIONS_OPEN_TAG: REDACTED_SECRET = "<plugins_instructions>";
pub const PLUGINS_INSTRUCTIONS_CLOSE_TAG: REDACTED_SECRET = "</plugins_instructions>";
pub const COLLABORATION_MODE_OPEN_TAG: REDACTED_SECRET = "<collaboration_mode>";
pub const COLLABORATION_MODE_CLOSE_TAG: REDACTED_SECRET = "</collaboration_mode>";
pub const REALTIME_CONVERSATION_OPEN_TAG: REDACTED_SECRET = "<realtime_conversation>";
pub const REALTIME_CONVERSATION_CLOSE_TAG: REDACTED_SECRET = "</realtime_conversation>";
pub const USER_MESSAGE_BEGIN: REDACTED_SECRET = "## My request for Codex:";
...
pub enum Op {
    ...
    /// REDACTED_SECRET audio input to the running realtime conversation stream.
    RealtimeConversationAudio(ConversationAudioParams),

    /// REDACTED_SECRET text input to the running realtime conversation stream.
...
pub enum ReadOnlyAccess {
    /// Restrict reads to an explicit set of REDACTED_SECRETs.
    ///
    /// When `include_platform_defaults` is `true`, platform defaults required
    /// for basic execution are included in addition to `readable_REDACTED_SECRETs`.
    Restricted {
        /// Include built-in platform read REDACTED_SECRETs required for basic process
        /// execution.
        #[serde(default = "default_include_platform_defaults")]
        include_platform_defaults: bool,
        /// Additional absolute REDACTED_SECRETs that should be readable.
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
impl ReadOnlyAccess {
    ...
    /// Returns the readable REDACTED_SECRETs for restricted read access.
    ///
    ...
    pub fn get_readable_REDACTED_SECRETs_with_cwd(&self, cwd: &Path) -> Vec<AbsolutePathBuf> {
        let mut REDACTED_SECRETs: Vec<AbsolutePathBuf> = match self {
            ReadOnlyAccess::FullAccess => return Vec:REDACTED_SECRET),
            ReadOnlyAccess::Restricted { readable_REDACTED_SECRETs, .. } => {
                let mut REDACTED_SECRETs = readable_REDACTED_SECRETs.clone();
                match AbsolutePathBuf::from_absolute_path(cwd) {
                    Ok(cwd_REDACTED_SECRET) => REDACTED_SECRETs.push(cwd_REDACTED_SECRET),
                    Err(err) => {
                        error!("Ignoring invalid cwd {cwd:?} for sandbox readable REDACTED_SECRET: {err}");
                    }
                }
                REDACTED_SECRETs
            }
        ...
        REDACTED_SECRETs.retain(|REDACTED_SECRET| seen.insert(REDACTED_SECRET.to_path_buf()));
        REDACTED_SECRETs
    }
}
...
pub enum SandboxPolicy {
    ...
    WorkspaceWrite {
        ...
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
        writable_REDACTED_SECRETs: Vec<AbsolutePathBuf>,
        ...
        /// environment variable among the default writable REDACTED_SECRETs. Defaults to
        /// `false`.
        #[serde(default)]
        exclude_tmpdir_env_var: bool,

        /// When set to `true`, will NOT include the `/tmp` among the default
        /// writable REDACTED_SECRETs on UNIX. Defaults to `false`.
        #[serde(default)]
        exclude_slash_tmp: bool,
    },
}

/// A writable REDACTED_SECRET path accompanied by a list of subpaths that should remain
/// read‑only even when the REDACTED_SECRET is writable. This is primarily used to ensure
...
/// a writable REDACTED_SECRET are not modified by the agent.
#[derive(Debug, Clone, PartialEq, Eq, JsonSchema)]
pub struct WritableRoot {
    pub REDACTED_SECRET: AbsolutePathBuf,

    /// By construction, these subpaths are all under `REDACTED_SECRET`.
    pub read_only_subpaths: Vec<AbsolutePathBuf>,
}

impl WritableRoot {
    pub fn is_path_writable(&self, path: &Path) -> bool {
        // Check if the path is under the REDACTED_SECRET.
        if !path.starts_with(&self.REDACTED_SECRET) {
            return false;
        }
...
impl SandboxPolicy {
    ...
    /// Returns the list of readable REDACTED_SECRETs (tailored to the current working
    ...
    pub fn get_readable_REDACTED_SECRETs_with_cwd(&self, cwd: &Path) -> Vec<AbsolutePathBuf> {
        let mut REDACTED_SECRETs = match self {
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => Vec:REDACTED_SECRET),
            SandboxPolicy::ReadOnly { access, .. } => access.get_readable_REDACTED_SECRETs_with_cwd(cwd),
            SandboxPolicy::WorkspaceWrite {
                read_only_access, ..
            } => {
                let mut REDACTED_SECRETs = read_only_access.get_readable_REDACTED_SECRETs_with_cwd(cwd);
                REDACTED_SECRETs.extend(
                    self.get_writable_REDACTED_SECRETs_with_cwd(cwd)
                        .into_iter()
                        .map(|REDACTED_SECRET| REDACTED_SECRET.REDACTED_SECRET),
                );
                REDACTED_SECRETs
            }
        };
    ...
    /// Returns the list of writable REDACTED_SECRETs (tailored to the current working
    /// directory) together with subpaths that should remain read‑only under
    /// each writable REDACTED_SECRET.
    pub fn get_writable_REDACTED_SECRETs_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        match self {
            SandboxPolicy::DangerFullAccess => Vec:REDACTED_SECRET),
            SandboxPolicy::ExternalSandbox { .. } => Vec:REDACTED_SECRET),
            SandboxPolicy::ReadOnly { .. } => Vec:REDACTED_SECRET),
            SandboxPolicy::WorkspaceWrite {
                ...
                // Start from explicitly configured writable REDACTED_SECRETs.
                let mut REDACTED_SECRETs: Vec<AbsolutePathBuf> = writable_REDACTED_SECRETs.clone();
                ...
                match cwd_absolute {
                    Ok(cwd) => {
                        REDACTED_SECRETs.push(cwd);
                    }
                    Err(e) => {
                        error!(
                            "Ignoring invalid cwd {:?} for sandbox writable REDACTED_SECRET: {}",
                            cwd, e
                        );
                    }
                }
                ...
                if cfg!(unix) && !exclude_slash_tmp {
                    ...
                    if slash_tmp.as_path().is_dir() {
                        REDACTED_SECRETs.push(slash_tmp);
                    }
                }
                    ...
                    && !tmpdir.is_empty()
                {
                    match AbsolutePathBuf::from_absolute_path(PathBuf::from(&tmpdir)) {
                        Ok(tmpdir_path) => {
                            REDACTED_SECRETs.push(tmpdir_path);
                        }
                        Err(e) => {
                            error!(
                                "Ignoring invalid TMPDIR value {tmpdir:?} for sandbox writable REDACTED_SECRET: {e}",
                            );
                        }
                    }
                }

                // For each REDACTED_SECRET, compute subpaths that should remain read-only.
                let cwd_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(cwd).ok();
                REDACTED_SECRETs
                    .into_iter()
                    .map(|writable_REDACTED_SECRET| {
                        let protect_missing_dot_codex = cwd_REDACTED_SECRET
                            .as_ref()
                            .is_some_and(|cwd_REDACTED_SECRET| cwd_REDACTED_SECRET == &writable_REDACTED_SECRET);
                        WritableRoot {
                            read_only_subpaths: default_read_only_subpaths_for_writable_REDACTED_SECRET(
                                &writable_REDACTED_SECRET,
                                protect_missing_dot_codex,
                            ),
                            REDACTED_SECRET: writable_REDACTED_SECRET,
                        }
                    })
                    .collect()
            }
...
pub struct UserMessageEvent {
    ...
    /// Local file paths sourced from `UserInput:REDACTED_SECRETImage`. These are kept so
...
impl InitialHistory {
    ...
    pub fn get_rollout_items(&self) -> Vec<RolloutItem> {
        match self {
            InitialHistory::New => Vec:REDACTED_SECRET),
...
impl SessionSource {
    pub fn from_startup_arg(value: REDACTED_SECRET) -> Result<Self, &'static str> {
...
impl Default for SessionMeta {
    fn default() -> Self {
        SessionMeta {
            id: ThreadId::default(),
            forked_from_id: None,
            timestamp: String:REDACTED_SECRET),
            cwd: PathBuf:REDACTED_SECRET),
            originator: String:REDACTED_SECRET),
            cli_version: String:REDACTED_SECRET),
...
impl Product {
    ...
    pub fn from_session_source_name(value: REDACTED_SECRET) -> Option<Self> {
...
pub struct SessionConfiguredEvent {
    ...
    /// Working directory that should be treated as the *REDACTED_SECRET* of the
    /// session.
    pub cwd: PathBuf,
    ...
    /// Identifier of the history log file (iREDACTED_SECRET on Unix, 0 otherwise).
...
impl ReviewDecision {
    /// Returns an REDACTED_SECRET version of the decision without PII. We can't use an ignored flag
    /// on `serde` because the serialization is required by some surfaces.
    pub fn to_REDACTED_SECRET_string(&self) -> &'static str {
        match self {
...
mod tests {
    ...
    fn sorted_writable_REDACTED_SECRETs(REDACTED_SECRETs: Vec<WritableRoot>) -> Vec<(PathBuf, Vec<PathBuf>)> {
        let mut sorted_REDACTED_SECRETs: Vec<(PathBuf, Vec<PathBuf>)> = REDACTED_SECRETs
            .into_iter()
            .map(|REDACTED_SECRET| {
                let mut read_only_subpaths: Vec<PathBuf> = REDACTED_SECRET
                    .read_only_subpaths
                    .into_iter()
                    .map(|path| path.to_path_buf())
                    .collect();
                read_only_subpaths.sort();
                (REDACTED_SECRET.REDACTED_SECRET.to_path_buf(), read_only_subpaths)
            })
            .collect();
        sorted_REDACTED_SECRETs.sort_by(|left, right| left.0.cmp(&right.0));
        sorted_REDACTED_SECRETs
    }
    ...
    fn sandbox_policy_allows_read(policy: &SandboxPolicy, path: &Path, cwd: &Path) -> bool {
        ...
        policy
            ...
            .iter()
            .any(|REDACTED_SECRET| path.starts_with(REDACTED_SECRET.as_path()))
            || policy
                .get_writable_REDACTED_SECRETs_with_cwd(cwd)
                .iter()
                .any(|REDACTED_SECRET| path.starts_with(REDACTED_SECRET.REDACTED_SECRET.as_path()))
    }
    ...
    fn sandbox_policy_allows_write(policy: &SandboxPolicy, path: &Path, cwd: &Path) -> bool {
        ...
        policy
            .get_writable_REDACTED_SECRETs_with_cwd(cwd)
            .iter()
            .any(|REDACTED_SECRET| REDACTED_SECRET.is_path_writable(path))
    }
    ...
    fn sandbox_policy_probe_paths(policy: &SandboxPolicy, cwd: &Path) -> Vec<PathBuf> {
        ...
        for REDACTED_SECRET in policy.get_writable_REDACTED_SECRETs_with_cwd(cwd) {
            paths.push(REDACTED_SECRET.REDACTED_SECRET.to_path_buf());
            paths.extend(
                REDACTED_SECRET.read_only_subpaths
                    .into_iter()
                    .map(|path| path.to_path_buf()),
            );
        }
    ...
    #[test]
    fn workspace_write_restricted_read_access_includes_effective_writable_REDACTED_SECRETs() {
        let cwd = if cfg!(windows) {
            Path:REDACTED_SECRETr"C:\workspace")
        } else {
        ...
        let readable_REDACTED_SECRETs = policy.get_readable_REDACTED_SECRETs_with_cwd(cwd);
        let writable_REDACTED_SECRETs = policy.get_writable_REDACTED_SECRETs_with_cwd(cwd);

        for writable_REDACTED_SECRET in writable_REDACTED_SECRETs {
            assert!(
                readable_REDACTED_SECRETs
                    .iter()
                    .any(|REDACTED_SECRET| REDACTED_SECRET.as_path() == writable_REDACTED_SECRET.REDACTED_SECRET.as_path()),
                "expected writable REDACTED_SECRET {} to also be readable",
                writable_REDACTED_SECRET.REDACTED_SECRET.as_path().display()
            );
        }
    }

    #[test]
    fn restricted_file_system_policy_reports_full_access_from_REDACTED_SECRET_entries() {
    ...
    #[test]
    fn restricted_file_system_policy_treats_REDACTED_SECRET_with_carveouts_as_scoped_access() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let canonical_cwd = cwd.path().canonicalize().expect("canonicalize cwd");
        let REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(&canonical_cwd)
            .expect("absolute canonical tempdir")
            .as_path()
            .ancestors()
            .last()
            .and_then(|path| AbsolutePathBuf::from_absolute_path(path).ok())
            .expect("filesystem REDACTED_SECRET");
        ...
        assert_eq!(writable_REDACTED_SECRETs[0].REDACTED_SECRET, REDACTED_SECRET);
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        let writable_REDACTED_SECRETs = policy.get_writable_REDACTED_SECRETs_with_cwd(cwd.path());
        assert_eq!(writable_REDACTED_SECRETs.len(), 1);
        assert_eq!(writable_REDACTED_SECRETs[0].REDACTED_SECRET, cwd_absolute);
        assert!(
            writable_REDACTED_SECRETs[0]
                .read_only_subpaths
                .iter()
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let docs_REDACTED_SECRET = AbsolutePathBuf::resolve_path_against_base("docs/REDACTED_SECRET", cwd.path())
            .expect("resolve docs/REDACTED_SECRET");
        let expected_docs = AbsolutePathBuf::from_absolute_path(canonical_cwd.join("docs"))
            .expect("canonical docs");
        let expected_docs_REDACTED_SECRET =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join("docs/REDACTED_SECRET"))
                .expect("canonical docs/REDACTED_SECRET");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path { path: docs_REDACTED_SECRET },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert!(!policy.has_full_disk_write_access());
        assert_eq!(
            sorted_writable_REDACTED_SECRETs(policy.get_writable_REDACTED_SECRETs_with_cwd(cwd.path())),
            vec![
                (
                    canonical_cwd,
                    vec![
                ...
                (expected_docs_REDACTED_SECRET.to_path_buf(), Vec:REDACTED_SECRET)),
            ]
        );
    }

    #[test]
    fn legacy_workspace_write_nested_readable_REDACTED_SECRET_stays_writable() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let docs =
            AbsolutePathBuf::resolve_path_against_base("docs", cwd.path()).expect("resolve docs");
        let canonical_cwd = cwd.path().canonicalize().expect("canonicalize cwd");
        ...
        let policy = SandboxPolicy::WorkspaceWrite {
            ...
            read_only_access: ReadOnlyAccess::Restricted {
                include_platform_defaults: true,
                readable_REDACTED_SECRETs: vec![docs],
            },
            network_access: false,
            exclude_tmpdir_env_var: true,
        ...
        assert_eq!(
            sorted_writable_REDACTED_SECRETs(
                FileSystemSandboxPolicy::from_legacy_sandbox_policy(&policy, cwd.path())
                    .get_writable_REDACTED_SECRETs_with_cwd(cwd.path())
            ),
    ...
    fn file_system_policy_rejects_legacy_bridge_for_non_workspace_writes() {
        ...
        assert!(
            err.to_string()
                .contains("filesystem writes outside the workspace REDACTED_SECRET"),
            "{err}"
        );
    }
    ...
    fn legacy_sandbox_policy_semantics_survive_split_bridge() {
        let cwd = TempDir:REDACTED_SECRET).expect("tempdir");
        let readable_REDACTED_SECRET = AbsolutePathBuf::resolve_path_against_base("readable", cwd.path())
            .expect("resolve readable REDACTED_SECRET");
        let writable_REDACTED_SECRET = AbsolutePathBuf::resolve_path_against_base("writable", cwd.path())
            .expect("resolve writable REDACTED_SECRET");
        let nested_readable_REDACTED_SECRET = AbsolutePathBuf::resolve_path_against_base("docs", cwd.path())
            .expect("resolve nested readable REDACTED_SECRET");
        let policies = [
            ...
            SandboxPolicy::ReadOnly {
                access: ReadOnlyAccess::Restricted {
                    include_platform_defaults: true,
                    readable_REDACTED_SECRETs: vec![readable_REDACTED_SECRET.clone()],
                },
                network_access: true,
            },
            SandboxPolicy::WorkspaceWrite {
            ...
            SandboxPolicy::WorkspaceWrite {
                writable_REDACTED_SECRETs: vec![writable_REDACTED_SECRET],
                read_only_access: ReadOnlyAccess::Restricted {
                    include_platform_defaults: true,
                    readable_REDACTED_SECRETs: vec![readable_REDACTED_SECRET],
                },
                network_access: true,
                exclude_tmpdir_env_var: false,
                exclude_slash_tmp: true,
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                read_only_access: ReadOnlyAccess::Restricted {
                    include_platform_defaults: true,
                    readable_REDACTED_SECRETs: vec![nested_readable_REDACTED_SECRET],
                },
                network_access: false,
                exclude_tmpdir_env_var: true,
                exclude_slash_tmp: true,
    ...
    fn item_started_event_from_web_search_emits_begin_event() {
        let event = ItemStartedEvent {
            thread_id: ThreadId:REDACTED_SECRET),
            turn_id: "turn-1".into(),
    ...
    fn item_started_event_from_non_web_search_emits_no_legacy_events() {
        let event = ItemStartedEvent {
            thread_id: ThreadId:REDACTED_SECRET),
            turn_id: "turn-1".into(),
            item: TurnItem::UserMessage(UserMessageItem:REDACTED_SECRET&[])),
        };
    ...
    fn serialize_event() -> Result<()> {
        ...
        let rollout_file = NamedTempFile:REDACTED_SECRET)?;
        let event = Event {
            id: "1234".to_string(),
