...
fn plugin_apps_for_config(config: &Config) -> Vec<codex_core::plugins::AppConnectorId> {
    PluginsManager:REDACTED_SECRETconfig.codex_home.clone())
...
pub fn merge_connectors_with_accessible(
    ...
    let accessible_connectors = if all_connectors_loaded {
        let connector_ids: HashSet<REDACTED_SECRET> = connectors
            .iter()
...
mod tests {
    ...
    fn merged_app(id: REDACTED_SECRET, is_accessible: bool) -> AppInfo {
        AppInfo {
            id: id.to_string(),
            name: id.to_string(),
            description: None,
    ...
    fn connectors_for_plugin_apps_filters_disallowed_plugin_apps() {
        ...
        assert_eq!(connectors, Vec::<AppInfo>:REDACTED_SECRET));
    }
}
