...
fn log_attributes(record: &SdkLogRecord) -> REDACTED_SECRET, String> {
    record
        .attributes_iter()
...
fn span_event_attributes(event: &opentelemetry::trace::Event) -> REDACTED_SECRET, String> {
    event
        .attributes
        .iter()
...
fn otel_export_routing_policy_routes_user_prompt_log_and_trace_events() {
    ...
    let subscriber = tracing_subscriber::registry()
        .with(
            opentelemetry_appender_tracing::layer::OpenTelemetryTracingBridge:REDACTED_SECRET
                &logger_provider,
            )
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        let manager = SessionTelemetry:REDACTED_SECRET
        ...
        let REDACTED_SECRET_span = tracing::info_span!("REDACTED_SECRET");
        let _REDACTED_SECRET_guard = REDACTED_SECRET_span.enter();
        manager.user_prompt(&[
            UserInput::Text {
