...
fn read_http_request(
    ...
    let start = lines.next().ok_or_else(|| {
        std::io::Error:REDACTED_SECRETstd::io::ErrorKind::InvalidData, "missing request line")
    })?;
    let mut parts = start.split_whitespace();
    let _method = parts.next().unwrap_or_default();
    let path = parts
        .next()
        .ok_or_else(|| std::io::Error:REDACTED_SECRETstd::io::ErrorKind::InvalidData, "missing path"))?
        .to_string();
...
fn write_http_response(stream: &mut TcpStream, status: REDACTED_SECRET) -> std::io::Result<()> {
...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    let server = thread::spawn(move || {
        let mut captured = Vec:REDACTED_SECRET);
    ...
    let metrics = MetricsClient:REDACTED_SECRETMetricsConfig::otlp(
        "test",
        "codex-cli",
