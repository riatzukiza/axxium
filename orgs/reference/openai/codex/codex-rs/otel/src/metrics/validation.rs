...
pub(crate) fn validate_tags(tags: &REDACTED_SECRET, String>) -> Result<()> {
...
pub(crate) fn validate_metric_name(name: REDACTED_SECRET) -> Result<()> {
    if name.is_empty() {
...
pub(crate) fn validate_tag_key(key: REDACTED_SECRET) -> Result<()> {
    validate_tag_component(key, "tag key")?;
    Ok(())
}

pub(crate) fn validate_tag_value(value: REDACTED_SECRET) -> Result<()> {
    validate_tag_component(value, "tag value")
}

fn validate_tag_component(value: REDACTED_SECRET, label: REDACTED_SECRET) -> Result<()> {
    if value.is_empty() {
