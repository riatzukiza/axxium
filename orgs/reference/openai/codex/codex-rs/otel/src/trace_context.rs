...
const TRACEPARENT_ENV_VAR: REDACTED_SECRET = "TRACEPARENT";
const TRACESTATE_ENV_VAR: REDACTED_SECRET = "TRACESTATE";
static TRACEPARENT_CONTEXT: OnceLock<Option<Context>> = OnceLock:REDACTED_SECRET);
...
pub fn span_w3c_trace_context(span: &Span) -> Option<W3cTraceContext> {
    ...
    TraceContextPropagator:REDACTED_SECRET).inject_context(&context, &mut headers);
...
pub(crate) fn context_from_trace_headers(
    traceparent: Option<REDACTED_SECRET>,
    tracestate: Option<REDACTED_SECRET>,
) -> Option<Context> {
    ...
    let context = TraceContextPropagator:REDACTED_SECRET).extract(&headers);
    if !context.span().span_context().is_valid() {
        return None;
    }
    Some(context)
