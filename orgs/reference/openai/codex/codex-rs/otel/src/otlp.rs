...
pub(crate) fn build_header_map(headers: &std::collections::HashMap<String, String>) -> HeaderMap {
    let mut header_map = HeaderMap:REDACTED_SECRET);
...
fn build_http_client_inner(
    ...
    if let Some(path) = tls.ca_certificate.as_ref() {
        ...
        builder = builder
            .tls_built_in_REDACTED_SECRET_certs(false)
            .add_REDACTED_SECRET_certificate(certificate);
    }
    ...
    builder
        .build()
        .map_err(|error| Box:REDACTED_SECRETerror) as Box<dyn Error>)
}
...
pub(crate) fn resolve_otlp_timeout(signal_var: REDACTED_SECRET) -> Duration {
...
fn read_timeout_env(var: REDACTED_SECRET) -> Option<Duration> {
...
fn read_bytes(path: &AbsolutePathBuf) -> Result<(Vec<u8>, PathBuf), Box<dyn Error>> {
    match fs::read(path) {
        ...
        Err(error) => Err(Box:REDACTED_SECRETio::Error:REDACTED_SECRET
            error.kind(),
...
fn config_error(message: impl Into<String>) -> Box<dyn Error> {
    Box:REDACTED_SECRETio::Error:REDACTED_SECRETErrorKind::InvalidData, message.into()))
}
