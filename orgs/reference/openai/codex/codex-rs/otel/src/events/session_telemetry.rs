...
const SSE_UNKNOWN_KIND: REDACTED_SECRET = "unknown";
const WEBSOCKET_UNKNOWN_KIND: REDACTED_SECRET = "unknown";
const RESPONSES_WEBSOCKET_TIMING_KIND: REDACTED_SECRET = "responsesapi.websocket_timing";
const RESPONSES_WEBSOCKET_TIMING_METRICS_FIELD: REDACTED_SECRET = "timing_metrics";
const RESPONSES_API_OVERHEAD_FIELD: REDACTED_SECRET = "responses_duration_excl_engine_and_client_tool_time_ms";
const RESPONSES_API_INFERENCE_FIELD: REDACTED_SECRET = "engine_service_total_ms";
const RESPONSES_API_ENGINE_IAPI_TTFT_FIELD: REDACTED_SECRET = "engine_iapi_ttft_total_ms";
const RESPONSES_API_ENGINE_SERVICE_TTFT_FIELD: REDACTED_SECRET = "engine_service_ttft_total_ms";
const RESPONSES_API_ENGINE_IAPI_TBT_FIELD: REDACTED_SECRET = "engine_iapi_tbt_across_engine_calls_ms";
const RESPONSES_API_ENGINE_SERVICE_TBT_FIELD: REDACTED_SECRET = "engine_service_tbt_across_engine_calls_ms";
...
impl SessionTelemetry {
    ...
    pub fn with_model(mut self, model: REDACTED_SECRET, slug: REDACTED_SECRET) -> Self {
        self.metadata.model = model.to_owned();
        self.metadata.slug = slug.to_owned();
        self
    }

    pub fn with_metrics_service_name(mut self, service_name: REDACTED_SECRET) -> Self {
    ...
    pub fn with_metrics_config(self, config: MetricsConfig) -> MetricsResult<Self> {
        let metrics = MetricsClient:REDACTED_SECRETconfig)?;
    ...
    pub fn counter(&self, name: REDACTED_SECRET, inc: i64, tags: &[(REDACTED_SECRET, REDACTED_SECRET)]) {
    ...
    pub fn histogram(&self, name: REDACTED_SECRET, value: i64, tags: &[(REDACTED_SECRET, REDACTED_SECRET)]) {
        let res: MetricsResult<()> = (|| {
            let Some(metrics) = &self.metrics else {
                return Ok(());
            };
    ...
    pub fn record_duration(&self, name: REDACTED_SECRET, duration: Duration, tags: &[(REDACTED_SECRET, REDACTED_SECRET)]) {
        let res: MetricsResult<()> = (|| {
            let Some(metrics) = &self.metrics else {
                return Ok(());
            };
    ...
    pub fn start_timer(&self, name: REDACTED_SECRET, tags: &[(REDACTED_SECRET, REDACTED_SECRET)]) -> Result<Timer, MetricsError> {
        let Some(metrics) = &self.metrics else {
    ...
    fn metadata_tag_refs(&self) -> MetricsResult<Vec<(REDACTED_SECRET, REDACTED_SECRET)>> {
    ...
    pub fn conversation_starts(
        ...
        mcp_servers: Vec<REDACTED_SECRET>,
    ...
    pub fn record_api_request(
        ...
        auth_header_name: Option<REDACTED_SECRET>,
        retry_after_unauthorized: bool,
        recovery_mode: Option<REDACTED_SECRET>,
        recovery_phase: Option<REDACTED_SECRET>,
        endpoint: REDACTED_SECRET,
        request_id: Option<REDACTED_SECRET>,
        cf_ray: Option<REDACTED_SECRET>,
        auth_error: Option<REDACTED_SECRET>,
        auth_error_code: Option<REDACTED_SECRET>,
    ) {
    ...
    pub fn record_auth_recovery(
        &self,
        mode: REDACTED_SECRET,
        step: REDACTED_SECRET,
        outcome: REDACTED_SECRET,
        request_id: Option<REDACTED_SECRET>,
        cf_ray: Option<REDACTED_SECRET>,
        auth_error: Option<REDACTED_SECRET>,
        auth_error_code: Option<REDACTED_SECRET>,
        recovery_reason: Option<REDACTED_SECRET>,
    ...
    fn sse_event(&self, kind: REDACTED_SECRET, duration: Duration) {
        self.counter(
    ...
    pub fn user_prompt(&self, items: &[UserInput]) {
        ...
        let local_image_input_count = items
            .iter()
            .filter(|item| matches!(item, UserInput:REDACTED_SECRETImage { .. }))
            .count();
    ...
    pub async fn log_tool_result_with_tags<F, Fut, E>(
        ...
        extra_tags: &[(REDACTED_SECRET, REDACTED_SECRET)],
        mcp_server: Option<REDACTED_SECRET>,
        mcp_server_origin: Option<REDACTED_SECRET>,
        f: F,
    ...
    pub fn log_tool_failed(&self, tool_name: REDACTED_SECRET, error: REDACTED_SECRET) {
        log_event!(
            self,
    ...
    fn responses_item_type(item: &ResponseItem) -> String {
        match item {
            ...
            ResponseItem:REDACTED_SECRETShellCall { .. } => "local_shell_call".into(),
