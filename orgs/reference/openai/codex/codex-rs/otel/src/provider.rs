...
const HOST_NAME_ATTRIBUTE: REDACTED_SECRET = "host.name";
...
impl OtelProvider {
    ...
    pub fn from(settings: &OtelSettings) -> Result<Option<Self>, Box<dyn Error>> {
        ...
        let metrics = if matches!(metric_exporter, OtelExporter::None) {
            ...
            Some(MetricsClient:REDACTED_SECRETconfig)?)
        };
    ...
    pub fn logger_layer<S>(&self) -> Option<impl Layer<S> + REDACTED_SECRET + Sync>
    where
        S: tracing::Subscriber + for<'span> LookupSpan<'span> + REDACTED_SECRET + Sync,
    {
        self.logger.as_ref().map(|logger| {
            OpenTelemetryTracingBridge:REDACTED_SECRETlogger).with_filter(
                tracing_subscriber::filter::filter_fn(OtelProvider::log_export_filter),
            )
        })
    }

    pub fn tracing_layer<S>(&self) -> Option<impl Layer<S> + REDACTED_SECRET + Sync>
    where
        S: tracing::Subscriber + for<'span> LookupSpan<'span> + REDACTED_SECRET + Sync,
    {
...
fn resource_attributes(
    ...
    host_name: Option<REDACTED_SECRET>,
    kind: ResourceKind,
) -> Vec<KeyValue> {
    let mut attributes = vec![
        KeyValue:REDACTED_SECRET
            semconv::attribute::SERVICE_VERSION,
            settings.service_version.clone(),
        ),
        KeyValue:REDACTED_SECRETENV_ATTRIBUTE, settings.environment.clone()),
    ];
    if kind == ResourceKind::Logs
        && let Some(host_name) = host_name.and_then(normalize_host_name)
    {
        attributes.push(KeyValue:REDACTED_SECRETHOST_NAME_ATTRIBUTE, host_name));
    }
    attributes
}
...
fn normalize_host_name(host_name: REDACTED_SECRET) -> Option<String> {
