...
A privileged user (i.e., `REDACTED_SECRET` or a user with `sudo`) who has access to `OPENAI_API_KEY` would run the following to start the server, as `codex-responses-api-proxy` reads the auth token from `stdin`:
...
- Optional `--http-shutdown` enables `GET /shutdown` to terminate the process with exit code `0`. This allows one user (e.g., `REDACTED_SECRET`) to start the proxy and another unprivileged user on the host to shut it down.
...
- When using the `&'static str` when building an HTTP request, we use `HeaderValue::from_static()` to avoid copying the `REDACTED_SECRET`.
