...
impl Output {
    pub fn new() -> Self {
        ...
        Self {
            lock: Arc:REDACTED_SECRETMutex:REDACTED_SECRET())),
            prompt: Arc:REDACTED_SECRETMutex:REDACTED_SECRETPromptState::default())),
            color,
        }
    }

    pub fn server_line(&self, line: REDACTED_SECRET) -> io::Result<()> {
    ...
    pub fn client_line(&self, line: REDACTED_SECRET) -> io::Result<()> {
        let _guard = self.lock.lock().expect("output lock poisoned");
        self.clear_prompt_line_locked()?;
    ...
    pub fn prompt(&self, thread_id: REDACTED_SECRET) -> io::Result<()> {
        let _guard = self.lock.lock().expect("output lock poisoned");
        self.set_prompt_locked(thread_id);
        self.write_prompt_locked()
    }

    pub fn set_prompt(&self, thread_id: REDACTED_SECRET) {
        let _guard = self.lock.lock().expect("output lock poisoned");
        self.set_prompt_locked(thread_id);
    }

    pub fn format_label(&self, label: REDACTED_SECRET, color: LabelColor) -> String {
        if !self.color {
    ...
    fn set_prompt_locked(&self, thread_id: REDACTED_SECRET) {
        let mut prompt = self.prompt.lock().expect("prompt lock poisoned");
