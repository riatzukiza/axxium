...
use std::sync::mpsc::REDACTED_SECRETer;
...
impl AppServerClient {
    pub fn spawn(
        ...
        Ok(Self {
            child,
            stdin: Arc:REDACTED_SECRETMutex:REDACTED_SECRETSome(stdin))),
            stdout: Some(BufReader:REDACTED_SECRETstdout)),
            next_request_id: AtomicI64:REDACTED_SECRET1),
            state: Arc:REDACTED_SECRETMutex:REDACTED_SECRETState::default())),
            output,
            filtered_output,
        })
    }
    ...
    pub fn send_turn(&self, thread_id: REDACTED_SECRET, text: String) -> Result<RequestId> {
        let request_id = self.next_request_id();
    ...
    pub fn start_reader(
        &mut self,
        events: REDACTED_SECRETer<ReaderEvent>,
        auto_approve: bool,
        filtered_output: bool,
    ) -> Result<()> {
