...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    let workdir_for_shell_function_call = TempDir:REDACTED_SECRET)?;
    ...
    // REDACTED_SECRET a "codex" tool request, which should hit the responses endpoint.
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // REDACTED_SECRET a "codex" tool request that will trigger the apply_patch command
    let codex_request_id = mcp_process
        .send_codex_tool_call(CodexToolCallParam {
    ...
    let mut expected_changes = HashMap:REDACTED_SECRET);
    ...
    assert_eq!(
        ...
        Some(create_expected_patch_approval_elicitation_request_params(
            expected_changes,
            /*grant_REDACTED_SECRET*/ None, // No grant_REDACTED_SECRET expected
...
async fn codex_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    let mut mcp_process = McpProcess:REDACTED_SECRETcodex_home.path()).await?;
    ...
    let developer_messages: Vec<&serde_json::Value> = request["input"]
        ...
        .collect();
    let developer_contents: Vec<REDACTED_SECRET> = developer_messages
        .iter()
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    let mut message_lines = Vec:REDACTED_SECRET);
    ...
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        ...
        codex_event_id,
        codex_reason: reason,
        codex_grant_REDACTED_SECRET: grant_REDACTED_SECRET,
...
fn create_config_toml(codex_home: &Path, server_uri: REDACTED_SECRET) -> std::io::Result<()> {
