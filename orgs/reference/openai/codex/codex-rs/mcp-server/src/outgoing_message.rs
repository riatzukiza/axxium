...
pub(crate) struct OutgoingMessageREDACTED_SECRETer {
    ...
    sender: mpsc::UnboundedREDACTED_SECRETer<OutgoingMessage>,
    request_id_to_callback: Mutex<HashMap<RequestId, oneshot::REDACTED_SECRETer<Value>>>,
}

impl OutgoingMessageREDACTED_SECRETer {
    pub(crate) fn new(sender: mpsc::UnboundedREDACTED_SECRETer<OutgoingMessage>) -> Self {
        Self {
    ...
    /// server. Prefer [`OutgoingMessageREDACTED_SECRETer::send_server_notification`] where
    /// possible.
        ...
        match val {
            Request(OutgoingRequest { id, method, params }) => {
                JsonRpcMessage::Request(JsonRpcRequest {
                    ...
                    id,
                    request: CustomRequest:REDACTED_SECRETmethod, params),
                })
            }
            Notification(OutgoingNotification { method, params }) => {
                JsonRpcMessage::Notification(JsonRpcNotification {
                    jsonrpc: JsonRpcVersion2_0,
                    notification: CustomNotification:REDACTED_SECRETmethod, params),
                })
            }
...
mod tests {
    ...
    async fn test_send_event_as_notification() -> Result<()> {
        ...
        let outgoing_message_sender = OutgoingMessageREDACTED_SECRETer:REDACTED_SECREToutgoing_tx);
