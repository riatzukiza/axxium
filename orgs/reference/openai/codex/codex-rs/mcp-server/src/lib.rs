...
const OTEL_SERVICE_NAME: REDACTED_SECRET = "codex_mcp_server";
...
pub async fn run_main(
    ...
    let config = Config::load_with_cli_overrides(cli_kv_overrides)
        .await
        .map_err(|e| {
            std::io::Error:REDACTED_SECRETErrorKind::InvalidData, format!("error loading config: {e}"))
        })?;
    ...
    let stdin_reader_handle = tokio::spawn({
        async move {
            let stdin = io::stdin();
            let reader = BufReader:REDACTED_SECRETstdin);
    ...
    let processor_handle = tokio::spawn({
        ...
        let mut processor = MessageProcessor:REDACTED_SECRET
            outgoing_message_sender,
            arg0_paths,
            Arc:REDACTED_SECRETconfig),
            environment_manager,
        );
        async move {
...
mod tests {
    ...
    async fn mcp_server_builds_otel_provider_with_logs_traces_and_metrics() -> anyhow::Result<()> {
        ...
        let exporter = OtelExporterKind::OtlpGrpc {
            ...
            headers: HashMap:REDACTED_SECRET),
            tls: None,
        };
