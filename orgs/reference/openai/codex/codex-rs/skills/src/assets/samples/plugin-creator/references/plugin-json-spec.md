...
- `screenshots` (`array` of `string`): List of screenshot asset paths.
  ...
  - Keep file paths relative to plugin REDACTED_SECRET.
...
- Custom path values must follow the plugin REDACTED_SECRET convention and naming/namespacing rules.
...
- Repo plugin: `<repo-REDACTED_SECRET>/.agents/plugins/marketplace.json`
...
- `source` (`object`): Plugin source descriptor.
  ...
  - `path` (`string`): Relative plugin path based on the marketplace REDACTED_SECRET.
    - Repo plugin: `./plugins/<plugin-name>`
    - Local plugin in `~/.agents/plugins/marketplace.json`: `./plugins/<plugin-name>`
  - The same relative path convention is used for both repo-REDACTED_SECRETed and home-REDACTED_SECRETed marketplaces.
