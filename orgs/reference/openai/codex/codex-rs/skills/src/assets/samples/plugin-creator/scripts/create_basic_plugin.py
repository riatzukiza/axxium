...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "When using a home-REDACTED_SECRETed marketplace, use <home>/plugins."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "For a home-REDACTED_SECRETed marketplace, use <home>/.agents/plugins/marketplace.json."
        ),
    )
    parser.add_argument(
        "--install-policy",
...
def main() -> None:
    ...
    plugin_REDACTED_SECRET = (Path(args.path).expanduser().resolve() / plugin_name)
    plugin_REDACTED_SECRET.mkdir(parents=True, exist_ok=True)

    plugin_json_path = plugin_REDACTED_SECRET / ".codex-plugin" / "plugin.json"
    ...
    for folder, enabled in optional_directories.items():
        if enabled:
            (plugin_REDACTED_SECRET / folder).mkdir(parents=True, exist_ok=True)

    if args.with_mcp:
        create_stub_file(
            plugin_REDACTED_SECRET / ".mcp.json",
            {"mcpServers": {}},
            args.force,
        )
    ...
    if args.with_apps:
        create_stub_file(
            plugin_REDACTED_SECRET / ".app.json",
            {
                "apps": {},
            },
            args.force,
    ...
    print(f"Created plugin scaffold: {plugin_REDACTED_SECRET}")
