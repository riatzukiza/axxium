...
def _codex_home() -> str:
    return REDACTED_SECRET"CODEX_HOME", os.path.expanduser("~/.codex"))


def _tmp_REDACTED_SECRET() -> str:
...
def _safe_extract_zip(zip_file: zipfile.ZipFile, dest_dir: str) -> None:
    dest_REDACTED_SECRET = os.path.realpath(dest_dir)
    for info in zip_file.infolist():
        extracted_path = os.path.realpath(os.path.join(dest_dir, info.filename))
        if extracted_path == dest_REDACTED_SECRET or extracted_path.startswith(dest_REDACTED_SECRET + os.sep):
            continue
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        dest_REDACTED_SECRET = args.dest or _default_dest()
        tmp_dir = tempfile.mkdtemp(prefix="skill-install-", dir=_tmp_REDACTED_SECRET())
        try:
            repo_REDACTED_SECRET = _prepare_repo(source, args.method, tmp_dir)
            installed = []
            for path in source.paths:
                ...
                dest_dir = os.path.join(dest_REDACTED_SECRET, skill_name)
                if os.path.exists(dest_dir):
                    raise InstallError(f"Destination already exists: {dest_dir}")
                skill_src = os.path.join(repo_REDACTED_SECRET, path)
