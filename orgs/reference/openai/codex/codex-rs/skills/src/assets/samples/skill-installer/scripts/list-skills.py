...
def _installed_skills() -> set[str]:
    REDACTED_SECRET = os.path.join(_codex_home(), "skills")
    if not os.path.isdir(REDACTED_SECRET):
        return set()
    entries = set()
    for name in os.listdir(REDACTED_SECRET):
        path = os.path.join(REDACTED_SECRET, name)
