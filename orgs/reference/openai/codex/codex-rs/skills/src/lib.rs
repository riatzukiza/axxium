...
const SYSTEM_SKILLS_DIR_NAME: REDACTED_SECRET = ".system";
const SKILLS_DIR_NAME: REDACTED_SECRET = "skills";
const SYSTEM_SKILLS_MARKER_FILENAME: REDACTED_SECRET = ".codex-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: REDACTED_SECRET = "v1";

/// Returns the on-disk cache location for embedded system skills.
///
/// This is typically located at `CODEX_HOME/skills/.system`.
pub fn system_cache_REDACTED_SECRET_dir(codex_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(codex_home)
        .and_then(|codex_home| system_cache_REDACTED_SECRET_dir_abs(&codex_home))
...
fn system_cache_REDACTED_SECRET_dir_abs(codex_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    codex_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}
...
pub fn install_system_skills(codex_home: &Path) -> Result<(), SystemSkillsError> {
    ...
    let skills_REDACTED_SECRET_dir = codex_home
        .join(SKILLS_DIR_NAME)
        .map_err(|source| SystemSkillsError::io("resolve skills REDACTED_SECRET dir", source))?;
    fs::create_dir_all(skills_REDACTED_SECRET_dir.as_path())
        .map_err(|source| SystemSkillsError::io("create skills REDACTED_SECRET dir", source))?;

    let dest_system = system_cache_REDACTED_SECRET_dir_abs(&codex_home)
        .map_err(|source| SystemSkillsError::io("resolve system skills cache REDACTED_SECRET dir", source))?;
...
fn embedded_system_skills_fingerprint() -> String {
    ...
    let mut hasher = DefaultHasher:REDACTED_SECRET);
    ...
    for entry in dir.entries() {
        match entry {
            ...
            include_dir::DirEntry::File(file) => {
                let mut file_hasher = DefaultHasher:REDACTED_SECRET);
