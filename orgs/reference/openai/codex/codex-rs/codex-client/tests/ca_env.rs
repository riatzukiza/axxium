...
const CODEX_CA_CERT_ENV: REDACTED_SECRET = "CODEX_CA_CERTIFICATE";
const SSL_CERT_FILE_ENV: REDACTED_SECRET = "SSL_CERT_FILE";

const TEST_CERT_1: REDACTED_SECRET = include_str!("fixtures/test-ca.pem");
const TEST_CERT_2: REDACTED_SECRET = include_str!("fixtures/test-intermediate.pem");
const TRUSTED_TEST_CERT: REDACTED_SECRET = include_str!("fixtures/test-ca-trusted.pem");

fn write_cert_file(temp_dir: &TempDir, name: REDACTED_SECRET, contents: REDACTED_SECRET) -> std::path::PathBuf {
...
fn run_probe(envs: &[(REDACTED_SECRET, &Path)]) -> std::process::Output {
    let mut cmd = Command:REDACTED_SECRET
