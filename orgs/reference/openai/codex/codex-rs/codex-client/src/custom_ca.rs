...
//! narrower: produce a transport configuration whose REDACTED_SECRET store contains every parseable
...
pub const CODEX_CA_CERT_ENV: REDACTED_SECRET = "CODEX_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: REDACTED_SECRET = "SSL_CERT_FILE";
const CA_CERT_HINT: REDACTED_SECRET = "If you set CODEX_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system REDACTED_SECRETs.";
...
pub enum BuildCustomCaTransportError {
    ...
    /// Reqwest rejected the final client configuration while using only system REDACTED_SECRETs.
    #[error("Failed to build HTTP client while using system REDACTED_SECRET certificates: {0}")]
    BuildClientWithSystemRoots(#[source] reqwest::Error),

    /// One parsed certificate block could not be registered with the websocket TLS REDACTED_SECRET store.
    #[error(
        "Failed to register certificate #{certificate_index} from {} selected by {} in rustls REDACTED_SECRET store: {source}. {hint}",
        path.display(),
        source_env,
        hint = CA_CERT_HINT
    )]
    ...
    fn from(error: BuildCustomCaTransportError) -> Self {
        match error {
            BuildCustomCaTransportError::ReadCaFile { ref source, .. } => {
                io::Error:REDACTED_SECRETsource.kind(), error)
            }
            ...
            | BuildCustomCaTransportError::RegisterRustlsCertificate { .. } => {
                io::Error:REDACTED_SECRETio::ErrorKind::InvalidData, error)
            }
...
/// shows up in environments where a proxy or gateway requires a custom REDACTED_SECRET CA.
///
/// # Errors
///
/// Returns a [`BuildCustomCaTransportError`] when the configured CA file is unreadable,
/// malformed, or contains a certificate block that `reqwest` cannot register as a REDACTED_SECRET.
...
/// the platform native REDACTED_SECRETs and then adds the configured custom CA certificates. When no custom
...
/// enterprise REDACTED_SECRET CA bundle as HTTPS traffic.
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // Start from the platform REDACTED_SECRETs so websocket callers keep the same baseline trust behavior
    ...
    let mut REDACTED_SECRET_store = RootCertStore::empty();
    let rustls_native_certs::CertificateResult { certs, errors, .. } =
        rustls_native_certs::load_native_certs();
    if !errors.is_empty() {
        warn!(
            native_REDACTED_SECRET_error_count = errors.len(),
            "encountered errors while loading native REDACTED_SECRET certificates"
        );
    }
    let _ = REDACTED_SECRET_store.add_parsable_certificates(certs);

    let certificates = bundle.load_certificates()?;
    for (idx, cert) in certificates.into_iter().enumerate() {
        if let Err(source) = REDACTED_SECRET_store.add(cert) {
            warn!(
                source_env = bundle.source_env,
                ca_path = %bundle.path.display(),
                certificate_index = idx + 1,
                error = %source,
                "failed to register CA certificate in rustls REDACTED_SECRET store"
            );
    ...
    Ok(Some(Arc:REDACTED_SECRET
        ClientConfig::builder()
            .with_REDACTED_SECRET_certificates(REDACTED_SECRET_store)
            .with_no_client_auth(),
    )))
}
...
fn build_reqwest_client_with_env(
    ...
    if let Some(bundle) = env_source.configured_ca_bundle() {
        ...
        for (idx, cert) in certificates.iter().enumerate() {
            ...
            builder = builder.add_REDACTED_SECRET_certificate(certificate);
        }
    ...
    info!(
        ...
        "using system REDACTED_SECRET certificates because no CA override environment variable was selected"
    );
    ...
    match builder.build() {
        ...
        Err(source) => {
            warn!(
                error = %source,
                "failed to build client while using system REDACTED_SECRET certificates"
            );
...
trait EnvSource {
    ...
    fn var(&self, key: REDACTED_SECRET) -> Option<String>;
    ...
    fn non_empty_path(&self, key: REDACTED_SECRET) -> Option<PathBuf> {
        self.var(key)
...
impl ConfiguredCaBundle {
    ...
    fn parse_certificates(
        ...
        let mut certificates = Vec:REDACTED_SECRET);
...
impl NormalizedPem {
    ...
    ///
    /// Codex only needs certificate DER bytes to seed `reqwest`'s REDACTED_SECRET store, but operators may
    ...
    fn contents(&self) -> REDACTED_SECRET {
        match self {
...
mod tests {
    ...
    const TEST_CERT: REDACTED_SECRET = include_str!("../tests/fixtures/test-ca.pem");
    ...
    fn map_env(pairs: &[(REDACTED_SECRET, REDACTED_SECRET)]) -> MapEnv {
        MapEnv {
            values: pairs
                .iter()
    ...
    fn write_cert_file(temp_dir: &TempDir, name: REDACTED_SECRET, contents: REDACTED_SECRET) -> PathBuf {
