...
impl CodexHttpClient {
        ...
        U: IntoUrl,
    {
        ...
        CodexRequestBuilder:REDACTED_SECRETself.inner.request(method.clone(), url), method, url_str)
    }
}
...
impl<'a> Injector for HeaderMapInjector<'a> {
    fn set(&mut self, key: REDACTED_SECRET, value: String) {
...
mod tests {
    ...
    fn inject_trace_headers_uses_current_span_context() {
        ...
        let extracted = TraceContextPropagator:REDACTED_SECRET).extract(&extractor);
    ...
    impl<'a> Extractor for HeaderMapExtractor<'a> {
        fn get(&self, key: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
            self.0.get(key).and_then(|value| value.to_str().ok())
        }

        fn keys(&self) -> Vec<REDACTED_SECRET> {
