...
fn mcp_tool(name: REDACTED_SECRET, description: REDACTED_SECRET) -> Tool {
    Tool {
        name: name.to_string().into(),
        title: None,
        description: Some(description.to_string().into()),
        input_schema: Arc:REDACTED_SECRETJsonObject::from_iter([(
            "type".to_string(),
            json!("object"),
        )])),
        output_schema: None,
...
fn filter_tool_suggest_discoverable_tools_for_codex_tui_omits_plugins() {
    ...
    assert_eq!(
        ...
        vec![DiscoverableTool::Connector(Box:REDACTED_SECRETAppInfo {
            id: "connector_google_calendar".to_string(),
            name: "Google Calendar".to_string(),
            description: Some("Plan events and schedules.".to_string()),
            logo_url: None,
