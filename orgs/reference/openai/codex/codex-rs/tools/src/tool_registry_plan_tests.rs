...
const CODEX_APPS_MCP_SERVER_NAME: REDACTED_SECRET = "codex_apps";
const DEFAULT_AGENT_TYPE_DESCRIPTION: REDACTED_SECRET = "Test agent type description.";
...
fn test_full_toolset_specs_for_gpt5_codex_unified_exec_web_search() {
    ...
    let available_models = Vec:REDACTED_SECRET);
    let config = ToolsConfig:REDACTED_SECRET&ToolsConfigParams {
    ...
    let mut actual = BTreeMap:REDACTED_SECRET);
    let mut duplicate_names = Vec:REDACTED_SECRET);
    for tool in &tools {
    ...
    let mut expected = BTreeMap:REDACTED_SECRET);
    for spec in [
...
fn image_generation_tools_require_feature_and_supported_model() {
    ...
    let available_models = Vec:REDACTED_SECRET);
    let default_tools_config = ToolsConfig:REDACTED_SECRET&ToolsConfigParams {
    ...
    let supported_tools_config = ToolsConfig:REDACTED_SECRET&ToolsConfigParams {
        model_info: &supported_model_info,
        available_models: &available_models,
...
fn tool_suggest_description_lists_discoverable_tools() {
    ...
    let discoverable_tools = vec![
        ...
        DiscoverableTool::Plugin(Box:REDACTED_SECRETDiscoverablePluginInfo {
...
fn mcp_tool(name: REDACTED_SECRET, description: REDACTED_SECRET, input_schema: serde_json::Value) -> rmcp::model::Tool {
    rmcp::model::Tool {
        name: name.to_string().into(),
        title: None,
        description: Some(description.to_string().into()),
        input_schema: std::sync::Arc:REDACTED_SECRETrmcp::model::object(input_schema)),
        output_schema: None,
        annotations: None,
        execution: None,
        icons: None,
...
fn discoverable_connector(id: REDACTED_SECRET, name: REDACTED_SECRET, description: REDACTED_SECRET) -> DiscoverableTool {
    let slug = name.replace(' ', "-").to_lowercase();
    DiscoverableTool::Connector(Box:REDACTED_SECRETAppInfo {
        id: id.to_string(),
        name: name.to_string(),
...
fn assert_contains_tool_names(tools: &[ConfiguredToolSpec], expected_subset: &[REDACTED_SECRET]) {
    use std::collections::HashSet;

    let mut names = HashSet:REDACTED_SECRET);
    let mut duplicates = Vec:REDACTED_SECRET);
...
fn assert_lacks_tool_name(tools: &[ConfiguredToolSpec], expected_absent: REDACTED_SECRET) {
    let names = tools
        .iter()
...
fn find_tool<'a>(tools: &'a [ConfiguredToolSpec], expected_name: REDACTED_SECRET) -> &'a ConfiguredToolSpec {
    tools
        .iter()
