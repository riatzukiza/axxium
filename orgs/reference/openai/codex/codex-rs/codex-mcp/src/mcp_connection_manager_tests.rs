...
fn create_test_tool(server_name: REDACTED_SECRET, tool_name: REDACTED_SECRET) -> ToolInfo {
    ToolInfo {
...
fn tool_filter_applies_enabled_list() {
    let filter = ToolFilter {
        ...
        disabled: HashSet:REDACTED_SECRET),
    };
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    manager.clients.insert(
        ...
        AsyncManagedClient {
            ...
            startup_complete: Arc:REDACTED_SECRETstd::sync::atomic::AtomicBool:REDACTED_SECRETfalse)),
            tool_plugin_provenance: Arc:REDACTED_SECRETToolPluginProvenance::default()),
        },
    );
...
async fn list_all_tools_does_not_block_when_startup_snapshot_cache_hit_is_empty() {
    ...
    manager.clients.insert(
        ...
        AsyncManagedClient {
            client: pending_client,
            startup_snapshot: Some(Vec:REDACTED_SECRET)),
            startup_complete: Arc:REDACTED_SECRETstd::sync::atomic::AtomicBool:REDACTED_SECRETfalse)),
            tool_plugin_provenance: Arc:REDACTED_SECRETToolPluginProvenance::default()),
        },
    );
...
async fn list_all_tools_uses_startup_snapshot_when_client_startup_fails() {
    ...
    let approval_policy = Constrained::allow_any(AskForApproval::OnFailure);
    let mut manager = McpConnectionManager::new_uninitialized(&approval_policy);
    let startup_complete = Arc:REDACTED_SECRETstd::sync::atomic::AtomicBool:REDACTED_SECRETtrue));
    manager.clients.insert(
        CODEX_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
