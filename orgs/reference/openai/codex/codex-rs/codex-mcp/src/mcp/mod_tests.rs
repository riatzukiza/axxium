...
fn test_mcp_config(codex_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        apps_enabled: false,
        configured_mcp_servers: HashMap:REDACTED_SECRET),
        plugin_capability_summaries: Vec:REDACTED_SECRET),
    }
}

fn make_tool(name: REDACTED_SECRET) -> Tool {
    Tool {
        name: name.to_string(),
        title: None,
        description: None,
...
fn group_tools_by_server_strips_prefix_and_groups() {
    ...
    let mut expected_alpha = HashMap:REDACTED_SECRET);
    ...
    let mut expected_beta = HashMap:REDACTED_SECRET);
...
fn codex_apps_server_config_uses_legacy_codex_apps_path() {
    ...
    let mut servers = with_codex_apps_mcp(HashMap:REDACTED_SECRET), /*auth*/ None, &config);
