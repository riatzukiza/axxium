...
const MCP_TOOL_NAME_PREFIX: REDACTED_SECRET = "mcp";
const MCP_TOOL_NAME_DELIMITER: REDACTED_SECRET = "__";
pub const CODEX_APPS_MCP_SERVER_NAME: REDACTED_SECRET = "codex_apps";
const CODEX_CONNECTORS_TOKEN_ENV_VAR: REDACTED_SECRET = "CODEX_CONNECTORS_TOKEN";

/// The Responses API requires tool names to match `^[a-zA-Z0-9_-]+$`.
/// MCP server/tool names are user-controlled, so sanitize the fully-qualified
/// name we expose to the model by replacing any disallowed character with `_`.
pub(crate) fn sanitize_responses_api_tool_name(name: REDACTED_SECRET) -> String {
...
pub fn qualified_mcp_tool_name_prefix(server_name: REDACTED_SECRET) -> String {
...
pub struct McpConfig {
    /// Base URL for ChatGPT-hosted app MCP servers, copied from the REDACTED_SECRET config.
...
impl ToolPluginProvenance {
    pub fn plugin_display_names_for_connector_id(&self, connector_id: REDACTED_SECRET) -> &[String] {
    ...
    pub fn plugin_display_names_for_mcp_server_name(&self, server_name: REDACTED_SECRET) -> &[String] {
...
fn normalize_codex_apps_base_url(base_url: REDACTED_SECRET) -> String {
...
fn codex_apps_mcp_url_for_base_url(base_url: REDACTED_SECRET) -> String {
...
pub async fn collect_mcp_snapshot(
    ...
    if mcp_servers.is_empty() {
        return McpListToolsResponseEvent {
            tools: HashMap:REDACTED_SECRET),
            resources: HashMap:REDACTED_SECRET),
            resource_templates: HashMap:REDACTED_SECRET),
            auth_statuses: HashMap:REDACTED_SECRET),
        };
    }
...
pub fn split_qualified_tool_name(qualified_name: REDACTED_SECRET) -> Option<(String, String)> {
...
pub fn group_tools_by_server(
    ...
    let mut grouped = HashMap:REDACTED_SECRET);
