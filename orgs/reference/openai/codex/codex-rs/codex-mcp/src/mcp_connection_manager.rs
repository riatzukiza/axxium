...
use codex_rmcp_client::REDACTED_SECRETElicitation;
...
const MCP_TOOL_NAME_DELIMITER: REDACTED_SECRET = "__";
...
const CODEX_APPS_TOOLS_CACHE_DIR: REDACTED_SECRET = "cache/codex_apps_tools";
const MCP_TOOLS_LIST_DURATION_METRIC: REDACTED_SECRET = "codex.mcp.tools.list.duration_ms";
const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: REDACTED_SECRET = "codex.mcp.tools.fetch_uncached.duration_ms";
const MCP_TOOLS_CACHE_WRITE_DURATION_METRIC: REDACTED_SECRET = "codex.mcp.tools.cache_write.duration_ms";

fn sha1_hex(s: REDACTED_SECRET) -> String {
    let mut hasher = Sha1:REDACTED_SECRET);
...
{
    let mut used_names = HashSet:REDACTED_SECRET);
    let mut seen_raw_names = HashSet:REDACTED_SECRET);
    let mut qualified_tools = HashMap:REDACTED_SECRET);
    for tool in tools {
...
type ResponderMap = HashMap<(String, RequestId), oneshot::REDACTED_SECRETer<ElicitationResponse>>;
...
impl ElicitationRequestManager {
    fn new(approval_policy: AskForApproval) -> Self {
        Self {
            requests: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap:REDACTED_SECRET))),
            approval_policy: Arc:REDACTED_SECRETStdMutex:REDACTED_SECRETapproval_policy)),
        }
    }
    ...
    fn make_sender(&self, server_name: String, tx_event: REDACTED_SECRETer<Event>) -> REDACTED_SECRETElicitation {
        let elicitation_requests = self.requests.clone();
        let approval_policy = self.approval_policy.clone();
        Box:REDACTED_SECRETmove |id, elicitation| {
...
impl AsyncManagedClient {
    ...
    fn new(
        ...
        let startup_complete = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        ...
        let fut = async move {
            let outcome = async {
                ...
                let client =
                    Arc:REDACTED_SECRETmake_rmcp_client(&server_name, config.transport, store_mode).await?);
...
pub const MCP_SANDBOX_STATE_CAPABILITY: REDACTED_SECRET = "codex/sandbox-state";

/// Custom MCP request to push sandbox state updates.
/// When used, the `params` field of the notification is [`SandboxState`].
pub const MCP_SANDBOX_STATE_METHOD: REDACTED_SECRET = "codex/sandbox-state/update";
...
impl McpConnectionManager {
    ...
    pub fn new_uninitialized(approval_policy: &Constrained<AskForApproval>) -> Self {
        Self {
            clients: HashMap:REDACTED_SECRET),
            server_origins: HashMap:REDACTED_SECRET),
            elicitation_requests: ElicitationRequestManager:REDACTED_SECRETapproval_policy.value()),
        }
    }
    ...
    pub fn server_origin(&self, server_name: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
    ...
    pub async fn new(
        ...
        let cancel_token = CancellationToken:REDACTED_SECRET);
        let mut clients = HashMap:REDACTED_SECRET);
        let mut server_origins = HashMap:REDACTED_SECRET);
        let mut join_set = JoinSet:REDACTED_SECRET);
        let elicitation_requests = ElicitationRequestManager:REDACTED_SECRETapproval_policy.value());
        let tool_plugin_provenance = Arc:REDACTED_SECRETtool_plugin_provenance);
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            let async_managed_client = AsyncManagedClient:REDACTED_SECRET
                server_name.clone(),
                cfg,
                store_mode,
                cancel_token.clone(),
            ...
            join_set.spawn(async move {
                ...
                let status = match &outcome {
                    Ok(_) => {
                        // REDACTED_SECRET sandbox state notification immediately after Ready
    ...
    async fn client_by_name(&self, name: REDACTED_SECRET) -> Result<ManagedClient> {
        self.clients
            .get(name)
    ...
    pub async fn wait_for_server_ready(&self, server_name: REDACTED_SECRET, timeout: Duration) -> bool {
    ...
    pub async fn required_startup_failures(
        ...
        let mut failures = Vec:REDACTED_SECRET);
    ...
    pub async fn list_all_resources(&self) -> HashMap<String, Vec<Resource>> {
        ...
        for (server_name, async_managed_client) in clients_snapshot {
            ...
            join_set.spawn(async move {
                let mut collected: Vec<Resource> = Vec:REDACTED_SECRET);
        ...
        let mut aggregated: HashMap<String, Vec<Resource>> = HashMap:REDACTED_SECRET);
    ...
    pub async fn list_all_resource_templates(&self) -> HashMap<String, Vec<ResourceTemplate>> {
        ...
        for (server_name, async_managed_client) in clients_snapshot {
            ...
            join_set.spawn(async move {
                let mut collected: Vec<ResourceTemplate> = Vec:REDACTED_SECRET);
                let mut cursor: Option<String> = None;
        ...
        let mut aggregated: HashMap<String, Vec<ResourceTemplate>> = HashMap:REDACTED_SECRET);
    ...
    pub async fn parse_tool_name(&self, tool_name: REDACTED_SECRET) -> Option<(String, String)> {
        self.list_all_tools()
            .await
            .get(tool_name)
...
async fn emit_update(
    submit_id: REDACTED_SECRET,
    tx_event: &REDACTED_SECRETer<Event>,
    update: McpStartupUpdateEvent,
) -> Result<(), async_channel::REDACTED_SECRETError<Event>> {
    tx_event
        .send(Event {
...
impl ToolFilter {
    ...
    fn allows(&self, tool_name: REDACTED_SECRET) -> bool {
...
fn normalize_codex_apps_namespace(server_name: REDACTED_SECRET, connector_name: Option<REDACTED_SECRET>) -> String {
    if server_name != CODEX_APPS_MCP_SERVER_NAME {
...
fn elicitation_capability_for_server(server_name: REDACTED_SECRET) -> Option<ElicitationCapability> {
    if server_name == CODEX_APPS_MCP_SERVER_NAME {
...
fn emit_duration(metric: REDACTED_SECRET, duration: Duration, tags: &[(REDACTED_SECRET, REDACTED_SECRET)]) {
...
fn validate_mcp_server_name(server_name: REDACTED_SECRET) -> Result<()> {
    let re = regex_lite::Regex:REDACTED_SECRETr"^[a-zA-Z0-9_-]+$")?;
