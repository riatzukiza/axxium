...
const AUTH_ERROR_HEADER: REDACTED_SECRET = "x-openai-authorization-error";
...
pub fn extract_response_debug_context(transport: &TransportError) -> ResponseDebugContext {
    ...
    let extract_header = |name: REDACTED_SECRET| {
        headers
            .as_ref()
...
mod tests {
    ...
    fn telemetry_error_messages_preserve_non_http_details() {
        ...
        assert_eq!(telemetry_api_error_message(REDACTED_SECRETeam), "socket closed");
    }
}
