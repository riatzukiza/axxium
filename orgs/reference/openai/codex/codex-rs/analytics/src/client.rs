...
pub(crate) struct AnalyticsEventsQueue {
    pub(crate) sender: mpsc::REDACTED_SECRETer<AnalyticsFact>,
...
impl AnalyticsEventsClient {
    pub fn new(
        ...
        Self {
            queue: AnalyticsEventsQueue:REDACTED_SECRETArc::clone(&auth_manager), base_url),
            analytics_enabled,
        }
    }
    ...
    pub fn track_response(&self, connection_id: u64, response: ClientResponse) {
        self.record_fact(AnalyticsFact::Response {
            connection_id,
            response: Box:REDACTED_SECRETresponse),
        });
    }
}
