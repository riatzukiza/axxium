...
impl AnalyticsReducer {
    ...
    async fn ingest_skill_invoked(
        ...
        for invocation in invocations {
            ...
            let repo_REDACTED_SECRET = get_git_repo_REDACTED_SECRET(invocation.skill_path.as_path());
            let repo_url = if let Some(REDACTED_SECRET) = repo_REDACTED_SECRET.as_ref() {
                collect_git_info(REDACTED_SECRET)
                    .await
...
pub(crate) fn skill_id_for_local_skill(
    repo_url: Option<REDACTED_SECRET>,
    repo_REDACTED_SECRET: Option<&Path>,
    skill_path: &Path,
    skill_name: REDACTED_SECRET,
) -> String {
    let path = normalize_path_for_skill_id(repo_url, repo_REDACTED_SECRET, skill_path);
...
///
/// - Repo-scoped skills use a path relative to the repo REDACTED_SECRET.
...
pub(crate) fn normalize_path_for_skill_id(
    ...
    match (repo_url, repo_REDACTED_SECRET) {
        (Some(_), Some(REDACTED_SECRET)) => {
            let resolved_REDACTED_SECRET = std::fs::canonicalize(REDACTED_SECRET).unwrap_or_else(|_| REDACTED_SECRET.to_path_buf());
            resolved_path
                .strip_prefix(&resolved_REDACTED_SECRET)
