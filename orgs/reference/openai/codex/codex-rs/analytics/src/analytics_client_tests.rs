...
fn sample_thread(thread_id: REDACTED_SECRET, ephemeral: bool) -> Thread {
    Thread {
...
fn sample_thread_start_response(thread_id: REDACTED_SECRET, ephemeral: bool, model: REDACTED_SECRET) -> ClientResponse {
...
fn sample_thread_resume_response(thread_id: REDACTED_SECRET, ephemeral: bool, model: REDACTED_SECRET) -> ClientResponse {
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    let repo_REDACTED_SECRET = PathBuf::from("/repo/REDACTED_SECRET");
    let skill_path = PathBuf::from("/repo/REDACTED_SECRET/.codex/skills/doc/SKILL.md");

    let path = normalize_path_for_skill_id(
        Some("https://example.com/repo.git"),
        Some(repo_REDACTED_SECRET.as_path()),
        skill_path.as_path(),
    );
...
#[test]
fn normalize_path_for_skill_id_repo_REDACTED_SECRET_not_in_skill_path_uses_absolute_path() {
    let repo_REDACTED_SECRET = PathBuf::from("/repo/REDACTED_SECRET");
...
fn app_used_dedupe_is_keyed_by_turn_and_connector() {
    ...
    let queue = AnalyticsEventsQueue {
        sender,
        app_used_emitted_keys: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashSet:REDACTED_SECRET))),
        plugin_used_emitted_keys: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashSet:REDACTED_SECRET))),
    };
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Response {
                connection_id: 7,
                response: Box:REDACTED_SECRETsample_thread_start_response(
                    "thread-no-client",
                    /*ephemeral*/ false,
                    "gpt-5",
                )),
    ...
    reducer
        .ingest(
            AnalyticsFact::Response {
                connection_id: 7,
                response: Box:REDACTED_SECRETsample_thread_resume_response(
