...
pub trait Readiness: REDACTED_SECRET + Sync + 'static {
...
pub struct ReadinessFlag {
    ...
    tx: watch::REDACTED_SECRETer<bool>,
}
...
impl ReadinessFlag {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ready: AtomicBool:REDACTED_SECRETfalse),
            next_id: AtomicI32:REDACTED_SECRET1), // Reserve 0.
            tokens: Mutex:REDACTED_SECRETHashSet:REDACTED_SECRET)),
            tx,
        }
    }
...
mod tests {
    ...
    async fn subscribe_and_mark_ready_roundtrip() -> Result<(), ReadinessError> {
        let flag = ReadinessFlag:REDACTED_SECRET);
    ...
    async fn wait_ready_unblocks_after_mark_ready() -> Result<(), ReadinessError> {
        let flag = Arc:REDACTED_SECRETReadinessFlag:REDACTED_SECRET));
        let token = flag.subscribe().await?;
