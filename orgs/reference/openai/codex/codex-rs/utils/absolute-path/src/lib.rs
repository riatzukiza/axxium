...
impl TryFrom<REDACTED_SECRET> for AbsolutePathBuf {
    type Error = std::io::Error;
...
thread_local! {
    static ABSOLUTE_PATH_BASE: RefCell<Option<PathBuf>> = const { RefCell:REDACTED_SECRETNone) };
}
...
mod tests {
    ...
    #[test]
    fn home_directory_REDACTED_SECRET_is_expanded_in_deserialization() {
        let Some(home) = home_dir() else {
            return;
        };
        let temp_dir = tempdir().expect("base dir");
        let abs_path_buf = {
            let _guard = AbsolutePathBufGuard:REDACTED_SECRETtemp_dir.path());
