...
pub fn json_to_toml(v: JsonValue) -> TomlValue {
    match v {
        JsonValue::Null => TomlValue::String(String:REDACTED_SECRET)),
...
mod tests {
    ...
    fn json_null_to_toml() {
        ...
        assert_eq!(TomlValue::String(String:REDACTED_SECRET)), json_to_toml(json_value));
    }
    ...
    fn json_object_nested() {
        ...
        let expected = {
            let mut inner = toml::value::Table:REDACTED_SECRET);
            inner.insert("inner".into(), TomlValue::Integer(2));

            let mut outer = toml::value::Table:REDACTED_SECRET);
