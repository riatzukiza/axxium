...
Lightweight helpers for spawning interactive processes either under a PTY (pseudo terminal) or regular pipes. The REDACTED_SECRET API is minimal and mirrors both backends so callers can switch based on their needs (e.g., enabling or disabling TTY).
...
- `ProcessHandle` exposes:
  - `writer_sender()` → `mpsc::REDACTED_SECRETer<Vec<u8>>` (stdin)
  - `resize(TerminalSize)`
  - `close_stdin()`
