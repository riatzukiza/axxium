...
async fn read_output_stream<R>(mut reader: R, output_tx: mpsc::REDACTED_SECRETer<Vec<u8>>)
where
    R: AsyncRead + Unpin,
{
...
async fn spawn_process_with_stdin_mode(
    ...
    let writer_handle = if let Some(stdin) = stdin {
        let writer = Arc:REDACTED_SECRETtokio::sync::Mutex:REDACTED_SECRETstdin));
    ...
    let stdout_handle = stdout.map(|stdout| {
        ...
        tokio::spawn(async move {
            read_output_stream(BufReader:REDACTED_SECRETstdout), stdout_tx).await;
        })
    });
    let stderr_handle = stderr.map(|stderr| {
        let stderr_tx = stderr_tx.clone();
        tokio::spawn(async move {
            read_output_stream(BufReader:REDACTED_SECRETstderr), stderr_tx).await;
        })
    });
    let mut reader_abort_handles = Vec:REDACTED_SECRET);
    ...
    let handle = ProcessHandle:REDACTED_SECRET
        writer_tx,
        Box:REDACTED_SECRETPipeChildTerminator {
            #[cfg(windows)]
            pid,
            #[cfg(unix)]
            process_group_id,
