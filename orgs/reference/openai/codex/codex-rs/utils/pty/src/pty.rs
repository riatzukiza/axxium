...
struct PtyChildTerminator {
    killer: Box<dyn portable_pty::ChildKiller + REDACTED_SECRET + Sync>,
    #[cfg(unix)]
...
fn platform_native_pty_system() -> Box<dyn portable_pty::PtySystem + REDACTED_SECRET> {
    #[cfg(windows)]
    {
        Box:REDACTED_SECRETcrate::win::ConPtySystem::default())
    }
...
async fn spawn_process_portable(
    ...
    let mut command_builder = CommandBuilder:REDACTED_SECRETarg0.as_ref().unwrap_or(&program.to_string()));
    ...
    let writer = Arc:REDACTED_SECRETtokio::sync::Mutex:REDACTED_SECRETwriter));
    ...
    let exit_status = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
    let wait_exit_status = Arc::clone(&exit_status);
    let exit_code = Arc:REDACTED_SECRETStdMutex:REDACTED_SECRETNone));
    ...
    let handle = ProcessHandle:REDACTED_SECRET
        writer_tx,
        Box:REDACTED_SECRETPtyChildTerminator {
            killer,
            #[cfg(unix)]
            process_group_id,
        }),
...
async fn spawn_process_preserving_fds(
    ...
    let mut command = StdCommand:REDACTED_SECRETprogram);
    ...
    let writer = Arc:REDACTED_SECRETtokio::sync::Mutex:REDACTED_SECRETmaster.try_clone()?));
    let writer_handle: JoinHandle<()> = tokio::spawn({
        let writer = Arc::clone(&writer);
        async move {
            while let Some(bytes) = writer_rx.recv().await {
    ...
    let handles = PtyHandles {
        ...
        _master: PtyMasterHandle::Opaque {
            ...
            _handle: Box:REDACTED_SECRETmaster),
        },
    };

    let handle = ProcessHandle:REDACTED_SECRET
        writer_tx,
        Box:REDACTED_SECRETRawPidTerminator { process_group_id }),
        reader_handle,
        Vec:REDACTED_SECRET),
        writer_handle,
        wait_handle,
