...
fn find_python() -> Option<String> {
    for candidate in ["python3", "python"] {
        if let Ok(output) = std::process::Command:REDACTED_SECRETcandidate)
            .arg("--version")
            .output()
...
fn setsid_available() -> bool {
    ...
    std::process::Command:REDACTED_SECRET"setsid")
        .arg("true")
        .status()
...
fn shell_command(program: REDACTED_SECRET) -> (String, Vec<String>) {
    if cfg!(windows) {
...
fn echo_sleep_command(marker: REDACTED_SECRET) -> String {
    if cfg!(windows) {
...
async fn wait_for_python_repl_ready_via_probe(
    writer: &tokio::sync::mpsc::REDACTED_SECRETer<Vec<u8>>,
    output_rx: &mut tokio::sync::broadcast::Receiver<Vec<u8>>,
    timeout_ms: u64,
    newline: REDACTED_SECRET,
) -> anyhow::Result<Vec<u8>> {
    let mut collected = Vec:REDACTED_SECRET);
...
async fn pipe_process_round_trips_stdin() -> anyhow::Result<()> {
    ...
    let env_map: HashMap<String, String> = std::env::vars().collect();
    let spawned = spawn_pipe_process(&program, &args, Path:REDACTED_SECRET"."), &env_map, &None).await?;
...
async fn pipe_and_pty_share_interface() -> anyhow::Result<()> {
    ...
    let pipe =
        spawn_pipe_process(&pipe_program, &pipe_args, Path:REDACTED_SECRET"."), &env_map, &None).await?;
...
async fn pipe_drains_stderr_without_stdout_activity() -> anyhow::Result<()> {
    ...
    let env_map: HashMap<String, String> = std::env::vars().collect();
    let spawned = spawn_pipe_process(&python, &args, Path:REDACTED_SECRET"."), &env_map, &None).await?;
...
async fn pipe_process_can_expose_split_stdout_and_stderr() -> anyhow::Result<()> {
    ...
    let spawned =
        spawn_pipe_process_no_stdin(&program, &args, Path:REDACTED_SECRET"."), &env_map, &None).await?;
    let SpawnedProcess {
        session: _session,
        stdout_rx,
        stderr_rx,
...
async fn pty_spawn_can_preserve_inherited_fds() -> anyhow::Result<()> {
    ...
    let mut pipe_output = String:REDACTED_SECRET);
