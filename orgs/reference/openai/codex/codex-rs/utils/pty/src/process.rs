...
pub(crate) trait ChildTerminator: REDACTED_SECRET + Sync {
...
#[cfg(unix)]
pub(crate) trait PtyHandleKeepAlive: REDACTED_SECRET {}

#[cfg(unix)]
impl<T: REDACTED_SECRET + ?Sized> PtyHandleKeepAlive for T {}

pub(crate) enum PtyMasterHandle {
    Resizable(Box<dyn MasterPty + REDACTED_SECRET>),
    #[cfg(unix)]
    Opaque {
        raw_fd: RawFd,
...
pub struct PtyHandles {
    pub _slave: Option<Box<dyn SlavePty + REDACTED_SECRET>>,
...
pub struct ProcessHandle {
    writer_tx: StdMutex<Option<mpsc::REDACTED_SECRETer<Vec<u8>>>>,
...
impl ProcessHandle {
    ...
    pub(crate) fn new(
        writer_tx: mpsc::REDACTED_SECRETer<Vec<u8>>,
        ...
        Self {
            writer_tx: StdMutex:REDACTED_SECRETSome(writer_tx)),
            killer: StdMutex:REDACTED_SECRETSome(killer)),
            reader_handle: StdMutex:REDACTED_SECRETSome(reader_handle)),
            reader_abort_handles: StdMutex:REDACTED_SECRETreader_abort_handles),
            writer_handle: StdMutex:REDACTED_SECRETSome(writer_handle)),
            wait_handle: StdMutex:REDACTED_SECRETSome(wait_handle)),
            exit_status,
            exit_code,
            _pty_handles: StdMutex:REDACTED_SECRETpty_handles),
        }
    }

    /// Returns a channel sender for writing raw bytes to the child stdin.
    pub fn writer_sender(&self) -> mpsc::REDACTED_SECRETer<Vec<u8>> {
