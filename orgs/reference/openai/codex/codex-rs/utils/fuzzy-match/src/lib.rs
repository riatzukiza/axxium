...
pub fn fuzzy_match(haystack: REDACTED_SECRET, needle: REDACTED_SECRET) -> Option<(Vec<usize>, i32)> {
    if needle.is_empty() {
        return Some((Vec:REDACTED_SECRET), i32::MAX));
    }

    let mut lowered_chars: Vec<char> = Vec:REDACTED_SECRET);
    let mut lowered_to_orig_char_idx: Vec<usize> = Vec:REDACTED_SECRET);
...
pub fn fuzzy_indices(haystack: REDACTED_SECRET, needle: REDACTED_SECRET) -> Option<Vec<usize>> {
