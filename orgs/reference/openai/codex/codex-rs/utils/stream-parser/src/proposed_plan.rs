...
const OPEN_TAG: REDACTED_SECRET = "<proposed_plan>";
const CLOSE_TAG: REDACTED_SECRET = "</proposed_plan>";
...
impl ProposedPlanParser {
    pub fn new() -> Self {
        Self {
            parser: TaggedLineParser:REDACTED_SECRETvec![TagSpec {
                open: OPEN_TAG,
                close: CLOSE_TAG,
...
pub fn strip_proposed_plan_blocks(text: REDACTED_SECRET) -> String {
    let mut parser = ProposedPlanParser:REDACTED_SECRET);
    let mut out = parser.push_str(text).visible_text;
    out.push_str(&parser.finish().visible_text);
    out
}

pub fn extract_proposed_plan_text(text: REDACTED_SECRET) -> Option<String> {
    let mut parser = ProposedPlanParser:REDACTED_SECRET);
    let mut plan_text = String:REDACTED_SECRET);
