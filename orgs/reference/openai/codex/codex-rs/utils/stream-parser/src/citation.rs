...
const CITATION_OPEN: REDACTED_SECRET = "<oai-mem-citation>";
const CITATION_CLOSE: REDACTED_SECRET = "</oai-mem-citation>";
...
impl CitationStreamParser {
    pub fn new() -> Self {
        Self {
            inner: InlineHiddenTagParser:REDACTED_SECRETvec![InlineTagSpec {
...
pub fn strip_citations(text: REDACTED_SECRET) -> (String, Vec<String>) {
    let mut parser = CitationStreamParser:REDACTED_SECRET);
...
mod tests {
    ...
    fn citation_parser_buffers_partial_open_tag_prefix() {
        ...
        assert_eq!(first.extracted, Vec::<String>:REDACTED_SECRET));
    ...
    fn citation_parser_preserves_partial_open_tag_at_eof_if_not_a_full_tag() {
        ...
        assert_eq!(out.extracted, Vec::<String>:REDACTED_SECRET));
    }
