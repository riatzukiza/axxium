    ...
    T: Clone + Eq,
{
    ...
    pub fn new(specs: Vec<InlineTagSpec<T>>) -> Self {
        ...
        Self {
            specs,
            pending: String:REDACTED_SECRET),
            active: None,
        }
    }
    ...
    fn push_visible_prefix(out: &mut StreamTextChunk<ExtractedInlineTag<T>>, pending: REDACTED_SECRET) {
        if !pending.is_empty() {
    ...
    T: Clone + Eq,
{
    ...
    fn push_str(&mut self, chunk: REDACTED_SECRET) -> StreamTextChunk<Self::Extracted> {
        ...
        loop {
            ...
            if let Some((open_idx, spec_idx)) = self.find_next_open() {
                ...
                self.active = Some(ActiveTag {
                    tag: spec.tag.clone(),
                    close: spec.close,
                    content: String:REDACTED_SECRET),
                });
                continue;
            }
...
fn longest_suffix_prefix_len(s: REDACTED_SECRET, needle: REDACTED_SECRET) -> usize {
...
mod tests {
    ...
    fn collect_chunks<P>(parser: &mut P, chunks: &[REDACTED_SECRET]) -> StreamTextChunk<P::Extracted>
    where
        P: StreamTextParser,
    {
    ...
    fn generic_inline_parser_supports_multiple_tag_types() {
        let mut parser = InlineHiddenTagParser:REDACTED_SECRETvec![
            InlineTagSpec {
                tag: Tag::A,
                open: "<a>",
                close: "</a>",
    ...
    fn generic_inline_parser_supports_non_ascii_tag_delimiters() {
        let mut parser = InlineHiddenTagParser:REDACTED_SECRETvec![InlineTagSpec {
            tag: Tag::A,
            open: "<é>",
            close: "</é>",
        }]);
    ...
    fn generic_inline_parser_rejects_empty_open_delimiter() {
        let _ = InlineHiddenTagParser:REDACTED_SECRETvec![InlineTagSpec {
            tag: Tag::A,
            open: "",
            close: "</a>",
        }]);
