    ...
    T: Copy + Eq,
{
    pub(crate) fn new(specs: Vec<TagSpec<T>>) -> Self {
        Self {
            ...
            active_tag: None,
            detect_tag: true,
            line_buffer: String:REDACTED_SECRET),
        }
    }

    pub(crate) fn parse(&mut self, delta: REDACTED_SECRET) -> Vec<TaggedLineSegment<T>> {
        let mut segments = Vec:REDACTED_SECRET);
        let mut run = String:REDACTED_SECRET);
    ...
    fn is_tag_prefix(&self, slug: REDACTED_SECRET) -> bool {
    ...
    fn match_open(&self, slug: REDACTED_SECRET) -> Option<T> {
        self.specs
            .iter()
    ...
    fn match_close(&self, slug: REDACTED_SECRET) -> Option<T> {
        self.specs
            .iter()
...
mod tests {
    ...
    fn parser() -> TaggedLineParser<Tag> {
        TaggedLineParser:REDACTED_SECRETvec![TagSpec {
            open: "<tag>",
            close: "</tag>",
            tag: Tag::Block,
        }])
