...
pub fn resolve_symlink_write_paths(path: &Path) -> io::Result<SymlinkWritePaths> {
    let REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(path)
        .map(AbsolutePathBuf::into_path_buf)
        .unwrap_or_else(|_| path.to_path_buf());
    let mut current = REDACTED_SECRET.clone();
    let mut visited = HashSet:REDACTED_SECRET);
...
pub fn write_atomically(write_path: &Path, contents: REDACTED_SECRET) -> io::Result<()> {
