...
fn canonicalize_override_key(key: REDACTED_SECRET) -> String {
...
/// Apply a single override onto `REDACTED_SECRET`, creating intermediate objects as
/// necessary.
fn apply_single_override(REDACTED_SECRET: &mut Value, path: REDACTED_SECRET, value: Value) {
    use toml::value::Table;

    let parts: Vec<REDACTED_SECRET> = path.split('.').collect();
    ...
    for (i, part) in parts.iter().enumerate() {
        ...
        if is_last {
            match current {
                ...
                _ => {
                    let mut tbl = Table:REDACTED_SECRET);
                    tbl.insert((*part).to_string(), value);
        ...
        match current {
            Value::Table(tbl) => {
                current = tbl
                    ...
                    .or_insert_with(|| Value::Table(Table:REDACTED_SECRET)));
            }
            _ => {
                *current = Value::Table(Table:REDACTED_SECRET));
...
fn parse_toml_value(raw: REDACTED_SECRET) -> Result<Value, toml::de::Error> {
