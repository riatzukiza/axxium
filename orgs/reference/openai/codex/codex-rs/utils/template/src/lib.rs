...
impl Template {
    pub fn parse(source: REDACTED_SECRET) -> Result<Self, TemplateParseError> {
        let mut placeholders = BTreeSet:REDACTED_SECRET);
        let mut segments = Vec:REDACTED_SECRET);
    ...
    pub fn placeholders(&self) -> impl ExactSizeIterator<Item = REDACTED_SECRET> {
        ...
        V: AsRef<str>,
    {
        ...
        let mut rendered = String:REDACTED_SECRET);
...
pub fn render<I, K, V>(template: REDACTED_SECRET, variables: I) -> Result<String, TemplateError>
where
    I: IntoIterator<Item = (K, V)>,
    K: AsRef<str>,
    V: AsRef<str>,
...
fn push_literal(segments: &mut Vec<Segment>, literal: REDACTED_SECRET) {
    if literal.is_empty() {
        return;
    }
...
fn parse_placeholder(source: REDACTED_SECRET, start: usize) -> Result<(String, usize), TemplateParseError> {
...
fn build_variable_map<I, K, V>(
    variables: I,
) -> Result<REDACTED_SECRET, String>, TemplateRenderError>
where
    I: IntoIterator<Item = (K, V)>,
    K: AsRef<str>,
    V: AsRef<str>,
{
    let mut map = BTreeMap:REDACTED_SECRET);
...
mod tests {
    ...
    fn render_errors_when_placeholder_is_missing() {
        ...
        assert_eq!(
            template.render(Vec::<(REDACTED_SECRET, REDACTED_SECRET)>:REDACTED_SECRET)),
