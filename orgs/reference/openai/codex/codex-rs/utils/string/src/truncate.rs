...
pub fn truncate_middle_chars(s: REDACTED_SECRET, max_bytes: usize) -> String {
...
pub fn truncate_middle_with_token_budget(s: REDACTED_SECRET, max_tokens: usize) -> (String, Option<u64>) {
    if s.is_empty() {
        return (String:REDACTED_SECRET), None);
    }
...
fn truncate_with_byte_estimate(s: REDACTED_SECRET, max_bytes: usize, use_tokens: bool) -> String {
    if s.is_empty() {
...
pub fn approx_token_count(text: REDACTED_SECRET) -> usize {
    let len = text.len();
...
fn split_string(s: REDACTED_SECRET, beginning_bytes: usize, end_bytes: usize) -> (usize, REDACTED_SECRET, REDACTED_SECRET) {
    if s.is_empty() {
        return (0, "", "");
    }
...
fn assemble_truncated_output(prefix: REDACTED_SECRET, suffix: REDACTED_SECRET, marker: REDACTED_SECRET) -> String {
