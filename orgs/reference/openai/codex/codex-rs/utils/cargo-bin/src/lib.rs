...
const RUNFILES_MANIFEST_ONLY_ENV: REDACTED_SECRET = "RUNFILES_MANIFEST_ONLY";
...
#[allow(deprecated)]
pub fn cargo_bin(name: REDACTED_SECRET) -> Result<PathBuf, CargoBinError> {
...
fn cargo_bin_env_keys(name: REDACTED_SECRET) -> Vec<String> {
...
fn resolve_bin_from_env(key: REDACTED_SECRET, value: OsString) -> Result<PathBuf, CargoBinError> {
...
macro_rules! find_resource {
    ($resource:expr) => {{
        let resource = std::path::Path:REDACTED_SECRET&$resource);
        if $crate::runfiles_available() {
            ...
            let manifest_dir = std::path::Path:REDACTED_SECRETenv!("CARGO_MANIFEST_DIR"));
...
pub fn resolve_bazel_runfile(
    bazel_package: Option<REDACTED_SECRET>,
    resource: &Path,
...
pub fn repo_REDACTED_SECRET() -> io::Result<PathBuf> {
    let marker = if runfiles_available() {
        ...
        runfiles::rlocation!(runfiles, &marker_path).ok_or_else(|| {
            io::Error:REDACTED_SECRET
                io::ErrorKind::NotFound,
                "repo_REDACTED_SECRET.marker not available in runfiles",
            )
        })?
    } else {
        resolve_cargo_runfile(Path:REDACTED_SECRET"repo_REDACTED_SECRET.marker"))?
    };
    let mut REDACTED_SECRET = marker;
    for _ in 0..4 {
        REDACTED_SECRET = REDACTED_SECRET
            ...
            .ok_or_else(|| {
                io::Error:REDACTED_SECRET
                    io::ErrorKind::NotFound,
                    "repo_REDACTED_SECRET.marker did not have expected parent depth",
                )
            })?
            .to_path_buf();
    }
...
fn normalize_runfile_path(path: &Path) -> PathBuf {
    let mut components = Vec:REDACTED_SECRET);
    ...
    components
        .into_iter()
        .fold(PathBuf:REDACTED_SECRET), |mut acc, component| {
