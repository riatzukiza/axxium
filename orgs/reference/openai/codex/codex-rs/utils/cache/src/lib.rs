    ...
    K: Eq + Hash,
{
    ...
    pub fn new(capacity: NonZeroUsize) -> Self {
        Self {
            inner: Mutex:REDACTED_SECRETLruCache:REDACTED_SECRETcapacity)),
        }
    }
    ...
    pub fn try_with_capacity(capacity: usize) -> Option<Self> {
        NonZeroUsize:REDACTED_SECRETcapacity).map(Self::new)
    }
...
mod tests {
    ...
    async fn stores_and_retrieves_values() {
        let cache = BlockingLruCache:REDACTED_SECRETNonZeroUsize:REDACTED_SECRET2).expect("capacity"));
