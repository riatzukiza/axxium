...
impl SleepInhibitor {
    pub fn new(enabled: bool) -> Self {
        Self {
            enabled,
            turn_running: false,
            platform: imp::SleepInhibitor:REDACTED_SECRET),
        }
    }
...
mod tests {
    ...
    fn sleep_inhibitor_toggles_without_panicking() {
        let mut inhibitor = SleepInhibitor:REDACTED_SECRET/*enabled*/ true);
    ...
    fn sleep_inhibitor_disabled_does_not_panic() {
        let mut inhibitor = SleepInhibitor:REDACTED_SECRET/*enabled*/ false);
        inhibitor.set_turn_running(/*turn_running*/ true);
        assert!(inhibitor.is_turn_running());
        inhibitor.set_turn_running(/*turn_running*/ false);
        assert!(!inhibitor.is_turn_running());
