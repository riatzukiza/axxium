...
const ASSERTION_REASON: REDACTED_SECRET = "Codex is running an active turn";
// Apple exposes this assertion type as a `CFSTR(...)` macro, so bindgen cannot generate a
// reusable `CFStringRef` constant for it.
const ASSERTION_TYPE_PREVENT_USER_IDLE_SYSTEM_SLEEP: REDACTED_SECRET = "PreventUserIdleSystemSleep";
...
impl MacSleepAssertion {
    fn create(name: REDACTED_SECRET) -> Result<Self, IOReturn> {
        let assertion_type = CFString:REDACTED_SECRETASSERTION_TYPE_PREVENT_USER_IDLE_SYSTEM_SLEEP);
        let assertion_name = CFString:REDACTED_SECRETname);
        let mut id: IOPMAssertionID = 0;
        // `core-foundation` and the generated bindings each declare an REDACTED_SECRET `__CFString` type,
