...
impl PowerRequest {
    fn new_system_required(reason: REDACTED_SECRET) -> Result<Self, String> {
        let mut wide_reason: Vec<u16> = OsStr:REDACTED_SECRETreason).encode_wide().chain(once(0)).collect();
