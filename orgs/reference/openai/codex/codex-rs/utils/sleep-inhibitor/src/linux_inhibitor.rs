...
const APP_ID: REDACTED_SECRET = "codex";
...
const BLOCKER_SLEEP_SECONDS: REDACTED_SECRET = "2147483647";
...
fn spawn_backend(backend: LinuxBackend) -> Result<Child, std::io::Error> {
    ...
    let mut command = match backend {
        LinuxBackend::SystemdInhibit => {
            let mut command = Command:REDACTED_SECRET"systemd-inhibit");
            command.args([
                "--what=idle",
                "--mode=block",
                "--who",
        ...
        LinuxBackend::GnomeSessionInhibit => {
            let mut command = Command:REDACTED_SECRET"gnome-session-inhibit");
            command.args([
                "--inhibit",
                "idle",
                "--reason",
