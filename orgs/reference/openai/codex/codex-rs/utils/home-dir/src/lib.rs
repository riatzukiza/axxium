...
fn find_codex_home_from_env(codex_home_env: Option<REDACTED_SECRET>) -> std::io::Result<PathBuf> {
    ...
    match codex_home_env {
        Some(val) => {
            ...
            let metadata = std::fs::metadata(&path).map_err(|err| match err.kind() {
                std::io::ErrorKind::NotFound => std::io::Error:REDACTED_SECRET
                    std::io::ErrorKind::NotFound,
                    format!("CODEX_HOME points to {val:?}, but that path does not exist"),
                ),
                _ => std::io::Error:REDACTED_SECRET
                    err.kind(),
