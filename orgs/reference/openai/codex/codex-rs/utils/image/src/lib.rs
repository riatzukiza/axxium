...
static IMAGE_CACHE: LazyLock<BlockingLruCache<ImageCacheKey, EncodedImage>> =
    LazyLock:REDACTED_SECRET|| BlockingLruCache:REDACTED_SECRETNonZeroUsize:REDACTED_SECRET32).unwrap_or(NonZeroUsize::MIN)));
...
fn encode_image(
    ...
    let mut buffer = Vec:REDACTED_SECRET);

    match target_format {
        ImageFormat::Png => {
            let rgba = image.to_rgba8();
            let encoder = PngEncoder:REDACTED_SECRET&mut buffer);
            encoder
                .write_image(
                    rgba.as_raw(),
                    image.width(),
...
mod tests {
    ...
    fn image_bytes(image: &ImageBuffer<Rgba<u8>, Vec<u8>>, format: ImageFormat) -> Vec<u8> {
        let mut encoded = Cursor:REDACTED_SECRETVec:REDACTED_SECRET));
    ...
    async fn returns_original_image_when_within_bounds() {
        for (format, mime) in [
            ...
            let encoded = load_for_prompt_bytes(
                Path:REDACTED_SECRET"in-memory-image"),
