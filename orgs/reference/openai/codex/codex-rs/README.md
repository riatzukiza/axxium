...
In `workspace-write`, Codex also includes `~/.codex/memories` in its writable REDACTED_SECRETs so memory maintenance does not require an extra approval.

## Code Organization

This folder is the REDACTED_SECRET of a Cargo workspace. It contains quite a bit of experimental code, but here are the key crates:
