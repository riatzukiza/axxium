...
impl App {
    pub fn new() -> Self {
        Self {
            ...
            apply_modal: None,
            best_of_modal: None,
            environments: Vec:REDACTED_SECRET),
            ...
            list_generation: 0,
            in_flight: std::collections::HashSet:REDACTED_SECRET),
        }
    }
...
impl DiffOverlay {
    pub fn new(task_id: TaskId, title: String, attempt_total_hint: Option<usize>) -> Self {
        let mut sd = ScrollableDiff:REDACTED_SECRET);
        sd.set_content(Vec:REDACTED_SECRET));
        Self {
            title,
            task_id,
            sd,
            base_can_apply: false,
            diff_lines: Vec:REDACTED_SECRET),
            text_lines: Vec:REDACTED_SECRET),
            prompt: None,
            attempts: vec![AttemptView::default()],
            selected_attempt: 0,
            current_view: DetailView::Prompt,
            base_turn_id: None,
            sibling_turn_ids: Vec:REDACTED_SECRET),
            attempt_total_hint,
        }
    }
...
mod tests {
    ...
    impl codex_cloud_tasks_client::CloudBackend for FakeBackend {
        async fn list_tasks(
            ...
            let max = max.min(20);
            let mut limited = Vec:REDACTED_SECRET);
            for task in out {
        ...
        async fn create_task(
            &self,
            _env_id: REDACTED_SECRET,
            _prompt: REDACTED_SECRET,
            _git_ref: REDACTED_SECRET,
            _qa_mode: bool,
            _best_of_n: usize,
    ...
    async fn load_tasks_uses_env_parameter() {
        ...
        let mut by_env = std::collections::HashMap:REDACTED_SECRET);
        by_env.insert(None, vec!["REDACTED_SECRET-1", "REDACTED_SECRET-2"]);
        ...
        // Act + Assert
        let REDACTED_SECRET = load_tasks(&backend, /*env*/ None).await.unwrap();
        assert_eq!(REDACTED_SECRET.len(), 2);
        assert_eq!(REDACTED_SECRET[0].title, "REDACTED_SECRET-1");
