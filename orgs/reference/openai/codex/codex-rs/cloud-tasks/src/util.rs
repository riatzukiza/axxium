...
pub fn set_user_agent_suffix(suffix: REDACTED_SECRET) {
...
pub fn normalize_base_url(input: REDACTED_SECRET) -> String {
...
pub fn extract_chatgpt_account_id(token: REDACTED_SECRET) -> Option<String> {
...
pub async fn load_auth_manager() -> Option<AuthManager> {
    ...
    let config = Config::load_with_cli_overrides(Vec:REDACTED_SECRET)).await.ok()?;
    Some(AuthManager:REDACTED_SECRET
        config.codex_home,
...
pub fn task_url(base_url: REDACTED_SECRET, task_id: REDACTED_SECRET) -> String {
    let normalized = normalize_base_url(base_url);
    if let Some(REDACTED_SECRET) = normalized.strip_suffix("/backend-api") {
        return format!("{REDACTED_SECRET}/codex/tasks/{task_id}");
    }
    if let Some(REDACTED_SECRET) = normalized.strip_suffix("/api/codex") {
        return format!("{REDACTED_SECRET}/codex/tasks/{task_id}");
    }
