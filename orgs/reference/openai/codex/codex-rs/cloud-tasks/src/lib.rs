...
async fn init_backend(user_agent_suffix: REDACTED_SECRET) -> anyhow::Result<BackendContext> {
    ...
    if use_mock {
        return Ok(BackendContext {
            backend: Arc:REDACTED_SECRETcodex_cloud_tasks_mock_client::MockClient),
            base_url,
        });
    }

    let ua = get_codex_user_agent();
    let mut http = codex_cloud_tasks_client::HttpClient:REDACTED_SECRETbase_url.clone())?.with_user_agent(ua);
    ...
    Ok(BackendContext {
        backend: Arc:REDACTED_SECREThttp),
        base_url,
    })
}
...
async fn resolve_environment_id(ctx: &BackendContext, requested: REDACTED_SECRET) -> anyhow::Result<String> {
...
fn resolve_query_input(query_arg: Option<String>) -> anyhow::Result<String> {
    match query_arg {
        ...
        maybe_dash => {
            ...
            let mut buffer = String:REDACTED_SECRET);
            std::io::stdin()
...
fn parse_task_id(raw: REDACTED_SECRET) -> anyhow::Result<codex_cloud_tasks_client::TaskId> {
...
async fn collect_attempt_diffs(
    ...
    let mut attempts = Vec:REDACTED_SECRET);
    if let Some(diff) =
...
fn format_task_status_lines(
    ...
    let mut meta_parts = Vec:REDACTED_SECRET);
...
fn spawn_preflight(
    app: &mut app::App,
    backend: &Arc<dyn codex_cloud_tasks_client::CloudBackend>,
    tx: &UnboundedREDACTED_SECRETer<app::AppEvent>,
    frame_tx: &UnboundedREDACTED_SECRETer<Instant>,
    title: String,
    job: ApplyJob,
) -> bool {
    ...
    tokio::spawn(async move {
        ...
        let event = match result {
            ...
            Err(e) => app::AppEvent::ApplyPreflightFinished {
                ...
                skipped: Vec:REDACTED_SECRET),
                conflicts: Vec:REDACTED_SECRET),
            },
        };
...
pub async fn run_main(cli: Cli, _codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                ...
                .unwrap_or_else(|_| EnvFilter:REDACTED_SECRETdefault_level)),
        )
    ...
    let backend_ui = CrosstermBackend:REDACTED_SECRETstdout);
    let mut terminal = Terminal:REDACTED_SECRETbackend_ui)?;
    terminal.clear()?;

    // App state
    let mut app = app::App:REDACTED_SECRET);
    // Initial load
    ...
    let mut events = EventStream:REDACTED_SECRET);
                    ...
                    match ev {
                        ...
                        app::AppEvent::DetailsDiffLoaded { id, title, diff } => {
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                let mut overlay = app::DiffOverlay:REDACTED_SECRETid.clone(), title, /*attempt_total_hint*/ None);
                                {
                ...
                match maybe_event {
                    ...
                    Some(Ok(Event::Key(key))) if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) => {
                        ...
                        if is_ctrl_o && app.new_task.is_some() {
                            ...
                            app.diff_overlay = None;
                            app.env_modal = Some(app::EnvModalState { query: String:REDACTED_SECRET), selected: 0 });
                        ...
                        if app.apply_modal.is_some() {
                                    ...
                                    if let Some(m) = app.apply_modal.take() {
                                        ...
                                        if spawn_preflight(&mut app, &backend, &tx, &frame_tx, title.clone(), job) {
                                            app.apply_modal = Some(app::ApplyModalState {
                                                ...
                                                result_message: None,
                                                result_level: None,
                                                skipped_paths: Vec:REDACTED_SECRET),
                                                conflict_paths: Vec:REDACTED_SECRET),
                            ...
                            match key.code {
                                ...
                                KeyCode::Char('n') => {
                                    if app.env_filter.is_none() {
                                        app.new_task = Some(crate::new_task::NewTaskPage:REDACTED_SECRET/*env_id*/ None, app.best_of_n));
                                    } else {
                                        app.new_task = Some(crate::new_task::NewTaskPage:REDACTED_SECRETapp.env_filter.clone(), app.best_of_n));
                                    }
                                ...
                                KeyCode::Enter => {
                                    ...
                                    if let Some(state) = app.env_modal.take() {
                                        ...
                                        let filtered: Vec<&app::EnvironmentRow> = app.environments.iter().filter(|r| {
                                            ...
                                            let mut hay = String:REDACTED_SECRET);
                            ...
                            match key.code {
                                ...
                                KeyCode::Char('n') => {
                                    ...
                                    app.new_task = Some(crate::new_task::NewTaskPage:REDACTED_SECRETenv_opt, app.best_of_n));
                                    app.status = "New Task: Enter to submit; Esc to cancel".to_string();
                                    needs_redraw = true;
                                }
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let overlay = app::DiffOverlay:REDACTED_SECRET
                                            task.id.clone(),
                                            task.title.clone(),
                                            task.attempt_total,
                                        );
...
fn conversation_lines(prompt: Option<String>, messages: &[String]) -> Vec<String> {
    let mut out: Vec<String> = Vec:REDACTED_SECRET);
    if let Some(p) = prompt {
        out.push("user:".to_string());
        for l in p.lines() {
            out.push(l.to_string());
        }
        out.push(String:REDACTED_SECRET));
    }
...
fn pretty_lines_from_error(raw: REDACTED_SECRET) -> Vec<String> {
    let mut lines: Vec<String> = Vec:REDACTED_SECRET);
...
mod tests {
    ...
    async fn branch_override_is_used_when_provided() {
        let git_ref = resolve_git_ref_with_git_info(
            ...
            &StubGitInfo:REDACTED_SECRET/*default_branch*/ None, /*current_branch*/ None),
        )
        .await;
    ...
    async fn falls_back_to_current_branch_when_default_is_missing() {
        let git_ref = resolve_git_ref_with_git_info(
            /*branch_override*/ None,
            &StubGitInfo:REDACTED_SECRET/*default_branch*/ None, Some("develop".to_string())),
        )
        .await;
    ...
    fn composer_input_renders_typed_characters() {
        let mut composer = ComposerInput:REDACTED_SECRET);
        let key = KeyEvent:REDACTED_SECRETKeyCode::Char('a'), KeyModifiers::NONE);
        match composer.input(key) {
            ComposerAction::Submitted(_) => panic!("unexpected submission"),
            ComposerAction::None => {}
        }

        let area = Rect:REDACTED_SECRET0, 0, 20, 5);
