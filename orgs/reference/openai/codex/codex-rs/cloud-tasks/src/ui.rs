...
static ROUNDED: OnceLock<bool> = OnceLock:REDACTED_SECRET);
...
fn overlay_block() -> Block<'static> {
    ...
    base.padding(Padding:REDACTED_SECRET2, 2, 1, 1))
}
...
fn draw_list(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let rows = Layout::default()
        ...
        .split(inner);
    let mut list = List:REDACTED_SECRETitems)
        .highlight_symbol("› ")
...
fn draw_footer(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let top = Layout::default()
        ...
        .split(rows[0]);
    let para = Paragraph:REDACTED_SECRETLine::from(help));
    ...
    let status = Paragraph:REDACTED_SECRETstatus_line);
...
fn draw_diff_overlay(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    if let Some(ov) = app.diff_overlay.as_mut() {
        ...
        if has_diff || has_text {
            let rows = Layout::default()
                ...
                .split(content_full);
            // Status bar label
            let mut spans: Vec<ratatui::text::Span> = Vec:REDACTED_SECRET);
            if has_diff && has_text {
            ...
            frame.render_widget(Paragraph:REDACTED_SECRETLine::from(spans)), rows[0]);
    ...
    if app.details_inflight && raw_empty {
        ...
        let scroll = app
            ...
            .unwrap_or(0);
        let content = Paragraph:REDACTED_SECRETText::from(styled_lines)).scroll((scroll, 0));
...
pub fn draw_apply_modal(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    if let Some(m) = &app.apply_modal {
        // Header
        let header = Paragraph:REDACTED_SECRETLine::from(
            format!("Apply '{}' ?", m.title).magenta().bold(),
        ))
        .wrap(Wrap { trim: true });
        // Footer instructions
        let footer =
            Paragraph:REDACTED_SECRETLine::from("Press Y to apply, P to preflight, N to cancel.").dim())
                .wrap(Wrap { trim: true });
        ...
        if app.apply_preflight_inflight {
            ...
            let mut body_lines: Vec<Line> = Vec:REDACTED_SECRET);
            ...
            let body = Paragraph:REDACTED_SECRETbody_lines).wrap(Wrap { trim: true });
...
fn style_conversation_lines(
    ...
    let mut styled: Vec<Line<'static>> = Vec:REDACTED_SECRET);
    ...
    for (display, &src_idx) in wrapped.iter().zip(indices.iter()) {
        ...
        if raw.is_empty() {
            let mut spans: Vec<Span> = Vec:REDACTED_SECRET);
...
fn conversation_text_spans(
    ...
    if let Some(indent) = bullet_indent {
        ...
        let mut continuation = String:REDACTED_SECRET);
...
fn style_diff_line(raw: REDACTED_SECRET) -> Line<'static> {
    use ratatui::style::Color;
    use ratatui::style::Modifier;
    use ratatui::style::Style;
    use ratatui::text::Span;
...
fn render_task_item(_app: &App, t: &codex_cloud_tasks_client::TaskSummary) -> ListItem<'static> {
    ...
    let mut meta: Vec<ratatui::text::Span> = Vec:REDACTED_SECRET);
    ...
    ListItem:REDACTED_SECRETvec![title, meta_line, sub, spacer])
}
...
fn draw_inline_spinner(
    ...
    frame.render_widget(Paragraph:REDACTED_SECRETline), area);
}
...
pub fn draw_env_modal(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let subheader = Paragraph:REDACTED_SECRETLine::from(
    ...
    let search = Paragraph:REDACTED_SECRETformat!("Search: {query}")).wrap(Wrap { trim: true });
    ...
    let mut items: Vec<ListItem> = Vec:REDACTED_SECRET);
    items.push(ListItem:REDACTED_SECRETLine::from("All Environments (Global)")));
    for env in envs.iter() {
        ...
        items.push(ListItem:REDACTED_SECRETLine::from(spans)));
    }
    ...
    let list = List:REDACTED_SECRETitems)
        .highlight_symbol("› ")
        .highlight_style(Style::default().bold())
...
pub fn draw_best_of_modal(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    let modal_area = Rect:REDACTED_SECRETmodal_x, modal_y, modal_width, modal_height);
    ...
    let hint = Paragraph:REDACTED_SECRETLine::from("Use ↑/↓ to choose, 1-4 jump".cyan().dim()))
        .wrap(Wrap { trim: true });
