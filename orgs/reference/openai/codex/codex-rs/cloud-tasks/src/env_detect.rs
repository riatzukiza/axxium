...
pub async fn autodetect_environment_id(
    ...
    let mut by_repo_envs: Vec<CodeEnvironment> = Vec:REDACTED_SECRET);
...
fn pick_environment_row(
    ...
    desired_label: Option<REDACTED_SECRET>,
...
fn get_git_origins() -> Vec<String> {
        ...
        && ok.status.success()
    {
        ...
        let mut urls = Vec:REDACTED_SECRET);
        for line in s.lines() {
        ...
        && ok.status.success()
    {
        ...
        let mut urls = Vec:REDACTED_SECRET);
        for line in s.lines() {
            let parts: Vec<REDACTED_SECRET> = line.split_whitespace().collect();
            if parts.len() >= 2 {
...
pub async fn list_environments(
    ...
    let mut map: HashMap<String, crate::app::EnvironmentRow> = HashMap:REDACTED_SECRET);
