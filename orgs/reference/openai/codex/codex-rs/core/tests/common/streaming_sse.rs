...
pub async fn start_streaming_sse_server(
    ...
    let state = Arc:REDACTED_SECRETTokioMutex:REDACTED_SECRETStreamingSseState {
        responses: VecDeque::from(responses),
        completions: VecDeque::from(completion_senders),
    }));
    let requests = Arc:REDACTED_SECRETTokioMutex:REDACTED_SECRETVec:REDACTED_SECRET)));
...
struct StreamingSseState {
    ...
    completions: VecDeque<oneshot::REDACTED_SECRETer<i64>>,
}

async fn take_next_stream(
    state: &TokioMutex<StreamingSseState>,
) -> Option<(Vec<StreamingSseChunk>, oneshot::REDACTED_SECRETer<i64>)> {
...
async fn read_http_request(stream: &mut tokio::net::TcpStream) -> (String, Vec<u8>) {
    let mut buf = Vec:REDACTED_SECRET);
    ...
    (String::from_utf8_lossy(&buf).into_owned(), Vec:REDACTED_SECRET))
}

fn parse_request_line(request: REDACTED_SECRET) -> Option<(REDACTED_SECRET, REDACTED_SECRET)> {
...
fn content_length(headers: REDACTED_SECRET) -> Option<usize> {
...
async fn write_http_response(
    stream: &mut tokio::net::TcpStream,
    status: i64,
    body: REDACTED_SECRET,
    content_type: REDACTED_SECRET,
) -> std::io::Result<()> {
...
mod tests {
    ...
    fn split_response(response: REDACTED_SECRET) -> (REDACTED_SECRET, REDACTED_SECRET) {
        response
            .split_once("\r\n\r\n")
            .expect("response missing header separator")
    }

    fn status_code(headers: REDACTED_SECRET) -> u16 {
    ...
    fn header_value<'a>(headers: &'a str, name: REDACTED_SECRET) -> Option<&'a str> {
        headers.lines().skip(1).find_map(|line| {
            let mut parts = line.splitn(2, ':');
    ...
    async fn connect(uri: REDACTED_SECRET) -> TcpStream {
    ...
    async fn read_until(stream: &mut TcpStream, needle: REDACTED_SECRET) -> (String, String) {
        let mut buf = Vec:REDACTED_SECRET);
        ...
        (String::from_utf8_lossy(&buf).into_owned(), String:REDACTED_SECRET))
    }

    async fn send_request(stream: &mut TcpStream, request: REDACTED_SECRET) {
        stream
    ...
    async fn get_models_returns_empty_list() {
        let (server, _) = start_streaming_sse_server(Vec:REDACTED_SECRET)).await;
...
"#;
        ...
        let resp = reqwest::Client:REDACTED_SECRET)
            .post(url)
            .json(&payload)
            .send()
            .await
    ...
    async fn take_next_stream_consumes_in_lockstep() {
        ...
        let state = TokioMutex:REDACTED_SECRETStreamingSseState {
