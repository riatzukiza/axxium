...
impl ResponseMock {
    fn new() -> Self {
        Self {
            requests: Arc:REDACTED_SECRETMutex:REDACTED_SECRETVec:REDACTED_SECRET))),
        }
    }
    ...
    /// provided `call_id`.
    pub fn saw_function_call(&self, call_id: REDACTED_SECRET) -> bool {
        self.requests()
            .iter()
    ...
    pub fn function_call_output_text(&self, call_id: REDACTED_SECRET) -> Option<String> {
        self.requests()
            .iter()
...
fn is_zstd_encoding(value: REDACTED_SECRET) -> bool {
    value
        .split(',')
        .any(|entry| entry.trim().eq_ignore_ascii_case("zstd"))
}

fn decode_body_bytes(body: &[u8], content_encoding: Option<REDACTED_SECRET>) -> Vec<u8> {
    if content_encoding.is_some_and(is_zstd_encoding) {
        zstd::stream::decode_all(std::io::Cursor:REDACTED_SECRETbody)).unwrap_or_else(|err| {
...
impl ResponsesRequest {
    ...
    pub fn body_contains_text(&self, text: REDACTED_SECRET) -> bool {
    ...
    pub fn message_input_texts(&self, role: REDACTED_SECRET) -> Vec<String> {
    ...
    pub fn message_input_text_groups(&self, role: REDACTED_SECRET) -> Vec<Vec<String>> {
        self.inputs_of_type("message")
            .into_iter()
            .filter(|item| item.get("role").and_then(Value::as_str) == Some(role))
            .filter_map(|item| item.get("content").and_then(Value::as_array).cloned())
    ...
    pub fn message_input_image_urls(&self, role: REDACTED_SECRET) -> Vec<String> {
        self.inputs_of_type("message")
            .into_iter()
            .filter(|item| item.get("role").and_then(Value::as_str) == Some(role))
            .filter_map(|item| item.get("content").and_then(Value::as_array).cloned())
    ...
    pub fn inputs_of_type(&self, ty: REDACTED_SECRET) -> Vec<Value> {
        self.input()
            .iter()
    ...
    pub fn function_call_output(&self, call_id: REDACTED_SECRET) -> Value {
        self.call_output(call_id, "function_call_output")
    }

    pub fn custom_tool_call_output(&self, call_id: REDACTED_SECRET) -> Value {
        self.call_output(call_id, "custom_tool_call_output")
    }

    pub fn tool_search_output(&self, call_id: REDACTED_SECRET) -> Value {
        self.call_output(call_id, "tool_search_output")
    }

    pub fn call_output(&self, call_id: REDACTED_SECRET, call_type: REDACTED_SECRET) -> Value {
        self.input()
            .iter()
            .find(|item| {
    ...
    pub fn has_function_call(&self, call_id: REDACTED_SECRET) -> bool {
    ...
    pub fn header(&self, name: REDACTED_SECRET) -> Option<String> {
        self.0
            .headers
            .get(name)
    ...
    pub fn query_param(&self, name: REDACTED_SECRET) -> Option<String> {
        self.0
            .url
            .query_pairs()
            .find(|(k, _)| k == name)
...
mod tests {
    ...
    fn request_with_input(input: Value) -> ResponsesRequest {
        ResponsesRequest(wiremock::Request {
            ...
            method: Method::POST,
            headers: HeaderMap:REDACTED_SECRET),
...
impl WebSocketHandshake {
    pub fn uri(&self) -> REDACTED_SECRET {
        &self.uri
    }
...
pub struct WebSocketTestServer {
    ...
    shutdown: oneshot::REDACTED_SECRETer<()>,
...
pub fn sse(events: Vec<Value>) -> String {
    ...
    let mut out = String:REDACTED_SECRET);
    for ev in events {
...
pub fn sse_completed(id: REDACTED_SECRET) -> String {
    sse(vec![ev_response_created(id), ev_completed(id)])
}

/// Convenience: SSE event for a completed response with a specific id.
pub fn ev_completed(id: REDACTED_SECRET) -> Value {
    serde_json::json!({
...
pub fn ev_response_created(id: REDACTED_SECRET) -> Value {
    serde_json::json!({
...
pub fn ev_completed_with_tokens(id: REDACTED_SECRET, total_tokens: i64) -> Value {
    serde_json::json!({
        "type": "response.completed",
        "response": {
            "id": id,
...
pub fn ev_assistant_message(id: REDACTED_SECRET, text: REDACTED_SECRET) -> Value {
    serde_json::json!({
...
pub fn user_message_item(text: REDACTED_SECRET) -> ResponseItem {
...
pub fn ev_message_item_added(id: REDACTED_SECRET, text: REDACTED_SECRET) -> Value {
    serde_json::json!({
...
pub fn ev_output_text_delta(delta: REDACTED_SECRET) -> Value {
    serde_json::json!({
...
pub fn ev_reasoning_item(id: REDACTED_SECRET, summary: &[REDACTED_SECRET], raw_content: &[REDACTED_SECRET]) -> Value {
...
pub fn ev_reasoning_item_added(id: REDACTED_SECRET, summary: &[REDACTED_SECRET]) -> Value {
    let summary_entries: Vec<Value> = summary
        .iter()
        .map(|text| serde_json::json!({"type": "summary_text", "text": text}))
        .collect();
...
pub fn ev_reasoning_summary_text_delta(delta: REDACTED_SECRET) -> Value {
    serde_json::json!({
...
pub fn ev_reasoning_text_delta(delta: REDACTED_SECRET) -> Value {
    serde_json::json!({
...
pub fn ev_web_search_call_added_partial(id: REDACTED_SECRET, status: REDACTED_SECRET) -> Value {
    serde_json::json!({
        "type": "response.output_item.added",
        "item": {
...
pub fn ev_web_search_call_done(id: REDACTED_SECRET, status: REDACTED_SECRET, query: REDACTED_SECRET) -> Value {
    serde_json::json!({
        "type": "response.output_item.done",
        "item": {
            "type": "web_search_call",
...
pub fn ev_image_generation_call(
    ...
    revised_prompt: REDACTED_SECRET,
...
pub fn ev_function_call(call_id: REDACTED_SECRET, name: REDACTED_SECRET, arguments: REDACTED_SECRET) -> Value {
    serde_json::json!({
        "type": "response.output_item.done",
        "item": {
            "type": "function_call",
...
pub fn ev_tool_search_call(call_id: REDACTED_SECRET, arguments: &serde_json::Value) -> Value {
    serde_json::json!({
        "type": "response.output_item.done",
        "item": {
...
pub fn ev_custom_tool_call(call_id: REDACTED_SECRET, name: REDACTED_SECRET, input: REDACTED_SECRET) -> Value {
    serde_json::json!({
        "type": "response.output_item.done",
        "item": {
...
pub fn ev_local_shell_call(call_id: REDACTED_SECRET, status: REDACTED_SECRET, command: Vec<REDACTED_SECRET>) -> Value {
    serde_json::json!({
        "type": "response.output_item.done",
        "item": {
...
pub fn ev_apply_patch_custom_tool_call(call_id: REDACTED_SECRET, patch: REDACTED_SECRET) -> Value {
    serde_json::json!({
        "type": "response.output_item.done",
        "item": {
            "type": "custom_tool_call",
...
pub fn ev_apply_patch_function_call(call_id: REDACTED_SECRET, patch: REDACTED_SECRET) -> Value {
...
pub fn ev_shell_command_call(call_id: REDACTED_SECRET, command: REDACTED_SECRET) -> Value {
    let args = serde_json::json!({ "command": command });
    ev_shell_command_call_with_args(call_id, &args)
}

pub fn ev_shell_command_call_with_args(call_id: REDACTED_SECRET, args: &serde_json::Value) -> Value {
    let arguments = serde_json::to_string(args).expect("serialize shell command arguments");
    ev_function_call(call_id, "shell_command", &arguments)
}

pub fn ev_apply_patch_shell_call(call_id: REDACTED_SECRET, patch: REDACTED_SECRET) -> Value {
...
pub fn ev_apply_patch_shell_call_via_heredoc(call_id: REDACTED_SECRET, patch: REDACTED_SECRET) -> Value {
...
pub fn ev_apply_patch_shell_command_call_via_heredoc(call_id: REDACTED_SECRET, patch: REDACTED_SECRET) -> Value {
...
pub fn sse_failed(id: REDACTED_SECRET, code: REDACTED_SECRET, message: REDACTED_SECRET) -> String {
...
pub fn sse_response(body: String) -> ResponseTemplate {
    ResponseTemplate:REDACTED_SECRET200)
...
where
    M: wiremock::Match + REDACTED_SECRET + Sync + 'static,
{
    let (mock, response_mock) = base_mock();
    mock.and(matcher)
...
fn base_mock() -> (MockBuilder, ResponseMock) {
    let response_mock = ResponseMock:REDACTED_SECRET);
...
fn models_mock() -> (MockBuilder, ModelsMock) {
    let models_mock = ModelsMock:REDACTED_SECRET);
...
pub async fn mount_compact_user_history_with_summary_sequence(
    ...
    let responder = UserHistorySummaryResponder {
        num_calls: AtomicUsize:REDACTED_SECRET0),
        summary_texts,
    };
    let (mock, response_mock) = compact_mock();
...
pub async fn start_mock_server() -> MockServer {
    ...
    let _ = mount_models_once(&server, ModelsResponse { models: Vec:REDACTED_SECRET) }).await;
...
pub async fn start_websocket_server(connections: Vec<Vec<Vec<Value>>>) -> WebSocketTestServer {
    let connections = connections
        ...
        .map(|requests| WebSocketConnectionConfig {
            requests,
            response_headers: Vec:REDACTED_SECRET),
            accept_delay: None,
...
pub async fn start_websocket_server_with_headers(
    ...
    let connections_log = Arc:REDACTED_SECRETMutex:REDACTED_SECRETVec:REDACTED_SECRET)));
    let handshakes_log = Arc:REDACTED_SECRETMutex:REDACTED_SECRETVec:REDACTED_SECRET)));
    let request_log_updated = Arc:REDACTED_SECRETNotify:REDACTED_SECRET));
    ...
    let connections = Arc:REDACTED_SECRETMutex:REDACTED_SECRETVecDeque::from(connections)));
    ...
    impl Respond for SeqResponder {
        fn respond(&self, _: &wiremock::Request) -> ResponseTemplate {
            ...
            match self.responses.get(call_num) {
                Some(body) => ResponseTemplate:REDACTED_SECRET200)
                    .insert_header("content-type", "text/event-stream")
...
fn validate_request_body_invariants(request: &wiremock::Request) {
    ...
    fn get_call_id(item: &Value) -> Option<REDACTED_SECRET> {
        item.get("call_id")
            .and_then(Value::as_str)
            .filter(|id| !id.is_empty())
    }

    fn gather_ids(items: &[Value], kind: REDACTED_SECRET) -> HashSet<String> {
        items
            .iter()
    ...
    fn gather_output_ids(items: &[Value], kind: REDACTED_SECRET, missing_msg: REDACTED_SECRET) -> HashSet<String> {
        items
            .iter()
            .filter(|item| item.get("type").and_then(Value::as_str) == Some(kind))
            .map(|item| {
