...
pub fn format_labeled_requests_snapshot(
    ...
    sections: &[(REDACTED_SECRET, &ResponsesRequest)],
    options: &ContextSnapshotOptions,
) -> String {
...
pub fn format_labeled_items_snapshot(
    scenario: REDACTED_SECRET,
    sections: &[(REDACTED_SECRET, &[Value])],
    options: &ContextSnapshotOptions,
) -> String {
    let sections = sections
        .iter()
...
fn format_snapshot_text(text: REDACTED_SECRET, options: &ContextSnapshotOptions) -> String {
...
fn normalize_snapshot_line_endings(text: REDACTED_SECRET) -> String {
    text.replace("\r\n", "\n").replace('\r', "\n")
}

fn canonicalize_snapshot_text(text: REDACTED_SECRET) -> String {
...
fn is_capability_instruction_text(text: REDACTED_SECRET) -> bool {
...
fn normalize_dynamic_snapshot_paths(text: REDACTED_SECRET) -> String {
    static SYSTEM_SKILL_PATH_RE: OnceLock<Regex> = OnceLock:REDACTED_SECRET);
    let system_skill_path_re = SYSTEM_SKILL_PATH_RE.get_or_init(|| {
        Regex:REDACTED_SECRETr"/[^)\n]*/skills/\.system/([^/\n]+)/SKILL\.md")
