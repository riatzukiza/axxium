...
pub fn process_is_alive(pid: REDACTED_SECRET) -> anyhow::Result<bool> {
    let status = std::process::Command:REDACTED_SECRET"kill")
        .args(["-0", pid])
        .status()
...
pub async fn wait_for_process_exit(pid: REDACTED_SECRET) -> anyhow::Result<()> {
