...
impl TestCodexExecBuilder {
    pub fn cmd(&self) -> assert_cmd::Command {
        let mut cmd = assert_cmd::Command:REDACTED_SECRET
...
fn toml_string_literal(value: REDACTED_SECRET) -> String {
...
pub fn test_codex_exec() -> TestCodexExecBuilder {
    TestCodexExecBuilder {
        home: TempDir:REDACTED_SECRET).expect("create temp home"),
        cwd: TempDir:REDACTED_SECRET).expect("create temp cwd"),
    }
}
