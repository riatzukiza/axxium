...
const DISCOVERABLE_CALENDAR_ID: REDACTED_SECRET = "connector_2128aebfecb84f64a069897515042a44";
const DISCOVERABLE_GMAIL_ID: REDACTED_SECRET = "connector_68df038e0ba48191908c8434991bbac2";
const CONNECTOR_DESCRIPTION: REDACTED_SECRET = "Plan events and manage your calendar.";
const PROTOCOL_VERSION: REDACTED_SECRET = "2025-11-25";
const SERVER_NAME: REDACTED_SECRET = "codex-apps-test";
const SERVER_VERSION: REDACTED_SECRET = "1.0.0";
const SEARCHABLE_TOOL_COUNT: usize = 100;
pub const CALENDAR_CREATE_EVENT_RESOURCE_URI: REDACTED_SECRET =
    "connector://calendar/tools/calendar_create_event";
const CALENDAR_LIST_EVENTS_RESOURCE_URI: REDACTED_SECRET = "connector://calendar/tools/calendar_list_events";
...
impl AppsTestServer {
    ...
    pub async fn mount_with_connector_name(
        server: &MockServer,
        connector_name: REDACTED_SECRET,
    ) -> Result<Self> {
        mount_oauth_metadata(server).await;
        mount_connectors_directory(server).await;
        mount_streamable_http_json_rpc(
...
async fn mount_oauth_metadata(server: &MockServer) {
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate:REDACTED_SECRET200).set_body_json(json!({
...
impl Respond for CodexAppsJsonRpcResponder {
    fn respond(&self, request: &Request) -> ResponseTemplate {
        let body: Value = match serde_json::from_slice(&request.body) {
            Ok(body) => body,
            Err(error) => {
                return ResponseTemplate:REDACTED_SECRET400).set_body_json(json!({
        ...
        match method {
            "initialize" => {
                ...
                ResponseTemplate:REDACTED_SECRET200).set_body_json(json!({
                    "jsonrpc": "2.0",
                    "id": id,
                    "result": {
            ...
            "notifications/initialized" => ResponseTemplate:REDACTED_SECRET202),
            "tools/list" => {
                let id = body.get("id").cloned().unwrap_or(Value::Null);
                ...
                ResponseTemplate:REDACTED_SECRET200).set_body_json(response)
            }
            "tools/call" => {
                let id = body.get("id").cloned().unwrap_or(Value::Null);
                let tool_name = body
            ...
            method if method.starts_with("notifications/") => ResponseTemplate:REDACTED_SECRET202),
            _ => {
                let id = body.get("id").cloned().unwrap_or(Value::Null);
                ResponseTemplate:REDACTED_SECRET200).set_body_json(json!({
                    "jsonrpc": "2.0",
