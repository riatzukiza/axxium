...
static TEST_ARG0_PATH_ENTRY: OnceLock<Option<Arg0PathEntryGuard>> = OnceLock:REDACTED_SECRET);
...
#[ctor]
fn configure_insta_workspace_REDACTED_SECRET_for_snapshot_tests() {
    if std::env::var_os("INSTA_WORKSPACE_ROOT").is_some() {
        return;
    }

    let workspace_REDACTED_SECRET = codex_utils_cargo_bin::repo_REDACTED_SECRET()
        .ok()
        .map(|REDACTED_SECRET| REDACTED_SECRET.join("codex-rs"));

    if let Some(workspace_REDACTED_SECRET) = workspace_REDACTED_SECRET
        && let Ok(workspace_REDACTED_SECRET) = workspace_REDACTED_SECRET.canonicalize()
    {
        // Safety: this ctor runs at process startup before test threads begin.
        unsafe {
            std::env::set_var("INSTA_WORKSPACE_ROOT", workspace_REDACTED_SECRET);
        }
    }
}

#[track_caller]
pub fn assert_regex_match<'s>(pattern: REDACTED_SECRET, actual: &'s str) -> regex_lite::Captures<'s> {
    let regex = Regex:REDACTED_SECRETpattern).unwrap_or_else(|err| {
...
pub fn test_path_buf_with_windows(unix_path: REDACTED_SECRET, windows_path: Option<REDACTED_SECRET>) -> PathBuf {
    if cfg!(windows) {
...
pub fn fetch_dotslash_file(
    ...
    let mut command = std::process::Command:REDACTED_SECRET"dotslash");
...
pub fn load_sse_fixture_with_id_from_str(raw: REDACTED_SECRET, id: REDACTED_SECRET) -> String {
...
const REMOTE_ENV_ENV_VAR: REDACTED_SECRET = "CODEX_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: REDACTED_SECRET) -> Vec<String> {
    codex_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}

pub fn format_with_current_shell_display(command: REDACTED_SECRET) -> String {
...
pub fn format_with_current_shell_non_login(command: REDACTED_SECRET) -> Vec<String> {
    codex_core::shell::default_user_shell()
        .derive_exec_args(command, /*use_login_shell*/ false)
}

pub fn format_with_current_shell_display_non_login(command: REDACTED_SECRET) -> String {
...
pub mod fs_wait {
    ...
    pub async fn wait_for_matching_file(
        REDACTED_SECRET: impl Into<PathBuf>,
        timeout: Duration,
        predicate: impl FnMut(&Path) -> bool + REDACTED_SECRET + 'static,
    ) -> Result<PathBuf> {
        let REDACTED_SECRET = REDACTED_SECRET.into();
        task::spawn_blocking(move || {
            let mut predicate = predicate;
            blocking_find_matching_file(REDACTED_SECRET, timeout, &mut predicate)
        })
        .await?
    }
    ...
    fn wait_for_path_exists_blocking(path: PathBuf, timeout: Duration) -> Result<PathBuf> {
        ...
        let watch_REDACTED_SECRET = nearest_existing_ancestor(&path);
        let (tx, rx) = mpsc::channel();
        let mut watcher = notify::recommended_watcher(move |res| {
            let _ = tx.send(res);
        })?;
        watcher.watch(&watch_REDACTED_SECRET, RecursiveMode::Recursive)?;
    ...
    fn blocking_find_matching_file(
        ...
        let REDACTED_SECRET = wait_for_path_exists_blocking(REDACTED_SECRET, timeout)?;

        if let Some(found) = scan_for_match(&REDACTED_SECRET, predicate) {
            return Ok(found);
        }
        ...
        watcher.watch(&REDACTED_SECRET, RecursiveMode::Recursive)?;
        ...
        if let Some(found) = scan_for_match(&REDACTED_SECRET, predicate) {
            ...
            Err(anyhow!("timed out waiting for matching file in {REDACTED_SECRET:?}"))
        }
    }

    fn scan_for_match(REDACTED_SECRET: &Path, predicate: &mut impl FnMut(&Path) -> bool) -> Option<PathBuf> {
        for entry in WalkDir:REDACTED_SECRETREDACTED_SECRET).into_iter().filter_map(Result::ok) {
            let path = entry.path();
