...
fn write_skill(home: &Path, name: REDACTED_SECRET, description: REDACTED_SECRET, body: REDACTED_SECRET) -> std::path::PathBuf {
...
fn system_skill_md_path(home: impl AsRef<Path>, name: REDACTED_SECRET) -> std::path::PathBuf {
    home.as_ref()
        .join("skills")
        .join(".system")
        .join(name)
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    const SYSTEM_SKILL_NAME: REDACTED_SECRET = "skill-creator";
