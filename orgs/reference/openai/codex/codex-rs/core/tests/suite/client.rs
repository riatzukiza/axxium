...
use codex_protocol::models:REDACTED_SECRETShellAction;
use codex_protocol::models:REDACTED_SECRETShellExecAction;
use codex_protocol::models:REDACTED_SECRETShellStatus;
...
fn assert_message_role(request_body: &serde_json::Value, role: REDACTED_SECRET) {
    assert_eq!(request_body["role"].as_str().unwrap(), role);
}

#[expect(clippy::unwrap_used)]
fn message_input_texts(item: &serde_json::Value) -> Vec<REDACTED_SECRET> {
    item["content"]
        .as_array()
        .unwrap()
        .iter()
...
fn message_input_text_contains(request: &ResponsesRequest, role: REDACTED_SECRET, needle: REDACTED_SECRET) -> bool {
    request
...
fn write_auth_json(
    codex_home: &TempDir,
    openai_api_key: Option<REDACTED_SECRET>,
    chatgpt_plan_type: REDACTED_SECRET,
...
impl ProviderAuthCommandFixture {
    fn new(tokens: &[REDACTED_SECRET]) -> std::io::Result<Self> {
        let tempdir = tempfile::tempdir()?;
        let tokens_file = tempdir.path().join("tokens.txt");
        let mut token_file_contents = String:REDACTED_SECRET);
        for token in tokens {
...
"#,
            ...
            ("./print-token.sh".to_string(), Vec:REDACTED_SECRET))
        };
...
fn non_zero_u64(value: u64) -> NonZeroU64 {
    match NonZeroU64:REDACTED_SECRETvalue) {
        Some(value) => value,
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let tmpdir = TempDir:REDACTED_SECRET).unwrap();
    ...
    let codex_home = Arc:REDACTED_SECRETTempDir:REDACTED_SECRET).unwrap());
...
async fn provider_auth_command_supplies_bearer_token() {
    ...
    .await;
    let auth_fixture = ProviderAuthCommandFixture:REDACTED_SECRET&["command-token"]).unwrap();
...
async fn provider_auth_command_refreshes_after_401() {
    ...
    let server = MockServer::start().await;
    let auth_fixture = ProviderAuthCommandFixture:REDACTED_SECRET&["first-token", "second-token"]).unwrap();

    Mock::given(method("POST"))
        .and(path("/v1/responses"))
        .and(header_regex("Authorization", "Bearer first-token"))
        .respond_with(ResponseTemplate:REDACTED_SECRET401).set_body_string("unauthorized"))
        .expect(1)
        .mount(&server)
        .await;
    Mock::given(method("POST"))
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let client = ModelClient:REDACTED_SECRET
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let first = ResponseTemplate:REDACTED_SECRET200)
        .insert_header("content-type", "text/event-stream")
        .set_body_raw(
            sse(vec![ev_response_created("resp1"), ev_completed("resp1")]),
            "text/event-stream",
    ...
    let thread_manager = ThreadManager:REDACTED_SECRET
        &config,
        auth_manager,
        SessionSource::Exec,
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    prompt.input.push(ResponseItem:REDACTED_SECRETShellCall {
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate:REDACTED_SECRET200)
        .insert_header("content-type", "text/event-stream")
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate:REDACTED_SECRET429)
...
/// the safest choice.
const EXISTING_ENV_VAR_WITH_NON_EMPTY_VALUE: REDACTED_SECRET = "PATH";
