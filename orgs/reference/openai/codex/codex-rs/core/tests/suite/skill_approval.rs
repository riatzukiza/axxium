...
fn write_skill_metadata(home: &Path, name: REDACTED_SECRET, contents: REDACTED_SECRET) -> Result<()> {
...
fn shell_command_arguments(command: REDACTED_SECRET) -> Result<String> {
...
fn write_skill_with_shell_script_contents(
    ...
    script_name: REDACTED_SECRET,
    script_contents: REDACTED_SECRET,
) -> Result<PathBuf> {
...
fn skill_script_command(test: &TestCodex, script_name: REDACTED_SECRET) -> Result<String> {
...
fn output_shows_sandbox_denial(output: REDACTED_SECRET) -> bool {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    assert!(
        !Path:REDACTED_SECREToutside_path).exists(),
