...
async fn stdio_server_propagates_whitelisted_env_vars() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("MCP_TEST_VALUE", OsStr:REDACTED_SECRETexpected_env_value));
    let rmcp_test_server_bin = stdio_server_bin()?;
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    let mut http_server_child = Command:REDACTED_SECRET&rmcp_http_server_bin)
        .kill_on_drop(true)
...
fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let handle = std::thread::Builder:REDACTED_SECRET)
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    let temp_home = Arc:REDACTED_SECRETtempdir()?);
