...
async fn run_turn(test: &TestCodex, prompt: REDACTED_SECRET) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestCodex, prompt: REDACTED_SECRET) -> anyhow::Result<Duration> {
...
async fn shell_tools_start_before_response_completed_when_stream_delayed() -> anyhow::Result<()> {
    ...
    let output_file = tempfile::NamedTempFile:REDACTED_SECRET)?;
    ...
    let mut builder = test_codex().with_model("gpt-5.1");
    let test = builder
        .build_with_streaming_server(REDACTED_SECRETeaming_server)
        .await?;
