...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let initial_cwd = TempDir:REDACTED_SECRET).unwrap();

    let repo_dir = TempDir:REDACTED_SECRET).unwrap();
    let repo_path = repo_dir.path();

    fn run_git(repo_path: &std::path::Path, args: &[REDACTED_SECRET]) {
        let output = std::process::Command:REDACTED_SECRET"git")
            .arg("-C")
            .arg(repo_path)
            .args(args)
            .output()
    ...
    let head_sha = std::process::Command:REDACTED_SECRET"git")
        .arg("-C")
        .arg(repo_path)
        .args(["rev-parse", "HEAD"])
        .output()
...
where
    F: FnOnce(&mut Config) + REDACTED_SECRET + 'static,
{
