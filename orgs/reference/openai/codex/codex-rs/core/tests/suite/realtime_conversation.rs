...
const MEMORY_PROMPT_PHRASE: REDACTED_SECRET =
    "You have access to a memory folder with guidance from prior runs.";
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: REDACTED_SECRET =
...
fn run_realtime_conversation_test_in_subprocess(
    test_name: REDACTED_SECRET,
    openai_api_key: Option<REDACTED_SECRET>,
) -> Result<()> {
    let mut command = Command:REDACTED_SECRETstd::env::current_exe()?);
    command
        .arg("--exact")
        .arg(test_name)
...
async fn seed_recent_thread(
    ...
    first_user_message: REDACTED_SECRET,
...
async fn conversation_disables_realtime_startup_context_with_empty_override() -> Result<()> {
    ...
    let mut builder = test_codex().with_config({
        ...
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.experimental_realtime_ws_backend_prompt = Some("prompt from config".to_string());
            config.experimental_realtime_ws_startup_context = Some(String:REDACTED_SECRET));
        }
    });
    let test = builder.build_with_websocket_server(&startup_server).await?;
    seed_recent_thread(
