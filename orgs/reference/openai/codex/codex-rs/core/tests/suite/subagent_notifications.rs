...
const TURN_0_FORK_PROMPT: REDACTED_SECRET = "seed fork context";
const TURN_1_PROMPT: REDACTED_SECRET = "spawn a child and continue";
const TURN_2_NO_WAIT_PROMPT: REDACTED_SECRET = "follow up without wait";
const CHILD_PROMPT: REDACTED_SECRET = "child: do work";
const INHERITED_MODEL: REDACTED_SECRET = "gpt-5.2-codex";
...
const SPAWNED_AGENT_DEVELOPER_INSTRUCTIONS: REDACTED_SECRET = "You are a newly spawned agent in a team of agents collaborating to complete a task. You can spawn sub-agents to handle subtasks, and those sub-agents can spawn their own sub-agents. You are responsible for returning the response to your assigned task in the final channel. When you give your response, the contents of your response in the final channel will be immediately delivered back to your parent agent. The prior conversation history was forked from your parent agent. Treat the next user message as your assigned task, and use the forked history only as background context.";
...
fn tool_parameter_description(
    ...
    parameter_name: REDACTED_SECRET,
) -> Option<String> {
    req.body_json()
        .get("tools")
...
fn role_block(description: REDACTED_SECRET, role_name: REDACTED_SECRET) -> Option<String> {
