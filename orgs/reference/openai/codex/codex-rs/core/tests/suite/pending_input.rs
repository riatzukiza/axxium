...
fn message_input_texts(body: &Value, role: REDACTED_SECRET) -> Vec<String> {
    body.get("input")
...
fn response_completed_chunks(response_id: REDACTED_SECRET) -> Vec<StreamingSseChunk> {
    vec![
...
async fn submit_user_input(codex: &CodexThread, text: REDACTED_SECRET) {
    codex
        .submit(Op::UserInput {
...
async fn submit_queue_only_agent_mail(codex: &CodexThread, text: REDACTED_SECRET) {
    codex
        .submit(Op::InterAgentCommunication {
            communication: InterAgentCommunication:REDACTED_SECRET
                AgentPath::try_from("/REDACTED_SECRET/worker")
...
async fn wait_for_agent_message(codex: &CodexThread, text: REDACTED_SECRET) {
...
fn assert_two_responses_input_snapshot(snapshot_name: REDACTED_SECRET, requests: &[Vec<u8>]) {
