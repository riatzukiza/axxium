...
impl AgentJobsResponder {
    fn new(spawn_args_json: String) -> Self {
        Self {
            spawn_args_json,
            seen_main: AtomicBool:REDACTED_SECRETfalse),
            call_counter: AtomicUsize:REDACTED_SECRET0),
        }
    }
}
...
fn decode_body_bytes(request: &wiremock::Request) -> Vec<u8> {
    ...
    {
        zstd::stream::decode_all(std::io::Cursor:REDACTED_SECRET&request.body))
...
fn extract_job_and_item(body: &Value) -> Option<(String, String)> {
    ...
    let job_id = Regex:REDACTED_SECRETr"Job ID:\s*([^\n]+)")
        .ok()?
        .captures(&combined)
        .and_then(|caps| caps.get(1))
        .map(|m| m.as_str().trim().to_string())?;
    let item_id = Regex:REDACTED_SECRETr"Item ID:\s*([^\n]+)")
        .ok()?
        .captures(&combined)
        .and_then(|caps| caps.get(1))
        .map(|m| m.as_str().trim().to_string())?;
...
fn parse_simple_csv_line(line: REDACTED_SECRET) -> Vec<String> {
...
async fn report_agent_job_result_rejects_wrong_thread() -> Result<()> {
    ...
    let responder = AgentJobsResponder:REDACTED_SECRETargs_json);
    ...
    let rows: Vec<REDACTED_SECRET> = output.lines().skip(1).collect();
...
async fn spawn_agents_on_csv_dedupes_item_ids() -> Result<()> {
    ...
    let mut item_ids = Vec:REDACTED_SECRET);
    for line in lines {
...
async fn spawn_agents_on_csv_stop_halts_future_items() -> Result<()> {
    ...
    let worker_calls = Arc:REDACTED_SECRETAtomicUsize:REDACTED_SECRET0));
    let responder = StopAfterFirstResponder:REDACTED_SECRETargs_json, worker_calls.clone());
    Mock::given(method("POST"))
        .and(path_regex(".*/responses$"))
        .respond_with(responder)
        .mount(&server)
