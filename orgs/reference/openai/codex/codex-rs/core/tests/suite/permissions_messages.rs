...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn permissions_message_includes_writable_REDACTED_SECRETs() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    .await;
    let writable = TempDir:REDACTED_SECRET)?;
    let writable_REDACTED_SECRET = AbsolutePathBuf::try_from(writable.path())?;
    ...
    let normalize_line_endings = |s: REDACTED_SECRET| s.replace("\r\n", "\n");
