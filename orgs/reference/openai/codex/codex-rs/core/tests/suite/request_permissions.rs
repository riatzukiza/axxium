...
fn shell_command_event(call_id: REDACTED_SECRET, command: REDACTED_SECRET) -> Result<Value> {
    let args = json!({
        "command": command,
        "timeout_ms": 1_000_u64,
    });
...
fn exec_command_event(call_id: REDACTED_SECRET, command: REDACTED_SECRET) -> Result<Value> {
    let args = json!({
        "cmd": command,
...
fn shell_event_with_raw_request_permissions(
    call_id: REDACTED_SECRET,
    command: REDACTED_SECRET,
    workdir: Option<REDACTED_SECRET>,
