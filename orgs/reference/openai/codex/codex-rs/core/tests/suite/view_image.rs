...
fn png_bytes(width: u32, height: u32, rgba: [u8; 4]) -> anyhow::Result<Vec<u8>> {
    ...
    let mut cursor = Cursor:REDACTED_SECRETVec:REDACTED_SECRET));
    DynamicImage::ImageRgba8(image).write_to(&mut cursor, image::ImageFormat::Png)?;
    Ok(cursor.into_inner())
}

async fn create_workspace_directory(test: &TestCodex, rel_path: REDACTED_SECRET) -> anyhow::Result<PathBuf> {
...
async fn user_turn_with_local_image_attaches_image() -> anyhow::Result<()> {
    ...
    codex
        .submit(Op::UserTurn {
            items: vec![UserInput:REDACTED_SECRETImage {
                path: abs_path.clone(),
            }],
