...
const TEST_TIMESTAMP: REDACTED_SECRET = "2025-01-01T00-00-00";
...
async fn read_config_toml(codex_home: &Path) -> io::Result<ConfigToml> {
    ...
    toml::from_str(&contents).map_err(|err| io::Error:REDACTED_SECRETio::ErrorKind::InvalidData, err))
}
...
fn parse_config_toml(contents: REDACTED_SECRET) -> io::Result<ConfigToml> {
    toml::from_str(contents).map_err(|err| io::Error:REDACTED_SECRETio::ErrorKind::InvalidData, err))
}
