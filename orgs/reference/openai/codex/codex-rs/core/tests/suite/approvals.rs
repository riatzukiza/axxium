...
const DEFAULT_UNIFIED_EXEC_JUSTIFICATION: REDACTED_SECRET =
        ...
        match self {
            ...
            ActionKind::FetchUrl {
                ...
                Mock::given(method("GET"))
                    .and(path(*endpoint))
                    .respond_with(
                        ResponseTemplate:REDACTED_SECRET200).set_body_string(response_body.to_string()),
                    )
                    .mount(server)
                    .await;
...
fn build_add_file_patch(patch_path: REDACTED_SECRET, content: REDACTED_SECRET) -> String {
    format!("*** Begin Patch\n*** Add File: {patch_path}\n+{content}\n*** End Patch\n")
}

fn shell_apply_patch_command(patch: REDACTED_SECRET) -> String {
...
fn exec_command_event(
    ...
    sandbox_permissions: SandboxPermissions,
    justification: Option<REDACTED_SECRET>,
) -> Result<Value> {
    let mut args = json!({
        "cmd": cmd.to_string(),
    });
        ...
        match self {
            ...
            Expectation::FileNotCreated {
                ...
                for needle in *message_contains {
                    if needle.contains('|') {
                        let options: Vec<REDACTED_SECRET> = needle.split('|').collect();
                        let matches_any =
...
fn parse_result(item: &Value) -> CommandResult {
    ...
    match serde_json::from_str::<Value>(output_str) {
        ...
        Err(_) => {
            let structured = Regex:REDACTED_SECRETr"(?s)^Exit code:\s*(-?\d+).*?Output:\n(.*)$").unwrap();
            let regex =
                Regex:REDACTED_SECRETr"(?s)^.*?Process exited with code (\d+)\n.*?Output:\n(.*)$").unwrap();
...
async fn expect_exec_approval(
    test: &TestCodex,
    expected_command: REDACTED_SECRET,
...
async fn expect_patch_approval(
    test: &TestCodex,
    expected_call_id: REDACTED_SECRET,
...
fn body_contains(req: &Request, text: REDACTED_SECRET) -> bool {
    let is_zstd = req
        .headers
    ...
    let bytes = if is_zstd {
        zstd::stream::decode_all(std::io::Cursor:REDACTED_SECRET&req.body)).ok()
    } else {
        Some(req.body.clone())
    };
    bytes
...
async fn approving_execpolicy_amendment_persists_policy_and_skips_future_prompts() -> Result<()> {
    ...
    let expected_execpolicy_amendment =
        ExecPolicyAmendment:REDACTED_SECRETvec!["touch".to_string(), "allow-prefix.txt".to_string()]);
...
async fn spawned_subagent_execpolicy_amendment_propagates_to_parent_session() -> Result<()> {
    ...
    const PARENT_PROMPT: REDACTED_SECRET = "spawn a child that repeats a command";
    const CHILD_PROMPT: REDACTED_SECRET = "run the same command twice";
    const SPAWN_CALL_ID: REDACTED_SECRET = "spawn-child-1";
    const CHILD_CALL_ID_1: REDACTED_SECRET = "child-touch-1";
    const PARENT_CALL_ID_2: REDACTED_SECRET = "parent-touch-2";
    ...
    let expected_execpolicy_amendment = ExecPolicyAmendment:REDACTED_SECRETvec![
        "touch".to_string(),
...
-> Result<()> {
    ...
    let server = start_mock_server().await;
    let home = Arc:REDACTED_SECRETTempDir:REDACTED_SECRET)?);
    fs::write(
...
"#,
    ...
    let mut builder = test_codex().with_home(home).with_config(move |config| {
        ...
        config.config_layer_stack = ConfigLayerStack:REDACTED_SECRETlayers, requirements, requirements_toml)
