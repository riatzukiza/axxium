...
fn extract_output_text(item: &Value) -> Option<REDACTED_SECRET> {
...
fn parse_unified_exec_output(raw: REDACTED_SECRET) -> Result<ParsedUnifiedExecOutput> {
...
fn collect_tool_outputs(bodies: &[Value]) -> Result<HashMap<String, ParsedUnifiedExecOutput>> {
    let mut outputs = HashMap:REDACTED_SECRET);
    for body in bodies {
        ...
        EventMsg::PatchApplyBegin(begin) if begin.call_id == call_id => {
            ...
            assert!(
                begin
                    .changes
                    .keys()
                    .any(|path| path.file_name() == Some(OsStr:REDACTED_SECRET"uexec_apply.txt"))),
...
async fn unified_exec_terminal_interaction_captures_delayed_output() -> Result<()> {
    ...
    let mut end_event = None;
    let mut task_completed = false;
    let mut terminal_events = Vec:REDACTED_SECRET);
    let mut delta_text = String:REDACTED_SECRET);
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut begin_events = Vec:REDACTED_SECRET);
    let mut end_events = Vec:REDACTED_SECRET);
    let mut task_completed = false;
    loop {
...
fn assert_command(command: &[String], expected_args: REDACTED_SECRET, expected_cmd: REDACTED_SECRET) {
