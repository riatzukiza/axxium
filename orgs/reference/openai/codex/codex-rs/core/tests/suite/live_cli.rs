...
fn run_live(prompt: REDACTED_SECRET) -> (assert_cmd::assert::Assert, TempDir) {
    #![expect(clippy::unwrap_used)]
    use std::io::Read;
    use std::io::Write;
    use std::thread;

    let dir = TempDir:REDACTED_SECRET).unwrap();
    ...
    let mut cmd = Command:REDACTED_SECRETcodex_utils_cargo_bin::cargo_bin("codex-rs").unwrap());
    ...
    // REDACTED_SECRET the terminating newline so Session::run exits after the first turn.
    child
        .stdin
        .as_mut()
    ...
    fn tee<R: Read + REDACTED_SECRET + 'static>(
        mut reader: R,
        mut writer: impl Write + REDACTED_SECRET + 'static,
