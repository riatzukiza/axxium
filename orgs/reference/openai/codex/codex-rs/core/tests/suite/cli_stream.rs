...
fn repo_REDACTED_SECRET() -> std::path::PathBuf {
    #[expect(clippy::expect_used)]
    codex_utils_cargo_bin::repo_REDACTED_SECRET().expect("failed to resolve repo REDACTED_SECRET")
}
...
async fn responses_mode_stream_cli() {
    ...
    let repo_REDACTED_SECRET = repo_REDACTED_SECRET();
    ...
    let home = TempDir:REDACTED_SECRET).unwrap();
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
    let bin = codex_utils_cargo_bin::cargo_bin("codex").unwrap();
    let mut cmd = AssertCommand:REDACTED_SECRETbin);
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    let custom = TempDir:REDACTED_SECRET).unwrap();
...
/// 3. REDACTED_SECRETs a "hello?" prompt and verifies the response
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let home = TempDir:REDACTED_SECRET)?;
    ...
    let mut cmd2 = AssertCommand:REDACTED_SECRETbin2);
    cmd2.arg("exec")
        .arg("--skip-git-repo-check")
        .arg("-c")
        .arg("openai_base_url=\"http://unused.local\"")
...
async fn integration_git_info_unit_test() {
    ...
    let temp_dir = TempDir:REDACTED_SECRET).unwrap();
    ...
    let init_output = std::process::Command:REDACTED_SECRET"git")
        .envs(envs.clone())
        .args(["init"])
    ...
    std::process::Command:REDACTED_SECRET"git")
        .envs(envs.clone())
    ...
    let commit_output = std::process::Command:REDACTED_SECRET"git")
        .envs(envs.clone())
    ...
    let expected_remote_url = std::process::Command:REDACTED_SECRET"git")
