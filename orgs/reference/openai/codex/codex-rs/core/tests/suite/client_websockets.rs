...
const MODEL: REDACTED_SECRET = "gpt-5.2-codex";
const OPENAI_BETA_HEADER: REDACTED_SECRET = "OpenAI-Beta";
const WS_V2_BETA_HEADER_VALUE: REDACTED_SECRET = "responses_websockets=2026-02-06";
const X_CLIENT_REQUEST_ID_HEADER: REDACTED_SECRET = "x-client-request-id";
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let enriched_turn_metadata = r#"{"turn_id":"turn-123","sandbox":"workspace-write","workspaces":[{"REDACTED_SECRET_path":"/tmp/repo","latest_git_commit_hash":"abc123","associated_remote_urls":["git@github.com:openai/codex.git"],"has_changes":true}]}"#;
    let prompt_one = prompt_with_input(vec![message_item("hello")]);
    let prompt_two = prompt_with_input(vec![
        message_item("hello"),
        assistant_message_item("msg-1", "assistant output"),
...
fn message_item(text: REDACTED_SECRET) -> ResponseItem {
...
fn assistant_message_item(id: REDACTED_SECRET, text: REDACTED_SECRET) -> ResponseItem {
    ResponseItem::Message {
...
fn prompt_with_input_and_instructions(input: Vec<ResponseItem>, instructions: REDACTED_SECRET) -> Prompt {
...
async fn websocket_harness_with_provider_options(
    ...
    let metrics = MetricsClient:REDACTED_SECRET
