...
const TOOL_SUGGEST_TOOL_NAME: REDACTED_SECRET = "tool_suggest";
...
fn function_tool_description(body: &Value, name: REDACTED_SECRET) -> Option<String> {
    body.get("tools")
        .and_then(Value::as_array)
        .and_then(|tools| {
...
fn configure_apps_without_search_tool(config: &mut Config, apps_base_url: REDACTED_SECRET) {
    config
        .features
        .enable(Feature::Apps)
