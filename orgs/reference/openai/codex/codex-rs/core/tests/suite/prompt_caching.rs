...
fn assert_default_env_context(text: REDACTED_SECRET, cwd: REDACTED_SECRET, shell: &Shell) {
...
fn assert_tool_names(body: &serde_json::Value, expected_names: &[REDACTED_SECRET]) {
    assert_eq!(
        body["tools"]
            .as_array()
            .unwrap()
...
async fn overrides_turn_context_but_keeps_cached_prefix_and_key_constant() -> anyhow::Result<()> {
    ...
    let writable = TempDir:REDACTED_SECRET).unwrap();
    let new_policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![writable.path().try_into().unwrap()],
...
async fn override_before_first_turn_emits_environment_context() -> anyhow::Result<()> {
    ...
    let env_texts: Vec<REDACTED_SECRET> = input
        .iter()
        .filter_map(|msg| {
    ...
    let permissions_texts: Vec<REDACTED_SECRET> = input
        .iter()
        .filter_map(|msg| {
    ...
    let user_texts: Vec<REDACTED_SECRET> = input
        .iter()
        .filter_map(|msg| {
            msg["content"].as_array().map(|content| {
                content
...
async fn per_turn_overrides_keep_cached_prefix_and_key_constant() -> anyhow::Result<()> {
    ...
    let new_cwd = TempDir:REDACTED_SECRET).unwrap();
    let writable = TempDir:REDACTED_SECRET).unwrap();
    let new_policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![writable.abs()],
        read_only_access: Default::default(),
        network_access: true,
        exclude_tmpdir_env_var: true,
        exclude_slash_tmp: true,
