...
const AFTER_SECOND_RESUME: REDACTED_SECRET = "AFTER_SECOND_RESUME";
const AFTER_ROLLBACK: REDACTED_SECRET = "AFTER_ROLLBACK";
...
fn normalize_line_endings_str(text: REDACTED_SECRET) -> String {
    if text.contains('\r') {
        text.replace("\r\n", "\n").replace('\r', "\n")
    } else {
        text.to_string()
...
fn extract_summary_user_text(request: &Value, summary_text: REDACTED_SECRET) -> String {
...
fn json_message_input_texts(request: &Value, role: REDACTED_SECRET) -> Vec<String> {
    request
        .get("input")
        .and_then(Value::as_array)
        .into_iter()
...
async fn snapshot_rollback_past_compaction_replays_append_only_history() -> Result<()> {
    ...
    const EDITED_AFTER_COMPACT: REDACTED_SECRET = "EDITED_AFTER_COMPACT";
    const SECOND_REPLY: REDACTED_SECRET = "SECOND_REPLY";
...
async fn snapshot_rollback_followup_turn_trims_context_updates() -> Result<()> {
    ...
    const MODEL: REDACTED_SECRET = "gpt-5.1-codex";
    const TURN_ONE_USER: REDACTED_SECRET = "turn 1 user";
    const TURN_TWO_USER: REDACTED_SECRET = "turn 2 user";
    const FOLLOWUP_USER: REDACTED_SECRET = "follow-up user";
    const ROLLED_BACK_DEV_INSTRUCTIONS: REDACTED_SECRET = "ROLLED_BACK_DEV_INSTRUCTIONS";
    const PRETURN_CONTEXT_DIFF_CWD: REDACTED_SECRET = "PRETURN_CONTEXT_DIFF_CWD";
...
async fn user_turn(conversation: &Arc<CodexThread>, text: REDACTED_SECRET) {
    conversation
        .submit(Op::UserInput {
