...
const FIXTURE_JSON: REDACTED_SECRET = r#"{
...
fn shell_responses(
    ...
    match output_type {
        ...
        ShellModelOutput:REDACTED_SECRETShell => Ok(vec![
            sse(vec![
                ev_response_created("resp-1"),
...
fn configure_shell_model(
    ...
    let builder = match (output_type, include_apply_patch_tool) {
        ...
        (ShellModelOutput:REDACTED_SECRETShell, true) => builder.with_model("gpt-5.1-codex"),
        (ShellModelOutput::Shell, true) => builder.with_model("gpt-5.1-codex"),
        (ShellModelOutput:REDACTED_SECRETShell, false) => builder.with_model("codex-mini-latest"),
...
#[test_case(ShellModelOutput:REDACTED_SECRETShell)]
...
$"#;
    ...
    let wall_time_regex = Regex:REDACTED_SECRETr"(?m)^Wall (?:time|Clock): ([0-9]+(?:\.[0-9]+)?) seconds$")
