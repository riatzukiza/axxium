...
fn extract_log_field(line: REDACTED_SECRET, key: REDACTED_SECRET) -> Option<String> {
...
fn assert_empty_mcp_tool_fields(line: REDACTED_SECRET) -> Result<(), String> {
...
fn extract_log_field_handles_empty_bare_values() {
    ...
    assert_eq!(extract_log_field(line, "mcp_server"), Some(String:REDACTED_SECRET)));
    assert_eq!(
        extract_log_field(line, "mcp_server_origin"),
        Some(String:REDACTED_SECRET))
    );
}
...
async fn responses_api_emits_api_request_event() {
    ...
    logs_assert(|lines: &[REDACTED_SECRET]| {
        lines
            .iter()
...
async fn handle_responses_span_records_response_kind_and_tool_name() {
    let buffer: &'static Mutex<Vec<u8>> = Box::leak(Box:REDACTED_SECRETMutex:REDACTED_SECRETVec:REDACTED_SECRET))));
...
async fn record_responses_sets_span_fields_for_response_events() {
    ...
    let expected = [
        ("created", None::<REDACTED_SECRET>, None::<REDACTED_SECRET>),
