...
struct TestScenario {
    ...
    repo_REDACTED_SECRET: PathBuf,
    file_in_repo_REDACTED_SECRET: PathBuf,
    file_in_dot_git_dir: PathBuf,
}

struct TestExpectations {
    file_outside_repo_is_writable: bool,
    file_in_repo_REDACTED_SECRET_is_writable: bool,
...
impl TestScenario {
    async fn run_test(&self, policy: &SandboxPolicy, expectations: TestExpectations) {
        ...
        assert_eq!(
            touch(&self.file_in_repo_REDACTED_SECRET, policy).await,
            expectations.file_in_repo_REDACTED_SECRET_is_writable
        );
        assert_eq!(
            self.file_in_repo_REDACTED_SECRET.exists(),
            expectations.file_in_repo_REDACTED_SECRET_is_writable
        );
...
/// If the user has added a workspace REDACTED_SECRET that is not a Git repo REDACTED_SECRET, then
...
async fn if_parent_of_repo_is_writable_then_dot_git_folder_is_writable() {
    let tmp = TempDir:REDACTED_SECRET).expect("should be able to create temp dir");
    let test_scenario = create_test_scenario(&tmp);
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![test_scenario.repo_parent.as_path().try_into().unwrap()],
    ...
    test_scenario
        .run_test(
            ...
            TestExpectations {
                ...
                file_in_repo_REDACTED_SECRET_is_writable: true,
...
/// When the writable REDACTED_SECRET is the REDACTED_SECRET of a Git repository (as evidenced by the
/// presence of a .git folder), then the .git folder should be read-only if
/// the policy is `WorkspaceWrite`.
#[tokio::test]
async fn if_git_repo_is_writable_REDACTED_SECRET_then_dot_git_folder_is_read_only() {
    let tmp = TempDir:REDACTED_SECRET).expect("should be able to create temp dir");
    let test_scenario = create_test_scenario(&tmp);
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![test_scenario.repo_REDACTED_SECRET.as_path().try_into().unwrap()],
        read_only_access: Default::default(),
        network_access: false,
        exclude_tmpdir_env_var: true,
        exclude_slash_tmp: true,
...
async fn read_only_forbids_all_writes() {
    ...
    test_scenario
        .run_test(
            ...
            TestExpectations {
                file_outside_repo_is_writable: false,
                file_in_repo_REDACTED_SECRET_is_writable: false,
                file_in_dot_git_dir_is_writable: false,
            },
        )
        .await;
...
async fn java_home_finds_runtime_under_seatbelt() {
    ...
    let java_home_path = Path:REDACTED_SECRET"/usr/libexec/java_home");
    if !java_home_path.exists() {
        eprintln!("/usr/libexec/java_home is not present, skipping test.");
        return;
    }

    let baseline_output = tokio::process::Command:REDACTED_SECRETjava_home_path)
...
fn create_test_scenario(tmp: &TempDir) -> TestScenario {
    ...
    let repo_REDACTED_SECRET = repo_parent.join("repo");
    let dot_git_dir = repo_REDACTED_SECRET.join(".git");

    std::fs::create_dir(&repo_REDACTED_SECRET).expect("should be able to create repo REDACTED_SECRET");
    std::fs::create_dir(&dot_git_dir).expect("should be able to create .git dir");

    TestScenario {
        file_outside_repo: repo_parent.join("outside.txt"),
        repo_parent,
        file_in_repo_REDACTED_SECRET: repo_REDACTED_SECRET.join("repo_file.txt"),
