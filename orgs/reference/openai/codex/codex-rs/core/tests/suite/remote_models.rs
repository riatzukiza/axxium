...
const REMOTE_MODEL_SLUG: REDACTED_SECRET = "codex-test";
...
async fn remote_models_merge_preserves_bundled_models_on_empty_response() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let _models_mock = mount_models_once(&server, ModelsResponse { models: Vec:REDACTED_SECRET) }).await;
...
async fn wait_for_model_available(manager: &Arc<ModelsManager>, slug: REDACTED_SECRET) -> ModelPreset {
...
fn test_remote_model(slug: REDACTED_SECRET, visibility: ModelVisibility, priority: i32) -> ModelInfo {
