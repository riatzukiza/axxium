...
async fn refresh_models_on_models_etag_mismatch_and_avoid_duplicate_models_fetch() -> Result<()> {
    ...
    const ETAG_1: REDACTED_SECRET = "\"models-etag-1\"";
    const ETAG_2: REDACTED_SECRET = "\"models-etag-2\"";
    const CALL_ID: REDACTED_SECRET = "local-shell-call-1";
    ...
    let spawn_models_mock = responses::mount_models_once_with_etag(
        &server,
        ModelsResponse { models: Vec:REDACTED_SECRET) },
        ETAG_1,
    )
    .await;
