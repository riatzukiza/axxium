...
async fn memories_startup_phase2_tracks_added_and_removed_inputs_across_runs() -> Result<()> {
    ...
    let memory_REDACTED_SECRET = home.path().join("memories");
    let raw_memories = tokio::fs::read_to_string(memory_REDACTED_SECRET.join("raw_memories.md")).await?;
    assert!(raw_memories.contains("raw memory A"));
    assert!(!raw_memories.contains("raw memory B"));
    let rollout_summaries = read_rollout_summary_bodies(&memory_REDACTED_SECRET).await?;
...
async fn web_search_pollution_moves_selected_thread_into_removed_phase2_inputs() -> Result<()> {
    ...
    assert_eq!(selection.retained_thread_ids, Vec::<ThreadId>:REDACTED_SECRET));
...
async fn seed_stage1_output(
    ...
    raw_memory: REDACTED_SECRET,
    rollout_summary: REDACTED_SECRET,
    rollout_slug: REDACTED_SECRET,
) -> Result<ThreadId> {
    let thread_id = ThreadId:REDACTED_SECRET);
    let mut metadata_builder = codex_state::ThreadMetadataBuilder:REDACTED_SECRET
        thread_id,
...
async fn seed_stage1_output_for_existing_thread(
    ...
    raw_memory: REDACTED_SECRET,
    rollout_summary: REDACTED_SECRET,
    rollout_slug: Option<REDACTED_SECRET>,
) -> Result<()> {
    let owner = ThreadId:REDACTED_SECRET);
    let claim = db
...
async fn read_rollout_summary_bodies(memory_REDACTED_SECRET: &Path) -> Result<Vec<String>> {
    let mut dir = tokio::fs::read_dir(memory_REDACTED_SECRET.join("rollout_summaries")).await?;
    let mut summaries = Vec:REDACTED_SECRET);
