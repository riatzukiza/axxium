...
fn image_generation_artifact_path(codex_home: &Path, session_id: REDACTED_SECRET, call_id: REDACTED_SECRET) -> PathBuf {
    fn sanitize(value: REDACTED_SECRET) -> String {
...
async fn plan_mode_strips_plan_from_agent_messages() -> anyhow::Result<()> {
    ...
    let mut agent_deltas = Vec:REDACTED_SECRET);
