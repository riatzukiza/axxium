...
const SAMPLE_PLUGIN_CONFIG_NAME: REDACTED_SECRET = "sample@test";
const SAMPLE_PLUGIN_DISPLAY_NAME: REDACTED_SECRET = "sample";
const SAMPLE_PLUGIN_DESCRIPTION: REDACTED_SECRET = "inspect sample data";

fn sample_plugin_REDACTED_SECRET(home: &TempDir) -> std::path::PathBuf {
    home.path().join("plugins/cache/test/sample/local")
}

fn write_sample_plugin_manifest_and_config(home: &TempDir) -> std::path::PathBuf {
    let plugin_REDACTED_SECRET = sample_plugin_REDACTED_SECRET(home);
    std::fs::create_dir_all(plugin_REDACTED_SECRET.join(".codex-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
fn write_plugin_skill_plugin(home: &TempDir) -> std::path::PathBuf {
    let plugin_REDACTED_SECRET = write_sample_plugin_manifest_and_config(home);
    let skill_dir = plugin_REDACTED_SECRET.join("skills/sample-search");
...
fn write_plugin_mcp_plugin(home: &TempDir, command: REDACTED_SECRET) {
    let plugin_REDACTED_SECRET = write_sample_plugin_manifest_and_config(home);
    std::fs::write(
...
fn tool_description(body: &serde_json::Value, tool_name: REDACTED_SECRET) -> Option<String> {
    body.get("tools")
        .and_then(serde_json::Value::as_array)
        .and_then(|tools| {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let codex_home = Arc:REDACTED_SECRETTempDir:REDACTED_SECRET)?);
...
async fn plugin_mcp_tools_are_listed() -> Result<()> {
    ...
    loop {
        ...
        let available_tools: Vec<REDACTED_SECRET> = tool_list.tools.keys().map(String::as_str).collect();
