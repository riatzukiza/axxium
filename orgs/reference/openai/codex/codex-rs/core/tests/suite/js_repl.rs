...
fn assert_js_repl_ok(req: &ResponsesRequest, call_id: REDACTED_SECRET, expected_output: REDACTED_SECRET) {
...
fn assert_js_repl_err(req: &ResponsesRequest, call_id: REDACTED_SECRET, expected_output: REDACTED_SECRET) {
    let (output, success) = custom_tool_output_text_and_success(req, call_id);
...
fn write_too_old_REDACTED_SECRET_script(dir: &Path) -> Result<std::path::PathBuf> {
    #[cfg(windows)]
    {
        let path = dir.join("old-REDACTED_SECRET.cmd");
    ...
    #[cfg(unix)]
    {
        let path = dir.join("old-REDACTED_SECRET.sh");
...
async fn run_js_repl_turn(
    ...
    calls: &[(REDACTED_SECRET, REDACTED_SECRET)],
) -> Result<ResponseMock> {
...
async fn assert_failed_cell_followup(
    server: &MockServer,
    prompt: REDACTED_SECRET,
    failing_cell: REDACTED_SECRET,
    followup_cell: REDACTED_SECRET,
    expected_followup_output: REDACTED_SECRET,
) -> Result<()> {
...
async fn js_repl_is_not_advertised_when_startup_REDACTED_SECRET_is_incompatible() -> Result<()> {
    ...
    let temp = tempdir()?;
    let old_REDACTED_SECRET = write_too_old_REDACTED_SECRET_script(temp.path())?;
    ...
    let mut builder = test_codex().with_config(move |config| {
        config
            ...
            .enable(Feature::JsRepl)
            .expect("test config should allow feature update");
        config.js_repl_REDACTED_SECRET_path = Some(old_REDACTED_SECRET);
    });
    let test = builder.build(&server).await?;
...
async fn js_repl_blocks_sensitive_builtin_imports() -> Result<()> {
    ...
    let mock = run_js_repl_turn(
        ...
        &[("call-1", "await import(\"REDACTED_SECRET:process\");")],
    )
    .await?;
    ...
    assert!(output.contains("Importing module \"REDACTED_SECRET:process\" is not allowed in js_repl"));
