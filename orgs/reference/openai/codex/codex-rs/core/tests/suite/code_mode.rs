...
fn custom_tool_output_items(req: &ResponsesRequest, call_id: REDACTED_SECRET) -> Vec<Value> {
...
fn function_tool_output_items(req: &ResponsesRequest, call_id: REDACTED_SECRET) -> Vec<Value> {
...
fn text_item(items: &[Value], index: usize) -> REDACTED_SECRET {
    items[index]
        .get("text")
        .and_then(Value::as_str)
...
fn extract_running_cell_id(text: REDACTED_SECRET) -> String {
...
fn custom_tool_output_last_non_empty_text(req: &ResponsesRequest, call_id: REDACTED_SECRET) -> Option<String> {
    match req.custom_tool_call_output(call_id).get("output") {
