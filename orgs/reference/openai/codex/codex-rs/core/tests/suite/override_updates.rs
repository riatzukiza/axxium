...
fn collab_mode_with_instructions(instructions: Option<REDACTED_SECRET>) -> CollaborationMode {
    CollaborationMode {
...
fn collab_xml(text: REDACTED_SECRET) -> String {
...
fn rollout_developer_texts(text: REDACTED_SECRET) -> Vec<String> {
...
fn rollout_environment_texts(text: REDACTED_SECRET) -> Vec<String> {
    let mut texts = Vec:REDACTED_SECRET);
    for line in text.lines() {
        let trimmed = line.trim();
        if trimmed.is_empty() {
...
async fn override_turn_context_without_user_turn_does_not_record_environment_update() -> Result<()> {
    ...
    let new_cwd = TempDir:REDACTED_SECRET)?;
