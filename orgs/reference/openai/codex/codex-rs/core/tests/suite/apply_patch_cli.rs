...
async fn apply_patch_cli_can_use_shell_command_output_as_patch_input() -> Result<()> {
    ...
    fn stdout_from_shell_output(output: REDACTED_SECRET) -> String {
    ...
    fn function_call_output_text(body: &serde_json::Value, call_id: REDACTED_SECRET) -> String {
        body.get("input")
            ...
            match call_num {
                0 => {
                    let command = if cfg!(windows) {
                        ...
                        let script = "$ProgressPreference = 'SilentlyContinue'; [Console]::OutputEncoding = [System.Text.UTF8Encoding]:REDACTED_SECRET$false); [System.IO.File]::ReadAllText('source.txt', [System.Text.UTF8Encoding]:REDACTED_SECRET$false))";
    ...
    let responder = DynamicApplyFromRead {
        num_calls: AtomicI32:REDACTED_SECRET0),
