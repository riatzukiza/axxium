...
fn approx_token_count(text: REDACTED_SECRET) -> i64 {
...
fn format_labeled_requests_snapshot(
    ...
    sections: &[(REDACTED_SECRET, &responses::ResponsesRequest)],
) -> String {
...
fn compacted_summary_only_output(summary: REDACTED_SECRET) -> Vec<ResponseItem> {
...
async fn snapshot_request_shape_remote_pre_turn_compaction_context_window_exceeded() -> Result<()> {
    ...
    let compact_mock = responses::mount_compact_response_once(
        harness.server(),
        ResponseTemplate:REDACTED_SECRET400).set_body_json(serde_json::json!({
            "error": {
