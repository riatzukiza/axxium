...
const FIRST_CONTINUATION_PROMPT: REDACTED_SECRET = "Retry with exactly the phrase meow meow meow.";
const SECOND_CONTINUATION_PROMPT: REDACTED_SECRET = "Now tighten it to just: meow.";
const BLOCKED_PROMPT_CONTEXT: REDACTED_SECRET = "Remember the blocked lighthouse note.";

fn write_stop_hook(home: &Path, block_prompts: &[REDACTED_SECRET]) -> Result<()> {
...
fn write_parallel_stop_hooks(home: &Path, prompts: &[REDACTED_SECRET]) -> Result<()> {
...
fn write_user_prompt_submit_hook(
    home: &Path,
    blocked_prompt: REDACTED_SECRET,
    additional_context: REDACTED_SECRET,
) -> Result<()> {
...
fn write_pre_tool_use_hook(
    home: &Path,
    matcher: Option<REDACTED_SECRET>,
...
fn rollout_hook_prompt_texts(text: REDACTED_SECRET) -> Result<Vec<String>> {
    let mut texts = Vec:REDACTED_SECRET);
...
fn ev_message_item_done(id: REDACTED_SECRET, text: REDACTED_SECRET) -> Value {
    serde_json::json!({
...
fn request_message_input_texts(body: &[u8], role: REDACTED_SECRET) -> Vec<String> {
...
async fn pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    assert!(
        Path:REDACTED_SECRETtranscript_path).exists(),
