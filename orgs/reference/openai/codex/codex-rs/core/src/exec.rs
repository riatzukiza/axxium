...
fn select_process_exec_tool_sandbox_type(
    ...
    SandboxManager:REDACTED_SECRET).select_initial(
...
pub struct StdoutStream {
    pub sub_id: String,
    pub call_id: String,
    pub tx_event: REDACTED_SECRETer<Event>,
}
...
pub fn build_exec_request(
    ...
    let (program, args) = command.split_first().ok_or_else(|| {
        CodexErr::Io(io::Error:REDACTED_SECRET
            io::ErrorKind::InvalidInput,
            "command args are empty",
        ))
    })?;

    let manager = SandboxManager:REDACTED_SECRET);
...
pub(crate) async fn execute_exec_request(
    ...
    stdout_stream: Option<StdoutStream>,
    after_spawn: Option<Box<dyn FnOnce() + REDACTED_SECRET>>,
) -> Result<ExecToolCallOutput> {
    let ExecRequest {
        command,
        cwd,
...
fn extract_create_process_as_user_error_code(err: REDACTED_SECRET) -> Option<String> {
...
#[cfg(target_os = "windows")]
fn windowsapps_path_kind(path: REDACTED_SECRET) -> &'static str {
...
fn record_windows_sandbox_spawn_failure(
    command_path: Option<REDACTED_SECRET>,
    windows_sandbox_level: codex_protocol::config_types::WindowsSandboxLevel,
    err: REDACTED_SECRET,
) {
    ...
    let exe = Path:REDACTED_SECRETpath)
        .file_name()
        ...
        Ok(raw_output) => {
            ...
            if timed_out {
                return Err(CodexErr::Sandbox(SandboxErr::Timeout {
                    output: Box:REDACTED_SECRETexec_output),
                }));
            }
...
pub(crate) fn is_likely_sandbox_denied(
    ...
    const SANDBOX_DENIED_KEYWORDS: [REDACTED_SECRET; 7] = [
...
pub(crate) fn resolve_windows_restricted_token_filesystem_overlay(
    ...
    if !file_system_sandbox_policy
        .get_unreadable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd)
        .is_empty()
    {
        return Err(
    ...
    let legacy_writable_REDACTED_SECRETs = sandbox_policy.get_writable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd);
    let split_writable_REDACTED_SECRETs =
        file_system_sandbox_policy.get_writable_REDACTED_SECRETs_with_cwd(sandbox_policy_cwd);
    let legacy_REDACTED_SECRET_paths: BTreeSet<PathBuf> = legacy_writable_REDACTED_SECRETs
        .iter()
        .map(|REDACTED_SECRET| normalize_windows_overlay_path(REDACTED_SECRET.REDACTED_SECRET.as_path()))
        .collect::<std::result::Result<_, _>>()?;
    let split_REDACTED_SECRET_paths: BTreeSet<PathBuf> = split_writable_REDACTED_SECRETs
        .iter()
        .map(|REDACTED_SECRET| normalize_windows_overlay_path(REDACTED_SECRET.REDACTED_SECRET.as_path()))
        .collect::<std::result::Result<_, _>>()?;

    if legacy_REDACTED_SECRET_paths != split_REDACTED_SECRET_paths {
        return Err(
            "windows unelevated restricted-token sandbox cannot enforce split writable REDACTED_SECRET sets directly; refusing to run unsandboxed"
                .to_string(),
        );
    }

    for writable_REDACTED_SECRET in &split_writable_REDACTED_SECRETs {
        for read_only_subpath in &writable_REDACTED_SECRET.read_only_subpaths {
            if split_writable_REDACTED_SECRETs.iter().any(|candidate| {
                candidate.REDACTED_SECRET.as_path() != writable_REDACTED_SECRET.REDACTED_SECRET.as_path()
                    && candidate
                        .REDACTED_SECRET
                        .as_path()
    ...
    let mut additional_deny_write_paths = BTreeSet:REDACTED_SECRET);
    for split_REDACTED_SECRET in &split_writable_REDACTED_SECRETs {
        let split_REDACTED_SECRET_path = normalize_windows_overlay_path(split_REDACTED_SECRET.REDACTED_SECRET.as_path())?;
        let Some(legacy_REDACTED_SECRET) = legacy_writable_REDACTED_SECRETs.iter().find(|candidate| {
            normalize_windows_overlay_path(candidate.REDACTED_SECRET.as_path())
                .is_ok_and(|candidate_path| candidate_path == split_REDACTED_SECRET_path)
        }) else {
            return Err(
                "windows unelevated restricted-token sandbox cannot enforce split writable REDACTED_SECRET sets directly; refusing to run unsandboxed"
                    .to_string(),
        ...
        for read_only_subpath in &split_REDACTED_SECRET.read_only_subpaths {
            if !legacy_REDACTED_SECRET
                .read_only_subpaths
                .iter()
...
async fn consume_output(
    ...
    let stdout_handle = tokio::spawn(read_output(
        BufReader:REDACTED_SECRETstdout_reader),
        stdout_stream.clone(),
        /*is_stderr*/ false,
        retained_bytes_cap,
    ));
    let stderr_handle = tokio::spawn(read_output(
        BufReader:REDACTED_SECRETstderr_reader),
        stdout_stream.clone(),
        /*is_stderr*/ true,
        retained_bytes_cap,
    ));
...
async fn read_output<R: AsyncRead + Unpin + REDACTED_SECRET + 'static>(
    mut reader: R,
    ...
    loop {
        ...
        if let Some(stream) = REDACTED_SECRETeam
