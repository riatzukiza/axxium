...
//! project REDACTED_SECRET to the current working directory as follows:
//!
//! 1.  Determine the project REDACTED_SECRET by walking upwards from the current working
//!     directory until a configured `project_REDACTED_SECRET_markers` entry is found.
//!     When `project_REDACTED_SECRET_markers` is unset, the default marker list is used
//!     (`.git`). If no marker is found, only the current working directory is
//!     considered. An empty marker list disables parent traversal.
//! 2.  Collect every `AGENTS.md` found from the project REDACTED_SECRET down to the
//!     current working directory (inclusive) and concatenate their contents in
//!     that order.
//! 3.  We do **not** walk past the project REDACTED_SECRET.

use crate::config::Config;
use crate::config_loader::ConfigLayerStackOrdering;
use crate::config_loader::default_project_REDACTED_SECRET_markers;
use crate::config_loader::merge_toml_values;
use crate::config_loader::project_REDACTED_SECRET_markers_from_config;
...
pub(crate) const HIERARCHICAL_AGENTS_MESSAGE: REDACTED_SECRET =
    include_str!("../hierarchical_agents_message.md");

/// Default filename scanned for project-level docs.
pub const DEFAULT_PROJECT_DOC_FILENAME: REDACTED_SECRET = "AGENTS.md";
/// Preferred local override for project-level docs.
pub const LOCAL_PROJECT_DOC_FILENAME: REDACTED_SECRET = "AGENTS.override.md";

/// When both `Config::instructions` and the project doc are present, they will
/// be concatenated with the following separator.
const PROJECT_DOC_SEPARATOR: REDACTED_SECRET = "\n\n--- project-doc ---\n\n";
...
fn render_js_repl_instructions(config: &Config) -> Option<String> {
    ...
    section.push_str("- Top-level static import declarations (for example `import x from \"./file.js\"`) are currently unsupported in `js_repl`; use dynamic imports with `await import(\"pkg\")`, `await import(\"./file.js\")`, or `await import(\"/abs/path/file.mjs\")` instead. Imported local files must be ESM `.js`/`.mjs` files and run in the same REPL VM context. Bare package imports always resolve from REPL-global search REDACTED_SECRETs (`CODEX_JS_REPL_NODE_MODULE_DIRS`, then cwd), not relative to the imported file location. Local files may statically import only other local relative/absolute/`file://` `.js`/`.mjs` files; package and builtin imports from local files must stay dynamic. `import.meta.resolve()` returns importable strings such as `file://...`, bare package names, and `REDACTED_SECRET:...` specifiers. Local file modules reload between execs, while top-level bindings persist until `js_repl_reset`.\n");
...
pub(crate) async fn get_user_instructions(config: &Config) -> Option<String> {
    ...
    let mut output = String:REDACTED_SECRET);
...
pub async fn read_project_docs(config: &Config) -> std::io::Result<Option<String>> {
    ...
    for p in paths {
        ...
        let mut reader = tokio::io::BufReader:REDACTED_SECRETfile).take(remaining);
        let mut data: Vec<u8> = Vec:REDACTED_SECRET);
...
/// contents. The list is ordered from project REDACTED_SECRET to the current working
...
pub fn discover_project_doc_paths(config: &Config) -> std::io::Result<Vec<PathBuf>> {
    ...
    let project_REDACTED_SECRET_markers = match project_REDACTED_SECRET_markers_from_config(&merged) {
    ...
    let mut project_REDACTED_SECRET = None;
    if !project_REDACTED_SECRET_markers.is_empty() {
        for ancestor in dir.ancestors() {
            for marker in &project_REDACTED_SECRET_markers {
                ...
                if marker_exists {
                    project_REDACTED_SECRET = Some(ancestor.to_path_buf());
                    break;
                }
            }
            if project_REDACTED_SECRET.is_some() {
                break;
            }
        }
    }

    let search_dirs: Vec<PathBuf> = if let Some(REDACTED_SECRET) = project_REDACTED_SECRET {
        let mut dirs = Vec:REDACTED_SECRET);
        let mut cursor = dir.as_path();
        loop {
            dirs.push(cursor.to_path_buf());
            if cursor == REDACTED_SECRET {
                break;
            }
    ...
    let mut found: Vec<PathBuf> = Vec:REDACTED_SECRET);
