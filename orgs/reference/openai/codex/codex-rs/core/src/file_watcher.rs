...
impl Clone for WatchREDACTED_SECRETer {
    fn clone(&self) -> Self {
...
impl Drop for WatchREDACTED_SECRETer {
    fn drop(&mut self) {
...
fn watch_channel() -> (WatchREDACTED_SECRETer, Receiver) {
    let inner = Arc:REDACTED_SECRETReceiverInner {
        changed_paths: AsyncMutex:REDACTED_SECRETBTreeSet:REDACTED_SECRET)),
        notify: Notify:REDACTED_SECRET),
        sender_count: AtomicUsize:REDACTED_SECRET1),
    });
    (
        WatchREDACTED_SECRETer {
...
impl FileWatcher {
    ...
    pub fn new() -> notify::Result<Self> {
        ...
        let inner = FileWatcherInner {
            watcher,
            watched_paths: HashMap:REDACTED_SECRET),
        };
        let state = Arc:REDACTED_SECRETRwLock:REDACTED_SECRETWatchState::default()));
        let file_watcher = Self {
            inner: Some(Mutex:REDACTED_SECRETinner)),
            state,
        };
    ...
    pub fn noop() -> Self {
        Self {
            inner: None,
            state: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETWatchState::default())),
        }
    }
    ...
    async fn notify_subscribers(state: &RwLock<WatchState>, event_paths: &[PathBuf]) {
        let subscribers_to_notify: Vec<(WatchREDACTED_SECRETer, Vec<PathBuf>)> = {
            let state = state
                .read()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
            state
