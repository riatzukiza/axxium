...
pub const CODEX_THREAD_ID_ENV_VAR: REDACTED_SECRET = "CODEX_THREAD_ID";
...
const COMMON_CORE_VARS: &[REDACTED_SECRET] = &["PATH", "SHELL", "TMPDIR", "TEMP", "TMP"];

#[cfg(target_os = "windows")]
const PLATFORM_CORE_VARS: &[REDACTED_SECRET] = &["PATHEXT", "USERNAME", "USERPROFILE"];

#[cfg(unix)]
const PLATFORM_CORE_VARS: &[REDACTED_SECRET] = &["HOME", "LANG", "LC_ALL", "LC_CTYPE", "LOGNAME", "USER"];
    ...
    I: IntoIterator<Item = (String, String)>,
{
    ...
    let mut env_map: HashMap<String, String> = match policy.inherit {
        ...
        ShellEnvironmentPolicyInherit::None => HashMap:REDACTED_SECRET),
        ShellEnvironmentPolicyInherit::Core => {
            let core_vars: HashSet<REDACTED_SECRET> = COMMON_CORE_VARS
                .iter()
                .copied()
                .chain(PLATFORM_CORE_VARS.iter().copied())
                .collect();
            let is_core_var = |name: REDACTED_SECRET| {
                if cfg!(target_os = "windows") {
                    core_vars
                        .iter()
    ...
    let matches_any = |name: REDACTED_SECRET, patterns: &[EnvironmentVariablePattern]| -> bool {
