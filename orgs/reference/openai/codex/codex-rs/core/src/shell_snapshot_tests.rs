...
fn bash_snapshot_filters_invalid_exports() -> Result<()> {
    let output = Command:REDACTED_SECRET"/bin/bash")
        .arg("-c")
...
fn bash_snapshot_preserves_multiline_exports() -> Result<()> {
    ...
    let validate = Command:REDACTED_SECRET"/bin/bash")
        .arg("-c")
        .arg("set -e; . \"$1\"")
        .arg("bash")
        .arg(&snapshot_path)
...
async fn try_new_creates_and_deletes_snapshot_file() -> Result<()> {
    ...
    let snapshot = ShellSnapshot::try_new(dir.path(), ThreadId:REDACTED_SECRET), dir.path(), &shell)
        .await
...
async fn try_new_uses_distinct_generation_paths() -> Result<()> {
    let dir = tempdir()?;
    let session_id = ThreadId:REDACTED_SECRET);
    let shell = Shell {
        shell_type: ShellType::Bash,
        shell_path: PathBuf::from("/bin/bash"),
        shell_snapshot: crate::shell::empty_shell_snapshot_receiver(),
...
async fn timed_out_snapshot_shell_is_terminated() -> Result<()> {
    ...
    loop {
        let kill_status = StdCommand:REDACTED_SECRET"kill")
            .arg("-0")
            .arg(pid.to_string())
            .stderr(Stdio::null())
            .stdout(Stdio::null())
...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    ...
    let live_session = ThreadId:REDACTED_SECRET);
    let orphan_session = ThreadId:REDACTED_SECRET);
    ...
    cleanup_stale_snapshots(codex_home, ThreadId:REDACTED_SECRET)).await?;
...
async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
    ...
    let stale_session = ThreadId:REDACTED_SECRET);
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    let active_session = ThreadId:REDACTED_SECRET);
