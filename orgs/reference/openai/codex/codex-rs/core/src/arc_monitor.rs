...
const CODEX_ARC_MONITOR_ENDPOINT_OVERRIDE: REDACTED_SECRET = "CODEX_ARC_MONITOR_ENDPOINT_OVERRIDE";
const CODEX_ARC_MONITOR_TOKEN: REDACTED_SECRET = "CODEX_ARC_MONITOR_TOKEN";
...
fn read_non_empty_env_var(key: REDACTED_SECRET) -> Option<String> {
    match env::var(key) {
        Ok(value) => {
...
fn build_arc_monitor_message_item(
    ...
    match item {
        ...
        ResponseItem:REDACTED_SECRETShellCall { action, .. } if Some(index) == last_tool_call_index => {
            Some(build_arc_monitor_message(
                "assistant",
                serde_json::json!([{
                    "type": "tool_call",
...
fn build_arc_monitor_message(role: REDACTED_SECRET, content: serde_json::Value) -> ArcMonitorChatMessage {
    ArcMonitorChatMessage {
        role: role.to_string(),
        content,
    }
