...
async fn formats_basic_record() {
    let exec_output = ExecToolCallOutput {
        exit_code: 0,
        stdout: StreamOutput:REDACTED_SECRET"hi".to_string()),
        stderr: StreamOutput:REDACTED_SECRETString:REDACTED_SECRET)),
        aggregated_output: StreamOutput:REDACTED_SECRET"hi".to_string()),
...
async fn uses_aggregated_output_over_streams() {
    let exec_output = ExecToolCallOutput {
        ...
        stderr: StreamOutput:REDACTED_SECRET"stderr-only".to_string()),
        aggregated_output: StreamOutput:REDACTED_SECRET"combined output wins".to_string()),
