...
const CONTEXTUAL_DEVELOPER_PREFIXES: &[REDACTED_SECRET] = &[
...
fn parse_user_message(message: &[ContentItem]) -> Option<UserMessageItem> {
    ...
    let mut content: Vec<UserInput> = Vec:REDACTED_SECRET);
    ...
    Some(UserMessageItem:REDACTED_SECRET&content))
}
...
fn parse_agent_message(
    ...
    let mut content: Vec<AgentMessageContent> = Vec:REDACTED_SECRET);
    ...
    match item {
        ...
        ResponseItem::WebSearchCall { id, action, .. } => {
            let (action, query) = match action {
                ...
                None => (WebSearchAction::Other, String:REDACTED_SECRET)),
            };
