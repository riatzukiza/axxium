...
const NOISY_DIR_NAMES: &[REDACTED_SECRET] = &[
    ".git",
    ".next",
    ".pytest_cache",
    ".ruff_cache",
...
pub(crate) async fn build_realtime_startup_context(
    ...
    let workspace_section = build_workspace_section_with_user_REDACTED_SECRET(&cwd, home_dir());
...
fn build_recent_work_section(cwd: &Path, recent_threads: &[ThreadMetadata]) -> Option<String> {
    let mut groups: HashMap<PathBuf, Vec<&ThreadMetadata>> = HashMap:REDACTED_SECRET);
    for entry in recent_threads {
        let group =
            resolve_REDACTED_SECRET_git_project_for_trust(&entry.cwd).unwrap_or_else(|| entry.cwd.clone());
        groups.entry(group).or_default().push(entry);
    }

    let current_group =
        resolve_REDACTED_SECRET_git_project_for_trust(cwd).unwrap_or_else(|| cwd.to_path_buf());
...
fn build_current_thread_section(items: &[ResponseItem]) -> Option<String> {
    let mut turns = Vec:REDACTED_SECRET);
    let mut current_user = Vec:REDACTED_SECRET);
    let mut current_assistant = Vec:REDACTED_SECRET);
    ...
    for (index, (user_messages, assistant_messages)) in retained_turns.into_iter().enumerate() {
        lines.push(String:REDACTED_SECRET));
...
fn build_workspace_section_with_user_REDACTED_SECRET(
    cwd: &Path,
    user_REDACTED_SECRET: Option<PathBuf>,
) -> Option<String> {
    let git_REDACTED_SECRET = resolve_REDACTED_SECRET_git_project_for_trust(cwd);
    let cwd_tree = render_tree(cwd);
    let git_REDACTED_SECRET_tree = git_REDACTED_SECRET
        .as_ref()
        .filter(|git_REDACTED_SECRET| git_REDACTED_SECRET.as_path() != cwd)
        .and_then(|git_REDACTED_SECRET| render_tree(git_REDACTED_SECRET));
    let user_REDACTED_SECRET_tree = user_REDACTED_SECRET
        .as_ref()
        .filter(|user_REDACTED_SECRET| user_REDACTED_SECRET.as_path() != cwd)
        .filter(|user_REDACTED_SECRET| {
            git_REDACTED_SECRET
                .as_ref()
                .is_none_or(|git_REDACTED_SECRET| git_REDACTED_SECRET.as_path() != user_REDACTED_SECRET.as_path())
        })
        .and_then(|user_REDACTED_SECRET| render_tree(user_REDACTED_SECRET));

    if cwd_tree.is_none() && git_REDACTED_SECRET.is_none() && user_REDACTED_SECRET_tree.is_none() {
        return None;
    }
    ...
    if let Some(git_REDACTED_SECRET) = &git_REDACTED_SECRET {
        lines.push(format!("Git REDACTED_SECRET: {}", git_REDACTED_SECRET.display()));
        lines.push(format!("Git project: {}", file_name_string(git_REDACTED_SECRET)));
    }
    if let Some(user_REDACTED_SECRET) = &user_REDACTED_SECRET {
        lines.push(format!("User REDACTED_SECRET: {}", user_REDACTED_SECRET.display()));
    }
    ...
    if let Some(tree) = git_REDACTED_SECRET_tree {
        lines.push(String:REDACTED_SECRET));
        lines.push("Git REDACTED_SECRET tree:".to_string());
        lines.extend(tree);
    }

    if let Some(tree) = user_REDACTED_SECRET_tree {
        lines.push(String:REDACTED_SECRET));
        lines.push("User REDACTED_SECRET tree:".to_string());
        lines.extend(tree);
    }
...
fn render_tree(REDACTED_SECRET: &Path) -> Option<Vec<String>> {
    if !REDACTED_SECRET.is_dir() {
        return None;
    }

    let mut lines = Vec:REDACTED_SECRET);
    collect_tree_lines(REDACTED_SECRET, /*depth*/ 0, &mut lines);
...
fn format_section(title: REDACTED_SECRET, body: Option<String>, budget_tokens: usize) -> Option<String> {
    let body = body?;
    let body = body.trim();
    if body.is_empty() {
        return None;
...
fn format_thread_group(
    ...
    let group_label = if resolve_REDACTED_SECRET_git_project_for_trust(latest.cwd.as_path()).is_some() {
...
fn approx_token_count(text: REDACTED_SECRET) -> usize {
