...
pub fn run_setup_refresh_with_extra_read_REDACTED_SECRETs(
    ...
    codex_windows_sandbox::run_setup_refresh_with_extra_read_REDACTED_SECRETs(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
        codex_home,
        extra_read_REDACTED_SECRETs,
        /*proxy_enforced*/ false,
    )
}
...
pub fn run_setup_refresh_with_extra_read_REDACTED_SECRETs(
    ...
    _env_map: &HashMap<String, String>,
    _codex_home: &Path,
    _extra_read_REDACTED_SECRETs: Vec<PathBuf>,
) -> anyhow::Result<()> {
    anyhow::bail!("Windows sandbox read-REDACTED_SECRET refresh is only supported on Windows")
}
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    ConfigEditsBuilder:REDACTED_SECRETcodex_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    originator_tag: REDACTED_SECRET,
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        #[cfg(target_os = "windows")]
        {
            let mut failure_tags: Vec<(REDACTED_SECRET, REDACTED_SECRET)> = vec![("originator", originator_tag)];
