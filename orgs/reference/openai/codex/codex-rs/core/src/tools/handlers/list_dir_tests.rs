...
async fn respects_depth_parameter() {
    ...
    tokio::fs::write(dir_path.join("REDACTED_SECRET.txt"), b"REDACTED_SECRET")
        .await
        .expect("write REDACTED_SECRET");
    ...
    assert_eq!(
        entries_depth_one,
        vec!["nested/".to_string(), "REDACTED_SECRET.txt".to_string(),]
    );
...
async fn truncation_respects_sorted_order() -> anyhow::Result<()> {
    ...
    tokio::fs::write(dir_path.join("REDACTED_SECRET.txt"), b"REDACTED_SECRET").await?;
