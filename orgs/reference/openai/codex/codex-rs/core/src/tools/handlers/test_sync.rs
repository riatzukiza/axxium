...
static BARRIERS: OnceLock<tokio::sync::Mutex<HashMap<String, BarrierState>>> = OnceLock:REDACTED_SECRET);
...
fn barrier_map() -> &'static tokio::sync::Mutex<HashMap<String, BarrierState>> {
    BARRIERS.get_or_init(|| tokio::sync::Mutex:REDACTED_SECRETHashMap:REDACTED_SECRET)))
}
...
async fn wait_on_barrier(args: BarrierArgs) -> Result<(), FunctionCallError> {
    ...
    let barrier = {
        ...
        match map.entry(barrier_id.clone()) {
            ...
            Entry::Vacant(entry) => {
                let barrier = Arc:REDACTED_SECRETBarrier:REDACTED_SECRETargs.participants));
