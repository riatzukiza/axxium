...
fn shell_payload_command(payload: &ToolPayload) -> Option<String> {
    match payload {
        ...
        ToolPayload:REDACTED_SECRETShell { params } => Some(codex_shell_command::parse_command::shlex_join(
            &params.command,
        )),
        _ => None,
    }
...
impl ShellCommandHandler {
    ...
    fn base_command(shell: &Shell, command: REDACTED_SECRET, use_login_shell: bool) -> Vec<String> {
...
impl ToolHandler for ShellHandler {
    ...
    fn matches_kind(&self, payload: &ToolPayload) -> bool {
        matches!(
            payload,
            ToolPayload::Function { .. } | ToolPayload:REDACTED_SECRETShell { .. }
        )
    }
    ...
    async fn is_mutating(&self, invocation: &ToolInvocation) -> bool {
        match &invocation.payload {
            ...
            ToolPayload:REDACTED_SECRETShell { params } => !is_known_safe_command(&params.command),
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        match payload {
            ...
            ToolPayload:REDACTED_SECRETShell { params } => {
                let exec_params =
                    Self::to_exec_params(&params, turn.as_ref(), session.conversation_id);
                Self::run_exec_like(RunExecLikeArgs {
                    tool_name: tool_name.clone(),
...
impl ShellHandler {
    async fn run_exec_like(args: RunExecLikeArgs) -> Result<FunctionToolOutput, FunctionCallError> {
        ...
        let event_ctx = ToolEventCtx:REDACTED_SECRET
            session.as_ref(),
            turn.as_ref(),
            &call_id,
        ...
        let mut orchestrator = ToolOrchestrator:REDACTED_SECRET);
        let mut runtime = {
            use ShellRuntimeBackend::*;
            match shell_runtime_backend {
                Generic => ShellRuntime:REDACTED_SECRET),
