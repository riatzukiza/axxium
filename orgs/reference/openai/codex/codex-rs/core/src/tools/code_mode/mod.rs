...
pub(crate) const PUBLIC_TOOL_NAME: REDACTED_SECRET = codex_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: REDACTED_SECRET = codex_code_mode::WAIT_TOOL_NAME;
...
impl CodeModeService {
    pub(crate) fn new(_js_repl_REDACTED_SECRET_path: Option<PathBuf>) -> Self {
        Self {
            inner: codex_code_mode::CodeModeService:REDACTED_SECRET),
        }
    }
    ...
    pub(crate) async fn start_turn_worker(
        ...
        let tool_runtime =
            ToolCallRuntime:REDACTED_SECRETrouter, Arc::clone(session), Arc::clone(turn), tracker);
        let host = Arc:REDACTED_SECRETCoreTurnHost { exec, tool_runtime });
