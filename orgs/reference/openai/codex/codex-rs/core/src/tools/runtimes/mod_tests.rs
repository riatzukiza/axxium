...
fn shell_with_snapshot(
    ...
    shell_path: REDACTED_SECRET,
    snapshot_path: PathBuf,
    snapshot_cwd: PathBuf,
) -> Shell {
    let (_tx, shell_snapshot) = watch::channel(Some(Arc:REDACTED_SECRETShellSnapshot {
        path: snapshot_path,
        cwd: snapshot_cwd,
    })));
    Shell {
...
fn maybe_wrap_shell_lc_with_snapshot_bootstraps_in_user_shell() {
    ...
    let rewritten =
        maybe_wrap_shell_lc_with_snapshot(&command, &session_shell, dir.path(), &HashMap:REDACTED_SECRET));
...
fn maybe_wrap_shell_lc_with_snapshot_skips_when_cwd_mismatch() {
    ...
    let rewritten =
        maybe_wrap_shell_lc_with_snapshot(&command, &session_shell, &command_cwd, &HashMap:REDACTED_SECRET));
...
fn maybe_wrap_shell_lc_with_snapshot_restores_explicit_override_precedence() {
    ...
    let output = Command:REDACTED_SECRET&rewritten[0])
        .args(&rewritten[1..])
