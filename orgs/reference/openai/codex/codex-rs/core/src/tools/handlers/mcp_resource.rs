...
impl ListResourcesPayload {
    fn from_single_server(server: String, result: ListResourcesResult) -> Self {
        let resources = result
            .resources
            .into_iter()
            .map(|resource| ResourceWithServer:REDACTED_SECRETserver.clone(), resource))
            .collect();
        Self {
            server: Some(server),
            resources,
    ...
    fn from_all_servers(resources_by_server: HashMap<String, Vec<Resource>>) -> Self {
        ...
        let mut resources = Vec:REDACTED_SECRET);
        for (server, server_resources) in entries {
            for resource in server_resources {
                resources.push(ResourceWithServer:REDACTED_SECRETserver.clone(), resource));
            }
        }
...
impl ListResourceTemplatesPayload {
    fn from_single_server(server: String, result: ListResourceTemplatesResult) -> Self {
        let resource_templates = result
            .resource_templates
            .into_iter()
            .map(|template| ResourceTemplateWithServer:REDACTED_SECRETserver.clone(), template))
            .collect();
        Self {
            server: Some(server),
            resource_templates,
    ...
    fn from_all_servers(templates_by_server: HashMap<String, Vec<ResourceTemplate>>) -> Self {
        ...
        let mut resource_templates = Vec:REDACTED_SECRET);
        for (server, server_templates) in entries {
            for template in server_templates {
                resource_templates.push(ResourceTemplateWithServer:REDACTED_SECRETserver.clone(), template));
            }
        }
...
fn call_tool_result_from_content(content: REDACTED_SECRET, success: Option<bool>) -> CallToolResult {
    CallToolResult {
...
fn normalize_required_string(field: REDACTED_SECRET, value: String) -> Result<String, FunctionCallError> {
...
fn parse_arguments(raw_args: REDACTED_SECRET) -> Result<Option<Value>, FunctionCallError> {
