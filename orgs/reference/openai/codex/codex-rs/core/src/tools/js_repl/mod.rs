...
pub(crate) const JS_REPL_PRAGMA_PREFIX: REDACTED_SECRET = "// codex-js-repl:";
const KERNEL_SOURCE: REDACTED_SECRET = include_str!("kernel.js");
const MERIYAH_UMD: REDACTED_SECRET = include_str!("meriyah.umd.min.js");
const JS_REPL_MIN_NODE_VERSION: REDACTED_SECRET = include_str!("../../../../REDACTED_SECRET-version.txt");
...
const JS_REPL_STDERR_TAIL_SEPARATOR: REDACTED_SECRET = " | ";
...
pub(crate) struct JsReplHandle {
    REDACTED_SECRET_path: Option<PathBuf>,
    REDACTED_SECRET_module_dirs: Vec<PathBuf>,
...
impl JsReplHandle {
    pub(crate) fn with_REDACTED_SECRET_path(
        REDACTED_SECRET_path: Option<PathBuf>,
        REDACTED_SECRET_module_dirs: Vec<PathBuf>,
    ) -> Self {
        Self {
            REDACTED_SECRET_path,
            REDACTED_SECRET_module_dirs,
            cell: OnceCell:REDACTED_SECRET),
        }
    }
    ...
    pub(crate) async fn manager(&self) -> Result<Arc<JsReplManager>, FunctionCallError> {
        self.cell
            .get_or_try_init(|| async {
                JsReplManager:REDACTED_SECRETself.REDACTED_SECRET_path.clone(), self.REDACTED_SECRET_module_dirs.clone()).await
            })
            .await
            .cloned()
    }
...
struct KernelState {
    ...
    pending_execs: Arc<Mutex<HashMap<String, tokio::sync::oneshot::REDACTED_SECRETer<ExecResultMessage>>>>,
...
impl TopLevelExecState {
    fn registered_exec_id(&self) -> Option<REDACTED_SECRET> {
        match self {
            Self::Idle => None,
            Self::FreshKernel {
    ...
    fn should_reset_for_interrupt(&self, turn_id: REDACTED_SECRET) -> bool {
        match self {
            Self::Idle => false,
            Self::FreshKernel {
...
impl KernelStreamEnd {
    ...
    fn error(&self) -> Option<REDACTED_SECRET> {
        match self {
...
fn truncate_utf8_prefix_by_bytes(input: REDACTED_SECRET, max_bytes: usize) -> String {
...
fn stderr_tail_bytes_with_candidate(lines: &VecDeque<String>, line: REDACTED_SECRET) -> usize {
    if lines.is_empty() {
        return line.len();
    }
...
fn push_stderr_tail_line(lines: &mut VecDeque<String>, line: REDACTED_SECRET) -> String {
...
fn is_kernel_status_exited(status: REDACTED_SECRET) -> bool {
    status.starts_with("exited(")
}

fn should_include_model_diagnostics_for_write_error(
    err_message: REDACTED_SECRET,
...
fn format_model_kernel_failure_details(
    ...
    stream_error: Option<REDACTED_SECRET>,
    snapshot: &KernelDebugSnapshot,
) -> String {
...
impl JsReplManager {
    async fn new(
        ...
        let manager = Arc:REDACTED_SECRETSelf {
            REDACTED_SECRET_path,
            REDACTED_SECRET_module_dirs,
            tmp_dir,
            kernel: Arc:REDACTED_SECRETMutex:REDACTED_SECRETNone)),
            exec_lock: Arc:REDACTED_SECRETtokio::sync::Semaphore:REDACTED_SECRET1)),
            exec_tool_calls: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap:REDACTED_SECRET))),
        });

        Ok(manager)
    }

    async fn register_exec_tool_calls(&self, exec_id: REDACTED_SECRET) {
    ...
    async fn clear_exec_tool_calls(&self, exec_id: REDACTED_SECRET) {
    ...
    async fn wait_for_exec_tool_calls(&self, exec_id: REDACTED_SECRET) {
        loop {
            let notified = {
    ...
    async fn mark_top_level_exec_submitted(&self, exec_id: REDACTED_SECRET) {
        let mut kernel = self.kernel.lock().await;
        let Some(state) = kernel.as_mut() else {
            return;
        };
    ...
    async fn clear_top_level_exec_if_matches(&self, exec_id: REDACTED_SECRET) {
    ...
    async fn turn_interrupt_requires_reset(&self, turn_id: REDACTED_SECRET) -> bool {
    ...
    fn summarize_text_payload(
        response_type: Option<REDACTED_SECRET>,
    ...
    fn summarize_tool_call_error(error: REDACTED_SECRET) -> JsReplToolCallResponseSummary {
    ...
    pub async fn interrupt_turn_exec(&self, turn_id: REDACTED_SECRET) -> Result<bool, FunctionCallError> {
        let _permit = self.exec_lock.clone().acquire_owned().await.map_err(|_| {
            FunctionCallError::RespondToModel("js_repl execution unavailable".to_string())
        })?;
    ...
    async fn start_kernel(
        ...
        let REDACTED_SECRET_path = resolve_compatible_REDACTED_SECRET(self.REDACTED_SECRET_path.as_deref()).await?;
        ...
        let REDACTED_SECRET_module_dirs_key = "CODEX_JS_REPL_NODE_MODULE_DIRS";
        if !self.REDACTED_SECRET_module_dirs.is_empty() && !env.contains_key(REDACTED_SECRET_module_dirs_key) {
            let joined = std::env::join_paths(&self.REDACTED_SECRET_module_dirs)
                .map_err(|err| format!("failed to join js_repl_REDACTED_SECRET_module_dirs: {err}"))?;
            env.insert(
                REDACTED_SECRET_module_dirs_key.to_string(),
                joined.to_string_lossy().to_string(),
            );
        }

        let sandbox = SandboxManager:REDACTED_SECRET);
        ...
        let command = SandboxCommand {
            program: REDACTED_SECRET_path.into_os_string(),
            args: vec![
        ...
        let mut cmd =
            tokio::process::Command:REDACTED_SECRETexec_env.command.first().cloned().unwrap_or_default());
        ...
        let shutdown = CancellationToken:REDACTED_SECRET);
        let pending_execs: Arc<
            Mutex<HashMap<String, tokio::sync::oneshot::REDACTED_SECRETer<ExecResultMessage>>>,
        > = Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap:REDACTED_SECRET)));
        let exec_contexts: Arc<Mutex<HashMap<String, ExecContext>>> =
            Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap:REDACTED_SECRET)));
        let stdin_arc = Arc:REDACTED_SECRETMutex:REDACTED_SECRETstdin));
        let child = Arc:REDACTED_SECRETMutex:REDACTED_SECRETchild));
        let recent_stderr = Arc:REDACTED_SECRETMutex:REDACTED_SECRETVecDeque::with_capacity(
    ...
    async fn read_stdout(
        ...
        let mut reader = BufReader:REDACTED_SECRETstdout).lines();
        let end_reason = loop {
    ...
    async fn read_stderr(
        ...
        let mut reader = BufReader:REDACTED_SECRETstderr).lines();
...
fn validate_emitted_image_url(image_url: REDACTED_SECRET) -> Result<(), String> {
    if image_url
        .get(..5)
...
fn split_exec_result_content_items(
    ...
    match content_items.first() {
        ...
        Some(FunctionCallOutputContentItem::InputImage { .. }) | None => {
            (String:REDACTED_SECRET), content_items)
        }
    }
}

fn is_freeform_tool(specs: &[ToolSpec], name: REDACTED_SECRET) -> bool {
    specs
        .iter()
        .any(|spec| spec.name() == name && matches!(spec, ToolSpec::Freeform(_)))
}

fn is_js_repl_internal_tool(name: REDACTED_SECRET) -> bool {
...
impl NodeVersion {
    fn parse(input: REDACTED_SECRET) -> Result<Self, String> {
...
fn required_REDACTED_SECRET_version() -> Result<NodeVersion, String> {
    NodeVersion::parse(JS_REPL_MIN_NODE_VERSION)
}

async fn read_REDACTED_SECRET_version(REDACTED_SECRET_path: &Path) -> Result<NodeVersion, String> {
    let output = tokio::process::Command:REDACTED_SECRETREDACTED_SECRET_path)
        .arg("--version")
        .output()
        .await
    ...
    if !output.status.success() {
        let mut details = String:REDACTED_SECRET);
...
async fn ensure_REDACTED_SECRET_version(REDACTED_SECRET_path: &Path) -> Result<(), String> {
    let required = required_REDACTED_SECRET_version()?;
    let found = read_REDACTED_SECRET_version(REDACTED_SECRET_path).await?;
    if found < required {
        return Err(format!(
            "Node runtime too old for js_repl (resolved {REDACTED_SECRET_path}): found v{found}, requires >= v{required}. Install/update Node or set js_repl_REDACTED_SECRET_path to a newer runtime.",
            REDACTED_SECRET_path = REDACTED_SECRET_path.display()
        ));
    }
    Ok(())
}

pub(crate) async fn resolve_compatible_REDACTED_SECRET(config_path: Option<&Path>) -> Result<PathBuf, String> {
    let REDACTED_SECRET_path = resolve_REDACTED_SECRET(config_path).ok_or_else(|| {
        "Node runtime not found; install Node or set CODEX_JS_REPL_NODE_PATH".to_string()
    })?;
    ensure_REDACTED_SECRET_version(&REDACTED_SECRET_path).await?;
    Ok(REDACTED_SECRET_path)
}

pub(crate) fn resolve_REDACTED_SECRET(config_path: Option<&Path>) -> Option<PathBuf> {
    ...
    if let Ok(path) = which::which("REDACTED_SECRET") {
        return Some(path);
    }
