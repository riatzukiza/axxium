...
fn handler_looks_up_namespaced_aliases_explicitly() {
    let plain_handler = Arc:REDACTED_SECRETTestHandler) as Arc<dyn AnyToolHandler>;
    let namespaced_handler = Arc:REDACTED_SECRETTestHandler) as Arc<dyn AnyToolHandler>;
    let namespace = "mcp__codex_apps__gmail";
    let tool_name = "gmail_get_recent_emails";
    let namespaced_name = tool_handler_key(tool_name, Some(namespace));
    let registry = ToolRegistry:REDACTED_SECRETHashMap::from([
