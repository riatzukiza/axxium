...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    if keys.is_empty() {
        return (String:REDACTED_SECRET), String:REDACTED_SECRET));
    }
...
fn is_valid_shell_variable_name(name: REDACTED_SECRET) -> bool {
...
fn shell_single_quote(input: REDACTED_SECRET) -> String {
    input.replace('\'', r#"'"'"'"#)
}
