...
fn resource(uri: REDACTED_SECRET, name: REDACTED_SECRET) -> Resource {
...
fn template(uri_template: REDACTED_SECRET, name: REDACTED_SECRET) -> ResourceTemplate {
...
fn resource_with_server_serializes_server_field() {
    let entry = ResourceWithServer:REDACTED_SECRET"test".to_string(), resource("memo://id", "memo"));
...
fn list_resources_payload_from_all_servers_is_sorted() {
    let mut map = HashMap:REDACTED_SECRET);
...
fn template_with_server_serializes_server_field() {
    let entry = ResourceTemplateWithServer:REDACTED_SECRET"srv".to_string(), template("memo://{id}", "memo"));
