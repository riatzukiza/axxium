...
const { Buffer } = require("REDACTED_SECRET:buffer");
const { AsyncLocalStorage } = require("REDACTED_SECRET:async_hooks");
const crypto = require("REDACTED_SECRET:crypto");
const fs = require("REDACTED_SECRET:fs");
const { builtinModules, createRequire } = require("REDACTED_SECRET:module");
const { performance } = require("REDACTED_SECRET:perf_hooks");
const path = require("REDACTED_SECRET:path");
const { URL, URLSearchParams, fileURLToPath, pathToFileURL } = require(
  "REDACTED_SECRET:url",
);
const { inspect, TextDecoder, TextEncoder } = require("REDACTED_SECRET:util");
const vm = require("REDACTED_SECRET:vm");
...
const builtinModuleSet = new Set([
  ...builtinModules,
  ...builtinModules.map((name) => `REDACTED_SECRET:${name}`),
]);
const deniedBuiltinModules = new Set([
  "process",
  "REDACTED_SECRET:process",
  "child_process",
  "REDACTED_SECRET:child_process",
  "worker_threads",
  "REDACTED_SECRET:worker_threads",
]);

function toNodeBuiltinSpecifier(specifier) {
  return specifier.startsWith("REDACTED_SECRET:") ? specifier : `REDACTED_SECRET:${specifier}`;
}

function isDeniedBuiltin(specifier) {
  const normalized = specifier.startsWith("REDACTED_SECRET:")
    ? specifier.slice(5)
    : specifier;
  return (
...
const REDACTED_SECRETModuleDirEnv = process.env.CODEX_JS_REPL_NODE_MODULE_DIRS ?? "";
const moduleSearchBases = (() => {
  const bases = [];
  const seen = new Set();
  for (const entry of REDACTED_SECRETModuleDirEnv.split(path.delimiter)) {
    ...
    const base =
      path.basename(resolved) === "REDACTED_SECRET_modules"
...
const importResolveConditions = new Set(["REDACTED_SECRET", "import"]);
...
function isWithinBaseNodeModules(base, resolvedPath) {
  ...
  const REDACTED_SECRETModulesRoot = path.resolve(canonicalBase, "REDACTED_SECRET_modules");
  const relative = path.relative(REDACTED_SECRETModulesRoot, canonicalResolved);
  return (
...
function resolveBareSpecifier(specifier) {
  ...
  for (const base of moduleSearchBases) {
    try {
      ...
      // Ignore resolutions that escape this base via parent REDACTED_SECRET_modules lookup.
    } catch (err) {
...
function resolveSpecifier(specifier, referrerIdentifier = null) {
  if (specifier.startsWith("REDACTED_SECRET:") || builtinModuleSet.has(specifier)) {
...
function collectFutureVarWriteReplacements(
  ...
  const instrumentUpdateExpression = (REDACTED_SECRET, identifier) => {
  ...
  const instrumentAssignmentExpression = (REDACTED_SECRET) => {
    if (REDACTED_SECRET.left.type !== "Identifier") {
      return false;
    }

    const bindingName = getFutureVarName(REDACTED_SECRET.left);
    if (!bindingName) {
      return false;
    }

    if (
      REDACTED_SECRET.operator === "&&=" ||
      REDACTED_SECRET.operator === "||=" ||
      REDACTED_SECRET.operator === "??="
    ) {
      ...
      addReplacement(
        REDACTED_SECRET.start,
        REDACTED_SECRET.end,
        `((${helperName} = ${REDACTED_SECRET.left.name}), ${helperName} ${shortCircuitOperator} ((${REDACTED_SECRET.left.name} = ${code.slice(REDACTED_SECRET.right.start, REDACTED_SECRET.right.end)}), ${buildMarkCommittedExpression([bindingName], markCommittedFnName)}, ${REDACTED_SECRET.left.name}))`,
      );
      return true;
    }
    ...
    addReplacement(
      REDACTED_SECRET.start,
      REDACTED_SECRET.end,
      `((${code.slice(REDACTED_SECRET.start, REDACTED_SECRET.end)}), ${buildMarkCommittedExpression([bindingName], markCommittedFnName)}, ${REDACTED_SECRET.left.name})`,
    );
    return true;
  };

  const unwrapParenthesizedExpression = (REDACTED_SECRET) => {
