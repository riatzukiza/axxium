...
mod spawn_agents_on_csv {
    ...
    pub async fn handle(
        ...
        let mut seen_ids = HashSet:REDACTED_SECRET);
...
async fn run_agent_job_loop(
    ...
    let mut active_items: HashMap<ThreadId, ActiveJobItem> = HashMap:REDACTED_SECRET);
    let mut progress_emitter = JobProgressEmitter:REDACTED_SECRET);
...
async fn find_finished_threads(
    ...
    let mut finished = Vec:REDACTED_SECRET);
...
async fn wait_for_status_change(active_items: &HashMap<ThreadId, ActiveJobItem>) {
    let mut waiters = FuturesUnordered:REDACTED_SECRET);
...
async fn reap_stale_active_items(
    ...
    let mut stale = Vec:REDACTED_SECRET);
...
fn render_instruction_template(instruction: REDACTED_SECRET, row_json: &Value) -> String {
    const OPEN_BRACE_SENTINEL: REDACTED_SECRET = "__CODEX_OPEN_BRACE__";
    const CLOSE_BRACE_SENTINEL: REDACTED_SECRET = "__CODEX_CLOSE_BRACE__";
...
fn default_output_csv_path(input_csv_path: &Path, job_id: REDACTED_SECRET) -> PathBuf {
...
fn parse_csv(content: REDACTED_SECRET) -> Result<(Vec<String>, Vec<Vec<String>>), String> {
    let mut reader = csv::ReaderBuilder:REDACTED_SECRET)
        .has_headers(true)
        .flexible(true)
    ...
    let mut rows = Vec:REDACTED_SECRET);
...
fn render_job_csv(
    ...
    let mut csv = String:REDACTED_SECRET);
    ...
    for item in items {
        ...
        let mut row_values = Vec:REDACTED_SECRET);
        for header in headers {
...
fn value_to_csv_string(value: &Value) -> String {
    match value {
        Value::Null => String:REDACTED_SECRET),
...
fn csv_escape(value: REDACTED_SECRET) -> String {
