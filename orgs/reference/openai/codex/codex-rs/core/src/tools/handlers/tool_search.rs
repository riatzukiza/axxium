...
impl ToolHandler for ToolSearchHandler {
    ...
    async fn handle(
        ...
        if entries.is_empty() {
            return Ok(ToolSearchOutput { tools: Vec:REDACTED_SECRET) });
        }

        let documents: Vec<Document<usize>> = entries
            .iter()
            .enumerate()
            .map(|(idx, (name, info))| Document:REDACTED_SECRETidx, build_search_text(name, info)))
            .collect();
        let search_engine =
...
fn build_search_text(name: REDACTED_SECRET, info: &ToolInfo) -> String {
    let mut parts = vec![
        name.to_string(),
