...
#[test]
fn REDACTED_SECRET_version_parses_v_prefix_and_suffix() {
...
fn stderr_tail_applies_line_and_byte_limits() {
    let mut lines = VecDeque:REDACTED_SECRET);
...
async fn wait_for_exec_tool_calls_map_drains_inflight_calls_without_hanging() {
    let exec_tool_calls = Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap:REDACTED_SECRET)));
...
async fn reset_waits_for_exec_lock_before_clearing_exec_tool_calls() {
    let manager = JsReplManager:REDACTED_SECRET/*REDACTED_SECRET_path*/ None, Vec:REDACTED_SECRET))
        .await
...
fn write_js_repl_test_package_source(base: &Path, name: REDACTED_SECRET, source: REDACTED_SECRET) -> anyhow::Result<()> {
    let pkg_dir = base.join("REDACTED_SECRET_modules").join(name);
...
fn write_js_repl_test_package(base: &Path, name: REDACTED_SECRET, value: REDACTED_SECRET) -> anyhow::Result<()> {
    write_js_repl_test_package_source(base, name, &format!("export const value = \"{value}\";\n"))?;
    Ok(())
}

fn write_js_repl_test_module(base: &Path, relative: REDACTED_SECRET, contents: REDACTED_SECRET) -> anyhow::Result<()> {
...
async fn js_repl_timeout_does_not_deadlock() -> anyhow::Result<()> {
    ...
    let tracker = Arc:REDACTED_SECRETtokio::sync::Mutex:REDACTED_SECRETTurnDiffTracker::default()));
...
async fn interrupt_turn_exec_clears_matching_submitted_exec() -> anyhow::Result<()> {
    ...
    let (_session, turn) = make_session_and_context().await;
    let turn = Arc:REDACTED_SECRETturn);
    let dependency_env = HashMap:REDACTED_SECRET);
...
#[tokio::test]
async fn js_repl_prefers_env_REDACTED_SECRET_module_dirs_over_config() -> anyhow::Result<()> {
    if !can_run_js_repl_runtime_tests().await {
        return Ok(());
    }
    ...
    turn.js_repl = Arc:REDACTED_SECRETJsReplHandle::with_REDACTED_SECRET_path(
        turn.config.js_repl_REDACTED_SECRET_path.clone(),
...
#[tokio::test]
async fn js_repl_falls_back_to_cwd_REDACTED_SECRET_modules() -> anyhow::Result<()> {
    if !can_run_js_repl_runtime_tests().await {
        return Ok(());
    }
...
#[tokio::test]
async fn js_repl_accepts_REDACTED_SECRET_modules_dir_entries() -> anyhow::Result<()> {
    if !can_run_js_repl_runtime_tests().await {
        return Ok(());
    }
    ...
    turn.js_repl = Arc:REDACTED_SECRETJsReplHandle::with_REDACTED_SECRET_path(
        turn.config.js_repl_REDACTED_SECRET_path.clone(),
        vec![base_dir.path().join("REDACTED_SECRET_modules")],
    ));
...
#[tokio::test]
async fn js_repl_local_files_expose_REDACTED_SECRET_like_import_meta() -> anyhow::Result<()> {
    if !can_run_js_repl_runtime_tests().await {
        return Ok(());
    }

    let cwd_dir = tempdir()?;
    let pkg_dir = cwd_dir.path().join("REDACTED_SECRET_modules").join("repl_meta_pkg");
    fs::create_dir_all(&pkg_dir)?;
    fs::write(
        pkg_dir.join("package.json"),
        "{\n  \"name\": \"repl_meta_pkg\",\n  \"version\": \"1.0.0\",\n  \"type\": \"module\",\n  \"exports\": {\n    \"import\": \"./index.js\"\n  }\n}\n",
    ...
    fs::write(
        ...
        "import { sep } from \"REDACTED_SECRET:path\";\nexport const value = `pkg:${typeof sep}`;\n",
    )?;
    write_js_repl_test_module(
        cwd_dir.path(),
        "child.js",
    ...
    assert!(result.output.contains("REDACTED_SECRET:fs"));
...
async fn js_repl_blocks_sensitive_builtin_imports_from_local_files() -> anyhow::Result<()> {
    ...
    assert!(
        err.to_string()
            .contains("Importing module \"REDACTED_SECRET:process\" is not allowed in js_repl")
    );
    Ok(())
}

#[tokio::test]
async fn js_repl_local_files_do_not_escape_REDACTED_SECRET_module_search_REDACTED_SECRETs() -> anyhow::Result<()> {
    if !can_run_js_repl_runtime_tests().await {
        return Ok(());
    }
    ...
    let err = manager
        ...
        .await
        .expect_err("expected parent REDACTED_SECRET_modules lookup to be rejected");
