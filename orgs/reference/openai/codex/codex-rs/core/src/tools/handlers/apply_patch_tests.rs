...
fn approval_keys_include_move_destination() {
    let tmp = TempDir:REDACTED_SECRET).expect("tmp");
    let cwd = tmp.path();
...
#[test]
fn write_permissions_for_paths_skip_dirs_already_writable_under_workspace_REDACTED_SECRET() {
    let tmp = TempDir:REDACTED_SECRET).expect("tmp");
    let cwd = tmp.path();
...
#[test]
fn write_permissions_for_paths_keep_dirs_outside_workspace_REDACTED_SECRET() {
    let tmp = TempDir:REDACTED_SECRET).expect("tmp");
