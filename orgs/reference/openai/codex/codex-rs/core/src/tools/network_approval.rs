...
impl DeferredNetworkApproval {
    pub(crate) fn registration_id(&self) -> REDACTED_SECRET {
...
impl PendingHostApproval {
    fn new() -> Self {
        Self {
            decision: Mutex:REDACTED_SECRETNone),
            notify: Notify:REDACTED_SECRET),
        }
    }
...
impl Default for NetworkApprovalService {
    fn default() -> Self {
        Self {
            active_calls: Mutex:REDACTED_SECRETIndexMap:REDACTED_SECRET)),
            call_outcomes: Mutex:REDACTED_SECRETHashMap:REDACTED_SECRET)),
            pending_host_approvals: Mutex:REDACTED_SECRETHashMap:REDACTED_SECRET)),
            session_approved_hosts: Mutex:REDACTED_SECRETHashSet:REDACTED_SECRET)),
            session_denied_hosts: Mutex:REDACTED_SECRETHashSet:REDACTED_SECRET)),
        }
    }
}
...
impl NetworkApprovalService {
    ...
    async fn register_call(&self, registration_id: String, turn_id: String) {
        ...
        active_calls.insert(
            key,
            Arc:REDACTED_SECRETActiveNetworkApprovalCall {
                registration_id,
                turn_id,
            }),
        );
    ...
    pub(crate) async fn unregister_call(&self, registration_id: REDACTED_SECRET) {
        let mut active_calls = self.active_calls.lock().await;
    ...
    async fn get_or_create_pending_approval(
        ...
        let created = Arc:REDACTED_SECRETPendingHostApproval:REDACTED_SECRET));
    ...
    async fn take_call_outcome(&self, registration_id: REDACTED_SECRET) -> Option<NetworkApprovalOutcome> {
        let mut call_outcomes = self.call_outcomes.lock().await;
        call_outcomes.remove(registration_id)
    }

    async fn record_call_outcome(&self, registration_id: REDACTED_SECRET, outcome: NetworkApprovalOutcome) {
        let mut call_outcomes = self.call_outcomes.lock().await;
        if matches!(
    ...
    fn format_network_target(protocol: REDACTED_SECRET, host: REDACTED_SECRET, port: u16) -> String {
    ...
    pub(crate) async fn handle_inline_policy_request(
        ...
        const REASON_NOT_ALLOWED: REDACTED_SECRET = "not_allowed";
...
pub(crate) fn build_blocked_request_observer(
    ...
    Arc:REDACTED_SECRETmove |blocked: BlockedRequest| {
...
pub(crate) fn build_network_policy_decider(
    ...
    Arc:REDACTED_SECRETmove |request: NetworkPolicyRequest| {
        let network_approval = Arc::clone(&network_approval);
