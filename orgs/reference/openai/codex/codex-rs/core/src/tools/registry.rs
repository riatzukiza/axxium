...
pub trait ToolHandler: REDACTED_SECRET + Sync {
    ...
    ) -> impl std::future::Future<Output = bool> + REDACTED_SECRET {
        async { false }
    }
    ...
    ) -> impl std::future::Future<Output = Result<Self::Output, FunctionCallError>> + REDACTED_SECRET;
}
...
trait AnyToolHandler: REDACTED_SECRET + Sync {
    fn matches_kind(&self, payload: &ToolPayload) -> bool;
    ...
    fn handle_any<'a>(
        ...
        Box::pin(async move {
            ...
            Ok(AnyToolResult {
                call_id,
                payload,
                result: Box:REDACTED_SECREToutput),
            })
        })
    }
}

pub(crate) fn tool_handler_key(tool_name: REDACTED_SECRET, namespace: Option<REDACTED_SECRET>) -> String {
...
impl ToolRegistry {
    ...
    fn handler(&self, name: REDACTED_SECRET, namespace: Option<REDACTED_SECRET>) -> Option<Arc<dyn AnyToolHandler>> {
        self.handlers
    ...
    #[cfg(test)]
    pub(crate) fn has_handler(&self, name: REDACTED_SECRET, namespace: Option<REDACTED_SECRET>) -> bool {
    ...
    pub(crate) async fn dispatch_any(
        ...
        let response_cell = tokio::sync::Mutex:REDACTED_SECRETNone);
        ...
        if let Some(outcome) = &post_tool_use_outcome {
            ...
            if let Some(replacement_text) = replacement_text {
                ...
                if let Some(result) = guard.as_mut() {
                    result.result = Box:REDACTED_SECRETFunctionToolOutput::from_text(
                        replacement_text,
                        /*success*/ None,
                    ));
                }
...
impl ToolRegistryBuilder {
    pub fn new() -> Self {
        Self {
            handlers: HashMap:REDACTED_SECRET),
            specs: Vec:REDACTED_SECRET),
        }
    }
    ...
    pub fn push_spec_with_parallel_support(
        ...
        self.specs
            .push(ConfiguredToolSpec:REDACTED_SECRETspec, supports_parallel_tool_calls));
    }
    ...
    pub fn build(self) -> (Vec<ConfiguredToolSpec>, ToolRegistry) {
        let registry = ToolRegistry:REDACTED_SECRETself.handlers);
        (self.specs, registry)
    }
}
...
fn unsupported_tool_call_message(
    ...
    namespace: Option<REDACTED_SECRET>,
) -> String {
...
impl From<&ToolPayload> for HookToolInput {
    fn from(payload: &ToolPayload) -> Self {
        match payload {
            ...
            ToolPayload:REDACTED_SECRETShell { params } => HookToolInput:REDACTED_SECRETShell {
...
fn hook_tool_kind(tool_input: &HookToolInput) -> HookToolKind {
    match tool_input {
        ...
        HookToolInput:REDACTED_SECRETShell { .. } => HookToolKind:REDACTED_SECRETShell,
