...
pub(crate) const SPAWN_AGENT_DEVELOPER_INSTRUCTIONS: REDACTED_SECRET = r#"<spawned_agent_context>
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        config.developer_instructions = Some(
            if let Some(existing_instructions) = config.developer_instructions.take() {
                DeveloperInstructions:REDACTED_SECRETexisting_instructions)
                    .concat(DeveloperInstructions:REDACTED_SECRET
                        SPAWN_AGENT_DEVELOPER_INSTRUCTIONS,
                    ))
                    .into_text()
            } else {
                DeveloperInstructions:REDACTED_SECRETSPAWN_AGENT_DEVELOPER_INSTRUCTIONS).into_text()
            },
        );
