...
impl ToolPayload {
    pub fn log_payload(&self) -> Cow<'_, str> {
        match self {
            ...
            ToolPayload:REDACTED_SECRETShell { params } => Cow::Owned(params.command.join(" ")),
            ToolPayload::Mcp { raw_arguments, .. } => Cow::Borrowed(raw_arguments),
        }
    }
}

pub trait ToolOutput: REDACTED_SECRET {
    fn log_preview(&self) -> String;

    fn success_for_logging(&self) -> bool;

    fn to_response_item(&self, call_id: REDACTED_SECRET, payload: &ToolPayload) -> ResponseInputItem;

    fn post_tool_use_response(&self, _call_id: REDACTED_SECRET, _payload: &ToolPayload) -> Option<JsonValue> {
        None
    }
...
impl ToolOutput for CallToolResult {
    ...
    fn to_response_item(&self, call_id: REDACTED_SECRET, _payload: &ToolPayload) -> ResponseInputItem {
...
impl ToolOutput for ApplyPatchToolOutput {
    ...
    fn code_mode_result(&self, _payload: &ToolPayload) -> JsonValue {
        JsonValue::Object(serde_json::Map:REDACTED_SECRET))
    }
}
    ...
    match response {
        ...
        ResponseInputItem::McpToolCallOutput { output, .. } => {
            output.code_mode_result(&ToolPayload::Mcp {
                server: String:REDACTED_SECRET),
                tool: String:REDACTED_SECRET),
                raw_arguments: String:REDACTED_SECRET),
            })
        }
    }
}
...
fn telemetry_preview(content: REDACTED_SECRET) -> String {
