...
pub(super) async fn try_run_zsh_fork(
    ...
    let exec_policy = Arc:REDACTED_SECRETRwLock:REDACTED_SECRET
    ...
    // escalation server.
    let stopwatch = Stopwatch:REDACTED_SECRETeffective_timeout);
    ...
    let escalate_server = EscalateServer:REDACTED_SECRET
    ...
    let exec_result = escalate_server
        .exec(exec_params, cancel_token, Arc:REDACTED_SECRETcommand_executor))
        .await
...
pub(crate) async fn prepare_unified_exec_zsh_fork(
    ...
    let escalation_session = escalate_server
        .start_session(CancellationToken:REDACTED_SECRET), Arc:REDACTED_SECRETcommand_executor))
        .map_err(|err| ToolError::Rejected(err.to_string()))?;
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let sandbox_manager = SandboxManager:REDACTED_SECRET);
...
fn map_exec_result(
    ...
    let output = ExecToolCallOutput {
        exit_code: result.exit_code,
        stdout: StreamOutput:REDACTED_SECRETresult.stdout.clone()),
        stderr: StreamOutput:REDACTED_SECRETresult.stderr.clone()),
        aggregated_output: StreamOutput:REDACTED_SECRETresult.output.clone()),
        duration: result.duration,
        timed_out: result.timed_out,
    };
    ...
    if result.timed_out {
        return Err(ToolError::Codex(CodexErr::Sandbox(SandboxErr::Timeout {
            output: Box:REDACTED_SECREToutput),
        })));
    }
