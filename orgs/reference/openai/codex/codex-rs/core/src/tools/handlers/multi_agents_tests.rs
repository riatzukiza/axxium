...
fn invocation(
    ...
    ToolInvocation {
        session,
        turn,
        tracker: Arc:REDACTED_SECRETMutex:REDACTED_SECRETTurnDiffTracker::default())),
...
fn parse_agent_id(id: REDACTED_SECRET) -> ThreadId {
...
async fn handler_rejects_non_function_payloads() {
    ...
    let invocation = invocation(
        Arc:REDACTED_SECRETsession),
        Arc:REDACTED_SECRETturn),
        "spawn_agent",
        ToolPayload::Custom {
...
async fn spawn_agent_uses_explorer_role_and_preserves_approval_policy() {
    ...
    turn.config = Arc:REDACTED_SECRETconfig);
...
async fn multi_agent_v2_spawn_requires_task_name() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let REDACTED_SECRET = manager
        .start_thread((*turn.config).clone())
        .await
        .expect("REDACTED_SECRET thread should start");
    session.services.agent_control = manager.agent_control();
    session.conversation_id = REDACTED_SECRET.thread_id;
    let mut config = (*turn.config).clone();
    config
        .features
...
async fn multi_agent_v2_spawn_returns_path_and_send_message_accepts_relative_path() {
    ...
    let turn = Arc:REDACTED_SECRETturn);
    ...
    assert_eq!(spawn_result.task_name, "/REDACTED_SECRET/test_process");
    ...
    assert_eq!(
        ...
        Some("/REDACTED_SECRET/test_process")
    );
        ...
        *id == child_thread_id
            && matches!(
                ...
                Op::InterAgentCommunication { communication }
                    if communication.author == AgentPath::REDACTED_SECRET()
                        && communication.recipient.as_str() == "/REDACTED_SECRET/test_process"
    ...
    REDACTED_SECRETMessageHandlerV2
        .handle(invocation(
            session.clone(),
            turn.clone(),
            "send_message",
...
#[tokio::test]
async fn multi_agent_v2_send_message_accepts_REDACTED_SECRET_target_from_child() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let REDACTED_SECRET = manager
        .start_thread((*turn.config).clone())
    ...
    let child_path = AgentPath::try_from("/REDACTED_SECRET/worker").expect("agent path");
    let child_thread_id = session
        .services
        .agent_control
        .spawn_agent_with_metadata(
            ...
            Some(SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                parent_thread_id: REDACTED_SECRET.thread_id,
                depth: 1,
    ...
    REDACTED_SECRETMessageHandlerV2
        ...
        .await
        .expect("send_message should accept the REDACTED_SECRET agent path");

    assert!(manager.captured_ops().iter().any(|(id, op)| {
        *id == REDACTED_SECRET.thread_id
            && matches!(
                op,
                Op::InterAgentCommunication { communication }
                    if communication.author == child_path
                        && communication.recipient == AgentPath::REDACTED_SECRET()
                        && communication.other_recipients.is_empty()
...
#[tokio::test]
async fn multi_agent_v2_followup_task_rejects_REDACTED_SECRET_target_from_child() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let REDACTED_SECRET = manager
        .start_thread((*turn.config).clone())
    ...
    let err = FollowupTaskHandlerV2
        ...
        .await
        .expect_err("followup_task should reject the REDACTED_SECRET target");

    assert_eq!(
        err,
        FunctionCallError::RespondToModel("Tasks can't be assigned to the REDACTED_SECRET agent".to_string())
    );
    let REDACTED_SECRET_ops = manager
        .captured_ops()
        .into_iter()
        .filter_map(|(id, op)| (id == REDACTED_SECRET.thread_id).then_some(op))
        .collect::<Vec<_>>();
    assert!(!REDACTED_SECRET_ops.iter().any(|op| matches!(op, Op::Interrupt)));
    assert!(
        !REDACTED_SECRET_ops
            .iter()
...
async fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message() {
    ...
    let agent_names = result
        ...
        .collect::<Vec<_>>();
    assert_eq!(agent_names, vec!["/REDACTED_SECRET", "/REDACTED_SECRET/worker"]);
    let REDACTED_SECRET_agent = result
        .agents
        .iter()
        .find(|agent| agent.agent_name == "/REDACTED_SECRET")
        .expect("REDACTED_SECRET agent should be listed");
    assert_eq!(REDACTED_SECRET_agent.last_task_message.as_deref(), Some("Main thread"));
    let worker = result
        .agents
        .iter()
        .find(|agent| agent.agent_name == "/REDACTED_SECRET/worker")
...
async fn multi_agent_v2_list_agents_filters_by_relative_path_prefix() {
    ...
    let mut config = (*turn.config).clone();
    let _ = config.features.enable(Feature::MultiAgentV2);
    turn.config = Arc:REDACTED_SECRETconfig.clone());

    let researcher_path = AgentPath::from_string("/REDACTED_SECRET/researcher".to_string()).expect("path");
    let worker_path = AgentPath::from_string("/REDACTED_SECRET/researcher/worker".to_string()).expect("path");
    session
        .services
        .agent_control
        .spawn_agent_with_metadata(
...
async fn multi_agent_v2_list_agents_omits_closed_agents() {
    ...
    assert_eq!(result.agents.len(), 1);
    assert_eq!(result.agents[0].agent_name, "/REDACTED_SECRET");
    assert_eq!(
...
async fn multi_agent_v2_send_message_rejects_legacy_items_field() {
    ...
    let Err(err) = REDACTED_SECRETMessageHandlerV2.handle(invocation).await else {
...
async fn multi_agent_v2_send_message_rejects_interrupt_parameter() {
    ...
    assert!(!ops_for_agent.iter().any(|op| matches!(
        ...
        Op::InterAgentCommunication { communication }
            if communication.author == AgentPath::REDACTED_SECRET()
                && communication.recipient.as_str() == "/REDACTED_SECRET/worker"
                && communication.other_recipients.is_empty()
                && communication.content == "continue"
                && !communication.trigger_turn
    )));
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            let saw_envelope = history_contains_inter_agent_communication(
                &history_items,
                &InterAgentCommunication:REDACTED_SECRET
                    AgentPath::REDACTED_SECRET(),
                    AgentPath::try_from("/REDACTED_SECRET/worker").expect("agent path"),
                    Vec:REDACTED_SECRET),
                    "continue".to_string(),
                    /*trigger_turn*/ true,
                ),
...
async fn multi_agent_v2_followup_task_completion_notifies_parent_on_every_turn() {
    ...
    let thread = manager
        ...
        .await
        .expect("worker thread should exist");
    let worker_path = AgentPath::try_from("/REDACTED_SECRET/worker").expect("worker path");
...
async fn multi_agent_v2_interrupted_turn_does_not_notify_parent() {
    ...
    let notifications = manager
        ...
        .filter_map(|(id, op)| {
            (id == REDACTED_SECRET.thread_id)
                ...
                .and_then(|op| match op {
                    Op::InterAgentCommunication { communication }
                        if communication.author.as_str() == "/REDACTED_SECRET/worker"
                            && communication.recipient == AgentPath::REDACTED_SECRET()
                            && communication.other_recipients.is_empty()
                            && !communication.trigger_turn =>
                    {
    ...
    assert_eq!(notifications, Vec::<String>:REDACTED_SECRET));
}
...
async fn multi_agent_v2_spawn_includes_agent_id_key_when_named() {
    ...
    assert_eq!(result["task_name"], "/REDACTED_SECRET/test_process");
    assert!(result.get("nickname").is_some());
    assert_eq!(success, Some(true));
}
...
async fn send_input_rejects_empty_message() {
    ...
    let invocation = invocation(
        ...
        Arc:REDACTED_SECRETturn),
        "send_input",
        function_payload(json!({"target": ThreadId:REDACTED_SECRET).to_string(), "message": ""})),
    );
    let Err(err) = REDACTED_SECRETInputHandler.handle(invocation).await else {
        panic!("empty message should be rejected");
    };
    assert_eq!(
        err,
...
async fn send_input_rejects_when_message_and_items_are_both_set() {
    ...
    let invocation = invocation(
        ...
        "send_input",
        function_payload(json!({
            "target": ThreadId:REDACTED_SECRET).to_string(),
            "message": "hello",
            "items": [{"type": "mention", "name": "drive", "path": "app://drive"}]
        })),
    );
...
async fn send_input_reports_missing_agent() {
    ...
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let agent_id = ThreadId:REDACTED_SECRET);
    let invocation = invocation(
        Arc:REDACTED_SECRETsession),
        Arc:REDACTED_SECRETturn),
        "send_input",
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let output = REDACTED_SECRETInputHandler
...
async fn resume_agent_rejects_when_depth_limit_exceeded() {
    ...
    let invocation = invocation(
        ...
        Arc:REDACTED_SECRETturn),
        "resume_agent",
        function_payload(json!({"id": ThreadId:REDACTED_SECRET).to_string()})),
    );
    let Err(err) = ResumeAgentHandler.handle(invocation).await else {
...
async fn wait_agent_rejects_non_positive_timeout() {
    ...
    let invocation = invocation(
        ...
        "wait_agent",
        function_payload(json!({
            "targets": [ThreadId:REDACTED_SECRET).to_string()],
            "timeout_ms": 0
        })),
    );
...
async fn multi_agent_v2_wait_agent_accepts_timeout_only_argument() {
    ...
    session.enqueue_mailbox_communication(InterAgentCommunication:REDACTED_SECRET
        worker_path,
        AgentPath::REDACTED_SECRET(),
        Vec:REDACTED_SECRET),
...
async fn wait_agent_returns_not_found_for_missing_agents() {
    ...
    let manager = thread_manager();
    session.services.agent_control = manager.agent_control();
    let id_a = ThreadId:REDACTED_SECRET);
    let id_b = ThreadId:REDACTED_SECRET);
    let invocation = invocation(
        Arc:REDACTED_SECRETsession),
        Arc:REDACTED_SECRETturn),
        "wait_agent",
...
async fn wait_agent_times_out_when_status_is_not_final() {
    ...
    assert_eq!(
        result,
        wait::WaitAgentResult {
            status: HashMap:REDACTED_SECRET),
            timed_out: true
        }
    );
    assert_eq!(success, None);
...
#[tokio::test]
async fn multi_agent_v2_close_agent_rejects_REDACTED_SECRET_target_and_id() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let REDACTED_SECRET = manager
        .start_thread((*turn.config).clone())
    ...
    let session = Arc:REDACTED_SECRETsession);
    let turn = Arc:REDACTED_SECRETturn);
    let REDACTED_SECRET_path_error = CloseAgentHandlerV2
        .handle(invocation(
            session.clone(),
            turn.clone(),
            "close_agent",
            function_payload(json!({"target": "/REDACTED_SECRET"})),
        ))
        .await
        .expect_err("close_agent should reject the REDACTED_SECRET path");
    assert_eq!(
        REDACTED_SECRET_path_error,
        FunctionCallError::RespondToModel("REDACTED_SECRET is not a spawned agent".to_string())
    );

    let REDACTED_SECRET_id_error = CloseAgentHandlerV2
        .handle(invocation(
            session,
            turn,
            "close_agent",
            function_payload(json!({"target": REDACTED_SECRET.thread_id.to_string()})),
        ))
        .await
        .expect_err("close_agent should reject the REDACTED_SECRET thread id");
    assert_eq!(
        REDACTED_SECRET_id_error,
        FunctionCallError::RespondToModel("REDACTED_SECRET is not a spawned agent".to_string())
    );
}
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let shutdown_report = manager
        ...
        .await;
    assert_eq!(shutdown_report.submit_failed, Vec::<ThreadId>:REDACTED_SECRET));
    assert_eq!(shutdown_report.timed_out, Vec::<ThreadId>:REDACTED_SECRET));
}
...
async fn build_agent_spawn_config_preserves_base_user_instructions() {
    ...
    turn.config = Arc:REDACTED_SECRETbase_config.clone());
    let base_instructions = BaseInstructions {
        text: "base".to_string(),
    };
...
async fn build_agent_resume_config_clears_base_instructions() {
    ...
    turn.config = Arc:REDACTED_SECRETbase_config);
    turn.approval_policy
        .set(AskForApproval::OnRequest)
        .expect("approval policy set");
