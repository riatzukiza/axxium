...
fn shell_tool_name(config: &ToolsConfig) -> Option<&'static str> {
    match config.shell_type {
        ...
        ConfigShellToolType:REDACTED_SECRET => Some("local_shell"),
...
fn model_info_from_models_json(slug: REDACTED_SECRET) -> ModelInfo {
    let config = test_config();
...
fn assert_model_tools(
    model_slug: REDACTED_SECRET,
    features: &Features,
    web_search_mode: Option<WebSearchMode>,
    expected_tools: &[REDACTED_SECRET],
) {
        ...
        ToolSpec::Function(ResponsesApiTool {
            ...
            parameters: JsonSchema::Object {
                properties: BTreeMap::from([(
                    "tags".to_string(),
                    JsonSchema::Array {
                        items: Box:REDACTED_SECRETJsonSchema::String { description: None }),
                        description: None
                    }
                )]),
                required: None,
