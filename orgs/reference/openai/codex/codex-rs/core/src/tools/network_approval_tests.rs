...
async fn pending_waiters_receive_owner_decision() {
    let pending = Arc:REDACTED_SECRETPendingHostApproval:REDACTED_SECRET));
...
fn denied_blocked_request(host: REDACTED_SECRET) -> BlockedRequest {
    BlockedRequest:REDACTED_SECRETBlockedRequestArgs {
        host: host.to_string(),
