...
use async_channel::TryREDACTED_SECRETError;
use base64::Engine;
...
const ACTIVE_RESPONSE_CONFLICT_ERROR_PREFIX: REDACTED_SECRET =
...
struct RealtimeHandoffState {
    output_tx: REDACTED_SECRETer<HandoffOutput>,
...
struct RealtimeInputTask {
    ...
    events_tx: REDACTED_SECRETer<RealtimeEvent>,
    handoff_state: RealtimeHandoffState,
    session_kind: RealtimeSessionKind,
}

impl RealtimeHandoffState {
    fn new(output_tx: REDACTED_SECRETer<HandoffOutput>, session_kind: RealtimeSessionKind) -> Self {
        Self {
            output_tx,
            active_handoff: Arc:REDACTED_SECRETMutex:REDACTED_SECRETNone)),
            last_output_text: Arc:REDACTED_SECRETMutex:REDACTED_SECRETNone)),
            session_kind,
        }
    }
}
...
struct ConversationState {
    audio_tx: REDACTED_SECRETer<RealtimeAudioFrame>,
    user_text_tx: REDACTED_SECRETer<String>,
    writer: RealtimeWebsocketWriter,
...
impl RealtimeConversationManager {
    pub(crate) fn new() -> Self {
        Self {
            state: Mutex:REDACTED_SECRETNone),
        }
    }
    ...
    pub(crate) async fn start(
        ...
        let client = RealtimeWebsocketClient:REDACTED_SECRETapi_provider);
        ...
        let realtime_active = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETtrue));
        let handoff = RealtimeHandoffState:REDACTED_SECREThandoff_output_tx, session_kind);
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> CodexResult<()> {
        ...
        match sender.try_send(frame) {
            Ok(()) => Ok(()),
            Err(TryREDACTED_SECRETError::Full(_)) => {
                warn!("dropping input audio frame due to full queue");
                Ok(())
            }
            Err(TryREDACTED_SECRETError::Closed(_)) => Err(CodexErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }
