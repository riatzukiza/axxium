...
pub(crate) fn load_agent_roles(
...
) -> std::io::Result<REDACTED_SECRET, AgentRoleConfig>> {
    ...
    let mut roles: REDACTED_SECRET, AgentRoleConfig> = BTreeMap:REDACTED_SECRET);
    for layer in layers {
        let mut layer_roles: REDACTED_SECRET, AgentRoleConfig> = BTreeMap:REDACTED_SECRET);
        let mut declared_role_files = BTreeSet:REDACTED_SECRET);
...
fn load_agent_roles_without_layers(
    ...
    let mut roles = BTreeMap:REDACTED_SECRET);
...
fn read_declared_role(
    declared_role_name: REDACTED_SECRET,
...
pub(crate) fn parse_agent_role_file_contents(
    ...
    role_name_hint: Option<REDACTED_SECRET>,
...
fn validate_agent_role_file_developer_instructions(
    role_file_label: &Path,
    developer_instructions: Option<REDACTED_SECRET>,
    ...
    match developer_instructions.map(str::trim) {
        ...
        Some(_) => Ok(()),
        None if require_present => Err(std::io::Error:REDACTED_SECRET
            std::io::ErrorKind::InvalidInput,
            format!(
...
fn normalize_agent_role_nickname_candidates(
    ...
    let mut seen_candidates = BTreeSet:REDACTED_SECRET);
