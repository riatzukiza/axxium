...
fn stdio_mcp(command: REDACTED_SECRET) -> McpServerConfig {
    McpServerConfig {
...
fn http_mcp(url: REDACTED_SECRET) -> McpServerConfig {
    McpServerConfig {
...
[permissions.workspace.filesystem.":project_REDACTED_SECRETs"]
"." = "write"
"docs" = "read"
                ...
                PermissionProfileToml {
                    filesystem: Some(FilesystemPermissionsToml {
                        entries: BTreeMap::from([
                            ...
                            (
                                ":project_REDACTED_SECRETs".to_string(),
...
fn permissions_profiles_network_populates_runtime_network_proxy_spec() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let cwd = TempDir:REDACTED_SECRET)?;
...
fn default_permissions_profile_populates_runtime_sandbox_policy() -> std::io::Result<()> {
    ...
    let memories_REDACTED_SECRET = codex_home.path().join("memories").abs();
    assert_eq!(
        ...
        FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::project_REDACTED_SECRETs(/*subpath*/ None),
                },
                access: FileSystemAccessMode::Write,
            },
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
                    value: FileSystemSpecialPath::project_REDACTED_SECRETs(Some("docs".into())),
                },
                access: FileSystemAccessMode::Read,
            },
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: memories_REDACTED_SECRET.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]),
    ...
    assert_eq!(
        ...
        &SandboxPolicy::WorkspaceWrite {
            writable_REDACTED_SECRETs: vec![memories_REDACTED_SECRET],
            read_only_access: ReadOnlyAccess::Restricted {
                include_platform_defaults: true,
                readable_REDACTED_SECRETs: vec![cwd.path().join("docs").abs(),],
            },
...
#[test]
fn permissions_profiles_reject_writes_outside_workspace_REDACTED_SECRET() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let cwd = TempDir:REDACTED_SECRET)?;
    std::fs::write(cwd.path().join(".git"), "gitdir: nowhere")?;
    ...
    .expect_err("writes outside the workspace REDACTED_SECRET should be rejected");
...
#[test]
fn permissions_profiles_reject_nested_entries_for_non_project_REDACTED_SECRETs() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let cwd = TempDir:REDACTED_SECRET)?;
    std::fs::write(cwd.path().join(".git"), "gitdir: nowhere")?;
    ...
    .expect_err("nested entries outside :project_REDACTED_SECRETs should be rejected");
...
fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert_eq!(
        config.permissions.file_system_sandbox_policy,
        FileSystemSandboxPolicy::restricted(Vec:REDACTED_SECRET))
    );
    assert_eq!(
        config.permissions.sandbox_policy.get(),
        &SandboxPolicy::ReadOnly {
...
fn permissions_profiles_allow_empty_filesystem_with_warning() -> std::io::Result<()> {
    let config = load_workspace_permission_profile(PermissionProfileToml {
        filesystem: Some(FilesystemPermissionsToml {
            entries: BTreeMap:REDACTED_SECRET),
        }),
        network: None,
    })?;
...
#[test]
fn permissions_profiles_reject_project_REDACTED_SECRET_parent_traversal() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let cwd = TempDir:REDACTED_SECRET)?;
    std::fs::write(cwd.path().join(".git"), "gitdir: nowhere")?;
    ...
    .expect_err("parent traversal should be rejected for project REDACTED_SECRET subpaths");
...
"#;
    ...
    let writable_REDACTED_SECRET = test_absolute_path("/my/workspace");
...
exclude_slash_tmp = true
"#,
        serde_json::json!(writable_REDACTED_SECRET)
    );
    ...
    if cfg!(target_os = "windows") {
        ...
        assert_eq!(
            resolution,
            SandboxPolicy::WorkspaceWrite {
                writable_REDACTED_SECRETs: vec![writable_REDACTED_SECRET.clone()],
...
fn legacy_sandbox_mode_config_builds_split_policies_without_drift() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let cwd = TempDir:REDACTED_SECRET)?;
    let extra_REDACTED_SECRET = test_absolute_path("/tmp/legacy-extra-REDACTED_SECRET");
    let cases = vec![
        (
...
exclude_slash_tmp = true
"#,
                serde_json::json!(extra_REDACTED_SECRET)
            ),
        ),
    ];
...
fn filter_mcp_servers_by_allowlist_enforces_identity_rules() {
    const MISMATCHED_COMMAND_SERVER: REDACTED_SECRET = "mismatched-command-should-disable";
    const MISMATCHED_URL_SERVER: REDACTED_SECRET = "mismatched-url-should-disable";
    const MATCHED_COMMAND_SERVER: REDACTED_SECRET = "matched-command-should-allow";
    const MATCHED_URL_SERVER: REDACTED_SECRET = "matched-url-should-allow";
    const DIFFERENT_NAME_SERVER: REDACTED_SECRET = "different-name-should-disable";

    const GOOD_CMD: REDACTED_SECRET = "good-cmd";
    const GOOD_URL: REDACTED_SECRET = "https://example.com/good";
    ...
    let requirements = Sourced:REDACTED_SECRET
        BTreeMap::from([
            (
                MISMATCHED_URL_SERVER.to_string(),
...
fn filter_mcp_servers_by_allowlist_blocks_all_when_empty() {
    ...
    let source = RequirementSource::LegacyManagedConfigTomlFromMdm;
    let requirements = Sourced:REDACTED_SECRETBTreeMap:REDACTED_SECRET), source.clone());
    filter_mcp_servers_by_requirements(&mut servers, Some(&requirements));
...
#[test]
fn add_dir_override_extends_workspace_writable_REDACTED_SECRETs() -> std::io::Result<()> {
    let temp_dir = TempDir:REDACTED_SECRET)?;
    ...
    let overrides = ConfigOverrides {
        ...
        additional_writable_REDACTED_SECRETs: vec![PathBuf::from("../backend"), backend.clone()],
        ..Default::default()
    };
    ...
    if cfg!(target_os = "windows") {
        ...
        match config.permissions.sandbox_policy.get() {
            SandboxPolicy::WorkspaceWrite { writable_REDACTED_SECRETs, .. } => {
                assert_eq!(
                    writable_REDACTED_SECRETs
                        .iter()
                        .filter(|REDACTED_SECRET| **REDACTED_SECRET == expected_backend)
                        .count(),
                    1,
                    "expected single writable REDACTED_SECRET entry for {}",
...
#[test]
fn workspace_write_always_includes_memories_REDACTED_SECRET_once() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let memories_REDACTED_SECRET = codex_home.path().join("memories");
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
                writable_REDACTED_SECRETs: vec![memories_REDACTED_SECRET.abs()],
                ..Default::default()
            }),
            ..Default::default()
        },
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            memories_REDACTED_SECRET.is_dir(),
            "expected memories REDACTED_SECRET directory to exist at {}",
            memories_REDACTED_SECRET.display()
        );
        let expected_memories_REDACTED_SECRET = memories_REDACTED_SECRET.abs();
        match config.permissions.sandbox_policy.get() {
            SandboxPolicy::WorkspaceWrite { writable_REDACTED_SECRETs, .. } => {
                assert_eq!(
                    writable_REDACTED_SECRETs
                        .iter()
                        .filter(|REDACTED_SECRET| **REDACTED_SECRET == expected_memories_REDACTED_SECRET)
                        .count(),
                    1,
                    "expected single writable REDACTED_SECRET entry for {}",
                    expected_memories_REDACTED_SECRET.display()
                );
            }
            other => panic!("expected workspace-write policy, got {other:?}"),
        }
...
fn web_search_mode_for_turn_falls_back_when_live_is_disallowed() -> anyhow::Result<()> {
    ...
    let web_search_mode = Constrained:REDACTED_SECRETWebSearchMode::Cached, move |candidate| {
...
async fn project_profile_overrides_user_profile() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let workspace = TempDir:REDACTED_SECRET)?;
...
fn profile_sandbox_mode_overrides_base() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let mut profiles = HashMap:REDACTED_SECRET);
    profiles.insert(
        "work".to_string(),
        ConfigProfile {
...
fn feature_table_overrides_legacy_flags() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let mut entries = BTreeMap:REDACTED_SECRET);
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let mut servers = BTreeMap:REDACTED_SECRET);
    servers.insert(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let empty = BTreeMap:REDACTED_SECRET);
    apply_blocking(
        codex_home.path(),
        /*profile*/ None,
...
fn to_mcp_config_preserves_apps_feature_from_config() -> std::io::Result<()> {
    ...
    let plugins_manager = PluginsManager:REDACTED_SECRETcodex_home.path().to_path_buf());
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path:REDACTED_SECRET"/tmp/codex-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder:REDACTED_SECRETcodex_home.path())
...
#[tokio::test]
async fn set_feature_enabled_profile_disable_overrides_REDACTED_SECRET_enable() -> anyhow::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
...
fn load_config_uses_requirements_guardian_developer_instructions() -> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let config_layer_stack = ConfigLayerStack:REDACTED_SECRET
        Vec:REDACTED_SECRET),
        Default::default(),
...
-> std::io::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let repo_REDACTED_SECRET = TempDir:REDACTED_SECRET)?;
    let nested_cwd = repo_REDACTED_SECRET.path().join("packages").join("app");
    std::fs::create_dir_all(repo_REDACTED_SECRET.path().join(".git"))?;
    std::fs::create_dir_all(&nested_cwd)?;

    let workspace_key = repo_REDACTED_SECRET.path().to_string_lossy().replace('\\', "\\\\");
    tokio::fs::write(
        codex_home.path().join(CONFIG_TOML_FILE),
        format!(
...
"#
    ...
    let standalone_agents_dir = repo_REDACTED_SECRET.path().join(".codex").join("agents");
...
"#
    ...
    let REDACTED_SECRET_agent = repo_REDACTED_SECRET
        .path()
        .join(".codex")
        .join("agents")
        .join("REDACTED_SECRET.toml");
    std::fs::create_dir_all(
        REDACTED_SECRET_agent
            .parent()
            .expect("REDACTED_SECRET agent should have a parent directory"),
    )?;
    std::fs::write(
        &REDACTED_SECRET_agent,
        r#"
name = "researcher"
description = "from REDACTED_SECRET"
developer_instructions = "Research carefully"
"#,
    )?;

    let nested_agent = repo_REDACTED_SECRET
        .path()
        .join("packages")
        .join(".codex")
        .join("agents")
...
"#,
    ...
    let sibling_agent = repo_REDACTED_SECRET
        .path()
        .join("packages")
        .join(".codex")
        .join("agents")
        ...
        agents: Some(AgentsToml {
            ...
            roles: BTreeMap::from([(
                ...
                AgentRoleToml {
                    description: Some("Research role".to_string()),
                    config_file: None,
                    nickname_candidates: Some(Vec:REDACTED_SECRET)),
                },
            )]),
        }),
        ..Default::default()
...
"#;
    ...
    // AGENTS.md file.
    let cwd_temp_dir = TempDir:REDACTED_SECRET).unwrap();
    let cwd = cwd_temp_dir.path().to_path_buf();
    // Make it look like a Git repo so it does not search for AGENTS.md in
    // a parent folder, either.
    std::fs::write(cwd.join(".git"), "gitdir: nowhere")?;

    let codex_home_temp_dir = TempDir:REDACTED_SECRET).unwrap();
...
fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            mcp_servers: Constrained::allow_any(HashMap:REDACTED_SECRET)),
            mcp_oauth_credentials_store_mode: Default::default(),
            mcp_oauth_callback_port: None,
            mcp_oauth_callback_url: None,
            model_providers: fixture.model_provider_map.clone(),
            project_doc_max_bytes: PROJECT_DOC_MAX_BYTES,
            project_doc_fallback_filenames: Vec:REDACTED_SECRET),
            tool_output_token_limit: None,
            agent_max_threads: DEFAULT_AGENT_MAX_THREADS,
            agent_max_depth: DEFAULT_AGENT_MAX_DEPTH,
            agent_roles: BTreeMap:REDACTED_SECRET),
            ...
            startup_warnings: Vec:REDACTED_SECRET),
            ...
            js_repl_REDACTED_SECRET_path: None,
            js_repl_REDACTED_SECRET_module_dirs: Vec:REDACTED_SECRET),
            zsh_path: None,
...
fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()> {
    ...
    let constrained = Constrained:REDACTED_SECRETWebSearchMode::Cached, move |candidate| {
    ...
    let requirements = crate::config_loader::ConfigRequirements {
        web_search_mode: crate::config_loader::ConstrainedWithSource:REDACTED_SECRET
            constrained,
    ...
    let config_layer_stack =
        crate::config_loader::ConfigLayerStack:REDACTED_SECRETVec:REDACTED_SECRET), requirements, requirements_toml)
...
fn test_set_project_trusted_writes_explicit_tables() -> anyhow::Result<()> {
    let project_dir = Path:REDACTED_SECRET"/some/path");
    let mut doc = DocumentMut:REDACTED_SECRET);
...
model = "foo""#;
    ...
    let new_project = Path:REDACTED_SECRET"/Users/mbolin/code/codex2");
...
{
    let project_dir = TempDir:REDACTED_SECRET)?;
    ...
    let constrained = Constrained:REDACTED_SECRETSandboxPolicy::DangerFullAccess, |candidate| {
...
fn derive_sandbox_policy_preserves_windows_downgrade_for_unsupported_fallback() -> anyhow::Result<()>
{
    ...
    let constrained = Constrained:REDACTED_SECRETSandboxPolicy::new_workspace_write_policy(), |candidate| {
...
fn test_resolve_oss_provider_from_profile() {
    let mut profiles = std::collections::HashMap:REDACTED_SECRET);
    let profile = ConfigProfile {
...
fn test_untrusted_project_gets_unless_trusted_approval_policy() -> anyhow::Result<()> {
    let codex_home = TempDir:REDACTED_SECRET)?;
    let test_project_dir = TempDir:REDACTED_SECRET)?;
...
-> std::io::Result<()> {
    ...
    let config = ConfigBuilder::default()
        .codex_home(codex_home.path().to_path_buf())
        .cloud_requirements(CloudRequirementsLoader:REDACTED_SECRETasync {
...
"#,
    ...
    let config = ConfigBuilder::default()
        .codex_home(codex_home.path().to_path_buf())
        .fallback_cwd(Some(codex_home.path().to_path_buf()))
        .cloud_requirements(CloudRequirementsLoader:REDACTED_SECRETasync move {
...
async fn feature_requirements_reject_collab_legacy_alias() {
    let codex_home = TempDir:REDACTED_SECRET).expect("tempdir");
