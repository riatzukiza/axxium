...
"#;
    ...
    let REDACTED_SECRET = item.as_table().expect("REDACTED_SECRET table");
    assert!(!REDACTED_SECRET.is_implicit(), "REDACTED_SECRET table should be explicit");

    let mcp_servers = REDACTED_SECRET
        .get("mcp_servers")
...
async fn read_includes_origins_and_layers() {
    ...
    let service = ConfigService:REDACTED_SECRET
