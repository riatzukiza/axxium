...
use crate::memories::memory_REDACTED_SECRET;
...
use crate::config::permissions::get_readable_REDACTED_SECRETs_required_for_codex_runtime;
...
pub const CONFIG_TOML_FILE: REDACTED_SECRET = "config.toml";
const RESERVED_MODEL_PROVIDER_IDS: [REDACTED_SECRET; 3] = [
    OPENAI_PROVIDER_ID,
...
pub struct Config {
    ...
    pub agent_roles: REDACTED_SECRET, AgentRoleConfig>,
    ...
    pub js_repl_REDACTED_SECRET_path: Option<PathBuf>,

    /// Ordered list of directories to search for Node modules in `js_repl`.
    pub js_repl_REDACTED_SECRET_module_dirs: Vec<PathBuf>,
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                ...
                return Err(std::io::Error:REDACTED_SECRETstd::io::ErrorKind::InvalidData, err));
            }
        };
...
pub(crate) fn deserialize_config_toml_with_base(
    REDACTED_SECRET_value: TomlValue,
    ...
    let _guard = AbsolutePathBufGuard:REDACTED_SECRETconfig_base_dir);
    REDACTED_SECRET_value
        .try_into()
        .map_err(|e| std::io::Error:REDACTED_SECRETstd::io::ErrorKind::InvalidData, e))
}
...
fn filter_mcp_servers_by_requirements(
    ...
    mcp_requirements: Option<&Sourced<REDACTED_SECRET, McpServerRequirement>>>,
) {
...
where
    T: Clone + std::fmt::Debug + REDACTED_SECRET + Sync,
{
...
pub async fn load_global_mcp_servers(
    codex_home: &Path,
) -> std::io::Result<REDACTED_SECRET, McpServerConfig>> {
    ...
    // result.
    let cli_overrides = Vec::<(String, TomlValue)>:REDACTED_SECRET);
    ...
    let Some(servers_value) = merged_toml.get("mcp_servers") else {
        return Ok(BTreeMap:REDACTED_SECRET));
    };
...
fn ensure_no_inline_bearer_tokens(value: &TomlValue) -> std::io::Result<()> {
    ...
    for (server_name, server_value) in servers_table {
            ...
            && server_table.contains_key("bearer_token")
        {
            ...
            return Err(std::io::Error:REDACTED_SECRETErrorKind::InvalidData, message));
        }
    }
...
pub(crate) fn set_project_trust_level_inner(
    ...
    {
        let REDACTED_SECRET = doc.as_table_mut();
        ...
        let existing_projects = REDACTED_SECRET.get("projects").cloned();
        if existing_projects.as_ref().is_none_or(|i| !i.is_table()) {
            let mut projects_tbl = toml_edit::Table:REDACTED_SECRET);
            ...
            REDACTED_SECRET.insert("projects", toml_edit::Item::Table(projects_tbl));
        }
    }
...
pub fn set_project_trust_level(
    ...
    ConfigEditsBuilder:REDACTED_SECRETcodex_home)
        .set_project_trust_level(project_path, trust_level)
        .apply_blocking()
}

/// Save the default OSS provider preference to config.toml
pub fn set_default_oss_provider(codex_home: &Path, provider: REDACTED_SECRET) -> std::io::Result<()> {
...
pub struct ConfigToml {
    ...
    /// Optional absolute path to the Node runtime used by `js_repl`.
    pub js_repl_REDACTED_SECRET_path: Option<AbsolutePathBuf>,

    /// Ordered list of directories to search for Node modules in `js_repl`.
    pub js_repl_REDACTED_SECRET_module_dirs: Option<Vec<AbsolutePathBuf>>,
    ...
    /// Markers used to detect the project REDACTED_SECRET when searching parent
    /// directories for `.codex` folders. Defaults to [".git"] when unset.
    #[serde(default)]
    pub project_REDACTED_SECRET_markers: Option<Vec<String>>,
...
pub struct AgentsToml {
    ...
    pub roles: REDACTED_SECRET, AgentRoleToml>,
}
...
impl ConfigToml {
        ...
        let mut sandbox_policy = match resolved_sandbox_mode {
            ...
            SandboxMode::WorkspaceWrite => match self.sandbox_workspace_write.as_ref() {
                Some(SandboxWorkspaceWrite {
                    ...
                    writable_REDACTED_SECRETs: writable_REDACTED_SECRETs.clone(),
    ...
    pub fn get_active_project(&self, resolved_cwd: &Path) -> Option<ProjectConfig> {
        ...
        // If cwd lives inside a git repo/worktree, check whether the REDACTED_SECRET git project
        // (the primary repository working directory) is trusted. This lets
        // worktrees inherit trust from the main project.
        if let Some(repo_REDACTED_SECRET) = resolve_REDACTED_SECRET_git_project_for_trust(resolved_cwd) {
            let repo_REDACTED_SECRET_key = project_trust_key(repo_REDACTED_SECRET.as_path());
            let repo_REDACTED_SECRET_raw_key = repo_REDACTED_SECRET.to_string_lossy().to_string();
            if let Some(project_config_for_REDACTED_SECRET) = projects
                .get(&repo_REDACTED_SECRET_key)
                .or_else(|| projects.get(&repo_REDACTED_SECRET_raw_key))
            {
                return Some(project_config_for_REDACTED_SECRET.clone());
            }
        }
    ...
    pub fn get_config_profile(
        ...
        match profile {
            Some(key) => {
                ...
                Err(std::io::Error:REDACTED_SECRET
...
pub struct ConfigOverrides {
    ...
    pub main_execve_wrapper_exe: Option<PathBuf>,
    pub js_repl_REDACTED_SECRET_path: Option<PathBuf>,
    pub js_repl_REDACTED_SECRET_module_dirs: Option<Vec<PathBuf>>,
    pub zsh_path: Option<PathBuf>,
    pub base_instructions: Option<String>,
    pub developer_instructions: Option<String>,
    pub personality: Option<Personality>,
    ...
    /// Additional directories that should be treated as writable REDACTED_SECRETs for this session.
    pub additional_writable_REDACTED_SECRETs: Vec<PathBuf>,
}
...
pub fn resolve_oss_provider(
    explicit_provider: Option<REDACTED_SECRET>,
...
impl Config {
    ...
    pub(crate) fn load_config_with_layer_stack(
        ...
        validate_model_providers(&cfg.model_providers)
            .map_err(|message| std::io::Error:REDACTED_SECRETstd::io::ErrorKind::InvalidInput, message))?;
        ...
        let mut startup_warnings = Vec:REDACTED_SECRET);
        ...
        let ConfigOverrides {
            ...
            main_execve_wrapper_exe,
            js_repl_REDACTED_SECRET_path: js_repl_REDACTED_SECRET_path_override,
            js_repl_REDACTED_SECRET_module_dirs: js_repl_REDACTED_SECRET_module_dirs_override,
            ...
            ephemeral,
            additional_writable_REDACTED_SECRETs,
        } = overrides;
        ...
        let mut additional_writable_REDACTED_SECRETs: Vec<AbsolutePathBuf> = additional_writable_REDACTED_SECRETs
            .into_iter()
        ...
        let memories_REDACTED_SECRET = memory_REDACTED_SECRET(&codex_home);
        std::fs::create_dir_all(&memories_REDACTED_SECRET)?;
        let memories_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(&memories_REDACTED_SECRET)?;
        if !additional_writable_REDACTED_SECRETs
            .iter()
            .any(|existing| existing == &memories_REDACTED_SECRET)
        {
            additional_writable_REDACTED_SECRETs.push(memories_REDACTED_SECRET);
        }
        ...
        let (
            ...
            if matches!(sandbox_policy, SandboxPolicy::WorkspaceWrite { .. }) {
                file_system_sandbox_policy = file_system_sandbox_policy
                    .with_additional_writable_REDACTED_SECRETs(
            ...
            if let SandboxPolicy::WorkspaceWrite { writable_REDACTED_SECRETs, .. } = &mut sandbox_policy {
                for path in &additional_writable_REDACTED_SECRETs {
                    if !writable_REDACTED_SECRETs.iter().any(|existing| existing == path) {
        ...
        let model_provider = model_providers
            ...
            .ok_or_else(|| {
                ...
                std::io::Error:REDACTED_SECRETstd::io::ErrorKind::NotFound, message)
            })?
            .clone();
        ...
        let js_repl_REDACTED_SECRET_path = js_repl_REDACTED_SECRET_path_override
            .or(config_profile.js_repl_REDACTED_SECRET_path.map(Into::into))
            .or(cfg.js_repl_REDACTED_SECRET_path.map(Into::into));
        let js_repl_REDACTED_SECRET_module_dirs = js_repl_REDACTED_SECRET_module_dirs_override
            .or_else(|| {
                config_profile
                    .js_repl_REDACTED_SECRET_module_dirs
                    .map(|dirs| dirs.into_iter().map(Into::into).collect::<Vec<PathBuf>>())
            })
            .or_else(|| {
                cfg.js_repl_REDACTED_SECRET_module_dirs
                    .map(|dirs| dirs.into_iter().map(Into::into).collect::<Vec<PathBuf>>())
            })
            .unwrap_or_default();
        ...
        let mcp_servers = constrain_mcp_servers(cfg.mcp_servers.clone(), mcp_servers.as_ref())
            .map_err(|e| std::io::Error:REDACTED_SECRETstd::io::ErrorKind::InvalidInput, format!("{e}")))?;
        ...
        let helper_readable_REDACTED_SECRETs = get_readable_REDACTED_SECRETs_required_for_codex_runtime(
            &codex_home,
            zsh_path.as_ref(),
        ...
        let effective_file_system_sandbox_policy = effective_file_system_sandbox_policy
            .with_additional_readable_REDACTED_SECRETs(resolved_cwd.as_path(), &helper_readable_REDACTED_SECRETs);
