...
pub struct PermissionsToml {
    #[serde(flatten)]
    pub entries: REDACTED_SECRET, PermissionProfileToml>,
}
...
pub struct FilesystemPermissionsToml {
    #[serde(flatten)]
    pub entries: REDACTED_SECRET, FilesystemPermissionToml>,
}
...
pub enum FilesystemPermissionToml {
    ...
    Scoped(REDACTED_SECRET, FileSystemAccessMode>),
}
...
pub(crate) fn get_readable_REDACTED_SECRETs_required_for_codex_runtime(
    codex_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(codex_home.join("tmp").join("arg0")).ok();
    let zsh_path = zsh_path.and_then(|path| AbsolutePathBuf::from_absolute_path(path).ok());
    let execve_wrapper_REDACTED_SECRET = main_execve_wrapper_exe.and_then(|path| {
        let path = AbsolutePathBuf::from_absolute_path(path).ok()?;
        if let Some(arg0_REDACTED_SECRET) = arg0_REDACTED_SECRET.as_ref()
            && path.as_path().starts_with(arg0_REDACTED_SECRET.as_path())
        {
            path.parent()
        } else {
            Some(path)
    ...
    if let Some(zsh_path) = zsh_path {
        readable_REDACTED_SECRETs.push(zsh_path);
    }
    if let Some(execve_wrapper_REDACTED_SECRET) = execve_wrapper_REDACTED_SECRET {
        readable_REDACTED_SECRETs.push(execve_wrapper_REDACTED_SECRET);
    }
...
fn parse_special_path(path: REDACTED_SECRET) -> Option<FileSystemSpecialPath> {
    match path {
        ":REDACTED_SECRET" => Some(FileSystemSpecialPath::Root),
        ":minimal" => Some(FileSystemSpecialPath::Minimal),
        ":project_REDACTED_SECRETs" => Some(FileSystemSpecialPath::project_REDACTED_SECRETs(/*subpath*/ None)),
...
fn parse_absolute_path(path: REDACTED_SECRET) -> io::Result<AbsolutePathBuf> {
    parse_absolute_path_for_platform(path, cfg!(windows))
}

fn parse_absolute_path_for_platform(path: REDACTED_SECRET, is_windows: bool) -> io::Result<AbsolutePathBuf> {
...
fn is_absolute_path_for_platform(path: REDACTED_SECRET, normalized_path: &Path, is_windows: bool) -> bool {
    if is_windows {
...
fn normalize_absolute_path_for_platform(path: REDACTED_SECRET, is_windows: bool) -> Cow<'_, Path> {
    if !is_windows {
        return Cow::Borrowed(Path:REDACTED_SECRETpath));
    }

    match normalize_windows_device_path(path) {
        Some(normalized) => Cow::Owned(PathBuf::from(normalized)),
        None => Cow::Borrowed(Path:REDACTED_SECRETpath)),
    }
}

fn normalize_windows_device_path(path: REDACTED_SECRET) -> Option<String> {
...
fn is_windows_absolute_path(path: REDACTED_SECRET) -> bool {
    is_windows_drive_absolute_path(path) || path.starts_with(r"\\")
}

fn is_windows_drive_absolute_path(path: REDACTED_SECRET) -> bool {
...
fn parse_relative_subpath(subpath: REDACTED_SECRET) -> io::Result<PathBuf> {
    let path = Path:REDACTED_SECRETsubpath);
    if !subpath.is_empty()
        && path
            .components()
...
fn missing_filesystem_entries_warning(profile_name: REDACTED_SECRET) -> String {
    format!(
