...
impl ManagedFeatures {
    pub(crate) fn from_configured(
        ...
        let (pinned_features, source) = match feature_requirements {
            ...
            None => (BTreeMap:REDACTED_SECRET), None),
        };
        ...
        Ok(Self {
            value: ConstrainedWithSource:REDACTED_SECRETConstrained::allow_any(normalized_features), source),
            pinned_features,
        })
    }
...
impl From<Features> for ManagedFeatures {
    fn from(features: Features) -> Self {
        Self {
            value: ConstrainedWithSource:REDACTED_SECRET
                Constrained::allow_any(features),
                /*source*/ None,
            ),
            pinned_features: BTreeMap:REDACTED_SECRET),
        }
    }
}
...
fn parse_feature_requirements(
    ...
    let mut pinned_features = BTreeMap:REDACTED_SECRET);
...
fn explicit_feature_settings_in_config(cfg: &ConfigToml) -> Vec<(String, Feature, bool)> {
    let mut explicit_settings = Vec:REDACTED_SECRET);
...
pub(crate) fn validate_feature_requirements_in_config_toml(
    ...
    fn validate_profile(
        cfg: &ConfigToml,
        profile_name: Option<REDACTED_SECRET>,
