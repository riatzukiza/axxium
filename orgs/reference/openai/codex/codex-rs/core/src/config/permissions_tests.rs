...
fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let arg0_REDACTED_SECRET = codex_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_REDACTED_SECRET.join("codex-arg0-session");
    let sibling_arg0_dir = arg0_REDACTED_SECRET.join("codex-arg0-other-session");
