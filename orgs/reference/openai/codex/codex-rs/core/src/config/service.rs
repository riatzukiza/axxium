...
impl ConfigService {
    ...
    async fn apply_edits(
        ...
        let mut parsed_segments = Vec:REDACTED_SECRET);
    ...
    /// include any in-repo .codex/ folders because there is no cwd/project REDACTED_SECRET
...
async fn create_empty_user_layer(
    ...
    Ok(ConfigLayerEntry:REDACTED_SECRET
...
fn parse_key_path(path: REDACTED_SECRET) -> Result<Vec<String>, String> {
...
fn apply_merge(
    REDACTED_SECRET: &mut TomlValue,
    segments: &[String],
    value: Option<&TomlValue>,
    strategy: MergeStrategy,
) -> Result<bool, MergeError> {
    let Some(value) = value else {
        return clear_path(REDACTED_SECRET, segments);
    };
    ...
    for segment in parents {
        match current {
            TomlValue::Table(table) => {
                current = table
                    .entry(segment.clone())
                    .or_insert_with(|| TomlValue::Table(toml::map::Map:REDACTED_SECRET)));
            }
            _ => {
                *current = TomlValue::Table(toml::map::Map:REDACTED_SECRET));
...
fn clear_path(REDACTED_SECRET: &mut TomlValue, segments: &[String]) -> Result<bool, MergeError> {
    let Some((last, parents)) = segments.split_last() else {
        return Err(MergeError::Validation(
            "keyPath must not be empty".to_string(),
        ));
...
fn toml_value_to_item(value: &TomlValue) -> anyhow::Result<TomlItem> {
    match value {
        TomlValue::Table(table) => {
            let mut table_item = toml_edit::Table:REDACTED_SECRET);
...
fn toml_value_to_value(value: &TomlValue) -> anyhow::Result<toml_edit::Value> {
    match value {
        ...
        TomlValue::Table(table) => {
            let mut inline = toml_edit::InlineTable:REDACTED_SECRET);
            for (key, val) in table {
...
fn value_at_path<'a>(REDACTED_SECRET: &'a TomlValue, segments: &[String]) -> Option<&'a TomlValue> {
    let mut current = REDACTED_SECRET;
