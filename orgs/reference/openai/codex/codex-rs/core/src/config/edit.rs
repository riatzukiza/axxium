...
const NOTICE_TABLE_KEY: REDACTED_SECRET = "notice";
...
pub enum ConfigEdit {
    ...
    ReplaceMcpServers(REDACTED_SECRET, McpServerConfig>),
...
pub fn syntax_theme_edit(name: REDACTED_SECRET) -> ConfigEdit {
    ConfigEdit::SetPath {
...
mod document_helpers {
    ...
    fn serialize_mcp_server_table(config: &McpServerConfig) -> TomlTable {
        let mut entry = TomlTable:REDACTED_SECRET);
    ...
    pub(super) fn new_implicit_table() -> TomlTable {
        let mut table = TomlTable:REDACTED_SECRET);
    ...
    {
        let mut array = TomlArray:REDACTED_SECRET);
        for value in iter {
            array.push(value);
        }
...
impl ConfigDocument {
    ...
    fn write_profile_value(&mut self, segments: &[REDACTED_SECRET], value: Option<TomlItem>) -> bool {
        match value {
    ...
    fn write_value(&mut self, scope: Scope, segments: &[REDACTED_SECRET], value: TomlItem) -> bool {
        let resolved = self.scoped_segments(scope, segments);
        self.insert(&resolved, value)
    }

    fn clear(&mut self, scope: Scope, segments: &[REDACTED_SECRET]) -> bool {
        let resolved = self.scoped_segments(scope, segments);
    ...
    fn replace_mcp_servers(&mut self, servers: &REDACTED_SECRET, McpServerConfig>) -> bool {
        if servers.is_empty() {
            return self.clear(Scope:REDACTED_SECRET, &["mcp_servers"]);
        }

        let REDACTED_SECRET = self.doc.as_table_mut();
        if !REDACTED_SECRET.contains_key("mcp_servers") {
        ...
        let Some(item) = REDACTED_SECRET.get_mut("mcp_servers") else {
            return false;
        };
    ...
    fn set_skill_config(&mut self, selector: SkillConfigSelector, enabled: bool) -> bool {
        ...
        {
            let REDACTED_SECRET = self.doc.as_table_mut();
            let skills_item = match REDACTED_SECRET.get_mut("skills") {
                Some(item) => item,
                None => {
                    if enabled {
                        return false;
                    ...
                    let Some(item) = REDACTED_SECRET.get_mut("skills") else {
                        return false;
                    };
                    item
                }
            ...
            let config_item = match skills_table.get_mut("config") {
                ...
                None => {
                    ...
                    skills_table.insert("config", TomlItem::ArrayOfTables(ArrayOfTables:REDACTED_SECRET)));
            ...
            if !matches!(config_item, TomlItem::ArrayOfTables(_)) {
                ...
                *config_item = TomlItem::ArrayOfTables(ArrayOfTables:REDACTED_SECRET));
            }
        ...
        if remove_skills_table {
            let REDACTED_SECRET = self.doc.as_table_mut();
            REDACTED_SECRET.remove("skills");
        }

        mutated
    }

    fn scoped_segments(&self, scope: Scope, segments: &[REDACTED_SECRET]) -> Vec<String> {
...
pub fn apply_blocking(
    codex_home: &Path,
    profile: Option<REDACTED_SECRET>,
    edits: &[ConfigEdit],
) -> anyhow::Result<()> {
    if edits.is_empty() {
        return Ok(());
    ...
    let serialized = match write_paths.read_path {
        Some(path) => match std::fs::read_to_string(&path) {
            ...
            Err(err) if err.kind() == std::io::ErrorKind::NotFound => String:REDACTED_SECRET),
            Err(err) => return Err(err.into()),
        },
        None => String:REDACTED_SECRET),
    };

    let doc = if serialized.is_empty() {
        DocumentMut:REDACTED_SECRET)
    } else {
    ...
    let mut document = ConfigDocument:REDACTED_SECRETdoc, profile);
    let mut mutated = false;
...
impl ConfigEditsBuilder {
    pub fn new(codex_home: &Path) -> Self {
        Self {
            ...
            profile: None,
            edits: Vec:REDACTED_SECRET),
        }
    }

    pub fn with_profile(mut self, profile: Option<REDACTED_SECRET>) -> Self {
        self.profile = profile.map(ToOwned::to_owned);
        self
    }

    pub fn set_model(mut self, model: Option<REDACTED_SECRET>, effort: Option<ReasoningEffort>) -> Self {
    ...
    pub fn set_hide_model_migration_prompt(mut self, model: REDACTED_SECRET, acknowledged: bool) -> Self {
        self.edits
    ...
    pub fn record_model_migration_seen(mut self, from: REDACTED_SECRET, to: REDACTED_SECRET) -> Self {
    ...
    pub fn replace_mcp_servers(mut self, servers: &REDACTED_SECRET, McpServerConfig>) -> Self {
        self.edits
    ...
    ///
    /// Disabling a default-false feature clears the REDACTED_SECRET-scoped key instead of
    ...
    /// `false` so they can override an inherited REDACTED_SECRET enable.
    pub fn set_feature_enabled(mut self, key: REDACTED_SECRET, enabled: bool) -> Self {
    ...
    pub fn set_windows_sandbox_mode(mut self, mode: REDACTED_SECRET) -> Self {
        let segments = if let Some(profile) = self.profile.as_ref() {
            vec![
                "profiles".to_string(),
                profile.clone(),
    ...
    pub fn set_realtime_microphone(mut self, microphone: Option<REDACTED_SECRET>) -> Self {
    ...
    pub fn set_realtime_speaker(mut self, speaker: Option<REDACTED_SECRET>) -> Self {
