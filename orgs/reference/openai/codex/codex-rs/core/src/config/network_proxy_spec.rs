...
impl NetworkProxySpec {
    ...
    pub async fn start_proxy(
        ...
        let mut builder = NetworkProxy::builder().state(Arc:REDACTED_SECRETstate));
        ...
        Ok(StartedNetworkProxy:REDACTED_SECRETproxy, handle))
    }
    ...
    fn build_state_with_audit_metadata(
        ...
        let reloader = Arc:REDACTED_SECRETStaticNetworkProxyReloader:REDACTED_SECRETstate.clone()));
...
fn upsert_network_domains(config: &mut NetworkProxyConfig, hosts: Vec<String>, allow: bool) {
    let mut incoming = HashSet:REDACTED_SECRET);
    for host in hosts {
