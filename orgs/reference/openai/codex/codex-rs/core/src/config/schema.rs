...
pub(crate) fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    validation.additional_properties = Some(Box:REDACTED_SECRETSchema::Bool(false)));
    object.object = Some(Box:REDACTED_SECRETvalidation));
...
pub(crate) fn mcp_servers_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    let validation = ObjectValidation {
        additional_properties: Some(Box:REDACTED_SECRETschema_gen.subschema_for::<RawMcpServerConfig>())),
        ..Default::default()
    };
    object.object = Some(Box:REDACTED_SECRETvalidation));
...
pub fn config_schema() -> RootSchema {
    SchemaSettings::draft07()
        ...
        .into_generator()
        .into_REDACTED_SECRET_schema_for::<ConfigToml>()
}
