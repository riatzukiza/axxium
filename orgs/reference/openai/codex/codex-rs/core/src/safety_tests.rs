...
fn test_writable_REDACTED_SECRETs_constraint() {
    ...
    // Policy limited to the workspace only; exclude system temp REDACTED_SECRETs so
    ...
    // With the parent dir explicitly added as a writable REDACTED_SECRET, the
    // outside write should be permitted.
    let policy_with_parent = SandboxPolicy::WorkspaceWrite {
        writable_REDACTED_SECRETs: vec![AbsolutePathBuf::try_from(parent).unwrap()],
        read_only_access: Default::default(),
        network_access: false,
        exclude_tmpdir_env_var: true,
        exclude_slash_tmp: true,
...
#[test]
fn granular_with_all_flags_true_matches_on_request_for_out_of_REDACTED_SECRET_patch() {
    let tmp = TempDir:REDACTED_SECRET).unwrap();
    let cwd = tmp.path().to_path_buf();
    let parent = cwd.parent().unwrap().to_path_buf();
    let add_outside =
...
#[test]
fn granular_sandbox_approval_false_rejects_out_of_REDACTED_SECRET_patch() {
    let tmp = TempDir:REDACTED_SECRET).unwrap();
    let cwd = tmp.path().to_path_buf();
    let parent = cwd.parent().unwrap().to_path_buf();
    let add_outside =
