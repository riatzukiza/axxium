...
const SNAPSHOT_DIR: REDACTED_SECRET = "shell_snapshots";
const EXCLUDED_EXPORT_VARS: &[REDACTED_SECRET] = &["PWD", "OLDPWD"];
...
impl ShellSnapshot {
    ...
    ) -> watch::REDACTED_SECRETer<Option<Arc<ShellSnapshot>>> {
    ...
    pub fn refresh_snapshot(
        ...
        session_cwd: PathBuf,
        shell: Shell,
        shell_snapshot_tx: watch::REDACTED_SECRETer<Option<Arc<ShellSnapshot>>>,
        session_telemetry: SessionTelemetry,
    ) {
        Self::spawn_snapshot_task(
            codex_home,
...
fn strip_snapshot_preamble(snapshot: REDACTED_SECRET) -> Result<String> {
...
async fn run_shell_script(shell: &Shell, script: REDACTED_SECRET, cwd: &Path) -> Result<String> {
    run_script_with_timeout(
        shell,
        script,
        SNAPSHOT_TIMEOUT,
...
async fn run_script_with_timeout(
    ...
    let mut handler = Command:REDACTED_SECRET&args[0]);
    handler.args(&args[1..]);
...
fn snapshot_session_id_from_file_name(file_name: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
