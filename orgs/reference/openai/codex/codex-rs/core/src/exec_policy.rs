...
const PROMPT_CONFLICT_REASON: REDACTED_SECRET =
    "approval required by policy, but AskForApproval is set to Never";
const REJECT_SANDBOX_APPROVAL_REASON: REDACTED_SECRET =
    "approval required by policy, but AskForApproval::Granular.sandbox_approval is false";
const REJECT_RULES_APPROVAL_REASON: REDACTED_SECRET =
    "approval required by policy rule, but AskForApproval::Granular.rules is false";
const RULES_DIR_NAME: REDACTED_SECRET = "rules";
const RULE_EXTENSION: REDACTED_SECRET = "rules";
const DEFAULT_POLICY_FILE: REDACTED_SECRET = "default.rules";
static BANNED_PREFIX_SUGGESTIONS: &[&[REDACTED_SECRET]] = &[
    &["python3"],
    &["python3", "-"],
    &["python3", "-c"],
    &["python"],
...
impl ExecPolicyManager {
    pub(crate) fn new(policy: Arc<Policy>) -> Self {
        Self {
            ...
            update_lock: tokio::sync::Mutex:REDACTED_SECRET()),
        }
    }
    ...
    pub(crate) async fn load(config_stack: &ConfigLayerStack) -> Result<Self, ExecPolicyError> {
        ...
        Ok(Self:REDACTED_SECRETArc:REDACTED_SECRETpolicy)))
    }
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        self.policy.store(Arc:REDACTED_SECRETupdated_policy));
        Ok(())
    }
...
impl Default for ExecPolicyManager {
    fn default() -> Self {
        Self:REDACTED_SECRETArc:REDACTED_SECRETPolicy::empty()))
    }
}
...
fn parse_starlark_line_from_message(message: REDACTED_SECRET) -> Option<(PathBuf, usize)> {
...
pub async fn load_exec_policy(config_stack: &ConfigLayerStack) -> Result<Policy, ExecPolicyError> {
    ...
    let mut policy_paths = Vec:REDACTED_SECRET);
...
fn derive_requested_execpolicy_amendment_from_prefix_rule(
    ...
    let amendment = ExecPolicyAmendment:REDACTED_SECRETprefix_rule.clone());
...
async fn collect_policy_files(dir: impl AsRef<Path>) -> Result<Vec<PathBuf>, ExecPolicyError> {
    ...
    let mut read_dir = match fs::read_dir(dir).await {
        ...
        Err(err) if err.kind() == ErrorKind::NotFound => return Ok(Vec:REDACTED_SECRET)),
        Err(source) => {
