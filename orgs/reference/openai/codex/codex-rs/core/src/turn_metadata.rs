...
struct WorkspaceGitMetadata {
    associated_remote_urls: Option<REDACTED_SECRET, String>>,
...
pub(crate) struct TurnMetadataBag {
    ...
    workspaces: REDACTED_SECRET, TurnMetadataWorkspace>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
fn build_turn_metadata_bag(
    ...
    turn_id: Option<String>,
    sandbox: Option<String>,
    repo_REDACTED_SECRET: Option<String>,
    workspace_git_metadata: Option<WorkspaceGitMetadata>,
) -> TurnMetadataBag {
    let mut workspaces = BTreeMap:REDACTED_SECRET);
    if let (Some(repo_REDACTED_SECRET), Some(workspace_git_metadata)) = (repo_REDACTED_SECRET, workspace_git_metadata)
        && !workspace_git_metadata.is_empty()
    {
        workspaces.insert(repo_REDACTED_SECRET, workspace_git_metadata.into());
    }
...
pub async fn build_turn_metadata_header(cwd: &Path, sandbox: Option<REDACTED_SECRET>) -> Option<String> {
    let repo_REDACTED_SECRET = get_git_repo_REDACTED_SECRET(cwd).map(|REDACTED_SECRET| REDACTED_SECRET.to_string_lossy().into_owned());
...
impl TurnMetadataState {
    pub(crate) fn new(
        ...
        Self {
            ...
            base_metadata,
            base_header,
            enriched_header: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETNone)),
            enrichment_task: Arc:REDACTED_SECRETMutex:REDACTED_SECRETNone)),
        }
    }
    ...
    pub(crate) fn spawn_git_enrichment_task(&self) {
        if self.repo_REDACTED_SECRET.is_none() {
            return;
        }
        ...
        *task_guard = Some(tokio::spawn(async move {
            ...
            let Some(repo_REDACTED_SECRET) = state.repo_REDACTED_SECRET.clone() else {
                return;
            };
