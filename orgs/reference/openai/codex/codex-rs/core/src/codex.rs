...
use async_channel::REDACTED_SECRETer;
...
use crate::tools::js_repl::resolve_compatible_REDACTED_SECRET;
...
pub struct Codex {
    pub(crate) tx_sub: REDACTED_SECRETer<Submission>,
...
pub(crate) const INITIAL_SUBMIT_ID: REDACTED_SECRET = "";
pub(crate) const SUBMISSION_CHANNEL_CAPACITY: usize = 512;
const CYBER_VERIFY_URL: REDACTED_SECRET = "https://chatgpt.com/cyber";
const CYBER_SAFETY_URL: REDACTED_SECRET = "https://developers.openai.com/codex/concepts/cyber-safety";
...
impl Codex {
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        if config.features.enabled(Feature::JsRepl)
            && let Err(err) = resolve_compatible_REDACTED_SECRET(config.js_repl_REDACTED_SECRET_path.as_deref()).await
        {
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc:REDACTED_SECRETExecPolicyManager::default())
        ...
        let config = Arc:REDACTED_SECRETconfig);
        ...
        let session = Session:REDACTED_SECRET
    ...
    pub async fn steer_input(
        &self,
        input: Vec<UserInput>,
        expected_turn_id: Option<REDACTED_SECRET>,
...
pub(crate) struct Session {
    ...
    tx_event: REDACTED_SECRETer<Event>,
    agent_status: watch::REDACTED_SECRETer<AgentStatus>,
    out_of_band_elicitation_paused: watch::REDACTED_SECRETer<bool>,
...
impl TurnSkillsContext {
    pub(crate) fn new(outcome: Arc<SkillLoadOutcome>) -> Self {
        Self {
            outcome,
            implicit_invocation_seen_skills: Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashSet:REDACTED_SECRET))),
        }
    }
}
...
impl TurnContext {
    ...
    pub(crate) async fn with_model(&self, model: String, models_manager: &ModelsManager) -> Self {
        ...
        let tools_config = ToolsConfig:REDACTED_SECRET&ToolsConfigParams {
        ...
        Self {
            ...
            config: Arc:REDACTED_SECRETconfig),
            ...
            tool_call_gate: Arc:REDACTED_SECRETReadinessFlag:REDACTED_SECRET)),
            truncation_policy,
    ...
    pub(crate) fn compact_prompt(&self) -> REDACTED_SECRET {
        self.compact_prompt
            .as_deref()
...
pub(crate) struct SessionConfiguration {
    ...
    /// Absolute working directory that should be treated as the *REDACTED_SECRET* of the
...
impl Session {
    ...
    fn make_turn_context(
        ...
        let per_turn_config = Arc:REDACTED_SECRETper_turn_config);
        let turn_metadata_state = Arc:REDACTED_SECRETTurnMetadataState:REDACTED_SECRET
        ...
        TurnContext {
            ...
            turn_metadata_state,
            turn_skills: TurnSkillsContext:REDACTED_SECRETskills_outcome),
            turn_timing_state: Arc:REDACTED_SECRETTurnTimingState::default()),
        }
    }
    ...
    async fn new(
        ...
        let (conversation_id, rollout_params) = match &initial_history {
            InitialHistory::New | InitialHistory::Forked(_) => {
                ...
                (
                    conversation_id,
                    RolloutRecorderParams:REDACTED_SECRET
                        conversation_id,
                        forked_from_id,
                        session_source,
                        BaseInstructions {
        ...
        let rollout_fut = async {
            if config.ephemeral {
                ...
                let rollout_recorder = RolloutRecorder:REDACTED_SECRET
                    &config,
                    rollout_params,
                    state_db_ctx.clone(),
                    state_builder.clone(),
        ...
        let mut post_session_configured_events = Vec::<Event>:REDACTED_SECRET);
        ...
        let mut session_telemetry = SessionTelemetry:REDACTED_SECRET
            conversation_id,
        ...
        session_telemetry.counter(
            ...
            &[(
                "is_git",
                if get_git_repo_REDACTED_SECRET(&session_configuration.cwd).is_some() {
                    "true"
                } else {
                    "false"
                },
        ...
        let state = SessionState:REDACTED_SECRETsession_configuration.clone());
        let managed_network_requirements_enabled = config.managed_network_requirements_enabled();
        let network_approval = Arc:REDACTED_SECRETNetworkApprovalService::default());
        ...
        let network_policy_decider_session = if managed_network_requirements_enabled {
            config
                ...
                .network
                .as_ref()
                .map(|_| Arc:REDACTED_SECRETRwLock:REDACTED_SECRETstd::sync::Weak::<Session>:REDACTED_SECRET))))
        } else {
            None
        };
        ...
        let hooks = Hooks:REDACTED_SECRETHooksConfig {
        ...
        let services = SessionServices {
            ...
            // McpConnectionManager:REDACTED_SECRET) once all its constructor args are
            ...
            mcp_connection_manager: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETMcpConnectionManager::new_uninitialized(
                &config.permissions.approval_policy,
            ))),
            mcp_startup_cancellation_token: Mutex:REDACTED_SECRETCancellationToken:REDACTED_SECRET)),
            unified_exec_manager: UnifiedExecProcessManager:REDACTED_SECRET
                config.background_terminal_max_timeout,
            ),
            shell_zsh_path: config.zsh_path.clone(),
            main_execve_wrapper_exe: config.main_execve_wrapper_exe.clone(),
            analytics_events_client: AnalyticsEventsClient:REDACTED_SECRET
                Arc::clone(&auth_manager),
                config.chatgpt_base_url.trim_end_matches('/').to_string(),
                config.analytics_enabled,
            ),
            hooks,
            rollout: Mutex:REDACTED_SECRETrollout_recorder),
            user_shell: Arc:REDACTED_SECRETdefault_shell),
            shell_snapshot_tx,
            ...
            tool_approvals: Mutex:REDACTED_SECRETApprovalStore::default()),
            skills_manager,
            ...
            model_client: ModelClient:REDACTED_SECRET
            ...
            code_mode_service: crate::tools::code_mode::CodeModeService:REDACTED_SECRET
                config.js_repl_REDACTED_SECRET_path.clone(),
            ),
        ...
        let js_repl = Arc:REDACTED_SECRETJsReplHandle::with_REDACTED_SECRET_path(
            config.js_repl_REDACTED_SECRET_path.clone(),
            config.js_repl_REDACTED_SECRET_module_dirs.clone(),
        ));
        let (out_of_band_elicitation_paused, _out_of_band_elicitation_paused_rx) =
            watch::channel(false);

        let (mailbox, mailbox_rx) = Mailbox:REDACTED_SECRET);
        let sess = Arc:REDACTED_SECRETSession {
            conversation_id,
            tx_event: tx_event.clone(),
            agent_status,
            out_of_band_elicitation_paused,
            state: Mutex:REDACTED_SECRETstate),
            features: config.features.clone(),
            pending_mcp_server_refresh_config: Mutex:REDACTED_SECRETNone),
            conversation: Arc:REDACTED_SECRETRealtimeConversationManager:REDACTED_SECRET)),
            active_turn: Mutex:REDACTED_SECRETNone),
            mailbox,
            mailbox_rx: Mutex:REDACTED_SECRETmailbox_rx),
            idle_pending_input: Mutex:REDACTED_SECRETVec:REDACTED_SECRET)),
            guardian_review_session: GuardianReviewSessionManager::default(),
            services,
            js_repl,
            next_internal_sub_id: AtomicU64:REDACTED_SECRET0),
        });
        ...
        let tool_plugin_provenance = mcp_manager.tool_plugin_provenance(config.as_ref());
        {
            ...
            cancel_guard.cancel();
            *cancel_guard = CancellationToken:REDACTED_SECRET);
        }
        let (mcp_connection_manager, cancel_token) = McpConnectionManager:REDACTED_SECRET
            &mcp_servers,
    ...
    pub(crate) fn get_tx_event(&self) -> REDACTED_SECRETer<Event> {
        self.tx_event.clone()
    }
    ...
    async fn record_initial_history(&self, conversation_history: InitialHistory) {
        ...
        match conversation_history {
            ...
            InitialHistory::Resumed(resumed_history) => {
                ...
                let curr: REDACTED_SECRET = turn_context.model_info.slug.as_str();
    ...
    async fn new_turn_from_configuration(
        ...
        let effective_skill_REDACTED_SECRETs = plugin_outcome.effective_skill_REDACTED_SECRETs();
        let skills_input = skills_load_input_from_config(&per_turn_config, effective_skill_REDACTED_SECRETs);
        let skills_outcome = Arc:REDACTED_SECRET
            self.services
                .skills_manager
        ...
        let turn_context = Arc:REDACTED_SECRETturn_context);
    ...
    pub(crate) async fn reload_user_config_layer(&self) {
        ...
        state.session_configuration.original_config_do_not_use = Arc:REDACTED_SECRETconfig);
    ...
    /// REDACTED_SECRETs the standard completion envelope from a spawned MultiAgentV2 child to its parent.
    async fn forward_child_completion_to_parent(
        ...
        let communication = InterAgentCommunication:REDACTED_SECRET
    ...
    pub(crate) async fn turn_context_for_sub_id(&self, sub_id: REDACTED_SECRET) -> Option<Arc<TurnContext>> {
    ...
    pub(crate) async fn record_execpolicy_amendment_message(
        ...
        let message: ResponseItem = DeveloperInstructions:REDACTED_SECRETtext.clone()).into();
    ...
    pub async fn request_patch_approval(
        ...
        reason: Option<String>,
        grant_REDACTED_SECRET: Option<PathBuf>,
    ...
    pub async fn notify_dynamic_tool_response(&self, call_id: REDACTED_SECRET, response: DynamicToolResponse) {
        let entry = {
            let mut active = self.active_turn.lock().await;
            match active.as_mut() {
                Some(at) => {
    ...
    pub async fn notify_approval(&self, approval_id: REDACTED_SECRET, decision: ReviewDecision) {
        let entry = {
            let mut active = self.active_turn.lock().await;
            match active.as_mut() {
                Some(at) => {
    ...
    pub(crate) async fn record_user_prompt_and_emit_turn_item(
        ...
        self.record_conversation_items(turn_context, std::slice::from_ref(&response_item))
            .await;
        let turn_item = TurnItem::UserMessage(UserMessageItem:REDACTED_SECRETinput));
    ...
    async fn maybe_start_ghost_snapshot(
        ...
        let task = GhostSnapshotTask:REDACTED_SECRETtoken);
        Arc:REDACTED_SECRETtask)
            .run(
                Arc:REDACTED_SECRETSessionTaskContext:REDACTED_SECRETself.clone())),
                turn_context.clone(),
                Vec:REDACTED_SECRET),
                cancellation_token,
            )
    ...
    pub(crate) async fn defer_mailbox_delivery_to_next_turn(&self, sub_id: REDACTED_SECRET) {
    ...
    pub(crate) async fn accept_mailbox_delivery_for_current_turn(&self, sub_id: REDACTED_SECRET) {
        let turn_state = self.turn_state_for_sub_id(sub_id).await;
        let Some(turn_state) = turn_state else {
            return;
        };
    ...
    async fn refresh_mcp_servers_inner(
        ...
        };
        {
            ...
            guard.cancel();
            *guard = CancellationToken:REDACTED_SECRET);
        }
        let (refreshed_manager, cancel_token) = McpConnectionManager:REDACTED_SECRET
            &mcp_servers,
            store_mode,
            auth_statuses,
...
mod handlers {
            ...
            .await
        {
            ...
            Err(SteerInputError::NoActiveTurn(items)) => {
                ...
                sess.spawn_task(
                    ...
                    items,
                    crate::tasks::RegularTask:REDACTED_SECRET),
                )
                .await;
            }
            Err(err) => {
    ...
    pub async fn run_user_shell_command(sess: &Arc<Session>, sub_id: String, command: String) {
        ...
        sess.spawn_task(
            ...
            Vec:REDACTED_SECRET),
            UserShellCommandTask:REDACTED_SECRETcommand),
        )
        .await;
    }
    ...
    pub async fn list_skills(
        ...
        let mut skills = Vec:REDACTED_SECRET);
        ...
        for cwd in cwds {
            ...
            let effective_skill_REDACTED_SECRETs = plugins_manager.effective_skill_REDACTED_SECRETs_for_layer_stack(
                &config_layer_stack,
                config.features.enabled(Feature::Plugins),
            );
            let skills_input = crate::SkillsLoadInput:REDACTED_SECRET
                cwd.clone(),
    ...
    pub async fn undo(sess: &Arc<Session>, sub_id: String) {
        let turn_context = sess.new_default_turn_with_sub_id(sub_id).await;
        sess.spawn_task(turn_context, Vec:REDACTED_SECRET), UndoTask:REDACTED_SECRET))
            .await;
    }
    ...
    pub async fn drop_memories(sess: &Arc<Session>, config: &Arc<Config>, sub_id: String) {
        let mut errors = Vec:REDACTED_SECRET);
        ...
        let memory_REDACTED_SECRET = crate::memories::memory_REDACTED_SECRET(&config.codex_home);
        if let Err(err) = crate::memories::clear_memory_REDACTED_SECRET_contents(&memory_REDACTED_SECRET).await {
            errors.push(format!(
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        truncation_policy: model_info.truncation_policy.into(),
        turn_metadata_state,
        turn_skills: TurnSkillsContext:REDACTED_SECRETparent_turn_context.turn_skills.outcome.clone()),
        turn_timing_state: Arc:REDACTED_SECRETTurnTimingState::default()),
    };
    ...
    let tc = Arc:REDACTED_SECRETreview_turn_context);
    tc.turn_metadata_state.spawn_git_enrichment_task();
    // TODO(ccunningham): Review turns currently rely on `spawn_task` for TurnComplete but do not
    // emit a parent TurnStarted. Consider giving review a full parent turn lifecycle
    // (TurnStarted + TurnComplete) for consistency with other standalone tasks.
    sess.spawn_task(tc.clone(), input, ReviewTask:REDACTED_SECRET)).await;
...
pub(crate) async fn run_turn(
    ...
    let mcp_tools = if turn_context.apps_enabled() || !mentioned_plugins.is_empty() {
            ...
            .await
        {
            ...
            Err(_) => HashMap:REDACTED_SECRET),
        }
    } else {
        HashMap:REDACTED_SECRET)
    };
    ...
    let connector_names_by_id = available_connectors
        ...
        .collect::<HashMap<REDACTED_SECRET, REDACTED_SECRET>>();
    ...
    let additional_contexts = if input.is_empty() {
        ...
        let user_prompt_submit_outcome = run_user_prompt_submit_hooks(
            &sess,
            &turn_context,
            UserMessageItem:REDACTED_SECRET&input).message(),
        )
        .await;
    ...
    let turn_diff_tracker = Arc:REDACTED_SECRETtokio::sync::Mutex:REDACTED_SECRETTurnDiffTracker:REDACTED_SECRET)));
    ...
    loop {
        ...
        let mut blocked_pending_input_contexts = Vec:REDACTED_SECRET);
        let mut requeued_pending_input = false;
        let mut accepted_pending_input = Vec:REDACTED_SECRET);
        if !pending_input.is_empty() {
...
async fn maybe_run_previous_model_inline_compact(
    ...
    let previous_model_turn_context = Arc:REDACTED_SECRET
        turn_context
...
fn filter_codex_apps_mcp_tools(
    ...
    let allowed: HashSet<REDACTED_SECRET> = connectors
        .iter()
...
fn codex_apps_connector_id(tool: &McpToolInfo) -> Option<REDACTED_SECRET> {
...
async fn run_sampling_request(
    ...
    turn_metadata_header: Option<REDACTED_SECRET>,
    input: Vec<ResponseItem>,
    explicitly_enabled_connectors: &HashSet<String>,
    ...
    let tool_runtime = ToolCallRuntime:REDACTED_SECRET
        Arc::clone(&router),
        Arc::clone(&sess),
        Arc::clone(&turn_context),
        Arc::clone(&turn_diff_tracker),
...
pub(crate) async fn built_tools(
    ...
    Ok(Arc:REDACTED_SECRETToolRouter::from_config(
...
impl PlanModeStreamState {
    fn new(turn_id: REDACTED_SECRET) -> Self {
        Self {
            pending_agent_message_items: HashMap:REDACTED_SECRET),
            started_agent_message_items: HashSet:REDACTED_SECRET),
            leading_whitespace_by_item: HashMap:REDACTED_SECRET),
            plan_item_state: ProposedPlanItemState:REDACTED_SECRETturn_id),
        }
    }
}
...
impl AssistantMessageStreamParsers {
    fn new(plan_mode: bool) -> Self {
        Self {
            plan_mode,
            parsers_by_item: HashMap:REDACTED_SECRET),
        }
    }

    fn parser_mut(&mut self, item_id: REDACTED_SECRET) -> &mut AssistantTextStreamParser {
        let plan_mode = self.plan_mode;
        self.parsers_by_item
            .entry(item_id.to_string())
            .or_insert_with(|| AssistantTextStreamParser:REDACTED_SECRETplan_mode))
    }

    fn seed_item_text(&mut self, item_id: REDACTED_SECRET, text: REDACTED_SECRET) -> ParsedAssistantTextDelta {
        if text.is_empty() {
    ...
    fn parse_delta(&mut self, item_id: REDACTED_SECRET, delta: REDACTED_SECRET) -> ParsedAssistantTextDelta {
        self.parser_mut(item_id).push_str(delta)
    }

    fn finish_item(&mut self, item_id: REDACTED_SECRET) -> ParsedAssistantTextDelta {
...
impl ProposedPlanItemState {
    ...
    async fn push_delta(&mut self, sess: &Session, turn_context: &TurnContext, delta: REDACTED_SECRET) {
        if self.completed {
            return;
        }
        if delta.is_empty() {
...
async fn maybe_complete_plan_item_from_message(
    ...
    if let ResponseItem::Message { role, content, .. } = item
        && role == "assistant"
    {
        let mut text = String:REDACTED_SECRET);
...
async fn try_run_sampling_request(
    ...
    let mut in_flight: FuturesOrdered<BoxFuture<'static, CodexResult<ResponseInputItem>>> =
        FuturesOrdered:REDACTED_SECRET);
    let mut needs_follow_up = false;
    let mut last_agent_message: Option<String> = None;
    let mut active_item: Option<TurnItem> = None;
    let mut should_emit_turn_diff = false;
    let plan_mode = turn_context.collaboration_mode.mode == ModeKind::Plan;
    let mut assistant_message_stream_parsers = AssistantMessageStreamParsers:REDACTED_SECRETplan_mode);
    let mut plan_mode_state = plan_mode.then(|| PlanModeStreamState:REDACTED_SECRET&turn_context.sub_id));
        ...
        match event {
            ...
            ResponseEvent::OutputItemDone(item) => {
                ...
                let preempt_for_mailbox_mail = match &item {
                    ...
                    ResponseItem:REDACTED_SECRETShellCall { .. }
