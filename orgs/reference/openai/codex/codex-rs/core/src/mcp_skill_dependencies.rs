...
const SKILL_MCP_DEPENDENCY_PROMPT_ID: REDACTED_SECRET = "skill_mcp_dependency_install";
const MCP_DEPENDENCY_OPTION_INSTALL: REDACTED_SECRET = "Install";
const MCP_DEPENDENCY_OPTION_SKIP: REDACTED_SECRET = "Continue anyway";
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut added = Vec:REDACTED_SECRET);
    ...
    if let Err(err) = ConfigEditsBuilder:REDACTED_SECRET&codex_home)
...
fn canonical_mcp_key(transport: REDACTED_SECRET, identifier: REDACTED_SECRET, fallback: REDACTED_SECRET) -> String {
...
fn canonical_mcp_server_key(name: REDACTED_SECRET, config: &McpServerConfig) -> String {
...
fn collect_missing_mcp_dependencies(
    ...
    let mut missing = HashMap:REDACTED_SECRET);
    let installed_keys: HashSet<String> = installed
        .iter()
        .map(|(name, config)| canonical_mcp_server_key(name, config))
        .collect();
    let mut seen_canonical_keys = HashSet:REDACTED_SECRET);
