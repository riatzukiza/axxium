...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: REDACTED_SECRET = "codex.external_agent_config.detect";
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: REDACTED_SECRET = "codex.external_agent_config.import";
...
impl ExternalAgentConfigService {
    ...
    pub fn detect(
        ...
        if params.include_home {
            self.detect_migrations(/*repo_REDACTED_SECRET*/ None, &mut items)?;
        }

        for cwd in params.cwds.as_deref().unwrap_or(&[]) {
            let Some(repo_REDACTED_SECRET) = find_repo_REDACTED_SECRET(Some(cwd))? else {
                continue;
            };
            self.detect_migrations(Some(&repo_REDACTED_SECRET), &mut items)?;
        }
    ...
    fn detect_migrations(
        &self,
        repo_REDACTED_SECRET: Option<&Path>,
        items: &mut Vec<ExternalAgentConfigMigrationItem>,
    ) -> io::Result<()> {
        let cwd = repo_REDACTED_SECRET.map(Path::to_path_buf);
        let source_settings = repo_REDACTED_SECRET.map_or_else(
            || self.claude_home.join("settings.json"),
            |repo_REDACTED_SECRET| repo_REDACTED_SECRET.join(".claude").join("settings.json"),
        );
        let target_config = repo_REDACTED_SECRET.map_or_else(
            || self.codex_home.join("config.toml"),
            |repo_REDACTED_SECRET| repo_REDACTED_SECRET.join(".codex").join("config.toml"),
        );
        ...
        let source_skills = repo_REDACTED_SECRET.map_or_else(
            || self.claude_home.join("skills"),
            |repo_REDACTED_SECRET| repo_REDACTED_SECRET.join(".claude").join("skills"),
        );
        let target_skills = repo_REDACTED_SECRET.map_or_else(
            || self.home_target_skills_dir(),
            |repo_REDACTED_SECRET| repo_REDACTED_SECRET.join(".agents").join("skills"),
        );
        ...
        let source_agents_md = if let Some(repo_REDACTED_SECRET) = repo_REDACTED_SECRET {
            find_repo_agents_md_source(repo_REDACTED_SECRET)?
        } else {
            let path = self.claude_home.join("CLAUDE.md");
            is_non_empty_text_file(&path)?.then_some(path)
        };
        let target_agents_md = repo_REDACTED_SECRET.map_or_else(
            || self.codex_home.join("AGENTS.md"),
            |repo_REDACTED_SECRET| repo_REDACTED_SECRET.join("AGENTS.md"),
        );
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<()> {
        let (source_settings, target_config) = if let Some(repo_REDACTED_SECRET) = find_repo_REDACTED_SECRET(cwd)? {
            (
                repo_REDACTED_SECRET.join(".claude").join("settings.json"),
                repo_REDACTED_SECRET.join(".codex").join("config.toml"),
            )
    ...
    fn import_skills(&self, cwd: Option<&Path>) -> io::Result<usize> {
        let (source_skills, target_skills) = if let Some(repo_REDACTED_SECRET) = find_repo_REDACTED_SECRET(cwd)? {
            (
                repo_REDACTED_SECRET.join(".claude").join("skills"),
                repo_REDACTED_SECRET.join(".agents").join("skills"),
            )
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(0);
        } else {
    ...
    fn import_agents_md(&self, cwd: Option<&Path>) -> io::Result<()> {
        let (source_agents_md, target_agents_md) = if let Some(repo_REDACTED_SECRET) = find_repo_REDACTED_SECRET(cwd)? {
            let Some(source_agents_md) = find_repo_agents_md_source(&repo_REDACTED_SECRET)? else {
                return Ok(());
            };
            (source_agents_md, repo_REDACTED_SECRET.join("AGENTS.md"))
        } else if cwd.is_some_and(|cwd| !cwd.as_os_str().is_empty()) {
            return Ok(());
        } else {
            (
...
fn find_repo_REDACTED_SECRET(cwd: Option<&Path>) -> io::Result<Option<PathBuf>> {
...
fn collect_subdirectory_names(path: &Path) -> io::Result<HashSet<OsString>> {
    let mut names = HashSet:REDACTED_SECRET);
    if !path.is_dir() {
        return Ok(names);
    }
...
fn find_repo_agents_md_source(repo_REDACTED_SECRET: &Path) -> io::Result<Option<PathBuf>> {
    for candidate in [
...
fn rewrite_claude_terms(content: REDACTED_SECRET) -> String {
...
fn build_config_from_external(settings: &JsonValue) -> io::Result<TomlValue> {
    let Some(settings_obj) = settings.as_object() else {
        return Err(invalid_data_error(
            "external agent settings REDACTED_SECRET must be an object",
        ));
    };
    ...
    if let Some(env) = settings_obj.get("env").and_then(JsonValue::as_object)
        && !env.is_empty()
    {
        let mut shell_policy = toml::map::Map:REDACTED_SECRET);
    ...
    Ok(TomlValue::Table(REDACTED_SECRET))
}
...
fn invalid_data_error(message: impl Into<String>) -> io::Error {
    io::Error:REDACTED_SECRETio::ErrorKind::InvalidData, message.into())
}
