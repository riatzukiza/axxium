...
fn user_message(text: REDACTED_SECRET) -> ResponseItem {
...
fn assistant_message(text: REDACTED_SECRET) -> ResponseItem {
    ResponseItem::Message {
        id: None,
...
fn skill_message(text: REDACTED_SECRET) -> ResponseItem {
    ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![ContentItem::InputText {
...
async fn regular_turn_emits_turn_started_without_waiting_for_startup_prewarm() {
    ...
    sess.set_session_startup_prewarm(
        crate::session_startup_prewarm::SessionStartupPrewarmHandle:REDACTED_SECRET
            handle,
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    crate::client::ModelClient:REDACTED_SECRET
        /*auth_manager*/ None,
...
fn developer_input_texts(items: &[ResponseItem]) -> Vec<REDACTED_SECRET> {
    items
        .iter()
...
fn user_input_texts(items: &[ResponseItem]) -> Vec<REDACTED_SECRET> {
    items
        .iter()
        .filter_map(|item| match item {
...
fn test_tool_runtime(session: Arc<Session>, turn_context: Arc<TurnContext>) -> ToolCallRuntime {
    let router = Arc:REDACTED_SECRETToolRouter::from_config(
    ...
    let tracker = Arc:REDACTED_SECRETtokio::sync::Mutex:REDACTED_SECRETTurnDiffTracker:REDACTED_SECRET)));
    ToolCallRuntime:REDACTED_SECRETrouter, session, turn_context, tracker)
}

fn make_connector(id: REDACTED_SECRET, name: REDACTED_SECRET) -> AppInfo {
    AppInfo {
        id: id.to_string(),
        name: name.to_string(),
        description: None,
...
fn assistant_message_stream_parsers_can_be_seeded_from_output_item_added_text() {
    let mut parsers = AssistantMessageStreamParsers:REDACTED_SECRET/*plan_mode*/ false);
    let item_id = "msg-1";
...
fn assistant_message_stream_parsers_seed_plan_parser_across_added_and_delta_boundaries() {
    let mut parsers = AssistantMessageStreamParsers:REDACTED_SECRET/*plan_mode*/ true);
    let item_id = "msg-1";
...
fn make_mcp_tool(
    server_name: REDACTED_SECRET,
    tool_name: REDACTED_SECRET,
    connector_id: Option<REDACTED_SECRET>,
    connector_name: Option<REDACTED_SECRET>,
) -> ToolInfo {
    ...
    ToolInfo {
        ...
        tool: Tool {
            ...
            input_schema: Arc:REDACTED_SECRETJsonObject::default()),
            output_schema: None,
            annotations: None,
            execution: None,
            icons: None,
...
async fn get_base_instructions_no_user_content() {
    ...
    let model_info_for_slug = |slug: REDACTED_SECRET, config: &Config| {
...
fn filter_connectors_for_input_skips_duplicate_slug_mentions() {
    ...
    let explicitly_enabled_connectors = HashSet:REDACTED_SECRET);
    let skill_name_counts_lower = HashMap:REDACTED_SECRET);
    ...
    assert_eq!(selected, Vec:REDACTED_SECRET));
}
...
fn collect_explicit_app_ids_from_skill_items_includes_linked_mentions() {
    ...
    let connector_ids =
        collect_explicit_app_ids_from_skill_items(&skill_items, &connectors, &HashMap:REDACTED_SECRET));
...
fn collect_explicit_app_ids_from_skill_items_skips_plain_mentions_with_skill_conflicts() {
    ...
    assert_eq!(connector_ids, HashSet::<String>:REDACTED_SECRET));
}
...
async fn reconstruct_history_uses_replacement_history_verbatim() {
    ...
    let rollout_items = vec![RolloutItem::Compacted(CompactedItem {
        message: String:REDACTED_SECRET),
...
async fn record_initial_history_new_defers_initial_context_until_first_turn() {
    ...
    assert_eq!(history.raw_items().to_vec(), Vec::<ResponseItem>:REDACTED_SECRET));
...
async fn thread_rollback_drops_last_turn_from_history() {
    ...
    let mut full_history = Vec:REDACTED_SECRET);
    ...
    let mut expected = Vec:REDACTED_SECRET);
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let session_configuration = SessionConfiguration {
        ...
        dynamic_tools: Vec:REDACTED_SECRET),
        persist_extended_history: false,
        inherited_shell_snapshot: None,
        user_shell_override: None,
    };

    let mut state = SessionState:REDACTED_SECRETsession_configuration);
...
async fn includes_timed_out_message() {
    let exec = ExecToolCallOutput {
        exit_code: 0,
        stdout: StreamOutput:REDACTED_SECRETString:REDACTED_SECRET)),
        stderr: StreamOutput:REDACTED_SECRETString:REDACTED_SECRET)),
        aggregated_output: StreamOutput:REDACTED_SECRET"Command output".to_string()),
...
async fn attach_rollout_recorder(session: &Arc<Session>) -> PathBuf {
    let config = session.get_config().await;
    let recorder = RolloutRecorder:REDACTED_SECRET
        config.as_ref(),
...
fn text_block(s: REDACTED_SECRET) -> serde_json::Value {
    json!({
        "type": "text",
        "text": s,
    })
...
async fn session_configuration_apply_preserves_split_file_system_policy_on_cwd_only_update() {
    ...
    let project_REDACTED_SECRET = workspace.path().join("project");
    let original_cwd = project_REDACTED_SECRET.join("subdir");
    ...
    session_configuration.sandbox_policy =
        codex_config::Constrained::allow_any(SandboxPolicy::WorkspaceWrite {
            writable_REDACTED_SECRETs: Vec:REDACTED_SECRET),
            read_only_access: ReadOnlyAccess::Restricted {
                include_platform_defaults: true,
                readable_REDACTED_SECRETs: vec![docs_dir.clone()],
            },
    ...
    let updated = session_configuration
        .apply(&SessionSettingsUpdate {
            cwd: Some(project_REDACTED_SECRET),
            ..Default::default()
        })
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let parent_outcome = session
        ...
        .skills_manager
        .skills_for_cwd(
            &crate::skills_load_input_from_config(&parent_config, Vec:REDACTED_SECRET)),
            /*force_reload*/ true,
        )
        .await;
...
"#,
    ...
    {
        let mut state = session.state.lock().await;
        state.session_configuration.original_config_do_not_use = Arc:REDACTED_SECRETchild_config);
    }
...
async fn session_configuration_apply_rederives_legacy_file_system_policy_on_cwd_update() {
    ...
    session_configuration.sandbox_policy =
        codex_config::Constrained::allow_any(SandboxPolicy::WorkspaceWrite {
            ...
            read_only_access: ReadOnlyAccess::Restricted {
                include_platform_defaults: true,
                readable_REDACTED_SECRETs: vec![docs_dir],
            },
            network_access: false,
            exclude_tmpdir_env_var: true,
            exclude_slash_tmp: true,
    ...
    let updated = session_configuration
        .apply(&SessionSettingsUpdate {
            cwd: Some(project_REDACTED_SECRET.clone()),
            ..Default::default()
        })
        .expect("cwd-only update should succeed");
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    ...
    let models_manager = Arc:REDACTED_SECRETModelsManager:REDACTED_SECRET
    ...
    let plugins_manager = Arc:REDACTED_SECRETPluginsManager:REDACTED_SECRETconfig.codex_home.clone()));
    let mcp_manager = Arc:REDACTED_SECRETMcpManager:REDACTED_SECRETArc::clone(&plugins_manager)));
    let skills_manager = Arc:REDACTED_SECRETSkillsManager:REDACTED_SECRET
        config.codex_home.clone(),
        /*bundled_skills_enabled*/ true,
    ));
    let result = Session:REDACTED_SECRET
        ...
        InitialHistory::New,
        SessionSource::Exec,
        Arc:REDACTED_SECRETcodex_exec_server::EnvironmentManager:REDACTED_SECRET
            /*exec_server_url*/ None,
        )),
        skills_manager,
        plugins_manager,
        mcp_manager,
        Arc:REDACTED_SECRETSkillsWatcher::noop()),
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let exec_policy = Arc:REDACTED_SECRETExecPolicyManager::default());
    let (agent_status_tx, _agent_status_rx) = watch::channel(AgentStatus::PendingInit);
    let model = ModelsManager::get_model_offline_for_tests(config.model.as_deref());
    let model_info = ModelsManager::construct_model_info_offline_for_tests(
        model.as_str(),
    ...
    let environment = Arc:REDACTED_SECRET
        codex_exec_server::Environment::create(/*exec_server_url*/ None)
            .await
            .expect("create environment"),
    );

    let skills_watcher = Arc:REDACTED_SECRETSkillsWatcher::noop());
    let services = SessionServices {
        ...
        hooks: Hooks:REDACTED_SECRETHooksConfig {
            legacy_notify_argv: config.notify.clone(),
            ..HooksConfig::default()
        }),
        rollout: Mutex:REDACTED_SECRETNone),
        user_shell: Arc:REDACTED_SECRETdefault_user_shell()),
    ...
    let skills_input =
        crate::skills_load_input_from_config(&per_turn_config, effective_skill_REDACTED_SECRETs);
    let skills_outcome = Arc:REDACTED_SECRETservices.skills_manager.skills_for_config(&skills_input));
    ...
    let (mailbox, mailbox_rx) = crate::agent::Mailbox:REDACTED_SECRET);
    let session = Session {
        conversation_id,
        tx_event,
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let session = Arc:REDACTED_SECRETsession);
...
async fn submit_with_id_captures_current_span_trace_context() {
    ...
    let codex = Codex {
        ...
        rx_event,
        agent_status,
        session: Arc:REDACTED_SECRETsession),
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let (sess, tc, rx) = make_session_and_context_with_rx().await;
    let captured_trace = Arc:REDACTED_SECRETstd::sync::Mutex:REDACTED_SECRETNone));
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let codex = Arc:REDACTED_SECRETCodex {
        tx_sub,
        rx_event,
        agent_status,
        session: Arc:REDACTED_SECRETsession),
...
async fn shutdown_and_wait_shuts_down_cached_guardian_subagent() {
    ...
    let parent_session = Arc:REDACTED_SECRETparent_session);
    ...
    let child_codex = Codex {
        ...
        session: Arc:REDACTED_SECRETchild_session),
...
pub(crate) async fn make_session_and_context_with_dynamic_tools_and_rx(
    ...
    let skills_input =
        crate::skills_load_input_from_config(&per_turn_config, effective_skill_REDACTED_SECRETs);
    let skills_outcome = Arc:REDACTED_SECRETservices.skills_manager.skills_for_config(&skills_input));
    let turn_context = Arc:REDACTED_SECRETSession::make_turn_context(
        conversation_id,
        Some(Arc::clone(&auth_manager)),
        &session_telemetry,
        session_configuration.provider.clone(),
...
pub(crate) async fn make_session_and_context_with_rx() -> (
    ...
    make_session_and_context_with_dynamic_tools_and_rx(Vec:REDACTED_SECRET)).await
}
...
async fn build_settings_update_items_emits_environment_item_for_network_changes() {
    ...
    let previous_context = Arc:REDACTED_SECRETprevious_context);
    ...
    requirements.network = Some(Sourced:REDACTED_SECRET
        NetworkConstraints {
            domains: Some(NetworkDomainPermissionsToml {
                entries: std::collections::BTreeMap::from([
                    (
    ...
    let layers = config
        ...
        .cloned()
        .collect();
    config.config_layer_stack = ConfigLayerStack:REDACTED_SECRET
        layers,
        requirements,
        config.config_layer_stack.requirements_toml().clone(),
    )
    .expect("rebuild config layer stack with network requirements");
    current_context.config = Arc:REDACTED_SECRETconfig);
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        cancellation_token: CancellationToken:REDACTED_SECRET),
    };
    ...
    let save_message: ResponseItem = DeveloperInstructions:REDACTED_SECRETformat!(
        "Generated images are saved to {} as {} by default.",
        image_output_dir.display(),
        image_output_path.display(),
    ))
    .into();
    let copy_message: ResponseItem = DeveloperInstructions:REDACTED_SECRET
...
async fn record_context_updates_and_set_reference_context_item_persists_baseline_without_emitting_diffs()
 {
    ...
    let update_items = session
        ...
        .await;
    assert_eq!(update_items, Vec:REDACTED_SECRET));
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    assert!(matches!(
        ...
        EventMsg::UserMessage(UserMessageEvent {
            ...
            && images == Some(Vec:REDACTED_SECRET))
...
async fn queue_only_mailbox_mail_waits_for_next_turn_after_answer_boundary() {
    ...
    let communication = InterAgentCommunication:REDACTED_SECRET
        AgentPath::try_from("/REDACTED_SECRET/worker").expect("worker path should parse"),
        AgentPath::REDACTED_SECRET(),
        Vec:REDACTED_SECRET),
    ...
    assert_eq!(sess.get_pending_input().await, Vec:REDACTED_SECRET));
...
async fn trigger_turn_mailbox_mail_waits_for_next_turn_after_answer_boundary() {
    ...
    sess.defer_mailbox_delivery_to_next_turn(&tc.sub_id).await;
    sess.enqueue_mailbox_communication(InterAgentCommunication:REDACTED_SECRET
        AgentPath::try_from("/REDACTED_SECRET/worker").expect("worker path should parse"),
        AgentPath::REDACTED_SECRET(),
        Vec:REDACTED_SECRET),
...
async fn abort_review_task_emits_exited_then_aborted_and_records_history() {
    ...
    sess.spawn_task(Arc::clone(&tc), input, ReviewTask:REDACTED_SECRET))
        .await;
...
async fn sample_rollout(
    ...
    let mut rollout_items = Vec:REDACTED_SECRET);
    let mut live_history = ContextManager:REDACTED_SECRET);
    ...
    let rebuilt1 = compact::build_compacted_history(Vec:REDACTED_SECRET), &user_messages1, summary1);
    ...
    let rebuilt2 = compact::build_compacted_history(Vec:REDACTED_SECRET), &user_messages2, summary2);
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    turn_context_raw
        ...
        .expect("test setup should allow updating approval policy");
    let session = Arc:REDACTED_SECRETsession);
    let mut turn_context = Arc:REDACTED_SECRETturn_context_raw);
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    turn_context_raw
        ...
        .expect("test setup should allow updating approval policy");
    let session = Arc:REDACTED_SECRETsession);
    let turn_context = Arc:REDACTED_SECRETturn_context_raw);
    let tracker = Arc:REDACTED_SECRETtokio::sync::Mutex:REDACTED_SECRETTurnDiffTracker:REDACTED_SECRET)));
