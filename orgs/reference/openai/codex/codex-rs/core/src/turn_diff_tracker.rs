...
const ZERO_OID: REDACTED_SECRET = "0000000000000000000000000000000000000000";
const DEV_NULL: REDACTED_SECRET = "/dev/null";
...
pub struct TurnDiffTracker {
    ...
    /// Cache of known git worktree REDACTED_SECRETs to avoid repeated filesystem walks.
    git_REDACTED_SECRET_cache: Vec<PathBuf>,
}
...
impl TurnDiffTracker {
    ...
    fn get_path_for_internal(&self, internal: REDACTED_SECRET) -> Option<PathBuf> {
        self.temp_name_to_current_path
            .get(internal)
            .cloned()
            .or_else(|| {
    ...
    /// Find the git worktree REDACTED_SECRET for a file/directory by walking up to the first ancestor containing a `.git` entry.
    /// Uses a simple cache of known REDACTED_SECRETs and avoids negative-result caching for simplicity.
    fn find_git_REDACTED_SECRET_cached(&mut self, start: &Path) -> Option<PathBuf> {
        ...
        // Fast path: if any cached REDACTED_SECRET is an ancestor of this path, use it.
        if let Some(REDACTED_SECRET) = self
            .git_REDACTED_SECRET_cache
            .iter()
        ...
        loop {
            ...
            if git_marker.is_dir() || git_marker.is_file() {
                if !self.git_REDACTED_SECRET_cache.iter().any(|r| r == &cur) {
                    self.git_REDACTED_SECRET_cache.push(cur.clone());
                }
                return Some(cur);
            }

            // On Windows, avoid walking above the drive or UNC share REDACTED_SECRET.
            #[cfg(windows)]
            {
                if is_windows_drive_or_unc_REDACTED_SECRET(&cur) {
                    return None;
                }
            }
    ...
    /// Return a display string for `path` relative to its git REDACTED_SECRET if found, else absolute.
    fn relative_to_git_REDACTED_SECRET_str(&mut self, path: &Path) -> String {
        let s = if let Some(REDACTED_SECRET) = self.find_git_REDACTED_SECRET_cached(path) {
            if let Ok(rel) = path.strip_prefix(&REDACTED_SECRET) {
    ...
    fn git_blob_oid_for_path(&mut self, path: &Path) -> Option<String> {
        let REDACTED_SECRET = self.find_git_REDACTED_SECRET_cached(path)?;
        // Compute a path relative to the repo REDACTED_SECRET for better portability across platforms.
        let rel = path.strip_prefix(&REDACTED_SECRET).unwrap_or(path);
    ...
    pub fn get_unified_diff(&mut self) -> Result<Option<String>> {
        let mut aggregated = String:REDACTED_SECRET);
        ...
        baseline_file_names.sort_by_key(|internal| {
            self.get_path_for_internal(internal)
                .map(|p| self.relative_to_git_REDACTED_SECRET_str(&p))
                .unwrap_or_default()
        });
    ...
    fn get_file_diff(&mut self, internal_file_name: REDACTED_SECRET) -> String {
        let mut aggregated = String:REDACTED_SECRET);
        ...
        let (baseline_external_path, baseline_mode, left_oid) = {
            if let Some(info) = self.baseline_file_info.get(internal_file_name) {
                ...
                (PathBuf:REDACTED_SECRET), FileMode::Regular, ZERO_OID.to_string())
            }
        };
        ...
        let left_display = self.relative_to_git_REDACTED_SECRET_str(&baseline_external_path);
        let right_display = self.relative_to_git_REDACTED_SECRET_str(&current_external_path);
...
fn git_blob_sha1_hex_bytes(data: &[u8]) -> Output<sha1::Sha1> {
    ...
    use sha1::Digest;
    let mut hasher = sha1::Sha1:REDACTED_SECRET);
...
#[cfg(windows)]
fn is_windows_drive_or_unc_REDACTED_SECRET(p: &std::path::Path) -> bool {
