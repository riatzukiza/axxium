...
static ACCESSIBLE_CONNECTORS_CACHE: LazyLock<StdMutex<Option<CachedAccessibleConnectors>>> =
    LazyLock:REDACTED_SECRET|| StdMutex:REDACTED_SECRETNone));
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config.features.apps_enabled_for_auth(auth.as_ref()) {
        return Some(Vec:REDACTED_SECRET));
    }
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let mcp_servers = with_codex_apps_mcp(HashMap:REDACTED_SECRET), auth.as_ref(), &mcp_config);
...
fn filter_tool_suggest_discoverable_connectors(
    ...
    let accessible_connector_ids: HashSet<REDACTED_SECRET> = accessible_connectors
        .iter()
...
fn tool_suggest_connector_ids(config: &Config) -> HashSet<String> {
    let mut connector_ids = PluginsManager:REDACTED_SECRETconfig.codex_home.clone())
...
pub(crate) fn app_tool_policy(
    ...
    tool_title: Option<REDACTED_SECRET>,
...
const DISALLOWED_CONNECTOR_IDS: &[REDACTED_SECRET] = &[
...
const FIRST_PARTY_CHAT_DISALLOWED_CONNECTOR_IDS: &[REDACTED_SECRET] =
    &["connector_0f9c9d4592e54d0a9a12b3f44a1e2010"];
const DISALLOWED_CONNECTOR_PREFIX: REDACTED_SECRET = "connector_openai_";
...
fn filter_disallowed_connectors_for_originator(
    connectors: Vec<AppInfo>,
    originator_value: REDACTED_SECRET,
) -> Vec<AppInfo> {
    connectors
        .into_iter()
        .filter(|connector| {
...
fn is_connector_id_allowed_for_originator(connector_id: REDACTED_SECRET, originator_value: REDACTED_SECRET) -> bool {
...
fn app_is_enabled(apps_config: &AppsConfigToml, connector_id: Option<REDACTED_SECRET>) -> bool {
...
{
    let mut connectors: HashMap<String, (AppInfo, BTreeSet<String>)> = HashMap:REDACTED_SECRET);
...
fn normalize_connector_value(value: Option<REDACTED_SECRET>) -> Option<String> {
    value
        .map(str::trim)
...
pub fn connector_install_url(name: REDACTED_SECRET, connector_id: REDACTED_SECRET) -> String {
    let slug = sanitize_slug(name);
    format!("https://chatgpt.com/apps/{slug}/{connector_id}")
}

pub fn sanitize_name(name: REDACTED_SECRET) -> String {
    sanitize_slug(name).replace("-", "_")
}

fn sanitize_slug(name: REDACTED_SECRET) -> String {
...
fn format_connector_label(name: REDACTED_SECRET, _id: REDACTED_SECRET) -> String {
    name.to_string()
}
