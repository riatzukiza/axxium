...
fn make_exec_output(
    ...
    aggregated: REDACTED_SECRET,
) -> ExecToolCallOutput {
    ExecToolCallOutput {
        exit_code,
        stdout: StreamOutput:REDACTED_SECRETstdout.to_string()),
        stderr: StreamOutput:REDACTED_SECRETstderr.to_string()),
        aggregated_output: StreamOutput:REDACTED_SECRETaggregated.to_string()),
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    let temp_dir = tempfile::TempDir:REDACTED_SECRET).expect("tempdir");
...
#[test]
fn windows_restricted_token_rejects_REDACTED_SECRET_write_read_only_carveouts() {
    let temp_dir = tempfile::TempDir:REDACTED_SECRET).expect("tempdir");
    let docs = temp_dir.path().join("docs");
    std::fs::create_dir_all(&docs).expect("create docs");
    let policy = SandboxPolicy::WorkspaceWrite {
