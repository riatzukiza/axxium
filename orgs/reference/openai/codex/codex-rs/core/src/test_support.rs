...
static TEST_MODEL_PRESETS: Lazy<Vec<ModelPreset>> = Lazy:REDACTED_SECRET|| {
...
pub fn get_model_offline(model: Option<REDACTED_SECRET>) -> String {
    ModelsManager::get_model_offline_for_tests(model)
}

pub fn construct_model_info_offline(model: REDACTED_SECRET, config: &Config) -> ModelInfo {
