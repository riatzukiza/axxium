...
impl Shell {
    ...
    pub fn derive_exec_args(&self, command: REDACTED_SECRET, use_login_shell: bool) -> Vec<String> {
        match self.shell_type {
...
fn get_shell_path(
    ...
    binary_name: REDACTED_SECRET,
    fallback_paths: &[REDACTED_SECRET],
) -> Option<PathBuf> {
...
const ZSH_FALLBACK_PATHS: &[REDACTED_SECRET] = &["/bin/zsh"];
...
const BASH_FALLBACK_PATHS: &[REDACTED_SECRET] = &["/bin/bash"];
...
const SH_FALLBACK_PATHS: &[REDACTED_SECRET] = &["/bin/sh"];
...
#[cfg(windows)]
const PWSH_FALLBACK_PATHS: &[REDACTED_SECRET] = &[r#"C:\Program Files\PowerShell\7\pwsh.exe"#];
#[cfg(not(windows))]
const PWSH_FALLBACK_PATHS: &[REDACTED_SECRET] = &["/usr/local/bin/pwsh"];

#[cfg(windows)]
const POWERSHELL_FALLBACK_PATHS: &[REDACTED_SECRET] =
