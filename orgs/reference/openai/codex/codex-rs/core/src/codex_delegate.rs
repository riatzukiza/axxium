...
pub(crate) async fn run_codex_thread_interactive(
    ...
    let CodexSpawnOk { codex, .. } = Codex::spawn(CodexSpawnArgs {
        ...
        auth_manager,
        models_manager,
        environment_manager: Arc:REDACTED_SECRETEnvironmentManager:REDACTED_SECRET
    ...
    let pending_mcp_invocations = Arc:REDACTED_SECRETMutex:REDACTED_SECRETHashMap::<String, McpInvocation>:REDACTED_SECRET)));
...
pub(crate) async fn run_codex_thread_one_shot(
    ...
    // REDACTED_SECRET the initial input to kick off the one-shot turn.
...
async fn forward_events(
    codex: Arc<Codex>,
    tx_sub: REDACTED_SECRETer<Event>,
    parent_session: Arc<Session>,
    parent_ctx: Arc<TurnContext>,
...
async fn handle_patch_approval(
    ...
    let decision = if let Some(decision) = guardian_decision {
        ...
        let decision_rx = parent_session
            .request_patch_approval(parent_ctx, call_id, changes, reason, grant_REDACTED_SECRET)
            .await;
        await_approval_with_cancel(
