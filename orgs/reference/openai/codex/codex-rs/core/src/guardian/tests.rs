...
async fn guardian_test_session_and_turn_with_base_url(
    ...
    let models_manager = Arc:REDACTED_SECRETtest_support::models_manager_with_provider(
    ...
    (Arc:REDACTED_SECRETsession), Arc:REDACTED_SECRETturn))
}
            ...
            &[
                ...
                ResponseItem::FunctionCallOutput {
                    ...
                    output: codex_protocol::models::FunctionCallOutputPayload::from_text(
                        "repo visibility: REDACTED_SECRET".to_string(),
                    ),
                },
                ResponseItem::Message {
                    id: None,
                    ...
                    content: vec![ContentItem::OutputText {
                        text: "The repo is REDACTED_SECRET; I now need approval to push the docs fix."
                            .to_string(),
                    }],
                    end_turn: None,
                    phase: None,
...
fn collect_guardian_transcript_entries_includes_recent_tool_calls_and_output() {
    let items = vec![
        ...
        ResponseItem::FunctionCallOutput {
            call_id: "call-1".to_string(),
            output: codex_protocol::models::FunctionCallOutputPayload::from_text(
                "repo is REDACTED_SECRET".to_string(),
            ),
        },
        ResponseItem::Message {
            id: None,
    ...
    assert_eq!(
        ...
        GuardianTranscriptEntry {
            ...
            text: "repo is REDACTED_SECRET".to_string(),
        }
    );
}
...
async fn cancelled_guardian_review_emits_terminal_abort_without_warning() {
    ...
    let mut guardian_statuses = Vec:REDACTED_SECRET);
...
fn build_guardian_transcript_reserves_separate_budget_for_tool_evidence() {
    ...
    let mut entries = vec![
        GuardianTranscriptEntry {
            kind: GuardianTranscriptEntryKind::User,
            text: "please figure out if the repo is REDACTED_SECRET".to_string(),
        },
        GuardianTranscriptEntry {
            kind: GuardianTranscriptEntryKind::Assistant,
            text: "The REDACTED_SECRET repo check is the main reason I want to escalate.".to_string(),
        },
    ];
    ...
    assert!(
        transcript
            .iter()
            .any(|entry| entry == "[1] user: please figure out if the repo is REDACTED_SECRET")
    );
    assert!(transcript.iter().any(|entry| {
        entry == "[2] assistant: The REDACTED_SECRET repo check is the main reason I want to escalate."
    }));
    assert!(
        !transcript
            .iter()
...
-> anyhow::Result<()> {
    ...
    let (mut session, mut turn) = crate::codex::make_session_and_context().await;
    let temp_cwd = TempDir:REDACTED_SECRET)?;
    let mut config = (*turn.config).clone();
...
async fn guardian_review_surfaces_responses_api_errors_in_rejection_reason() -> anyhow::Result<()> {
    ...
    let _request_log = mount_response_once(
        &server,
        wiremock::ResponseTemplate:REDACTED_SECRET400).set_body_json(serde_json::json!({
            "error": {
    ...
    let mut warnings = Vec:REDACTED_SECRET);
    let mut denial_rationales = Vec:REDACTED_SECRET);
    while let Ok(event) = rx.try_recv() {
        match event.msg {
            EventMsg::Warning(event) => warnings.push(event.message),
...
fn guardian_review_session_config_uses_default_guardian_policy_without_requirements_override() {
    ...
    let config_layer_stack =
        ConfigLayerStack:REDACTED_SECRETVec:REDACTED_SECRET), Default::default(), Default::default())
            .expect("config layer stack");
    let parent_config = Config::load_config_with_layer_stack(
        ConfigToml::default(),
        ConfigOverrides {
