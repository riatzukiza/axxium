...
const GUARDIAN_FOLLOWUP_REVIEW_REMINDER: REDACTED_SECRET = concat!(
...
struct GuardianReviewSessionReuseKey {
    ...
    js_repl_REDACTED_SECRET_path: Option<PathBuf>,
    js_repl_REDACTED_SECRET_module_dirs: Vec<PathBuf>,
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            js_repl_REDACTED_SECRET_path: spawn_config.js_repl_REDACTED_SECRET_path.clone(),
            js_repl_REDACTED_SECRET_module_dirs: spawn_config.js_repl_REDACTED_SECRET_module_dirs.clone(),
...
impl GuardianReviewSessionManager {
        ...
        .await
        {
            Ok(mut state) => {
                ...
                if state.trunk.is_none() {
                    let spawn_cancel_token = CancellationToken:REDACTED_SECRET);
                    ...
                    .await
                    {
                        Ok(Ok(review_session)) => Arc:REDACTED_SECRETreview_session),
                        Ok(Err(err)) => {
    ...
    pub(crate) async fn cache_for_test(&self, codex: Codex) {
        ...
        self.state.lock().await.trunk = Some(Arc:REDACTED_SECRETGuardianReviewSession {
            reuse_key,
            codex,
            cancel_token: CancellationToken:REDACTED_SECRET),
            has_prior_review: AtomicBool:REDACTED_SECRETfalse),
            review_lock: Mutex:REDACTED_SECRET()),
            last_committed_rollout_items: Mutex:REDACTED_SECRETNone),
        }));
    }
    ...
    pub(crate) async fn register_ephemeral_for_test(&self, codex: Codex) {
        ...
        self.state
            ...
            .await
            .ephemeral_reviews
            .push(Arc:REDACTED_SECRETGuardianReviewSession {
                reuse_key,
                codex,
                cancel_token: CancellationToken:REDACTED_SECRET),
                has_prior_review: AtomicBool:REDACTED_SECRETfalse),
    ...
    async fn run_ephemeral_review(
        ...
        self.register_active_ephemeral(Arc::clone(&review_session))
            .await;
        let mut cleanup =
            EphemeralReviewCleanup:REDACTED_SECRETArc::clone(&self.state), Arc::clone(&review_session));
...
async fn spawn_guardian_review_session(
    ...
    Ok(GuardianReviewSession {
        ...
        cancel_token,
        reuse_key,
        has_prior_review: AtomicBool:REDACTED_SECREThas_prior_review),
        review_lock: Mutex:REDACTED_SECRET()),
        last_committed_rollout_items: Mutex:REDACTED_SECRETNone),
    })
}
...
async fn append_guardian_followup_reminder(review_session: &GuardianReviewSession) {
    ...
    let reminder: ResponseItem =
        DeveloperInstructions:REDACTED_SECRETGUARDIAN_FOLLOWUP_REVIEW_REMINDER).into();
    review_session
        .codex
        .session
...
mod tests {
    ...
    async fn run_before_review_deadline_with_cancel_cancels_token_on_abort() {
        let external_cancel = CancellationToken:REDACTED_SECRET);
