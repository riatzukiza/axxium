...
pub(crate) fn collect_guardian_transcript_entries(
    ...
    let mut tool_names_by_call_id = HashMap:REDACTED_SECRET);
    ...
    for item in items {
        let entry = match item {
            ...
            ResponseItem:REDACTED_SECRETShellCall { action, .. } => serialized_entry(
...
pub(crate) fn guardian_truncate_text(content: REDACTED_SECRET, token_cap: usize) -> String {
    if content.is_empty() {
...
pub(crate) fn parse_guardian_assessment(text: Option<REDACTED_SECRET>) -> anyhow::Result<GuardianAssessment> {
