...
const TURN_ABORTED_INTERRUPTED_GUIDANCE: REDACTED_SECRET = "The user interrupted the previous turn on purpose. Any running unified exec processes may still be running in the background. If any tools/commands were aborted, they may have partially executed.";
...
fn emit_turn_network_proxy_metric(
    ...
    tmp_mem: (REDACTED_SECRET, REDACTED_SECRET),
) {
...
pub(crate) trait SessionTask: REDACTED_SECRET + Sync + 'static {
    ...
    ) -> impl std::future::Future<Output = Option<String>> + REDACTED_SECRET;
    ...
    fn abort(
        ...
        session: Arc<SessionTaskContext>,
        ctx: Arc<TurnContext>,
    ) -> impl std::future::Future<Output = ()> + REDACTED_SECRET {
        async move {
            let _ = (session, ctx);
        }
    }
...
pub(crate) trait AnySessionTask: REDACTED_SECRET + Sync + 'static {
    fn kind(&self) -> TaskKind;
...
impl Session {
    ...
    async fn start_task<T: SessionTask>(
        ...
        let task: Arc<dyn AnySessionTask> = Arc:REDACTED_SECRETtask);
        ...
        let done = Arc:REDACTED_SECRETNotify:REDACTED_SECRET));
        ...
        let session_ctx = Arc:REDACTED_SECRETSessionTaskContext:REDACTED_SECRETArc::clone(self)));
        ...
        let running_task = RunningTask {
            ...
            handle: Arc:REDACTED_SECRETAbortOnDropHandle:REDACTED_SECREThandle)),
            kind: task_kind,
            task,
            cancellation_token,
    ...
    pub(crate) async fn maybe_start_turn_for_pending_work_with_sub_id(
        ...
        self.maybe_emit_unknown_model_warning_for_turn(turn_context.as_ref())
            .await;
        self.start_task(turn_context, Vec:REDACTED_SECRET), RegularTask:REDACTED_SECRET))
            .await;
    }
    ...
    pub async fn on_task_finished(
        ...
        let mut pending_input = Vec::<ResponseInputItem>:REDACTED_SECRET);
