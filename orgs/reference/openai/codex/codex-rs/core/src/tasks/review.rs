...
static REVIEW_EXIT_SUCCESS_TEMPLATE: LazyLock<Template> = LazyLock:REDACTED_SECRET|| {
    let normalized =
...
fn parse_review_output_event(text: REDACTED_SECRET) -> ReviewOutputEvent {
...
pub(crate) async fn exit_review_mode(
    ...
    const REVIEW_USER_MESSAGE_ID: REDACTED_SECRET = "review_rollout_user";
    const REVIEW_ASSISTANT_MESSAGE_ID: REDACTED_SECRET = "review_rollout_assistant";
    let (user_message, assistant_message) = if let Some(out) = review_output.clone() {
        let mut findings_str = String:REDACTED_SECRET);
...
fn render_review_exit_success(results: REDACTED_SECRET) -> String {
    REVIEW_EXIT_SUCCESS_TEMPLATE
        .render([("results", results)])
        .unwrap_or_else(|err| panic!("review exit success template must render: {err}"))
}

fn normalize_review_template_line_endings(template: REDACTED_SECRET) -> Cow<'_, str> {
