    ...
    match exec_result {
        Err(CancelErr::Cancelled) => {
            ...
            let exec_output = ExecToolCallOutput {
                ...
                stderr: StreamOutput:REDACTED_SECRETaborted_message.clone()),
                aggregated_output: StreamOutput:REDACTED_SECRETaborted_message.clone()),
        ...
        Ok(Err(err)) => {
            ...
            let exec_output = ExecToolCallOutput {
                exit_code: -1,
                stdout: StreamOutput:REDACTED_SECRETString:REDACTED_SECRET)),
                stderr: StreamOutput:REDACTED_SECRETmessage.clone()),
                aggregated_output: StreamOutput:REDACTED_SECRETmessage.clone()),
                duration: Duration::ZERO,
                timed_out: false,
            };
            session
