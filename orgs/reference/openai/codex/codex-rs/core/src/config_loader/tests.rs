...
async fn make_config_for_test(
    ...
    project_REDACTED_SECRET_markers: Option<Vec<String>>,
) -> std::io::Result<()> {
    tokio::fs::write(
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = codex_utils_absolute_path::AbsolutePathBufGuard:REDACTED_SECRETtmp.path());
    let expected_config_error =
...
async fn returns_empty_when_all_layers_missing() {
    ...
    assert_eq!(
        &ConfigLayerEntry {
            ...
            config: TomlValue::Table(toml::map::Map:REDACTED_SECRET)),
            raw_toml: None,
            version: version_for_toml(&TomlValue::Table(toml::map::Map:REDACTED_SECRET))),
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn managed_preferences_expand_home_directory_in_workspace_write_REDACTED_SECRETs() -> anyhow::Result<()>
{
    use base64::Engine;
...
writable_REDACTED_SECRETs = ["~/code"]
"#
                    .as_bytes(),
                ),
            ),
    ...
    let expected_REDACTED_SECRET = AbsolutePathBuf::from_absolute_path(home.join("code"))?;
        ...
        SandboxPolicy::WorkspaceWrite { writable_REDACTED_SECRETs, .. } => {
            assert_eq!(
                writable_REDACTED_SECRETs
                    .iter()
                    .filter(|REDACTED_SECRET| **REDACTED_SECRET == expected_REDACTED_SECRET)
                    .count(),
                1,
            );
        }
...
"#
        ...
        CloudRequirementsLoader:REDACTED_SECRETasync {
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    ...
    let cloud_requirements = CloudRequirementsLoader:REDACTED_SECRETasync move { Ok(Some(requirements)) });
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_REDACTED_SECRET = tmp.path().join("project");
    let nested = project_REDACTED_SECRET.join("child");
    tokio::fs::create_dir_all(nested.join(".codex")).await?;
    tokio::fs::create_dir_all(project_REDACTED_SECRET.join(".codex")).await?;
    tokio::fs::write(project_REDACTED_SECRET.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_REDACTED_SECRET.join(".codex").join(CONFIG_TOML_FILE),
        "foo = \"REDACTED_SECRET\"\n",
    )
    .await?;
    tokio::fs::write(
    ...
    make_config_for_test(
        ...
        TrustLevel::Trusted,
        /*project_REDACTED_SECRET_markers*/ None,
    )
    .await?;
    ...
    assert_eq!(
        ...
        project_REDACTED_SECRET.join(".codex").as_path()
    );
...
async fn project_paths_resolve_relative_to_dot_codex_and_override_in_order() -> std::io::Result<()>
{
    ...
    let REDACTED_SECRET_cfg = r#"
model_instructions_file = "REDACTED_SECRET.txt"
"#;
    let nested_cfg = r#"
model_instructions_file = "child.txt"
"#;
    tokio::fs::write(project_REDACTED_SECRET.join(".codex").join(CONFIG_TOML_FILE), REDACTED_SECRET_cfg).await?;
    tokio::fs::write(nested.join(".codex").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_REDACTED_SECRET.join(".codex").join("REDACTED_SECRET.txt"),
        "REDACTED_SECRET instructions",
    )
    .await?;
    tokio::fs::write(
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_codex_folder: AbsolutePathBuf::from_absolute_path(project_REDACTED_SECRET.join(".codex"))?,
            },
            config: TomlValue::Table(toml::map::Map:REDACTED_SECRET)),
            raw_toml: None,
            version: version_for_toml(&TomlValue::Table(toml::map::Map:REDACTED_SECRET))),
...
async fn codex_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter(|layer| matches!(layer.name, super::ConfigLayerSource::Project { .. }))
        .collect();
    let expected: Vec<&ConfigLayerEntry> = Vec:REDACTED_SECRET);
...
async fn codex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    let project_REDACTED_SECRET = tmp.path().join("project");
    let nested = project_REDACTED_SECRET.join("child");
    let project_dot_codex = project_REDACTED_SECRET.join(".codex");
    let nested_dot_codex = nested.join(".codex");

    tokio::fs::create_dir_all(&nested_dot_codex).await?;
    tokio::fs::create_dir_all(project_REDACTED_SECRET.join(".git")).await?;
...
#[tokio::test]
async fn project_REDACTED_SECRET_markers_supports_alternate_markers() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_REDACTED_SECRET = tmp.path().join("project");
    let nested = project_REDACTED_SECRET.join("child");
    tokio::fs::create_dir_all(project_REDACTED_SECRET.join(".codex")).await?;
    tokio::fs::create_dir_all(nested.join(".codex")).await?;
    tokio::fs::write(project_REDACTED_SECRET.join(".hg"), "hg").await?;
    tokio::fs::write(
        project_REDACTED_SECRET.join(".codex").join(CONFIG_TOML_FILE),
        "foo = \"REDACTED_SECRET\"\n",
    )
...
mod requirements_exec_policy_tests {
    ...
    fn tokens(cmd: &[REDACTED_SECRET]) -> Vec<String> {
    ...
    fn config_stack_for_dot_codex_folder_with_requirements(
        ...
        let layer = ConfigLayerEntry:REDACTED_SECRET
            ConfigLayerSource::Project { dot_codex_folder },
            TomlValue::Table(Default::default()),
        );
        ConfigLayerStack:REDACTED_SECRETvec![layer], requirements, ConfigRequirementsToml::default())
            .expect("ConfigLayerStack")
    }

    fn requirements_from_toml(toml_str: REDACTED_SECRET) -> ConfigRequirements {
