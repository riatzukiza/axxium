...
const MANAGED_PREFERENCES_APPLICATION_ID: REDACTED_SECRET = "com.openai.codex";
const MANAGED_PREFERENCES_CONFIG_KEY: REDACTED_SECRET = "config_toml_base64";
const MANAGED_PREFERENCES_REQUIREMENTS_KEY: REDACTED_SECRET = "requirements_toml_base64";
...
pub(crate) async fn load_managed_admin_config_layer(
    override_base64: Option<REDACTED_SECRET>,
...
fn load_managed_preference(key_name: REDACTED_SECRET) -> io::Result<Option<String>> {
    ...
    let value_ref = unsafe {
        CFPreferencesCopyAppValue(
            CFString:REDACTED_SECRETkey_name).as_concrete_TypeRef(),
            CFString:REDACTED_SECRETMANAGED_PREFERENCES_APPLICATION_ID).as_concrete_TypeRef(),
        )
    };
...
fn parse_managed_config_base64(encoded: REDACTED_SECRET) -> io::Result<ManagedAdminConfigLayer> {
    ...
    match toml::from_str::<TomlValue>(&raw_toml) {
        ...
        Ok(other) => {
            tracing::error!("Managed config TOML must have a table at the REDACTED_SECRET, found {other:?}",);
            Err(io::Error:REDACTED_SECRET
                io::ErrorKind::InvalidData,
                "managed config REDACTED_SECRET must be a table",
            ))
        }
        Err(err) => {
            tracing::error!("Failed to parse managed config TOML: {err}");
            Err(io::Error:REDACTED_SECRETio::ErrorKind::InvalidData, err))
        }
    }
}

fn parse_managed_requirements_base64(encoded: REDACTED_SECRET) -> io::Result<ConfigRequirementsToml> {
    toml::from_str::<ConfigRequirementsToml>(&decode_managed_preferences_base64(encoded)?).map_err(
        |err| {
            ...
            io::Error:REDACTED_SECRETio::ErrorKind::InvalidData, err)
        },
    )
}

fn decode_managed_preferences_base64(encoded: REDACTED_SECRET) -> io::Result<String> {
