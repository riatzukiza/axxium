...
- `merge_toml_values(base, overlay)` (REDACTED_SECRET helper used elsewhere)
...
let cli_overrides: Vec<(String, TomlValue)> = Vec:REDACTED_SECRET);
...
- `state.rs`: REDACTED_SECRET types (`ConfigLayerEntry`, `ConfigLayerStack`) + merge/origins convenience methods.
