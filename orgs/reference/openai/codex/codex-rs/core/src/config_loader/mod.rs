...
pub use codex_config::default_project_REDACTED_SECRET_markers;
...
pub use codex_config::project_REDACTED_SECRET_markers_from_config;
#[cfg(test)]
...
pub const SYSTEM_CONFIG_TOML_FILE_UNIX: REDACTED_SECRET = "/etc/codex/config.toml";

#[cfg(windows)]
const DEFAULT_PROGRAM_DATA_DIR_WINDOWS: REDACTED_SECRET = r"C:\ProgramData";
...
/// - tree      parent directories up to REDACTED_SECRET looking for `./.codex/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    let mut layers = Vec::<ConfigLayerEntry>:REDACTED_SECRET);
    ...
    if let Some(cwd) = cwd {
        let mut merged_so_far = TomlValue::Table(toml::map::Map:REDACTED_SECRET));
        for layer in &layers {
        ...
        let project_REDACTED_SECRET_markers = match project_REDACTED_SECRET_markers_from_config(&merged_so_far) {
            Ok(markers) => markers.unwrap_or_else(default_project_REDACTED_SECRET_markers),
            Err(err) => {
        ...
        let project_trust_context = match project_trust_context(
            &merged_so_far,
            &cwd,
            &project_REDACTED_SECRET_markers,
            codex_home,
            &user_file,
        )
        .await
        ...
        let project_layers = load_project_layers(
            &cwd,
            &project_trust_context.project_REDACTED_SECRET,
    ...
    if let Some(cli_overrides_layer) = cli_overrides_layer {
        layers.push(ConfigLayerEntry:REDACTED_SECRET
    ...
    ConfigLayerStack:REDACTED_SECRET
        layers,
    ...
    let toml_value = match tokio::fs::read_to_string(toml_file).await {
        ...
        Err(e) => {
            if e.kind() == io::ErrorKind::NotFound {
                Ok(TomlValue::Table(toml::map::Map:REDACTED_SECRET)))
            } else {
                Err(io::Error:REDACTED_SECRET
                    e.kind(),
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path:REDACTED_SECRET"/etc/codex/requirements.toml"))
}
...
fn system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path:REDACTED_SECRETSYSTEM_CONFIG_TOML_FILE_UNIX))
}
...
struct ProjectTrustContext {
    project_REDACTED_SECRET: AbsolutePathBuf,
    project_REDACTED_SECRET_key: String,
    repo_REDACTED_SECRET_key: Option<String>,
...
impl ProjectTrustContext {
    fn decision_for_dir(&self, dir: &AbsolutePathBuf) -> ProjectTrustDecision {
        ...
        if let Some(trust_level) = self.projects_trust.get(&self.project_REDACTED_SECRET_key).copied() {
            return ProjectTrustDecision {
                trust_level: Some(trust_level),
                trust_key: self.project_REDACTED_SECRET_key.clone(),
            };
        }

        if let Some(repo_REDACTED_SECRET_key) = self.repo_REDACTED_SECRET_key.as_ref()
            && let Some(trust_level) = self.projects_trust.get(repo_REDACTED_SECRET_key).copied()
        {
            return ProjectTrustDecision {
                trust_level: Some(trust_level),
                trust_key: repo_REDACTED_SECRET_key.clone(),
            };
        }
        ...
        ProjectTrustDecision {
            trust_level: None,
            trust_key: self
                .repo_REDACTED_SECRET_key
                .clone()
                .unwrap_or_else(|| self.project_REDACTED_SECRET_key.clone()),
        }
    }
...
fn project_layer_entry(
    ...
    if config_toml_exists && let Some(reason) = trust_context.disabled_reason_for_dir(layer_dir) {
        ...
        ConfigLayerEntry:REDACTED_SECRETsource, config)
    }
}
...
async fn project_trust_context(
    ...
    cwd: &AbsolutePathBuf,
    project_REDACTED_SECRET_markers: &[String],
    ...
    let project_trust_config: ProjectTrustConfigToml = {
        ...
        merged_config
            .clone()
            .try_into()
            .map_err(|err| std::io::Error:REDACTED_SECRETstd::io::ErrorKind::InvalidData, err))?
    };

    let project_REDACTED_SECRET = find_project_REDACTED_SECRET(cwd, project_REDACTED_SECRET_markers).await?;
    let projects = project_trust_config.projects.unwrap_or_default();

    let project_REDACTED_SECRET_key = project_trust_key(project_REDACTED_SECRET.as_path());
    let repo_REDACTED_SECRET = resolve_REDACTED_SECRET_git_project_for_trust(cwd.as_path());
    let repo_REDACTED_SECRET_key = repo_REDACTED_SECRET.as_ref().map(|REDACTED_SECRET| project_trust_key(REDACTED_SECRET));
    ...
    let projects_trust = projects
        ...
        .filter_map(|(key, project)| {
            project
                .trust_level
                .map(|trust_level| (project_trust_key(Path:REDACTED_SECRET&key)), trust_level))
        })
        .collect();

    Ok(ProjectTrustContext {
        project_REDACTED_SECRET,
        project_REDACTED_SECRET_key,
        repo_REDACTED_SECRET_key,
        projects_trust,
...
pub(crate) fn resolve_relative_paths_in_config_toml(
    ...
    let _guard = AbsolutePathBufGuard:REDACTED_SECRETbase_dir);
...
fn copy_shape_from_original(original: &TomlValue, resolved: &TomlValue) -> TomlValue {
    match (original, resolved) {
        (TomlValue::Table(original_table), TomlValue::Table(resolved_table)) => {
            let mut table = toml::map::Map:REDACTED_SECRET);
        ...
        (TomlValue::Array(original_array), TomlValue::Array(resolved_array)) => {
            let mut items = Vec:REDACTED_SECRET);
...
async fn find_project_REDACTED_SECRET(
    cwd: &AbsolutePathBuf,
    project_REDACTED_SECRET_markers: &[String],
...
/// `project_REDACTED_SECRET`, inclusive. The list is ordered in _increasing_ precdence,
/// starting from folders closest to `project_REDACTED_SECRET` (which is the lowest
...
async fn load_project_layers(
    ...
    let mut dirs = cwd
        ...
        .scan(false, |done, a| {
            if *done {
                ...
                if a == project_REDACTED_SECRET.as_path() {
                    *done = true;
                }
                Some(a)
            }
    ...
    let mut layers = Vec:REDACTED_SECRET);
    for dir in dirs {
...
"#;
        ...
        let mut expected_toml_value = toml::map::Map:REDACTED_SECRET);
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path:REDACTED_SECRET"OpenAI").join("Codex").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path:REDACTED_SECRET"OpenAI").join("Codex").join("config.toml"))
        );
    }
}
