...
async fn test_config_with_cli_overrides(
    ...
    let home = TempDir:REDACTED_SECRET).expect("create temp dir");
    let config = ConfigBuilder::default()
        .codex_home(home.path().to_path_buf())
        .cli_overrides(cli_overrides)
        .loader_overrides(LoaderOverrides {
            #[cfg(target_os = "macos")]
            managed_preferences_base64: Some(String:REDACTED_SECRET)),
            macos_managed_config_requirements_base64: Some(String:REDACTED_SECRET)),
...
async fn test_config() -> (TempDir, Config) {
    test_config_with_cli_overrides(Vec:REDACTED_SECRET)).await
}

fn text_input(text: REDACTED_SECRET) -> Op {
    vec![UserInput::Text {
        text: text.to_string(),
...
fn assistant_message(text: REDACTED_SECRET, phase: Option<MessagePhase>) -> ResponseItem {
...
fn spawn_agent_call(call_id: REDACTED_SECRET) -> ResponseItem {
...
fn history_contains_text(history_items: &[ResponseItem], needle: REDACTED_SECRET) -> bool {
    history_items.iter().any(|item| {
...
async fn persist_thread_for_tree_resume(thread: &Arc<CodexThread>, message: REDACTED_SECRET) {
    thread
...
async fn get_status_returns_not_found_without_manager() {
    let control = AgentControl::default();
    let got = control.get_status(ThreadId:REDACTED_SECRET)).await;
...
async fn resume_agent_errors_when_manager_dropped() {
    ...
    let (_home, config) = test_config().await;
    let err = control
        .resume_agent_from_rollout(config, ThreadId:REDACTED_SECRET), SessionSource::Exec)
        .await
...
async fn send_input_errors_when_thread_missing() {
    let harness = AgentControlHarness:REDACTED_SECRET).await;
...
async fn get_status_returns_not_found_for_missing_thread() {
    let harness = AgentControlHarness:REDACTED_SECRET).await;
    let status = harness.control.get_status(ThreadId:REDACTED_SECRET)).await;
...
async fn append_message_records_assistant_message() {
    ...
    let (thread_id, thread) = harness.start_thread().await;
    let message =
        "author: /REDACTED_SECRET\nrecipient: /REDACTED_SECRET/worker\nother_recipients: []\nContent: hello from tests";
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    let trigger_message = InterAgentCommunication:REDACTED_SECRET
        AgentPath::REDACTED_SECRET(),
        AgentPath::try_from("/REDACTED_SECRET/worker").expect("agent path"),
        Vec:REDACTED_SECRET),
...
async fn spawn_agent_fork_last_n_turns_keeps_only_recent_turns() {
    ...
    parent_thread
        ...
        .await;
    let queued_communication = InterAgentCommunication:REDACTED_SECRET
        AgentPath::REDACTED_SECRET(),
        AgentPath::try_from("/REDACTED_SECRET/worker").expect("agent path"),
        Vec:REDACTED_SECRET),
    ...
    let triggered_communication = InterAgentCommunication:REDACTED_SECRET
        AgentPath::REDACTED_SECRET(),
        AgentPath::try_from("/REDACTED_SECRET/worker").expect("agent path"),
        Vec:REDACTED_SECRET),
...
async fn resume_agent_releases_slot_after_resume_failure() {
    ...
    let _ = control
        .resume_agent_from_rollout(config.clone(), ThreadId:REDACTED_SECRET), SessionSource::Exec)
        .await
...
async fn multi_agent_v2_completion_ignores_dead_direct_parent() {
    let harness = AgentControlHarness:REDACTED_SECRET).await;
    let (REDACTED_SECRET_thread_id, REDACTED_SECRET_thread) = harness.start_thread().await;
    let mut config = harness.config.clone();
    let _ = config.features.enable(Feature::MultiAgentV2);
    let worker_path = AgentPath::REDACTED_SECRET().join("worker_a").expect("worker path");
    let worker_thread_id = harness
        ...
        .spawn_agent(
            ...
            Some(SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                parent_thread_id: REDACTED_SECRET_thread_id,
                depth: 1,
    ...
    let REDACTED_SECRET_history_items = REDACTED_SECRET_thread
        .codex
        .session
        .clone_history()
        .await
    ...
    assert!(!has_subagent_notification(&REDACTED_SECRET_history_items));
}

#[tokio::test]
async fn multi_agent_v2_completion_queues_message_for_direct_parent() {
    let harness = AgentControlHarness:REDACTED_SECRET).await;
    let (_REDACTED_SECRET_thread_id, REDACTED_SECRET_thread) = harness.start_thread().await;
    ...
    let expected = (
        worker_thread_id,
        Op::InterAgentCommunication {
            communication: InterAgentCommunication:REDACTED_SECRET
                tester_path.clone(),
                worker_path.clone(),
                Vec:REDACTED_SECRET),
...
async fn completion_watcher_notifies_parent_when_child_is_missing() {
    let harness = AgentControlHarness:REDACTED_SECRET).await;
    let (parent_thread_id, parent_thread) = harness.start_thread().await;
    let child_thread_id = ThreadId:REDACTED_SECRET);
...
async fn spawn_thread_subagent_uses_role_specific_nickname_candidates() {
    let mut harness = AgentControlHarness:REDACTED_SECRET).await;
...
async fn resume_thread_subagent_restores_stored_nickname_and_role() {
    ...
    let (parent_thread_id, _parent_thread) = harness.start_thread().await;
    let agent_path = AgentPath::from_string("/REDACTED_SECRET/explorer".to_string())
...
async fn resume_agent_from_rollout_reads_archived_rollout_path() {
    ...
    let archived_REDACTED_SECRET = harness
        .config
        .codex_home
        .join(crate::ARCHIVED_SESSIONS_SUBDIR);
    tokio::fs::create_dir_all(&archived_REDACTED_SECRET)
        .await
        .expect("archived REDACTED_SECRET should exist");
    let archived_rollout_path = archived_REDACTED_SECRET.join(
        rollout_path
            .file_name()
...
async fn resume_agent_from_rollout_reopens_open_descendants_after_manager_shutdown() {
    ...
    let report = harness
        ...
        .await;
    assert_eq!(report.submit_failed, Vec::<ThreadId>:REDACTED_SECRET));
    assert_eq!(report.timed_out, Vec::<ThreadId>:REDACTED_SECRET));
...
async fn resume_agent_from_rollout_uses_edge_data_when_descendant_metadata_source_is_stale() {
    ...
    stale_metadata.source =
        serde_json::to_string(&SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            parent_thread_id: ThreadId:REDACTED_SECRET),
            depth: 99,
            agent_path: None,
            agent_nickname: None,
            agent_role: Some("worker".to_string()),
...
async fn resume_agent_from_rollout_skips_descendants_when_parent_resume_fails() {
    ...
    let resumed_parent_thread_id = harness
        ...
        .await
        .expect("REDACTED_SECRET resume should succeed");
    assert_eq!(resumed_parent_thread_id, parent_thread_id);
    assert_ne!(
        harness.control.get_status(parent_thread_id).await,
        AgentStatus::NotFound
