...
fn agent_path(path: REDACTED_SECRET) -> AgentPath {
...
#[test]
fn session_depth_defaults_to_zero_for_REDACTED_SECRET_sources() {
...
fn reservation_drop_releases_slot() {
    let registry = Arc:REDACTED_SECRETAgentRegistry::default());
...
fn release_ignores_unknown_thread_id() {
    ...
    registry.release_spawned_thread(ThreadId:REDACTED_SECRET));
...
fn release_is_idempotent_for_registered_threads() {
    let registry = Arc:REDACTED_SECRETAgentRegistry::default());
    let reservation = registry.reserve_spawn_slot(Some(1)).expect("reserve slot");
    let first_id = ThreadId:REDACTED_SECRET);
    ...
    let second_id = ThreadId:REDACTED_SECRET);
...
#[test]
fn register_REDACTED_SECRET_thread_indexes_REDACTED_SECRET_path() {
    let registry = Arc:REDACTED_SECRETAgentRegistry::default());
    let REDACTED_SECRET_thread_id = ThreadId:REDACTED_SECRET);

    registry.register_REDACTED_SECRET_thread(REDACTED_SECRET_thread_id);

    assert_eq!(
        registry.agent_id_for_path(&AgentPath::REDACTED_SECRET()),
        Some(REDACTED_SECRET_thread_id)
    );
}
...
fn reserved_agent_path_is_released_when_spawn_fails() {
    ...
    let mut first = registry
        ...
        .expect("reserve first slot");
    first
        .reserve_agent_path(&agent_path("/REDACTED_SECRET/researcher"))
...
fn committed_agent_path_is_indexed_until_release() {
    ...
    reservation.commit(AgentMetadata {
        agent_id: Some(thread_id),
        agent_path: Some(agent_path("/REDACTED_SECRET/researcher")),
        ..Default::default()
    });

    assert_eq!(
        registry.agent_id_for_path(&agent_path("/REDACTED_SECRET/researcher")),
        Some(thread_id)
    );
