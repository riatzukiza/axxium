...
pub const DEFAULT_ROLE_NAME: REDACTED_SECRET = "default";
const AGENT_TYPE_UNAVAILABLE_ERROR: REDACTED_SECRET = "agent type is currently not available";
...
mod reload {
    ...
    fn build_config_layer_stack(
        config: &Config,
        role_layer_toml: &TomlValue,
        active_profile_name: Option<REDACTED_SECRET>,
        ...
        Ok(ConfigLayerStack:REDACTED_SECRET
            layers,
    ...
    fn resolved_profile_layer(
        ...
        Ok(Some(ConfigLayerEntry:REDACTED_SECRET
    ...
    fn role_layer(role_layer_toml: TomlValue) -> ConfigLayerEntry {
        ConfigLayerEntry:REDACTED_SECRETConfigLayerSource::SessionFlags, role_layer_toml)
    }
    ...
    fn reload_overrides(config: &Config, preserve_current_provider: bool) -> ConfigOverrides {
        ConfigOverrides {
            ...
            js_repl_REDACTED_SECRET_path: config.js_repl_REDACTED_SECRET_path.clone(),
            ..Default::default()
        }
    }
}
...
pub(crate) mod spawn_tool_spec {
    ...
    pub(crate) fn build(user_defined_agent_roles: &REDACTED_SECRET, AgentRoleConfig>) -> String {
    ...
    fn build_from_configs(
        built_in_roles: &REDACTED_SECRET, AgentRoleConfig>,
        user_defined_roles: &REDACTED_SECRET, AgentRoleConfig>,
    ) -> String {
        let mut seen = BTreeSet:REDACTED_SECRET);
        let mut formatted_roles = Vec:REDACTED_SECRET);
    ...
    fn format_role(name: REDACTED_SECRET, declaration: &AgentRoleConfig) -> String {
            ...
            let locked_settings_note = declaration
                ...
                .map(|role_toml| {
                    ...
                    match (model, reasoning_effort) {
                        ...
                        (None, None) => String:REDACTED_SECRET),
                    }
                })
                .unwrap_or_default();
...
mod built_in {
    ...
    pub(super) fn configs() -> &'static REDACTED_SECRET, AgentRoleConfig> {
        static CONFIG: LazyLock<REDACTED_SECRET, AgentRoleConfig>> = LazyLock:REDACTED_SECRET|| {
            BTreeMap::from([
                (
...
//                 )
    ...
    pub(super) fn config_file_contents(path: &Path) -> Option<&'static str> {
        const EXPLORER: REDACTED_SECRET = include_str!("builtins/explorer.toml");
        const AWAITER: REDACTED_SECRET = include_str!("builtins/awaiter.toml");
        match path.to_str()? {
