...
async fn write_role_config(home: &TempDir, name: REDACTED_SECRET, contents: REDACTED_SECRET) -> PathBuf {
...
async fn apply_role_defaults_to_default_and_leaves_config_unchanged() {
    let (_home, mut config) = test_config_with_cli_overrides(Vec:REDACTED_SECRET)).await;
...
async fn apply_role_returns_unavailable_for_invalid_user_role_toml() {
    let (home, mut config) = test_config_with_cli_overrides(Vec:REDACTED_SECRET)).await;
...
writable_REDACTED_SECRETs = ["./sandbox-REDACTED_SECRET"]
"#,
    )
    .await;
    config.agent_roles.insert(
...
"#,
    ...
    let plugins_manager = Arc:REDACTED_SECRETPluginsManager:REDACTED_SECREThome.path().to_path_buf()));
...
fn spawn_tool_spec_marks_role_locked_model_and_reasoning_effort() {
    let tempdir = TempDir:REDACTED_SECRET).expect("create temp dir");
...
fn built_in_config_file_contents_resolves_explorer_only() {
    assert_eq!(
        built_in::config_file_contents(Path:REDACTED_SECRET"missing.toml")),
        None
    );
}
