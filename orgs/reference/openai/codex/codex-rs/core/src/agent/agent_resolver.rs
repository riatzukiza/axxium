...
pub(crate) async fn resolve_agent_target(
    ...
    register_session_REDACTED_SECRET(session, turn);
...
fn register_session_REDACTED_SECRET(session: &Arc<Session>, turn: &Arc<TurnContext>) {
    session
        .services
        .agent_control
        .register_session_REDACTED_SECRET(session.conversation_id, &turn.session_source);
}
