...
const AGENT_NAMES: REDACTED_SECRET = include_str!("agent_names.txt");
const ROOT_LAST_TASK_MESSAGE: REDACTED_SECRET = "Main thread";
...
fn agent_nickname_candidates(
    ...
    role_name: Option<REDACTED_SECRET>,
) -> Vec<String> {
...
/// An `AgentControl` instance is intended to be created at most once per REDACTED_SECRET thread/session
/// tree. That same `AgentControl` is then shared with every sub-agent spawned from that REDACTED_SECRET,
/// which keeps the registry scoped to that REDACTED_SECRET thread rather than the entire `ThreadManager`.
...
impl AgentControl {
    ...
    pub(crate) async fn resume_agent_from_rollout(
        ...
        let REDACTED_SECRET_depth = thread_spawn_depth(&session_source).unwrap_or(0);
        ...
        let mut resume_queue = VecDeque::from([(thread_id, REDACTED_SECRET_depth)]);
    ...
    /// REDACTED_SECRET rich user input items to an existing agent thread.
    ...
    pub(crate) fn register_session_REDACTED_SECRET(
        &self,
        current_thread_id: ThreadId,
        current_session_source: &SessionSource,
    ) {
        if thread_spawn_parent_thread_id(current_session_source).is_none() {
            self.state.register_REDACTED_SECRET_thread(current_thread_id);
        }
    }
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        current_session_source: &SessionSource,
        agent_reference: REDACTED_SECRET,
    ) -> CodexResult<ThreadId> {
        let current_agent_path = current_session_source
            .get_agent_path()
            .unwrap_or_else(AgentPath::REDACTED_SECRET);
    ...
    pub(crate) async fn list_agents(
        &self,
        current_session_source: &SessionSource,
        path_prefix: Option<REDACTED_SECRET>,
        ...
        let REDACTED_SECRET_path = AgentPath::REDACTED_SECRET();
        let mut agents = Vec::with_capacity(live_agents.len().saturating_add(1));
        if resolved_prefix
            .as_ref()
            .is_none_or(|prefix| agent_matches_prefix(Some(&REDACTED_SECRET_path), prefix))
            && let Some(REDACTED_SECRET_thread_id) = self.state.agent_id_for_path(&REDACTED_SECRET_path)
            && let Ok(REDACTED_SECRET_thread) = state.get_thread(REDACTED_SECRET_thread_id).await
        {
            agents.push(ListedAgent {
                agent_name: REDACTED_SECRET_path.to_string(),
                agent_status: REDACTED_SECRET_thread.agent_status().await,
    ...
    fn prepare_thread_spawn(
        ...
        if depth == 1 {
            self.state.register_REDACTED_SECRET_thread(parent_thread_id);
        }
        ...
        let candidate_name_refs: Vec<REDACTED_SECRET> = candidate_names.iter().map(String::as_str).collect();
    ...
    async fn live_thread_spawn_children(
        ...
        let state = self.upgrade()?;
        let mut children_by_parent = HashMap::<ThreadId, Vec<(ThreadId, AgentMetadata)>>:REDACTED_SECRET);
    ...
    async fn live_thread_spawn_descendants(
        &self,
        REDACTED_SECRET_thread_id: ThreadId,
    ) -> CodexResult<Vec<ThreadId>> {
        let mut children_by_parent = self.live_thread_spawn_children().await?;
        let mut descendants = Vec:REDACTED_SECRET);
        let mut stack = children_by_parent
            .remove(&REDACTED_SECRET_thread_id)
            .unwrap_or_default()
            .into_iter()
    ...
    match initial_operation {
        Op::UserInput { items, .. } => items
            ...
            .map(|item| match item {
                ...
                UserInput:REDACTED_SECRETImage { path } => format!("[local_image:{}]", path.display()),
