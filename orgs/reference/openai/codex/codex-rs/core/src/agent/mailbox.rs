...
pub(crate) struct Mailbox {
    tx: mpsc::UnboundedREDACTED_SECRETer<InterAgentCommunication>,
    next_seq: AtomicU64,
    seq_tx: watch::REDACTED_SECRETer<u64>,
}
...
impl Mailbox {
    pub(crate) fn new() -> (Self, MailboxReceiver) {
        ...
        (
            Self {
                tx,
                next_seq: AtomicU64:REDACTED_SECRET0),
                seq_tx,
            },
            MailboxReceiver {
                rx,
                pending_mails: VecDeque:REDACTED_SECRET),
            },
        )
    }
...
mod tests {
    ...
    async fn mailbox_assigns_monotonic_sequence_numbers() {
        let (mailbox, _receiver) = Mailbox:REDACTED_SECRET);
    ...
    async fn mailbox_drains_in_delivery_order() {
        let (mailbox, mut receiver) = Mailbox:REDACTED_SECRET);
