...
fn detects_zsh() {
    ...
    assert_eq!(shell_path, std::path::Path:REDACTED_SECRET"/bin/zsh"));
}
...
fn shell_works(shell: Option<Shell>, command: REDACTED_SECRET, required: bool) -> bool {
    if let Some(shell) = shell {
        let args = shell.derive_exec_args(command, /*use_login_shell*/ false);
        let output = Command:REDACTED_SECRETargs[0].clone())
            .args(&args[1..])
            .output()
            .unwrap();
...
async fn test_current_shell_detects_zsh() {
    let shell = Command:REDACTED_SECRET"sh")
        .arg("-c")
        .arg("echo $SHELL")
        .output()
        .unwrap();
