...
pub const SUMMARIZATION_PROMPT: REDACTED_SECRET = include_str!("../templates/compact/prompt.md");
pub const SUMMARY_PREFIX: REDACTED_SECRET = include_str!("../templates/compact/summary_prefix.md");
...
async fn run_compact_task_inner(
    ...
    let compaction_item = TurnItem::ContextCompaction(ContextCompactionItem:REDACTED_SECRET));
    ...
    let mut new_history = build_compacted_history(Vec:REDACTED_SECRET), &user_messages, &summary_text);
...
pub fn content_items_to_text(content: &[ContentItem]) -> Option<String> {
    let mut pieces = Vec:REDACTED_SECRET);
    for item in content {
        match item {
...
pub(crate) fn is_summary_message(message: REDACTED_SECRET) -> bool {
...
fn build_compacted_history_with_limit(
    ...
    let mut selected_messages: Vec<String> = Vec:REDACTED_SECRET);
    if max_tokens > 0 {
