...
pub const REVIEW_PROMPT: REDACTED_SECRET = include_str!("../review_prompt.md");

// Centralized templates for review-related user messages
pub const REVIEW_EXIT_SUCCESS_TMPL: REDACTED_SECRET = include_str!("../templates/review/exit_success.xml");
pub const REVIEW_EXIT_INTERRUPTED_TMPL: REDACTED_SECRET =
...
fn reserialize_shell_outputs(items: &mut [ResponseItem]) {
    let mut shell_call_ids: HashSet<String> = HashSet:REDACTED_SECRET);

    items.iter_mut().for_each(|item| match item {
        ResponseItem:REDACTED_SECRETShellCall { call_id, id, .. } => {
...
fn is_shell_tool_name(name: REDACTED_SECRET) -> bool {
...
fn parse_structured_shell_output(raw: REDACTED_SECRET) -> Option<String> {
...
fn strip_total_output_header(output: REDACTED_SECRET) -> Option<(REDACTED_SECRET, u32)> {
