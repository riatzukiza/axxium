...
fn approval_metadata(
    ...
    connector_description: Option<REDACTED_SECRET>,
    tool_title: Option<REDACTED_SECRET>,
    tool_description: Option<REDACTED_SECRET>,
...
async fn mcp_tool_call_span_records_expected_fields() {
    let buffer: &'static std::sync::Mutex<Vec<u8>> =
        Box::leak(Box:REDACTED_SECRETstd::sync::Mutex:REDACTED_SECRETVec:REDACTED_SECRET))));
    let subscriber = tracing_subscriber::fmt()
        .with_level(true)
        .with_ansi(false)
        .with_max_level(Level::TRACE)
        .with_span_events(FmtSpan::FULL)
        .with_writer(MockWriter:REDACTED_SECRETbuffer))
        .finish();
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    turn_context.config = Arc:REDACTED_SECRETconfig);
    let key = McpToolApprovalKey {
        server: "docs".to_string(),
        connector_id: None,
        tool_name: "search".to_string(),
...
async fn guardian_mode_skips_auto_when_annotations_do_not_require_approval() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate:REDACTED_SECRET200))
        .expect(0)
        .mount(&server)
        .await;
...
async fn approve_mode_blocks_when_arc_returns_interrupt_for_model() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(ResponseTemplate:REDACTED_SECRET200).set_body_json(serde_json::json!({
            "outcome": "steer-model",
