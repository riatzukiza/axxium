...
fn app(id: REDACTED_SECRET) -> AppInfo {
    AppInfo {
        id: id.to_string(),
        name: id.to_string(),
        description: None,
...
fn named_app(id: REDACTED_SECRET, name: REDACTED_SECRET) -> AppInfo {
    AppInfo {
        id: id.to_string(),
        name: name.to_string(),
...
fn plugin_names(names: &[REDACTED_SECRET]) -> Vec<String> {
    names.iter().map(ToString::to_string).collect()
}

fn test_tool_definition(tool_name: REDACTED_SECRET) -> Tool {
    Tool {
...
fn google_calendar_accessible_connector(plugin_display_names: &[REDACTED_SECRET]) -> AppInfo {
    AppInfo {
...
fn codex_app_tool(
    ...
    plugin_display_names: &[REDACTED_SECRET],
) -> ToolInfo {
...
fn filter_tool_suggest_discoverable_connectors_excludes_accessible_apps_even_when_disabled() {
    ...
    assert_eq!(filtered, Vec::<AppInfo>:REDACTED_SECRET));
}
