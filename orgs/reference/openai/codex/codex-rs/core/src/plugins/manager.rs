...
const DEFAULT_SKILLS_DIR_NAME: REDACTED_SECRET = "skills";
const DEFAULT_MCP_CONFIG_FILE: REDACTED_SECRET = ".mcp.json";
const DEFAULT_APP_CONFIG_FILE: REDACTED_SECRET = ".app.json";
pub const OPENAI_CURATED_MARKETPLACE_NAME: REDACTED_SECRET = "openai-curated";
pub const OPENAI_CURATED_MARKETPLACE_DISPLAY_NAME: REDACTED_SECRET = "OpenAI Curated";
static CURATED_REPO_SYNC_STARTED: AtomicBool = AtomicBool:REDACTED_SECRETfalse);
...
impl PluginsManager {
    ...
    pub fn new_with_restriction_product(
        ...
        Self {
            ...
            store: PluginStore:REDACTED_SECRETcodex_home),
            featured_plugin_ids_cache: RwLock:REDACTED_SECRETNone),
            cached_enabled_outcome: RwLock:REDACTED_SECRETNone),
            remote_sync_lock: Mutex:REDACTED_SECRET()),
            restriction_product,
            analytics_events_client: RwLock:REDACTED_SECRETNone),
        }
    }
    ...
    /// Resolve plugin skill REDACTED_SECRETs for a config layer stack without touching the plugins cache.
    pub fn effective_skill_REDACTED_SECRETs_for_layer_stack(
        &self,
    ...
    async fn install_resolved_plugin(
        ...
        if let Some(analytics_events_client) = analytics_events_client {
            analytics_events_client.track_plugin_installed(plugin_telemetry_metadata_from_REDACTED_SECRET(
                &result.plugin_id,
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        let plugin_telemetry = self
            .store
            .active_plugin_REDACTED_SECRET(&plugin_id)
        ...
        ConfigEditsBuilder:REDACTED_SECRET&self.codex_home)
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let curated_marketplace_REDACTED_SECRET = curated_plugins_repo_path(self.codex_home.as_path());
        let curated_marketplace_path = AbsolutePathBuf::try_from(
            curated_marketplace_REDACTED_SECRET.join(".agents/plugins/marketplace.json"),
        )
        .map_err(|_| PluginRemoteSyncError:REDACTED_SECRETMarketplaceNotFound)?;
        let curated_marketplace = match load_marketplace(&curated_marketplace_path) {
            Ok(marketplace) => marketplace,
            Err(MarketplaceError::MarketplaceNotFound { .. }) => {
                return Err(PluginRemoteSyncError:REDACTED_SECRETMarketplaceNotFound);
            }
        ...
        let mut local_plugin_names = HashSet:REDACTED_SECRET);
        for plugin in curated_marketplace.plugins {
            ...
            let plugin_id = PluginId:REDACTED_SECRETplugin_name.clone(), marketplace_name.clone())?;
            let plugin_key = plugin_id.as_key();
            let source_path = match plugin.source {
                MarketplacePluginSource:REDACTED_SECRET { path } => path,
            };
        ...
        let mut remote_installed_plugin_names = HashSet::<String>:REDACTED_SECRET);
        ...
        let mut config_edits = Vec:REDACTED_SECRET);
        let mut installs = Vec:REDACTED_SECRET);
        let mut uninstalls = Vec:REDACTED_SECRET);
    ...
    pub fn list_marketplaces_for_config(
        &self,
        config: &Config,
        additional_REDACTED_SECRETs: &[AbsolutePathBuf],
        ...
        let marketplace_outcome = list_marketplaces(&self.marketplace_REDACTED_SECRETs(additional_REDACTED_SECRETs))?;
        let mut seen_plugin_keys = HashSet:REDACTED_SECRET);
    ...
    pub fn read_plugin_for_config(
        ...
        let plugin_id = PluginId:REDACTED_SECRETplugin.name.clone(), marketplace.name.clone()).map_err(
            |err| match err {
        ...
        let (installed_plugins, enabled_plugins) = self.configured_plugin_states(config);
        let source_path = match &plugin.source {
            MarketplacePluginSource:REDACTED_SECRET { path } => path.clone(),
        };
        ...
        let mut mcp_server_names = Vec:REDACTED_SECRET);
    ...
    fn start_curated_repo_sync(self: &Arc<Self>) {
        ...
        if let Err(err) = std::thread::Builder:REDACTED_SECRET)
    ...
    fn marketplace_REDACTED_SECRETs(&self, additional_REDACTED_SECRETs: &[AbsolutePathBuf]) -> Vec<AbsolutePathBuf> {
        // Treat the curated catalog as an extra marketplace REDACTED_SECRET so plugin listing can surface it
        // without requiring every caller to know where it is stored.
        let mut REDACTED_SECRETs = additional_REDACTED_SECRETs.to_vec();
        let curated_repo_REDACTED_SECRET = curated_plugins_repo_path(self.codex_home.as_path());
        if curated_repo_REDACTED_SECRET.is_dir()
            && let Ok(curated_repo_REDACTED_SECRET) = AbsolutePathBuf::try_from(curated_repo_REDACTED_SECRET)
        {
            REDACTED_SECRETs.push(curated_repo_REDACTED_SECRET);
        }
        REDACTED_SECRETs.sort_unstable_by(|left, right| left.as_path().cmp(right.as_path()));
        REDACTED_SECRETs.dedup();
        REDACTED_SECRETs
    }
}
        ...
        .filter(|plugin| plugin.error.is_some())
    {
        if let Some(error) = plugin.error.as_deref() {
            warn!(
                ...
                path = %plugin.REDACTED_SECRET.display(),
...
pub(crate) fn load_plugins_from_layer_stack(
    ...
    let mut seen_mcp_server_names = HashMap::<String, String>:REDACTED_SECRET);
...
fn refresh_curated_plugin_cache(
    codex_home: &Path,
    plugin_version: REDACTED_SECRET,
    configured_curated_plugin_ids: &[PluginId],
) -> Result<bool, String> {
    let store = PluginStore:REDACTED_SECRETcodex_home.to_path_buf());
    let curated_marketplace_path = AbsolutePathBuf::try_from(
    ...
    let mut plugin_sources = HashMap::<String, AbsolutePathBuf>:REDACTED_SECRET);
    for plugin in curated_marketplace.plugins {
        let plugin_name = plugin.name;
...
fn configured_plugins_from_stack(
    ...
    let Some(user_layer) = config_layer_stack.get_user_layer() else {
        return HashMap:REDACTED_SECRET);
    };
...
fn configured_curated_plugin_ids_from_codex_home(codex_home: &Path) -> Vec<PluginId> {
    ...
    let user_config = match fs::read_to_string(&config_path) {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => return Vec:REDACTED_SECRET),
        Err(err) => {
            warn!(
...
fn load_plugin(
    ...
    let active_plugin_REDACTED_SECRET = plugin_id
        .as_ref()
        .ok()
        .and_then(|plugin_id| store.active_plugin_REDACTED_SECRET(plugin_id));
    let REDACTED_SECRET = active_plugin_REDACTED_SECRET
        .clone()
        .unwrap_or_else(|| match &plugin_id {
            Ok(plugin_id) => store.plugin_base_REDACTED_SECRET(plugin_id),
            Err(_) => store.REDACTED_SECRET().clone(),
        });
    ...
    let plugin_REDACTED_SECRET = match plugin_id {
        Ok(_) => match active_plugin_REDACTED_SECRET {
            Some(plugin_REDACTED_SECRET) => plugin_REDACTED_SECRET,
            None => {
    ...
    if !plugin_REDACTED_SECRET.as_path().is_dir() {
        loaded_plugin.error = Some("path does not exist or is not a directory".to_string());
        return loaded_plugin;
    }

    let Some(manifest) = load_plugin_manifest(plugin_REDACTED_SECRET.as_path()) else {
    ...
    loaded_plugin.skill_REDACTED_SECRETs = plugin_skill_REDACTED_SECRETs(plugin_REDACTED_SECRET.as_path(), manifest_paths);
    let resolved_skills = load_plugin_skills(
        plugin_REDACTED_SECRET.as_path(),
        manifest_paths,
        restriction_product,
        skill_config_rules,
    );
    ...
    let mut mcp_servers = HashMap:REDACTED_SECRET);
    for mcp_config_path in plugin_mcp_config_paths(plugin_REDACTED_SECRET.as_path(), manifest_paths) {
        let plugin_mcp = load_mcp_servers_from_file(plugin_REDACTED_SECRET.as_path(), &mcp_config_path);
        for (name, config) in plugin_mcp.mcp_servers {
            if mcp_servers.insert(name.clone(), config).is_some() {
                warn!(
                    plugin = %plugin_REDACTED_SECRET.display(),
    ...
    loaded_plugin.apps = load_plugin_apps(plugin_REDACTED_SECRET.as_path());
    loaded_plugin
}
...
fn load_plugin_skills(
    plugin_REDACTED_SECRET: &Path,
    manifest_paths: &PluginManifestPaths,
    restriction_product: Option<Product>,
    skill_config_rules: &SkillConfigRules,
) -> ResolvedPluginSkills {
    let outcome = load_skills_from_REDACTED_SECRETs(
        plugin_skill_REDACTED_SECRETs(plugin_REDACTED_SECRET, manifest_paths)
            .into_iter()
            .map(|path| SkillRoot {
                path,
...
fn plugin_skill_REDACTED_SECRETs(plugin_REDACTED_SECRET: &Path, manifest_paths: &PluginManifestPaths) -> Vec<PathBuf> {
    let mut paths = default_skill_REDACTED_SECRETs(plugin_REDACTED_SECRET);
...
fn default_skill_REDACTED_SECRETs(plugin_REDACTED_SECRET: &Path) -> Vec<PathBuf> {
    let skills_dir = plugin_REDACTED_SECRET.join(DEFAULT_SKILLS_DIR_NAME);
...
fn plugin_mcp_config_paths(
    ...
    default_mcp_config_paths(plugin_REDACTED_SECRET)
}

fn default_mcp_config_paths(plugin_REDACTED_SECRET: &Path) -> Vec<AbsolutePathBuf> {
    let mut paths = Vec:REDACTED_SECRET);
    let default_path = plugin_REDACTED_SECRET.join(DEFAULT_MCP_CONFIG_FILE);
...
pub fn load_plugin_apps(plugin_REDACTED_SECRET: &Path) -> Vec<AppConnectorId> {
    if let Some(manifest) = load_plugin_manifest(plugin_REDACTED_SECRET) {
        return load_apps_from_paths(
            plugin_REDACTED_SECRET,
            plugin_app_config_paths(plugin_REDACTED_SECRET, &manifest.paths),
        );
    }
    load_apps_from_paths(plugin_REDACTED_SECRET, default_app_config_paths(plugin_REDACTED_SECRET))
}
...
fn plugin_app_config_paths(
    ...
    default_app_config_paths(plugin_REDACTED_SECRET)
}

fn default_app_config_paths(plugin_REDACTED_SECRET: &Path) -> Vec<AbsolutePathBuf> {
    let mut paths = Vec:REDACTED_SECRET);
    let default_path = plugin_REDACTED_SECRET.join(DEFAULT_APP_CONFIG_FILE);
    if default_path.is_file()
        && let Ok(default_path) = AbsolutePathBuf::try_from(default_path)
    {
        paths.push(default_path);
...
fn load_apps_from_paths(
    ...
    let mut connector_ids = Vec:REDACTED_SECRET);
...
pub fn plugin_telemetry_metadata_from_REDACTED_SECRET(
    plugin_id: &PluginId,
    plugin_REDACTED_SECRET: &Path,
) -> PluginTelemetryMetadata {
    let Some(manifest) = load_plugin_manifest(plugin_REDACTED_SECRET) else {
        return PluginTelemetryMetadata::from_plugin_id(plugin_id);
    };

    let manifest_paths = &manifest.paths;
    let has_skills = !plugin_skill_REDACTED_SECRETs(plugin_REDACTED_SECRET, manifest_paths).is_empty();
    let mut mcp_server_names = Vec:REDACTED_SECRET);
    for path in plugin_mcp_config_paths(plugin_REDACTED_SECRET, manifest_paths) {
        mcp_server_names.extend(
            load_mcp_servers_from_file(plugin_REDACTED_SECRET, &path)
                .mcp_servers
                .into_keys(),
        );
    }
    ...
    PluginTelemetryMetadata {
        ...
        capability_summary: Some(PluginCapabilitySummary {
            ...
            has_skills,
            mcp_server_names,
            app_connector_ids: load_plugin_apps(plugin_REDACTED_SECRET),
        }),
    }
}

pub fn load_plugin_mcp_servers(plugin_REDACTED_SECRET: &Path) -> HashMap<String, McpServerConfig> {
    let Some(manifest) = load_plugin_manifest(plugin_REDACTED_SECRET) else {
        return HashMap:REDACTED_SECRET);
    };

    let mut mcp_servers = HashMap:REDACTED_SECRET);
    for mcp_config_path in plugin_mcp_config_paths(plugin_REDACTED_SECRET, &manifest.paths) {
        let plugin_mcp = load_mcp_servers_from_file(plugin_REDACTED_SECRET, &mcp_config_path);
        for (name, config) in plugin_mcp.mcp_servers {
...
pub fn installed_plugin_telemetry_metadata(
    ...
    let store = PluginStore:REDACTED_SECRETcodex_home.to_path_buf());
    let Some(plugin_REDACTED_SECRET) = store.active_plugin_REDACTED_SECRET(plugin_id) else {
        return PluginTelemetryMetadata::from_plugin_id(plugin_id);
    };

    plugin_telemetry_metadata_from_REDACTED_SECRET(plugin_id, plugin_REDACTED_SECRET.as_path())
}
...
fn normalize_plugin_mcp_servers(
    ...
    for (name, config_value) in plugin_mcp_servers {
        let normalized = normalize_plugin_mcp_server_value(plugin_REDACTED_SECRET, config_value);
...
fn normalize_plugin_mcp_server_value(
    ...
    let mut object = match value {
        ...
        _ => return JsonMap:REDACTED_SECRET),
    };
    ...
    if let Some(JsonValue::String(cwd)) = object.get("cwd")
        && !Path:REDACTED_SECRETcwd).is_absolute()
    {
        object.insert(
            "cwd".to_string(),
            JsonValue::String(plugin_REDACTED_SECRET.join(cwd).display().to_string()),
        );
    }
