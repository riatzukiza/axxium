...
const GITHUB_API_BASE_URL: REDACTED_SECRET = "https://api.github.com";
const GITHUB_API_ACCEPT_HEADER: REDACTED_SECRET = "application/vnd.github+json";
const GITHUB_API_VERSION_HEADER: REDACTED_SECRET = "2022-11-28";
const OPENAI_PLUGINS_OWNER: REDACTED_SECRET = "openai";
const OPENAI_PLUGINS_REPO: REDACTED_SECRET = "plugins";
const CURATED_PLUGINS_RELATIVE_DIR: REDACTED_SECRET = ".tmp/plugins";
const CURATED_PLUGINS_SHA_FILE: REDACTED_SECRET = ".tmp/plugins.sha";
...
fn sync_openai_plugins_repo_with_transport_overrides(
    codex_home: &Path,
    git_binary: REDACTED_SECRET,
    api_base_url: REDACTED_SECRET,
...
fn sync_openai_plugins_repo_via_git(codex_home: &Path, git_binary: REDACTED_SECRET) -> Result<String, String> {
    ...
    let clone_output = run_git_command_with_timeout(
        Command:REDACTED_SECRETgit_binary)
...
fn prepare_curated_repo_parent_and_temp_dir(repo_path: &Path) -> Result<TempDir, String> {
    ...
    let clone_dir = tempfile::Builder:REDACTED_SECRET)
...
fn write_curated_plugins_sha(sha_path: &Path, remote_sha: REDACTED_SECRET) -> Result<(), String> {
...
fn git_ls_remote_head_sha(git_binary: REDACTED_SECRET) -> Result<String, String> {
...
fn git_head_sha(repo_path: &Path, git_binary: REDACTED_SECRET) -> Result<String, String> {
    let output = Command:REDACTED_SECRETgit_binary)
        .env("GIT_OPTIONAL_LOCKS", "0")
        .arg("-C")
        .arg(repo_path)
        .arg("rev-parse")
...
fn ensure_git_success(output: &Output, context: REDACTED_SECRET) -> Result<(), String> {
...
async fn fetch_curated_repo_remote_sha(api_base_url: REDACTED_SECRET) -> Result<String, String> {
...
async fn fetch_github_text(client: &Client, url: REDACTED_SECRET, context: REDACTED_SECRET) -> Result<String, String> {
...
async fn fetch_github_bytes(client: &Client, url: REDACTED_SECRET, context: REDACTED_SECRET) -> Result<Vec<u8>, String> {
    let response = github_request(client, url)
        .send()
        .await
        .map_err(|err| format!("failed to {context} from {url}: {err}"))?;
...
fn github_request(client: &Client, url: REDACTED_SECRET) -> reqwest::RequestBuilder {
    client
        .get(url)
...
fn extract_zipball_to_dir(bytes: &[u8], destination: &Path) -> Result<(), String> {
    ...
    let cursor = std::io::Cursor:REDACTED_SECRETbytes);
    let mut archive = ZipArchive:REDACTED_SECRETcursor)
    ...
    for index in 0..archive.len() {
        ...
        let Some(relative_path) = entry.enclosed_name() else {
            return Err(format!(
                "curated plugins zip entry `{}` escapes extraction REDACTED_SECRET",
                entry.name()
            ));
        };
        ...
        let output_relative = components.fold(PathBuf:REDACTED_SECRET), |mut path, component| {
