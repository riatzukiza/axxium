...
fn write_plugin(REDACTED_SECRET: &Path, dir_name: REDACTED_SECRET, manifest_name: REDACTED_SECRET) {
    let plugin_REDACTED_SECRET = REDACTED_SECRET.join(dir_name);
    fs::create_dir_all(plugin_REDACTED_SECRET.join(".codex-plugin")).unwrap();
    fs::create_dir_all(plugin_REDACTED_SECRET.join("skills")).unwrap();
    fs::write(
        plugin_REDACTED_SECRET.join(".codex-plugin/plugin.json"),
        format!(r#"{{"name":"{manifest_name}"}}"#),
    )
    .unwrap();
    fs::write(plugin_REDACTED_SECRET.join("skills/SKILL.md"), "skill").unwrap();
    fs::write(plugin_REDACTED_SECRET.join(".mcp.json"), r#"{"mcpServers":{}}"#).unwrap();
}

fn plugin_config_toml(enabled: bool, plugins_feature_enabled: bool) -> String {
    let mut REDACTED_SECRET = toml::map::Map:REDACTED_SECRET);

    let mut features = toml::map::Map:REDACTED_SECRET);
    features.insert(
        "plugins".to_string(),
        Value::Boolean(plugins_feature_enabled),
    );
    REDACTED_SECRET.insert("features".to_string(), Value::Table(features));

    let mut plugin = toml::map::Map:REDACTED_SECRET);
    plugin.insert("enabled".to_string(), Value::Boolean(enabled));

    let mut plugins = toml::map::Map:REDACTED_SECRET);
    plugins.insert("sample@test".to_string(), Value::Table(plugin));
    REDACTED_SECRET.insert("plugins".to_string(), Value::Table(plugins));

    toml::to_string(&Value::Table(REDACTED_SECRET)).expect("plugin test config should serialize")
}

fn load_plugins_from_config(config_toml: REDACTED_SECRET, codex_home: &Path) -> PluginLoadOutcome {
    write_file(&codex_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config_blocking(codex_home, codex_home);
    PluginsManager:REDACTED_SECRETcodex_home.to_path_buf()).plugins_for_config(&config)
}
...
fn load_plugins_loads_default_skills_and_mcp_servers() {
    let codex_home = TempDir:REDACTED_SECRET).unwrap();
    let plugin_REDACTED_SECRET = codex_home
        .path()
        .join("plugins/cache")
  ...
  "description": "Plugin that includes the sample MCP server and Skills"
    ...
    write_file(
        &plugin_REDACTED_SECRET.join("skills/sample-search/SKILL.md"),
        "---\nname: sample-search\ndescription: search sample data\n---\n",
    );
    write_file(
        &plugin_REDACTED_SECRET.join(".mcp.json"),
        r#"{
  "mcpServers": {
    "sample": {
      "type": "http",
    ...
    write_file(
        &plugin_REDACTED_SECRET.join(".app.json"),
        r#"{
  "apps": {
    "example": {
    ...
    assert_eq!(
        ...
        vec![LoadedPlugin {
            ...
            REDACTED_SECRET: AbsolutePathBuf::try_from(plugin_REDACTED_SECRET.clone()).unwrap(),
            enabled: true,
            skill_REDACTED_SECRETs: vec![plugin_REDACTED_SECRET.join("skills")],
            disabled_skill_paths: HashSet:REDACTED_SECRET),
    ...
    assert_eq!(
        outcome.effective_skill_REDACTED_SECRETs(),
        vec![plugin_REDACTED_SECRET.join("skills")]
    );
...
fn load_plugins_resolves_disabled_skill_names_against_loaded_plugin_skills() {
    ...
    let plugin_REDACTED_SECRET = codex_home
        ...
        .join("plugins/cache")
        .join("test/sample/local");
    let skill_path = plugin_REDACTED_SECRET.join("skills/sample-search/SKILL.md");
...
"#;
    ...
    assert_eq!(
        ...
        &[PluginCapabilitySummary {
            ...
            description: None,
            has_skills: true,
            mcp_server_names: Vec:REDACTED_SECRET),
...
fn plugin_telemetry_metadata_uses_default_mcp_config_path() {
  ...
  "mcpServers": {
    ...
    let metadata = plugin_telemetry_metadata_from_REDACTED_SECRET(
        &PluginId::parse("sample@test").expect("plugin id should parse"),
        &plugin_REDACTED_SECRET,
    );
...
fn load_plugins_uses_manifest_configured_component_paths() {
  ...
  "apps": "./config/custom.app.json"
    ...
    write_file(
        &plugin_REDACTED_SECRET.join("skills/default-skill/SKILL.md"),
        "---\nname: default-skill\ndescription: default skill\n---\n",
    );
    write_file(
        &plugin_REDACTED_SECRET.join("custom-skills/custom-skill/SKILL.md"),
  ...
  "mcpServers": {
    ...
    write_file(
        &plugin_REDACTED_SECRET.join("config/custom.mcp.json"),
        r#"{
  "mcpServers": {
    "custom": {
      "type": "http",
  ...
  "apps": {
    ...
    write_file(
        &plugin_REDACTED_SECRET.join("config/custom.app.json"),
        r#"{
  "apps": {
    "custom": {
      "id": "connector_custom"
    ...
    assert_eq!(
        outcome.plugins()[0].skill_REDACTED_SECRETs,
        vec![
            plugin_REDACTED_SECRET.join("custom-skills"),
            plugin_REDACTED_SECRET.join("skills")
        ]
    );
    assert_eq!(
  ...
  "mcpServers": {
    ...
    assert_eq!(
        ...
        vec![LoadedPlugin {
            ...
            REDACTED_SECRET: AbsolutePathBuf::try_from(plugin_REDACTED_SECRET).unwrap(),
            enabled: false,
            skill_REDACTED_SECRETs: Vec:REDACTED_SECRET),
            disabled_skill_paths: HashSet:REDACTED_SECRET),
            has_enabled_skills: false,
            mcp_servers: HashMap:REDACTED_SECRET),
            apps: Vec:REDACTED_SECRET),
            error: None,
        }]
    );
    assert!(outcome.effective_skill_REDACTED_SECRETs().is_empty());
...
fn effective_apps_dedupes_connector_ids_across_plugins() {
    let codex_home = TempDir:REDACTED_SECRET).unwrap();
    let plugin_a_REDACTED_SECRET = codex_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_REDACTED_SECRET = codex_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_REDACTED_SECRET.join(".codex-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
        &plugin_a_REDACTED_SECRET.join(".app.json"),
        r#"{
  "apps": {
    "example": {
      "id": "connector_example"
    ...
    write_file(
        &plugin_b_REDACTED_SECRET.join(".codex-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
        &plugin_b_REDACTED_SECRET.join(".app.json"),
        r#"{
  "apps": {
    "chat": {
      "id": "connector_example"
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    let codex_home = TempDir:REDACTED_SECRET).unwrap();
    let connector = |id: REDACTED_SECRET| AppConnectorId(id.to_string());
    let http_server = |url: REDACTED_SECRET| McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url: url.to_string(),
            bearer_token_env_var: None,
            http_headers: None,
    ...
    let plugin = |config_name: REDACTED_SECRET, dir_name: REDACTED_SECRET, manifest_name: REDACTED_SECRET| LoadedPlugin {
        config_name: config_name.to_string(),
        manifest_name: Some(manifest_name.to_string()),
        manifest_description: None,
        REDACTED_SECRET: AbsolutePathBuf::try_from(codex_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_REDACTED_SECRETs: Vec:REDACTED_SECRET),
        disabled_skill_paths: HashSet:REDACTED_SECRET),
        has_enabled_skills: false,
    ...
    let summary = |config_name: REDACTED_SECRET, display_name: REDACTED_SECRET| PluginCapabilitySummary {
        config_name: config_name.to_string(),
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_REDACTED_SECRETs: vec![codex_home.path().join("skills-plugin/skills")],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_REDACTED_SECRETs: vec![codex_home.path().join("disabled-plugin/skills")],
...
fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let outcome = PluginsManager:REDACTED_SECRETcodex_home.path().to_path_buf()).plugins_for_config(&config);
...
async fn install_plugin_updates_config_with_relative_path_and_plugin_key() {
    ...
    let repo_REDACTED_SECRET = tmp.path().join("repo");
    fs::create_dir_all(repo_REDACTED_SECRET.join(".git")).unwrap();
    fs::create_dir_all(repo_REDACTED_SECRET.join(".agents/plugins")).unwrap();
    write_plugin(&repo_REDACTED_SECRET, "sample-plugin", "sample-plugin");
    fs::write(
        repo_REDACTED_SECRET.join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "debug",
  "plugins": [
    {
    ...
    let result = PluginsManager:REDACTED_SECRETtmp.path().to_path_buf())
    ...
    assert_eq!(
        ...
        PluginInstallOutcome {
            plugin_id: PluginId:REDACTED_SECRET"sample-plugin".to_string(), "debug".to_string()).unwrap(),
...
"#,
    ...
    let manager = PluginsManager:REDACTED_SECRETtmp.path().to_path_buf());
    manager
...
"#,
    ...
    let config = load_config(tmp.path(), &repo_REDACTED_SECRET).await;
    let marketplaces = PluginsManager:REDACTED_SECRETtmp.path().to_path_buf())
        .list_marketplaces_for_config(&config, &[AbsolutePathBuf::try_from(repo_REDACTED_SECRET).unwrap()])
        .unwrap()
        .marketplaces;
        ...
        ConfiguredMarketplace {
            ...
            plugins: vec![
                ConfiguredMarketplacePlugin {
                    ...
                    source: MarketplacePluginSource:REDACTED_SECRET {
...
"#,
    ...
    assert_eq!(marketplaces, Vec:REDACTED_SECRET));
}
...
async fn read_plugin_for_config_returns_plugins_disabled_when_feature_disabled() {
    ...
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_REDACTED_SECRET.join(".agents/plugins/marketplace.json")).unwrap();
    fs::write(
...
"#,
    ...
    let config = load_config(tmp.path(), &repo_REDACTED_SECRET).await;
    let err = PluginsManager:REDACTED_SECRETtmp.path().to_path_buf())
...
async fn read_plugin_for_config_uses_user_layer_skill_settings_only() {
    let tmp = tempfile::tempdir().unwrap();
    let repo_REDACTED_SECRET = tmp.path().join("repo");
    let plugin_REDACTED_SECRET = repo_REDACTED_SECRET.join("enabled-plugin");
    fs::create_dir_all(repo_REDACTED_SECRET.join(".git")).unwrap();
    fs::create_dir_all(repo_REDACTED_SECRET.join(".agents/plugins")).unwrap();
    write_file(
        &repo_REDACTED_SECRET.join(".agents/plugins/marketplace.json"),
...
"#,
    ...
    write_file(
        &repo_REDACTED_SECRET.join(".codex/config.toml"),
        r#"[[skills.config]]
...
"#,
    ...
    let config = load_config(tmp.path(), &repo_REDACTED_SECRET).await;
    let outcome = PluginsManager:REDACTED_SECRETtmp.path().to_path_buf())
        .read_plugin_for_config(
            &config,
            &PluginReadRequest {
                plugin_name: "enabled-plugin".to_string(),
...
async fn list_marketplaces_includes_curated_repo_marketplace() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_REDACTED_SECRET = curated_plugins_repo_path(tmp.path());
    let plugin_REDACTED_SECRET = curated_REDACTED_SECRET.join("plugins/linear");
...
"#,
    ...
    fs::create_dir_all(curated_REDACTED_SECRET.join(".agents/plugins")).unwrap();
    fs::create_dir_all(plugin_REDACTED_SECRET.join(".codex-plugin")).unwrap();
    fs::write(
        curated_REDACTED_SECRET.join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "openai-curated",
  "interface": {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        ConfiguredMarketplace {
            ...
            path: AbsolutePathBuf::try_from(curated_REDACTED_SECRET.join(".agents/plugins/marketplace.json"))
                .unwrap(),
            ...
            plugins: vec![ConfiguredMarketplacePlugin {
                ...
                source: MarketplacePluginSource:REDACTED_SECRET {
                    path: AbsolutePathBuf::try_from(curated_REDACTED_SECRET.join("plugins/linear")).unwrap(),
                },
                policy: MarketplacePluginPolicy {
                    installation: MarketplacePluginInstallPolicy::Available,
                    authentication: MarketplacePluginAuthPolicy::OnInstall,
...
async fn list_marketplaces_uses_first_duplicate_plugin_entry() {
    let tmp = tempfile::tempdir().unwrap();
    let repo_a_REDACTED_SECRET = tmp.path().join("repo-a");
    let repo_b_REDACTED_SECRET = tmp.path().join("repo-b");
    fs::create_dir_all(repo_a_REDACTED_SECRET.join(".git")).unwrap();
    fs::create_dir_all(repo_b_REDACTED_SECRET.join(".git")).unwrap();
    fs::create_dir_all(repo_a_REDACTED_SECRET.join(".agents/plugins")).unwrap();
    fs::create_dir_all(repo_b_REDACTED_SECRET.join(".agents/plugins")).unwrap();
    fs::write(
        repo_a_REDACTED_SECRET.join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "debug",
  "plugins": [
    {
    ...
    .unwrap();
    fs::write(
        repo_b_REDACTED_SECRET.join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "debug",
  "plugins": [
    {
...
"#,
    ...
    let config = load_config(tmp.path(), &repo_a_REDACTED_SECRET).await;
    let marketplaces = PluginsManager:REDACTED_SECRETtmp.path().to_path_buf())
        .list_marketplaces_for_config(
            &config,
            &[
                AbsolutePathBuf::try_from(repo_a_REDACTED_SECRET).unwrap(),
                AbsolutePathBuf::try_from(repo_b_REDACTED_SECRET).unwrap(),
            ],
        )
        .unwrap()
        .marketplaces;
...
async fn sync_plugins_from_remote_reconciles_cache_and_config() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_REDACTED_SECRET = curated_plugins_repo_path(tmp.path());
    write_openai_curated_marketplace(&curated_REDACTED_SECRET, &["linear", "gmail", "calendar"]);
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate:REDACTED_SECRET200).set_body_string(
            r#"[
  ...
  {"id":"2","name":"gmail","marketplace_name":"openai-curated","version":"1.0.0","enabled":false}
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            installed_plugin_ids: Vec:REDACTED_SECRET),
            enabled_plugin_ids: vec!["linear@openai-curated".to_string()],
            disabled_plugin_ids: Vec:REDACTED_SECRET),
  ...
  {"id":"2","name":"gmail","marketplace_name":"openai-curated","version":"1.0.0","enabled":false}
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            ...
            enabled_plugin_ids: vec!["linear@openai-curated".to_string()],
            disabled_plugin_ids: Vec:REDACTED_SECRET),
            uninstalled_plugin_ids: Vec:REDACTED_SECRET),
        }
    );
...
async fn sync_plugins_from_remote_ignores_unknown_remote_plugins() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_REDACTED_SECRET = curated_plugins_repo_path(tmp.path());
    write_openai_curated_marketplace(&curated_REDACTED_SECRET, &["linear"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
  ...
  {"id":"1","name":"plugin-one","marketplace_name":"openai-curated","version":"1.0.0","enabled":true}
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            installed_plugin_ids: Vec:REDACTED_SECRET),
            enabled_plugin_ids: Vec:REDACTED_SECRET),
            disabled_plugin_ids: Vec:REDACTED_SECRET),
...
async fn sync_plugins_from_remote_keeps_existing_plugins_when_install_fails() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_REDACTED_SECRET = curated_plugins_repo_path(tmp.path());
    write_openai_curated_marketplace(&curated_REDACTED_SECRET, &["linear", "gmail"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    fs::remove_dir_all(curated_REDACTED_SECRET.join("plugins/gmail")).unwrap();
    write_plugin(
        &tmp.path().join("plugins/cache/openai-curated"),
        "linear/local",
        "linear",
...
async fn sync_plugins_from_remote_uses_first_duplicate_local_plugin_entry() {
  ...
  "plugins": [
    ...
    .unwrap();
    write_plugin(&curated_REDACTED_SECRET, "plugins/gmail-first", "gmail");
    write_plugin(&curated_REDACTED_SECRET, "plugins/gmail-second", "gmail");
    fs::write(curated_REDACTED_SECRET.join("plugins/gmail-first/marker.txt"), "first").unwrap();
    fs::write(
        curated_REDACTED_SECRET.join("plugins/gmail-second/marker.txt"),
        "second",
    )
    .unwrap();
    write_file(
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .and(header("authorization", "Bearer Access Token"))
        .and(header("chatgpt-account-id", "account_id"))
        .respond_with(ResponseTemplate:REDACTED_SECRET200).set_body_string(r#"["chat-plugin"]"#))
        .mount(&server)
        .await;
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate:REDACTED_SECRET200).set_body_string(r#"["codex-plugin"]"#))
        .mount(&server)
        .await;
...
fn refresh_curated_plugin_cache_replaces_existing_local_version_with_sha() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_REDACTED_SECRET = curated_plugins_repo_path(tmp.path());
    write_openai_curated_marketplace(&curated_REDACTED_SECRET, &["slack"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    let plugin_id = PluginId:REDACTED_SECRET
        "slack".to_string(),
...
fn load_plugins_ignores_project_config_files() {
    let codex_home = TempDir:REDACTED_SECRET).unwrap();
    let project_REDACTED_SECRET = codex_home.path().join("project");
    let plugin_REDACTED_SECRET = codex_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_REDACTED_SECRET.join(".codex/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );

    let stack = ConfigLayerStack:REDACTED_SECRET
        vec![ConfigLayerEntry:REDACTED_SECRET
            ConfigLayerSource::Project {
                dot_codex_folder: AbsolutePathBuf::try_from(project_REDACTED_SECRET.join(".codex")).unwrap(),
            },
    ...
    let outcome = load_plugins_from_layer_stack(
        &stack,
        &PluginStore:REDACTED_SECRETcodex_home.path().to_path_buf()),
        Some(Product::Codex),
    );
