...
struct RawPluginManifest {
    ...
    // resolving them under the plugin REDACTED_SECRET.
    #[serde(default)]
...
pub(crate) fn load_plugin_manifest(plugin_REDACTED_SECRET: &Path) -> Option<PluginManifest> {
    let manifest_path = plugin_REDACTED_SECRET.join(PLUGIN_MANIFEST_PATH);
    ...
    match serde_json::from_str::<RawPluginManifest>(&contents) {
        Ok(manifest) => {
            ...
            let name = plugin_REDACTED_SECRET
                .file_name()
            ...
            let interface = interface.and_then(|interface| {
                ...
                let interface = PluginManifestInterface {
                    ...
                    privacy_policy_url,
                    terms_of_service_url,
                    default_prompt: resolve_default_prompts(plugin_REDACTED_SECRET, default_prompt.as_ref()),
                    brand_color,
            ...
            Some(PluginManifest {
                ...
                paths: PluginManifestPaths {
                    skills: resolve_manifest_path(plugin_REDACTED_SECRET, "skills", skills.as_deref()),
                    mcp_servers: resolve_manifest_path(
                        plugin_REDACTED_SECRET,
                        "mcpServers",
                        mcp_servers.as_deref(),
                    ),
                    apps: resolve_manifest_path(plugin_REDACTED_SECRET, "apps", apps.as_deref()),
                },
                interface,
            })
        }
...
fn resolve_interface_asset_path(
    ...
    field: &'static str,
    path: Option<REDACTED_SECRET>,
) -> Option<AbsolutePathBuf> {
    resolve_manifest_path(plugin_REDACTED_SECRET, field, path)
}
...
fn resolve_default_prompts(
    ...
    match value? {
        RawPluginManifestDefaultPrompt::String(prompt) => {
            resolve_default_prompt_str(plugin_REDACTED_SECRET, "interface.defaultPrompt", prompt)
                .map(|prompt| vec![prompt])
        }
        RawPluginManifestDefaultPrompt::List(values) => {
            let mut prompts = Vec:REDACTED_SECRET);
            for (index, item) in values.iter().enumerate() {
                ...
                match item {
                    RawPluginManifestDefaultPromptEntry::String(prompt) => {
                        ...
                        if let Some(prompt) =
                            resolve_default_prompt_str(plugin_REDACTED_SECRET, &field, prompt)
                        {
                            prompts.push(prompt);
                        }
                    }
...
fn resolve_default_prompt_str(plugin_REDACTED_SECRET: &Path, field: REDACTED_SECRET, prompt: REDACTED_SECRET) -> Option<String> {
    let prompt = prompt.split_whitespace().collect::<Vec<_>>().join(" ");
    if prompt.is_empty() {
        warn_invalid_default_prompt(plugin_REDACTED_SECRET, field, "prompt must not be empty");
        return None;
    }
...
fn warn_invalid_default_prompt(plugin_REDACTED_SECRET: &Path, field: REDACTED_SECRET, message: REDACTED_SECRET) {
    let manifest_path = plugin_REDACTED_SECRET.join(PLUGIN_MANIFEST_PATH);
    tracing::warn!(
        path = %manifest_path.display(),
...
fn resolve_manifest_path(
    ...
    // `plugin.json` paths are required to be relative to the plugin REDACTED_SECRET and we return the
    ...
    let Some(relative_path) = path.strip_prefix("./") else {
        tracing::warn!("ignoring {field}: path must start with `./` relative to plugin REDACTED_SECRET");
        return None;
    };
    ...
    let mut normalized = std::path::PathBuf:REDACTED_SECRET);
    for component in Path:REDACTED_SECRETrelative_path).components() {
        match component {
            ...
            _ => {
                tracing::warn!("ignoring {field}: path must stay within the plugin REDACTED_SECRET");
                return None;
            }
        }
    }

    AbsolutePathBuf::try_from(plugin_REDACTED_SECRET.join(normalized))
        .map_err(|err| {
...
mod tests {
    ...
    fn write_manifest(plugin_REDACTED_SECRET: &Path, interface: REDACTED_SECRET) {
        fs::create_dir_all(plugin_REDACTED_SECRET.join(".codex-plugin")).expect("create manifest dir");
        fs::write(
  ...
  "interface": {interface}
    ...
    fn load_manifest(plugin_REDACTED_SECRET: &Path) -> PluginManifest {
        load_plugin_manifest(plugin_REDACTED_SECRET).expect("load plugin manifest")
    }

    #[test]
    fn plugin_interface_accepts_legacy_default_prompt_string() {
        let tmp = tempdir().expect("tempdir");
        let plugin_REDACTED_SECRET = tmp.path().join("demo-plugin");
        write_manifest(
            &plugin_REDACTED_SECRET,
            r#"{
    ...
    "defaultPrompt": "  Summarize   my inbox  "
        ...
        let manifest = load_manifest(&plugin_REDACTED_SECRET);
