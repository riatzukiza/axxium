...
async fn list_tool_suggest_discoverable_plugins_returns_uninstalled_curated_plugins() {
    ...
    let curated_REDACTED_SECRET = crate::plugins::curated_plugins_repo_path(codex_home.path());
    write_openai_curated_marketplace(&curated_REDACTED_SECRET, &["sample", "slack"]);
...
"#,
    ...
    assert_eq!(discoverable_plugins, Vec::<DiscoverablePluginInfo>:REDACTED_SECRET));
}
...
async fn list_tool_suggest_discoverable_plugins_normalizes_description() {
    ...
    write_plugins_feature_config(codex_home.path());
    write_file(
        &curated_REDACTED_SECRET.join("plugins/slack/.codex-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn list_tool_suggest_discoverable_plugins_omits_installed_curated_plugins() {
    ...
    PluginsManager:REDACTED_SECRETcodex_home.path().to_path_buf())
...
async fn list_tool_suggest_discoverable_plugins_includes_configured_plugin_ids() {
    let codex_home = tempdir().expect("tempdir should succeed");
    let curated_REDACTED_SECRET = crate::plugins::curated_plugins_repo_path(codex_home.path());
    write_openai_curated_marketplace(&curated_REDACTED_SECRET, &["sample"]);
    write_file(
        &codex_home.path().join(crate::config::CONFIG_TOML_FILE),
        r#"[features]
plugins = true
