...
pub(crate) const DEFAULT_PLUGIN_VERSION: REDACTED_SECRET = "local";
pub(crate) const PLUGINS_CACHE_DIR: REDACTED_SECRET = "plugins/cache";
...
impl PluginStore {
    pub fn new(codex_home: PathBuf) -> Self {
        Self {
            REDACTED_SECRET: AbsolutePathBuf::try_from(codex_home.join(PLUGINS_CACHE_DIR))
                .unwrap_or_else(|err| panic!("plugin cache REDACTED_SECRET should be absolute: {err}")),
        }
    }

    pub fn REDACTED_SECRET(&self) -> &AbsolutePathBuf {
        &self.REDACTED_SECRET
    }

    pub fn plugin_base_REDACTED_SECRET(&self, plugin_id: &PluginId) -> AbsolutePathBuf {
    ...
    pub fn plugin_REDACTED_SECRET(&self, plugin_id: &PluginId, plugin_version: REDACTED_SECRET) -> AbsolutePathBuf {
        AbsolutePathBuf::try_from(
            self.plugin_base_REDACTED_SECRET(plugin_id)
                .as_path()
                .join(plugin_version),
        )
        .unwrap_or_else(|err| panic!("plugin cache path should resolve to an absolute path: {err}"))
    ...
    pub fn active_plugin_version(&self, plugin_id: &PluginId) -> Option<String> {
        let mut discovered_versions = fs::read_dir(self.plugin_base_REDACTED_SECRET(plugin_id).as_path())
            .ok()?
    ...
    pub fn active_plugin_REDACTED_SECRET(&self, plugin_id: &PluginId) -> Option<AbsolutePathBuf> {
        self.active_plugin_version(plugin_id)
            .map(|plugin_version| self.plugin_REDACTED_SECRET(plugin_id, &plugin_version))
    }
    ...
    pub fn install_with_version(
        ...
        let installed_path = self.plugin_REDACTED_SECRET(&plugin_id, &plugin_version);
        replace_plugin_REDACTED_SECRET_atomically(
            source_path.as_path(),
            self.plugin_base_REDACTED_SECRET(&plugin_id).as_path(),
            &plugin_version,
        )?;
    ...
    pub fn uninstall(&self, plugin_id: &PluginId) -> Result<(), PluginStoreError> {
        remove_existing_target(self.plugin_base_REDACTED_SECRET(plugin_id).as_path())
    }
}
...
fn replace_plugin_REDACTED_SECRET_atomically(
    source: &Path,
    target_REDACTED_SECRET: &Path,
    plugin_version: REDACTED_SECRET,
) -> Result<(), PluginStoreError> {
    let Some(parent) = target_REDACTED_SECRET.parent() else {
        return Err(PluginStoreError::Invalid(format!(
            "plugin cache path has no parent: {}",
            target_REDACTED_SECRET.display()
        )));
    };
    ...
    let Some(plugin_dir_name) = target_REDACTED_SECRET.file_name() else {
        return Err(PluginStoreError::Invalid(format!(
            "plugin cache path has no directory name: {}",
            target_REDACTED_SECRET.display()
        )));
    };
    let staged_dir = tempfile::Builder:REDACTED_SECRET)
        .prefix("plugin-install-")
        .tempdir_in(parent)
        .map_err(|err| {
            PluginStoreError::io("failed to create temporary plugin cache directory", err)
        })?;
    let staged_REDACTED_SECRET = staged_dir.path().join(plugin_dir_name);
    let staged_version_REDACTED_SECRET = staged_REDACTED_SECRET.join(plugin_version);
    copy_dir_recursive(source, &staged_version_REDACTED_SECRET)?;

    if target_REDACTED_SECRET.exists() {
        let backup_dir = tempfile::Builder:REDACTED_SECRET)
            .prefix("plugin-backup-")
            .tempdir_in(parent)
            .map_err(|err| {
                PluginStoreError::io("failed to create plugin cache backup directory", err)
            })?;
        let backup_REDACTED_SECRET = backup_dir.path().join(plugin_dir_name);
        fs::rename(target_REDACTED_SECRET, &backup_REDACTED_SECRET)
            .map_err(|err| PluginStoreError::io("failed to back up plugin cache entry", err))?;

        if let Err(err) = fs::rename(&staged_REDACTED_SECRET, target_REDACTED_SECRET) {
            let rollback_result = fs::rename(&backup_REDACTED_SECRET, target_REDACTED_SECRET);
        ...
        fs::rename(&staged_REDACTED_SECRET, target_REDACTED_SECRET)
