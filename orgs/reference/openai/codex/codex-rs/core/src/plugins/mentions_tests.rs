...
fn text_input(text: REDACTED_SECRET) -> UserInput {
    UserInput::Text {
        text: text.to_string(),
...
fn plugin(config_name: REDACTED_SECRET, display_name: REDACTED_SECRET) -> PluginCapabilitySummary {
...
fn collect_explicit_app_ids_ignores_non_app_paths() {
    ...
    assert_eq!(app_ids, HashSet::<String>:REDACTED_SECRET));
}
...
fn collect_explicit_plugin_mentions_ignores_non_plugin_paths() {
    ...
    assert_eq!(mentioned, Vec::<PluginCapabilitySummary>:REDACTED_SECRET));
}
