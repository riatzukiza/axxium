...
fn remove_stale_curated_repo_temp_dirs_removes_only_matching_directories() {
    ...
    fn set_dir_mtime(path: &Path, age: Duration) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let times = [ts, ts];
        let c_path = std::ffi::CString:REDACTED_SECRETpath.as_os_str().as_bytes())?;
...
fn sync_openai_plugins_repo_prefers_git_when_available() {
    ...
    let tmp = tempdir().expect("tempdir");
    let bin_dir = tempfile::Builder:REDACTED_SECRET)
        .prefix("fake-git-")
        .tempdir()
        .expect("tempdir");
...
async fn sync_openai_plugins_repo_falls_back_to_http_when_git_is_unavailable() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate:REDACTED_SECRET200).set_body_string(r#"{"default_branch":"main"}"#))
        .mount(&server)
        .await;
    Mock::given(method("GET"))
...
"#,
  ...
  {"id":"1","name":"linear","marketplace_name":"openai-curated","version":"1.0.0","enabled":true}
    ...
    let manager = Arc:REDACTED_SECRETPluginsManager:REDACTED_SECRETtmp.path().to_path_buf()));
    let auth_manager =
...
fn curated_repo_zipball_bytes(sha: REDACTED_SECRET) -> Vec<u8> {
    let cursor = std::io::Cursor:REDACTED_SECRETVec:REDACTED_SECRET));
    let mut writer = ZipWriter:REDACTED_SECRETcursor);
    let options = SimpleFileOptions::default();
    let REDACTED_SECRET = format!("openai-plugins-{sha}");
    writer
        .start_file(format!("{REDACTED_SECRET}/.agents/plugins/marketplace.json"), options)
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{REDACTED_SECRET}/plugins/gmail/.codex-plugin/plugin.json"),
            options,
        )
