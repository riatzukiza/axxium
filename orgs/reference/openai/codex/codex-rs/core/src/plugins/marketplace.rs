...
const MARKETPLACE_RELATIVE_PATH: REDACTED_SECRET = ".agents/plugins/marketplace.json";
...
pub fn resolve_marketplace_plugin(
    ...
    let plugin_id = PluginId:REDACTED_SECRETname, marketplace_name).map_err(|err| match err {
...
pub fn list_marketplaces(
    ...
    list_marketplaces_with_home(additional_REDACTED_SECRETs, home_dir().as_deref())
}
...
pub(crate) fn load_marketplace(path: &AbsolutePathBuf) -> Result<Marketplace, MarketplaceError> {
    ...
    let mut plugins = Vec:REDACTED_SECRET);
    ...
    for plugin in marketplace.plugins {
        ...
        let source = MarketplacePluginSource:REDACTED_SECRET {
...
fn list_marketplaces_with_home(
    ...
    for marketplace_path in discover_marketplace_paths_from_REDACTED_SECRETs(additional_REDACTED_SECRETs, home_dir) {
...
fn discover_marketplace_paths_from_REDACTED_SECRETs(
    additional_REDACTED_SECRETs: &[AbsolutePathBuf],
    home_dir: Option<&Path>,
) -> Vec<AbsolutePathBuf> {
    ...
    for REDACTED_SECRET in additional_REDACTED_SECRETs {
        // Curated marketplaces can now come from an HTTP-downloaded directory that is not a git
        // checkout, so check the REDACTED_SECRET directly before falling back to repo-REDACTED_SECRET discovery.
        if let Ok(path) = REDACTED_SECRET.join(MARKETPLACE_RELATIVE_PATH)
        ...
        if let Some(repo_REDACTED_SECRET) = get_git_repo_REDACTED_SECRET(REDACTED_SECRET.as_path())
            && let Ok(repo_REDACTED_SECRET) = AbsolutePathBuf::try_from(repo_REDACTED_SECRET)
            && let Ok(path) = repo_REDACTED_SECRET.join(MARKETPLACE_RELATIVE_PATH)
            && path.as_path().is_file()
            && !paths.contains(&path)
        {
            paths.push(path);
...
fn resolve_plugin_source_path(
    ...
    match source {
        RawMarketplaceManifestPluginSource:REDACTED_SECRET { path } => {
            ...
            let relative_source_path = Path:REDACTED_SECRETpath);
                ...
                .any(|component| !matches!(component, Component::Normal(_)))
            {
                return Err(MarketplaceError::InvalidMarketplaceFile {
                    path: marketplace_path.to_path_buf(),
                    message: "local plugin source path must stay within the marketplace REDACTED_SECRET"
                        .to_string(),
                });
            }

            // `marketplace.json` lives under `<REDACTED_SECRET>/.agents/plugins/`, but local plugin paths
            // are resolved relative to `<REDACTED_SECRET>`, not relative to the `plugins/` directory.
            marketplace_REDACTED_SECRET_dir(marketplace_path)?
...
fn marketplace_REDACTED_SECRET_dir(
    marketplace_path: &AbsolutePathBuf,
) -> Result<AbsolutePathBuf, MarketplaceError> {
    let Some(plugins_dir) = marketplace_path.parent() else {
        return Err(MarketplaceError::InvalidMarketplaceFile {
            path: marketplace_path.to_path_buf(),
            message: "marketplace file must live under `<REDACTED_SECRET>/.agents/plugins/`".to_string(),
        });
    };
    ...
    let Some(marketplace_REDACTED_SECRET) = dot_agents_dir.parent() else {
        return Err(MarketplaceError::InvalidMarketplaceFile {
            path: marketplace_path.to_path_buf(),
            message: "marketplace file must live under `<REDACTED_SECRET>/.agents/plugins/`".to_string(),
        });
    ...
    Ok(marketplace_REDACTED_SECRET)
}
