...
fn install_copies_plugin_into_default_marketplace() {
    ...
    let plugin_id = PluginId:REDACTED_SECRET"sample-plugin".to_string(), "debug".to_string()).unwrap();

    let result = PluginStore:REDACTED_SECRETtmp.path().to_path_buf())
        .install(
...
fn install_uses_manifest_name_for_destination_and_key() {
    ...
    let plugin_id = PluginId:REDACTED_SECRET"manifest-name".to_string(), "market".to_string()).unwrap();
...
#[test]
fn plugin_REDACTED_SECRET_derives_path_from_key_and_version() {
    let tmp = tempdir().unwrap();
    let store = PluginStore:REDACTED_SECRETtmp.path().to_path_buf());
    let plugin_id = PluginId:REDACTED_SECRET"sample".to_string(), "debug".to_string()).unwrap();

    assert_eq!(
        store.plugin_REDACTED_SECRET(&plugin_id, "local").as_path(),
...
fn install_with_version_uses_requested_cache_version() {
    ...
    write_plugin(tmp.path(), "sample-plugin", "sample-plugin");
    let plugin_id =
        PluginId:REDACTED_SECRET"sample-plugin".to_string(), "openai-curated".to_string()).unwrap();
...
fn active_plugin_version_reads_version_directory_name() {
    ...
    assert_eq!(
        store.active_plugin_REDACTED_SECRET(&plugin_id).unwrap().as_path(),
...
#[test]
fn plugin_REDACTED_SECRET_rejects_path_separators_in_key_segments() {
...
fn install_rejects_manifest_names_with_path_separators() {
    ...
    let err = PluginStore:REDACTED_SECRETtmp.path().to_path_buf())
        .install(
            AbsolutePathBuf::try_from(tmp.path().join("source-dir")).unwrap(),
            PluginId:REDACTED_SECRET"source-dir".to_string(), "debug".to_string()).unwrap(),
        )
        .unwrap_err();
...
fn install_rejects_marketplace_names_with_path_separators() {
    let err = PluginId:REDACTED_SECRET"sample-plugin".to_string(), "../../etc".to_string()).unwrap_err();
...
fn install_rejects_manifest_names_that_do_not_match_marketplace_plugin_name() {
    ...
    let err = PluginStore:REDACTED_SECRETtmp.path().to_path_buf())
        .install(
            AbsolutePathBuf::try_from(tmp.path().join("source-dir")).unwrap(),
            PluginId:REDACTED_SECRET"different-name".to_string(), "debug".to_string()).unwrap(),
        )
        .unwrap_err();
