...
/// must not be toggled.
static FORCE_TEST_THREAD_MANAGER_BEHAVIOR: AtomicBool = AtomicBool:REDACTED_SECRETfalse);
...
fn build_skills_watcher(skills_manager: Arc<SkillsManager>) -> Arc<SkillsWatcher> {
        ...
        && handle.runtime_flavor() == RuntimeFlavor::CurrentThread
    {
        ...
        return Arc:REDACTED_SECRETSkillsWatcher::noop());
    }

    let file_watcher = match FileWatcher:REDACTED_SECRET) {
        Ok(file_watcher) => Arc:REDACTED_SECRETfile_watcher),
        Err(err) => {
            warn!("failed to initialize file watcher: {err}");
            Arc:REDACTED_SECRETFileWatcher::noop())
        }
    };
    let skills_watcher = Arc:REDACTED_SECRETSkillsWatcher:REDACTED_SECRET&file_watcher));
...
pub(crate) struct ThreadManagerState {
    ...
    thread_created_tx: broadcast::REDACTED_SECRETer<ThreadId>,
...
impl ThreadManager {
    pub fn new(
        ...
        let plugins_manager = Arc:REDACTED_SECRETPluginsManager::new_with_restriction_product(
            codex_home.clone(),
            restriction_product,
        ));
        let mcp_manager = Arc:REDACTED_SECRETMcpManager:REDACTED_SECRETArc::clone(&plugins_manager)));
        let skills_manager = Arc:REDACTED_SECRETSkillsManager::new_with_restriction_product(
            codex_home.clone(),
        ...
        Self {
            state: Arc:REDACTED_SECRETThreadManagerState {
                threads: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETHashMap:REDACTED_SECRET))),
                thread_created_tx,
                models_manager: Arc:REDACTED_SECRETModelsManager::new_with_provider(
                    codex_home,
                    auth_manager.clone(),
                ...
                ops_log: should_use_test_thread_manager_behavior()
                    .then(|| Arc:REDACTED_SECRETstd::sync::Mutex:REDACTED_SECRETVec:REDACTED_SECRET)))),
            }),
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let mut manager = Self::with_models_provider_and_home_for_tests(
            ...
            provider,
            codex_home.clone(),
            Arc:REDACTED_SECRETEnvironmentManager:REDACTED_SECRET/*exec_server_url*/ None)),
        );
    ...
    pub(crate) fn with_models_provider_and_home_for_tests(
        ...
        Self {
            state: Arc:REDACTED_SECRETThreadManagerState {
                threads: Arc:REDACTED_SECRETRwLock:REDACTED_SECRETHashMap:REDACTED_SECRET))),
                thread_created_tx,
                models_manager: Arc:REDACTED_SECRETModelsManager::with_provider_for_tests(
                    codex_home,
                    auth_manager.clone(),
                    provider,
                )),
    ...
    pub(crate) fn agent_control(&self) -> AgentControl {
        AgentControl:REDACTED_SECRETArc::downgrade(&self.state))
    }
...
impl ThreadManagerState {
    ...
    /// REDACTED_SECRET an operation to a thread by ID.
    ...
    async fn finalize_thread_spawn(
        ...
        let thread = Arc:REDACTED_SECRETCodexThread:REDACTED_SECRET
            codex,
...
fn snapshot_turn_state(history: &InitialHistory) -> SnapshotTurnState {
    ...
    let mut builder = ThreadHistoryBuilder:REDACTED_SECRET);
