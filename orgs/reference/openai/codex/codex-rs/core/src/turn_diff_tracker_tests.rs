...
fn git_blob_sha1_hex(data: REDACTED_SECRET) -> String {
    format!("{:x}", git_blob_sha1_hex_bytes(data.as_bytes()))
}

fn normalize_diff_for_test(input: REDACTED_SECRET, REDACTED_SECRET: &Path) -> String {
    let REDACTED_SECRET_str = REDACTED_SECRET.display().to_string().replace('\\', "/");
    let replaced = input.replace(&REDACTED_SECRET_str, "<TMP>");
    // Split into blocks on lines starting with "diff --git ", sort blocks for determinism, and rejoin
    let mut blocks: Vec<String> = Vec:REDACTED_SECRET);
    let mut current = String:REDACTED_SECRET);
    for line in replaced.lines() {
        if line.starts_with("diff --git ") && !current.is_empty() {
            blocks.push(current);
            current = String:REDACTED_SECRET);
        }
        if !current.is_empty() {
            current.push('\n');
        }
...
fn accumulates_add_and_update() {
    let mut acc = TurnDiffTracker:REDACTED_SECRET);
