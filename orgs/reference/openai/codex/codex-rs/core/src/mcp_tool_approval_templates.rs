...
const CONNECTOR_NAME_TEMPLATE_VAR: REDACTED_SECRET = "{connector_name}";

static CONSEQUENTIAL_TOOL_MESSAGE_TEMPLATES: LazyLock<
    Option<Vec<ConsequentialToolMessageTemplate>>,
> = LazyLock:REDACTED_SECRETload_consequential_tool_message_templates);
...
fn render_question_template(template: REDACTED_SECRET, connector_name: Option<REDACTED_SECRET>) -> Option<String> {
...
fn render_tool_params(
    ...
    let mut display_params = Vec:REDACTED_SECRET);
    let mut display_names = HashSet:REDACTED_SECRET);
    let mut handled_names = HashSet:REDACTED_SECRET);
...
mod tests {
    ...
    fn returns_none_when_no_exact_match_exists() {
        let templates = vec![ConsequentialToolMessageTemplate {
            ...
            tool_title: "create_event".to_string(),
            template: "Allow {connector_name} to create an event?".to_string(),
            template_params: Vec:REDACTED_SECRET),
        }];
    ...
    fn renders_literal_template_without_connector_substitution() {
        ...
        assert_eq!(
            ...
            Some(RenderedMcpToolApprovalTemplate {
                ...
                tool_params: Some(json!({})),
                tool_params_display: Vec:REDACTED_SECRET),
            })
        );
    }
