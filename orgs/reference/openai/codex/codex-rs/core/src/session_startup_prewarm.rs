    ...
    fn resolution_from_join_result(
        ...
        match result {
            Ok(Ok(prewarmed_session)) => {
                SessionStartupPrewarmResolution::Ready(Box:REDACTED_SECRETprewarmed_session))
            }
            Ok(Err(err)) => {
...
impl Session {
    pub(crate) async fn schedule_startup_prewarm(self: &Arc<Self>, base_instructions: String) {
        ...
        self.set_session_startup_prewarm(SessionStartupPrewarmHandle:REDACTED_SECRET
            startup_prewarm,
            started_at,
...
async fn schedule_startup_prewarm_inner(
    ...
    let startup_turn_context = session
        ...
        .await;
    let startup_cancellation_token = CancellationToken:REDACTED_SECRET);
