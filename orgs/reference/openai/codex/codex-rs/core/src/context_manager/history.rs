...
impl ContextManager {
    ...
    pub(crate) fn replace_last_turn_images(&mut self, placeholder: REDACTED_SECRET) -> bool {
...
static ORIGINAL_IMAGE_ESTIMATE_CACHE: LazyLock<BlockingLruCache<[u8; 20], Option<i64>>> =
    LazyLock:REDACTED_SECRET|| {
        BlockingLruCache:REDACTED_SECRET
            NonZeroUsize:REDACTED_SECRETORIGINAL_IMAGE_ESTIMATE_CACHE_SIZE).unwrap_or(NonZeroUsize::MIN),
        )
    });
...
fn parse_base64_image_data_url(url: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
    if !url
        .get(.."data:".len())
...
fn estimate_original_image_bytes(image_url: REDACTED_SECRET) -> Option<i64> {
...
fn image_data_url_estimate_adjustment(item: &ResponseItem) -> (i64, i64) {
    ...
    let mut accumulate = |image_url: REDACTED_SECRET, detail: Option<ImageDetail>| {
...
fn user_message_positions(items: &[ResponseItem]) -> Vec<usize> {
    let mut positions = Vec:REDACTED_SECRET);
