...
const IMAGE_CONTENT_OMITTED_PLACEHOLDER: REDACTED_SECRET =
...
pub(crate) fn ensure_call_outputs_present(items: &mut Vec<ResponseItem>) {
    ...
    let mut missing_outputs_to_insert: Vec<(usize, ResponseItem)> = Vec:REDACTED_SECRET);
    ...
    for (idx, item) in items.iter().enumerate() {
        match item {
            ...
            ResponseItem:REDACTED_SECRETShellCall { call_id, .. } => {
    ...
    match item {
        ...
        ResponseItem::FunctionCallOutput { call_id, .. } => {
            if let Some(pos) = items.iter().position(|i| {
                ...
                matches!(i, ResponseItem:REDACTED_SECRETShellCall { call_id: Some(existing), .. } if existing == call_id)
            }) {
                items.remove(pos);
            }
        }
