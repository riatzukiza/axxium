...
fn assistant_msg(text: REDACTED_SECRET) -> ResponseItem {
...
fn inter_agent_assistant_msg(text: REDACTED_SECRET) -> ResponseItem {
    let communication = InterAgentCommunication:REDACTED_SECRET
        AgentPath::REDACTED_SECRET(),
        AgentPath::REDACTED_SECRET().join("worker").unwrap(),
        Vec:REDACTED_SECRET),
        text.to_string(),
        /*trigger_turn*/ true,
    );
...
fn create_history_with_items(items: Vec<ResponseItem>) -> ContextManager {
    let mut h = ContextManager:REDACTED_SECRET);
...
fn user_msg(text: REDACTED_SECRET) -> ResponseItem {
    ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![ContentItem::OutputText {
...
fn user_input_text_msg(text: REDACTED_SECRET) -> ResponseItem {
    ResponseItem::Message {
        id: None,
        role: "user".to_string(),
...
fn developer_msg(text: REDACTED_SECRET) -> ResponseItem {
    ResponseItem::Message {
        id: None,
...
fn developer_msg_with_fragments(texts: &[REDACTED_SECRET]) -> ResponseItem {
    ResponseItem::Message {
        id: None,
        role: "developer".to_string(),
        content: texts
...
fn custom_tool_call_output(call_id: REDACTED_SECRET, output: REDACTED_SECRET) -> ResponseItem {
...
fn reasoning_msg(text: REDACTED_SECRET) -> ResponseItem {
...
fn truncate_exec_output(content: REDACTED_SECRET) -> String {
    truncate_text(content, TruncationPolicy::Tokens(EXEC_FORMAT_MAX_TOKENS))
}

fn approx_token_count_for_text(text: REDACTED_SECRET) -> i64 {
...
fn get_history_for_prompt_drops_ghost_commits() {
    let items = vec![ResponseItem::GhostSnapshot {
        ghost_commit: GhostCommit:REDACTED_SECRET
            "ghost-1".to_string(),
            /*parent*/ None,
            Vec:REDACTED_SECRET),
            Vec:REDACTED_SECRET),
...
fn record_items_truncates_function_call_output_content() {
    let mut history = ContextManager:REDACTED_SECRET);
...
fn assert_truncated_message_matches(message: REDACTED_SECRET, line: REDACTED_SECRET, expected_removed: usize) {
    let pattern = truncated_message_pattern(line);
    let regex = Regex:REDACTED_SECRET&pattern).unwrap_or_else(|err| {
...
fn truncated_message_pattern(line: REDACTED_SECRET) -> String {
...
fn normalize_adds_missing_output_for_local_shell_call_with_id() {
    let items = vec![ResponseItem:REDACTED_SECRETShellCall {
        id: None,
        call_id: Some("shell-1".to_string()),
        status: LocalShellStatus::Completed,
        action: LocalShellAction::Exec(LocalShellExecAction {
...
fn original_detail_images_scale_with_dimensions() {
    ...
    let mut bytes = std::io::Cursor:REDACTED_SECRETVec:REDACTED_SECRET));
    image
