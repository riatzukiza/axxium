...
async fn remote_process(write_status: WriteStatus) -> UnifiedExecProcess {
    ...
    let started = StartedExecProcess {
        process: Arc:REDACTED_SECRETMockExecProcess {
            process_id: "test-process".to_string().into(),
            write_response: WriteResponse {
                status: write_status,
            },
            read_responses: Mutex:REDACTED_SECRETVecDeque:REDACTED_SECRET)),
            wake_tx,
        }),
    };
...
async fn remote_process_waits_for_early_exit_event() {
    ...
    let started = StartedExecProcess {
        process: Arc:REDACTED_SECRETMockExecProcess {
            ...
            read_responses: Mutex:REDACTED_SECRETVecDeque::from([ReadResponse {
                chunks: Vec:REDACTED_SECRET),
                next_seq: 2,
                exited: true,
                exit_code: Some(17),
