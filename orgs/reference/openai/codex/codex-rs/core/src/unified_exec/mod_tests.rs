...
async fn exec_command_with_tty(
    ...
    let process = Arc:REDACTED_SECRET
        manager
            .open_session_with_exec_env(
                process_id,
                &request,
                tty,
                Box:REDACTED_SECRETNoopSpawnLifecycle),
                turn.environment.as_ref(),
            )
            .await?,
    );
    let context =
        UnifiedExecContext:REDACTED_SECRETArc::clone(session), Arc::clone(turn), "call".to_string());
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    const TEST_VAR_VALUE: REDACTED_SECRET = "unified_exec_var_123";
...
async fn remote_exec_server_rejects_inherited_fd_launches() -> anyhow::Result<()> {
    ...
    turn.environment = Arc:REDACTED_SECRETremote_test_env.environment().clone());
    ...
    let err = manager
        .open_session_with_exec_env(
            ...
            &request,
            /*tty*/ true,
            Box:REDACTED_SECRETTestSpawnLifecycle {
