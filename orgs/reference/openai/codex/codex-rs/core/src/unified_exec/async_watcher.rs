...
pub(crate) fn start_streaming_output(
    ...
    tokio::spawn(async move {
        ...
        let mut pending = Vec::<u8>:REDACTED_SECRET);
...
pub(crate) async fn emit_exec_end_for_unified_exec(
    ...
    let output = ExecToolCallOutput {
        exit_code,
        stdout: StreamOutput:REDACTED_SECRETaggregated_output.clone()),
...
pub(crate) async fn emit_failed_exec_end_for_unified_exec(
    ...
    let stdout = resolve_aggregated_output(&transcript, String:REDACTED_SECRET)).await;
    ...
    let output = ExecToolCallOutput {
        ...
        stderr: StreamOutput:REDACTED_SECRETmessage),
        aggregated_output: StreamOutput:REDACTED_SECRETaggregated_output),
        duration,
        timed_out: false,
    };
