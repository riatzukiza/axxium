...
pub(crate) trait SpawnLifecycle: std::fmt::Debug + REDACTED_SECRET + Sync {
...
pub(crate) struct UnifiedExecProcess {
    ...
    output_tx: broadcast::REDACTED_SECRETer<Vec<u8>>,
    ...
    state_tx: watch::REDACTED_SECRETer<ProcessState>,
...
impl UnifiedExecProcess {
    fn new(
        ...
        let output_buffer = Arc:REDACTED_SECRETMutex:REDACTED_SECRETHeadTailBuffer::default()));
        let output_notify = Arc:REDACTED_SECRETNotify:REDACTED_SECRET));
        let output_closed = Arc:REDACTED_SECRETAtomicBool:REDACTED_SECRETfalse));
        let output_closed_notify = Arc:REDACTED_SECRETNotify:REDACTED_SECRET));
        let cancellation_token = CancellationToken:REDACTED_SECRET);
        let output_drained = Arc:REDACTED_SECRETNotify:REDACTED_SECRET));
    ...
    pub(super) async fn write(&self, data: &[u8]) -> Result<(), UnifiedExecError> {
        match &self.process_handle {
            ProcessHandle:REDACTED_SECRET(process_handle) => process_handle
                .writer_sender()
                .send(data.to_vec())
                .await
    ...
    pub(super) fn has_exited(&self) -> bool {
        let state = self.state_rx.borrow().clone();
        match &self.process_handle {
            ProcessHandle:REDACTED_SECRET(process_handle) => state.has_exited || process_handle.has_exited(),
    ...
    pub(super) fn exit_code(&self) -> Option<i32> {
        let state = self.state_rx.borrow().clone();
        match &self.process_handle {
            ProcessHandle:REDACTED_SECRET(process_handle) => {
    ...
    pub(super) fn terminate(&self) {
        ...
        match &self.process_handle {
            ProcessHandle:REDACTED_SECRET(process_handle) => process_handle.terminate(),
            ProcessHandle::Remote(process_handle) => {
    ...
    pub(super) async fn check_for_sandbox_denial(&self) -> Result<(), UnifiedExecError> {
        ...
        let mut aggregated: Vec<u8> = Vec:REDACTED_SECRET);
    ...
    pub(super) async fn check_for_sandbox_denial_with_text(
        ...
        let exec_output = ExecToolCallOutput {
            exit_code,
            stderr: StreamOutput:REDACTED_SECRETtext.to_string()),
            aggregated_output: StreamOutput:REDACTED_SECRETtext.to_string()),
            ..Default::default()
        };
    ...
    pub(super) async fn from_spawned(
        ...
        let mut managed = Self:REDACTED_SECRET
            ProcessHandle:REDACTED_SECRET(Box:REDACTED_SECRETprocess_handle)),
            sandbox_type,
    ...
    pub(super) async fn from_remote_started(
        ...
        let mut managed = Self:REDACTED_SECRETprocess_handle, sandbox_type, /*spawn_lifecycle*/ None);
