...
/// Helper that returns a `Config` pointing at `REDACTED_SECRET` and using `limit` as
...
/// been configured.
async fn make_config(REDACTED_SECRET: &TempDir, limit: usize, instructions: Option<REDACTED_SECRET>) -> Config {
    ...
    config.cwd = REDACTED_SECRET.abs();
...
async fn make_config_with_fallback(
    REDACTED_SECRET: &TempDir,
    limit: usize,
    instructions: Option<REDACTED_SECRET>,
    fallbacks: &[REDACTED_SECRET],
) -> Config {
    let mut config = make_config(REDACTED_SECRET, limit, instructions).await;
...
async fn make_config_with_project_REDACTED_SECRET_markers(
    REDACTED_SECRET: &TempDir,
    limit: usize,
    instructions: Option<REDACTED_SECRET>,
    markers: &[REDACTED_SECRET],
) -> Config {
    let codex_home = TempDir:REDACTED_SECRET).unwrap();
    let cli_overrides = vec![(
        "project_REDACTED_SECRET_markers".to_string(),
        TomlValue::Array(
            markers
                .iter()
...
/// placed at the repository REDACTED_SECRET (identified by `.git`).
#[tokio::test]
async fn finds_doc_in_repo_REDACTED_SECRET() {
    ...
    // Put the doc at the repo REDACTED_SECRET.
    fs::write(repo.path().join("AGENTS.md"), "REDACTED_SECRET level doc").unwrap();
    ...
    assert_eq!(res, "REDACTED_SECRET level doc");
}
...
async fn merges_existing_instructions_with_project_doc() {
    ...
    const INSTRUCTIONS: REDACTED_SECRET = "base instructions";
...
async fn keeps_existing_instructions_when_doc_missing() {
    ...
    const INSTRUCTIONS: REDACTED_SECRET = "some instructions";
...
/// When both the repository REDACTED_SECRET and the working directory contain
/// AGENTS.md files, their contents are concatenated from REDACTED_SECRET to cwd.
#[tokio::test]
async fn concatenates_REDACTED_SECRET_and_cwd_docs() {
    let repo = tempfile::tempdir().expect("tempdir");
    ...
    // Repo REDACTED_SECRET doc.
    fs::write(repo.path().join("AGENTS.md"), "REDACTED_SECRET doc").unwrap();
    ...
    let res = get_user_instructions(&cfg).await.expect("doc expected");
    assert_eq!(res, "REDACTED_SECRET doc\n\ncrate doc");
}

#[tokio::test]
async fn project_REDACTED_SECRET_markers_are_honored_for_agents_discovery() {
    let REDACTED_SECRET = tempfile::tempdir().expect("tempdir");
    fs::write(REDACTED_SECRET.path().join(".codex-REDACTED_SECRET"), "").unwrap();
    fs::write(REDACTED_SECRET.path().join("AGENTS.md"), "parent doc").unwrap();

    let nested = REDACTED_SECRET.path().join("dir1");
    fs::create_dir_all(nested.join(".git")).unwrap();
    fs::write(nested.join("AGENTS.md"), "child doc").unwrap();

    let mut cfg = make_config_with_project_REDACTED_SECRET_markers(
        &REDACTED_SECRET,
        /*limit*/ 4096,
        /*instructions*/ None,
        &[".codex-REDACTED_SECRET"],
    )
    .await;
    cfg.cwd = nested.abs();
    ...
    let expected_parent =
        dunce::canonicalize(REDACTED_SECRET.path().join("AGENTS.md")).expect("canonical parent doc path");
    let expected_child =
...
fn create_skill(codex_home: PathBuf, name: REDACTED_SECRET, description: REDACTED_SECRET) {
