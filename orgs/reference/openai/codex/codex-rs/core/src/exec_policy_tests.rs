...
fn host_absolute_path(segments: &[REDACTED_SECRET]) -> String {
...
fn host_program_path(name: REDACTED_SECRET) -> String {
...
fn starlark_string(value: REDACTED_SECRET) -> String {
...
async fn child_uses_parent_exec_policy_when_non_exec_policy_layers_differ() {
    ...
    child_config.config_layer_stack = ConfigLayerStack:REDACTED_SECRET
        layers,
...
async fn child_does_not_use_parent_exec_policy_when_requirements_exec_policy_differs() {
    ...
    requirements.exec_policy = Some(Sourced:REDACTED_SECRET
        RequirementsExecPolicy:REDACTED_SECRETpolicy),
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    let requirements = ConfigRequirements {
        exec_policy: Some(codex_config::Sourced:REDACTED_SECRET
            codex_config::RequirementsExecPolicy:REDACTED_SECRETrequirements_exec_policy),
...
async fn ignores_policies_outside_policy_dir() {
    ...
    let config_stack = config_stack_for_dot_codex_folder(temp_dir.path());
    fs::write(
        temp_dir.path().join("REDACTED_SECRET.rules"),
...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let config_stack = ConfigLayerStack:REDACTED_SECRET
        layers,
        ConfigRequirements::default(),
        ConfigRequirementsToml::default(),
    )?;
...
async fn mixed_rule_and_sandbox_prompt_prioritizes_rule_for_rejection_decision() {
    ...
    let mut parser = PolicyParser:REDACTED_SECRET);
    parser
        .parse("test.rules", policy_src)
        .expect("parse policy");
    let manager = ExecPolicyManager:REDACTED_SECRETArc:REDACTED_SECRETparser.build()));
    let command = vec![
        "bash".to_string(),
        "-lc".to_string(),
...
async fn heuristics_apply_when_other_commands_match_policy() {
    ...
    parser
        .parse("test.rules", policy_src)
        .expect("parse policy");
    let policy = Arc:REDACTED_SECRETparser.build());
    let command = vec![
        "bash".to_string(),
        "-lc".to_string(),
    ...
    assert_eq!(
        ExecPolicyManager:REDACTED_SECRETpolicy)
            .create_exec_approval_requirement_for_command(ExecApprovalRequest {
                command: &command,
                approval_policy: AskForApproval::UnlessTrusted,
                sandbox_policy: &SandboxPolicy::DangerFullAccess,
...
fn derive_requested_execpolicy_amendment_returns_none_for_empty_prefix_rule() {
    assert_eq!(
        None,
        derive_requested_execpolicy_amendment_for_test(Some(&Vec:REDACTED_SECRET)), &[])
    );
}
...
fn derive_requested_execpolicy_amendment_allows_non_exact_banned_prefix_rule_match() {
    ...
    assert_eq!(
        Some(ExecPolicyAmendment:REDACTED_SECRETprefix_rule.clone())),
        derive_requested_execpolicy_amendment_for_test(Some(&prefix_rule), &[])
    );
}
...
fn vec_str(items: &[REDACTED_SECRET]) -> Vec<String> {
...
async fn verify_approval_requirement_for_unsafe_powershell_command() {
    ...
    let policy = ExecPolicyManager:REDACTED_SECRETArc:REDACTED_SECRETPolicy::empty()));
    ...
    let expected_amendment = Some(ExecPolicyAmendment:REDACTED_SECRETvec_str(&[
        "pwsh",
        "-Command",
        "echo hi @(calc)",
    ])));
    ...
    assert_eq!(
        ExecApprovalRequirement::NeedsApproval {
            reason: None,
            proposed_execpolicy_amendment: Some(ExecPolicyAmendment:REDACTED_SECRETvec_str(&[
                "rm",
                "-rf",
                "/important/data",
            ]))),
    ...
    let policy = match policy_src {
        Some(src) => {
            ...
            parser
                ...
                .expect("parse policy");
            Arc:REDACTED_SECRETparser.build())
        }
        None => Arc:REDACTED_SECRETPolicy::empty()),
    };

    let requirement = ExecPolicyManager:REDACTED_SECRETpolicy)
        .create_exec_approval_requirement_for_command(ExecApprovalRequest {
            command: &command,
            approval_policy,
