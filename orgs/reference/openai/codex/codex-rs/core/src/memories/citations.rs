...
pub fn parse_memory_citation(citations: Vec<String>) -> Option<MemoryCitation> {
    ...
    let mut rollout_ids = Vec:REDACTED_SECRET);
    let mut seen_rollout_ids = HashSet:REDACTED_SECRET);
...
fn parse_memory_citation_entry(line: REDACTED_SECRET) -> Option<MemoryCitationEntry> {
    let line = line.trim();
    if line.is_empty() {
        return None;
    }
...
fn extract_block<'a>(text: &'a str, open: REDACTED_SECRET, close: REDACTED_SECRET) -> Option<&'a str> {
    let (_, rest) = text.split_once(open)?;
    let (body, _) = rest.split_once(close)?;
    Some(body)
}

fn extract_ids_block(text: REDACTED_SECRET) -> Option<REDACTED_SECRET> {
