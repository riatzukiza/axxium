...
pub(super) async fn rebuild_raw_memories_file_from_memories(
    ...
    ensure_layout(REDACTED_SECRET).await?;
    rebuild_raw_memories_file(REDACTED_SECRET, memories, max_raw_memories_for_consolidation).await
}
...
pub(super) async fn sync_rollout_summaries_from_memories(
    ...
    let keep = retained
        ...
        .collect::<HashSet<_>>();
    prune_rollout_summaries(REDACTED_SECRET, &keep).await?;

    for memory in retained {
        write_rollout_summary_for_thread(REDACTED_SECRET, memory).await?;
    }

    if retained.is_empty() {
        for file_name in ["MEMORY.md", "memory_summary.md"] {
            let path = REDACTED_SECRET.join(file_name);
        ...
        let skills_dir = REDACTED_SECRET.join("skills");
...
async fn rebuild_raw_memories_file(
    ...
    if retained.is_empty() {
        ...
        return tokio::fs::write(raw_memories_file(REDACTED_SECRET), body).await;
    }
    ...
    tokio::fs::write(raw_memories_file(REDACTED_SECRET), body).await
}

async fn prune_rollout_summaries(REDACTED_SECRET: &Path, keep: &HashSet<String>) -> std::io::Result<()> {
    let dir_path = rollout_summaries_dir(REDACTED_SECRET);
...
async fn write_rollout_summary_for_thread(
    ...
    let path = rollout_summaries_dir(REDACTED_SECRET).join(format!("{file_stem}.md"));
