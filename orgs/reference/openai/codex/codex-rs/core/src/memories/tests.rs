...
use crate::memories::clear_memory_REDACTED_SECRET_contents;
...
#[test]
fn memory_REDACTED_SECRET_uses_shared_global_path() {
    let dir = tempdir().expect("tempdir");
    let codex_home = dir.path().join("codex");
    assert_eq!(memory_REDACTED_SECRET(&codex_home), codex_home.join("memories"));
}
...
#[tokio::test]
async fn clear_memory_REDACTED_SECRET_contents_preserves_REDACTED_SECRET_directory() {
    let dir = tempdir().expect("tempdir");
    let REDACTED_SECRET = dir.path().join("memory");
    let nested_dir = REDACTED_SECRET.join("rollout_summaries");
    tokio::fs::create_dir_all(&nested_dir)
        .await
        .expect("create rollout summaries dir");
    tokio::fs::write(REDACTED_SECRET.join("MEMORY.md"), "stale memory index\n")
        .await
    ...
    clear_memory_REDACTED_SECRET_contents(&REDACTED_SECRET)
        .await
        .expect("clear memory REDACTED_SECRET contents");

    assert!(
        tokio::fs::try_exists(&REDACTED_SECRET)
            .await
            .expect("check memory REDACTED_SECRET existence"),
        "memory REDACTED_SECRET should still exist after clearing contents"
    );
    let mut entries = tokio::fs::read_dir(&REDACTED_SECRET)
        .await
        .expect("read memory REDACTED_SECRET after clear");
    assert!(
        entries
            .next_entry()
            .await
            ...
            .is_none(),
        "memory REDACTED_SECRET should be empty after clearing contents"
    );
}

#[cfg(unix)]
#[tokio::test]
async fn clear_memory_REDACTED_SECRET_contents_rejects_symlinked_REDACTED_SECRET() {
    let dir = tempdir().expect("tempdir");
    ...
    let REDACTED_SECRET = dir.path().join("memory");
    std::os::unix::fs::symlink(&target, &REDACTED_SECRET).expect("create memory REDACTED_SECRET symlink");

    let err = clear_memory_REDACTED_SECRET_contents(&REDACTED_SECRET)
        .await
        .expect_err("symlinked memory REDACTED_SECRET should be rejected");
    assert_eq!(err.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
        tokio::fs::try_exists(&target_file)
            .await
            .expect("check target file existence"),
        "rejecting a symlinked memory REDACTED_SECRET should not delete the symlink target"
    );
}
...
async fn sync_rollout_summaries_and_raw_memories_file_keeps_latest_memories_only() {
    let dir = tempdir().expect("tempdir");
    let REDACTED_SECRET = dir.path().join("memory");
    ensure_layout(&REDACTED_SECRET).await.expect("ensure layout");

    let keep_id = ThreadId::default().to_string();
    let drop_id = ThreadId::default().to_string();
    let keep_path = rollout_summaries_dir(&REDACTED_SECRET).join(format!("{keep_id}.md"));
    let drop_path = rollout_summaries_dir(&REDACTED_SECRET).join(format!("{drop_id}.md"));
    ...
    let mut dir = tokio::fs::read_dir(rollout_summaries_dir(&REDACTED_SECRET))
        .await
    ...
    let raw_memories = tokio::fs::read_to_string(raw_memories_file(&REDACTED_SECRET))
        .await
...
async fn sync_rollout_summaries_uses_timestamp_hash_and_sanitized_slug_filename() {
    ...
    let stale_unslugged_path = rollout_summaries_dir(&REDACTED_SECRET).join(format!("{thread_id}.md"));
    let stale_old_slug_path =
        rollout_summaries_dir(&REDACTED_SECRET).join(format!("{thread_id}--old-slug.md"));
    ...
    let summary = tokio::fs::read_to_string(rollout_summaries_dir(&REDACTED_SECRET).join(file_name))
        .await
...
- Confirmed no ordering regressions."
    ...
    let summary = tokio::fs::read_to_string(
        rollout_summaries_dir(&REDACTED_SECRET).join(canonical_rollout_summary_file),
    )
    .await
    .expect("read rollout summary");
    assert!(summary.contains("rollout_path: /tmp/rollout-200.jsonl"));
...
mod phase2 {
    ...
    impl DispatchHarness {
        async fn new() -> Self {
            ...
            let manager = ThreadManager::with_models_provider_and_home_for_tests(
                ...
                config.codex_home.clone(),
                std::sync::Arc:REDACTED_SECRETcodex_exec_server::EnvironmentManager:REDACTED_SECRET
        ...
        async fn seed_stage1_output(&self, source_updated_at: i64) {
            let thread_id = ThreadId:REDACTED_SECRET);
            let mut metadata_builder = ThreadMetadataBuilder:REDACTED_SECRET
                thread_id,
                self.config
                    .codex_home
    ...
    async fn dispatch_skips_when_global_job_is_not_dirty() {
        let harness = DispatchHarness:REDACTED_SECRET).await;
    ...
    async fn dispatch_skips_when_global_job_is_already_running() {
        ...
        let claimed = harness
            .state_db
            .try_claim_global_phase2_job(ThreadId:REDACTED_SECRET), /*lease_seconds*/ 3_600)
            .await
    ...
    async fn dispatch_reclaims_stale_global_lock_and_starts_consolidation() {
        ...
        let stale_claim = harness
            .state_db
            .try_claim_global_phase2_job(ThreadId:REDACTED_SECRET), /*lease_seconds*/ 0)
            .await
        ...
        pretty_assertions::assert_eq!(config_snapshot.cwd, memory_REDACTED_SECRET(&harness.config.codex_home));
        match config_snapshot.sandbox_policy {
            SandboxPolicy::WorkspaceWrite { writable_REDACTED_SECRETs, .. } => {
                assert!(
                    writable_REDACTED_SECRETs
                        .iter()
                        .any(|REDACTED_SECRET| REDACTED_SECRET.as_path() == harness.config.codex_home.as_path()),
                    "consolidation subagent should have codex_home as writable REDACTED_SECRET"
                );
            }
    ...
    async fn dispatch_with_empty_stage1_outputs_rebuilds_local_artifacts() {
        let harness = DispatchHarness:REDACTED_SECRET).await;
        let REDACTED_SECRET = memory_REDACTED_SECRET(&harness.config.codex_home);
        let summaries_dir = rollout_summaries_dir(&REDACTED_SECRET);
        tokio::fs::create_dir_all(&summaries_dir)
            .await
            .expect("create rollout summaries dir");

        let stale_summary_path = summaries_dir.join(format!("{}.md", ThreadId:REDACTED_SECRET)));
        tokio::fs::write(&stale_summary_path, "stale summary\n")
            .await
            .expect("write stale rollout summary");
        let raw_memories_path = raw_memories_file(&REDACTED_SECRET);
        tokio::fs::write(&raw_memories_path, "stale raw memories\n")
            .await
            .expect("write stale raw memories");
        let memory_index_path = REDACTED_SECRET.join("MEMORY.md");
        tokio::fs::write(&memory_index_path, "stale memory index\n")
            .await
            .expect("write stale memory index");
        let memory_summary_path = REDACTED_SECRET.join("memory_summary.md");
        tokio::fs::write(&memory_summary_path, "stale memory summary\n")
            .await
            .expect("write stale memory summary");
        let stale_skill_file = REDACTED_SECRET.join("skills/demo/SKILL.md");
        ...
        assert!(
            !tokio::fs::try_exists(REDACTED_SECRET.join("skills"))
                .await
    ...
    async fn dispatch_marks_job_for_retry_when_sandbox_policy_cannot_be_overridden() {
        ...
        phase2::run(&harness.session, Arc:REDACTED_SECRETconstrained_config)).await;
    ...
    async fn dispatch_marks_job_for_retry_when_syncing_artifacts_fails() {
        ...
        let REDACTED_SECRET = memory_REDACTED_SECRET(&harness.config.codex_home);
        tokio::fs::write(&REDACTED_SECRET, "not a directory")
            .await
            .expect("create file at memory REDACTED_SECRET");
    ...
    async fn dispatch_marks_job_for_retry_when_rebuilding_raw_memories_fails() {
        ...
        harness.seed_stage1_output(/*source_updated_at*/ 100).await;
        let REDACTED_SECRET = memory_REDACTED_SECRET(&harness.config.codex_home);
        tokio::fs::create_dir_all(raw_memories_file(&REDACTED_SECRET))
            .await
