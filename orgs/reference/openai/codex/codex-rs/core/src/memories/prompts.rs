...
static CONSOLIDATION_PROMPT_TEMPLATE: LazyLock<Template> = LazyLock:REDACTED_SECRET|| {
    parse_embedded_template(
        include_str!("../../templates/memories/consolidation.md"),
        "memories/consolidation.md",
    )
});
static STAGE_ONE_INPUT_TEMPLATE: LazyLock<Template> = LazyLock:REDACTED_SECRET|| {
    parse_embedded_template(
        include_str!("../../templates/memories/stage_one_input.md"),
        "memories/stage_one_input.md",
    )
});
static MEMORY_TOOL_DEVELOPER_INSTRUCTIONS_TEMPLATE: LazyLock<Template> = LazyLock:REDACTED_SECRET|| {
    parse_embedded_template(
...
fn parse_embedded_template(source: &'static str, template_name: REDACTED_SECRET) -> Template {
...
/// Builds the consolidation subagent prompt for a specific memory REDACTED_SECRET.
pub(super) fn build_consolidation_prompt(
    memory_REDACTED_SECRET: &Path,
    selection: &Phase2InputSelection,
) -> String {
    let memory_REDACTED_SECRET = memory_REDACTED_SECRET.display().to_string();
    let phase2_input_selection = render_phase2_input_selection(selection);
    CONSOLIDATION_PROMPT_TEMPLATE
        .render([
            ("memory_REDACTED_SECRET", memory_REDACTED_SECRET.as_str()),
            ("phase2_input_selection", phase2_input_selection.as_str()),
        ])
        .unwrap_or_else(|err| {
        warn!("failed to render memories consolidation prompt template: {err}");
        format!(
            "## Memory Phase 2 (Consolidation)\nConsolidate Codex memories in: {memory_REDACTED_SECRET}\n\n{phase2_input_selection}"
        )
    })
}
...
pub(super) fn build_stage_one_input_message(
    ...
    rollout_path: &Path,
    rollout_cwd: &Path,
    rollout_contents: REDACTED_SECRET,
...
pub(crate) async fn build_memory_tool_developer_instructions(codex_home: &Path) -> Option<String> {
    let base_path = memory_REDACTED_SECRET(codex_home);
