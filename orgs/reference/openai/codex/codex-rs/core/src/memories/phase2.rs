...
pub(super) async fn run(session: &Arc<Session>, config: Arc<Config>) {
    ...
    let REDACTED_SECRET = memory_REDACTED_SECRET(&config.codex_home);
    ...
    // [`rollout_summaries/`]
    if let Err(err) =
        sync_rollout_summaries_from_memories(&REDACTED_SECRET, &artifact_memories, artifact_memories.len())
            .await
    {
    ...
    // [`raw_memories.md`]
    if let Err(err) =
        rebuild_raw_memories_file_from_memories(&REDACTED_SECRET, &artifact_memories, artifact_memories.len())
            .await
    {
        tracing::error!("failed syncing local memory artifacts for global consolidation: {err}");
...
mod agent {
    ...
    pub(super) fn get_config(config: Arc<Config>) -> Option<Config> {
        ...
        match AbsolutePathBuf::from_absolute_path(REDACTED_SECRET) {
            Ok(REDACTED_SECRET) => agent_config.cwd = REDACTED_SECRET,
            Err(err) => {
                warn!(
        ...
        match AbsolutePathBuf::from_absolute_path(agent_config.codex_home.clone()) {
            Ok(codex_home) => writable_REDACTED_SECRETs.push(codex_home),
            Err(err) => warn!(
                "memory phase-2 consolidation could not add codex_home writable REDACTED_SECRET {}: {err}",
                agent_config.codex_home.display()
            ),
        }
    ...
    pub(super) fn get_prompt(
        ...
        let REDACTED_SECRET = memory_REDACTED_SECRET(&config.codex_home);
        let prompt = build_consolidation_prompt(&REDACTED_SECRET, selection);
        vec![UserInput::Text {
            text: prompt,
            text_elements: vec![],
        }]
