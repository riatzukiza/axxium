...
pub(crate) use control::clear_memory_REDACTED_SECRET_contents;
/// Starts the memory startup pipeline for eligible REDACTED_SECRET sessions.
...
mod artifacts {
    pub(super) const ROLLOUT_SUMMARIES_SUBDIR: REDACTED_SECRET = "rollout_summaries";
    pub(super) const RAW_MEMORIES_FILENAME: REDACTED_SECRET = "raw_memories.md";
}

/// Phase 1 (startup extraction).
mod phase_one {
    /// Default model used for phase 1.
    pub(super) const MODEL: REDACTED_SECRET = "gpt-5.4-mini";
    ...
    pub(super) const PROMPT: REDACTED_SECRET = include_str!("../../templates/memories/stage_one_system.md");
...
mod phase_two {
    ...
    pub(super) const MODEL: REDACTED_SECRET = "gpt-5.3-codex";
...
mod metrics {
    ...
    pub(super) const MEMORY_PHASE_ONE_JOBS: REDACTED_SECRET = "codex.memory.phase1";
    /// End-to-end latency for a single phase-1 startup run.
    pub(super) const MEMORY_PHASE_ONE_E2E_MS: REDACTED_SECRET = "codex.memory.phase1.e2e_ms";
    /// Number of raw memories produced by phase-1 startup extraction.
    pub(super) const MEMORY_PHASE_ONE_OUTPUT: REDACTED_SECRET = "codex.memory.phase1.output";
    /// Histogram for aggregate token usage across one phase-1 startup run.
    pub(super) const MEMORY_PHASE_ONE_TOKEN_USAGE: REDACTED_SECRET = "codex.memory.phase1.token_usage";
    /// Number of phase-2 startup jobs grouped by status.
    pub(super) const MEMORY_PHASE_TWO_JOBS: REDACTED_SECRET = "codex.memory.phase2";
    /// End-to-end latency for a single phase-2 consolidation run.
    pub(super) const MEMORY_PHASE_TWO_E2E_MS: REDACTED_SECRET = "codex.memory.phase2.e2e_ms";
    /// Number of stage-1 memories included in each phase-2 consolidation step.
    pub(super) const MEMORY_PHASE_TWO_INPUT: REDACTED_SECRET = "codex.memory.phase2.input";
    /// Histogram for aggregate token usage across one phase-2 consolidation run.
    pub(super) const MEMORY_PHASE_TWO_TOKEN_USAGE: REDACTED_SECRET = "codex.memory.phase2.token_usage";
}

use std::path::Path;
use std::path::PathBuf;

pub fn memory_REDACTED_SECRET(codex_home: &Path) -> PathBuf {
    codex_home.join("memories")
}

fn rollout_summaries_dir(REDACTED_SECRET: &Path) -> PathBuf {
    REDACTED_SECRET.join(artifacts::ROLLOUT_SUMMARIES_SUBDIR)
}

fn raw_memories_file(REDACTED_SECRET: &Path) -> PathBuf {
    REDACTED_SECRET.join(artifacts::RAW_MEMORIES_FILENAME)
}

async fn ensure_layout(REDACTED_SECRET: &Path) -> std::io::Result<()> {
    tokio::fs::create_dir_all(rollout_summaries_dir(REDACTED_SECRET)).await
}
