...
async fn test_recent_commits_non_git_directory_returns_empty() {
    let temp_dir = TempDir:REDACTED_SECRET).expect("Failed to create temp dir");
...
async fn test_collect_git_info_with_remote() {
    ...
    let remote_url_output = Command:REDACTED_SECRET"git")
...
async fn test_get_git_working_tree_state_clean_repo() {
    ...
    let remote_sha = Command:REDACTED_SECRET"git")
    ...
    assert_eq!(state.sha, GitSha:REDACTED_SECRET&remote_sha));
...
#[test]
fn resolve_REDACTED_SECRET_git_project_for_trust_returns_none_outside_repo() {
    let tmp = TempDir:REDACTED_SECRET).expect("tempdir");
    assert!(resolve_REDACTED_SECRET_git_project_for_trust(tmp.path()).is_none());
}

#[tokio::test]
async fn resolve_REDACTED_SECRET_git_project_for_trust_regular_repo_returns_repo_REDACTED_SECRET() {
    let temp_dir = TempDir:REDACTED_SECRET).expect("Failed to create temp dir");
    let repo_path = create_test_git_repo(&temp_dir).await;
    let expected = std::fs::canonicalize(&repo_path).unwrap();

    assert_eq!(
        resolve_REDACTED_SECRET_git_project_for_trust(&repo_path),
        Some(expected.clone())
    );
    let nested = repo_path.join("sub/dir");
    std::fs::create_dir_all(&nested).unwrap();
    assert_eq!(resolve_REDACTED_SECRET_git_project_for_trust(&nested), Some(expected));
}

#[tokio::test]
async fn resolve_REDACTED_SECRET_git_project_for_trust_detects_worktree_and_returns_main_REDACTED_SECRET() {
    let temp_dir = TempDir:REDACTED_SECRET).expect("Failed to create temp dir");
    let repo_path = create_test_git_repo(&temp_dir).await;

    // Create a linked worktree
    let wt_REDACTED_SECRET = temp_dir.path().join("wt");
    let _ = std::process::Command:REDACTED_SECRET"git")
        .args([
            "worktree",
            "add",
            wt_REDACTED_SECRET.to_str().unwrap(),
            "-b",
            "feature/x",
        ])
        .current_dir(&repo_path)
    ...
    let got =
        resolve_REDACTED_SECRET_git_project_for_trust(&wt_REDACTED_SECRET).and_then(|p| std::fs::canonicalize(p).ok());
    assert_eq!(got, expected);
    let nested = wt_REDACTED_SECRET.join("nested/sub");
    std::fs::create_dir_all(&nested).unwrap();
    let got_nested =
        resolve_REDACTED_SECRET_git_project_for_trust(&nested).and_then(|p| std::fs::canonicalize(p).ok());
    assert_eq!(got_nested, expected);
}

#[test]
fn resolve_REDACTED_SECRET_git_project_for_trust_detects_worktree_pointer_without_git_command() {
    let tmp = TempDir:REDACTED_SECRET).expect("tempdir");
    let repo_REDACTED_SECRET = tmp.path().join("repo");
    let common_dir = repo_REDACTED_SECRET.join(".git");
    let worktree_git_dir = common_dir.join("worktrees").join("feature-x");
    let worktree_REDACTED_SECRET = tmp.path().join("wt");
    std::fs::create_dir_all(&worktree_git_dir).unwrap();
    std::fs::create_dir_all(&worktree_REDACTED_SECRET).unwrap();
    std::fs::create_dir_all(worktree_REDACTED_SECRET.join("nested")).unwrap();
    std::fs::write(
        worktree_REDACTED_SECRET.join(".git"),
        format!("gitdir: {}\n", worktree_git_dir.display()),
    )
    .unwrap();

    let expected = std::fs::canonicalize(&repo_REDACTED_SECRET).unwrap();
    assert_eq!(
        resolve_REDACTED_SECRET_git_project_for_trust(&worktree_REDACTED_SECRET),
        Some(expected.clone())
    );
    assert_eq!(
        resolve_REDACTED_SECRET_git_project_for_trust(&worktree_REDACTED_SECRET.join("nested")),
        Some(expected)
    );
}

#[test]
fn resolve_REDACTED_SECRET_git_project_for_trust_non_worktrees_gitdir_returns_none() {
    let tmp = TempDir:REDACTED_SECRET).expect("tempdir");
    ...
    assert!(resolve_REDACTED_SECRET_git_project_for_trust(&proj).is_none());
    assert!(resolve_REDACTED_SECRET_git_project_for_trust(&proj.join("nested")).is_none());
}
...
fn test_git_info_serialization() {
    let git_info = GitInfo {
        commit_hash: Some(GitSha:REDACTED_SECRET"abc123def456")),
