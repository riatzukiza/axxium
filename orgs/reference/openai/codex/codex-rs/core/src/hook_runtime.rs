...
pub(crate) async fn inspect_pending_input(
    ...
    if let Some(TurnItem::UserMessage(user_message)) = parse_turn_item(&response_item) {
        ...
        if user_prompt_submit_outcome.should_stop {
            ...
            PendingInputHookDisposition::Accepted(Box:REDACTED_SECRETPendingInputRecord::UserMessage {
        ...
        PendingInputHookDisposition::Accepted(Box:REDACTED_SECRETPendingInputRecord::ConversationItem {
            response_item,
        }))
    }
}
...
fn additional_context_messages(additional_contexts: Vec<String>) -> Vec<ResponseItem> {
    additional_contexts
        .into_iter()
        .map(|additional_context| DeveloperInstructions:REDACTED_SECRETadditional_context).into())
        .collect()
}
