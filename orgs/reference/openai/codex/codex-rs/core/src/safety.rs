...
pub fn assess_patch_safety(
    ...
    // writable REDACTED_SECRETs, so we should still run `apply_patch` in a sandbox in that case.
...
fn is_write_patch_constrained_to_writable_paths(
    ...
    fn normalize(path: &Path) -> Option<PathBuf> {
        let mut out = PathBuf:REDACTED_SECRET);
    ...
    // Determine whether `path` is inside **any** writable REDACTED_SECRET. Both `path`
    // and REDACTED_SECRETs are converted to absolute, normalized forms before the
    // prefix check.
