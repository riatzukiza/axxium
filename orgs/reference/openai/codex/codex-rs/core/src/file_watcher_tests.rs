...
fn path(name: REDACTED_SECRET) -> PathBuf {
    PathBuf::from(name)
}

fn notify_event(kind: EventKind, paths: Vec<PathBuf>) -> Event {
    let mut event = Event:REDACTED_SECRETkind);
    for path in paths {
...
async fn throttled_receiver_coalesces_within_interval() {
    ...
    let mut throttled = ThrottledWatchReceiver:REDACTED_SECRETrx, TEST_THROTTLE_INTERVAL);
...
fn register_dedupes_by_path_and_scope() {
    let watcher = Arc:REDACTED_SECRETFileWatcher::noop());
...
fn recursive_registration_downgrades_to_non_recursive_after_drop() {
    ...
    let REDACTED_SECRET = temp_dir.path().join("watched-dir");
    std::fs::create_dir(&REDACTED_SECRET).expect("create REDACTED_SECRET");

    let watcher = Arc:REDACTED_SECRETFileWatcher:REDACTED_SECRET).expect("watcher"));
    let (subscriber, _rx) = watcher.add_subscriber();
    let non_recursive = subscriber.register_path(REDACTED_SECRET.clone(), /*recursive*/ false);
    let recursive = subscriber.register_path(REDACTED_SECRET.clone(), /*recursive*/ true);

    {
        let inner = watcher.inner.as_ref().expect("watcher inner");
        let inner = inner.lock().expect("inner lock");
        assert_eq!(
            inner.watched_paths.get(&REDACTED_SECRET),
...
fn unregister_holds_state_lock_until_unwatch_finishes() {
    ...
    let registration = unregister_subscriber.register_path(REDACTED_SECRET.clone(), /*recursive*/ true);
    ...
    let register_REDACTED_SECRET = REDACTED_SECRET.clone();
    let register_thread = std::thread::spawn(move || {
        let registration =
            register_subscriber.register_path(register_REDACTED_SECRET, /*recursive*/ false);
    ...
    assert_eq!(watcher.watch_counts_for_test(&REDACTED_SECRET), Some((1, 0)));
...
async fn matching_subscribers_are_notified() {
    ...
    let mut skills_rx = ThrottledWatchReceiver:REDACTED_SECRETskills_rx, TEST_THROTTLE_INTERVAL);
    let mut plugins_rx = ThrottledWatchReceiver:REDACTED_SECRETplugins_rx, TEST_THROTTLE_INTERVAL);
...
async fn non_recursive_watch_ignores_grandchildren() {
    ...
    let mut rx = ThrottledWatchReceiver:REDACTED_SECRETrx, TEST_THROTTLE_INTERVAL);
