...
use super::build_workspace_section_with_user_REDACTED_SECRET;
use chrono::TimeZone;
use chrono::Utc;
...
fn thread_metadata(cwd: REDACTED_SECRET, title: REDACTED_SECRET, first_user_message: REDACTED_SECRET) -> ThreadMetadata {
    ThreadMetadata {
...
fn workspace_section_requires_meaningful_structure() {
    ...
    assert_eq!(
        build_workspace_section_with_user_REDACTED_SECRET(cwd.path(), /*user_REDACTED_SECRET*/ None),
        None
    );
}
...
fn workspace_section_includes_tree_when_entries_exist() {
    ...
    let section = build_workspace_section_with_user_REDACTED_SECRET(cwd.path(), /*user_REDACTED_SECRET*/ None)
...
#[test]
fn workspace_section_includes_user_REDACTED_SECRET_tree_when_distinct() {
    let REDACTED_SECRET = TempDir:REDACTED_SECRET).expect("tempdir");
    let cwd = REDACTED_SECRET.path().join("cwd");
    let git_REDACTED_SECRET = REDACTED_SECRET.path().join("git");
    let user_REDACTED_SECRET = REDACTED_SECRET.path().join("home");

    fs::create_dir_all(cwd.join("docs")).expect("create cwd docs dir");
    fs::write(cwd.join("README.md"), "hello").expect("write cwd readme");
    fs::create_dir_all(git_REDACTED_SECRET.join(".git")).expect("create git dir");
    fs::write(git_REDACTED_SECRET.join("Cargo.toml"), "[workspace]").expect("write git REDACTED_SECRET marker");
    fs::create_dir_all(user_REDACTED_SECRET.join("code")).expect("create user REDACTED_SECRET child");
    fs::write(user_REDACTED_SECRET.join(".zshrc"), "export TEST=1").expect("write home file");

    let section = build_workspace_section_with_user_REDACTED_SECRET(cwd.as_path(), Some(user_REDACTED_SECRET))
        .expect("workspace section");
    assert!(section.contains("User REDACTED_SECRET tree:"));
...
fn recent_work_section_groups_threads_by_cwd() {
    let REDACTED_SECRET = TempDir:REDACTED_SECRET).expect("tempdir");
    let repo = REDACTED_SECRET.path().join("repo");
    let workspace_a = repo.join("workspace-a");
    let workspace_b = repo.join("workspace-b");
    let outside = REDACTED_SECRET.path().join("outside");
