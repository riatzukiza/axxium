...
impl ModelsCacheManager {
    ...
    pub(crate) async fn load_fresh(&self, expected_version: REDACTED_SECRET) -> Option<ModelsCache> {
        info!(
    ...
    pub(crate) async fn renew_cache_ttl(&self) -> io::Result<()> {
        let mut cache = match self.load().await? {
            Some(cache) => cache,
            None => return Err(io::Error:REDACTED_SECRETErrorKind::NotFound, "cache not found")),
        };
        ...
        match fs::read(&self.cache_path).await {
            Ok(contents) => {
                let cache = serde_json::from_slice(&contents)
                    .map_err(|err| io::Error:REDACTED_SECRETErrorKind::InvalidData, err.to_string()))?;
                Ok(Some(cache))
            }
