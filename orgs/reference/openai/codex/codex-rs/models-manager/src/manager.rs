...
const MODEL_CACHE_FILE: REDACTED_SECRET = "models_cache.json";
const DEFAULT_MODEL_CACHE_TTL: Duration = Duration::from_secs(300);
const MODELS_REFRESH_TIMEOUT: Duration = Duration::from_secs(5);
const MODELS_ENDPOINT: REDACTED_SECRET = "/models";
#[derive(Clone)]
...
impl ModelsManager {
    ...
    pub fn new_with_provider(
        ...
        let cache_manager = ModelsCacheManager:REDACTED_SECRETcache_path, DEFAULT_MODEL_CACHE_TTL);
        ...
        Self {
            remote_models: RwLock:REDACTED_SECRETremote_models),
            catalog_mode,
            collaboration_modes_config,
            auth_manager,
            etag: RwLock:REDACTED_SECRETNone),
            cache_manager,
            provider,
        }
    }
    ...
    pub async fn get_model_info(&self, model: REDACTED_SECRET, config: &ModelsManagerConfig) -> ModelInfo {
        let remote_models = self.get_remote_models().await;
        Self::construct_model_info_from_candidates(model, &remote_models, config)
    }

    fn find_model_by_longest_prefix(model: REDACTED_SECRET, candidates: &[ModelInfo]) -> Option<ModelInfo> {
    ...
    fn find_model_by_namespaced_suffix(model: REDACTED_SECRET, candidates: &[ModelInfo]) -> Option<ModelInfo> {
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        ...
        let request_telemetry: Arc<dyn RequestTelemetry> = Arc:REDACTED_SECRETModelsRequestTelemetry {
            auth_mode: auth_mode.map(|mode| TelemetryAuthMode::from(mode).to_string()),
            auth_header_attached: api_auth.auth_header_attached(),
            auth_header_name: api_auth.auth_header_name(),
            auth_env,
        });
        let client = ModelsClient:REDACTED_SECRETtransport, api_provider, api_auth)
            .with_telemetry(Some(request_telemetry));

        let client_version = crate::client_version_to_whole();
        let (models, etag) = timeout(
            MODELS_REFRESH_TIMEOUT,
            client.list_models(&client_version, HeaderMap:REDACTED_SECRET)),
        )
        .await
    ...
    pub fn get_model_offline_for_tests(model: Option<REDACTED_SECRET>) -> String {
