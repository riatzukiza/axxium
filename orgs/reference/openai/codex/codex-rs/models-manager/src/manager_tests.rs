...
fn remote_model(slug: REDACTED_SECRET, display: REDACTED_SECRET, priority: i32) -> ModelInfo {
...
"#,
    ...
    fn auth_config(&self) -> ModelProviderAuthInfo {
        ...
        ModelProviderAuthInfo {
            ...
            timeout_ms: NonZeroU64:REDACTED_SECRETtimeout_ms).unwrap(),
...
struct TagCollectorLayer {
    tags: Arc<Mutex<REDACTED_SECRET, String>>>,
}
...
async fn get_model_info_tracks_fallback_usage() {
    ...
    let manager = ModelsManager:REDACTED_SECRET
...
async fn refresh_available_models_uses_provider_auth_token() {
    let server = MockServer::start().await;
    let auth_script = ProviderAuthScript:REDACTED_SECRET&["provider-token"]).unwrap();
...
fn models_request_telemetry_emits_auth_env_feedback_tags_on_failure() {
    let tags = Arc:REDACTED_SECRETMutex:REDACTED_SECRETBTreeMap:REDACTED_SECRET)));
