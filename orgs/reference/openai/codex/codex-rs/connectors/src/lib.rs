    ...
    Fut: Future<Output = anyhow::Result<DirectoryListResponse>>,
{
    ...
    match response {
        Ok(response) => Ok(response
            ...
            .filter(|app| !is_hidden_directory_app(app))
            .collect()),
        Err(_) => Ok(Vec:REDACTED_SECRET)),
    }
}

fn merge_directory_apps(apps: Vec<DirectoryApp>) -> Vec<DirectoryApp> {
    let mut merged: HashMap<String, DirectoryApp> = HashMap:REDACTED_SECRET);
    for app in apps {
...
fn connector_install_url(name: REDACTED_SECRET, connector_id: REDACTED_SECRET) -> String {
    let slug = connector_name_slug(name);
    format!("https://chatgpt.com/apps/{slug}/{connector_id}")
}

fn connector_name_slug(name: REDACTED_SECRET) -> String {
...
fn normalize_connector_name(name: REDACTED_SECRET, connector_id: REDACTED_SECRET) -> String {
...
mod tests {
    ...
    fn cache_key(id: REDACTED_SECRET) -> AllConnectorsCacheKey {
        AllConnectorsCacheKey:REDACTED_SECRET
    ...
    fn app(id: REDACTED_SECRET, name: REDACTED_SECRET) -> DirectoryApp {
        DirectoryApp {
            id: id.to_string(),
            name: name.to_string(),
            description: None,
