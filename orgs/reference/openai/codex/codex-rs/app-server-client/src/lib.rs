...
async fn forward_in_process_event<F>(
    event_tx: &mpsc::REDACTED_SECRETer<InProcessServerEvent>,
    ...
    F: FnMut(ServerRequest),
{
    ...
    match event_tx.try_send(event) {
        ...
        Err(mpsc::error::TryREDACTED_SECRETError::Full(event)) => {
            *skipped_events = skipped_events.saturating_add(1);
            warn!("dropping in-process app-server event because consumer queue is full");
            if let InProcessServerEvent::ServerRequest(request) = event {
                reject_server_request(request);
        ...
        Err(mpsc::error::TryREDACTED_SECRETError::Closed(_)) => ForwardEventResult::DisableStream,
    }
}
...
/// Internal command sent from REDACTED_SECRET facade methods to the worker task.
///
...
enum ClientCommand {
    Request {
        ...
        response_tx: oneshot::REDACTED_SECRETer<IoResult<RequestResult>>,
    },
    Notify {
        notification: ClientNotification,
        response_tx: oneshot::REDACTED_SECRETer<IoResult<()>>,
    },
...
pub struct InProcessAppServerClient {
    command_tx: mpsc::REDACTED_SECRETer<ClientCommand>,
...
impl InProcessAppServerClient {
    ...
    /// REDACTED_SECRETs a typed client request and returns raw JSON-RPC result.
    ///
    ...
    /// REDACTED_SECRETs a typed client request and decodes the successful response body.
    ///
    ...
    /// REDACTED_SECRETs a typed client notification.
...
mod tests {
    ...
    where
        F: FnOnce(tokio_tungstenite::WebSocketStream<tokio::net::TcpStream>) -> Fut
            + REDACTED_SECRET
            + 'static,
        Fut: std::future::Future<Output = ()> + REDACTED_SECRET + 'static,
    {
    ...
    fn command_execution_output_delta_notification(delta: REDACTED_SECRET) -> ServerNotification {
    ...
    fn agent_message_delta_notification(delta: REDACTED_SECRET) -> ServerNotification {
    ...
    fn item_completed_notification(text: REDACTED_SECRET) -> ServerNotification {
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        let receive_task = tokio::spawn(async move {
            let mut events = Vec:REDACTED_SECRET);
            for _ in 0..5 {
                events.push(
    ...
    async fn remote_backpressure_preserves_transcript_notifications() {
        ...
        let mut remaining_events = Vec:REDACTED_SECRET);
        for _ in 0..4 {
        ...
        let mut transcript_event_names = Vec:REDACTED_SECRET);
    ...
    fn typed_request_error_exposes_sources() {
        let transport = TypedRequestError::Transport {
            ...
            source: IoError:REDACTED_SECRETErrorKind::BrokenPipe, "closed"),
        };
    ...
    async fn runtime_start_args_leave_manager_bootstrap_to_app_server() {
        let config = Arc:REDACTED_SECRETbuild_test_config().await);
