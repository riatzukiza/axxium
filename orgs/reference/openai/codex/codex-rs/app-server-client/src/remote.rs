...
pub struct RemoteAppServerClient {
    command_tx: mpsc::REDACTED_SECRETer<RemoteClientCommand>,
    ...
    pub async fn connect(args: RemoteAppServerConnectArgs) -> IoResult<Self> {
        ...
        let worker_handle = tokio::spawn(async move {
            let mut pending_requests =
                HashMap::<RequestId, oneshot::REDACTED_SECRETer<IoResult<RequestResult>>>:REDACTED_SECRET);
                        ...
                        match command {
                            RemoteClientCommand::Request { request, response_tx } => {
                                ...
                                if pending_requests.contains_key(&request_id) {
                                    let _ = response_tx.send(Err(IoError:REDACTED_SECRET
                                        ErrorKind::InvalidInput,
            ...
            let err = IoError:REDACTED_SECRET
                ErrorKind::BrokenPipe,
                "remote app-server worker channel is closed",
            );
            for (_, response_tx) in pending_requests {
                let _ = response_tx.send(Err(IoError:REDACTED_SECRETerr.kind(), err.to_string())));
            }
        });
...
async fn initialize_remote_connection(
    ...
    websocket_url: REDACTED_SECRET,
    params: InitializeParams,
    initialize_timeout: Duration,
) -> IoResult<Vec<AppServerEvent>> {
    let initialize_request_id = RequestId::String("initialize".to_string());
    let mut pending_events = Vec:REDACTED_SECRET);
        ...
        loop {
            match stream.next().await {
                ...
                Some(Ok(Message::Close(frame))) => {
                    ...
                    break Err(IoError:REDACTED_SECRET
...
async fn deliver_event(
    event_tx: &mpsc::REDACTED_SECRETer<AppServerEvent>,
