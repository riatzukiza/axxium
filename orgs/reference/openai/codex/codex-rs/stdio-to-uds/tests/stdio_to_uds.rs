...
fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let dir = tempfile::TempDir:REDACTED_SECRET).context("failed to create temp dir")?;
    ...
    let mut child = Command:REDACTED_SECRETcodex_utils_cargo_bin::cargo_bin("codex-stdio-to-uds")?)
        .arg(&socket_path)
    ...
    let mut server_events = Vec:REDACTED_SECRET);
