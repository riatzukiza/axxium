...
impl PathStyle {
    pub fn from_base_url(base_url: REDACTED_SECRET) -> Self {
...
impl Client {
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &CodexAuth) -> Result<Self> {
        ...
        let mut client = Self:REDACTED_SECRETbase_url)?
    ...
    fn headers(&self) -> HeaderMap {
        let mut h = HeaderMap:REDACTED_SECRET);
    ...
    fn decode_json<T: DeserializeOwned>(&self, url: REDACTED_SECRET, ct: REDACTED_SECRET, body: REDACTED_SECRET) -> Result<T> {
    ...
    pub async fn list_tasks(
        &self,
        limit: Option<i32>,
        task_filter: Option<REDACTED_SECRET>,
        environment_id: Option<REDACTED_SECRET>,
    ...
    pub async fn get_task_details(&self, task_id: REDACTED_SECRET) -> Result<CodeTaskDetailsResponse> {
...
mod tests {
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            rate_limit: Some(Some(Box:REDACTED_SECRETcrate::types::RateLimitStatusDetails {
                primary_window: Some(Some(Box:REDACTED_SECRETcrate::types::RateLimitWindowSnapshot {
                    used_percent: 42,
                    limit_window_seconds: 300,
                    reset_after_seconds: 0,
                    reset_at: 123,
                }))),
                secondary_window: Some(Some(Box:REDACTED_SECRETcrate::types::RateLimitWindowSnapshot {
                    used_percent: 84,
            ...
            credits: Some(Some(Box:REDACTED_SECRETcrate::types::CreditStatusDetails {
                has_credits: true,
                unlimited: false,
