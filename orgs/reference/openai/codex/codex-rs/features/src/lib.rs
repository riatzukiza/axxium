...
impl Features {
    ...
    pub fn with_defaults() -> Self {
        let mut set = BTreeSet:REDACTED_SECRET);
        ...
        Self {
            enabled: set,
            legacy_usages: BTreeSet:REDACTED_SECRET),
        }
    }
    ...
    pub fn record_legacy_usage_force(&mut self, alias: REDACTED_SECRET, feature: Feature) {
    ...
    pub fn record_legacy_usage(&mut self, alias: REDACTED_SECRET, feature: Feature) {
    ...
    pub fn apply_map(&mut self, m: &REDACTED_SECRET, bool>) {
        for (k, v) in m {
            match k.as_str() {
                "web_search_request" => {
...
fn legacy_usage_notice(alias: REDACTED_SECRET, feature: Feature) -> (String, Option<String>) {
...
pub fn feature_for_key(key: REDACTED_SECRET) -> Option<Feature> {
    for spec in FEATURES {
        if spec.key == key {
            return Some(spec.id);
        }
...
pub fn canonical_feature_for_key(key: REDACTED_SECRET) -> Option<Feature> {
    FEATURES
        .iter()
...
pub fn is_known_feature_key(key: REDACTED_SECRET) -> bool {
...
pub fn unstable_features_warning_event(
    ...
    features: &Features,
    config_path: REDACTED_SECRET,
) -> Option<Event> {
    if suppress_unstable_features_warning {
        return None;
    }

    let mut under_development_feature_keys = Vec:REDACTED_SECRET);
