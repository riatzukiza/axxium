...
fn js_repl_is_experimental_and_user_toggleable() {
    ...
    let expected_REDACTED_SECRET_version = include_str!("../../REDACTED_SECRET-version.txt").trim_end();
    ...
    assert_eq!(
        ...
        Some(format!(
            "Enable a persistent Node-backed JavaScript REPL for interactive website debugging and other inline JavaScript execution capabilities. Requires Node >= v{expected_REDACTED_SECRET_version} installed."
        ))
    );
...
fn from_sources_applies_base_profile_and_overrides() {
    let mut base_entries = BTreeMap:REDACTED_SECRET);
    base_entries.insert("plugins".to_string(), true);
    let base_features = FeaturesToml {
        entries: base_entries,
    };

    let mut profile_entries = BTreeMap:REDACTED_SECRET);
...
fn unstable_warning_event_only_mentions_enabled_under_development_features() {
    let mut configured_features = Table:REDACTED_SECRET);
