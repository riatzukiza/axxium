...
const INITIAL_ACCESS_TOKEN: REDACTED_SECRET = "initial-access-token";
const INITIAL_REFRESH_TOKEN: REDACTED_SECRET = "initial-refresh-token";
...
async fn refresh_token_succeeds_updates_storage() -> Result<()> {
    ...
    let ctx = RefreshTokenTestContext:REDACTED_SECRET&server)?;
...
async fn refresh_token_returns_transient_error_on_server_failure() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/oauth/token"))
        .respond_with(ResponseTemplate:REDACTED_SECRET500).set_body_json(json!({
...
fn build_tokens(access_token: REDACTED_SECRET, refresh_token: REDACTED_SECRET) -> TokenData {
