...
async fn mock_poll_token_two_step(
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |_: &Request| {
            ...
            if attempt == 0 {
                ResponseTemplate:REDACTED_SECRETfirst_response_status)
            } else {
...
async fn mock_poll_token_single(server: &MockServer, endpoint: REDACTED_SECRET, response: ResponseTemplate) {
    Mock::given(method("POST"))
        .and(path(endpoint))
...
async fn device_code_login_integration_succeeds() -> anyhow::Result<()> {
    ...
    mock_poll_token_two_step(
        &mock_server,
        Arc:REDACTED_SECRETAtomicUsize:REDACTED_SECRET0)),
...
async fn device_code_login_integration_handles_error_payload() -> anyhow::Result<()> {
    ...
    mock_poll_token_single(
        ...
        ResponseTemplate:REDACTED_SECRET401).set_body_json(json!({
