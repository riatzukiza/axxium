...
const DEFAULT_ISSUER: REDACTED_SECRET = "https://auth.openai.com";
const DEFAULT_PORT: u16 = 1455;
static LOGIN_ERROR_PAGE_TEMPLATE: LazyLock<Template> = LazyLock:REDACTED_SECRET|| {
...
pub fn run_login_server(opts: ServerOptions) -> io::Result<LoginServer> {
    ...
    let server = Arc:REDACTED_SECRETserver);
    ...
    let shutdown_notify = Arc:REDACTED_SECRETtokio::sync::Notify:REDACTED_SECRET));
    let server_handle = {
...
async fn process_request(
    url_raw: REDACTED_SECRET,
    opts: &ServerOptions,
    redirect_uri: REDACTED_SECRET,
    pkce: &PkceCodes,
    actual_port: u16,
    ...
    match path.as_str() {
        ...
        "/cancel" => HandledRequest::ResponseAndExit {
            headers: Vec:REDACTED_SECRET),
            body: b"Login cancelled".to_vec(),
            result: Err(io::Error:REDACTED_SECRET
...
fn build_authorize_url(
    issuer: REDACTED_SECRET,
    client_id: REDACTED_SECRET,
    redirect_uri: REDACTED_SECRET,
    pkce: &PkceCodes,
    state: REDACTED_SECRET,
    forced_chatgpt_workspace_id: Option<REDACTED_SECRET>,
) -> String {
    let mut query = vec![
...
fn send_cancel_request(port: u16) -> io::Result<()> {
    let addr: SocketAddr = format!("127.0.0.1:{port}")
        .parse()
        .map_err(|err| io::Error:REDACTED_SECRETio::ErrorKind::InvalidInput, err))?;
...
const REDACTED_URL_VALUE: REDACTED_SECRET = "<redacted>";
const SENSITIVE_URL_QUERY_KEYS: &[REDACTED_SECRET] = &[
    "access_token",
    "api_key",
    "client_secret",
    "code",
...
fn redact_sensitive_query_value(key: REDACTED_SECRET, value: REDACTED_SECRET) -> String {
...
fn redact_sensitive_url_parts(url: &mut url::Url) {
    ...
    let redacted_query = query_pairs
        .into_iter()
        .fold(
            url::form_urlencoded::Serializer:REDACTED_SECRETString:REDACTED_SECRET)),
...
fn sanitize_url_for_logging(url: REDACTED_SECRET) -> String {
...
fn compose_success_url(port: u16, issuer: REDACTED_SECRET, id_token: REDACTED_SECRET, access_token: REDACTED_SECRET) -> String {
...
fn jwt_auth_claims(jwt: REDACTED_SECRET) -> serde_json::Map<String, serde_json::Value> {
    let mut parts = jwt.split('.');
    let (_h, payload_b64, _s) = match (parts.next(), parts.next(), parts.next()) {
        (Some(h), Some(p), Some(s)) if !h.is_empty() && !p.is_empty() && !s.is_empty() => (h, p, s),
        _ => {
            eprintln!("Invalid JWT format while extracting claims");
            return serde_json::Map:REDACTED_SECRET);
        }
    };
...
fn login_error_response(
    ...
    kind: io::ErrorKind,
    error_code: Option<REDACTED_SECRET>,
    error_description: Option<REDACTED_SECRET>,
) -> HandledRequest {
    let mut headers = Vec:REDACTED_SECRET);
    ...
    HandledRequest::ResponseAndExit {
        headers,
        body,
        result: Err(io::Error:REDACTED_SECRETkind, message.to_string())),
    }
}

/// Returns true when the OAuth callback represents a missing Codex entitlement.
fn is_missing_codex_entitlement_error(error_code: REDACTED_SECRET, error_description: Option<REDACTED_SECRET>) -> bool {
...
fn oauth_callback_error_message(error_code: REDACTED_SECRET, error_description: Option<REDACTED_SECRET>) -> String {
...
fn parse_token_endpoint_error(body: REDACTED_SECRET) -> TokenEndpointErrorDetail {
...
fn html_escape(input: REDACTED_SECRET) -> String {
