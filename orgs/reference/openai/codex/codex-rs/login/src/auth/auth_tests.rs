...
fn login_with_api_key_overwrites_existing_auth_json() {
    ...
    let storage = FileAuthStorage:REDACTED_SECRETdir.path().to_path_buf());
    let auth = storage
...
fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let error = RefreshTokenFailedError:REDACTED_SECRET
...
async fn external_bearer_only_auth_manager_uses_cached_provider_token() {
    let script = ProviderAuthScript:REDACTED_SECRET&["provider-token", "next-token"]).unwrap();
...
async fn unauthorized_recovery_uses_external_refresh_for_bearer_manager() {
    let script = ProviderAuthScript:REDACTED_SECRET&["provider-token", "refreshed-provider-token"]).unwrap();
    let mut auth_config = script.auth_config();
    auth_config.refresh_interval_ms = 0;
    let manager = AuthManager::external_bearer_only(auth_config);
...
"#,
            ...
            std::fs::set_permissions(&script_path, permissions)?;
            ("./fail.sh".to_string(), Vec:REDACTED_SECRET))
        };
...
#[cfg(test)]
impl EnvVarGuard {
    fn set(key: &'static str, value: REDACTED_SECRET) -> Self {
