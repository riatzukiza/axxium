...
async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
    ...
    let storage = FileAuthStorage:REDACTED_SECRETcodex_home.path().to_path_buf());
...
fn id_token_with_prefix(prefix: REDACTED_SECRET) -> IdTokenInfo {
    #[derive(Serialize)]
    struct Header {
        alg: &'static str,
        typ: &'static str,
...
fn auth_with_prefix(prefix: REDACTED_SECRET) -> AuthDotJson {
    AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
...
fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
    ...
    let storage = KeyringAuthStorage:REDACTED_SECRET
        codex_home.path().to_path_buf(),
        Arc:REDACTED_SECRETmock_keyring.clone()),
    );
...
fn auto_auth_storage_load_prefers_keyring_value() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = AutoAuthStorage:REDACTED_SECRET
        codex_home.path().to_path_buf(),
        Arc:REDACTED_SECRETmock_keyring.clone()),
    );
...
fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let storage = AutoAuthStorage:REDACTED_SECRETcodex_home.path().to_path_buf(), Arc:REDACTED_SECRETmock_keyring));
