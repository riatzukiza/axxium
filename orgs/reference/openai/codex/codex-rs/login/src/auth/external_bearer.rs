...
impl BearerTokenRefresher {
    pub(crate) fn new(config: ModelProviderAuthInfo) -> Self {
        Self {
            state: Arc:REDACTED_SECRETExternalBearerAuthState:REDACTED_SECRETconfig)),
        }
    }
}
...
impl ExternalBearerAuthState {
    fn new(config: ModelProviderAuthInfo) -> Self {
        Self {
            config,
            cached_token: Mutex:REDACTED_SECRETNone),
        }
    }
}
...
async fn run_provider_auth_command(config: &ModelProviderAuthInfo) -> io::Result<String> {
    ...
    let mut command = Command:REDACTED_SECRET&program);
    command
        .args(&config.args)
...
fn resolve_provider_auth_program(command: REDACTED_SECRET, cwd: &Path) -> io::Result<PathBuf> {
    let path = Path:REDACTED_SECRETcommand);
    if path.is_absolute() {
