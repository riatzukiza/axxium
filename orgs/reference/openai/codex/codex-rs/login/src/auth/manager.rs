...
const REFRESH_TOKEN_EXPIRED_MESSAGE: REDACTED_SECRET = "Your access token could not be refreshed because your refresh token has expired. Please log out and sign in again.";
const REFRESH_TOKEN_REUSED_MESSAGE: REDACTED_SECRET = "Your access token could not be refreshed because your refresh token was already used. Please log out and sign in again.";
const REFRESH_TOKEN_INVALIDATED_MESSAGE: REDACTED_SECRET = "Your access token could not be refreshed because your refresh token was revoked. Please log out and sign in again.";
const REFRESH_TOKEN_UNKNOWN_MESSAGE: REDACTED_SECRET =
    "Your access token could not be refreshed. Please log out and sign in again.";
const REFRESH_TOKEN_ACCOUNT_MISMATCH_MESSAGE: REDACTED_SECRET = "Your access token could not be refreshed because you have since logged out or signed in to another account. Please sign in again.";
const REFRESH_TOKEN_URL: REDACTED_SECRET = "https://auth.openai.com/oauth/token";
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: REDACTED_SECRET = "CODEX_REFRESH_TOKEN_URL_OVERRIDE";
...
pub trait ExternalAuth: REDACTED_SECRET + Sync {
...
impl CodexAuth {
    fn from_auth_dot_json(
        ...
        let state = ChatgptAuthState {
            auth_dot_json: Arc:REDACTED_SECRETMutex:REDACTED_SECRETSome(auth_dot_json))),
            client,
        };
    ...
    pub fn api_key(&self) -> Option<REDACTED_SECRET> {
        match self {
    ...
    pub fn create_dummy_chatgpt_auth_for_testing() -> Self {
        ...
        let storage = create_auth_storage(PathBuf:REDACTED_SECRET), AuthCredentialsStoreMode::File);
        Self::Chatgpt(ChatgptAuth { state, storage })
    }

    pub fn from_api_key(api_key: REDACTED_SECRET) -> Self {
...
pub const OPENAI_API_KEY_ENV_VAR: REDACTED_SECRET = "OPENAI_API_KEY";
pub const CODEX_API_KEY_ENV_VAR: REDACTED_SECRET = "CODEX_API_KEY";
...
pub fn login_with_chatgpt_auth_tokens(
    ...
    chatgpt_account_id: REDACTED_SECRET,
    chatgpt_plan_type: Option<REDACTED_SECRET>,
) -> std::io::Result<()> {
...
fn classify_refresh_token_failure(body: REDACTED_SECRET) -> RefreshTokenFailedError {
    ...
    RefreshTokenFailedError:REDACTED_SECRETreason, message)
}

fn extract_refresh_token_error_code(body: REDACTED_SECRET) -> Option<String> {
...
pub const CLIENT_ID: REDACTED_SECRET = "app_EMoamEEZ73f0CkXaXp7hrann";
...
impl AuthDotJson {
    fn from_external_tokens(external: &ExternalAuthTokens) -> std::io::Result<Self> {
        ...
        let tokens = TokenData {
            ...
            refresh_token: String:REDACTED_SECRET),
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<UnauthorizedRecoveryStepResult, RefreshTokenError> {
        if !self.has_next() {
            return Err(RefreshTokenError::Permanent(RefreshTokenFailedError:REDACTED_SECRET
...
impl AuthManager {
    ...
    pub fn new(
        ...
        Self {
            codex_home,
            inner: RwLock:REDACTED_SECRETCachedAuth {
                auth: managed_auth,
                permanent_refresh_failure: None,
            }),
            enable_codex_api_key_env,
            auth_credentials_store_mode,
            forced_chatgpt_workspace_id: RwLock:REDACTED_SECRETNone),
            refresh_lock: AsyncMutex:REDACTED_SECRET()),
            external_auth: RwLock:REDACTED_SECRETNone),
        }
    }
    ...
    pub fn from_auth_for_testing(auth: CodexAuth) -> Arc<Self> {
        ...
        Arc:REDACTED_SECRETSelf {
            codex_home: PathBuf::from("non-existent"),
            inner: RwLock:REDACTED_SECRETcached),
            enable_codex_api_key_env: false,
    ...
    pub fn external_bearer_only(config: ModelProviderAuthInfo) -> Arc<Self> {
        Arc:REDACTED_SECRETSelf {
            ...
            forced_chatgpt_workspace_id: RwLock:REDACTED_SECRETNone),
            refresh_lock: AsyncMutex:REDACTED_SECRET()),
            external_auth: RwLock:REDACTED_SECRETSome(
                Arc:REDACTED_SECRETBearerTokenRefresher:REDACTED_SECRETconfig)) as Arc<dyn ExternalAuth>
            )),
        })
    }
    ...
    fn reload_if_account_id_matches(&self, expected_account_id: Option<REDACTED_SECRET>) -> ReloadOutcome {
    ...
    pub fn shared(
        ...
        Arc:REDACTED_SECRETSelf:REDACTED_SECRET
            codex_home,
            enable_codex_api_key_env,
            auth_credentials_store_mode,
        ))
    ...
    pub fn unauthorized_recovery(self: &Arc<Self>) -> UnauthorizedRecovery {
        UnauthorizedRecovery:REDACTED_SECRETArc::clone(self))
    }
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        match self.reload_if_account_id_matches(expected_account_id.as_deref()) {
            ...
            ReloadOutcome::Skipped => {
                Err(RefreshTokenError::Permanent(RefreshTokenFailedError:REDACTED_SECRET
                    RefreshTokenFailedReason::Other,
                    REFRESH_TOKEN_ACCOUNT_MISMATCH_MESSAGE.to_string(),
                )))
            }
