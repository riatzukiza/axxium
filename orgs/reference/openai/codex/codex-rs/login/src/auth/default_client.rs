...
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock:REDACTED_SECRET|| Mutex:REDACTED_SECRETNone));
pub const DEFAULT_ORIGINATOR: REDACTED_SECRET = "codex_cli_rs";
pub const CODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: REDACTED_SECRET = "CODEX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: REDACTED_SECRET = "x-openai-internal-codex-residency";
...
static ORIGINATOR: LazyLock<RwLock<Option<Originator>>> = LazyLock:REDACTED_SECRET|| RwLock:REDACTED_SECRETNone));
static REQUIREMENTS_RESIDENCY: LazyLock<RwLock<Option<ResidencyRequirement>>> =
    LazyLock:REDACTED_SECRET|| RwLock:REDACTED_SECRETNone));
...
pub fn is_first_party_originator(originator_value: REDACTED_SECRET) -> bool {
...
pub fn is_first_party_chat_originator(originator_value: REDACTED_SECRET) -> bool {
...
fn sanitize_user_agent(candidate: String, fallback: REDACTED_SECRET) -> String {
...
pub fn create_client() -> CodexHttpClient {
    ...
    CodexHttpClient:REDACTED_SECRETinner)
}
...
/// a custom-CA or builder failure is logged and falls back to `reqwest::Client:REDACTED_SECRET)`.
pub fn build_reqwest_client() -> reqwest::Client {
    try_build_reqwest_client().unwrap_or_else(|error| {
        tracing::warn!(error = %error, "failed to build default reqwest client");
        reqwest::Client:REDACTED_SECRET)
    })
}
