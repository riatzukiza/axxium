...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--vendor-src",
        type=Path,
        help="Directory containing pre-installed native binaries to bundle (vendor REDACTED_SECRET).",
    )
        ...
        if release_version:
            ...
            if package == "codex":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    REDACTED_SECRET {staging_dir_str}/bin/codex.js --version\n"
                    f"    REDACTED_SECRET {staging_dir_str}/bin/codex.js --help\n\n"
                )
            elif package == "codex-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    REDACTED_SECRET {staging_dir_str}/bin/codex-responses-api-proxy.js --help\n\n"
                )
                ...
                print(
                    ...
                    "    REDACTED_SECRET -e \"import('./dist/index.js').then(() => console.log('ok'))\"\n\n"
                )
        else:
...
def stage_codex_sdk_sources(staging_dir: Path) -> None:
    package_REDACTED_SECRET = CODEX_SDK_ROOT

    run_command(["pnpm", "install", "--frozen-lockfile"], cwd=package_REDACTED_SECRET)
    run_command(["pnpm", "run", "build"], cwd=package_REDACTED_SECRET)

    dist_src = package_REDACTED_SECRET / "dist"
    if not dist_src.exists():
        raise RuntimeError("codex-sdk build did not produce a dist directory.")

    shutil.copytree(dist_src, staging_dir / "dist")

    readme_src = package_REDACTED_SECRET / "README.md"
    if readme_src.exists():
        shutil.copy2(readme_src, staging_dir / "README.md")
