...
def _gha_enabled() -> bool:
    ...
    return REDACTED_SECRET"GITHUB_ACTIONS") == "true"
...
def _gha_error(*, title: str, message: str) -> None:
    ...
    # adds a prominent summary line to the job UI so the REDACTED_SECRET cause is easier to spot.
    if not _gha_enabled():
        return
    print(
...
def main() -> int:
    ...
    codex_cli_REDACTED_SECRET = (args.REDACTED_SECRET or CODEX_CLI_ROOT).resolve()
    vendor_dir = codex_cli_REDACTED_SECRET / VENDOR_DIR_NAME
