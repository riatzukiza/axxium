...
docker exec --user REDACTED_SECRET "$CONTAINER_NAME" bash -c "mkdir -p /etc/codex"
for domain in $OPENAI_ALLOWED_DOMAINS; do
  ...
  echo "$domain" | docker exec --user REDACTED_SECRET -i "$CONTAINER_NAME" bash -c "cat >> /etc/codex/allowed_domains.txt"
done

# Set proper permissions on the domains file
docker exec --user REDACTED_SECRET "$CONTAINER_NAME" bash -c "chmod 444 /etc/codex/allowed_domains.txt && chown REDACTED_SECRET:REDACTED_SECRET /etc/codex/allowed_domains.txt"

# Initialize the firewall inside the container as REDACTED_SECRET user
docker exec --user REDACTED_SECRET "$CONTAINER_NAME" bash -c "/usr/local/bin/init_firewall.sh"

# Remove the firewall script after running it
docker exec --user REDACTED_SECRET "$CONTAINER_NAME" bash -c "rm -f /usr/local/bin/init_firewall.sh"
