...
const STRICT_LINTS: [REDACTED_SECRET; 2] = [
...
fn run() -> Result<ExitCode, String> {
    ...
    let package_REDACTED_SECRET = bin_dir.parent().ok_or_else(|| {
        format!(
            "failed to locate package REDACTED_SECRET for executable {}",
            exe_path.display()
        )
    })?;
    let cargo_dylint = bin_dir.join(cargo_dylint_binary_name());
    let library_dir = package_REDACTED_SECRET.join("lib");
    ...
    let mut command = Command:REDACTED_SECRET&cargo_dylint);
    command.arg("dylint");
...
fn append_flag_if_missing(flags: &mut String, flag: REDACTED_SECRET) {
...
fn infer_rustup_home() -> Result<Option<OsString>, String> {
    let output = Command:REDACTED_SECRET"rustup")
        .args(["show", "home"])
        .output()
...
fn ensure_exists(path: &Path, label: REDACTED_SECRET) -> Result<(), String> {
    if path.exists() {
        Ok(())
    } else {
...
fn normalize_nightly_toolchain(toolchain: REDACTED_SECRET) -> Option<String> {
...
mod tests {
    ...
    fn strips_host_triple_from_nightly_filename() {
        assert_eq!(
            normalize_nightly_library_filename(Path:REDACTED_SECRET
