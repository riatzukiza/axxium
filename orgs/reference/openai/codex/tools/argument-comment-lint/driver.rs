...
impl rustc_driver::Callbacks for Callbacks {
    fn config(&mut self, config: &mut rustc_interface::Config) {
        ...
        config.register_lints = Some(Box:REDACTED_SECRETmove |sess, lint_store| {
...
fn is_rustc(arg: REDACTED_SECRET) -> bool {
    Path:REDACTED_SECRETarg).file_stem().and_then(|stem| stem.to_str()) == Some("rustc")
}
