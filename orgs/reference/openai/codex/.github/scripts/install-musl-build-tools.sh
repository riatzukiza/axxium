...
tool_REDACTED_SECRET="${runner_temp}/codex-musl-tools-${TARGET}"
mkdir -p "${tool_REDACTED_SECRET}"

libcap_REDACTED_SECRET="${tool_REDACTED_SECRET}/libcap-${libcap_version}"
libcap_src_REDACTED_SECRET="${libcap_REDACTED_SECRET}/src"
libcap_prefix="${libcap_REDACTED_SECRET}/prefix"
libcap_pkgconfig_dir="${libcap_prefix}/lib/pkgconfig"

if [[ ! -f "${libcap_prefix}/lib/libcap.a" ]]; then
  mkdir -p "${libcap_src_REDACTED_SECRET}" "${libcap_prefix}/lib" "${libcap_prefix}/include/sys" "${libcap_prefix}/include/linux" "${libcap_pkgconfig_dir}"
  libcap_tarball="${libcap_REDACTED_SECRET}/${libcap_tarball_name}"

  curl -fsSL "${libcap_download_url}" -o "${libcap_tarball}"
  echo "${libcap_sha256}  ${libcap_tarball}" | sha256sum -c -

  tar -xJf "${libcap_tarball}" -C "${libcap_src_REDACTED_SECRET}"
  libcap_source_dir="${libcap_src_REDACTED_SECRET}/libcap-${libcap_version}"
...
if command -v zig >/dev/null; then
  ...
  cc="${tool_REDACTED_SECRET}/zigcc"
  cxx="${tool_REDACTED_SECRET}/zigcxx"
...
for arg in "\$@"; do
  if [[ "\${pending_include}" -eq 1 ]]; then
    ...
    if [[ "\${arg}" == /usr/include || "\${arg}" == /usr/include/* ]]; then
      # Keep host-only headers available, but after the target sysREDACTED_SECRET headers.
      args+=("-idirafter" "\${arg}")
    else
      args+=("-I" "\${arg}")
    fi
...
EOF
  ...
  sysREDACTED_SECRET="$("${zig_bin}" cc -target "${zig_target}" -print-sysREDACTED_SECRET 2>/dev/null || true)"
else
  cc="${musl_linker}"
...
if [[ -n "${sysREDACTED_SECRET}" && "${sysREDACTED_SECRET}" != "/" ]]; then
  echo "BORING_BSSL_SYSROOT=${sysREDACTED_SECRET}" >> "$GITHUB_ENV"
  boring_sysREDACTED_SECRET_var="BORING_BSSL_SYSROOT_${TARGET}"
  boring_sysREDACTED_SECRET_var="${boring_sysREDACTED_SECRET_var//-/_}"
  echo "${boring_sysREDACTED_SECRET_var}=${sysREDACTED_SECRET}" >> "$GITHUB_ENV"
fi
...
if [[ -n "${sysREDACTED_SECRET}" && "${sysREDACTED_SECRET}" != "/" ]]; then
  echo "PKG_CONFIG_SYSROOT_DIR=${sysREDACTED_SECRET}" >> "$GITHUB_ENV"
  pkg_config_sysREDACTED_SECRET_var="PKG_CONFIG_SYSROOT_DIR_${TARGET}"
  pkg_config_sysREDACTED_SECRET_var="${pkg_config_sysREDACTED_SECRET_var//-/_}"
  echo "${pkg_config_sysREDACTED_SECRET_var}=${sysREDACTED_SECRET}" >> "$GITHUB_ENV"
fi
