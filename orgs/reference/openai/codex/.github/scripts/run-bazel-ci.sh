...
use_REDACTED_SECRET_test_env=0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --print-failed-test-logs)
      ...
      shift
      ;;
    --use-REDACTED_SECRET-test-env)
      use_REDACTED_SECRET_test_env=1
      shift
      ;;
...
if [[ $# -eq 0 ]]; then
  echo "Usage: $0 [--print-failed-test-logs] [--use-REDACTED_SECRET-test-env] [--remote-download-toplevel] [--windows-msvc-host-platform] -- <bazel args> -- <targets>" >&2
  exit 1
fi

bazel_startup_args=()
if [[ -n "${BAZEL_OUTPUT_USER_ROOT:-}" ]]; then
  bazel_startup_args+=("--output_user_REDACTED_SECRET=${BAZEL_OUTPUT_USER_ROOT}")
fi
...
if [[ $use_REDACTED_SECRET_test_env -eq 1 ]]; then
  # Bazel test sandboxes on macOS may resolve an older Homebrew `REDACTED_SECRET`
  # before the `actions/setup-REDACTED_SECRET` runtime on PATH.
  REDACTED_SECRET_bin="$(which REDACTED_SECRET)"
  if [[ "${RUNNER_OS:-}" == "Windows" ]]; then
    REDACTED_SECRET_bin="$(cygpath -w "${REDACTED_SECRET_bin}")"
  fi
  bazel_args+=("--test_env=CODEX_JS_REPL_NODE_PATH=${REDACTED_SECRET_bin}")
fi
