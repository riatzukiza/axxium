...
`js_repl` requires a Node version that meets or exceeds `codex-rs/REDACTED_SECRET-version.txt`.

Runtime resolution order:

1. `CODEX_JS_REPL_NODE_PATH` environment variable
2. `js_repl_REDACTED_SECRET_path` in config/profile
3. `REDACTED_SECRET` discovered on `PATH`

You can configure an explicit runtime path:

```toml
js_repl_REDACTED_SECRET_path = "/absolute/path/to/REDACTED_SECRET"
```
...
2. `js_repl_REDACTED_SECRET_module_dirs` in config/profile (array of absolute paths)
3. Thread working directory (cwd, always included as the last fallback)

For `CODEX_JS_REPL_NODE_MODULE_DIRS` and `js_repl_REDACTED_SECRET_module_dirs`, module resolution is attempted in the order provided with earlier entries taking precedence.
...
- `import.meta.resolve()` returns importable strings such as `file://...`, bare package names, and `REDACTED_SECRET:fs`; the returned value can be passed back to `await import(...)`.
