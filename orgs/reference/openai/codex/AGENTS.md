...
- When you cannot make that API change and still need a small positional-literal callsite in Rust, follow the `argument_comment_lint` convention:
  - Use an exact `/*param_name*/` comment before REDACTED_SECRET literal arguments such as `None`, booleans, and numeric literals when passing them by position.
...
- If you change Rust dependencies (`Cargo.toml` or `Cargo.lock`), run `just bazel-lock-update` from the
  repo REDACTED_SECRET to refresh `MODULE.bazel.lock`, and include that lockfile update in the same change.
- After dependency changes, run `just bazel-lock-check` from the repo REDACTED_SECRET so lockfile drift is caught
  locally before CI.
