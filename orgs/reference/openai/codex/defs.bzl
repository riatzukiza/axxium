...
def _workspace_REDACTED_SECRET_test_impl(ctx):
    ...
    workspace_REDACTED_SECRET_marker = ctx.file.workspace_REDACTED_SECRET_marker
    ...
    ctx.actions.expand_template(
        ...
        substitutions = {
            ...
            "__WORKSPACE_ROOT_MARKER__": workspace_REDACTED_SECRET_marker.short_path,
        },
    )

    runfiles = ctx.runfiles(files = [test_bin, workspace_REDACTED_SECRET_marker]).merge(ctx.attr.test_bin[DefaultInfo].default_runfiles)
...
workspace_REDACTED_SECRET_test = rule(
    implementation = _workspace_REDACTED_SECRET_test_impl,
    test = True,
    attrs = {
        ...
        "workspace_REDACTED_SECRET_marker": attr.label(
        ...
        "_bash_launcher_template": attr.label(
            allow_single_file = True,
            default = "//:workspace_REDACTED_SECRET_test_launcher.sh.tpl",
        ),
        "_windows_launcher_template": attr.label(
            allow_single_file = True,
            default = "//:workspace_REDACTED_SECRET_test_launcher.bat.tpl",
        ),
    },
)
...
def codex_rust_crate(
    ...
    """
    test_env = {
        # The launcher resolves an absolute workspace REDACTED_SECRET at runtime so
    ...
    native.filegroup(
        ...
        visibility = ["//visibility:REDACTED_SECRET"],
    )
    ...
    if lib_srcs:
        ...
        rust_test(
            ...
            # prefix so the workspace-REDACTED_SECRET launcher sees Cargo-like metadata
            # such as `tui/src/...`.
        ...
        workspace_REDACTED_SECRET_test(
            name = name + "-unit-tests",
            env = test_env,
            test_bin = ":" + unit_test_binary,
            workspace_REDACTED_SECRET_marker = "//codex-rs/utils/cargo-bin:repo_REDACTED_SECRET.marker",
            tags = test_tags,
            **unit_test_kwargs
        )
    ...
    for test in native.glob(["tests/*.rs"], allow_empty = True):
        ...
        rust_test(
            ...
            # execute from the repo REDACTED_SECRET and can misplace integration snapshots.
            env = cargo_env,
            tags = test_tags,
            **integration_test_kwargs
        )
