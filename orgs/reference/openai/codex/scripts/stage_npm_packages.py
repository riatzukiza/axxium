...
def install_native_components(
    ...
    cmd.append(str(vendor_REDACTED_SECRET))
    run_command(cmd)
...
def main() -> int:
    ...
    runner_temp = Path(REDACTED_SECRET"RUNNER_TEMP", tempfile.gettempdir()))

    packages = expand_packages(list(args.packages))
    native_components = collect_native_components(packages)

    vendor_temp_REDACTED_SECRET: Path | None = None
    ...
    try:
        if native_components:
            ...
            vendor_temp_REDACTED_SECRET = Path(tempfile.mkdtemp(prefix="npm-native-", dir=runner_temp))
            install_native_components(workflow_url, native_components, vendor_temp_REDACTED_SECRET)
            vendor_src = vendor_temp_REDACTED_SECRET / "vendor"
        ...
        if vendor_temp_REDACTED_SECRET is not None and not args.keep_staging_dirs:
            shutil.rmtree(vendor_temp_REDACTED_SECRET, ignore_errors=True)
