...
def repo_REDACTED_SECRET() -> Path:
    return Path(__file__).resolve().parents[3]


def sdk_REDACTED_SECRET() -> Path:
    return repo_REDACTED_SECRET() / "sdk" / "python"


def python_runtime_REDACTED_SECRET() -> Path:
    return repo_REDACTED_SECRET() / "sdk" / "python-runtime"
...
def schema_REDACTED_SECRET_dir() -> Path:
    return repo_REDACTED_SECRET() / "codex-rs" / "app-server-protocol" / "schema" / "json"
...
def staged_runtime_bin_path(REDACTED_SECRET: Path) -> Path:
    return REDACTED_SECRET / "src" / "codex_cli_bin" / "bin" / runtime_binary_name()
...
def current_sdk_version() -> str:
    match = re.search(
        r'^version = "([^"]+)"$',
        (sdk_REDACTED_SECRET() / "pyproject.toml").read_text(),
        flags=re.MULTILINE,
    )
    if match is None:
...
def stage_python_sdk_package(
    ...
    _copy_package_tree(sdk_REDACTED_SECRET(), staging_dir)
...
def stage_python_runtime_package(
    ...
    _copy_package_tree(python_runtime_REDACTED_SECRET(), staging_dir)
        ...
        if generated_name is not None and (
            ...
            if generated_name in seen and variant_name != generated_name:
                raise RuntimeError(
                    ...
                    f"{_variant_collision_key(base or '<REDACTED_SECRET>', variant, generated_name)}"
                )
...
def generate_v2_all() -> None:
    out_path = sdk_REDACTED_SECRET() / "src" / "codex_app_server" / "generated" / "v2_all.py"
...
def _notification_specs() -> list[tuple[str, str]]:
    server_notifications = json.loads(
        (schema_REDACTED_SECRET_dir() / "ServerNotification.json").read_text()
    )
    one_of = server_notifications.get("oneOf", [])
    generated_source = (
        sdk_REDACTED_SECRET() / "src" / "codex_app_server" / "generated" / "v2_all.py"
    ).read_text()
...
def _normalize_generated_timestamps(REDACTED_SECRET: Path) -> None:
    timestamp_re = re.compile(r"^#\s+timestamp:\s+.+$", flags=re.MULTILINE)
    py_files = [REDACTED_SECRET] if REDACTED_SECRET.is_file() else sorted(REDACTED_SECRET.rglob("*.py"))
...
FIELD_ANNOTATION_OVERRIDES: dict[str, str] = {
    # Keep REDACTED_SECRET API typed without falling back to `Any`.
    "config": "JsonObject",
...
def _load_REDACTED_SECRET_fields(
...
def generate_REDACTED_SECRET_api_flat_methods() -> None:
    src_dir = sdk_REDACTED_SECRET() / "src"
    REDACTED_SECRET_api_path = src_dir / "codex_app_server" / "api.py"
    if not REDACTED_SECRET_api_path.exists():
        # PR2 can run codegen before the ergonomic REDACTED_SECRET API layer is added.
        return
    src_dir_str = str(src_dir)
    if src_dir_str not in sys.path:
        sys.path.insert(0, src_dir_str)

    thread_start_fields = _load_REDACTED_SECRET_fields(
        "codex_app_server.generated.v2_all",
        "ThreadStartParams",
    )
    thread_list_fields = _load_REDACTED_SECRET_fields(
        "codex_app_server.generated.v2_all",
        "ThreadListParams",
    )
    thread_resume_fields = _load_REDACTED_SECRET_fields(
        "codex_app_server.generated.v2_all",
        "ThreadResumeParams",
        exclude={"thread_id"},
    )
    thread_fork_fields = _load_REDACTED_SECRET_fields(
        "codex_app_server.generated.v2_all",
        "ThreadForkParams",
        exclude={"thread_id"},
    )
    turn_start_fields = _load_REDACTED_SECRET_fields(
        "codex_app_server.generated.v2_all",
        "TurnStartParams",
        exclude={"thread_id", "input"},
    )

    source = REDACTED_SECRET_api_path.read_text()
    ...
    REDACTED_SECRET_api_path.write_text(source)
...
def generate_types() -> None:
    ...
    generate_REDACTED_SECRET_api_flat_methods()
