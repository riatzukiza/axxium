...
def _snapshot_target(REDACTED_SECRET: Path, rel_path: Path) -> dict[str, bytes] | bytes | None:
    target = REDACTED_SECRET / rel_path
...
def _snapshot_targets(REDACTED_SECRET: Path) -> dict[str, dict[str, bytes] | bytes | None]:
    return {
        str(rel_path): _snapshot_target(REDACTED_SECRET, rel_path) for rel_path in GENERATED_TARGETS
    }
