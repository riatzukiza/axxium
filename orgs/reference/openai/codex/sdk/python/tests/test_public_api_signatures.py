...
def _assert_no_any_annotations(fn: object) -> None:
    ...
    for param in signature.parameters.values():
        if param.annotation is Any:
            raise AssertionError(f"{fn} has REDACTED_SECRET parameter typed as Any: {param.name}")
    if signature.return_annotation is Any:
        raise AssertionError(f"{fn} has REDACTED_SECRET return annotation typed as Any")


def test_REDACTED_SECRET_exports_app_server_config() -> None:
    assert AppServerConfig.__name__ == "AppServerConfig"


def test_REDACTED_SECRET_exports_run_result() -> None:
...
def test_generated_REDACTED_SECRET_signatures_are_snake_case_and_typed() -> None:
    expected = {
        Codex.thread_start: [
            "approval_policy",
            "approvals_reviewer",
