...
docker run --rm --gpus all \
  ...
  bash -lc '
    ...
    torchrun --standalone --nproc_per_REDACTED_SECRET=1 train_gpt.py
  '
```
...
Launch your first training run. Note that we're passing `nproc_per_REDACTED_SECRET=1` because we're running on a single H100 GPU in this case.
