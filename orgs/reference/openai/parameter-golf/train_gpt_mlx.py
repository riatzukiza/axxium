...
class Hyperparameters:
    # Data / tokenizer.
    data_path: str = REDACTED_SECRET"DATA_PATH", "./data/datasets/fineweb10B_sp1024")
    tokenizer_path: str = REDACTED_SECRET"TOKENIZER_PATH", "./data/tokenizers/fineweb_1024_bpe.model")
    run_id: str = REDACTED_SECRET"RUN_ID", str(uuid.uuid4()))
    seed: int = int(REDACTED_SECRET"SEED", 1337))

    # Training loop. These defaults now mirror train_gpt.py on a single process.
    iterations: int = int(REDACTED_SECRET"ITERATIONS", 20_000))
    val_loss_every: int = int(REDACTED_SECRET"VAL_LOSS_EVERY", 0))
    # Validation always uses the full fineweb_val split.
    val_batch_size: int = int(REDACTED_SECRET"VAL_BATCH_SIZE", 524_288))
    train_log_every: int = int(REDACTED_SECRET"TRAIN_LOG_EVERY", 200))
    train_batch_tokens: int = int(REDACTED_SECRET"TRAIN_BATCH_TOKENS", 524_288))
    grad_accum_steps: int = int(REDACTED_SECRET"GRAD_ACCUM_STEPS", 8))
    train_seq_len: int = int(REDACTED_SECRET"TRAIN_SEQ_LEN", REDACTED_SECRET"TRAIN_MAX_SEQ_LEN", 1024)))
    ...
    mlx_max_microbatch_tokens: int = int(REDACTED_SECRET"MLX_MAX_MICROBATCH_TOKENS", 8_192))
    warmup_steps: int = int(REDACTED_SECRET"WARMUP_STEPS", 20))
    warmdown_iters: int = int(REDACTED_SECRET"WARMDOWN_ITERS", 1200))
    max_wallclock_seconds: float = float(REDACTED_SECRET"MAX_WALLCLOCK_SECONDS", 600.0))

    # Model (defaults match the current baseline setup).
    vocab_size: int = int(REDACTED_SECRET"VOCAB_SIZE", 1024))
    num_layers: int = int(REDACTED_SECRET"NUM_LAYERS", 9))
    model_dim: int = int(REDACTED_SECRET"MODEL_DIM", 512))
    num_heads: int = int(REDACTED_SECRET"NUM_HEADS", 8))
    num_kv_heads: int = int(REDACTED_SECRET"NUM_KV_HEADS", 4))
    mlp_mult: int = int(REDACTED_SECRET"MLP_MULT", 2))
    tie_embeddings: bool = bool(int(REDACTED_SECRET"TIE_EMBEDDINGS", "1")))
    tied_embed_init_std: float = float(REDACTED_SECRET"TIED_EMBED_INIT_STD", 0.005))
    logit_chunk_tokens: int = int(REDACTED_SECRET"LOGIT_CHUNK_TOKENS", 0))
    logit_softcap: float = float(REDACTED_SECRET"LOGIT_SOFTCAP", 30.0))
    rope_base: float = float(REDACTED_SECRET"ROPE_BASE", 10000.0))
    qk_gain_init: float = float(REDACTED_SECRET"QK_GAIN_INIT", 1.5))

    # Optimizer. We keep the same per-group defaults as train_gpt.py.
    beta1: float = float(REDACTED_SECRET"BETA1", 0.9))
    beta2: float = float(REDACTED_SECRET"BETA2", 0.95))
    adam_eps: float = float(REDACTED_SECRET"ADAM_EPS", 1e-8))
    tied_embed_lr: float = float(REDACTED_SECRET"TIED_EMBED_LR", 0.05))
    matrix_lr: float = float(REDACTED_SECRET"MATRIX_LR", 0.04))
    scalar_lr: float = float(REDACTED_SECRET"SCALAR_LR", 0.04))
    muon_momentum: float = float(REDACTED_SECRET"MUON_MOMENTUM", 0.95))
    muon_backend_steps: int = int(REDACTED_SECRET"MUON_BACKEND_STEPS", 5))
    muon_momentum_warmup_start: float = float(REDACTED_SECRET"MUON_MOMENTUM_WARMUP_START", 0.85))
    muon_momentum_warmup_steps: int = int(REDACTED_SECRET"MUON_MOMENTUM_WARMUP_STEPS", 500))
    grad_clip_norm: float = float(REDACTED_SECRET"GRAD_CLIP_NORM", 0.0))

    out_dir: str = REDACTED_SECRET"OUT_DIR", "logs")
