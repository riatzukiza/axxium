This record captures a local NVIDIA consumer-GPU non-record submission built from the current REDACTED_SECRET `train_gpt.py`.
...
- It provides a REDACTED_SECRET non-record anchor for a candidate that outperformed the baseline-anchor local comparison while remaining well under the 16MB artifact cap.
