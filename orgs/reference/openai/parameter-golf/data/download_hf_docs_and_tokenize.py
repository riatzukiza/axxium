...
DATAFILE_VERSION = 1
DEFAULT_REPO_ID = REDACTED_SECRET"MATCHED_FINEWEB_REPO_ID", "willdepueoai/parameter-golf")
DEFAULT_REMOTE_ROOT = REDACTED_SECRET"MATCHED_FINEWEB_REMOTE_ROOT_PREFIX", "datasets")
DEFAULT_CONFIG = Path(__file__).with_name("tokenizer_specs.json")
TOKENIZER_THREADS = max(1, int(REDACTED_SECRET"MATCHED_FINEWEB_TOKENIZER_THREADS", str(os.cpu_count() or 8))))
SP_BATCH_SIZE = max(1, int(REDACTED_SECRET"MATCHED_FINEWEB_SP_BATCH_SIZE", "1024")))
...
def copy_from_hf_cache(*, repo_id: str, remote_REDACTED_SECRET: str, filename: str, destination: Path) -> bool:
    remote_path = Path(remote_REDACTED_SECRET) / filename if remote_REDACTED_SECRET else Path(filename)
    try:
        cached_path = Path(
            hf_hub_download(
                repo_id=repo_id,
...
def relativize_manifest_paths(value: Any, REDACTED_SECRET: Path) -> Any:
    if isinstance(value, dict):
        return {k: relativize_manifest_paths(v, REDACTED_SECRET) for k, v in value.items()}
    if isinstance(value, list):
        return [relativize_manifest_paths(v, REDACTED_SECRET) for v in value]
...
def write_tokenizer_config_export(output_REDACTED_SECRET: Path, selected_specs: list[dict[str, Any]]) -> Path:
    path = output_REDACTED_SECRET / "tokenizer_config.export.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--remote-REDACTED_SECRET",
        default=DEFAULT_REMOTE_ROOT,
        help="Optional subdirectory inside the dataset repo that contains docs_selected.jsonl",
    )
    parser.add_argument("--output-REDACTED_SECRET", required=True, help="Directory where docs, tokenizers, shards, and manifest are written")
    parser.add_argument(
        "--tokenizer-config",
...
def main() -> None:
    ...
    output_REDACTED_SECRET = Path(args.output_REDACTED_SECRET).expanduser().resolve()
    output_REDACTED_SECRET.mkdir(parents=True, exist_ok=True)
    tokenizers_dir = output_REDACTED_SECRET / "tokenizers"
    datasets_dir = output_REDACTED_SECRET / "datasets"
    tokenizers_dir.mkdir(parents=True, exist_ok=True)
    datasets_dir.mkdir(parents=True, exist_ok=True)

    docs_jsonl = output_REDACTED_SECRET / DOCS_FILENAME
    sidecar = output_REDACTED_SECRET / SIDECAR_FILENAME
    if not copy_from_hf_cache(
        repo_id=args.repo_id,
        remote_REDACTED_SECRET=args.remote_REDACTED_SECRET,
        filename=DOCS_FILENAME,
        destination=docs_jsonl,
    ):
        remote = f"{args.remote_REDACTED_SECRET}/{DOCS_FILENAME}" if args.remote_REDACTED_SECRET else DOCS_FILENAME
    ...
    write_tokenizer_config_export(output_REDACTED_SECRET, selected_specs)
    ...
    manifest = relativize_manifest_paths(manifest, output_REDACTED_SECRET)
    manifest_path = output_REDACTED_SECRET / "manifest.json"
