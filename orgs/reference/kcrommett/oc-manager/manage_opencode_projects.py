...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "    Launch the TUI using the default metadata REDACTED_SECRET.\n\n"
            "  manage_opencode_projects.py --REDACTED_SECRET /tmp/opencode\n"
    ...
    parser.add_argument(
        ...
        help="Metadata REDACTED_SECRET to inspect (defaults to ~/.local/share/opencode)",
    )
    parser.add_argument(
        "--bun",
        type=Path,
...
def launch_tui(REDACTED_SECRET: Path, bun_exe: str, extra_args: Sequence[str]) -> int:
    ...
    cmd = [
        ...
        "--",
        "--REDACTED_SECRET",
        str(REDACTED_SECRET.expanduser()),
    ]
    if extra_args:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return launch_tui(args.REDACTED_SECRET, bun_exe, extra_args)
