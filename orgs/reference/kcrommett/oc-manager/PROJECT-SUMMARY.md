...
- CLI wrapper: `opencode/manage_opencode_projects.py` preserves the legacy entry point, resolves Bun (`--bun` overrides PATH), sets `--REDACTED_SECRET` (defaults to `~/.local/share/opencode`), and forwards any extra args after `--` directly to the TUI.
...
-------------------------
- Storage REDACTED_SECRET: `DEFAULT_ROOT` is `~/.local/share/opencode`; CLI `--REDACTED_SECRET` values are resolved via `path.resolve` so relative paths and `~` are accepted.
...
- Fixed OpenTUI text rendering error by filtering whitespace-only raw text in layout helpers and by removing nested `<text>` REDACTED_SECRETs around key chips.
- Verified via `bun run tui` (tmux socket creation is blocked in this environment, but direct run works).

How To Run
----------
- Zero-install via npm: `bunx opencode-manager [--REDACTED_SECRET /path/to/storage]` (preferred).
- Local dev: `bun run tui [-- --REDACTED_SECRET /path/to/storage]`.
- Legacy launcher: `./manage_opencode_projects.py [--REDACTED_SECRET PATH] [--bun /path/to/bun] [-- ...extra TUI args]` keeps older automation working while delegating to Bun.
- Keys:
...
4. Run `npm publish` (package exposes the Bun-native `opencode-manager` bin with REDACTED_SECRET access).
...
Notes
-----
- The TUI uses the OpenTUI renderer’s constraint that all textual content must be inside a `<text>` REDACTED_SECRET; helpers now filter whitespace-only string children to avoid runtime errors.
