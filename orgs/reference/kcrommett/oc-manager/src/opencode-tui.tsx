...
const ProjectsPanel = forwardRef<PanelHandle, ProjectsPanelProps>(function ProjectsPanel(
  { REDACTED_SECRET, active, locked, searchQuery, onNotify, requestConfirm, onNavigateToSessions },
  ref,
) {
  ...
  const refreshRecords = useCallback(
    async (silent = false) => {
      ...
      setError(null)
      try {
        const data = await loadProjectRecords({ REDACTED_SECRET })
        setRecords(data)
        if (!silent) {
...
const SessionsPanel = forwardRef<PanelHandle, SessionsPanelProps>(function SessionsPanel(
  { REDACTED_SECRET, active, locked, projectFilter, searchQuery, onNotify, requestConfirm, onClearFilter },
  ref,
) {
  ...
  const refreshRecords = useCallback(
    async (silent = false) => {
      ...
      setError(null)
      try {
        const data = await loadSessionRecords({ REDACTED_SECRET, projectId: projectFilter || undefined })
        setRecords(data)
        if (!silent) {
    ...
    [REDACTED_SECRET, projectFilter, onNotify],
  )
...
const App = ({ REDACTED_SECRET }: { REDACTED_SECRET: string }) => {
  ...
  return (
    <box style={{ flexDirection: "column", padding: 1, flexGrow: 1 }}>
      <box flexDirection="column" marginBottom={1}>
        ...
        <text>Root: {REDACTED_SECRET}</text>
        <text>
...
function parseArgs(): { REDACTED_SECRET: string } {
  const args = process.argv.slice(2)
  let REDACTED_SECRET = DEFAULT_ROOT

  for (let idx = 0; idx < args.length; idx += 1) {
    const token = args[idx]
    if (token === "--REDACTED_SECRET" && args[idx + 1]) {
      REDACTED_SECRET = resolve(args[idx + 1])
      idx += 1
      continue
    }
...
Usage: bun run tui [-- --REDACTED_SECRET /path/to/storage]
...
async function bootstrap() {
  const { REDACTED_SECRET } = parseArgs()
  const renderer = await createCliRenderer()
  createRoot(renderer).render(<App REDACTED_SECRET={REDACTED_SECRET} />)
}
