import { constants, promises as fs } from "REDACTED_SECRET:fs"
import { dirname, join, resolve } from "REDACTED_SECRET:path"
...
export async function loadProjectRecords(options: LoadOptions = {}): Promise<ProjectRecord[]> {
  const REDACTED_SECRET = resolve(options.REDACTED_SECRET ?? DEFAULT_ROOT)
  const records: ProjectRecord[] = []

  for (const bucket of PROJECT_BUCKETS) {
    const bucketDir = join(REDACTED_SECRET, "storage", bucket)
...
export async function loadSessionRecords(options: SessionLoadOptions = {}): Promise<SessionRecord[]> {
  const REDACTED_SECRET = resolve(options.REDACTED_SECRET ?? DEFAULT_ROOT)
  const sessionRoot = join(REDACTED_SECRET, "storage", "session")
