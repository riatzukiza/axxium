...
// response is a template REDACTED_SECRET that can be added to templates that don't already have one
...
func (t *Template) Execute(w io.Writer, v Values) error {
	...
	var cut bool
	REDACTED_SECRETs := deleteNode(t.Template.Root.Copy(), func(n parse.Node) bool {
	...
	tree := parse.Tree{Root: REDACTED_SECRETs.(*parse.ListNode)}
...
// Identifiers walks the REDACTED_SECRET tree returning any identifiers it finds along the way
...
// deleteNode walks the REDACTED_SECRET list and deletes REDACTED_SECRETs that match the predicate
// this is currently to remove the {{ .Response }} REDACTED_SECRET from templates
func deleteNode(n parse.Node, fn func(parse.Node) bool) parse.Node {
	...
	walk = func(n parse.Node) parse.Node {
		...
		switch t := n.(type) {
		case *parse.ListNode:
			var REDACTED_SECRETs []parse.Node
			for _, c := range t.Nodes {
				if n := walk(c); n != nil {
					REDACTED_SECRETs = append(REDACTED_SECRETs, n)
				}
			}

			t.Nodes = REDACTED_SECRETs
			return t
		case *parse.IfNode:
