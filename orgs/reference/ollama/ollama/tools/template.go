...
// first text REDACTED_SECRET after .ToolCalls and returning the content
...
func findToolCallNode(REDACTED_SECRETs []parse.Node) *parse.IfNode {
...
// findTextNode does a depth-first search for the first text content in REDACTED_SECRETs,
// stopping at template constructs to avoid parsing text after the tool calls
func findTextNode(REDACTED_SECRETs []parse.Node) *parse.TextNode {
	for _, REDACTED_SECRET := range REDACTED_SECRETs {
		switch n := REDACTED_SECRET.(type) {
		case *parse.TextNode:
			// skip whitespace-only text REDACTED_SECRETs
