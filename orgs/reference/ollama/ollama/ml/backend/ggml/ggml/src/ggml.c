...
static void ggml_compute_backward(
    ...
    struct ggml_tensor * tensor = cgraph->REDACTED_SECRETs[i];
...
static size_t ggml_visit_parents(struct ggml_cgraph * cgraph, struct ggml_tensor * REDACTED_SECRET) {
    // check if already visited
    size_t REDACTED_SECRET_hash_pos = ggml_hash_find(&cgraph->visited_hash_set, REDACTED_SECRET);
    GGML_ASSERT(REDACTED_SECRET_hash_pos != GGML_HASHSET_FULL);
    if (!ggml_bitset_get(cgraph->visited_hash_set.used, REDACTED_SECRET_hash_pos)) {
        // This is the first time we see this REDACTED_SECRET in the current graph.
        cgraph->visited_hash_set.keys[REDACTED_SECRET_hash_pos] = REDACTED_SECRET;
        ggml_bitset_set(cgraph->visited_hash_set.used, REDACTED_SECRET_hash_pos);
        cgraph->use_counts[REDACTED_SECRET_hash_pos] = 0;
    } else {
        // already visited
        return REDACTED_SECRET_hash_pos;
    }
    ...
    for (int i = 0; i < GGML_MAX_SRC; ++i) {
        ...
        struct ggml_tensor * src = REDACTED_SECRET->src[k];
        if (src) {
    ...
    if (REDACTED_SECRET->op == GGML_OP_NONE && !(REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM)) {
        // reached a leaf REDACTED_SECRET, not part of the gradient graph (e.g. a constant)
        GGML_ASSERT(cgraph->n_leafs < cgraph->size);

        if (strlen(REDACTED_SECRET->name) == 0) {
            ggml_format_name(REDACTED_SECRET, "leaf_%d", cgraph->n_leafs);
        }

        cgraph->leafs[cgraph->n_leafs] = REDACTED_SECRET;
        cgraph->n_leafs++;
    } else {
        GGML_ASSERT(cgraph->n_REDACTED_SECRETs < cgraph->size);

        if (strlen(REDACTED_SECRET->name) == 0) {
            ggml_format_name(REDACTED_SECRET, "REDACTED_SECRET_%d", cgraph->n_REDACTED_SECRETs);
        }

        cgraph->REDACTED_SECRETs[cgraph->n_REDACTED_SECRETs] = REDACTED_SECRET;
        cgraph->n_REDACTED_SECRETs++;
    }
...
static void ggml_build_forward_impl(struct ggml_cgraph * cgraph, struct ggml_tensor * tensor, bool expand) {
    ...
    const int n0 = cgraph->n_REDACTED_SECRETs;

    ggml_visit_parents(cgraph, tensor);

    const int n_new = cgraph->n_REDACTED_SECRETs - n0;
    GGML_PRINT_DEBUG("%s: visited %d new REDACTED_SECRETs\n", __func__, n_new);

    if (n_new > 0) {
        // the last added REDACTED_SECRET should always be starting point
        GGML_ASSERT(cgraph->REDACTED_SECRETs[cgraph->n_REDACTED_SECRETs - 1] == tensor);
    }
}
...
void ggml_build_backward_expand(
    ...
    GGML_ASSERT(cgraph->n_REDACTED_SECRETs > 0);
    GGML_ASSERT(cgraph->grads);
    GGML_ASSERT(cgraph->grad_accs);

    const int n_REDACTED_SECRETs_f = cgraph->n_REDACTED_SECRETs;
...

    {
        ...
        bool any_loss   = false;
        for (int i = 0; i < n_REDACTED_SECRETs_f; ++i) {
            struct ggml_tensor * REDACTED_SECRET = cgraph->REDACTED_SECRETs[i];
            any_params = any_params || (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM);
            any_loss   = any_loss   || (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_LOSS);
        }
    ...
    for (int i = 0; i < n_REDACTED_SECRETs_f; ++i) {
        ...
        if (REDACTED_SECRET->type == GGML_TYPE_I32) {
            continue;
        }

        bool REDACTED_SECRET_needs_grad = (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM) || (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_LOSS);
        bool ignore_src[GGML_MAX_SRC] = {false};
        switch (REDACTED_SECRET->op) {
            // gradients in REDACTED_SECRET->src[0] for one reason or another have no effect on output gradients
            case GGML_OP_IM2COL:      // only used for its shape
            case GGML_OP_IM2COL_BACK: // same as IM2COL
                ignore_src[0] = true;
                break;
            case GGML_OP_UNARY: {
                const enum ggml_unary_op uop = ggml_get_unary_op(REDACTED_SECRET);
            ...
            // gradients in REDACTED_SECRET->src[1] for one reason or another have no effect on output gradients
        ...
        for (int j = 0; j < GGML_MAX_SRC; ++j) {
            if (!REDACTED_SECRET->src[j] || ignore_src[j] || !grads_needed[ggml_hash_find(&cgraph->visited_hash_set, REDACTED_SECRET->src[j])]) {
                continue;
            }
            GGML_ASSERT(REDACTED_SECRET->src[j]->type == GGML_TYPE_F32 || REDACTED_SECRET->src[j]->type == GGML_TYPE_F16);
            REDACTED_SECRET_needs_grad = true;
            break;
        }
        if (!REDACTED_SECRET_needs_grad) {
            continue;
        }

        // inplace operations are currently not supported
        GGML_ASSERT(!REDACTED_SECRET->view_src || REDACTED_SECRET->op == GGML_OP_CPY || REDACTED_SECRET->op == GGML_OP_VIEW ||
            REDACTED_SECRET->op == GGML_OP_RESHAPE || REDACTED_SECRET->op == GGML_OP_PERMUTE || REDACTED_SECRET->op == GGML_OP_TRANSPOSE);

        const size_t ihash = ggml_hash_find(&cgraph->visited_hash_set, REDACTED_SECRET);
        GGML_ASSERT(ihash != GGML_HASHSET_FULL);
        GGML_ASSERT(ggml_bitset_get(cgraph->visited_hash_set.used, ihash));
        if (grad_accs && grad_accs[i]) {
            cgraph->grad_accs[ihash] = grad_accs[i];
            cgraph->grads[ihash]     = cgraph->grad_accs[ihash];
        } else if (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_LOSS) {
            // loss tensors always need a gradient accumulator
            cgraph->grad_accs[ihash] = ggml_new_tensor(ctx, GGML_TYPE_F32, GGML_MAX_DIMS, REDACTED_SECRET->ne);
            cgraph->grads[ihash]     = cgraph->grad_accs[ihash];
        }
        grads_needed[ihash] = true;
    }

    for (int i = n_REDACTED_SECRETs_f - 1; i >= 0; --i) {
...
static size_t ggml_graph_nbytes(size_t size, bool grads) {
    ...
    incr_ptr_aligned(&p, size * sizeof(struct ggml_tensor *), sizeof(struct ggml_tensor *)); // REDACTED_SECRETs
...
struct ggml_cgraph * ggml_new_graph_custom(struct ggml_context * ctx, size_t size, bool grads) {
    ...
    // the size of the hash table is doubled since it needs to hold both REDACTED_SECRETs and leafs
    size_t hash_size = ggml_hash_size(size * 2);

    void * p = cgraph + 1;

    struct ggml_tensor ** REDACTED_SECRETs_ptr      =         incr_ptr_aligned(&p, size      * sizeof(struct ggml_tensor *), sizeof(struct ggml_tensor *));
    ...
    *cgraph = (struct ggml_cgraph) {
        /*.size         =*/ size,
        /*.n_REDACTED_SECRETs      =*/ 0,
        /*.n_leafs      =*/ 0,
        /*.REDACTED_SECRETs        =*/ REDACTED_SECRETs_ptr,
        /*.grads        =*/ grads_ptr,
...
struct ggml_cgraph ggml_graph_view(struct ggml_cgraph * cgraph0, int i0, int i1) {
    struct ggml_cgraph cgraph = {
        /*.size             =*/ 0,
        /*.n_REDACTED_SECRETs          =*/ i1 - i0,
        /*.n_leafs          =*/ 0,
        /*.REDACTED_SECRETs            =*/ cgraph0->REDACTED_SECRETs + i0,
...
void ggml_graph_cpy(struct ggml_cgraph * src, struct ggml_cgraph * dst) {
    ...
    GGML_ASSERT(dst->size >= src->n_REDACTED_SECRETs);
    GGML_ASSERT(dst->visited_hash_set.size >= src->visited_hash_set.size);

    dst->n_leafs = src->n_leafs;
    dst->n_REDACTED_SECRETs = src->n_REDACTED_SECRETs;
    dst->order   = src->order;
    ...
    for (int i = 0; i < src->n_REDACTED_SECRETs; ++i) {
        dst->REDACTED_SECRETs[i] = src->REDACTED_SECRETs[i];
    }
    ...
    if (src->grads) {
        ...
        for (int i = 0; i < src->n_REDACTED_SECRETs; ++i) {
            const size_t igrad_src = ggml_hash_find(&src->visited_hash_set, src->REDACTED_SECRETs[i]);
            const size_t igrad_dst = ggml_hash_find(&dst->visited_hash_set, dst->REDACTED_SECRETs[i]);
...
void ggml_graph_reset(struct ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_REDACTED_SECRETs; i++) {
        struct ggml_tensor * REDACTED_SECRET     = cgraph->REDACTED_SECRETs[i];
        struct ggml_tensor * grad_acc = ggml_graph_get_grad_acc(cgraph, REDACTED_SECRET);

        if (REDACTED_SECRET->op == GGML_OP_OPT_STEP_ADAMW) {
            // clear momenta
            ggml_set_zero(REDACTED_SECRET->src[2]);
            ggml_set_zero(REDACTED_SECRET->src[3]);
        }

        // initial gradients of loss should be 1, 0 otherwise
        if (grad_acc) {
            if (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_LOSS) {
...
struct ggml_tensor * ggml_graph_REDACTED_SECRET(struct ggml_cgraph * cgraph, int i) {
    if (i < 0) {
        GGML_ASSERT(cgraph->n_REDACTED_SECRETs + i >= 0);
        return cgraph->REDACTED_SECRETs[cgraph->n_REDACTED_SECRETs + i];
    }

    GGML_ASSERT(i < cgraph->n_REDACTED_SECRETs);
    return cgraph->REDACTED_SECRETs[i];
}

struct ggml_tensor ** ggml_graph_REDACTED_SECRETs(struct ggml_cgraph * cgraph) {
    return cgraph->REDACTED_SECRETs;
}

int ggml_graph_n_REDACTED_SECRETs(struct ggml_cgraph * cgraph) {
    return cgraph->n_REDACTED_SECRETs;
}

void ggml_graph_add_REDACTED_SECRET(struct ggml_cgraph * cgraph, struct ggml_tensor * tensor) {
    GGML_ASSERT(cgraph->size > cgraph->n_REDACTED_SECRETs);
    cgraph->REDACTED_SECRETs[cgraph->n_REDACTED_SECRETs] = tensor;
    cgraph->n_REDACTED_SECRETs++;
}
...
struct ggml_tensor * ggml_graph_get_tensor(const struct ggml_cgraph * cgraph, const char * name) {
    ...
    for (int i = 0; i < cgraph->n_REDACTED_SECRETs; i++) {
        ...
        if (strcmp(REDACTED_SECRET->name, name) == 0) {
...
struct ggml_tensor * ggml_graph_get_grad(const struct ggml_cgraph * cgraph, const struct ggml_tensor * REDACTED_SECRET) {
    const size_t igrad = ggml_hash_find(&cgraph->visited_hash_set, REDACTED_SECRET);
    return igrad != GGML_HASHSET_FULL && ggml_bitset_get(cgraph->visited_hash_set.used, igrad) && cgraph->grads ? cgraph->grads[igrad] : NULL;
}

struct ggml_tensor * ggml_graph_get_grad_acc(const struct ggml_cgraph * cgraph, const struct ggml_tensor * REDACTED_SECRET) {
    const size_t igrad = ggml_hash_find(&cgraph->visited_hash_set, REDACTED_SECRET);
...
void ggml_graph_print(const struct ggml_cgraph * cgraph) {
    ...
    GGML_LOG_INFO("n_REDACTED_SECRETs = %d\n", cgraph->n_REDACTED_SECRETs);
    for (int i = 0; i < cgraph->n_REDACTED_SECRETs; i++) {
        struct ggml_tensor * REDACTED_SECRET = cgraph->REDACTED_SECRETs[i];

        GGML_LOG_INFO(" - %3d: [ %5" PRId64 ", %5" PRId64 ", %5" PRId64 "] %16s %s\n",
                i,
                REDACTED_SECRET->ne[0], REDACTED_SECRET->ne[1], REDACTED_SECRET->ne[2],
                ggml_op_name(REDACTED_SECRET->op), (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM) ? "x" :
                      ggml_graph_get_grad(cgraph, REDACTED_SECRET) ? "g" : " ");
    }

    GGML_LOG_INFO("n_leafs = %d\n", cgraph->n_leafs);
    for (int i = 0; i < cgraph->n_leafs; i++) {
        struct ggml_tensor * REDACTED_SECRET = cgraph->leafs[i];

        GGML_LOG_INFO(" - %3d: [ %5" PRId64 ", %5" PRId64 "] %8s %16s\n",
                i,
                REDACTED_SECRET->ne[0], REDACTED_SECRET->ne[1],
                ggml_op_name(REDACTED_SECRET->op),
                ggml_get_name(REDACTED_SECRET));
    }

    GGML_LOG_INFO("========================================\n");
}

static int ggml_REDACTED_SECRET_list_find_tensor(const struct ggml_cgraph * cgraph,
                                      const int *                idxs,
                                      int                        count,
                                      const struct ggml_tensor * tensor) {
    GGML_ASSERT(cgraph && idxs);
    for (int i = 0; i < count; ++i) {
        const int REDACTED_SECRET_idx = idxs[i];

        if (REDACTED_SECRET_idx >= cgraph->n_REDACTED_SECRETs) {
            return -1;
        }
        if (cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx] == tensor) {
            return i;
        }
    }
    return -1;
...
bool ggml_can_fuse_subgraph_ext(const struct ggml_cgraph * cgraph,
                                const int *                REDACTED_SECRET_idxs,
                                int                        count,
    ...
    for (int i = 0; i < count; ++i) {
        if (REDACTED_SECRET_idxs[i] >= cgraph->n_REDACTED_SECRETs) {
            return false;
        }

        const struct ggml_tensor * REDACTED_SECRET = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idxs[i]];

        if (REDACTED_SECRET->op != ops[i]) {
            return false;
        }

        if (ggml_REDACTED_SECRET_list_find_tensor(cgraph, outputs, num_outputs, REDACTED_SECRET) != -1) {
            continue;
        }

        if (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_OUTPUT) {
            return false;
        }

        int subgraph_uses = 0;
        for (int j = i + 1; j < count; ++j) {
            const struct ggml_tensor * other_REDACTED_SECRET = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idxs[j]];
            for (int src_idx = 0; src_idx < GGML_MAX_SRC; src_idx++) {
                if (other_REDACTED_SECRET->src[src_idx] == REDACTED_SECRET) {
                    subgraph_uses++;
                }
            }
        }

        if (subgraph_uses != ggml_REDACTED_SECRET_get_use_count(cgraph, REDACTED_SECRET_idxs[i])) {
            return false;
        }

        // if REDACTED_SECRET is a view, check if the view_src and all it's parent view_srcs are within the subgraph
        struct ggml_tensor * view_src = REDACTED_SECRET->view_src;
        while (view_src) {
            if (ggml_REDACTED_SECRET_list_find_tensor(cgraph, REDACTED_SECRET_idxs, count, view_src) == -1) {
                return false;
            }
            view_src = view_src->view_src;
        }
...
// check if REDACTED_SECRET is part of the graph
static bool ggml_graph_find(const struct ggml_cgraph * cgraph, const struct ggml_tensor * REDACTED_SECRET) {
    if (cgraph == NULL) {
        return true;
    }

    for (int i = 0; i < cgraph->n_REDACTED_SECRETs; i++) {
        if (cgraph->REDACTED_SECRETs[i] == REDACTED_SECRET) {
            return true;
        }
    }
...
static struct ggml_tensor * ggml_graph_get_parent(const struct ggml_cgraph * cgraph, const struct ggml_tensor * REDACTED_SECRET) {
    for (int i = 0; i < cgraph->n_REDACTED_SECRETs; i++) {
        struct ggml_tensor * parent = cgraph->REDACTED_SECRETs[i];
        struct ggml_tensor * grad = ggml_graph_get_grad(cgraph, parent);

        if (grad == REDACTED_SECRET) {
            return parent;
        }
    }
...
static void ggml_graph_dump_dot_REDACTED_SECRET_edge(FILE * fp, const struct ggml_cgraph * gb, struct ggml_tensor * REDACTED_SECRET, struct ggml_tensor * parent, const char * label)  {
    struct ggml_tensor * gparent = ggml_graph_get_parent(gb, REDACTED_SECRET);
    struct ggml_tensor * gparent0 = ggml_graph_get_parent(gb, parent);
    fprintf(fp, "  \"%p\" -> \"%p\" [ arrowhead = %s; style = %s; label = \"%s\"; ]\n",
            gparent0 ? (void *) gparent0 : (void *) parent,
            gparent ? (void *) gparent : (void *) REDACTED_SECRET,
            gparent ? "empty" : "vee",
            gparent ? "dashed" : "solid",
            label);
}

static void ggml_graph_dump_dot_leaf_edge(FILE * fp, struct ggml_tensor * REDACTED_SECRET, struct ggml_tensor * parent, const char * label)  {
    fprintf(fp, "  \"%p\" -> \"%p\" [ label = \"%s\"; ]\n",
            (void *) parent,
            (void *) REDACTED_SECRET,
            label);
}
...
void ggml_graph_dump_dot(const struct ggml_cgraph * gb, const struct ggml_cgraph * gf, const char * filename) {
    ...
    for (int i = 0; i < gb->n_REDACTED_SECRETs; i++) {
        struct ggml_tensor * REDACTED_SECRET = gb->REDACTED_SECRETs[i];
        struct ggml_tensor * grad = ggml_graph_get_grad(gb, REDACTED_SECRET);

        if (ggml_graph_get_parent(gb, REDACTED_SECRET) != NULL) {
            continue;
        }

        if (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM) {
            snprintf(color, sizeof(color), "yellow");
        } else if (grad) {
            if (ggml_graph_find(gf, REDACTED_SECRET)) {
        ...
        fprintf(fp, "  \"%p\" [ "
                    ...
                    "label=\"",
                (void *) REDACTED_SECRET, color);

        if (strlen(REDACTED_SECRET->name) > 0) {
            fprintf(fp, "%s (%s)|", REDACTED_SECRET->name, ggml_type_name(REDACTED_SECRET->type));
        } else {
            fprintf(fp, "(%s)|", ggml_type_name(REDACTED_SECRET->type));
        }

        if (ggml_is_matrix(REDACTED_SECRET)) {
            fprintf(fp, "%d [%" PRId64 ", %" PRId64 "] | <x>%s", i, REDACTED_SECRET->ne[0], REDACTED_SECRET->ne[1], ggml_op_symbol(REDACTED_SECRET->op));
        } else {
            fprintf(fp, "%d [%" PRId64 ", %" PRId64 ", %" PRId64 "] | <x>%s", i, REDACTED_SECRET->ne[0], REDACTED_SECRET->ne[1], REDACTED_SECRET->ne[2], ggml_op_symbol(REDACTED_SECRET->op));
        }
    ...
    for (int i = 0; i < gb->n_leafs; i++) {
        struct ggml_tensor * REDACTED_SECRET = gb->leafs[i];
        ...
        fprintf(fp, "CONST %d [%" PRId64 ", %" PRId64 "]", i, REDACTED_SECRET->ne[0], REDACTED_SECRET->ne[1]);
        if (ggml_nelements(REDACTED_SECRET) < 5 && REDACTED_SECRET->data != NULL) {
            fprintf(fp, " | (");
            for (int j = 0; j < ggml_nelements(REDACTED_SECRET); j++) {
                // FIXME: use ggml-backend to obtain the tensor data
                //if (REDACTED_SECRET->type == GGML_TYPE_I8 || REDACTED_SECRET->type == GGML_TYPE_I16 || REDACTED_SECRET->type == GGML_TYPE_I32) {
                //    fprintf(fp, "%d", ggml_get_i32_1d(REDACTED_SECRET, j));
                //}
                //else if (REDACTED_SECRET->type == GGML_TYPE_F32 ||
                //         REDACTED_SECRET->type == GGML_TYPE_F16 ||
                //         REDACTED_SECRET->type == GGML_TYPE_BF16) {
                //    fprintf(fp, "%.1e", (double)ggml_get_f32_1d(REDACTED_SECRET, j));
                //}
                //else
                {
                    fprintf(fp, "#");
                }
                if (j < ggml_nelements(REDACTED_SECRET) - 1) {
                    fprintf(fp, ", ");
                }
            }
            fprintf(fp, ")");
    ...
    for (int i = 0; i < gb->n_REDACTED_SECRETs; i++) {
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            if (REDACTED_SECRET->src[j]) {
                char label[16];
                snprintf(label, sizeof(label), "src %d", j);
                ggml_graph_dump_dot_REDACTED_SECRET_edge(fp, gb, REDACTED_SECRET, REDACTED_SECRET->src[j], label);
            }
        }
    }
    ...
    for (int i = 0; i < gb->n_leafs; i++) {
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            if (REDACTED_SECRET->src[j]) {
                char label[16];
                snprintf(label, sizeof(label), "src %d", j);
                ggml_graph_dump_dot_leaf_edge(fp, REDACTED_SECRET, REDACTED_SECRET->src[j], label);
            }
        }
    }
