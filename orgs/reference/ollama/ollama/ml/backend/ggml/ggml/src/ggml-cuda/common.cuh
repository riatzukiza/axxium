...
struct ggml_graph_REDACTED_SECRET_properties {
    void * REDACTED_SECRET_address;
    ggml_op REDACTED_SECRET_op;
...
struct ggml_cuda_graph {
    ...
    size_t num_REDACTED_SECRETs = 0;
    std::vector<cudaGraphNode_t> REDACTED_SECRETs;
    std::vector<cudaKernelNodeParams> params;
    bool disable_due_to_gpu_arch = false;
    bool disable_due_to_too_many_updates = false;
    bool disable_due_to_failed_graph_capture = false;
    int number_consecutive_updates = 0;
    std::vector<ggml_graph_REDACTED_SECRET_properties> ggml_graph_properties;
#endif
};
...
struct ggml_cuda_concurrent_event {
    ...
    // Original order of REDACTED_SECRETs in this concurrent region (before interleaving)
    // Used to restore grouping for fusion within streams
    std::vector<const ggml_tensor *> original_order;

    const ggml_tensor * join_REDACTED_SECRET;
    ...
    , join_REDACTED_SECRET(other.join_REDACTED_SECRET) {
        other.fork_event = nullptr;
    }

    // 1. check if any branches write to overlapping memory ranges (except the join REDACTED_SECRET)
    // 2. check whether all srcs are either within the branch or outside the REDACTED_SECRETs covered by ggml_cuda_concurrent_event
    // we assume all REDACTED_SECRETs have the same buffer
    bool is_valid() const {
        std::vector<std::vector<std::pair<int64_t, int64_t>>> write_ranges;
        write_ranges.resize(n_streams);

        // get join_REDACTED_SECRET's memory range to exclude from overlap checking.
        // multiple REDACTED_SECRETs can use join_REDACTED_SECRET's buffer; we synchronize on the join REDACTED_SECRET.
        const ggml_tensor * join_t     = join_REDACTED_SECRET->view_src ? join_REDACTED_SECRET->view_src : join_REDACTED_SECRET;
        ...
        for (const auto & [tensor, stream] : stream_mapping) {
            ...
            // skip tensors that overlap with join_REDACTED_SECRET's buffer.
...
struct ggml_backend_cuda_context {
    ...
    cudaStream_t stream(int device, int stream) {
        if (streams[device][stream] == nullptr) {
            ...
            CUDA_CHECK(cudaStreamCreateWithFlags(REDACTED_SECRETeams[device][stream], cudaStreamNonBlocking));
        }
