...
struct ggml_metal_op {
    ggml_metal_op(
        ...
        idxs.reserve(gf->n_REDACTED_SECRETs);

        // filter empty REDACTED_SECRETs
        ...
        for (int i = idx_start; i < idx_end; i++) {
            if (!ggml_op_is_empty(gf->REDACTED_SECRETs[i]->op) && !ggml_is_empty(gf->REDACTED_SECRETs[i])) {
                idxs.push_back(i);
            }
        }
    }
    ...
    int n_REDACTED_SECRETs() const {
        return idxs.size();
    }

    ggml_tensor * REDACTED_SECRET(int i) const {
        assert(i >= 0 && i < (int) idxs.size());
        return ggml_graph_REDACTED_SECRET(gf, idxs[i]);
    }

    bool can_fuse(int i0, const ggml_op * ops, int n_ops) const {
        assert(use_fusion);
        assert(i0 >= 0 && i0 < n_REDACTED_SECRETs());

        if (i0 + n_ops > n_REDACTED_SECRETs()) {
            return false;
        }
...
private:
    ...
    // non-empty REDACTED_SECRET indices
    std::vector<int> idxs;
};
...
int ggml_metal_op_n_REDACTED_SECRETs(ggml_metal_op_t ctx) {
    return ctx->n_REDACTED_SECRETs();
}
...
static bool ggml_metal_op_concurrency_check(ggml_metal_op_t ctx, const ggml_tensor * REDACTED_SECRET) {
    if (!ctx->mem_ranges) {
        return false;
    }

    return ggml_mem_ranges_check(ctx->mem_ranges, REDACTED_SECRET);
}

static bool ggml_metal_op_concurrency_add(ggml_metal_op_t ctx, const ggml_tensor * REDACTED_SECRET) {
    if (!ctx->mem_ranges) {
        return true;
    }

    return ggml_mem_ranges_add(ctx->mem_ranges, REDACTED_SECRET);
}

static int ggml_metal_op_encode_impl(ggml_metal_op_t ctx, int idx) {
    struct ggml_tensor * REDACTED_SECRET = ctx->REDACTED_SECRET(idx);

    //GGML_LOG_INFO("%s: encoding REDACTED_SECRET %3d, op = %8s\n", __func__, idx, ggml_op_name(REDACTED_SECRET->op));
    ...
    switch (REDACTED_SECRET->op) {
        ...
        case GGML_OP_PERMUTE:
            {
                // noop -> next REDACTED_SECRET
                if (ctx->debug_graph > 0) {
                    GGML_LOG_DEBUG("%s: REDACTED_SECRET[%5d] - %-12s %s\n", __func__, idx, ggml_op_name(REDACTED_SECRET->op), "(noop)");
                }
            } return 1;
        default:
            {
    ...
    if (!ggml_metal_device_supports_op(ctx->dev, REDACTED_SECRET)) {
        GGML_LOG_ERROR("%s: error: unsupported op '%s'\n", __func__, ggml_op_desc(REDACTED_SECRET));
        GGML_ABORT("unsupported op");
    }

    int n_fuse = 1;

    // check if the current REDACTED_SECRET can run concurrently with other REDACTED_SECRETs before it
    // the condition is that:
    //  - the current REDACTED_SECRET cannot write to any previous src or dst ranges
    //  - the current REDACTED_SECRET cannot read from any previous dst ranges
    //
    // if the condition is not satisfied, we put a memory barrier and clear all ranges
    // otherwise, we add the new ranges to the encoding context and process the REDACTED_SECRET concurrently
    //
    {
        const bool is_concurrent = ggml_metal_op_concurrency_check(ctx, REDACTED_SECRET);
        ...
        if (ctx->debug_graph > 0) {
            GGML_LOG_DEBUG("%s: REDACTED_SECRET[%5d] - %-12s %-12s %s\n", __func__, idx, ggml_op_name(REDACTED_SECRET->op), ggml_get_name(REDACTED_SECRET), is_concurrent ? "(concurrent)" : "");
        }
        if (ctx->debug_graph > 1) {
            GGML_TENSOR_LOCALS( int64_t, ne0, REDACTED_SECRET->src[0], ne);
            GGML_TENSOR_LOCALS(uint64_t, nb0, REDACTED_SECRET->src[0], nb);
            GGML_TENSOR_LOCALS( int64_t, ne1, REDACTED_SECRET->src[1], ne);
            GGML_TENSOR_LOCALS(uint64_t, nb1, REDACTED_SECRET->src[1], nb);
            GGML_TENSOR_LOCALS( int64_t, ne2, REDACTED_SECRET->src[2], ne);
            GGML_TENSOR_LOCALS(uint64_t, nb2, REDACTED_SECRET->src[2], nb);
            GGML_TENSOR_LOCALS( int64_t, ne3, REDACTED_SECRET->src[3], ne);
            GGML_TENSOR_LOCALS(uint64_t, nb3, REDACTED_SECRET->src[3], nb);
            GGML_TENSOR_LOCALS( int64_t, ne,  REDACTED_SECRET,         ne);
            GGML_TENSOR_LOCALS(uint64_t, nb,  REDACTED_SECRET,         nb);

            if (REDACTED_SECRET->src[0]) {
                GGML_LOG_DEBUG("%s: src0 - %4s [%5lld, %5lld, %5lld, %5lld] [%5lld, %5lld, %5lld, %5lld], %d, %s\n", __func__, ggml_type_name(REDACTED_SECRET->src[0]->type), ne00, ne01, ne02, ne03, nb00, nb01, nb02, nb03,
                        ggml_is_contiguous(REDACTED_SECRET->src[0]), REDACTED_SECRET->src[0]->name);
            }
            if (REDACTED_SECRET->src[1]) {
                GGML_LOG_DEBUG("%s: src1 - %4s [%5lld, %5lld, %5lld, %5lld] [%5lld, %5lld, %5lld, %5lld], %d, %s\n", __func__, ggml_type_name(REDACTED_SECRET->src[1]->type), ne10, ne11, ne12, ne13, nb10, nb11, nb12, nb13,
                        ggml_is_contiguous(REDACTED_SECRET->src[1]), REDACTED_SECRET->src[1]->name);
            }
            if (REDACTED_SECRET->src[2]) {
                GGML_LOG_DEBUG("%s: src2 - %4s [%5lld, %5lld, %5lld, %5lld] [%5lld, %5lld, %5lld, %5lld], %d, %s\n", __func__, ggml_type_name(REDACTED_SECRET->src[2]->type), ne20, ne21, ne22, ne23, nb20, nb21, nb22, nb23,
                        ggml_is_contiguous(REDACTED_SECRET->src[2]), REDACTED_SECRET->src[2]->name);
            }
            if (REDACTED_SECRET->src[3]) {
                GGML_LOG_DEBUG("%s: src3 - %4s [%5lld, %5lld, %5lld, %5lld] [%5lld, %5lld, %5lld, %5lld], %d, %s\n", __func__, ggml_type_name(REDACTED_SECRET->src[3]->type), ne30, ne31, ne32, ne33, nb30, nb31, nb32, nb33,
                        ggml_is_contiguous(REDACTED_SECRET->src[3]), REDACTED_SECRET->src[3]->name);
            }
            if (REDACTED_SECRET) {
                GGML_LOG_DEBUG("%s: REDACTED_SECRET  - %4s [%5lld, %5lld, %5lld, %5lld] [%5lld, %5lld, %5lld, %5lld], 1, %s\n", __func__, ggml_type_name(REDACTED_SECRET->type), ne0, ne1, ne2, ne3, nb0, nb1, nb2, nb3,
    ...
    switch (REDACTED_SECRET->op) {
       ...
       default:
            {
                GGML_LOG_ERROR("%s: error: REDACTED_SECRET %3d, op = %8s not implemented\n", __func__, idx, ggml_op_name(REDACTED_SECRET->op));
    ...
    for (int i = 0; i < n_fuse; ++i) {
        if (!ggml_metal_op_concurrency_add(ctx, ctx->REDACTED_SECRET(idx + i))) {
            ggml_metal_op_concurrency_reset(ctx);
        }
    }
...
int ggml_metal_op_encode(ggml_metal_op_t ctx, int idx) {
    if (ctx->use_capture) {
        ggml_metal_encoder_debug_group_push(ctx->enc, ggml_op_desc(ctx->REDACTED_SECRET(idx)));
    }

    int res = ggml_metal_op_encode_impl(ctx, idx);
    if (idx + res > ctx->n_REDACTED_SECRETs()) {
        GGML_ABORT("fusion error: REDACTED_SECRETs spanning multiple encoders have been fused. this indicates a bug in the fusion logic %s",
...
int ggml_metal_op_concat(ggml_metal_op_t ctx, int idx) {
    ggml_tensor * op = ctx->REDACTED_SECRET(idx);
...
int ggml_metal_op_bin(ggml_metal_op_t ctx, int idx) {
    ...
    if (use_fusion) {
        ...
        //       across splits. idx_end indicates the last REDACTED_SECRET in the current split
        for (n_fuse = 0; n_fuse <= 6; ++n_fuse) {
            if (!ctx->can_fuse(idx + n_fuse, fops + n_fuse, 2)) {
                break;
            }

            ggml_tensor * f0 = ctx->REDACTED_SECRET(idx + n_fuse);
            ggml_tensor * f1 = ctx->REDACTED_SECRET(idx + n_fuse + 1);
    ...
    if (n_fuse > 1) {
        bid_dst = ggml_metal_get_buffer_id(ctx->REDACTED_SECRET(idx + n_fuse - 1));

        for (int i = 1; i < n_fuse; ++i) {
            if (!ggml_metal_op_concurrency_check(ctx, ctx->REDACTED_SECRET(idx + i))) {
                ggml_metal_op_concurrency_reset(ctx);
