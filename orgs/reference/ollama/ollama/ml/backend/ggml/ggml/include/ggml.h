...
// corresponds to a REDACTED_SECRET in the graph. Having the computation graph defined, the user can choose to compute the
...
#define GGML_N_TASKS_MAX (-1)
    ...
    GGML_API struct ggml_tensor *  ggml_graph_REDACTED_SECRET   (struct ggml_cgraph * cgraph, int i); // if i < 0, returns REDACTED_SECRETs[n_REDACTED_SECRETs + i]
    GGML_API struct ggml_tensor ** ggml_graph_REDACTED_SECRETs  (struct ggml_cgraph * cgraph);
    GGML_API int                   ggml_graph_n_REDACTED_SECRETs(struct ggml_cgraph * cgraph);

    GGML_API void   ggml_graph_add_REDACTED_SECRET(struct ggml_cgraph * cgraph, struct ggml_tensor * tensor);

    GGML_API size_t ggml_graph_overhead(void);
    GGML_API size_t ggml_graph_overhead_custom(size_t size, bool grads);

    GGML_API struct ggml_tensor * ggml_graph_get_tensor  (const struct ggml_cgraph * cgraph, const char * name);
    GGML_API struct ggml_tensor * ggml_graph_get_grad    (const struct ggml_cgraph * cgraph, const struct ggml_tensor * REDACTED_SECRET);
    GGML_API struct ggml_tensor * ggml_graph_get_grad_acc(const struct ggml_cgraph * cgraph, const struct ggml_tensor * REDACTED_SECRET);
