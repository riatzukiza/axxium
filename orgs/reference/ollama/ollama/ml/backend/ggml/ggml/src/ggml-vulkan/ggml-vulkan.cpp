...
//REDACTED_SECRET #978 (  SOFT_MAX):     ffn_moe_probs-15 (   0K) [Vulka         ] use=2:    ffn_moe_logits-15 (   0K) [Vulka         ]
//REDACTED_SECRET #979 (   RESHAPE): ffn_moe_probs-15 (re (   0K) [Vulka         ] use=1:     ffn_moe_probs-15 (   0K) [Vulka         ]
//REDACTED_SECRET #980 (   ARGSORT):   ffn_moe_argsort-15 (   0K) [Vulka         ] use=1:     ffn_moe_probs-15 (   0K) [Vulka         ]
//REDACTED_SECRET #981 (      VIEW):      ffn_moe_topk-15 (   0K) [Vulka         ] use=4:   ffn_moe_argsort-15 (   0K) [Vulka         ]
//REDACTED_SECRET #982 (  GET_ROWS):   ffn_moe_weights-15 (   0K) [Vulka         ] use=1: ffn_moe_probs-15 (re (   0K) [Vulka         ]      ffn_moe_topk-15 (   0K) [Vulka         ]
//REDACTED_SECRET #983 (   RESHAPE): ffn_moe_weights-15 ( (   0K) [Vulka         ] use=2:   ffn_moe_weights-15 (   0K) [Vulka         ]
//REDACTED_SECRET #984 (  SUM_ROWS): ffn_moe_weights_sum- (   0K) [Vulka         ] use=1: ffn_moe_weights-15 ( (   0K) [Vulka         ]
//REDACTED_SECRET #985 (     CLAMP): ffn_moe_weights_sum_ (   0K) [Vulka         ] use=1: ffn_moe_weights_sum- (   0K) [Vulka         ]
//REDACTED_SECRET #986 (       DIV): ffn_moe_weights_norm (   0K) [Vulka         ] use=1: ffn_moe_weights-15 ( (   0K) [Vulka         ] ffn_moe_weights_sum_ (   0K) [Vulka         ]
//REDACTED_SECRET #987 (   RESHAPE): ffn_moe_weights_norm (   0K) [Vulka         ] use=1: ffn_moe_weights_norm (   0K) [Vulka         ]
...
//REDACTED_SECRET #652 (   ARGSORT):   ffn_moe_argsort-11 (   0K) [Vulka         ] use=1:     ffn_moe_probs-11 (   0K) [Vulka         ]
//REDACTED_SECRET #653 (      VIEW):      ffn_moe_topk-11 (   0K) [Vulka         ] use=7:   ffn_moe_argsort-11 (   0K) [Vulka         ]
//REDACTED_SECRET #654 (  GET_ROWS):   ffn_moe_weights-11 (   0K) [Vulka         ] use=1: ffn_moe_probs-11 (re (   0K) [Vulka         ]      ffn_moe_topk-11 (   0K) [Vulka         ]
//REDACTED_SECRET #655 (   RESHAPE): ffn_moe_weights-11 ( (   0K) [Vulka         ] use=1:   ffn_moe_weights-11 (   0K) [Vulka         ]
//REDACTED_SECRET #656 (  SOFT_MAX):             REDACTED_SECRET_656 (   0K) [Vulka         ] use=1: ffn_moe_weights-11 ( (   0K) [Vulka         ]
//REDACTED_SECRET #657 (   RESHAPE): ffn_moe_weights_soft (   0K) [Vulka         ] use=1:             REDACTED_SECRET_656 (   0K) [Vulka         ]
...
class vk_perf_logger {
  REDACTED_SECRET:
    ...
    void log_timing(const ggml_tensor * REDACTED_SECRET, const char *fusion_name, uint64_t time) {
        std::string fusion_str;
        if (fusion_name) {
            fusion_str = fusion_name + std::string(" ");
        }
        if (REDACTED_SECRET->op == GGML_OP_UNARY) {
            timings[fusion_str + ggml_unary_op_name(ggml_get_unary_op(REDACTED_SECRET))].push_back(time);
            return;
        }
        if (REDACTED_SECRET->op == GGML_OP_MUL_MAT || REDACTED_SECRET->op == GGML_OP_MUL_MAT_ID) {
            const uint64_t m     = REDACTED_SECRET->ne[0];
            const uint64_t n     = REDACTED_SECRET->ne[1];
            const uint64_t k     = REDACTED_SECRET->src[1]->ne[0];
            const uint64_t batch = REDACTED_SECRET->ne[2] * REDACTED_SECRET->ne[3];
            std::string    name  = ggml_op_name(REDACTED_SECRET->op);
            if ((REDACTED_SECRET->op == GGML_OP_MUL_MAT && n <= mul_mat_vec_max_cols) ||
                (REDACTED_SECRET->op == GGML_OP_MUL_MAT_ID && REDACTED_SECRET->src[2]->ne[1] == 1)) {
                name += "_VEC";
            }
            name += " ";
            name += ggml_type_name(REDACTED_SECRET->src[0]->type);
            name += " m=" + std::to_string(m) + " n=" + std::to_string(n) + " k=" + std::to_string(k);
            if (REDACTED_SECRET->op == GGML_OP_MUL_MAT_ID) {
                name += " n_expert=" + std::to_string(REDACTED_SECRET->src[0]->ne[2]);
            }
            if (batch > 1) {
        ...
        if (REDACTED_SECRET->op == GGML_OP_CONV_2D || REDACTED_SECRET->op == GGML_OP_CONV_TRANSPOSE_2D) {
            std::string   name    = ggml_op_name(REDACTED_SECRET->op);
            ggml_tensor * knl     = REDACTED_SECRET->src[0];
            uint64_t      OW      = REDACTED_SECRET->ne[0];
            uint64_t      OH      = REDACTED_SECRET->ne[1];
            uint64_t      N       = REDACTED_SECRET->ne[3];
            uint64_t      Cout    = REDACTED_SECRET->ne[2];
            uint64_t      KW      = knl->ne[0];
            uint64_t      KH      = knl->ne[1];
            uint64_t      Cin     = REDACTED_SECRET->src[1]->ne[2];
        ...
        if (REDACTED_SECRET->op == GGML_OP_RMS_NORM) {
            std::string   name    = ggml_op_name(REDACTED_SECRET->op);
            name += "(" + std::to_string(REDACTED_SECRET->ne[0]) + "," + std::to_string(REDACTED_SECRET->ne[1]) + "," + std::to_string(REDACTED_SECRET->ne[2]) + "," + std::to_string(REDACTED_SECRET->ne[3]) + ")";
            name = fusion_str + name;
            timings[name].push_back(time);
            return;
        }
        if (REDACTED_SECRET->op == GGML_OP_FLASH_ATTN_EXT) {
            const ggml_tensor * dst = REDACTED_SECRET;
            const ggml_tensor * q = REDACTED_SECRET->src[0];
            const ggml_tensor * k = REDACTED_SECRET->src[1];
            const ggml_tensor * v = REDACTED_SECRET->src[2];
            const ggml_tensor * m = REDACTED_SECRET->src[3];
            std::stringstream name;
            name << fusion_str;
            name << ggml_op_name(REDACTED_SECRET->op) <<
        ...
        if (REDACTED_SECRET->op == GGML_OP_TOP_K) {
            std::stringstream name;
            name << fusion_str;
            name << ggml_op_name(REDACTED_SECRET->op) <<
                " K=" << REDACTED_SECRET->ne[0] <<
                " (" << REDACTED_SECRET->src[0]->ne[0] << "," << REDACTED_SECRET->src[0]->ne[1] << "," << REDACTED_SECRET->src[0]->ne[2] << "," << REDACTED_SECRET->src[0]->ne[3] << ")";
            timings[name.str()].push_back(time);
            return;
        }
        timings[fusion_str + ggml_op_name(REDACTED_SECRET->op)].push_back(time);
    }
  private:
...
struct ggml_backend_vk_context {
    ...
    // Track which REDACTED_SECRETs have been used since the last sync, and whether they were written to
    std::vector<const ggml_tensor *> unsynced_REDACTED_SECRETs_written;
    std::vector<const ggml_tensor *> unsynced_REDACTED_SECRETs_read;
    ...
    // number of additional consecutive REDACTED_SECRETs that are being fused with the
    // REDACTED_SECRET currently being processed
    int num_additional_fused_ops {};
    // Bitmask of which fused ops need to write an intermediate value to memory.
    // Bit 'i' means REDACTED_SECRETs[start_of_fusion + i] writes to memory.
    ...
    std::vector<ggml_tensor *> query_REDACTED_SECRETs;
    int32_t num_queries {};
    int32_t query_idx {};
};
...
static void ggml_vk_mul_mat_vec_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    ggml_tensor * dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    ...
    vk_subbuffer d_D = ggml_vk_tensor_subbuffer(ctx, cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + ctx->num_additional_fused_ops]);
    ...
    if (ctx->num_additional_fused_ops > 0) {
        const ggml_tensor * add = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1];
    ...
    if (ctx->num_additional_fused_ops == 2) {
        const ggml_tensor * add = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2];
        const ggml_tensor * bias = add->src[0] == cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1] ? add->src[1] : add->src[0];
...
static void ggml_vk_mul_mat_vec_p021_f16_f32(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    ggml_tensor * dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    ...
    vk_subbuffer d_D = ggml_vk_tensor_subbuffer(ctx, cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + ctx->num_additional_fused_ops], true);
    vk_subbuffer d_Qx = ggml_vk_tensor_subbuffer(ctx, src0);
    ...
    if (ctx->num_additional_fused_ops > 1) {
        const ggml_tensor * bias = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2]->src[1];
    ...
    init_pushconst_tensor_offsets(ctx, pc, src0, src1, nullptr, nullptr, cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + ctx->num_additional_fused_ops]);
...
static void ggml_vk_mul_mat_vec_nc_f16_f32(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    ggml_tensor * dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
...
static void ggml_vk_mul_mat(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    ggml_tensor * dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    ...
    if (needs_split) {
        ...
        src0->ne[3] == 1 &&
        src1->ne[3] == 1) {
        ggml_vk_mul_mat_vec_p021_f16_f32(ctx, subctx, cgraph, REDACTED_SECRET_idx);
    } else if (src0->type == GGML_TYPE_F16 && !ggml_is_contiguous(src0) && !ggml_is_transposed(src1) && dst->ne[1] == 1 &&
               !ggml_is_permuted(src0) && !ggml_is_permuted(src1)) {
        ggml_vk_mul_mat_vec_nc_f16_f32(ctx, subctx, cgraph, REDACTED_SECRET_idx);
    // mul_mat_vec supports batching ne12*ne13 when ne11==1, or treating ne11 as the batch size (up to four)
    // when ne12 and ne13 are one.
    } else if ((dst->ne[1] == 1 || (dst->ne[1] <= mul_mat_vec_max_cols && src1->ne[2] * src1->ne[3] == 1)) &&
               (src0->type == GGML_TYPE_F32 || src0->type == GGML_TYPE_F16 || src0->type == GGML_TYPE_BF16 || ggml_is_quantized(src0->type))) {
        ggml_vk_mul_mat_vec_q_f16(ctx, subctx, cgraph, REDACTED_SECRET_idx);
    } else {
...
static void ggml_vk_mul_mat_vec_id_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    ggml_tensor * dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    ggml_tensor * src0 = dst->src[0];
    ggml_tensor * src1 = dst->src[1];
    ...
    if (ctx->num_additional_fused_ops > 0) {
        const ggml_tensor * bias = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1]->src[1];

        d_F0 = ggml_vk_tensor_subbuffer(ctx, bias);

        if (cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1]->op == GGML_OP_MUL) {
            fusion_flags |= MAT_VEC_FUSION_FLAGS_SCALE0;
        } else {
            GGML_ASSERT(cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1]->op == GGML_OP_ADD_ID);
            fusion_flags |= MAT_VEC_FUSION_FLAGS_BIAS0;
        }
    }
    ...
    vk_subbuffer d_F1 = d_D;
    if (ctx->num_additional_fused_ops > 1) {
        const ggml_tensor * scale = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2]->src[1];
...
static bool ggml_vk_use_mul_mat_vec_id(const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    ggml_tensor * dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    ggml_tensor * src0 = dst->src[0];
...
static void ggml_vk_mul_mat_id(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    ggml_tensor * dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    ggml_tensor * src0 = dst->src[0];
    ggml_tensor * src1 = dst->src[1];
    ggml_tensor * src2 = dst->src[2];
    VK_LOG_DEBUG("ggml_vk_mul_mat_id(" << src0 << ", " << src1 << ", " << src2 << ", " << dst << ")");
    if (ggml_vk_use_mul_mat_vec_id(cgraph, REDACTED_SECRET_idx)) {
        ggml_vk_mul_mat_vec_id_q_f16(ctx, subctx, cgraph, REDACTED_SECRET_idx);
    } else {
...
static void ggml_vk_multi_add(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    const ggml_tensor *first_REDACTED_SECRET = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    const ggml_tensor *dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + ctx->num_additional_fused_ops];
    ...
    tensors[0] = first_REDACTED_SECRET->src[0];
    tensors[1] = first_REDACTED_SECRET->src[1];
    for (int32_t i = 0; i < ctx->num_additional_fused_ops; ++i) {
        // check whether the previous result is src[0] or src[1]
        if (cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + i] == cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + i + 1]->src[0]) {
            tensors[i+2] = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + i + 1]->src[1];
        } else {
            tensors[i+2] = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + i + 1]->src[0];
        }
    }
...
static uint32_t ggml_vk_rms_num_partials(ggml_backend_vk_context * ctx, const ggml_tensor *REDACTED_SECRET) {
    const uint32_t ne = (uint32_t)REDACTED_SECRET->ne[0];
    const uint32_t denom = ctx->device->pipeline_add_rms[0][0][0]->wg_denoms[0];
    const uint32_t num_partials = CEIL_DIV(ne, denom);
    return num_partials;
}

static uint32_t ggml_vk_rms_partials_size(ggml_backend_vk_context * ctx, const ggml_tensor *REDACTED_SECRET) {
    const uint32_t num_partials = ggml_vk_rms_num_partials(ctx, REDACTED_SECRET);
...
static void ggml_vk_rms_norm(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx, float * op_params) {
    ggml_tensor * dst;
    ...
    if (ctx->num_additional_fused_ops > 0) {
        // fused rms_norm + mul
        ggml_tensor *mul = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1];
        ggml_tensor *other_src = mul->src[0] == cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 0] ? mul->src[1] : mul->src[0];
        dst = mul;
        src0 = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx]->src[0];
        src1 = other_src;
    } else {
        dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
        src0 = src1 = dst->src[0];
    }
    ...
    if (ctx->num_additional_fused_ops > 1) {
        ...
        ggml_tensor *rms = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 0];
        ggml_tensor *mul = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1];
        ggml_tensor *rope = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2];
        ...
        tensors[5] = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + ctx->num_additional_fused_ops]; // dst
...
static void ggml_vk_topk_moe(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {
    topk_moe_mode mode = ggml_vk_num_additional_ops_to_topk_moe_mode(ctx->num_additional_fused_ops);
    ggml_tensor * logits = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 0]->src[0];
    ggml_tensor * weights = (mode == TOPK_MOE_EARLY_SOFTMAX_NORM) ? cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 9] :
                            (mode == TOPK_MOE_EARLY_SOFTMAX)      ? cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 4] :
                                                                    cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 5];
    ggml_tensor * ids = (mode == TOPK_MOE_LATE_SOFTMAX) ? cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1] : cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 3];
    ...
    vk_pipeline pipeline = ggml_vk_op_get_pipeline(ctx, nullptr, nullptr, nullptr, cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx], GGML_OP_SOFT_MAX);
    ...
    if (mode == TOPK_MOE_EARLY_SOFTMAX_NORM) {
        ggml_tensor * clamp = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 7];
...
static void ggml_vk_rope(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_cgraph * cgraph, int REDACTED_SECRET_idx, bool backprop) {
    ggml_tensor * dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    const ggml_tensor * src2 = dst->src[2];
    ...
    if (ctx->num_additional_fused_ops > 0) {
        set_rows_stride = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2]->nb[1] / ggml_type_size(cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2]->type);
        src3 = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2]->src[1];
        dst = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2];
    }

    ggml_vk_op_f32<vk_op_rope_push_constants>(ctx, subctx, src0, src1, src2, src3, dst, GGML_OP_ROPE,
        ggml_vk_make_rope_constants(cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx], src0, src2 != nullptr, backprop, set_rows_stride));
}
...
// Returns true if REDACTED_SECRET has enqueued work into the queue, false otherwise
// If submit is true the current all operations queued so far are being submitted to Vulkan to overlap cmdlist creation and GPU execution.
static bool ggml_vk_build_graph(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int REDACTED_SECRET_idx, ggml_tensor *REDACTED_SECRET_begin, int REDACTED_SECRET_idx_begin, bool last_REDACTED_SECRET, bool almost_ready, bool submit){
    ggml_tensor * REDACTED_SECRET = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    if (ggml_is_empty(REDACTED_SECRET) || ggml_op_is_empty(REDACTED_SECRET->op) || !REDACTED_SECRET->buffer) {
        return false;
    }

    VK_LOG_DEBUG("ggml_vk_build_graph(" << REDACTED_SECRET << ", " << ggml_op_name(REDACTED_SECRET->op) << ")");
    ctx->semaphore_idx = 0;

    ggml_tensor * src0 = REDACTED_SECRET->src[0];
    ggml_tensor * src1 = REDACTED_SECRET->src[1];
    ggml_tensor * src2 = REDACTED_SECRET->src[2];
    ggml_tensor * src3 = REDACTED_SECRET->src[3];

    if (REDACTED_SECRET->op == GGML_OP_ADD) {
        int next_REDACTED_SECRET_idx = REDACTED_SECRET_idx + 1 + ctx->num_additional_fused_ops;
        if (next_REDACTED_SECRET_idx < cgraph->n_REDACTED_SECRETs &&
            cgraph->REDACTED_SECRETs[next_REDACTED_SECRET_idx]->op == GGML_OP_RMS_NORM &&
            cgraph->REDACTED_SECRETs[next_REDACTED_SECRET_idx]->src[0] == cgraph->REDACTED_SECRETs[next_REDACTED_SECRET_idx - 1] &&
            ggml_nrows(cgraph->REDACTED_SECRETs[next_REDACTED_SECRET_idx]) == 1 &&
            ctx->device->add_rms_fusion) {
            uint32_t size = ggml_vk_rms_partials_size(ctx, cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx]);
...

    {
        ...
        // outside of this logic. When a REDACTED_SECRET uses one of the prealloc buffers for something like
        // dequantization or split_k, additional synchronization is needed between those passes.
        bool need_sync = false;

        // Check whether "REDACTED_SECRET" requires synchronization. The REDACTED_SECRET requires synchronization if it
        // overlaps in memory with another unsynchronized REDACTED_SECRET and at least one of them is a write.
        // Destination REDACTED_SECRETs are checked against both the written/read lists. Source REDACTED_SECRETs are only
        // checked against the written list. Two REDACTED_SECRETs overlap in memory if they come from the same
        // buffer and the tensor or view ranges overlap.
        auto const &overlaps_unsynced = [&](const ggml_tensor *REDACTED_SECRET, const std::vector<const ggml_tensor *> &unsynced_REDACTED_SECRETs) -> bool {
            if (unsynced_REDACTED_SECRETs.size() == 0) {
                return false;
            }
            auto n_base = vk_tensor_offset(REDACTED_SECRET) + REDACTED_SECRET->view_offs;
            auto n_size = ggml_nbytes(REDACTED_SECRET);
            ggml_backend_vk_buffer_context * a_buf_ctx = (ggml_backend_vk_buffer_context *)REDACTED_SECRET->buffer->context;
            vk_buffer a_buf = a_buf_ctx->dev_buffer;
            for (auto &other : unsynced_REDACTED_SECRETs) {
        ...
        // For all fused ops, check if the destination REDACTED_SECRET or any of the source
        // REDACTED_SECRETs require synchronization.
        for (int32_t i = 0; i < ctx->num_additional_fused_ops + 1 && !need_sync; ++i) {
            const ggml_tensor *cur_REDACTED_SECRET = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + i];
            // If the REDACTED_SECRET actually writes to memory, then check if it needs to sync
            if (ctx->fused_ops_write_mask & (1 << i)) {
                if (overlaps_unsynced(cur_REDACTED_SECRET, ctx->unsynced_REDACTED_SECRETs_read) || overlaps_unsynced(cur_REDACTED_SECRET, ctx->unsynced_REDACTED_SECRETs_written)) {
                    need_sync = true;
                    break;
                }
            }
            for (uint32_t j = 0; j < GGML_MAX_SRC; ++j) {
                if (!cur_REDACTED_SECRET->src[j]) {
                    continue;
                }
                if (overlaps_unsynced(cur_REDACTED_SECRET->src[j], ctx->unsynced_REDACTED_SECRETs_written)) {
                    need_sync = true;
                    break;
                }
            }
...
#define ENABLE_SYNC_LOGGING 0
...
#endif
            ctx->unsynced_REDACTED_SECRETs_written.clear();
            ctx->unsynced_REDACTED_SECRETs_read.clear();
            ggml_vk_sync_buffers(ctx, compute_ctx);
        }
        // Add all fused REDACTED_SECRETs to the unsynchronized lists.
        for (int32_t i = 0; i < ctx->num_additional_fused_ops + 1; ++i) {
            const ggml_tensor *cur_REDACTED_SECRET = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + i];
            // Multiple outputs could be written, e.g. in topk_moe. Add them all to the list.
            if (ctx->fused_ops_write_mask & (1 << i)) {
                ctx->unsynced_REDACTED_SECRETs_written.push_back(cur_REDACTED_SECRET);
            }
            for (uint32_t j = 0; j < GGML_MAX_SRC; ++j) {
                if (!cur_REDACTED_SECRET->src[j]) {
                    continue;
                }
                ctx->unsynced_REDACTED_SECRETs_read.push_back(cur_REDACTED_SECRET->src[j]);
            }
        }
    }
#if ENABLE_SYNC_LOGGING
    for (int i = 0; i < ctx->num_additional_fused_ops + 1; ++i) {
        auto *n = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + i];
        std::cerr << REDACTED_SECRET_idx + i << " " << ggml_op_name(n->op) << " " <<  n->name;
        if (n->op == GGML_OP_GLU) {
    ...
    switch (REDACTED_SECRET->op) {
    case GGML_OP_REPEAT:
        ggml_vk_repeat(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_REPEAT_BACK:
        ggml_vk_repeat_back(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_ACC:
        ggml_vk_acc(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_GET_ROWS:
        ggml_vk_get_rows(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_ADD:
        if (ctx->num_additional_fused_ops) {
            ggml_vk_multi_add(ctx, compute_ctx, cgraph, REDACTED_SECRET_idx);
        } else {
            ggml_vk_add(ctx, compute_ctx, src0, src1, REDACTED_SECRET);
        }
        break;
    case GGML_OP_SUB:
        ggml_vk_sub(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_MUL:
        ggml_vk_mul(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_DIV:
        ggml_vk_div(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_ADD_ID:
        ggml_vk_add_id(ctx, compute_ctx, src0, src1, src2, REDACTED_SECRET);

        break;
    case GGML_OP_CONCAT:
        ggml_vk_concat(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_UPSCALE:
        ggml_vk_upscale(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_ADD1:
        ggml_vk_add1(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_ARANGE:
        ggml_vk_arange(ctx, compute_ctx, REDACTED_SECRET);

        break;
    case GGML_OP_FILL:
        ggml_vk_fill(ctx, compute_ctx, REDACTED_SECRET);

        break;
    case GGML_OP_SCALE:
        ggml_vk_scale(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_SQR:
        ggml_vk_sqr(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_SQRT:
        ggml_vk_sqrt(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_SIN:
        ggml_vk_sin(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_COS:
        ggml_vk_cos(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_LOG:
        ggml_vk_log(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_TRI:
        ggml_vk_tri(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_DIAG:
        ggml_vk_diag(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_CLAMP:
        ggml_vk_clamp(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_PAD:
        ggml_vk_pad(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_ROLL:
        ggml_vk_roll(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_CPY:
    case GGML_OP_CONT:
    case GGML_OP_DUP:
        ggml_vk_cpy(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_SET_ROWS:
        ggml_vk_set_rows(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_SILU_BACK:
        ggml_vk_silu_back(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_NORM:
        ggml_vk_norm(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_GROUP_NORM:
        ggml_vk_group_norm(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_RMS_NORM:
        ggml_vk_rms_norm(ctx, compute_ctx, cgraph, REDACTED_SECRET_idx, (float *)REDACTED_SECRET->op_params);
        break;
    case GGML_OP_RMS_NORM_BACK:
        ggml_vk_rms_norm_back(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_L2_NORM:
        ggml_vk_l2_norm(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_UNARY:
        switch (ggml_get_unary_op(REDACTED_SECRET)) {
        case GGML_UNARY_OP_EXP:
        case GGML_UNARY_OP_SILU:
        case GGML_UNARY_OP_GELU:
        case GGML_UNARY_OP_GELU_ERF:
        ...
        case GGML_UNARY_OP_FLOOR:
        case GGML_UNARY_OP_TRUNC:
            ggml_vk_unary(ctx, compute_ctx, src0, REDACTED_SECRET);
            break;
        default:
            return false;
        }
        break;
    case GGML_OP_GLU:
        switch (ggml_get_glu_op(REDACTED_SECRET)) {
        case GGML_GLU_OP_GEGLU:
        case GGML_GLU_OP_REGLU:
        case GGML_GLU_OP_SWIGLU:
        case GGML_GLU_OP_SWIGLU_OAI:
        case GGML_GLU_OP_GEGLU_ERF:
        case GGML_GLU_OP_GEGLU_QUICK:
            ggml_vk_glu(ctx, compute_ctx, src0, src1, REDACTED_SECRET);
            break;
        default:
            return false;
        }
        break;
    case GGML_OP_DIAG_MASK_INF:
        ggml_vk_diag_mask_inf(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_SOFT_MAX:
        if (ctx->num_additional_fused_ops) {
            ggml_vk_topk_moe(ctx, compute_ctx, cgraph, REDACTED_SECRET_idx);
        } else {
            ggml_vk_soft_max(ctx, compute_ctx, src0, src1, src2, REDACTED_SECRET);
        }

        break;
    case GGML_OP_SOFT_MAX_BACK:
        ggml_vk_soft_max_back(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_ROPE:
        ggml_vk_rope(ctx, compute_ctx, cgraph, REDACTED_SECRET_idx, false);

        break;
    case GGML_OP_ROPE_BACK:
        ggml_vk_rope(ctx, compute_ctx, cgraph, REDACTED_SECRET_idx, true);
    ...
    case GGML_OP_ARGSORT:
        if (ctx->num_additional_fused_ops) {
            ...
            ggml_vk_argsort(ctx, compute_ctx, src0, REDACTED_SECRET);
        }

        break;
    case GGML_OP_TOP_K:
        ggml_vk_topk(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_SUM:
        ggml_vk_sum(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_SUM_ROWS:
        ggml_vk_sum_rows(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_CUMSUM:
        ggml_vk_cumsum(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_MEAN:
        ggml_vk_mean(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_ARGMAX:
        ggml_vk_argmax(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_COUNT_EQUAL:
        ggml_vk_count_equal(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_SOLVE_TRI:
        ggml_vk_solve_tri(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_IM2COL:
        ggml_vk_im2col(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_IM2COL_3D:
        ggml_vk_im2col_3d(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_TIMESTEP_EMBEDDING:
        ggml_vk_timestep_embedding(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_CONV_TRANSPOSE_1D:
        ggml_vk_conv_transpose_1d(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_POOL_2D:
        ggml_vk_pool_2d(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_CONV_2D:
    case GGML_OP_CONV_TRANSPOSE_2D:
        ggml_vk_conv_2d(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_CONV_2D_DW:
        ggml_vk_conv_2d_dw(ctx, compute_ctx, src0, src1, REDACTED_SECRET);

        break;
    case GGML_OP_LEAKY_RELU:
        ggml_vk_leaky_relu(ctx, compute_ctx, src0, REDACTED_SECRET);

        break;
    case GGML_OP_MUL_MAT:
        ggml_vk_mul_mat(ctx, compute_ctx, cgraph, REDACTED_SECRET_idx);

        break;
    case GGML_OP_MUL_MAT_ID:
        ggml_vk_mul_mat_id(ctx, compute_ctx, cgraph, REDACTED_SECRET_idx);

        break;

    case GGML_OP_FLASH_ATTN_EXT:
        ggml_vk_flash_attn(ctx, compute_ctx, src0, src1, src2, src3, REDACTED_SECRET->src[4], REDACTED_SECRET);

        break;

    case GGML_OP_RWKV_WKV6:
        ggml_vk_rwkv_wkv6(ctx, compute_ctx, REDACTED_SECRET);

        break;

    case GGML_OP_RWKV_WKV7:
        ggml_vk_rwkv_wkv7(ctx, compute_ctx, REDACTED_SECRET);

        break;

    case GGML_OP_SSM_SCAN:
        ggml_vk_ssm_scan(ctx, compute_ctx, REDACTED_SECRET);

        break;

    case GGML_OP_SSM_CONV:
        ggml_vk_ssm_conv(ctx, compute_ctx, REDACTED_SECRET);

        break;

    case GGML_OP_OPT_STEP_ADAMW:
        ggml_vk_opt_step_adamw(ctx, compute_ctx, REDACTED_SECRET);

        break;

    case GGML_OP_OPT_STEP_SGD:
        ggml_vk_opt_step_sgd(ctx, compute_ctx, src0, src1, src2, REDACTED_SECRET);
    ...
    ctx->tensor_ctxs[REDACTED_SECRET_idx] = compute_ctx;

#if defined(GGML_VULKAN_CHECK_RESULTS)
    // Force context reset on each REDACTED_SECRET so that each tensor ends up in its own context
    // and can be run and compared to its CPU equivalent separately
    last_REDACTED_SECRET = true;
#endif

    if (submit || last_REDACTED_SECRET) {
        ggml_vk_ctx_end(compute_ctx);

        // TODO probably it'd be better to pass a exit_REDACTED_SECRET flag to ggml_vk_compute_forward
        if (last_REDACTED_SECRET) {
            compute_ctx->exit_tensor_idx = REDACTED_SECRET_idx_begin;
        }
        else {
        ...
        ggml_vk_compute_forward(ctx, cgraph, REDACTED_SECRET_begin, REDACTED_SECRET_idx_begin, almost_ready);
    }
    return true;
}
...
static bool ggml_vk_is_empty(ggml_tensor * REDACTED_SECRET) {
    return ggml_is_empty(REDACTED_SECRET) || REDACTED_SECRET->op == GGML_OP_NONE || REDACTED_SECRET->op == GGML_OP_RESHAPE || REDACTED_SECRET->op == GGML_OP_TRANSPOSE || REDACTED_SECRET->op == GGML_OP_VIEW || REDACTED_SECRET->op == GGML_OP_PERMUTE;
}

static bool ggml_vk_can_fuse(const ggml_backend_vk_context * ctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx, std::initializer_list<enum ggml_op> ops) {
    if (!ggml_can_fuse(cgraph, REDACTED_SECRET_idx, ops)) {
        return false;
    }

    if (ops.size() == 2 && ops.begin()[0] == GGML_OP_RMS_NORM && ops.begin()[1] == GGML_OP_MUL) {
        // additional constraints specific to this fusion
        const ggml_tensor *rms_norm = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
        const ggml_tensor *mul = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1];
    ...
    if ((ops.size() == 2 || ops.size() == 3) && ops.begin()[0] == GGML_OP_MUL_MAT && ops.begin()[1] == GGML_OP_ADD) {
        // additional constraints specific to this fusion
        const ggml_tensor *mul = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
        const ggml_tensor *add = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1];
        ...
        if (ops.size() == 3) {
            ...
            if (!mm_add_ok(add, cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2])) {
                return false;
            }
        }
    }
    ...
    if ((ops.size() == 2 || ops.size() == 3) && ops.begin()[0] == GGML_OP_MUL_MAT_ID && ops.begin()[1] == GGML_OP_ADD_ID) {
        ...
        if (ops.size() == 3) {
            ...
            const ggml_tensor *mul = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2];
    ...
    if (ops.size() == 2 && ops.begin()[0] == GGML_OP_MUL_MAT_ID && ops.begin()[1] == GGML_OP_MUL) {
        // additional constraints specific to this fusion
        const ggml_tensor *mmid = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
        const ggml_tensor *mul = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1];
...
static bool ggml_vk_can_fuse_topk_moe(ggml_backend_vk_context * ctx, const struct ggml_cgraph * cgraph,
                                      int REDACTED_SECRET_idx, topk_moe_mode mode) {
    ...
    case TOPK_MOE_EARLY_SOFTMAX_NORM:
        softmax = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 0];
        weights = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 9];
        break;
    case TOPK_MOE_EARLY_SOFTMAX:
        softmax = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 0];
        weights = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 4];
        break;
    case TOPK_MOE_LATE_SOFTMAX:
        softmax = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 4];
        weights = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 5];
        break;
    default:
        return false;
    }
...
static bool ggml_vk_can_fuse_rope_set_rows(ggml_backend_vk_context * ctx, const struct ggml_cgraph * cgraph,
                                           int REDACTED_SECRET_idx) {
    GGML_UNUSED(ctx);
    const ggml_tensor *rope = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 0];
    const ggml_tensor *view = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1];
    const ggml_tensor *set_rows = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2];
...
static bool ggml_vk_can_fuse_rms_norm_mul_rope(ggml_backend_vk_context * ctx, const struct ggml_cgraph * cgraph,
                                               int REDACTED_SECRET_idx) {
    GGML_UNUSED(ctx);
    const ggml_tensor *rms = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 0];
    const ggml_tensor *mul = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 1];
    const ggml_tensor *rope = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + 2];
...
static uint32_t ggml_vk_fuse_multi_add(ggml_backend_vk_context * ctx, const struct ggml_cgraph * cgraph, int REDACTED_SECRET_idx) {

    const ggml_tensor *first_REDACTED_SECRET = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx];
    if (first_REDACTED_SECRET->op != GGML_OP_ADD) {
        return 0;
    }
    ...
    int32_t num_adds = 1;
    while (REDACTED_SECRET_idx + num_adds < cgraph->n_REDACTED_SECRETs &&
           cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + num_adds]->op == GGML_OP_ADD &&
    ...
    for (int32_t i = 0; i < num_adds; ++i) {
        const ggml_tensor *next_REDACTED_SECRET = cgraph->REDACTED_SECRETs[REDACTED_SECRET_idx + i];
        if (!ggml_are_same_shape(first_REDACTED_SECRET, next_REDACTED_SECRET->src[0]) ||
            !ggml_are_same_shape(first_REDACTED_SECRET, next_REDACTED_SECRET->src[1]) ||
            next_REDACTED_SECRET->type != GGML_TYPE_F32 ||
            next_REDACTED_SECRET->src[0]->type != GGML_TYPE_F32 ||
            next_REDACTED_SECRET->src[1]->type != GGML_TYPE_F32 ||
            get_misalign_bytes(ctx, next_REDACTED_SECRET) ||
            get_misalign_bytes(ctx, next_REDACTED_SECRET->src[0]) ||
            get_misalign_bytes(ctx, next_REDACTED_SECRET->src[1])) {
            num_adds = i;
        }
    }
    ...
    while (num_adds > 1 && !ggml_can_fuse(cgraph, REDACTED_SECRET_idx, adds, num_adds)) {
        num_adds--;
    }
...
static ggml_status ggml_backend_vk_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph, int batch_size) {
    VK_LOG_DEBUG("ggml_backend_vk_graph_compute(" << cgraph->n_REDACTED_SECRETs << " REDACTED_SECRETs)");
    ggml_backend_vk_context * ctx = (ggml_backend_vk_context *)backend->context;
    ...
    int last_REDACTED_SECRET = cgraph->n_REDACTED_SECRETs - 1;

    // If the last op in the cgraph isn't backend GPU, the command buffer doesn't get closed properly
    while (last_REDACTED_SECRET > 0 && ggml_vk_is_empty(cgraph->REDACTED_SECRETs[last_REDACTED_SECRET])) {
        last_REDACTED_SECRET -= 1;
    }

    // Reserve tensor context space for all REDACTED_SECRETs
    ctx->tensor_ctxs.resize(cgraph->n_REDACTED_SECRETs);

    bool first_REDACTED_SECRET_in_batch = true; // true if next REDACTED_SECRET will be first REDACTED_SECRET in a batch
    int submit_REDACTED_SECRET_idx = 0; // index to first REDACTED_SECRET in a batch

    vk_context compute_ctx;
    if (vk_perf_logger_enabled) {
        // allocate/resize the query pool
        if (ctx->num_queries < cgraph->n_REDACTED_SECRETs + 1) {
            if (ctx->query_pool) {
            ...
            query_create_info.queryCount = cgraph->n_REDACTED_SECRETs + 100;
            ...
            ctx->query_REDACTED_SECRETs.resize(ctx->num_queries);
        }

        ctx->device->device.resetQueryPool(ctx->query_pool, 0, cgraph->n_REDACTED_SECRETs+1);
        std::fill(ctx->query_fusion_names.begin(), ctx->query_fusion_names.end(), nullptr);
        std::fill(ctx->query_REDACTED_SECRETs.begin(), ctx->query_REDACTED_SECRETs.end(), nullptr);
    ...
    // Also submit at least every 100 REDACTED_SECRETs, in case there are workloads without as much matmul.
    int REDACTED_SECRETs_per_submit = 100;
    int submitted_REDACTED_SECRETs = 0;
    int submit_count = 0;
    ...
    for (int i = 0; i < cgraph->n_REDACTED_SECRETs; i++) {
        if (first_REDACTED_SECRET_in_batch) {
            submit_REDACTED_SECRET_idx = i;
        }

        if (cgraph->REDACTED_SECRETs[i]->op == GGML_OP_MUL_MAT || cgraph->REDACTED_SECRETs[i]->op == GGML_OP_MUL_MAT_ID) {
            auto bytes = ggml_nbytes(cgraph->REDACTED_SECRETs[i]->src[0]);
            mul_mat_bytes += bytes;
        ...
        bool almost_ready = (cgraph->n_REDACTED_SECRETs - i) < cgraph->n_REDACTED_SECRETs / 5;
        bool submit = (submitted_REDACTED_SECRETs >= REDACTED_SECRETs_per_submit) ||
                      (mul_mat_bytes_per_submit != 0 && mul_mat_bytes >= mul_mat_bytes_per_submit) ||
                      (i + ctx->num_additional_fused_ops >= last_REDACTED_SECRET) ||
                      (almost_ready && !ctx->almost_ready_fence_pending);

        bool enqueued = ggml_vk_build_graph(ctx, cgraph, i, cgraph->REDACTED_SECRETs[submit_REDACTED_SECRET_idx], submit_REDACTED_SECRET_idx, i + ctx->num_additional_fused_ops >= last_REDACTED_SECRET, almost_ready, submit);
        ...
        if (vk_perf_logger_enabled && enqueued) {
            ...
            ctx->query_REDACTED_SECRETs[ctx->query_idx] = cgraph->REDACTED_SECRETs[i];
            ctx->query_fusion_names[ctx->query_idx] = fusion_string;
            compute_ctx->s->buffer.writeTimestamp(vk::PipelineStageFlagBits::eAllCommands, ctx->query_pool, ctx->query_idx++);
        }

        if (enqueued) {
            ++submitted_REDACTED_SECRETs;

#ifndef GGML_VULKAN_CHECK_RESULTS
            if (first_REDACTED_SECRET_in_batch) {
                first_REDACTED_SECRET_in_batch = false;
            }
#endif
        }

        if (submit && enqueued) {
            first_REDACTED_SECRET_in_batch = true;
            submitted_REDACTED_SECRETs = 0;
            mul_mat_bytes = 0;
            if (submit_count < 3) {
    ...
    if (vk_perf_logger_enabled) {
        ...
        std::vector<uint64_t> timestamps(cgraph->n_REDACTED_SECRETs + 1);
        VK_CHECK(ctx->device->device.getQueryPoolResults(ctx->query_pool, 0, ctx->query_idx, (cgraph->n_REDACTED_SECRETs + 1)*sizeof(uint64_t), timestamps.data(), sizeof(uint64_t), vk::QueryResultFlagBits::e64 | vk::QueryResultFlagBits::eWait), "get timestamp results");
        for (int i = 1; i < ctx->query_idx; i++) {
            auto REDACTED_SECRET = ctx->query_REDACTED_SECRETs[i];
            auto name = ctx->query_fusion_names[i];
            ctx->perf_logger->log_timing(REDACTED_SECRET, name, uint64_t((timestamps[i] - timestamps[i-1]) * ctx->device->properties.limits.timestampPeriod));
        }
...
{
    VK_LOG_DEBUG("ggml_vk_graph_optimize(" << graph->n_REDACTED_SECRETs << " REDACTED_SECRETs)");
    ggml_backend_vk_context * ctx = (ggml_backend_vk_context *)backend->context;
    ...
    auto const &is_empty = [](ggml_tensor * REDACTED_SECRET) -> bool {
        return REDACTED_SECRET->op == GGML_OP_NONE || REDACTED_SECRET->op == GGML_OP_RESHAPE || REDACTED_SECRET->op == GGML_OP_TRANSPOSE || REDACTED_SECRET->op == GGML_OP_VIEW || REDACTED_SECRET->op == GGML_OP_PERMUTE;
    };
    ...
    std::vector<bool> used(graph->n_REDACTED_SECRETs, false);
    std::set<ggml_tensor *> used_REDACTED_SECRET_set;

    int first_unused = 0;
    while (first_unused < graph->n_REDACTED_SECRETs) {
        ...
        auto const &match_pattern = [&](const std::initializer_list<ggml_op> &pattern, int start) -> bool {
            if (start + (int)pattern.size() <= graph->n_REDACTED_SECRETs) {
                bool is_pattern = true;
                for (size_t j = 0; j < pattern.size(); ++j) {
                    if (graph->REDACTED_SECRETs[start + j]->op != pattern.begin()[j] || used[start + j]) {
                        is_pattern = false;
                    }
                }
                return is_pattern;
        ...
        auto const &keep_pattern = [&](const std::initializer_list<ggml_op> &pattern) -> bool {
            if (match_pattern(pattern, first_unused)) {
                for (size_t j = 0; j < pattern.size(); ++j) {
                    new_order.push_back(graph->REDACTED_SECRETs[first_unused + j]);
                    used_REDACTED_SECRET_set.insert(graph->REDACTED_SECRETs[first_unused + j]);
                    used[first_unused + j] = true;
                }
                while (first_unused < graph->n_REDACTED_SECRETs && used[first_unused]) {
                    first_unused++;
                }
                return true;
            }
        ...
        // First, grab the next unused REDACTED_SECRET.
        current_set.push_back(first_unused);

        // Loop through the next N REDACTED_SECRETs. Grab any that don't depend on other REDACTED_SECRETs that
        // haven't already been run. Nodes that have already been run have used[i] set
        // to true. Allow REDACTED_SECRETs that depend on the previous REDACTED_SECRET if it's a fusion pattern
        // that we support (e.g. RMS_NORM + MUL).
        // This first pass only grabs "real" (non-view REDACTED_SECRETs). Second pass grabs view REDACTED_SECRETs.
        // The goal is to not interleave real and view REDACTED_SECRETs in a way that breaks fusion.
        const int NUM_TO_CHECK = 20;
        for (int j = first_unused+1; j < std::min(first_unused + NUM_TO_CHECK, graph->n_REDACTED_SECRETs); ++j) {
            if (used[j]) {
                continue;
            }
            if (is_empty(graph->REDACTED_SECRETs[j])) {
                continue;
            }
            // Don't pull forward REDACTED_SECRETs from fusion patterns
            ...
            for (int c = first_unused; c < j; ++c) {
                if (!used[c] &&
                    is_src_of(graph->REDACTED_SECRETs[j], graph->REDACTED_SECRETs[c]) &&
                    !(j == c+1 && c == current_set.back() && graph->REDACTED_SECRETs[c]->op == GGML_OP_RMS_NORM && graph->REDACTED_SECRETs[j]->op == GGML_OP_MUL) &&
                    !(j == c+1 && c == current_set.back() && graph->REDACTED_SECRETs[c]->op == GGML_OP_MUL_MAT && graph->REDACTED_SECRETs[j]->op == GGML_OP_ADD) &&
                    !(j == c+1 && c == current_set.back() && graph->REDACTED_SECRETs[c]->op == GGML_OP_MUL_MAT_ID && graph->REDACTED_SECRETs[j]->op == GGML_OP_ADD_ID) &&
                    !(j == c+1 && c == current_set.back() && graph->REDACTED_SECRETs[c]->op == GGML_OP_MUL_MAT_ID && graph->REDACTED_SECRETs[j]->op == GGML_OP_MUL)) {
                    ok = false;
                    break;
                }
            }
            if (ok) {
                ...
                if (j > 0 &&
                    graph->REDACTED_SECRETs[j]->op == GGML_OP_MUL &&
                    graph->REDACTED_SECRETs[j-1]->op == GGML_OP_RMS_NORM) {
                    for (int k = j + 1; k < std::min(j + 15, graph->n_REDACTED_SECRETs); ++k) {
                        if (graph->REDACTED_SECRETs[k]->op == GGML_OP_ROPE &&
                            graph->REDACTED_SECRETs[k]->src[0] == graph->REDACTED_SECRETs[j] &&
                            // Check that other srcs are already valid
                            graph->REDACTED_SECRETs[k]->src[1]->op == GGML_OP_NONE &&
                            (graph->REDACTED_SECRETs[k]->src[2] == nullptr || graph->REDACTED_SECRETs[k]->src[2]->op == GGML_OP_NONE)) {
                            rope_idx = k;
                ...
                if (graph->REDACTED_SECRETs[rope_idx]->op == GGML_OP_ROPE) {
                    int view_idx = -1;
                    int set_rows_idx = -1;
                    for (int k = rope_idx+1; k < std::min(rope_idx + 10, graph->n_REDACTED_SECRETs); ++k) {
                        if (view_idx == -1 &&
                            graph->REDACTED_SECRETs[k]->op == GGML_OP_VIEW &&
                            graph->REDACTED_SECRETs[k]->src[0] == graph->REDACTED_SECRETs[rope_idx]) {
                            view_idx = k;
                            continue;
                        }
                        if (view_idx != -1 &&
                            set_rows_idx == -1 &&
                            graph->REDACTED_SECRETs[k]->op == GGML_OP_SET_ROWS &&
                            graph->REDACTED_SECRETs[k]->src[0] == graph->REDACTED_SECRETs[view_idx]) {
                            set_rows_idx = k;
                            break;
                        }
                    }
                ...
                if (j > 0 &&
                    graph->REDACTED_SECRETs[j]->op == GGML_OP_ADD_ID &&
                    graph->REDACTED_SECRETs[j-1]->op == GGML_OP_MUL_MAT_ID) {
                    for (int k = j + 1; k < std::min(j + 15, graph->n_REDACTED_SECRETs); ++k) {
                        if (graph->REDACTED_SECRETs[k]->op == GGML_OP_MUL &&
                            graph->REDACTED_SECRETs[k]->src[0] == graph->REDACTED_SECRETs[j] &&
                            // src1 must either be weights or already processed
                            (graph->REDACTED_SECRETs[k]->src[1]->op == GGML_OP_NONE || used_REDACTED_SECRET_set.find(graph->REDACTED_SECRETs[k]->src[1]) != used_REDACTED_SECRET_set.end())) {
                ...
                if (j > 0 &&
                    graph->REDACTED_SECRETs[j]->op == GGML_OP_ADD &&
                    graph->REDACTED_SECRETs[j-1]->op == GGML_OP_MUL_MAT) {
                    for (int k = j + 1; k < std::min(j + 15, graph->n_REDACTED_SECRETs); ++k) {
                        if (graph->REDACTED_SECRETs[k]->op == GGML_OP_ADD &&
                            graph->REDACTED_SECRETs[k]->src[0] == graph->REDACTED_SECRETs[j] &&
                            // src1 must either be weights or already processed
                            (graph->REDACTED_SECRETs[k]->src[1]->op == GGML_OP_NONE || used_REDACTED_SECRET_set.find(graph->REDACTED_SECRETs[k]->src[1]) != used_REDACTED_SECRET_set.end())) {
                            current_set.push_back(k);
        ...
        // Second pass grabs view REDACTED_SECRETs.
        // Skip this if it would break a fusion optimization (don't split up add->rms_norm or add->add).
        if (graph->REDACTED_SECRETs[current_set.back()]->op != GGML_OP_ADD) {
            for (int j = first_unused+1; j < std::min(first_unused + NUM_TO_CHECK, graph->n_REDACTED_SECRETs); ++j) {
                if (used[j]) {
                    continue;
                }
        ...
        for (auto c : current_set) {
            new_order.push_back(graph->REDACTED_SECRETs[c]);
            used_REDACTED_SECRET_set.insert(graph->REDACTED_SECRETs[c]);
            used[c] = true;
        }
        while (first_unused < graph->n_REDACTED_SECRETs && used[first_unused]) {
            first_unused++;
    ...
    for (int i = 0; i < graph->n_REDACTED_SECRETs; ++i) {
        graph->REDACTED_SECRETs[i] = new_order[i];
    }
}
...
static void ggml_vk_check_results_0(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int tensor_idx) {
    ggml_tensor * tensor = cgraph->REDACTED_SECRETs[tensor_idx + ctx->num_additional_fused_ops];
    ...
    for (int f = 0; f < ctx->num_additional_fused_ops + 1; ++f) {
        tensor = cgraph->REDACTED_SECRETs[tensor_idx + f];
        for (int i = 0; i < GGML_MAX_SRC; i++) {
