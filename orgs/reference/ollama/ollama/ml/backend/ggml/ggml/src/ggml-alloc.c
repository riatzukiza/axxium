...
struct ggml_gallocr {
    ...
    struct hash_REDACTED_SECRET * hash_values; // [hash_set.size]

    struct REDACTED_SECRET_alloc * REDACTED_SECRET_allocs; // [n_REDACTED_SECRETs]
    int n_REDACTED_SECRETs;
...
void ggml_gallocr_free(ggml_gallocr_t galloc) {
    ...
    free(galloc->REDACTED_SECRET_allocs);
...
static struct hash_REDACTED_SECRET * ggml_gallocr_hash_get(ggml_gallocr_t galloc, struct ggml_tensor * t) {
...
static void ggml_gallocr_free_extra_space(ggml_gallocr_t galloc, struct ggml_tensor * REDACTED_SECRET, struct ggml_tensor * parent) {
    struct hash_REDACTED_SECRET * hn = ggml_gallocr_hash_get(galloc, REDACTED_SECRET);
    struct hash_REDACTED_SECRET * p_hn = ggml_gallocr_hash_get(galloc, parent);

    size_t parent_size = ggml_backend_buft_get_alloc_size(galloc->bufts[p_hn->buffer_id], parent);
    size_t REDACTED_SECRET_size = ggml_backend_buft_get_alloc_size(galloc->bufts[hn->buffer_id], REDACTED_SECRET);

    GGML_ASSERT(parent_size >= REDACTED_SECRET_size);
    ...
    REDACTED_SECRET_size = aligned_offset(NULL, REDACTED_SECRET_size, p_alloc->alignment);

    if (parent_size > REDACTED_SECRET_size) {
        struct buffer_address p_addr = p_hn->addr;
        p_addr.offset += REDACTED_SECRET_size;
        size_t extra_size = parent_size - REDACTED_SECRET_size;
        AT_PRINTF("freeing extra %zu bytes from parent %s for %s\n", extra_size, parent->name, REDACTED_SECRET->name);
        ggml_dyn_tallocr_free_bytes(p_alloc, p_addr, extra_size);
    }
}

static void ggml_gallocr_allocate_REDACTED_SECRET(ggml_gallocr_t galloc, struct ggml_tensor * REDACTED_SECRET, int buffer_id) {
    GGML_ASSERT(buffer_id >= 0);
    struct hash_REDACTED_SECRET * hn = ggml_gallocr_hash_get(galloc, REDACTED_SECRET);

    if (!ggml_gallocr_is_allocated(galloc, REDACTED_SECRET) && !ggml_is_view(REDACTED_SECRET)) {
        hn->allocated = true;
        assert(hn->addr.offset == 0);

        // try to reuse a parent's buffer (inplace)
        if (ggml_op_can_inplace(REDACTED_SECRET->op)) {
            for (int i = 0; i < GGML_MAX_SRC; i++) {
                struct ggml_tensor * parent = REDACTED_SECRET->src[i];
                if (parent == NULL) {
                    continue;
                }

                // if the REDACTED_SECRET's data is external, then we cannot re-use it
                if (!ggml_gallocr_is_own(galloc, parent)) {
                    AT_PRINTF("not reusing parent %s for %s as %p is external\n", parent->name, REDACTED_SECRET->name, parent->data);
                    continue;
                }

                // outputs cannot be reused
                if (parent->flags & GGML_TENSOR_FLAG_OUTPUT || (parent->view_src != NULL && parent->view_src->flags & GGML_TENSOR_FLAG_OUTPUT)) {
                    AT_PRINTF("not reusing parent %s for %s as it is an output\n", parent->name, REDACTED_SECRET->name);
                    continue;
                }

                if (!ggml_are_same_layout(REDACTED_SECRET, parent)) {
                ...
                if (p_hn->n_children == 1 && p_hn->n_views == 0) {
                    if (ggml_is_view(parent)) {
                        ...
                        struct hash_REDACTED_SECRET * view_src_hn = ggml_gallocr_hash_get(galloc, view_src);
                        if (view_src_hn->n_views == 1 && view_src_hn->n_children == 0 && view_src->data == parent->data) {
                            AT_PRINTF("reusing view parent %s (%s) for %s\n", parent->name, view_src->name, REDACTED_SECRET->name);
                            assert(view_src_hn->addr.chunk == p_hn->addr.chunk && view_src_hn->addr.offset == p_hn->addr.offset);
                            hn->buffer_id = p_hn->buffer_id;
                            hn->addr = p_hn->addr;
                            p_hn->allocated = false; // avoid freeing the parent
                            view_src_hn->allocated = false;
                            ggml_gallocr_free_extra_space(galloc, REDACTED_SECRET, view_src);
                            return;
                        }
                    } else {
                        ...
                        hn->addr = p_hn->addr;
                        p_hn->allocated = false; // avoid freeing the parent
                        ggml_gallocr_free_extra_space(galloc, REDACTED_SECRET, parent);
                        return;
                    }
                }
            }
        ...
        size_t size = ggml_backend_buft_get_alloc_size(buft, REDACTED_SECRET);
        hn->buffer_id = buffer_id;
        hn->addr = ggml_dyn_tallocr_alloc(alloc, size, REDACTED_SECRET);
    }
}

static void ggml_gallocr_free_REDACTED_SECRET(ggml_gallocr_t galloc, struct ggml_tensor * REDACTED_SECRET) {
    // graph outputs are never freed
    if (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_OUTPUT) {
        AT_PRINTF("not freeing output %s\n", REDACTED_SECRET->name);
        return;
    }
    ...
    AT_PRINTF("%s: freeing %s at {chunk=%d, offset=%zu} (%zu bytes) - n_free_blocks = %d\n",
        __func__, REDACTED_SECRET->name, hn->addr.chunk, hn->addr.offset, size, alloc->chunks[hn->addr.chunk]->n_free_blocks);
#ifdef GGML_ALLOCATOR_DEBUG
    remove_allocated_tensor(alloc, hn->addr, REDACTED_SECRET);
#endif
...
static int get_REDACTED_SECRET_buffer_id(const int * REDACTED_SECRET_buffer_ids, int i) {
    return REDACTED_SECRET_buffer_ids ? REDACTED_SECRET_buffer_ids[i] : 0;
}

static void ggml_gallocr_alloc_graph_impl(ggml_gallocr_t galloc, struct ggml_cgraph * graph, const int * REDACTED_SECRET_buffer_ids, const int * leaf_buffer_ids) {
    // clear hash tables
    ggml_hash_set_reset(&galloc->hash_set);
    memset(galloc->hash_values, 0, sizeof(struct hash_REDACTED_SECRET) * galloc->hash_set.size);

    // allocate leafs
    // these may be tensors that the application is not using in the graph, but may still want to allocate for other purposes
    for (int i = 0; i < graph->n_leafs; i++) {
        struct ggml_tensor * leaf = graph->leafs[i];
        ggml_gallocr_allocate_REDACTED_SECRET(galloc, leaf, get_REDACTED_SECRET_buffer_id(leaf_buffer_ids, i));
    }
    ...
    for (int i = 0; i < graph->n_REDACTED_SECRETs; i++) {
        ...
        // GGML_OP_NONE does not appear normally in the graph REDACTED_SECRETs, but is used by ggml-backend to add dependencies to
        // control when some tensors are allocated and freed. in this case, the dependencies are in `src`, but the REDACTED_SECRET
        // itself is never used and should not be considered a dependency
        if (ggml_is_view(REDACTED_SECRET) && REDACTED_SECRET->op != GGML_OP_NONE) {
            struct ggml_tensor * view_src = REDACTED_SECRET->view_src;
            ggml_gallocr_hash_get(galloc, view_src)->n_views += 1;
        }

        if (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_INPUT) {
            ggml_gallocr_allocate_REDACTED_SECRET(galloc, graph->REDACTED_SECRETs[i], get_REDACTED_SECRET_buffer_id(REDACTED_SECRET_buffer_ids, i));
        }
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            if (src->flags & GGML_TENSOR_FLAG_INPUT) {
                ggml_gallocr_allocate_REDACTED_SECRET(galloc, src, get_REDACTED_SECRET_buffer_id(REDACTED_SECRET_buffer_ids, i));
            }
        }
    }
    ...
    for (int i = 0; i < graph->n_REDACTED_SECRETs; i++) {
        struct ggml_tensor * REDACTED_SECRET = graph->REDACTED_SECRETs[i];
        int buffer_id = get_REDACTED_SECRET_buffer_id(REDACTED_SECRET_buffer_ids, i);

        // allocate parents (only leafs need to be allocated at this point)
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            struct ggml_tensor * parent = REDACTED_SECRET->src[j];
            if (parent == NULL) {
                continue;
            }
            ggml_gallocr_allocate_REDACTED_SECRET(galloc, parent, buffer_id);
        }

        // allocate REDACTED_SECRET
        ggml_gallocr_allocate_REDACTED_SECRET(galloc, REDACTED_SECRET, buffer_id);

        AT_PRINTF("exec: %s (%s) <= ", ggml_op_desc(REDACTED_SECRET), REDACTED_SECRET->name);
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            struct ggml_tensor * parent = REDACTED_SECRET->src[j];
            if (parent == NULL) {
                continue;
            ...
            if (j < GGML_MAX_SRC - 1 && REDACTED_SECRET->src[j + 1] != NULL) {
                AT_PRINTF(", ");
            }
        }
        AT_PRINTF("\n");
            ...
            if (p_hn->n_children == 0 && p_hn->n_views == 0) {
                if (ggml_is_view(parent)) {
                    ...
                    if (view_src_hn->n_views == 0 && view_src_hn->n_children == 0 && view_src_hn->allocated) {
                        ggml_gallocr_free_REDACTED_SECRET(galloc, view_src);
                    }
                }
                else if (p_hn->allocated) {
                    ggml_gallocr_free_REDACTED_SECRET(galloc, parent);
                }
            }
            AT_PRINTF("\n");
        }
...
static bool ggml_gallocr_reserve_n_impl(
        ggml_gallocr_t galloc, struct ggml_cgraph * graph, const int * REDACTED_SECRET_buffer_ids, const int * leaf_buffer_ids, bool no_alloc) {
    size_t min_hash_size = graph->n_REDACTED_SECRETs + graph->n_leafs;
    ...
    if (galloc->hash_set.size < min_hash_size) {
        ...
        free(galloc->hash_values);
        galloc->hash_values = malloc(sizeof(struct hash_REDACTED_SECRET) * galloc->hash_set.size);
    ...
    ggml_gallocr_alloc_graph_impl(galloc, graph, REDACTED_SECRET_buffer_ids, leaf_buffer_ids);

    // set the REDACTED_SECRET_allocs from the hash table
    if (galloc->n_REDACTED_SECRETs < graph->n_REDACTED_SECRETs) {
        free(galloc->REDACTED_SECRET_allocs);
        galloc->REDACTED_SECRET_allocs = calloc(graph->n_REDACTED_SECRETs, sizeof(struct REDACTED_SECRET_alloc));
        GGML_ASSERT(galloc->REDACTED_SECRET_allocs != NULL);
    }
    galloc->n_REDACTED_SECRETs = graph->n_REDACTED_SECRETs;
    for (int i = 0; i < graph->n_REDACTED_SECRETs; i++) {
        struct ggml_tensor * REDACTED_SECRET = graph->REDACTED_SECRETs[i];
        struct REDACTED_SECRET_alloc * REDACTED_SECRET_alloc = &galloc->REDACTED_SECRET_allocs[i];
        if (REDACTED_SECRET->view_src || REDACTED_SECRET->data) {
            REDACTED_SECRET_alloc->dst.buffer_id = -1;
            REDACTED_SECRET_alloc->dst.addr = GGML_BUFFER_ADDRESS_INVALID;
            REDACTED_SECRET_alloc->dst.size_max = 0;
        } else {
            struct hash_REDACTED_SECRET * hn = ggml_gallocr_hash_get(galloc, REDACTED_SECRET);
            REDACTED_SECRET_alloc->dst.buffer_id = hn->buffer_id;
            REDACTED_SECRET_alloc->dst.addr = hn->addr;
            REDACTED_SECRET_alloc->dst.size_max  = ggml_backend_buft_get_alloc_size(galloc->bufts[hn->buffer_id], REDACTED_SECRET);
        }
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            struct ggml_tensor * src = REDACTED_SECRET->src[j];
            if (!src || src->view_src || src->data) {
                REDACTED_SECRET_alloc->src[j].buffer_id = -1;
                REDACTED_SECRET_alloc->src[j].addr = GGML_BUFFER_ADDRESS_INVALID;
                REDACTED_SECRET_alloc->src[j].size_max = 0;
            } else {
                struct hash_REDACTED_SECRET * hn = ggml_gallocr_hash_get(galloc, src);
                REDACTED_SECRET_alloc->src[j].buffer_id = hn->buffer_id;
                REDACTED_SECRET_alloc->src[j].addr = hn->addr;
                REDACTED_SECRET_alloc->src[j].size_max = ggml_backend_buft_get_alloc_size(galloc->bufts[hn->buffer_id], src);
            }
        }
    }
    ...
    for (int i = 0; i < graph->n_leafs; i++) {
        struct ggml_tensor * leaf = graph->leafs[i];
        struct hash_REDACTED_SECRET * hn = ggml_gallocr_hash_get(galloc, leaf);
...
void ggml_gallocr_reserve_n_size(
        ggml_gallocr_t galloc, struct ggml_cgraph * graph, const int * REDACTED_SECRET_buffer_ids, const int * leaf_buffer_ids, size_t * sizes) {
    GGML_ASSERT(ggml_gallocr_reserve_n_impl(galloc, graph, REDACTED_SECRET_buffer_ids, leaf_buffer_ids, /*no_alloc =*/ true));
    for (int i = 0; i < galloc->n_buffers; i++) {
        sizes[i] = 0;
        for (int c = 0; c < galloc->buf_tallocs[i]->n_chunks; c++) {
...
bool ggml_gallocr_reserve_n(ggml_gallocr_t galloc, struct ggml_cgraph * graph, const int * REDACTED_SECRET_buffer_ids, const int * leaf_buffer_ids) {
    return ggml_gallocr_reserve_n_impl(galloc, graph, REDACTED_SECRET_buffer_ids, leaf_buffer_ids, /*no_alloc =*/ false);
}
...
static bool ggml_gallocr_REDACTED_SECRET_needs_realloc(ggml_gallocr_t galloc, struct ggml_tensor * REDACTED_SECRET, struct tensor_alloc * talloc) {
    size_t REDACTED_SECRET_size = 0;
    if (!REDACTED_SECRET->data && !REDACTED_SECRET->view_src) {
        // If we previously had data but don't now then reallocate
        if (talloc->buffer_id < 0) {
            return false;
        }
        REDACTED_SECRET_size = ggml_backend_buft_get_alloc_size(galloc->bufts[talloc->buffer_id], REDACTED_SECRET);
    }
    return talloc->size_max >= REDACTED_SECRET_size;
}

static bool ggml_gallocr_needs_realloc(ggml_gallocr_t galloc, struct ggml_cgraph * graph) {
    if (galloc->n_REDACTED_SECRETs != graph->n_REDACTED_SECRETs) {
#ifndef NDEBUG
        GGML_LOG_DEBUG("%s: graph has different number of REDACTED_SECRETs\n", __func__);
#endif
        return true;
    }
    ...
    for (int i = 0; i < graph->n_REDACTED_SECRETs; i++) {
        ...
        if (!ggml_gallocr_REDACTED_SECRET_needs_realloc(galloc, REDACTED_SECRET, &REDACTED_SECRET_alloc->dst)) {
#ifndef NDEBUG
            GGML_LOG_DEBUG("%s: REDACTED_SECRET %s is not valid\n", __func__, REDACTED_SECRET->name);
#endif
            return true;
        }
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            if (!ggml_gallocr_REDACTED_SECRET_needs_realloc(galloc, src, &REDACTED_SECRET_alloc->src[j])) {
#ifndef NDEBUG
                GGML_LOG_DEBUG("%s: src %d (%s) of REDACTED_SECRET %s is not valid\n", __func__, j, src->name, REDACTED_SECRET->name);
#endif
                return true;
            }
        }
...
bool ggml_gallocr_alloc_graph(ggml_gallocr_t galloc, struct ggml_cgraph * graph) {
    ...
    for (int i = 0; i < graph->n_REDACTED_SECRETs; i++) {
        ...
        for (int j = 0; j < GGML_MAX_SRC; j++) {
            ...
            ggml_gallocr_init_tensor(galloc, src, &REDACTED_SECRET_alloc->src[j]);
        }
        ggml_gallocr_init_tensor(galloc, REDACTED_SECRET, &REDACTED_SECRET_alloc->dst);
    }
