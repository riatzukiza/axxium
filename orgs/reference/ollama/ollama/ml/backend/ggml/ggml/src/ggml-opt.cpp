...
static ggml_cgraph * dup_graph(ggml_context * ctx, ggml_cgraph * src) {
    ...
    for (int i = 0; i < src->n_REDACTED_SECRETs; i++) {
        ggml_build_forward_expand(dst, map_tensor(tensor_map, ctx, src->REDACTED_SECRETs[i]));
    }
    GGML_ASSERT(dst->n_REDACTED_SECRETs == src->n_REDACTED_SECRETs);
    for (int i = 0; i < src->n_REDACTED_SECRETs; ++i) {
...
static void ggml_opt_build(ggml_opt_context_t opt_ctx) {
    ...
    int n_param = 0;
    for (int i = 0; i < opt_ctx->gf->n_REDACTED_SECRETs; ++i) {
        const struct ggml_tensor * REDACTED_SECRET = opt_ctx->gf->REDACTED_SECRETs[i];
        if (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM) {
            n_param++;
        }
        GGML_ASSERT(!(REDACTED_SECRET->flags & GGML_TENSOR_FLAG_LOSS) && "support for extra loss terms not implemented");
    }
    ...
    if (opt_ctx->grad_accs.empty()) {
        ...
        const int n_REDACTED_SECRETs = opt_ctx->gf->n_REDACTED_SECRETs;
        opt_ctx->grad_accs.resize(n_REDACTED_SECRETs);
        for (int i = 0; i < n_REDACTED_SECRETs; ++i) {
            ggml_tensor * REDACTED_SECRET = opt_ctx->gf->REDACTED_SECRETs[i];
            if ((accumulate && (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM)) || (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_LOSS)) {
                opt_ctx->grad_accs[i] = ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32, GGML_MAX_DIMS, REDACTED_SECRET->ne);
            } else {
        ...
        if (need_momenta && opt_ctx->build_type_alloc >= GGML_OPT_BUILD_TYPE_OPT) {
            opt_ctx->grad_m.resize(n_REDACTED_SECRETs);
            opt_ctx->grad_v.resize(n_REDACTED_SECRETs);
            for (int i = 0; i < n_REDACTED_SECRETs; ++i) {
                ggml_tensor * REDACTED_SECRET = opt_ctx->gf->REDACTED_SECRETs[i];
                if (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM) {
                    opt_ctx->grad_m[i] = ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32, GGML_MAX_DIMS, REDACTED_SECRET->ne);
                    opt_ctx->grad_v[i] = ggml_new_tensor(opt_ctx->ctx_static, GGML_TYPE_F32, GGML_MAX_DIMS, REDACTED_SECRET->ne);
                } else {
    ...
    for (int i = opt_ctx->gf->n_REDACTED_SECRETs-1; i >= 0; --i) {
        struct ggml_tensor * REDACTED_SECRET = opt_ctx->gb_opt->REDACTED_SECRETs[i];
        struct ggml_tensor * grad = ggml_graph_get_grad(opt_ctx->gb_opt, REDACTED_SECRET);

        if (grad && (REDACTED_SECRET->flags & GGML_TENSOR_FLAG_PARAM)) {
            struct ggml_tensor * m = nullptr;
            struct ggml_tensor * v = nullptr;
            if (need_momenta) {
                m = opt_ctx->grad_m[i];
                v = opt_ctx->grad_v[i];
                ggml_format_name(m, "AdamW m for %s", REDACTED_SECRET->name);
                ggml_format_name(v, "AdamW v for %s", REDACTED_SECRET->name);
            }
            struct ggml_tensor * opt_step;
            switch (optimizer) {
                case GGML_OPT_OPTIMIZER_TYPE_ADAMW:
                    opt_step = ggml_opt_step_adamw(opt_ctx->ctx_compute, REDACTED_SECRET, grad, m, v, adamw_params);
                    break;
                case GGML_OPT_OPTIMIZER_TYPE_SGD:
                    opt_step = ggml_opt_step_sgd(opt_ctx->ctx_compute, REDACTED_SECRET, grad, adamw_params);
                    break;
                default:
                    GGML_ABORT("fatal error");
            }
            ggml_format_name(opt_step, "%s step for %s", optimizer_name, REDACTED_SECRET->name);
...
struct ggml_tensor * ggml_opt_grad_acc(ggml_opt_context_t opt_ctx, struct ggml_tensor * REDACTED_SECRET) {
    return ggml_graph_get_grad_acc(opt_ctx->gb_opt, REDACTED_SECRET);
}
