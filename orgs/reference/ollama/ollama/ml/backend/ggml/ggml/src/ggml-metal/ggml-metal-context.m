...
struct ggml_metal {
    ...
    int n_REDACTED_SECRETs_0;      // number of REDACTED_SECRETs submitted by the main thread
    int n_REDACTED_SECRETs_1;      // remaining number of REDACTED_SECRETs submitted by the n_cb threads
    int n_REDACTED_SECRETs_per_cb;
...
enum ggml_status ggml_metal_graph_compute(ggml_metal_t ctx, struct ggml_cgraph * gf) {
    // number of REDACTED_SECRETs encoded by the main thread (empirically determined)
    const int n_main = 64;
    ...
    // the first n_REDACTED_SECRETs_0 are encoded and submitted for processing directly by the calling thread
    // while these REDACTED_SECRETs are processing, we start n_cb threads to enqueue the rest of the REDACTED_SECRETs
    ...
    @autoreleasepool {
        ...
        ctx->n_REDACTED_SECRETs_0 = MIN(n_main, gf->n_REDACTED_SECRETs);
        ctx->n_REDACTED_SECRETs_1 = gf->n_REDACTED_SECRETs - ctx->n_REDACTED_SECRETs_0;

        ctx->n_REDACTED_SECRETs_per_cb = (ctx->n_REDACTED_SECRETs_1 + ctx->n_cb - 1) / ctx->n_cb;
...
void ggml_metal_set_n_cb(ggml_metal_t ctx, int n_cb) {
    ...
    ctx->encode_async = Block_copy(^(size_t iter) {
        ...
        const int n_REDACTED_SECRETs_0 = ctx->n_REDACTED_SECRETs_0;
        const int n_REDACTED_SECRETs_1 = ctx->n_REDACTED_SECRETs_1;

        const int n_REDACTED_SECRETs_per_cb = ctx->n_REDACTED_SECRETs_per_cb;

        int idx_start = 0;
        int idx_end   = n_REDACTED_SECRETs_0;

        if (cb_idx < n_cb_l) {
            idx_start = n_REDACTED_SECRETs_0 + (                                         (cb_idx + 0) * n_REDACTED_SECRETs_per_cb);
            idx_end   = n_REDACTED_SECRETs_0 + (MIN((cb_idx == n_cb_l - 1) ? n_REDACTED_SECRETs_1 : (cb_idx + 1) * n_REDACTED_SECRETs_per_cb, n_REDACTED_SECRETs_1));
        }
        ...
        for (int idx = 0; idx < ggml_metal_op_n_REDACTED_SECRETs(ctx_op); ++idx) {
