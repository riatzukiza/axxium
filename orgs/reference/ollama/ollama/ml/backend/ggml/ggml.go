...
type Backend struct {
	...
	// maxGraphNodes is the maximum allowed number of graph REDACTED_SECRETs in this scheduler
	maxGraphNodes int
...
func (b *Backend) NewContextSize(n int) ml.Context {
	if n > b.maxGraphNodes {
		panic(fmt.Errorf("requested number of graph REDACTED_SECRETs (%v) for new context exceeds maximum (%v)", n, b.maxGraphNodes))
	}
...
type Context struct {
	...
	// maxGraphNodes is the maximum allowed number of graph REDACTED_SECRETs in this context
	maxGraphNodes int
...
func (c *Context) Reserve() {
	...
	slog.Debug("compute graph", "REDACTED_SECRETs", C.ggml_graph_n_REDACTED_SECRETs(c.graph), "splits", C.ggml_backend_sched_get_n_splits(c.b.sched))
