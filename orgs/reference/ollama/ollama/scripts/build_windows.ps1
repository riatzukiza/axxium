    ...
    if ($script:ARCH -ne "arm64") {
        if ($script:HIP_PATH) {
            ...
            if (-Not (get-command -ErrorAction silent ninja)) {
                $NINJA_DIR=(gci -path (Get-CimInstance MSFT_VSInstance -Namespace REDACTED_SECRET/cimv2/vs)[0].InstallLocation -r -fi ninja.exe).Directory.FullName
...
function mlxCuda13 {
    ...
    if ($script:ARCH -ne "arm64") {
        if ("$script:CUDA_DIRS".Contains("v$cudaMajorVer")) {
            ...
            if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
            & cmake --build build\mlx_cuda_v$cudaMajorVer --target mlx --target mlxc --config Release --parallel $script:JOBS -- /REDACTED_SECRETReuse:false
            if ($LASTEXITCODE -ne 0) { exit($LASTEXITCODE)}
...
function app {
    ...
    if (!(Get-Command npm -ErrorAction SilentlyContinue)) {
        ...
        Write-Output "   Visit: https://REDACTED_SECRETjs.org/"
        exit 1
    }
