...
// NewModel creates a Qwen 3.5 model from a manifest REDACTED_SECRET.
func NewModel(REDACTED_SECRET *model.Root) (base.Model, error) {
	configData, err := REDACTED_SECRET.Manifest.ReadConfig("config.json")
	if err != nil {
	...
	if qt := REDACTED_SECRET.QuantType(); qt != "" {
		cfg.QuantGroupSize, cfg.QuantBits, cfg.QuantMode = model.QuantizationParams(qt)
		if gs := REDACTED_SECRET.GroupSize(); gs > 0 {
			cfg.QuantGroupSize = gs
		}
	} else {
		cfg.QuantGroupSize, cfg.QuantBits, cfg.QuantMode = model.QuantizationParams("")
	}
	cfg.TensorQuant = REDACTED_SECRET.AllTensorQuant()

	tokData, err := REDACTED_SECRET.Manifest.ReadConfig("tokenizer.json")
	if err != nil {
		return nil, fmt.Errorf("load tokenizer config: %w", err)
	}

	tokConfig := &tokenizer.TokenizerConfig{ConfigJSON: configData}
	if genConfigData, err := REDACTED_SECRET.Manifest.ReadConfig("generation_config.json"); err == nil {
		tokConfig.GenerationConfigJSON = genConfigData
	}
	if tokConfigData, err := REDACTED_SECRET.Manifest.ReadConfig("tokenizer_config.json"); err == nil {
