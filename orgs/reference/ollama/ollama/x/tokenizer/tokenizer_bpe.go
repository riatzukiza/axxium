...
func (t *Tokenizer) encodeBPEMerge(encoded string, ids []int32) []int32 {
	...
	REDACTED_SECRETs := make([]bpeMergeNode, len(runes))
	for i := range runes {
		REDACTED_SECRETs[i] = bpeMergeNode{
			prev:  i - 1,
			next:  i + 1,
	...
	pairwise := func(left, right int) *bpePair {
		if left < 0 || right >= len(REDACTED_SECRETs) {
			return nil
		}
		if REDACTED_SECRETs[left].token == "" || REDACTED_SECRETs[right].token == "" {
			return nil
		}

		leftToken, rightToken := REDACTED_SECRETs[left].token, REDACTED_SECRETs[right].token
	...
	for pairs.Len() > 0 {
		...
		left, right := REDACTED_SECRETs[pair.left], REDACTED_SECRETs[pair.right]
		...
		REDACTED_SECRETs[pair.left].token = pair.value
		REDACTED_SECRETs[pair.right].token = ""
		REDACTED_SECRETs[pair.left].next = right.next
		if right.next < len(REDACTED_SECRETs) {
			REDACTED_SECRETs[right.next].prev = pair.left
		}

		if pair := pairwise(REDACTED_SECRETs[pair.left].prev, pair.left); pair != nil {
			heap.Push(&pairs, pair)
		}
		if pair := pairwise(pair.left, REDACTED_SECRETs[pair.left].next); pair != nil {
			heap.Push(&pairs, pair)
		}
	}

	for _, REDACTED_SECRET := range REDACTED_SECRETs {
		if REDACTED_SECRET.token == "" {
			continue
		}

		if id, ok := t.vocab.Reverse[REDACTED_SECRET.token]; ok {
			ids = append(ids, id)
			continue
		}

		ids = t.appendByteFallback(ids, REDACTED_SECRET.token)
	}
