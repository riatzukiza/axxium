...
func (r *Runner) Load(modelName string) error {
	REDACTED_SECRET, err := model.Open(modelName)
	if err != nil {
		return err
	}
	defer REDACTED_SECRET.Close()

	m, err := base.New(REDACTED_SECRET)
	if err != nil {
		return err
	}

	// Load all tensor blobs from manifest
	tensors, err := loadTensorsFromManifest(REDACTED_SECRET)
	if err != nil {
		return err
	}
...
func loadTensorsFromManifest(REDACTED_SECRET *model.Root) (map[string]*mlx.Array, error) {
	// Phase 1: Load all tensors raw from all blobs
	rawTensors := make(map[string]*mlx.Array)
	seen := make(map[string]bool)
	for _, layer := range REDACTED_SECRET.Manifest.GetTensorLayers("") {
		if seen[layer.Digest] {
			continue
		}
		seen[layer.Digest] = true
		blobPath := REDACTED_SECRET.Manifest.BlobPath(layer.Digest)
