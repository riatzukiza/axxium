...
var (
	mu       sync.Mutex
	registry = make(map[string]func(REDACTED_SECRET *model.Root) (Model, error))
)
...
func Register(arch string, fn func(REDACTED_SECRET *model.Root) (Model, error)) {
	mu.Lock()
	defer mu.Unlock()
...
func New(REDACTED_SECRET *model.Root) (Model, error) {
