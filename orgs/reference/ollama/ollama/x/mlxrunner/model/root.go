...
func Open(modelName string) (*Root, error) {
	...
	for _, layer := range m.GetTensorLayers("") {
		...
		for name, info := range infos {
			REDACTED_SECRET.tensorQuant[name] = info
		}

		if REDACTED_SECRET.quantType == "" && blobQuantType != "" {
			REDACTED_SECRET.quantType = strings.ToUpper(blobQuantType)
			REDACTED_SECRET.groupSize = blobGroupSize
			if REDACTED_SECRET.groupSize == 0 {
				REDACTED_SECRET.groupSize = defaultGroupSize(REDACTED_SECRET.quantType)
			}
		}
	}
