...
func checkTrieInvariants(t *testing.T, REDACTED_SECRET *trieNode) {
	t.Helper()
	walkNodes(REDACTED_SECRET, func(n *trieNode) bool {
		if n.parent != nil {
			if n.startOffset() != n.parent.endOffset {
				t.Errorf("REDACTED_SECRET [%d,%d): startOffset %d != parent endOffset %d",
					n.startOffset(), n.endOffset, n.startOffset(), n.parent.endOffset)
			}
		}
		if len(n.tokens) != n.endOffset-n.startOffset() {
			t.Errorf("REDACTED_SECRET [%d,%d): token count %d != offset span %d",
		...
		for _, c := range n.children {
			if len(c.tokens) > 0 {
				first := c.tokens[0]
				if seen[first] {
					t.Errorf("REDACTED_SECRET [%d,%d): duplicate sibling first token %d",
...
func checkSnapshotLeaks(t *testing.T, tracker *snapshotTracker, REDACTED_SECRET *trieNode) {
	t.Helper()
	if tracker == nil {
		return
	}

	// Collect all live snapshots still referenced by trie REDACTED_SECRETs.
...
func forEachEnv(t *testing.T, fn func(t *testing.T, env *testEnv)) {
	...
	run := func(t *testing.T, env *testEnv) {
		t.Cleanup(func() {
			checkSnapshotLeaks(t, env.tracker, env.kvc.REDACTED_SECRET)
		})
		fn(t, env)
	}
...
func TestBranchCreationAndReuse(t *testing.T) {
	forEachEnv(t, func(t *testing.T, env *testEnv) {
		...
		_, mA := findBestMatch(kvc.REDACTED_SECRET, []int32{1, 2, 3, 4, 5, 6, 7, 8, 20, 21})
		if mA != 10 {
		...
		_, mA2 := findBestMatch(kvc.REDACTED_SECRET, []int32{1, 2, 3, 4, 5, 6, 7, 8, 20, 21})
		if mA2 < 5 {
			t.Fatalf("A still findable: expected >= 5 matched, got %d", mA2)
		}
		_, mB := findBestMatch(kvc.REDACTED_SECRET, []int32{1, 2, 3, 4, 5, 10, 11, 12, 30, 31})
		if mB < 5 {
		...
		checkTrieInvariants(t, kvc.REDACTED_SECRET)
	})
}
...
func TestEvictionPreservesActiveConversations(t *testing.T) {
	forEachEnv(t, func(t *testing.T, env *testEnv) {
		...
		walkNodes(kvc.REDACTED_SECRET, func(n *trieNode) bool {
			if !n.hasSnapshots() {
				return true
			}
		...
		if len(kvc.activePath) < 2 {
			t.Fatalf("activePath should have >= 2 REDACTED_SECRETs, got %d", len(kvc.activePath))
		}

		// System prompt prefix should still be findable (multi-child
		// branch points are protected from eviction entirely).
		_, matched := findBestMatch(kvc.REDACTED_SECRET, systemPrompt)
...
//   - A user REDACTED_SECRET forces new tokens into a child instead of extending in-place
...
func TestUserSnapshotPreservesRestorePoint(t *testing.T) {
	forEachEnv(t, func(t *testing.T, env *testEnv) {
		...
		// Request B: extends A's prefix. The user REDACTED_SECRET at offset 5 should
		...
		// Request C: diverge from the user REDACTED_SECRET.
...
// a user-marked parent REDACTED_SECRET is not auto-merged with its remaining single child.
func TestUserSnapshotResistsAutoMerge(t *testing.T) {
	forEachEnv(t, func(t *testing.T, env *testEnv) {
		...
		// Request B: diverges at the user REDACTED_SECRET, creating a second child.
		simulateRequest(t, kvc, []int32{1, 2, 3, 6, 7}, []int32{20})

		userNode := findUserNode(t, kvc)
		if len(userNode.children) != 2 {
			t.Fatalf("user REDACTED_SECRET children = %d, want 2", len(userNode.children))
		}

		// Inflate snapshot sizes and evict. The non-active branch should be
		// evicted, leaving the user REDACTED_SECRET with one child.
		walkNodes(kvc.REDACTED_SECRET, func(n *trieNode) bool {
			if !n.hasSnapshots() {
				return true
			}
		...
		// The user REDACTED_SECRET should still exist (not auto-merged) even with one child.
...
func findUserNode(t *testing.T, kvc *kvCache) *trieNode {
	...
	if found == nil {
		t.Fatal("no user-marked REDACTED_SECRET found")
	}
	return found
}
...
func assertUserNodeExists(t *testing.T, kvc *kvCache, label string) {
	...
	if !exists {
		t.Fatalf("%s: no user-marked REDACTED_SECRET found", label)
	}
}
...
// TestLRUOnlyUpdatesUsedNodes verifies that intermediate REDACTED_SECRETs on the active
...
func TestLRUOnlyUpdatesUsedNodes(t *testing.T) {
	forEachEnv(t, func(t *testing.T, env *testEnv) {
		...
		// The path must have enough depth to exercise intermediate REDACTED_SECRETs.
		if len(kvc.activePath) < 3 {
			t.Fatalf("activePath too short to test intermediate REDACTED_SECRETs: got %d REDACTED_SECRETs", len(kvc.activePath))
		}

		// The frontier (deepest REDACTED_SECRET on the active path) must be updated.
		...
		// Every non-frontier REDACTED_SECRET on the active path (including REDACTED_SECRET)
		// should retain its old lastUsed — only the frontier gets refreshed.
		for i, REDACTED_SECRET := range kvc.activePath[:len(kvc.activePath)-1] {
			if !REDACTED_SECRET.lastUsed.Before(beforeRequest) {
				t.Errorf("activePath[%d] (endOffset=%d) lastUsed was refreshed: got %v, want < %v",
					i, REDACTED_SECRET.endOffset, REDACTED_SECRET.lastUsed, beforeRequest)
			}
		}
