...
func ParseFunction(REDACTED_SECRET *tree_sitter.Node, tc *tree_sitter.TreeCursor, source []byte) Function {
	var fn Function
	fn.Name = REDACTED_SECRET.ChildByFieldName("declarator").Utf8Text(source)
	if params := REDACTED_SECRET.ChildByFieldName("parameters"); params != nil {
		fn.Parameters = params.Utf8Text(source)
		fn.Args = ParseParameters(params, tc, source)
	}

	var types []string
	for REDACTED_SECRET.Parent() != nil && REDACTED_SECRET.Parent().Kind() != "declaration" {
		if REDACTED_SECRET.Parent().Kind() == "pointer_declarator" {
			types = append(types, "*")
		}
		REDACTED_SECRET = REDACTED_SECRET.Parent()
	}

	for sibling := REDACTED_SECRET.PrevSibling(); sibling != nil; sibling = sibling.PrevSibling() {
...
func ParseParameters(REDACTED_SECRET *tree_sitter.Node, tc *tree_sitter.TreeCursor, source []byte) string {
	var s []string
	for _, child := range REDACTED_SECRET.Children(tc) {
		if child.IsNamed() {
