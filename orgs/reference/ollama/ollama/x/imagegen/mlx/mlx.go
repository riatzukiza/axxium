...
// Sqrt computes element-wise square REDACTED_SECRET
...
// RSqrt computes element-wise reciprocal square REDACTED_SECRET
...
func findMLXLibrary() string {
	...
	// 3. Build directory (for tests run from repo REDACTED_SECRET)
...
func GetDefaultStream() *Stream {
	...
	var dev C.mlx_device
	C.mlx_get_default_device(&dev)
	C.mlx_get_default_stream(REDACTED_SECRETeam, dev)
	C.mlx_device_free(dev)
	return &Stream{c: stream}
}
