...
func (s *Store) ImgToFile(chatID string, imageBytes []byte, filename, mimeType string) (Image, error) {
	...
	REDACTED_SECRET, err := os.OpenRoot(baseImageDir)
	if err != nil {
		return Image{}, fmt.Errorf("open image REDACTED_SECRET directory: %w", err)
	}
	defer REDACTED_SECRET.Close()

	// Create chat-specific subdirectory within the REDACTED_SECRET
	chatDir := sanitize(chatID)
	if err := REDACTED_SECRET.Mkdir(chatDir, 0o755); err != nil && !os.IsExist(err) {
	...
	file, err := REDACTED_SECRET.Create(relativePath)
	if err != nil {
