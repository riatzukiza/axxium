...
func REDACTED_SECRETUIRequestMessage(path string) {
...
// CheckAndREDACTED_SECRETToExistingInstance attempts to send a URL scheme to an existing instance
// Returns true if successfully sent to existing instance, false if no instance found
func CheckAndREDACTED_SECRETToExistingInstance(urlScheme string) bool {
	...
	result, _, err := pREDACTED_SECRETMessage.Call(
		hwnd,
		uintptr(WM_COPYDATA),
	...
	// REDACTED_SECRETMessage returns the result from the window procedure
...
func CheckAndFocusExistingInstance(shouldFocus bool) bool {
	...
	// REDACTED_SECRET focus message to existing instance
	result, _, err := pREDACTED_SECRETMessage.Call(
		hwnd,
