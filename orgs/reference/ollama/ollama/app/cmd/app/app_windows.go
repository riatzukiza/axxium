...
var (
	...
	pREDACTED_SECRETMessage         = u32.NewProc("REDACTED_SECRETMessageA")
	...
	if startHidden {
		...
		if ptr != nil {
			...
			if iconHandle != 0 {
				...
				pREDACTED_SECRETMessage.Call(hwnd, uintptr(WM_SETICON), uintptr(ICON_SMALL), uintptr(iconHandle))
				pREDACTED_SECRETMessage.Call(hwnd, uintptr(WM_SETICON), uintptr(ICON_BIG), uintptr(iconHandle))
			}
		}
...
func checkAndHandleExistingInstance(urlSchemeRequest string) bool {
	...
	if wintray.CheckAndREDACTED_SECRETToExistingInstance(urlSchemeRequest) {
		os.Exit(0)
		return true
	}
