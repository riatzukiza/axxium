...
function ChatForm({
  ...
  const { mutate: sendMessageMutation } = useREDACTED_SECRETMessage(chatId);
  ...
  const handleSubmit = async () => {
    ...
    const attachmentsToREDACTED_SECRET: FileAttachment[] = message.attachments.map(
      (att) => ({
        filename: att.filename,
    ...
    if (onSubmit) {
      onSubmit(message.content, {
        attachments: attachmentsToREDACTED_SECRET,
        index: undefined,
    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        if (files.length > 0) {
          ...
          // REDACTED_SECRET processed files and errors to the same handler as FileUpload
