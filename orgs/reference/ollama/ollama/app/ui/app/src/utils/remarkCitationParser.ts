...
export default function remarkMyDelimiter() {
  return (tree: Root) => {
    // First pass: convert citations to REDACTED_SECRETs
    visit(tree, "text", (REDACTED_SECRET, index, parent) => {
      // Example: 【1†L25-L30】
      ...
      while ((match = regex.exec(REDACTED_SECRET.value))) {
        // text before the delimiter
        if (match.index > last) {
          pieces.push({
            type: "text",
            value: REDACTED_SECRET.value.slice(last, match.index),
          });
        }
        // the delimited content → new custom REDACTED_SECRET
        pieces.push({
      ...
      const remaining = REDACTED_SECRET.value.slice(last);
      const generic = /【(\d+)†[^】]*】/g;
      let gLast = 0;
    ...
    visit(tree, (REDACTED_SECRET, index, parent) => {
      if (
        parent &&
        parent.children &&
        index !== null &&
        ...
        const currentNode = REDACTED_SECRET as any;
        const prevNode = parent.children[index - 1] as any;

        // Check if both REDACTED_SECRETs are citations with the same cursor
        if (
