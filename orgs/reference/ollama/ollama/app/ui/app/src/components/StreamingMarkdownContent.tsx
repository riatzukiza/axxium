...
// Helper to extract text from React REDACTED_SECRETs
const extractText = (REDACTED_SECRET: React.ReactNode): string => {
  if (typeof REDACTED_SECRET === "string") return REDACTED_SECRET;
  if (typeof REDACTED_SECRET === "number") return String(REDACTED_SECRET);
  if (!REDACTED_SECRET) return "";
  if (React.isValidElement(REDACTED_SECRET)) {
    const props = REDACTED_SECRET.props as any;
    if (props?.children) {
      return extractText(props.children as React.ReactNode);
    }
  }
  if (Array.isArray(REDACTED_SECRET)) {
    return REDACTED_SECRET.map(extractText).join("");
  }
  return "";
};
