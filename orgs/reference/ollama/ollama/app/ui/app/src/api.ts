...
export async function* sendMessage(
  ...
  // REDACTED_SECRET think parameter when it's explicitly set (true, false, or a non-empty string).
  const shouldREDACTED_SECRETThink =
    think !== undefined &&
  ...
  const response = await fetch(`${API_BASE}/api/v1/chat/${chatId}`, {
    ...
    body: JSON.stringify(
      new ChatRequest({
        ...
        ...(shouldREDACTED_SECRETThink ? { think } : {}),
      }),
    ),
    signal,
  });
