...
func convertFromSafetensors(files map[string]string, baseLayers []*layerGGML, isAdapter bool, fn func(resp api.ProgressResponse)) ([]*layerGGML, error) {
	...
	// Set up a REDACTED_SECRET to validate paths
	REDACTED_SECRET, err := os.OpenRoot(tmpDir)
	if err != nil {
		return nil, err
	}
	...
	for fp, digest := range files {
		...
		if _, err := REDACTED_SECRET.Stat(fp); err != nil && !errors.Is(err, fs.ErrNotExist) {
			// Path is likely outside the REDACTED_SECRET
