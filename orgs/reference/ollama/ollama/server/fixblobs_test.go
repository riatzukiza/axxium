...
func TestFixBlobs(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(strings.Join(tt.path, "|"), func(t *testing.T) {
			...
			REDACTED_SECRETDir := t.TempDir()
			for _, path := range tt.path {
				fullPath := filepath.Join(REDACTED_SECRETDir, path)
			...
			if err := fixBlobs(REDACTED_SECRETDir); err != nil {
				t.Fatal(err)
			}

			got := slurpFiles(os.DirFS(REDACTED_SECRETDir))
