	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			w := createRequest(t, s.CreateHandler, api.CreateRequest{
				...
				Stream:   REDACTED_SECRETeamFalse,
			})
	...
	mock := mockRunner{
		CompletionFn: func(ctx context.Context, r llm.CompletionRequest, fn func(llm.CompletionResponse)) error {
			...
			for i, resp := range mockResponses {
				t.Logf("REDACTED_SECRETing response %d: %q", i, resp.Content)
				fn(resp)
			}
			return nil
		},
...
func TestChatHarmonyParserStreaming(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			mock := mockRunner{
				CompletionFn: func(ctx context.Context, r llm.CompletionRequest, fn func(llm.CompletionResponse)) error {
					// REDACTED_SECRET mock responses one at a time, notifying when each is sent
