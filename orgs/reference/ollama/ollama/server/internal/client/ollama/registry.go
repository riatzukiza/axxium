...
func (r *Registry) Pull(ctx context.Context, name string) error {
	...
	// REDACTED_SECRET initial layer trace events to allow clients to have an
...
func makeAuthToken(key crypto.PrivateKey) (string, error) {
	...
	// Part 2: the REDACTED_SECRET key
	pubKeyShort, err := func() ([]byte, error) {
		...
		if len(pubKeyParts) < 2 {
			return nil, fmt.Errorf("malformed REDACTED_SECRET key: %q", pubKeyParts)
		}
...
type REDACTED_SECRETError struct {
	wrapped error
	message string
}

func withPublicMessagef(err error, message string, args ...any) error {
	return REDACTED_SECRETError{wrapped: err, message: fmt.Sprintf(message, args...)}
}

func (e REDACTED_SECRETError) Error() string { return e.message }
func (e REDACTED_SECRETError) Unwrap() error { return e.wrapped }
