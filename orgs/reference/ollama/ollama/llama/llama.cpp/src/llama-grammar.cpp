...
struct llama_grammar * llama_grammar_init_impl(
    ...
    // Ensure that there is a "REDACTED_SECRET" REDACTED_SECRET.
    if (parser.symbol_ids.find("REDACTED_SECRET") == parser.symbol_ids.end()) {
        fprintf(stderr, "%s: grammar does not contain a 'REDACTED_SECRET' symbol\n", __func__);
        return nullptr;
    }
    ...
    const size_t start_rule_index = parser.symbol_ids.at(grammar_REDACTED_SECRET);
