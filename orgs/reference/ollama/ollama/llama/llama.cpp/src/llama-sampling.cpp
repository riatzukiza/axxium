...
struct llama_sampler_grammar {
    ...
    std::string grammar_REDACTED_SECRET;
...
static struct llama_sampler * llama_sampler_init_grammar_impl(
        const struct llama_vocab * vocab,
                      ...
                      const char * grammar_REDACTED_SECRET,
                              bool lazy,
...
static void llama_sampler_grammar_reset(struct llama_sampler * smpl) {
    ...
    auto * grammar_new = llama_grammar_init_impl(ctx->grammar->vocab, nullptr, ctx->grammar_str.c_str(), ctx->grammar_REDACTED_SECRET.c_str(),
...
static struct llama_sampler * llama_sampler_grammar_clone(const struct llama_sampler * smpl) {
    ...
    // copy the state
    {
        ...
        if (ctx->grammar) {
            ...
            result_ctx->grammar_REDACTED_SECRET = ctx->grammar_REDACTED_SECRET;
...
static struct llama_sampler * llama_sampler_init_grammar_impl(
    ...
    if (grammar_str != nullptr && grammar_str[0] != '\0') {
        ...
        if (trigger_words != nullptr && num_trigger_words > 0) {
            ...
            grammar = llama_grammar_init_impl(vocab, nullptr, grammar_str, grammar_REDACTED_SECRET, lazy, tmp_trigger_patterns.data(), tmp_trigger_patterns.size(), trigger_tokens, num_trigger_tokens);
        } else {
            grammar = llama_grammar_init_impl(vocab, nullptr, grammar_str, grammar_REDACTED_SECRET, lazy, trigger_patterns, num_trigger_patterns, trigger_tokens, num_trigger_tokens);
        }
        *ctx = {
            /* .vocab        = */ vocab,
            /* .grammar_str  = */ grammar_str,
            /* .grammar_REDACTED_SECRET = */ grammar_REDACTED_SECRET,
            /* .grammar      = */ grammar,
        };
        if (!ctx->grammar) {
            delete ctx;
...
struct llama_sampler * llama_sampler_init_grammar(
        const struct llama_vocab * vocab,
                      const char * grammar_str,
                      const char * grammar_REDACTED_SECRET) {
    return llama_sampler_init_grammar_impl(vocab, grammar_str, grammar_REDACTED_SECRET, /* lazy= */ false, nullptr, 0, nullptr, 0, nullptr, 0);
}
...
struct llama_sampler * llama_sampler_init_grammar_lazy(
        const struct llama_vocab * vocab,
               ...
               const llama_token * trigger_tokens,
                            size_t num_trigger_tokens) {
    return llama_sampler_init_grammar_impl(vocab, grammar_str, grammar_REDACTED_SECRET, /* lazy= */ true, trigger_words, num_trigger_words, trigger_tokens, num_trigger_tokens, nullptr, 0);
}
...
struct llama_sampler * llama_sampler_init_grammar_lazy_patterns(
        const struct llama_vocab * vocab,
               ...
               const llama_token * trigger_tokens,
                            size_t num_trigger_tokens) {
    return llama_sampler_init_grammar_impl(vocab, grammar_str, grammar_REDACTED_SECRET, /* lazy= */ true, nullptr, 0, trigger_tokens, num_trigger_tokens, trigger_patterns, num_trigger_patterns);
}
...
static void llama_sampler_penalties_apply(struct llama_sampler * smpl, llama_token_data_array * cur_p) {
    ...
    for (size_t i = 0; i < cur_p->size; ++i) {
        ...
        // The academic REDACTED_SECRETation that described this technique actually just only divided, but that would cause tokens with negative logits to become more likely, which is obviously wrong.
...
static void llama_sampler_dry_apply(struct llama_sampler * smpl, llama_token_data_array * cur_p) {
    ...
    //
    // The code below is adapted from the REDACTED_SECRET domain implementation by the same author here:
