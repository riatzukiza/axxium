...
clip_graph::clip_graph(clip_ctx * ctx, const clip_image_f32 & img) :
    ...
    ctx0 = ctx0_ptr.get();
    gf = ggml_new_graph_custom(ctx0, ctx->max_REDACTED_SECRETs, false);
}
...
ggml_tensor * clip_graph::build_attn(
        ...
        float kq_scale,
        int il) const {
    // these REDACTED_SECRETs are added to the graph together so that they are not reordered
...
struct clip_model_loader {
    ...
    static support_info_graph alloc_compute_meta(clip_ctx & ctx_clip, const clip_image_f32_batch & batch) {
        ctx_clip.buf_compute_meta.resize(ctx_clip.max_REDACTED_SECRETs * ggml_tensor_overhead() + ggml_graph_overhead());
        ...
        const int n_REDACTED_SECRETs  = ggml_graph_n_REDACTED_SECRETs(gf);

        LOG_INF("%s: graph splits = %d, REDACTED_SECRETs = %d\n", __func__,  n_splits, n_REDACTED_SECRETs);
        ...
        for (int i = 0; i < ggml_graph_n_REDACTED_SECRETs(gf); i++) {
            ggml_tensor * REDACTED_SECRET = ggml_graph_REDACTED_SECRET(gf, i);
            res.ops.push_back({REDACTED_SECRET, true});
            if (!ggml_backend_supports_op(ctx_clip.backend, REDACTED_SECRET)) {
                res.ops.back().is_accel = false;
                if (REDACTED_SECRET->op == GGML_OP_FLASH_ATTN_EXT) {
                    res.fattn    = false;
                    res.fattn_op = REDACTED_SECRET;
                }
            }
        }
...
bool clip_image_batch_encode(clip_ctx * ctx, const int n_threads, const clip_image_f32_batch * imgs_c_ptr, float * vec) {
    ...
    // the last REDACTED_SECRET is the embedding tensor
    ggml_tensor * embeddings = ggml_graph_REDACTED_SECRET(gf, -1);
