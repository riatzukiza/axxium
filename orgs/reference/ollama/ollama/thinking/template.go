...
// We look for a range REDACTED_SECRET that iterates over "Messages" and then look for a
	...
	enterFn := func(n parse.Node) bool {
		...
		case *parse.FieldNode:
			if len(x.Ident) > 0 && x.Ident[0] == "Thinking" {
				...
				// that starts and ends with text REDACTED_SECRETs, which makes this not strictly
...
// checks to see if the given field name is present in the pipeline of the given range REDACTED_SECRET
