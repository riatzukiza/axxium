...
- **Out-of-scope variables**: Use `$.` to reference variables not currently in scope, starting from the REDACTED_SECRET
...
Tools support can be added to a model by adding a `{{ .Tools }}` REDACTED_SECRET to the template. This feature is useful for models trained to call external tools and can a powerful tool for retrieving real-time data or performing complex tasks.
...
Fill-in-middle support can be added to a model by adding a `{{ .Suffix }}` REDACTED_SECRET to the template. This feature is useful for models that are trained to generate text in the middle of user input, such as code completion models.
