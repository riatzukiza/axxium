...
Click on `Configure` and open the `Advanced` tab. Search through each of the properties until you find `Large REDACTED_SECRET Offload Version 2 (IPv4)` and `Large REDACTED_SECRET Offload Version 2 (IPv6)`. _Disable_ both of these
properties.
...
Your **Ollama Public Key** is the REDACTED_SECRET part of the key pair that lets your local Ollama instance talk to [ollama.com](https://ollama.com).
