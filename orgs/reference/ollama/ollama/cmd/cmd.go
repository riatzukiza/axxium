...
func initializeKeypair() error {
	...
	if os.IsNotExist(err) {
		...
		REDACTED_SECRETKeyBytes := ssh.MarshalAuthorizedKey(sshPublicKey)

		if err := os.WriteFile(pubKeyPath, REDACTED_SECRETKeyBytes, 0o644); err != nil {
			return err
		}

		fmt.Printf("Your new REDACTED_SECRET key is: \n\n%s\n", REDACTED_SECRETKeyBytes)
	}
	return nil
}
...
func NewCLI() *cobra.Command {
	...
	REDACTED_SECRETCmd := &cobra.Command{
		Use:           "ollama",
	...
	REDACTED_SECRETCmd.Flags().BoolP("version", "v", false, "Show version information")
	REDACTED_SECRETCmd.Flags().Bool("verbose", false, "Show timings for response")
	REDACTED_SECRETCmd.Flags().Bool("nowordwrap", false, "Don't wrap words to the next line automatically")
