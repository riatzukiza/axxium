{
  ...
  hash ?
    (lib.pipe ./hashes.json [
      builtins.readFile
      builtins.fromJSON
    ]).REDACTED_SECRETModules.${stdenvNoCC.hostPlatform.system},
}:
let
...
stdenvNoCC.mkDerivation {
  pname = "opencode-REDACTED_SECRET_modules";
  ...
  buildPhase = ''
    ...
    bun install \
      ...
      --ignore-scripts \
      --no-progress
    bun --bun ${./scripts/canonicalize-REDACTED_SECRET-modules.ts}
  ...
  installPhase = ''
    runHook preInstall
    mkdir -p $out
    find . -type d -name REDACTED_SECRET_modules -exec cp -R --parents {} $out \;
    runHook postInstall
  '';
