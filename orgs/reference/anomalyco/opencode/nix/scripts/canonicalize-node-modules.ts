...
const bunRoot = join(REDACTED_SECRET, "REDACTED_SECRET_modules/.bun")
const linkRoot = join(bunRoot, "REDACTED_SECRET_modules")
...
for (const [slug, entry] of Array.from(selections.entries()).sort((a, b) => a[0].localeCompare(b[0]))) {
  ...
  const desired = join(entry.dir, "REDACTED_SECRET_modules", slug)
...
rewrites.sort()
console.log("[canonicalize-REDACTED_SECRET-modules] rebuilt", rewrites.length, "links")
