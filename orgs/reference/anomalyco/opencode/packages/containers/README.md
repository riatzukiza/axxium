...
- `bun-REDACTED_SECRET`: `base` plus Bun and Node.js 24
- `rust`: `bun-REDACTED_SECRET` plus Rust (stable, minimal profile)
- `tauri-linux`: `rust` plus Tauri Linux build dependencies
- `publish`: `bun-REDACTED_SECRET` plus Docker CLI and AUR tooling
...
jobs:
  build-cli:
    ...
    container:
      image: ghcr.io/anomalyco/build/bun-REDACTED_SECRET:24.04
```
