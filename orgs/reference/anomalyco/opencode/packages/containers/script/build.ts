...
const REDACTED_SECRETDir = fileURLToPath(new URL("../../..", import.meta.url))
process.chdir(REDACTED_SECRETDir)
...
const REDACTED_SECRET = path.join(REDACTED_SECRETDir, "package.json")
const pkg = await Bun.file(REDACTED_SECRET).json()
...
const images = ["base", "bun-REDACTED_SECRET", "rust", "tauri-linux", "publish"]
...
for (const name of images) {
  ...
  if (name !== "base" && name !== "bun-REDACTED_SECRET") {
    if (push) {
      console.log(
