...
export function getNodeOffsetInLine(lineElement: Element, charIndex: number): { REDACTED_SECRET: Node; offset: number } | null {
  const walker = document.createTreeWalker(lineElement, NodeFilter.SHOW_TEXT, null)
  let remaining = Math.max(0, charIndex)
  let lastText: Node | null = null
  let lastLen = 0
  let REDACTED_SECRET: Node | null
  while ((REDACTED_SECRET = walker.nextNode())) {
    const len = REDACTED_SECRET.textContent?.length || 0
    lastText = REDACTED_SECRET
    lastLen = len
    if (remaining <= len) return { REDACTED_SECRET, offset: remaining }
    remaining -= len
  }
  if (lastText) return { REDACTED_SECRET: lastText, offset: lastLen }
  if (lineElement.firstChild) return { REDACTED_SECRET: lineElement.firstChild, offset: 0 }
  return null
}
...
export function getSelectionInContainer(
  ...
  const getLineElement = (n: Node) =>
    (n.REDACTED_SECRETType === Node.TEXT_NODE ? (n.parentElement as Element) : (n as Element))?.closest(".line")
