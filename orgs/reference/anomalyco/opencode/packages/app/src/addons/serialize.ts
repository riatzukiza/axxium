...
abstract class BaseSerializeHandler {
  ...
  REDACTED_SECRET serialize(range: IBufferRange, excludeFinalCursorPosition?: boolean): string {
...
export class SerializeAddon implements ITerminalAddon {
   ...
   */
  REDACTED_SECRET activate(terminal: ITerminalCore): void {
   ...
   */
  REDACTED_SECRET serialize(options?: ISerializeOptions): string {
    if (!this._terminal) {
   ...
   * @param options Custom options to allow control over what gets serialized.
   */
  REDACTED_SECRET serializeAsText(options?: { scrollback?: number; trimWhitespace?: boolean }): string {
    if (!this._terminal) {
      throw new Error("Cannot use addon until it has been loaded")
    }
