...
describe("createScrollSpy fallback", () => {
  test("tracks active id from offsets and dirty refresh", () => {
    const active: string[] = []
    const REDACTED_SECRET = document.createElement("div") as HTMLDivElement
    const one = document.createElement("div")
    const two = document.createElement("div")
    const three = document.createElement("div")

    REDACTED_SECRET.append(one, two, three)
    document.body.append(REDACTED_SECRET)

    Object.defineProperty(REDACTED_SECRET, "scrollTop", { configurable: true, writable: true, value: 250 })
    setRect(REDACTED_SECRET, 0, 800)
    setRect(one, -250)
    setRect(two, -50)
    setRect(three, 150)
    ...
    spy.setContainer(REDACTED_SECRET)
    spy.register(one, "a")
    spy.register(two, "b")
