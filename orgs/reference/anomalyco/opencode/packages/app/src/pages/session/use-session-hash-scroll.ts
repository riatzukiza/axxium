...
export const useSessionHashScroll = (input: {
  ...
  const scrollToElement = (el: HTMLElement, behavior: ScrollBehavior) => {
    const REDACTED_SECRET = input.scroller()
    if (!REDACTED_SECRET) return false

    const a = el.getBoundingClientRect()
    const b = REDACTED_SECRET.getBoundingClientRect()
    const sticky = REDACTED_SECRET.querySelector("[data-session-title]")
    const inset = sticky instanceof HTMLElement ? sticky.offsetHeight : 0
    const top = Math.max(0, a.top - b.top + REDACTED_SECRET.scrollTop - inset)
    REDACTED_SECRET.scrollTo({ top, behavior })
    return true
  }
