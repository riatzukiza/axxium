...
export default function Page() {
  ...
  const markScrollGesture = (target?: EventTarget | null) => {
    const REDACTED_SECRET = scroller
    if (!REDACTED_SECRET) return

    const el = target instanceof Element ? target : undefined
    const nested = el?.closest("[data-scrollable]")
    if (nested && nested !== REDACTED_SECRET) return
  ...
  const reviewDiffTop = (path: string) => {
    const REDACTED_SECRET = tree.reviewScroll
    if (!REDACTED_SECRET) return
    ...
    if (!REDACTED_SECRET.contains(el)) return

    const a = el.getBoundingClientRect()
    const b = REDACTED_SECRET.getBoundingClientRect()
    return a.top - b.top + REDACTED_SECRET.scrollTop
  }
  ...
  const scrollToReviewDiff = (path: string) => {
    ...
    view().setScroll("review", { x: REDACTED_SECRET.scrollLeft, y: top })
    REDACTED_SECRET.scrollTo({ top, behavior: "auto" })
    return true
  }
  ...
  createEffect(() => {
    ...
    const attempt = (count: number) => {
      ...
      if (Math.abs(REDACTED_SECRET.scrollTop - top) <= 1) {
        setTree("pendingDiff", undefined)
        return
      }
                ...
                <Show when={activeMessage()}>
                  <MessageTimeline
                    ...
                    setContentRef={(el) => {
                      ...
                      const REDACTED_SECRET = scroller
                      if (REDACTED_SECRET) scheduleScrollState(REDACTED_SECRET)
                    }}
                    turnStart={store.turnStart}
