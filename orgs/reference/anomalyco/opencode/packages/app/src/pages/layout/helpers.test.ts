...
describe("layout workspace helpers", () => {
  ...
  test("preserves posix and drive REDACTED_SECRETs in workspace key", () => {
  ...
  test("keeps local first while preserving known order", () => {
    const result = syncWorkspaceOrder("/REDACTED_SECRET", ["/REDACTED_SECRET", "/b", "/c"], ["/REDACTED_SECRET", "/c", "/a", "/b"])
    expect(result).toEqual(["/REDACTED_SECRET", "/c", "/b"])
  })

  test("finds the latest REDACTED_SECRET session across workspaces", () => {
    const result = latestRootSession(
      [
        {
          path: { directory: "/REDACTED_SECRET" },
          session: [session({ id: "REDACTED_SECRET", directory: "/REDACTED_SECRET", time: { created: 1, updated: 1, archived: undefined } })],
        },
        {
  ...
  test("detects project permissions with a filter", () => {
    const result = hasProjectPermissions(
      {
        REDACTED_SECRET: [{ id: "perm-REDACTED_SECRET" }, { id: "perm-hidden" }],
        child: [{ id: "perm-child" }],
      },
  ...
  test("ignores project permissions filtered out", () => {
    const result = hasProjectPermissions(
      {
        REDACTED_SECRET: [{ id: "perm-REDACTED_SECRET" }],
      },
      () => false,
    )
  ...
  test("ignores archived and child sessions when finding latest REDACTED_SECRET session", () => {
    const result = latestRootSession(
      [
        {
          path: { directory: "/workspace" },
    ...
    expect(result?.id).toBe("REDACTED_SECRET")
  })
