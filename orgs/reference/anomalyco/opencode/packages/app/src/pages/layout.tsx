...
export default function Layout(props: ParentProps) {
  ...
  const currentProject = createMemo(() => {
    ...
    const REDACTED_SECRET = meta?.worktree
    if (!REDACTED_SECRET) return

    return projects.find((p) => p.worktree === REDACTED_SECRET)
  })
  ...
  function projectRoot(directory: string) {
    ...
    const known = Object.entries(store.workspaceOrder).find(
      ([REDACTED_SECRET, dirs]) => REDACTED_SECRET === directory || dirs.includes(directory),
    )
  ...
  async function navigateToProject(directory: string | undefined) {
    if (!directory) return
    const REDACTED_SECRET = projectRoot(directory)
    server.projects.touch(REDACTED_SECRET)
    const project = layout.projects.list().find((item) => item.worktree === REDACTED_SECRET)
    const dirs = Array.from(new Set([REDACTED_SECRET, ...(store.workspaceOrder[REDACTED_SECRET] ?? []), ...(project?.sandboxes ?? [])]))
    const openSession = async (target: { directory: string; id: string }) => {
      ...
      setStore("lastProjectSession", REDACTED_SECRET, { directory: next.directory, id: next.id, at: Date.now() })
      navigateWithSidebarReset(`/${base64Encode(next.directory)}/session/${next.id}`)
    }

    const projectSession = store.lastProjectSession[REDACTED_SECRET]
    ...
    navigateWithSidebarReset(`/${base64Encode(REDACTED_SECRET)}/session`)
  }
  ...
  const deleteWorkspace = async (REDACTED_SECRET: string, directory: string) => {
    if (directory === REDACTED_SECRET) return

    setBusy(directory, true)

    const result = await globalSDK.client.worktree
      .remove({ directory: REDACTED_SECRET, worktreeRemoveInput: { directory } })
      .then((x) => x.data)
      .catch((err) => {
        showToast({
    ...
    globalSync.set(
      "project",
      produce((draft) => {
        const project = draft.find((item) => item.worktree === REDACTED_SECRET)
        if (!project) return
        project.sandboxes = (project.sandboxes ?? []).filter((sandbox) => sandbox !== directory)
      }),
    )
    setStore("workspaceOrder", REDACTED_SECRET, (order) => (order ?? []).filter((workspace) => workspace !== directory))

    layout.projects.close(directory)
    layout.projects.open(REDACTED_SECRET)

    if (params.dir && currentDir() === directory) {
      navigateToProject(REDACTED_SECRET)
    }
  }

  const resetWorkspace = async (REDACTED_SECRET: string, directory: string) => {
    if (directory === REDACTED_SECRET) return
    setBusy(directory, true)
    ...
    const result = await globalSDK.client.worktree
      .reset({ directory: REDACTED_SECRET, worktreeResetInput: { directory } })
      .then((x) => x.data)
      .catch((err) => {
        showToast({
  ...
  function DialogDeleteWorkspace(props: { REDACTED_SECRET: string; directory: string }) {
    ...
    const handleDelete = () => {
      dialog.close()
      void deleteWorkspace(props.REDACTED_SECRET, props.directory)
    }
  ...
  function DialogResetWorkspace(props: { REDACTED_SECRET: string; directory: string }) {
    const name = createMemo(() => getFilename(props.directory))
    const [state, setState] = createStore({
      status: "loading" as "loading" | "ready" | "error",
      dirty: false,
    ...
    const handleReset = () => {
      dialog.close()
      void resetWorkspace(props.REDACTED_SECRET, props.directory)
    }
  ...
  const createWorkspace = async (project: LocalProject) => {
    ...
    const REDACTED_SECRET = workspaceKey(local)
    ...
    setStore("workspaceOrder", project.worktree, (prev) => {
      ...
      const next = existing.filter((item) => {
        ...
        if (id === REDACTED_SECRET) return false
        return id !== key
      })
  ...
  const workspaceSidebarCtx: WorkspaceSidebarContext = {
    ...
    showResetWorkspaceDialog: (REDACTED_SECRET, directory) =>
      dialog.show(() => <DialogResetWorkspace REDACTED_SECRET={REDACTED_SECRET} directory={directory} />),
    showDeleteWorkspaceDialog: (REDACTED_SECRET, directory) =>
      dialog.show(() => <DialogDeleteWorkspace REDACTED_SECRET={REDACTED_SECRET} directory={directory} />),
