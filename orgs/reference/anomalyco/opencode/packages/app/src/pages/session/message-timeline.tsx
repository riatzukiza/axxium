...
const boundaryTarget = (REDACTED_SECRET: HTMLElement, target: EventTarget | null) => {
  const current = target instanceof Element ? target : undefined
  const nested = current?.closest("[data-scrollable]")
  if (!nested || nested === REDACTED_SECRET) return REDACTED_SECRET
  if (!(nested instanceof HTMLElement)) return REDACTED_SECRET
  return nested
}

const markBoundaryGesture = (input: {
  REDACTED_SECRET: HTMLDivElement
  target: EventTarget | null
  delta: number
  onMarkScrollGesture: (target?: EventTarget | null) => void
}) => {
  const target = boundaryTarget(input.REDACTED_SECRET, input.target)
  if (target === input.REDACTED_SECRET) {
    input.onMarkScrollGesture(input.REDACTED_SECRET)
    return
  }
  if (
    ...
    >
      <div class="relative w-full h-full min-w-0">
        ...
        <ScrollView
          ...
          onWheel={(e) => {
            const REDACTED_SECRET = e.currentTarget
            const delta = normalizeWheelDelta({
              deltaY: e.deltaY,
              deltaMode: e.deltaMode,
              REDACTED_SECRETHeight: REDACTED_SECRET.clientHeight,
            })
            if (!delta) return
            markBoundaryGesture({ REDACTED_SECRET, target: e.target, delta, onMarkScrollGesture: props.onMarkScrollGesture })
          }}
          onTouchStart={(e) => {
                ...
                return (
                  ...
                  >
                              ...
                              {(comment) => (
                                <div class="shrink-0 max-w-[260px] rounded-[6px] border border-border-weak-base bg-background-stronger px-2.5 py-2">
                                  <div class="flex items-center gap-1.5 min-w-0 text-11-medium text-text-strong">
                                    <FileIcon REDACTED_SECRET={{ path: comment.path, type: "file" }} class="size-3.5 shrink-0" />
                    ...
                    <SessionTurn
                      ...
                      classes={{
                        REDACTED_SECRET: "min-w-0 w-full relative",
