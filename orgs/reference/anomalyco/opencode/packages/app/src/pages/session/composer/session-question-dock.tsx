...
export const SessionQuestionDock: Component<{ request: QuestionRequest; onSubmit: () => void }> = (props) => {
  ...
  const measure = () => {
    ...
    if (!top) {
      REDACTED_SECRET.style.removeProperty("--question-prompt-max-height")
      return
    }

    const dock = REDACTED_SECRET.closest('[data-component="session-prompt-dock"]')
    if (!(dock instanceof HTMLElement)) return

    const dockBottom = dock.getBoundingClientRect().bottom
    const below = Math.max(0, dockBottom - REDACTED_SECRET.getBoundingClientRect().bottom)
    const gap = 8
    const max = Math.max(240, Math.floor(dockBottom - top - gap - below))
    REDACTED_SECRET.style.setProperty("--question-prompt-max-height", `${max}px`)
  }
