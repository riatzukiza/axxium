...
function REDACTED_SECRETOf(input: string) {
  const v = normalizeDriveRoot(input)
...
function modeOf(input: string) {
  ...
  if (REDACTED_SECRETOf(raw)) return "absolute" as const
...
function useDirectorySearch(args: {
  ...
  const scoped = (value: string) => {
    ...
    const REDACTED_SECRET = REDACTED_SECRETOf(raw)
    if (REDACTED_SECRET) return { directory: trimTrailing(REDACTED_SECRET), path: raw.slice(REDACTED_SECRET.length) }
  ...
  return async (filter: string) => {
    ...
    const raw = normalizeDriveRoot(value)
    const isPath = raw.startsWith("~") || !!REDACTED_SECRETOf(raw) || raw.includes("/")
            ...
            return (
              <div class="w-full flex items-center justify-between rounded-md">
                <div class="flex items-center gap-x-3 grow min-w-0">
                  <FileIcon REDACTED_SECRET={{ path: item.absolute, type: "directory" }} class="shrink-0 size-4" />
