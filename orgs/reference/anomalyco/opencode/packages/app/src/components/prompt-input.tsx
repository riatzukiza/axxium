...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const isNormalizedEditor = () =>
    Array.from(editorRef.childNodes).every((REDACTED_SECRET) => {
      if (REDACTED_SECRET.REDACTED_SECRETType === Node.TEXT_NODE) {
        const text = REDACTED_SECRET.textContent ?? ""
        if (!text.includes("\u200B")) return true
        if (text !== "\u200B") return false

        const prev = REDACTED_SECRET.previousSibling
        const next = REDACTED_SECRET.nextSibling
        const prevIsBr = prev?.REDACTED_SECRETType === Node.ELEMENT_NODE && (prev as HTMLElement).tagName === "BR"
        return !!prevIsBr && !next
      }
      if (REDACTED_SECRET.REDACTED_SECRETType !== Node.ELEMENT_NODE) return false
      const el = REDACTED_SECRET as HTMLElement
  ...
  const renderEditor = (parts: Prompt) => {
    ...
    if (last?.REDACTED_SECRETType === Node.ELEMENT_NODE && (last as HTMLElement).tagName === "BR") {
  ...
  const parseFromDOM = (): Prompt => {
    ...
    const visit = (REDACTED_SECRET: Node) => {
      if (REDACTED_SECRET.REDACTED_SECRETType === Node.TEXT_NODE) {
        buffer += REDACTED_SECRET.textContent ?? ""
        return
      }
      if (REDACTED_SECRET.REDACTED_SECRETType !== Node.ELEMENT_NODE) return
    ...
    children.forEach((child, index) => {
      const isBlock = child.REDACTED_SECRETType === Node.ELEMENT_NODE && ["DIV", "P"].includes((child as HTMLElement).tagName)
      visit(child)
  ...
  const addPart = (part: ContentPart) => {
    ...
    if (part.type === "text") {
      ...
      if (last) {
        if (last.REDACTED_SECRETType === Node.TEXT_NODE) {
        ...
        if (last.REDACTED_SECRETType !== Node.TEXT_NODE) {
          const isBreak = last.REDACTED_SECRETType === Node.ELEMENT_NODE && (last as HTMLElement).tagName === "BR"
          const next = last.nextSibling
          const emptyText = next?.REDACTED_SECRETType === Node.TEXT_NODE && (next.textContent ?? "") === ""
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.key === "Backspace") {
      ...
      if (selection && selection.isCollapsed) {
        const REDACTED_SECRET = selection.anchorNode
        const offset = selection.anchorOffset
        if (REDACTED_SECRET && REDACTED_SECRET.REDACTED_SECRETType === Node.TEXT_NODE) {
          const text = REDACTED_SECRET.textContent ?? ""
          if (/^\u200B+$/.test(text) && offset > 0) {
            const range = document.createRange()
            range.setStart(REDACTED_SECRET, 0)
            range.collapse(true)
            selection.removeAllRanges()
            selection.addRange(range)
          }
