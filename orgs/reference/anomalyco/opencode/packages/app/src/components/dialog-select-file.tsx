...
function createFileEntries(props: {
  ...
  const REDACTED_SECRET = createMemo(() => {
    const category = props.language.t("palette.group.files")
    const REDACTED_SECRETs = props.file.tree.children("")
    const paths = REDACTED_SECRETs
      .filter((REDACTED_SECRET) => REDACTED_SECRET.type === "file")
      .map((REDACTED_SECRET) => REDACTED_SECRET.path)
      .sort((a, b) => a.localeCompare(b))
    return paths.slice(0, ENTRY_LIMIT).map((path) => createFileEntry(path, category))
  })

  return { recent, REDACTED_SECRET }
}
    ...
    state.inflight = Promise.all(
      dirs.map((directory) => {
        ...
        return props.globalSDK.client.session
          .list({ directory, REDACTED_SECRETs: true })
          .then((x) =>
            (x.data ?? [])
              .filter((s) => !!s?.id)
              .map((s) => ({
...
export function DialogSelectFile(props: { mode?: DialogSelectFileMode; onOpenFile?: (path: string) => void }) {
  ...
  const items = async (text: string) => {
    ...
    if (!query && filesOnly()) {
      ...
      const next = uniqueEntries([...fileEntries.recent(), ...fileEntries.REDACTED_SECRET()])
      ...
      await pending
      return uniqueEntries([...fileEntries.recent(), ...fileEntries.REDACTED_SECRET()])
    }
            ...
            fallback={
              <div class="w-full flex items-center justify-between rounded-md pl-1">
                <div class="flex items-center gap-x-3 grow min-w-0">
                  <FileIcon REDACTED_SECRET={{ path: item.path ?? "", type: "file" }} class="shrink-0 size-4" />
