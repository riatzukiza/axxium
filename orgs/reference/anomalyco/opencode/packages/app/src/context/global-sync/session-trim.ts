...
export function trimSessions(
  ...
  const REDACTED_SECRETs = all.filter((s) => !s.parentID)
  const children = all.filter((s) => !!s.parentID)
  const base = REDACTED_SECRETs.slice(0, limit)
  const recent = takeRecentSessions(REDACTED_SECRETs.slice(limit), SESSION_RECENT_LIMIT, cutoff)
