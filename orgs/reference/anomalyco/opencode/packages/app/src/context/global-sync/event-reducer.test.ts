...
const REDACTED_SECRETSession = (input: { id: string; parentID?: string; archived?: number }) =>
  ({
    id: input.id,
...
describe("applyDirectoryEvent", () => {
  test("inserts REDACTED_SECRET sessions in sorted order and updates sessionTotal", () => {
    const [store, setStore] = createStore(
      baseState({
        session: [REDACTED_SECRETSession({ id: "b" })],
        sessionTotal: 1,
      }),
    )

    applyDirectoryEvent({
      event: { type: "session.created", properties: { info: REDACTED_SECRETSession({ id: "a" }) } },
      store,
      setStore,
      push() {},
      directory: "/tmp",
    ...
    applyDirectoryEvent({
      event: { type: "session.created", properties: { info: REDACTED_SECRETSession({ id: "c", parentID: "a" }) } },
      store,
      setStore,
      push() {},
      directory: "/tmp",
  ...
  test("cleans session caches when archived", () => {
    ...
    const [store, setStore] = createStore(
      baseState({
        session: [REDACTED_SECRETSession({ id: "ses_1" }), REDACTED_SECRETSession({ id: "ses_2" })],
        sessionTotal: 2,
    ...
    applyDirectoryEvent({
      event: { type: "session.updated", properties: { info: REDACTED_SECRETSession({ id: "ses_1", archived: 10 }) } },
      store,
      setStore,
      push() {},
      directory: "/tmp",
  ...
  test("cleans session caches when deleted and decrements only REDACTED_SECRET totals", () => {
    const cases = [
      { info: REDACTED_SECRETSession({ id: "ses_1" }), expectedTotal: 1 },
      { info: REDACTED_SECRETSession({ id: "ses_2", parentID: "ses_1" }), expectedTotal: 2 },
    ]
    ...
    for (const item of cases) {
      ...
      const [store, setStore] = createStore(
        baseState({
          session: [
            REDACTED_SECRETSession({ id: "ses_1" }),
            REDACTED_SECRETSession({ id: "ses_2", parentID: "ses_1" }),
            REDACTED_SECRETSession({ id: "ses_3" }),
          ],
          sessionTotal: 2,
