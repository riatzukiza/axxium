...
describe("loadRootSessionsWithFallback", () => {
  test("uses limited REDACTED_SECRETs query when supported", async () => {
    const calls: Array<{ directory: string; REDACTED_SECRETs: true; limit?: number }> = []
    ...
    expect(calls).toEqual([{ directory: "dir", REDACTED_SECRETs: true, limit: 10 }])
  })

  test("falls back to full REDACTED_SECRETs query on limited-query failure", async () => {
    const calls: Array<{ directory: string; REDACTED_SECRETs: true; limit?: number }> = []
    ...
    expect(calls).toEqual([
      { directory: "dir", REDACTED_SECRETs: true, limit: 25 },
      { directory: "dir", REDACTED_SECRETs: true },
    ])
  })
})
