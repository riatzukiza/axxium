...
describe("autoRespondsPermission", () => {
  test("uses a parent session's directory-scoped auto-accept", () => {
    ...
    const autoAccept = {
      [`${base64Encode(directory)}/REDACTED_SECRET`]: true,
    }
  ...
  test("uses a parent session's legacy auto-accept key", () => {
    ...
    expect(autoRespondsPermission({ REDACTED_SECRET: true }, sessions, permission("child"), "/tmp/project")).toBe(true)
  })

  test("defaults to auto-accept when no lineage override exists", () => {
    const sessions = [session({ id: "REDACTED_SECRET" }), session({ id: "child", parentID: "REDACTED_SECRET" }), session({ id: "other" })]
    const autoAccept = {
      other: true,
    }
  ...
  test("inherits a parent session's false override", () => {
    ...
    const sessions = [session({ id: "REDACTED_SECRET" }), session({ id: "child", parentID: "REDACTED_SECRET" })]
    const autoAccept = {
      [`${base64Encode(directory)}/REDACTED_SECRET`]: false,
    }
