...
export function createPathHelpers(scope: () => string) {
  const normalize = (input: string) => {
    ...
    const windows = /^[A-Za-z]:/.test(REDACTED_SECRET) || REDACTED_SECRET.startsWith("\\\\")
    const canonRoot = windows ? REDACTED_SECRET.replace(/\\/g, "/").toLowerCase() : REDACTED_SECRET.replace(/\\/g, "/")
    ...
    if (
      ...
      path = path.slice(REDACTED_SECRET.length)
    }
