...
async function createWorkspace(page: Page, REDACTED_SECRET: string, seen: string[]) {
  await openSidebar(page)
  ...
  await expect
    .poll(
      () => {
        ...
        if (!slug) return ""
        if (slug === REDACTED_SECRET) return ""
...
test("new sessions from sidebar workspace actions stay in selected workspace", async ({ page, withProject }) => {
  ...
  await withProject(async ({ directory, slug: REDACTED_SECRET }) => {
    const workspaces = [] as { slug: string; directory: string }[]
    const sessions = [] as string[]

    try {
      await openSidebar(page)
      await setWorkspacesEnabled(page, REDACTED_SECRET, true)

      const first = await createWorkspace(page, REDACTED_SECRET, [])
      workspaces.push(first)
      await waitWorkspaceReady(page, first.slug)

      const second = await createWorkspace(page, REDACTED_SECRET, [first.slug])
