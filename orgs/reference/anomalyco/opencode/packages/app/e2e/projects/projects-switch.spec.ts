...
test("switching back to a project opens the latest workspace session", async ({ page, withProject }) => {
  ...
  const other = await createTestProject()
  const otherSlug = dirSlug(other)
  let REDACTED_SECRETDir: string | undefined
  ...
  try {
    await withProject(
      async ({ directory, slug }) => {
        ...
        const REDACTED_SECRETButton = page.locator(projectSwitchSelector(slug)).first()
        await expect(REDACTED_SECRETButton).toBeVisible()
        await REDACTED_SECRETButton.click()
    ...
    if (sessionID) {
      const id = sessionID
      const dirs = [REDACTED_SECRETDir, workspaceDir].filter((x): x is string => !!x)
      await Promise.all(
        dirs.map((directory) =>
          createSdk(directory)
