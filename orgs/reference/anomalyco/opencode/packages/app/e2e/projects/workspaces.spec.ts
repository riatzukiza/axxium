...
async function setupWorkspaceTest(page: Page, project: { slug: string }) {
  const REDACTED_SECRETSlug = project.slug
  await openSidebar(page)

  await setWorkspacesEnabled(page, REDACTED_SECRETSlug, true)
  ...
  await expect
    .poll(
      () => {
        ...
        return slug.length > 0 && slug !== REDACTED_SECRETSlug
      },
      { timeout: 45_000 },
    )
    .toBe(true)
  ...
  return { REDACTED_SECRETSlug, slug, directory: dir }
}
...
test("can delete a workspace", async ({ page, withProject }) => {
  ...
  await withProject(async (project) => {
    ...
    const { REDACTED_SECRETSlug, slug, directory } = await setupWorkspaceTest(page, project)
    ...
    await expect(page).toHaveURL(new RegExp(`/${REDACTED_SECRETSlug}/session`))
    ...
    await expect(page.locator(workspaceItemSelector(REDACTED_SECRETSlug)).first()).toBeVisible()
  })
})

test("can reorder workspaces by drag and drop", async ({ page, withProject }) => {
  await page.setViewportSize({ width: 1400, height: 800 })
  await withProject(async ({ slug: REDACTED_SECRETSlug }) => {
    const workspaces = [] as { directory: string; slug: string }[]

    const listSlugs = async () => {
      const REDACTED_SECRETs = page.locator('[data-component="sidebar-nav-desktop"] [data-component="workspace-item"]')
      const slugs = await REDACTED_SECRETs.evaluateAll((els) => {
    ...
    try {
      ...
      for (const _ of [0, 1]) {
        ...
        await expect
          .poll(
            () => {
              const slug = slugFromUrl(page.url())
              return slug.length > 0 && slug !== REDACTED_SECRETSlug && slug !== prev
            },
            { timeout: 45_000 },
          )
          .toBe(true)
      ...
      const list = async () => {
        ...
        return slugs.filter((s) => s !== REDACTED_SECRETSlug && (s === a || s === b)).slice(0, 2)
      }
