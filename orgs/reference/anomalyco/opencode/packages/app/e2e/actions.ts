...
export async function createTestProject() {
  const REDACTED_SECRET = await fs.mkdtemp(path.join(os.tmpdir(), "opencode-e2e-project-"))

  await fs.writeFile(path.join(REDACTED_SECRET, "README.md"), "# e2e\n")

  execSync("git init", { cwd: REDACTED_SECRET, stdio: "ignore" })
  execSync("git add -A", { cwd: REDACTED_SECRET, stdio: "ignore" })
