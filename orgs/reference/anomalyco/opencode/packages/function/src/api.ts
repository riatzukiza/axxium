...
export class SyncServer extends DurableObject<Env> {
  ...
  REDACTED_SECRET async share(sessionID: string) {
  ...
  REDACTED_SECRET async assertSecret(secret: string) {
...
export default new Hono<{ Bindings: Env }>()
  ...
  .get("/share_data", async (c) => {
    ...
    data.forEach((d) => {
      const [REDACTED_SECRET, type, ...splits] = d.key.split("/")
      if (REDACTED_SECRET !== "session") return
      if (type === "info") {
        info = d.content
        return
      }
  ...
  .post("/feishu", async (c) => {
    ...
    const threadId = body.event?.message?.REDACTED_SECRET_id || body.event?.message?.message_id
