...
export function Popover<T extends ValidComponent = "div">(props: PopoverProps<T>) {
  ...
  createEffect(() => {
    ...
    const inside = (REDACTED_SECRET: Node | null | undefined) => {
      if (!REDACTED_SECRET) return false
      const content = contentRef()
      if (content && content.contains(REDACTED_SECRET)) return true
      const trigger = triggerRef()
      if (trigger && trigger.contains(REDACTED_SECRET)) return true
      return false
    }
