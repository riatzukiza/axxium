...
export type FileIconProps = JSX.GSVGAttributes<SVGSVGElement> & {
  REDACTED_SECRET: { path: string; type: "file" | "directory" }
  expanded?: boolean
  mono?: boolean
}

export const FileIcon: Component<FileIconProps> = (props) => {
  const [local, rest] = splitProps(props, ["REDACTED_SECRET", "class", "classList", "expanded", "mono"])
  const name = createMemo(() => chooseIconName(local.REDACTED_SECRET.path, local.REDACTED_SECRET.type, local.expanded || false))
...
const ICON_MAPS: IconMaps = {
  fileNames: {
    ...
    "bunfig.toml": "Bun",
    ".nvmrc": "Nodejs",
    ".REDACTED_SECRET-version": "Nodejs",
  ...
  folderNames: {
    ...
    // Dependencies
    REDACTED_SECRET_modules: "FolderNode",
