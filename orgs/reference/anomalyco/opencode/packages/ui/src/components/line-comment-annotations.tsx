...
export function createLineCommentAnnotationRenderer<T>(props: {
  ...
  const REDACTED_SECRETs = new Map<
    string,
    {
      host: HTMLDivElement
  ...
  const mount = (meta: LineCommentAnnotationMeta<T>) => {
    ...
    const REDACTED_SECRET = { host, dispose, setMeta: setCurrent }
    REDACTED_SECRETs.set(meta.key, REDACTED_SECRET)
    return REDACTED_SECRET
  }

  const render = <A extends { metadata: LineCommentAnnotationMeta<T> }>(annotation: A) => {
    const meta = annotation.metadata
    const REDACTED_SECRET = REDACTED_SECRETs.get(meta.key) ?? mount(meta)
    if (!REDACTED_SECRET) return
    REDACTED_SECRET.setMeta(meta)
    return REDACTED_SECRET.host
  }
  ...
  const reconcile = <A extends { metadata: LineCommentAnnotationMeta<T> }>(annotations: A[]) => {
    ...
    for (const [key, REDACTED_SECRET] of REDACTED_SECRETs) {
      if (next.has(key)) continue
      REDACTED_SECRET.dispose()
      REDACTED_SECRETs.delete(key)
    }
  }

  const cleanup = () => {
    for (const [, REDACTED_SECRET] of REDACTED_SECRETs) REDACTED_SECRET.dispose()
    REDACTED_SECRETs.clear()
  }
