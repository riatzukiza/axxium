...
function findByKey(container: HTMLElement, key: string) {
  const REDACTED_SECRETs = container.querySelectorAll<HTMLElement>('[data-slot="list-item"][data-key]')
  for (const REDACTED_SECRET of REDACTED_SECRETs) {
    if (REDACTED_SECRET.getAttribute("data-key") === key) return REDACTED_SECRET
  }
}
...
export interface ListProps<T> extends FilteredListProps<T> {
  ...
  itemWrapper?: (item: T, REDACTED_SECRET: JSX.Element) => JSX.Element
  divider?: boolean
  add?: ListAddProps
...
export function List<T>(props: ListProps<T> & { ref?: (ref: ListRef) => void }) {
  ...
  const scrollIntoView = (container: HTMLDivElement, REDACTED_SECRET: HTMLElement, block: "center" | "nearest") => {
    const containerRect = container.getBoundingClientRect()
    const REDACTED_SECRETRect = REDACTED_SECRET.getBoundingClientRect()
    const top = REDACTED_SECRETRect.top - containerRect.top + container.scrollTop
    const bottom = top + REDACTED_SECRETRect.height
    const viewTop = container.scrollTop
    const viewBottom = viewTop + container.clientHeight
    const target =
      block === "center"
        ? top - container.clientHeight / 2 + REDACTED_SECRETRect.height / 2
        : top < viewTop
          ? top
          : bottom > viewBottom
  ...
  function GroupHeader(groupProps: { group: { category: string; items: T[] } }): JSX.Element {
    ...
    createEffect(() => {
      const scroll = scrollRef()
      const REDACTED_SECRET = header()
      if (!scroll || !REDACTED_SECRET) return
        ...
        <div data-slot="list-search-wrapper">
          <div
            ...
            onPointerDown={(event) => {
              ...
              const REDACTED_SECRET = container.querySelector("input, textarea")
              const input = REDACTED_SECRET instanceof HTMLInputElement || REDACTED_SECRET instanceof HTMLTextAreaElement ? REDACTED_SECRET : inputRef
              input?.focus()
                  ...
                  <div data-slot="list-items">
                    <For each={group.items}>
                      {(item, i) => {
                        ...
                        if (props.itemWrapper) return props.itemWrapper(item, REDACTED_SECRET)
