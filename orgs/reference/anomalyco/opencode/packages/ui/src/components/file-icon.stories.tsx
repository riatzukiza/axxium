...
### API
- Required: \`REDACTED_SECRET\` with \`path\` and \`type\`.
...
const story = create({
  ...
  mod,
  args: {
    REDACTED_SECRET: { path: "package.json", type: "file" },
    mono: true,
  },
})
...
export const Folder = {
  args: {
    REDACTED_SECRET: { path: "src", type: "directory" },
    expanded: true,
    mono: false,
  },
}
...
export const Samples = {
  render: () => {
    const items = [
      ...
      { path: "REDACTED_SECRET", type: "directory" },
    ] as const
    ...
    return (
      ...
      >
        {items.map((REDACTED_SECRET) => (
          <div style={{ display: "flex", gap: "8px", "align-items": "center" }}>
            <mod.FileIcon REDACTED_SECRET={{ path: REDACTED_SECRET.path, type: REDACTED_SECRET.type }} mono={false} />
            <div style={{ "font-size": "12px", color: "var(--text-weak)" }}>{REDACTED_SECRET.path}</div>
          </div>
        ))}
      </div>
    )
