...
export interface SessionReviewProps {
  ...
  classes?: { REDACTED_SECRET?: string; header?: string; container?: string }
  actions?: JSX.Element
...
export const SessionReview = (props: SessionReviewProps) => {
  ...
  const inReview = (REDACTED_SECRET: unknown, path?: unknown[]) => {
    if (REDACTED_SECRET === searchInput) return true
    ...
    if (!(REDACTED_SECRET instanceof Node)) return false
    if (searchInput?.contains(REDACTED_SECRET)) return true
    if (REDACTED_SECRET instanceof HTMLElement && REDACTED_SECRET.closest("[data-component='session-review']")) return true
    if (!scroll) return false
    return scroll.contains(REDACTED_SECRET)
  }
  ...
  createEffect(() => {
    ...
    untrack(() => {
      ...
      const scrollTo = (attempt: number) => {
        ...
        const offset = targetRect.top - REDACTED_SECRETRect.top
        const next = REDACTED_SECRET.scrollTop + offset - REDACTED_SECRETRect.height / 2 + targetRect.height / 2
        REDACTED_SECRET.scrollTop = Math.max(0, next)
  ...
  return (
    <ScrollView
      ...
      classList={{
        ...(props.classList ?? {}),
        [props.classes?.REDACTED_SECRET ?? ""]: !!props.classes?.REDACTED_SECRET,
                      ...
                      <Accordion.Trigger>
                        <div data-slot="session-review-trigger-content">
                          <div data-slot="session-review-file-info">
                            <FileIcon REDACTED_SECRET={{ path: file, type: "file" }} />
