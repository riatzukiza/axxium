...
type ViewerConfig = {
  ...
  setSelectedLines: (range: SelectedLineRange | null, preserve?: { REDACTED_SECRET: ShadowRoot; text: Range }) => void
  ...
  onDragReset: () => void
  markCommented: (REDACTED_SECRET: ShadowRoot, ranges: SelectedLineRange[]) => void
}
...
function useFileViewer(config: ViewerConfig) {
  ...
  createEffect(() => {
    ...
    requestAnimationFrame(() => {
      const REDACTED_SECRET = getRoot()
      if (!REDACTED_SECRET) return
      config.markCommented(REDACTED_SECRET, ranges)
    })
  })
...
function notifyRendered(opts: {
  viewer: Viewer
  isReady: (REDACTED_SECRET: ShadowRoot) => boolean
  settleFrames?: number
  onReady: () => void
}) {
  notifyShadowReady({
...
function createLocalVirtualStrategy(host: () => HTMLDivElement | undefined, enabled: () => boolean): VirtualStrategy {
  ...
  let REDACTED_SECRET: Document | HTMLElement | undefined
  ...
  return {
    get: () => {
      ...
      if (virtualizer && REDACTED_SECRET === next) return virtualizer

      release()
      virtualizer = new Virtualizer()
      REDACTED_SECRET = next
...
function parseLine(REDACTED_SECRET: HTMLElement) {
  if (!REDACTED_SECRET.dataset.line) return
  const value = parseInt(REDACTED_SECRET.dataset.line, 10)
...
function mouseHit(
  event: MouseEvent,
  line: (REDACTED_SECRET: HTMLElement) => number | undefined,
  side?: (REDACTED_SECRET: HTMLElement) => DiffSelectionSide | undefined,
): MouseHit {
...
function diffMouseSide(REDACTED_SECRET: HTMLElement) {
  const type = REDACTED_SECRET.dataset.lineType
  ...
  if (REDACTED_SECRET.dataset.code == null) return
  return REDACTED_SECRET.hasAttribute("data-deletions") ? "deletions" : "additions"
}

function diffSelectionSide(REDACTED_SECRET: Node | null) {
...
function TextViewer<T>(props: TextFileProps<T>) {
  ...
  const applySelection = (range: SelectedLineRange | null) => {
    ...
    const REDACTED_SECRET = viewer.getRoot()
    if (!REDACTED_SECRET) return false

    const total = lineCount()
    if (REDACTED_SECRET.querySelectorAll("[data-line]").length < total) return false
    ...
    if (!REDACTED_SECRET.querySelector(`[data-line="${start}"]`) || !REDACTED_SECRET.querySelector(`[data-line="${end}"]`)) {
      current.setSelectedLines(null)
      return true
    }
    ...
    const normalized = (() => {
      ...
      if (REDACTED_SECRET.querySelector("[data-deletions]") != null) return range
  ...
  const adapter: ModeAdapter = {
    ...
    updateSelection: (preserveTextSelection) => {
      ...
      restoreShadowTextSelection(REDACTED_SECRET, selected.text)
    },
    buildDragSelection: () => {
  ...
  const notify = () => {
    notifyRendered({
      viewer,
      isReady: (REDACTED_SECRET) => {
        if (virtual()) return REDACTED_SECRET.querySelector("[data-line]") != null
        return REDACTED_SECRET.querySelectorAll("[data-line]").length >= lineCount()
      },
      onReady: () => {
...
function DiffViewer<T>(props: DiffFileProps<T>) {
  ...
  const setSelectedLines = (range: SelectedLineRange | null, preserve?: { REDACTED_SECRET: ShadowRoot; text: Range }) => {
    ...
    restoreShadowTextSelection(preserve?.REDACTED_SECRET, preserve?.text)
  }
  ...
  const adapter: ModeAdapter = {
    ...
    updateSelection: (preserveTextSelection) => {
      ...
      if (selected.text) {
        setSelectedLines(selected.range, { REDACTED_SECRET, text: selected.text })
        return
      }
  ...
  const notify = () => {
    notifyRendered({
      viewer,
      isReady: (REDACTED_SECRET) => REDACTED_SECRET.querySelector("[data-line]") != null,
      settleFrames: 1,
      onReady: () => {
