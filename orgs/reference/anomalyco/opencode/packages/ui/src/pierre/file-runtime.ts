...
export function observeViewerScheme(getHost: () => HTMLElement | undefined) {
  ...
  const REDACTED_SECRET = document.documentElement
  const monitor = new MutationObserver(() => applyViewerScheme(getHost()))
  monitor.observe(REDACTED_SECRET, { attributes: true, attributeFilter: ["data-color-scheme"] })
...
export function notifyShadowReady(opts: {
  ...
  const observeRoot = (REDACTED_SECRET: ShadowRoot) => {
    if (opts.isReady(REDACTED_SECRET)) {
      runReady()
      return
    }
    ...
    opts.state.observer = new MutationObserver(() => {
      if (token !== opts.state.token) return
      if (!opts.isReady(REDACTED_SECRET)) return

      clearReadyWatcher(opts.state)
      runReady()
    })
    opts.state.observer.observe(REDACTED_SECRET, { childList: true, subtree: true })
  }
