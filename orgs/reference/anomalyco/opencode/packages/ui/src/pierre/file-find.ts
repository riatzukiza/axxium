...
function isEditable(REDACTED_SECRET: unknown): boolean {
  if (!(REDACTED_SECRET instanceof HTMLElement)) return false
  if (REDACTED_SECRET.closest("[data-prevent-autofocus]")) return true
  if (REDACTED_SECRET.isContentEditable) return true
  return /^(INPUT|TEXTAREA|SELECT|BUTTON)$/.test(REDACTED_SECRET.tagName)
}

function hostForNode(REDACTED_SECRET: unknown) {
  if (!(REDACTED_SECRET instanceof Node)) return
  for (const host of hosts) {
    const el = host.element()
    if (el && el.isConnected && el.contains(REDACTED_SECRET)) return host
  }
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const syncOverlayScroll = () => {
    if (mode !== "overlay") return
    const REDACTED_SECRET = opts.getRoot()

    const next = REDACTED_SECRET
      ? Array.from(REDACTED_SECRET.querySelectorAll("[data-code]")).filter(
  ...
  const positionBar = () => {
    ...
    const REDACTED_SECRET = scrollParent(wrapper) ?? wrapper
    const rect = REDACTED_SECRET.getBoundingClientRect()
    const title = parseFloat(getComputedStyle(REDACTED_SECRET).getPropertyValue("--session-title-height"))
  ...
  const scan = (REDACTED_SECRET: ShadowRoot, value: string) => {
    const needle = value.toLowerCase()
    const ranges: FileFindHit[] = []
    const cols = Array.from(REDACTED_SECRET.querySelectorAll("[data-content] [data-line], [data-column-content]")).filter(
      (REDACTED_SECRET): REDACTED_SECRET is HTMLElement => REDACTED_SECRET instanceof HTMLElement,
    )
    ...
    for (const col of cols) {
      ...
      const REDACTED_SECRETs: Text[] = []
      const ends: number[] = []
      const walker = document.createTreeWalker(col, NodeFilter.SHOW_TEXT)
      let REDACTED_SECRET = walker.nextNode()
      let pos = 0
      while (REDACTED_SECRET) {
        if (REDACTED_SECRET instanceof Text) {
          pos += REDACTED_SECRET.data.length
          REDACTED_SECRETs.push(REDACTED_SECRET)
          ends.push(pos)
        }
        REDACTED_SECRET = walker.nextNode()
      }
      if (REDACTED_SECRETs.length === 0) continue
      ...
      const locate = (offset: number) => {
        ...
        return { REDACTED_SECRET: REDACTED_SECRETs[lo], offset: offset - prev }
      }
      ...
      while (at !== -1) {
        ...
        range.setStart(start.REDACTED_SECRET, start.offset)
        range.setEnd(end.REDACTED_SECRET, end.offset)
        ranges.push({
          range,
          side,
          line,
  ...
  const apply = (args?: { reset?: boolean; scroll?: boolean }) => {
    ...
    const ranges = scan(REDACTED_SECRET, value)
  ...
  createEffect(() => {
    ...
    observer?.observe(REDACTED_SECRET)
