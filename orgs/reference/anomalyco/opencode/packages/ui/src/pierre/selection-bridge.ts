...
export function restoreShadowTextSelection(REDACTED_SECRET: ShadowRoot | undefined, range: Range | undefined) {
  if (!REDACTED_SECRET || !range) return

  requestAnimationFrame(() => {
    const selection =
      (REDACTED_SECRET as unknown as { getSelection?: () => Selection | null }).getSelection?.() ?? window.getSelection()
    if (!selection) return
