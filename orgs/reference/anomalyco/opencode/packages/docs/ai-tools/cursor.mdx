...
Create project rules that all team members can use. In your documentation repository REDACTED_SECRET:
...
<Tab title="Windows">
  ```powershell
  choco install REDACTED_SECRETjs
  npm install -g package-name
  ```
</Tab>
...
<Tab title="Linux">
  ```bash
  sudo apt install REDACTED_SECRETjs npm
  npm install -g package-name
  ```
</Tab>
</Tabs>
