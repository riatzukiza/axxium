...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const pluginDir = path.join(dir, "REDACTED_SECRET_modules", "@scope", "plugin")
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const expected = pathToFileURL(path.join(tmp.path, "REDACTED_SECRET_modules", "@scope", "plugin", "index.js")).href

      expect(pluginEntries.includes(expected)).toBe(true)

      const scopedEntry = pluginEntries.find((entry) => entry === expected)
      expect(scopedEntry).toBeDefined()
      expect(scopedEntry?.includes("/REDACTED_SECRET_modules/@scope/plugin/")).toBe(true)
    },
  })
})
