    ...
    test("reads .gitignore via Filesystem.exists() and readText()", async () => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          await fs.writeFile(gitignorePath, "REDACTED_SECRET_modules\ndist\n", "utf-8")
          ...
          expect(content).toContain("REDACTED_SECRET_modules")
        },
      })
    })
    ...
    test("handles missing .gitignore gracefully", async () => {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const REDACTED_SECRETs = await File.list()
          expect(Array.isArray(REDACTED_SECRETs)).toBe(true)
        },
      })
    })
  })
