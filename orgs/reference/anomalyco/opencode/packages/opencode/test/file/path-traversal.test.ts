...
describe("Instance.containsPath", () => {
  ...
  test("returns true for path inside worktree but outside directory (monorepo subdirectory scenario)", async () => {
    ...
    await Instance.provide({
      directory: subdir,
      fn: () => {
        // .opencode at worktree REDACTED_SECRET, but we're running from packages/lib
        ...
        // worktree REDACTED_SECRET itself
  ...
  test("handles directory === worktree (running from repo REDACTED_SECRET)", async () => {
    await using tmp = await tmpdir({ git: true })
