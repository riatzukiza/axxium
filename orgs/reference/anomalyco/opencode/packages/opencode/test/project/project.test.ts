...
describe("Project.fromDirectory with worktrees", () => {
  test("should set worktree to REDACTED_SECRET when called from REDACTED_SECRET", async () => {
    const p = await loadProject()
    await using tmp = await tmpdir({ git: true })
  ...
  test("should set worktree to REDACTED_SECRET when called from a worktree", async () => {
    const p = await loadProject()
    await using tmp = await tmpdir({ git: true })
...
describe("Project.discover", () => {
  test("should discover favicon.png in REDACTED_SECRET", async () => {
    const p = await loadProject()
    await using tmp = await tmpdir({ git: true })
    const { project } = await p.fromDirectory(tmp.path)
