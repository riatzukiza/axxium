...
describe("Session.list", () => {
  ...
  test("filters REDACTED_SECRET sessions", async () => {
    await Instance.provide({
      directory: projectRoot,
      fn: async () => {
        const REDACTED_SECRET = await Session.create({ title: "REDACTED_SECRET-session" })
        const child = await Session.create({ title: "child-session", parentID: REDACTED_SECRET.id })

        const sessions = [...Session.list({ REDACTED_SECRETs: true })]
        const ids = sessions.map((s) => s.id)

        expect(ids).toContain(REDACTED_SECRET.id)
