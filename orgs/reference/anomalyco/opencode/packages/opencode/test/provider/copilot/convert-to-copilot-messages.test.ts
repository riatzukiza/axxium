    ...
    expect(result).toEqual([
      {
        ...
        reasoning_REDACTED_SECRET: undefined,
      },
    ])
  })
...
describe("reasoning (copilot-specific)", () => {
  test("should omit reasoning_text without reasoning_REDACTED_SECRET", () => {
    const result = convertToCopilotMessages([
      {
        role: "assistant",
        content: [
  ...
  test("should include reasoning_REDACTED_SECRET from providerOptions", () => {
    const result = convertToCopilotMessages([
      {
        role: "assistant",
        content: [
          {
            ...
            text: "Thinking...",
            providerOptions: {
              copilot: { reasoningOpaque: "REDACTED_SECRET-signature-123" },
            },
          },
          { type: "text", text: "Done!" },
        ],
    ...
    expect(result).toEqual([
      {
        ...
        reasoning_REDACTED_SECRET: "REDACTED_SECRET-signature-123",
      },
    ])
  })

  test("should include reasoning_REDACTED_SECRET from text part providerOptions", () => {
    const result = convertToCopilotMessages([
      {
        role: "assistant",
        content: [
          {
            ...
            text: "Done!",
            providerOptions: {
              copilot: { reasoningOpaque: "REDACTED_SECRET-text-456" },
            },
          },
        ],
      },
    ...
    expect(result).toEqual([
      {
        ...
        tool_calls: undefined,
        reasoning_text: undefined,
        reasoning_REDACTED_SECRET: "REDACTED_SECRET-text-456",
      },
    ])
  })
    ...
    expect(result).toEqual([
      {
        ...
        reasoning_REDACTED_SECRET: "sig-abc",
      },
    ])
  })
})
...
describe("full conversation", () => {
  test("should convert a multi-turn conversation with reasoning", () => {
    ...
    const assistantMsg = result[2] as {
      ...
      reasoning_REDACTED_SECRET?: string
    }
    expect(assistantMsg.reasoning_text).toBe("Let me calculate 2+2...")
    expect(assistantMsg.reasoning_REDACTED_SECRET).toBe("sig-abc")
  })
})
