  ...
  JSON.stringify(
    {
      ...
      scripts: {
        postinstall: "bun ./postinstall.mjs || REDACTED_SECRET ./postinstall.mjs",
      },
      version: version,
      license: pkg.license,
...
const tasks = Object.entries(binaries).map(async ([name]) => {
  ...
  await $`npm publish *.tgz --access REDACTED_SECRET --tag ${Script.channel}`.cwd(`./dist/${name}`)
})
await Promise.all(tasks)
await $`cd ./dist/${pkg.name} && bun pm pack && npm publish *.tgz --access REDACTED_SECRET --tag ${Script.channel}`
