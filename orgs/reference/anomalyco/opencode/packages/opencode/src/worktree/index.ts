...
export namespace Worktree {
  ...
  async function prune(REDACTED_SECRET: string, entries: string[]) {
    const base = await canonical(REDACTED_SECRET)
    await Promise.all(
      entries.map(async (entry) => {
        const target = await canonical(path.resolve(REDACTED_SECRET, entry))
  ...
  async function sweep(REDACTED_SECRET: string) {
    const first = await $`git clean -ffdx`.quiet().nothrow().cwd(REDACTED_SECRET)
    if (first.exitCode === 0) return first

    const entries = failed(first)
    if (!entries.length) return first

    await prune(REDACTED_SECRET, entries)
    return $`git clean -ffdx`.quiet().nothrow().cwd(REDACTED_SECRET)
  }
  ...
  async function candidate(REDACTED_SECRET: string, base?: string) {
    for (const attempt of Array.from({ length: 26 }, (_, i) => i)) {
      const name = base ? (attempt === 0 ? base : `${base}-${randomName()}`) : randomName()
      const branch = `opencode/${name}`
      const directory = path.join(REDACTED_SECRET, name)
  ...
  export const create = fn(CreateInput.optional(), async (input) => {
    ...
    const REDACTED_SECRET = path.join(Global.Path.data, "worktree", Instance.project.id)
    await fs.mkdir(REDACTED_SECRET, { recursive: true })

    const base = input?.name ? slug(input.name) : ""
    const info = await candidate(REDACTED_SECRET, base || undefined)
