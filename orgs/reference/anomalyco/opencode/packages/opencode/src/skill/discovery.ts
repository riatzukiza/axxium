  ...
  export async function pull(url: string): Promise<string[]> {
    ...
    await Promise.all(
      list.map(async (skill) => {
        const REDACTED_SECRET = path.join(cache, skill.name)
        await Promise.all(
          skill.files.map(async (file) => {
            const link = new URL(file, `${host}/${skill.name}/`).href
            const dest = path.join(REDACTED_SECRET, file)
            await mkdir(path.dirname(dest), { recursive: true })
            await get(link, dest)
          }),
        )

        const md = path.join(REDACTED_SECRET, "SKILL.md")
        if (await Filesystem.exists(md)) result.push(REDACTED_SECRET)
      }),
    )
