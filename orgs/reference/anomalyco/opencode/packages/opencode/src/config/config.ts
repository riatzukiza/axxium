...
export namespace Config {
  ...
  export async function installDependencies(dir: string) {
    ...
    if (!hasGitIgnore)
      await Filesystem.write(gitignore, ["REDACTED_SECRET_modules", "package.json", "bun.lock", ".gitignore"].join("\n"))
  ...
  export async function needsInstall(dir: string) {
    ...
    const REDACTED_SECRETModules = path.join(dir, "REDACTED_SECRET_modules")
    if (!existsSync(REDACTED_SECRETModules)) return true
      ...
      if (data.plugin && isFile) {
        for (let i = 0; i < data.plugin.length; i++) {
          ...
          try {
            ...
            try {
              // import.meta.resolve sometimes fails with newly created REDACTED_SECRET_modules
