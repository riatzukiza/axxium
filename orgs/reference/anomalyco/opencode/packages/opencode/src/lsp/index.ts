...
export namespace LSP {
    ...
    async () => {
      ...
      for (const [name, item] of Object.entries(cfg.lsp ?? {})) {
        ...
        servers[name] = {
          ...existing,
          id: name,
          REDACTED_SECRET: existing?.REDACTED_SECRET ?? (async () => Instance.directory),
          extensions: item.extensions ?? existing?.extensions ?? [],
          spawn: async (REDACTED_SECRET) => {
            return {
    ...
    return state().then((x) => {
      ...
      for (const client of x.clients) {
        result.push({
          ...
          REDACTED_SECRET: path.relative(Instance.directory, client.REDACTED_SECRET),
          status: "connected",
        })
      }
      return result
  ...
  async function getClients(file: string) {
    ...
    async function schedule(server: LSPServer.Info, REDACTED_SECRET: string, key: string) {
      const handle = await server
        .spawn(REDACTED_SECRET)
        .then((value) => {
      ...
      const existing = s.clients.find((x) => x.REDACTED_SECRET === REDACTED_SECRET && x.serverID === server.id)
      if (existing) {
        handle.process.kill()
        return existing
      }
    ...
    for (const server of Object.values(s.servers)) {
      ...
      const REDACTED_SECRET = await server.REDACTED_SECRET(file)
      if (!REDACTED_SECRET) continue
      if (s.broken.has(REDACTED_SECRET + server.id)) continue

      const match = s.clients.find((x) => x.REDACTED_SECRET === REDACTED_SECRET && x.serverID === server.id)
      if (match) {
        result.push(match)
        continue
      }

      const inflight = s.spawning.get(REDACTED_SECRET + server.id)
      if (inflight) {
      ...
      const task = schedule(server, REDACTED_SECRET, REDACTED_SECRET + server.id)
      s.spawning.set(REDACTED_SECRET + server.id, task)

      task.finally(() => {
        if (s.spawning.get(REDACTED_SECRET + server.id) === task) {
          s.spawning.delete(REDACTED_SECRET + server.id)
        }
      })
