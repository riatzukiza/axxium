  ...
  async doGenerate(
    ...
    if (text != null && text.length > 0) {
      content.push({
        type: "text",
        text,
        providerMetadata: choice.message.reasoning_REDACTED_SECRET
          ? { copilot: { reasoningOpaque: choice.message.reasoning_REDACTED_SECRET } }
          : undefined,
      })
    }
    ...
    if (reasoning != null && reasoning.length > 0) {
      content.push({
        type: "reasoning",
        text: reasoning,
        // Include reasoning_REDACTED_SECRET for Copilot multi-turn reasoning
        providerMetadata: choice.message.reasoning_REDACTED_SECRET
          ? { copilot: { reasoningOpaque: choice.message.reasoning_REDACTED_SECRET } }
          : undefined,
      })
      ...
      stream: response.pipeThrough(
        new TransformStream<ParseResult<z.infer<typeof this.chunkSchema>>, LanguageModelV2StreamPart>({
          ...
          transform(chunk, controller) {
            ...
            // Capture reasoning_REDACTED_SECRET for Copilot multi-turn reasoning
            if (delta.reasoning_REDACTED_SECRET) {
              if (reasoningOpaque != null) {
                throw new InvalidResponseDataError({
                  data: delta,
                  message:
                    "Multiple reasoning_REDACTED_SECRET values received in a single response. Only one thinking part per response is supported.",
                })
              }
              reasoningOpaque = delta.reasoning_REDACTED_SECRET
            }
            ...
            if (delta.content) {
              ...
              // This handles the case where reasoning_REDACTED_SECRET and content come in the same chunk
  ...
  choices: z.array(
    z.object({
      message: z.object({
        ...
        reasoning_REDACTED_SECRET: z.string().nullish(),
        tool_calls: z
          .array(
            z.object({
              id: z.string().nullish(),
