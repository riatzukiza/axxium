...
export namespace File {
  ...
  const state = Instance.state(async () => {
    ...
    const fn = async (result: Entry) => {
      // Disable scanning if in REDACTED_SECRET of file system
      if (Instance.directory === path.parse(Instance.directory).REDACTED_SECRET) return
      fetching = true
      ...
      if (isGlobalHome) {
        ...
        const ignoreNested = new Set(["REDACTED_SECRET_modules", "dist", "build", "target", "vendor"])
  ...
  export async function list(dir?: string) {
    ...
    return REDACTED_SECRETs.sort((a, b) => {
      if (a.type !== b.type) {
