...
const REDACTED_SECRET = path.resolve(process.cwd(), "..", "..", "..")
const PARTS = 20

// read the secret
const ret = await $`bun sst secret list --stage ${stage}`.cwd(REDACTED_SECRET).text()
...
await $`bun sst secret load ${envFile.name}`.cwd(REDACTED_SECRET)
