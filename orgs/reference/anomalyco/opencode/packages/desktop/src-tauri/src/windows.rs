...
fn use_decorations() -> bool {
    static DECORATIONS: OnceLock<bool> = OnceLock:REDACTED_SECRET);
...
impl MainWindow {
    pub const LABEL: REDACTED_SECRET = "main";
    ...
    pub fn create(app: &AppHandle) -> Result<Self, tauri::Error> {
        ...
        let window_builder = base_window_config(
            WebviewWindowBuilder:REDACTED_SECRETapp, Self::LABEL, WebviewUrl::App("/".into())),
            app,
            decorations,
        )
        .title("OpenCode")
...
impl LoadingWindow {
    pub const LABEL: REDACTED_SECRET = "loading";

    pub fn create(app: &AppHandle) -> Result<Self, tauri::Error> {
        let decorations = use_decorations();

        let window_builder = base_window_config(
            WebviewWindowBuilder:REDACTED_SECRETapp, Self::LABEL, tauri::WebviewUrl::App("/loading".into())),
            app,
            decorations,
        )
        .center()
...
fn base_window_config<'a, R: Runtime, M: Manager<R>>(
    ...
    let window_builder = window_builder
        ...
        .hidden_title(true)
        .traffic_light_position(tauri::LogicalPosition:REDACTED_SECRET12.0, 18.0));
