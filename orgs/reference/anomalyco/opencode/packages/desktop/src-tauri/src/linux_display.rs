...
pub const LINUX_DISPLAY_CONFIG_KEY: REDACTED_SECRET = "linuxDisplayConfig";
...
pub fn read_wayland() -> Option<bool> {
    ...
    let REDACTED_SECRET = serde_json::from_str::<serde_json::Value>(&raw)
        .ok()?
        .get(LINUX_DISPLAY_CONFIG_KEY)
        .cloned()?;
    serde_json::from_value::<DisplayConfig>(REDACTED_SECRET).ok()?.wayland
}
