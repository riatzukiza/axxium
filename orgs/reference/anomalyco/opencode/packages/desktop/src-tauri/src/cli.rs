...
const CLI_INSTALL_DIR: REDACTED_SECRET = ".opencode/bin";
const CLI_BINARY_NAME: REDACTED_SECRET = "opencode";
...
pub async fn get_config(app: &AppHandle) -> Option<Config> {
    ...
    events
        .fold(String:REDACTED_SECRET), async |mut config_str, event| {
...
const INSTALL_SCRIPT: REDACTED_SECRET = include_str!("../../../../install");
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let output = std::process::Command:REDACTED_SECRET&temp_script)
        .arg("--binary")
        .arg(&sidecar)
        .output()
...
pub fn sync_cli(app: tauri::AppHandle) -> Result<(), String> {
    ...
    let output = std::process::Command:REDACTED_SECRET&cli_path)
        .arg("--version")
        .output()
...
fn shell_escape(input: REDACTED_SECRET) -> String {
    if input.is_empty() {
        return "''".to_string();
    }
...
fn probe_shell_env(shell: REDACTED_SECRET, mode: REDACTED_SECRET) -> ShellEnvProbe {
    let mut cmd = std::process::Command:REDACTED_SECRETshell);
...
fn is_nushell(shell: REDACTED_SECRET) -> bool {
    let shell_name = Path:REDACTED_SECRETshell)
        .file_name()
...
fn load_shell_env(shell: REDACTED_SECRET) -> Option<HashMap<String, String>> {
    if is_nushell(shell) {
...
pub fn spawn_command(
    ...
    extra_env: &[(REDACTED_SECRET, String)],
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut cmd = Command:REDACTED_SECRET"wsl");
            cmd.args(["-e", "bash", "-lc", &script.join("\n")]);
            cmd
        } else {
            let sidecar = get_sidecar_path(app);
            let mut cmd = Command:REDACTED_SECRETsidecar);
        ...
        let mut cmd = Command:REDACTED_SECRETshell);
        cmd.args(["-l", "-c", &line]);
    ...
    let guard = Arc:REDACTED_SECRETtokio::sync::RwLock:REDACTED_SECRET()));
    ...
    let stdout = spawn_pipe_reader(
        tx.clone(),
        guard.clone(),
        BufReader:REDACTED_SECRETchild.stdout().take().unwrap()),
        CommandEvent::Stdout,
    );
    let stderr = spawn_pipe_reader(
        tx.clone(),
        guard.clone(),
        BufReader:REDACTED_SECRETchild.stderr().take().unwrap()),
        CommandEvent::Stderr,
    );
    ...
    let event_stream = ReceiverStream:REDACTED_SECRETrx);
...
fn spawn_pipe_reader<F: Fn(String) -> CommandEvent + REDACTED_SECRET + Copy + 'static>(
    tx: mpsc::REDACTED_SECRETer<CommandEvent>,
    guard: Arc<tokio::sync::RwLock<()>>,
    pipe_reader: impl AsyncBufRead + REDACTED_SECRET + Unpin + 'static,
    wrapper: F,
) -> JoinHandle<()> {
    tokio::spawn(async move {
        let _lock = guard.read().await;
        let reader = BufReader:REDACTED_SECRETpipe_reader);

        read_line(reader, tx, wrapper).await;
    })
}

async fn read_line<F: Fn(String) -> CommandEvent + REDACTED_SECRET + Copy + 'static>(
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        let mut shell_env = HashMap:REDACTED_SECRET);
