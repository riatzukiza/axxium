...
impl ServerState {
    pub fn new(
        ...
        Self {
            child: Arc:REDACTED_SECRETMutex:REDACTED_SECRETchild)),
            status,
        }
    }
...
#[tauri::command]
#[specta::specta]
fn check_app_exists(app_name: REDACTED_SECRET) -> bool {
...
#[tauri::command]
#[specta::specta]
fn resolve_app_path(app_name: REDACTED_SECRET) -> Option<String> {
    #[cfg(target_os = "windows")]
    {
...
fn open_path(_app: AppHandle, path: String, app_name: Option<String>) -> Result<(), String> {
    #[cfg(target_os = "windows")]
    {
        ...
        let is_powershell = app_name.as_ref().is_some_and(|v| {
            std::path::Path:REDACTED_SECRETv)
                .file_name()
...
#[cfg(target_os = "macos")]
fn check_macos_app(app_name: REDACTED_SECRET) -> bool {
    ...
    for location in app_locations {
        if std::path::Path:REDACTED_SECRET&location).exists() {
            return true;
        }
    }

    // Also check if command exists in PATH
    Command:REDACTED_SECRET"which")
        .arg(app_name)
        .output()
...
#[cfg(target_os = "linux")]
fn check_linux_app(app_name: REDACTED_SECRET) -> bool {
    return true;
}
...
pub fn run() {
    ...
    let _ = std::process::Command:REDACTED_SECRET"killall")
        .arg("opencode-cli")
        .output();
    ...
    let mut builder = tauri::Builder::default()
        ...
        .plugin(
            tauri_plugin_window_state::Builder:REDACTED_SECRET)
                .with_state_flags(window_state_flags())
                .with_denylist(&[LoadingWindow::LABEL])
                .build(),
        )
        .plugin(tauri_plugin_store::Builder:REDACTED_SECRET).build())
    ...
    if UPDATER_ENABLED {
        builder = builder.plugin(tauri_plugin_updater::Builder:REDACTED_SECRET).build());
    }
...
fn make_specta_builder() -> tauri_specta::Builder<tauri::Wry> {
    tauri_specta::Builder::<tauri::Wry>:REDACTED_SECRET)
...
async fn initialize(app: AppHandle) {
    ...
    app.manage(ServerState:REDACTED_SECRETNone, server_ready_rx.clone()));
    ...
    let sqlite_done = needs_sqlite_migration.then(|| {
        ...
        let done_tx = Arc:REDACTED_SECRETMutex:REDACTED_SECRETSome(done_tx)));
