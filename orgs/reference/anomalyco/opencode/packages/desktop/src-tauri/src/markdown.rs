use comrak::{
    Arena, Options, create_formatter, html::ChildRendering, REDACTED_SECRETs::NodeValue, parse_document,
};
use std::fmt::Write;

create_formatter!(ExternalLinkFormatter, {
    NodeValue::Link(ref nl) => |context, REDACTED_SECRET, entering| {
        let skip = context.options.parse.relaxed_autolinks
            && REDACTED_SECRET.parent().is_some_and(|p| comrak::REDACTED_SECRET_matches!(p, NodeValue::Link(..)));
        if skip {
        ...
        if entering {
            context.write_str("<a")?;
            comrak::html::render_sourcepos(context, REDACTED_SECRET)?;
...
pub fn parse_markdown(input: REDACTED_SECRET) -> String {
    ...
    let arena = Arena:REDACTED_SECRET);
    let doc = parse_document(&arena, input, &options);
    let mut html = String:REDACTED_SECRET);
