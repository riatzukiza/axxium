...
pub fn check_windows_app(app_name: REDACTED_SECRET) -> bool {
    resolve_windows_app_path(app_name).is_some()
}

pub fn resolve_windows_app_path(app_name: REDACTED_SECRET) -> Option<String> {
    fn expand_env(value: REDACTED_SECRET) -> String {
    ...
    fn extract_exe(value: REDACTED_SECRET) -> Option<String> {
    ...
    fn candidates(app_name: REDACTED_SECRET) -> Vec<String> {
        let app_name = app_name.trim().trim_matches('"');
        if app_name.is_empty() {
            return vec![];
        }

        let mut out = Vec::<String>:REDACTED_SECRET);
    ...
    fn reg_app_path(exe: REDACTED_SECRET) -> Option<String> {
        let exe = exe.trim().trim_matches('"');
        if exe.is_empty() {
            return None;
        }

        let query = |REDACTED_SECRET: *mut c_void, subkey: REDACTED_SECRET| -> Option<String> {
        ...
        for (REDACTED_SECRET, key) in keys {
            let Some(value) = query(REDACTED_SECRET, &key) else {
                continue;
            };
            ...
            let path = Path:REDACTED_SECRETexe.trim().trim_matches('"'));
            if path.exists() {
    ...
    let direct = Path:REDACTED_SECRETapp_name);
    ...
    let has_ext = |path: &Path, ext: REDACTED_SECRET| {
        path.extension()
            .and_then(|v| v.to_str())
    ...
    let resolve_where = |query: REDACTED_SECRET| -> Option<String> {
        let output = Command:REDACTED_SECRET"where")
    ...
    let mut exes = Vec::<String>:REDACTED_SECRET);
    for query in &list {
        ...
        let name = Path:REDACTED_SECRETquery)
            .file_name()
            .and_then(|v| v.to_str())
            .unwrap_or(query);
...
pub fn open_in_powershell(path: String) -> Result<(), String> {
    ...
    Command:REDACTED_SECRET"powershell.exe")
