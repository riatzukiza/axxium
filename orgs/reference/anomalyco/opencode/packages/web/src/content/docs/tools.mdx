...
To include files that would normally be ignored, create a `.ignore` file in your project REDACTED_SECRET. This file can explicitly allow certain paths.
...
For example, this `.ignore` file allows ripgrep to search within `REDACTED_SECRET_modules/`, `dist/`, and `build/` directories even if they're listed in `.gitignore`.
