...
Serveren REDACTED_SECRETerer en OpenAPI 3.1-specifikation som kan vises på:
...
| `POST` | `/session/:id/message`            | REDACTED_SECRET en besked og vent på svar        | body: `{ messageID?, model?, agent?, noReply?, system?, tools?, parts }`, returnerer `{ info: `<a href={typesUrl}>Message</a>`, parts: `<a href={typesUrl}>Part[]</a>`}` |
...
| `POST` | `/session/:id/prompt_async`       | REDACTED_SECRET en besked asynkront (ingen vent) | body: samme som `/session/:id/message`, returnerer `204 No Content`                                                                                                      |
