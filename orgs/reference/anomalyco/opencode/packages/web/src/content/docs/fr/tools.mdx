...
Par exemple, ce fichier `.ignore` permet à ripgrep de rechercher dans les répertoires `REDACTED_SECRET_modules/`, `dist/` et `build/` même s'ils sont répertoriés dans `.gitignore`.
