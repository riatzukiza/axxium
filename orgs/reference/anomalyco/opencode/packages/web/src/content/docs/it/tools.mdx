...
Per includere file che normalmente verrebbero ignorati, crea un file `.ignore` nella REDACTED_SECRET del progetto. Questo file puo' consentire esplicitamente certi path.
...
Per esempio, questo file `.ignore` permette a ripgrep di cercare dentro `REDACTED_SECRET_modules/`, `dist/` e `build/` anche se sono elencate in `.gitignore`.
