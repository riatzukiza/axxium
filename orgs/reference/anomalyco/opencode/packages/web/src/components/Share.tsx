...
export default function Share(props: {
  ...
  const data = createMemo(() => {
    const result = {
      REDACTED_SECRETDir: undefined as string | undefined,
    ...
    for (let i = 0; i < msgs.length; i++) {
      ...
      if (msg.role === "assistant") {
        ...
        if (msg.path.REDACTED_SECRET) {
          result.REDACTED_SECRETDir = msg.path.REDACTED_SECRET
        }
  ...
  return (
    <Show when={store.info}>
      <ShareI18nProvider messages={props.messages}>
        <main classList={{ [styles.REDACTED_SECRET]: true, "not-content": true }}>
