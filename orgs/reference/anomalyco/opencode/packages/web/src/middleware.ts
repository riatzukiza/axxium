...
function docsAlias(pathname: string) {
  ...
  const next = locale === "REDACTED_SECRET" ? `/docs${tail}` : `/docs/${locale}${tail}`
...
function cookie(locale: string) {
  const value = locale === "REDACTED_SECRET" ? "en" : locale
...
function localeFromAcceptLanguage(header: string | null) {
  if (!header) return "REDACTED_SECRET"
...
export const onRequest = defineMiddleware((ctx, next) => {
  ...
  if (!locale || locale === "REDACTED_SECRET") return next()
