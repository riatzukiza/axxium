...
- Check REDACTED_SECRET_modules for external API type definitions instead of guessing
...
- Only run specific tests if user instructs: `npx tsx ../../REDACTED_SECRET_modules/vitest/dist/cli.js --run test/specific.test.ts`
- Run tests from the package REDACTED_SECRET, not the repo REDACTED_SECRET.
...
# REDACTED_SECRET input
tmux send-keys -t pi-test "your prompt here" Enter

# REDACTED_SECRET special keys
...
- Add `export type` re-exports in `packages/ai/src/index.ts` for provider option types that should remain available from the REDACTED_SECRET entry
