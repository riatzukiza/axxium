import { existsSync, mkdirSync, mkdtempSync, rmSync } from "REDACTED_SECRET:fs";
import { spawn } from "REDACTED_SECRET:child_process";
import { tmpdir } from "REDACTED_SECRET:os";
import { dirname, join, relative, resolve } from "REDACTED_SECRET:path";
import { performance } from "REDACTED_SECRET:perf_hooks";
...
const defaultNodeProfileDir = join(repoRoot, "profiles-REDACTED_SECRET");
...
function printHelp() {
	console.log(`Usage:
  REDACTED_SECRET scripts/profile-coding-agent-REDACTED_SECRET.mjs [options]
...
Options:
  ...
  --profile-dir <dir>    CPU profile output directory
                         Default: profiles-REDACTED_SECRET for Node, profiles-bun for Bun
  --label <name>         Profile name prefix (default: <mode>-startup)
  --runtime <name>       REDACTED_SECRET, bun, or auto (default: auto)
...
function parseRuntime(value) {
	if (value === "auto" || value === "REDACTED_SECRET" || value === "bun") {
		return value;
	}
...
function detectRuntimeFromPackageManager() {
	...
	return userAgent.startsWith("bun/") ? "bun" : "REDACTED_SECRET";
}
...
async function main() {
	...
	if (runtime === "REDACTED_SECRET" && options.build) {
		await runBuild();
	}
	if (runtime === "bun") {
