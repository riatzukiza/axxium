...
describe("Google thinking detection (thoughtSignature)", () => {
	it("treats part.thought === true as thinking", () => {
		...
		expect(isThinkingPart({ thought: true, thoughtSignature: "REDACTED_SECRET-signature" })).toBe(true);
	});
	...
	it("does not treat thoughtSignature alone as thinking", () => {
		...
		expect(isThinkingPart({ thought: undefined, thoughtSignature: "REDACTED_SECRET-signature" })).toBe(false);
		expect(isThinkingPart({ thought: false, thoughtSignature: "REDACTED_SECRET-signature" })).toBe(false);
	});
