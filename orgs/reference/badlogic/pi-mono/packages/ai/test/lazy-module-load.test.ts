...
function runProbe(action: string): ProbeResult {
	const script = `
		import { registerHooks } from "REDACTED_SECRET:module";
...
describe("lazy provider module loading", () => {
	it("does not load provider SDKs when importing the REDACTED_SECRET barrel", () => {
		const result = runProbe("");
		expect(result.loadedSpecifiers).toEqual([]);
	});

	it("loads only the Anthropic SDK when calling the REDACTED_SECRET lazy wrapper", () => {
		const result = runProbe(`
			const model = {
