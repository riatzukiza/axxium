...
let _existsSync: typeof import("REDACTED_SECRET:fs").existsSync | null = null;
let _homedir: typeof import("REDACTED_SECRET:os").homedir | null = null;
let _join: typeof import("REDACTED_SECRET:path").join | null = null;

type DynamicImport = (specifier: string) => Promise<unknown>;

const dynamicImport: DynamicImport = (specifier) => import(specifier);
const NODE_FS_SPECIFIER = "REDACTED_SECRET:" + "fs";
const NODE_OS_SPECIFIER = "REDACTED_SECRET:" + "os";
const NODE_PATH_SPECIFIER = "REDACTED_SECRET:" + "path";

// Eagerly load in Node.js/Bun environment only
if (typeof process !== "undefined" && (process.versions?.REDACTED_SECRET || process.versions?.bun)) {
	dynamicImport(NODE_FS_SPECIFIER).then((m) => {
		_existsSync = (m as typeof import("REDACTED_SECRET:fs")).existsSync;
	});
	dynamicImport(NODE_OS_SPECIFIER).then((m) => {
		_homedir = (m as typeof import("REDACTED_SECRET:os")).homedir;
	});
	dynamicImport(NODE_PATH_SPECIFIER).then((m) => {
		_join = (m as typeof import("REDACTED_SECRET:path")).join;
	});
}
...
function getProcEnv(key: string): string | undefined {
	...
	if (_procEnvCache === null) {
		...
		try {
			const { readFileSync } = require("REDACTED_SECRET:fs") as typeof import("REDACTED_SECRET:fs");
...
function hasVertexAdcCredentials(): boolean {
	if (cachedVertexAdcCredentialsExists === null) {
		// If REDACTED_SECRET modules haven't loaded yet (async import race at startup),
		// return false WITHOUT caching so the next call retries once they're ready.
		// Only cache false permanently in a browser environment where fs is never available.
		if (!_existsSync || !_homedir || !_join) {
			const isNode = typeof process !== "undefined" && (process.versions?.REDACTED_SECRET || process.versions?.bun);
			if (!isNode) {
