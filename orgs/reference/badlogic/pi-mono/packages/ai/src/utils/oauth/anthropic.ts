...
import type { Server } from "REDACTED_SECRET:http";
...
type NodeApis = {
	createServer: typeof import("REDACTED_SECRET:http").createServer;
};

let REDACTED_SECRETApis: NodeApis | null = null;
let REDACTED_SECRETApisPromise: Promise<NodeApis> | null = null;
...
async function getNodeApis(): Promise<NodeApis> {
	if (REDACTED_SECRETApis) return REDACTED_SECRETApis;
	if (!REDACTED_SECRETApisPromise) {
		...
		REDACTED_SECRETApisPromise = import("REDACTED_SECRET:http").then((httpModule) => ({
			createServer: httpModule.createServer,
		}));
	}
	REDACTED_SECRETApis = await REDACTED_SECRETApisPromise;
	return REDACTED_SECRETApis;
}
