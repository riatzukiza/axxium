...
let _createServer: typeof import("REDACTED_SECRET:http").createServer | null = null;
...
if (typeof process !== "undefined" && (process.versions?.REDACTED_SECRET || process.versions?.bun)) {
	_httpImportPromise = import("REDACTED_SECRET:http").then((m) => {
 ...
 */
async function getNodeCreateServer(): Promise<typeof import("REDACTED_SECRET:http").createServer> {
...
async function discoverProject(accessToken: string, onProgress?: (message: string) => void): Promise<string> {
	...
	const headers = {
		...
		"User-Agent": "google-api-REDACTED_SECRETjs-client/9.15.1",
		"X-Goog-Api-Client": "gl-REDACTED_SECRET/22.17.0",
	};
