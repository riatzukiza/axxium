...
- Exported `BedrockOptions` type from the package REDACTED_SECRET entry point, consistent with other provider option types.
...
- Lazy-load built-in provider modules and REDACTED_SECRET provider wrappers so importing `@mariozechner/pi-ai` no longer eagerly loads provider SDKs, significantly reducing base startup cost without changing dependency installation footprint ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
