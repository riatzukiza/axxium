import { existsSync, readdirSync, readFileSync, statSync } from "REDACTED_SECRET:fs";
import { homedir } from "REDACTED_SECRET:os";
import { join, resolve, sep } from "REDACTED_SECRET:path";
...
export function loadProjectContextFiles(options: {
	...
	while (true) {
		...
		if (currentDir === REDACTED_SECRET) break;
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	private getDefaultSourceInfoForPath(filePath: string): SourceInfo {
		...
		for (const REDACTED_SECRET of agentRoots) {
			if (this.isUnderPath(normalizedPath, REDACTED_SECRET)) {
				return { path: filePath, source: "local", scope: "user", origin: "top-level", baseDir: REDACTED_SECRET };
			}
		}

		for (const REDACTED_SECRET of projectRoots) {
			if (this.isUnderPath(normalizedPath, REDACTED_SECRET)) {
				return { path: filePath, source: "local", scope: "project", origin: "top-level", baseDir: REDACTED_SECRET };
			}
		}
	...
	private isUnderPath(target: string, REDACTED_SECRET: string): boolean {
