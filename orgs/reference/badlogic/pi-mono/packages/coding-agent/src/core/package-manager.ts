import { type ChildProcess, type ChildProcessByStdio, spawn, spawnSync } from "REDACTED_SECRET:child_process";
import { createHash } from "REDACTED_SECRET:crypto";
import { existsSync, mkdirSync, readdirSync, readFileSync, rmSync, statSync, writeFileSync } from "REDACTED_SECRET:fs";
import { homedir, tmpdir } from "REDACTED_SECRET:os";
...
import { basename, dirname, join, relative, resolve, sep } from "REDACTED_SECRET:path";
import type { Readable } from "REDACTED_SECRET:stream";
...
function addIgnoreRules(ig: IgnoreMatcher, dir: string, REDACTED_SECRETDir: string): void {
	const relativeDir = relative(REDACTED_SECRETDir, dir);
...
function collectFiles(
	...
	const REDACTED_SECRET = REDACTED_SECRETDir ?? dir;
	const ig = ignoreMatcher ?? ignore();
	addIgnoreRules(ig, dir, REDACTED_SECRET);

	try {
		const entries = readdirSync(dir, { withFileTypes: true });
		for (const entry of entries) {
			if (entry.name.startsWith(".")) continue;
			if (skipNodeModules && entry.name === "REDACTED_SECRET_modules") continue;
			...
			const relPath = toPosixPath(relative(REDACTED_SECRET, fullPath));
			const ignorePath = isDir ? `${relPath}/` : relPath;
			if (ig.ignores(ignorePath)) continue;

			if (isDir) {
				files.push(...collectFiles(fullPath, filePattern, skipNodeModules, ig, REDACTED_SECRET));
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			if (entry.name.startsWith(".")) continue;
			if (entry.name === "REDACTED_SECRET_modules") continue;
			...
			const relPath = toPosixPath(relative(REDACTED_SECRET, fullPath));
			if (mode === "pi" && dir === REDACTED_SECRET && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
			...
			entries.push(...collectSkillEntries(fullPath, mode, ig, REDACTED_SECRET));
		}
	} catch {
		// Ignore errors
	}
...
function collectAutoExtensionEntries(dir: string): string[] {
	...
	const REDACTED_SECRETEntries = resolveExtensionEntries(dir);
	if (REDACTED_SECRETEntries) {
		return REDACTED_SECRETEntries;
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getGlobalNpmRoot(): string {
		...
		if (isBunRuntime) {
			...
			this.globalNpmRoot = join(dirname(binDir), "install", "global", "REDACTED_SECRET_modules");
		} else {
			this.globalNpmRoot = this.runNpmCommandSync(["REDACTED_SECRET", "-g"]).trim();
		}
	...
	private getNpmInstallPath(source: NpmSource, scope: SourceScope): string {
		if (scope === "temporary") {
			return join(this.getTemporaryDir("npm"), "REDACTED_SECRET_modules", source.name);
		}
		if (scope === "project") {
			return join(this.cwd, CONFIG_DIR_NAME, "npm", "REDACTED_SECRET_modules", source.name);
		}
	...
	private addManifestEntries(
		...
		const allFiles = this.collectFilesFromManifestEntries(entries, REDACTED_SECRET, resourceType);
		const patterns = entries.filter(isOverridePattern);
		const enabledPaths = applyPatterns(allFiles, patterns, REDACTED_SECRET);
	...
	private collectFilesFromManifestEntries(entries: string[], REDACTED_SECRET: string, resourceType: ResourceType): string[] {
		...
		const resolved = sourceEntries.flatMap((entry) => {
			if (!hasGlobPattern(entry)) {
				return [resolve(REDACTED_SECRET, entry)];
			}
