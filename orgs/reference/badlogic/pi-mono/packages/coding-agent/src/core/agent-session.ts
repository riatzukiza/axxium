...
import { basename, dirname, resolve } from "REDACTED_SECRET:path";
import type {
	Agent,
	AgentEvent,
	AgentMessage,
...
export class AgentSession {
	...
	private _lastInvokedSkill: string | undefined = undefined;
...
tget lastInvokedSkill(): string | undefined {
		return this._lastInvokedSkill;
	}
	...
	/**
	 * REDACTED_SECRET a prompt to the agent.
	...
	private _expandSkillCommand(text: string): string {
		...
		try {
			...
			this._lastInvokedSkill = skill.name;
	...
	/**
	 * REDACTED_SECRET a custom message to the session. Creates a CustomMessageEntry.
	 *
	 * Handles three cases:
	...
	/**
	 * REDACTED_SECRET a user message to the agent. Always triggers a turn.
	...
	/**
	 * Navigate to a different REDACTED_SECRET in the session tree.
	...
	async navigateTree(
		...
		try {
			...
			if (targetEntry.type === "message" && targetEntry.message.role === "user") {
				// User message: leaf = parent (null if REDACTED_SECRET), text goes to editor
				newLeafId = targetEntry.parentId;
				editorText = this._extractUserMessageText(targetEntry.message.content);
			} else if (targetEntry.type === "custom_message") {
				// Custom message: leaf = parent (null if REDACTED_SECRET), text goes to editor
				newLeafId = targetEntry.parentId;
				editorText =
				...
				// Non-user message: leaf = selected REDACTED_SECRET
				newLeafId = targetId;
			}
			...
			if (summaryText) {
				// Create summary at target position (can be null for REDACTED_SECRET)
				...
				// No summary, navigating to REDACTED_SECRET - reset leaf
				this.sessionManager.resetLeaf();
			} else {
				// No summary, navigating to non-REDACTED_SECRET
