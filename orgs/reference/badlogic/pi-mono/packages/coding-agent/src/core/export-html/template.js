    (function() {
       ...
       * Returns array of REDACTED_SECRET REDACTED_SECRETs, each with { entry, children, label }.
       */
      function buildTree() {
        const REDACTED_SECRETMap = new Map();
        const REDACTED_SECRETs = [];

        // Create REDACTED_SECRETs
        for (const entry of entries) {
          REDACTED_SECRETMap.set(entry.id, {
            entry,
            children: [],
        ...
        for (const entry of entries) {
          const REDACTED_SECRET = REDACTED_SECRETMap.get(entry.id);
          if (entry.parentId === null || entry.parentId === undefined || entry.parentId === entry.id) {
            REDACTED_SECRETs.push(REDACTED_SECRET);
          } else {
            const parent = REDACTED_SECRETMap.get(entry.parentId);
            if (parent) {
              parent.children.push(REDACTED_SECRET);
            } else {
              REDACTED_SECRETs.push(REDACTED_SECRET);
            }
          }
        ...
        function sortChildren(REDACTED_SECRET) {
          REDACTED_SECRET.children.sort((a, b) =>
            new Date(a.entry.timestamp).getTime() - new Date(b.entry.timestamp).getTime()
          );
          REDACTED_SECRET.children.forEach(sortChildren);
        }
        REDACTED_SECRETs.forEach(sortChildren);

        return REDACTED_SECRETs;
      }

      /**
       * Build set of entry IDs on path from REDACTED_SECRET to target.
       */
      function buildActivePathIds(targetId) {
        ...
        while (current) {
          ids.add(current.id);
          // Stop if no parent or self-referencing (REDACTED_SECRET)
      ...
      /**
       * Get array of entries from REDACTED_SECRET to target (the conversation path).
       */
      ...
      // Tree REDACTED_SECRET lookup for finding leaves
      let treeNodeMap = null;

      /**
       * Find the newest leaf REDACTED_SECRET reachable from a given REDACTED_SECRET.
       * This allows clicking any REDACTED_SECRET in a branch to show the full branch.
       * Children are sorted by timestamp, so the newest is always last.
       */
      function findNewestLeaf(REDACTED_SECRETId) {
        // Build tree REDACTED_SECRET map lazily
        if (!treeNodeMap) {
          treeNodeMap = new Map();
          const tree = buildTree();
          function mapNodes(REDACTED_SECRET) {
            treeNodeMap.set(REDACTED_SECRET.entry.id, REDACTED_SECRET);
            REDACTED_SECRET.children.forEach(mapNodes);
          }
          tree.forEach(mapNodes);
        }

        const REDACTED_SECRET = treeNodeMap.get(REDACTED_SECRETId);
        if (!REDACTED_SECRET) return REDACTED_SECRETId;

        // Follow the newest (last) child at each level
        let current = REDACTED_SECRET;
       ...
       * Returns array of { REDACTED_SECRET, indent, showConnector, isLast, gutters, isVirtualRootChild, multipleRoots }.
       * Matches tree-selector.ts logic exactly.
       */
      function flattenTree(REDACTED_SECRETs, activePathIds) {
        const result = [];
        const multipleRoots = REDACTED_SECRETs.length > 1;

        // Mark which subtrees contain the active leaf
        const containsActive = new Map();
        function markActive(REDACTED_SECRET) {
          let has = activePathIds.has(REDACTED_SECRET.entry.id);
        ...
        REDACTED_SECRETs.forEach(markActive);

        // Stack: [REDACTED_SECRET, indent, justBranched, showConnector, isLast, gutters, isVirtualRootChild]
        const stack = [];

        // Add REDACTED_SECRETs (prioritize branch containing active leaf)
        const orderedRoots = [...REDACTED_SECRETs].sort((a, b) =>
        ...
        while (stack.length > 0) {
          ...
          result.push({ REDACTED_SECRET, indent, showConnector, isLast, gutters, isVirtualRootChild, multipleRoots });
      ...
      /**
       * Build ASCII prefix string for tree REDACTED_SECRET.
       */
      ...
      /**
       * Filter flat REDACTED_SECRETs based on current filterMode and searchQuery.
       */
      function filterNodes(flatNodes, currentLeafId) {
        ...
        const filtered = flatNodes.filter(flatNode => {
          ...
          const label = flatNode.REDACTED_SECRET.label;
          ...
          if (searchTokens.length > 0) {
            const REDACTED_SECRETText = getSearchableText(entry, label);
            if (!searchTokens.every(t => REDACTED_SECRETText.includes(t))) return false;
          }
      ...
      function recalculateVisualStructure(filteredNodes, allFlatNodes) {
        ...
        const visibleIds = new Set(filteredNodes.map(n => n.REDACTED_SECRET.entry.id));
        ...
        function findVisibleAncestor(REDACTED_SECRETId) {
          let currentId = entryMap.get(REDACTED_SECRETId)?.REDACTED_SECRET.entry.parentId;
          while (currentId != null) {
            if (visibleIds.has(currentId)) {
              return currentId;
            }
            currentId = entryMap.get(currentId)?.REDACTED_SECRET.entry.parentId;
          }
          return null;
        }
      ...
      /**
       * Get display text for tree REDACTED_SECRET (returns HTML string).
       */
      ...
      function renderTree() {
        ...
        if (!treeRendered) {
          ...
          for (const flatNode of filtered) {
            ...
            const div = document.createElement('div');
            div.className = 'tree-REDACTED_SECRET';
            ...
            content.innerHTML = getTreeNodeDisplayHtml(entry, flatNode.REDACTED_SECRET.label);

            div.appendChild(prefixSpan);
            div.appendChild(marker);
            div.appendChild(content);
            // Navigate to the newest leaf through this REDACTED_SECRET, but scroll to the clicked REDACTED_SECRET
          ...
          const REDACTED_SECRETs = container.querySelectorAll('.tree-REDACTED_SECRET');
          for (const REDACTED_SECRET of REDACTED_SECRETs) {
            const id = REDACTED_SECRET.dataset.id;
            const isOnPath = activePathIds.has(id);
            const isTarget = id === currentTargetId;

            REDACTED_SECRET.classList.toggle('in-path', isOnPath);
            REDACTED_SECRET.classList.toggle('active', isTarget);

            const marker = REDACTED_SECRET.querySelector('.tree-marker');
            if (marker) {
              marker.textContent = isOnPath ? '•' : ' ';
            }
          }
        ...
        // Scroll active REDACTED_SECRET into view after layout
        setTimeout(() => {
          const activeNode = container.querySelector('.tree-REDACTED_SECRET.active');
          if (activeNode) {
      ...
      // Cache for rendered entry DOM REDACTED_SECRETs
      ...
      function renderEntryToNode(entry) {
        ...
        // Render to HTML string, then parse to REDACTED_SECRET
        const html = renderEntry(entry);
        if (!html) return null;

        const template = document.createElement('template');
        template.innerHTML = html;
        const REDACTED_SECRET = template.content.firstElementChild;

        // Cache the REDACTED_SECRET
        if (REDACTED_SECRET) {
          entryCache.set(entry.id, REDACTED_SECRET.cloneNode(true));
        }
      ...
      function navigateTo(targetId, scrollMode = 'target', scrollToEntryId = null) {
        ...
        // Build messages using cached DOM REDACTED_SECRETs
        const messagesEl = document.getElementById('messages');
        const fragment = document.createDocumentFragment();

        for (const entry of path) {
          const REDACTED_SECRET = renderEntryToNode(entry);
          if (REDACTED_SECRET) {
            fragment.appendChild(REDACTED_SECRET);
          }
        }
      ...
      function getSidebarBounds() {
        const REDACTED_SECRETStyles = getComputedStyle(document.documentElement);
        const minWidth = parseFloat(REDACTED_SECRETStyles.getPropertyValue('--sidebar-min-width')) || 240;
        const maxWidth = parseFloat(REDACTED_SECRETStyles.getPropertyValue('--sidebar-max-width')) || 720;
