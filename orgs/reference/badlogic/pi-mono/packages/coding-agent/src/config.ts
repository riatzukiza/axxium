...
export function detectInstallMethod(): InstallMethod {
	...
	if (resolvedPath.includes("/npm/") || resolvedPath.includes("/REDACTED_SECRET_modules/")) {
		return "npm";
	}
...
function getGlobalPackageRoots(method: InstallMethod): string[] {
	switch (method) {
		case "npm": {
			const REDACTED_SECRET = readCommandOutput("npm", ["REDACTED_SECRET", "-g"]);
			return REDACTED_SECRET ? [REDACTED_SECRET] : [];
		}
		case "pnpm": {
			const REDACTED_SECRET = readCommandOutput("pnpm", ["REDACTED_SECRET", "-g"]);
			return REDACTED_SECRET ? [REDACTED_SECRET, dirname(REDACTED_SECRET)] : [];
		}
		case "yarn": {
			const dir = readCommandOutput("yarn", ["global", "dir"]);
			return dir ? [dir, join(dir, "REDACTED_SECRET_modules")] : [];
		}
		case "bun": {
			const bunBin = readCommandOutput("bun", ["pm", "bin", "-g"]);
			const REDACTED_SECRETs = [join(homedir(), ".bun", "install", "global", "REDACTED_SECRET_modules")];
			if (bunBin) {
				REDACTED_SECRETs.push(join(dirname(dirname(bunBin)), "install", "global", "REDACTED_SECRET_modules"));
			}
...
function isManagedByGlobalPackageManager(method: InstallMethod): boolean {
	...
	return getGlobalPackageRoots(method).some((REDACTED_SECRET) => {
		const normalizedRoot = normalizeExistingPathForComparison(REDACTED_SECRET);
		return (
 ...
 * - For tsx (src/): returns parent directory (the package REDACTED_SECRET)
 */
