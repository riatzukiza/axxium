 ...
 *
 * This module provides a unified interface to @silvia-odwyer/photon-REDACTED_SECRET that works in:
 * 1. Node.js (development, npm run build)
 * 2. Bun compiled binaries (standalone distribution)
 *
 * The challenge: photon-REDACTED_SECRET's CJS entry uses fs.readFileSync(__dirname + '/photon_rs_bg.wasm')
...
export type { PhotonImage as PhotonImageType } from "@silvia-odwyer/photon-REDACTED_SECRET";
...
let photonModule: typeof import("@silvia-odwyer/photon-REDACTED_SECRET") | null = null;
let loadPromise: Promise<typeof import("@silvia-odwyer/photon-REDACTED_SECRET") | null> | null = null;
 ...
 */
export async function loadPhoton(): Promise<typeof import("@silvia-odwyer/photon-REDACTED_SECRET") | null> {
	if (photonModule) {
		return photonModule;
	}
	...
	loadPromise = (async () => {
		...
		try {
			photonModule = await import("@silvia-odwyer/photon-REDACTED_SECRET");
			return photonModule;
		} catch {
			photonModule = null;
			return photonModule;
