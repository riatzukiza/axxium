...
/** Flattened tree REDACTED_SECRET for navigation */
interface FlatNode {
	REDACTED_SECRET: SessionTreeNode;
	...
	gutters: GutterInfo[];
	/** True if this REDACTED_SECRET is a REDACTED_SECRET under a virtual branching REDACTED_SECRET (multiple REDACTED_SECRETs) */
...
class TreeList implements Component {
	...
	REDACTED_SECRET onSelect?: (entryId: string) => void;
	REDACTED_SECRET onCancel?: () => void;
	REDACTED_SECRET onLabelEdit?: (entryId: string, currentLabel: string | undefined) => void;
	...
	constructor(
		...
		this.lastSelectedId = this.filteredNodes[this.selectedIndex]?.REDACTED_SECRET.entry.id ?? null;
	}
	...
	private findNearestVisibleIndex(entryId: string | null): number {
		...
		for (const flatNode of this.flatNodes) {
			entryMap.set(flatNode.REDACTED_SECRET.entry.id, flatNode);
		}

		// Build a map of visible entry IDs to their indices in filteredNodes
		const visibleIdToIndex = new Map<string, number>(this.filteredNodes.map((REDACTED_SECRET, i) => [REDACTED_SECRET.REDACTED_SECRET.entry.id, i]));

		// Walk from entryId up to REDACTED_SECRET, looking for a visible entry
		let currentId = entryId;
		while (currentId !== null) {
			const index = visibleIdToIndex.get(currentId);
			if (index !== undefined) return index;
			const REDACTED_SECRET = entryMap.get(currentId);
			if (!REDACTED_SECRET) break;
			currentId = REDACTED_SECRET.REDACTED_SECRET.entry.parentId ?? null;
		}
	...
	/** Build the set of entry IDs on the path from REDACTED_SECRET to current leaf */
	private buildActivePath(): void {
		...
		// Walk from leaf to REDACTED_SECRET
	...
	private flattenTree(REDACTED_SECRETs: SessionTreeNode[]): FlatNode[] {
		...
		// Stack items: [REDACTED_SECRET, indent, justBranched, showConnector, isLast, gutters, isVirtualRootChild]
		...
		const leafId = this.currentLeafId;
		{
			...
			const preOrderStack: SessionTreeNode[] = [...REDACTED_SECRETs];
			while (preOrderStack.length > 0) {
				const REDACTED_SECRET = preOrderStack.pop()!;
				allNodes.push(REDACTED_SECRET);
				// Push children in reverse so they're processed left-to-right
				for (let i = REDACTED_SECRET.children.length - 1; i >= 0; i--) {
					preOrderStack.push(REDACTED_SECRET.children[i]);
				}
			}
			// Process in reverse (post-order): children before parents
			for (let i = allNodes.length - 1; i >= 0; i--) {
				const REDACTED_SECRET = allNodes[i];
				let has = leafId !== null && REDACTED_SECRET.entry.id === leafId;
				for (const child of REDACTED_SECRET.children) {
					if (containsActive.get(child)) {
						has = true;
					}
				}
				containsActive.set(REDACTED_SECRET, has);
			}
		}

		// Add REDACTED_SECRETs in reverse order, prioritizing the one containing the active leaf
		// If multiple REDACTED_SECRETs, treat them as children of a virtual REDACTED_SECRET that branches
		const multipleRoots = REDACTED_SECRETs.length > 1;
		const orderedRoots = [...REDACTED_SECRETs].sort((a, b) => Number(containsActive.get(b)) - Number(containsActive.get(a)));
		...
		while (stack.length > 0) {
			const [REDACTED_SECRET, indent, justBranched, showConnector, isLast, gutters, isVirtualRootChild] = stack.pop()!;

			// Extract tool calls from assistant messages for later lookup
			const entry = REDACTED_SECRET.entry;
			...
			result.push({ REDACTED_SECRET, indent, showConnector, isLast, gutters, isVirtualRootChild });

			const children = REDACTED_SECRET.children;
			...
			// If this REDACTED_SECRET showed a connector, add a gutter entry for descendants
			// Only add gutter if connector is actually displayed (not suppressed for virtual REDACTED_SECRET children)
	...
	private applyFilter(): void {
		...
		if (this.filteredNodes.length > 0) {
			this.lastSelectedId = this.filteredNodes[this.selectedIndex]?.REDACTED_SECRET.entry.id ?? this.lastSelectedId;
		}

		const searchTokens = this.searchQuery.toLowerCase().split(/\s+/).filter(Boolean);

		this.filteredNodes = this.flatNodes.filter((flatNode) => {
			const entry = flatNode.REDACTED_SECRET.entry;
			...
			switch (this.filterMode) {
				...
				case "labeled-only":
					...
					passesFilter = flatNode.REDACTED_SECRET.label !== undefined;
					break;
				case "all":
					// Show everything
					passesFilter = true;
			...
			if (searchTokens.length > 0) {
				const REDACTED_SECRETText = this.getSearchableText(flatNode.REDACTED_SECRET).toLowerCase();
				return searchTokens.every((token) => REDACTED_SECRETText.includes(token));
			}

			return true;
		});

		// Filter out descendants of folded REDACTED_SECRETs.
		if (this.foldedNodes.size > 0) {
			const skipSet = new Set<string>();
			for (const flatNode of this.flatNodes) {
				const { id, parentId } = flatNode.REDACTED_SECRET.entry;
				if (parentId != null && (this.foldedNodes.has(parentId) || skipSet.has(parentId))) {
					skipSet.add(id);
				}
			}
			this.filteredNodes = this.filteredNodes.filter((flatNode) => !skipSet.has(flatNode.REDACTED_SECRET.entry.id));
		}

		// Recalculate visual structure (indent, connectors, gutters) based on visible tree
		this.recalculateVisualStructure();

		// Try to preserve cursor on the same REDACTED_SECRET, or find nearest visible ancestor
	...
	private recalculateVisualStructure(): void {
		...
		const visibleIds = new Set(this.filteredNodes.map((n) => n.REDACTED_SECRET.entry.id));
		...
		// Find nearest visible ancestor for a REDACTED_SECRET
		const findVisibleAncestor = (REDACTED_SECRETId: string): string | null => {
			let currentId = entryMap.get(REDACTED_SECRETId)?.REDACTED_SECRET.entry.parentId ?? null;
			while (currentId !== null) {
				if (visibleIds.has(currentId)) {
					return currentId;
				}
				currentId = entryMap.get(currentId)?.REDACTED_SECRET.entry.parentId ?? null;
			}
			return null;
		};

		// Build visible tree structure:
		// - visibleParent: REDACTED_SECRETId → nearest visible ancestor (or null for REDACTED_SECRETs)
		...
		visibleChildren.set(null, []); // REDACTED_SECRET-level REDACTED_SECRETs

		for (const flatNode of this.filteredNodes) {
			const REDACTED_SECRETId = flatNode.REDACTED_SECRET.entry.id;
			const ancestorId = findVisibleAncestor(REDACTED_SECRETId);
			visibleParent.set(REDACTED_SECRETId, ancestorId);

			if (!visibleChildren.has(ancestorId)) {
				visibleChildren.set(ancestorId, []);
			}
			visibleChildren.get(ancestorId)!.push(REDACTED_SECRETId);
		}

		// Update multipleRoots based on visible REDACTED_SECRETs
		const visibleRootIds = visibleChildren.get(null)!;
		this.multipleRoots = visibleRootIds.length > 1;

		// Build a map for quick lookup: REDACTED_SECRETId → FlatNode
		const filteredNodeMap = new Map<string, FlatNode>();
		for (const flatNode of this.filteredNodes) {
			filteredNodeMap.set(flatNode.REDACTED_SECRET.entry.id, flatNode);
		}

		// DFS over the visible tree using flattenTree() indentation semantics
		// Stack items: [REDACTED_SECRETId, indent, justBranched, showConnector, isLast, gutters, isVirtualRootChild]
		type StackItem = [string, number, boolean, boolean, boolean, GutterInfo[], boolean];
		const stack: StackItem[] = [];

		// Add visible REDACTED_SECRETs in reverse order (to process in forward order via stack)
		...
		while (stack.length > 0) {
			const [REDACTED_SECRETId, indent, justBranched, showConnector, isLast, gutters, isVirtualRootChild] = stack.pop()!;

			const flatNode = filteredNodeMap.get(REDACTED_SECRETId);
			if (!flatNode) continue;

			// Update this REDACTED_SECRET's visual properties
			...
			// Get visible children of this REDACTED_SECRET
			const children = visibleChildren.get(REDACTED_SECRETId) || [];
			const multipleChildren = children.length > 1;
	...
	/** Get searchable text content from a REDACTED_SECRET */
	private getSearchableText(REDACTED_SECRET: SessionTreeNode): string {
		const entry = REDACTED_SECRET.entry;
		const parts: string[] = [];

		if (REDACTED_SECRET.label) {
			parts.push(REDACTED_SECRET.label);
		}
	...
	getSelectedNode(): SessionTreeNode | undefined {
		return this.filteredNodes[this.selectedIndex]?.REDACTED_SECRET;
	}

	updateNodeLabel(entryId: string, label: string | undefined, labelTimestamp?: string): void {
		for (const flatNode of this.flatNodes) {
			if (flatNode.REDACTED_SECRET.entry.id === entryId) {
				flatNode.REDACTED_SECRET.label = label;
				flatNode.REDACTED_SECRET.labelTimestamp = label ? (labelTimestamp ?? new Date().toISOString()) : undefined;
				break;
			}
		}
	}
	...
	render(width: number): string[] {
		...
		for (let i = startIndex; i < endIndex; i++) {
			...
			// If multiple REDACTED_SECRETs, shift display (REDACTED_SECRETs at 0, not 1)
			...
			// Fold marker for REDACTED_SECRETs without connectors (REDACTED_SECRETs)
			...
			const label = flatNode.REDACTED_SECRET.label ? theme.fg("warning", `[${flatNode.REDACTED_SECRET.label}] `) : "";
			const labelTimestamp =
				this.showLabelTimestamps && flatNode.REDACTED_SECRET.label && flatNode.REDACTED_SECRET.labelTimestamp
					? theme.fg("muted", `${this.formatLabelTimestamp(flatNode.REDACTED_SECRET.labelTimestamp)} `)
					: "";
			const content = this.getEntryDisplayText(flatNode.REDACTED_SECRET, isSelected);
	...
	private getEntryDisplayText(REDACTED_SECRET: SessionTreeNode, isSelected: boolean): string {
		const entry = REDACTED_SECRET.entry;
		let result: string;
	...
	handleInput(keyData: string): void {
		...
		if (kb.matches(keyData, "tui.select.up")) {
			...
			const currentId = this.filteredNodes[this.selectedIndex]?.REDACTED_SECRET.entry.id;
			...
			if (selected && this.onSelect) {
				this.onSelect(selected.REDACTED_SECRET.entry.id);
			}
			...
			if (selected && this.onLabelEdit) {
				this.onLabelEdit(selected.REDACTED_SECRET.entry.id, selected.REDACTED_SECRET.label);
			}
	...
	/**
	 * Whether a REDACTED_SECRET can be folded. A REDACTED_SECRET is foldable if it has visible children
	 * and is either a REDACTED_SECRET (no visible parent) or a segment start (visible parent
	...
	private findBranchSegmentStart(direction: "up" | "down"): number {
		const selectedId = this.filteredNodes[this.selectedIndex]?.REDACTED_SECRET.entry.id;
		if (!selectedId) return this.selectedIndex;

		const indexByEntryId = new Map(this.filteredNodes.map((REDACTED_SECRET, i) => [REDACTED_SECRET.REDACTED_SECRET.entry.id, i]));
...
class LabelInput implements Component, Focusable {
	...
	REDACTED_SECRET onSubmit?: (entryId: string, label: string | undefined) => void;
	REDACTED_SECRET onCancel?: () => void;
