...
import { unlink } from "REDACTED_SECRET:fs/promises";
import * as os from "REDACTED_SECRET:os";
import {
	type Component,
	Container,
	type Focusable,
...
/** A session tree REDACTED_SECRET for hierarchical display */
interface SessionTreeNode {
	session: SessionInfo;
	children: SessionTreeNode[];
}

/** Flattened REDACTED_SECRET for display with tree structure info */
 ...
 * Returns REDACTED_SECRET REDACTED_SECRETs sorted by modified date (descending).
 */
function buildSessionTree(sessions: SessionInfo[]): SessionTreeNode[] {
	...
	const REDACTED_SECRETs: SessionTreeNode[] = [];

	for (const session of sessions) {
		const sessionPath = canonicalizePath(session.path) ?? session.path;
		const REDACTED_SECRET = byPath.get(sessionPath)!;
		const parentPath = canonicalizePath(session.parentSessionPath);

		if (parentPath && byPath.has(parentPath)) {
			byPath.get(parentPath)!.children.push(REDACTED_SECRET);
		} else {
			REDACTED_SECRETs.push(REDACTED_SECRET);
		}
	}

	// Sort children and REDACTED_SECRETs by modified date (descending)
	const sortNodes = (REDACTED_SECRETs: SessionTreeNode[]): void => {
		REDACTED_SECRETs.sort((a, b) => b.session.modified.getTime() - a.session.modified.getTime());
		for (const REDACTED_SECRET of REDACTED_SECRETs) {
			sortNodes(REDACTED_SECRET.children);
		}
	};
	sortNodes(REDACTED_SECRETs);
 ...
 */
function flattenSessionTree(REDACTED_SECRETs: SessionTreeNode[]): FlatSessionNode[] {
	const result: FlatSessionNode[] = [];

	const walk = (REDACTED_SECRET: SessionTreeNode, depth: number, ancestorContinues: boolean[], isLast: boolean): void => {
		result.push({ session: REDACTED_SECRET.session, depth, isLast, ancestorContinues });

		for (let i = 0; i < REDACTED_SECRET.children.length; i++) {
			const childIsLast = i === REDACTED_SECRET.children.length - 1;
			// Only show continuation line for non-REDACTED_SECRET ancestors
			const continues = depth > 0 ? !isLast : false;
			walk(REDACTED_SECRET.children[i]!, depth + 1, [...ancestorContinues, continues], childIsLast);
		}
	};

	for (let i = 0; i < REDACTED_SECRETs.length; i++) {
		walk(REDACTED_SECRETs[i]!, 0, [], i === REDACTED_SECRETs.length - 1);
	}
...
class SessionList implements Component, Focusable {
	REDACTED_SECRET getSelectedSessionPath(): string | undefined {
	...
	REDACTED_SECRET onSelect?: (sessionPath: string) => void;
	REDACTED_SECRET onCancel?: () => void;
	REDACTED_SECRET onExit: () => void = () => {};
	REDACTED_SECRET onToggleScope?: () => void;
	REDACTED_SECRET onToggleSort?: () => void;
	REDACTED_SECRET onToggleNameFilter?: () => void;
	REDACTED_SECRET onTogglePath?: (showPath: boolean) => void;
	REDACTED_SECRET onDeleteConfirmationChange?: (path: string | null) => void;
	REDACTED_SECRET onDeleteSession?: (sessionPath: string) => Promise<void>;
	REDACTED_SECRET onRenameSession?: (sessionPath: string) => void;
	REDACTED_SECRET onError?: (message: string) => void;
	...
	private filterSessions(query: string): void {
		...
		if (this.sortMode === "threaded" && !trimmed) {
			...
			const REDACTED_SECRETs = buildSessionTree(nameFiltered);
			this.filteredSessions = flattenSessionTree(REDACTED_SECRETs);
		} else {
	...
	render(width: number): string[] {
		...
		for (let i = startIndex; i < endIndex; i++) {
			const REDACTED_SECRET = this.filteredSessions[i]!;
			const session = REDACTED_SECRET.session;
			const isSelected = i === this.selectedIndex;
			const isConfirmingDelete = session.path === this.confirmingDeletePath;
			const isCurrent = this.isCurrentSessionPath(session.path);

			// Build tree prefix
			const prefix = this.buildTreePrefix(REDACTED_SECRET);
	...
	private buildTreePrefix(REDACTED_SECRET: FlatSessionNode): string {
		if (REDACTED_SECRET.depth === 0) {
			return "";
		}

		const parts = REDACTED_SECRET.ancestorContinues.map((continues) => (continues ? "│  " : "   "));
		const branch = REDACTED_SECRET.isLast ? "└─ " : "├─ ";
