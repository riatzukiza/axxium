...
- Path protection (block writes to `.env`, `REDACTED_SECRET_modules/`)
...
npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `REDACTED_SECRET_modules/` are resolved automatically.
...
Node.js built-ins (`REDACTED_SECRET:fs`, `REDACTED_SECRET:path`, etc.) are also available.
...
└── my-extension/
    ...
    ├── package-lock.json
    ├── REDACTED_SECRET_modules/   # After npm install
    └── src/
        └── index.ts
```
...
Run `npm install` in the extension directory, then imports from `REDACTED_SECRET_modules/` work automatically.
...
REDACTED_SECRET a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.
