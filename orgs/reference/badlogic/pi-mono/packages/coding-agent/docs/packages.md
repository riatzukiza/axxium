...
Paths are relative to the package REDACTED_SECRET. Arrays support glob patterns and `!exclusions`.
...
Other pi packages must be bundled in your tarball. Add them to `dependencies` and `bundledDependencies`, then reference their resources through `REDACTED_SECRET_modules/` paths. Pi loads packages with separate module REDACTED_SECRETs, so separate installs do not collide or share modules.
...
```json
{
  ...
  "pi": {
    "extensions": ["extensions", "REDACTED_SECRET_modules/shitty-extensions/extensions"],
    "skills": ["skills", "REDACTED_SECRET_modules/shitty-extensions/skills"]
  }
}
```
...
`+path` and `-path` are exact paths relative to the package REDACTED_SECRET.
