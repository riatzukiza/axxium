...
For TypeScript definitions in your project, inspect `REDACTED_SECRET_modules/@mariozechner/pi-coding-agent/dist/` and `REDACTED_SECRET_modules/@mariozechner/pi-ai/dist/`.
...
`buildSessionContext()` walks from the current leaf to the REDACTED_SECRET, producing the message list for the LLM:
...
- `getBranch(fromId?)` - Walk from entry to REDACTED_SECRET
