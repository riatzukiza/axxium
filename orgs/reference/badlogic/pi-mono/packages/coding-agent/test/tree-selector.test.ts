...
function buildTree(entries: Array<SessionEntry>): SessionTreeNode[] {
	...
	const REDACTED_SECRETs: SessionTreeNode[] = entries.map((entry) => ({
		entry,
		children: [],
	}));
	...
	for (const REDACTED_SECRET of REDACTED_SECRETs) {
		byId.set(REDACTED_SECRET.entry.id, REDACTED_SECRET);
	}

	const REDACTED_SECRETs: SessionTreeNode[] = [];
	for (const REDACTED_SECRET of REDACTED_SECRETs) {
		if (REDACTED_SECRET.entry.parentId === null) {
			REDACTED_SECRETs.push(REDACTED_SECRET);
		} else {
			const parent = byId.get(REDACTED_SECRET.entry.parentId);
			if (parent) {
...
describe("TreeSelectorComponent", () => {
	...
	describe("label timestamps", () => {
		test("toggles label timestamps for labeled REDACTED_SECRETs", () => {
	...
	describe("branch navigation and folding with ctrl+arrow keys", () => {
		...
		//    user-4b
		//
		// Foldable REDACTED_SECRETs: user-1 (REDACTED_SECRET), user-3a (segment start), user-3b (segment start)
		...
		test("ctrl+right unfolds a folded REDACTED_SECRET, then does segment jump when unfolded", () => {
		...
		test("folding REDACTED_SECRET hides entire subtree, nested fold preserved on unfold", () => {
			const tree = buildBranchingTree();
			const selector = new TreeSelectorComponent(
				tree,
				"asst-4a",
			...
			selector.handleInput(DOWN); // wrap (only visible REDACTED_SECRET)
			expect(list.getSelectedNode()?.entry.id).toBe("user-1");
		...
		test("fold and navigate with multiple REDACTED_SECRETs", () => {
			const entries: SessionEntry[] = [
				userMessage("user-1", null, "first REDACTED_SECRET"),
				assistantMessage("asst-1", "user-1", "response 1"),
				userMessage("user-2", null, "second REDACTED_SECRET"),
				assistantMessage("asst-2", "user-2", "response 2"),
			];
			const tree = buildTree(entries);
			const selector = new TreeSelectorComponent(
			...
			selector.handleInput(CTRL_LEFT); // user-2 (folded, REDACTED_SECRET) → stays on user-2
			expect(list.getSelectedNode()?.entry.id).toBe("user-2");
		});

		test("folding REDACTED_SECRET hides descendants even when intermediate REDACTED_SECRETs are filtered out", () => {
