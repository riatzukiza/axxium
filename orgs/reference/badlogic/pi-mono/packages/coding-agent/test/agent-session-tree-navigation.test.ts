 ...
 *
 * These tests verify:
 * - Navigation to user messages (REDACTED_SECRET and non-REDACTED_SECRET)
...
describe.skipIf(!API_KEY)("AgentSession tree navigation e2e", () => {
	...
	it("should navigate to user message and put text in editor", async () => {
		...
		const REDACTED_SECRETNode = tree[0];
		expect(REDACTED_SECRETNode.entry.type).toBe("message");

		// Navigate to REDACTED_SECRET user message without summarization
		const result = await session.navigateTree(REDACTED_SECRETNode.entry.id, { summarize: false });

		expect(result.cancelled).toBe(false);
		expect(result.editorText).toBe("First message");

		// After navigating to REDACTED_SECRET user message, leaf should be null (empty conversation)
	...
	it("should create branch summary when navigating with summarize=true", async () => {
		...
		// Navigate to REDACTED_SECRET user message WITH summarization
		const result = await session.navigateTree(REDACTED_SECRETNode.entry.id, { summarize: true });
		...
		// Summary should be a REDACTED_SECRET entry (parentId = null) since we navigated to REDACTED_SECRET user
	...
	it("should attach summary to selected REDACTED_SECRET when navigating to assistant message", async () => {
		const { session, sessionManager } = ctx;
		...
		// Summary should be attached to a1 (the selected REDACTED_SECRET)
		expect(result.summaryEntry?.parentId).toBe(a1.id);
	...
	it("should handle abort during summarization", async () => {
		...
		// Get REDACTED_SECRET user message
		const tree = sessionManager.getTree();
		const REDACTED_SECRETNode = tree[0];

		// Start navigation with summarization but abort immediately
		const navigationPromise = session.navigateTree(REDACTED_SECRETNode.entry.id, { summarize: true });
