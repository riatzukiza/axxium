...
describe("SessionManager append and tree traversal", () => {
	...
	describe("getPath", () => {
		...
		it("returns full path from REDACTED_SECRET to leaf", () => {
			const session = SessionManager.inMemory();
		...
		it("returns path from specified entry to REDACTED_SECRET", () => {
			const session = SessionManager.inMemory();
	...
	describe("getTree", () => {
		...
		it("returns single REDACTED_SECRET for linear session", () => {
			const session = SessionManager.inMemory();
			...
			const REDACTED_SECRET = tree[0];
			expect(REDACTED_SECRET.entry.id).toBe(id1);
			expect(REDACTED_SECRET.children).toHaveLength(1);
			expect(REDACTED_SECRET.children[0].entry.id).toBe(id2);
			expect(REDACTED_SECRET.children[0].children).toHaveLength(1);
			expect(REDACTED_SECRET.children[0].children[0].entry.id).toBe(id3);
			expect(REDACTED_SECRET.children[0].children[0].children).toHaveLength(0);
		});
		...
		it("returns tree with branches after branch", () => {
			...
			const REDACTED_SECRET2 = REDACTED_SECRET.children[0];
			expect(REDACTED_SECRET2.entry.id).toBe(id2);
			expect(REDACTED_SECRET2.children).toHaveLength(2); // id3 and id4 are siblings

			const childIds = REDACTED_SECRET2.children.map((c) => c.entry.id).sort();
		...
		it("handles multiple branches at same point", () => {
			...
			const _id1 = session.appendMessage(userMsg("REDACTED_SECRET"));
			...
			const tree = session.getTree();
			const REDACTED_SECRET2 = tree[0].children[0];
			expect(REDACTED_SECRET2.entry.id).toBe(id2);
			expect(REDACTED_SECRET2.children).toHaveLength(3);

			const branchIds = REDACTED_SECRET2.children.map((c) => c.entry.id).sort();
		...
		it("handles deep branching", () => {
			...
			// Verify structure
			const REDACTED_SECRET2 = tree[0].children[0];
			expect(REDACTED_SECRET2.children).toHaveLength(2); // id3 and id5

			const REDACTED_SECRET5 = REDACTED_SECRET2.children.find((c) => c.entry.id === id5)!;
			expect(REDACTED_SECRET5.children).toHaveLength(2); // id6 and id7

			const REDACTED_SECRET3 = REDACTED_SECRET2.children.find((c) => c.entry.id === id3)!;
			expect(REDACTED_SECRET3.children).toHaveLength(1); // id4
		});
	});
