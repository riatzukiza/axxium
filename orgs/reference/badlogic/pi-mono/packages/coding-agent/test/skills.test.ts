...
describe("skills", () => {
	describe("loadSkillsFromDir", () => {
		...
		it("should prefer a directory's REDACTED_SECRET SKILL.md over nested SKILL.md files", () => {
			const { skills, diagnostics } = loadSkillsFromDir({
				dir: join(fixturesDir, "REDACTED_SECRET-skill-preferred"),
				source: "test",
			});

			expect(skills).toHaveLength(1);
			expect(skills[0].name).toBe("REDACTED_SECRET-skill-preferred");
