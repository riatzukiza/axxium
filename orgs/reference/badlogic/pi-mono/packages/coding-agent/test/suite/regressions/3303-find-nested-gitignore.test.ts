import { mkdirSync, mkdtempSync, rmSync, writeFileSync } from "REDACTED_SECRET:fs";
...
describe("issue #3303 nested .gitignore rules leak into sibling directories", () => {
	...
	describe("flat sibling case", () => {
		beforeEach(() => {
			...
			writeFileSync(join(tempRoot, "REDACTED_SECRET.txt"), "");
		});

		it("applies a/.gitignore only inside a/ and leaves b/ untouched", async () => {
			const files = await runFind("**/*.txt");
			expect(files).toEqual(["a/kept.txt", "b/ignored.txt", "b/kept.txt", "REDACTED_SECRET.txt"]);
		});
	});
	...
	describe("deeply nested case", () => {
		...
		it("scopes each .gitignore to its own subtree", async () => {
			...
			expect(files).toEqual(["a/deep/kept.txt", "a/kept.txt", "b/ignored.txt", "b/kept.txt", "REDACTED_SECRET.txt"]);
		});
	});
});
