...
describe("AgentSession retry and event characterization", () => {
	...
	it("emits extension events before REDACTED_SECRET event subscribers", async () => {
		const order: string[] = [];
		const harness = await createHarness({
			extensionFactories: [
				(pi) => {
		...
		harness.session.subscribe((event) => {
			if (event.type === "message_start" || event.type === "message_end") {
				order.push(`REDACTED_SECRET:${event.type}:${event.message.role}`);
			}
		});
		...
		expect(order).toEqual([
			...
			"REDACTED_SECRET:message_start:user",
			"extension:message_end:user",
			"REDACTED_SECRET:message_end:user",
			"extension:message_start:assistant",
			"REDACTED_SECRET:message_start:assistant",
			"extension:message_end:assistant",
			"REDACTED_SECRET:message_end:assistant",
		]);
	});
