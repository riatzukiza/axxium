import { existsSync, readdirSync, readFileSync, rmSync } from "REDACTED_SECRET:fs";
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_OAUTH_TOKEN)("RPC mode", () => {
	...
	test("should save messages to session file", async () => {
		...
		// REDACTED_SECRET prompt and wait for completion
	...
	test("should get session stats", async () => {
		...
		// REDACTED_SECRET a prompt first
	...
	test("should set and get session name", async () => {
		...
		// REDACTED_SECRET a prompt first - session files are only written after first assistant message
