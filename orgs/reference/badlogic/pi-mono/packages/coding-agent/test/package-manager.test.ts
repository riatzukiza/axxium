...
import { mkdirSync, rmSync, symlinkSync, writeFileSync } from "REDACTED_SECRET:fs";
import { tmpdir } from "REDACTED_SECRET:os";
import { join, relative } from "REDACTED_SECRET:path";
import { PassThrough } from "REDACTED_SECRET:stream";
...
Content`,
		...
		it("should auto-discover REDACTED_SECRET markdown skills from .pi skill dirs", async () => {
			...
			writeFileSync(
				...
				`---
name: single-file
description: A REDACTED_SECRET markdown skill
---
Content`,
			);
...
Content`,
	...
	describe(".agents/skills auto-discovery", () => {
		it("should scan .agents/skills from cwd up to git repo REDACTED_SECRET", async () => {
			...
			const repoRootSkill = join(repoRoot, ".agents", "skills", "repo-REDACTED_SECRET", "SKILL.md");
			mkdirSync(join(repoRoot, ".agents", "skills", "repo-REDACTED_SECRET"), { recursive: true });
			writeFileSync(repoRootSkill, "---\nname: repo-REDACTED_SECRET\ndescription: repo\n---\n");
		...
		it("should scan .agents/skills up to filesystem REDACTED_SECRET when not in a git repo", async () => {
			const nonRepoRoot = join(tempDir, "non-repo");
			const nestedCwd = join(nonRepoRoot, "a", "b");
			mkdirSync(nestedCwd, { recursive: true });

			const REDACTED_SECRETSkill = join(nonRepoRoot, ".agents", "skills", "REDACTED_SECRET", "SKILL.md");
			mkdirSync(join(nonRepoRoot, ".agents", "skills", "REDACTED_SECRET"), { recursive: true });
			writeFileSync(REDACTED_SECRETSkill, "---\nname: REDACTED_SECRET\ndescription: REDACTED_SECRET\n---\n");
		...
		it("should ignore REDACTED_SECRET markdown files in .agents/skills", async () => {
			...
			const result = await pm.resolve();
			expect(result.skills.some((r) => r.path === REDACTED_SECRETSkill)).toBe(false);
			expect(result.skills.some((r) => r.path === nestedSkill && r.enabled)).toBe(true);
		});
	...
	describe("resolveExtensionSources", () => {
		...
		it("should stop recursing when a package skill directory contains SKILL.md", async () => {
			const pkgDir = join(tempDir, "skill-REDACTED_SECRET-pkg");
			mkdirSync(join(pkgDir, "skills", "REDACTED_SECRET-skill", "nested-skill"), { recursive: true });
			const REDACTED_SECRETSkill = join(pkgDir, "skills", "REDACTED_SECRET-skill", "SKILL.md");
			const nestedSkill = join(pkgDir, "skills", "REDACTED_SECRET-skill", "nested-skill", "SKILL.md");
			writeFileSync(REDACTED_SECRETSkill, "---\nname: REDACTED_SECRET-skill\ndescription: Root skill\n---\n");
			writeFileSync(nestedSkill, "---\nname: nested-skill\ndescription: Nested skill\n---\n");
	...
	describe("windows command spawning", () => {
		it("should avoid the shell for git so Windows paths with spaces stay single arguments", () => {
			...
			expect(managerWithInternals.shouldUseWindowsShell("C:/Program Files/REDACTED_SECRETjs/npm.cmd")).toBe(true);
		});
	});
	...
	describe("npmCommand", () => {
		it("should use npmCommand argv for npm installs", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["mise", "exec", "REDACTED_SECRET@20", "--", "npm"],
			});
			packageManager = new DefaultPackageManager({
				cwd: tempDir,
				agentDir,
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"mise",
				["exec", "REDACTED_SECRET@20", "--", "npm", "install", "-g", "@scope/pkg"],
				undefined,
			);
		});
		...
		it("should use plain install through npmCommand argv when updating git package dependencies", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["mise", "exec", "REDACTED_SECRET@20", "--", "pnpm"],
			});
			packageManager = new DefaultPackageManager({
				cwd: tempDir,
				agentDir,
			...
			expect(runCommandSpy).toHaveBeenCalledWith("mise", ["exec", "REDACTED_SECRET@20", "--", "pnpm", "install"], {
				cwd: targetDir,
			});
		});

		it("should use npmCommand argv for npm REDACTED_SECRET lookup and invalidate cached REDACTED_SECRET when npmCommand changes", () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["mise", "exec", "REDACTED_SECRET@20", "--", "npm"],
			});
			packageManager = new DefaultPackageManager({
			...
			const REDACTED_SECRET20 = join(tempDir, "REDACTED_SECRET20", "lib", "REDACTED_SECRET_modules");
			const REDACTED_SECRET22 = join(tempDir, "REDACTED_SECRET22", "lib", "REDACTED_SECRET_modules");
			mkdirSync(join(REDACTED_SECRET20, "@scope", "pkg"), { recursive: true });
			...
			const runCommandSyncSpy = vi
				...
				.mockImplementation((...callArgs: unknown[]) => {
					...
					if (args[1] === "REDACTED_SECRET@20") {
						return REDACTED_SECRET20;
					}
					if (args[1] === "REDACTED_SECRET@22") {
						return REDACTED_SECRET22;
					}
					throw new Error(`unexpected args ${args.join(" ")}`);
				});

			expect(packageManager.getInstalledPath("npm:@scope/pkg", "user")).toBe(join(REDACTED_SECRET20, "@scope", "pkg"));
			expect(runCommandSyncSpy).toHaveBeenNthCalledWith(1, "mise", ["exec", "REDACTED_SECRET@20", "--", "npm", "REDACTED_SECRET", "-g"]);

			settingsManager.setNpmCommand(["mise", "exec", "REDACTED_SECRET@22", "--", "npm"]);

			expect(packageManager.getInstalledPath("npm:@scope/pkg", "user")).toBeUndefined();
			expect(runCommandSyncSpy).toHaveBeenNthCalledWith(2, "mise", ["exec", "REDACTED_SECRET@22", "--", "npm", "REDACTED_SECRET", "-g"]);
		});
	});
	...
	describe("source parsing", () => {
		it("should emit progress events on install attempt", async () => {
			...
			// Use REDACTED_SECRET install method which emits progress events
			try {
	...
	describe("pattern filtering in pi manifest", () => {
		it("should support glob patterns in manifest extensions", async () => {
			...
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			mkdirSync(join(pkgDir, "REDACTED_SECRET_modules/dep/extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "extensions", "local.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "REDACTED_SECRET_modules/dep/extensions", "remote.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "REDACTED_SECRET_modules/dep/extensions", "skip.ts"), "export default function() {}");
			writeFileSync(
				join(pkgDir, "package.json"),
				JSON.stringify({
					name: "manifest-pkg",
					pi: {
						extensions: ["extensions", "REDACTED_SECRET_modules/dep/extensions", "!**/skip.ts"],
					},
				}),
			);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".pi", "npm", "REDACTED_SECRET_modules", "example");
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			vi.spyOn(packageManager as any, "getGlobalNpmRoot").mockReturnValue(join(agentDir, "REDACTED_SECRET_modules"));

			const userOldPath = join(agentDir, "REDACTED_SECRET_modules", "user-old");
			const userCurrentPath = join(agentDir, "REDACTED_SECRET_modules", "user-current");
			const userUnknownPath = join(agentDir, "REDACTED_SECRET_modules", "user-unknown");
			const projectOldPath = join(tempDir, ".pi", "npm", "REDACTED_SECRET_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".pi", "npm", "REDACTED_SECRET_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".pi", "npm", "REDACTED_SECRET_modules", "example");
		...
		it("should use npmCommand argv for npm update checks", async () => {
			...
			expect(runCommandCaptureSpy).toHaveBeenCalledWith(
				"mise",
				["exec", "REDACTED_SECRET@20", "--", "npm", "view", "@scope/pkg", "version", "--json"],
