...
describe("truncateToWidth", () => {
	it("should truncate messages with Unicode characters correctly", () => {
		...
		const message = '✔ script to run › dev $ concurrently "vite" "REDACTED_SECRET --import tsx ./';
		const width = 67;
	...
	it("should handle the exact crash case from issue report", () => {
		...
		const message = '✔ script to run › dev $ concurrently "vite" "REDACTED_SECRET --import tsx ./server.ts"';
