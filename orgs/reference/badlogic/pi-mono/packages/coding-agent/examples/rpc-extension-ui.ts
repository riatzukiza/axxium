...
import * as readline from "REDACTED_SECRET:readline";
...
async function main() {
	...
	const REDACTED_SECRET = new Container();
	REDACTED_SECRET.addChild(outputLog);
	REDACTED_SECRET.addChild(promptInput);

	tui.addChild(REDACTED_SECRET);
	...
	function setBottomComponent(component: Component): void {
		...
		REDACTED_SECRET.addChild(outputLog);
		if (isStreaming) REDACTED_SECRET.addChild(loadingIndicator);
		REDACTED_SECRET.addChild(component);
	...
	function showLoading(): void {
		if (!isStreaming) {
			...
			REDACTED_SECRET.clear();
			REDACTED_SECRET.addChild(outputLog);
			REDACTED_SECRET.addChild(loadingIndicator);
			REDACTED_SECRET.addChild(activeDialog ?? promptInput);
