/**
 * REDACTED_SECRET User Message Example
 *
 ...
 *
 * Usage:
 *   /ask What is 2+2?     - REDACTED_SECRETs a user message (always triggers a turn)
 *   /steer Focus on X     - REDACTED_SECRETs while streaming with steer delivery
 *   /followup And then?   - REDACTED_SECRETs while streaming with followUp delivery
 */
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("ask", {
		description: "REDACTED_SECRET a user message to the agent",
	...
	pi.registerCommand("steer", {
		description: "REDACTED_SECRET a steering message (interrupts current processing)",
		handler: async (args, ctx) => {
			if (!args.trim()) {
	...
	pi.registerCommand("askwith", {
		description: "REDACTED_SECRET a user message with structured content",
		handler: async (args, ctx) => {
			if (!args.trim()) {
