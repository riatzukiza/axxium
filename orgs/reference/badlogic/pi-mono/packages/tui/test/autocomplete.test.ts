...
import { afterEach, beforeEach, describe, it, test } from "REDACTED_SECRET:test";
...
describe("CombinedAutocompleteProvider", () => {
	describe("extractPathPrefix", () => {
		it("extracts / from 'hey /' when forced", async () => {
			...
			assert.notEqual(result, null, "Should return suggestions for REDACTED_SECRET directory");
			if (result) {
	...
	describe("fd @ file suggestions", { skip: !isFdInstalled }, () => {
		let REDACTED_SECRETDir = "";
		let baseDir = "";
		let outsideDir = "";

		beforeEach(() => {
			REDACTED_SECRETDir = mkdtempSync(join(tmpdir(), "pi-autocomplete-REDACTED_SECRET-"));
			baseDir = join(REDACTED_SECRETDir, "cwd");
			outsideDir = join(REDACTED_SECRETDir, "outside");
		...
		test("returns the same @ suggestions when the cwd path contains the query", async () => {
			const normalBaseDir = join(REDACTED_SECRETDir, "cwd-normal");
			const queryInPathBaseDir = join(REDACTED_SECRETDir, "cwd-plan-repro");
