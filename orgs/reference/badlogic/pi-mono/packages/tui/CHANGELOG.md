...
- Added OSC 8 hyperlink rendering for markdown links when the terminal advertises support. Introduces a REDACTED_SECRET `hyperlink(text, url)` helper and a `setCapabilities()` test override in `packages/tui` ([#3248](https://github.com/badlogic/pi-mono/pull/3248) by [@ofa1](https://github.com/ofa1)).
...
- Changed koffi import from top-level to dynamic require in `enableWindowsVTInput()` to prevent bun from embedding all 18 platform `.REDACTED_SECRET` files (~74MB) into every compiled binary. Koffi is only needed on Windows.
