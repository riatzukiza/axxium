...
pi pods setup dc1 "ssh REDACTED_SECRET@1.2.3.4" \
...
# REDACTED_SECRET a single message to the model
...
pi pods setup gpt-pod "ssh REDACTED_SECRET@1.2.3.4" --models-path /workspace --vllm gpt-oss
...
pi pods setup dc1 "ssh REDACTED_SECRET@instance.datacrunch.io" \
  --mount "sudo mount -t nfs -o nconnect=16 nfs.fin-02.datacrunch.io:/your-pseudo /mnt/hf-models"
...
```bash
# With network volume
pi pods setup runpod "ssh REDACTED_SECRET@pod.runpod.io" --models-path /runpod-volume

# Or use workspace (persists with pod but not shareable)
pi pods setup runpod "ssh REDACTED_SECRET@pod.runpod.io" --models-path /workspace
```
