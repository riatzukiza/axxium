...
### Pods
```bash
pi pods setup dc1 "ssh REDACTED_SECRET@1.2.3.4" --mount "mount -t nfs..."  # Setup pod (requires HF_TOKEN, PI_API_KEY env vars)
...
```bash
pi pods setup dc1 "ssh REDACTED_SECRET@1.2.3.4" --mount "mount -t nfs..."
```
...
```json
{
  "pods": {
    "dc1": {
      "ssh": "ssh REDACTED_SECRET@1.2.3.4",
      "gpus": [
