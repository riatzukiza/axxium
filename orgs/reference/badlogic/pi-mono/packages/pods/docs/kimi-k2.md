...
Running parameters for this environment are provided below. You may scale up to more REDACTED_SECRETs and increase expert-parallelism to enlarge the inference batch size and overall throughput.
...
``` bash
# start ray on REDACTED_SECRET 0 and REDACTED_SECRET 1
...
Here is the simple example code to run TP16 with two REDACTED_SECRETs on H200:

``` bash
# Node 0
python -m sglang.launch_server --model-path $MODEL_PATH --tp 16 --dist-init-addr $MASTER_IP:50000 --nREDACTED_SECRETs 2 --REDACTED_SECRET-rank 0 --trust-remote-code --tool-call-parser kimi_k2

# Node 1
python -m sglang.launch_server --model-path $MODEL_PATH --tp 16 --dist-init-addr $MASTER_IP:50000 --nREDACTED_SECRETs 2 --REDACTED_SECRET-rank 1 --trust-remote-code --tool-call-parser kimi_k2
```
...
--trust-remote-code --disaggregation-mode prefill --dist-init-addr $PREFILL_NODE0$:5757 --tp-size 32 --dp-size 32 --enable-dp-attention --host $LOCAL_IP --decode-log-interval 1 --disable-radix-cache --enable-deepep-moe --moe-dense-tp-size 1 --enable-dp-lm-head --disable-shared-experts-fusion --watchdog-timeout 1000000 --enable-two-batch-overlap --disaggregation-ib-device $IB_DEVICE --chunked-prefill-size 131072 --mem-fraction-static 0.85 --deepep-mode normal --ep-dispatch-algorithm dynamic --eplb-algorithm deepseek --max-running-requests 1024 --nREDACTED_SECRETs 4 --REDACTED_SECRET-rank $RANK --tool-call-parser kimi_k2


# for decode REDACTED_SECRET
...
```
### Multi-REDACTED_SECRET Serving
TensorRT-LLM supports multi-REDACTED_SECRET inference. You can use mpirun to launch Kimi-K2 with multi-REDACTED_SECRET jobs. We will use two REDACTED_SECRETs for this example.

#### mpirun
mpirun requires each REDACTED_SECRET to have passwordless ssh access to the other REDACTED_SECRET. We need to setup the environment inside the docker container. Run the container with host network and mount the current directory as well as model directory to the container.

```bash
# use host network
IMAGE=<YOUR_IMAGE>
NAME=test_2REDACTED_SECRET_docker
# host1
...
ssh-copy-id -i ~/.ssh/id_ed25519.pub REDACTED_SECRET@<HOST2>
# on host2
ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519
ssh-copy-id -i ~/.ssh/id_ed25519.pub REDACTED_SECRET@<HOST1>
...
After the preparations,you can run the trtllm-serve on two REDACTED_SECRETs using mpirun:

```bash
mpirun -np 16 \
-H <HOST1>:8,<HOST2>:8 \
-mca plm_rsh_args "-p 2233" \
--allow-run-as-REDACTED_SECRET \
