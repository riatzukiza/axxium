...
- `@slack/web-api` - REDACTED_SECRETs messages back to Slack
...
4. This routes your app's interactions and events over WebSockets instead of REDACTED_SECRET HTTP endpoints
...
// REDACTED_SECRET message to that DM
await webClient.chat.postMessage({
...
const channels = await webClient.conversations.list({
  types: 'REDACTED_SECRET_channel,private_channel'
});
...
✅ **No REDACTED_SECRET URL needed** - works behind firewall  
