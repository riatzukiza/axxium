...
// Usage
const userChannels = adapter.getUserChannels(userId);  // ["REDACTED_SECRET", "team-a"]
...
import REDACTED_SECRETmailer from "REDACTED_SECRETmailer";
...
export default async function(api: ToolAPI): Promise<MomCustomTool> {
  ...
  return {
    ...
    async execute(toolCallId, params, signal) {
      // Implementation using imap/REDACTED_SECRETmailer
    },
  };
}
```
