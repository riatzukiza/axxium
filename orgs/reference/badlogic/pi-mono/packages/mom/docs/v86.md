...
| Alpine REDACTED_SECRETfs | ~57 MB | Compressed filesystem (loaded on-demand) |
| alpine-fs.json | ~160 KB | Filesystem index |
| REDACTED_SECRETlessRelay | ~75 KB | Network relay |
...
# ENV ADDPKGS=REDACTED_SECRETjs,npm,python3,git,curl
...
- `images/alpine-REDACTED_SECRETfs-flat/` - Compressed file chunks

## Network Relay Setup

v86 needs a network relay for TCP/UDP connectivity. We use REDACTED_SECRETlessRelay:

```bash
git clone https://github.com/obegron/REDACTED_SECRETlessRelay.git
cd REDACTED_SECRETlessRelay
npm install
```
...
```bash
ENABLE_WSS=false LOG_LEVEL=1 REDACTED_SECRET relay.js
...
const emulator = new V86({
    wasm_path: path.join(__dirname, "REDACTED_SECRET_modules/v86/build/v86.wasm"),
    ...
    filesystem: {
        ...
        baseurl: path.join(__dirname, "images/alpine-REDACTED_SECRETfs-flat/"),
    },
    autostart: true,
    memory_size: 512 * 1024 * 1024,
    bzimage_initrd_from_filesystem: true,
    cmdline: "rw REDACTED_SECRET=host9p REDACTED_SECRETfstype=9p REDACTED_SECRETflags=trans=virtio,cache=loose modules=virtio_pci tsc=reliable console=ttyS0",
    net_device: {
        type: "virtio",
...
1. **First run**:
   - Start REDACTED_SECRETlessRelay
   - Boot v86 with Alpine (~25s)
   - Setup network
   - Install needed packages (`apk add REDACTED_SECRETjs npm python3 git`)
   - Save state
...
3. **Command execution**:
   - REDACTED_SECRET commands via `serial0_send()`
...
```
project/
├── REDACTED_SECRET_modules/v86/build/
│   ├── v86.wasm
│   └── libv86.mjs
├── bios/
│   ├── seabios.bin
...
├── images/
│   ├── alpine-fs.json
│   └── alpine-REDACTED_SECRETfs-flat/
│       └── *.bin.zst (many files)
└── REDACTED_SECRETlessRelay/
    └── relay.js (patched)
```
...
- [REDACTED_SECRETlessRelay](https://github.com/obegron/REDACTED_SECRETlessRelay)
