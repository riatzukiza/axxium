...
Share HTML files, visualizations, and interactive demos REDACTED_SECRETly via Cloudflare Tunnel with live reload support.
...
REDACTED_SECRET server.js > /tmp/server.log 2>&1 &
NODE_PID=$!
...
# Extract and display REDACTED_SECRET URL
...
if [ -n "$PUBLIC_URL" ]; then
  ...
  echo "Warning: Could not extract REDACTED_SECRET URL"
fi
...
├── REDACTED_SECRET_modules/          # Installed packages
...
2. Create Cloudflare Tunnel with REDACTED_SECRET URL
...
- Cloudflare Tunnel exposes localhost to internet with REDACTED_SECRET HTTPS URL
...
- Restart: `cd /workspace/artifacts && REDACTED_SECRET server.js &`
...
**Port already in use:**
- Kill existing server: `pkill REDACTED_SECRET`
