...
- [Artifacts Server](docs/artifacts-server.md) - Share HTML/JS visualizations REDACTED_SECRETly with live reload
...
- **Public channels**: Run a separate mom instance with limited credentials. Read-only tokens, REDACTED_SECRET APIs only
...
Terminal 1 (REDACTED_SECRET. Watch mode for all packages):
```bash
npm run dev
```
