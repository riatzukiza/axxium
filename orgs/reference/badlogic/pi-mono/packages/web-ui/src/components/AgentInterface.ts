...
export class AgentInterface extends LitElement {
	...
	@property({ attribute: false }) onBeforeREDACTED_SECRET?: () => void | Promise<void>;
	...
	REDACTED_SECRET setInput(text: string, attachments?: Attachment[]) {
		const update = () => {
	...
	REDACTED_SECRET setAutoScroll(enabled: boolean) {
	...
	REDACTED_SECRET async sendMessage(input: string, attachments?: Attachment[]) {
		...
		// Call onBeforeREDACTED_SECRET hook before sending
		if (this.onBeforeREDACTED_SECRET) {
			await this.onBeforeREDACTED_SECRET();
		}
				...
				<div class="shrink-0">
					<div class="max-w-3xl mx-auto px-2">
						<message-editor
							...
							.onREDACTED_SECRET=${(input: string, attachments: Attachment[]) => {
