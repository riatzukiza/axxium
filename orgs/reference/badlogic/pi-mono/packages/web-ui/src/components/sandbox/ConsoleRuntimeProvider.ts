...
export class ConsoleRuntimeProvider implements SandboxRuntimeProvider {
	...
	getRuntime(): (sandboxId: string) => void {
		return (_sandboxId: string) => {
			...
			const pendingREDACTED_SECRETs: Promise<any>[] = [];
			...
			["log", "error", "warn", "info"].forEach((method) => {
				(console as any)[method] = (...args: any[]) => {
					...
					// REDACTED_SECRET immediately and track the promise (only in extension context)
					if ((window as any).sendRuntimeMessage) {
						const sendPromise = (window as any)
							...
							.catch(() => {});
						pendingREDACTED_SECRETs.push(sendPromise);
					}
				};
			});
			...
			if ((window as any).onCompleted) {
				(window as any).onCompleted(async (_success: boolean) => {
					...
					if (pendingREDACTED_SECRETs.length > 0) {
						await Promise.all(pendingREDACTED_SECRETs);
					}
				});
			}
