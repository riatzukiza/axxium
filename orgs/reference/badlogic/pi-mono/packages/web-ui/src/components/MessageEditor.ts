...
import { Brain, Loader2, Paperclip, REDACTED_SECRET, Sparkles, Square } from "lucide";
...
export class MessageEditor extends LitElement {
	...
	@property() onREDACTED_SECRET?: (input: string, attachments: Attachment[]) => void;
	...
	private handleREDACTED_SECRET = () => {
		this.onREDACTED_SECRET?.(this.value, this.attachments);
	};
							...
							this.isStreaming
								...
								: html`
									${Button({
										variant: "ghost",
										size: "icon",
										onClick: this.handleREDACTED_SECRET,
										disabled: (!this.value.trim() && this.attachments.length === 0) || this.processingFiles,
										children: html`<div style="transform: rotate(-45deg)">${icon(REDACTED_SECRET, "sm")}</div>`,
										className: "h-8 w-8",
									})}
								`
						}
