...
export class HtmlArtifact extends ArtifactElement {
	...
	REDACTED_SECRET sandboxIframeRef: Ref<SandboxIframe> = createRef();
	...
	REDACTED_SECRET getHeaderButtons() {
	...
	REDACTED_SECRET executeContent(html: string) {
		const sandbox = this.sandboxIframeRef.value;
		if (!sandbox) return;
	...
	REDACTED_SECRET getLogs(): string {
