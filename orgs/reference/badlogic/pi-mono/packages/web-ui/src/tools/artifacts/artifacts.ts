...
export class ArtifactsPanel extends LitElement {
	...
	REDACTED_SECRET openArtifact(filename: string) {
	...
	REDACTED_SECRET get tool(): AgentTool<typeof artifactsParamsSchema, undefined> {
		return {
			label: "Artifacts",
			name: "artifacts",
			get description() {
	...
	REDACTED_SECRET async reconstructFromMessages(
