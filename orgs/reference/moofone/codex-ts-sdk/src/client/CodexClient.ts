...
import type {
  ...
  ReviewRequestInput,
  REDACTED_SECRETMessageOptions,
  REDACTED_SECRETUserTurnOptions,
  StatusResponse,
} from '../types/options';
...
export class CodexClient extends EventEmitter {
  ...
  async sendMessage(text: string, options: REDACTED_SECRETMessageOptions = {}): Promise<void> {
  ...
  async sendUserTurn(text: string, options: REDACTED_SECRETUserTurnOptions = {}): Promise<void> {
    const session = this.requireSession();
...
function isSandboxPolicyValue(value: unknown): value is SandboxPolicy {
  ...
  if (candidate.mode === 'workspace-write') {
    ...
    if (
      workspacePolicy.writable_REDACTED_SECRETs !== undefined &&
      (!Array.isArray(workspacePolicy.writable_REDACTED_SECRETs) ||
        workspacePolicy.writable_REDACTED_SECRETs.some((entry) => typeof entry !== 'string'))
    ) {
      return false;
    }
