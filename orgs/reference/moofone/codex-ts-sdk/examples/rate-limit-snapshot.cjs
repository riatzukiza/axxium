...
Usage: REDACTED_SECRET rate-limit-snapshot.cjs [options]
...
Example:
  ...
  CODEX_INTERNAL_ORIGINATOR_OVERRIDE=codex_cli_rs REDACTED_SECRET rate-limit-snapshot.cjs

  # Test with mock data
  REDACTED_SECRET rate-limit-snapshot.cjs --mock

  # Schedule with cron (every 8 hours)
  0 */8 * * * cd /path/to/project && CODEX_INTERNAL_ORIGINATOR_OVERRIDE=codex_cli_rs REDACTED_SECRET examples/rate-limit-snapshot.cjs
  `);
  process.exit(0);
}
