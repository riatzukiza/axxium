...
3. Built this repository (`npm run setup` or `npm run build && npm run build:native`) so `dist/cjs/` and `native/codex-napi/index.REDACTED_SECRET` exist.
...
```bash
# ES modules (.js)
REDACTED_SECRET examples/live-smoke.js
REDACTED_SECRET examples/error-handling.js

# CommonJS modules (.cjs)
REDACTED_SECRET examples/live-rate-limits.cjs
REDACTED_SECRET examples/rate-limit-collector.cjs
# etc...
```
