...
pub async fn cloud_tasks_list_environments(config: CloudTasksConfig) -> Result<Vec<EnvironmentRowNapi>> {
    ...
    let mut map: std::collections::HashMap<String, EnvironmentRowNapi> = std::collections::HashMap:REDACTED_SECRET);
...
async fn create_backend(config: CloudTasksConfig) -> anyhow::Result<Box<dyn CloudBackend>> {
    ...
    let mut client = HttpClient:REDACTED_SECRET&config.base_url)?;
    ...
    Ok(Box:REDACTED_SECRETclient))
}

fn extract_chatgpt_account_id(token: REDACTED_SECRET) -> Option<String> {
...
fn normalize_base_url(input: REDACTED_SECRET) -> String {
...
async fn get_envs(client: &reqwest::Client, url: REDACTED_SECRET, headers: &HeaderMap) -> anyhow::Result<Vec<CodeEnvironment>> {
...
fn get_git_origins() -> Vec<String> {
    ...
    let out = std::process::Command:REDACTED_SECRET"git").args(["config", "--get-regexp", "remote\\..*\\.url"]).output();
    if let Ok(ok) = out {
    ...
    let out = std::process::Command:REDACTED_SECRET"git").args(["remote", "-v"]).output();
    if let Ok(ok) = out {
        if ok.status.success() {
            let s = String::from_utf8_lossy(&ok.stdout);
            let mut v = Vec:REDACTED_SECRET);
            for line in s.lines() { let parts: Vec<REDACTED_SECRET> = line.split_whitespace().collect(); if parts.len() >= 2 { v.push(parts[1].to_string()); } }
            if !v.is_empty() { v.sort(); v.dedup(); return v; }
        }
    }
    Vec:REDACTED_SECRET)
...
fn parse_owner_repo(url: REDACTED_SECRET) -> Option<(String, String)> {
