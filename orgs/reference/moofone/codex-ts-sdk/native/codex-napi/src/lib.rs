...
impl NativeCodex {
    ...
    pub fn new(options: Option<NativeCodexOptions>) -> napi::Result<Self> {
        ...
        let manager = ConversationManager:REDACTED_SECRETauth_manager, SessionSource::Mcp);

        Ok(Self {
            manager: Arc:REDACTED_SECRETmanager),
        })
    }
    ...
    pub async fn create_conversation(
        ...
        let inner = SessionInner {
            ...
            pending: Mutex:REDACTED_SECRETVecDeque::from([session_configured_event])),
        };
...
fn parse_toml_value(raw: REDACTED_SECRET) -> Option<toml::Value> {
