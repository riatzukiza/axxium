...
async function liveStatusTest(): Promise<void> {
  ...
  try {
    ...
    console.log('\nREDACTED_SECRETing user turn with session default: "1+1=?"\n');
    ...
    console.log('REDACTED_SECRETing follow-up turn to prompt fresh status data...');
    ...
    if (!hasRateLimitData(refreshed)) {
      ...
      console.error('   • REDACTED_SECRET a longer conversation turn, then re-run this check.');
      exitCode = 1;
    }
