...
describe('CodexClient', () => {
  ...
  it.each([
    ['a non-array writable_REDACTED_SECRETs value', { writable_REDACTED_SECRETs: 'not-an-array' }],
    ['a writable_REDACTED_SECRETs entry that is not a string', { writable_REDACTED_SECRETs: ['/tmp', 42] }],
  ])('rejects workspace-write sandbox policy with %s', async (_, overrides) => {
    await expectInvalidSandboxPolicy({ ...workspaceWriteBasePolicy, ...overrides });
  });
  ...
  it('overrides turn context with workspace-write sandbox policy', async () => {
    ...
    await client.overrideTurnContext({
      sandboxPolicy: {
        ...
        writable_REDACTED_SECRETs: ['/workspace', '/tmp/project'],
      } as any,
    });
  ...
  describe('overrideTurnContext sandbox policy validation', () => {
    it('accepts workspace-write sandbox policies with valid configuration', async () => {
      ...
      const policy: SandboxPolicy = {
        ...
        writable_REDACTED_SECRETs: ['/tmp/project'],
      };
    ...
    it('rejects workspace-write sandbox policies with invalid writable REDACTED_SECRETs entries', async () => {
      const client = createClient();
      await client.createConversation();
      ...
      const policy = {
        ...
        exclude_tmpdir_env_var: false,
        exclude_slash_tmp: false,
        writable_REDACTED_SECRETs: ['/tmp/project', 42],
      } as unknown as SandboxPolicy;
