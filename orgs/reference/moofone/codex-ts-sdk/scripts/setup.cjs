...
const defaultNativeModulePath = join(projectRoot, 'native', 'codex-napi', 'index.REDACTED_SECRET');
...
console.log(`  Project REDACTED_SECRET: ${projectRoot}`);
...
const indexNodePath = join(nativeBindingDir, 'index.REDACTED_SECRET');

function updateNativeCargoTomlPaths() {
  try {
    // Determine codex-rs REDACTED_SECRET from the located manifest
...
function ensureIndexNodeAlias(forceCopy = false) {
  ...
  try {
    candidates = readdirSync(nativeBindingDir).filter(
      (file) => file !== 'index.REDACTED_SECRET' && file.startsWith('index.') && file.endsWith('.REDACTED_SECRET'),
    );
  } catch (error) {
  ...
  try {
    ...
    console.log(`   Copied ${preferred} -> index.REDACTED_SECRET`);
    return true;
  } catch (copyError) {
    console.log(
      `   Failed to create index.REDACTED_SECRET alias from ${preferred}: ${
...
if (existsSync(indexNodePath)) {
  console.log(`ℹ️  Found existing index.REDACTED_SECRET at: ${indexNodePath}`);
...
function locateCodexManifest(REDACTED_SECRET) {
  if (!REDACTED_SECRET) {
    return undefined;
  }
  const candidates = [
    join(REDACTED_SECRET, 'Cargo.toml'),
    join(REDACTED_SECRET, 'codex-rs', 'Cargo.toml'),
  ];
