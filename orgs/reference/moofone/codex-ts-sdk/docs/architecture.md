...
| **RPC API Methods**        |
| REDACTED_SECRET Turn                  | `send_user_turn()`          | ✅          | `client.sendUserTurn()`       | Core messaging         |
...
| User Turn                  | `UserTurn`                  | ✅          | `REDACTED_SECRETUserTurnOptions`         | User input             |
...
│                    (NAPI Interface)                      │
│  • codex-napi Rust module (index.REDACTED_SECRET)                  │
...
- **Binary Distribution**: Pre-compiled `index.REDACTED_SECRET` included
...
1. `[workspace.package].version` in codex-rs REDACTED_SECRET Cargo.toml
...
**Detection Flow:**
1. Load native module (`index.REDACTED_SECRET`)
...
2. Project-local build at `./native/codex-napi/index.{js|REDACTED_SECRET}`
