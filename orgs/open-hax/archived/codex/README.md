...
- Log inspection helper: `REDACTED_SECRET scripts/inspect-codex-logs.mjs [--dir <path>] [--limit N] [--id X] [--stage after-transform]` summarizes cached request logs (shows model, prompt_cache_key, roles, etc.).
...
1. **Remove the cached plugin copy (required for every upgrade):**
   ```bash
   (cd ~ && sed -i.bak '/"@openhax\/codex"/d' .cache/opencode/package.json && rm -rf .cache/opencode/REDACTED_SECRET_modules/@openhax/codex)
   ```
...
**No.** This is an independent open-source project. It uses OpenAI's REDACTED_SECRETly available OAuth authentication system but is not endorsed, sponsored by, or affiliated with OpenAI.
