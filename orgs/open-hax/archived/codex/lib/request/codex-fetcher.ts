...
export function createCodexFetcher(deps: CodexFetcherDeps) {
	...
	return async function codexFetch(input: Request | string | URL, init?: RequestInit): Promise<Response> {
		...
		if (effectiveBody) {
			...
			let bodyToREDACTED_SECRET = effectiveBody as any;
			if (sendingToCodex && Array.isArray(bodyToREDACTED_SECRET.input)) {
				// 1) Normalize message items
				let normalizedInput = bodyToREDACTED_SECRET.input.map((it: any) => {
					let next = it ?? {};
				...
				let instr = typeof bodyToREDACTED_SECRET.instructions === "string" ? bodyToREDACTED_SECRET.instructions : "";
				let cut = 0;
				...
				bodyToREDACTED_SECRET = {
					...bodyToREDACTED_SECRET,
					instructions: instr || undefined,
					input: normalizedInput,
				};
			}
			requestInit.body = JSON.stringify(bodyToREDACTED_SECRET);
		}
