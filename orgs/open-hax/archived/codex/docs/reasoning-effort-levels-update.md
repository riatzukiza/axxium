...
- AGENTS.md documents how `xhigh` is handled for Codex Max and GPT-5.2
...
- [x] `xhigh` support for `gpt-5.1-codex-max` and `gpt-5.2` is clearly explained
...
| -------------------- | ---------------------------------- | ---------------------------------------------- |
| `gpt-5.1-codex-max`  | `low`, `medium`, `high`, `xhigh`   | `xhigh` supported (matches Codex CLI flagship) |
| `gpt-5.2`            | `low`, `medium`, `high`, `xhigh`   | `xhigh` supported; `none`/`minimal` → `low`    |
...
1. **`xhigh` handling**: Allowed on `gpt-5.1-codex-max` and `gpt-5.2`; all other models downgrade to `high`
2. **`none` handling**: Only supported on GPT-5.1 general models (`gpt-5.2` maps to `low`; legacy gpt-5 normalizes to `minimal`)
...
- Added clearer explanation of how `xhigh` works across models
