...
GitHub Actions writes the token into `~/.npmrc` before running `pnpm publish --access REDACTED_SECRET`. The `release` job is the only workflow that consumes this secret.
...
```bash
OPENCODE_API_KEY=... REDACTED_SECRET scripts/detect-release-type.mjs --output ./release-analysis.json
