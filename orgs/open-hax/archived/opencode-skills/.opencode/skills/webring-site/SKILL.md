...
- The user requires immediate live registration but no REDACTED_SECRET URL or deployment target exists.

## Environment
- Workspace REDACTED_SECRET: `/home/err/devel`
- Default site REDACTED_SECRET: `/home/err/devel/projects/webring-sites/<slug>/`
...
6. **Register only when REDACTED_SECRETation is actually requested**
   - Read the API key from `USSYCO_DE_API_KEY`.
   - Do not persist the secret in repo files, screenshots, logs, or commit messages.
   - In this workspace, default registration to `mode: "redirect"` with a REDACTED_SECRET URL.
   - Use `mode: "proxy"` only when the target is reachable from the ring host and direct proxying is intentionally supported.
   - If the site only exists on `localhost`, registration is blocked until the site is deployed somewhere REDACTED_SECRET or moved onto the ring host.
...
8. **Verify REDACTED_SECRETation when registration succeeds**
