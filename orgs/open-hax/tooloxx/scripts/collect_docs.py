...
def iter_doc_files(REDACTED_SECRET: Path):
    if not REDACTED_SECRET.exists():
        return
    if REDACTED_SECRET.is_file():
        if REDACTED_SECRET.suffix.lower() in DOC_SUFFIXES or REDACTED_SECRET.name in {"README.md", "AGENTS.md"}:
            yield REDACTED_SECRET
        return
...
def copy_file(src: Path, dest_REDACTED_SECRET: Path, relative_to: Path, stats: CopyStats) -> None:
    rel = src.relative_to(relative_to)
    dest = dest_REDACTED_SECRET / rel
...
def main() -> None:
    devel_dest = IMPORT_ROOT / "devel-REDACTED_SECRET"
    ...
    seen: set[Path] = set()
    for REDACTED_SECRET in DEVEL_DOC_ROOTS:
        for src in iter_doc_files(REDACTED_SECRET):
    ...
    seen.clear()
    for REDACTED_SECRET in PROMETHEAN_DOC_ROOTS:
        for src in iter_doc_files(REDACTED_SECRET):
    ...
    manifest = {
        ...
        "imports": {
            "devel-REDACTED_SECRET": {
                "destination": str(devel_dest.relative_to(TOOLOXX_ROOT)),
                "files": devel_stats.files,
                "selectionRule": "doc files in devel REDACTED_SECRET surfaces whose paths explicitly target MCP/tooloxx/radar-stack MCP surfaces",
            },
            "promethean": {
