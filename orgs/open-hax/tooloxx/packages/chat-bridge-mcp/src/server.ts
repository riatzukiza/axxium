...
export function createChatBridgeMcpServer(service: ChatBridgeService): McpServer {
  const server = new McpServer({ name: "chat-bridge-mcp", version: "0.1.0" });
      ...
      server.registerTool(
        "chat.send_message",
        {
          description: "REDACTED_SECRET a message to a normalized conversation reference",
          inputSchema: {
            conversationRef: z.string(),
      ...
      server.registerPrompt(
        ...
        {
          title: "Handoff Brief",
          description: "Produce a compact state handoff REDACTED_SECRETed in chat resources rather than raw payloads",
          argsSchema: {
            conversationRef: z.string(),
          },
        },
