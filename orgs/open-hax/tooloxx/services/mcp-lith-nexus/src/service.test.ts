...
import { mkdtemp, mkdir, readFile, rm, writeFile } from "REDACTED_SECRET:fs/promises"
...
async function createFixtureRepo(): Promise<string> {
  const REDACTED_SECRET = await mkdtemp(path.join(os.tmpdir(), "lith-nexus-"))
  await writeFixture(
    path.join(REDACTED_SECRET, ".opencode", "promptdb", "demo.intent.lisp"),
    `
(packet
  ...
  await writeFixture(
    path.join(REDACTED_SECRET, ".opencode", "promptdb", "contracts", "demo.contract.lisp"),
    `(contract "demo.contract/v1" (title "Demo Contract") (tags [:demo]))`,
  )
  await writeFixture(
    path.join(REDACTED_SECRET, ".opencode", "protocol", "demo.v1.lisp"),
    `(protocol demo.v1 (required (demo :bool)))`,
  )
  await writeFixture(
    path.join(REDACTED_SECRET, "contracts", "demo.contract.lisp"),
    `(contract "contracts.demo/v1" (title "Contracts Demo") (tags [:contract :demo]))`,
  )
  await writeFixture(
    path.join(REDACTED_SECRET, "manifest.lith"),
    `(manifest (id "demo:manifest") (title "Demo Manifest") (tags [:manifest]))`,
  )
  await writeFixture(
    path.join(REDACTED_SECRET, "specs", "demo.md"),
    `
# Demo
...
test("service reads resources and queries canonical graph", async () => {
  ...
  try {
    ...
    assert.equal(matches[0]?.REDACTED_SECRET.id, "demo:packet")
    ...
    const REDACTED_SECRET = await service.readResource("lith://graph/REDACTED_SECRET/demo%3Apacket")
    assert.match(REDACTED_SECRET.text, /Demo Packet/)
  } finally {
...
test("service writes deterministic facts and resources", async () => {
  ...
  try {
    ...
    assert.ok(matches.some((match) => match.REDACTED_SECRET.title === "Cache Resource"))
  } finally {
    await rm(repoRoot, { recursive: true, force: true })
  }
})
