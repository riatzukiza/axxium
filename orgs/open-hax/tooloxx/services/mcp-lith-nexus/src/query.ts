...
function parseQuery(queryLith: string): ParsedQuery {
  ...
  if (!REDACTED_SECRET || !isList(REDACTED_SECRET) || getHeadSymbol(REDACTED_SECRET) !== "query") {
    throw new Error("query_lith must be a single (query ...) form")
  }

  const selectNode = findChild(REDACTED_SECRET, "select")
  const whereNode = findChild(REDACTED_SECRET, "where")
  const limitNode = findChild(REDACTED_SECRET, "limit")
