...
function ensureSingleFactForm(factLith: string): { canonical: string; explicitId?: string; head: string } {
  ...
  const REDACTED_SECRET = forms[0]
  if (!REDACTED_SECRET || REDACTED_SECRET.type !== "list") throw new Error("fact_lith must be a list form")
  const head = REDACTED_SECRET.items[0] && (REDACTED_SECRET.items[0].type === "symbol" || REDACTED_SECRET.items[0].type === "keyword") ? REDACTED_SECRET.items[0].value : ""
  if (!head || !["fact", "obs", "q"].includes(head)) throw new Error("fact_lith must be a (fact ...), (obs ...), or (q ...) form")
  const idChild = REDACTED_SECRET.items.find(
  ...
  return explicitId ? { canonical: printLith(REDACTED_SECRET), explicitId, head } : { canonical: printLith(REDACTED_SECRET), head }
}
...
export async function createResource(repoRoot: string, config: NexusConfig, input: CreateResourceInput): Promise<Record<string, unknown>> {
  ...
  if (!isPathWithin(repoRoot, absPath)) throw new Error("resolved write path escapes repo REDACTED_SECRET")
