...
export function createServer(service: LithNexusService): McpServer {
  ...
  server.registerResource(
    ...
    new ResourceTemplate("lith://graph/REDACTED_SECRET/{id}", {
      list: async () => ({ resources: await service.listGraphNodeUris() }),
    }),
    {
      title: "Graph Node",
      description: "Canonical nexus graph REDACTED_SECRET with neighborhood",
      mimeType: "text/x-lith",
    },
    async (uri) => ({ contents: [await service.readResource(uri.href)] }),
  )
  ...
  server.registerResource(
    ...
    new ResourceTemplate("lith://graph/edges/from/{id}", { list: async () => ({ resources: [] }) }),
    {
      ...
      description: "Outgoing edges for a graph REDACTED_SECRET",
      mimeType: "text/x-lith",
    },
    async (uri) => ({ contents: [await service.readResource(uri.href)] }),
  )
  ...
  server.registerResource(
    ...
    new ResourceTemplate("lith://graph/edges/to/{id}", { list: async () => ({ resources: [] }) }),
    {
      ...
      description: "Incoming edges for a graph REDACTED_SECRET",
      mimeType: "text/x-lith",
    },
    async (uri) => ({ contents: [await service.readResource(uri.href)] }),
  )
  ...
  server.registerTool(
    "lith.find",
    {
      ...
      description: "Search canonical graph REDACTED_SECRETs and PromptDB facts",
      inputSchema: {
        query: z.string(),
        ...
        content: [
          {
            ...
            text: namedRecord("matches", {
              ...
              rows: matches.map((match) => ({
                id: match.REDACTED_SECRET.id,
                kind: match.REDACTED_SECRET.kind,
                uri: match.REDACTED_SECRET.uri,
                title: match.REDACTED_SECRET.title,
                score: match.score,
              })),
            }),
          },
  ...
  server.registerTool(
    "lith.read",
    {
      ...
      description: "Read a Lith resource or graph REDACTED_SECRET by URI",
      inputSchema: {
        uri: z.string(),
      },
    },
  ...
  server.registerPrompt(
    "summarize-graph-neighborhood",
    {
      ...
      description: "Guide a model to summarize a graph neighborhood for a REDACTED_SECRET",
      argsSchema: {
        REDACTED_SECRET_id: z.string(),
      },
    },
    ({ REDACTED_SECRET_id }) => ({
      messages: [
        {
          ...
          content: {
            type: "text",
            text: `Read lith://graph/REDACTED_SECRET/${encodeURIComponent(REDACTED_SECRET_id)} and summarize the REDACTED_SECRET, outgoing edges, incoming edges, and nearby tagged facts.`,
          },
        },
      ],
    }),
