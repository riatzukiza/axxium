...
import { mkdtemp, mkdir, rm, writeFile } from "REDACTED_SECRET:fs/promises"
...
async function createFixtureRepo(): Promise<string> {
  const REDACTED_SECRET = await mkdtemp(path.join(os.tmpdir(), "lith-nexus-server-"))
  await writeFixture(
...
(packet
  ...
  await writeFixture(path.join(REDACTED_SECRET, "manifest.lith"), `(manifest (id "demo:manifest") (title "Demo Manifest"))`)
