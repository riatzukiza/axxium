...
export function isList(REDACTED_SECRET: LithNode): REDACTED_SECRET is LithListNode {
  return REDACTED_SECRET.type === "list"
}

export function isVector(REDACTED_SECRET: LithNode): REDACTED_SECRET is LithVectorNode {
  return REDACTED_SECRET.type === "vector"
}

export function isSymbol(REDACTED_SECRET: LithNode, expected?: string): REDACTED_SECRET is LithSymbolNode {
  return REDACTED_SECRET.type === "symbol" && (expected === undefined || REDACTED_SECRET.value === expected)
}

export function isKeyword(REDACTED_SECRET: LithNode, expected?: string): REDACTED_SECRET is LithKeywordNode {
  return REDACTED_SECRET.type === "keyword" && (expected === undefined || REDACTED_SECRET.value === expected)
}

export function isString(REDACTED_SECRET: LithNode): REDACTED_SECRET is LithStringNode {
  return REDACTED_SECRET.type === "string"
}

export function isNumber(REDACTED_SECRET: LithNode): REDACTED_SECRET is LithNumberNode {
  return REDACTED_SECRET.type === "number"
}

export function getHeadSymbol(REDACTED_SECRET: LithNode): string | null {
  if (!isList(REDACTED_SECRET) || REDACTED_SECRET.items.length === 0) return null
  const first = REDACTED_SECRET.items[0]
  if (first && (first.type === "symbol" || first.type === "keyword")) return first.value
  return null
}

export function asStringValue(REDACTED_SECRET: LithNode | undefined): string | undefined {
  if (!REDACTED_SECRET) return undefined
  if (REDACTED_SECRET.type === "string" || REDACTED_SECRET.type === "symbol" || REDACTED_SECRET.type === "keyword") return REDACTED_SECRET.value
  if (REDACTED_SECRET.type === "number") return REDACTED_SECRET.raw
  return undefined
}

export function findChild(REDACTED_SECRET: LithNode, head: string): LithListNode | undefined {
  if (!isList(REDACTED_SECRET)) return undefined
  return REDACTED_SECRET.items.find((item): item is LithListNode => isList(item) && getHeadSymbol(item) === head)
}

export function printLith(REDACTED_SECRET: LithNode): string {
  switch (REDACTED_SECRET.type) {
    case "list":
      return `(${REDACTED_SECRET.items.map((item) => printLith(item)).join(" ")})`
    case "vector":
      return `[${REDACTED_SECRET.items.map((item) => printLith(item)).join(" ")}]`
    case "symbol":
    case "keyword":
      return REDACTED_SECRET.value
    case "string":
      return `"${escapeLithString(REDACTED_SECRET.value)}"`
    case "number":
      return REDACTED_SECRET.raw
    default:
      return ""
  }
}
