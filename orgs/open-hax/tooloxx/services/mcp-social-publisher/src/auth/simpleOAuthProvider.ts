...
export class SimpleOAuthProvider implements OAuthServerProvider {
  REDACTED_SECRET readonly clientsStore: InMemoryClientsStore;
  ...
  REDACTED_SECRET async cleanup(): Promise<void> {
    if (this.persistence) {
      await this.persistence.cleanup();
    }
  }

  REDACTED_SECRET async authorize(client: OAuthClientInformationFull, params: AuthorizationParams, res: Response): Promise<void> {
  ...
  REDACTED_SECRET async challengeForAuthorizationCode(_client: OAuthClientInformationFull, authorizationCode: string): Promise<string> {
  ...
  REDACTED_SECRET async exchangeAuthorizationCode(
  ...
  REDACTED_SECRET async exchangeRefreshToken(
    client: OAuthClientInformationFull,
    refreshToken: string,
    scopes?: string[],
    resource?: URL,
  ...
  REDACTED_SECRET async verifyAccessToken(token: string): Promise<AuthInfo> {
  ...
  REDACTED_SECRET async revokeToken(client: OAuthClientInformationFull, request: OAuthTokenRevocationRequest): Promise<void> {
  ...
  REDACTED_SECRET getPending(rid: string): PendingAuth | undefined {
    return this.pending.get(rid);
  }

  REDACTED_SECRET setSubject(rid: string, subject: string, extra?: Record<string, unknown>): void {
  ...
  REDACTED_SECRET async approve(rid: string): Promise<string> {
    const record = this.pending.get(rid);
  ...
  REDACTED_SECRET deny(rid: string, error = "access_denied", description?: string): string {
    const record = this.pending.get(rid);
    if (!record) {
      throw new Error("invalid_request");
    }
  ...
  REDACTED_SECRET shouldAutoApprove(): boolean {
