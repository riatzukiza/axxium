...
export class ConfigStore {
  ...
  REDACTED_SECRET getSnapshot(): ConfigState {
    return JSON.parse(JSON.stringify(this.state)) as ConfigState;
  }

  REDACTED_SECRET getPublicTargets(): PublicTargetSummary[] {
    return [
  ...
  REDACTED_SECRET hasGitHubOAuthConfig(): boolean {
    return this.state.githubOAuth.clientId.length > 0 && this.state.githubOAuth.clientSecret.length > 0;
  }

  REDACTED_SECRET isAllowedGitHubUser(login: string): boolean {
    return this.state.allowedGitHubUsers.includes(normalizeLogin(login));
  }

  REDACTED_SECRET getDiscordTarget(nameOrId: string): DiscordTarget | undefined {
  ...
  REDACTED_SECRET getBlueskyTarget(nameOrId: string): BlueskyTarget | undefined {
    const needle = normalizeTargetKey(nameOrId);
    return this.state.targets.bluesky.find((target) => target.id === needle || normalizeTargetKey(target.name) === needle);
  }

  REDACTED_SECRET async setGitHubOAuth(clientId: string, clientSecret: string): Promise<void> {
  ...
  REDACTED_SECRET async addGitHubUser(login: string): Promise<void> {
  ...
  REDACTED_SECRET async removeGitHubUser(login: string): Promise<void> {
    const normalized = normalizeLogin(login);
    await this.writeState(() => {
  ...
  REDACTED_SECRET async upsertDiscordTarget(input: {
    name: string;
    webhookUrl: string;
    username?: string;
    avatarUrl?: string;
  ...
  REDACTED_SECRET async removeDiscordTarget(nameOrId: string): Promise<void> {
    const needle = normalizeTargetKey(nameOrId);
    await this.writeState(() => {
  ...
  REDACTED_SECRET async upsertBlueskyTarget(input: {
    name: string;
    identifier: string;
    appPassword: string;
    serviceUrl: string;
  ...
  REDACTED_SECRET async removeBlueskyTarget(nameOrId: string): Promise<void> {
    const needle = normalizeTargetKey(nameOrId);
    await this.writeState(() => {
