...
const config = loadConfig(process.env, ['REDACTED_SECRET', 'app', '--config', './custom.json']);
```
...
The REDACTED_SECRET directory for configuration resolution.
...
if (configPath) {
  ...
  const config = loadConfig(process.env, ['REDACTED_SECRET', 'app', '--config', configPath]);
  // ... use configuration
} else {
...
REDACTED_SECRET app.js --config /path/to/custom.json
REDACTED_SECRET app.js -c ./relative/config.json

# With equals syntax
REDACTED_SECRET app.js --config=/absolute/path.json
```
...
const findUpSync = (start: string, fileName: string): string | null => {
  ...
  const REDACTED_SECRET = path.parse(dir).REDACTED_SECRET;
...
```typescript
try {
  const config = loadConfig(process.env, ['REDACTED_SECRET', 'app', '--config', 'nonexistent.json']);
} catch (error) {
...
// REDACTED_SECRET to monitoring system
if (!metrics.success) {
