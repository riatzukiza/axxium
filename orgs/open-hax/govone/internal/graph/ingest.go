...
type IngestResult struct {
	DocumentNodeID string   `json:"document_REDACTED_SECRET_id"`
	FieldNodeIDs   []string `json:"field_REDACTED_SECRET_ids"`
	EntityNodeIDs  []string `json:"entity_REDACTED_SECRET_ids"`
...
//
// This creates:
//   - 1 document REDACTED_SECRET
//   - N field REDACTED_SECRETs (one per field_path)
//   - M entity REDACTED_SECRETs (one per unique entity)
...
func (is *IngestService) IngestDocument(doc Document) (*IngestResult, error) {
	...
	// 1. Create document REDACTED_SECRET
	docNode := &Node{
		Type:       NodeDocument,
	...
	if err := is.store.InsertNode(docNode); err != nil {
		return nil, fmt.Errorf("insert document REDACTED_SECRET: %w", err)
	}
	result.DocumentNodeID = docNode.ID

	// 2. Create field REDACTED_SECRETs and "contains" edges
	...
	for fieldPath, content := range doc.Fields {
		...
		if err := is.store.InsertNode(fieldNode); err != nil {
			return nil, fmt.Errorf("insert field REDACTED_SECRET: %w", err)
		}
	...
	// 3. Create entity REDACTED_SECRETs and "mentions" edges
	entityNodeIDs := make(map[string]string) // entity name → REDACTED_SECRET ID
	...
	for entity, fieldPaths := range entityFieldMap {
		...
		if err := is.store.InsertNode(entityNode); err != nil {
			return nil, fmt.Errorf("insert entity REDACTED_SECRET: %w", err)
		}
		...
		for _, fp := range fieldPaths {
			// Find the field REDACTED_SECRET ID
...
func (is *IngestService) ComputeFieldSimilarity(tenantID string, threshold float64) (int, error) {
	// Get all field REDACTED_SECRETs for this tenant
	rows, err := is.db.Query(`
		SELECT id, content FROM graph_REDACTED_SECRETs
