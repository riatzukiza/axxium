...
//
//  2. Claim Resolution: Map each claim to graph REDACTED_SECRETs/entities via string matching
//     and entity resolution against the verified fact store.
//
//  3. Subgraph Retrieval: Get the verified subgraph (2-hop) around resolved REDACTED_SECRETs.
//
//  4. Coherence Scoring:
//     For each claim:
...
type Claim struct {
	...
	EntityIDs []string `json:"entity_ids,omitempty"` // resolved entity REDACTED_SECRET IDs
}
...
func (ci *CoherenceInspector) Inspect(ctx context.Context, tenantID string, output string, claims []Claim, scoreKey string) (*InspectionResult, error) {
	...
	// Phase 1: Resolve claims to graph REDACTED_SECRETs
	resolvedClaims := 0
	for i := range claims {
