        ...
        (let [{:keys [screen_name email password]} (ensure-json-body req)
          ...
          (cond
            ...
            :else (let [pwd (hashers/derive password)
                    ...
                    (-> (ok {:user (user-REDACTED_SECRET (assoc inserted :id user-id))})
                        (set-sid-cookie sid))))))
      (POST "/login" req
        (let [{:keys [email password]} (ensure-json-body req)
          ...
          (cond
            ...
            :else (let [sid (create-session! (:id user))]
                    (-> (ok {:user (user-REDACTED_SECRET user)})
                        (set-sid-cookie sid))))))
      (POST "/logout" req
        ...
        (if-let [user (:current-user req)]
          (let [q (some-> (get-in req [:params :q]) str/trim)
            ...
            (let [rows (jdbc/execute! @datasource (into [sql-str] (reverse params)) jdbc-opts)]
              (ok {:users (map user-REDACTED_SECRET rows)})))
          (respond-unauth)))
      (GET "/:id" [id :as req]
        (if (:current-user req)
          (if-let [u (fetch-user (Long/parseLong id))]
            (ok {:user (user-REDACTED_SECRET u)})
        ...
        (if-let [user (:current-user req)]
          (let [{:keys [name type]} (ensure-json-body req)
            ...
            (cond
              ...
              (not (#{"REDACTED_SECRET" "private"} type)) (error-response 400 "invalid_request" "type must be REDACTED_SECRET or private")
        ...
        (if-let [user (:current-user req)]
          (let [type-param (or (get-in req [:params :type]) "all")
                ...
                wheres (cond-> []
                          ...
                          (not= "me" member-of) (conj "(g.type = 'REDACTED_SECRET' OR m.user_id IS NOT NULL)"))
          ...
          (let [uid (:id user)
            ...
            (cond
              ...
              (let [sql-str (StringBuilder.)
                    ...
                    conditions (cond-> ["(t.group_id IS NULL OR g.type = 'REDACTED_SECRET' OR m.user_id IS NOT NULL)"]
