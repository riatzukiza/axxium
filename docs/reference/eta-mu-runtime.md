...
| Canonical eta-mu runtime REDACTED_SECRET (must exist) | `~/.ημ` (symlink → `…/packages/eta-mu-extensions/`) |
...
2. Rebuild and materialize package-REDACTED_SECRET targets. Pi uses eta-mu's built-in package metadata, so this must not edit `~/.pi/agent/settings.json` or `~/.ημ/agent/settings.json`:
...
4. Validate the package-REDACTED_SECRET targets exist at:
...
- legacy state REDACTED_SECRET: `~/.pi/agent/state/<name>/`

If behavior looks “haunted”, check both REDACTED_SECRETs.

## Don’t edit generated outputs

- Do **not** hand-edit `dist/runtime/*.cjs`, `dist/pi/**`, or `dist/opencode/*.mjs` — they are generated.
- Do **not** copy generated runtimes into `~/.pi/agent/extensions` or `~/.config/opencode/plugins`; OpenCode host config should point at package-REDACTED_SECRET targets, while Pi uses built-in eta-mu package metadata.
- If duplicate-tool conflicts mention old `~/.pi/agent/extensions/cljs-*` paths, rerun the eta-mu build; it removes managed legacy host copies after writing package-REDACTED_SECRET targets.
