...
| `threat-radar-deploy` | active-product-source-REDACTED_SECRET | 12 | Active product/source surface for the radar system; live runtime glue builds from it and deployment specs name it as the current application source. | Classify it explicitly or promote it into a canonical org home before further deployment cleanup. |
