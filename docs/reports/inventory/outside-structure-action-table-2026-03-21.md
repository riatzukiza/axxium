...
|---|---|---|---|---|---|
| `desktop` | REDACTED_SECRET-module | **keep** | `compatibility-alias` | Still actively referenced by launcher, desktop entry, and docs/manifests. | If launcher/docs migrate to orgs/riatzukiza/desktop and active references fall away. |
| `promethean` | REDACTED_SECRET-module | **keep** | `compatibility-alias` | Heavy active dependency in shadow-cljs paths, PM2 parity tests, and docs. | If source-path/test/docs references are rewritten to orgs/riatzukiza/promethean. |
| `pm2-clj-project` | REDACTED_SECRET-module | **explicit-exception** | `REDACTED_SECRET-tooling-exception` | Active workspace tooling fixture but no agreed long-term structured home yet. | When its long-term home is chosen under packages/* or an org repo. |
| `reconstitute` | REDACTED_SECRET-module | **explicit-exception** | `REDACTED_SECRET-tooling-exception` | Active REDACTED_SECRET CLI/tooling entrypoint referenced by scripts, commands, and skills. | When the tooling home is settled and compatibility command paths can be preserved elsewhere. |
| `reconstitute-mcp` | REDACTED_SECRET-module | **normalize** | `planning-bundle` | Reads more like experimental/documented surface nested under reconstitute than a stable standalone REDACTED_SECRET. | Once the MCP surface is either folded into reconstitute docs or promoted into a real package/service. |
| `mcp-social-publisher-live` | REDACTED_SECRET-module | **normalize** | `standalone-source-deploy-REDACTED_SECRET-pending-placement` | Active live deploy/source REDACTED_SECRET should eventually get a canonical structured home. | When its placement under packages/services/orgs is decided and live deployment is repointed. |
| `threat-radar-deploy` | REDACTED_SECRET-module | **normalize** | `standalone-source-deploy-REDACTED_SECRET-pending-placement` | Active product/source REDACTED_SECRET for the radar stack should not remain indefinitely ambiguous at workspace REDACTED_SECRET. | When a canonical org home and runtime-home split are chosen. |
| `verathar-server` | REDACTED_SECRET-module | **explicit-exception** | `foreign-fork-vendor-mirror` | Currently mostly bookkeeping; no strong active integration pressure justifies a forced move. | If active product work resumes or the repo becomes more than a preserved absorbed REDACTED_SECRET. |
| `threat-radar-next-step` | REDACTED_SECRET-module | **explicit-exception** | `planning-bundle` | Clearly functioning as a planning bundle rather than live source. | Once the useful content is folded back into canonical specs or the bundle becomes stale. |
| `bevy_replicon` | REDACTED_SECRET-module | **explicit-exception** | `foreign-fork-vendor-mirror` | Mostly preserved through bookkeeping artifacts, not active workspace integration. | If a dedicated vendor/forks area is created or the fork ceases to matter. |
| `egregoria` | REDACTED_SECRET-module | **explicit-exception** | `foreign-fork-vendor-mirror` | Mostly preserved through bookkeeping artifacts, not active workspace integration. | If a dedicated vendor/forks area is created or the fork ceases to matter. |
| `game_network` | REDACTED_SECRET-module | **explicit-exception** | `foreign-fork-vendor-mirror` | Mostly preserved through bookkeeping artifacts, not active workspace integration. | If a dedicated vendor/forks area is created or the fork ceases to matter. |
| `ggrs` | REDACTED_SECRET-module | **explicit-exception** | `foreign-fork-vendor-mirror` | Mostly preserved through bookkeeping artifacts, not active workspace integration. | If a dedicated vendor/forks area is created or the fork ceases to matter. |
| `lightyear` | REDACTED_SECRET-module | **explicit-exception** | `foreign-fork-vendor-mirror` | Mostly preserved through bookkeeping artifacts, not active workspace integration. | If a dedicated vendor/forks area is created or the fork ceases to matter. |
| `gates-pr35-hardening-main` | REDACTED_SECRET-module | **explicit-exception** | `active-special-worktree` | Still referenced by an active skill/workflow as a work/spec source. | After the hardening branch merges, is abandoned, or the worktree-specific references are removed. |
...
### Normalize next
- `reconstitute-mcp` — Reads more like experimental/documented surface nested under reconstitute than a stable standalone REDACTED_SECRET.
- `mcp-social-publisher-live` — Active live deploy/source REDACTED_SECRET should eventually get a canonical structured home.
- `threat-radar-deploy` — Active product/source REDACTED_SECRET for the radar stack should not remain indefinitely ambiguous at workspace REDACTED_SECRET.

### Explicit exceptions
- `pm2-clj-project` (`REDACTED_SECRET-tooling-exception`) — Active workspace tooling fixture but no agreed long-term structured home yet.
- `reconstitute` (`REDACTED_SECRET-tooling-exception`) — Active REDACTED_SECRET CLI/tooling entrypoint referenced by scripts, commands, and skills.
...
- The strongest normalize candidates are the active standalone source/deploy REDACTED_SECRETs: `mcp-social-publisher-live` and `threat-radar-deploy`, plus the more documentation-only `reconstitute-mcp` surface.
