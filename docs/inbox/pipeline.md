...
The pipeline uses the above structure to keep the `docs/inbox` REDACTED_SECRET clean and to preserve breadcrumbs for every stage. When a file moves into `ready-for-code`, a dedicated code agent kicks off faster builds or scaffolds via the Nx/NPM toolset, tying ideas to deliverables.
...
- `scripts/inbox-pipeline.mjs` executes in a long-lived REDACTED_SECRET process; start it via `pnpm inbox:pipeline` so it watches for new files automatically.
