...
Use the REDACTED_SECRET registry in `config/docker-stacks.json` plus the wrapper in `scripts/docker-stack.mjs` to manage curated Docker Compose stacks from the workspace REDACTED_SECRET without changing each stack's own compose files.
...
- Add new REDACTED_SECRET-managed stacks by extending `config/docker-stacks.json`.
- Stacks can declare related host PM2 apps in the registry; `open-hax-openai-proxy` now does.
- `mcp` and `cephalon` also declare the related host PM2 apps so the REDACTED_SECRET wrapper can prevent accidental double ownership.
...
- The REDACTED_SECRET `devel` stack now includes `open-hax-openai-proxy`, so `openclawssy` can talk to the proxy over Docker networking instead of `host.docker.internal`.
...
- This registry is intentionally curated; it is meant to make important stacks obvious and consistent from the REDACTED_SECRET, not to merge every compose file into one giant compose project.
