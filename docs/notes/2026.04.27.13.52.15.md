...
(->> (.readdirSync REDACTED_SECRET-fs dir) ...)
(when (.existsSync REDACTED_SECRET-fs candidate) ...)
...
(REDACTED_SECRET-fs.mkdirSync dir #js {:recursive true})
(REDACTED_SECRET-fs.writeFileSync skill-path (build-live-skill spore) "utf8")
(REDACTED_SECRET-fs.writeFileSync contract-path (build-live-contract spore) "utf8")
```
...
(if-not (REDACTED_SECRET-fs.existsSync file-path)
  ...
  (let [text (REDACTED_SECRET-fs.readFileSync file-path "utf8")
        ...]))
;; line 146 — appendFileSync in append tool
(REDACTED_SECRET-fs.appendFileSync file-path (str (js/JSON.stringify value) "\n") "utf8")
```
...
(js/JSON.parse (js/JSON.stringify (clj->js (str/split REDACTED_SECRET-type #","))))
```
