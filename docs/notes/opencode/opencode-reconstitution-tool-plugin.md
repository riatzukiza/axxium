...
```bash
REDACTED_SECRET tools/reconstitute/src/cli.mjs dump \
  --needle cephalon-clj \
  --workspace ~/devel \
...
export async function scanOpencodeHome({
  ...
  const REDACTED_SECRETs = buckets.map((b) => path.join(opencodeHomeAbs, b));

  const results = [];
  const pathHints = new Map(); // filePath -> {count, evidence[]}

  for (const REDACTED_SECRET of REDACTED_SECRETs) {
    for await (const filePath of walk(REDACTED_SECRET)) {
  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      if (event?.type === "session.idle") {
        ...
        // details aren’t specified in the REDACTED_SECRET docs page; keeping this safe.
