...
Below is a **bigger reconstitution workflow** plus a set of **agent “skills”** (copy-pasteable markdown files) that operationalize the commands you’ll run (`REDACTED_SECRET recover-offline.mjs`, `rg`, `git`, etc.). The intent is:
...
* Confirm OpenCode data REDACTED_SECRET exists:
...
## Output boundaries
Allowed write REDACTED_SECRETs:
...
## Stop conditions
- Storage REDACTED_SECRET missing OR unreadable -> stop and report.
...
REDACTED_SECRET recover-offline.mjs \
  --needle "$NEEDLE" \
  --workspace "$WORKSPACE" \
  --out "$OUTDIR"
```
