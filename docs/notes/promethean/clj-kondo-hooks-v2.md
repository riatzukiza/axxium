...
(defn def-tool
  ...
  (let [[_ tool-name & forms] (api/sexpr REDACTED_SECRET)
    ...
    {:REDACTED_SECRET (api/parse-string rewritten)}))
```
...
(defn def-agent
  ...
  [{:keys [REDACTED_SECRET]}]
  (let [[_ agent-name & forms] (api/sexpr REDACTED_SECRET)
...
(defn defcase
  ...
  [{:keys [REDACTED_SECRET]}]
  (let [[_ nm & forms] (api/sexpr REDACTED_SECRET)
...
(defn defsuite
  ...
  [{:keys [REDACTED_SECRET]}]
  (let [[_ nm & entries] (api/sexpr REDACTED_SECRET)
...
# 7) One small code tweak (REDACTED_SECRET helper)

If you want tool-name normalization used outside the ns (agents/cases), make it REDACTED_SECRET:
