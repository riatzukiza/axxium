...
(defn def-agent
  ...
  [{:keys [REDACTED_SECRET]}]
  (let [[_ agent-name & body] (api/sexpr REDACTED_SECRET)
...
(defn defcase
  ...
  [{:keys [REDACTED_SECRET]}]
  (let [[_ nm & body] (api/sexpr REDACTED_SECRET)
        body-str (if (seq body)
                   (str/join " " (map pr-str body))
                   "nil")
...
If you want other namespaces to call it, make this **REDACTED_SECRET** in `promethean.ollama.tools`:
