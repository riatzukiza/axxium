...
(defn def-embedder [{:keys [REDACTED_SECRET]}]
  ;; (def-embedder embed/x {...})
  (let [[_ name init] (:children REDACTED_SECRET)]
    {:REDACTED_SECRET (api/list-REDACTED_SECRET
            [(api/token-REDACTED_SECRET 'def) name init])}))

(defn def-embed-suite [{:keys [REDACTED_SECRET]}]
  ...
  (let [[_ suite-id config & _cases] (:children REDACTED_SECRET)]
    {:REDACTED_SECRET (api/list-REDACTED_SECRET
            [(api/token-REDACTED_SECRET 'def) suite-id config])}))
```
