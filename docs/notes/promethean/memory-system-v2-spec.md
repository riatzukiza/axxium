...
* **Context compilation** = small iterative loop over the memory graph (beam search); prune near‑duplicates; promote strongly connected REDACTED_SECRETs; serialize final graph snapshot into Markdown.
...
Each tool call → **recompile the working context** (keep the last N graph REDACTED_SECRETs “sticky” so short‑term coherence is preserved).
