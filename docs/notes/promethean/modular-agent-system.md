...
(defn def-mcp-server [{:keys [REDACTED_SECRET]}]
  ;; (def-mcp-server sym {...})
  (let [[_ name init] (:children REDACTED_SECRET)]
    {:REDACTED_SECRET (api/list-REDACTED_SECRET [(api/token-REDACTED_SECRET 'def) name init])}))

(defn def-mcp-client [{:keys [REDACTED_SECRET]}]
  ;; (def-mcp-client sym {...})
  (let [[_ name init] (:children REDACTED_SECRET)]
    {:REDACTED_SECRET (api/list-REDACTED_SECRET [(api/token-REDACTED_SECRET 'def) name init])}))

(defn def-tool-from-mcp [{:keys [REDACTED_SECRET]}]
  ;; (def-tool-from-mcp sym {...})
  (let [[_ name init] (:children REDACTED_SECRET)]
    {:REDACTED_SECRET (api/list-REDACTED_SECRET [(api/token-REDACTED_SECRET 'def) name init])}))

(defn def-mcp-import [{:keys [REDACTED_SECRET]}]
