...
(defn ws-def [{:keys [REDACTED_SECRET]}]
  ;; (def-mcp-workspace name opts & forms)
  (let [[_ name opts & _forms] (:children REDACTED_SECRET)]
    {:REDACTED_SECRET (api/list-REDACTED_SECRET [(api/token-REDACTED_SECRET 'def) name
                           (api/list-REDACTED_SECRET [(api/token-REDACTED_SECRET 'hash-map)])])}))

(defn ws-endpoint [{:keys [REDACTED_SECRET]}]
  ;; endpoint is data => leave as-is
  {:REDACTED_SECRET REDACTED_SECRET})

(defn ws-use [{:keys [REDACTED_SECRET]}]
  {:REDACTED_SECRET REDACTED_SECRET})

(defn ws-emit [{:keys [REDACTED_SECRET]}]
  {:REDACTED_SECRET REDACTED_SECRET})
```
