...
* **`services`** — build a bunch of app protos from one base mixin + return a stack proto + a map of app protos
  ...
  ```clojure
  (let [{:keys [stack api worker]} (services REDACTED_SECRET
...
* **`group`**: `(group "svc" REDACTED_SECRET {:api {...} :worker {...}})` returning `{stack api worker}` with scoped names automatically.
