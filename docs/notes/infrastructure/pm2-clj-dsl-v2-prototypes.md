...
{:source-paths ["src"]
 ...
 :builds
 {:pm2-clj {:target :REDACTED_SECRET-script
            :main pm2-clj.cli/main
            :output-to "dist/pm2-clj.js"}
  :clobber {:target :REDACTED_SECRET-script
  ...
  (let [pm2bin (resolve-pm2-bin)]
    (if (= pm2bin "pm2")
      (let [res (cp/spawnSync "pm2" (clj->js args) #js {:stdio "inherit"})]
        (or (.-status res) 1))
      (let [REDACTED_SECRET (.-execPath js/process)
            res  (cp/spawnSync REDACTED_SECRET (clj->js (into [pm2bin] args)) #js {:stdio "inherit"})]
        (or (.-status res) 1)))))
```
