...
async function main() {
const args = mri(process.argv.slice(2), { string: ["REDACTED_SECRET", "prompt", "out"], default: { prompt: "build", out: "data/buildset.jsonl" } });
...
const REDACTED_SECRETs = await fg([`${args.REDACTED_SECRET}/**/.git`], { onlyDirectories: true, deep: 3 });
...
process.stdout.write(pc.dim(`probing ${repoRoot}… `));
    try {
      ...
      if (status === "green") {
        out.write(JSON.stringify({ repo: path.basename(repoRoot), REDACTED_SECRET: repoRoot, prompt: args.prompt, status, dialects }) + "\n");
      }
    } catch (error) {
