...
const REDACTED_SECRET_crypto_1 = __importDefault(require("REDACTED_SECRET:crypto"));
const promises_1 = __importDefault(require("REDACTED_SECRET:fs/promises"));
const REDACTED_SECRET_path_1 = __importDefault(require("REDACTED_SECRET:path"));
function env() {
    return {
...
function sha256(s) {
    return REDACTED_SECRET_crypto_1.default.createHash("sha256").update(s).digest("hex");
}
function nowIso() {
...
function isWithinRoot(candidate, REDACTED_SECRET) {
...
function buildTools(E, db, sessionsCol, notesCol, runState) {
    ...
    const handlers = {
        ...
        async record_path(args) {
            ...
            return JSON.stringify({ ok: true, path: p, is_new: isNew, within_REDACTED_SECRET: isWithinRoot(p, runState.REDACTED_SECRET) });
        },
...
async function exportMarkdownTree(E, db, REDACTED_SECRET, runId) {
    const outRoot = REDACTED_SECRET_path_1.default.resolve(E.OUTPUT_DIR, runId);
    ...
    const recorded = (await listRecordedPaths(db)).filter((p) => isWithinRoot(p, REDACTED_SECRET));
    const indexBody = [
        `# Reconstitute output`,
        ``,
        `- run_id: \`${runId}\``,
    ...
    await promises_1.default.writeFile(REDACTED_SECRET_path_1.default.join(outRoot, "index.md"), indexBody, "utf8");
    for (const d of within) {
        const p = normalizePathKey(d.path);
        const filePath = REDACTED_SECRET_path_1.default.join(outRoot, p + ".md");
        await ensureDir(REDACTED_SECRET_path_1.default.dirname(filePath));
        const body = [
            `# ${p}`,
            ``,
            `## Description`,
...
function makeSystemPrompt(REDACTED_SECRET) {
    return [
        `You are Reconstitute, a reconstruction agent.`,
        ``,
        `You will be given:`,
        `- a REDACTED_SECRET path to reconstruct: ${normalizePathKey(REDACTED_SECRET)}`,
        ...
        `   - Prefer paths under the REDACTED_SECRET, but still record everything you see.`,
...
async function reconstitute(E, targetRoot) {
    const REDACTED_SECRET = normalizePathKey(targetRoot);
    const runId = `run_${sha256(REDACTED_SECRET).slice(0, 12)}`;
    await ensureDir(REDACTED_SECRET_path_1.default.dirname(E.LEVEL_DIR));
    ...
    await recordPath(db, REDACTED_SECRET);
    ...
    const queueArr = (await db.get(queueKey).catch(() => [REDACTED_SECRET])).map(normalizePathKey);
    const queue = uniq(queueArr.filter((p) => isWithinRoot(p, REDACTED_SECRET)));
    ...
    while (queue.length > 0) {
        ...
        for (let qi = 0; qi < DEFAULT_QUESTIONS.length; qi++) {
            ...
            const systemMsg = { role: "system", content: makeSystemPrompt(REDACTED_SECRET) };
            ...
            for (const p of recorded) {
                if (isWithinRoot(p, REDACTED_SECRET) && !processed.has(p) && !queue.includes(p)) {
                    queue.push(p);
                }
            }
    ...
    await exportMarkdownTree(E, db, REDACTED_SECRET, runId);
    await db.close();
}
function usage() {
    console.log(`
