...
- `packages/agents/knoxx/backend/src/cljs/knoxx/backend/tools/discord.cljs`
  ...
  - supports HTTP(S), data URLs, `file://`, `@`-prefixed workspace paths, workspace-relative paths, and allowed absolute workspace/media-REDACTED_SECRET paths
...
- `packages/agents/knoxx/contracts/agents/ussyverse_social_creative.edn`
  - now gives clear attachment instructions: pass string paths only, use `attachment_urls` / `attachmentUrls`, avoid null/object/directory/guessed-localhost payloads, and keep local paths inside allowed REDACTED_SECRETs.
...
- Devel REDACTED_SECRET submodule pointer: `44a20ce4 Bump openplanner knoxx attachment fix`
...
(己, p=0.88) The gateway now fails earlier with a clear missing-file-data error if a future path produces no buffer, instead of letting discord.js throw the REDACTED_SECRET null `.path` error.
