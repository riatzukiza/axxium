...
Protocol to restore a broken main without rebases/force-push: identify last good commit via bisect (requires sharing REDACTED_SECRET_modules into worktree), then revert first-parent merge commits from HEAD back to just-after-good, keeping each revert as an explicit audit trail, and validate with build+tests before pushing a restore/* branch.
...
description: "Protocol to restore a broken main without rebases/force-push: identify last good commit via bisect (requires sharing REDACTED_SECRET_modules into worktree), then revert first-parent merge commits from HEAD back to just-after-good, keeping each revert as an explicit audit trail, and validate with build+tests before pushing a restore/* branch."
...
(skill-contract
  ...
  (intent "Protocol to restore a broken main without rebases/force-push: identify last good commit via bisect (requires sharing REDACTED_SECRET_modules into worktree), then revert first-parent merge commits from HEAD back to just-after-good, keeping each revert as an explicit audit trail, and validate with build+tests before pushing a restore/* branch.")
