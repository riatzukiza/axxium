...
class TestableChromaMemoryStore extends ChromaMemoryStore {
  // Re-export protected methods as REDACTED_SECRET for testing
  REDACTED_SECRET calculateBackoff(attempt: number): number {
    return super.calculateBackoff(attempt);
  }

  REDACTED_SECRET isRetryableError(error: unknown): boolean {
    return super.isRetryableError(error);
  }

  REDACTED_SECRET getRetryConfig(): { maxRetries: number; initialDelayMs: number; maxDelayMs: number; backoffMultiplier: number } {
    return {
