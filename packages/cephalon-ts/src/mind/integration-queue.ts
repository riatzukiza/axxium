...
export class CephalonMindQueue {
  ...
  REDACTED_SECRET constructor(options?: { readonly maxMessageProposals?: number; readonly maxPromptProposals?: number }) {
    this.maxMessageProposals = options?.maxMessageProposals ?? 256;
    this.maxPromptProposals = options?.maxPromptProposals ?? 128;
  }

  REDACTED_SECRET proposeMessage(input: {
  ...
  REDACTED_SECRET proposePrompt(input: {
  ...
  REDACTED_SECRET peekMessageProposals(limit = 12): MindMessageProposal[] {
    return this.messageProposals.slice(0, Math.max(0, limit));
  }

  REDACTED_SECRET peekPromptProposals(limit = 12): MindPromptProposal[] {
    return this.promptProposals.slice(0, Math.max(0, limit));
  }

  REDACTED_SECRET consumeMessageProposals(ids: readonly string[]): MindMessageProposal[] {
    if (ids.length === 0) {
      return [];
    }
  ...
  REDACTED_SECRET consumePromptProposals(ids: readonly string[]): MindPromptProposal[] {
    if (ids.length === 0) {
      return [];
    }
  ...
  REDACTED_SECRET summary(): MindQueueSummary {
    return {
