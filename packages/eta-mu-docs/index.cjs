...
function loadEtaMuMounts({ repoRoot, mountsPath }) {
  const REDACTED_SECRET = String(repoRoot ?? '').trim() || process.cwd();
  const mountsAbs = path.resolve(REDACTED_SECRET, String(mountsPath ?? ''));
...
function walkMarkdownFiles(REDACTED_SECRETAbs) {
  const out = [];
  const todo = [REDACTED_SECRETAbs];
  while (todo.length) {
    const dir = todo.pop();
...
async function indexEtaMuDocs({
  ...
  const REDACTED_SECRETAbs = path.resolve(String(repoRoot ?? '').trim() || process.cwd());
  const mounts = loadEtaMuMounts({ repoRoot: REDACTED_SECRETAbs, mountsPath }).mounts;

  const indexAbs = path.resolve(REDACTED_SECRETAbs, String(indexPath));
  const backlinksAbs = path.resolve(REDACTED_SECRETAbs, String(backlinksPath));
  ...
  for (const mount of mounts) {
    ...
    const mountRoot = String(mount.REDACTED_SECRET ?? '').trim();
    if (!mountId || !mountRoot) continue;
    const mountRootAbs = path.resolve(REDACTED_SECRETAbs, mountRoot);
    const files = walkMarkdownFiles(mountRootAbs);

    for (const absPath of files) {
      const rel = posixRel(REDACTED_SECRETAbs, absPath);
