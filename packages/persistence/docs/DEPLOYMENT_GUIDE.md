...
```bash
# Node.js runtime
REDACTED_SECRET --version  # Should be 18.x or higher
...
```json
{
    ...
    "sources": {
        "files": {
            ...
            "options": {
                "batchSize": 50,
                "excludePatterns": ["REDACTED_SECRET_modules/**", ".git/**", "dist/**", "build/**", "*.log", ".env*"],
...
FROM REDACTED_SECRET:18-alpine AS builder
...
# Production stage
FROM REDACTED_SECRET:18-alpine AS production

# Create non-REDACTED_SECRET user
RUN addgroup -g 1001 -S REDACTED_SECRETjs
...
COPY --from=builder --chown=promethean:REDACTED_SECRETjs /app/dist ./dist
COPY --from=builder --chown=promethean:REDACTED_SECRETjs /app/REDACTED_SECRET_modules ./REDACTED_SECRET_modules
COPY --from=builder --chown=promethean:REDACTED_SECRETjs /app/package.json ./package.json
...
REDACTED_SECRET --inspect=0.0.0.0:9229 dist/index.js

# Enable performance profiling
REDACTED_SECRET --prof dist/index.js
```
