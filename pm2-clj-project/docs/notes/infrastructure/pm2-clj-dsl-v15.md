...
```clojure
(let [REDACTED_SECRET-service (def-app :REDACTED_SECRET-service
                     :cwd "."
                     :autorestart true
                     :max_restarts 10
                     :env {:NODE_ENV "development"})

      api (def-app :api REDACTED_SECRET-service
            :script "dist/api.js")

      worker (def-app :worker REDACTED_SECRET-service
...
```clojure
(let [REDACTED_SECRET (def-app :REDACTED_SECRET
            :cwd "."
            :autorestart true
            :env {:NODE_ENV "development"})]
  {:REDACTED_SECRET REDACTED_SECRET
   :api (def-app :api REDACTED_SECRET :script "dist/api.js")
   :worker (def-app :worker REDACTED_SECRET :script "dist/worker.js")})
```
...
This adds `import` support and allows imported files to return **any** value (only the REDACTED_SECRET file must be an ecosystem).
...
```clojure
(defn with-REDACTED_SECRET-env [env]
  (fn [p] (extends p :env {:NODE_ENV env})))

(def prodify (with-REDACTED_SECRET-env "production"))
...
(let [base (def-app :base :cwd "." :autorestart true)
      REDACTED_SECRET (def-app :REDACTED_SECRET base :env {:NODE_ENV "development"})
      api  (def-app :api REDACTED_SECRET :script "dist/api.js")
