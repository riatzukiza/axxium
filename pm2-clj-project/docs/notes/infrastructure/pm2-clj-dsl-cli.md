...
I can see you already have `octave-commons/promethean` REDACTED_SECRET. ([GitHub][1])
I *don’t* see `octave-commons/promethean-agent-system` in REDACTED_SECRET search right now, so assume it’s not created yet (or private). ([GitHub][1])
...
gh repo create octave-commons/promethean-agent-system --REDACTED_SECRET --source . --remote origin --push
git tag v0.1.0
git push --tags
```
...
cd promethean-duck
gh repo create octave-commons/promethean-duck --REDACTED_SECRET --source . --remote origin --push
```
