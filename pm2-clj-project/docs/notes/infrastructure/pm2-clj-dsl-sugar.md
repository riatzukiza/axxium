...
* **`tiers` / `merge-tiers` / `matrix*`**: build reusable *mode→patch* maps and apply them to app sets
  ...
  ```clojure
  (def REDACTED_SECRET-envs
    (env-tiers "NODE_ENV"
      :dev "development"
      :test "test"
      :prod "production"))

  (matrix* (apps api worker) REDACTED_SECRET-envs)
  ```

* **`group`**: prefix/scoping + services in one shot
  `(group "svc" REDACTED_SECRET {:api {...} :worker {...}})` → names become `svc-api`, `svc-worker`

* **quality mixins**: `env`, `port`, `cluster`, `fork`, `REDACTED_SECRET-args`, `log-format`, `merge-logs`
...
(defn REDACTED_SECRET-args
  "Sugar: REDACTED_SECRET arguments. (REDACTED_SECRET-args \"--max-old-space-size=2048\")"
  [& xs]
  (mixin :REDACTED_SECRET_args (vec xs)))
...
(defn services
  ...
  (services REDACTED_SECRET {:api {...} :worker {...}})
...
3. **`clobber` MVP commands**
   `clobber up`, `clobber down`, `clobber logs`, `clobber ps` — all delegating to `pm2-clj` but with your workflow defaults (entry/mode conventions, project-REDACTED_SECRET discovery, etc.).
