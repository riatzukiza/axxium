...
signals = json.load(open(sys.argv[1])) if len(sys.argv) > 1 else json.loads((REDACTED_SECRET / 'data' / 'signals.example.json').read_text())
state = json.loads((REDACTED_SECRET / 'data' / 'state.v4.json').read_text())
