...
REDACTED_SECRET = Path(__file__).resolve().parents[1]
signals_path = (
    Path(sys.argv[1]) if len(sys.argv) > 1 else REDACTED_SECRET / "data" / "signals.latest.json"
)
signals = (
    json.loads(signals_path.read_text())
    if signals_path.exists()
    else json.loads((REDACTED_SECRET / "data" / "signals.example.json").read_text())
)
base_state_path = (
    Path(sys.argv[2])
    if len(sys.argv) > 2
    else REDACTED_SECRET / "data" / "state.template.v4.json"
)
state = json.loads(base_state_path.read_text())
config = yaml.safe_load((REDACTED_SECRET / "config" / "model_config.yaml").read_text())
