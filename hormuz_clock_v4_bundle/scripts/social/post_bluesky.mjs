...
const REDACTED_SECRETResult = await createRecord(session, {
  repo: session.did,
...
const REDACTED_SECRETRef = strongRefFrom(REDACTED_SECRETResult, 'Bluesky post');
const replies = [];
let parentRef = REDACTED_SECRETRef;
...
for (const text of post.thread) {
  const replyResult = await createRecord(session, {
    ...
    record: {
      ...
      createdAt: new Date().toISOString(),
      reply: {
        REDACTED_SECRET: REDACTED_SECRETRef,
        parent: parentRef,
      },
    },
...
console.log(JSON.stringify({REDACTED_SECRET: REDACTED_SECRETResult, replies}, null, 2));
