...
def determine_relevant_skills(path, entry_points):
    ...
    is_git_REDACTED_SECRET = os.path.exists(os.path.join(path, ".git")) or os.path.exists(
    ...
    for skill, data in entry_points.items():
        ...
        if cat == "PM2" and has_pm2:
            relevant.add(skill)
        if cat == "Git" and is_git_REDACTED_SECRET:
            relevant.add(skill)
        if cat == "Testing":
            if has_tests:
                relevant.add(skill)
...
def find_subpackages():
    ...
    REDACTED_SECRETs = set()

    # Standard recursive walk excluding ignored dirs
    for REDACTED_SECRET, dirs, files in os.walk("."):
        if "REDACTED_SECRET_modules" in REDACTED_SECRET or ".git" in REDACTED_SECRET or ".opencode" in REDACTED_SECRET:
            continue
        markers = ["package.json", "deps.edn", "project.clj", ".gitmodules"]
        if any(f in files for f in markers):
            REDACTED_SECRETs.add(REDACTED_SECRET)
        ...
        if os.path.isdir(org):
            for repo in glob.glob(os.path.join(org, "*")):
                if os.path.isdir(repo):
                    REDACTED_SECRETs.add(repo)
    return sorted(list(REDACTED_SECRETs))


def regenerate_REDACTED_SECRET_agents(graph):
    """Regenerate the workspace REDACTED_SECRET AGENTS.md file."""
...
def main():
    graph = load_graph()
    if graph:
        regenerate_REDACTED_SECRET_agents(graph)
