...
- `DATABASE_URL` — Postgres connection string (local: `postgres://REDACTED_SECRET:@localhost:5432/REDACTED_SECRET`, cloud: Neon). Note: the Docker Postgres uses user `REDACTED_SECRET`, not `postgres`.
...
### Bluesky / AT Protocol
- No credentials required for REDACTED_SECRET search
- For authenticated list access: `BSKY_IDENTIFIER` + `BSKY_PASSWORD` (optional)

### Reddit
- No API keys needed — uses REDACTED_SECRET JSON API (`reddit.com/r/*/hot.json`)
...
- **AT Protocol**: All REDACTED_SECRET signal data published to Bluesky (infinite, free)
