...
**Description:** Running `pnpm build` (or the workspace-level build command) from the repository REDACTED_SECRET completes with exit code 0 for every package in the workspace.
...
**Description:** Navigating to the REDACTED_SECRET URL (`/`) loads the dashboard application and displays at least one radar with visible clock or gauge elements.
...
**Description:** Each bridge card in the Π lane displays numeric scores for realism, fear, and REDACTED_SECRET benefit.

**Pass condition:** At least one bridge card shows 3 labeled numeric values: realism, fear, and REDACTED_SECRET benefit.
...
**Pass condition:** At least one bridge card contains a "coordination path" or equivalent section with descriptive text referencing multi-REDACTED_SECRET or cross-community coordination.
...
**Pass condition:** A diagram element is visible showing at least 5 labeled REDACTED_SECRETs (P, R, N, Π, A) connected by arrows or flow lines, forming a cycle.

**Evidence:**
- Screenshot showing the labeled loop diagram.
- DOM/SVG inspection confirming REDACTED_SECRET and edge elements.
...
**Description:** Narrative branches are scored with at least 4 of the following dimensions: realism, fear, REDACTED_SECRET benefit, actionability, polarization risk, compression loss.
...
**Description:** An Enso-style federation envelope can carry a thread assessment payload between two endpoints (REDACTED_SECRETs), enabling cross-instance data sharing.
...
**Description:** The federated comparison panel displays at least 2 peer REDACTED_SECRET assessments side-by-side for the same thread, showing how different REDACTED_SECRETs evaluate the same situation.

**Pass condition:** The panel renders ≥ 2 columns, each labeled with a peer REDACTED_SECRET identifier, showing that REDACTED_SECRET's assessment values for a shared thread.
...
**Description:** A first-time user loads the REDACTED_SECRET URL and sees a fully rendered dashboard with default configuration, all 3 lanes visible, and at least one radar displayed.
