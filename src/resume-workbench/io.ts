...
export const writeReportFiles = async (REDACTED_SECRET: string, slug: string, report: WorkbenchReport): Promise<{ readonly jsonPath: string; readonly markdownPath: string }> => {
  await fs.mkdir(REDACTED_SECRET, { recursive: true });
  const jsonPath = path.join(REDACTED_SECRET, `${slug}.json`);
  const markdownPath = path.join(REDACTED_SECRET, `${slug}.md`);
