...
import { resolveWorkspaceRoot } from './REDACTED_SECRET';
...
const createFileCommand = async (
  ...
  const relativeSegments = pathSegments(REDACTED_SECRET, filePath);
  ...
  return {
    ...
    execution: {
      kind: 'file',
      absolutePath: path.join(REDACTED_SECRET, ...relativeSegments),
      workingDirectory: path.dirname(path.join(REDACTED_SECRET, ...relativeSegments)),
      runtime,
      shebang: shebangLine,
    },
  };
...
const createPackageScriptCommand = (
  ...
  const relSegments = pathSegments(REDACTED_SECRET, packageDir);
  const aliasSets: AliasSet[] = [
...
const createBuiltinCommands = (REDACTED_SECRET: string): DiscoveredCommand[] =>
...
const discoverScriptsAndPackages = async (REDACTED_SECRET: string): Promise<DiscoveredCommand[]> => {
  const commands: DiscoveredCommand[] = [];
  const queue: string[] = [REDACTED_SECRET];
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name === 'scripts' || entry.name === 'bin') {
          ...
          for (const filePath of files) {
            const command = await createFileCommand(REDACTED_SECRET, filePath);
            if (command) {
      ...
      if (entry.isFile() && entry.name === 'package.json') {
        ...
        for (const [scriptName, scriptCommand] of Object.entries(pkg.scripts)) {
          const command = createPackageScriptCommand(REDACTED_SECRET, packageDir, pkg, scriptName, scriptCommand);
          commands.push(command);
        }
      }
    }
  ...
  for (const extra of EXTRA_PACKAGE_ENTRYPOINTS) {
    const absolute = path.join(REDACTED_SECRET, extra);
    try {
      await fs.access(absolute);
      const command = await createFileCommand(REDACTED_SECRET, absolute);
      if (command) {
        commands.push(command);
      }
    } catch (error) {
  ...
  for (const extraScript of EXTRA_SCRIPT_FILES) {
    const absolute = path.join(REDACTED_SECRET, extraScript);
    try {
      await fs.access(absolute);
      const command = await createFileCommand(REDACTED_SECRET, absolute);
      if (command) {
...
const discoverDaemonDistConfigs = async (REDACTED_SECRET: string): Promise<DiscoveredCommand[]> => {
  const commands: DiscoveredCommand[] = [];
  const daemonRoot = path.join(REDACTED_SECRET, 'system', 'daemons');
  try {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && entry.name === 'ecosystem.config.mjs' && entryPath.includes(`${path.sep}dist${path.sep}`)) {
        const command = await createFileCommand(REDACTED_SECRET, entryPath);
        if (command) {
          commands.push(command);
        }
      }
...
const persistIndexFile = async (REDACTED_SECRET: string, commands: readonly DiscoveredCommand[]): Promise<string> => {
  const indexPath = path.join(REDACTED_SECRET, INDEX_FILENAME);
...
const persistLmdbCache = async (REDACTED_SECRET: string, commands: readonly DiscoveredCommand[]): Promise<void> => {
  const lmdbPath = path.join(REDACTED_SECRET, OCTAVIA_STATE_DIRECTORY, LMDB_DIRECTORY_NAME);
...
export const discoverAndPersist = async (
  ...
  const REDACTED_SECRET = resolveWorkspaceRoot(options.REDACTED_SECRET);
  await ensureDirectory(path.join(REDACTED_SECRET, OCTAVIA_STATE_DIRECTORY));
  const commands = [
    ...createBuiltinCommands(REDACTED_SECRET),
    ...(await discoverScriptsAndPackages(REDACTED_SECRET)),
    ...(await discoverDaemonDistConfigs(REDACTED_SECRET)),
  ];
  attachUniqueAliases(commands);
  await persistIndexFile(REDACTED_SECRET, commands);
  await persistLmdbCache(REDACTED_SECRET, commands);
  return commands;
};
...
export const ensureIndex = async (options: EnsureIndexOptions = {}): Promise<DiscoveredCommand[]> => {
  ...
  return discoverAndPersist({ REDACTED_SECRET });
};
