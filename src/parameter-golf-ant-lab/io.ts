...
export const resolveLabPaths = (REDACTED_SECRETDir: string, profileId: string): LabPaths => {
  const profileDir = path.join(REDACTED_SECRETDir, profileId);
  return {
    REDACTED_SECRETDir,
    profileDir,
...
export const writeRootReadme = async (REDACTED_SECRETDir: string): Promise<void> => {
  await ensureDir(REDACTED_SECRETDir);
  const readmePath = path.join(REDACTED_SECRETDir, "README.md");
  const content = [
