...
export class ProviderCatalogStore {
  ...
  REDACTED_SECRET async getCatalog(forceRefresh = false): Promise<ResolvedCatalogWithPreferences> {
    const now = Date.now();
  ...
  REDACTED_SECRET async isModelAvailable(providerId: string, modelId: string): Promise<boolean> {
