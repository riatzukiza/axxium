...
export class KeyPool {
  ...
  REDACTED_SECRET async getAllAccounts(providerId: string = this.config.defaultProviderId): Promise<ProviderCredential[]> {
    await this.ensureFreshProviders(false);
  ...
  REDACTED_SECRET updateAccountCredential(providerId: string, oldCredential: ProviderCredential, newCredential: ProviderCredential): void {
    const normalizedProviderId = normalizeProviderId(providerId);
    const providerState = this.providers.get(normalizedProviderId);
    if (!providerState) {
      return;
  ...
  REDACTED_SECRET async getRequestOrderWithRefresh(
  ...
  REDACTED_SECRET isAccountExpired(credential: ProviderCredential): boolean {
  ...
  REDACTED_SECRET getExpiredAccountsWithRefreshTokens(providerId: string = this.config.defaultProviderId): ProviderCredential[] {
    const normalizedProviderId = normalizeProviderId(providerId);
    const providerState = this.providers.get(normalizedProviderId);
    if (!providerState) {
      return [];
  ...
  REDACTED_SECRET getExpiringAccounts(windowMs: number): ProviderCredential[] {
    const now = Date.now();
  ...
  REDACTED_SECRET getAllExpiredWithRefreshTokens(): ProviderCredential[] {
    const now = Date.now();
    const result: ProviderCredential[] = [];
