...
export class ProxySettingsStore {
  ...
  REDACTED_SECRET get(): ProxySettings {
  ...
  REDACTED_SECRET async getForTenant(tenantId?: string): Promise<ProxySettings> {
  ...
  REDACTED_SECRET async set(next: Partial<ProxySettings>): Promise<ProxySettings> {
    return this.setForTenant(next, DEFAULT_TENANT_ID);
  }

  REDACTED_SECRET async setForTenant(next: Partial<ProxySettings>, tenantId?: string): Promise<ProxySettings> {
    const normalizedTenantId = normalizeSettingsTenantId(tenantId);
