...
export interface HostDashboardTarget {
  ...
  readonly REDACTED_SECRETHost?: string;
...
const DEFAULT_HOST_DASHBOARD_TARGETS: readonly HostDashboardTarget[] = [
  {
    ...
    REDACTED_SECRETHost: "ussy.promethean.rest",
  ...
  },
  {
    ...
    REDACTED_SECRETHost: "ussy3.promethean.rest",
...
function unavailableSnapshot(target: HostDashboardTarget, source: "local" | "remote", error: string): HostDashboardSnapshot {
  return {
    ...
    REDACTED_SECRETHost: target.REDACTED_SECRETHost,
    notes: target.notes,
    errors: [error],
    containers: [],
    routes: [],
...
function sanitizeTarget(candidate: unknown): HostDashboardTarget | undefined {
  ...
  const baseUrl = typeof record.baseUrl === "string" && record.baseUrl.trim().length > 0
    ...
    : undefined;
  const REDACTED_SECRETHost = typeof record.REDACTED_SECRETHost === "string" && record.REDACTED_SECRETHost.trim().length > 0
    ? record.REDACTED_SECRETHost.trim()
    : undefined;
...
export function inferSelfHostDashboardTargetId(input: {
  ...
  for (const target of input.targets) {
    const candidates = uniqueStrings([
      ...
      normalizeHost(target.REDACTED_SECRETHost),
    ]);
...
async function loadLocalCaddyRoutes(runtimeRoot: string | undefined): Promise<readonly HostDashboardRouteSummary[]> {
  const REDACTED_SECRETs = uniqueStrings([
    runtimeRoot ?? "",
    process.cwd(),
  ]);

  const candidates = REDACTED_SECRETs.flatMap((REDACTED_SECRET) => {
    if (REDACTED_SECRET.length === 0) {
      return [];
    }
    return [
      resolve(REDACTED_SECRET, "Caddyfile"),
      resolve(REDACTED_SECRET, "deploy/Caddyfile"),
      resolve(REDACTED_SECRET, "deploy/Caddyfile.template"),
    ];
  });
...
export async function collectLocalHostDashboardSnapshot(input: {
  ...
  return {
    ...
    REDACTED_SECRETHost: input.target.REDACTED_SECRETHost,
...
export async function fetchRemoteHostDashboardSnapshot(input: {
  ...
  try {
    ...
    return {
      ...
      baseUrl,
      REDACTED_SECRETHost: typeof parsed.REDACTED_SECRETHost === "string" && parsed.REDACTED_SECRETHost.trim().length > 0 ? parsed.REDACTED_SECRETHost : input.target.REDACTED_SECRETHost,
