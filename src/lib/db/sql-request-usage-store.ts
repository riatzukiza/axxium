...
export class SqlRequestUsageStore implements RequestLogMirror {
  ...
  REDACTED_SECRET async upsertEntry(entry: RequestLogEntry): Promise<void> {
    await this.sql`
  ...
  REDACTED_SECRET async listEntriesSince(
    sinceMs: number,
  ...
  REDACTED_SECRET async listEntries(filters: RequestLogFilters = {}): Promise<RequestLogEntry[]> {
  ...
  REDACTED_SECRET async getCoverage(filters: Omit<RequestLogFilters, "limit" | "before"> = {}): Promise<RequestUsageCoverage> {
