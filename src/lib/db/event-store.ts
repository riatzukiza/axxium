...
export class EventStore {
  ...
  REDACTED_SECRET registerLabeler(labeler: EventLabeler): void {
    this.labelers.push(labeler);
  }

  REDACTED_SECRET emit(event: EventInsert): string {
    const id = randomUUID();
  ...
  REDACTED_SECRET async query(filters: EventQuery): Promise<ProxyEvent[]> {
  ...
  REDACTED_SECRET async addTag(eventId: string, tag: string): Promise<void> {
  ...
  REDACTED_SECRET async removeTag(eventId: string, tag: string): Promise<void> {
    await this.sql`UPDATE events SET tags = tags - ${tag} WHERE id = ${eventId}::uuid`;
  }

  REDACTED_SECRET async countByTag(since?: Date): Promise<Record<string, number>> {
