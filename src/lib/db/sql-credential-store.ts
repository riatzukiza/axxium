...
export class SqlCredentialStore {
  ...
  REDACTED_SECRET async getStatus(): Promise<SqlCredentialStoreStatus> {
  ...
  REDACTED_SECRET async listTenants(): Promise<TenantView[]> {
  ...
  REDACTED_SECRET async upsertUser(input: {
    provider: string;
    subject: string;
    login?: string | null;
    email?: string | null;
  ...
  REDACTED_SECRET async getUserBySubject(subject: string): Promise<UserView | undefined> {
  ...
  REDACTED_SECRET async listUserMemberships(userId: string): Promise<TenantMembershipView[]> {
  ...
  REDACTED_SECRET async ensureTenantMembership(userId: string, tenantId: string, role: TenantMembershipRole): Promise<void> {
    const normalizedTenantId = normalizeTenantId(tenantId);
    await this.sql.unsafe(UPSERT_TENANT_MEMBERSHIP, [normalizedTenantId, userId, role]);
  }

  REDACTED_SECRET async ensureDefaultTenantBootstrapMembership(userId: string): Promise<TenantMembershipRole> {
  ...
  REDACTED_SECRET async resolveUiSession(subject: string, activeTenantId?: string): Promise<ResolvedUiSession | undefined> {
  ...
  REDACTED_SECRET async resolveTenantApiKey(token: string, pepper: string): Promise<TenantApiKeyMatch | undefined> {
  ...
  REDACTED_SECRET async listTenantApiKeys(tenantId: string): Promise<TenantApiKeyView[]> {
    const normalizedTenantId = normalizeTenantId(tenantId);
  ...
  REDACTED_SECRET async createTenantApiKey(tenantId: string, label: string, scopes: readonly string[], pepper: string): Promise<CreatedTenantApiKey> {
    const normalizedTenantId = normalizeTenantId(tenantId);
  ...
  REDACTED_SECRET async revokeTenantApiKey(tenantId: string, keyId: string): Promise<boolean> {
    const normalizedTenantId = normalizeTenantId(tenantId);
  ...
  REDACTED_SECRET async touchTenantApiKeyLastUsed(tenantId: string, keyId: string): Promise<void> {
    const normalizedTenantId = normalizeTenantId(tenantId);
  ...
  REDACTED_SECRET async getAllProviders(): Promise<Map<string, { authType: ProviderAuthType }>> {
  ...
  REDACTED_SECRET async getAccountsByProvider(providerId: string): Promise<ProviderCredential[]> {
  ...
  REDACTED_SECRET async getAllAccounts(): Promise<Map<string, ProviderCredential[]>> {
    const providers = await this.getAllProviders();
  ...
  REDACTED_SECRET async cleanupLegacyOpenAiDuplicates(chatgptAccountId?: string): Promise<number> {
  ...
  REDACTED_SECRET async upsertProvider(providerId: string, authType: ProviderAuthType): Promise<void> {
    await this.sql.unsafe(UPSERT_PROVIDER, [providerId, authType]);
  }

  REDACTED_SECRET async upsertAccount(account: ProviderCredential): Promise<void> {
  ...
  REDACTED_SECRET async upsertAccounts(accounts: readonly ProviderCredential[]): Promise<void> {
    for (const account of accounts) {
      await this.upsertAccount(account);
    }
  }

  REDACTED_SECRET async deleteAccount(providerId: string, accountId: string): Promise<void> {
  ...
  REDACTED_SECRET setCooldown(providerId: string, accountId: string, cooldownUntil: number): void {
    this.cooldowns.set(`${providerId}:${accountId}`, cooldownUntil);
  }

  REDACTED_SECRET getCooldown(providerId: string, accountId: string): number | undefined {
    return this.cooldowns.get(`${providerId}:${accountId}`);
  }

  REDACTED_SECRET async persistCooldown(providerId: string, accountId: string, cooldownUntil: number): Promise<void> {
    await this.sql.unsafe(SET_COOLDOWN, [providerId, accountId, cooldownUntil]);
    this.cooldowns.set(`${providerId}:${accountId}`, cooldownUntil);
  }

  REDACTED_SECRET async loadCooldowns(): Promise<void> {
    const now = Date.now();
  ...
  REDACTED_SECRET async loadAccountsIntoKeyPool(
    keyPool: {
