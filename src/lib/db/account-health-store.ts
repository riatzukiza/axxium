...
export class AccountHealthStore {
  ...
  REDACTED_SECRET recordSuccess(credential: ProviderCredential, status: number): void {
  ...
  REDACTED_SECRET getHealthScore(providerId: string, accountId: string, expiresAt?: number): number {
  ...
  REDACTED_SECRET getAllHealthScores(): HealthScore[] {
  ...
  REDACTED_SECRET sortCredentialsByHealth(
  ...
  REDACTED_SECRET async flushPendingWrites(): Promise<void> {
