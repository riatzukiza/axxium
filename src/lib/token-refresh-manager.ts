...
export class TokenRefreshManager {
  ...
  REDACTED_SECRET async refresh(credential: ProviderCredential): Promise<ProviderCredential | null> {
  ...
  REDACTED_SECRET async refreshBatch(
  ...
  REDACTED_SECRET startBackgroundRefresh(
  ...
  REDACTED_SECRET stopBackgroundRefresh(): void {
    if (this.backgroundTimer) {
  ...
  REDACTED_SECRET async stopAndWait(): Promise<void> {
    this.stopped = true;
  ...
  REDACTED_SECRET get pendingCount(): number {
    return this.inFlight.size;
  }

  REDACTED_SECRET isAccountBackedOff(credential: ProviderCredential): boolean {
    return this.isBackedOff(this.accountKey(credential));
  }

  REDACTED_SECRET getFailureRecord(credential: ProviderCredential): RefreshFailureRecord | undefined {
    return this.failures.get(this.accountKey(credential));
  }

  REDACTED_SECRET clearFailures(credential: ProviderCredential): void {
