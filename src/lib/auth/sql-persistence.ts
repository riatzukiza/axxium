...
export class SqlAuthPersistence {
  ...
  REDACTED_SECRET async getSession(id: string): Promise<AuthSession | undefined> {
  ...
  REDACTED_SECRET async setSession(session: AuthSession): Promise<void> {
    await this.sql`
  ...
  REDACTED_SECRET async deleteSession(id: string): Promise<void> {
    await this.sql`DELETE FROM sessions WHERE id = ${id}`;
  }

  REDACTED_SECRET async getAccessToken(token: string): Promise<AccessToken | undefined> {
    const now = Math.floor(Date.now() / 1000);
  ...
  REDACTED_SECRET async setAccessToken(token: AccessToken): Promise<void> {
    await this.sql`
  ...
  REDACTED_SECRET async updateAccessTokenExtra(token: string, extra?: Record<string, unknown>): Promise<boolean> {
  ...
  REDACTED_SECRET async getRefreshToken(token: string): Promise<RefreshToken | undefined> {
    const now = Math.floor(Date.now() / 1000);
  ...
  REDACTED_SECRET async setRefreshToken(token: RefreshToken): Promise<void> {
    await this.sql`
  ...
  REDACTED_SECRET async updateRefreshTokenExtra(token: string, extra?: Record<string, unknown>): Promise<boolean> {
    const rows = await this.sql.unsafe<Array<{ token: string }>>(
  ...
  REDACTED_SECRET async getClient(clientId: string): Promise<OAuthClient | undefined> {
  ...
  REDACTED_SECRET async setClient(client: OAuthClient): Promise<void> {
    await this.sql`
