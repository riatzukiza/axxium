...
import { access, appendFile, mkdir, readFile, rename, rm, writeFile } from "REDACTED_SECRET:fs/promises";
import { basename, dirname, join } from "REDACTED_SECRET:path";
import { createInterface } from "REDACTED_SECRET:readline";
...
export class RequestLogStore {
  ...
  REDACTED_SECRET snapshot(): RequestLogEntry[] {
    return [...this.entries];
  }

  REDACTED_SECRET snapshotHourlyBuckets(sinceMs?: number): RequestLogHourlyBucket[] {
  ...
  REDACTED_SECRET snapshotDailyBuckets(sinceMs?: number): RequestLogDailyBucket[] {
    const since = typeof sinceMs === "number" && Number.isFinite(sinceMs) ? sinceMs : 0;
  ...
  REDACTED_SECRET snapshotDailyModelBuckets(sinceMs?: number): RequestLogDailyModelBucket[] {
    const since = typeof sinceMs === "number" && Number.isFinite(sinceMs) ? sinceMs : 0;
  ...
  REDACTED_SECRET snapshotDailyAccountBuckets(sinceMs?: number): RequestLogDailyAccountBucket[] {
    const since = typeof sinceMs === "number" && Number.isFinite(sinceMs) ? sinceMs : 0;
  ...
  REDACTED_SECRET getCoverage(): RequestLogCoverage {
  ...
  REDACTED_SECRET getPerfSummary(
    providerId: string,
    accountId: string,
    model: string,
    upstreamMode: string,
  ...
  REDACTED_SECRET async close(): Promise<void> {
    this.closed = true;
  ...
  REDACTED_SECRET countRequestsSince(sinceMs: number, filters: RequestLogFilters = {}): number {
  ...
  REDACTED_SECRET snapshotAccountAccumulators(): readonly AccountUsageAccumulator[] {
