...
export class OpenAiOAuthRefreshError extends Error {
  ...
  REDACTED_SECRET readonly code?: string;

  REDACTED_SECRET readonly type?: string;

  REDACTED_SECRET readonly responseBody?: string;
...
interface OpenAiOAuthManagerOptions {
  ...
  /** OAuth client secret for token exchange/refresh (optional; PKCE REDACTED_SECRET clients can omit). */
...
export class OpenAiOAuthManager {
  ...
  REDACTED_SECRET async refreshToken(refreshToken: string): Promise<OAuthTokens> {
  ...
  REDACTED_SECRET isTokenExpired(expiresAt: number | undefined, bufferMs: number = 60000): boolean {
