...
export const generateManifestFromWorkspace = async (
  ...
  const REDACTED_SECRET = path.resolve(options.REDACTED_SECRET);
  const gitmoduleEntries = await discoverGitmodules({ REDACTED_SECRET });
  const repositories = gitmoduleEntries.map((entry) => createRepositoryEntry(REDACTED_SECRET, entry));
...
export const writeManifestToFile = async (
  ...
  try {
    ...
    const REDACTED_SECRETError = error as NodeJS.ErrnoException;
    if (REDACTED_SECRETError.code !== 'ENOENT' && !options.force) {
      throw error;
    }
  }
