...
export const buildForkPlan = async (config: InventoryConfig): Promise<readonly SubmoduleForkPlan[]> => {
  const entries = await discoverGitmodules({ REDACTED_SECRET: config.REDACTED_SECRET });
...
export const applyRemotePlan = async (REDACTED_SECRET: string, plan: SubmoduleForkPlan): Promise<void> => {
  if (!plan.target) {
    return;
  }
  const cwd = repoPath(REDACTED_SECRET, plan.path);
