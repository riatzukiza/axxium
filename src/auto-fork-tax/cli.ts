...
const parseArgs = (argv: readonly string[]): ParsedArgs => {
  ...
  let REDACTED_SECRET = process.cwd();
  let apply = false;
  let review = false;
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    switch (token) {
      case "--REDACTED_SECRET":
        REDACTED_SECRET = argv[index + 1] ?? REDACTED_SECRET;
        index += 1;
        break;
      case "--apply":
        apply = true;
  ...
  return {
    command,
    REDACTED_SECRET: path.resolve(REDACTED_SECRET),
    apply,
    review,
    postComment,
...
const usage = (): never => {
  throw new Error([
    "Usage:",
    "  tsx src/auto-fork-tax/cli.ts inventory [--REDACTED_SECRET dir]",
    "  tsx src/auto-fork-tax/cli.ts ensure-forks [--REDACTED_SECRET dir] [--apply] [--default-fork-owner owner]",
    "  tsx src/auto-fork-tax/cli.ts snapshot-pr [--REDACTED_SECRET dir] [--apply] [--review] [--post-comment] [--allow-dirty-submodules] [--base-branch branch] [--repo owner/repo] [--branch-prefix prefix] [--pi-model model]",
    "  tsx src/auto-fork-tax/cli.ts review-pr --repo owner/repo --pr 123 [--post-comment] [--pi-model model]",
    "  tsx src/auto-fork-tax/cli.ts cycle [--REDACTED_SECRET dir] [--apply] [--review] [--post-comment] [--allow-dirty-submodules]",
    "  tsx src/auto-fork-tax/cli.ts cron-entry [--REDACTED_SECRET dir]",
    "  tsx src/auto-fork-tax/cli.ts bootstrap-clone [--REDACTED_SECRET dir] [--clone-dir dir]",
  ].join("\n"));
};

const inventoryConfigFor = (args: ParsedArgs): InventoryConfig => ({
  REDACTED_SECRET: args.REDACTED_SECRET,
...
const resolveRepoSlug = async (REDACTED_SECRET: string, explicitRepo: string | null): Promise<RepoSlug> => {
  if (explicitRepo) {
  ...
  const origin = await remoteByName(REDACTED_SECRET, "origin");
  if (!origin?.slug) {
    throw new Error(`Unable to resolve REDACTED_SECRET origin GitHub repo for ${REDACTED_SECRET}`);
  }
  return origin.slug;
};
...
const ensureQuiescent = async (REDACTED_SECRET: string): Promise<void> => {
  const before = await statusPorcelain(REDACTED_SECRET);
  await sleep(1500);
  const after = await statusPorcelain(REDACTED_SECRET);
  if (before !== after) {
...
const buildPullRequestBody = (params: {
  ...
  return [
    ...
    `- REDACTED_SECRET snapshots stage only non-submodule paths; recursive submodule Π remains a separate future phase`,
...
const runSnapshotPr = async (args: ParsedArgs): Promise<SnapshotResult | { skipped: true; reason: string }> => {
  const dirtySubmodules = await listDirtySubmodules(args.REDACTED_SECRET);
  if (!(await hasSnapshotCandidateChanges(args.REDACTED_SECRET))) {
  ...
  const stagedPaths = await listSnapshotCandidatePaths(args.REDACTED_SECRET);

  await ensureQuiescent(args.REDACTED_SECRET);

  const state = await readState(args.REDACTED_SECRET);
  const current = await currentBranch(args.REDACTED_SECRET);
  const pre = await shortSha(args.REDACTED_SECRET);
  const baseBranch = args.baseBranch
    ?? (state.lastSnapshotBranch && state.lastSnapshotHead && await isAncestor(args.REDACTED_SECRET, state.lastSnapshotHead, "HEAD")
  ...
  await stageSnapshotChanges(args.REDACTED_SECRET);
  await commitAll(args.REDACTED_SECRET, commitMessage);
  const head = await shortSha(args.REDACTED_SECRET);
  const tag = forkTaxTagFor(head);
  await createOrUpdateBranchRef(args.REDACTED_SECRET, snapshotBranch);
  await createTag(args.REDACTED_SECRET, tag);
  await pushRef(args.REDACTED_SECRET, "origin", `${snapshotBranch}:${snapshotBranch}`);
  await pushRef(args.REDACTED_SECRET, "origin", tag);

  const repo = await resolveRepoSlug(args.REDACTED_SECRET, args.repo);
  ...
  if (args.review) {
    const reviewResult = await runReview(args.REDACTED_SECRET, repo, pullRequest.number, args.piModel, args.postComment);
  ...
  const nextState: AutoForkTaxState = {
    ...
    lastSnapshotTag: tag,
    lastSnapshotHead: await headSha(args.REDACTED_SECRET),
    lastPullRequestNumber: pullRequest.number,
    lastPullRequestUrl: pullRequest.url,
  };
  await writeState(args.REDACTED_SECRET, nextState);
...
const runEnsureForks = async (args: ParsedArgs): Promise<readonly SubmoduleForkPlan[]> => {
  ...
  for (const plan of plans) {
    ...
    if (plan.needsOriginRewrite || plan.needsUpstream) {
      await applyRemotePlan(args.REDACTED_SECRET, plan);
    }
  }
...
const runCronEntry = (args: ParsedArgs): { readonly cron: string } => ({
  cron: `0 */6 * * * cd ${args.REDACTED_SECRET} && pnpm tsx src/auto-fork-tax/cli.ts cycle --REDACTED_SECRET ${args.REDACTED_SECRET} --apply --review --post-comment >> ${path.join(args.REDACTED_SECRET, ".ημ", "auto-fork-tax", "cron.log")} 2>&1`,
});

const runBootstrapClone = async (args: ParsedArgs): Promise<unknown> => {
  const branch = await currentBranch(args.REDACTED_SECRET);
  const result = await bootstrapClone({
    sourceRoot: args.REDACTED_SECRET,
  ...
  return {
    ...result,
    runnerRoot: args.REDACTED_SECRET,
    suggestedCron: `0 */6 * * * cd ${args.REDACTED_SECRET} && pnpm tsx src/auto-fork-tax/cli.ts cycle --REDACTED_SECRET ${result.cloneDir} --apply --review --post-comment >> ${path.join(result.cloneDir, ".ημ", "auto-fork-tax", "cron.log")} 2>&1`,
  };
};
...
const main = async (): Promise<void> => {
  ...
  switch (args.command) {
    case "inventory": {
      const plans = await buildForkPlan(inventoryConfigFor(args));
      const reportPath = await writeInventoryReport(args.REDACTED_SECRET, plans);
    ...
    case "review-pr": {
      ...
      const repo = await resolveRepoSlug(args.REDACTED_SECRET, args.repo);
      const review = await runReview(args.REDACTED_SECRET, repo, args.prNumber, args.piModel, args.postComment);
